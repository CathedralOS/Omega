...
fn segmented_home_replay_is_independent_and_non_authoritative() {
    ...
    let stage = root.join(
        "omega-rust/omega/pipeline/omega-selected-instructions-to-register-homes/src/assignment/fixed_precolored_segment_homes",
    );
...
fn fixed_view_copy_consumes_segment_evidence_without_legacy_transition_fallback() {
    let root = workspace_root();
    let owner = root.join(
        "omega-rust/omega/pipeline/omega-selected-instructions-to-register-homes/src/rewrites/allocation_recovery/fixed_view_copy",
    );
    let entrance =
