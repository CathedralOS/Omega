...
fn closed_conformance_dispatch_never_authorizes_with_a_compact_identity_alone() {
    ...
    let terminal = [
        source(
            "omega-rust/psi/representations/terminal-psi/src/terminal_module/boundary/conformances.rs",
        ),
        source("omega-rust/psi/representations/terminal-psi/src/terminal_module/proof/outputs.rs"),
    ]
    .join("\n");
    let verifier = source("omega-rust/psi/semantics/terminal-verifier/src/validation/evidence.rs");
