...
command -v python3 >/dev/null 2>&1 || {
    echo "Beta evaluator slice: skipped (python3 absent)"
    exit 0
}
...
"$TMP/assembler" < "$OMEGA_REPO_ROOT/tools/alpha/beta-evaluator/evaluator.alphaasm" > "$TMP/evaluator.tape"
...
sys.stdout.buffer.write(b"BETAREQ\x01" + struct.pack("<II", len(source), len(invocation_input)) + source + invocation_input)
'
}
...
echo "Beta evaluator development slice: 42/42 cases passed"