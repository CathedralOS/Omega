...
def main():
    ...
    names = set()
    for name, request, expected, timeout in cases():
        if name in names:
        ...
        if (result.returncode, result.stdout, result.stderr) != (0, expected, b""):
            raise SystemExit(
                f"Derivation ground {name}: expected 0/{expected.hex()}, "
    ...
    print(f"Derivation ground: {len(names)} exact diagnostic vectors passed; no proof verdicts")
