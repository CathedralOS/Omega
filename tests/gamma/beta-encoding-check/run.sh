...
# bound closure plus the bound shared producer entry produces the theory
# section inside the evaluator, and the checker's bound closure carries the
# bound explicit diagnostic entry.
...
python3 "$OMEGA_REPO_ROOT/tools/bootstrap/source_closure.py" \
    ...
    "$CHECK_TMP/producer.gamma" --prefix "$OMEGA_PATH_BETA_ENCODING_PRODUCER_ENTRY"
