...
def cases():
    ...
    yield vector("exact_then_adjacent_transition",
                 compared(1, 67108864) + failure(4097, 4, 2, 67108864, 67108865), owners,
                 right=2, definitions=different, entry="budget", timeout=600)
    yield vector("invalid_ids_precede_exhausted_counter",
                 compared(1, 67108864) + failure(4097), owners,
                 right=1, definitions=different, entry="budget", timeout=600)
    yield vector("terminal_resume_cannot_return_boolean_at_exhaustion",
                 compared(0, 67108863) + failure(12289, 4, 2, 67108864, 67108865), owners,
                 right=2, definitions=different, entry="resume", timeout=600)
    yield vector("pending_parent_cannot_complete_at_exhaustion",
                 failure(20481, 4, 2, 67108864, 67108865),
                 (ZERO, record(1, 2, 1, 1), record(1, 2, 1, 1)), left=2, right=3,
                 entry="pending", timeout=600)
    count = 46484
    ...
    yield vector("two_46484_deep_chains", compared(1, 92968), first + second,
                 left=count, right=2 * count, timeout=600)
    count = 1024
    ...
    yield vector("separately_encoded_shared_dags", compared(1, 4094), first + second,
                 left=count, right=2 * count, definitions=branching, timeout=600)
