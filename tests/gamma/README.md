...
| `derivation-formation/` | Checks conservative theory formation, sorted clause-local templates, structural recursion, and exact/adjacent component provisions; grants no proof acceptance. | Replace only with a complete checker gate retaining these formation, ordering, and resource controls. |
| `derivation-ground/` | Validates sorted ground applications and owner-only roots with a read-only witness extension, including deep/shared DAGs and failure ordering; grants no proof acceptance. | Replace only with a complete checker gate retaining these term, custody, and depth controls. |
...
| `product-comparison-experiment/` | Exploratory `GAMMA-PRODUCT-COMPARISON` evidence: the seven-field Delta continuation as nested pairs versus a source-emulated checked named product versus a boundary kind check versus actual Delta nominal data compiled by the canonical Delta compiler, with failure, forgery, size, receipt, and reference-interpreter cost observations. | Delete when the board item is decided and its evidence is retained elsewhere. |
| `call-depth-experiment/` | Exploratory `BOOTSTRAP-AUTHORING-READABILITY` evidence: the selected evaluator's 256-context call boundary under non-tail versus proper-tail recursion, with the containment and failure argument for an evaluator-owned explicit stack. | Delete when the board item's stack comparison is decided and its evidence is retained elsewhere. |
