...
| --- | --- | --- |
| `functional-evaluator.sh` | Reconstructs the selected Beta evaluator and runs the evaluator-development gate, including scalar/effect and self-augmentation controls. | Replace only with a stronger selected Gamma evaluator gate. |
...
| `self-augmentation-experiment/` | Holds the Gamma-authored `const` augmenter and exact receipt fixtures executed by `evaluator-development/run.sh`, including result byte 42. | Replace when a meaningful staged Delta augmenter subsumes this mechanism proof. |
| `seed-native-acceptance/` | Accepts the selected evaluator tape stamped into both audited Alpha seeds as native containers, plus a native execution receipt on seed-execution hosts. | Replace only with a gate covering the real evaluator tape inside both audited containers and its native receipt. |
