...
def cases():
    ...
    for name, selector, expected in (
        ...
        ("exact_bulk", 5, compared(1, 67108864)),
        ("adjacent_bulk", 6, failure(907, 4, 2, 67108864, 67108865)),
        ("maximum_valid_amount", 4, failure(907, 4, 2, 67108864, 2147483647)),
    ):
        yield vector(name, expected, (ZERO,) * 8, left=selector, entry="bulk")
    yield vector("bulk_requested_exceeds_u31", failure(907, 4, 2, 67108864, 2147483648),
    ...
    yield vector("positive_amount_after_exhaustion", failure(907, 4, 2, 67108864, 67108865),
                 (ZERO,) * 8, left=7, right=2, entry="bulk")
    owners = (ZERO, record(2, 1, 1, 1), ZERO, ZERO, ZERO, ZERO)
    for selector, name, expected in (
        (1, "exact_unfold_then_adjacent", compared(1, 67108864) + failure(907, 4, 2, 67108864, 67108865)),
        (2, "after_variable_before_terminal_resume", failure(901, 4, 2, 67108864, 67108865)),
        (3, "template_index_bulk_refuses_exact_sum", failure(901, 4, 2, 67108864, 67108866)),
        (4, "clause_reservation_owns_coordinate", failure(903, 4, 2, 67108864, 67108865)),
