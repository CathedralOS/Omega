#!/usr/bin/env sh
# BOOTSTRAP-AUTHORING-READABILITY experiment: the selected evaluator's live
# call-context boundary exercised by ordinary non-tail recursion at the exact
# 256-context admission, one level beyond it, at the Delta customer's 1,024
# admitted expression depth, and through the application failure mapping; a
# proper-tail control shows the bound does not apply to tail calls.
# fixtures.tsv pins exit code and published byte per fixture.
#
...
command -v python3 >/dev/null 2>&1 || {
    echo "Gamma call-depth comparison: skipped (python3 absent)"
    exit 0
}
