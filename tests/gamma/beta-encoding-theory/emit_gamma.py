...
DESCRIPTIONS = {
    ...
    61: "is_nil(ByteList)",
    62: "choose(Bool, TokenClass, TokenClass)",
    63: "choose(Bool, DState, DState)",
    64: "choose(Bool, ScanResult, ScanResult)",
    65: "choose(Ordering, on_equal, on_other) -> ScanResult",
    66: "choose(Ordering, on_greater, on_other) -> ScanResult",
    67: "choose(Ordering, on_greater, on_other) -> EncodeResult",
    68: "choose(Bool, Admission, Admission)",
    69: "adm_of_word(WordResult): WordValue admits, Overflow exhausts",
    70: "rev_onto(ByteList accumulator, ByteList): reverse onto",
    71: "append(ByteList, ByteList tail)",
    72: "flatten(Fragment, ByteList tail): source-order flattening",
    73: "admit_leaf(Byte, Admission): envelope byte plus checked count",
    74: "admit(Admission, Source): ordered envelope fold",
    75: "shift_in(Word, Nibble): value*16 + nibble modulo 2^64",
    76: "dr1_hex(HexResult): 'r' then digit opens DRd1",
    77: "dre_hex(HexResult): 're' then digit opens DReg2(0xe, n)",
    78: "drd1_hex(HexResult, Nibble): digit closes DReg2",
    79: "dzx_hex(HexResult): first 0x digit opens DZgo",
    80: "dzgo_hex(HexResult, Byte, Word, Byte): digit or ':' in 0x word",
    81: "dfa_step(DState, Byte): one token-automaton transition",
    82: "dfa_fold(DState, ByteList): run the automaton over the tail",
    83: "dfa_end(DState): completed-token class projection",
    84: "classify_first(Byte): first-byte state selection",
    85: "classify(ByteList): Nil is Empty, Cons runs the automaton",
    86: "state_comment_true(State): mark comment mode, keep fields",
    87: "sr_comment_true(ScanResult): mark comment mode on the state",
    88: "wr_succ(WordResult): chained checked successor",
    89: "emit_byte_count(WordResult, Byte, Expect, Bool, Expect, Word, "
        "Status, Word, Fragment)",
    90: "emit_byte(Byte, Expect next, State, Fragment)",
    91: "emit_word_count: counted emit of word_bytes(w)",
    92: "emit_word(Word, Expect next, State, Fragment)",
    93: "choose(Bool, Bool, Bool)",
    94: "status_ok(Status): only Ok continues",
    95: "expect_ready(Expect): only Ready publishes",
    96: "choose(Bool, EncodeResult, EncodeResult)",
    97: "enc_choose_status(Status, ok, invalid, exhausted)",
    98: "sr_state(ScanResult) -> State",
    99: "sr_fragment(ScanResult) -> Fragment",
    100: "sr_choose_expect(Expect, ScanResult x6)",
    101: "dispatch(Bool, Expect, Word, Status, Word, TokenClass)",
    102: "flush(State): emit a completed pending token",
    103: "scan_byte(State, Byte): one scanning transition",
    104: "scan(State, Source): byte-order traversal with fragments",
    105: "finish_state(State, Fragment): Ready+Ok publishes output",
    106: "finish(ScanResult): EOF flush then final projection",
    107: "encode_admitted(Admission, Source, source limit, output limit)",
    108: "encode(Source, source limit, output limit)",
}
...
def main():
    ...
    rows = "\n".join(
        ...
        for identity in range(58, 109))
    dispatcher = [
        "; Emit functions 58..108 in exact wire order; earlier identities",
