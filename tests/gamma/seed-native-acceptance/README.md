...
actually ships — the artifact every seed-executing Gamma gate runs — in both
audited seeds:

- the host-selected seed is stamped through the real materializer (which
  re-signs on macOS), and the non-host seed is stamped at its own recorded
  hole offset;
- each stamped container must satisfy the native format, entry, loader, and
  ...
  no re-sign ran;
- on seed-execution hosts (macOS arm64, Windows x64), the materialized
  container then executes a minimal Gamma source natively and publishes its
  exact receipt byte for byte.
...
the real evaluator tape inside both audited containers — the probe tape does
