...
#   acceptance - the evaluator tape is stamped into BOTH audited Alpha seeds:
...
echo "--- stamped evaluator native acceptance (both audited seeds) ---"
...
case "$ALPHA_SEED" in
  alpha_x64_windows.exe)
    ...
    OTHER_SEED="$OMEGA_PATH_ALPHA/alpha_arm64_macos"
    OTHER_FORMAT=macho
  ...
  *)
    STAMP_FORMAT=macho
    STAMP_PRISTINE=
    OTHER_SEED="$OMEGA_PATH_ALPHA/alpha_x64_windows.exe"
    OTHER_FORMAT=pe
...
# The non-host seed is stamped at its own recorded hole offset; only its
...
echo "acceptance ok — the evaluator tape satisfies the native-container contract in both audited seeds"
...
echo "Gamma seed native acceptance: stamped evaluator satisfies the native-container contract on both audited seeds"
