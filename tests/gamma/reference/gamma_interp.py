#!/usr/bin/env python3
# gamma_interp.py SOURCEFILE — an independent REFERENCE INTERPRETER for the Gamma language: it runs Gamma
...
#
# WHY THIS EXISTS — executable reference meaning for Gamma compiler validation.
# Compiler agreement and self-reproduction say nothing about whether the Gamma compiler compiles correctly. Gamma's written
# small-step meaning lives in ../SEMANTICS.md; this interpreter is an independent, UNTRUSTED executable
# approximation used for finite regression runs. `gamma-correctness-fuzz.sh` runs random programs both ways — interpret here
# vs. compile-with-Gamma-and-run-on-the-VM — and asserts they agree, so a Gamma compiler miscompile surfaces as a
...
from gamma_parser import lex, Parser, flatten_blocks
