...
def cases():
    ...
    yield "outer_short", b"X" * 23, rejected(23, 1), 60
    valid = envelope(example())
    yield "outer_identity", b"X" + valid[1:], rejected(0, 2), 60
    high = MAGIC + words(0x80000000, 0, 0, 0)
    yield "outer_length_high_bit", high, rejected(11, 3), 60
    missing = MAGIC + words(1, 0, 0, 0)
    yield "outer_extent", missing, rejected(8, 4), 60
    yield "outer_trailing", valid + b"X", rejected(228, 5), 60
    ...
    yield "outer_request_capacity_forwarded", oversized, incomplete, 600
