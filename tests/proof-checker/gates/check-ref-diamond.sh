#!/usr/bin/env sh
# TRUST-ANCHOR DIAMOND, independent point — an auditable reference checker
# (tests/proof-checker/reference/check_ref.py) agrees with
...
# tests/proof-checker/reference/check_ref.py is one INDEPENDENT diagnostic realization
...
. "$OMEGA_REPO_ROOT/tools/bootstrap/proof-checker/artifact_env.sh" || exit $?
...
if python3 corpus/fuzz/check-ref-fuzz.py "$T/check.exe" "${1:-200}" > "$T/out" 2>&1; then
  echo "trust-anchor diamond (independent tests/proof-checker/reference/check_ref.py agrees with source/alpha/checker/implementations/gamma/check.gamma on the COMPLETE rule set — logic, first-order, equality, induction, predicates, lemmas, TV certs): $(cat "$T/out")"
else
