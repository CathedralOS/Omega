...
# DEFINITIONAL equality (implementations/gamma/eq.gamma: reduce both sides to normal form) must agree with
# the interpreter's OPERATIONAL evaluation (interp.gamma running Delta's own `plus`)
...
cd "$OMEGA_REPO_ROOT/tests/proof-checker"
SEED="${OMEGA_PATH_ALPHA}"/$ALPHA_SEED
T=$(mktemp -d); trap 'rm -rf -- "$T"' EXIT
stamp_gamma_compiler "$T/gc.exe" >/dev/null
b() { "$T/gc.exe" < "$1" > "$T/x.tape" && stamp_seed "$T/x.tape" "$SEED" "$2" >/dev/null 2>&1; }
b "$OMEGA_PATH_ALPHA_CHECKER/implementations/gamma/eq.gamma" "$T/eq.exe" || { echo "build implementations/gamma/eq.gamma failed"; exit 1; }
b "$OMEGA_REPO_ROOT/tests/delta/interpreter/interp.gamma" "$T/interp.exe" || { echo "build interp.gamma failed"; exit 1; }
# Delta operational defs: plus/mult + neq (Nat eq); append/length + leq (List eq);
# g embeds a user-Nat (Z/S constructors) into Peano — the operational twin of implementations/gamma/eq.gamma's
...
dia() {
  veq=$(printf '%s' "$2" | "$T/eq.exe")                              # implementations/gamma/eq.gamma: equal/differ
...
#    desc        implementations/gamma/eq.gamma (definitional)                                gamma (operational, plus)                                          expect
...
# Lists: append/length, definitional (implementations/gamma/eq.gamma) vs operational (interpreter)
...
# user-FUNCTION reduction: implementations/gamma/eq.gamma's (fun 7 …) rules vs interp's operational (def g …)
...
# BINARY user-function: implementations/gamma/eq.gamma's user-add(x,y) over user-Nat reduces to the same value
...
# COMPOSED user-function: implementations/gamma/eq.gamma's user-mult (fid 11, whose rule body CALLS user-add fid 10)
