...
#                  (tests/beta/compiler/selfhost.sh).
...
# exact assembler construction; rebuilding the native container is a separate
...
if [ -f "$OMEGA_REPO_ROOT/tests/beta/compiler/selfhost.sh" ]; then
  if sh "$OMEGA_REPO_ROOT/tests/beta/compiler/selfhost.sh"; then :; else rc=1; fi
else
  echo "reproduction SKIP — tests/beta/compiler/selfhost.sh not found"
fi

echo ""
if [ $rc = 0 ]; then
  if [ "$ALPHA_VERIFY_MODE" = full ]; then
    echo "alpha seed VERIFIED ✓ (provenance diagnostic + behavior + assembler construction)"
  else
    echo "alpha direct edge VERIFIED ✓ (behavior + exact assembler construction; provenance diagnostic omitted)"
  fi
else
