...
#   reconstruction - the VM reproduces the off-chain Alpha Tape assembler.
...
echo "--- reconstruction (off-chain assembler) ---"
if [ -f "$OMEGA_REPO_ROOT/tests/alpha/tape-assembly/compiler/reconstruction.sh" ]; then
  if sh "$OMEGA_REPO_ROOT/tests/alpha/tape-assembly/compiler/reconstruction.sh"; then :; else rc=1; fi
else
  echo "reconstruction SKIP — Alpha Tape Assembly gate not found"
fi
