...
fn call_mut(
    ...
    let calls = call_positions(plan);
    let legalized_operations::LegalizedScalarInstructionKind::Call(call) =
        &mut plan.scalar_functions[0].blocks[0].instructions[calls[index]].kind
    else {
        unreachable!()
    };
    call
...
#[test]
fn structural_call_order_callee_plan_arguments_lineage_and_evidence_fail_closed() {
    for target in [
        ...
        *placement = replacement;
        ...
        corrupted.scalar_functions[0].blocks[0].instructions[3]
            ...
            .as_mut()
            .unwrap()
            .value = ValueId::new(STRUCTURAL_CALL_PRESERVING_FIRST_RESULT).unwrap();
        expect_rejected(corrupted);
        ...
        let mut corrupted = original.clone();
        let first_call = call_positions(&corrupted)[0];
        corrupted.scalar_functions[0].blocks[0].instructions[first_call].fuel[0].units += 1;
        expect_rejected(corrupted);
    }
}

#[test]
fn structural_call_cross_target_selected_and_allocator_receipts_fail_closed() {
