...
fn original_victim_extends_the_recursive_schedule_and_crosses_spill_pseudos() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let store = function
            .schedule
            .iter()
            .find(|event| matches!(event, omega_selected_instructions_to_register_homes::RecursiveSpillEvent::Store { action, .. } if *action == id(2, 0)))
            .unwrap();
        assert!(matches!(
            store,
            omega_selected_instructions_to_register_homes::RecursiveSpillEvent::Store {
                point: LiveRangePoint(14),
                before_instruction,
                before_reload: Some(source_pressure),
                source: omega_selected_instructions_to_register_homes::RecursiveSpillStoredValue::Original(VirtualRegisterId(5)),
                ..
            } if before_instruction.0 == 7 && *source_pressure == id(1, 0)
        ));
        assert!(matches!(
            function.schedule.iter().find(|event| matches!(event, omega_selected_instructions_to_register_homes::RecursiveSpillEvent::Reload { action, .. } if *action == id(2, 0))),
            Some(omega_selected_instructions_to_register_homes::RecursiveSpillEvent::Reload {
        ...
        assert!(matches!(
            ...
            omega_selected_instructions_to_register_homes::SpillPseudoInstruction::Store {
                ...
                source: omega_selected_instructions_to_register_homes::SpillPseudoStoredValue::Original(VirtualRegisterId(5)),
                ..
            } if action == id(2, 0)
        ));
...
fn independent_replay_rejects_original_lineage_schedule_policy_and_usage_corruption() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        for corrupt in [
            |plan: &mut omega_selected_instructions_to_register_homes::RecursiveSpillInsertionPlan| {
                plan.generalized_spill_insertion =
                    omega_selected_instructions_to_register_homes::GeneralizedSpillInsertionIdentity::from_bytes([0xd0; 32])
            },
            |plan: &mut omega_selected_instructions_to_register_homes::RecursiveSpillInsertionPlan| {
                plan.recovery_actions =
                    omega_selected_instructions_to_register_homes::GeneralizedSpillRecoveryActionIdentity::from_bytes([0xd1; 32])
            },
            |plan: &mut omega_selected_instructions_to_register_homes::RecursiveSpillInsertionPlan| {
            ...
            |plan: &mut omega_selected_instructions_to_register_homes::RecursiveSpillInsertionPlan| {
                plan.allocator_availability =
                    omega_selected_instructions_to_register_homes::AllocatorAvailabilityIdentity::from_bytes([0xd3; 32])
            },
            |plan: &mut omega_selected_instructions_to_register_homes::RecursiveSpillInsertionPlan| {
        ...
        wrong_policy.policy = omega_selected_instructions_to_register_homes::RecursiveSpillInsertionPolicy::EpochTwoReloadVictimBlockLocalUnsignedU64ClosedIntervalFirstFitV1;
        assert_eq!(
        ...
        *source = omega_selected_instructions_to_register_homes::RecursiveSpillStoredValue::Reload(
            id(0, 0),
        );
        assert_eq!(
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        for budget in insufficient {
            assert!(matches!(
                ...
                Err(omega_selected_instructions_to_register_homes::RecursiveSpillInsertionError::BudgetExceeded {
                    required,
                    budget: actual,
