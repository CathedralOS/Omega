...
const ROOT_MUTATIONS: [fn(
    ...
    |plan| {
        plan.register_environment =
            register_model::TargetRegisterEnvironmentIdentity::from_bytes([0xe1; 32])
    },
    |plan| {
        plan.allocator_availability =
            register_homes::AllocatorAvailabilityIdentity::from_bytes([0xe2; 32])
    },
    |plan| {
        plan.optimization_unit = optimization_core::OptimizationUnitIdentity::from_bytes([0xe3; 32])
    },
    |plan| plan.fuel_schedule = semantic_vocabulary::FuelScheduleIdentity::new(99_992).unwrap(),
];
