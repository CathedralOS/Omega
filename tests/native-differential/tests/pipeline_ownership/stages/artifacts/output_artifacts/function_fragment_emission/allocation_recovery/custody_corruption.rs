...
fn generic_allocation_recovery_realization_rejects_detached_and_corrupt_custody() {
    ...
    swap_fixed_frame_realization_source_for_test(&mut source, &mut foreign);
    assert!(validate_fixed_frame_function_relative_realization(&source).is_err());

    let mut encoding = staged_fixed_view_allocation_recovery_realization(target);
    corrupt_fixed_frame_realization_encoding_for_test(&mut encoding);
    assert!(validate_fixed_frame_function_relative_realization(&encoding).is_err());

    let mut layout = staged_fixed_view_allocation_recovery_realization(target);
    corrupt_fixed_frame_realization_layout_for_test(&mut layout);
    assert!(validate_fixed_frame_function_relative_realization(&layout).is_err());

    let mut exit = staged_fixed_view_allocation_recovery_realization(target);
    corrupt_fixed_frame_realization_exit_for_test(&mut exit);
    assert!(validate_fixed_frame_function_relative_realization(&exit).is_err());

    let mut manifest = staged_fixed_view_allocation_recovery_realization(target);
    corrupt_fixed_frame_realization_manifest_for_test(&mut manifest);
    assert!(validate_fixed_frame_function_relative_realization(&manifest).is_err());

    let mut custody = staged_fixed_view_allocation_recovery_realization(target);
    corrupt_fixed_frame_realization_custody_for_test(&mut custody);
    assert!(validate_fixed_frame_function_relative_realization(&custody).is_err());
}
