...
fn aarch64_movn_reaches_fragments_text_object_artifact_and_callable_for_both_routes() {
    ...
    assert!(matches!(source.replay_for_test(), FunctionFragmentReplayInputs::PostAllocationMachine(_)));
    source
};
                ...
                let realization = match emitted.source().replay_for_test() {
                ...
                assert_eq!(&fragment_encoded[8..12], &13_u32.to_le_bytes());
