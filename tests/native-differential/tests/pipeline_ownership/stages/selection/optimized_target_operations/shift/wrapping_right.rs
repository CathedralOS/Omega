...
#[test]
fn optimized_target_lowering_retains_wrapping_shift_right_parameter_custody() {
        ...
        for count_type in integers.iter().copied() {
            for (target_profile, registers, stack) in boolean_equal_location_cases() {
                for (parameter_count, expected) in [(2, registers), (10, stack)] {
                    ...
                    let (semantic, proof) = wrapping_integer_shift_right_parameters_return_artifact(
                        value_type,
                        count_type,
                        parameter_count,
                    );
                    ...
                    let AbstractToTargetFunctionTranslationDisposition::Validated(
                        ...
                        panic!("optimized wrapping shift-right must retain its family row")
                    };
