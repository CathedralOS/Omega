...
#[test]
fn optimized_target_lowering_retains_wrapping_integer_divide_parameter_custody() {
    ...
    for scalar_type in integers {
        for (target_profile, registers, stack) in boolean_equal_location_cases() {
            for (parameter_count, expected) in [(2, registers), (10, stack)] {
                ...
                let (semantic, proof) = wrapping_integer_divide_parameters_return_artifact(
                    scalar_type,
                    parameter_count,
                );
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("optimized wrapping integer divide must retain its family row")
                };
