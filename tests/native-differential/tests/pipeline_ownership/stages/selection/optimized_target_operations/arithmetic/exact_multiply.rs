...
#[test]
fn optimized_target_lowering_retains_exact_integer_multiply_parameter_custody() {
    ...
    for scalar_type in integers {
        for (target_profile, registers, stack) in boolean_equal_location_cases() {
            for (parameter_count, expected) in [(2, registers), (10, stack)] {
                ...
                let (semantic, proof) =
                    exact_integer_multiply_parameters_return_artifact(scalar_type, parameter_count);
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineExactIntegerMultiplyParameters(row),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("optimized exact integer multiply must retain its family row")
                };
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerExpression {
                        ...
                        expression: TargetIntegerExpression::ExactMultiply {
                            psi_operation,
