//! Public optimized-target custody for proof-bearing saturating divide over constant operands.

use super::super::*;

#[test]
fn optimized_target_lowering_retains_saturating_divide_immediate_operand_custody() {
    for target_profile in [
        ...
        for scalar_type in native_fixed_types() {
            for (left, right) in boundary_pairs(scalar_type) {
                let expected = scalar_type.saturating_div(left, right).unwrap();
                let (semantic, proof) =
                    saturating_integer_divide_immediate_operands_return_artifact(
                        scalar_type,
                        left,
                        right,
                    );
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineSaturatingIntegerDivideImmediateOperands(row),
                ) = target.translation_validation().function_roster()[0].translation()
                else {
                    panic!("optimized constant-operand saturating divide must retain its proof-bearing family")
                };
...
fn boundary_pairs(scalar_type: IntegerType) -> [(IntegerValue, IntegerValue); 4] {
    ...
    [
        ...
        (scalar_type.maximum_value(), negative_one_or_maximum),
