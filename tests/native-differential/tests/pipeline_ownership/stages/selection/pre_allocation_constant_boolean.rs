//! Constant-boolean folding through the pre-allocation executor and the
//! `optimize_selected_instructions` entrance, on compiler-produced selected
//! plans. `staged_constant_boolean_with_selections` selects a machine
//! returning a Boolean computed from constant compares — each
//! `CompareI64` over uniquely materialized literals followed by a
//! `MaterializeBoolean*` the exact rule folds — plus a parameter compare
//! and a compare over two boolean results that stay inadmissible;
//! `staged_parameter_boolean_with_selections` selects the same flag reader
//! over two parameters whose compare declines.
...
use crate::tests::{
    ...
    stage_optimized_register_homes_after_pre_allocation, staged_constant_boolean_with_selections,
    staged_parameter_boolean_with_selections, validate_pre_allocation_optimization_custody,
};
...
fn constant_boolean_legality(target: NativeTarget) -> StagedOptimizedAllocationLegality {
    legality(staged_constant_boolean_with_selections(
        target,
        fold_suite(),
...
fn boolean_reader_count(run: &StagedPreAllocationOptimizationRun) -> usize {
    run.current()
        .selected_plan()
        .functions
        .iter()
...
/// The constants' compare publishes a flag state the fold decides, so each
/// admissible `MaterializeBoolean*` becomes a `MaterializeI64` in commit
/// order — and folding `equal`/`less_than` to literal materializations
/// turns the outer `BooleanEqual`'s compare operands into literals the
/// next sweep folds in turn. The parameter compare and the outer compare
/// it feeds stay boolean: their operands are never compile-time constant.
#[test]
fn positive_folds_every_admissible_flag_reader() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let run = run_pre_allocation_optimizations(constant_boolean_legality(target)).unwrap();
        assert_eq!(run.steps().len(), 3, "{target:?}");
        assert_eq!(
            run.policy(),
            PreAllocationPolicy::CONSTANT_BOOLEAN_V1,
            "{target:?}"
        );
        for iteration in run.custody().iterations() {
            assert!(
                matches!(
                    ...
                    PreAllocationTransformationIdentity::ConstantBoolean(_),
                ),
                "{target:?}: every committed step is a constant-boolean fold"
            );
        }
        assert_eq!(
            boolean_reader_count(&run),
            2,
            "{target:?}: the parameter compare and the compare it feeds stay boolean"
        );
        assert_eq!(
            run.attempt().candidates(),
            run.attempt().declined(),
            "{target:?}: the terminal sweep declines the remaining readers"
        );
        assert!(
            run.attempt().candidates() > 0,
            "{target:?}: the terminal sweep still sees flag readers"
        );
        assert_eq!(
...
/// The parameter compare's flag state is not compile-time constant, so
/// every flag reader in the plan declines and the run publishes the
/// unchanged program.
#[test]
fn negative_declines_every_candidate_and_publishes_unchanged() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let selected = staged_parameter_boolean_with_selections(
            target,
            fold_suite(),
            crate::tests::selected_lowering_budget(),
        );
...
fn measured_budget_admits_exact_usage_and_refuses_one_less() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        .unwrap();
        run_pre_allocation_optimizations(legality(staged_constant_boolean_with_selections(
            target,
            fold_suite(),
            exact,
        )))
        ...
        assert!(
            matches!(
                run_pre_allocation_optimizations(legality(
                    staged_constant_boolean_with_selections(target, fold_suite(), short),
                )),
                Err(
                    OptimizedPreAllocationCustodyError::PreAllocationBudgetExceeded { .. }
                        | OptimizedPreAllocationCustodyError::ConstantBoolean(
                            crate::tests::ConstantBooleanError::WorkBudgetExceeded
                        )
                )
            ),
            "{target:?}"
...
/// Without the selection the suite carries no executed pre-allocation
/// phase, so the entrance publishes identity evidence and the plan keeps
/// every flag-reading `MaterializeBoolean*` the selection emitted.
#[test]
fn disabled_selection_leaves_the_plan_untouched() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let selected = staged_constant_boolean_with_selections(
            target,
        ...
        assert_eq!(
            before
                ...
                .count(),
            5,
            "{target:?}: every flag reader stays a boolean materialization"
        );
        assert!(matches!(
...
fn repeated_runs_are_deterministic() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let first = run_pre_allocation_optimizations(constant_boolean_legality(target)).unwrap();
        let second = run_pre_allocation_optimizations(constant_boolean_legality(target)).unwrap();
...
/// Every single-field substitution of the retained custody receipt fails
/// the independent replay. Foreign receipts come from the opposite
/// target's authentic run.
#[test]
fn custody_rejects_every_one_field_substitution() {
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let build = |target: NativeTarget| {
            run_pre_allocation_optimizations(constant_boolean_legality(target)).unwrap()
        };
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        assert_eq!(
            output
                ...
                .count(),
            2,
            "{target:?}: the entrance's published program keeps only the inadmissible readers"
        );
...
/// constant-boolean family identity, in order, beside the analyses and
/// home identities it governs.
#[test]
fn downstream_homes_publish_the_constant_boolean_transformation() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let run = run_pre_allocation_optimizations(constant_boolean_legality(target)).unwrap();
        ...
        assert_eq!(transformations.len(), 3, "{target:?}");
        for transformation in transformations {
            assert!(
                matches!(
                    transformation,
                    PostAllocationSelectedTransformation::ConstantBoolean(_),
                ),
                "{target:?}: every ledger row is a constant-boolean fold"
            );
        }
    }
}
