...
const WIDER_NESTED_AFFINE_ARRAY_SOURCE: &str = r#"
    data Token { value: u64; }
    data Helper {}
    machine Helper::take(token: Token) {}
    data Root {}
    machine Root::enter(values: [[Token; 4]; 2]) {
        Helper::take(values[1][3]);
        Helper::take(values[0][1]);
    }
"#;
...
fn wider_nested_affine_array_plan() -> omega_abstract_operations::AbstractOperationPlan {
    ...
    let syntax = parse_syntax_trees(&tokens).expect("parse wider nested affine array source");
    let resolved = lower_syntax_trees(&syntax).expect("resolve wider nested affine array source");
    let typed =
        lower_symbol_resolved_trees(&resolved).expect("type wider nested affine array source");
    let checked = lower_typed_trees(typed).expect("check wider nested affine array source");
    let terminal =
        lower_machine(&checked, "Root::enter").expect("lower wider nested affine array Psi");
    let semantics =
        encode_module(&terminal.semantic_module).expect("encode wider nested affine Psi");
    let proof =
...
fn nested_affine_octet_plan() -> omega_abstract_operations::AbstractOperationPlan {
    ...
    let syntax = parse_syntax_trees(&tokens).expect("parse nested affine octet source");
    let resolved = lower_syntax_trees(&syntax).expect("resolve nested affine octet source");
    let typed = lower_symbol_resolved_trees(&resolved).expect("type nested affine octet source");
    let checked = lower_typed_trees(typed).expect("check nested affine octet source");
    let terminal = lower_machine(&checked, "Root::enter").expect("lower nested affine octet Psi");
    let semantics =
        encode_module(&terminal.semantic_module).expect("encode nested affine octet Psi");
    let proof =
        encode_proof_bundle(&terminal.proof_bundle).expect("encode nested affine octet proof");
    lower_artifact_sections(&semantics, &proof, &AdmissionProfile::default())
...
fn nested_affine_nonet_plan() -> omega_abstract_operations::AbstractOperationPlan {
    ...
    let syntax = parse_syntax_trees(&tokens).expect("parse nested affine nonet source");
    let resolved = lower_syntax_trees(&syntax).expect("resolve nested affine nonet source");
    let typed = lower_symbol_resolved_trees(&resolved).expect("type nested affine nonet source");
    let checked = lower_typed_trees(typed).expect("check nested affine nonet source");
    let terminal = lower_machine(&checked, "Root::enter").expect("lower nested affine nonet Psi");
    let semantics =
        encode_module(&terminal.semantic_module).expect("encode nested affine nonet Psi");
    let proof =
...
fn nested_affine_eleven_plan() -> omega_abstract_operations::AbstractOperationPlan {
    ...
    let syntax = parse_syntax_trees(&tokens).expect("parse nested affine length-eleven source");
    let resolved = lower_syntax_trees(&syntax).expect("resolve nested affine length-eleven source");
    let typed =
        lower_symbol_resolved_trees(&resolved).expect("type nested affine length-eleven source");
    let checked = lower_typed_trees(typed).expect("check nested affine length-eleven source");
    let terminal =
        lower_machine(&checked, "Root::enter").expect("lower nested affine length-eleven Psi");
    let semantics =
        encode_module(&terminal.semantic_module).expect("encode nested affine length-eleven Psi");
    let proof = encode_proof_bundle(&terminal.proof_bundle)
...
fn nested_affine_thirteen_plan() -> omega_abstract_operations::AbstractOperationPlan {
    ...
    let syntax = parse_syntax_trees(&tokens).expect("parse nested affine length-thirteen source");
    let resolved =
        lower_syntax_trees(&syntax).expect("resolve nested affine length-thirteen source");
    let typed =
        lower_symbol_resolved_trees(&resolved).expect("type nested affine length-thirteen source");
    let checked = lower_typed_trees(typed).expect("check nested affine length-thirteen source");
    let terminal =
        lower_machine(&checked, "Root::enter").expect("lower nested affine length-thirteen Psi");
    let semantics =
        encode_module(&terminal.semantic_module).expect("encode nested affine length-thirteen Psi");
    let proof = encode_proof_bundle(&terminal.proof_bundle)
        .expect("encode nested affine length-thirteen proof");
    lower_artifact_sections(&semantics, &proof, &AdmissionProfile::default())
...
fn nested_affine_fourteen_plan() -> omega_abstract_operations::AbstractOperationPlan {
    ...
    let syntax = parse_syntax_trees(&tokens).expect("parse nested affine length-fourteen source");
    let resolved =
        lower_syntax_trees(&syntax).expect("resolve nested affine length-fourteen source");
    let typed =
        lower_symbol_resolved_trees(&resolved).expect("type nested affine length-fourteen source");
    let checked = lower_typed_trees(typed).expect("check nested affine length-fourteen source");
    let terminal =
        lower_machine(&checked, "Root::enter").expect("lower nested affine length-fourteen Psi");
    let semantics =
        encode_module(&terminal.semantic_module).expect("encode nested affine length-fourteen Psi");
    let proof = encode_proof_bundle(&terminal.proof_bundle)
        .expect("encode nested affine length-fourteen proof");
    lower_artifact_sections(&semantics, &proof, &AdmissionProfile::default())
...
fn nested_affine_fifteen_plan() -> omega_abstract_operations::AbstractOperationPlan {
    ...
    let syntax = parse_syntax_trees(&tokens).expect("parse nested affine length-fifteen source");
    let resolved =
        lower_syntax_trees(&syntax).expect("resolve nested affine length-fifteen source");
    let typed =
        lower_symbol_resolved_trees(&resolved).expect("type nested affine length-fifteen source");
    let checked = lower_typed_trees(typed).expect("check nested affine length-fifteen source");
    let terminal =
        lower_machine(&checked, "Root::enter").expect("lower nested affine length-fifteen Psi");
    let semantics =
        encode_module(&terminal.semantic_module).expect("encode nested affine length-fifteen Psi");
    let proof = encode_proof_bundle(&terminal.proof_bundle)
...
fn assert_wider_nested_affine_array_native_custody(
    ...
    let caller = plan
        ...
        .iter()
        .find(|function| function.machine == caller_machine)
        .expect("wider nested caller remains present");
    let root_place = caller.structural_parameters[0].place;
    let root_type = caller.structural_parameters[0].structural_type;
    let StructuralTypeShape::FixedArray {
        element: inner_type,
    ...
    missing
        ...
        .remove(0);
    let missing_unit =
        reconstruct_psi_optimization_unit_seed(&missing, TerminalFuelSchedule::CURRENT.identity())
            .expect("missing wider cleanup remains representable as optimizer input");
    assert!(validate_psi_optimization_unit(&missing_unit).is_err());
    assert!(lower_to_target_operations(&missing, NativeTarget::linux_x64()).is_err());

    let mut duplicate = plan.clone();
    let duplicate_actions = duplicate
        .functions
        .iter_mut()
        .find(|function| function.machine == caller_machine)
        .unwrap()
        ...
        .unwrap();
    duplicate_actions.insert(0, duplicate_actions[0].clone());
    let duplicate_unit = reconstruct_psi_optimization_unit_seed(
        &duplicate,
        TerminalFuelSchedule::CURRENT.identity(),
    )
    .expect("duplicated wider cleanup remains representable as optimizer input");
    assert!(validate_psi_optimization_unit(&duplicate_unit).is_err());
    assert!(lower_to_target_operations(&duplicate, NativeTarget::linux_x64()).is_err());

    let mut wrong_path = plan.clone();
    let wrong_call = wrong_path
        .functions
        .iter_mut()
        .find(|function| function.machine == caller_machine)
        .unwrap()
    ...
    for target in [
        ...
        for forged in [
            ...
            assert!(
                ...
                "object replay rejects wider nested call geometry drift",
            );
        }

        let mut tampered_machine = machine.clone();
        tampered_machine
            .functions
            .iter_mut()
            .find(|function| function.machine == caller_machine)
            ...
            .actions
            .reverse();
        assert!(build_object_artifact(&tampered_machine).is_err());
