...
fn produce_source(entry: &str, source: &str) -> CanonicalTerminalArtifact {
    let tokens = source_files_to_tokens::Lexer::new(source)
        .tokenize().expect("tokenize scalar-case source");
    let syntax = tokens_to_syntax_trees::parse_syntax_trees(&tokens).expect("parse scalar-case source");
...
fn publish(artifact: &CanonicalTerminalArtifact, target: NativeTarget)
    -> (image_emission::ExecutableImage, usize)
{
    let module = terminal_codec::decode_module(artifact.semantic_bytes()).unwrap();
    let selections = OptimizationSelections::new([]).unwrap();
    let optimized = optimize_artifact_sections(
        artifact.semantic_bytes(), artifact.proof_bytes(), &AdmissionProfile::default(),
    ...
    let target_operations = abstract_operations_to_target_operations::lower_optimized_to_target_operations(
        optimized, target,
    ...
    let physical = native_realization::stage_optimized_verified_physical_pipeline(
        target_operations, post_terminal.selections(),
    ...
    let record = image_emission::build_installation_record(
        &image, semantic_vocabulary::ProfileDecisionId::new(1).unwrap(),
    ).unwrap();
    ...
    assert_eq!(image_emission::encode_installation_record(&decoded).unwrap(), bytes);
...
fn source_scalar_case_results_reach_canonical_terminal() {
    for entry in ["choose", "collect"] { let _ = produce(entry); }
}
...
fn assert_host(entry: &str, driver: &str) {
    ...
    {
        let _ = (entry, driver);
        eprintln!("SKIP: direct aggregate native runtime requires Linux x64/ARM64 or macOS ARM64; Windows indirect return remains unsupported");
    }
}
