...
use target_operations::{
    CallSiteOwner, ScalarParameterLocation, TargetIntegerExpression, TargetOperation,
};
...
use terminal_psi::{OperationKind, TerminalAffineCleanupAction, TerminalMachineResult, Terminator};
...
fn source_scalar_result_precedes_nominal_cleanup_through_all_native_artifacts() {
    ...
    assert!(matches!(
        ...
        [AbstractOperation::IntegerConstant { .. }, AbstractOperation::Return {
            cleanup_actions,
            ..
    ...
    for case in target_cases() {
        ...
        assert!(matches!(
            ...
            TargetOperation::ScalarReturnWithCleanup {
                ...
                && matches!(scalar.as_ref(), TargetOperation::ReturnIntegerImmediate { .. })
        ));
        ...
        assert!(matches!(
            ...
            Some(AssignedOperation::ScalarReturnWithCleanup { .. })
        ));
        ...
        let installation_bytes = encode_installation_record(&installation)
    ...
    for case in target_cases() {
        ...
        assert!(matches!(
            target_plan
                ...
                .find(|function| function.machine == entry_machine)
                .map(|function| &function.operation),
            Some(TargetOperation::BooleanControlWithCleanup { .. })
        ));
        let assigned = assign_registers(&target_plan)
            .unwrap_or_else(|error| panic!("{:?} assignment failed: {error:?}", case.target));
        assert!(matches!(
            assigned
                ...
                .find(|function| function.machine == entry_machine)
                .map(|function| &function.operation),
            Some(AssignedOperation::BooleanControlWithCleanup { .. })
        ));
        ...
        assert_eq!(
            ...
            emitted_edges
                .iter()
                .copied()
                .map(|edge| CallSiteOwner::CleanupAction {
                    edge,
                    action_ordinal: 1,
                })
                .collect()
...
fn nominal_boolean_convergence_has_one_physical_cleanup_tail_on_all_targets() {
    ...
    for case in target_cases() {
        ...
        assert!(matches!(
            &target_entry.operation,
            TargetOperation::ScalarReturnWithCleanup { scalar, .. }
        ...
        assert!(matches!(
            assigned
                ...
                .find(|function| function.machine == entry_machine)
                .map(|function| &function.operation),
            Some(AssignedOperation::ScalarReturnWithCleanup { scalar, .. })
                if matches!(scalar.as_ref(),
        ...
        assert!(build_object_artifact(&forged_structural_read).is_err());
        ...
        assert!(build_object_artifact(&forged_structural_source).is_err());
        ...
        assert!(build_object_artifact(&forged_structural_field).is_err());
        ...
        assert!(build_object_artifact(&forged_structural_field_offset).is_err());
        ...
        function.bytes[mutation_offset] ^= match case.target.architecture {
            target::Architecture::X86_64 => 0x08,
            target::Architecture::Aarch64 => 0x20,
        };
        let ScalarControlFlowEvidence::BooleanSharedConvergence {
            structural_conditions,
            ..
        ...
        assert!(build_object_artifact(&coherently_forged_structural_bytes).is_err());
        ...
        let ScalarControlFlowEvidence::BooleanSharedConvergence { joins, .. } = &mut function
            .scalar_stack
            .as_mut()
        ...
        joins.pop();
        assert!(build_object_artifact(&missing_join).is_err());
    }
}
...
fn arbitrary_exact_mixed_shift_chains_emit_on_every_native_target() {
    ...
    assert_eq!(
        abstract_plan
            ...
            .iter()
            .flat_map(|function| &function.operations)
            .filter(|operation| matches!(operation, AbstractOperation::IntegerExactCast { .. }))
            .count(),
        65,
    );
    assert_eq!(
        abstract_plan
            ...
            .iter()
            .flat_map(|function| &function.operations)
            .filter(|operation| matches!(operation, AbstractOperation::IntegerWiden { .. }))
            .count(),
        16,
    );
    assert_eq!(
        abstract_plan
            ...
            .flat_map(|function| &function.operations)
            .filter(|operation| {
                matches!(operation, AbstractOperation::WrappingIntegerAdd { .. })
            })
            .count(),
        48,
    );
    assert_eq!(
        abstract_plan
            ...
            .flat_map(|function| &function.operations)
            .filter(|operation| {
                matches!(operation, AbstractOperation::WrappingIntegerSubtract { .. })
            })
            .count(),
        22,
    );
    assert_eq!(
        abstract_plan
            ...
            .flat_map(|function| &function.operations)
            .filter(|operation| {
                matches!(operation, AbstractOperation::WrappingIntegerMultiply { .. })
            })
            .count(),
        65,
    );
    assert_eq!(
        abstract_plan
            ...
            .iter()
            .flat_map(|function| &function.operations)
            .filter(|operation| matches!(operation, AbstractOperation::ExactIntegerDivide { .. }))
            .count(),
        16,
    );
    assert_eq!(
...
fn affine_cast_affine_sandwich_emits_on_every_native_target() {
    ...
    assert_eq!(
        abstract_plan
            ...
            .iter()
            .flat_map(|function| &function.operations)
            .filter(|operation| matches!(operation, AbstractOperation::WrappingIntegerAdd { .. }))
            .count(),
        5,
    );
    assert_eq!(
...
fn nominal_integer_comparison_convergence_has_one_physical_cleanup_tail_on_all_targets() {
    ...
    for addend in [entry.parameters[11].id, entry.parameters[12].id] {
    ...
    for subtrahend in [entry.parameters[13].id, entry.parameters[14].id] {
        let obligation = entry
            .blocks
            .iter()
            .flat_map(|block| &block.operations)
    ...
    let runtime_divisor_obligations = entry
        .blocks
        .iter()
        .flat_map(|block| &block.operations)
        .filter_map(|operation| match operation.kind {
    ...
    let runtime_shift_left_obligations = entry
        .blocks
        .iter()
        .flat_map(|block| &block.operations)
        .filter_map(|operation| match operation.kind {
    ...
    for factor in [entry.parameters[6].id, entry.parameters[7].id] {
        let obligation = entry
            .blocks
            .iter()
            .flat_map(|block| &block.operations)
    ...
    let mixed_add_subtract_obligations =
        operations
            .iter()
            .find_map(|outer| {
                ...
                (inner_left == entry.parameters[1].id && has_u8_constant(inner_right, 3))
    ...
    let offset_chain_cast_obligations = operations
        ...
        .find_map(|cast| {
            ...
            (inner_left == entry.parameters[1].id && has_u8_constant(inner_right, 3)).then_some([
                inner_obligation,
                middle_obligation,
                outer_obligation,
                cast_obligation,
    ...
    let find_cast_then_offset = |subtract: bool| {
        operations.iter().find_map(|outer| {
            ...
            (operand == entry.parameters[0].id).then_some([cast_obligation, arithmetic_obligation])
        })
    };
    ...
    let find_multiply_chain_then_cast = |outer_factor| {
        operations.iter().find_map(|cast| {
            ...
            (inner_left == entry.parameters[1].id).then_some([
                inner_obligation,
                outer_obligation,
                cast_obligation,
            ])
    ...
    let nested_multiply_obligations =
        operations
            .iter()
            .find_map(|outer| {
                let OperationKind::ExactIntegerMultiply {
                ...
                (inner_left == entry.parameters[1].id && has_u8_constant(inner_right, 2))
                    .then_some([inner_obligation, middle_obligation, outer_obligation])
            })
    ...
    let nested_divide_remainder_obligations =
        operations
            .iter()
            .find_map(|outer| {
    ...
    let signed_remainder_cast_obligations =
        find_cast_after_divide_remainder(entry.parameters[4].id, IntegerValue::Signed(-3), true)
    ...
    let signed_cast_then_shift_right_obligations =
        find_two_link_cast_then_shift_right(entry.parameters[4].id)
            .expect("native path retains one signed post-cast right-shift chain");
    let cross_cast_then_shift_right_obligations =
        find_two_link_cast_then_shift_right(entry.parameters[5].id)
...
fn contextual_scalar_cleanup_is_verified_then_projected_on_all_targets() {
    ...
    for case in target_cases() {
        ...
        let TargetOperation::ReturnIntegerExpression { expression, .. } = scalar.as_ref() else {
            panic!("scalar cleanup retains a runtime input-derived integer expression")
        };
        let TargetIntegerExpression::WrappingMultiply { left, right, .. } = expression else {
...
fn assert_source_structural_return(
    ...
    verify_module(
        &module,
        &terminal_codec::decode_proof_bundle(&proof_bytes).expect("proof bundle decodes"),
        &AdmissionProfile::default(),
    )
    ...
    assert_eq!(abstract_plan.psi, original_identity);
    ...
    assert_eq!(
        ...
        AbstractFunctionResult::Structural(structural_result.clone())
    );
    assert_eq!(
    ...
    for case in target_cases() {
        ...
        assert_eq!(target_plan.psi, original_identity);
        ...
        assert_eq!(machine_code.psi, original_identity);
        ...
        dropped_claim.functions[0]
            ...
            .clear();
        assert!(
            build_object_artifact(&dropped_claim).is_err(),
        ...
        if parameter_cleanup_count != 0 || local_cleanup_count != 0 {
            ...
            dropped_cleanup.functions[0]
                ...
                .clear();
            assert!(
                build_object_artifact(&dropped_cleanup).is_err(),
        ...
        if local_cleanup_count != 0 {
            ...
            missing_local.functions[0]
                ...
                .clear();
            assert!(
                build_object_artifact(&missing_local).is_err(),
            ...
            assert!(
                build_object_artifact(&aliased_local).is_err(),
            ...
            assert!(
                build_object_artifact(&mutated_local_type).is_err(),
            ...
            if local_cleanup_count == 2 {
                ...
                assert!(
                    build_object_artifact(&duplicate_local).is_err(),
                ...
                *declaration_ordinal = 2;
                assert!(
                    build_object_artifact(&gapped_ordinal).is_err(),
                ...
                reordered_cleanup.functions[0]
                    ...
                    .swap(0, 1);
                assert!(
                    build_object_artifact(&reordered_cleanup).is_err(),
        ...
        if parameter_cleanup_count != 0 {
            ...
            assert!(
                build_object_artifact(&aliased_parameter).is_err(),
            ...
            if parameter_cleanup_count > 1 {
                ...
                assert!(
                    build_object_artifact(&duplicate_tail).is_err(),
        ...
        let object = build_object_artifact(&machine_code).unwrap_or_else(|error| {
        ...
        assert_eq!(container.psi, original_identity);
        ...
        let changed_claim = decode_installation_record(&changed_claim_bytes)
        ...
        assert!(
            validate_installation_record(&changed_claim, &image).is_err(),
        ...
        if local_cleanup_count != 0 {
            ...
            assert_eq!(
                decode_installation_record(&changed_local_type_bytes),
                Err(InstallationError::InvalidStructuralReturn(machine.id)),
        ...
        if local_cleanup_count == 2 {
            ...
            assert_eq!(
                decode_installation_record(&changed_second_ordinal_bytes),
                Err(InstallationError::InvalidStructuralReturn(machine.id)),
        ...
        if expected_cleanup.len() >= 2 {
            ...
            assert_eq!(
                decode_installation_record(&reordered_cleanup_bytes),
                Err(InstallationError::InvalidStructuralReturn(machine.id)),
        ...
        if parameter_cleanup_count != 0 || local_cleanup_count != 0 {
            ...
            assert_eq!(
                decode_installation_record(&changed_cleanup_bytes),
                Err(InstallationError::InvalidStructuralReturn(machine.id)),
    ...
    for case in target_cases() {
        ...
        assert!(matches!(
            target_plan
                ...
                .find(|function| function.machine == entry)
                .map(|function| &function.operation),
            Some(TargetOperation::ReturnStructuralCall { .. })
        ));
        let assigned = assign_registers(&target_plan)
            .unwrap_or_else(|error| panic!("{:?} assignment failed: {error:?}", case.target));
        assert!(matches!(
            assigned
                ...
                .find(|function| function.machine == entry)
                .map(|function| &function.operation),
            Some(AssignedOperation::ReturnStructuralCall { .. })
        ));
        let machine_code = emit_machine_code(&assigned)
            .unwrap_or_else(|error| panic!("{:?} emission failed: {error:?}", case.target));
        ...
        changed_machine_code
            ...
            .returned_claims[0] = semantic_vocabulary::ClaimId::new(99).unwrap();
        assert!(
            build_object_artifact(&changed_machine_code).is_err(),
...
fn two_fragment_structural_result_call_is_exact_on_direct_aggregate_abis() {
    ...
    let AbstractOperation::CallStructural { callee, .. } = &abstract_plan
        .functions
        .iter()
        .find(|function| function.machine == entry)
    ...
    for case in target_cases() {
        ...
        reordered_callee
            ...
            .swap(0, 1);
        assert!(
            build_object_artifact(&reordered_callee).is_err(),
        ...
        reordered_call
            ...
            .swap(0, 1);
        assert!(
            build_object_artifact(&reordered_call).is_err(),
        ...
        let installation =
            build_installation_record(&image, ProfileDecisionId::new(1).expect("profile decision"))
                .unwrap_or_else(|error| {
                    panic!("{:?} wide installation failed: {error:?}", case.target)
                });
        let encoded = encode_installation_record(&installation)
        ...
        let decoded = decode_installation_record(&reordered_installation)
...
fn host_structural_round_trip(bytes: &[u8], value: u64, cleanup_count: usize) -> bool {
    ...
    let directory = std::env::temp_dir().join(format!(
        "omega-native-structural-return-{}-{nonce}-{sequence}",
        std::process::id()
    ));
    ...
    let driver = if cleanup_count == 8 {
        format!(
            "#include <stdint.h>\n\
extern uint64_t entry(uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t);\n\
int main(void) {{ return entry(UINT64_C({value}), 1, 2, 3, 4, 5, 6, 7, 8) == UINT64_C({value}) ? 0 : 1; }}\n"
        )
    } else if cleanup_count == 1 {
        format!(
            "#include <stdint.h>\n\
extern uint64_t entry(uint64_t, uint64_t);\n\
int main(void) {{ return entry(UINT64_C({value}), UINT64_C(0xcafe)) == UINT64_C({value}) ? 0 : 1; }}\n"
        )
    } else {
        format!(
            "#include <stdint.h>\n\
extern uint64_t entry(uint64_t);\n\
int main(void) {{ return entry(UINT64_C({value})) == UINT64_C({value}) ? 0 : 1; }}\n"
        )
    };
