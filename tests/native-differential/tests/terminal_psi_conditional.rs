...
fn conditional_round_trips_executes_and_lowers_both_ordered_successors() {
    ...
    assert!(matches!(
        ...
        TargetIntegerControl::Return { psi_return_edge, .. }
...
fn bounded_two_return_conditional_derives_native_stack_by_arm_maximum() {
    ...
    for (target, expected_peak) in [
        ...
        assert_eq!(
            derive_stack_demand(&artifact, MachineId::new(1).unwrap())
...
fn bounded_conditional_call_arm_composes_native_stack_closure() {
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let artifact =
            build_object_artifact(&machine_code).expect("conditional call arm object validates");
        let demand = derive_stack_demand(&artifact, MachineId::new(1).unwrap())
...
fn conditional_arms_lower_through_computed_jumps_to_a_shared_tail() {
    ...
    assert!(matches!(
        ...
        TargetIntegerExpression::WrappingAdd { left, .. }
            if matches!(left.as_ref(), TargetIntegerExpression::WrappingAdd { .. })
    ));
    let TargetIntegerControl::Return {
    ...
    assert!(matches!(
        false_expression,
        TargetIntegerExpression::WrappingAdd { left, .. }
            if matches!(left.as_ref(), TargetIntegerExpression::WrappingMultiply { .. })
    ));
...
fn nested_boolean_result_conditional_reaches_native_control() {
    ...
    assert!(matches!(
        when_false.control.as_ref(),
        TargetBooleanControl::ReturnParameter { psi_return_edge, .. }
            if *psi_return_edge == EdgeId::new(5).unwrap()
    ));
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let target_plan = lower_to_target_operations(&abstract_plan, target)
...
fn run_host_boolean_conditional(
    ...
    let directory = fresh_scratch_directory("omega-native-boolean-conditional");
    ...
    let driver = format!(
        "#include <stdbool.h>\n\
extern bool entry(bool, bool, bool, bool, bool);\n\
...
fn run_host_runtime_nested_conditional(
    ...
    let directory = fresh_scratch_directory("omega-native-runtime-nested");
    let _cleanup = ScratchDirectory(directory.clone());
    let assembly_path = directory.join("entry.s");
    let driver_path = directory.join("driver.c");
    let executable_path = directory.join("entry");
    ...
    let driver = format!(
        "#include <stdbool.h>\n#include <stdint.h>\n\
extern uint8_t entry(uint8_t, bool, uint8_t, bool);\n\
int main(void) {{ return entry({first}, {}, {third}, {}); }}\n",
