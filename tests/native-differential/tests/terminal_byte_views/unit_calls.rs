...
use semantic_vocabulary::{
    BlockId, ContractId, EdgeId, MachineId, OperationId, PlaceId, ScalarType, ValueId,
};
use target::NativeTarget;
use terminal_psi::{
    Operation, OperationKind, OperationResult, StructuralAccess, TerminalMachineResult,
...
fn unit_byte_view_calls_cross_lower_with_true_unit_results() {
    ...
    for machine in module
        .machines
        .iter()
        .filter(|machine| machine.id != MachineId::new(1).unwrap())
    {
    ...
    for target in [
        ...
        assert_eq!(
            actual,
            [(100, 105, 1), (200, 205, 100), (200, 207, 100)].map(|(caller, operation, callee)| (
                MachineId::new(caller).unwrap(),
...
fn unit_byte_view_calls_return_without_corrupting_descriptor_or_stack() {
    ...
    eprintln!(
        "SKIP Unit byte-view call execution: Linux/macOS cc hosts supported; Windows runtime route unavailable"
    );
}
