...
fn nested_conditional_stack_facts_survive_installation_and_reject_forgery() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        for nested_false_arm in [false, true] {
            ...
            {
                let ScalarControlFlowEvidence::ConditionalTree { decisions, .. } = &mut forged
                    .functions[0]
                    .scalar_stack
                    .as_mut()
            ...
            let image = emit_executable_image(&artifact, 31 + u16::from(nested_false_arm))
...
fn four_leaf_conditional_stack_facts_survive_installation_and_reject_forgery() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let mut forged = emitted.clone();
        let ScalarControlFlowEvidence::ConditionalTree { decisions, .. } = &mut forged.functions[0]
            .scalar_stack
            .as_mut()
        ...
        let artifact =
            build_object_artifact(&emitted).expect("object boundary replays all four leaves");
        let demand = derive_stack_demand(&artifact, machine_id(1))
            .expect("derive four-leaf conditional stack demand");
        let image = emit_executable_image(&artifact, 33)
...
fn nested_crash_leaf_stack_facts_survive_installation_and_reject_forgery() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let image = emit_executable_image(&artifact, 35)
...
fn four_leaf_crash_stack_facts_survive_installation_and_reject_forgery() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let image =
            emit_executable_image(&artifact, 37).expect("emit four-leaf crash executable image");
        let installation =
...
fn nested_division_stack_facts_survive_installation_and_reject_forgery() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let ScalarControlFlowEvidence::ConditionalTree { branches, .. } = &emitted.functions[0]
            .scalar_stack
            .as_ref()
        ...
        if target.architecture == Architecture::X86_64 {
            let mut forged = emitted.clone();
            let ScalarControlFlowEvidence::ConditionalTree { branches, .. } = &mut forged.functions
                [0]
            .scalar_stack
            .as_mut()
        ...
        let image =
            emit_executable_image(&artifact, 39).expect("emit nested division executable image");
        let installation =
...
fn nested_condition_division_stack_facts_survive_installation() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let image = emit_executable_image(&artifact, 40)
...
fn four_leaf_division_stack_facts_survive_installation_and_reject_forgery() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let image =
            emit_executable_image(&artifact, 41).expect("emit four-leaf division executable image");
        let installation =
            build_installation_record(&image, ProfileDecisionId::new(6).expect("profile decision"))
...
fn deeper_conditional_tree_survives_installation_and_rejects_forgery() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let image =
            emit_executable_image(&artifact, 43).expect("emit deeper conditional executable image");
        let installation =
...
fn install_signed_division_in_nested_leaf(
    ...
    *scalar_type = signed;
    ...
    let TargetIntegerControl::Conditional { when_true, .. } = nested_arm.control.as_mut() else {
        unreachable!()
    };
...
fn install_signed_division_conditions(plan: &mut TargetOperationPlan) {
    let signed = IntegerType::new(IntegerSign::Signed, 64).expect("i64");
    ...
    when_true.control = Box::new(TargetIntegerControl::ConditionalExpression {
    ...
    function.operation = TargetOperation::ReturnIntegerExpressionConditionalControl {
...
fn four_leaf_conditional_plan(target: NativeTarget) -> TargetOperationPlan {
    let mut plan = nested_conditional_plan(target, false);
    let condition_register = match target.architecture {
...
fn nested_conditional_plan(target: NativeTarget, nested_false_arm: bool) -> TargetOperationPlan {
    ...
    let nested = TargetConditionalIntegerArm {
        psi_edge: edge_id(if nested_false_arm { 2 } else { 1 }),
        control: Box::new(TargetIntegerControl::Conditional {
            condition_source: value_id(2),
            condition_parameter_index: if nested_false_arm { 2 } else { 1 },
            condition_location: ScalarParameterLocation::Register(if nested_false_arm {
                registers[2]
            } else {
                registers[1]
            }),
    ...
    TargetOperationPlan {
        ...
        functions: vec![TargetFunction {
            ...
            operation: TargetOperation::ReturnIntegerConditionalControl {
                ...
                condition_location: ScalarParameterLocation::Register(registers[0]),
                scalar_type,
                when_true,
                when_false,
            },
