...
/// `let`-held borrows beyond a head-of-body receiver position — as store
/// roots, call arguments, reborrow sources, or subloans held live across a
/// parent receiver call — all meet the same checked-control-plan hole.
#[test]
fn held_borrow_uses_still_miss_the_checked_control_plan() {
    for (name, source) in [
        (
            "mid-body let borrow",
        ...
        (
            "let borrow reborrowed as call argument",
            "machine stamp(slot: &write u64, value: u64) { slot = value; }
            machine forward(root: &mut u64, value: u64) {
                let held: &write u64 = &write root;
        ...
        (
            "scalar let borrow as store root",
        ...
        (
            "let mut borrow as call argument",
        ...
        (
            "mut receiver on parent with live write subloan",
            "data Record [copy] { value: u16; }
            machine Record::replace(&write self, value: u16) { self.value = value; }
