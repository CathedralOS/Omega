...
fn canonical_o0_agrees_from_terminal_meaning_through_runnable_linux_image() {
    ...
    for (target, seed) in [
        ...
        assert_eq!(image.psi(), abstract_plan.psi);
        ...
        let installation_bytes =
            encode_installation_record(&installation).expect("installation encoding");
        assert_eq!(
...
fn realize_image(
    ...
    emit_executable_image(&object, 3).expect("O0 Linux executable image")
}
