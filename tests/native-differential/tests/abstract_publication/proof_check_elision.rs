...
fn proof_check_elision_projects_dead_exact_work_and_retains_evidence() {
    ...
    let optimized = publish_optimization_run(run(dead_exact_add_verified(), selections)).unwrap();
...
fn proof_check_elision_projects_live_exact_identity_with_fact_and_fuel_custody() {
    let selections = OptimizationSelections::new([Optimization::ProofCheckElision]).unwrap();
    let optimized =
        publish_optimization_run(run(live_exact_add_zero_verified(), selections)).unwrap();
...
fn proof_check_elision_projects_and_lowers_live_exact_divide_by_one() {
    let selections = OptimizationSelections::new([Optimization::ProofCheckElision]).unwrap();
    let optimized =
        publish_optimization_run(run(live_exact_divide_by_one_verified(), selections)).unwrap();
...
fn proof_check_elision_projects_and_lowers_live_exact_multiply_by_zero() {
    let selections = OptimizationSelections::new([Optimization::ProofCheckElision]).unwrap();
    let optimized =
        publish_optimization_run(run(live_exact_multiply_by_zero_verified(), selections)).unwrap();
...
fn proof_check_elision_projects_and_lowers_live_exact_zero_dividend() {
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let optimized =
            publish_optimization_run(run(live_exact_zero_dividend_verified(), selections.clone()))
                .unwrap();
...
fn proof_check_elision_projects_and_lowers_live_exact_self_subtract() {
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let optimized =
            publish_optimization_run(run(live_exact_self_subtract_verified(), selections.clone()))
                .unwrap();
...
fn proof_check_elision_projects_and_lowers_live_exact_self_divide() {
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let optimized =
            publish_optimization_run(run(live_exact_self_divide_verified(), selections.clone()))
                .unwrap();
