...
fn independent_validation_rejects_projected_operation_corruption() {
    ...
    let optimized = publish_optimization_run(run(exact_add_verified(), selections)).unwrap();
...
fn independent_validation_rejects_block_offset_corruption() {
    ...
    let optimized =
        publish_optimization_run(run(redundant_block_parameter_verified(), selections)).unwrap();
    let mut corrupted = optimized.plan().clone();
