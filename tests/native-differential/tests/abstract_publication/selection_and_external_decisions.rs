...
fn current_program_outlives_publication_and_its_replay_evidence() {
    ...
    .unwrap();
    let program = optimized.shared_program();
    assert!(std::sync::Arc::ptr_eq(
        &program,
        &optimized.shared_program()
    ));
    assert!(std::ptr::eq(program.as_ref(), optimized.plan()));
    assert_eq!(optimized.evidence().commits.len(), 1);
    drop(optimized);
    assert_eq!(std::sync::Arc::strong_count(&program), 1);
    assert!(matches!(
        program.functions[0].operations[2],
...
fn empty_selection_projects_the_original_plan_deterministically() {
    ...
    let first = publish_optimization_run(run(empty_verified(), selections.clone())).unwrap();
    let second = publish_optimization_run(run(empty_verified(), selections)).unwrap();
...
fn projection_rejects_detached_external_decision_recording() {
    ...
    assert!(matches!(
        publish_optimization_run(optimized),
...
fn externally_replayed_psi_decisions_reach_verified_terminal_projection() {
    ...
    .unwrap();
    let optimized = publish_optimization_run(replayed).unwrap();
...
fn lower_only_suite_records_no_psi_completion() {
    ...
    let optimized =
        publish_optimization_run(run_pipeline(empty_verified(), selections.clone())).unwrap();
...
fn mixed_suite_records_only_its_psi_completion() {
    ...
    let psi_selections =
        OptimizationSelections::new([Optimization::SparseConditionalConstantPropagation]).unwrap();
    let optimized =
        publish_optimization_run(run_pipeline(exact_add_verified(), selections.clone())).unwrap();
