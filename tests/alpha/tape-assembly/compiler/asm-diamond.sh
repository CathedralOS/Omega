...
# not ported from assembler.alphaasm). This gate compares both assemblers over a
...
. "$OMEGA_REPO_ROOT/tools/alpha/tape-assembly/artifact_env.sh" || exit $?
cd "$OMEGA_GATE_DIR"
command -v python3 >/dev/null 2>&1 || { echo "asm-diamond SKIP — no python3"; exit 0; }
T=$(mktemp -d); trap 'rm -rf "$T"' EXIT
materialize_alpha_tape_assembler "$T/assembler" >/dev/null
ASM="$T/assembler"
PASS=0; FAIL=0
...
cmp_asm "assembler.alphaasm (self)" "$OMEGA_PATH_ALPHA_TAPE_ASSEMBLER_SOURCE"
for ex in examples/*.alphaasm; do [ -f "$ex" ] && cmp_asm "example $(basename "$ex")" "$ex"; done

echo "assembler diamond (independent reference asm_ref.py assembles byte-identically to assembler.alphaasm): $PASS ok, $FAIL failed"
[ "$FAIL" = 0 ] || exit 1
