"""Compare the canonical Epsilon edge with an independent semantic model.

Source, stdin, rejection and profile mutations cover the synthetic corpus.
D customer refinement runs with its literal composition checks in the
sibling d-composition gate, sharing each base evaluator invocation.

Usage: python3 gate.py <artifact-directory>
run.sh materializes the bound evaluator, compiler, closure, entry and support.
"""
...
def main():
    if len(sys.argv) != 2:
        raise SystemExit("usage: gate.py <artifact-directory>; D customers use d-composition/run.sh")
    ...
    print(
        ...
        f"observations agreed byte-for-byte, {stats['profile']} profile "
