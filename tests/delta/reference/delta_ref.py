#!/usr/bin/env python3
# delta_ref.py - an INDEPENDENT reference evaluator for Delta (the meaning substrate), written from
# delta/LANGUAGE.md + interp.gamma's grammar, NOT ported from interp.gamma. Reads a Delta program on stdin,
# prints the signed decimal result, and exits with its low byte (matching interp.gamma).
#
# WHY THIS EXISTS - interp.gamma is the current executable Delta semantics, but
# its ADT/match/recursion evaluation needs one independent discriminator. This is that
# implementation. delta-diamond-py.sh runs random Delta programs through BOTH interp.gamma and delta_ref.py
# and asserts they agree. UNTRUSTED and checked, like the other *_ref tools; the runtime never runs it.
#
# Delta:  E := INT | (+ E E)|(- E E)|(* E E)|(/ E E)|(% E E)|(eq E E)|(lt E E) | (if E E E) | (let x E E)
