...
- **BOOTSTRAP-AUTHORING-READABILITY.** Improve the implementation-to-expressiveness
  ...
  [runtime source composition](#bootstrap-runtime-source-composition) below owns
  the internal artifact work. Neither comparison needs an owner-approval gate.
...
- **BOOTSTRAP-RUNTIME-SOURCE.** Replace the Delta compiler's
  [encoded runtime emission](bootstrap/3_delta/implementation/emission/bytes.gamma)
  with ordinary, independently readable Gamma source for its byte runtime and
  `ConformanceBytesV1` adapter. This is internal bootstrap implementation work,
  not an Omega language, module-system, or product-artifact decision. Compare
  source-owned copying with explicitly declared byte-only materialization;
  account for evaluator, compiler, tooling, and proof costs together.
  The earlier diagnostic at `355ea00d55` on macOS used a 73-line, 1,957-byte
  runtime module and preserved twelve application observations, including
  empty/NUL/high-byte data, concatenation, indexing, and exact traps. It left
  the generated main adapter unchanged and bypassed production composition;
  that result alone does not establish closed custody or output accounting.
  Acceptance: author runtime code once, bind the exact ordered support members
  into the compiler artifact, and produce self-contained source receipts through
  a generic mechanism. Count every support byte and generated byte before
  publication, including exact-limit and adjacent-refusal controls. Reject
  missing/corrupt members and preserve atomic failure. No host semantic parser,
  output patcher, filename discovery, ambient file access, or unbound dependency
  may choose program meaning. Migrate the
  [composed-artifact contract](bootstrap/2_gamma/COMPOSED_ARTIFACT.md),
  [Delta receipt contract](bootstrap/3_delta/LANGUAGE.md), and their consumers
  with the implementation and evidence before selecting it. Preserve full
  runtime conformance and required proofs. Contract reconciliation is engineering
  work, not a request for permission; escalate only an actual proposed change to
  Omega semantics or weakening of trust or required assurances before relying
  on it.
