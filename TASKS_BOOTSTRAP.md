...
- [ ] **BUILD-EPSILON-COMPILER.** Implement
  ...
  - [x] **IMPLEMENTATION — EPSILON TRANSITION SUBJECT/PATTERN FACTS.** Classify
    ...
    start. Resolve every case pattern independently through the formed catalog;
    retain exact owner/case custody before payload arity and subject
    compatibility; and grant each syntactic D24 binder its positional payload
    type/place only after both joins complete. Scalar selectors retain semantic
  ...
  - [ ] **IMPLEMENTATION — D36 EPSILON CALLABLE SHAPE.** Enforce D36 without
    ...
    source; D51 supersedes its case/machine census, receiverless-qualified
    path, and special `self` carrier, whose removal is tracked separately.
    ...
    - [ ] Complete explicit call-head, postfix-statement, and transition-
      ...
      - [x] Complete settled qualified call-head and discarded postfix-
        ...
        authored call suffix, while a completed constructor or machine result
        followed by another suffix remains an ordinary noncallable base.
        Postfix use carries its outer diagnostic anchor through grouping,
        selects an unqualified machine before same-spelled local fallback, and
        admits only exact machine custody with authored application syntax.
        ...
        dependent failure. Resultless arguments remain pending D52's exact
        sibling candidate rather than acquiring a second mismatch here.
        Value/resultless results may
      ...
      - [ ] **IMPLEMENTATION — D51 EPSILON RECEIVER NORMALIZATION.** Require
        ...
        census; allow a case and receiver method to share a spelling because
        constructor, receiver, and free-machine syntax select disjoint
  ...
  - [ ] **IMPLEMENTATION — D37 EPSILON PREMISE-DAG COMPOSITION.** Retain every
    ...
    - [x] Generalize the complete-result value-use branch across scalar
      ...
      anchored argument-terminal branch.
      Unary/binary type joins now wait for every `i32` value fact while
      independently retaining resultless and exact-call `never` failures.
      Every authored application argument independently retains its explicitly
      anchored `never` failure before callee resolution/admission and even when
      arity fails. Let initialization and `assert` compare
      only complete values at the initializer/asserted-expression anchor;
      assignment independently checks the left value/place category and right
      value category, then compares the right value with the retained storage
    ...
    - [x] Thread each enclosing machine's exact optional return type through
      ...
      This completed behavior describes the current source only:
    ...
    - [ ] **IMPLEMENTATION — D52 RESULTLESS ARGUMENT ANCHOR.** In the existing
      ...
      argument contribute `TypeMismatch` at the authored argument expression's
      first byte, including outer grouping. Keep grouped `never` anchored at
      its exact call head. Derive the argument relation independently of the
      enclosing callee and arity, then let the minimum-coordinate accumulator
    ...
    - [ ] **IMPLEMENTATION — D53 LOCAL EPSILON BLOCK EXITS.** Remove the
      ...
      a reachability/fixed-point pass. Check unused states under the enclosing
      machine category. Map a resultless machine continuation to falloff, a
      `never` continuation to no normal return, and a value continuation to
      `TypeMismatch` at its start. Diagnose incompatible falloff at the exact
      closing brace. Replace the current first-following-statement rule with
      the terminating-delimiter rule for the first later executable construct
...
- [ ] **OWN-OMEGA-D.** Author one exact package-resolved closure `D` at
  ...
  - [ ] **IMPLEMENTATION-INCOMPLETE — D25/D59 OMEGA-COMPILER-REQUEST-WIRE-V1.**
    ...
    exact scalar profile provisions; apply D59's framing-before-capacity order,
    exact subject commitment, shared diagnostic selection, and `INT32_MAX`
    publication normalization. Neither compiler may use the Rust request
