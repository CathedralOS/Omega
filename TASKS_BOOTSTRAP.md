...
- [x] Establish the canonical owner layout for every implementation that
  ...
  - [ ] **DESIGN-BLOCKED — OWNER Q9.** Materialize the Gamma compiler source, tape,
    and adjacent validation in `source/gamma/compiler/`; section 3 owns the
    implementation.
  - [ ] **DESIGN-BLOCKED — OWNER Q8/Q9.** Materialize the Delta compiler source,
    tape, and adjacent validation in `source/delta/compiler/`; section 4 owns
    the implementation.
  - [ ] **DESIGN-BLOCKED — OWNER Q8.** Author
  ...
  - [ ] **DEPENDENCY-BLOCKED — OWNER Q9, missing Gamma/Delta compilers, and missing
...
- [ ] **BLOCKED — OWNER Q11 (Beta compiler outcome): ADMIT-ALPHA-BETA-COMPILER.** Audit the canonical
  ...
  - [ ] **DESIGN-BLOCKED — OWNER Q10:** Enforce Beta definite initialization across
  ...
  - [x] Bind the compiler's practical fixed resource profile and exercise exact
    ...
    source capacities.
  - [ ] **OWNER-BLOCKED — OWNER Q11:** Project malformed source and each private
...
- [ ] **OWNER-BLOCKED — OWNER Q13.** Close exact
  ...
  - [ ] **OWNER-BLOCKED — OWNER Q13:** Turn the ground assembly judgment into a derivation certificate
    ...
    - OWNER Q13 must choose between bounded subject-bound chunk equalities whose exact
...
- [ ] **DESIGN-BLOCKED — OWNER Q9: BUILD-GAMMA-COMPILER.** Define the complete Gamma source contract, then
  ...
  compilation. OWNER Q9 must first select one typed executable grammar, entry/stream
...
- [x] Keep `interp.beta` and `typeck.beta` only as reusable compiler components
  ...
  explicit Q9 absorption/deletion conditions; neither is accepted as a
  ...
  differential cases. Q9's compiler task owns the later absorb-or-delete step.
  - [x] Delete the interpreter's dead environment lookup and the
    ...
    interpreter-first claims to classify both executables as pre-Q9 oracles.
...
- [ ] **DEPENDENCY-BLOCKED — OWNER Q9 and missing `gamma_compiler.beta`.** Check the
...
- [ ] **DESIGN-BLOCKED — OWNER Q8: FREEZE-DELTA-V1.** Finish one self-contained Delta grammar, static
  ...
  language; it does not inherit Omega meaning merely by sharing spelling. Q8
...
- [ ] **DESIGN-BLOCKED — OWNER Q8/Q9: BUILD-DELTA-COMPILER.** Implement
  ...
  participates.
- [ ] **DESIGN-BLOCKED — OWNER Q8.** Derive compact positive, negative, trap, and
  ...
  translator.
- [ ] **DEPENDENCY-BLOCKED — OWNER Q9 and missing `delta_compiler.gamma`.** Run that
  ...
  - [x] Delete the unrun 43-file pre-migration Delta corpus rather than
    ...
    After OWNER Q8 freezes Delta v1, derive a compact positive/negative suite from
    the frozen contract and run it through the actual Gamma-written compiler.
- [ ] **DEPENDENCY-BLOCKED — OWNER Q8/Q9 and missing Gamma/Delta compilers.** Check
...
- [ ] **DESIGN-BLOCKED — OWNER Q8: OWN-OMEGA-D.** Author one exact package-resolved closure `D` at
  ...
  of Q8. The deleted prototype remains available in Git for selectively
  re-deriving an isolated algorithm, but it cannot be restored or copied as a
  compiler-shaped starting point.
- [ ] **DEPENDENCY-BLOCKED — OWNER Q8 and missing `D`.** Make `D` implement the
...
- [ ] **DEPENDENCY-BLOCKED — OWNER Q14 and current ranked-runtime acceptance.**
