...
- [x] Establish the canonical owner layout for every implementation that
  ...
  - [ ] **DESIGN-BLOCKED — Q12.** Materialize the Gamma compiler source, tape,
    and adjacent validation in `source/gamma/compiler/`; section 3 owns the
    implementation.
  - [ ] **DESIGN-BLOCKED — Q11/Q12.** Materialize the Delta compiler source,
    tape, and adjacent validation in `source/delta/compiler/`; section 4 owns
    the implementation.
  - [ ] **DESIGN-BLOCKED — Q11.** Author
  ...
  - [ ] **DEPENDENCY-BLOCKED — Q12, missing Gamma/Delta compilers, and missing
...
- [ ] **BLOCKED — DESIGN Q13 / OWNER Q14: ADMIT-ALPHA-BETA-COMPILER.** Audit the canonical
  ...
  - [ ] **DESIGN-BLOCKED — Q13:** Enforce Beta definite initialization across
  ...
  - [x] Bind the compiler's practical fixed resource profile and exercise exact
    ...
    source capacities.
  - [ ] **OWNER-BLOCKED — Q14:** Project malformed source and each private
...
- [ ] **OWNER-BLOCKED — Q16.** Close exact
  ...
  - [ ] **OWNER-BLOCKED — Q16:** Turn the ground assembly judgment into a derivation certificate
    ...
    - Q16 must choose between bounded subject-bound chunk equalities whose exact
...
- [ ] **DESIGN-BLOCKED — Q12: BUILD-GAMMA-COMPILER.** Define the complete Gamma source contract, then
  ...
  compilation. Q12 must first select one typed executable grammar, entry/stream
...
- [x] Keep `interp.beta` and `typeck.beta` only as reusable compiler components
  ...
  explicit Q12 absorption/deletion conditions; neither is accepted as a
  ...
  differential cases. Q12's compiler task owns the later absorb-or-delete step.
  - [x] Delete the interpreter's dead environment lookup and the
    ...
    interpreter-first claims to classify both executables as pre-Q12 oracles.
...
- [ ] **DEPENDENCY-BLOCKED — Q12 and missing `gamma_compiler.beta`.** Check the
...
- [ ] **DESIGN-BLOCKED — Q11: FREEZE-DELTA-V1.** Finish one self-contained Delta grammar, static
  ...
  language; it does not inherit Omega meaning merely by sharing spelling. Q11
...
- [ ] **DESIGN-BLOCKED — Q11/Q12: BUILD-DELTA-COMPILER.** Implement
  ...
  participates.
- [ ] **DESIGN-BLOCKED — Q11.** Derive compact positive, negative, trap, and
  ...
  translator.
- [ ] **DEPENDENCY-BLOCKED — Q12 and missing `delta_compiler.gamma`.** Run that
  ...
  - [x] Delete the unrun 43-file pre-migration Delta corpus rather than
    ...
    After Q11 freezes Delta v1, derive a compact positive/negative suite from
    the frozen contract and run it through the actual Gamma-written compiler.
- [ ] **DEPENDENCY-BLOCKED — Q11/Q12 and missing Gamma/Delta compilers.** Check
...
- [ ] **DESIGN-BLOCKED — Q11: OWN-OMEGA-D.** Author one exact package-resolved closure `D` at
  ...
  of Q11. The deleted prototype remains available in Git for selectively
  re-deriving an isolated algorithm, but it cannot be restored or copied as a
  compiler-shaped starting point.
- [ ] **DEPENDENCY-BLOCKED — Q11 and missing `D`.** Make `D` implement the
...
- [ ] **DEPENDENCY-BLOCKED — Q17 and current ranked-runtime acceptance.**
