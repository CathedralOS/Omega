...
- **Toward Omega's proof surface — STARTED.** rungs/omega.md: Omega = the lower rungs PLUS contracts +
  ...
  accepts every proof emitted). `prover-test.sh` (814 ok): propositional tautologies (or-comm, distribution,
  ...
  sum-witness machinery, and a≤b ⊢ a*c≤b*c (MULT-MONO) is a mult-scaling witness source factoring through the
  banked right-distributivity. **The bridge is now WIRED, not just demonstrated:** the contract compiler
