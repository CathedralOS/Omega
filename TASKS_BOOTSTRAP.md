...
11. **Epsilon contracts — the verification-compiler, BUILT.** What the frontier once called
    ...
    library — now GENERATED by the prover, `prover-contract-lib.py`) for obligations that hold only up to
    a banked theorem (add-zero, commutativity, le-trans). Conservative (never a false cert) and cross-checked against
...
- **Toward Omega's proof surface — STARTED.** rungs/omega.md: Omega = the lower rungs PLUS contracts +
  ...
  P(2)), equality REWRITING via eqelim/transport -- one rule (abstract a side into a motive, prove the
  motive on the other side, both orientations through a derived symmetric proof) yields symmetry, transitivity,
  congruence and transport -- AND INEQUALITY (`Lt`/`Le` desugared at parse to a Peano existential, no kernel
  `<`: a≤b := ∃k. a+k=b, a<b := ∃k. a+(s k)=b), so concrete bounds (1<3, 0<5) and the direct weakening x<y ⊢
  x≤y discharge through the existing ∃ + equality machinery. (Inequality goals carry FREE individual vars,
  ...
  accepts every proof emitted). `prover-test.sh` (819 ok): propositional tautologies (or-comm, distribution,
  ...
  and MULT-ASSOC ∀a b c. (a*b)*c=a*(b*c) — a banked contract lemma (discharge.rs id 5), via natind-first on
  ...
  sum-witness machinery; a≤b ⊢ a*c≤b*c (MULT-MONO) and, with a positivity guard, 0<c ∧ a<b ⊢ a*c<b*c (STRICT
  mult-mono) are mult-scaling witness sources factoring through the banked right-distributivity. **The bridge is now WIRED, not just demonstrated:** the contract compiler
  (`discharge.rs`) cites a 7-lemma library GENERATED by the prover (`prover-contract-lib.py`, one self-contained
