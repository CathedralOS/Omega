...
- [x] Establish the canonical owner layout for every implementation that
  ...
  - [ ] **DESIGN-BLOCKED — Q10.** Materialize the Gamma compiler source, tape,
    and adjacent validation in `source/gamma/compiler/`; section 3 owns the
    implementation.
  - [ ] **DESIGN-BLOCKED — Q9/Q10.** Materialize the Delta compiler source,
    tape, and adjacent validation in `source/delta/compiler/`; section 4 owns
    the implementation.
  - [ ] **DESIGN-BLOCKED — Q9.** Author
  ...
  - [ ] **DEPENDENCY-BLOCKED — Q10, missing Gamma/Delta compilers, and missing
...
- [ ] **BLOCKED — DESIGN Q11 / OWNER Q12: ADMIT-ALPHA-BETA-COMPILER.** Audit the canonical
  ...
  - [ ] **DESIGN-BLOCKED — Q11:** Enforce Beta definite initialization across
  ...
  - [x] Bind the compiler's practical fixed resource profile and exercise exact
    ...
    source capacities.
  - [ ] **OWNER-BLOCKED — Q12:** Project malformed source and each private
...
- [ ] **OWNER-BLOCKED — Q14.** Close exact
  ...
  - [ ] **OWNER-BLOCKED — Q14:** Turn the ground assembly judgment into a derivation certificate
    ...
    - Q14 must choose between bounded subject-bound chunk equalities whose exact
...
- [ ] **DESIGN-BLOCKED — Q10: BUILD-GAMMA-COMPILER.** Define the complete Gamma source contract, then
  ...
  compilation. Q10 must first select one typed executable grammar, entry/stream
...
- [x] Keep `interp.beta` and `typeck.beta` only as reusable compiler components
  ...
  explicit Q10 absorption/deletion conditions; neither is accepted as a
  ...
  differential cases. Q10's compiler task owns the later absorb-or-delete step.
  - [x] Delete the interpreter's dead environment lookup and the
    ...
    interpreter-first claims to classify both executables as pre-Q10 oracles.
...
- [ ] **DEPENDENCY-BLOCKED — Q10 and missing `gamma_compiler.beta`.** Check the
...
- [ ] **DESIGN-BLOCKED — Q9: FREEZE-DELTA-V1.** Finish one self-contained Delta grammar, static
  ...
  language; it does not inherit Omega meaning merely by sharing spelling. Q9
...
- [ ] **DESIGN-BLOCKED — Q9/Q10: BUILD-DELTA-COMPILER.** Implement
  ...
  participates.
- [ ] **DESIGN-BLOCKED — Q9.** Derive compact positive, negative, trap, and
  ...
  translator.
- [ ] **DEPENDENCY-BLOCKED — Q10 and missing `delta_compiler.gamma`.** Run that
  ...
  - [x] Delete the unrun 43-file pre-migration Delta corpus rather than
    ...
    After Q9 freezes Delta v1, derive a compact positive/negative suite from
    the frozen contract and run it through the actual Gamma-written compiler.
- [ ] **DEPENDENCY-BLOCKED — Q9/Q10 and missing Gamma/Delta compilers.** Check
...
- [ ] **DESIGN-BLOCKED — Q9: OWN-OMEGA-D.** Author one exact package-resolved closure `D` at
  ...
  of Q9. The deleted prototype remains available in Git for selectively
  re-deriving an isolated algorithm, but it cannot be restored or copied as a
  compiler-shaped starting point.
- [ ] **DEPENDENCY-BLOCKED — Q9 and missing `D`.** Make `D` implement the
...
- [ ] **DEPENDENCY-BLOCKED — Q15 and current ranked-runtime acceptance.**
