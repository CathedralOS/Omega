...
fn psi_optimization_execution_record_rejects_every_one_field_substitution() {
    ...
    assert_eq!(
        ...
        Err(PsiOptimizationExecutionRecordDecodeError::Selection(
            terminal_codec::optimization::PsiOptimizationSelectionDecodeError::Truncated
        ))
    );
    let mut mutated = encoded.clone();
    ...
    for (name, roster, expected) in [
        (
            ...
            terminal_codec::optimization::PsiOptimizationSelectionDecodeError::WrongMagic,
        ),
        (
            "the selections version",
            inner(&|inner| inner[8..12].copy_from_slice(&u32::MAX.to_le_bytes())),
            terminal_codec::optimization::PsiOptimizationSelectionDecodeError::UnsupportedVersion(
                u32::MAX,
            ),
        ),
        (
            ...
            terminal_codec::optimization::PsiOptimizationSelectionDecodeError::UnknownTag(0xFE),
        ),
        (
            "a duplicated pass",
            inner(&|inner| inner[17] = inner[16]),
            terminal_codec::optimization::PsiOptimizationSelectionDecodeError::Duplicate(
                PsiOptimization::ControlFlowCleanup,
            ),
        ),
        (
            ...
            terminal_codec::optimization::PsiOptimizationSelectionDecodeError::NonCanonicalOrder,
        ),
    ] {
        assert_eq!(
    ...
    assert_eq!(
        ...
        Err(PsiOptimizationExecutionRecordDecodeError::Selection(
            terminal_codec::optimization::PsiOptimizationSelectionDecodeError::TrailingBytes
        ))
    );
