...
#[test]
fn exact_subtract_definition_bound_owns_its_appended_rule_tag() {
    let truth = || ProofNode {
        conclusion: Proposition::Truth,
        rule: ProofRule::Primitive(PrimitiveJudgment::Truth),
    };
    ...
    let bytes = encode_proof_bundle(&bundle).expect("exact-subtract definition proof encodes");
    assert_eq!(bytes[34], 26, "appended exact-subtract definition rule tag");
    assert_eq!(decode_proof_bundle(&bytes), Ok(bundle));
}
