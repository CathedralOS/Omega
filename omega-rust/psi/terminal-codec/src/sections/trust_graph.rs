...
use terminal_semantics::operation_semantic_row;
#[cfg(test)]
use terminal_semantics::{OperationSemanticCustody, OperationSemanticRow, OperationSemanticTag};
...
const EVIDENCE_PROVENANCE_SOURCE: &[u8] =
    include_bytes!("../../../terminal-verifier/src/verification/evidence_provenance.rs");
const VERIFIER_CALL_COMPOSITION_SOURCE: &[u8] =
    include_bytes!("../../../terminal-verifier/src/verification/call_composition.rs");
...
const PROOF_ADMISSION_INTEGER_AFFINE_SOURCE: &[u8] =
    include_bytes!("../../../proof-admission/src/integer_rules/integer_affine.rs");
const PROOF_ADMISSION_INTEGER_CAST_SOURCE: &[u8] =
    include_bytes!("../../../proof-admission/src/integer_rules/integer_cast.rs");
const PROOF_ADMISSION_INTEGER_FORBIDDEN_ROOT_SOURCE: &[u8] =
    include_bytes!("../../../proof-admission/src/integer_rules/integer_forbidden_root.rs");
...
const PROPOSITION_SOURCE: &[u8] =
    include_bytes!("../../../foundation/semantic-vocabulary/src/proposition.rs");
const PROPOSITION_VALUE_IDS_SOURCE: &[u8] =
    include_bytes!("../../../foundation/semantic-vocabulary/src/proposition/value_ids.rs");
...
pub fn current_rust_operation_semantics_trust_identity(
    operation: &terminal_psi::OperationKind,
) -> &'static str {
...
mod tests {
    ...
    fn representation_source_closure_retains_every_concept_file_exactly() {
        assert_source_closure_covers_tree(
            &Path::new(env!("CARGO_MANIFEST_DIR")).join("../terminal-psi/src"),
    ...
    fn current_graph_is_closed_canonical_and_explicitly_not_fully_derived() {
        ...
        assert_eq!(OperationSemanticRow::ALL.len(), 80);
        for (tag, identity) in [
            (
        ...
        let store = OperationSemanticRow::ALL
            ...
            .expect("write-only primitive store row");
        assert_eq!(
            store.identity(),
            "schema:operation:write-only-primitive-store",
        );
        assert!(graph.nodes().iter().any(|node| {
            node.identity() == store.identity()
                && node.kind() == TrustDependencyKind::StructuralEffectSchema
        }));
        assert_eq!(
    ...
    fn graph_identity_binds_exact_dependency_edges_and_source_bytes() {
        ...
        let without_canonical_goal_source = TrustDependencyNode::new(
            ...
            &[
                ...
                ("terminal-semantics/lib.rs", TERMINAL_SEMANTICS_SOURCE),
                (
