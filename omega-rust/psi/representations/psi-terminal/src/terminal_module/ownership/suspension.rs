...
fn hash_carry_policy(hash: &mut Sha256, policy: CarryPolicy) {
    hash.update([
        u8::from(policy.suspension == psi_language_semantics::CarrySuspension::Allowed),
        u8::from(policy.cpu == psi_language_semantics::CarryCpu::Any),
        u8::from(policy.host_thread == psi_language_semantics::CarryHostThread::Any),
        u8::from(policy.address == psi_language_semantics::CarryAddress::Movable),
    ]);
}
