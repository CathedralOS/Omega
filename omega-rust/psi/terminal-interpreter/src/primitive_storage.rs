...
impl LocalStructuralIdentities {
    ...
    pub(crate) fn new(module: &TerminalModule, arguments: &[TerminalStructuralValue]) -> Self {
        Self {
            reserved: module
                .machines
                .iter()
    ...
    pub(crate) fn allocate(&mut self) -> Result<u64, TerminalInterpretError> {
    ...
    pub(crate) fn commit_cursor(&mut self, cursor: Option<u64>) {
        self.next = cursor;
    }
...
impl TerminalExecution {
    ...
    pub(crate) fn execute_primitive_establishment(
        &mut self,
        operation: &Operation,
        value: ValueId,
    ) -> Result<(), TerminalInterpretError> {
    ...
    pub(crate) fn execute_primitive_read(
        &mut self,
        operation: &Operation,
        source: PlaceId,
        path: &[StructuralPathSegment],
    ...
    pub(crate) fn execute_primitive_store(
        &mut self,
        operation: &Operation,
        destination: PlaceId,
        value: ValueId,
    ...
    pub(crate) fn retire_plain_locals(&mut self) {
...
mod tests {
    use crate::errors::TerminalInterpretError;
    use crate::primitive_storage::LocalStructuralIdentities;
