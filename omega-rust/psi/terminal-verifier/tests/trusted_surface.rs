...
fn repository_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(3)
        .expect("crate lives three directories below the repository root")
        .to_path_buf()
}
...
fn scratch_tree() -> (tempfile_guard::Guard, PathBuf) {
    ...
    let source = repository_root().join("omega-rust/psi/proof-admission/src");
    let destination = root.join("omega-rust/psi/proof-admission/src");
...
fn source_coverage_fires_on_an_unclaimed_source_file() {
    ...
    let rogue = root.join("omega-rust/psi/proof-admission/src/rogue_rule.rs");
    ...
    assert!(
        failures.iter().any(|failure| matches!(
            ...
            if path == "omega-rust/psi/proof-admission/src/rogue_rule.rs"
        )),
...
fn source_coverage_fires_on_a_changed_implementation() {
    let (_guard, root) = scratch_tree();
    let kernel = root.join("omega-rust/psi/proof-admission/src/kernel.rs");
    ...
    let mismatch = failures.iter().find(|failure| {
        matches!(
            ...
            if *path == "omega-rust/psi/proof-admission/src/kernel.rs"
        )
    });
