...
use crate::validation::{BTreeMap, ModuleError, PlaceId, StructuralDomainId, StructuralTypeId};
pub(in crate::validation) fn validate_projected_qualification_roster(
    place: PlaceId,
    ...
    for qualification in qualifications {
        let Some(projected_type) = (!qualification.path.is_empty())
            .then(|| {
                crate::validation::foundation::resolve_structural_path_in_types(
                    types,
                    root_type,
                    &qualification.path,
                )
