...
pub(super) fn validate_suspension_call_plans(module: &TerminalModule) -> Result<(), ModuleError> {
        ...
        .zip(&module.suspension_call_plans)
    {
        if site.operation != plan.operation
            ...
            || site.frontier_commitment != terminal_psi::suspension_frontier_commitment(plan)
        {
    ...
    for plan in &module.suspension_call_plans {
        ...
        if plan.effective != expected_policy
            || plan.effective.suspension != language_semantics::CarrySuspension::Allowed
            || plan.live_values.iter().any(|live| {
                live.effective.suspension != language_semantics::CarrySuspension::Allowed
            })
        {
            return invalid(
                Some(plan.operation),
        ...
        for live in &plan.live_values {
            ...
            match (&live.place, live.value_type) {
                (
                    ...
                    terminal_psi::TerminalSuspensionValueType::Scalar(expected),
                ) => {
                ...
                (
                    terminal_psi::TerminalSuspensionPlace::Structural { place, path },
                    terminal_psi::TerminalSuspensionValueType::Structural(expected),
                ) => {
                    let Some(root_type) =
...
) -> Option<(&TerminalMachine, &terminal_psi::Block, usize)> {
...
fn terminal_call_target(
    kind: &OperationKind,
) -> Option<terminal_psi::TerminalSuspensionCallTarget> {
    match kind {
        ...
        | OperationKind::CallStructuralWithScalarArguments { callee, .. } => {
            Some(terminal_psi::TerminalSuspensionCallTarget::Machine(*callee))
        }
        OperationKind::BoundaryCall { boundary, .. } => Some(
            terminal_psi::TerminalSuspensionCallTarget::Boundary(*boundary),
        ),
        ...
        | OperationKind::CallDynamicUnit {
            ...
            terminal_psi::TerminalSuspensionCallTarget::DynamicDescriptor {
        ...
        | OperationKind::CallDynamicParameterUnit {
            ...
            terminal_psi::TerminalSuspensionCallTarget::DynamicParameter {
...
fn scalar_storage_matches(
    ...
    kind: &OperationKind,
    value: ValueId,
    storage: terminal_psi::TerminalSuspensionStorage,
) -> bool {
    match storage {
        terminal_psi::TerminalSuspensionStorage::Persistent => false,
        terminal_psi::TerminalSuspensionStorage::Parameter => machine
            .parameters
            .iter()
            .chain(block.parameters.iter())
            .any(|parameter| parameter.id == value),
        terminal_psi::TerminalSuspensionStorage::Local => block.operations[..operation_index]
            .iter()
...
fn structural_storage_matches(
    ...
    match storage {
        terminal_psi::TerminalSuspensionStorage::Persistent => machine
        ...
        terminal_psi::TerminalSuspensionStorage::CallArgument => structural_call_arguments(kind)
            .iter()
