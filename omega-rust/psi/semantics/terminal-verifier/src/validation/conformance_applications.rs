...
fn callable_result_matches_machine(
    callable: &terminal_psi::ClosedConformanceRealizationCallable,
    ...
    for application in &module.closed_conformance_applications {
        ...
        let dynamic_argument_consumed = module.dynamic_dispatch.arguments.iter().any(|argument| {
            let selection_ordinal = match argument.source {
                terminal_psi::TerminalDynamicDescriptorSource::Selection { ordinal } => ordinal,
                ...
                terminal_psi::TerminalDynamicDescriptorSource::Parameter { .. } => return false,
            };
        ...
        match (
            ...
            ([callable], [row]) => {
                ...
                let dynamic_dispatch_consumed = module
                    ...
                    || module.dynamic_dispatch.stored_dispatches.iter().any(|dispatch| {
                        dispatch.owner == application.owner
                            && dispatch.declaring_trait_identity == row.declaring_trait_identity
                            && dispatch.public_requirement_identity
                                == row.public_requirement_identity
                            ...
                            && dispatch.realization_callable_identity
                                == callable.source_callable_identity
                            && dispatch.realization == callable.machine
                            && module.dynamic_dispatch.stored_descriptors.iter().any(
                                |descriptor| {
                                    descriptor.owner == dispatch.owner
                                        && descriptor.ordinal == dispatch.descriptor_ordinal
                                        && module.dynamic_dispatch.selections.iter().any(
