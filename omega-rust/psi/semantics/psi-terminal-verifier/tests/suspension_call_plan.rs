...
use psi_terminal_verifier::{ModuleError, SuspensionCallPlanError, validate_module};
...
fn fixture() -> TerminalModule {
    let boolean = psi_core::ScalarType::Boolean;
    ...
    let mut module = TerminalModule {
        ...
        suspension_call_plans: vec![TerminalSuspensionCallPlan {
            operation: id(2),
            crossing: id(41),
            target: psi_terminal::TerminalSuspensionCallTarget::Machine(id(2)),
            effective: cpu_local(),
            live_value_count: 3,
            live_values: vec![
                live(
    ...
    module
        ...
        .push(psi_terminal::TerminalSuspensionCallSite {
...
fn refresh_site(module: &mut TerminalModule) {
    ...
    module.suspension_call_sites[0] = psi_terminal::TerminalSuspensionCallSite {
        operation: plan.operation,
        crossing: plan.crossing,
        target: plan.target,
        frontier_commitment: psi_terminal::suspension_frontier_commitment(plan),
...
fn suspension_call_plan_mutations_fail_independently() {
    ...
    wrong_type.suspension_call_plans[0].live_values[0].value_type =
        TerminalSuspensionValueType::Scalar(psi_core::ScalarType::Integer(
...
fn suspension_call_plan_rejoins_operation_target_after_site_validation() {
    ...
    module.suspension_call_plans[0].target =
        psi_terminal::TerminalSuspensionCallTarget::Machine(id(1));
    refresh_site(&mut module);
