...
fn canonical_terminal_bytes_identity() -> &'static str {
    "root:canonical-terminal-bytes-format-94-vocabulary-105"
}
    ...
    vec![
        ...
        TrustDependencyNode::new(
            ...
            &[
                ...
                (
                    "proof-admission/closed_integer.rs",
                    PROOF_ADMISSION_CLOSED_INTEGER_SOURCE,
                ),
                (
    ...
    OperationSemanticRow::ALL
        ...
        .map(|row| {
            ...
            if call_composition.is_some() {
                ...
                exact_sources.push((
                    "terminal-semantics/call_composition/fixed_byte_view.rs",
                    TERMINAL_FIXED_BYTE_VIEW_SOURCE,
                ));
                exact_sources.push((
                    "terminal-verifier/verification/call_composition.rs",
                    VERIFIER_CALL_COMPOSITION_SOURCE,
