...
fn suspension_call_plan_round_trips_canonically_and_rejects_prior_format() {
    ...
    assert_eq!(&bytes[8..10], 84_u16.to_le_bytes());
...
fn current_vocabulary_has_one_stable_canonical_encoding_and_identity() {
    ...
    assert_eq!(
        ...
        "2140959c83c22f06618c8273df0d4db6648c3334ef647c3472b11d3ddfe27dce"
    );
    assert_eq!(
...
fn decoder_rejects_noncanonical_or_ambiguous_bytes() {
    ...
    future_format[8..10].copy_from_slice(&85_u16.to_le_bytes());
    assert_eq!(
        decode_module(&future_format),
        Err(CodecError::UnsupportedFormatMarker(85))
    );
