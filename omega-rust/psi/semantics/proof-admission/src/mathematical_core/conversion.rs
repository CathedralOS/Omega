//! Typed conversion for the Π fragment: β weak-head normalization under a
//! step ceiling, and definitional proof irrelevance gated on the *shared
//! type's* sort — never on the shape of either side. No eta in this slice.
...
/// β-only weak-head normalization. Each reduction consumes one step.
pub fn weak_head_normalize(
    ...
    loop {
        let Term::Apply { function, argument } = arena.get(current) else {
            return Ok(current);
        };
