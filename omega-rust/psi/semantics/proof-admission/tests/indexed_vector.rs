...
fn induction_computes_on_cons_with_a_neutral_tail() {
    ...
    // computation across the packed `IW` pair costs 702,825 budgeted
    // steps over this description.
    assert_eq!(total - budget.remaining(), 702_825);
}
...
fn induction_computes_on_nil_with_a_vacuous_child_function() {
    ...
    // child position is dead — costs 708,353 budgeted steps.
    assert_eq!(total - budget.remaining(), 708_353);
}
...
fn a_vector_certificate_verifies_with_exact_closure_and_bounded_cost() {
    ...
    assert_eq!(arena.len(), 936_112);
    assert_eq!(spent, 11_593);
}
