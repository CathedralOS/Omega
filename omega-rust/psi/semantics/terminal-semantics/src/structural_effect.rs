...
impl StructuralEffectSemanticRow {
    pub const ALL: [Self; 30] = [
        ...
        Self {
            tag: OperationSemanticTag::StructuralByteSequenceFieldRead,
            schema: structural_effect_leaf(
        ...
        Self {
            tag: OperationSemanticTag::IndexedPrimitiveRead,
            schema: structural_effect_leaf(
                StructuralEffectResultShape::Scalar,
                StructuralEffectCustody::ExactReadablePrimitiveRoot,
                StructuralEffectAction::ReadIndexedPrimitive,
...
const fn is_structural_effect_tag(tag: OperationSemanticTag) -> bool {
    matches!(
        ...
        OperationSemanticTag::EstablishPrimitiveLocal
            ...
            | OperationSemanticTag::IndexedPrimitiveRead
...
pub enum StructuralEffectObservation {
    ...
    /// Read one scalar element that `index` selects within the fixed array
    /// the path resolves to. The verifier publishes `index < declared extent`
    /// from the module's array shape; the element itself is not a named leaf,
    /// so no element-value equation is retained.
    ...
    /// Read one byte from the exact field's live storage. The result is the
    /// read value; index < length is proven against the field's own length
    /// observation and provenance is artifact validation, not an equation.
    ByteSequenceFieldByteRead {
        source: PlaceId,
        path: Vec<terminal_psi::StructuralPathSegment>,
        field: StructuralFieldId,
        index: ValueId,
...
impl StructuralEffectObservation {
    ...
    pub fn canonical_obligation(&self) -> Option<(ObligationId, Proposition)> {
        ...
        match self {
            ...
            | Self::IndexedPrimitiveRead { .. } => None,
            ...
            | Self::ByteSequenceFieldByteRead {
                index,
                length,
                obligation,
                ..
...
fn validate_structural_effect_schema(
    ...
    let action_tag = match schema.action {
        ...
        StructuralEffectAction::ReadIndexedPrimitive => OperationSemanticTag::IndexedPrimitiveRead,
        StructuralEffectAction::StoreScalarField => {
            OperationSemanticTag::StructuralScalarFieldStore
        }
        StructuralEffectAction::EstablishByteSequencePlace => {
                ...
                | StructuralEffectAction::ReadByteSequenceFieldByte => {
                    schema.result
                        == match schema.action {
                            ...
                            _ => StructuralEffectResultShape::Unit,
                        }
                        && schema.custody
                            == StructuralEffectCustody::ExactStructuralByteSequenceField
                        && schema.external_effect == StructuralEffectExternalEffect::None
...
pub fn structural_effect_leaf_observation_in(
    ...
    let observation = match (schema.action, &operation.kind) {
        ...
        ) => StructuralEffectObservation::ByteSequenceFieldByteRead {
            source: *source,
            path: path.clone(),
            field: *field,
            index: *index,
        ...
        (
            ...
            StructuralEffectObservation::IndexedPrimitiveRead {
                source: *source,
                path: path.clone(),
                index: *index,
                obligation: *obligation,
...
mod tests {
    ...
    fn inventory_is_exact_unique_and_keeps_axes_separate() {
        assert_eq!(StructuralEffectSemanticRow::ALL.len(), 30);
        assert_eq!(
            StructuralEffectSemanticRow::ALL
                .iter()
                .map(|row| row.tag())
