...
fn validate_expression_call_bounds(
    ...
    if let Some(operator) = typed_trees::operator::resolve_named_expression_call(program, call) {
        ...
        if call.receiver.is_valid() && parameters.len() == explicit_arguments.len() + 1 {
            let receiver_type = crate::value_custody::places::declared_place_type(
                program,
                current_machine,
                Some(current_state),
                call.receiver,
        ...
        let operand_types = arguments
            ...
            .collect::<Vec<_>>();
        match crate::declarations::operators::validate_named_operator_application(
            program,
            symbols,
            operator,
            &call.machine_arguments,
            ...
            Ok(Some(bindings)) if operator.is_boundary => {
                let application =
                    crate::declarations::operators::validated_boundary_operator_application(
                        crate::ValidatedBoundaryOperatorApplicationUseSite::Expression(expression),
                        operator,
                        bindings,
                    );
                crate::declarations::operators::retain_validated_boundary_operator_application(
            ...
            Ok(None) if operator.is_boundary && call.machine_arguments.is_empty() => {
                if let Some(application) =
                    crate::declarations::operators::validated_symbolic_boundary_operator_application(
                        program,
                        current_machine,
                        crate::ValidatedBoundaryOperatorApplicationUseSite::Expression(expression),
                        operator,
    ...
    let receiver_type_reference = crate::value_custody::places::declared_place_type(
        program,
        current_machine,
        Some(current_state),
        call.receiver,
    ...
    if let Some(receiver_type_reference) = receiver_type_reference
        && let Some(candidate) =
            crate::proof_contracts::quotients::legacy_attached_quotient_call_candidate(
                program,
                receiver_type_reference,
                call.target.as_str(),
            )
        ...
        && callee_machine.attached_data_symbol.is_valid()
    {
        ...
        if receiver_type_reference.is_none()
            ...
            && crate::value_custody::expression_types::value_concrete_data_symbol(
                program,
                current_machine,
                Some(current_state),
                *argument,
