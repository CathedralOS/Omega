...
fn collect_transition_target_expression_calls(
    ...
    match program.statement_table.transition_target(target) {
        typed_trees::statement::TransitionTargetNode::Named {
            ...
            let target = crate::declarations::transitions::named_transition_call_symbol(
                program,
                machine,
                path.symbol,
            );
...
fn collect_expression_calls(
    ...
    match program.expression_table.expression(expression) {
        ExpressionNode::Match(dispatch) => {
            ...
            {
                collect_expression_calls(program, child, statement_index, call_ordinal, calls);
            }
        }
...
fn signature_operational(
    ...
    for target in super::declared_signature_invocations(program, signature) {
        let service = match target {
            ...
            flow_effects::InvocationTarget::Service(symbol) => Some(symbol),
        };
...
fn retain_static_machine_bindings(
    ...
    let Some((machine, _)) =
        crate::declarations::transitions::resolved_transition_target_state(program, target)
    else {
        return result;
    };
    let parameters = program
