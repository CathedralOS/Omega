...
pub fn static_machine_parameter_contract_bytes(
    ...
    for (ordinal, parameter) in parameters.iter().enumerate() {
        ...
        for invocation in crate::machine_calls::effect_inference::declared_signature_invocations(
            program, signature,
        ) {
            let mut encoded = Vec::new();
            match invocation {
...
fn validate_call_selection(
    ...
    let selected_entry =
        crate::declarations::transitions::resolved_transition_target_state(program, *selected);
    let actual_entry =
