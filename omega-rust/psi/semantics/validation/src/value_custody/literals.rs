...
pub(crate) fn validate_suffix_landing(
    ...
    let Some(unwrapped) = crate::value_custody::places::unwrapped_type_reference(program, declared)
    else {
        return;
    };
...
pub(crate) fn validate_suffix_landings(program: &TypedTrees, diagnostics: &mut Vec<Diagnostic>) {
    for (_, node) in program.expression_table.expression_entries() {
        ...
        for field in program.expression_table.struct_fields(literal.fields) {
            let Some(field_type) =
                crate::value_custody::struct_literals::selected_construction_field_type(
                    program,
                    literal,
                    field.field_symbol,
                )
