...
fn normalize_projection_subject(
    ...
    let carrier =
        crate::value_custody::places::unwrapped_type_reference(context.program, final_type)
            .ok_or_else(|| {
                "the projected structural place has no resolved carrier type".to_owned()
            })?;
...
fn retain_payload_case_segments(program: &TypedTrees, segments: &mut Vec<ContentPlaceSegment>) {
    ...
    for segment in source {
        if let ContentPlaceSegment::Field(field) = &segment
            && let Some(variant_symbol) = facts::payload_variant_for_field(program, field.symbol)
            && !matches!(
                segments.last(),
...
fn structural_place_type(
    ...
    for segment in segments {
        let unwrapped = crate::value_custody::places::unwrapped_type_reference(program, current)
        ...
        match segment {
            ContentPlaceSegment::Case(case) => {
                ...
                let data =
                    crate::value_custody::places::data_definition_for_type(program, unwrapped)
                        .or_else(|| {
    ...
    match program.type_reference_table.type_reference(type_reference) {
        ...
        TypeReferenceNode::Constrained {
            ...
            program
                ...
                .iter()
                .any(|constraint| matches!(constraint, typed_trees::types::TypeConstraintNode::Domain(candidate) if candidate.semantic_id == domain))
