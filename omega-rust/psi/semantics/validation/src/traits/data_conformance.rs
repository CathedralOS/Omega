    ...
    for (index, conformance) in program.conformances().iter().enumerate() {
        ...
        for previous in &program.conformances()[..index] {
            ...
            match (&previous.subject, &conformance.subject) {
                (
                    typed_trees::trait_definition::ConformanceSubject::Carrier(previous_type),
                    typed_trees::trait_definition::ConformanceSubject::Carrier(type_name),
...
fn validate_data_satisfies_trait(
    ...
    explicit_type_arguments: &[typed_trees::types::TypeReferenceHandle],
    conformance_name: Option<&str>,
    diagnostics: &mut Vec<Diagnostic>,
    visited_traits: &mut Vec<symbols::SymbolHandle>,
) {
