...
use super::{
    ...
    State, TypedTrees, collect_guard, entry_comparisons, exact_integer_parameter,
    field_coordinates, fields, integer_bindings, meanings, parameter_comparisons, projections,
    scalar_entry, telescoped_bindings, validate_mapping,
};
...
pub(crate) fn mixed_call_endpoints_are_pinned(
    ...
    let prove = || -> Option<()> {
        ...
        let equalities = edges
            .iter()
            .map(|edge| argument_sources(program, members, edge, &projections))
            .collect::<Option<Vec<_>>>()?;
...
/// Arithmetic combines inputs without adding new ones; any other spelling --
/// a builtin member like `.len`, a call, an index -- has no named input and
/// keeps the endpoint undiscovered rather than guessing a carrier.
fn endpoint_inputs<'program>(
    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Atomic(atomic) => {
            endpoint_inputs(program, state, atomic.value, inputs, projections, depth + 1)
        }
        ...
        // A member projection's input is the exact leaf coordinate it reads:
        // conservation transports that value, not the record spelling.
        ...
        {
            endpoint_inputs(program, state, binary.left, inputs, projections, depth + 1)?;
            endpoint_inputs(program, state, binary.right, inputs, projections, depth + 1)
        }
        _ => None,
    }
}
