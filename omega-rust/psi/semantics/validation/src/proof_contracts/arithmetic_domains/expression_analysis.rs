...
pub(super) fn analyze(
    ...
    if let Some(value) =
        ...
        && let Some(literal) =
            crate::value_custody::literals::land_integer_value(&value.value, primitive)
    {
        return Analysis {
    ...
    if let Some(evaluated) = crate::value_custody::literals::anonymous_numeric_value(
        program,
        expression,
        &mut |expression| {
    ...
    match program.expression_table.expression(expression) {
        ExpressionNode::Binary(binary) => {
                ...
                )
            {
                for (operand, analysis) in [(binary.left, &mut left), (binary.right, &mut right)] {
                    ...
                    let Some(evaluated) = crate::value_custody::literals::anonymous_numeric_value(
                        program,
                        operand,
                        &mut |expression| {
                            crate::value_custody::literals::has_anonymous_operator_meaning(program, expression)
                    ...
                    if let Some(literal) = evaluated
                        .value
                        .to_integer_exact()
                        .and_then(|value| crate::value_custody::literals::land_integer_value(&value, primitive))
                    {
            ...
            if policy_bridge.is_some_and(|bridge| {
                bridge
                    .trap_predicates
                    .contains(&numerics::integer_policy::IntegerTrapPredicate::ResultOutsideCarrier)
            ...
            if !matches!(operator, BinaryOperator::Divide | BinaryOperator::Modulo)
                && policy_bridge.is_some_and(|bridge| {
                    bridge.formation_conditions.contains(
                        &numerics::integer_policy::IntegerFormationCondition::ResultRepresentable,
                    )
                })
                && let Some(primitive) = primitive
                && let Some(range) = primitive_range(primitive)
            ...
            let shift_count_law = match operator {
                BinaryOperator::ShiftLeft => Some(
                    numerics::integer_policy::integer_policy_bridge(
                        numerics::integer_policy::IntegerPolicyPrimitive::ShiftLeft,
                        effective_domain,
                    )
                    .shift_count_law,
                ),
                BinaryOperator::ShiftRight => Some(
                    numerics::integer_policy::integer_policy_bridge(
                        numerics::integer_policy::IntegerPolicyPrimitive::ShiftRight,
                        effective_domain,
                    )
                    .shift_count_law,
                ),
            ...
            if shift_count_law == Some(numerics::integer_policy::ShiftCountLaw::MustBeWithinWidth)
        ...
        ExpressionNode::Cast(cast) => {
            ...
            let anonymous = crate::value_custody::literals::anonymous_numeric_value(
                program,
                cast.value,
                &mut |expression| {
            ...
            {
                match evaluated.value.to_integer_exact().and_then(|value| {
                    crate::value_custody::literals::land_integer_value(&value, primitive)
                }) {
        ...
        _ => match declared_place_type_raw(program, machine, state, expression).or_else(|| {
            ...
            Some(handle) => {
                ...
                let primitive =
                    crate::value_custody::places::unwrapped_type_reference(program, handle)
