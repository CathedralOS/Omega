...
fn validate_progress_profile_domains(program: &TypedTrees, diagnostics: &mut Vec<Diagnostic>) {
    for domain in program.domain_definitions().iter().filter(|domain| {
        domain.classification == Some(language_semantics::DomainClassification::ProgressProfile)
    }) {
    ...
    for domain in program.domain_definitions() {
        ...
        for constituent in &alias.constituents {
            ...
            if label == "Carry::Portable"
                || language_semantics::CarryPermission::from_name(&label).is_some()
            {
...
fn normalized_domain_facts(
    ...
    let mut facts = fact_plan
        ...
        .filter_map(|fact| match fact.payload {
            facts::FactPayload::BooleanExpression(expression) => Some(format!(
                "bool:{}",
...
fn domain_membership_facts(
    fact_plan: &FactPlan,
    symbol: SymbolHandle,
) -> impl Iterator<Item = facts::DomainMembershipFact> + '_ {
