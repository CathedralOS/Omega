    ...
    for machine in program.machines() {
        for state in program.machine_states(machine) {
                ...
                .enumerate()
            {
                ...
                let Some(source_symbol) = crate::value_custody::places::declared_place_leaf_symbol(
                    program,
                    machine,
                    Some(state),
                    statement_index,
    ...
    for machine in program.machines() {
        for state in program.machine_states(machine) {
                ...
                .enumerate()
            {
                ...
                let Some(target_symbol) = crate::value_custody::places::declared_place_leaf_symbol(
                    program,
                    machine,
                    Some(state),
                    statement_index,
...
fn validate_dynamic_call_arguments_in_statement(
    ...
    for root in crate::machine_calls::calls::statement_value_expression_roots(program, statement) {
                ...
                .enumerate()
            {
                if let StatementNode::Call(call) = statement {
                    let dynamic_target = program
                        ...
                        .last()
                        .and_then(|receiver| {
                            crate::machine_calls::calls::declared_receiver_type_reference(
                                program,
                                machine,
                                state,
                                receiver.as_str(),
                    ...
                    let target = dynamic_target.or_else(|| {
                        crate::value_custody::placed_views::statement_call_target(
...
fn collect_dynamic_expression_call_updates(
    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Cast(cast) => visit!(cast.value),
        ExpressionNode::Call(call) => {
            if let Some(receiver_type) = crate::value_custody::places::declared_place_type_raw(
                program,
                machine,
                Some(state),
                call.receiver,
