...
mod integer_ranges;
pub use integer_ranges::{closed_integer_range_bound, declared_integer_range};
...
pub(crate) fn validate_type_reference_handle_with_type_parameters(
    ...
    lifetime_parameters: &[typed_trees::name::Identifier],
) {
...
fn validate_routed_service_carrier_placement(
    ...
    match typed_trees::service::classify_exact_bound_service_carrier(program, type_reference) {
        Err(error) => {
...
struct TypeParameterScope<'program> {
    ...
    lifetime_parameters: &'program [typed_trees::name::Identifier],
}
