...
/// One authored boundary-operator use whose requirement is supplied by a
/// single checked provider body in the evaluated program.
///
/// Early const-generic probes run before Omega settles provider plans, so the
/// deferred selected-execution route cannot reach them. When the authored
/// selection admits exactly one boundary requirement and exactly one
/// `satisfies` machine supplies it, that provider body is already the use's
/// only possible meaning: rebinding the use to an ordinary call targeting the
/// provider's entry state makes the provider join the call closure like any
/// authored invocation (termination, suspension, blocking, linear carriers,
/// and selection authority all apply unchanged), and the interpreter runs the
/// real body rather than folding builtin arithmetic over the same operands. A
/// use that resolves to zero or several requirements or providers keeps its
/// authored expression and the ordinary selection-authority rejection.
struct UniqueProviderRebinding {
    expression: ExpressionHandle,
    provider_state: SymbolHandle,
    provider_name: String,
    operands: [ExpressionHandle; 2],
}

/// Rebind every uniquely selected authored boundary-operator use in `program`
/// to its sole checked provider's entry-state call. `program` must be the
/// caller's private evaluation copy: the authored expression is replaced, so
/// the same tree must drive both admission inference and interpretation. The
/// caller's source-owned tree is never passed here.
pub(crate) fn rebind_selected_provider_operators(program: &mut TypedTrees) {
    ...
    for rebinding in rebindings {
        let arguments = program
            .expression_table
            .insert_expression_handles(rebinding.operands.iter().copied());
        *program
            .expression_table
            .expression_mut(rebinding.expression) =
            ExpressionNode::Call(typed_trees::expression::TableCallExpression {
                receiver: ExpressionHandle::invalid(),
                target_symbol: rebinding.provider_state,
                static_machine_parameter: SymbolHandle::invalid(),
                target: typed_trees::name::Identifier::generated(
                    rebinding.provider_name.clone(),
                ),
...
fn collect_provider_rebinding(
    program: &TypedTrees,
    machine: &Machine,
    state: &State,
    expression: ExpressionHandle,
    visited: &mut Vec<ExpressionHandle>,
    rebindings: &mut Vec<UniqueProviderRebinding>,
) {
    if !expression.is_valid() || visited.contains(&expression) {
        return;
    }
    ...
    if let ExpressionNode::Binary(binary) = program.expression_table.expression(expression)
        ...
        && let Some(rebinding) = unique_provider_rebinding(program, expression, binary)
    {
        rebindings.push(rebinding);
    }
    ...
    for child in children {
        collect_provider_rebinding(program, machine, state, child, visited, rebindings);
    }
}

/// Derive the sole checked provider body for one authored boundary-operator
/// use. Every leg returns `None` rather than guessing: the authored selection
/// must name exactly one boundary requirement, exactly one `satisfies` machine
/// must bind it through a checked conformance, and that provider must be a
/// concrete checked body whose executable entry signature matches the
/// requirement exactly. Anything less keeps the authored expression for the
/// ordinary selection-authority rejection.
fn unique_provider_rebinding(
    program: &TypedTrees,
    expression: ExpressionHandle,
    binary: &typed_trees::expression::TableBinaryExpression,
) -> Option<UniqueProviderRebinding> {
    let candidates = typed_trees_to_checked_trees::typed_operator_authored_selection_candidates(
        program, expression,
    );
    let [requirement_symbol] = candidates.as_slice() else {
        return None;
    };
    let operator = typed_trees::operator::declaration_by_symbol(program, *requirement_symbol)?;
    ...
    let providers = program
        ...
        .iter()
        .filter(|provider| {
            provider.supply_mode == language_semantics::MachineSupplyMode::CheckedBody
                        ...
                        conformance.external_binding.is_none()
                            ...
                            .is_some_and(|resolved| resolved.symbol == operator.symbol)
                                ...
                                .is_some_and(|(resolved, _)| {
                                    resolved.symbol == operator.symbol
                                }))
                    })
                    .count()
                    == 1
    ...
    let entry = program.machine_states(provider).first()?;
    ...
    let entry_parameters: Vec<_> = program
        ...
        .collect();
    if entry_parameters.len() != operands.len()
    ...
    Some(UniqueProviderRebinding {
        expression,
        provider_state: entry.symbol,
        provider_name: provider.name.as_str().to_owned(),
        operands,
    })
}
...
mod tests {
    ...
    // A helper's authored `%` whose requirement has exactly one `satisfies`
    // provider is rebound to that provider's entry-state call: the provider
    // body is the use's only possible meaning before plans settle, and builtin
    // `+` in the same closure keeps builtin meaning.
    #[test]
    fn unique_satisfies_provider_rebinds_authored_operator_use() {
        let mut typed = typed_from_source(
            r#"
            data Math {}
        ...
        let modulos = binary_expressions(&typed, typed_trees::expression::BinaryOperator::Modulo);
        let [remainder] = modulos.as_slice() else {
            panic!("expected exactly one `%` expression");
        };
        super::rebind_selected_provider_operators(&mut typed);
        assert!(matches!(
            typed.expression_table.expression(*remainder),
            ExpressionNode::Call(_)
        ));
        let sums = binary_expressions(&typed, typed_trees::expression::BinaryOperator::Add);
        let [sum] = sums.as_slice() else {
            panic!("expected exactly one `+` expression");
        };
        assert!(matches!(
            typed.expression_table.expression(*sum),
            ExpressionNode::Binary(_)
        ));
    }

    // Two `satisfies` providers for one requirement keep the authored
    // expression: the probe cannot pick a meaning, so the ordinary
    // selection-authority rejection still owns this use.
    #[test]
    fn ambiguous_satisfies_providers_keep_authored_expression() {
        let mut typed = typed_from_source(
            r#"
            data Math {}
            boundary operator % Math::remainder(left: u64, right: u64) -> u64;
