...
pub fn compute_plan_laid_layouts_with_authority(
    ...
    for record in records {
        ...
        let schema_fields = typed
            ...
            .iter()
            .filter_map(|member| match member {
                typed_trees::data::DataMember::Field(field) => (!field.relevance.is_erased())
            ...
            match field_entries.as_slice() {
                        ...
                        }) =>
                {
                    let typed_trees::types::TypeReferenceNode::FixedArray {
                        length: typed_trees::types::FixedArrayLength::Literal(element_count),
                        ..
...
fn stored_integer_write_is_total(
    ...
    let (stored_minimum, stored_maximum) = match interpretation {
        ...
        layout_plans::IntegerInterpretation::Unsigned => (0, (1i128 << stored_width) - 1),
    };
