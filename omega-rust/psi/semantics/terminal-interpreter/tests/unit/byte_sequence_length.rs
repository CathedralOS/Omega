...
fn byte_sequence_length_artifact_rejects_stale_vocabulary() {
    ...
    assert_eq!(&semantic[10..12], &81_u16.to_le_bytes());
    semantic[10..12].copy_from_slice(&80_u16.to_le_bytes());
...
fn verifier_rejects_wrong_length_result_and_unknown_or_unestablished_source() {
    ...
    assert!(matches!(
        ...
        Err(ModuleError::ByteSequenceLiteralEstablishmentMismatch(_))
    ));
