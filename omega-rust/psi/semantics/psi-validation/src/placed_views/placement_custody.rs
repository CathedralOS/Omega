...
/// most twenty-two additional represented records.
...
const MAX_CUSTODY_RECORD_DEPTH: usize = 23;
...
fn unsupported_nested_custody_diagnostic(
    ...
    Diagnostic::error(format!(
        "custody conformance `{}` disagrees with `{plan_name}`: normalized decision for `{}` is {}, but its represented type contains non-runtime custody outside the exact twenty-three-record acyclic, non-generic, case-free projection spine",
        program.symbols.display_path(conformance.symbol, "::"),
