...
impl<'program> Evaluator<'program> {
    ...
    pub(in crate::interpreter::evaluator) fn tick(&mut self) -> EvalResult<()> {
        self.usage
            .charge_step()
    ...
    pub(in crate::interpreter::evaluator) fn allocate_cell(
        &self,
        value: Value,
    ) -> EvalResult<Cell> {
    ...
    pub(in crate::interpreter::evaluator) fn allocate_text(
        &self,
        bytes: Vec<u8>,
    ) -> EvalResult<Value> {
    ...
    pub(in crate::interpreter::evaluator) fn charge_build_log_bytes(
        &mut self,
        bytes: usize,
    ) -> EvalResult<()> {
    ...
    pub(in crate::interpreter::evaluator) fn charge_filesystem_operation_attempt(
        &mut self,
    ) -> EvalResult<()> {
        if let Some(sponsor) = &self.build_evaluation_sponsor {
            sponsor
    ...
    pub(in crate::interpreter::evaluator) fn populate_data_fields(
        &mut self,
        data: &DataDefinition,
    ...
    pub(in crate::interpreter::evaluator) fn default_value_for_type(
        &mut self,
    ...
    pub(in crate::interpreter::evaluator) fn default_for_type(
        &self,
        type_reference: typed_trees::types::TypeReferenceHandle,
    ) -> EvalResult<Value> {
        Ok(
    ...
    pub(in crate::interpreter::evaluator) fn run_state_collect(
        &mut self,
        machine: &'program Machine,
        state: &'program State,
        instance: Cell,
...
mod build_evaluation_configuration_tests {
    use crate::interpreter::evaluator::STEP_BUDGET;
    use crate::interpreter::evaluator::{Evaluator, TypedTrees};
