...
impl<'program> Evaluator<'program> {
    ...
    fn eval_call_expression(
        ...
        if let Some(value) = self.try_build_product_provider_value_call(handle, call, frame)? {
            return Ok(value);
        }
        ...
        if let Some(value) = self.try_product_provider_ref_path_value_call(call, frame)? {
            return Ok(value);
        }
    ...
    fn try_float_boundary_value_call(
        ...
        let core_format = typed_trees::operator::resolve_named_expression_call(self.program, call)
            .and_then(|operator| {
                self.program
