...
- Granted argument evaluation additionally configures explicit filesystem
  authority or replay and retains partial work/observations on evaluator failure.
...
- [filesystem_replay.rs](src/filesystem_replay.rs) constructs and validates
  retained replay records; its children own individual source/output/failure lanes.
...
Focused coverage is in `tests/const_values.rs`, `tests/build_arguments.rs`,
`tests/resolved_state_execution.rs`, and
`tests/unknown_descriptor_operation_replay.rs`, with replay, filesystem
preparation, and sponsor unit tests beside their implementations.
