...
pub(crate) enum LoweredBooleanDecisionExit {
    Return,
    Jump { target: BlockId },
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct PendingConditionalBindingBlock {
    pub(crate) id: BlockId,
    pub(crate) parameters: Vec<ValueDeclaration>,
    pub(crate) target: BlockId,
    pub(crate) arguments: Vec<LoweredDirectExpression>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct PendingMixedTupleBindingBlocks {
    pub(crate) first_id: BlockId,
    pub(crate) original_parameter_count: usize,
    pub(crate) arguments: Vec<LoweredDirectExpression>,
    pub(crate) stage_parameters: Vec<Vec<ValueDeclaration>>,
    pub(crate) target: BlockId,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum PendingNestedBlockGroup {
    ConditionalBinding(PendingConditionalBindingBlock),
    TupleBinding(PendingMixedTupleBindingBlocks),
}

impl PendingNestedBlockGroup {
    pub(crate) fn first_id(&self) -> BlockId {
        match self {
...
pub(crate) fn bind_boolean_decision<F>(
...
pub(crate) fn lower_boolean_control_decision(
...
pub(crate) fn lower_boolean_value_decision(
    expression: &LoweredBooleanReturnExpression,
) -> LoweredBooleanDecision {
...
pub(crate) fn boolean_decision_block_count(decision: &LoweredBooleanDecision) -> usize {
    match decision {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct LoweredBooleanDecisionTarget {
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn emit_inlined_boolean_value_blocks(
    decision: &LoweredBooleanDecision,
    parameters: &[ValueDeclaration],
    block_parameters: Vec<ValueDeclaration>,
    exit: LoweredBooleanDecisionExit,
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn emit_inlined_boolean_guard_blocks(
    decision: &LoweredBooleanDecision,
    parameters: &[ValueDeclaration],
    block_parameters: Vec<ValueDeclaration>,
    when_true_target: &LoweredBooleanDecisionTarget,
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn emit_reserved_boolean_tuple_stage_blocks(
    decision: &LoweredBooleanDecision,
    parameters: &[ValueDeclaration],
    block_parameters: Vec<ValueDeclaration>,
    next_stage: BlockId,
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn build_scalar_conditional_target(
    target: usize,
...
pub(crate) fn scalar_source_block(identity_base: u64, state: usize) -> BlockId {
    block_id(
        identity_base
