...
use crate::psi_lowering::{LoweringError, operation_id, unsupported};
...
pub(in crate::psi_lowering) struct SourceCallCoordinate {
    pub(in crate::psi_lowering) state: symbols::SymbolHandle,
    pub(in crate::psi_lowering) statement_index: usize,
    pub(in crate::psi_lowering) call_ordinal: usize,
}
...
pub(in crate::psi_lowering) struct OperationBuffer {
    ...
    pub(in crate::psi_lowering) structural_values:
        Vec<(u32, terminal_psi::StructuralOperationResult)>,
    pub(in crate::psi_lowering) selected_ieee_float_comparisons:
        Vec<lowered_psi::LoweredSelectedIeeeFloatComparisonOccurrence>,
    pub(in crate::psi_lowering) next_identity: u64,
    pub(in crate::psi_lowering) operations: Vec<Operation>,
    /// Temporary observations available on the current emission path only.
    pub(in crate::psi_lowering) byte_lengths: Vec<(PlaceId, ValueId)>,
    pub(in crate::psi_lowering) source_calls: Vec<LoweredSourceCallOccurrence>,
    pub(in crate::psi_lowering) selected_ieee_float_fmas:
        Vec<LoweredSelectedIeeeFloatFmaOccurrence>,
}

impl OperationBuffer {
    pub(in crate::psi_lowering) fn new(identity_base: u64) -> Self {
        Self {
    ...
    pub(in crate::psi_lowering) fn allocate(&mut self) -> OperationId {
    ...
    pub(in crate::psi_lowering) fn record_source_call(
        &mut self,
    ...
    pub(in crate::psi_lowering) fn record_source_call_with_values(
        &mut self,
        coordinate: SourceCallCoordinate,
        source_site: Option<checked_trees::NominalMachineUseSite>,
        operation: OperationId,
    ...
    pub(in crate::psi_lowering) fn record_selected_ieee_float_fma(
        &mut self,
        coordinate: SourceCallCoordinate,
        operation: OperationId,
