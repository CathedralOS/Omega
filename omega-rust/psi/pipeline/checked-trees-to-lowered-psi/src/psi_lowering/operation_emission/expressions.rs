...
use crate::psi_lowering::{LoweringError, obligation_id, unsupported, value_id};
...
pub(in crate::psi_lowering) enum LoweredDirectExpression {
    PrimitiveRead {
        source: PlaceId,
...
impl LoweredDirectExpression {
    pub(in crate::psi_lowering) fn value_type(
        &self,
    ...
    pub(in crate::psi_lowering) const fn scalar_type(&self) -> ScalarType {
        match self {
...
pub(in crate::psi_lowering) fn emit_byte_length(
    source: PlaceId,
    next_value_identity: &mut u64,
    operations: &mut OperationBuffer,
) -> ValueId {
...
pub(in crate::psi_lowering) fn emit_direct_expression(
