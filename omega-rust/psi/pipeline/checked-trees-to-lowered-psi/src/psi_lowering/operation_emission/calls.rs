...
use crate::psi_lowering::scalar_graph_lowering::{
    contains_short_circuit, direct_expression_contains_short_circuit,
};
use crate::psi_lowering::{
    LoweringError, block_id, edge_id, lower_checked_crash_route_buckets, obligation_id,
...
pub(in crate::psi_lowering) struct LoweredDirectCallBinding {
    ...
    pub(in crate::psi_lowering) uses_structural_frame: bool,
    pub(in crate::psi_lowering) crash_continuations: Vec<checked_trees::CrashRouteBucket>,
    pub(in crate::psi_lowering) parameter_relative_crash_routes:
        Vec<checked_trees::CrashRouteBucket>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::psi_lowering) enum ScalarCallCrashScope {
    CallerValues,
    Arguments,
}

pub(in crate::psi_lowering) struct CallEmissionContext<'a> {
    pub(in crate::psi_lowering) machine_ids: &'a [(symbols::SymbolHandle, MachineId)],
    pub(in crate::psi_lowering) requirement_counts: &'a [(symbols::SymbolHandle, usize)],
    pub(in crate::psi_lowering) next_obligation_identity: u64,
    pub(in crate::psi_lowering) obligation_limit: u64,
}

impl CallEmissionContext<'_> {
    pub(in crate::psi_lowering) fn allocate_requirement(
        &mut self,
    ) -> Result<ObligationId, LoweringError> {
...
pub(in crate::psi_lowering) fn emit_staged_scalar_call_binding(
    call: &LoweredDirectCallBinding,
