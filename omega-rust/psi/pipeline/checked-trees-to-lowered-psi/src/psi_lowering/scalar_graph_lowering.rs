...
fn prepare_scalar_graph_machine_with_contract_mode(
    ...
    crate::psi_lowering::runtime_requirements::validate_graph_parameter_ranges(
        checked,
        machine,
        plan.requires(),
    )?;
...
pub(super) fn lower_scalar_call(
    ...
    let target = if structural_arguments.is_empty() {
        crate::psi_lowering::scalar_call_closure::callee::CheckedScalarCallee::find(
            checked,
            target_machine,
        )?
    } else {
        crate::psi_lowering::scalar_call_closure::callee::CheckedScalarCallee::find_for_unit_call(
            checked,
            target_machine,
        )?
    };
...
pub(super) fn lower_checked_scalar_expression_with_parameters(
    ...
    structural_fields: &[crate::psi_lowering::scalar_bindings::StructuralScalarFieldBinding],
    structural_cases: &[crate::psi_lowering::scalar_bindings::structural_cases::StructuralCaseBinding],
    ...
    match expression {
        ...
        CheckedScalarExpression::StorageRead {
            ...
            let source = crate::psi_lowering::scalar_bindings::primitive_storage_place(
                primitive_storage,
                *symbol,
                scalar_type,
            )?;
...
mod primitive_read_tests {
    ...
    fn primitive_read_values_cannot_be_recovered_from_known_ssa_inputs() {
        ...
        assert!(
            crate::psi_lowering::shared_runtime_parameters::shared_boolean_runtime_parameters(
                &equality
            )
            .is_none()
        );
        assert!(
            crate::psi_lowering::shared_runtime_parameters::normalize_shared_boolean_comparison_leaves(&equality)
                .is_none()
        );
    }
}
