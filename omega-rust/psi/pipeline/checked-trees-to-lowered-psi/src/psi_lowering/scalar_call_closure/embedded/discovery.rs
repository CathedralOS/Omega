...
pub(crate) fn checked_scalar_call_closure_with_structural_roots(
    ...
    while let Some(machine) = closure.get(next).copied() {
        ...
        let direct_targets = match callee {
            crate::psi_lowering::scalar_call_closure::callee::CheckedScalarCallee::Graph(graph) => {
                graph
                    .states
                    .iter()
            ...
            crate::psi_lowering::scalar_call_closure::callee::CheckedScalarCallee::Boundary(_) => {
                Vec::new()
            }
            crate::psi_lowering::scalar_call_closure::callee::CheckedScalarCallee::Structural(
                _,
            ) => Vec::new(),
            crate::psi_lowering::scalar_call_closure::callee::CheckedScalarCallee::Operations(
                _,
            ) => Vec::new(),
        };
        let mut computed = source_checked_computation_targets(checked, machine)?;
        let mut computed_structural =
            crate::psi_lowering::scalar_computations::structural_call_targets(checked, machine)?;
        if let crate::psi_lowering::scalar_call_closure::callee::CheckedScalarCallee::Operations(
            plan,
        ) = callee
        {
...
fn source_checked_computation_targets(
    ...
    crate::psi_lowering::scalar_computations::call_targets(checked, machine)
}
