...
impl<'checked> CheckedScalarCallee<'checked> {
    ...
    pub(crate) fn find_for_unit_call(
        ...
        match selected {
            ...
            (None, Some(boundary), None) => {
                crate::psi_lowering::boundary_scalar_return::validate_boundary_scalar_return(
                    checked, boundary,
                )?;
                Ok(Self::Boundary(boundary))
            }
            (None, None, Some(plan)) => {
                crate::psi_lowering::structural_scalar_return::validate_scalar_callee(
                    checked, plan,
                )?;
    ...
    pub(crate) fn prepare(
        ...
        primitive_locals: &[crate::psi_lowering::scalar_graph_lowering::primitive_locals::PrimitiveLocal],
        ...
        match self {
            ...
            Self::Graph(graph) => {
                ...
                let prepared =
                    crate::psi_lowering::scalar_graph_lowering::prepare_scalar_graph_in_namespace(
                        checked,
                        graph,
            ...
            Self::Boundary(plan) => Ok(PreparedScalarCallee::Boundary {
                ...
                requirement_count: usize::from(
                    !crate::psi_lowering::boundary_scalar_return::checked_requirements(
                        checked, plan,
                    )?
                    .is_empty(),
                ),
