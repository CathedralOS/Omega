...
use crate::psi_lowering::{CheckedTrees, LoweredSourceCallOccurrence, LoweringError, unsupported};

use crate::psi_lowering::reborrow_root_handoff::{
...
pub(crate) fn retain_selected_reborrow_restored_call_uses(
    ...
    terminal_machines: &[terminal_psi::TerminalMachine],
    ...
    for (_, certificate) in borrow.reborrow_restored_call_use_certificates.iter() {
        ...
        let exact_terminal_call = operation.result == terminal_psi::OperationResult::Unit
            ...
            && callee.result == terminal_psi::TerminalMachineResult::Unit
            && callee_parameter.position == 0
            && !callee_parameter.is_self
            && callee_parameter.access == terminal_psi::StructuralAccess::MutableBorrow;
        ...
        rows.push(TerminalReborrowRestoredCallUse {
            ...
            call_boundary: terminal_psi::TerminalBorrowBoundarySource::Call {
