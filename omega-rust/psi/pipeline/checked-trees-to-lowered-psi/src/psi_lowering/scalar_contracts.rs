...
use super::*;
#[cfg(test)]
use crate::psi_lowering::contract_predicates::canonical_equality;
use crate::psi_lowering::contract_predicates::{PredicateTerms, connective};
...
pub(super) fn proposition(
    ...
    crate::psi_lowering::contract_predicates::proposition(
        predicate,
        &ScalarContractTerms { namespace },
    )
}
...
impl PredicateTerms for ScalarContractTerms<'_> {
    fn integer(&self, expression: &CheckedScalarExpression) -> Result<ScalarTerm, LoweringError> {
        crate::psi_lowering::crash_routes::checked_scalar_term(expression, self.namespace)
    }

    fn boolean(&self, expression: &CheckedBooleanExpression) -> Result<ScalarTerm, LoweringError> {
        crate::psi_lowering::crash_routes::checked_boolean_scalar_term(expression, self.namespace)
    }
