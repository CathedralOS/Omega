...
pub(super) fn validate(
    ...
    let (machine, state) =
        crate::psi_lowering::scalar_source_custody::authored_state(checked, plan.state)?;
    ...
    let expected_scalar_parameters =
        crate::psi_lowering::attached_unit::checked_scalar_source_parameters(checked, state)?;
    ...
    if scalar_arguments.iter().any(|argument| {
        ...
        crate::psi_lowering::call_source_custody::initializers::validate(
            checked,
            plan.machine,
            plan.state,
            *coordinate,
