...
pub(super) fn lower_unit_effect_closure(
    ...
    crate::psi_lowering::machine_dispatch::SourceMappedLowered::new(
        closure.lowered,
        closure.machine_ids,
    )
}
...
fn assemble_unit_closure(
    ...
    let scalar_graph_parameters = scalar_callees
        ...
        .map(|callee| {
            ...
            let locals = if let CheckedScalarCallee::Graph(graph) = callee {
                crate::psi_lowering::scalar_graph_lowering::primitive_locals::allocate(
                    checked,
                    graph,
                    &type_ids,
                    &mut next_place,
    ...
    for (signature, admitted) in machine_signatures.iter().zip(admitted_bodies) {
        ...
        evaluation.record_fields = crate::psi_lowering::scalar_computations::fields::prepare(
            checked,
            plan.machine,
            &structural_types,
        )?;
        evaluation.arrays = crate::psi_lowering::scalar_computations::arrays::prepare(
            checked,
            plan.machine,
            &structural_types,
            &mut next_place,
        )?;
        evaluation.cases = crate::psi_lowering::scalar_computations::cases::prepare(
            checked,
            plan.machine,
            &structural_types,
            &mut next_place,
        ...
        evaluation.primitive_storage =
            crate::psi_lowering::scalar_source_custody::primitive_references::bindings(
                checked,
                plan.state,
        ...
        for step in argument_schedule::build(checked, plan)? {
            ...
            let kind = match operation {
                ...
                CheckedUnitEffectOperationPlan::EstablishPrimitiveLocal {
                    ...
                    let value = crate::psi_lowering::scalar_bindings::ScalarBindings::new(
                        source_value_count,
                    )
...
qualifications: Default::default(), id: emit_direct_expression(&argument, &scalar_result_values, &mut next_value_identity, &mut operations), scalar_type })
                    ...
                    } else if !crate::psi_lowering::call_source_custody::initializers::discards_result(
                        checked,
                        plan.state,
                        *coordinate,
                    )? {
                ...
                CheckedUnitEffectOperationPlan::StructuralByteSequenceFieldByteStore(store) => {
                    let bindings = crate::psi_lowering::scalar_bindings::ScalarBindings::new(
                    ...
                    crate::psi_lowering::structural_byte_sequence_index_store::emit(
                        store,
                        parameters,
                        &structural_types,
                        &index,
                ...
                CheckedUnitEffectOperationPlan::ByteSequenceWrite(write) => {
                    ...
                    crate::psi_lowering::byte_sequence_write::emit(
                        write,
                        parameters,
                        &structural_types,
                        &index,
                ...
                CheckedUnitEffectOperationPlan::StructuralScalarFieldStore(store) => {
                    ...
                    let lowered =
                        crate::psi_lowering::structural_scalar_store::lower_structural_scalar_store_place(
                            store,
                            store.statement_index,
                            destination,
                            &structural_types,
        ...
        let normal_guarantees = if let Some((_, result)) = scalar_return {
            ...
            let refined = crate::psi_lowering::scalar_contracts::with_result_range(
                checked,
                plan.state,
                scalar_parameters.len(),
                &contract.closed_scalar_values,
            )?;
            crate::psi_lowering::scalar_contracts::clauses(refined.ensures(), &namespace)?
                .into_iter()
                .map(|proposition| {
                    Ok(ContractClause {
    ...
    for (index, machine) in prepared_scalar_machines.into_iter().enumerate() {
        ...
        if let PreparedScalarCallee::Structural { plan, .. } = &machine {
            let mut lowered = crate::psi_lowering::structural_scalar_return::lower_structural_scalar_return_machine_in_namespace(
        ...
        let PreparedScalarCallee::Graph(machine) = machine else {
            ...
            let mut emitted =
                crate::psi_lowering::boundary_scalar_return::emit_boundary_scalar_return(
                    checked,
                    plan,
                    lower_unit_parameters(
                        &plan.structural_parameters,
                    ...
                    crate::psi_lowering::boundary_scalar_return::BoundaryScalarReturnCatalogs {
                        structural_types: &structural_types,
                        type_ids: &type_ids,
                        service_ids: &service_ids,
                    },
                    crate::psi_lowering::boundary_scalar_return::BoundaryScalarReturnIdentities {
        ...
        let mut lowered =
            crate::psi_lowering::scalar_graph_module::build_scalar_graph_module_in_namespace(
                &machine.states,
                machine.result_type,
    ...
    let mut lowered_structural_result_realizations =
        crate::psi_lowering::affine_return::lower_claim_free_affine_return_machines(
            checked,
