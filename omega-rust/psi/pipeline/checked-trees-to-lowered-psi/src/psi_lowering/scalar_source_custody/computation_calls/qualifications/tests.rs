...
fn qualifications(checked: &CheckedTrees) -> Vec<CheckedScalarComputationHandle> {
    crate::psi_lowering::scalar_computations::reachable_nodes(checked, &[root(checked).root])
        .expect("closure")
        .into_iter()
        .filter(|handle| {
            matches!(
...
fn qualification_match_replays_checked_custody_and_publishes_exact_terminal_membership() {
    ...
    let lowered =
        crate::psi_lowering::lower_machine(&checked, "choose").expect("qualified Terminal graph");
...
fn qualification_keeps_nested_selection_and_call_operands_in_their_source_scopes() {
    for body in [
        ...
        let targets = crate::psi_lowering::scalar_computations::call_targets(
            &checked,
            root(&checked).machine,
        )
...
fn indexed_call_replay_rechecks_parameter_contract_instance() {
    ...
    for erase_arguments in [false, true] {
        let mut checked = checked(source);
        crate::psi_lowering::lower_machine(&checked, "choose")
        ...
        assert!(
            crate::psi_lowering::lower_machine(&checked, "choose").is_err(),
