...
mod tests {
    ...
    fn dynamic_callee_fixture() -> (CheckedTrees, TerminalModule, Owners, symbols::SymbolHandle) {
        ...
        let selection =
            crate::psi_lowering::machine_dispatch::select_terminal_machine(&checked, "Main::main")
                .expect("dynamic root selection");
        let root = selection.machine;
        let mut lowered =
            crate::psi_lowering::machine_dispatch::lower_selected_machine(&checked, selection)
        ...
        crate::psi_lowering::closed_reach_applications::retain_closed_reach_applications(
            &checked,
            &owners,
