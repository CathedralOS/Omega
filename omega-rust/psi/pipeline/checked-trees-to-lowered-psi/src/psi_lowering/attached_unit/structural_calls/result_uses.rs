...
pub(crate) fn validate_linear_result_consumer(
    ...
    let (_, producer_state) = crate::psi_lowering::scalar_source_custody::authored_state(
        checked,
    ...
    crate::psi_lowering::call_source_custody::initializers::validate_structural(
        checked,
        machine,
        state,
        *producer_coordinate,
...
pub(crate) fn validate_consumer(
        ...
        .enumerate()
    {
        ...
        for candidate in &caller.operations {
            ...
            {
                crate::psi_lowering::call_source_custody::initializers::validate_discarded_structural(
                    checked,
                    caller.machine,
                    caller.state,
                    producer_coordinate,
            ...
            let names_result = if producer_coordinate.call_ordinal == 0 {
                ...
                let source = crate::psi_lowering::call_source_custody::authored::locate_source(
                    checked,
                    caller.state,
                    producer_coordinate,
                )?;
                ...
                if matches {
                    let signature =
                        crate::psi_lowering::call_source_custody::authored::target_signature(
                            checked,
...
fn validate_nested_execution_order(
    ...
    let expected = crate::psi_lowering::call_source_custody::authored::nested::authored_postorder(
        checked,
        caller.state,
        statement_index,
    )?;
