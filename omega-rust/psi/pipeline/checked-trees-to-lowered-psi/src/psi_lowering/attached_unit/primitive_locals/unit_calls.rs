...
use crate::psi_lowering::{LoweringError, unsupported};

pub(in crate::psi_lowering::attached_unit) fn validate(
    ...
    let authored = crate::psi_lowering::call_source_custody::authored::locate_source(
        checked,
        caller.state,
        *coordinate,
    )?;
    let (machine, state) =
        crate::psi_lowering::scalar_source_custody::authored_state(checked, caller.state)?;
