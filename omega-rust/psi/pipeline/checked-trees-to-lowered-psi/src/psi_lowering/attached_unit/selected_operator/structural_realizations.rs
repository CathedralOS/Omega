...
pub(in crate::psi_lowering::attached_unit) struct LoweredSelectedStructuralScalarRealizations {
    pub(in crate::psi_lowering::attached_unit) machines: Vec<TerminalMachine>,
    pub(in crate::psi_lowering::attached_unit) evidence: Vec<ObligationEvidence>,
    pub(in crate::psi_lowering::attached_unit) source_calls: Vec<LoweredSourceCallOccurrence>,
}

pub(in crate::psi_lowering::attached_unit) fn lower_selected_structural_scalar_realizations(
    ...
    for (index, source_machine) in roots.iter().enumerate() {
        ...
        let mut lowered =
            crate::psi_lowering::structural_scalar_return::lower_structural_scalar_return_machine_in_namespace(
                &staged,
                realization,
                terminal_machine,
                identity_base,
