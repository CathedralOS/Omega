...
pub(in crate::psi_lowering::attached_unit::composed_control) fn emit(
    ...
    let claims = crate::psi_lowering::attached_unit::claims::lower_unit_entry_claims(
        plan.machine,
        entry.state,
        &entry.entry_claims,
        &parameters,
    ...
    let content_entry_claims =
        crate::psi_lowering::content_conservation::lower_whole_content_entry_claims(
            checked,
    ...
    let content_identity_reshuffles =
        crate::psi_lowering::attached_unit::claims::lower_result_identity(
            checked,
            plan.machine,
            entry.state,
            &entry.structural_parameters,
    ...
    for (position, state) in plan.states.iter().enumerate() {
        ...
        let mut evaluation = crate::psi_lowering::attached_unit::argument_evaluation::Evaluation {
            structural_value_owners: Vec::new(),
            selection_cleanups: Vec::new(),
            structural_locals: Vec::new(),
            local_cases: Vec::new(),
            record_fields: crate::psi_lowering::scalar_computations::fields::prepare(
                checked,
                plan.machine,
                &catalogs.structural_types,
            )?,
            arrays: crate::psi_lowering::scalar_computations::arrays::prepare(
                checked,
                plan.machine,
                &catalogs.structural_types,
                &mut catalogs.next_place,
            )?,
            cases: crate::psi_lowering::scalar_computations::cases::prepare(
                checked,
                plan.machine,
                &catalogs.structural_types,
                &mut catalogs.next_place,
        ...
        let current_rank =
            if let Some(scalar_position) = ranking::scalar_parameter_position(plan, state) {
                ...
                ranking::parameter_position(plan, state).map(|parameter_position| {
                    crate::psi_lowering::operation_emission::emit_byte_length(
        ...
        evaluation.structural_fields =
            crate::psi_lowering::scalar_bindings::StructuralScalarFieldBinding::collect(
                &evaluation.structural_parameters,
                &catalogs.structural_types,
            );
        evaluation.structural_cases =
            crate::psi_lowering::scalar_bindings::structural_cases::StructuralCaseBinding::collect(
                &evaluation.structural_parameters,
                &catalogs.structural_types,
            );
    ...
    structural_places.extend(
        blocks
            .iter()
            .flat_map(|block| {
                crate::psi_lowering::scalar_computations::arrays::declarations(&block.operations)
                    .chain(
                        crate::psi_lowering::scalar_computations::cases::declarations(
                            &block.operations,
                        ),
                    )
            })
    ...
    if let Some(attachment) = attachment {
        ...
        let boundaries = catalogs
            ...
            .collect::<Vec<_>>();
        structural_places.extend(
            crate::psi_lowering::attached_unit::provider_attachments::lower_provider_attachment_places(
                attachment,
                declaration,
    ...
    let mut machine = TerminalMachine {
        ...
        declared_service_reach: crate::psi_lowering::attached_unit::lower_declared_service_reach(
            checked,
            plan.machine,
