...
pub(in crate::psi_lowering::attached_unit) fn emit_callable_body(
    checked: &CheckedTrees,
    plan: &checked_trees::CheckedComposedUnitControlMachinePlan,
    admitted: admission::AdmittedComposedUnit<'_>,
...
pub(super) fn emit_call_operations(
    ...
    for operation in planned_operations {
        if matches!(
            ...
            let declaration = crate::psi_lowering::attached_unit::structural_values::emit(
                checked,
                machine,
                state.state,
                operation,
        ...
        if let CheckedUnitEffectOperationPlan::ByteSequenceWrite(write) = operation {
            ...
            let kind = crate::psi_lowering::byte_sequence_write::emit(
                write,
                parameters,
                &catalogs.structural_types,
                &index,
            ...
            operation
        {
            ...
            let kind = crate::psi_lowering::structural_byte_sequence_index_store::emit(
                store,
                parameters,
                &catalogs.structural_types,
                &index,
