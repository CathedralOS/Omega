...
pub(crate) use crate::psi_lowering::scalar_call_closure::embedded::EmbeddedScalarCalls as ComposedScalarCalls;
    ...
    for state in states {
            ...
            .flat_map(CheckedUnitEffectOperationPlan::with_value_calls)
        {
            ...
            if let CheckedUnitEffectOperationPlan::StructuralScalarFieldStore(store) = operation {
                if let Some(root) =
                    crate::psi_lowering::structural_scalar_store_source::computation_root(
                        checked,
                        machine,
                        state.state,
                        store,
...
pub(super) fn selected_targets(
    ...
    let roots = selected_roots(checked, machine, states)?;
    crate::psi_lowering::scalar_call_closure::embedded::computation_targets(checked, &roots)
}
