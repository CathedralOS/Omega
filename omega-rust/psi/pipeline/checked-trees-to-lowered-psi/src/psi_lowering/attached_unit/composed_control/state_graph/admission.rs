...
pub(in crate::psi_lowering::attached_unit::composed_control) fn has_shared_graph_custody(
...
pub(in crate::psi_lowering::attached_unit) struct AdmittedGraph<'a> {
    pub(super) source_states: &'a [checked_trees::state::State],
    pub(in crate::psi_lowering::attached_unit::composed_control) boundaries:
        Vec<(&'a CheckedBoundaryMachinePlan, String)>,
    pub(in crate::psi_lowering::attached_unit::composed_control) internal_targets: Vec<(
        crate::psi_lowering::attached_unit::bodies::UnitBody<'a>,
        String,
    )>,
}

pub(in crate::psi_lowering::attached_unit::composed_control) fn admit<'a>(
    ...
    for (source, state) in source_states.iter().zip(&plan.states) {
        crate::psi_lowering::attached_unit::claims::validate_entry_claims(
            checked,
            plan.machine,
            source,
        ...
        if crate::psi_lowering::attached_unit::parameters::checked_scalar_source_parameters(
            checked, source,
    ...
    if let Some(attachment) = attachment {
        for state in &plan.states {
            for operation in &state.operations {
                crate::psi_lowering::attached_unit::provider_attachments::validate_call_source(
                    checked,
                    plan.machine,
                    state.state,
                    operation,
        ...
        let called = boundaries
            ...
            .collect::<Vec<_>>();
        crate::psi_lowering::attached_unit::provider_attachments::validate_provider_attachment_requirements(
            attachment,
            &plan.provider_attachment_requirements,
            &called,
        )?;
