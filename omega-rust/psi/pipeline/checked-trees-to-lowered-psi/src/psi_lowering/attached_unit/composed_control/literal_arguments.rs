...
pub(super) fn evaluate(
    ...
    let arguments = crate::psi_lowering::attached_unit::byte_subslices::arguments(operation);
    if !arguments
        .iter()
    ...
    let signature = crate::psi_lowering::call_source_custody::authored::target_signature(
        checked,
        machine,
        authored.source_target,
    )?;
    let source_positions =
        crate::psi_lowering::call_source_custody::literal_arguments::structural_positions(
            checked,
            &signature,
            arguments.len(),
        )?;
