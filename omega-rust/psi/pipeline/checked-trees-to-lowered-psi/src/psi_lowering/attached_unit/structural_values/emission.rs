    ...
    pub(super) fn value(
        ...
        match node.kind {
            ...
            CheckedStructuralValueKind::Case(construction) => {
                ...
                let slot = crate::psi_lowering::scalar_computations::cases::reserve(
                    self.checked,
                    construction.expression,
                    &source,
                ...
                let fields = crate::psi_lowering::scalar_computations::cases::fields(
                    self.checked,
                    &construction,
                )?
                .to_vec();
                ...
                crate::psi_lowering::scalar_computations::cases::emit(
                    &completed,
                    &self.values[field_start..],
                    self.next_value,
                    self.operations,
                )?;
                let declaration = crate::psi_lowering::scalar_computations::cases::declarations(
    ...
    fn rebind_local_cases(&mut self) -> Result<(), LoweringError> {
        ...
        for (symbol, argument) in &self.evaluation.structural_locals {
            ...
            if self.structural_types.iter().any(|declaration| {
                ...
                cases.push(
                    crate::psi_lowering::scalar_bindings::structural_cases::LocalCaseBinding::new(
                        self.checked,
                        *symbol,
                        local.type_reference,
                        argument.place,
