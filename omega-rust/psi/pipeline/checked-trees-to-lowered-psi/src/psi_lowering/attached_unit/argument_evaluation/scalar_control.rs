...
impl Evaluation {
    ...
    pub(crate) fn scalar_control_result(
        ...
        let mut expansion = crate::psi_lowering::scalar_computations::Expansion::new(
            checked,
            &qualifications,
            machine.machine,
            1,
        ...
        let terminator = match &control.terminator {
            CheckedScalarStateTerminator::Conditional {
                ...
                crate::psi_lowering::scalar_graph_lowering::guards::lower(
                    checked,
                    machine.state,
                    *guard_statement_ordinal,
                    &bindings,
                    ...
                    (
                        true_target,
                        crate::psi_lowering::scalar_computations::parameters(&source_types),
                    ),
                    (
                        false_target,
                        crate::psi_lowering::scalar_computations::parameters(&source_types),
            ...
            CheckedScalarStateTerminator::Guarded { arms, fallback } => {
                ...
                for (guard, target) in arms.iter().zip(targets).rev() {
                    let terminator = crate::psi_lowering::scalar_graph_lowering::guards::evaluate(
                        checked,
                        machine.state,
