...
use crate::psi_lowering::nonzero_divisor_certificate::produce_checked_canonical_integer_proof;
...
pub(super) fn retain_provable(lowered: &mut LoweredPsi) -> Result<(), LoweringError> {
    ...
    'discovery: loop {
        ...
        for mut candidate in candidates {
            ...
            {
                previous.predicate = crate::psi_lowering::contract_predicates::connective(
        ...
        for mut candidate in grouped {
            if let Some(existing) = module.scalar_block_invariants.iter_mut().find(|existing| {
                ...
                let predicate = crate::psi_lowering::contract_predicates::connective(
...
fn canonical_predicate(predicate: Proposition) -> Result<Proposition, LoweringError> {
    Ok(match predicate {
        Proposition::Equal(left, right) => {
            crate::psi_lowering::contract_predicates::canonical_equality(left, right)?
        }
...
fn canonical_members(
    ...
    for member in members {
        result = crate::psi_lowering::contract_predicates::connective(
            result,
