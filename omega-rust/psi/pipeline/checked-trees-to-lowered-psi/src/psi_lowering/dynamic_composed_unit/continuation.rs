...
pub(super) fn lower(
    ...
    let mut catalogs = crate::psi_lowering::attached_unit::lower_dynamic_control_catalogs(
        checked,
        plan,
        continuation,
        stored,
    ...
    for (state, block) in continuation.leaves.iter().zip(leaf_blocks) {
        let (leaf, mut occurrences) =
            crate::psi_lowering::attached_unit::emit_dynamic_control_leaf(
                checked,
                plan.caller_machine,
                state,
                block,
    ...
    let provider_boundaries = catalogs
        ...
        .collect::<Vec<_>>();
    let provider_places = crate::psi_lowering::attached_unit::lower_provider_attachment_places(
        caller_attachment,
        attachment,
