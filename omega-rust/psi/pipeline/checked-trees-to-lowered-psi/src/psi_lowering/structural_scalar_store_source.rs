...
pub(super) fn validate(
    ...
    crate::psi_lowering::attached_unit::primitive_locals::validate_roster(checked, plan)?;
    let (machine, state) =
    ...
    for (statement_index, statement) in statements.iter().enumerate() {
        ...
        if let [write] = writes.as_slice() {
            ...
            crate::psi_lowering::byte_sequence_write::validate_assignment(
                checked,
                plan.machine,
                plan.state,
                assignment,
        ...
        if let [store] = indexed_stores.as_slice() {
            ...
            crate::psi_lowering::structural_byte_sequence_index_store::validate_assignment(
                checked,
                plan.machine,
                plan.state,
                assignment,
        ...
        if let [store] = byte_stores.as_slice() {
            ...
            crate::psi_lowering::structural_byte_sequence_store::validate_assignment(
                checked,
                plan.machine,
                plan.state,
                assignment,
    ...
    for operation in &plan.operations {
        ...
        } = operation
        {
            if let checked_trees::CheckedPrimitiveStoreDestination::Local { symbol } = destination {
                crate::psi_lowering::attached_unit::primitive_locals::source(
                    checked,
                    plan,
                    *symbol,
                    *statement_index,
                )?;
                crate::psi_lowering::primitive_store::validate_symbol_assignment(
                    checked,
                    plan.state,
                    *statement_index,
                    *symbol,
