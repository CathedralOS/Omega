...
fn retains_and_replays_source_checked_transport_backed_lift_without_execution() {
    ...
    let language_semantics::quotient_correspondence::QuotientTheoremCorrespondence::ForwardPreconditionTransport(transport) =
    ...
    let bytes = terminal_codec::encode_module(&module).expect("encode transport row");
...
fn installs_complete_direct_define_batch_without_executable_authority() {
    ...
    assert_eq!(terminal_codec::decode_module(&bytes), Ok(module.clone()));
}
...
fn replay_failure_leaves_the_module_unchanged() {
    ...
    assert!(matches!(
        error,
        LoweringError::InvalidTerminalModule(terminal_verifier::ModuleError::UnknownEntryMachine(
            _
        ))
    ));
    assert_eq!(module, before);
