...
fn incomplete_schema_projection_prunes_forwarded_dependencies_to_a_fixed_point() {
    ...
    crate::psi_lowering::closed_reach_applications::prune_incomplete_closed_reach_applications(
        &mut unchanged,
    );
...
fn standalone_scalar_helpers_reject_reachful_or_missing_contracts() {
    ...
    let selected = checked
        ...
        .symbol;
    assert!(
        crate::psi_lowering::scalar_call_closure::embedded::EmbeddedScalarCalls::prepare_targets(
            &checked,
            &[selected],
            &[],
            0,
    ...
    assert!(
        crate::psi_lowering::scalar_call_closure::requires_shared_catalog(&checked, selected)
            .is_err()
    );
}
