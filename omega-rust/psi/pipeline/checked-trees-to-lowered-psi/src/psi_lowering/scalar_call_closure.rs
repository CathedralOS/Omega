...
pub(super) fn requires_shared_catalog(
    ...
    while let Some(machine) = pending.pop() {
        ...
        }) || !crate::psi_lowering::scalar_computations::structural_call_targets(
            checked, machine,
        )?
        .is_empty()
            || !crate::psi_lowering::scalar_computations::cases::type_roots(checked, machine)?
                .is_empty()
        {
...
pub(super) fn lower_scalar_call_closure(
    ...
    let qualifications =
        crate::psi_lowering::scalar_qualifications::PreparedScalarQualifications::prepare(
            checked, closure,
        )?;
