...
impl Context<'_> {
    ...
    fn computation(
        ...
        let valid = match &node.kind {
            Computation::StructuralField {
                ...
                crate::scalar_graph::scalar_source_custody::authored_state(self.checked, self.state)
                    .is_ok_and(|(machine, _)| {
                        crate::scalar_graph::scalar_computations::fields::validate_source(
                            self.checked,
                            machine.symbol,
                            self.state,
                            self.statement,
            ...
            Computation::CaseMembership {
                ...
                node.primitive_type == PrimitiveType::Bool
                    ...
                    .is_ok_and(|(machine, _)| {
                        crate::scalar_graph::scalar_computations::cases::source::membership(
                            self.checked,
                            machine.symbol,
                            self.state,
                            *source_expression,
    ...
    fn domain(&self, source: ExpressionHandle, depth: usize) -> Option<ArithmeticDomain> {
        ...
        match self.checked.expression_table.expression(source) {
            ...
            _ => {
                let (machine, state) = crate::scalar_graph::scalar_source_custody::authored_state(
                    self.checked,
                    self.state,
                )
                .ok()?;
