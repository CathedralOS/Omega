...
pub(super) fn validate(
    ...
    if argument.access != CheckedStructuralAccess::SharedBorrow || !argument.path.is_empty() {
        return unsupported("record operand changed its shared whole-place custody");
    }
    ...
    let named = match table.expression(expression) {
        ...
        _ => return unsupported("record operand has no whole named source"),
    };
    let ExpressionNode::Name(name) = table.expression(named) else {
        return unsupported("record operand has no exact named referent");
    };
    ...
    {
        return unsupported("record operand substituted its source identity");
    }
    ...
    if target.is_self {
        ...
        if (*self_symbol != target_owner.symbol
            ...
            || !call.has_receiver
            || call.receiver_symbol != name.symbol
        ...
        if accesses
            .iter()
            .any(|access| access.root_symbol == name.symbol && access.kind.is_exclusive())
        {
