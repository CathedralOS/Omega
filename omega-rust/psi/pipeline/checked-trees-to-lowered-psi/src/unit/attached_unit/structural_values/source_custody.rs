    ...
    while let Some((handle, expression, reference, source_arm, projected_leaf)) = pending.pop() {
        ...
        match node.kind.clone() {
            ...
            CheckedStructuralValueKind::Call { source_call } => {
                ...
                let exact = crate::unit::attached_unit::retain_exact_flow_call(
                    checked,
                    machine,
                    state,
                    *coordinate,
            ...
            CheckedStructuralValueKind::Case(construction) => {
                ...
                let fields = crate::scalar_graph::scalar_computations::cases::source::construction(
                    checked,
                    &construction,
                )?;
                    ...
                    match (&arm.pattern, &authored_arm.pattern) {
                        ...
                        (
                            ...
                            if matches!(primitive, PrimitiveType::F32 | PrimitiveType::F64) {
                                let occurrence =
                                    crate::scalar_graph::scalar_computations::comparisons::occurrence(
                                        checked,
                                        arm.equality_use,
                                        machine,
                                        state,
