...
fn forwarded_shared_view_chain_lowers_and_verifies() {
    ...
    // roster: the shared view formal arrives on the state's incoming edge as a
    // whole-parameter `SharedBorrow` transfer — the callee re-borrows the
    // source place instead of taking custody it never had.
    ...
    let lowered = lower_machine(&checked, TerminalMachineSelection::Name("scan"))
        .expect("forwarded shared-view chain composes and lowers");
    terminal_verifier::verify_module(
        &lowered.semantic_module,
        &lowered.proof_bundle,
        &proof_admission::AdmissionProfile::default(),
    )
    .expect("forwarded shared-view chain verifies");
}

#[test]
fn forwarded_shared_view_cycle_keeps_its_acyclic_frontier() {
    // The seek -> check -> seek backedge is a nested SCC that never reenters
    // the graph entry: cycle lowering still needs a ranked loop plan, which
    // only entry-backed cycles mint today.
    let checked = crate::front_end::checked_program(
        r#"
    ...
    let error = lower_machine(&checked, TerminalMachineSelection::Name("find_equals"))
        .expect_err("nested-SCC cycles keep their acyclic frontier decline");
    assert!(
        matches!(
            &error,
            crate::lowering_error::LoweringError::Unsupported(message)
                if message.contains("acyclic")
        ),
