...
fn primitive_reference_write_then_scalar_return_reaches_terminal() {
    ...
    let artifact = terminal_production::produce_terminal_artifact(&checked, "reset")
...
fn write_only_parameter_delivery_and_distinct_result_keep_dense_scalar_order() {
    ...
    let artifact = terminal_production::produce_terminal_artifact(&checked, "replace").unwrap();
    execute(
        &artifact,
...
fn attached_store_return_retains_its_exact_owner() {
    ...
    let artifact =
        terminal_production::produce_terminal_artifact(&checked, "First::reset").unwrap();
