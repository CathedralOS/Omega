...
pub(super) fn append_copied_field_predicates(
    ...
    if !matches!(source_place.root, psi_facts::PlaceRoot::Symbol(symbol) if symbol.is_valid())
        || !matches!(destination_place.root, psi_facts::PlaceRoot::Symbol(symbol) if symbol.is_valid())
    {
        return;
    }
    ...
    for fact in facts {
        ...
        let copied_place = semantic.append_place(psi_facts::Place {
...
fn stable_segment(segment: &psi_facts::PlaceSegment) -> bool {
    match segment {
