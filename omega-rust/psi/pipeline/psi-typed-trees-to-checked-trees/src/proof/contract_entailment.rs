use psi_checked_trees::{CheckedContractEntailmentAssumptionDischarge, MachineContractPlans};
use psi_core::{
...
use psi_validation::{ContractEntailmentStandDown, ContractEntailmentStandDownReason};
...
pub enum CheckedContractEntailmentAssumptionDischargeRecheckError {
    ...
    InvalidContext(psi_core::PropositionError),
    KernelRejected(psi_proof_admission::ProofError),
}
...
) -> Result<Vec<CheckedContractEntailmentAssumptionDischarge>, Vec<psi_diagnostics::Diagnostic>> {
    let stand_downs = psi_validation::collect_contract_entailment_stand_downs(program);
...
pub fn recheck_contract_entailment_assumption_discharge(
    ...
    let stand_down = psi_validation::collect_contract_entailment_stand_downs(program)
        .into_iter()
        .find(|stand_down| {
            stand_down.reason == ContractEntailmentStandDownReason::UnrecognizedInductiveBody
...
) -> Result<Option<ReconstructedDischarge>, CheckedContractEntailmentAssumptionDischargeRecheckError>
{
    ...
    let psi_typed_trees::domain::ProofFact::Expression(goal_expression) = fact else {
        return Err(
...
fn accept_reconstructed(
    ...
    let proof = psi_proof_admission::ProofNode {
        conclusion: reconstructed.certificate.goal().clone(),
        rule: psi_proof_admission::ProofRule::Assumption {
    ...
    psi_proof_admission::accept_certificate(
