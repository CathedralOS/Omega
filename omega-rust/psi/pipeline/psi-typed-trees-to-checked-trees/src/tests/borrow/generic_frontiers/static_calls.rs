...
fn stale_frontier_nodes_never_supply_an_empty_result_proof() {
    ...
    let stale = psi_arena::Handle::from_parts(unit.arena_index(), unit.generation() + 1);
...
fn closed_bindings_reject_missing_selection_conflicting_arguments_and_borrow_erasure() {
    ...
    let arguments = program
        ...
        .to_vec();
    let project = |program: &psi_typed_trees::TypedTrees, selected: &[_], arguments: &[_]| {
        psi_validation::closed_static_call_type_bindings(
    ...
    let borrowed = program.expression_table.insert(ExpressionNode::Borrow(
        psi_typed_trees::expression::TableBorrowExpression {
            target: arguments[0],
