...
fn write_only_nested_fixed_byte_array_field_call_retains_exact_ordered_field_path() {
    ...
    assert_eq!(
        ...
        "whole aggregate replacement must remain the exact ordered field path"
    );
}

#[test]
fn write_only_nested_fixed_byte_element_call_retains_fields_and_exact_index() {
    let source = r#"
        data Inner {
            bytes: [u8; 4];
            spare: u8;
...
#[test]
fn write_only_nested_fixed_byte_range_call_retains_fields_and_exact_window() {
    let source = r#"
        data Inner {
            bytes: [u8; 4];
            spare: u8;
