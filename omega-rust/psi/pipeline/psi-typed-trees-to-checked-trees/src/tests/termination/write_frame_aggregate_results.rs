...
fn aggregate_result_program(body: &str, extra: &str, scalar: &str) -> psi_typed_trees::TypedTrees {
    let source = format!(
        r#"
...
fn aggregate_helper_result_writes_invalidate_prior_arithmetic_facts() {
    for (name, body) in [
        ...
        match psi_validation::validate_program(&aggregate_result_program(body, "", "u8")) {
            Err(diagnostics)
