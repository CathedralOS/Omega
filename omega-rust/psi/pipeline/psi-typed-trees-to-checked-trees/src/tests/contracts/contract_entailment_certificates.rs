use super::*;
use psi_checked_trees::{CheckedContractEntailmentAssumptionDischarge, MachineContractCommitment};
use psi_core::Proposition;

fn checked_with_contracts(requires: &str, ensures: &str) -> psi_checked_trees::CheckedTrees {
    let source = format!(
        r#"
...
fn unrecognized_body_emits_kernel_checked_assumption_discharge() {
    ...
    let stand_downs = psi_validation::collect_contract_entailment_stand_downs(&checked.typed);
...
fn non_assumption_goal_remains_uncertified() {
    ...
    assert_eq!(
        psi_validation::collect_contract_entailment_stand_downs(&checked.typed).len(),
        1,
