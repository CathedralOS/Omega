...
fn place_segments(path: &[BorrowOwnerSegment]) -> Vec<psi_facts::PlaceSegment> {
    path.iter()
        .map(|segment| match segment {
            BorrowOwnerSegment::Field(symbol) => psi_facts::PlaceSegment::Field { symbol: *symbol },
            BorrowOwnerSegment::Case(variant) => {
                psi_facts::PlaceSegment::Case { variant: *variant }
            }
            BorrowOwnerSegment::FixedIndex(index) => {
                psi_facts::PlaceSegment::FixedIndex { index: *index }
            }
            BorrowOwnerSegment::DynamicIndex => psi_facts::PlaceSegment::Index {
