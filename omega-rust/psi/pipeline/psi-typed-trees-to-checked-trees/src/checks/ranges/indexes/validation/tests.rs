...
use psi_typed_trees::expression::{ExpressionNode, TableIndexedExpression};

fn fixture(
    collection_type: &str,
    access: &str,
) -> (psi_typed_trees::TypedTrees, TableIndexedExpression) {
    let source =
...
fn result(collection_type: &str, access: &str, prove_index: bool) -> BoundsCheckResult {
    ...
    let mut diagnostics = vec![psi_diagnostics::Diagnostic::error("earlier error")];
...
fn nested_index_traversal_checks_each_collection_extent() {
    for (access, accepted) in [("[3][1]", true), ("[4][1]", false), ("[3][2]", false)] {
        ...
        let frames = psi_validation::CallFrameResolver::new(&program);
