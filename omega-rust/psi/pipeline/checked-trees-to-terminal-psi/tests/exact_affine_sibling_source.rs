...
fn landed_affine_sibling_custody_crosses_source_codec_and_independent_verification() {
    ...
    assert!(
        terminal_verifier::verify_module(&decoded_module, &stale, &AdmissionProfile::default(),)
            .is_err(),
...
fn ten_definition_affine_divisor_remains_outside_the_source_frontier() {
    ...
    checked_trees_to_terminal_psi::lower_machine(
        &checked,
...
fn affine_to_partial_cast_exact_division_and_remainder_cross_source_codec_verification_and_interpretation()
 {
    ...
    let typed = lower_symbol_resolved_trees(&resolved).expect("type");
    let checked = lower_typed_trees(typed).expect("check");
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::divide")
...
fn partial_cast_to_affine_exact_division_and_remainder_cross_source_codec_verification_and_interpretation()
 {
    ...
    let typed = lower_symbol_resolved_trees(&resolved).expect("type");
    let checked = lower_typed_trees(typed).expect("check");
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::divide_after_cast")
...
fn affine_cast_affine_exact_division_and_remainder_cross_source_codec_verification_and_interpretation()
 {
    ...
    let checked = lower_typed_trees(typed).expect("check");
    let lowered =
        checked_trees_to_terminal_psi::lower_machine(&checked, "Root::divide_across_cast")
    ...
    assert!(
        checked_trees_to_terminal_psi::lower_machine(&checked, "Root::divide_across_cast").is_err(),
