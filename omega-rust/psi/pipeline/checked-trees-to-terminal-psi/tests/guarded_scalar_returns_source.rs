...
fn a_nonzero_guard_does_not_license_signed_division_overflow() {
    ...
    for form in [BranchForm::Separate, BranchForm::Combined] {
        let checked = checked_source(source, form);
        let result = checked_trees_to_terminal_psi::lower_machine(&checked, "value");
        assert!(
            matches!(
                result,
                Err(checked_trees_to_terminal_psi::LoweringError::OperationProofUnavailable(_))
            ),
...
fn branch_return_coordinates_cannot_select_a_siblings_valid_value() {
    ...
    for form in [BranchForm::Separate, BranchForm::Combined] {
        let checked = checked_source(source, form);
        checked_trees_to_terminal_psi::lower_machine(&checked, "value")
        ...
        for mutation in 0..3 {
            ...
            assert!(
                checked_trees_to_terminal_psi::lower_machine(&changed, "value").is_err(),
                "mutation {mutation}"
            );
        }
    }
