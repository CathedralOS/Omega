...
use terminal_psi::{EvidenceContractLaneKind, EvidenceTermDeclaration, OperationKind};
...
fn source_projection_uses_canonical_term_and_exact_requirement_identity() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::project")
...
fn source_producer_provenance_is_separate_canonical_verified_proof_data() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::produce")
...
fn argumented_proof_output_retains_substitution_and_erased_input_identity() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::relay")
...
fn generic_proof_output_target_identity_binds_the_closed_conformance_application() {
    ...
    let first_lowered = checked_trees_to_terminal_psi::lower_machine(&first, "Root::relay")
        .expect("first closed generic proof output should lower");
    let first_replay = checked_trees_to_terminal_psi::lower_machine(&first, "Root::relay")
        .expect("the same exact specialization should lower deterministically");
    let second_lowered = checked_trees_to_terminal_psi::lower_machine(&second, "Root::relay")
    ...
    assert_eq!(
        checked_trees_to_terminal_psi::lower_machine(&changed_exact_specialization, "Root::relay",),
    ...
    assert_eq!(specialization.report_fingerprint, retained_report);
    assert_eq!(
        checked_trees_to_terminal_psi::lower_machine(&substituted_commitment, "Root::relay"),
        Err(checked_trees_to_terminal_psi::LoweringError::Unsupported(
            "evidence machine specialization commitment does not replay",
        )),
...
fn static_requirement_proof_output_keeps_public_identity_and_private_dispatch_separate() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, &machine_name)
    ...
    let baseline =
        checked_trees_to_terminal_psi::lower_machine(&baseline_checked, &baseline_machine_name)
...
fn static_requirement_i32_result_uses_one_ordinary_scalar_call_without_runtime_overhead() {
    ...
    let baseline = checked_trees_to_terminal_psi::lower_machine(&baseline_checked, &baseline_name)
...
fn ordinary_attached_scalar_machine_remains_outside_the_scalar_entry_lane() {
    ...
    assert!(matches!(
        checked_trees_to_terminal_psi::lower_machine(&checked, &selection.name),
        Err(checked_trees_to_terminal_psi::LoweringError::Unsupported(_))
    ));
}
...
fn runtime_value_proof_output_links_one_scalar_call_and_executes_once() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "relay")
    ...
    let baseline = checked_trees_to_terminal_psi::lower_machine(&baseline, "relay")
    ...
    let proof_only =
        checked_trees_to_terminal_psi::lower_machine(&check(PROOF_OUTPUT_SOURCE), "Root::relay")
