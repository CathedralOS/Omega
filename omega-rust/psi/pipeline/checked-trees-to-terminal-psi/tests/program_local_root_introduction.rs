...
fn lowered_source(source: &str) -> checked_trees_to_terminal_psi::LoweredTerminalPsi {
    let tokens = Lexer::new(source).tokenize().expect("tokenize");
    let syntax = parse_syntax_trees(&tokens).expect("parse");
    let resolved = lower_syntax_trees(&syntax).expect("resolve");
    let typed = lower_symbol_resolved_trees(&resolved).expect("type");
    let checked = lower_typed_trees(typed).expect("check");
    checked_trees_to_terminal_psi::lower_machine(&checked, "Root::run")
        .expect("lower program-local introduction schema")
}

fn lowered() -> checked_trees_to_terminal_psi::LoweredTerminalPsi {
