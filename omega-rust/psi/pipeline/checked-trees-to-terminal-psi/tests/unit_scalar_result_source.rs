...
fn rejection_message(checked: &checked_trees::CheckedTrees) -> &'static str {
    match checked_trees_to_terminal_psi::lower_machine(checked, "Main::main") {
        Err(checked_trees_to_terminal_psi::LoweringError::Unsupported(message)) => message,
...
fn attached_unit_scalar_boundary_result_reaches_later_call_in_terminal_psi() {
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked(), "Main::main")
