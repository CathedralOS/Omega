...
fn two_element_affine_array_cleanup_crosses_source_codec_verifier_and_interpreter() {
    ...
    for (machine, moved, residual) in [("Root::first", 0, 1), ("Root::second", 1, 0)] {
        let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, machine)
...
fn affine_triple_residuals_follow_the_exact_decreasing_live_index_order() {
    ...
    assert!(checked_trees_to_terminal_psi::lower_machine(&checked, "Root::all").is_err());

    let one = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::one")
