...
fn source_unit_retains_ordered_empty_affine_local_cleanup() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::cleanup")
...
fn source_unit_construction_prefix_reaches_verified_interpreted_terminal_psi() {
    ...
    let typed = lower_symbol_resolved_trees(&resolved).expect("type");
    let checked = lower_typed_trees(typed).expect("check");
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::cleanup_prefix")
...
fn wider_construction_prefixes_replay_codec_order_mutations_and_exact_fuel() {
    for (source, prefix_length, root_length) in [
        ...
        let checked = lower_typed_trees(typed).expect("check");
        let lowered =
            checked_trees_to_terminal_psi::lower_machine(&checked, "Root::cleanup_prefix")
...
fn result_bearing_boundary_rejects_missing_canonical_contract_custody() {
    ...
    assert_eq!(
        checked_trees_to_terminal_psi::lower_machine(&checked, "Root::enter"),
        Err(checked_trees_to_terminal_psi::LoweringError::Unsupported(
...
fn source_content_custody_unit_exit_retains_projection_and_consumes_claim() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::exit")
...
fn whole_root_source_passthrough_reaches_verified_and_interpreted_terminal_psi() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Main::forward")
...
fn direct_internal_structural_result_call_gets_an_exact_checked_plan() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Main::through_call")
...
fn structural_return_discards_one_claim_free_affine_parameter_after_materialization() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Main::forward_and_drop")
...
fn structural_return_establishes_and_discards_one_trivial_affine_local() {
    ...
    let lowered =
        checked_trees_to_terminal_psi::lower_machine(&checked, "Main::forward_with_local")
...
fn structural_return_establishes_multiple_locals_in_declaration_order_and_discards_in_reverse() {
    ...
    let lowered =
        checked_trees_to_terminal_psi::lower_machine(&checked, "Main::forward_with_two_locals")
...
fn structural_return_cleans_local_before_affine_parameter() {
    let checked = checked_source();
    let lowered =
        checked_trees_to_terminal_psi::lower_machine(&checked, "Main::forward_with_local_and_drop")
...
fn structural_return_cleans_locals_then_every_affine_tail_parameter_in_reverse_order() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(
        &checked,
...
fn producer_fences_locals_and_authored_contracts() {
    ...
    assert!(checked_trees_to_terminal_psi::lower_machine(&checked, "Main::through_local").is_err());
    assert!(checked_trees_to_terminal_psi::lower_machine(&checked, "Main::contracted").is_err());
    for rejected in [
        ...
        assert!(
            checked_trees_to_terminal_psi::lower_machine(&checked, rejected).is_err(),
...
fn lowering_rejects_a_stale_checked_claim_join() {
    ...
    assert!(matches!(
        checked_trees_to_terminal_psi::lower_machine(&checked, "Main::forward"),
        Err(checked_trees_to_terminal_psi::LoweringError::Unsupported(
...
fn lowering_rejects_stale_structural_return_cleanup_coordinates() {
    ...
    assert!(matches!(
        checked_trees_to_terminal_psi::lower_machine(&checked, "Main::forward_and_drop"),
        Err(checked_trees_to_terminal_psi::LoweringError::Unsupported(
            "structural result plan is not one exact whole-root linear transfer with affine cleanup"
        ))
    ));
...
fn lowering_rejects_stale_affine_local_declaration_and_cleanup_rows() {
    ...
    assert!(
        checked_trees_to_terminal_psi::lower_machine(&checked, "Main::forward_with_local").is_err()
    );
