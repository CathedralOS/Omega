...
fn verifier_rejects_unknown_direct_boolean_member_identity() {
    ...
    let typed = lower_symbol_resolved_trees(&resolved).expect("type");
    let checked = lower_typed_trees(typed).expect("check");
    let mut lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::enter")
        .expect("direct Boolean member crash route lowers");
...
fn exact_member_division_and_remainder_rebase_safe_literals_end_to_end() {
    ...
    let mut unsafe_divisor =
        checked_trees_to_terminal_psi::lower_machine(&checked, "Helper::inspect")
    ...
    *predicate = CrashPredicateTerm::new(proposition);
    ...
    let runtime_divisor = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::enter")
...
fn nested_payload_sum_equality_retains_exact_record_case_payload_paths_end_to_end() {
    ...
    let typed = lower_symbol_resolved_trees(&resolved).expect("type");
    let checked = lower_typed_trees(typed).expect("check");
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Whole::enter")
...
fn mixed_aggregate_equality_retains_common_fields_cases_and_call_rebasing_end_to_end() {
    ...
    let typed = lower_symbol_resolved_trees(&resolved).expect("type");
    let checked = lower_typed_trees(typed).expect("check");
    let equal = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::enter")
        .expect("mixed equality lowers through the direct call");
    let different = checked_trees_to_terminal_psi::lower_machine(&checked, "Different::enter")
...
fn unsupported_mixed_aggregate_equality_shapes_remain_fenced() {
        ...
        .chain(NESTED_MIXED_AGGREGATE_EQUALITY_FENCE_SOURCES)
    {
        ...
        let result = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::enter");
        assert!(matches!(
            result,
...
fn ieee_float_aggregate_equality_is_atomic_and_canonical_end_to_end() {
    ...
    let reversed = checked_trees_to_terminal_psi::lower_machine(&checked, "Reverse::enter")
    ...
    let aggregate_different =
        checked_trees_to_terminal_psi::lower_machine(&checked, "AggregateDifferent::enter")
    ...
    *predicate = CrashPredicateTerm::new(proposition);
    ...
    let projected_different =
        checked_trees_to_terminal_psi::lower_machine(&checked, "ProjectedDifferent::enter")
...
fn byte_sequence_aggregate_equality_is_content_atomic_end_to_end() {
    ...
    let bounded = checked_trees_to_terminal_psi::lower_machine(&checked, "BoundedRoot::enter")
