...
fn selected_witness_tail_use_is_canonical_and_runtime_free() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::caller")
    ...
    *source = result;
    ...
    omitted_checked_use
        ...
        .tail_use = None;
    assert!(
        checked_trees_to_terminal_psi::lower_machine(&omitted_checked_use, "Root::caller").is_err()
    );
}
...
fn guarded_payloadless_source_call_rejoins_selected_evidence_and_uses_four_fuel() {
    ...
    lost_selection
        ...
        .selected_evidence
        .clear();
    assert!(checked_trees_to_terminal_psi::lower_machine(&lost_selection, "Root::caller").is_err());
    ...
    assert!(checked_trees_to_terminal_psi::lower_machine(&wrong_arm, "Root::caller").is_err());
    ...
    assert!(
        checked_trees_to_terminal_psi::lower_machine(&wrong_guarantee, "Root::caller").is_err()
    );
    ...
    assert!(checked_trees_to_terminal_psi::lower_machine(&wider_validity, "Root::caller").is_err());
}
...
fn omitted_guarded_selector_retains_fact_only_callee_without_runtime_delta() {
    let omitted = checked_trees_to_terminal_psi::lower_machine(
    ...
    let selected =
        checked_trees_to_terminal_psi::lower_machine(&checked(GUARDED_CALL_SOURCE), "Root::caller")
...
fn exact_payloadless_case_return_is_canonical_verified_and_executable() {
    ...
    let lowered = checked_trees_to_terminal_psi::lower_machine(&checked, "Root::choose")
...
fn guarded_payloadless_case_return_retains_active_evidence_and_vacuous_siblings() {
    let baseline = checked_trees_to_terminal_psi::lower_machine(&checked_source(), "Root::choose")
