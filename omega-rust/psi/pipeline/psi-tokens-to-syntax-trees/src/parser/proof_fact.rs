...
use psi_syntax_trees::item::{ProofFact, ProofMembershipFact};
use psi_tokens::PunctuationKind;

fn copy_item_path_to_expression_path(
    syntax_trees: &mut SyntaxTrees,
    span: HandleSpan<psi_syntax_trees::identifier::Identifier>,
...
fn expand_carry_portable_item_paths(
    syntax_trees: &mut SyntaxTrees,
    domains: Vec<HandleSpan<psi_syntax_trees::identifier::Identifier>>,
) -> Vec<HandleSpan<psi_syntax_trees::identifier::Identifier>> {
    ...
    for domain in domains {
        ...
        for permission in psi_language_core::CarryPermission::ALL {
            let mut parts = permission.name().split("::");
            let namespace = syntax_trees.items.append_identifier_path_member(
                psi_syntax_trees::identifier::Identifier::generated(
