...
pub(crate) fn build_checked_scalar_graph_plans_with_call_frames(
    ...
    let resolved_arguments = machines
        .iter()
        .map(|graph| {
            successors::resolve_arguments(program, proof_terms, &machines, graph)
        })
        .collect::<Vec<_>>();
        ...
        .map(|graph| {
            ...
            graph.states.iter().all(|retained| {
                ...
                let Some((owner, state)) =
                    crate::semantic_calls::find_state_with_machine(program, retained.state)
                else {
                    return false;
                };
...
fn checked_successor(
    ...
    let target = if let Some(target_index) =
        ...
        crate::semantic_calls::find_machine_by_entry_state(program, path.symbol)?.1
            .symbol
    };
