...
pub(super) fn landed_exact_shift_literal_count(
    ...
    let landing = match literal.landing().map(|landing| landing.landed_type) {
        Some(numerics::literals::LandedIntegerType::I8)
        | Some(numerics::literals::LandedIntegerType::I16)
        | Some(numerics::literals::LandedIntegerType::I32)
        | Some(numerics::literals::LandedIntegerType::I64) => literal
            .value_i64()
            .and_then(|value| u64::try_from(value).ok()),
        Some(numerics::literals::LandedIntegerType::U8)
        | Some(numerics::literals::LandedIntegerType::U16)
        | Some(numerics::literals::LandedIntegerType::U32)
        | Some(numerics::literals::LandedIntegerType::U64) => literal.value_u64(),
        _ => return None,
    };
