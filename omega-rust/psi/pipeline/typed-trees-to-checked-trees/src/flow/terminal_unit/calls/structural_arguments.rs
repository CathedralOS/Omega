...
use crate::flow::terminal_unit::byte_subslice;
use crate::flow::terminal_unit::calls::argument_paths::{
    byte_sequence_literal_argument, projected_argument_path, projected_argument_path_with_identity,
};
use crate::flow::terminal_unit::calls::boundary_admission::{
    boundary_argument_presentation_is_admitted, fixed_byte_array_mutable_view_is_admitted,
    is_registered_boundary_scalar_target,
};
use crate::flow::terminal_unit::calls::computation_arguments;
use crate::flow::terminal_unit::calls::reference_forwarding;
...
pub(crate) fn structural_call_arguments(
    ...
    for target in target_parameters {
            ...
            if !target_machine.supply_mode.is_boundary_declaration()
                && (target_machine.supply_mode != MachineSupplyMode::CheckedBody
                    || (!is_unit(program, target_state.return_type)
                        ...
                        && crate::flow::terminal_unit::control::checked_structural_result_type(
                            program,
        ...
        if crate::flow::terminal_unit::primitive_store::primitive_local_before(
            program,
            caller_state,
            statement_index,
            source_symbol,
...
pub(crate) fn exact_structural_argument_access(
    ...
    let returned_loans = crate::find_state(program, state)
        .and_then(|source_state| {
            ...
            if crate::flow::terminal_unit::reference_results::is_reference_record(
                program,
                ...
                return crate::flow::terminal_unit::reference_results::local_record_loans(
                    program,
                    facts,
                    machine,
                    source_state,
            ...
            crate::flow::terminal_unit::reference_results::result_loan(
                program,
                facts,
                machine,
                source_state,
