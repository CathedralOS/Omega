...
pub(in crate::flow) fn build_call_operation(
    ...
    if if boundary {
        ...
        match &expected_call_result {
            ...
            Some(ExpectedCallValueResult::Structural(expected))
                if crate::flow::terminal_unit::reference_results::parts(
                    program,
    ...
    if boundary {
        ...
        let reference_loan = if crate::flow::terminal_unit::reference_results::parts(
            program,
            target_state.return_type,
        )
        .is_some()
        ...
        if structural_arguments
            ...
            && (reference_loan.is_valid()
                || crate::flow::terminal_unit::reference_results::is_reference_record(
                    program,
                    target_state.return_type,
                )
                || (matches!(
