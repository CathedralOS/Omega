...
fn exact_static_data_receiver(
    ...
    if selected.is_valid() && program.symbols.get(selected).kind == symbols::SymbolKind::Data {
        ...
        )
    {
        ...
        if program.symbols.get(candidate).kind != symbols::SymbolKind::Data
...
fn collection_view_operation_for_receiver(
    program: &TypedTrees,
    operation: language_semantics::declaration_selection::CollectionViewOperation,
    receiver: InferredType,
) -> Option<language_semantics::declaration_selection::CollectionViewOperation> {
