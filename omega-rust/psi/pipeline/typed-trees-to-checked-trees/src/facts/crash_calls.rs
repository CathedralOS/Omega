...
struct CallArgumentSubstitution {
    ...
    scalar: Vec<Option<checked_trees::CheckedScalarExpression>>,
}
...
impl SummaryCrashBucket {
    fn unconditional(cause: checked_trees::CrashCause) -> Self {
        Self {
            cause,
    ...
    fn into_checked(self) -> Option<checked_trees::CrashRouteBucket> {
        ...
        for guard in bucket.alternative_guards() {
            match guard {
                ...
                checked_trees::CrashRouteGuard::Predicate(identity) => {
                    ...
                    let entry_predicate = super::crash_entry_values::substitute_entry(
                        &predicate,
