                ...
                for constraint in constraints {
                    ...
                    if let typed_trees::types::TypeConstraintNode::Domain(domain) = constraint
                        && let Some((minimum, maximum)) =
                            declared_domain_predicate_bounds(program, domain)
                    {
...
fn declared_domain_predicate_bounds(
    program: &typed_trees::TypedTrees,
...
fn self_predicate_bounds(
    ...
    let Some(value) = validation::closed_integer_range_bound(program, literal) else {
        return Vec::new();
    };
