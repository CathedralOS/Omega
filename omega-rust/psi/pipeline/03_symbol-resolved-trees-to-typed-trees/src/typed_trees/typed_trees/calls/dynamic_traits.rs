...
use crate::typed_trees::expression::{
    ExpressionHandle, ExpressionNode, MatchPattern, StaticMachineArgument,
};
use crate::typed_trees::name::Identifier;
use crate::typed_trees::signature::StateSignature;
use crate::typed_trees::trait_definition::{DynamicSignatureIneligibility, TraitDefinition};
    ...
    pub fn dynamic_signature_eligibility(
        ...
        if !self.state_signature_type_parameters(requirement).is_empty()
            && !matches!(
                ...
                crate::typed_trees::finite_family::FamilyProbe::Finite { .. }
            )
        {
...
fn collect_type_reference_symbols(
    ...
    match program.type_reference_table.type_reference(type_reference) {
        ...
        TypeReferenceNode::FixedArray {
            ...
            if let crate::typed_trees::types::FixedArrayLength::ConstParameter { symbol, .. } =
                length
            {
                symbols.push(*symbol);
            }
...
mod tests {
    ...
    use crate::typed_trees::TypedTrees;
    use crate::typed_trees::domain::ProofFact;
    use crate::typed_trees::domain::ProofMembershipFact;
    use crate::typed_trees::name::Identifier;
    use crate::typed_trees::signature::StateSignature;
    use crate::typed_trees::signature::{
        NativeCallbackParameter, SignatureContract, StateParameter,
    };
    ...
    fn contract_naming_a_private_domain_is_not_surface() {
        let (program, trait_definition) = requirement(|program, signature| {
            let domain = crate::typed_trees::domain::DomainDefinition {
                symbol: SymbolHandle::from_arena_index(40),
                is_public: false,
                ..crate::typed_trees::domain::DomainDefinition::default()
            };
            program.push_domain_definition(domain);
            let mut contract = SignatureContract {
                kind: crate::typed_trees::signature::SignatureContractKind::Requires,
