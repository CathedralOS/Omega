...
impl TypedTrees {
    ...
    pub fn finite_signature_family(
        &self,
        signature: &crate::typed_trees::signature::StateSignature,
    ) -> FamilyProbe {
            ...
            .filter(|parameter| {
                matches!(
                    ...
                    crate::typed_trees::data::TypeParameterKind::Const { .. }
                        | crate::typed_trees::data::TypeParameterKind::Value { .. }
                )
            })
            .collect::<Vec<_>>();
        ...
        let crate::typed_trees::domain::ProofFact::Expression(root) = fact else {
            return FamilyProbe::NotFinite(
    ...
    fn or_alternatives(
        &self,
        expression: crate::typed_trees::expression::ExpressionHandle,
    ) -> Vec<crate::typed_trees::expression::ExpressionHandle> {
        let mut alternatives = Vec::new();
        let mut stack = vec![expression];
        while let Some(current) = stack.pop() {
            match self.expression_table.expression(current) {
                crate::typed_trees::expression::ExpressionNode::Binary(binary)
                    if binary.operator == crate::typed_trees::expression::BinaryOperator::Or =>
                {
    ...
    fn and_conjuncts(
        ...
        while let Some(current) = stack.pop() {
            match self.expression_table.expression(current) {
                crate::typed_trees::expression::ExpressionNode::Binary(binary)
                    if binary.operator == crate::typed_trees::expression::BinaryOperator::And =>
                {
                    stack.push(binary.right);
                    stack.push(binary.left);
                }
    ...
    fn equality_assignment(
        &self,
        binders: &[&crate::typed_trees::data::TypeParameter],
        expression: crate::typed_trees::expression::ExpressionHandle,
    ) -> Option<(usize, FamilyValue)> {
        let crate::typed_trees::expression::ExpressionNode::Binary(binary) =
            self.expression_table.expression(expression)
        else {
            return None;
        };
        if binary.operator != crate::typed_trees::expression::BinaryOperator::Equal {
            return None;
        }
        ...
        let carrier = match &binders[binder].kind {
            crate::typed_trees::data::TypeParameterKind::Const { type_reference }
            | crate::typed_trees::data::TypeParameterKind::Value { type_reference } => {
                *type_reference
            }
            _ => return None,
        };
    ...
    fn literal_operand(
        ...
        match self.expression_table.expression(expression) {
            ...
            crate::typed_trees::expression::ExpressionNode::Boolean(value) => Some(FamilyValue {
    ...
    fn value_fits_carrier(
        ...
        primitive == crate::typed_trees::types::PrimitiveType::Bool
    }
}
...
fn primitive_integer_range(
    primitive: crate::typed_trees::types::PrimitiveType,
) -> Option<(i128, i128)> {
