...
use crate::typed_trees::expression::{ExpressionNode, ExpressionTable};
...
fn qualified_result_lookup_preserves_the_exact_predicate_target() {
    ...
    let bounds = types.insert_constraints([TypeConstraintNode::Range {
        minimum: crate::typed_trees::expression::ExpressionHandle::from_parts(1, 0),
        maximum: crate::typed_trees::expression::ExpressionHandle::from_parts(2, 0),
        end_inclusive: true,
    }]);
...
fn arithmetic_result_lookup_requires_exact_carrier_and_policy_only_shape() {
    ...
    let constraints = types.insert_constraints([
        ...
        TypeConstraintNode::Range {
            minimum: crate::typed_trees::expression::ExpressionHandle::invalid(),
            maximum: crate::typed_trees::expression::ExpressionHandle::invalid(),
            end_inclusive: true,
        },
    ]);
...
fn type_reference_table_copies_table_payloads_without_tree_roundtrip() {
    ...
    let semantic_id = language_semantics::SemanticDomainId(9);
    let predicate_body = language_semantics::DomainPredicateBody::Present;
...
fn type_reference_symbol_remap_reaches_nested_types_and_constraints() {
    ...
    let subject = expressions.insert(ExpressionNode::Name(
        crate::typed_trees::expression::TableNamePath {
            members,
            member_symbols,
            head_symbol: old,
            symbol: old,
