...
use crate::typed_trees::domain::ProofFact;
use crate::typed_trees::machine::{Machine, OwnedData};
use crate::typed_trees::signature::{StateParameter, StateSignature};
use crate::typed_trees::state::State;
use crate::typed_trees::trait_definition::TraitDefinition;
...
pub(crate) fn conformance_snapshot(
    program: &TypedTrees,
    conformance: &crate::typed_trees::trait_definition::Conformance,
) -> ConformanceSnapshot {
    let (implementation, rows) = match &conformance.implementation {
        ...
        crate::typed_trees::trait_definition::ConformanceImplementation::Closed { rows } => (
            "closed",
            rows.iter()
                .map(|row| ConformanceRowSnapshot {
                    ...
                    source: match row.source {
                        crate::typed_trees::trait_definition::ConformanceRowSource::Inline => "inline",
                        crate::typed_trees::trait_definition::ConformanceRowSource::Reference => "reference",
...
fn signature_contract_snapshot(
    program: &TypedTrees,
    contract: &crate::typed_trees::signature::SignatureContract,
) -> SignatureContractSnapshot {
    let (kind, crash_cause) = match &contract.kind {
        crate::typed_trees::signature::SignatureContractKind::Requires => ("requires", None),
        crate::typed_trees::signature::SignatureContractKind::Ensures => ("ensures", None),
        crate::typed_trees::signature::SignatureContractKind::EnsuresForResultCase { .. } => {
            ("ensures_for_result_case", None)
        }
        crate::typed_trees::signature::SignatureContractKind::Crashes { cause } => {
            return SignatureContractSnapshot {
                kind: "crashes",
                binding: contract.binding.as_ref().map(ToString::to_string),
                crash_cause: Some(match cause {
                    crate::typed_trees::signature::CrashCause::Trap => "Trap",
                    crate::typed_trees::signature::CrashCause::Abort => "Abort",
                }),
