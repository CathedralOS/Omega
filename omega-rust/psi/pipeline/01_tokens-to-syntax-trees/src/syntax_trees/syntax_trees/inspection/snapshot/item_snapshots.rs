    ...
    match item {
        ...
        Item::Conformance(value) => ItemSnapshot::Conformance {
            ...
            type_name: match &value.subject {
                crate::syntax_trees::item::ConformanceSubject::Carrier(type_name) => {
                    Some(snapshot_identifier(type_name))
                }
                crate::syntax_trees::item::ConformanceSubject::Subjectless => None,
            },
            subjectless: matches!(
                value.subject,
...
fn snapshot_operator(
    ...
    operator: &crate::syntax_trees::item::OperatorDefinition,
) -> OperatorSnapshot {
    OperatorSnapshot {
...
pub(crate) fn snapshot_type_parameter(
    syntax_trees: &SyntaxTrees,
    parameter: &crate::syntax_trees::item::TypeParameter,
) -> TypeParameterSnapshot {
    let (kind, const_type, machine_contract, machine_requirement, proposition_contract) =
        match &parameter.kind {
            crate::syntax_trees::item::TypeParameterKind::Type => ("type", None, None, None, None),
            ...
            crate::syntax_trees::item::TypeParameterKind::Machine { contract } => match contract {
                Some(crate::syntax_trees::item::MachineParameterContract::RequirementIdentity) => {
                    ("machine_requirement", None, None, None, None)
                }
                Some(crate::syntax_trees::item::MachineParameterContract::Structural(
                    signature,
                )) => (
                    "machine",
                    None,
                ...
                Some(crate::syntax_trees::item::MachineParameterContract::Nominal {
                    requirement,
                }) => (
                    "machine",
                    None,
...
fn snapshot_data_properties(
    properties: crate::syntax_trees::item::DataProperties,
    ...
    CapabilityContractSnapshot {
        kind: match &contract.kind {
            ...
            CapabilityContractKind::Crashes { cause } => CapabilityContractKindSnapshot::Crashes {
                cause: match cause {
                    crate::syntax_trees::item::CrashCause::Trap => "Trap",
                    crate::syntax_trees::item::CrashCause::Abort => "Abort",
                },
            },
        },
