...
const NESTED_AFFINE_NONET_SOURCE: &str = r#"
    data Token { value: u64; }
    data Sink {}
    machine Sink::take(token: Token) {}
    data Root {}
    machine Root::enter(values: [[Token; 9]; 2]) {
...
const NESTED_AFFINE_ELEVEN_SOURCE: &str = r#"
    data Token { value: u64; }
    data Sink {}
    machine Sink::take(token: Token) {}
    data Root {}
    machine Root::enter(values: [[Token; 11]; 2]) {
...
const NESTED_AFFINE_THIRTEEN_SOURCE: &str = r#"
    data Token { value: u64; }
    data Sink {}
    machine Sink::take(token: Token) {}
    data Root {}
    machine Root::enter(values: [[Token; 13]; 2]) {
        Sink::take(values[1][12]);
        Sink::take(values[0][1]);
    }
"#;

const NESTED_AFFINE_FOURTEEN_SOURCE: &str = r#"
    data Token { value: u64; }
    data Sink {}
    machine Sink::take(token: Token) {}
    data Root {}
    machine Root::enter(values: [[Token; 14]; 2]) {
        Sink::take(values[1][13]);
        Sink::take(values[0][1]);
    }
"#;

const NESTED_AFFINE_FIFTEEN_SOURCE: &str = r#"
    data Token { value: u64; }
    data Sink {}
    machine Sink::take(token: Token) {}
    data Root {}
    machine Root::enter(values: [[Token; 15]; 2]) {
...
fn two_element_affine_array_cleanup_crosses_source_codec_verifier_and_interpreter() {
    ...
    for (machine, moved, residual) in [("Root::first", 0, 1), ("Root::second", 1, 0)] {
        ...
        let terminal_psi::StructuralTypeShape::FixedArray { length, .. } = &mut declaration.shape
        else {
            unreachable!()
        };
        *length = 3;
    ...
    for (machine, expected_paths, residual) in [
        ...
        *length = 2;
        ...
        callee_contract
            ...
            .contract
            .requires
            .push(semantic_vocabulary::Proposition::Truth);
        assert!(
            terminal_verifier::verify_module(
                &callee_contract,
                &lowered.proof_bundle,
...
fn assert_nested_affine_array_cleanup_crosses_source_codec_verifier_and_interpreter(
    ...
    let terminal_psi::StructuralTypeShape::FixedArray { element: inner, .. } = outer.shape else {
        unreachable!()
    };
    ...
    let terminal_psi::StructuralTypeShape::FixedArray { length, .. } = &mut inner.shape else {
        unreachable!()
    };
    *length = match *length {
        3 => 4,
...
fn direct_field_partial_affine_cleanup_crosses_source_codec_verifier_and_interpreter() {
    ...
    let terminal_psi::StructuralTypeShape::Record { fields } = &root_shape.shape else {
    ...
    let root_shape = scalar_as_structural
        ...
        .find(|shape| shape.id == root.structural_type)
        .expect("mixed scalar/affine root shape");
    let terminal_psi::StructuralTypeShape::Record { fields } = &mut root_shape.shape else {
        unreachable!()
    };
