...
//! `atomic_lets.rs` parses the atomic compare-exchange, fetch, swap and
//! store forms, `destructure_and_proof_output.rs` parses destructuring and
...
use crate::bodies::statements::discard_and_local_data::{
    ...
    parse_if_transition_statement_handle, parse_local_data_statement_handle,
};
...
pub(crate) fn parse_statement_handle<'tokens, 'source>(
    syntax_trees: &mut SyntaxTrees,
    input: Input<'tokens, 'source>,
