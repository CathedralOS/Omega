use crate::{CheckedBoundaryOperatorApplicationScope, finalize_terminal_artifact};
...
use crate::terminal_production::stage_timings::{
...
pub struct TerminalProductionRequest<'a> {
    ...
    pub optimization_selections: terminal_codec::optimization::PsiOptimizationSelections,
}
    ...
    ) -> Result<(Option<LoweredPsi>, PsiOptimizationStageResult), TerminalArtifactProductionError>
    {
        timings
            .record_result(TerminalProductionStage::LedgerCheck, || {
                crate::terminal_production::checked_ledger::verify(self.checked)
            })
            .map_err(TerminalArtifactProductionError::Lowering)?;
        let lowered = timings
...
fn checked_boundary_operator_scope(
    ...
    crate::checked_boundary_operator_scope(checked, artifact, lowered)
...
mod tests {
    ...
    fn rebound_dynamic_produced(
        ...
        crate::terminal_production::ProducedTerminalArtifact<()>,
    ) {
    ...
    /// again.
    fn retained_lowered(
        produced: &crate::terminal_production::ProducedTerminalArtifact<()>,
    ...
    fn a_rebound_dynamic_call_rejoins_one_exact_descriptor_table_occurrence() {
        use crate::CheckedDynamicCallLane;
        ...
        let replayed =
            crate::checked_boundary_operator_scope(&checked, produced.artifact(), &lowered)
    ...
    fn a_stale_or_duplicated_dynamic_occurrence_rejects() {
        ...
        let replay = |corrupted: &checked_trees_to_lowered_psi::lowered_psi::LoweredPsi| {
            crate::checked_boundary_operator_scope(&checked, produced.artifact(), corrupted)
                .unwrap_err()
        };
    ...
    fn a_substituted_dynamic_selection_identity_rejects() {
        let (checked, produced) = rebound_dynamic_produced(REBOUND_DYNAMIC_SOURCE);
        let lowered = retained_lowered(&produced);
        let replay = |corrupted: &typed_trees_to_checked_trees::checked_trees::CheckedTrees| {
            crate::checked_boundary_operator_scope(corrupted, produced.artifact(), &lowered)
                .unwrap_err()
        };
        ...
        ) -> &mut typed_trees_to_checked_trees::checked_trees::CheckedDynamicScalarCallPlan
        {
            checked
                ...
                .find_map(|plan| match plan {
                    typed_trees_to_checked_trees::checked_trees::CheckedDynamicDispatchPlan::Scalar(
                        typed_trees_to_checked_trees::checked_trees::CheckedDynamicBinding::Rebound { latest, .. },
                    ) => Some(latest),
                    _ => None,
                })
    ...
    fn a_foreign_dynamic_call_scope_rejects() {
        ...
        let error = crate::checked_boundary_operator_scope(&foreign, produced.artifact(), &lowered)
            .unwrap_err();
        assert!(
            [
