...
use typed_trees_to_checked_trees::checked_trees::data::{DataDefinition, DataMember, DataVariant};
use typed_trees_to_checked_trees::checked_trees::types::{
    FixedArrayLength, PrimitiveType, TypeReferenceHandle, TypeReferenceNode,
};
use typed_trees_to_checked_trees::checked_trees::{CheckedTerminalMachineSelection, CheckedTrees};
...
pub(super) fn derive(
    ...
    if (*symbol != machine.symbol && *symbol != definition.symbol)
        ...
        || typed_trees_to_checked_trees::validation::data_requires_establishment(
            &checked.typed,
            definition,
        )
    {
    ...
    let requires_cleanup = typed_trees_to_checked_trees::validation::data_requires_nominal_drop(
        &checked.typed,
        definition,
    );
    let entry = module
...
/// same establishment rule `typed_trees_to_checked_trees::validation::data_requires_establishment` applies
...
fn zero_valid_byte_field(
    ...
    if !typed_trees_to_checked_trees::validation::has_stable_observable_contents(
        &checked.typed,
        reference,
    ) {
        return false;
    ...
    let predicates =
        typed_trees_to_checked_trees::checked_trees::byte_predicates::type_reference_domain_predicates(&checked.typed, reference);
...
mod tests {
    ...
    use crate::terminal_production::TerminalProductionCustody;
    use crate::terminal_production::TerminalProductionRequest;
    use crate::terminal_production::TerminalProductionTimings;
