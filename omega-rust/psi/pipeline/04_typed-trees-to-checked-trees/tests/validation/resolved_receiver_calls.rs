...
fn rejects(source: &str, fragment: &str) {
    let diagnostics = typed_trees_to_checked_trees::validation::validate_program(
...
fn accepts(source: &str) {
    typed_trees_to_checked_trees::validation::validate_program(&crate::front_end::typed_program(
        source,
    ))
...
fn nested_receiver_path_calls_obey_result_use_and_arity() {
    for (body, expected) in [
        ...
        let diagnostics = typed_trees_to_checked_trees::validation::validate_program(
            &with_resolved_nested_receiver_call(body),
        )
        .expect_err("invalid resolved receiver call must reject");
        assert!(
            diagnostics
    ...
    typed_trees_to_checked_trees::validation::validate_program(&accepted)
