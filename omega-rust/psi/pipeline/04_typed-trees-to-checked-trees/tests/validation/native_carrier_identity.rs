...
fn local_calling_lookalike_cannot_exempt_a_foreign_slice() {
    for (relative, origin) in [
        ...
        assert!(
            typed_trees_to_checked_trees::validation::standard_calling_traits(&program).is_none()
        );
        let errors = typed_trees_to_checked_trees::validation::validate_program(&program)
            .expect_err("no calling authority");
        assert!(
            errors
                .iter()
...
fn exact_standard_calling_declarations_retain_the_custom_policy_route() {
    ...
    let (calling, policy) =
        typed_trees_to_checked_trees::validation::standard_calling_traits(&program)
            .expect("exact declarations");
    assert_ne!(calling.symbol, policy.symbol);
    typed_trees_to_checked_trees::validation::validate_program(&program)
        .expect("custom calling route remains available");
}
...
fn public_calling_vocabulary_is_recognized_by_content_not_package_filename() {
    ...
    assert!(typed_trees_to_checked_trees::validation::standard_calling_traits(&program).is_some());
    ...
    assert!(typed_trees_to_checked_trees::validation::standard_calling_traits(&program).is_none());
}
...
fn exact_core_vector_still_requires_an_explicit_native_adapter() {
    let errors = typed_trees_to_checked_trees::validation::validate_program(&typed(
        VECTOR,
        "vec.omg",
