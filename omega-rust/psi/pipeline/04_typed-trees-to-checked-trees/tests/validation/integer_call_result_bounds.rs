...
fn result_comparisons_and_boolean_composition_publish_exact_cast_bounds() {
    for (guarantee, minimum, maximum) in [
        ...
        let facts = typed_trees_to_checked_trees::validation::validate_specialized_program(
            &program,
            typed_trees_to_checked_trees::validation::OpaquePropertyValidation::Required(&[]),
        )
...
fn assignment_cast_cannot_reuse_a_value_retired_by_an_earlier_write() {
    ...
    let diagnostics =
        typed_trees_to_checked_trees::validation::validate_program(&program).unwrap_err();
    assert!(
        diagnostics
            .iter()
