...
fn typed(source: &str) -> TypedTrees {
    ...
    typed_trees_to_checked_trees::validation::land_float_literal_destinations(&mut program);
    program
}
...
fn boolean_destinations_cannot_hide_anonymous_zero_division() {
    for expression in ["1 / 0.0 > 0", "0.0 / 0 == 0", "7.0 / (2 - 2.0) != 1"] {
        ...
        let errors = typed_trees_to_checked_trees::validation::validate_program(&program)
            .expect_err("invalid anonymous value");
        assert!(
            errors
                .iter()
...
fn a_shared_large_operand_keeps_its_other_runtime_width_obligation() {
    ...
    let errors = typed_trees_to_checked_trees::validation::validate_program(&program)
        .expect_err("the shared integer use remains invalid");
    assert!(
        errors
            .iter()
