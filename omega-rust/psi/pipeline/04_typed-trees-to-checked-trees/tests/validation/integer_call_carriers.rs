...
fn free_call_remainder_retains_exact_narrowing_evidence() {
    ...
    typed_trees_to_checked_trees::validation::validate_program(&program).expect("valid source");
...
fn free_call_carrier_does_not_license_unproved_narrowing_or_overflow() {
    for (expression, expected) in [
        ...
        let diagnostics = typed_trees_to_checked_trees::validation::validate_program(&program)
            .expect_err("unsafe full carrier");
        assert!(
            diagnostics
                .iter()
...
fn free_call_arithmetic_uses_the_declared_result_policy() {
    for policy in ["Wrapping", "Saturating"] {
        ...
        typed_trees_to_checked_trees::validation::validate_program(&program)
            .expect("qualified overflow has declared semantics");
    }
    let program = crate::front_end::typed_program(
    ...
    let diagnostics = typed_trees_to_checked_trees::validation::validate_program(&program)
        .expect_err("mixed explicit policies");
    assert!(
        diagnostics
            .iter()
...
fn free_call_result_carrier_does_not_follow_the_destination() {
    ...
    let diagnostics = typed_trees_to_checked_trees::validation::validate_program(&program)
        .expect_err("selected u16 carrier");
    assert!(
        diagnostics
            .iter()
...
fn free_call_declared_range_remains_available_to_exact_arithmetic() {
    ...
    typed_trees_to_checked_trees::validation::validate_program(&program)
        .expect("enforced declared range constrains result");
}
