...
fn scalar_transition_continuation_has_independent_argument_custody() {
    ...
    let checked = lower_typed_trees(program, &CheckingRequest::settled())
        .expect("both combined arms remain checked");
...
fn assignment_value_fact_retains_stable_guard_range() {
    ...
    assert_eq!(guarded.minimum, numerics::bignum::BigInt::from_i64(-128));
    assert_eq!(guarded.maximum, numerics::bignum::BigInt::from_i64(127));
}
