        ...
        match item {
            ...
            Item::Proposition(proposition) => {
                let body = match proposition.body {
                    tokens_to_syntax_trees::syntax_trees::item::PropositionBody::Primitive => {
                        PropositionBodySurface::Primitive
                    }
                    tokens_to_syntax_trees::syntax_trees::item::PropositionBody::Witness {
                        evidence,
                    } => {
                        collect_bounded_type_site_tree(
                            &mut report,
                    ...
                    tokens_to_syntax_trees::syntax_trees::item::PropositionBody::Transparent {
                        ..
...
fn collect_operator(
    ...
    operator: &tokens_to_syntax_trees::syntax_trees::item::OperatorDefinition,
    owner: &str,
) {
...
fn collect_state_node(
    ...
    syntax_trees: &SyntaxTrees,
    machine: &Machine,
    state: &tokens_to_syntax_trees::syntax_trees::item::StateNode,
) {
    collect_contracts(
        report,
        syntax_trees,
    ...
    for statement in syntax_trees.items.statements(state.statements) {
        let tokens_to_syntax_trees::syntax_trees::statement::StatementNode::LocalData(local_data) =
...
fn collect_trait_definition(
    report: &mut ProofSurfaceReport,
    syntax_trees: &SyntaxTrees,
    trait_definition: &tokens_to_syntax_trees::syntax_trees::item::TraitDefinition,
) {
...
fn collect_state_signature_node(
    report: &mut ProofSurfaceReport,
    syntax_trees: &SyntaxTrees,
    state: &tokens_to_syntax_trees::syntax_trees::item::StateSignatureNode,
    owner: &str,
) {
...
fn collect_signature_parts(
    report: &mut ProofSurfaceReport,
    syntax_trees: &SyntaxTrees,
    parameters: arena::HandleSpan<tokens_to_syntax_trees::syntax_trees::item::StateParameterHandle>,
...
fn operator_name(
    syntax_trees: &SyntaxTrees,
    name: arena::HandleSpan<tokens_to_syntax_trees::syntax_trees::identifier::Identifier>,
) -> String {
    syntax_trees
        .items
        .identifier_path_members(name)
        .iter()
        .map(tokens_to_syntax_trees::syntax_trees::identifier::Identifier::as_str)
        .collect::<Vec<_>>()
        .join("::")
}
