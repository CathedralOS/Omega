...
use crate::proof_engine::checker::diagnostics::expression_display_name;
...
fn guard_conjunct_proves_dependent_upper(
    ...
    let Some(bound) =
        symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::symbolic_max_bound(
...
pub(crate) fn incoming_guard_proves_dependent_call_upper(
    ...
    let requires_proves = |contracts: &[symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContract]| {
        contracts
            .iter()
            ...
            .any(|fact| {
                matches!(fact, symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact::Expression(condition)
                    if proves(*condition))
            })
    };
...
fn target_enters_state(
    ...
    match proof_plan.program.statement_table.transition_target(target) {
        ...
        symbol_resolved_trees_to_typed_trees::typed_trees::statement::TransitionTargetNode::SelfTarget => source.symbol == state.symbol,
        _ => false,
    }
}
...
pub(crate) fn dependent_field_floor(
    ...
    let field_type = crate::proof_engine::obligations::data_field_type_by_name(
        program,
        data,
        max_field.as_str(),
    )?;
...
fn place_preserved_in_statements(
    ...
    for statement in statements {
        ...
        if value_written
            .iter()
            .any(|written| crate::validation::frame_paths_overlap(&field_path, written))
        {
            return false;
        }
        match statement {
...
fn sibling_conjunct_proves(
    ...
    let Some(bound) =
        symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::sibling_len_bound(
            &proof_plan.program.expression_table,
            bound_side,
        )
