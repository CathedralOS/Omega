...
fn state_return_certificate<'program>(
    ...
    if let Some((minimum, maximum)) = crate::validation::arrival_integer_expression_bounds(
        program,
    ...
    let anonymous_owned = proof_plan
        ...
        .and_then(|primitive| {
            crate::validation::land_anonymous_integer_expression(
                ...
                primitive,
                |expression| {
                    crate::validation::has_anonymous_operator_meaning(
                        proof_plan.program,
                        expression,
                    )
                },
...
fn arrival_argument_certificate(
    ...
    let (minimum, maximum) = crate::validation::arrival_integer_expression_bounds(
        proof_plan.program,
        obligation.machine_symbol,
        obligation.state_symbol,
        obligation.statement_index,
...
fn guarded_refold_certificate(
    ...
    let state = program
        ...
        .iter()
        .find(|state| state.symbol == obligation.state_symbol)?;
    let place_type = crate::proof_engine::obligations::expression_type_reference(
    ...
    let place_constraints: Vec<ProofConstraint> =
        crate::proof_engine::obligations::expression_constraints(program, machine, state, place)
            .into_iter()
            .collect();
    let mut facts = Vec::new();
        ...
        let landed = program
            ...
            .and_then(|primitive| {
                crate::validation::land_anonymous_integer_expression(
                    ...
                    primitive,
                    |expression| {
                        crate::validation::has_anonymous_operator_meaning(program, expression)
                    },
                )
            });
...
fn closed_math_term(
    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Binary(binary) => {
            if !crate::validation::has_anonymous_operator_meaning(program, expression) {
                return None;
            }
...
mod coverage_tests {
    ...
    use crate::proof_engine::checker::AssignmentRangeContext;
    use crate::proof_engine::checker::measurement::ProofPlanMeasurements;
    use crate::proof_engine::checker::requires_conditions::{same_place, stable_expression};
    use crate::proof_engine::obligations::{
