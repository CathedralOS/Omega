...
use crate::proof_engine::obligations::constraints::{
...
use crate::proof_engine::obligations::program_queries::{
    dehoisted_operand, expression_type_reference,
};
use crate::proof_engine::obligations::ranges::{
    constrained_type_reference, integer_range_from_constraints,
};
...
/// `symbol_resolved_trees_to_typed_trees::typed_trees::boundary::called_boundary_signature` -- validation's
    ...
    for statement in &statements[..upto] {
        match statement {
            StatementNode::Call(call) => {
                    ...
                    call_frames.and_then(|frames| frames.may_write_paths(machine, call))
                {
                    witnesses.retain(|(place, _)| {
                        written
                            .iter()
                            .all(|written| !crate::validation::frame_paths_overlap(place, written))
                    });
                } else {
                    witnesses.clear();
                }
                let Some(signature) =
                    symbol_resolved_trees_to_typed_trees::typed_trees::boundary::called_boundary_signature(program, machine, call)
                else {
                    continue;
                };
            ...
            StatementNode::Assignment(assignment) if !witnesses.is_empty() => {
                ...
                witnesses
                    .retain(|(place, _)| !crate::validation::frame_paths_overlap(place, &target));
            }
            _ => {}
        }
    }
...
fn table_transition_target_state_and_arguments<'program>(
    ...
    &'program State,
    &'program [symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle],
)> {
