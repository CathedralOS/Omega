...
use crate::proof_engine::obligations::collection::estimated_proof_obligation_capacity;
use crate::proof_engine::obligations::ranges::float_range_bound;
use arena::{Arena, HandleSpan};
use numerics::bignum::BigInt;
use std::fmt;
use symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees;
use symbol_resolved_trees_to_typed_trees::typed_trees::expression::{
    BinaryOperator, ExpressionHandle, FloatLiteral,
};
...
use symbol_resolved_trees_to_typed_trees::typed_trees::types::{
    PrimitiveType, TypeConstraintNode, TypeReferenceHandle,
};
...
impl ProofConstraint {
    ...
    fn exact_domain_interval(
        ...
        let (minimum, maximum) = crate::validation::exact_declared_domain_carrier_interval(
            program,
    ...
    fn range_from_expression_handles(
        ...
        if !matches!(
            ...
            // proof-integer bound.
            let integer_bound = crate::validation::closed_integer_range_bound(program, minimum);
            ...
            if let Some(minimum) = integer_bound.as_ref().and_then(BigInt::to_i64)
                && let Some(symbolic) = symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::symbolic_range_maximum(
            ...
            if let Some(minimum) = integer_bound.as_ref().and_then(BigInt::to_i64)
                && let Some(sibling) = symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::sibling_range_maximum(
                    &program.expression_table,
                    maximum,
                    end_inclusive,
                )
