...
use symbol_resolved_trees_to_typed_trees::typed_trees::operator::{
    ClosedOperatorApplicationArgument, OperatorDefinition,
};
use symbol_resolved_trees_to_typed_trees::typed_trees::statement::{StatementHandle, TableCall};
use symbol_resolved_trees_to_typed_trees::typed_trees::types::{
    TypeReferenceHandle, TypeReferenceNode,
};
use symbols::SymbolHandle;

use crate::validation::declarations::symbols::TopLevelSymbols;
...
pub fn canonical_closed_operator_realization_bytes(
    ...
    retained: &[symbol_resolved_trees_to_typed_trees::typed_trees::operator::ClosedOperatorRealizationApplication],
    ...
    for (requirement_symbol, operator) in expected {
        ...
        let reconstructed = symbol_resolved_trees_to_typed_trees::typed_trees::operator::closed_operator_realization_application(
        ...
        for (ordinal, (parameter, argument)) in parameters.iter().zip(&row.arguments).enumerate() {
            ...
            match (&parameter.kind, argument) {
                ...
                {
                    crate::validation::value_custody::type_references::validate_exact_const_identity(
                        program,
                        *declared_carrier,
                        value,
                    )
...
pub fn checked_operator_application_matches_realization(
    program: &TypedTrees,
    demand: &crate::checked_trees::CheckedBoundaryOperatorApplicationDemand,
    realization: &symbol_resolved_trees_to_typed_trees::typed_trees::operator::ClosedOperatorRealizationApplication,
) -> bool {
...
pub fn validate_named_operator_application(
    ...
    let Some(inferred) = symbol_resolved_trees_to_typed_trees::typed_trees::operator::closed_operator_application_for_operands(
        program,
        operator,
        operand_types,
    ) else {
...
pub fn landed_integer_literal_type_reference(
    ...
    let atom = match program.expression_table.expression(expression) {
        ExpressionNode::Integer(literal) => match literal.landing()?.landed_type {
            ...
            numerics::literals::LandedIntegerType::Addr => symbols::BuiltinTypeAtom::Address,
        },
        _ => return None,
    };
    let symbol = program
...
pub fn validated_symbolic_boundary_operator_application(
    ...
    let bindings = symbol_resolved_trees_to_typed_trees::typed_trees::operator::symbolic_operator_type_application_for_operands(
        program,
        machine,
        operator,
        operand_types,
...
fn statement_value_receiver_type(
    ...
    let receiver = program
        ...
        .last()?;
    crate::validation::machine_calls::calls::declared_receiver_type_reference(
        program,
        machine,
        state,
        receiver.as_str(),
    ...
    for (parameter, argument) in parameters.iter().zip(application) {
        match (&parameter.kind, argument) {
            (
                ...
                let bounds =
                    crate::validation::proof_contracts::properties::declared_property_requirements(
                        &parameter.bounds,
                    );
                let bound_labels = bounds.iter().map(ToString::to_string).collect::<Vec<_>>();
                for property in bounds {
                    if crate::validation::proof_contracts::properties::type_satisfies_declared_property(
                        program,
                        symbols,
                        &[],
                        *type_reference,
