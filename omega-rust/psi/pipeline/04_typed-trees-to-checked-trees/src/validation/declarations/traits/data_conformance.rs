...
pub(crate) fn validate_conformances(
    ...
    for (index, conformance) in program.conformances().iter().enumerate() {
        let (subject_name, carrier_name) = match &conformance.subject {
            symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::ConformanceSubject::Carrier(type_name) => {
        ...
        for previous in &program.conformances()[..index] {
            ...
            match (&previous.subject, &conformance.subject) {
                (
                    symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::ConformanceSubject::Carrier(previous_type),
                    symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::ConformanceSubject::Carrier(type_name),
...
fn validate_data_satisfies_trait(
    ...
    explicit_type_arguments: &[symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceHandle],
    conformance_name: Option<&str>,
    diagnostics: &mut Vec<Diagnostic>,
