...
use crate::validation::value_custody::type_references::{
    type_reference_label, type_references_match,
};
...
use symbol_resolved_trees_to_typed_trees::typed_trees::signature::{
    SignatureContractKind, StateParameter, StateSignature,
};
...
pub(crate) fn validate_machine_state_satisfies_trait_signature_with_arguments(
    ...
    crate::validation::machine_calls::machine_parameters::validate_trait_callable_parameter_refinement(
        program,
        &format!(
        ...
        .enumerate()
    {
        match (&required.kind, &actual.kind) {
            (TypeParameterKind::Type, TypeParameterKind::Type) => {
                if symbol_resolved_trees_to_typed_trees::typed_trees::data::type_parameter_demands_stronger_properties(
                    required, actual,
                ) {
                    diagnostics.push(Diagnostic::error(format!(
...
pub(crate) fn type_reference_lifetimes_match_requirement_application(
    ...
    actual: TypeReferenceHandle,
    required: TypeReferenceHandle,
    target_lifetimes: &[symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier],
    machine: &Machine,
    raw_application: &[u32],
) -> bool {
    if !actual.is_valid() || !required.is_valid() {
...
fn requirement_lifetime_matches(
    actual: Option<&symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier>,
    required: Option<&symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier>,
    target_lifetimes: &[symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier],
    machine: &Machine,
    raw_application: &[u32],
) -> bool {
