...
fn validate_machine_top_level_requirement_conformance(
        ...
        .enumerate()
    {
        if matches!(
            (&required.kind, &actual.kind),
            (TypeParameterKind::Type, TypeParameterKind::Type)
        ) && symbol_resolved_trees_to_typed_trees::typed_trees::data::type_parameter_demands_stronger_properties(required, actual)
        {
            diagnostics.push(Diagnostic::error(format!(
    ...
    if conformance.external_binding.is_none() && !conformance.via_expression.is_valid() {
        crate::validation::machine_calls::machine_parameters::validate_callable_contract_refinement(
            program,
            &label,
...
fn validate_machine_operator_conformance(
    ...
    let operator = if let Some(operator) = symbol_resolved_trees_to_typed_trees::typed_trees::operator::resolve_satisfied_checked_operator(
        program,
        machine,
        namespace,
        requirement_name,
        ...
        let retained = symbol_resolved_trees_to_typed_trees::typed_trees::operator::resolve_specialized_checked_operator_application(
            program,
            machine,
            namespace,
            requirement_name,
    ...
    if conformance.external_binding.is_none() && !conformance.via_expression.is_valid() {
        crate::validation::proof_contracts::contract_entailment::check_operator_contract_conformance(
            program,
            machine,
            operator,
            diagnostics,
...
fn validate_machine_single_requirement(
    ...
    requirement_name: &symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier,
...
fn validate_named_contract_lane(
    ...
    requirement: &StateSignature,
    kind: SignatureContractKind,
    required: &[&symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContract],
    actual: &[&symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContract],
