...
use symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::{
    DynamicSignatureIneligibility, TraitDefinition,
};
        ...
        for state in program.machine_states(machine) {
                ...
                .enumerate()
            {
                ...
                let Some(source_symbol) =
                    crate::validation::value_custody::places::declared_place_leaf_symbol(
                        program,
                        machine,
                        Some(state),
                        statement_index,
                ...
                let matches = nominal_matches
                    ...
                    .filter(|conformance| {
                        matches!(
                            ...
                            symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::ConformanceImplementation::Closed { .. }
                        )
                    })
                    .collect::<Vec<_>>();
                    ...
                    if nominal_matches.iter().any(|candidate| {
                        candidate.symbol == *exact_symbol
                            && matches!(
                                ...
                                symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::ConformanceImplementation::AttachedRequirementMachines
                            )
                    }) {
                        diagnostics.push(Diagnostic::error(format!(
                ...
                for field in program.expression_table.struct_fields(literal.fields) {
                    let Some(declared_field) =
                        program.data_members(data).iter().find_map(|member| {
                            let symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(declared) = member else {
                                return None;
                            };
...
fn validate_dynamic_call_arguments_in_statement(
    ...
    for root in crate::validation::machine_calls::calls::statement_value_expression_roots(
        program, statement,
    ) {
...
fn validate_dynamic_call_arguments(
    ...
    for (parameter, argument) in parameters.zip(
        ...
        let source_type = crate::validation::value_custody::places::declared_place_type_raw(
            program,
            caller,
            Some(caller_state),
            argument,
                ...
                .enumerate()
            {
                if let StatementNode::Call(call) = statement {
                    ...
                    let target = dynamic_target.or_else(|| {
                        crate::validation::value_custody::placed_views::statement_call_target(
                            program, machine, state, call,
                        )
                    });
