    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Name(path) => {
            ...
            for (offset, member_name) in member_names.iter().enumerate().skip(1) {
                ...
                let symbol = exact_symbol
                    .or_else(|| {
                        crate::fact_plan::resolve_place_member_symbol(
                            program,
                            facts,
                            place,
                            member_name.as_str(),
                ...
                facts.push_place_segment(place, crate::fact_plan::PlaceSegment::Field { symbol });
            }
            place
        }
        ExpressionNode::Member(member) => {
            ...
            if let Some(variant) = crate::fact_plan::payload_variant_for_field(program, symbol) {
                facts.push_place_segment(place, crate::fact_plan::PlaceSegment::Case { variant });
            }
            facts.push_place_segment(place, crate::fact_plan::PlaceSegment::Field { symbol });
            place
        }
        ExpressionNode::Indexed(indexed) => {
            ...
            let segment = program
                ...
                .unwrap_or(crate::fact_plan::PlaceSegment::Index {
...
fn append_domain_expression_dependency_places(
    program: &TypedTrees,
    facts: &mut FactPlan,
    domain_target: symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceHandle,
    expression: symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle,
