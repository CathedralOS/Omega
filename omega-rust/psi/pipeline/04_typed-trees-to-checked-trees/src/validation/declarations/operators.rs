...
pub(crate) fn validate_operator_declarations(
    program: &TypedTrees,
...
fn validate_operator_types(
    ...
    for contract in program.operator_contracts(operator) {
        ...
        for fact in program.proof_facts.span_or_empty(contract.facts) {
            if !matches!(
                fact,
                symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact::Expression(_)
            ) {
...
fn validate_spelling_overlap(
    program: &TypedTrees,
    operators: &[symbol_resolved_trees_to_typed_trees::typed_trees::operator::OperatorDefinition],
    diagnostics: &mut Vec<Diagnostic>,
) {
