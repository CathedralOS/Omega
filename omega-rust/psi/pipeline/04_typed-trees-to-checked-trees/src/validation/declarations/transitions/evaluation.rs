...
use crate::validation::machine_calls::calls::CallFrameResolver;
    ...
    pub(crate) fn collect<'program>(
        ...
        for (positive, target) in [(true, transition.target), (false, transition.continuation)] {
            ...
            let mut current = if positive {
                crate::validation::proof_contracts::arithmetic_domains::guard_narrowed_environment(
                    program,
                    machine,
                    Some(state),
                    &transition.guard,
                ...
                crate::validation::proof_contracts::arithmetic_domains::fall_through_narrowed_environment(
                    program,
                    machine,
                    Some(state),
                    &transition.guard,
