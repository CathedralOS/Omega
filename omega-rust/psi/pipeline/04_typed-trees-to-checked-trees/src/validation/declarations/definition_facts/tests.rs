...
use symbol_resolved_trees_to_typed_trees::typed_trees::domain::{
    DomainDefinition, ProofFact, ProofMembershipFact,
};
...
fn builds_definition_fact_plan_for_domains() {
    ...
    assert_eq!(
        facts
            .facts_at_point(crate::fact_plan::ProgramPoint::Definition {
                symbol: alive_domain_symbol,
            })
            .count(),
        2
    ...
    let domain_context = facts
        .contexts_at_point(crate::fact_plan::ProgramPoint::Definition {
            symbol: alive_domain_symbol,
        })
        .next()
