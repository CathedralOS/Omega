...
use crate::validation::proof_contracts::proof_facts;
...
pub(super) fn validate(
    ...
    if let Some(state) = state {
        crate::validation::value_custody::locals::StateValueScope {
            program,
            machine,
            state,
            machine_symbols,
    ...
    if !proof_facts::is_boolean_asm_fact_expression(program, machine, state, fact.expression) {
        let kind = match fact.kind {
            symbol_resolved_trees_to_typed_trees::typed_trees::statement::AssemblyFactKind::Requires => "requires",
            symbol_resolved_trees_to_typed_trees::typed_trees::statement::AssemblyFactKind::Ensures => "ensures",
        };
