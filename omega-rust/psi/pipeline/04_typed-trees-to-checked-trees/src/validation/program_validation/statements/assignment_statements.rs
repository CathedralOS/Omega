...
use crate::validation::value_custody::places::validate_assignment_target_handle;
...
pub(super) fn validate<'a, 'p>(
    ...
    if diagnostics.len() == before {
        ...
        if let Some(stated) = current_state.and_then(|state| {
            crate::validation::proof_contracts::default_domains::stored_field_where_interval(
                program,
                machine,
                state,
                assignment.target,
