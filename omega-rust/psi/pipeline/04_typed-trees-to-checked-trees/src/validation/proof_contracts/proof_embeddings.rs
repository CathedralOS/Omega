...
pub(crate) fn validate_proof_embeddings(
    ...
    for machine in program.machines() {
        if classification.is_proof_machine(program, machine) {
            ...
            crate::validation::value_custody::content_projections::content_projection_body_expression(
                program, machine,
            )
        {
            collect_expression_nodes(program, expression, &mut proof_nodes, &mut proof_seen);
...
pub(crate) fn expression_type_reference(
    ...
    match program.expression_table.expression(expression) {
        ExpressionNode::Name(path) => {
            ...
            .or_else(|| {
                crate::validation::proof_contracts::contract_results::type_reference(
                    program, expression,
                )
            })
        }
        ExpressionNode::Member(member) => {
            ...
            if let ExpressionNode::Name(receiver) =
                ...
                && let Some(field) = crate::validation::value_custody::places::exact_self_field(
        ...
        ExpressionNode::Indexed(indexed) => {
            ...
            let collection = crate::validation::value_custody::places::unwrapped_type_reference(
                program, collection,
            )?;
