        ...
        for state in program.machine_states(machine) {
            ...
            for goal in required {
                let structurally_entailed = structural_proposition_goals.iter().any(
                    |(label, application)| {
                        label == &goal
                            && crate::validation::proof_contracts::contract_entailment::transparent_proposition_application_entailed(
                                program,
                                machine,
                                state,
                                application,
...
pub fn select_subjectless_evidence_conformance<'program>(
    ...
    conformance_symbol: symbols::SymbolHandle,
    source_name: &str,
    expected_interface: &symbol_resolved_trees_to_typed_trees::typed_trees::proposition::NormalizedEvidenceInterfaceIdentity,
) -> Option<(
    ...
    use symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::{
        ConformanceImplementation, ConformanceSubject,
    };
    ...
    let selected_interface = symbol_resolved_trees_to_typed_trees::typed_trees::proposition::NormalizedEvidenceInterfaceIdentity {
...
) -> Vec<(symbols::SymbolHandle, String, String)> {
...
fn proposition_labels(
    ...
    substitutions: &[(symbols::SymbolHandle, String, String)],
