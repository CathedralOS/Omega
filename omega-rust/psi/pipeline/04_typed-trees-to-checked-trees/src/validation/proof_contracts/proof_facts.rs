...
pub(crate) fn validate_proposition_definitions(
    ...
    for proposition in program.propositions() {
        ...
        if let symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionBody::Witness { evidence } = proposition.body {
            ...
            ProofFact::Proposition(application) => {
                ...
                let parameter_signature = program
                    ...
                    .find_map(|parameter| match &parameter.kind {
                        symbol_resolved_trees_to_typed_trees::typed_trees::data::TypeParameterKind::Proposition { contract }
                    ...
                    .enumerate()
                {
                    ...
                    let matches = match instantiated_type {
                        PropositionParameterInstantiation::Unchanged(expected_type) => {
                            crate::validation::value_custody::expression_types::argument_matches_type_reference_handle(
                                program,
                                *argument,
                                expected_type,
                            )
...
enum PropositionParameterInstantiation<'application> {
    Unchanged(symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceHandle),
    Selected(&'application symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionBinderArgument),
    Invalid,
}

fn proposition_parameter_instantiation<'application>(
    program: &TypedTrees,
    declaration: &symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionDefinition,
    application: &'application symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionApplication,
...
fn argument_matches_selected_proposition_type(
    ...
    selected: &symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionBinderArgument,
    owner: ProofFactOwner<'_>,
) -> bool {
    if let Some(actual) = proof_argument_declared_type(program, argument, owner) {
        if let (Some(actual), Some(selected)) = (
            ...
            symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::from_name(
                &selected.display_name(),
            ),
        ) {
            ...
            program
                ...
                .find_map(|field| match field {
                    symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                        field,
                    ) if field.name.as_str() == member_name => Some(field.type_reference),
                    _ => None,
                })
    {
...
pub(crate) fn carry_permission(
    program: &TypedTrees,
    domain: arena::HandleSpan<Identifier>,
...
fn is_implicit_case_domain(program: &TypedTrees, domain: arena::HandleSpan<Identifier>) -> bool {
...
fn is_boolean_domain_fact_expression(
    ...
    match program.expression_table.expression(expression) {
        ExpressionNode::Name(_) | ExpressionNode::Member(_) | ExpressionNode::Indexed(_) => {
            crate::validation::value_custody::expression_types::domain_expression_result_type_reference(
        ...
        ExpressionNode::Unary(unary) => {
            unary.operator == symbol_resolved_trees_to_typed_trees::typed_trees::expression::UnaryOperator::LogicalNot
...
fn domain_path_label(program: &TypedTrees, domain: arena::HandleSpan<Identifier>) -> String {
...
pub(crate) fn string_literal_grants_domain(
    ...
    symbol_resolved_trees_to_typed_trees::typed_trees::byte_predicates::domain_byte_predicate(
        program,
        domain_symbol,
    )
