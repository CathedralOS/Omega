                ...
                .filter(|contract| contract.kind == SignatureContractKind::Ensures)
            {
                for fact in program.proof_facts.span_or_empty(contract.facts) {
                    ...
                    if let Some(permission) =
                        crate::validation::proof_contracts::proof_facts::carry_permission(
                            program,
                            membership.domain,
                        )
                    {
...
fn lifetime_erased_nominal_carriers_match(
    program: &TypedTrees,
    application: symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceHandle,
    nominal: symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceHandle,
) -> bool {
