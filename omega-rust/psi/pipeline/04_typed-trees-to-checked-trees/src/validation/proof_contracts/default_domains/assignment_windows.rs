...
use crate::validation::proof_contracts::default_domains::data_reads::range_gates_hold;
use crate::validation::proof_contracts::default_domains::place_queries::{
...
use crate::validation::proof_contracts::default_domains::state_flow::PlaceValuation;
use crate::validation::proof_contracts::default_domains::symbolic_values::{
    expression_sequence_measures, expression_symbol, expression_symbolic_value,
    fold_with_valuation, integer_literal_value,
};
use crate::validation::proof_contracts::default_domains::{InvariantWindow, TrackedPlace};
...
pub(crate) fn handle_assignment<'program>(
    ...
    if definition.where_facts.is_empty()
        ...
        && !crate::validation::value_custody::data::data_requires_establishment(program, definition)
    {
        return;
    }
        ...
        .position(|place| place.spelling == receiver_spelling)
    {
        ...
        tracked.push(TrackedPlace {
            ...
            established: !crate::validation::value_custody::data::data_requires_establishment(
                program, definition,
            ) || !self_rooted,
        ...
        program
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    field,
                ) if field.name.as_str() == field_name => Some(field.type_reference),
                _ => None,
            });
    ...
    if !field_is_where_mentioned(program, place.definition, &field_name)
        ...
        && !field_type.is_some_and(|field_type| {
            crate::validation::value_custody::data::type_requires_establishment(program, field_type)
        })
    {
        return;
    }
        ...
        match fact {
            ...
            symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact::Membership(
                ...
                if mentioned == Some(field_name.as_str()) {
                    if !crate::validation::proof_contracts::proof_facts::string_literal_grants_domain(
                        program,
                        value,
...
fn case_fact_spans_mentioning(
    ...
    field_name: &str,
) -> Vec<arena::HandleSpan<symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact>> {
    program
        .data_members(place.definition)
        .iter()
        .filter_map(|member| {
