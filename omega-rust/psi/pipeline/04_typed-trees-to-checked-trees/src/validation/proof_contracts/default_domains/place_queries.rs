...
use language_core::{is_receiver_rooted, is_self_receiver};
    ...
    program
        ...
        .filter(|definition| {
            !definition.where_facts.is_empty()
                ...
                || crate::validation::value_custody::data::data_requires_establishment(
                    program, definition,
                )
        })
}
...
pub(super) fn data_has_case_where_facts(program: &TypedTrees, definition: &DataDefinition) -> bool {
    program.data_members(definition).iter().any(|member| {
        matches!(member, symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(variant)
                    ...
                    .any(|arm| {
                        expression_mentions_name(program, arm.value, name)
                            || match arm.pattern {
                                ...
                                symbol_resolved_trees_to_typed_trees::typed_trees::expression::MatchPattern::Wildcard => false,
                            }
                    })
        }
