...
fn newly_reached_cycle_preserves_equal_literals_in_either_field_order() {
    ...
    for source in [
        ...
        assert!(
            build_open_invariant_crash_sites(&crate::validation::front_end::typed_program(&source))
                .is_empty()
        );
    }
}
...
fn transported_open_windows_still_require_closure_or_crash_evidence() {
    ...
    let mut diagnostics = Vec::new();
    validate_default_domain_writes(
        &crate::validation::front_end::typed_program(&source.replace("crash Trap;", "")),
        &mut diagnostics,
    );
    assert!(
        diagnostics
    ...
    assert!(
        build_open_invariant_crash_sites(&crate::validation::front_end::typed_program(
