...
fn a_live_unsigned_ceiling_proves_joint_addition_fits_its_carrier() {
    ...
    for source in [
        ...
        let result = crate::validation::validate_program(
            &crate::validation::front_end::typed_program(&source),
        );
...
fn bounded_return_rejects_one_unguarded_predecessor() {
    ...
    let diagnostics = crate::validation::validate_program(&program).expect_err("unguarded arrival");
