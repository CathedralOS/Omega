...
pub(crate) fn validate_anonymous_integer_primitive_range(
    ...
    let evaluated = crate::validation::value_custody::literals::anonymous_numeric_value(
        program,
        expression,
        &mut |expression| {
    ...
    if let Some(literal) =
        crate::validation::value_custody::literals::land_integer_value(&value, primitive)
    {
...
pub(crate) fn collect_exact_integer_cast_facts(
    ...
    expression: ExpressionHandle,
    environment: &ValueEnvironment,
    facts: &mut Vec<crate::validation::ExactIntegerCastFact>,
) {
    if !expression.is_valid() {
        return;
    }
                ...
                && target_type != PrimitiveType::Addr
            {
                    ...
                    && integer_interval_fits_primitive(interval, source_type, target_type)
                {
                    ...
                    if let (Some(minimum), Some(maximum)) = (effective.low, effective.high) {
                        facts.push(crate::validation::ExactIntegerCastFact {
                            expression,
                            source_type,
                            target_type,
