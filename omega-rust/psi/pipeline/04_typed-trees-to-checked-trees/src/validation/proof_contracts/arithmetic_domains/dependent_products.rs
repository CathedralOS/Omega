...
pub(super) fn refine_dependent_product(
    ...
    let (a_expr, fb_expr) = {
        let left_is_field = symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::symbolic_max_bound(
...
fn strict_dependent_atom_field(
    ...
    let raw = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        expression,
...
fn requires_product_coupling(
    program: &TypedTrees,
    machine: &Machine,
    fa: &symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier,
    fb: &symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier,
) -> Option<i64> {
...
pub(super) fn refine_dependent_product_factor(
    ...
    let (a_expr, fb_expr) = {
        let left_is_field =
            symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::symbolic_max_bound(&program.expression_table, left)
                .is_some_and(|bound| bound.offset == 0);
        if left_is_field {
            (right, left)
        } else {
