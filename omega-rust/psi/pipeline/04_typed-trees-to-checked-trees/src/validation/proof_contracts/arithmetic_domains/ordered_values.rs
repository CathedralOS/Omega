...
use crate::validation::value_custody::places::{
    collection_length_receiver, declared_member_path_type,
};
...
pub(super) fn byte_store_collection(
    ...
    if !crate::validation::place_has_builtin_coordinates(program, machine, Some(state), target) {
        return None;
    }
    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Member(member) => {
            ...
            let field = if member.case_variant.is_some() {
                crate::validation::value_custody::places::declared_case_projection_field(
                    program,
                    machine,
                    Some(state),
                    expression,
                ...
                crate::validation::exact_self_field(program, machine, expression)?.symbol
            } else {
                crate::validation::value_custody::places::declared_member_field_symbol(
                    program,
                    machine,
                    Some(state),
                    expression,
        ...
        ExpressionNode::Call(call) => {
            ...
            crate::validation::machine_calls::denotational_calls::plain_value_call_target(
                program, call,
            )?;
            let plans = crate::validation::operand_call_plans(program, frames);
            let (_, entry) =
                crate::validation::machine_calls::denotational_calls::normal_return_call_candidate(
