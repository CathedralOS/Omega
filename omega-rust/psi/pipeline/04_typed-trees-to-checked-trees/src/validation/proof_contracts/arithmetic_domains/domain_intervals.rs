...
pub fn exact_declared_domain_carrier_interval(
    program: &symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees,
    carrier: symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType,
    domain: &symbol_resolved_trees_to_typed_trees::typed_trees::types::DomainConstraint,
) -> Option<(numerics::bignum::BigInt, numerics::bignum::BigInt)> {
    let (minimum, maximum) = exact_declared_domain_interval(program, domain)?;
    let (carrier_minimum, carrier_maximum) =
        crate::validation::proof_contracts::proof_embeddings::primitive_range(carrier)?;
    Some((
...
fn predicate_interval(
    ...
    use symbol_resolved_trees_to_typed_trees::typed_trees::types::DomainConstraintSubject;
...
fn self_predicate_bounds(
    ...
    let Some(value) = crate::validation::closed_integer_range_bound(program, literal) else {
        return false;
    };
