...
fn qualified_record_projection_retains_type_but_does_not_erase_arithmetic_policy() {
    for (field_type, accepted) in [("u64 in Tag", true), ("u64 in Wrapping", false)] {
        ...
        let scalar = crate::validation::closed_record_scalar_projection(&program, projection);
        let integer = crate::validation::value_custody::literals::closed_record_integer_projection(
            &program, projection,
        );
...
fn closed_record_fields_share_exact_bounds_and_keep_projection_custody() {
    ...
    let diagnose = |program: &TypedTrees| {
        ...
        crate::validation::proof_contracts::arithmetic_domains::validate_arithmetic_domains(
            program,
            machine,
            Some(state),
            expression,
    ...
    let point = crate::validation::value_custody::literals::closed_record_integer_projection(
        &original, projection,
    )
    .unwrap();
...
fn closed_integer_points_retain_width_and_each_typed_operation() {
    for (expression, expected) in [
        ...
        assert_eq!(
            crate::validation::closed_integer_range_bound(&program, root)
...
fn wide_points_do_not_grant_landing_or_static_identity_to_variable_operands() {
    ...
    let program = crate::validation::front_end::typed_program(
        "machine value(input: u64[2..=2]) -> u64 { input + 1 }",
    );
    let machine = &program.machines()[0];
    let state = &program.machine_states(machine)[0];
    let symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Expression(
    ...
    assert!(
        crate::validation::closed_integer_range_bound(&program, root).is_none(),
...
fn enforced_type_bounds_reject_a_same_spelled_nominal_impostor() {
    let mut program = crate::validation::front_end::typed_program("data Impostor {}");
    let symbol = program.data_definitions()[0].symbol;
    let handle = program
        .type_reference_table
        .insert(TypeReferenceNode::Named {
        symbol,
        name: symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier::generated_static(
            "u8",
        ),
    });
    assert_eq!(
...
fn immutable_bounds_do_not_select_carrier_width_by_spelling() {
    let mut program =
        crate::validation::front_end::typed_program("machine read(input: u64) -> u64 { input }");
...
fn computed_bounds_retain_carrier_identity_without_input_refinements() {
    let program = crate::validation::front_end::typed_program(
        "machine value(input: u64 [20..=30]) -> u64 { input % 5 }",
    );
    let machine = &program.machines()[0];
    let state = &program.machine_states(machine)[0];
    let symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Expression(
