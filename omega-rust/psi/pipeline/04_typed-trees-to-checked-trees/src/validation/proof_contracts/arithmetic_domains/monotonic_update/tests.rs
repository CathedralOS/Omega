...
fn bounds(
    ...
    let source_id = sources
        ...
        .source_id;
    let program = crate::validation::front_end::typed_program_from_source_map(
        sources,
        &[(source_id, &source)],
    );
    ...
    let target = crate::validation::exact_self_field(&program, machine, assignment.target)
    ...
    let counter = [binary.left, binary.right]
        .into_iter()
        .find_map(|operand| crate::validation::exact_self_field(&program, machine, operand))
