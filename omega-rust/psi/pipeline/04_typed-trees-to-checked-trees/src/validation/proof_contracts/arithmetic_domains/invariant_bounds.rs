...
pub(crate) fn standing_integer_interval(
    ...
    let place_type = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        expression,
...
fn stored_place_interval(
    ...
    let facts = crate::validation::proof_contracts::default_domains::where_fact_interval(
        program,
        machine,
        Some(state),
        expression,
...
pub fn stored_integer_bounds(
    ...
    let carrier = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        expression,
...
fn bounds(
    ...
    if let Some(value) =
        crate::validation::value_custody::literals::closed_record_integer_projection(
            program, expression,
        )
        .or_else(|| program.closed_integer_value_in(expression, machine.symbol))
    {
        ...
        while let Some(handle) = pending.pop() {
            ...
            if matches!(node, ExpressionNode::Member(_)) {
                has_member = true;
                if crate::validation::value_custody::literals::closed_record_integer_projection(
                    program, handle,
                )
                .is_none()
                    && program
    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Member(_) => {
            ...
            let standing = crate::validation::proof_contracts::default_domains::where_fact_interval(
