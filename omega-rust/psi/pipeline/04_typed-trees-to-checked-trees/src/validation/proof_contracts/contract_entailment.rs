...
use crate::validation::proof_contracts::contract_entailment::citations::{
    collect_citation_equations, instantiated_fact_established, is_citation_statement,
    suggest_missing_citation,
};
use crate::validation::proof_contracts::contract_entailment::stand_downs::{
    fact_mentions_proof_only_data, fact_mentions_zero_value, record_expression_stand_down,
};
use crate::validation::proof_contracts::contract_entailment::structural_case_arms::{
...
pub(crate) fn validate_machine_contract_entailment_with_stand_downs(
    ...
    machine: &Machine,
    diagnostics: &mut Vec<Diagnostic>,
    stand_downs: &mut Vec<crate::validation::ContractEntailmentStandDown>,
) {
...
pub(crate) fn validate_machine_contract_entailment_with_outcomes(
            ...
            match fact {
                ...
                ProofFact::Proposition(application) => {
                    if matches!(contract.kind, SignatureContractKind::Requires) {
                        ...
                        stand_downs.push(crate::validation::ContractEntailmentStandDown {
                            machine_symbol: machine.symbol,
                            contract_index,
                            fact_index,
                            reason:
                                crate::validation::ContractEntailmentStandDownReason::UnsupportedEnsuresFact,
                        });
                    }
    ...
    let judge_structural = |fact: ExpressionHandle| -> StructuralJudgment {
            ...
            && let ExpressionNode::Binary(comparison) = program.expression_table.expression(fact)
        {
            ...
            if !crate::validation::reserved_result_place(program, comparison.left)
    ...
    let ensures: Vec<ExpressionHandle> = ensures
        ...
        .filter(|fact| {
            ...
            let Some(held) = mention else {
                ...
                if crate::validation::machine_calls::fact_call_projections::expression_contains_call_projection(
                    program, *fact,
            ...
            Judgment::Unknown { goal_in_language } => {
                if goal_in_language && requires_fully_visible {
                    ...
                    record_expression_stand_down(
                        machine,
                        *fact,
                        crate::validation::ContractEntailmentStandDownReason::OutsideEntailmentLanguage,
                        &ensures_coordinates,
                        stand_downs,
                    );
                }
...
fn fact_call_targets(program: &TypedTrees, expression: ExpressionHandle) -> FactCallTargets {
    ...
    match program.expression_table.expression(expression) {
        ExpressionNode::Match(dispatch) => merge(
            &mut crate::validation::value_custody::expression_types::match_children(
                program, *dispatch,
            ),
        ),
        ...
        ExpressionNode::Call(call) => {
            let declared = call.target_symbol.is_valid()
                || symbol_resolved_trees_to_typed_trees::typed_trees::operator::resolve_named_expression_call(program, call).is_some()
                || program
                    .machines()
                    .iter()
