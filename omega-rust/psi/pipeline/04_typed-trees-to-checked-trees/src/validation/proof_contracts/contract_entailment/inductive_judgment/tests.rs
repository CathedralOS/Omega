...
fn mutating_guards_cannot_make_a_reachable_bad_return_vacuous() {
    ...
    let machine = typed
        ...
        .unwrap();
    assert!(
        crate::validation::proven_machine_contract_expressions(&typed, machine.symbol).is_empty(),
...
fn prove(source: &str, machine_name: &str) -> (usize, usize) {
    ...
    let machine = typed
        ...
        .unwrap();
    (
        crate::validation::proven_machine_contract_expressions(&typed, machine.symbol).len(),
        crate::validation::collect_contract_entailment_stand_downs(&typed).len(),
    )
}
