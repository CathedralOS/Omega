...
pub(super) fn endpoint_statically_formed(
    ...
    crate::validation::proof_contracts::arithmetic_domains::declared_integer_expression_lands(
...
fn operations_land_under(
    ...
    if shifting {
        ...
        let Some(width) = primitive
            .and_then(crate::validation::proof_contracts::arithmetic_domains::integer_bit_width)
        else {
            return false;
        };
...
impl<'program> FieldCoordinate<'program> {
    ...
    pub(super) fn comparisons(&self, program: &TypedTrees) -> Vec<Comparison> {
        ...
        if let Some((minimum, maximum)) =
            crate::validation::enforced_integer_type_bounds(program, self.field.type_reference)
        {
            comparisons.extend([
                (
                    BinaryOperator::GreaterOrEqual,
