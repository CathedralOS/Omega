...
use symbol_resolved_trees_to_typed_trees::typed_trees::data::{
    DataDefinition, DataField, DataMember, DataVariant,
};
use symbol_resolved_trees_to_typed_trees::typed_trees::expression::{
    ExpressionHandle, TableStructLiteral,
};
...
pub(in super::super) fn is_case_observation(
    ...
    matches!(program.expression_table.expression(expression), symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Binary(binary) if binary.operator == symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::CaseMembership)
        || program
            .expression_table
...
fn zero_term(
    ...
    if crate::validation::value_custody::data::type_requires_establishment(program, type_reference)
    {
        return None;
    }
        ...
        for constraint in program.type_reference_table.constraints(*constraints) {
            match constraint {
                ...
                TypeConstraintNode::Range {
                    ...
                    let maximum = crate::validation::closed_integer_range_maximum(
                        program,
                        *maximum,
                        *end_inclusive,
                    )?;
    ...
    if crate::validation::value_custody::data::data_requires_establishment(program, definition) {
        return None;
    }
    let variant = program
        .data_members(definition)
