...
pub(super) fn quotient_equality_from_requires(
    ...
    proposition_requires: &[&symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionApplication],
    ...
    let proposition_match = proposition_requires.iter().any(|application| {
        ...
        let forward =
            crate::validation::proof_contracts::quotients::exact_relation_application_matches(
                program,
                application,
                relation.relation_symbol,
                left_symbol,
...
fn transparent_relation_fact_matches(
    ...
    let symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionBody::Transparent {
        proposition: symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionFormula::BooleanExpression(formula),
    } = relation.body
    else {
        return false;
    };
...
fn relation_fact_call(
    program: &TypedTrees,
    fact: ExpressionHandle,
) -> Option<&symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableCallExpression> {
