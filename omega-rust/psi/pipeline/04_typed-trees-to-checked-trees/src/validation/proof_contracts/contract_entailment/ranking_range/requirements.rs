...
fn prove(
    ...
    let site_invariant =
        crate::validation::machine_calls::call_cycles::runtime_ranking::ranged_member_site_invariant(
            program,
            caller,
            caller_state,
        );
    ...
    for binding in &caller_bindings {
        ...
        if let Some((minimum, maximum)) =
            crate::validation::enforced_integer_type_bounds(program, parameter.type_reference)
        {
            let StrictArithmeticBindingValue::Atom { identity, .. } = &binding.value else {
                return None;
            };
