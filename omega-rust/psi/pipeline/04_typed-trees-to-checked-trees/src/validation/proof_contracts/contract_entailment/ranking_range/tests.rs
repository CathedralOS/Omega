...
fn slice_length_projection_does_not_numeric_bind_its_descriptor() {
    let program = crate::validation::front_end::typed_program(
        "machine length(values: &[u8]) -> u64 { values.len }",
    );
    let machine = &program.machines()[0];
    let state = &program.machine_states(machine)[0];
...
fn slice_projection_checks_subslice_geometry_before_using_its_length() {
    let program = crate::validation::front_end::typed_program(
        "machine tail(values: &[u8]) -> &[u8] { values[1..] }",
    );
    let machine = &program.machines()[0];
    let state = &program.machine_states(machine)[0];
...
mod scalar_views {
    ...
    fn computation_bodies_classify_only_when_strictly_increasing_and_builtin() {
        let program = crate::validation::front_end::typed_program(DOUBLED);
        ...
        for body in [
            ...
            let program =
                crate::validation::front_end::typed_program(&DOUBLED.replace("value * 2", body));
            assert!(
        ...
        let identity = crate::validation::front_end::typed_program(
            &DOUBLED.replace("{ value * 2 }", "{ value }"),
        );
        ...
        let widened = crate::validation::front_end::typed_program(
        ...
        let authored = crate::validation::front_end::typed_program(&format!(
    ...
    fn computed_ranks_prove_membership_descent_and_carrier_formation() {
        ...
        let narrow = crate::validation::front_end::typed_program(
            &DOUBLED.replace("in 0..=200", "in 0..=100"),
        );
        assert!(!entry(&narrow));
        ...
        let overflowing = crate::validation::front_end::typed_program(
            &DOUBLED
                .replace("[0..=100]", "[0..=200]")
        ...
        let unbounded = crate::validation::front_end::typed_program(
        ...
        let stalled = crate::validation::front_end::typed_program(
...
mod field_views {
    ...
    fn nested_projection_ranks_and_pins_through_the_exact_chain() {
        ...
        let stalled = crate::validation::front_end::typed_program(&NESTED.replace(
        ...
        let unrelated = crate::validation::front_end::typed_program(&NESTED.replace(
            "requires countdown.inner.remaining <= countdown.bounds.limit;",
        ...
        let foreign = crate::validation::front_end::typed_program(&format!(
    ...
    fn computed_endpoint_lands_under_a_flow_bound_not_in_the_declaration() {
        ...
        let program = crate::validation::front_end::typed_program(FLOW_BOUND);
        assert!(entry(&program));
        let proof = self_edge(&program).expect("self edge");
        assert!(proof.membership_and_pinning && proof.strictly_decreases);
        // Without the flow bound the same endpoint can overflow its carrier.
        let unbounded = crate::validation::front_end::typed_program(
            &FLOW_BOUND.replace(" && countdown.limit <= 5", ""),
        );
        assert!(!entry(&unbounded));
    }
    ...
    fn computed_endpoint_operations_land_under_the_same_hypotheses() {
        ...
        let overflowing = crate::validation::front_end::typed_program(&FLOW_BOUND.replace(
            "countdown.limit + 1",
        ...
        // polynomial.
        let underflowing = crate::validation::front_end::typed_program(
        ...
        let mixed = crate::validation::front_end::typed_program(
        ...
        let anonymous_carrier = crate::validation::front_end::typed_program(
        ...
        // proves.
        let landing = crate::validation::front_end::typed_program(
    ...
    fn borrowed_subject_reads_its_referent_and_arrives_as_a_borrow() {
        let program = crate::validation::front_end::typed_program(BORROWED);
        assert!(entry(&program));
        let proof = self_edge(&program).expect("self edge");
        assert!(proof.membership_and_pinning && proof.strictly_decreases);
        let replaced =
            crate::validation::front_end::typed_program(&BORROWED.replace("}, ceiling)", "}, 5)"));
        assert!(self_edge(&replaced).is_none());
        // The formal itself forwarded keeps the referent, so no descent.
        let forwarded = crate::validation::front_end::typed_program(
        ...
        let owned = crate::validation::front_end::typed_program(
            &BORROWED.replace("walk(&Card {", "walk(Card {"),
        );
...
mod remainder_endpoints {
    ...
    fn remainder_endpoints_transport_through_value_equal_actuals() {
        ...
        let program = crate::validation::front_end::typed_program(COMPUTED_COPY);
        assert!(entry(&program));
        for premises in [
            RankingRangePremises::RankInvariant,
            RankingRangePremises::EntryInvariant,
        ...
        let moved = crate::validation::front_end::typed_program(MOVED_COPY);
        assert!(entry(&moved));
    ...
    fn runtime_remainder_transports_dividend_and_divisor_independently() {
        ...
        let unguarded = crate::validation::front_end::typed_program(&source);
        ...
        for changed in [
            ...
            let program = crate::validation::front_end::typed_program(&changed);
            assert!(
                entry(&program),
    ...
    fn runtime_remainder_actuals_substitute_at_the_edge() {
        ...
        let zeroed = crate::validation::front_end::typed_program(
            &source.replace("cap % step)", "cap % (step - step))"),
        );
    ...
    fn runtime_quotient_actuals_substitute_at_the_edge() {
        ...
        let moved = crate::validation::front_end::typed_program(
            &source
...
mod moved_record_copy {
    ...
    fn a_rebuilt_literal_names_the_moved_copy_of_a_required_entry() {
        let program = crate::validation::front_end::typed_program(PACKED);
        let remaining =
    ...
    fn a_rebuilt_literal_without_a_step_keeps_both_claims_contested() {
        ...
        let program = crate::validation::front_end::typed_program(
            &PACKED.replace("pair.left - 1", "pair.left"),
        );
    ...
    fn contested_stepped_copies_transport_one_proved_value() {
        ...
        let program = crate::validation::front_end::typed_program(TWO_LITERALS);
        let remaining =
            program.state_parameters(&program.machine_states(machine(&program))[0])[0].symbol;
        for premises in [
            RankingRangePremises::RankInvariant,
        ...
        let independent = crate::validation::front_end::typed_program(&TWO_LITERALS.replace(
        ...
        let divergent = crate::validation::front_end::typed_program(&TWO_LITERALS.replace(
    ...
    fn a_literal_reseed_keeps_the_stale_copy_contested() {
        ...
        // its equal.
        let program = crate::validation::front_end::typed_program(STEPPED_STALE);
        let remaining =
            program.state_parameters(&program.machine_states(machine(&program))[0])[0].symbol;
        let mappings = mappings(&program, RankingRangePremises::RankInvariant).expect("telescope");
        assert_eq!(mappings[1], vec![remaining, remaining]);
