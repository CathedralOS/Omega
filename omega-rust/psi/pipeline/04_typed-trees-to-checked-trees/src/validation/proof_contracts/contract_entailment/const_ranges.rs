...
pub(crate) fn const_range_bound_is_supported(
    ...
    crate::validation::closed_integer_range_bound(program, expression).is_some()
...
fn scope_engine<'program>(
    ...
    let mut bind_scoped_integer = |symbol, type_reference| {
        ...
        if let Some((minimum, maximum)) =
            crate::validation::proof_contracts::arithmetic_domains::enforced_integer_type_bounds(
                program,
                type_reference,
            )
        {
...
fn scoped_value_bound(
    ...
    program
        ...
        || program
            ...
            .iter()
            .any(|statement| {
                matches!(statement, symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::LocalData(local) if local.symbol == path.symbol)
            })
}
...
fn value_bounds(
    ...
    if (crate::validation::closed_integer_range_bound(program, value).is_some()
    ...
    if let ExpressionNode::Call(call) = program.expression_table.expression(value) {
        ...
        let (_, endpoints, end_inclusive) =
            crate::validation::declared_integer_range(program, entry.return_type)?;
        return Some([
    ...
    if let Some((_, endpoints, end_inclusive)) =
        crate::validation::declared_integer_range(program, reference)
    {
        return Some([
            term(
                program,
...
fn call_bindings(
    ...
    for (argument, parameter) in arguments.iter().zip(
        ...
        if let Some(actual_type) = value_type(program, caller, state, *argument) {
            use symbol_resolved_trees_to_typed_trees::typed_trees::types::FixedArrayLength;
            if let (
        ...
        let Some((carrier, required, required_end_inclusive)) =
            crate::validation::declared_integer_range(
                program,
        ...
        let Some((actual_carrier, actual, actual_end_inclusive)) =
            crate::validation::declared_integer_range(program, unreferenced(program, actual_type))
        else {
            continue;
        };
...
pub(crate) fn validate_const_range_call_in_environment(
    ...
    let proven = (|| {
        ...
        for (argument, parameter) in arguments.iter().zip(
            ...
            let (_, endpoints, end_inclusive) = crate::validation::declared_integer_range(
                program,
                unreferenced(program, parameter.type_reference),
            )?;
            let required = [
