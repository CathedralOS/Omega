...
use crate::validation::machine_calls::calls::machine_state_by_symbol;
        ...
        | StructuralTerm::CallProjection {
            ...
            } = term
            {
                ...
                if !program.data_members(data).iter().any(|member| {
                    matches!(member, symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(candidate)
...
fn arithmetic_requirement(
        ...
        .chain(state)
    {
        for parameter in program.state_parameters(caller_state) {
            let Some(primitive) = crate::validation::value_custody::recasts::exact_primitive_type(
                program,
