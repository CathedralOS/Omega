...
use crate::validation::proven_machine_contract_expressions;
...
fn unsupported_body_does_not_become_a_positive_outcome() {
    ...
    assert!(!crate::validation::collect_contract_entailment_stand_downs(&program).is_empty());
}
...
fn inherited_law_matches_retain_exact_authored_proof_roots() {
    ...
    let matches = crate::validation::matched_machine_law_guarantees(&program, machine.symbol);
...
fn inherited_law_matching_does_not_prove_a_false_authored_claim() {
    ...
    let machine = &program.machines()[0];
    assert_eq!(
        crate::validation::matched_machine_law_guarantees(&program, machine.symbol).len(),
        1
    );
    assert!(
...
fn inherited_law_matching_rejects_exact_requirement_drift() {
    ...
    assert!(crate::validation::matched_machine_law_guarantees(&program, machine.symbol).is_empty());
}
