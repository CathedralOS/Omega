use super::{
    ...
    TransitionTargetNode, TypedTrees, is_arm_pattern_marker,
};
use crate::validation::proof_contracts::contract_entailment::stand_downs::{
    record_all_expression_stand_downs, record_expression_stand_down,
};
#[cfg(test)]
mod tests;
...
fn record_inductive_stand_downs(
    ...
    machine: &Machine,
    ensures: &[ExpressionHandle],
    reason: crate::validation::ContractEntailmentStandDownReason,
...
fn discharges_strict_decrease(
    ...
    let Some(decreases) = symbol_resolved_trees_to_typed_trees::typed_trees::ranking::resolve_machine_witness_subjects(program, machine)
    else {
        return false;
    };
    let order = witness
    ...
    let measure = if increasing_to {
        let Some(arguments) =
            symbol_resolved_trees_to_typed_trees::typed_trees::ranking::resolve_machine_witness_view_arguments(program, machine)
        else {
            return false;
        };
