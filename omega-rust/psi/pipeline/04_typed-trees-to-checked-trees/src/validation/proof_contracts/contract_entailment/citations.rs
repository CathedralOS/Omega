...
use crate::validation::proof_contracts::contract_entailment::RESULT_BINDER;
use crate::validation::proof_contracts::contract_entailment::law_conformance::{
    diagnostic_shape_match, display_structural_term, term_mentions_variable,
};
use crate::validation::proof_contracts::contract_entailment::refuted_requires::collect_instantiated_conjuncts;
...
pub(super) struct CitationTarget {
    symbol: SymbolHandle,
    machine_arguments:
        Vec<symbol_resolved_trees_to_typed_trees::typed_trees::expression::StaticMachineArgument>,
...
pub(crate) fn intake_citation_for_edge(
    ...
    let facts = |kind: symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContractKind| {
        program
    ...
    for fact in facts(symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContractKind::Requires) {
    ...
    for fact in facts(symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContractKind::Ensures) {
        if super::structural_terms::is_case_observation(program, fact) {
            continue;
        }
        let ExpressionNode::Binary(binary) = program.expression_table.expression(fact) else {
...
pub(crate) fn intake_statement_citation_for_edge(
    ...
    let collect = |kind: symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContractKind| {
        program
            .machine_contracts(callee)
            .iter()
            .filter(|contract| contract.kind == kind)
    ...
    for fact in collect(symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContractKind::Requires) {
        if super::structural_terms::is_case_observation(program, fact) {
            return;
        }
        let ExpressionNode::Binary(binary) = program.expression_table.expression(fact) else {
    ...
    for fact in collect(symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContractKind::Ensures) {
        if super::structural_terms::is_case_observation(program, fact) {
            continue;
        }
        let ExpressionNode::Binary(binary) = program.expression_table.expression(fact) else {
