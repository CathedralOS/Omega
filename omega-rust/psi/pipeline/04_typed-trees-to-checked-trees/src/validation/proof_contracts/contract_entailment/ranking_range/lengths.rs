...
fn call_receiver_is_mutable(program: &TypedTrees, target_symbol: SymbolHandle) -> bool {
    crate::validation::machine_calls::calls::machine_state_by_symbol(program, target_symbol)
        .map(|(_, callee)| {
            program
        ...
        while let Some((expression, depth)) = pending.pop() {
            ...
            match program.expression_table.expression(expression) {
                ExpressionNode::Member(member) => {
                    let resolved = crate::validation::value_custody::places::collection_length_receiver(
                        program,
                        machine,
                        Some(state),
                        expression,
                    ...
                    .or_else(|| {
                        let receiver = crate::validation::value_custody::places::collection_length_receiver(
                            program,
                            machine,
                            Some(root),
                            expression,
