...
pub fn measure_constraints_cover_subject(
    ...
    let Some((subject_low, subject_high)) =
        crate::validation::immutable_integer_expression_bounds(program, machine, state, subject)
    else {
        return false;
    };
    constraints.iter().all(|(minimum, maximum, end_inclusive)| {
        let Some((minimum, minimum_high)) = crate::validation::immutable_integer_expression_bounds(
            program, machine, state, *minimum,
        ) else {
            return false;
        };
        let Some((maximum_low, mut maximum)) =
            crate::validation::immutable_integer_expression_bounds(
                program, machine, state, *maximum,
            )
        else {
            return false;
        };
...
pub fn identity_subject_matches(
    ...
    let reference = if let ExpressionNode::Name(path) = program.expression_table.expression(subject)
    {
        ...
        crate::validation::expression_result_type_reference(program, machine, state, subject)
    };
...
fn owning_machine<'program>(
    program: &'program TypedTrees,
    state: &State,
) -> Option<&'program symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine> {
