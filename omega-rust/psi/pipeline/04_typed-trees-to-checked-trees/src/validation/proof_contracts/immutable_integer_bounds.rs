...
struct BoundMaps<'program> {
    ...
    /// Positions behind `projected_integer_bound_field`: a projected
    /// receiver's declared type resolves to the data definition, machine, or
    /// machine-attached data naming it. First matches win, as the scans did.
    data_by_symbol: HashMap<SymbolHandle, usize>,
    machines_by_symbol: HashMap<SymbolHandle, usize>,
    data_by_name: HashMap<&'program str, usize>,
}
...
impl<'program> ImmutableBoundLookup<'program> {
    ...
    fn maps(&self) -> &BoundMaps<'program> {
        self.maps.get_or_init(|| BoundMaps::build(self.program))
    }
...
fn projected_integer_bound_field<'program>(
    ...
    SymbolHandle,
    &'program symbol_resolved_trees_to_typed_trees::typed_trees::data::DataField,
    bool,
)> {
...
pub fn indexed_integer_bound_root(
    ...
    lookup: &ImmutableBoundLookup<'_>,
    expression: ExpressionHandle,
) -> Option<(SymbolHandle, Vec<crate::fact_plan::PlaceSegment>, bool)> {
    ...
    segments.push(crate::fact_plan::PlaceSegment::FixedIndex { index });
    let receiver = crate::validation::value_custody::places::unwrapped_type_reference(
        program,
        type_reference,
    )?;
