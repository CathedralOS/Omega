...
pub(in crate::validation::proof_contracts::quotients) struct RepresentativePreconditionPartition {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::validation::proof_contracts::quotients) struct DefinePreconditionCorrespondence {
...
fn derive_public_precondition_partition_from_positions(
    ...
    for parameter in public_parameters {
        if !public_positions.contains(&Some(parameter.symbol))
            && crate::validation::proof_contracts::quotients::legacy_candidates::quotient_for_type(
                program,
...
fn expression_depends_on_any(
    ...
    match program.expression_table.expression(expression) {
        ExpressionNode::Match(dispatch) => {
            crate::validation::value_custody::expression_types::match_children(program, *dispatch)
