...
pub(in crate::validation::proof_contracts::quotients) struct DefineRuntimeCorrespondence {
...
pub(super) enum ClosedLiftLiteral {
    ...
    ByteString {
        ...
        target_type: symbol_resolved_trees_to_typed_trees::typed_trees::type_identity::NormalizedTypeIdentity,
    },
    FixedByteArray {
        bytes: std::sync::Arc<[u8]>,
        target_type: symbol_resolved_trees_to_typed_trees::typed_trees::type_identity::NormalizedTypeIdentity,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::validation::proof_contracts::quotients) struct DirectLiftRuntimeCorrespondence {
...
pub(super) fn closed_lift_literal_for_representative(
    ...
    if let ExpressionNode::String(bytes) = program.expression_table.expression(expression) {
        let exact_target = exact_shared_byte_slice(program, representative_type)
            || crate::validation::value_custody::expression_types::bounded_byte_buffer_capacity(
                program,
                representative_type,
            )
...
fn quotient_carrier_matches_type(
    ...
    let Some(carrier_symbol) =
        crate::validation::proof_contracts::quotients::equivalence_selection::base_data_symbol(
            program,
            metadata.carrier,
        )
    else {
