...
pub(in crate::validation::proof_contracts::quotients) struct VerifiedTheoremSchema {
...
pub(super) fn verify_selected_theorem_schema(
    ...
    for (position, premise) in expected.relation_premises.iter().enumerate() {
        ...
        let Some(found) = requires.iter().position(|actual| {
            crate::validation::proof_contracts::quotients::equivalence_selection::fact_is_exact_relation_pair(
                program,
                actual.fact,
                relation,
                left,
