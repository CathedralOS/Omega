...
pub(super) use terminal_bridge::canonical_direct_correspondence;
...
pub(super) fn derive_direct_terminal_plan(
    ...
    let theorem_operational = crate::validation::infer_operational_may(program);
    let theorem_reaches = crate::validation::infer_service_reaches(program, &theorem_operational);
    let theorem_evidence = request
        ...
        .map(|evidence| {
            ...
            let crash_free =
                crate::validation::machine_calls::denotational_calls::has_no_crash_routes(
                    program,
...
> {
    ...
    for (position, argument) in program
        ...
        .enumerate()
    {
        let argument_type = crate::validation::value_custody::places::declared_place_type_raw(
            program,
            machine,
            Some(state),
            *argument,
