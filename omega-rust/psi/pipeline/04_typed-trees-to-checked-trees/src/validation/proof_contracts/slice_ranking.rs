...
fn tail_bound(
    program: &TypedTrees,
    bound_lookup: &crate::validation::proof_contracts::immutable_integer_bounds::ImmutableBoundLookup<'_>,
    expression: ExpressionHandle,
) -> Option<TailBound> {
    if let Some(value) = crate::validation::normalize_immutable_integer_bound_to_usize(
        program,
        bound_lookup,
        expression,
    ) {
    ...
    if let Some(bound) =
        crate::validation::immutable_integer_bound_symbol_offset(program, bound_lookup, expression)
    {
    ...
    if let Some(leaf) = crate::validation::normalize_immutable_integer_bound_expression(
        program,
        bound_lookup,
        expression,
    ) {
...
fn bound_symbol_floor(program: &TypedTrees, symbol: SymbolHandle) -> Option<i64> {
    let type_reference =
        crate::validation::value_custody::places::bound_symbol_declared_type(program, symbol)?;
    ...
    let declared = crate::validation::proof_contracts::arithmetic_domains::enforced_declared_range(
        program,
        type_reference,
    )
