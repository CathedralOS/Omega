...
use crate::validation::proof_contracts::default_domains::assignment_windows::handle_assignment;
use crate::validation::proof_contracts::default_domains::assignment_windows::preserve_proven_establishment;
use crate::validation::proof_contracts::default_domains::assignment_windows::refuse_open_windows;
use crate::validation::proof_contracts::default_domains::data_reads::attached_value_established;
use crate::validation::proof_contracts::default_domains::data_reads::scan_statement_reads;
...
fn analyze_default_domain_writes(
    ...
    let call_frames = crate::validation::machine_calls::calls::CallFrameResolver::new(program);
...
fn walk_state(
    program: &TypedTrees,
    call_frames: Option<&crate::validation::machine_calls::calls::CallFrameResolver<'_>>,
    machine: &Machine,
    state: &State,
                ...
                if let Some(written) = written {
                    tracked.retain(|place| {
                        !written.iter().any(|written| {
                            crate::validation::machine_calls::calls::frame_paths_overlap(
                                &place.spelling,
                                written,
                            )
                        })
    ...
    if !poisoned_all {
        for (spelling, fields, active_case) in entry_valuations {
            let poisoned = poisoned_paths.iter().any(|written| {
                crate::validation::machine_calls::calls::frame_paths_overlap(spelling, written)
            });
