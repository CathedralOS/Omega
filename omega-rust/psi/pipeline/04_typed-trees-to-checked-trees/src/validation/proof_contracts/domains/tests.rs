...
use crate::fact_plan::{Fact, FactOrigin, FactPayload, FactPlan, FactRef};
...
fn repeated_capacity_domains(second_fact: &str) -> TypedTrees {
    crate::validation::front_end::typed_program_from_texts(&[&format!(
...
fn domain_predicate_places_use_the_carrier_type_under_logical_operators() {
    for (carrier, fields, predicate, accepted) in [
        ...
        let program = crate::validation::front_end::typed_program_from_texts(&[&format!(
            "data Player {{ {fields} }} domain {carrier}::Selected requires {predicate};"
        )]);
        let fact_plan = crate::validation::build_definition_fact_plan(&program);
...
fn sibling_sources_in_one_module_keep_the_same_consistency_group() {
    let program = crate::validation::front_end::typed_program_from_texts(&[
...
fn different_module_owners_cannot_share_a_tampered_semantic_identity() {
    let mut program = crate::validation::front_end::typed_program_from_texts(&[
