...
use crate::validation::proof_contracts::proof_facts::{
    ProofFactOwner, validate_domain_fact_payloads,
};
...
fn validate_domain_aliases(
    ...
    for domain in program.domain_definitions() {
        ...
        if !symbol_resolved_trees_to_typed_trees::typed_trees::domain::index_parameters(
            program, domain,
        )
        .is_empty()
        {
...
) -> Option<&'program symbol_resolved_trees_to_typed_trees::typed_trees::data::TypeParameter> {
...
fn same_normalized_domain_owner(
    program: &TypedTrees,
    left: &symbol_resolved_trees_to_typed_trees::typed_trees::domain::DomainDefinition,
    right: &symbol_resolved_trees_to_typed_trees::typed_trees::domain::DomainDefinition,
) -> bool {
...
fn normalized_domain_facts(
    ...
    let mut facts = fact_plan
        ...
        .filter_map(|fact| match fact.payload {
            crate::fact_plan::FactPayload::BooleanExpression(expression) => Some(format!(
                "bool:{}",
...
fn domain_membership_facts(
    fact_plan: &FactPlan,
    symbol: SymbolHandle,
) -> impl Iterator<Item = crate::fact_plan::DomainMembershipFact> + '_ {
...
mod unmanaged_root_tests {
    ...
    fn packaged(entries: &[(&str, &str)]) -> TypedTrees {
        ...
        crate::validation::front_end::typed_program_from_merged_source_map(map, &texts)
    }
