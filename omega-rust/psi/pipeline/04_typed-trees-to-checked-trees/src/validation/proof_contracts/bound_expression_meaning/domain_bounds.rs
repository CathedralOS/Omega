    ...
    let subject_type = match program.expression_table.expression(expression) {
        ExpressionNode::Name(path) => {
            ...
            let Some(subject_type) =
                crate::validation::value_custody::places::bound_symbol_declared_type(
                    program,
                    path.symbol,
                )
            else {
        ...
        ExpressionNode::Member(_) => {
            let Some(subject_type) =
                crate::validation::proof_contracts::immutable_integer_bounds::projected_integer_bound_subject_type(
                    program,
                    bound_lookup,
                    expression,
                )
...
fn integer_carrier(
    ...
    for _ in 0..128 {
        match program.type_reference_table.type_reference(reference) {
            ...
            _ => {
                let primitive = crate::validation::value_custody::recasts::exact_primitive_type(
                    program, reference,
                )?;
