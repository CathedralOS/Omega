...
fn result_bound_operand_types_require_exact_primitive_and_field_contract_owners() {
    let mut program = crate::validation::front_end::typed_program(
    ...
    for (owner_index, foreign_index) in [(0, 1), (2, 3)] {
        ...
        if matches!(
            ...
            assert!(crate::validation::reserved_result_place(&program, copied).is_none());
            let foreign = &program.machines()[foreign_index];
            assert!(
                super::operand_type(
                    &program,
...
fn membership_rejoins_the_nominal_subject_owner_and_case_symbols() {
    ...
    for replacement in [
        ...
        *altered.expression_table.expression_mut(comparison.right) =
        ...
        crate::validation::value_custody::struct_literals::validate_struct_literal_fields(
            &altered,
            &mut diagnostics,
        );
        assert!(
    ...
    *altered.expression_table.expression_mut(comparison.right) = ExpressionNode::Name(
        symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableNamePath {
            symbol: foreign_case.symbol,
            member_symbols,
            ..*original_case
...
fn membership_constructor_subject_retains_its_own_owner_and_case() {
    ...
    assert!(is_exact_membership(&program, member));
    for replacement in [
        symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableStructLiteral {
