...
//! `symbol_resolved_trees_to_typed_trees::typed_trees::proof_only`, not refused here.
...
pub(crate) fn validate_proof_only_consumption(
    ...
    for machine in program.machines() {
        ...
        if crate::validation::value_custody::content_projections::is_content_projection_machine(
            program, machine,
        ) {
            continue;
        }
    ...
    for schema in program.wire_schemas() {
        for member in program.wire_members(schema.members) {
            let symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireMember::Field(field) =
                member
            else {
                continue;
            };
