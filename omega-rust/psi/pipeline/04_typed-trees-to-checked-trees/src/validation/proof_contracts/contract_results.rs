...
pub struct ReservedResultPlace {
    ...
    pub segments: Vec<crate::fact_plan::PlaceSegment>,
}
...
pub fn reserved_result_place(
    ...
    for member in members.into_iter().rev() {
        let receiver = crate::validation::value_custody::places::unwrapped_type_reference(
            program,
            type_reference,
        )?;
        ...
        let field = crate::validation::value_custody::places::exact_data_member_field(
            program,
            data,
            ...
            for fact in program.proof_facts.span_or_empty(contract.facts) {
                match fact {
                    ProofFact::Expression(root) => {
                        crate::validation::value_custody::expression_types::collect_expression_nodes(
...
mod tests {
    ...
    fn nested_result_fields_retain_exact_root_owner_and_coordinates() {
        let program = crate::validation::front_end::typed_program(PROJECTED_RESULT);
    ...
    fn projected_result_rejects_unauthored_graft_and_foreign_selection() {
        ...
        let symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
            foreign_field,
        ) = &program.data_members(foreign)[0]
        else {
            panic!("foreign field");
        };
    ...
    fn requirement_result_scalars_keep_exact_signature_owners() {
        let program = crate::validation::front_end::typed_program(REQUIREMENT_RESULTS);
            ...
            program
                ...
                .any(|(handle, _)| {
                    crate::validation::proof_contracts::proof_embeddings::integer_embedding_argument(
                        &program, handle,
                    ) == Some((symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::U64, expression))
                })
        );
        let mut grafted = program.clone();
