...
fn append_call_ensured_domain_atoms(
    ...
    for contract in contracts {
        ...
        for fact in program.proof_facts.span_or_empty(contract.facts) {
            ...
            // declared type.
            if crate::validation::proof_contracts::contract_results::reserved_result_owner(
                program,
                membership.value,
            )
            .is_none()
...
fn call_signature_contracts<'program>(
    ...
    Vec<&'program symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContract>,
> {
    if let Some((machine, state)) = crate::validation::machine_calls::calls::machine_state_by_symbol(
        program,
        call.target_symbol,
    ) {
        return Some(
...
fn expression_atoms(
    ...
    match program.expression_table.expression(value) {
        ...
        ExpressionNode::Call(call) => {
            if let Some(return_type) =
                crate::validation::proof_contracts::arithmetic_domains::call_return_type(
                    program, call,
                )
            {
    ...
    if let Some(source_type) = crate::validation::value_custody::places::declared_place_type_raw(
