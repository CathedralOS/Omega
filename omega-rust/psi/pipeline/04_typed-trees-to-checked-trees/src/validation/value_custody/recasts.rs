...
pub(crate) fn validate_recasts(program: &TypedTrees, diagnostics: &mut Vec<Diagnostic>) {
    ...
    let dynamic_rebindings =
        crate::validation::declarations::traits::collect_dynamic_conformance_selections(program)
            .unwrap_or_default();
                            ...
                            && matches!(
                                ...
                                ExpressionNode::Cast(cast)
                                    if cast.form.is_recast()
                                        && crate::validation::declarations::traits::dynamic_trait_symbol(
                                            program,
                                            cast.target_type,
                                        )
                                        .is_some()
                        ...
                        .type_reference(local.type_reference)
                {
                    ...
                    match program.expression_table.expression(initializer) {
                        ExpressionNode::Cast(cast) if cast.form.is_recast() => {
                            ...
                            if crate::validation::declarations::traits::dynamic_trait_symbol(
                                program,
                                cast.target_type,
                            )
                            .is_none()
