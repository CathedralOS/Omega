...
use symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableNamePath;
...
pub(super) fn field<'program>(
    ...
    crate::validation::exact_self_field(program, machine(program, root)?, expression)
}
...
fn attached_field<'program>(
    ...
    record(program, root)?;
    crate::validation::value_custody::places::exact_attached_field(
        program,
        machine(program, root)?,
        symbol,
        name,
    )
}
...
fn admits_call(program: &TypedTrees, owner: &DataDefinition, target: SymbolHandle) -> bool {
    let Some((callee, state)) =
        crate::validation::machine_calls::calls::machine_state_by_symbol(program, target)
    else {
        return false;
    };
