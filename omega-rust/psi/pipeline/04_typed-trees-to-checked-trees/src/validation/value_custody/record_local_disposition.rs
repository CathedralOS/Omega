...
use crate::checked_trees::{CheckFacts, CheckedStructuralAccess, CheckedStructuralValueKind};
...
pub fn record_local_disposition(
    ...
    if !local.symbol.is_valid()
        ...
        || !crate::validation::has_plain_owned_contents_with_numeric_constraints(program, local.type_reference)
    {
        return None;
    }
    let events = || {
...
fn local_provenance(
    ...
    let origin = crate::validation::expression_permission_provenance(
        program,
        local.initial_value,
        &mut |expression| {
