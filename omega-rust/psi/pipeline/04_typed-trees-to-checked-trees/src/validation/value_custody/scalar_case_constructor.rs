...
fn is_case_value(
    ...
    if !crate::validation::has_plain_owned_contents(program, reference) {
        return false;
    }
...
pub fn scalar_case_value_source(
    ...
    let source = crate::validation::plain_owned_value_source(program, expression, reference)?;
...
fn case_constructor_parts(
    ...
    let (owner, selected, fields) = match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Name(name) => (
            crate::validation::exact_case_reference_owner(program, expression)?,
            name.symbol,
            &[][..],
        ),
        _ => return None,
    ...
    if !owner.type_parameters.is_empty()
        || !crate::validation::has_plain_owned_contents_with_numeric_constraints(program, reference)
        || program
            .data_members(owner)
            .iter()
