...
fn domain_is_declared(
    ...
    constraint.symbol.is_valid()
        && program.domain_definitions().iter().any(|domain| {
            ...
            && if symbol_resolved_trees_to_typed_trees::typed_trees::domain::has_generic_carrier(
                program, domain,
            ) {
                constraint.arguments.len()
                    ...
                    .len()
            } else if symbol_resolved_trees_to_typed_trees::typed_trees::domain::index_parameters(
                program, domain,
            )
            .is_empty()
            {
...
pub(super) fn validate_type_constraints_node(
    ...
    for constraint in constraints {
        match constraint {
            ...
            TypeConstraintNode::Domain(name)
                if symbol_resolved_trees_to_typed_trees::typed_trees::wire::is_layout_domain_constraint(name) =>
            {
                ...
                match (
                    ...
                    _ if primitive_type.accepts_integer_literal() => {
                        ...
                        if matches!(
                            ...
                            crate::validation::proof_contracts::contract_entailment::const_range_bound_is_supported(
                                program,
...
fn validate_layout_domain_constraint(
    ...
    let Some((schema_name, grammar)) =
        symbol_resolved_trees_to_typed_trees::typed_trees::wire::layout_domain_constraint_arguments(
            program, domain,
        )
    else {
        return;
...
fn range_bound_pending_evaluation_shape(
    ...
    if crate::validation::closed_integer_range_bound(program, expression).is_some() {
        return (true, false);
    }
...
/// through the shared `symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges` recognizer.
fn dependent_state_parameter_range_error(
    program: &TypedTrees,
    owner: &TypeReferenceOwner<'_>,
    minimum: symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle,
    ...
    if crate::validation::closed_integer_range_bound(program, minimum)
    ...
    if let Some(scoped) =
        symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::scoped_range_maximum(
            &program.expression_table,
            maximum,
            end_inclusive,
        )
    ...
    let Some(symbolic) =
        symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::symbolic_range_maximum(
            &program.expression_table,
            maximum,
            end_inclusive,
        )
    ...
    let field_is_dischargeable = crate::validation::exact_attached_field(
        program,
        machine,
...
fn scoped_maximum_error(
    program: &TypedTrees,
    owner: &TypeReferenceOwner<'_>,
    scoped: symbol_resolved_trees_to_typed_trees::typed_trees::dependent_ranges::ScopedNameBound,
) -> Option<String> {
    ...
    let scalar_value = |type_reference: TypeReferenceHandle| {
        crate::validation::value_custody::places::unwrapped_type_reference(program, type_reference)
            .and_then(|unwrapped| program.primitive_type_reference(unwrapped))
            .is_some_and(|primitive| primitive.accepts_integer_literal())
    };
