...
pub fn normalize_open_index_expressions(program: &mut TypedTrees) -> Result<(), Vec<Diagnostic>> {
    ...
    for (expression, index_type) in sites {
        ...
        normalizations.push(symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::OpenIndexNormalization {
            expression,
            index_type,
            operations,
...
fn const_binder_owners(program: &TypedTrees) -> HashMap<SymbolHandle, SymbolHandle> {
    ...
    let mut seed = |parameters: &[symbol_resolved_trees_to_typed_trees::typed_trees::data::TypeParameter], owner: SymbolHandle| {
...
fn normalize_open_index_expression_operations(
    ...
    operations: &mut Vec<
        symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::OpenIndexOperationSelection,
    >,
    diagnostics: &mut Vec<Diagnostic>,
) {
    ...
    let laws = crate::validation::proof_contracts::contract_entailment::declared_index_algebra_laws(
        program,
        trait_definition,
    ...
    if !laws.commutativity.is_empty() || !laws.associativity.is_empty() {
        ...
        operations.push(symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::OpenIndexOperationSelection {
            expression,
            spelling,
