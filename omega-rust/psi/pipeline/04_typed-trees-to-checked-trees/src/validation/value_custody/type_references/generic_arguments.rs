...
pub(super) fn validate_machine_data_argument(
    ...
    crate::validation::machine_calls::machine_parameters::validate_data_machine_selection(
        program,
        base_name,
        parameter,
        contract,
            ...
            let eligible =
                ...
                .is_some_and(|definition| {
                    ...
                    definition.supply_mode == language_semantics::DataSupplyMode::CheckedShape
                        ...
                        && match symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition::shape_kind_from_members(members)
                        {
                            ...
                            symbol_resolved_trees_to_typed_trees::typed_trees::data::DataShapeKind::Empty
                            | symbol_resolved_trees_to_typed_trees::typed_trees::data::DataShapeKind::Mixed => false,
                        }
                });
            visiting.pop();
            eligible
...
fn exact_const_primitive(
    ...
    let primitive = match program.symbols.builtin_type_atom(*symbol) {
        Some(symbols::BuiltinTypeAtom::Bool) => PrimitiveType::Bool,
        Some(symbols::BuiltinTypeAtom::I8) => PrimitiveType::I8,
        Some(symbols::BuiltinTypeAtom::I16) => PrimitiveType::I16,
        Some(symbols::BuiltinTypeAtom::I32) => PrimitiveType::I32,
        Some(symbols::BuiltinTypeAtom::I64) => PrimitiveType::I64,
        Some(symbols::BuiltinTypeAtom::U8) => PrimitiveType::U8,
        Some(symbols::BuiltinTypeAtom::U16) => PrimitiveType::U16,
        Some(symbols::BuiltinTypeAtom::U32) => PrimitiveType::U32,
        Some(symbols::BuiltinTypeAtom::U64) => PrimitiveType::U64,
        Some(symbols::BuiltinTypeAtom::Address) => PrimitiveType::Addr,
        Some(_) => return None,
...
fn validate_const_integer_range(
    ...
    } = program.type_reference_table.type_reference(parameter_type)
    {
        for constraint in program.type_reference_table.constraints(*constraints) {
            ...
            let bounds = crate::validation::closed_integer_range_bound(program, *minimum).zip(
                crate::validation::closed_integer_range_maximum(program, *maximum, *end_inclusive),
            );
...
pub(super) fn validate_generic_argument_bounds(
    ...
    for (parameter, argument) in parameters.iter().zip(argument_handles) {
        let bounds = crate::validation::proof_contracts::properties::declared_property_requirements(
            &parameter.bounds,
        );
            ...
            for (literal, accepted) in [("1", false), ("2", true), ("3", true), ("4", false)] {
                let argument = program
                    ...
                    .insert(TypeReferenceNode::Named {
                        ...
                        name: symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier::generated(literal),
                    });
                let result =
