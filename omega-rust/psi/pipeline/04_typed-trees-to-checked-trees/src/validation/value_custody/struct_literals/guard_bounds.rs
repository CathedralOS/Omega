...
pub(super) fn has_immutable_inputs(
    ...
    match program.expression_table.expression(expression) {
            ...
            immutable_place_type(program, machine, state, expression).is_some_and(|handle| {
                ...
                if program.symbols.builtin_type_atom(*symbol).is_none() {
                    return symbol.is_valid()
                        &&
                            crate::validation::machine_calls::effect_inference::plan_scope::data_definition_by_symbol(program, *symbol).is_some();
                }
        ...
        ExpressionNode::Binary(binary) => {
            crate::validation::proof_contracts::bound_expression_meaning::has_builtin_binary_expression_meaning(
                program,
                machine,
                Some(state),
                expression,
