...
fn synthesized_payload_tags_are_not_constructor_values() {
    ...
    let generated: Vec<_> = program.expression_table.expression_entries().filter_map(|(expression, node)| {
        matches!(node, ExpressionNode::Binary(binary) if binary.operator == symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::CaseMembership)
...
fn array_elements_require_explicit_integer_carrier_conversions() {
    for source in [
        ...
        let program = crate::validation::front_end::typed_program(source);
        let diagnostics = crate::validation::validate_program(&program).expect_err(source);
        assert!(
...
fn array_elements_preserve_anonymous_landing_and_explicit_conversions() {
    for source in [
        ...
        let program = crate::validation::front_end::typed_program(source);
        assert!(
            crate::validation::validate_program(&program).is_ok(),
            "{source}"
        );
    }
}
