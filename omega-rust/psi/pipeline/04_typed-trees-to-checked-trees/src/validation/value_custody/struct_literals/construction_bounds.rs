...
pub(super) fn validate_literal_case_constraints(
    ...
    for fact in program.proof_facts.span_or_empty(variant.where_facts) {
        if let symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact::Expression(
            expression,
        ) = fact
        {
        ...
        match fact {
            ...
            symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact::Membership(
                ...
                let proven = authored_value.map_or_else(
                    || {
                        crate::validation::proof_contracts::proof_facts::domain_admits_empty_bytes(
                            program,
                            membership.domain_symbol,
                        )
                    },
                    |value| {
                        crate::validation::proof_contracts::proof_facts::string_literal_grants_domain(
                            program,
                            value,
                            membership.domain_symbol,
                        )
