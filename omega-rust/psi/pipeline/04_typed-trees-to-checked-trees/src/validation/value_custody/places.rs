...
use crate::validation::value_custody::struct_literals::data_declares_field;
...
use terminal_psi::access_plans::BorrowPolarity;
...
fn data_definition_by_symbol(
    ...
    match crate::validation::machine_calls::effect_inference::plan_scope::memoized_data_definition_lookup(
        program, symbol,
    ) {
        crate::validation::machine_calls::effect_inference::plan_scope::DataDefinitionLookup::Unique(
            position,
...
pub fn declared_place_type_raw(
    ...
    match program.expression_table.expression(handle) {
        ExpressionNode::ArrayLiteral(_) => {
            crate::validation::declared_constant_array_type(program, handle)
        }
...
pub(crate) use projected_members::declared_case_projection_field;
