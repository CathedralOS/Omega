...
use terminal_psi::access_plans::{AccessFieldEntry, FieldAccess};
...
pub(super) fn exact_access_entry_for_field<'view>(
    view: &'view symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::PlacedViewPlan,
    field: &symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::PlacedFieldPlan,
...
fn expected_accessor_operations(access: &FieldAccess) -> Vec<&'static str> {
    match access {
        ...
        FieldAccess::External { read, write, .. } => {
            let mut operations = Vec::new();
            match read {
                terminal_psi::access_plans::ExternalRead::None => {}
                terminal_psi::access_plans::ExternalRead::Read => operations.push("read"),
                terminal_psi::access_plans::ExternalRead::Take => operations.push("take"),
            }
            if *write {
                operations.push("write");
            }
...
pub(crate) fn validate_plans(program: &TypedTrees, diagnostics: &mut Vec<Diagnostic>) {
    for view in &program.placed_view_plans {
        ...
        if view.fields.len() != accessible_field_count
            ...
            || program.data_members(view_data).iter().any(|member| {
                matches!(
                    member,
