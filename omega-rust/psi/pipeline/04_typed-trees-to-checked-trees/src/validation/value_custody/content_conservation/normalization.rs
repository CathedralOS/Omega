...
fn normalize_projection_subject(
    ...
    let borrowed = match context.program.expression_table.expression(borrowed) {
        ExpressionNode::Borrow(inner)
            if inner.access == language_semantics::ReferenceAccess::Shared =>
        {
            inner.target
        }
    ...
    let carrier = crate::validation::value_custody::places::unwrapped_type_reference(
        context.program,
        final_type,
    )
...
fn structural_place_type(
    ...
    for segment in segments {
        let unwrapped =
            crate::validation::value_custody::places::unwrapped_type_reference(program, current)
        ...
        match segment {
            ContentPlaceSegment::Case(case) => {
                ...
                let data =
                    crate::validation::value_custody::places::data_definition_for_type(program, unwrapped)
                        .or_else(|| {
                            (at_root && self_data.is_valid())
                                .then(|| {

                                        crate::validation::machine_calls::effect_inference::plan_scope::data_definition_by_symbol(program, self_data)
                                })
                                .flatten()
                        })
                        .ok_or_else(|| {
                            format!("`{}` is selected from a non-sum carrier", case.name)
                        })?;
                let variant = program
                    .data_members(data)
                    .iter()
...
fn type_has_domain(
    ...
    domain: language_semantics::SemanticDomainId,
) -> bool {
    ...
    match program.type_reference_table.type_reference(type_reference) {
        ...
        TypeReferenceNode::Constrained {
            ...
            program
                ...
                .iter()
                .any(|constraint| matches!(constraint, symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeConstraintNode::Domain(candidate) if candidate.semantic_id == domain))
