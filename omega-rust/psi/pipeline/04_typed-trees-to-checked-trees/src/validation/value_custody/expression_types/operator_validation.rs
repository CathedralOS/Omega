...
pub(crate) fn validate_binary_operand_types(
    ...
    crate::validation::proof_contracts::arithmetic_domains::report_out_of_range_comparison_literal(
        program,
        machine,
        state,
        operator,
    ...
    crate::validation::proof_contracts::arithmetic_domains::report_mismatched_width_operands(
        program,
        machine,
        state,
        operator,
...
pub(super) fn expression_is_float_typed(
    ...
    match program.expression_table.expression(operand) {
        ...
        ExpressionNode::Call(call) => {
            crate::validation::proof_contracts::arithmetic_domains::call_return_type(program, call)
                .and_then(|return_type| program.primitive_type_reference(return_type))
                .is_some_and(|primitive| primitive.accepts_float_literal())
        }
        _ => crate::validation::value_custody::places::declared_place_type(
...
fn report_array_operator_operands(
    ...
    for operand in [left, right] {
        if let Some(operand_type) = crate::validation::value_custody::places::declared_place_type(
            program, machine, state, operand,
