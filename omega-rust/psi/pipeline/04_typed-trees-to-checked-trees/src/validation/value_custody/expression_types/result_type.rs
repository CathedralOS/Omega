...
mod domain_carrier_subjects {
    ...
    fn membership_subject<'program>(
        ...
        &'program symbol_resolved_trees_to_typed_trees::typed_trees::domain::DomainDefinition,
        ExpressionHandle,
    ) {
        let domain = program
            .domain_definitions()
...
enum ExpressionOwner<'program> {
    ...
    Domain(&'program symbol_resolved_trees_to_typed_trees::typed_trees::domain::DomainDefinition),
    Parameters {
        owner_symbol: symbols::SymbolHandle,
...
fn result_type(
    ...
    let result = match program.expression_table.expression(expression) {
        ExpressionNode::Match(dispatch) => {
            ...
            join_result_type_references(
                ...
                &arms
                    ...
                    .map(|(arm, reference)| {
                        reference.is_none()
                            && crate::validation::value_custody::literals::has_anonymous_numeric_results(
                                program, arm.value,
                            )
                    })
                    .collect::<Vec<_>>(),
        ...
        ExpressionNode::Call(call) => {
            crate::validation::machine_calls::calls::resolved_call_result_type(program, call).or_else(|| {
                symbol_resolved_trees_to_typed_trees::typed_trees::operator::resolve_named_expression_call(program, call).and_then(
                    |operator| {
        ...
        _ => match owner {
            ExpressionOwner::Parameters {
                owner_symbol,
                parameters,
            } => crate::validation::reserved_result_place(program, expression)
                .filter(|place| place.machine_symbol == owner_symbol)
                .map(|place| place.type_reference)
                .or_else(|| {
                    crate::validation::value_custody::places::parameter_scoped_type_reference(
                ...
                match program.expression_table.expression(expression) {
                    ...
                    ExpressionNode::Member(member) => {
                        result_type(program, owner, member.receiver, active).and_then(|receiver| {
                            if let Some(data) =
                                crate::validation::value_custody::places::data_definition_for_type(
                                    program, receiver,
                                )
                            {
                    ...
                    ExpressionNode::Indexed(indexed) => {
                        result_type(program, owner, indexed.collection, active).and_then(
                            |collection| match program.type_reference_table.type_reference(
                                crate::validation::value_custody::places::unwrapped_type_reference(
                                    program, collection,
                                )?,
                            ) {
...
fn binary_result(
        ...
        )
    {
        ...
        if !symbol_resolved_trees_to_typed_trees::typed_trees::operator::selected_trait_operator_meanings(
            program,
            owner.symbol(),
            spelling,
            &operands,
