...
pub(super) fn value_class(
    program: &TypedTrees,
    machine: Option<&symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine>,
    ...
    if let ExpressionNode::Binary(binary) = program.expression_table.expression(value) {
        ...
        if crate::validation::proof_contracts::arithmetic_domains::is_arithmetic(binary.operator) {
    ...
    if let ExpressionNode::Unary(unary) = program.expression_table.expression(value) {
        return match unary.operator {
            symbol_resolved_trees_to_typed_trees::typed_trees::expression::UnaryOperator::BitwiseNot => Some(ValueClass::Numeric),
            symbol_resolved_trees_to_typed_trees::typed_trees::expression::UnaryOperator::LogicalNot => Some(ValueClass::Boolean),
        };
    }
    ...
    let machine = machine?;
    let primitive = crate::validation::value_custody::places::declared_place_type(
...
pub(crate) fn report_cross_class_store(
    ...
    if let Some(machine) = machine
        && let Some(actual) = crate::validation::builtin_constant_array_projection_type(
            program,
            machine.symbol,
            value,
        )
