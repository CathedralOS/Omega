...
fn retained_primitive(program: &TypedTrees, expression: ExpressionHandle) -> Option<PrimitiveType> {
    ...
    crate::validation::value_custody::places::unwrapped_type_reference(program, reference)
...
pub fn validate_match_dispatch(
    ...
    if let Some(destination) = declared_value_type(program, machine, state, peer) {
        for arm in arms {
            crate::validation::proof_contracts::arithmetic_domains::validate_anonymous_integer_range(
                program,
                destination,
                arm.value,
                "typed match result",
...
fn plain_local_owner_selection(
    ...
    if crate::validation::scalar_case_constructor(program, expression).is_some() {
        ...
        ExpressionNode::Name(path) => {
            ...
            let admitted_source = match program.type_multiplicity(reference) {
                language_semantics::Multiplicity::Affine => {
                    crate::validation::affine_owned_value_source(program, expression, reference)
                        == Some(path.symbol)
                }
                language_semantics::Multiplicity::Linear => {
                    crate::validation::linear_owned_value_source(program, expression, reference)
                        == Some(path.symbol)
                }
                _ => false,
            };
        ...
        match program.expression_table.expression(cursor) {
            ...
            ExpressionNode::Name(path) => {
                ...
                return program.type_multiplicity(root_reference)
                    == language_semantics::Multiplicity::Affine
                    && crate::validation::has_linear_owned_contents(program, root_reference)
                    && crate::validation::value_custody::owned_value_source::whole_owned_value_source(
                        program,
                        cursor,
                        root_reference,
                    ) == Some(path.symbol)
    ...
    loop {
        match program.expression_table.expression(cursor) {
            ...
            ExpressionNode::Name(path) => {
                ...
                return program.type_multiplicity(root_reference)
                    == language_semantics::Multiplicity::Affine
                    && crate::validation::plain_owned_value_source(program, cursor, root_reference)
                        == Some(path.symbol)
                    && crate::validation::has_plain_owned_contents_with_numeric_constraints(
                        program,
                        root_reference,
...
fn selected_shared_borrow_place(
    ...
    if !crate::validation::has_linear_owned_contents(program, referent) {
        return false;
    }
...
pub(super) fn fresh_payloadless_case(
    ...
    if !crate::validation::has_plain_owned_contents_with_numeric_constraints(program, reference) {
        return None;
    }
...
fn has_unrouted_scalar_contents(program: &TypedTrees, mut reference: TypeReferenceHandle) -> bool {
    use symbol_resolved_trees_to_typed_trees::typed_trees::types::{
        DomainConstraintSubject, TypeConstraintNode, TypeReferenceNode,
    };
...
fn is_scalar_value(
    ...
    if let Some(reference) = declared_value_type(program, machine, state, expression) {
        return crate::validation::value_custody::places::unwrapped_type_reference(
            program, reference,
        )
        .and_then(|reference| program.primitive_type_reference(reference))
        .is_some();
    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Binary(_) => crate::validation::has_builtin_bound_expression_meaning(
            program,
            machine,
            Some(state),
            expression,
...
fn selected_expression_transfers_owned(
    ...
    while let Some(expression) = pending.pop() {
        ...
        match node {
            ExpressionNode::Call(call) => {
                let parameters = crate::validation::machine_calls::calls::machine_state_by_symbol(
                    program,
                    call.target_symbol,
                )
        ...
        crate::validation::value_custody::literals::expression_children::children(
            program,
            node,
        ...
        program
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(
                    case,
                ) if case.symbol == selected => Some(case.symbol),
                _ => None,
            })?;
    Some((reference, case))
}
...
fn case_dispatch_subject(
    ...
    if crate::validation::value_custody::scalar_case_constructor::scalar_case_constructor(
        program, subject,
    )
    .is_some()
    {
    ...
    match program.expression_table.expression(subject) {
        ...
        ExpressionNode::Member(member) => {
            ...
            if matches!(
                ...
                // field's own walls.
                crate::validation::value_custody::places::exact_self_field(
        ...
        ExpressionNode::Indexed(indexed) => {
            if !crate::validation::value_custody::places::has_retained_builtin_index_meaning(
                program, subject,
            ) {
                return None;
            }
...
fn check_compatible_values(
    ...
    let peer_type = declared_value_type(program, machine, state, peer).map(|reference| {
        crate::validation::value_custody::places::assignment_value_type(program, reference)
    });
    let value_type = declared_value_type(program, machine, state, value).map(|reference| {
        crate::validation::value_custody::places::assignment_value_type(program, reference)
    });
    let mismatch = if let (Some(peer_type), Some(value_type)) = (peer_type, value_type) {
        ...
        crate::validation::value_custody::literals::validate_suffix_landing(
            program,
            anonymous,
            reference,
            diagnostics,
