...
fn source_primitive(
    ...
    match program.expression_table.expression(value) {
        ...
        ExpressionNode::Member(_) | ExpressionNode::Indexed(_) => machine
            .and_then(|machine| {
                crate::validation::value_custody::places::declared_place_type(program, machine, state, value)
            })
            .and_then(|reference| primitive(program, reference)),
        ...
        ExpressionNode::Binary(binary)
            if crate::validation::value_custody::literals::has_anonymous_operator_meaning(program, value) =>
        {
