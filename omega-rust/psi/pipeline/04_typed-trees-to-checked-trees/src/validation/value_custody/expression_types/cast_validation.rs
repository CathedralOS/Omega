...
pub(crate) fn validate_cast_types(
    ...
    crate::validation::value_custody::type_references::validate_indexed_qualification_arguments(
        program,
        machine,
        cast,
        diagnostics,
    ...
    if crate::validation::proof_contracts::contract_entailment::proof_nat_cast(program, cast) {
        ...
        if !crate::validation::proof_contracts::contract_entailment::proof_integer_nonnegative(
            program,
            cast.value,
            &hypotheses,
        ) {
...
fn validate_quotient_mint(
    ...
    state: Option<&symbol_resolved_trees_to_typed_trees::typed_trees::state::State>,
    cast: &TableCastExpression,
    quotient_data: &symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition,
    diagnostics: &mut Vec<Diagnostic>,
) {
...
fn expression_data_symbol(
    ...
    if let Some(type_reference) = crate::validation::value_custody::places::declared_place_type(
        program, machine, state, expression,
    )
    .or_else(|| match program.expression_table.expression(expression) {
