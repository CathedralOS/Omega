...
use symbol_resolved_trees_to_typed_trees::typed_trees::data::{
    DataDefinition, DataMember, DataShapeKind,
};
...
fn is_closed_ranged_integer_scalar(
    ...
    } = program.type_reference_table.type_reference(current)
    {
        for constraint in program.type_reference_table.constraints(*constraints) {
            ...
            if crate::validation::closed_integer_range_bound(program, *minimum).is_none()
...
fn is_qualified_write_only_leaf(program: &TypedTrees, type_reference: TypeReferenceHandle) -> bool {
    is_closed_ranged_integer_scalar(program, type_reference)
        || crate::validation::is_arithmetic_policy_only_integer(program, type_reference)
        ...
        && constraints.iter().all(|constraint| {
            matches!(
                ...
                TypeConstraintNode::Domain(domain)
                    if crate::validation::value_custody::storage_contents::is_plain_value_domain(
                        program, domain,
                    )
            )
        })
...
fn subloan_referee_matches(
    ...
    if crate::validation::value_custody::type_references::type_references_match(
        program, actual, declared,
    ) {
        return true;
    }
    let (
    ...
    match statement {
        ...
        StatementNode::Call(call) => {
            ...
            let declared_parameters =
                crate::validation::machine_calls::calls::machine_state_by_symbol(
                    program,
                    call.target_symbol,
                )
                .map(|(_, callee_state)| {
...
fn validate_write_only_fixed_array_range_assignment<'p>(
    ...
    let end = crate::validation::normalize_immutable_integer_bound_to_usize(
        program,
        bound_lookup,
        range.end,
    )
    ...
    crate::validation::value_custody::struct_literals::validate_array_literal_elements_for_shape(
        program,
        machine,
        state,
        value,
...
fn validate_transition_target(
    ...
    match program.statement_table.transition_target(target) {
        TransitionTargetNode::Named {
            ...
            let declared_parameters =
                crate::validation::declarations::transitions::resolved_transition_target_state(
                    program,
                    path.symbol,
                )
...
fn validate_expression(
    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Call(call) => {
            ...
            let self_is_argument = crate::validation::value_custody::places::declared_place_type(
                program,
                machine,
                Some(state),
                call.receiver,
