...
pub fn local_scalar_record_field(
    ...
    for member in members.iter().rev().take(members.len() - 1) {
        let record = crate::validation::machine_calls::effect_inference::plan_scope::data_definition_by_symbol(
            program,
...
fn expression_at_statement(
    ...
    let mut pending = crate::validation::machine_calls::calls::statement_value_expression_roots(
        program, statement,
    );
...
fn local_plain_record(
    ...
    if !crate::validation::has_plain_owned_contents_with_numeric_constraints(program, carrier) {
        return None;
    }
...
fn has_realized_record_fields(
    program: &TypedTrees,
    record: &symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition,
    ...
    let valid = members.len() == record.members.count() as usize
        && members.iter().enumerate().all(|(ordinal, member)| {
            ...
            && (matches!(
                ...
                TypeReferenceNode::Named { symbol, .. } if crate::validation::machine_calls::effect_inference::plan_scope::data_definition_by_symbol(program, *symbol)
