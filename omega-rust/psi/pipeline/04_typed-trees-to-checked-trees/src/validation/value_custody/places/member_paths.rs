            ...
            let field = program
                ...
                .find_map(|member| match member {
                    symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                        field,
                    ) if field.name.as_str() == field_name => Some(field),
                    _ => None,
                })?;
            field
                .type_reference
...
pub(super) fn data_field_or_payload<'program>(
    ...
    for member in program.data_members(data) {
        if let symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(field) =
            member
        ...
        if let symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(
            variant,
        ) = member
        {
