...
fn absent_symbols_do_not_merge_distinct_fields_of_the_same_owner() {
    ...
    spare.member =
        symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier::generated("spare");
    let spare = program
        .expression_table
...
fn case_projection_is_not_a_direct_attached_field() {
    ...
    member.case_variant = Some(
        symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier::generated("Case"),
    );
    assert!(exact_self_field(&program, &program.machines()[0], own).is_none());
}
