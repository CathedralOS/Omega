...
pub fn plain_owned_value_source(
    ...
    whole_owned_value_source(program, expression, reference)
        .filter(|_| crate::validation::has_plain_owned_contents(program, reference))
}
...
pub(crate) fn whole_owned_value_source(
    ...
    let actual = crate::validation::value_custody::expression_types::named_value_type_reference(
        program, path,
    )?;
