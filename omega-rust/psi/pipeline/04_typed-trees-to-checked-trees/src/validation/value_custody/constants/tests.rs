...
fn destination_rechecks_live_constant_constructor_without_rebuilding_ledger() {
    ...
    let other = program
        ...
        .unwrap()
        .symbol;
    assert!(crate::validation::checked_argument_matches_type_reference(
...
fn constant_type_custody_rejects_stale_handles_and_wrapper_cycles() {
    let program = crate::validation::front_end::typed_program("const VALUE:[u8;0] = [];");
...
fn live_case_and_field_selections_reject_substitution_with_original_ledger() {
    ...
    crate::validation::validate_program(&program).expect("valid case constant");
    for change_case in [false, true] {
        ...
        assert!(crate::validation::validate_program(&changed).is_err());
    }
}
