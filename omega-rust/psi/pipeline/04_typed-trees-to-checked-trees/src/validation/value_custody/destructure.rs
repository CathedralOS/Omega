    ...
    for machine in program.machines() {
        for state in program.machine_states(machine) {
            for statement in program.statement_table.statements(state.statement_nodes) {
                ...
                let Some(data) = crate::validation::value_custody::places::data_definition_for_type(
                    program, resolved,
                ) else {
                    diagnostics.push(Diagnostic::error(format!(
...
fn validate_arm_pattern_marker(
    ...
    let Some(data) =
        crate::validation::value_custody::places::data_definition_for_type(program, declared)
    else {
        return;
    };
    if data.quotient.is_some() {
        ...
        let Some(case) = program
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(
                    case,
                ) if case.name.as_str() == variant => Some(case),
                _ => None,
            })
        else {
