...
pub(super) fn exact_interior_byte_region_offset(
    ...
    let type_reference = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        indexed.index,
...
pub(super) fn interior_byte_region_source(
    ...
    let symbol_resolved_trees_to_typed_trees::typed_trees::types::FixedArrayLength::Literal(length) =
        length
    else {
        return InteriorByteRegion::NotInteriorShape;
    };
    ...
    let offset = match program.expression_table.expression(indexed.index) {
        ...
        _ => {
            ...
            let declared_high =
                crate::validation::proof_contracts::arithmetic_domains::standing_integer_interval(
                    program,
                    machine,
                    state,
                    indexed.index,
...
mod tests {
    ...
    fn fact_free_walk_resolves_same_spelling_by_exact_symbol() {
        ...
        let first_symbol = symbols::SymbolHandle::from_arena_index(21);
        let selected_symbol = symbols::SymbolHandle::from_arena_index(22);
        let bool_type = named_type(&mut program, symbols::SymbolHandle::invalid(), "bool");
        let u8_type = named_type(&mut program, symbols::SymbolHandle::invalid(), "u8");
