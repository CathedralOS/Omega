...
use crate::validation::value_custody::recasts::record_eligibility::direct_record_view_type_is_fact_free;
...
use crate::validation::value_custody::recasts::representation_types::exact_scalar_representation_type;
use crate::validation::value_custody::recasts::scalar_representation::{
...
pub(crate) fn judge_scalar_recast(
    ...
    let let_primitive =
        crate::validation::value_custody::places::unwrapped_type_reference(program, let_referee)
...
fn judge_slice_recast(
    ...
    let Some(source_type) = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        source,
...
pub(crate) fn report_unspelled_reference_pun(
    ...
    let Some(source_primitive) = crate::validation::value_custody::places::declared_place_type(
        program,
        machine,
        Some(state),
        source,
    )
    .and_then(|type_reference| exact_scalar_representation_type(program, type_reference)) else {
        return;
    };
