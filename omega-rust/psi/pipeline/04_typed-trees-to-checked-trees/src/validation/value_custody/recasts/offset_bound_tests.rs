...
fn a_stable_stride_witness_bounds_the_dependent_view() {
    let typed = crate::validation::front_end::typed_program(STRIDE_WALK_STABLE_WITNESS);
...
fn a_drifted_stride_witness_refuses_the_dependent_view() {
    let typed = crate::validation::front_end::typed_program(STRIDE_WALK_DRIFTED_WITNESS);
...
fn a_congruent_runtime_offset_tiles_the_slice_view() {
    let typed = crate::validation::front_end::typed_program(CONGRUENT_STRIDE_OFFSET);
    let result = crate::validation::validate_program(&typed);
    assert!(
        result.is_ok(),
...
fn a_non_congruent_runtime_offset_refuses_the_slice_view() {
    let typed = crate::validation::front_end::typed_program(NON_CONGRUENT_STRIDE_OFFSET);
    let diagnostics = crate::validation::validate_program(&typed)
