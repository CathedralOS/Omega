...
fn same_signature_slice_bounds_and_const_array_extents_remain_valid() {
    ...
    let outcome = crate::validation::validate_program(
        &crate::validation::front_end::typed_program(
...
fn type_bounds_retain_exact_parameter_identity_without_name_repair() {
    ...
    path.symbol = entry;
    let diagnostics =
        crate::validation::validate_program(&program).expect_err("foreign same-name bound");
    assert!(
        diagnostics
            .iter()
            .any(|diagnostic| diagnostic.message.contains("explicit parameter scope")),
