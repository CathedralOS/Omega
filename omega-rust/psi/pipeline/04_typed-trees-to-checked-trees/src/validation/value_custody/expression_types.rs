...
pub fn argument_matches_type_reference_handle(
    ...
    if let ExpressionNode::Call(call) = program.expression_table.expression(argument)
        && let Some(actual) =
            crate::validation::machine_calls::calls::resolved_call_result_type(program, call)
        && matches!(
    ...
    match program.type_reference_table.type_reference(type_reference) {
        ...
        TypeReferenceNode::Named {
            ...
            if let Some(primitive_type) = PrimitiveType::from_name(type_name) {
                if crate::validation::value_custody::literals::land_anonymous_integer_expression(
                    program,
                    argument,
                    primitive_type,
                    |expression| {
                        crate::validation::value_custody::literals::has_anonymous_operator_meaning(
                            program, expression,
                        )
                    },
                )
...
pub(crate) fn named_value_type_reference(
    ...
    if path.symbol.is_valid()
        ...
        && let symbol_resolved_trees_to_typed_trees::typed_trees::data::TypeParameterKind::Const {
            type_reference,
        }
...
pub fn bounded_byte_buffer_capacity(
    ...
    let has_value_domain = program
        ...
        .any(|constraint| match constraint {
            TypeConstraintNode::Domain(name) => {
                !symbol_resolved_trees_to_typed_trees::typed_trees::wire::is_layout_domain_constraint(name)
    ...
    match length {
        symbol_resolved_trees_to_typed_trees::typed_trees::types::FixedArrayLength::Literal(capacity) => Some(*capacity),
...
pub(crate) fn expression_type_name_handle(
    ...
    match program.expression_table.expression(argument) {
        ...
        ExpressionNode::Unary(unary) => match unary.operator {
            symbol_resolved_trees_to_typed_trees::typed_trees::expression::UnaryOperator::BitwiseNot => "integer",
            symbol_resolved_trees_to_typed_trees::typed_trees::expression::UnaryOperator::LogicalNot => "bool",
        },
...
mod tests {
    ...
    use symbol_resolved_trees_to_typed_trees::typed_trees::machine::{Machine, OwnedData};
    use symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier;
    use symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionDefinition;
    ...
    fn typed_source(source: &str) -> TypedTrees {
        ...
        let source_id = sources
            ...
            .source_id;
        crate::validation::front_end::typed_program_from_source_map(sources, &[(source_id, source)])
    }
