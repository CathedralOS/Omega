...
use crate::validation::value_custody::literals::expression_children::children;
...
pub(crate) fn anonymous_integer_landing_warnings(program: &TypedTrees) -> Vec<Diagnostic> {
        ...
        for constraint in program.type_reference_table.constraints(constraints) {
            ...
            } = constraint
            {
                for endpoint in [*minimum, *maximum] {
                    if crate::validation::closed_integer_range_bound(program, endpoint).is_none() {
                        continue;
                    }
    ...
    for (expression, node) in program.expression_table.iter_expressions() {
        ...
        if crate::validation::proof_contracts::contract_entailment::proof_integer_expression(
            program, expression,
        ) {
...
pub(in crate::validation::value_custody::literals) fn append_destination_literals(
    program: &TypedTrees,
                        ...
                        for target in [transition.target, transition.continuation] {
                            ...
                            match program.statement_table.transition_target(target) {
                                TransitionTargetNode::Value(expression)
                                    if transition.exit
                                        == symbol_resolved_trees_to_typed_trees::typed_trees::statement::TransitionExit::Ordinary
                                ...
                                TransitionTargetNode::Named {
                                    ...
                                    let destinations = (transition.exit == symbol_resolved_trees_to_typed_trees::typed_trees::statement::TransitionExit::Ordinary
            ...
            while let Some(expression) = pending.pop() {
                ...
                match node {
                    ExpressionNode::Match(dispatch) => {
                        ...
                        if let Some(destination) = crate::validation::value_custody::expression_types::expression_result_type_reference(program, machine, state, expression) {
                            let mut ordinal = 1;
                            for arm in program.expression_table.match_arms(dispatch.arms) {
                                if matches!(arm.pattern, symbol_resolved_trees_to_typed_trees::typed_trees::expression::MatchPattern::Value(_)) {
                                    ordinal += 1;
                                }
                            ...
                            ) =>
                    {
                        for (ordinal, (operand, peer)) in [(binary.left, binary.right), (binary.right, binary.left)].into_iter().enumerate() {
                            let destination = crate::validation::value_custody::expression_types::expression_result_type_reference(program, machine, state, peer)
                                .and_then(|reference| crate::validation::value_custody::places::unwrapped_type_reference(program, reference));
...
fn call_argument_destinations(
    ...
    crate::validation::machine_calls::effect_inference::plan_scope::memoized_call_argument_destinations(
        program,
        target,
        argument_count,
