...
fn typed(source_text: &str) -> TypedTrees {
    ...
    let source_id = sources
        ...
        .source_id;
    crate::validation::front_end::typed_program_from_source_map(
        sources,
...
fn call_destinations_reject_missing_ambiguous_generic_and_wrong_arity_targets() {
    ...
    let (machine, _) =
        crate::validation::machine_calls::calls::machine_state_by_symbol(&program, target).unwrap();
    ...
    generic
        ...
        .unwrap()
        .lifetime_parameters
        .push(symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier::default());
