...
use symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableMatchExpression;
...
fn width_gate_does_not_assign_a_carrier_to_anonymous_comparison_inputs() {
    for (subject, arms) in [
        ...
        crate::validation::value_custody::literals::validate_literal_widths(
            &program,
            &mut diagnostics,
        );
...
fn comparison_input_custody_does_not_bless_a_shared_runtime_root() {
    ...
    let mut diagnostics = Vec::new();
    crate::validation::value_custody::literals::validate_literal_widths(&program, &mut diagnostics);
