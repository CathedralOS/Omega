...
pub(super) fn admit_assignment_value<'p>(
    ...
    let destination = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        target,
    ...
    .or_else(|| {
        crate::validation::value_custody::places::declared_indexed_projection_type_raw(
            program,
            machine,
            Some(state),
            target,
    ...
    destination.is_some_and(|destination| {
        admitted(
            crate::validation::value_custody::places::assignment_value_type(program, destination),
            value,
        )
    })
}
...
fn static_window_bound<'p>(
    ...
    crate::validation::normalize_immutable_integer_bound_to_usize(program, bound_lookup, expression)
        .map_or(WindowBound::Invalid, WindowBound::Known)
}
...
fn admit_result_values_inner(
    ...
    if let ExpressionNode::Match(dispatch) = program.expression_table.expression(expression) {
        ...
        let destination =
            match crate::validation::value_custody::expression_types::expression_result_type_reference(
                ...
                None if crate::validation::value_custody::literals::has_anonymous_numeric_results(
                    program, expression,
                ) =>
                {
                    destination
