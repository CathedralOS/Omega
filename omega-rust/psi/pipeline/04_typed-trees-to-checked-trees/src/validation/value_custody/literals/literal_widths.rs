...
fn u64_blessed_literals(program: &TypedTrees) -> Vec<ExpressionHandle> {
    ...
    for (_, node) in program.expression_table.expression_entries() {
        ...
        for field in program.expression_table.struct_fields(literal.fields) {
            ...
            let Some(field_type) =
                crate::validation::value_custody::struct_literals::selected_construction_field_type(
                    program,
                    literal,
                    field.field_symbol,
                )
...
fn primitive_is_u64_classed(
    program: &TypedTrees,
    unwrapped: symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceHandle,
) -> bool {
    matches!(
...
fn place_is_u64_classed(
    ...
    let Some(declared) = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        place,
    ...
    let Some(unwrapped) =
        crate::validation::value_custody::places::unwrapped_type_reference(program, declared)
    else {
        return false;
    };
