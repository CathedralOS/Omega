...
use crate::validation::value_custody::places::declared_place_type;
...
pub(super) fn validate_wire_decode_call(
    ...
    current_machine: &symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine,
    current_state: Option<&symbol_resolved_trees_to_typed_trees::typed_trees::state::State>,
    ...
    let mut repeated_fields: Vec<(
        &WireField,
        symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireRepeatedEncoding,
    )> = Vec::new();
    ...
    for member in program.wire_members(schema.members) {
        ...
        let scalar = primitive.and_then(symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireScalarEncoding::for_primitive);
        ...
        && let Some(value_data) = named_data_definition(program, value_type)
    {
        ...
        if program.data_members(value_data).iter().any(|member| {
            matches!(
                member,
        ...
        for (field, schema_primitive) in &current_fields {
            ...
            if symbol_resolved_trees_to_typed_trees::typed_trees::wire::type_reference_carries_range(
                program,
    ...
    let case_names: Vec<&str> = members
        ...
        .filter_map(|member| match member {
            symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(
                variant,
            ) if variant.payload.is_empty() => Some(variant.name.as_str()),
            _ => None,
        })
        .collect();
