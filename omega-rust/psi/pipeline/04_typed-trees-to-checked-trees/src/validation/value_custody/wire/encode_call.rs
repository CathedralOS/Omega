...
pub(super) fn validate_wire_encode_call(
    ...
    let mut worst_case_bytes =
        symbol_resolved_trees_to_typed_trees::typed_trees::wire::wire_varint_bytes(era).len();
    ...
    for member in program.wire_members(schema.members) {
        ...
        if let Some((_, _, max_count)) = program.wire_field_repeated_carrier(field) {
            ...
            worst_case_bytes +=
                symbol_resolved_trees_to_typed_trees::typed_trees::wire::wire_varint_bytes(
                    field.number,
                )
                .len()
        ...
        if program.is_borrowed_byte_slice(field.type_reference) {
            max_field_number = max_field_number.max(field.number);
            worst_case_bytes += symbol_resolved_trees_to_typed_trees::typed_trees::wire::wire_varint_bytes(field.number).len()
                + symbol_resolved_trees_to_typed_trees::typed_trees::wire::WIRE_TEXT_LENGTH_MAX_VARINT_LENGTH;
        ...
        let encoding = primitive.and_then(symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireFieldEncoding::for_primitive);
        ...
        max_field_number = max_field_number.max(field.number);
        let tag_bytes = symbol_resolved_trees_to_typed_trees::typed_trees::wire::wire_varint_bytes(
            field.number,
        )
        .len();
        ...
        worst_case_bytes += tag_bytes + match encoding {
            symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireFieldEncoding::Scalar(
                scalar,
            ) => scalar.max_varint_length(),
            symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireFieldEncoding::Text => {
                text_fields.push(field);
                symbol_resolved_trees_to_typed_trees::typed_trees::wire::WIRE_TEXT_LENGTH_MAX_VARINT_LENGTH
            }
        };
