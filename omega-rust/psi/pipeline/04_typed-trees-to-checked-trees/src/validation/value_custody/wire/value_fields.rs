use crate::validation::value_custody::places::unwrapped_type_reference;
use diagnostics::Diagnostic;
use symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees;
use symbol_resolved_trees_to_typed_trees::typed_trees::types::{
    TypeReferenceHandle, TypeReferenceNode,
};
use symbol_resolved_trees_to_typed_trees::typed_trees::wire::{WireField, WireMember, WireSchema};
...
pub(super) fn validate_nested_value_field(
    ...
    schema: &WireSchema,
    machine_name: &str,
    value_data: &symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition,
    field: &WireField,
    child: &WireSchema,
        ...
        program
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    data_field,
                ) if data_field.name == field.name => Some(data_field),
                _ => None,
            })
    else {
    ...
    if program.data_members(child_value_data).iter().any(|member| {
        matches!(
            member,
    ...
    for member in program.wire_members(child.members) {
        ...
        let Some(child_value_field) =
            program
                ...
                .find_map(|member| match member {
                    symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                        data_field,
                    ) if data_field.name == child_field.name => Some(data_field),
                    _ => None,
                })
        else {
            diagnostics.push(Diagnostic::error(format!(
        ...
        if machine_name == "decode"
            && symbol_resolved_trees_to_typed_trees::typed_trees::wire::type_reference_carries_range(
                program,
                child_value_field.type_reference,
            )
            && crate::validation::scalar_representation_range(
                program,
                child_value_field.type_reference,
            )
            .is_none()
...
pub(super) fn validate_repeated_value_field(
    ...
    value_data: &symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition,
    field: &WireField,
    repeated: symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireRepeatedEncoding,
    diagnostics: &mut Vec<Diagnostic>,
) {
    let find_field = |name: &str| {
        program
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    data_field,
                ) if data_field.name.as_str() == name => Some(data_field),
                _ => None,
            })
    };
    ...
    match find_field(field.name.as_str()) {
        ...
        Some(value_field) => {
            ...
            if machine_name == "decode"
                && let Some(element_type) = symbol_resolved_trees_to_typed_trees::typed_trees::wire::repeated_element_type(
                    program,
                    value_field.type_reference,
                    repeated.carrier,
                )
                && symbol_resolved_trees_to_typed_trees::typed_trees::wire::type_reference_carries_range(program, element_type)
                && crate::validation::scalar_representation_range(program, element_type).is_none()
            {
                diagnostics.push(Diagnostic::error(format!(
