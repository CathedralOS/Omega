...
fn target_neutral_report_matches(
    program: &TypedTrees,
    plan: &symbol_resolved_trees_to_typed_trees::typed_trees::PlanLaidLayout,
    schema: &symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition,
    schema_fields: &[&symbol_resolved_trees_to_typed_trees::typed_trees::data::DataField],
) -> bool {
    ...
    for field in schema_fields {
        ...
        match entries.as_slice() {
            [entry] => match entry.placement {
                terminal_psi::layout_plans::LayoutPlacementReport::At { offset } => {
                    offsets.push(offset)
                }
                terminal_psi::layout_plans::LayoutPlacementReport::IntegerAt { .. }
                | terminal_psi::layout_plans::LayoutPlacementReport::Bits { .. } => {
...
fn stored_integer_write_is_total(
    ...
    let (admitted_minimum, admitted_maximum) =
        if let Some(range) = crate::validation::scalar_representation_range(program, field_type) {
    ...
    for (field_index, field) in schema_fields.iter().enumerate() {
        ...
        match entries.as_slice() {
            [entry]
                if matches!(
                    entry.placement,
                    terminal_psi::layout_plans::LayoutPlacementReport::At { .. }
                ) =>
            {
                let terminal_psi::layout_plans::LayoutPlacementReport::At { offset } =
                    entry.placement
                else {
                    unreachable!()
                };
            ...
            {
                let terminal_psi::layout_plans::LayoutPlacementReport::IntegerAt {
                    offset,
                    stored_width,
                    interpretation,
                } = entry.placement
                ...
                integer_fields.push(
                    symbol_resolved_trees_to_typed_trees::typed_trees::PlanLaidIntegerField {
                        field_index,
                        stored_width_bits,
                        interpretation,
                    ...
                    }) =>
            {
                let mut element_offsets = entries
                    ...
                    .map(|entry| match entry.placement {
                        terminal_psi::layout_plans::LayoutPlacementReport::At { offset } => offset,
                        _ => unreachable!(),
                    })
                    .collect::<Vec<_>>();
                ...
                offsets.push(offset);
                repeated_fields.push(
                    symbol_resolved_trees_to_typed_trees::typed_trees::PlanLaidRepeatedField {
                        field_index,
                        element_stride,
                    },
                );
                    ...
                    }) =>
            {
                ...
                for entry in entries {
                    let terminal_psi::layout_plans::LayoutPlacementReport::Bits {
                        container,
                        container_width,
                        destination_lsb,
                        source_lsb,
                    ...
                    fragments.push(
                        symbol_resolved_trees_to_typed_trees::typed_trees::PlanLaidBitFragment {
                ...
                bit_fields.push(
                    symbol_resolved_trees_to_typed_trees::typed_trees::PlanLaidBitField {
                        field_index,
                        fragments,
                    },
                );
...
fn runtime_fields<'program>(
    ...
    data: &symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition,
) -> Vec<&'program symbol_resolved_trees_to_typed_trees::typed_trees::data::DataField> {
    program
        .data_members(data)
        .iter()
        .filter_map(|member| match member {
