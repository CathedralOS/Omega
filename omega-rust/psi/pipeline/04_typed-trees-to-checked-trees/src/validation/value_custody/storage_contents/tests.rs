...
fn empty_fixed_arrays_preserve_their_element_contents_classification() {
    for carrier in ["[u8; 0]", "Empty", "Holder<[u8; 0]>"] {
        let program = crate::validation::front_end::typed_program(&format!(
...
fn primitive_ranges_inside_owned_records_do_not_introduce_cleanup() {
    ...
    let nested = crate::validation::front_end::typed_program(
...
fn primitive_range_admission_does_not_erase_other_constraints() {
    let mut program =
        crate::validation::front_end::typed_program("machine inspect(value: u64 [3..=5]) {}");
    ...
    let constraints = program.type_reference_table.insert_constraints([
        symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeConstraintNode::Named(
            "atomic".into(),
        ),
    ]);
...
fn stable_observation_rejects_nested_mutation_authority_and_unknown_qualifiers() {
    ...
    for carrier in ["u64", "Holder<Scalar>", "&[u8]", "&Holder<Bytes>"] {
        for qualifier in ["atomic", "unknown"] {
            let mut program = crate::validation::front_end::typed_program(&format!(
                "data Holder<T> {{ value: T; }}
            ...
            for reference in primitives {
                ...
                let constraints = program.type_reference_table.insert_constraints([
                    symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeConstraintNode::Named(qualifier.into()),
                ]);
...
fn service_seam_contents_keep_the_runtime_exclusions() {
    ...
    for carrier in [
        ...
        let program = crate::validation::front_end::typed_program(carrier);
        assert!(
...
fn stable_observation_does_not_erase_domain_authority_metadata() {
    let original = crate::validation::front_end::typed_program(
        ...
        for change in ["route", "role", "classification", "alias_or_unresolved"] {
            ...
            if mutate_declaration {
                ...
                match change {
                    ...
                    _ => {
                        declaration.alias =
                            Some(symbol_resolved_trees_to_typed_trees::typed_trees::domain::DomainAliasDefinition::default())
                    }
                }
            } else {
                ...
                program.type_reference_table.set_constraint_at_offset(
                    constraints,
                    0,
                    symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeConstraintNode::Domain(changed),
                );
            }
            assert!(
                !has_stable_observable_contents(&program, reference),
