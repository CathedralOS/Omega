...
pub(super) fn check_component(
    ...
    let frames = crate::validation::machine_calls::calls::CallFrameResolver::new(program);
        ...
        .expression(projection::unwrapped(program, argument))
    {
        ...
        ExpressionNode::Binary(binary) => {
            ...
            match (binary.operator, positive) {
                (symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::Subtract, true) => {
                    bound.meet(CarrierBound::AtMost)
                }
                (symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::Add, true) => {
        ...
        ExpressionNode::Indexed(indexed)
            if crate::validation::value_custody::places::has_builtin_subslice_meaning(
                program,
                machine,
                Some(source),
    ...
    match program.expression_table.expression(expression) {
        ...
        ExpressionNode::Indexed(indexed)
            if crate::validation::value_custody::places::has_builtin_subslice_meaning(
                ...
                Some(state),
                expression,
            ) || crate::validation::value_custody::places::place_has_builtin_coordinates(
                program,
                machine,
                Some(state),
                expression,
