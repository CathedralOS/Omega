...
fn eligible(program: &TypedTrees, call: &TableCallExpression) -> bool {
    ...
    let reaches = crate::validation::infer_service_reaches(program, &operational);
...
fn normal_return_equality_does_not_require_premature_checked_termination() {
    ...
    assert!(
        !crate::validation::machine_calls::denotational_calls::unconditionally_terminates(
            &program,
            machine.symbol
        )
    );
...
fn requires_remain_caller_obligations_not_an_eligibility_theorem() {
    ...
    assert!(
        crate::validation::machine_calls::fact_call_projections::validate_checked_call_candidate(
            &program,
            &call,
            &operational,
            &reaches,
...
fn absent_effect_summaries_are_not_evidence_of_purity() {
    ...
    assert!(
        normal_return_call_candidate(
            &program,
            &call,
            &crate::flow_effects::OperationalPlan::default(),
            &reaches
        )
        .is_err()
    );
