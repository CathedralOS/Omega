...
use symbol_resolved_trees_to_typed_trees::typed_trees::type_identity::NormalizedNamedCallableIdentity;
...
fn selected_named_statement_callable_symbol(
    ...
    let candidates = symbol_resolved_trees_to_typed_trees::typed_trees::operator::named_statement_call_candidates(program, call);
...
fn select_overload_symbol(
    program: &TypedTrees,
    provisional_identity: &symbol_resolved_trees_to_typed_trees::typed_trees::type_identity::NormalizedNamedCallableIdentity,
    overloads: &[(
        SymbolHandle,
        symbol_resolved_trees_to_typed_trees::typed_trees::type_identity::NormalizedNamedCallableIdentity,
    )],
    expected_result: Option<TypeReferenceHandle>,
    diagnostics: &mut Vec<Diagnostic>,
    kind: &str,
    ...
    for (_, expression) in program.expression_table.expression_entries() {
        match expression {
            ExpressionNode::StructLiteral(literal) => {
                ...
                for field in program.expression_table.struct_fields(literal.fields) {
                    let Some(field_type) =
                        crate::validation::value_custody::struct_literals::construction_field_type(
                            program,
                            data_definition,
