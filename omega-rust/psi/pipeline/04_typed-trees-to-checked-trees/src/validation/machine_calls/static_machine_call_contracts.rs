...
pub fn static_machine_parameter_contract_bytes(
    ...
    for (ordinal, parameter) in parameters.iter().enumerate() {
        ...
        match contract {
            symbol_resolved_trees_to_typed_trees::typed_trees::data::MachineParameterContractView::Structural(_) => bytes.push(1),
        ...
        for invocation in
            crate::validation::machine_calls::effect_inference::declared_signature_invocations(
                program, signature,
            )
        {
            let mut encoded = Vec::new();
            match invocation {
                crate::flow_effects::InvocationTarget::Parameter(ordinal) => {
                    encoded.push(1);
                    encoded.extend(ordinal.to_le_bytes());
                }
                crate::flow_effects::InvocationTarget::Service(service) => {
                    encoded.push(2);
...
pub fn validate_static_machine_call_contracts(
    ...
    if !program.machine_specializations.is_empty() {
        let inferred = crate::validation::infer_service_reaches(program, operational);
        for specialization in &program.machine_specializations {
