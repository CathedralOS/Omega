...
fn validate_direct_projection(
    ...
    if data.supply_mode != language_semantics::DataSupplyMode::CheckedShape
        || DataShapeKind::Record
            != symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition::shape_kind_from_members(
...
pub(crate) fn validate_checked_call_candidate<'program>(
    ...
    if program.state_parameters(state).iter().any(|parameter| {
        crate::validation::proof_contracts::quotients::type_has_forbidden_denotational_content(
            program,
            &proof_only,
    ...
    if !crate::validation::machine_calls::denotational_calls::has_pure_effect_closure(
        machine.symbol,
        state.symbol,
        program
    ...
    if !crate::validation::machine_calls::denotational_calls::has_observation_free_checked_closure(
        program,
        machine.symbol,
        operational,
    ) {
    ...
    if !crate::validation::machine_calls::denotational_calls::has_no_crash_routes(
        program,
        machine.symbol,
        operational,
    ) {
