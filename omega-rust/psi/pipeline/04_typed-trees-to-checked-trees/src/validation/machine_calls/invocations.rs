...
pub(crate) fn validate_invocation_contracts(
    ...
    let plan = crate::validation::infer_synchronous_invocations(program);
...
fn validate_target_names(
    program: &TypedTrees,
    owner: &str,
    targets: &[symbol_resolved_trees_to_typed_trees::typed_trees::signature::AuthoredInvocation],
    parameters: &[StateParameter],
    diagnostics: &mut Vec<Diagnostic>,
) {
    let mut seen = Vec::new();
    for target in targets {
        if target.target != symbol_resolved_trees_to_typed_trees::typed_trees::signature::AuthoredInvocationTarget::Unresolved
...
fn validate_conformance_refinement(
    program: &TypedTrees,
    machine: &Machine,
    summary: &crate::flow_effects::MachineInvocationInference,
    diagnostics: &mut Vec<Diagnostic>,
) {
    for conformance in program.machine_trait_conformances(machine) {
        ...
        for requirement in matching {
            let allowed = crate::validation::declared_signature_invocations(program, requirement);
            let parameters = program.state_signature_parameters(requirement);
            let self_forwarded = trait_definition.is_boundary
                && crate::validation::has_self_forwarded_boundary_parameter(
                    program,
                    machine,
            ...
            let actual = summary
                ...
                .filter_map(|target| match *target {
                    crate::flow_effects::InvocationTarget::Parameter(0) if self_forwarded => None,
                    crate::flow_effects::InvocationTarget::Parameter(index) if self_forwarded => {
                        Some(crate::flow_effects::InvocationTarget::Parameter(index - 1))
                    }
                    target => Some(target),
                })
                .collect::<Vec<_>>();
...
fn report_missing_targets(
    program: &TypedTrees,
    parameters: &[StateParameter],
    actual: &[crate::flow_effects::InvocationTarget],
    allowed: &[crate::flow_effects::InvocationTarget],
...
fn invocation_target_label(
    program: &TypedTrees,
    parameters: &[StateParameter],
    target: crate::flow_effects::InvocationTarget,
) -> String {
    match target {
        crate::flow_effects::InvocationTarget::Parameter(index) => parameters
            .iter()
            .filter(|parameter| !parameter.is_self)
            .nth(index as usize)
