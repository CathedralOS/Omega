...
fn attached_call_requires_its_selected_state_and_receiver_owner() {
    ...
    crate::validation::validate_program(&program).expect("exact selected call");
    let call = program
        .expression_table
        .iter_expressions()
    ...
    for target in [other_state, symbols::SymbolHandle::invalid()] {
        ...
        assert!(
            crate::validation::validate_program(&forged).is_err(),
...
fn an_attached_call_cannot_use_a_collection_element_as_its_receiver_owner() {
    ...
    let source = "data First { value: u64; }
        ...
        machine read(value: &First) -> u64 { value.read() }";
    let program = crate::validation::front_end::typed_program(source);
    crate::validation::validate_program(&program).expect("ordinary nominal receiver");
    let machine = program
        .machines()
        .iter()
