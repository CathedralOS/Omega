...
pub(super) fn receiver_is_declared(
    scope: &CallScope<'_>,
    receiver_members: &[symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier],
    arguments: &[ExpressionHandle],
    diagnostics: &mut Vec<Diagnostic>,
) -> bool {
    let CallScope {
    ...
    if let Some(first) = receiver_members.first()
        && !crate::validation::value_custody::locals::is_named_operator_namespace(
            program,
            &[call.receiver_symbol],
            &namespace,
            call.target_symbol,
        ...
        )
    {
        ...
        if !root.is_valid()
            || !current_state.is_some_and(|state| {
                crate::validation::value_custody::locals::state_value_root_is_known(
                    program,
                    current_machine,
                    state,
