...
use symbol_resolved_trees_to_typed_trees::typed_trees::statement::{
    StatementNode, TableAssignment, TransitionTargetNode,
};
...
fn reference_result_nested_operand_call_is_supported(
    ...
    if bindings.next().is_some()
        || local.is_mutable
        || crate::validation::machine_calls::reference_result_custody::parts(
            program,
    ...
    let Some((position, leaf_source)) =
        crate::validation::machine_calls::reference_result_custody::source_leaf(
            program,
            destination,
        )
    else {
    ...
    let Some((edge, projected)) =
        crate::validation::machine_calls::reference_result_custody::declared_field_path(
            program,
            nested_destination.return_type,
    ...
    let Some(sources) =
        crate::validation::machine_calls::reference_result_custody::returned_record_sources(
            program,
            nested_destination,
        )
    else {
                    ...
                    prior.iter().any(|statement| {
                        matches!(statement, StatementNode::LocalData(local)
                            if local.symbol == name.symbol
                                && !local.is_mutable
                                && crate::validation::machine_calls::reference_result_custody::is_reference_record(
                                    program,
                                    local.type_reference,
                                )
...
fn ordinary_structural_result_type(
    ...
    match program.type_multiplicity(reference) {
        language_semantics::Multiplicity::Linear => {
            crate::validation::structural_result_qualifications(program, reference).is_ok()
        }
        ...
        language_semantics::Multiplicity::Unrestricted => {
            crate::validation::is_closed_primitive_array_type(program, reference)
        }
    }
}
...
fn unit_type(
    ...
    loop {
        match program.type_reference_table.type_reference(type_reference) {
            ...
            symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Unit => return true,
            _ => return false,
        }
    }
}
...
fn member_store_signature_is_supported(
    ...
    parameters.iter().all(|parameter| {
        ...
        if partial_affine_signature
            && let symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Reference { referee, .. } = program
                .type_reference_table
                .type_reference(parameter.type_reference)
        {
...
fn member_store_path_is_supported(
    ...
    hops: &[MemberStoreHop],
    result_primitive: symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType,
) -> bool {
...
fn mutable_store_referee(
    ...
    loop {
        match program.type_reference_table.type_reference(type_reference) {
            symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Constrained { .. } => return None,
            symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Reference {
                access,
                referee: next,
                ..
...
fn type_is_reference(
    ...
    loop {
        match program.type_reference_table.type_reference(type_reference) {
            ...
            symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Reference { .. } => return true,
            _ => return false,
        }
    }
}
...
fn field_store_owner(
    ...
    loop {
        match program.type_reference_table.type_reference(type_reference) {
            symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Named { name, .. } => {
                return program
                    .data_definitions()
                    .iter()
        ...
        program
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    field,
                ) if field.name.as_str() == hop.name.as_str() => Some(field.symbol),
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(
                    variant,
                ) => (variant.name.as_str() == hop.name.as_str())
    ...
    let mut fields =
        program
            ...
            .filter_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    field,
                ) if !field.relevance.is_erased() && field.symbol == resolved => Some(field),
                _ => None,
            });
