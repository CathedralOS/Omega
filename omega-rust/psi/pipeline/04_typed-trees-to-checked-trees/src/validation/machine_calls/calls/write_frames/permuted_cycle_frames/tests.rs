...
use crate::validation::machine_calls::calls::write_frames::CYCLE_EQUATIONS;
use crate::validation::machine_calls::calls::write_frames::PREFIX_WALKS;
use crate::validation::machine_calls::calls::write_frames::transition_topology::reachable_cycle_edges_can_permute_write_parameters;
...
fn count_mismatched_cycle_declines_before_building_equations() {
    let program = crate::validation::front_end::typed_program(REFERENCE_CYCLE);
...
fn copy_value_cycle_edges_permute_and_solve_to_complete_frames() {
    let program = crate::validation::front_end::typed_program(KIND_CYCLE);
    let machine = &program.machines()[0];
