...
fn static_paths_cannot_continue_through_runtime_bindings() {
    ...
    ].into_iter().filter(|source| crate::validation::validate_program(&crate::validation::front_end::typed_program(source)).is_ok()).collect::<Vec<_>>();
    assert!(
        accepted.is_empty(),
...
fn selected_lexical_fields_and_static_case_values_remain_legal() {
    for source in [
        ...
        crate::validation::validate_program(&crate::validation::front_end::typed_program(source))
...
fn missing_head_cannot_hide_a_retained_runtime_root_in_the_leaf() {
    ...
    assert!(crate::validation::validate_program(&program).is_err());
    ...
    assert!(
        crate::validation::validate_program(&program).is_err(),
