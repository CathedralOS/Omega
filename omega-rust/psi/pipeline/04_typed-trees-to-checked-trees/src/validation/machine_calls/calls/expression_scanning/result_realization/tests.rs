...
fn owned_scalar_nested_borrows_admit_unchanged_ordered_fixture() {
    let program = crate::validation::front_end::typed_program(ORDERED);
    crate::validation::validate_program(&program)
...
fn owned_scalar_nested_call_admission_keeps_parameter_restrictions() {
    let original = crate::validation::front_end::typed_program(ORDERED);
    let caller = original
        .machines()
        .iter()
        .find(|machine| machine.name.as_str() == "inspect")
