...
pub(in crate::validation::machine_calls::calls::write_frames) fn assigned_stored_origins(
    program: &TypedTrees,
    machine: &Machine,
    state: &State,
    ...
    let target_type = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        assignment.target,
