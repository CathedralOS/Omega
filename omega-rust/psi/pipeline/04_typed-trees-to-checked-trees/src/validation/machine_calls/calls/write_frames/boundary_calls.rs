...
pub(super) fn receiver_requires_boundary_frame(
    ...
    let has_boundary_receiver = match receiver {
        [root, member] if is_self_receiver(root) => {
            crate::validation::value_custody::places::machine_attached_data(
                program,
                current_machine,
            )
            .is_some_and(|data| {
...
fn boundary_receiver_type_symbol(
    ...
    for _ in 0..program.type_reference_table.type_reference_count() {
        ...
        if let Some(carrier) =
            symbol_resolved_trees_to_typed_trees::typed_trees::service::classify_exact_bound_service_carrier(program, reference).ok()?
        {
...
fn boundary_trait_signature_and_receiver_inner<'program>(
    ...
    let (trait_definition, has_runtime_receiver, receiver_reference) = match receiver_members {
        ...
        [root, receiver] if is_self_receiver(root) => {
            let data = crate::validation::value_custody::places::machine_attached_data(
                program,
                current_machine,
            )?;
...
fn requirement_signature_in_trait<'program>(
    program: &'program TypedTrees,
    definition: &'program symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::TraitDefinition,
...
) -> Option<&'program symbol_resolved_trees_to_typed_trees::typed_trees::signature::StateSignature>
{
    ...
    for (depth, member) in members.iter().enumerate() {
        ...
        data = crate::validation::machine_calls::effect_inference::plan_scope::data_definition_by_symbol(
            program,
