...
fn cited_strict_decrease(
    ...
    measure_name: Option<&symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier>,
) -> bool {
    ...
    for contract in program.machine_contracts(machine) {
        ...
        for fact in program.proof_facts.span_or_empty(contract.facts) {
            if let symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact::Expression(expression) = fact
                ...
                && binary.operator == symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::Equal
            {
    ...
    for other in program.machine_states(machine) {
        for statement in program.statement_table.statements(other.statement_nodes) {
            ...
            let symbol_resolved_trees_to_typed_trees::typed_trees::statement::TransitionGuardNode::When(guard) = transition.guard else {
                continue;
            };
...
fn ensures_is_strict_decrease(
    ...
    measure_name: &symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier,
) -> bool {
        ...
        .any(|statement| {
            ...
            let guard = match program.expression_table.expression(guard) {
                ExpressionNode::Binary(wrapper)
                    if wrapper.operator == symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::Equal
                        && matches!(
            ...
            if positive.operator != symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::Greater
...
pub(crate) fn proof_call_has_structural_descent(
    ...
    let Some(subjects) = symbol_resolved_trees_to_typed_trees::typed_trees::ranking::resolve_machine_witness_subjects(program, machine)
    else {
        return false;
    };
        ...
        let descends = argument.is_some_and(|argument| {
            strict_subterm_of_measure(program, argument, measure_symbol, measure_name.as_ref())
                ...
                || measure_name.as_ref().is_some_and(|name| {
                    crate::validation::proof_contracts::contract_entailment::proof_edge_strict_decrease_judged(
                        program,
                        machine,
                        state,
                        argument,
...
fn substate_parameter_descends(
    ...
    let parameters: Vec<(
        ...
        .collect();
    let mut descending: Vec<symbols::SymbolHandle> = Vec::new();
    loop {
