...
pub fn unit_statement_call_is_supported(
    ...
    let mut occurrences = statements.iter().enumerate().filter_map(|(position, statement)| {
        matches!(statement, symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Expression(root) if *root == expression)
            .then_some(position)
    });
...
pub fn unit_return_call_is_supported(
    ...
    if !unit_type(program, state.return_type)
        ...
        || !matches!(
            ...
            Some(symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Expression(tail)) if *tail == expression
        )
    {
        return false;
    }
...
pub(in crate::validation::machine_calls::calls) fn call_returns_unit(
    program: &TypedTrees,
    machine: &Machine,
    expression: ExpressionHandle,
) -> bool {
