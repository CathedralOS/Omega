...
use crate::validation::machine_calls::calls::write_frames::transparent_results::transparent_call_result_origins;
...
fn receiver_referee_name(
    ...
    let referent_name = |mut expression: ExpressionHandle| {
        let type_reference = loop {
            match crate::validation::value_custody::places::declared_place_type_raw(
                program,
                current_machine,
                Some(state),
                expression,
...
pub(super) fn projected_carrier_reference_origins(
    ...
    for member_name in member_names {
        let data = crate::validation::machine_calls::effect_inference::plan_scope::data_definition_by_symbol(
            program,
            carrier_symbol,
        )?;
        let field = program
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    field,
                ) if field.name.as_str() == member_name => Some(field),
                _ => None,
            })?;
...
fn carried_reference_origin(
    ...
    let reference = crate::validation::value_custody::places::declared_place_type_raw(
        program,
        machine,
        Some(state),
        expression,
