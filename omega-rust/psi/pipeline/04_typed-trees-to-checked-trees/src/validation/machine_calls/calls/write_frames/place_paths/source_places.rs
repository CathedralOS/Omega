...
use crate::fact_plan::{FactPlan, PlaceRoot, PlaceSegment};
...
use crate::validation::machine_calls::calls::write_frames::caller_aliases::{
    CallerWriteSite, caller_statement_owner,
};

#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub(in crate::validation::machine_calls::calls::write_frames) struct FrameSourcePlace {
    pub(in crate::validation::machine_calls::calls::write_frames) root: SymbolHandle,
    pub(in crate::validation::machine_calls::calls::write_frames) segments: Vec<PlaceSegment>,
    ...
    pub(in crate::validation::machine_calls::calls::write_frames) builtin_coordinates: bool,
}

impl FrameSourcePlace {
    pub(in crate::validation::machine_calls::calls::write_frames) fn from_expression(
        program: &TypedTrees,
    ...
    pub(in crate::validation::machine_calls::calls::write_frames) fn append_segments(
        &self,
    ...
    pub(in crate::validation::machine_calls::calls::write_frames) fn append_source(
        &self,
        source: &Self,
    ) -> Self {
    ...
    pub(in crate::validation::machine_calls::calls::write_frames) fn projected(
        &self,
        program: &TypedTrees,
    ...
    pub(in crate::validation::machine_calls::calls::write_frames) fn append_relative(
        &self,
        relative: &Self,
    ) -> Self {
...
fn source_has_builtin_coordinates(
    ...
    source_owner(program, root, expression).is_some_and(|(machine, state)| {
        crate::validation::place_has_builtin_coordinates(program, machine, Some(state), expression)
    })
}
    ...
    match declaration.kind {
        ...
        SymbolKind::Machine | SymbolKind::Field => {
            ...
            let owns_root = match declaration.kind {
                ...
                _ => crate::validation::exact_attached_field(
                    program,
                    machine,
                    root,
