...
pub fn structural_state_contract_scalar_predicates(
    ...
    for parameter in program.state_parameters(state) {
        ...
        if program.primitive_type_reference(reference).is_some() {
            if !parameter.is_self && !parameter.is_mutable {
                expected.extend(
                    crate::validation::scalar_interval_domains(program, reference)
                        .into_iter()
    ...
    for contract in program.state_contracts(state) {
        ...
        if contract.kind != symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContractKind::Requires {
            return None;
        }
...
> {
    ...
    for (_, event) in facts.flow.ownership.permissions.iter() {
        if event.machine_symbol != machine
            ...
            || event.root != crate::fact_plan::PlaceRoot::Symbol(parameter)
...
fn retained_claim_segments(
    facts: &CheckFacts,
    span: arena::HandleSpan<crate::fact_plan::PlaceSegment>,
) -> Result<&[crate::fact_plan::PlaceSegment], &'static str> {
...
pub fn reconstruct_structural_call_custody(
    ...
    if target.supply_mode != language_semantics::MachineSupplyMode::CheckedBody
        || crate::validation::machine_calls::reference_result_custody::result_multiplicity(
            program,
    ...
    let result_qualifications =
        if crate::validation::machine_calls::reference_result_custody::parts(
            program,
            destination.return_type,
        )
        .is_some()
    ...
    if result.multiplicity != Multiplicity::Linear {
        ...
        if crate::validation::machine_calls::reference_result_custody::is_reference_record(
            program,
            destination.return_type,
        ) {
            ...
            if let Some(local) = local_destination {
                if program.normalized_type_identity(local.type_reference)
                    != program.normalized_type_identity(destination.return_type)
                    || crate::validation::machine_calls::reference_result_custody::local_record_loans(
                        program,
                        facts,
                        caller_machine,
                        source_state,
            ...
            } else if !crate::validation::machine_calls::reference_result_custody::nested_call_result_loans(
                program,
                facts,
                caller_machine,
                source_state,
        ...
        let reference_loan = if crate::validation::machine_calls::reference_result_custody::parts(
            program,
            destination.return_type,
        )
        .is_some()
        {
            crate::validation::machine_calls::reference_result_custody::result_loan(
                program,
                facts,
                caller_machine,
                source_state,
    ...
    for transfer in transfers {
        if transfer.root != crate::fact_plan::PlaceRoot::Symbol(path.symbol)
