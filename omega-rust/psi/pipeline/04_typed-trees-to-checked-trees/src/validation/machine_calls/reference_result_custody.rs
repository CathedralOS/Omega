...
use crate::checked_trees::{
    CheckFacts, CheckedStructuralAccess, CheckedUnitStructuralResultBindingPlan,
};
...
pub fn construction_sources(
    ...
    expression: symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle,
    reference: TypeReferenceHandle,
) -> Option<Vec<crate::checked_trees::CheckedReferenceResultSourcePlan>> {
...
fn construction_sources_with_calls(
    ...
    if parts(program, reference).is_some() || shared_borrowed_parts(program, reference).is_some() {
        return Some(vec![
            crate::checked_trees::CheckedReferenceResultSourcePlan {
                path: Vec::new(),
    ...
    if let ExpressionNode::Call(call) = program.expression_table.expression(expression) {
        ...
        sources
            .into_iter()
            .map(|mut source| {
                let crate::checked_trees::CheckedUnitStructuralArgumentSourcePlan::Parameter {
                    parameter_index,
                ...
                source.source = if source.source.path.is_empty() {
                    ...
                    if *last != crate::checked_trees::CheckedUnitStructuralPathSegment::Referent
...
fn formal_record_sources(
    ...
    fn visit(
        ...
        reference: TypeReferenceHandle,
        parameter_index: u32,
        path: &mut Vec<crate::checked_trees::CheckedUnitStructuralPathSegment>,
        output: &mut Vec<crate::checked_trees::CheckedReferenceResultSourcePlan>,
    ) -> Option<()> {
        if let Some((referent, access)) =
            parts(program, reference).or_else(|| shared_borrowed_parts(program, reference))
        {
            let mut source_path = path.clone();
            source_path.push(crate::checked_trees::CheckedUnitStructuralPathSegment::Referent);
            output.push(crate::checked_trees::CheckedReferenceResultSourcePlan {
                path: path.clone(),
...
pub fn local_record_loans(
    ...
    Vec<(
        crate::checked_trees::CheckedReferenceResultSourcePlan,
        arena::Handle<crate::checked_trees::BorrowLoanFact>,
    )>,
> {
    ...
    for source in sources {
        ...
        for segment in &source.path {
            let crate::checked_trees::CheckedUnitStructuralPathSegment::Field(identity) = segment
            else {
                return None;
            };
            ...
            let field = program
                ...
                .find_map(|member| match member {
                    symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                        field,
                    ) if field.path_identity() == *identity => Some(field),
                    _ => None,
                })?;
            owner_path.push(crate::checked_trees::BorrowLoanOwnerSegment::Field(
                field.symbol,
            ));
...
fn moved_record_source(
    ...
    state: &symbol_resolved_trees_to_typed_trees::typed_trees::state::State,
    statement_index: u32,
    result_path: &[crate::checked_trees::CheckedUnitStructuralPathSegment],
) -> Option<
    Option<(
        SymbolHandle,
        arena::Handle<crate::checked_trees::BorrowLoanFact>,
...
fn call_actual_leaf_loan(
    ...
    source: &crate::checked_trees::CheckedReferenceResultSourcePlan,
) -> Option<arena::Handle<crate::checked_trees::BorrowLoanFact>> {
    let [
        path @ ..,
...
fn record_loan_is_active(
    ...
    entries.next().is_none()
        && facts.flow.contexts.constraint_refs.span(entry.entry_constraints).is_some_and(|constraints|
            constraints.iter().filter(|constraint| matches!(constraint.kind, crate::checked_trees::FlowConstraintKind::BorrowLoan { loan: active } if active == loan)).count() == 1)
...
pub(crate) fn declared_field_path<'a>(
    ...
    members: impl Iterator<Item = &'a symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableMemberExpression>,
) -> Option<(
    Vec<crate::checked_trees::CheckedUnitStructuralPathSegment>,
    TypeReferenceHandle,
)> {
    ...
    for member in members {
        ...
        path.push(
            crate::checked_trees::CheckedUnitStructuralPathSegment::Field(field.path_identity()),
        );
        selected = field.type_reference;
    }
    Some((path, selected))
...
pub fn projected_record_argument(
    ...
    statement_index: u32,
    source_statement: u32,
    path: &[crate::checked_trees::CheckedUnitStructuralPathSegment],
) -> bool {
    !path.is_empty()
        && source_statement < statement_index
        && local_record_loans(program, facts, machine, state, source_statement).is_some_and(
...
pub fn record_argument(
    ...
    if entries.next().is_some()
        || facts.flow.contexts.constraint_refs.span(entry.entry_constraints)?.iter()
            .filter(|constraint| matches!(constraint.kind, crate::checked_trees::FlowConstraintKind::BorrowLoan { loan: active } if active == *loan))
            .count() != 1
    {
        return None;
    }
    ...
    Some(crate::checked_trees::CheckedUnitStructuralArgumentPlan {
        source: crate::checked_trees::CheckedUnitStructuralArgumentSourcePlan::StructuralResult {
    ...
    for member in members.iter().rev() {
        ...
        let field = program
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    field,
                ) if field.symbol == member_symbol => Some(field),
                _ => None,
            })?;
        leaf_path.push(
            crate::checked_trees::CheckedUnitStructuralPathSegment::Field(field.path_identity()),
...
pub fn release_statement(
    ...
    machine: SymbolHandle,
    state: SymbolHandle,
    handle: arena::Handle<crate::checked_trees::BorrowLoanFact>,
) -> Option<u32> {
