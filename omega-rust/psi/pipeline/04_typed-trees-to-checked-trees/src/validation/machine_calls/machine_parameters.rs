...
use symbol_resolved_trees_to_typed_trees::typed_trees::data::{
    MachineParameterContract, TypeParameter, TypeParameterKind,
};
...
fn validate_static_machine_arguments_with_facts(
    ...
    let invocations = crate::validation::infer_synchronous_invocations(program);
    ...
    for (handle, expression) in program.expression_table.iter_expressions() {
        if let ExpressionNode::Call(call) = expression {
            ...
            // machine authority.
            if !symbol_resolved_trees_to_typed_trees::typed_trees::operator::named_expression_call_candidates(program, call).is_empty() {
                continue;
            }
        ...
        for state in program.machine_states(machine) {
                ...
                .iter_statements(state.statement_nodes)
            {
                if let StatementNode::Call(call) = statement {
                    if !symbol_resolved_trees_to_typed_trees::typed_trees::operator::named_statement_call_candidates(program, call)
                        .is_empty()
                    {
                        continue;
                    }
