...
pub(super) struct CallableParts<'a> {
    ...
    pub(super) invocations: Vec<crate::flow_effects::InvocationTarget>,
...
impl<'a> CallableParts<'a> {
    ...
    fn of_signature(
        ...
        Self {
            ...
            invocations: crate::validation::declared_signature_invocations(program, signature),
...
pub(super) fn validate_selected_callable_shape(
    ...
    invocations: &crate::flow_effects::InvocationInferencePlan,
    generic_call: &str,
...
fn validate_callable_parts(
    ...
    let allowed_invocations =
        crate::validation::declared_signature_invocations(program, requirement);
    for invocation in &actual.invocations {
        if allowed_invocations.contains(invocation) {
            continue;
        }
        let name = match invocation {
            crate::flow_effects::InvocationTarget::Parameter(index) => required_parameters
                .iter()
        ...
        .enumerate()
    {
        match (&required.kind, &actual.kind) {
            (TypeParameterKind::Type, TypeParameterKind::Type) => {
                if symbol_resolved_trees_to_typed_trees::typed_trees::data::type_parameter_demands_stronger_properties(required, actual) {
                    diagnostics.push(Diagnostic::error(format!(
