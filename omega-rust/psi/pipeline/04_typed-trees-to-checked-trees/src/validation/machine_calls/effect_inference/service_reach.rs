...
use crate::flow_effects::{
    ...
    OperationalPlan, ServiceReachInferencePlan, StateServiceReachInference,
};
...
struct MachineReachWork {
    ...
    calls: Vec<(
        SymbolHandle,
        DirectServiceReach,
        HandleSpan<crate::flow_effects::StaticMachineCallBinding>,
    )>,
}
...
struct CallTargetIndex {
    ...
    parameter_contract: symbols::SymbolKeyMap<
        SymbolHandle,
        arena::Handle<symbol_resolved_trees_to_typed_trees::typed_trees::data::TypeParameter>,
    >,
}

thread_local! {
    static CALL_TARGET_INDEX: std::cell::RefCell<
        Option<(symbol_resolved_trees_to_typed_trees::typed_trees::ProgramIdentity, usize, CallTargetIndex)>,
...
pub(crate) fn infer_service_reaches_uncached(
    ...
    for machine in program.machines() {
        ...
        let dependency = crate::flow_effects::ServiceReachDependency {
...
fn indexed_contract_view<'program>(
    ...
    contract: &'program symbol_resolved_trees_to_typed_trees::typed_trees::data::MachineParameterContract,
) -> Option<
    symbol_resolved_trees_to_typed_trees::typed_trees::data::MachineParameterContractView<'program>,
> {
    match contract {
        symbol_resolved_trees_to_typed_trees::typed_trees::data::MachineParameterContract::RequirementIdentity => None,
        symbol_resolved_trees_to_typed_trees::typed_trees::data::MachineParameterContract::Structural(signature) => Some(
...
fn direct_service_reach_for_operational_call(
    program: &TypedTrees,
    call: &crate::flow_effects::CallOperational,
) -> DirectServiceReach {
    ...
    if let Some(contract) = with_call_target_index(program, |index| {
        ...
        if let symbol_resolved_trees_to_typed_trees::typed_trees::data::MachineParameterContractView::Structural(signature) = contract {
            extend_service_set(
                &mut reach.services,
                program
                    .service_reach_rows
...
fn substitute_dependency(
    ...
    dependency: &DependencyWork,
    bindings: HandleSpan<crate::flow_effects::StaticMachineCallBinding>,
...
fn direct_service_reach_for_call(program: &TypedTrees, target: SymbolHandle) -> DirectServiceReach {
    ...
    let target = crate::validation::exact_compiler_intrinsic_boundary_requirement(program, target)
    ...
    for function in symbols::BuiltinFunction::asm_intrinsics() {
    ...
    let parameter_signature = with_call_target_index(program, |index| {
        index
            ...
            .and_then(|&(machine_index, parameter_index)| {
                ...
                let symbol_resolved_trees_to_typed_trees::typed_trees::data::TypeParameterKind::Machine { contract } = &parameter.kind
                else {
                    return None;
                };
                indexed_contract_view(program, index, contract)
...
fn extend_invocation_target_services(
    ...
    invocations: &[crate::flow_effects::InvocationTarget],
    services: &mut Vec<ServiceReachId>,
) {
    for target in invocations {
        let symbol = match target {
            ...
            crate::flow_effects::InvocationTarget::Service(symbol) => *symbol,
        };
