...
use symbol_resolved_trees_to_typed_trees::typed_trees::statement::{StatementNode, TableCall};
use symbols::SymbolHandle;

use crate::flow_effects::{InvocationInferencePlan, InvocationTarget, MachineInvocationInference};

use crate::validation::proof_contracts::default_domains::call_summaries::{
...
fn build_machine_work(
    ...
    MachineWork {
        ...
        uses_published: machine.supply_mode != language_semantics::MachineSupplyMode::CheckedBody
            || machine.is_public
...
fn collect_transition_target_expression_calls(
    ...
    match program.statement_table.transition_target(target) {
        symbol_resolved_trees_to_typed_trees::typed_trees::statement::TransitionTargetNode::Named {
            ...
            let target = crate::validation::declarations::transitions::named_transition_call_symbol(
                program,
                machine.symbol,
                path.symbol,
            );
...
fn boundary_service_for_receiver_path(
    ...
    path: &[symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier],
) -> Option<SymbolHandle> {
...
fn declared_targets(
    program: &TypedTrees,
    declarations: &[symbol_resolved_trees_to_typed_trees::typed_trees::signature::AuthoredInvocation],
    ...
    for declaration in declarations {
        let target = match declaration.target {
            symbol_resolved_trees_to_typed_trees::typed_trees::signature::AuthoredInvocationTarget::Unresolved => None,
            ...
            symbol_resolved_trees_to_typed_trees::typed_trees::signature::AuthoredInvocationTarget::Service(symbol) => program
                .traits()
                .iter()
...
fn machine_entry_parameters<'a>(
    ...
    machine: &Machine,
) -> Vec<&'a symbol_resolved_trees_to_typed_trees::typed_trees::signature::StateParameter> {
    program
...
fn boundary_service_for_type(
    ...
    let symbol = symbol_resolved_trees_to_typed_trees::typed_trees::service::exact_bound_service_requirement(program, type_reference)
        .unwrap_or_else(|| {
            program
                .type_reference_table
                .type_reference(type_reference)
