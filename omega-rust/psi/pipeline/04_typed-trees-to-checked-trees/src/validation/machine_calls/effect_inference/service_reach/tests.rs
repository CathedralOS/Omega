...
fn top_level_invocations_keep_fixed_services_and_exact_machine_bound() {
    ...
    let plan = infer_service_reaches(
        &program,
    ...
    assert_eq!(
        ...
        [
            crate::flow_effects::InvocationTarget::Parameter(0),
            crate::flow_effects::InvocationTarget::Parameter(1),
        ]
    );
...
fn nominal_dependencies_substitute_per_call_and_preserve_structural_rows() {
    let program = crate::validation::front_end::typed_program(&format!("{CONTRACT}
