...
use crate::checked_trees::{CheckedValueOrigin, CrashCause};
...
use symbol_resolved_trees_to_typed_trees::typed_trees::signature::SignatureContract;
use symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementHandle;
use symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceHandle;
use symbols::SymbolHandle;

pub use numerics::arithmetic::ArithmeticPolicyAdapter as CheckedArithmeticPolicyAdapter;
...
/// `effects`. The provider-planning boundary is responsible for copying
...
pub enum CheckedOperatorOccurrence {
    ...
    MatchEquality {
        source_arm:
            Handle<symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableMatchArm>,
    },
}
...
impl CheckedOperatorFacts {
    ...
    /// Named calls select their operator directly; the authored declaration
    /// contracts are the requirement.
    pub fn has_crash_qualified_uses(
        &self,
        program: &symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees,
    ) -> bool {
        ...
        }) || self.named_uses.iter().any(|(_, named_use)| {
...
mod tests {
    ...
    fn checked_operator_facts_constructor_keeps_use_root_explicit() {
        ...
        let contracts =
            HandleSpan::from_parts(arena::Handle::<SignatureContract>::from_arena_index(20), 3);
