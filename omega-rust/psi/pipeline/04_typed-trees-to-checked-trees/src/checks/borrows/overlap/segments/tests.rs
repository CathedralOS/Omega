...
use crate::checked_trees::BorrowCompatibilityPremiseRelation;
use crate::checked_trees::expression::{
...
fn named_bound(
    ...
    program.expression_table.insert_tree(
        &symbol_resolved_trees_to_typed_trees::typed_trees::expression::Expression::Name(
...
fn unresolved_field_does_not_prove_disjointness() {
    ...
    let unresolved = crate::fact_plan::PlaceSegment::default();
    let resolved = crate::fact_plan::PlaceSegment::Field {
...
fn fixed_and_legacy_literal_indices_compare_by_value() {
    ...
    assert!(place_segments_may_overlap(
        &program,
        &[crate::fact_plan::PlaceSegment::FixedIndex { index: 0 }],
        &[crate::fact_plan::PlaceSegment::Index { expression: zero }],
    ));
    assert!(!place_segments_may_overlap(
        &program,
        &[crate::fact_plan::PlaceSegment::FixedIndex { index: 0 }],
...
fn fixed_range_preserves_pure_constant_index_folding() {
    ...
    assert!(!place_segments_may_overlap(
        &program,
        &[crate::fact_plan::PlaceSegment::FixedRange { start: 0, end: 3 }],
        &[crate::fact_plan::PlaceSegment::Index { expression: three }],
    ));
    assert!(place_segments_may_overlap(
        &program,
        &[crate::fact_plan::PlaceSegment::FixedRange { start: 0, end: 4 }],
        &[crate::fact_plan::PlaceSegment::Index { expression: three }],
    ));
}
...
fn fixed_index_before_tail_range_is_disjoint() {
    ...
    let tail = program.expression_table.insert(ExpressionNode::Range(
        symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableRangeExpression {
            start: one,
    ...
    assert!(!place_segments_may_overlap(
        &program,
        &[crate::fact_plan::PlaceSegment::FixedIndex { index: 0 }],
        &[crate::fact_plan::PlaceSegment::Index { expression: tail }],
    ));
    assert!(place_segments_may_overlap(
        &program,
        &[crate::fact_plan::PlaceSegment::FixedIndex { index: 1 }],
...
fn fixed_structural_containment_is_directional() {
    let program = symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees::default();
    let range = |start, end| crate::fact_plan::PlaceSegment::FixedRange { start, end };
    let index = |index| crate::fact_plan::PlaceSegment::FixedIndex { index };
...
fn immutable_copy_chain_proves_same_dynamic_extent() {
    ...
    let index = |expression| crate::fact_plan::PlaceSegment::Index { expression };
    ...
    assert_eq!(
        ...
        vec![
            (
                ...
                0,
                crate::checked_trees::BorrowCompatibilitySelectorPosition::Index,
            ),
            (
...
fn stated_premise_disjoins_a_fixed_index_before_a_symbolic_window() {
    ...
    let window = range_bounds(&mut program, cut, four);
    let index = |expression| crate::fact_plan::PlaceSegment::Index { expression };
    let left = [crate::fact_plan::PlaceSegment::FixedIndex { index: 0 }];
    let right = [index(window)];
