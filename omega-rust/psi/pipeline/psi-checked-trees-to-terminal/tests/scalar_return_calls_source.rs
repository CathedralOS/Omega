...
fn checked_arms(source: &str, combined: bool) -> psi_checked_trees::CheckedTrees {
...
fn combine_value_machine_arms(syntax: &mut psi_syntax_trees::SyntaxTrees) {
    use psi_syntax_trees::statement::StatementNode;

    let machine = syntax
        .root_items()
        .find_map(|item| match item {
            psi_syntax_trees::item::Item::Machine(machine) if machine.name.as_str() == "value" => {
                Some(machine.clone())
            }
            _ => None,
        })
    ...
    syntax.items.state_mut(state).statements =
        psi_arena::HandleSpan::from_parts(statements.start(), statements.count() - 1);
}
...
fn return_call_preserves_normal_and_explicit_crash_outcomes() {
    for (cause_name, cause) in [
        ("Trap", psi_terminal::CrashCause::Trap),
        ("Abort", psi_terminal::CrashCause::Abort),
    ] {
        let source = format!(
            r#"
...
fn guarded_state_argument_short_circuit_rejects_unconditional_call_hoisting() {
    ...
    for operand in [
        ...
        for combined in [false, true] {
            ...
            let error = psi_checked_trees_to_terminal::lower_machine(&checked, "value")
