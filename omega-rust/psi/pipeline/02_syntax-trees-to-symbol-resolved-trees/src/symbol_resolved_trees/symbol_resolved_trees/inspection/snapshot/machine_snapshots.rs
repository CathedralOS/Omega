...
use crate::symbol_resolved_trees::machine::{Machine, OwnedData};
use crate::symbol_resolved_trees::signature::{StateParameter, StateSignature};
use crate::symbol_resolved_trees::state::State;
use crate::symbol_resolved_trees::symbol_resolved_trees::inspection::snapshot::declaration_snapshots::domain_fact_snapshots;
use crate::symbol_resolved_trees::symbol_resolved_trees::inspection::snapshot::statement_and_expression_snapshots::{
...
pub(crate) fn conformance_snapshot(
    ...
    let (implementation, rows) = match &conformance.implementation {
        ...
        crate::symbol_resolved_trees::trait_definition::ConformanceImplementation::Closed { rows } => (
            "closed",
            rows.iter()
                .map(|row| ConformanceRowSnapshot {
                    ...
                    source: match row.source {
                        crate::symbol_resolved_trees::trait_definition::ConformanceRowSource::Inline => "inline",
                        crate::symbol_resolved_trees::trait_definition::ConformanceRowSource::Reference => "reference",
...
fn signature_contract_snapshot(
    program: &SymbolResolvedTrees,
    contract: &crate::symbol_resolved_trees::signature::SignatureContract,
    ...
    let (kind, crash_cause) = match &contract.kind {
        ...
        crate::symbol_resolved_trees::signature::SignatureContractKind::Crashes { cause } => {
            return SignatureContractSnapshot {
                kind: "crashes",
                binding: contract.binding.as_ref().map(ToString::to_string),
                crash_cause: Some(match cause {
                    crate::symbol_resolved_trees::signature::CrashCause::Trap => "Trap",
                    crate::symbol_resolved_trees::signature::CrashCause::Abort => "Abort",
                }),
