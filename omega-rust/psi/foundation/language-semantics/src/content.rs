...
pub fn terminal_projection_report_fingerprint(
    ...
    expression: &semantic_vocabulary::ContentProjectionExpression,
) -> u64 {
    fn encode_terminal_scalar(
        expression: &semantic_vocabulary::ContentProjectionScalar,
        output: &mut Vec<u8>,
    ) {
        use semantic_vocabulary::ContentProjectionScalar as Scalar;
        match expression {
    ...
    match expression {
        semantic_vocabulary::ContentProjectionExpression::IntervalSet(members) => {
            bytes.push(3);
        ...
        semantic_vocabulary::ContentProjectionExpression::CountedQuantity(magnitude) => {
            bytes.push(2);
