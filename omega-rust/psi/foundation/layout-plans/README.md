...
- [layout_reports.rs](src/layout_reports.rs): validated geometry reports,
  conventional sum layouts, replay fingerprints and matching.
- [field_values.rs](src/field_values.rs): ordinary scalar and aggregate values
  supplied to a materializer, with their validating schemas.
- [symbolic_values.rs](src/symbolic_values.rs): entry stub and data symbol
  identities, relocation targets, symbolic field paths, interior layouts.
- [placement.rs](src/placement.rs): byte order, consumption instants, phases,
  address ranges, regimes, scopes, and placement constraints.
- [post_handoff_writer.rs](src/post_handoff_writer.rs): the reusable writer
  fragment ABI and the plans a provider replays after handoff.
- [materialization.rs](src/materialization.rs): materialization plans, their
...
- [stored_integer_writes.rs](src/stored_integer_writes.rs): scalar fragments,
  stored-integer fit validation, and container reads and writes.
- [materialization_field_identities.rs](src/materialization_field_identities.rs):
