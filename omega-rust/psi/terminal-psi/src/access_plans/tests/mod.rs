...
use crate::extents::ExtentLoan;
use crate::extents::ProviderExistingContentGrant;
...
fn extent_id<T>(
    identity: u64,
    constructor: fn(u64) -> Result<T, crate::extents::ExtentDiagnostic>,
) -> T {
    constructor(identity).expect("normalized extent identity")
}

fn provider_issuance(seed: u64) -> crate::extents::ExtentProviderIssuance {
    let base = seed * 16;
    crate::extents::ExtentProviderIssuance::from_normalized_identities([
        base + 1,
        base + 2,
        base + 3,
        base + 4,
...
fn extent_rights(identities: &[u64]) -> ExtentRights {
    ExtentRights::from_normalized_identities(identities.iter().copied().map(|identity| {
        extent_id(
            identity,
            crate::extents::ExtentRightId::from_normalized_identity,
        )
    }))
}

fn uart_extent(base: u64, length: u64) -> crate::extents::Extent {
    uart_extent_with_lineage(base, length, 1)
}

fn uart_extent_with_lineage(base: u64, length: u64, lineage: u64) -> crate::extents::Extent {
...
fn uart_extent_with_root(
    ...
    provider: u64,
    lineage: u64,
) -> crate::extents::Extent {
    uart_root_grant(provider, lineage)
        .mint(base, length)
        .expect("UART extent")
}

fn uart_root_grant(provider: u64, lineage: u64) -> crate::extents::ExtentRootGrant {
...
fn uart_root_grant_with_mapping(
    ...
    provenance: u64,
    era: u64,
) -> crate::extents::ExtentRootGrant {
    crate::extents::ExtentRootGrant::from_admitted_provider(
        provider_issuance(provider),
        extent_id(
            lineage,
            crate::extents::ExtentLineageId::from_normalized_identity,
        ),
        extent_id(2, AddressSpaceId::from_normalized_identity),
        extent_rights(&[3, 4]),
        extent_id(provenance, ExtentProvenanceId::from_normalized_identity),
        extent_id(era, crate::extents::MappingEraId::from_normalized_identity),
    )
}
...
fn device_requirement_mapped_range(offset: u64, length: u64) -> MappedRangeReceiptContext {
    let source = crate::extents::ExtentRootGrant::from_admitted_provider(
    ...
    let destination = crate::extents::ExtentRootGrant::from_admitted_provider(
