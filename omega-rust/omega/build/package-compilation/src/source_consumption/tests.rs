...
pub(super) fn canonical_row_fixture() -> Vec<ConsumedSourceUnit> {
...
/// One-field substitution matrix over the retained source-consumption
/// custody: each `ConsumedSourceUnit` row field, each roster-shape axis, and
/// each reconciled package-graph axis the commitment hashes, declared as
/// `*FieldForTest` inventories in `test_support` and driven by the shared
/// `run_one_field_substitution_matrix`. Every representable substitution
/// either keeps the canonical roster order and recomputes a divergent
/// commitment, or cannot keep the order and rejects at derivation as
/// non-canonical. Build-purpose edges are compilation-local nameability,
/// never bound by this commitment -- the durable closure projection is
/// product-only.
#[test]
fn source_consumption_commitment_rejects_every_one_field_substitution() {
    use super::super::{PackageCompilationInputs, PackageDependencyBinding, PackageSourceBinding};
    use super::test_support::{
        ConsumedSourceRosterAxisForTest, ConsumedSourceUnitFieldForTest,
        PackageGraphConsumptionFieldForTest, canonical_source_consumption_inputs,
        check_source_consumption_custody, corrupt_source_consumption_graph_for_test,
        corrupt_source_consumption_roster_for_test, corrupt_source_consumption_unit_for_test,
        foreign_source_consumption_donor, honest_source_consumption_custody_case,
        source_consumption_custody, source_consumption_graph_outcome,
        source_consumption_roster_outcome, source_consumption_unit_outcome,
    };
    ...
    optimization_core::run_one_field_substitution_matrix(&OneFieldSubstitutionMatrix {
        family: "consumed source unit row",
        fields: ConsumedSourceUnitFieldForTest::INVENTORY,
        honest: &honest_source_consumption_custody_case,
        donor: foreign_source_consumption_donor(),
        custody: &source_consumption_custody,
        substitute: &corrupt_source_consumption_unit_for_test,
        check: &check_source_consumption_custody,
        outcome: &source_consumption_unit_outcome,
        joined_replay: None,
    });

    optimization_core::run_one_field_substitution_matrix(&OneFieldSubstitutionMatrix {
        family: "consumed source roster",
        fields: ConsumedSourceRosterAxisForTest::INVENTORY,
        honest: &honest_source_consumption_custody_case,
        donor: foreign_source_consumption_donor(),
        custody: &source_consumption_custody,
        substitute: &corrupt_source_consumption_roster_for_test,
        check: &check_source_consumption_custody,
        outcome: &source_consumption_roster_outcome,
        joined_replay: None,
    });

    optimization_core::run_one_field_substitution_matrix(&OneFieldSubstitutionMatrix {
        family: "package graph consumption",
        fields: PackageGraphConsumptionFieldForTest::INVENTORY,
        honest: &honest_source_consumption_custody_case,
        donor: foreign_source_consumption_donor(),
        custody: &source_consumption_custody,
        substitute: &corrupt_source_consumption_graph_for_test,
        check: &check_source_consumption_custody,
        outcome: &source_consumption_graph_outcome,
        joined_replay: None,
    });
    ...
    let (package, inputs, root) = canonical_source_consumption_inputs();
