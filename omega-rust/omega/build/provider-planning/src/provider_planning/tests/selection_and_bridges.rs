...
fn selected_provider_provenance_retains_owner_controlled_composition_mode() {
    ...
    assert_eq!(
        crate::provider_planning::selection_provenance::ProviderSelectionProvenance::BuildOverride(
            vec![fused]
        )
        .composition_mode()
    ...
    assert!(
        crate::provider_planning::selection_provenance::ProviderSelectionProvenance::TargetDefault(
            vec![independent]
        )
        .composition_mode()
    ...
    assert_eq!(
        crate::provider_planning::selection_provenance::ProviderSelectionProvenance::UniqueCoveringCandidate
            .composition_mode()
...
fn checked_synchronous_invocation_targets_reject_every_exact_drift() {
    ...
    for (drift, expected) in cases {
        ...
        match drift {
            ...
            CheckedInvocationDrift::InvalidService => {
                inferred.machines[0].inferred_transitive =
                    vec![flow_effects::InvocationTarget::Service(
            ...
            CheckedInvocationDrift::NonBoundaryService => {
                typed.push_trait_definition(typed_trees::trait_definition::TraitDefinition {
                    symbol: symbols::SymbolHandle::from_arena_index(99),
                    name: typed_trees::name::Identifier::generated("pkg::Plain"),
                    ..Default::default()
                });
                inferred.machines[0].inferred_transitive =
                    vec![flow_effects::InvocationTarget::Service(
...
fn external_root_bridge_requires_one_exact_retained_boundary_slot() {
    ...
    let facts = effects::SelectedProviderPlanFacts::from_selection(
        &[first.clone(), second],
...
fn external_root_bridge_rejects_compact_equal_exact_plan_substitution() {
    ...
    let facts = effects::SelectedProviderPlanFacts::from_selected_plans(vec![plan])
...
fn granted_selected_plan_does_not_stamp_a_different_exact_requirement() {
    ...
    let mut owner = typed_trees::trait_definition::TraitDefinition {
        symbol: owner_symbol,
        is_boundary: true,
        name: typed_trees::name::Identifier::generated("PairBase"),
        ..Default::default()
    };
    for (symbol, name) in [
