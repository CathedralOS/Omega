...
fn replay_scalar_values(
    values: &[BuildFilesystemScalarOperandValue],
) -> Vec<checked_interpreter::FilesystemScalarOperandValue> {
    values
        .iter()
        .copied()
...
fn unknown_descriptor_seek_failure_records_authored_scalars_exactly() {
    ...
    assert_eq!(
        ...
        vec![
            (
                1,
                checked_interpreter::FilesystemScalarOperandValue::I64(-17)
            ),
            (2, checked_interpreter::FilesystemScalarOperandValue::I32(2)),
        ]
    );
    assert!(!replay.has_output_attempts());
}
...
fn unknown_descriptor_at_failures_round_trip_exact_authored_inputs() {
    ...
    for (operation_tag, operation_name) in [(14, "open_at"), (15, "unlink_at")] {
        ...
        assert_eq!(
            flags.value(),
            checked_interpreter::FilesystemScalarOperandValue::I32(0x241)
        );
        assert!(!replay.has_output_attempts());
        assert!(replay.output_entries().is_empty());
    }
    ...
    for (operation_tag, offset) in fixtures {
        ...
        assert_eq!(
            ...
            offset
                .map(|offset| vec![(
                    2,
                    checked_interpreter::FilesystemScalarOperandValue::I64(offset),
                )])
                .unwrap_or_default()
        );
        assert!(!replay.has_output_attempts());
...
fn unknown_descriptor_read_file_metadata_failure_round_trips_exact_carrier() {
    let limits = BuildFilesystemReplayRecordLimits::default();
    for buffer in [
        vec![0; checked_interpreter::FILESYSTEM_METADATA_API_CARRIER_BYTES],
        vec![11; checked_interpreter::FILESYSTEM_METADATA_API_CARRIER_BYTES + 19],
    ] {
...
fn unknown_descriptor_read_file_metadata_failure_rejects_carrier_drift() {
    let limits = BuildFilesystemReplayRecordLimits::default();
    let carrier = || vec![1; checked_interpreter::FILESYSTEM_METADATA_API_CARRIER_BYTES];
    ...
    let short = unknown_descriptor_read_file_metadata_summary(vec![
        1;
        checked_interpreter::FILESYSTEM_METADATA_API_CARRIER_BYTES
            - 1
    ]);
...
fn unknown_descriptor_get_osfhandle_failure_round_trips_exact_model() {
    ...
    assert_eq!(
        attempt.result(),
        Some(checked_interpreter::FilesystemOperationResult::Scalar(-2))
    );
