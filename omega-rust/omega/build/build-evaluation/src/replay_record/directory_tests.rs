use super::{
    ...
    BuildFilesystemReplayRecordLimits, BuildFilesystemReplayVerdict, BuildFilesystemRoot,
...
fn recovery_rehydrates_exact_empty_output_directory_tree() {
    ...
    assert_eq!(
        directories
            .iter()
            .map(checked_interpreter::FilesystemOutputDirectoryReplayRecord::output_relative_path)
            .collect::<Vec<_>>(),
...
fn recovery_rejects_directory_path_over_explicit_ceiling() {
    ...
    let overlong =
        vec![b'a'; checked_interpreter::MAX_FILESYSTEM_REPLAY_OUTPUT_DIRECTORY_PATH_BYTES + 1];
