...
fn source_prefixed_directory_replay() -> checked_interpreter::FilesystemReplay {
    let read = checked_interpreter::FilesystemReplayReadRecord::new(
    ...
    let chain = checked_interpreter::FilesystemSourceReadChainReplayRecord::new(
    ...
    let source = checked_interpreter::FilesystemSourceInputReplayRecord::new(vec![
        checked_interpreter::FilesystemSourceInputReplayEventRecord::ReadChain(chain),
    ])
    .expect("Source input is nonempty");
    let remove = checked_interpreter::FilesystemOutputAbsentRemoveReplayRecord::new(
        checked_interpreter::FilesystemOutputAbsentRemoveKind::Directory,
        crate::BUILD_OUTPUT_ROOT_IDENTITY,
        b"missing-directory".to_vec(),
    )
    .expect("absent directory removal is canonical");
    let record = checked_interpreter::FilesystemInputOutputAbsentRemovesReplayRecord::new(
        Some(source),
        vec![remove],
    )
    .expect("Source-prefixed absent directory removal is canonical");
    checked_interpreter::FilesystemReplay::from_input_output_absent_removes_record(record)
...
fn source_prefix_classifier_stops_before_directory_removal() {
    ...
    assert_eq!(
        crate::evidence::replay_eligibility::source_input_replay_prefix_end(replay.attempts()),
        Some(3)
    );
}
