...
pub(super) fn validate_output_symlink_shape(
    ...
    if attempt.operation != SYMLINK_OPERATION_TAG
        ...
        || *target_ordinal != 0
        || target.is_empty()
        || target.len() > checked_interpreter::MAX_FILESYSTEM_REPLAY_OUTPUT_SYMLINK_TARGET_BYTES
        || target.contains(&0)
        || rooted.ordinal != 1
        || rooted.root != 1
        || rooted.bytes.len()
            > checked_interpreter::MAX_FILESYSTEM_REPLAY_OUTPUT_DIRECTORY_PATH_BYTES
...
pub(super) fn rehydrate_output_symlink_shape(
    ...
    checked_interpreter::FilesystemOutputSymlinkReplayRecord,
...
> {
    ...
    checked_interpreter::FilesystemOutputSymlinkReplayRecord::new(
