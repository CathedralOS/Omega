...
use crate::evidence::replay_record::attempt_codec::ShapeAuthorizedPath;
use crate::evidence::replay_record::attempt_codec::ShapeMetadata;
use crate::evidence::replay_record::attempt_codec::ShapeMutableBytes;
use crate::evidence::replay_record::attempt_codec::ShapeResult;
use crate::evidence::replay_record::attempt_codec::ShapeReturnedPath;
...
fn source_read_link_rehydrates_the_exact_attempt() {
    ...
    let source = checked_interpreter::FilesystemSourceInputReplayRecord::new(vec![
        checked_interpreter::FilesystemSourceInputReplayEventRecord::ReadLink(record),
    ])
    .expect("typed Source event");
    let replay = checked_interpreter::FilesystemReplay::from_source_input_record(source)
    ...
    assert_eq!(
        attempt.result(),
        Some(checked_interpreter::FilesystemOperationResult::Scalar(6))
    );
