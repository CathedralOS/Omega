...
use crate::BuildFilesystemObservedByteRegionKind;
...
fn recovery_rehydrates_exact_portable_and_windows_hard_links() {
    ...
    assert_eq!(
        ...
        vec![
            (
                checked_interpreter::FilesystemOutputHardLinkReplayKind::Portable,
                b"locked.bin".as_slice(),
                b"portable.bin".as_slice(),
            ),
            (
                checked_interpreter::FilesystemOutputHardLinkReplayKind::Windows,
                b"portable.bin".as_slice(),
...
fn recovery_rehydrates_exact_output_lock_pair() {
    ...
    assert_eq!(
        ...
        checked_interpreter::FilesystemScalarOperandValue::I32(6)
    );
    assert_eq!(
        attempts[5].scalar_operands()[0].value(),
        checked_interpreter::FilesystemScalarOperandValue::I32(8)
    );
    assert_eq!(
        attempts[4].result(),
        Some(checked_interpreter::FilesystemOperationResult::Scalar(0))
    );
