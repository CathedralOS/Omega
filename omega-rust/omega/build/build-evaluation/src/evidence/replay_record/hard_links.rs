...
use crate::evidence::replay_record::attempt_codec::AttemptShape;
use crate::evidence::replay_record::attempt_codec::ShapeResult;
use crate::evidence::replay_record::attempt_codec::ShapeRootedPath;
use crate::evidence::replay_record::attempt_codec::ShapeScalar;
use crate::evidence::replay_record::record::clone_bytes;
...
pub(super) fn rehydrate_output_hard_link_shape(
    ...
    checked_interpreter::FilesystemOutputHardLinkReplayRecord,
    BuildFilesystemReplayRecordError,
> {
    let (kind, existing, output, _, _) = hard_link_shape(attempt)?;
    checked_interpreter::FilesystemOutputHardLinkReplayRecord::new(
...
impl HardLinkKind {
    ...
    const fn interpreter_kind(self) -> checked_interpreter::FilesystemOutputHardLinkReplayKind {
        match self {
            Self::Portable => checked_interpreter::FilesystemOutputHardLinkReplayKind::Portable,
            Self::Windows => checked_interpreter::FilesystemOutputHardLinkReplayKind::Windows,
        }
    }
}
...
fn valid_output_path(path: &[u8]) -> bool {
    path.len() <= checked_interpreter::MAX_FILESYSTEM_REPLAY_OUTPUT_DIRECTORY_PATH_BYTES
        && checked_interpreter::filesystem_root_relative_path_is_canonical(path, false)
}

fn authorization_matches(
    authorized: &crate::evidence::replay_record::attempt_codec::ShapeAuthorizedPath<'_>,
