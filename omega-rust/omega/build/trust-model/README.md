...
- [trust_admission.rs](src/trust_admission.rs) defines admission values, strong
  subject digests, and identity comparison. Compact report identities are not
  admission authority.
- [trust_report.rs](src/trust_report.rs) reconstructs diagnostic report evidence.
- [provider_grants.rs](src/provider_grants.rs) resolves selected-provider grants;
  [non_provider_grants.rs](src/non_provider_grants.rs) resolves the separate
  accepted-machine selectors.
- [accepted_templates.rs](src/accepted_templates.rs) holds exact accepted-template
