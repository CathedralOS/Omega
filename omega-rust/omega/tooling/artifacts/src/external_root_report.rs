...
fn push_external_root_json(output: &mut String, record: &InstalledRootRecord) {
    ...
    for (index, (root, input)) in record.stack.realization.inputs().enumerate() {
        ...
        match input.body_evidence() {
            ...
            external_roots::StackLocalEvidence::AdmittedProvider {
                validation_receipt, ..
            } => {
                output.push_str(
        ...
        output.push_str(match entry_evidence.arrival_origin() {
            external_roots::ArrivalStackRealizationOrigin::NoHardwareArrival => {
                "no_hardware_arrival"
            }
            external_roots::ArrivalStackRealizationOrigin::X86_64TargetRule => "x86_64_target_rule",
            external_roots::ArrivalStackRealizationOrigin::OpaqueProvider => "opaque_provider",
        });
        output.push_str("\", \"adapter_origin\": \"");
        output.push_str(match entry_evidence.adapter_origin() {
            external_roots::AdapterStackRealizationOrigin::None => "none",
            external_roots::AdapterStackRealizationOrigin::GeneratedProgramStorageSemanticWrapper => {
                "generated_program_storage_semantic_wrapper"
            }
            external_roots::AdapterStackRealizationOrigin::OpaqueProvider => {
                "opaque_provider"
            }
        });
        ...
        .enumerate()
    {
        ...
        match &summary.local_evidence {
            external_roots::FixedFuelLocalEvidence::TerminalEntry(binding) => {
            ...
            external_roots::FixedFuelLocalEvidence::TerminalSegment(binding) => {
                let certificate = binding.certificate();
                output.push_str(
            ...
            external_roots::FixedFuelLocalEvidence::AdmittedProvider {
                validation_receipt, ..
            } => {
                output.push_str(
                    ", \"origin\": \"admitted_provider\", \"provider_validation_receipt\": ",
...
fn push_entry_stack_realization_json(
    output: &mut String,
    realization: &calling_conventions::ValidatedEntryStackRealization,
) {
    output.push('[');
    for (context_index, context) in realization.realization().contexts.iter().enumerate() {
        ...
        for (epoch_index, epoch) in context.epochs.iter().enumerate() {
            ...
            output.push_str(match epoch.stage {
                calling_conventions::EntryStackStage::Enter => "enter",
                calling_conventions::EntryStackStage::Body => "body",
                calling_conventions::EntryStackStage::Exit => "exit",
            });
...
fn push_stack_domain_ref_json(output: &mut String, domain: calling_conventions::StackDomainRef) {
    match domain {
        calling_conventions::StackDomainRef::Interrupted => {
            output.push_str("{\"kind\": \"interrupted\"}")
        }
        calling_conventions::StackDomainRef::Dedicated { class } => {
            output.push_str("{\"kind\": \"dedicated\", \"class\": ");
            output.push_str(&class.to_string());
            output.push('}');
        }
        calling_conventions::StackDomainRef::ProviderSelected => {
            output.push_str("{\"kind\": \"provider_selected\"}")
        }
    }
}

fn push_stack_domain_json(output: &mut String, domain: external_roots::StackDomain) {
    match domain {
        external_roots::StackDomain::Interrupted => output.push_str("{\"kind\": \"interrupted\"}"),
        external_roots::StackDomain::Dedicated { class } => {
            output.push_str("{\"kind\": \"dedicated\", \"class\": ");
            output.push_str(&class.to_string());
            output.push('}');
        }
        external_roots::StackDomain::ProviderSelected => {
            output.push_str("{\"kind\": \"provider_selected\"}")
        }
    }
}

fn push_preemption_json(output: &mut String, preemption: calling_conventions::Preemption) {
    match preemption {
        calling_conventions::Preemption::NotApplicable => {
            output.push_str("{\"kind\": \"not_applicable\"}")
        }
        calling_conventions::Preemption::Masked => output.push_str("{\"kind\": \"masked\"}"),
        calling_conventions::Preemption::Nestable { maximum_depth } => {
