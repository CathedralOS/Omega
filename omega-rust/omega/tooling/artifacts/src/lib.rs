...
//! `external-roots` out of the default dependency graph otherwise. The
...
//! Driven by `compiler`'s pipeline, which decides what to render;
//! `visualizations` produces the JSON this crate writes, and
//! `effects` owns the carriers behind that.
//!
...
//! `compilation-report` implement the same `.{name}.{pid}.tmp` staging
