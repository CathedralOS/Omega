    ...
    program
        ...
        .for_each_mut(|_, fact| {
            if fact.machine == machine {
                fact.interface = language_semantics::ServiceReachInterface::PublishedCeiling(empty);
            }
        });
    program
        .facts
...
fn machine_contract_manifest_reads_independent_mutation_facts() {
    ...
    retained_frame.frame = facts::NormalizedWriteFrame::complete(vec!["self.value".to_owned()]);
...
fn machine_contract_manifest_distinguishes_published_empty_from_internal_empty_reach() {
    ...
    for (machine, interface) in [
        (
            public_symbol,
            language_semantics::ServiceReachInterface::PublishedCeiling(empty),
        ),
        (
            private_symbol,
