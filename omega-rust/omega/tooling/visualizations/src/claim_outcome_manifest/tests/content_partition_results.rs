...
fn content_partition_result_rewrite_validation_fixture() -> CheckedTrees {
    ...
    let (source, target) = {
        ...
        substitution
            .target
            .segments
            .push(language_semantics::content::ContentPlaceSegment::FixedIndex(2));
    ...
    let output_segments = program
        ...
        .ownership
        .segments
        .insert_many([facts::PlaceSegment::FixedIndex { index: 2 }]);
    assert!(source.segments.is_empty());
    assert_eq!(
        target.segments,
        vec![language_semantics::content::ContentPlaceSegment::FixedIndex(2)]
    );
...
fn content_partition_result_rewrite_manifest_rejects_duplicate_source() {
    ...
    duplicate
        .target
        .segments
        .push(language_semantics::content::ContentPlaceSegment::FixedIndex(3));
...
fn content_partition_result_rewrite_manifest_rejects_missing_substitution_pair() {
    ...
    row.result_rewrites[0]
        .target
        .segments
        .push(language_semantics::content::ContentPlaceSegment::FixedIndex(9));
    let row = row.clone();
    validate_content_partition_result_rewrites(&program, &row);
}
...
fn content_partition_result_rewrite_manifest_rejects_invalid_event_path() {
    ...
    program
        ...
        .for_each_mut(|_, event| {
            if event.claim_identity == row.result_rewrites[0].claim_identity {
                event.segments =
                    arena::HandleSpan::from_parts(arena::Handle::from_arena_index(999), 1);
            }
        });
    validate_content_partition_result_rewrites(&program, &row);
}
...
fn content_partition_result_rewrite_manifest_rejects_runtime_event_path() {
    ...
    let runtime_path =
        program
            ...
            .ownership
            .segments
            .insert_many([facts::PlaceSegment::Index {
...
fn content_partition_result_rewrite_manifest_rejects_invalid_outcome_span() {
    ...
    program
        ...
        .claim_outcome_maps
        .for_each_mut(|_, map| {
            map.entries = arena::HandleSpan::from_parts(arena::Handle::from_arena_index(999), 1);
        });
    validate_content_partition_result_rewrites(&program, &row);
}
...
fn content_partition_result_rewrite_manifest_rejects_output_path_mismatch() {
    ...
    let mismatched = program
        ...
        .ownership
        .segments
        .insert_many([facts::PlaceSegment::FixedIndex { index: 3 }]);
    let entries = program
        .facts
        .flow
        .ownership
