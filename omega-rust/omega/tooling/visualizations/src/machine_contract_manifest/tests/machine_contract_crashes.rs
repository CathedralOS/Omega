...
fn crash_source_coordinate_call(
    ...
    call_ordinal: u32,
    target: SymbolHandle,
) -> checked_trees::CheckedCrashCallSite {
    checked_trees::CheckedCrashCallSite::new(
        checked_trees::CrashCallSiteLocation::new(state, statement_ordinal, call_ordinal),
        target,
        target,
        1,
        Vec::new(),
...
fn machine_contract_manifest_crash_target_coordinates_reject_local_capsule() {
    ...
    program
        ...
        .contract_plans
        .crash_capsules
        .push(checked_trees::CrashContractCapsule::new(
            local_machine,
            local_state,
            0x1111,
            Vec::new(),
...
fn machine_contract_manifest_crash_target_coordinates_preserve_capsule_payload() {
    let (mut program, _, _, _, trait_owner, trait_signature) = crash_target_coordinate_fixture();
    let bucket = checked_trees::CrashRouteBucket::unconditional(checked_trees::CrashCause::Abort);
    program
        .facts
        .contract_plans
        .crash_capsules
...
fn machine_contract_manifest_crash_source_coordinates_reject_invalid_call_span() {
    ...
    program.facts.flow.control.states.for_each_mut(|_, flow| {
        flow.calls = arena::HandleSpan::from_parts(arena::Handle::from_arena_index(999), 1);
    });
    let machine = program
        .machines()
        .iter()
