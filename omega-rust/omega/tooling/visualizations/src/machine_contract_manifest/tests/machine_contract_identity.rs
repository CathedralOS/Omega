...
fn machine_contract_manifest_keeps_interface_and_witness_separate() {
    ...
    let crash = checked_trees::CrashPlan::published_ceiling(vec![
        checked_trees::CrashRouteBucket::unconditional(checked_trees::CrashCause::Abort),
    ]);
    ...
    let abandoned_claim = language_semantics::PermissionClaimIdentity::Established {
    ...
    let crash = crash
        .with_checked_sites(vec![
            checked_trees::CheckedCrashSite::new(
                checked_trees::CrashSiteLocation::new(state_symbol, 4),
                checked_trees::CrashCause::Abort,
                vec![abort_bucket],
                vec![abandoned_claim],
            )
            .with_path_guard_conjuncts(vec![
                checked_trees::CrashPredicateIdentity::from_canonical_bytes(vec![1, 9, 0, 0, 0, 0]),
            ])
            .with_path_guard_consequences(vec![
                checked_trees::CrashPredicateIdentity::from_canonical_bytes(vec![1, 4, 1]),
            ]),
        ])
        .expect("one crash site per source location")
        .with_checked_calls(vec![
            checked_trees::CheckedCrashCallSite::new(
                checked_trees::CrashCallSiteLocation::new(state_symbol, 7, 2),
                symbol,
                state_symbol,
                0x1234,
                vec![checked_trees::CrashRouteBucket::unconditional(
            ...
            checked_trees::CheckedCrashCallSite::new(
                checked_trees::CrashCallSiteLocation::new(state_symbol, 8, 0),
    ...
    program.facts.synchronous_invocations.machines.push(
        checked_trees::MachineSynchronousInvocationFact {
            machine: symbol,
            published_targets: vec![flow_effects::InvocationTarget::Parameter(0)],
            checked_inferred_targets: vec![flow_effects::InvocationTarget::Parameter(0)],
            plan: language_semantics::SynchronousInvocationPlan {
                interface: language_semantics::SynchronousInvocationInterface::PublishedCeiling,
    ...
    program
        ...
        .push(MachineSpecialization {
            ...
            template_contract_commitment:
                typed_trees::typed_trees::MachineTemplateCommitment::from_digest([0x11; 32]),
