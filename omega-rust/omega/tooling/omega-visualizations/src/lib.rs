...
//! `omega-compiler/src/pipeline/artifacts.rs` on every package-aware build.
...
//! `omega-compiler/src/pipeline/artifacts.rs` calls into this crate eight times
//! per build. `omega-effects` owns every carrier rendered here.
//!
...
//! `omega-effects/src/isolated_executable_scopes.rs`. This is the mirror of the
//! seam recorded in `omega-effects`, where a LIVE writer in this crate consumes
