    ...
    context
        ...
        .map(|row| {
            let owner = match row.owner {
                terminal_verifier::ReconstructedTerminalObligationOwner::ScalarBlockInvariant { machine, header, edge } => ProofQuestionOwner::ScalarBlockInvariant { machine, header, edge },
