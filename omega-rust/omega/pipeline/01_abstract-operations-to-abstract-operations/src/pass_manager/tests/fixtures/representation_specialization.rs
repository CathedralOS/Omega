...
/// whatever value the caller supplied, so the whole selected registry must
/// decline it.
...
/// The established-place positive for field values: two scalar field reads on
/// `p`'s place fold in one candidate whose producer is the `EstablishRecord`
/// that fixed both initializers to constants.
pub(in crate::pass_manager::tests) fn verified_field_value_unit() -> VerifiedPsiOptimizationUnit {
    let (module, proof) = lowered_module(
...
/// The field-value boundary decline: a real field read the unit cannot prove,
/// so the selection's admission edge commits nothing.
pub(in crate::pass_manager::tests) fn verified_field_value_decline_unit()
-> VerifiedPsiOptimizationUnit {
    let (module, proof) =
...
/// The cyclic freeze for field values: a bound-proven field read inside a
/// machine carrying an authenticated cyclic component must stay byte-exact
/// under the selection. The same source restrictions apply, so the Terminal
/// module is built directly: an unranked self-loop header observes its owned
/// `Token` parameter, and `Token`'s record field carries a declared
/// `BoundedInteger` singleton bound that proves the stored value with no
/// producer at all.
pub(in crate::pass_manager::tests) fn verified_cyclic_field_value_unit()
-> VerifiedPsiOptimizationUnit {
    use semantic_vocabulary::{
        BlockId, BoundedIntegerType, ContractId, EdgeId, IntegerSign, IntegerType, IntegerValue,
        MachineId, OperationId, PlaceId, ScalarType, StructuralFieldId, StructuralPlaceKind,
        StructuralTypeId, ValueId,
    };
    use terminal_psi::{
        Block, MachineContract, Operation, OperationKind, OperationResult, StructuralAccess,
