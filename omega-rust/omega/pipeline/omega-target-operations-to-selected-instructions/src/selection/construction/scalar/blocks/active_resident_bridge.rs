//! Pressure-bearing active-resident exact-add bridge block construction.
...
pub(in crate::selection::construction::scalar) fn active_resident_exact_add_bridge_chain(
    context: &ScalarConstructionContext<'_>,
    id: SelectedBlockId,
    source_block: psi_core::BlockId,
    source: &SourceLeaf,
) -> Result<SelectedBlock, SelectedInstructionError> {
    let SourceLeafValue::ActiveResidentExactAddBridgeChain(chain) = &source.value else {
