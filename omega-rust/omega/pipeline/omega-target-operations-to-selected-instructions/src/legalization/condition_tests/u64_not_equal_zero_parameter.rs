...
fn source_rejects_attachment_block_parameter_nested_and_extra_operation_grammar() {
    ...
    let attachment = id::<psi_core::StructuralTypeId>(91);
    ...
    block_parameter
        .abstracted
        .block_entries
        .push(omega_abstract_operations::AbstractBlockEntry {
            block,
...
fn replay_rejects_every_condition_custody_corruption() {
    ...
    reject_corruption!(
        zero,
        |value: &mut omega_legalized_operations::LegalizedImmediate| value.source_value = id(96)
    );
    reject_corruption!(
        zero,
        |value: &mut omega_legalized_operations::LegalizedImmediate| value.value =
            IntegerValue::Unsigned(1)
    );
    reject_corruption!(
        zero,
        |value: &mut omega_legalized_operations::LegalizedImmediate| value.constant_operation =
            id(96)
    );
    reject_corruption!(
        zero,
        |value: &mut omega_legalized_operations::LegalizedImmediate| {
    ...
    reject_corruption!(
        zero,
        |value: &mut omega_legalized_operations::LegalizedImmediate| value.fuel[0].units += 1
    );
