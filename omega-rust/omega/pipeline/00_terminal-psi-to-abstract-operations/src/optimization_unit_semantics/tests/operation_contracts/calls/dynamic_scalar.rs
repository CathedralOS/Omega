...
use crate::optimization_unit::recompute_psi_optimization_unit_identity;
...
fn dynamic_unit() -> crate::optimization_unit::PsiOptimizationUnit {
    let source = r#"
        trait Measure {
    ...
    let proof = encode_proof_section(&terminal.semantic_module, &terminal.proof_bundle)
        .expect("encode proof");
    let plan = crate::lower_artifact(
        crate::ArtifactSections {
    ...
    crate::optimization_unit::reconstruct_psi_optimization_unit_seed(
        &plan,
...
fn dynamic_dispatch(
    unit: &mut crate::optimization_unit::PsiOptimizationUnit,
) -> &mut crate::abstract_operations::AbstractReboundDynamicDispatch {
    unit.functions
        .iter_mut()
