...
use crate::abstract_operations::{
    AbstractFunctionResult, AbstractOperation as O, AbstractOperationPlan,
};
...
use crate::optimization_unit_semantics::tests::fixtures::primitive_locals::{
    primitive_local_call_plan, primitive_local_plan,
};
...
fn local_borrow_call_preserves_read_observations_and_rejects_owned_escape() {
    ...
    assert!(matches!(
        ...
        Err(crate::optimization_unit_semantics::OptimizationUnitValidationError::StructuralPlaceNotAvailable { .. })
    ));
...
fn local_reentry_preserves_storage_operations_and_requires_explicit_cycle_admission() {
    ...
    *function.operations.last_mut().unwrap() = O::Jump {
    ...
    assert_eq!(
        crate::optimization_unit_semantics::validate_psi_optimization_unit_with_admitted_cycle_machines(&unit, &[unit.entry]),
        Ok(())
    );
}
...
fn activation_local_cannot_escape_as_structural_return() {
    ...
    *function.operations.last_mut().unwrap() = O::ReturnStructural {
    ...
    assert!(matches!(
        ...
        Err(crate::optimization_unit_semantics::OptimizationUnitValidationError::StructuralReturnSourceContractMismatch { .. })
    ));
}
