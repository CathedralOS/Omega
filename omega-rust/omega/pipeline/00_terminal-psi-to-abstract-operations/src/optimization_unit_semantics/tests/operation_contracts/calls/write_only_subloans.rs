...
fn indexed_write_only_paths_cannot_fall_back_to_linear_multiplicity() {
    ...
    let rejects_linear = |mut unit: PsiOptimizationUnit| {
        ...
        let types = unit
            ...
            .collect();
        assert!(
            !crate::optimization_unit_semantics::unit_validation::operation_contracts::structural_arguments_match(
                &unit.functions[0],
                &arguments,
                &unit.functions[1].structural_parameters,
                &types,
                crate::optimization_unit_semantics::unit_validation::operation_contracts::StructuralProjectionPolicy::Unit,
                false,
            )
        );
        refresh_node_derivatives(&mut unit, 0, 0, 0);
