...
use crate::optimization_unit_semantics::tests::fixtures::structural_catalog::boolean_structural_field_unit;
    ...
    *psi_operation = id(4_863, OperationId::new);
    ...
    let mutate_residual =
        |unit: &mut PsiOptimizationUnit,
         mutate: &dyn Fn(&mut terminal_psi::StructuralAffineDiscard)| {
            ...
            let [terminal_psi::TerminalAffineCleanupAction::DiscardResidual(residual)] =
    ...
    mutate_residual(&mut wrong_path, &|residual| {
        residual.path = vec![terminal_psi::StructuralPathSegment::Field("right".into())];
    });
    assert!(matches!(
