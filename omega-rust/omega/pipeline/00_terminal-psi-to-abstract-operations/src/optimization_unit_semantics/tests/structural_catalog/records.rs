...
use semantic_vocabulary::{
    IntegerValue, OperationId, PlaceId, ScalarType, StructuralPlaceKind, StructuralTypeId, ValueId,
};
use terminal_psi::{
...
fn bounded_record_fields_require_exact_current_value_and_interval_authority() {
    ...
    let facts = facts
        ...
        .map(|(operation, obligation)| {
            crate::optimization_unit::AcceptedObligationFact::new(
                original.psi,
                [42; 32],
        ...
        .collect();
    let original =
        crate::optimization_unit::attach_accepted_obligation_facts(original, facts).unwrap();
    assert_eq!(validate_psi_optimization_unit(&original), Ok(()));
    for mutation in 0..4 {
        let mut changed = original.clone();
        ...
        *fact = crate::optimization_unit::AcceptedObligationFact::new(
            fact.psi,
