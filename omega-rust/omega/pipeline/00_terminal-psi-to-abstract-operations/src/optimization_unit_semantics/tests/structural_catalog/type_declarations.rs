...
fn structural_field_namespaces_require_canonical_ids_and_unique_nonempty_identities() {
    ...
    let candidate = structural_catalog_unit(vec![structural_type(
        440,
        terminal_psi::StructuralTypeShape::Sum { cases: vec![case] },
    )]);
    assert_eq!(
        validate_psi_optimization_unit(&candidate),
        Err(
...
fn structural_cases_require_canonical_unique_nonempty_declarations() {
    ...
    for shape in [
        terminal_psi::StructuralTypeShape::Sum { cases: Vec::new() },
        terminal_psi::StructuralTypeShape::Mixed {
            fields: Vec::new(),
            cases: Vec::new(),
        },
    ...
    let candidate = structural_catalog_unit(vec![structural_type(
        441,
        terminal_psi::StructuralTypeShape::Sum { cases: vec![empty] },
    )]);
    assert_eq!(
        validate_psi_optimization_unit(&candidate),
        Err(
...
fn structural_field_erasure_matrix_matches_canonical_terminal_admission() {
    ...
    let invalid = vec![
        ...
        terminal_psi::StructuralFieldType::Structural(owner),
    ];
    for field_type in invalid {
        let field = structural_leaf_field(1, terminal_psi::BindingRelevance::Erased, field_type);
...
fn relevant_erased_field_requires_an_exact_record_attachment() {
    ...
    let provider_place = |attachment, provider_field| terminal_psi::StructuralPlaceDeclaration {
    ...
    for (attachment, provider_field_id) in [
        ...
        (
            Some(owner),
            Some(id(2, semantic_vocabulary::StructuralFieldId::new)),
        ),
