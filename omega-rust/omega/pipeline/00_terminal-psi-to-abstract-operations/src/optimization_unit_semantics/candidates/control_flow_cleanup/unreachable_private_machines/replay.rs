...
pub(super) fn validate(
    ...
    let crate::optimization_unit::PsiRewriteDecisionPoint::MachineSet(decision_machines) =
    ...
    let expected_custody = expected_machines
        .iter()
        .map(|machine| crate::optimization_unit::PrunedMachineCustody {
            machine: *machine,
        ...
        match operation {
            ...
            | O::CallUnitWithDynamicArguments {
                ...
                for argument in dynamic_arguments {
                    if let crate::abstract_operations::AbstractDynamicDescriptorSource::Selection {
                        application,
                        ..
                    }
...
pub(crate) fn pruned_machine_provenance(
    unit: &PsiOptimizationUnit,
    machines: &[MachineId],
) -> Option<Vec<crate::optimization_unit::ProvenanceRewrite>> {
        ...
        for block in &function.blocks {
            for (node_index, node) in block.nodes.iter().enumerate() {
                ...
                if !node.provenance.is_empty() {
                    rows.push(crate::optimization_unit::ProvenanceRewrite {
                        input,
...
#[cfg(test)]
mod tests {
    use crate::abstract_operations::AbstractResult;
    ...
    fn machine_references_include_stored_dynamic_dispatch_targets() {
        let mut unit = crate::optimization_unit_semantics::tests::fixtures::scalar_units::unit();
