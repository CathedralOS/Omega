...
use crate::optimization_unit_semantics::candidates::sparse_conditional_constant_propagation::literal_boolean_fact;
use crate::optimization_unit_semantics::unit_validation::derived_metadata::reconstruct_declared_places;
...
pub(crate) fn reconstruct_conditional_fold_accounting(
    ...
    let mut realized = vec![crate::optimization_unit::ProvenanceRewrite {
        input: selected_site,
        disposition: ProvenanceDisposition::RealizedAt(selected_site),
        sources: selected.provenance.clone(),
        fuel: selected.fuel.clone(),
    }];
    let mut unreachable = vec![crate::optimization_unit::ProvenanceRewrite {
        input: rejected_site,
        ...
        if removed.contains(&block.id) {
            for (node_index, node) in block.nodes.iter().enumerate() {
                ...
                if !node.provenance.is_empty() {
                    ...
                    unreachable.push(crate::optimization_unit::ProvenanceRewrite {
                        input: site,
