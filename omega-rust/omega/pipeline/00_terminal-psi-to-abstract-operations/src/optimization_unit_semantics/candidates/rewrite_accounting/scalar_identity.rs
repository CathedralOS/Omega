...
use crate::optimization_unit_semantics::candidates::rewrite_accounting::dead_scalar::reconstruct_dead_scalar_node_accounting;
...
pub(crate) fn reconstruct_proof_certified_scalar_identity_accounting(
    ...
    Vec<BlockId>,
    Vec<crate::optimization_unit::ProvenanceRewrite>,
)> {
...
fn reconstruct_scalar_identity_accounting(
    ...
    for use_block in &function.blocks {
        ...
        for (index, node) in use_block.nodes.iter().enumerate() {
            ...
            provenance.push(crate::optimization_unit::ProvenanceRewrite {
                input: site,
