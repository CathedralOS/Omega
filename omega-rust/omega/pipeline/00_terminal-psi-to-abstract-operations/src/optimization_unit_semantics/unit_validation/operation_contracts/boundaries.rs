...
use crate::optimization_unit_semantics::unit_validation::operation_contracts::claim_transfers::function_claim_input;
            ...
            domains.contains_key(&requirement.domain)
                && arguments
                    ...
                    .is_some_and(|argument| {
                        structural_source_contract(caller, argument.place, true).is_some_and(
                            |source| {
                                source.carries_qualification(&argument.path, requirement.domain)
                            },
                        )
                    })
        })
...
pub(crate) fn boundary_completion_matches(
    caller: &PsiOptimizationFunction,
    arguments: &[terminal_psi::StructuralArgument],
    sources: &[crate::abstract_operations::CompletionClaimSource],
    ...
    let mut expected_sources = caller
        ...
        .iter()
        .cloned()
        .map(|entry| crate::abstract_operations::CompletionClaimSource {
            claim: entry.claim,
            entry: Some(entry),
            content: None,
        })
    ...
    for content in &caller.content_entry_claims {
            ...
            .find(|source| source.claim == content.claim)
        {
            ...
            expected_sources.push(crate::abstract_operations::CompletionClaimSource {
                claim: content.claim,
                entry: None,
