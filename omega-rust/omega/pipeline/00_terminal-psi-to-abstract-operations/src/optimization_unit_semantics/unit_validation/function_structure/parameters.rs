...
use crate::optimization_unit::{PsiOptimizationFunction, ValueDefinitionSite};
...
pub(super) fn validate_dynamic_descriptor_parameters(
    ...
    for node in &entry.nodes {
        match &node.operation {
            crate::abstract_operations::AbstractOperation::DynamicDescriptorParameter {
                parameter,
    ...
    for node in function.blocks.iter().flat_map(|block| &block.nodes) {
        if let crate::abstract_operations::AbstractOperation::CallDynamicParameterScalar {
            dynamic_dispatch,
            ..
        } = &node.operation
        {
