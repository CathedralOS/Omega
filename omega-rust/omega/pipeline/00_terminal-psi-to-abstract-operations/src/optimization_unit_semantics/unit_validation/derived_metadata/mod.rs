        ...
        for node in &block.nodes {
            for event in &node.ownership {
                let claims: &[ClaimId] = match event {
                    crate::optimization_unit::OwnershipEvent::ClaimTransfer(claims)
                    | crate::optimization_unit::OwnershipEvent::ClaimCompletion(claims)
                    | crate::optimization_unit::OwnershipEvent::StructuralReturn(claims)
                    | crate::optimization_unit::OwnershipEvent::CrashFrontier(claims) => claims,
                    crate::optimization_unit::OwnershipEvent::Cleanup(_) => continue,
                };
                for claim in claims {
