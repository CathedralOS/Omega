...
use target_operations_to_selected_instructions::selected_instructions::effects::program::encoding as effect_codec;
use target_operations_to_selected_instructions::{SelectedInstructionId, VirtualRegisterId};
...
pub(super) fn decode_instruction(
    ...
    let address = match byte(cursor)? {
        14 => Some(crate::PhysicalAddressOperation::SaveFloatingControl {
            slot: effect_codec::decode_local_storage_slot(cursor).map_err(map_field_error)?,
        }),
        15 => Some(crate::PhysicalAddressOperation::RestoreFloatingControl {
            slot: effect_codec::decode_local_storage_slot(cursor).map_err(map_field_error)?,
        }),
        ...
        tag @ (12 | 13) => {
            ...
            let byte_offset = u32_field(cursor)?;
            let width =
                target_operations_to_selected_instructions::PackedByteWidth::from_byte_size(byte(
                    cursor,
                )?)
        ...
        tag @ (2 | 3) => {
            let slot = match byte(cursor)? {
                2 => target_operations_to_selected_instructions::FrameStorageSlotId::Incoming {
                    parameter_index: u32_field(cursor)?,
                    abi_stack_byte_offset: u32_field(cursor)?,
                },
                slot_tag @ (0 | 3) => {
                    target_operations_to_selected_instructions::FrameStorageSlotId::Outgoing(
                        target_operations_to_selected_instructions::OutgoingArgumentSlotId {
                            role: if slot_tag == 0 {
                                target_operations_to_selected_instructions::OutgoingArgumentSlotRole::Argument
                            } else {
                                target_operations_to_selected_instructions::OutgoingArgumentSlotRole::ValueCopy
                            },
                ...
                1 => target_operations_to_selected_instructions::FrameStorageSlotId::Local(
