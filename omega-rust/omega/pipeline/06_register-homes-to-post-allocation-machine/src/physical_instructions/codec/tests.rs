...
fn outgoing_copy_and_pointer_slots_round_trip_with_independent_role_identity() {
    use target_operations_to_selected_instructions::{
        FrameStorageSlotId, OutgoingArgumentSlotRole,
    };
...
fn floating_control_codec_binds_direction_and_private_frame_slot() {
    for restore in [false, true] {
        let mut source = plan();
        let slot = target_operations_to_selected_instructions::LocalStorageSlotId::Boundary {
            operation: OperationId::new(137).unwrap(),
        };
        ...
        row.alternative.key.family = if restore {
            target_operations_to_selected_instructions::MachineAlternativeFamily::RestoreFloatingControl
        } else {
            target_operations_to_selected_instructions::MachineAlternativeFamily::SaveFloatingControl
        };
        row.alternative.encoded.memory = if restore {
            target_operations_to_selected_instructions::MachineEncodedMemoryEffect::ReadFrameStorageV1 {
                stack_pointer: target_operations_to_selected_instructions::register_model::RegisterViewId(7),
                byte_count: 8,
            }
        } else {
            target_operations_to_selected_instructions::MachineEncodedMemoryEffect::WriteFrameStorageV1 {
                stack_pointer: target_operations_to_selected_instructions::register_model::RegisterViewId(7),
                byte_count: 8,
            }
        };
...
fn boundary_scratch_codec_binds_tag_operation_geometry_and_address() {
    ...
    for mutation in 0..4 {
        ...
        match mutation {
            ...
            _ => changed.functions[0].blocks[0].instructions[0].address =
...
fn read_byte_codec_binds_structural_home_operation_geometry_and_address() {
    let mut source = plan();
    let slot = target_operations_to_selected_instructions::LocalStorageSlotId::Structural {
        place: semantic_vocabulary::PlaceId::new(139).unwrap(),
        operation: OperationId::new(137).unwrap(),
    };
    source.functions[0].local_storage_slots.push(
...
fn compiler_spill_slot_codec_retains_function_local_identity() {
    let mut source = plan();
    let slot = target_operations_to_selected_instructions::LocalStorageSlotId::Spill {
