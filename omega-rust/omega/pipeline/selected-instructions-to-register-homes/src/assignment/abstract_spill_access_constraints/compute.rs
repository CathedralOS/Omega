//! Direct abstract access placement and dependency construction.
...
pub(super) fn compute(
    ...
    if policy != AbstractSpillAccessConstraintPolicy::BlockLocalDataBarrierAndOverlapV1 {
    ...
    for (function, row) in source.plan().functions.iter().enumerate() {
        let placements = accesses::project(function, row)?;
        let dependencies = dependencies::derive(function, row, &placements)?;
