        ...
        for fragment in &register.fragments {
            ...
            for segment in &fragment.segments {
                ...
                let domain = match segment.opening {
                    FixedPrecoloredSourceSegmentOpening::SourceRangeStartV1 => create(
                        function,
            ...
            let domain = current.ok_or(FixedPrecoloredSegmentHomeError::SegmentMismatch {
                function,
                register: register.virtual_register.0,
                segment: 0,
            })?;
            closing_by_block.insert(fragment.block, domain);
        }
    }
    Ok(domains)
}
