//! Canonical identity for target-neutral spill-pseudo lowering.

use sha2::{Digest, Sha256};

use crate::{
    LogicalSpillStorageClass, SpillPseudoInstruction, SpillPseudoInstructionPlan,
    SpillPseudoInstructionPlanIdentity, SpillPseudoInstructionPolicy, SpillPseudoStoredValue,
};

pub fn spill_pseudo_instruction_plan_identity(
    plan: &SpillPseudoInstructionPlan,
) -> SpillPseudoInstructionPlanIdentity {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(b"omega.spill-pseudo-instructions.v1\0");
    ...
    bytes.push(match plan.policy {
        SpillPseudoInstructionPolicy::RecursiveLogicalScheduleV1 => 0,
    });
    ...
    SpillPseudoInstructionPlanIdentity(Sha256::digest(bytes).into())
}
...
fn length(bytes: &mut Vec<u8>, value: usize) {
    bytes.extend_from_slice(
        &u64::try_from(value)
            .expect("spill-pseudo length fits u64")
            .to_le_bytes(),
    );
}
