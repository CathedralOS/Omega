...
fn ordinary_macho_image() -> (
    ...
    let object = resolved_layout_to_resolved_layout::image_emission::build_object_artifact(&plan)
        .expect("ordinary Mach-O object");
    let original =
        resolved_layout_to_resolved_layout::image_emission::emit_direct_executable_image(
            &object, 3,
        )
...
fn relocated_macho_bss_is_zero_filled_and_writable() {
    ...
    image
        .data_regions
        .push(resolved_layout_to_resolved_layout::image::FinalDataRegion {
            origin: resolved_layout_to_resolved_layout::image::FinalDataRegionOrigin::CompilerData,
            section_offset: 0,
