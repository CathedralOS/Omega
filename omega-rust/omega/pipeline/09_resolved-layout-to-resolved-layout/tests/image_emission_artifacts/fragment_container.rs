...
pub(super) struct CompiledFragmentEntry {
    pub container: Arc<resolved_layout_to_resolved_layout::object_file::StagedOptimizedRelocationFreeObjectContainer>,
    pub artifact: resolved_layout_to_resolved_layout::image_emission::ObjectArtifact,
    pub signature: resolved_layout_to_resolved_layout::program_entry_plan::SelectedProgramEntrySourceSignature,
}

/// The source-to-checked ladder: one source-free resolution, typed lowering,
/// then settled checking.
fn checked_source(source: &str) -> typed_trees_to_checked_trees::checked_trees::CheckedTrees {
...
fn entry_signature(
    ...
    let receiver = checked
        ...
        .map_or(
            resolved_layout_to_resolved_layout::program_entry_plan::ProgramEntrySourceReceiverSignature::Free,
            |parameter| {
                resolved_layout_to_resolved_layout::program_entry_plan::ProgramEntrySourceReceiverSignature::ProvisionedMutable {
    ...
    resolved_layout_to_resolved_layout::program_entry_plan::SelectedProgramEntrySourceSignature::from_checked_typed_entry(
...
> {
    ...
    let optimized = omega::compiler::native::optimize_artifact_sections(
        &semantic,
        &proof,
        &proof_admission::AdmissionProfile::default(),
        omega::compiler::native::compiler_baseline_request_v1(&selections),
    )
    ...
    let physical = omega::compiler::native::stage_optimized_verified_physical_pipeline(
        optimized_target,
        post_terminal.selections(),
    )
    .expect("entry machine reaches physical realization");
    let fragments = resolved_layout_to_resolved_layout::machine_emission::stage_optimized_function_fragment_emission(
        physical.into_function_fragment_emission_source(),
    )
    .expect("function fragments");
    let framed = resolved_layout_to_resolved_layout::machine_emission::stage_function_fragment_frame_application(fragments)
        .expect("frame application");
    let placed = resolved_layout_to_resolved_layout::machine_emission::stage_optimized_fixed_frame_text_section(framed)
        .expect("placed text section");
    Arc::new(
        resolved_layout_to_resolved_layout::object_file::stage_optimized_relocation_free_object_container(placed)
...
pub(super) fn compile_attached_entry(
    ...
    let artifact = resolved_layout_to_resolved_layout::image_emission::build_function_fragment_object_artifact(Arc::clone(&container))
        .expect("fragment object artifact");
    resolved_layout_to_resolved_layout::image_emission::validate_function_fragment_object_artifact(
        &container, &artifact,
    )
