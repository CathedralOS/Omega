...
fn windows_object() -> resolved_layout_to_resolved_layout::image_emission::ObjectArtifact {
    let mut plan = two_function_plan();
    plan.target = NativeTarget::windows_x64();
    build_object_artifact(&plan).expect("COFF object")
}

fn macos_object() -> resolved_layout_to_resolved_layout::image_emission::ObjectArtifact {
    let mut plan = two_function_plan();
