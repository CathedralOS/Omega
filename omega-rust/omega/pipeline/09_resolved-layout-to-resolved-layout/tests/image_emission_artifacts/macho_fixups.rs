...
use resolved_layout_to_resolved_layout::object_file::{RelocationKind, SymbolKind};
...
fn fixup_image(with_import: bool) -> ExecutableImageOutput {
    ...
    resolved_layout_to_resolved_layout::image_macho::emit_macho_aarch64_executable(image)
        .expect("emit pointer and storage fixture")
}
...
fn validate_mapping(output: &ExecutableImageOutput) -> Result<(), diagnostics::Diagnostic> {
    resolved_layout_to_resolved_layout::image_macho::validate_macho_aarch64_loader_mapping(
        &output.bytes,
...
fn validate_fixups(
    ...
    let rebases = [
        resolved_layout_to_resolved_layout::image_macho::MachoRebasePointer {
            data_offset: 0,
    ...
    let imports = [
        resolved_layout_to_resolved_layout::image_macho::MachoImportPointer {
            data_offset: 16,
            install_name: b"/usr/lib/libSystem.B.dylib",
            symbol: b"_write",
        },
    ];
    resolved_layout_to_resolved_layout::image_macho::validate_macho_aarch64_loader_fixups(
        &output.bytes,
        &output.final_data_bytes,
        &rebases,
...
fn macho_fixup_object_projection_retains_addends_and_normalized_import_targets() {
    ...
    for addend in [-3, 3] {
        relocations
            ...
            .addend = addend;
        let image = resolved_layout_to_resolved_layout::image::build_final_image(
            resolved_layout_to_resolved_layout::image::FinalImageInput {
                target,
                object: &object,
        ...
        let output = resolved_layout_to_resolved_layout::image::emitted_direct_executable_output(
            resolved_layout_to_resolved_layout::image_macho::emit_macho_aarch64_executable(image)
                .expect("emit object fixup fixture"),
        );
        let validate_object = |candidate: &ObjectPlan, candidate_relocations: &RelocationPlan| {
            resolved_layout_to_resolved_layout::image_macho::validate_macho_aarch64_object_fixups(
                candidate,
...
fn validate_complete_placement(
    ...
    resolved_layout_to_resolved_layout::image::validate_placed_executable_region_inventory(
        &output.executable_regions,
        &output.final_text_bytes,
    )?;
    resolved_layout_to_resolved_layout::image::validate_placed_data_region_inventory(
        &output.data_regions,
        &output.final_data_bytes,
    )?;
    ...
    resolved_layout_to_resolved_layout::image_macho::validate_macho_aarch64_import_binding_pairing(
        &output.final_text_bytes,
        &output.executable_regions,
        &output.data_regions,
    )
