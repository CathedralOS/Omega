...
use omega::component_publication::{
    InstalledArtifactMemoryImages, bind_installed_artifact,
...
use resolved_layout_to_resolved_layout::image_emission::{
    ExecutableImage, InstallationError, ObjectArtifact, build_installation_record,
...
use resolved_layout_to_resolved_layout::object_file::{
    ObjectSymbolHandle, RelocationKind, SectionKind,
};
...
fn install_flattened_image(
    ...
    code: Vec<u8>,
    entries: Vec<omega::executable_installation::ArtifactEntry>,
    relocations: Vec<omega::executable_installation::DecodedArtifactRelocation>,
    placement_base: u64,
    resolve: impl FnMut(terminal_psi::layout_plans::RelocationTarget) -> Option<u64>,
) -> omega::executable_installation::InstalledCode {
    let scope =
        terminal_psi::layout_plans::ArtifactInstallationScopeId::from_normalized_identity(0x7101)
    ...
    let contracts =
        omega::executable_installation::MachineContractSetId::from_normalized_identity(0x7102)
            .expect("contract set");
    let footprint =
        omega::executable_installation::MachineFootprintId::from_normalized_identity(0x7103)
            .expect("footprint");
    let artifact = omega::executable_installation::Artifact::from_canonical_decode(
        omega::executable_installation::ArtifactId::from_normalized_identity(0x7104)
            .expect("artifact identity"),
        architecture,
        code,
        contracts,
        footprint,
        omega::executable_installation::PlacementPlanId::from_normalized_identity(0x7105)
            .expect("placement plan"),
        constraints,
        omega::executable_installation::EntrySetId::from_normalized_identity(0x7106)
            .expect("entry set"),
        entries,
        omega::executable_installation::RelocationSetId::from_normalized_identity(0x7107)
            .expect("relocation set"),
        relocations,
        omega::executable_installation::ArtifactAuthorityCommitments::from_canonical_evidence(
            contracts,
    ...
    let admitted = omega::executable_installation::admit_executable(
        &artifact,
        omega::executable_installation::ArtifactAdmissionEvidence::from_validator(
            omega::executable_installation::AdmissionReceiptId::from_normalized_identity(0x7108)
    ...
    let rights = terminal_psi::extents::ExtentRights::from_normalized_identities([
        terminal_psi::extents::ExtentRightId::from_normalized_identity(0x7109)
            .expect("extent right"),
    ]);
    let extent = terminal_psi::extents::ExtentRootGrant::from_admitted_provider(
        ...
        terminal_psi::extents::ExtentLineageId::from_normalized_identity(0x7117).expect("lineage"),
        terminal_psi::extents::AddressSpaceId::from_normalized_identity(0x7118)
            .expect("address space"),
        rights.clone(),
        terminal_psi::extents::ExtentProvenanceId::from_normalized_identity(0x7119)
            .expect("provenance"),
        terminal_psi::extents::MappingEraId::from_normalized_identity(0x711a).expect("era"),
    )
    .mint(placement_base, extent_len)
    .expect("placement extent");
    let placement = omega::executable_installation::CodePlacementAuthority::from_admitted_provider(
        omega::executable_installation::CodePlacementId::from_normalized_identity(0x711b)
            .expect("placement"),
        // The authority's scope identity must equal the artifact installation
        // scope the placement site carries.
        omega::executable_installation::InstallationScopeId::from_normalized_identity(0x7101)
            .expect("installation scope"),
        omega::executable_installation::InstallationAudience::DormantLocal,
        &extent,
        rights,
        constraints,
        terminal_psi::layout_plans::PlacementSite {
            base_address: placement_base,
            phase: terminal_psi::layout_plans::PlacementPhase::Load,
            machine_regime: None,
    ...
    let materialized = omega::executable_installation::materialize_admitted_artifact(
        &admitted, &placement, resolve,
    )
    .expect("materialized artifact");
    let frozen = omega::executable_installation::materialize_and_freeze(
        &admitted,
        placement,
        materialized.clone(),
        omega::executable_installation::MaterializationReceipt::from_materialized(
            &materialized,
            omega::executable_installation::MachineFootprintId::from_normalized_identity(0x711d)
                .expect("materialized footprint"),
            true,
        ),
    )
    .expect("frozen artifact");
    let validation = omega::executable_installation::FinalValidationCertificate::from_validator(
        omega::executable_installation::FinalValidationId::from_normalized_identity(0x711e)
            .expect("validation"),
        &frozen,
        true,
    );
    let validated = omega::executable_installation::validate_final_placement(frozen, &validation)
        .expect("validated artifact");
    let authority =
        omega::executable_installation::InstallAuthority::from_admitted_provider(&validated);
    let receipt = omega::executable_installation::InstallationReceipt::from_provider(
        omega::executable_installation::InstalledCodeId::from_normalized_identity(0x711f)
            .expect("installed code"),
        &validated,
        true,
        omega::executable_installation::WxEnforcement::HardwareEnforced,
    );
    omega::executable_installation::install_validated(validated, authority, receipt)
        .expect("installed code")
}
...
fn install_parts(
    ...
    u64,
    Vec<omega::executable_installation::ArtifactEntry>,
    Vec<omega::executable_installation::DecodedArtifactRelocation>,
    Vec<(terminal_psi::layout_plans::RelocationTarget, u64)>,
    Vec<(ObjectSymbolHandle, terminal_psi::layout_plans::EntryStubId)>,
) {
    let layout = image.output().final_image_layout;
    let base = layout.text_address;
    let mut entries = Vec::new();
    let mut entry_stubs = Vec::new();
    let mut symbol_targets: Vec<(
        ObjectSymbolHandle,
        (terminal_psi::layout_plans::RelocationTarget, u64),
    )> = Vec::new();
    let mut next_identity = 0x7400_u64;
    let mut next_text = |symbol: ObjectSymbolHandle, text_offset: usize| {
        let stub = terminal_psi::layout_plans::EntryStubId::from_normalized_identity(next_identity)
            .expect("entry stub identity");
        next_identity += 1;
        (
            symbol,
            (
                terminal_psi::layout_plans::RelocationTarget::Entry(stub),
    ...
    let mut admit_text_entry = |symbol: ObjectSymbolHandle, text_offset: usize| {
        ...
        entries.push(
            omega::executable_installation::ArtifactEntry::from_canonical_decode(
                stub,
    ...
    for table in object.dynamic_conformance_tables() {
        let data =
            terminal_psi::layout_plans::DataSymbolId::from_normalized_identity(next_identity)
                .expect("data symbol identity");
        next_identity += 1;
        symbol_targets.push((
            table.symbol,
            (
                terminal_psi::layout_plans::RelocationTarget::Data(data),
    ...
    let relocations = object
        ...
        .map(|(_, relocation)| {
            ...
            let kind = match relocation.kind {
                RelocationKind::Absolute64 => {
                    omega::executable_installation::ArtifactRelocationKind::Absolute64
                }
                RelocationKind::X86_64Relative32 => {
                    omega::executable_installation::ArtifactRelocationKind::X86Relative32
                }
                RelocationKind::Aarch64Page21 => {
                    omega::executable_installation::ArtifactRelocationKind::Aarch64Page21
                }
                RelocationKind::Aarch64PageOffset12 => {
                    omega::executable_installation::ArtifactRelocationKind::Aarch64PageOffset12
                }
                RelocationKind::Aarch64Branch26 => {
                    omega::executable_installation::ArtifactRelocationKind::Aarch64Branch26
                }
            };
            omega::executable_installation::DecodedArtifactRelocation {
                kind,
                destination_offset,
                target: target.0,
        ...
        .collect();
    let resolution: Vec<(terminal_psi::layout_plans::RelocationTarget, u64)> = symbol_targets
        .into_iter()
        .map(|(_, target)| target)
        .collect();
...
fn entry_stub(
    entry_stubs: &[(ObjectSymbolHandle, terminal_psi::layout_plans::EntryStubId)],
    symbol: ObjectSymbolHandle,
) -> Option<terminal_psi::layout_plans::EntryStubId> {
    entry_stubs
        .iter()
...
fn resolver(
    resolution: Vec<(terminal_psi::layout_plans::RelocationTarget, u64)>,
) -> impl FnMut(terminal_psi::layout_plans::RelocationTarget) -> Option<u64> {
    move |target| {
        resolution
            .iter()
...
fn installed_artifact_join_requires_complete_placed_custody() {
    ...
    *claimed.boundary_opaque_applications_mut_for_test() =
        resolved_layout_to_resolved_layout::boundary_applications::BoundaryOpaqueRepresentationApplications::new(vec![
            resolved_layout_to_resolved_layout::boundary_applications::BoundaryOpaqueRepresentationApplication {
...
fn claimed_custody() -> &'static resolved_layout_to_resolved_layout::boundary_applications::BoundaryOpaqueRepresentationApplications{
...
fn installed_private_function_entry_binds_exact_row_and_occurrence() {
    ...
    let unadmitted = terminal_psi::layout_plans::EntryStubId::from_normalized_identity(0x7fff)
