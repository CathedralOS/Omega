...
fn privileged_effect_and_exact_provider_execution_survive_installation() {
    ...
    let mut bytes = resolved_layout_to_resolved_layout::image_emission::x86_encoding::encode_immediate_port_write(0x20, 0x20).to_vec();
    bytes.push(0xc3);
    let plan = MachineCodePlan {
        ...
        functions: vec![MachineCodeFunction {
            ...
            boundary_settlements: vec![BoundarySettlementRecord {
                ...
                boundary,
                execution: post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryExecutionRecord::AdmittedProvider(
