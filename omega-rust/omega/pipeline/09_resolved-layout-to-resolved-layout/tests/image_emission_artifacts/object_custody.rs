...
use resolved_layout_to_resolved_layout::object_file::{
    SectionKind, SymbolKind, object_symbol_name,
};
...
fn admitted_x86_fma_provider_selects_generic_slot_and_reaches_an_exact_image() {
    for (profile, format, slot) in [
        ...
        resolved_layout_to_resolved_layout::image_emission::validate_direct_executable_image(
            &artifact, &image,
        )
...
fn source_free_x86_fma_object_rejects_stripped_and_mutated_custody() {
    ...
    changed.functions[0].x86_scalar_fma[0].destination =
        abstract_operations_to_target_operations::calling_conventions::MachineRegister::X86Xmm(3);
    refresh_x86_fma_identity(&mut changed.functions[0].x86_scalar_fma[0]);
    candidates.push(changed);
    let mut changed = baseline.clone();
    changed.functions[0].x86_scalar_fma[0].addend =
        abstract_operations_to_target_operations::calling_conventions::MachineRegister::X86Xmm(4);
    refresh_x86_fma_identity(&mut changed.functions[0].x86_scalar_fma[0]);
    candidates.push(changed);
    let mut changed = baseline.clone();
    changed.functions[0].x86_scalar_fma[0].multiplicand =
        abstract_operations_to_target_operations::calling_conventions::MachineRegister::X86Xmm(5);
    refresh_x86_fma_identity(&mut changed.functions[0].x86_scalar_fma[0]);
    candidates.push(changed);
    let mut changed = baseline.clone();
    ...
    for (target, destination) in [
        ...
        for value in [0, 1, 37, 255, 256, -1, i32::MIN, i32::MAX] {
            ...
            let bytes = match target.architecture {
                target::Architecture::X86_64 => target_operations_to_selected_instructions::isa_x86_64::encode_hosted_exit_process_i32(value),
                target::Architecture::Aarch64 => {
                    target_operations_to_selected_instructions::isa_aarch64::encode_hosted_exit_process_i32(target, value).unwrap()
                }
            };
...
fn installation_boundary_settlement_rejects_every_one_field_substitution() {
    ...
    let fabricated_runtime_argument = post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarArgumentRecord {
        parameter_index: 0,
    ...
    let scalar_result =
        post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryResultRecord::Scalar(post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryScalarResultRecord {
    ...
    type ReplayMutation = (
        ...
        Box<
            dyn Fn(
                &mut resolved_layout_to_resolved_layout::image_emission::ObjectBoundarySettlement,
            ),
        >,
    );
    ...
    let rejected: Vec<RejectedMutation> = vec![
        ...
        (
            ...
            0,
            Box::new(|row| {
                row.settlement.execution = post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryExecutionRecord::CompilerBuiltin(
                    abstract_operations_to_target_operations::target_operations::CompilerBuiltinExecution::HostedExitProcessI32,
                );
            }),
            provider_custody(98),
        ...
        (
            ...
            Box::new(|row| {
                ...
                row.settlement.completion_provider_custody.push(
                    post_allocation_machine_to_selected_form_encoding::machine_code::CompletionProviderCustodyBinding {
                        ...
                        provider_execution: post_allocation_machine_to_selected_form_encoding::machine_code::ProviderExecutionRecord::new(
                            11, 12, 13, 14, 15,
                        )
                        .unwrap(),
                    },
        ...
        (
            ...
            1,
            Box::new(|row| {
                row.settlement.execution = post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryExecutionRecord::AdmittedProvider(
                    post_allocation_machine_to_selected_form_encoding::machine_code::ProviderExecutionRecord::new(970, 971, 972, 973, 974).unwrap(),
                );
            }),
        ...
        (
            ...
            1,
            Box::new(|row| {
                row.settlement.realization = abstract_operations_to_target_operations::target_operations::LinuxWriteLineRealization.into();
            }),
            provider_custody(100),
        ),
        (
        ...
        (
            ...
            1,
            Box::new(|row| {
                row.settlement.native_result = post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryResultRecord::Scalar(
                    post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryScalarResultRecord {
                        value: semantic_vocabulary::ValueId::new(99).unwrap(),
