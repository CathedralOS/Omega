...
use crate::image_pe::bytes::{write_i32_at, write_u16_at, write_u32_at, write_u64_at};
use crate::image_pe::constants::IMAGE_BASE;
use crate::image_pe::layout::align_to;
        ...
        let (library, lookup) = match import {
            FinalImageImportPlan::StringBackedBootstrap { library } => {
                let library = if library.is_empty() {
                    abstract_operations_to_target_operations::calling_conventions::windows_import_library(&symbol)
...
mod tests {
    ...
    fn installed_import_thunks_enter_the_executable_region_inventory() {
        ...
        let symbol = image.symbol_table.symbols.insert(FinalImageSymbol {
            ...
            kind: crate::object_file::SymbolKind::Import,
