...
use crate::image_macho::file_layout::layout::align_to;
...
use abstract_operations_to_target_operations::calling_conventions::{
    DARWIN_LIBOBJC_PATH, darwin_import_library,
};
...
pub(crate) fn validate_import_thunk_footprints(
    ...
    for thunk in thunks {
        ...
        let canonical = match isa {
            ...
            MachoIsa::X86_64 => {
                target_operations_to_selected_instructions::isa_x86_64::decode_x86_64_import_thunk(
                    bytes,
                )
                .is_some()
            }
...
fn thunk_bound_pointer_address(
    final_text_bytes: &[u8],
    region: &crate::image::PlacedExecutableRegion,
    isa: MachoIsa,
