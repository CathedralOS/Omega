...
use crate::image_macho::dyld_linking::loader_mapping::{region, wide, word};
...
fn validate_macho_object_fixups(
    ...
    for (handle, symbol) in referenced_imports() {
        ...
        let (install_name, bind_symbol) = if let Some(import) = locators.next() {
            ...
            (
                abstract_operations_to_target_operations::calling_conventions::darwin_import_library(&symbol.name).as_bytes(),
