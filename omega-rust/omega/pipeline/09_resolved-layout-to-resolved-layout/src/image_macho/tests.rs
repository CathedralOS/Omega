...
fn validate_mapping(
    output: &crate::image::ExecutableImageOutput,
...
fn x86_64_loader_mapping_accepts_eager_import_storage() {
    ...
    image
        .symbol_table
        .imports
        .insert(crate::image::FinalImageImport {
            symbol_handle: imported,
            import: crate::image::FinalImageImportPlan::StringBackedBootstrap {
                library: String::new(),
            },
        });
    image
    ...
    let thunk = output
        ...
        .regions
        .iter()
        .find(|region| region.origin == crate::image::FinalExecutableRegionOrigin::ImportThunk)
