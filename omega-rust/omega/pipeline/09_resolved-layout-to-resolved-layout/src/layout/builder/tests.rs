...
use crate::layout::{DataShape, FieldLayout, TypeLayout};
...
fn private_callback_layout(
    offset: u64,
    size: usize,
) -> symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::PlanLaidLayout {
    symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::PlanLaidLayout {
        ...
        validated_layout: terminal_psi::layout_plans::LayoutPlanReport {
            schema_report_fingerprint: 0x51,
            entries: vec![terminal_psi::layout_plans::LayoutFieldEntryReport {
                field: "payload".to_owned(),
                member_identity: None,
                placement: terminal_psi::layout_plans::LayoutPlacementReport::At { offset: 0 },
            }],
            offsets: Some(vec![0]),
            size: Some(size as u64),
            align: 8,
        },
        private_callback_demands: vec![terminal_psi::layout_plans::PrivateCallbackLayoutDemandReport {
            slot_application: symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::ClosedConformanceApplication {
                ...
                commitment:
                    symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::ClosedConformanceApplicationCommitment::from_digest(
                        [0x92; 32],
                    ),
            },
...
fn private_callback_fields() -> [FieldLayout; 1] {
    [FieldLayout {
        ...
        name: typed_trees_to_checked_trees::checked_trees::name::Identifier::from("payload"),
        offset: 0,
        layout: TypeLayout {
            size: 8,
            alignment: 8,
...
) -> Result<Vec<crate::layout::TargetClosedPrivateCallbackDemand>, diagnostics::Diagnostic> {
    let fingerprint = terminal_psi::layout_plans::normalized_native_layout_plan_report_fingerprint(
        &terminal_psi::layout_plans::NativeLayoutPlanReport {
            layout: plan.validated_layout.clone(),
            private_callback_demands: plan.private_callback_demands.clone(),
        },
    );
    let layout =
        abstract_operations_to_target_operations::calling_conventions::callback_layout_plan_id(
            fingerprint,
            target.pointer_size,
...
fn target_closes_private_callback_geometry_and_rejects_exact_mutations() {
    ...
    assert_eq!(
        closed.requirement,
        abstract_operations_to_target_operations::calling_conventions::callback_requirement_id(
    ...
    changed_application.private_callback_demands[0]
        .slot_application
        .commitment =
        symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::ClosedConformanceApplicationCommitment::from_digest([0x93; 32]);
    let changed =
...
fn quotient_over_proof_only_carrier_stays_proof_only() {
    ...
    let quotient_symbol = checked
        ...
        .symbol;
    let classification =
        typed_trees_to_checked_trees::checked_trees::proof_only::classify(&checked.typed);
    assert!(matches!(
        classification.reason(quotient_symbol),
        Some(
            typed_trees_to_checked_trees::checked_trees::proof_only::ProofOnlyReason::Quotient { .. }
        )
    ));
}
