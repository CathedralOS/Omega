...
use crate::layout::project_conventional_record_with_recursive_nested_sums_materialization_layout;
use omega::build_time_evaluation::ValidatedConstRecordSumChildMaterialization;
use omega::build_time_evaluation::validate_const_materializable_record_with_recursive_nested_sums;
use terminal_psi::layout_plans::ConventionalRecordSumChildHop;
use terminal_psi::layout_plans::ConventionalRecordSumChildInterior;
use terminal_psi::layout_plans::ConventionalRecordSumChildLayoutReport;
use terminal_psi::layout_plans::ConventionalRecursiveRecordSumPathsLayoutReport;
use terminal_psi::layout_plans::ConventionalSumLayoutReport;
...
fn recursive_empty_arrays_follow_inside_out_extent_arithmetic() {
    for element in ["Choice", "Cell"] {
        ...
        let plan = crate::layout::build_layout_plan(&program, NativeTarget::host(), &[]).unwrap();
        let outer = program
            .data_definitions()
            .iter()
        ...
        assert!(
            crate::layout::build_layout_plan(&overflowing, NativeTarget::host(), &[]).is_err(),
...
fn recursive_empty_array_rows_reject_substituted_element_geometry() {
    ...
    let plan = crate::layout::build_layout_plan(&checked, NativeTarget::host(), &[]).unwrap();
    let outer = checked
        .data_definitions()
        .iter()
        .find(|definition| definition.name.as_str() == "Outer")
    ...
    for field_index in 0..2 {
        for mutation in 0..5 {
            ...
            match mutation {
                ...
                4 => malformed
                    .repeated_fields
                    .push(crate::layout::RepeatedFieldLayout {
                        field: field.symbol,
                        element_stride: 16,
                    }),
                _ => unreachable!(),
        ...
        for owner_name in owners {
            ...
            for mutation in 0..6 {
                ...
                match mutation {
                    ...
                    1 => changed_plan.bit_fields.push(crate::layout::BitFieldLayout {
                        field: field_symbol,
                        fragments: Vec::new(),
                    }),
                    2 => changed_plan
                        .stored_integers
                        .push(crate::layout::StoredIntegerLayout {
                            field: field_symbol,
                            stored_width_bits: 8,
                            interpretation:
