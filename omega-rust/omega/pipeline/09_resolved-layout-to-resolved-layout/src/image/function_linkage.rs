...
use crate::object_file::{ObjectPlan, SectionKind, SymbolKind, SymbolSection};
use diagnostics::Diagnostic;

use crate::image::symbols::final_image_symbol_handle;
...
pub fn validate_final_image_function_linkage(
    ...
    for (binding_index, binding) in object.layout.function_symbols.iter() {
        let Some((object_symbol_handle, object_symbol)) =
            crate::object_file::object_function_symbol(object, binding.identity)
        else {
...
mod tests {
    ...
    use crate::image::{FinalImageInput, FinalImageSymbol, build_final_image};
    use crate::object_file::{FunctionSymbolPlan, SymbolPlan};
    ...
    fn linked_image() -> (FinalImage, ObjectPlan) {
        ...
        let symbol = object.layout.symbols.insert(SymbolPlan {
            name: crate::object_file::entry_symbol_name(target),
        ...
        let image = build_final_image(FinalImageInput {
            target,
            object: &object,
            relocations: &crate::object_file::RelocationPlan::with_target(target),
            text_bytes: &[0; 8],
            data_bytes: &[],
        });
        (image, object)
    ...
    fn final_image_retains_exact_identity_owned_function_symbols() {
        ...
        wrong_entry.symbol_table.entry_symbol = arena::Handle::invalid();
