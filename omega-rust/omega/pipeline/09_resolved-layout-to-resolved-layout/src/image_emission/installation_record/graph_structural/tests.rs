...
use abstract_operations_to_target_operations::calling_conventions::{MachineRegister, ValueShape};
...
fn direct_structural_roster_retains_exact_borrowed_home_subset() {
    ...
    for owned_bytes in [0, 2] {
        ...
        function.unit_parameters = shapes
            .iter()
            .enumerate()
            .map(|(position, shape)| post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterRecord {
        ...
        function
            ...
            .push(post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterHomeRecord {
                ...
                indirect: true,
                location: post_allocation_machine_to_selected_form_encoding::machine_code::StructuralSourceLocation::IncomingBorrowedPointer {
                    location: borrowed_structural::pointer_location(&plan.parameters[1]).unwrap(),
                },
            });
        function.parameter_abi = Some(post_allocation_machine_to_selected_form_encoding::machine_code::ParameterFunctionAbiRecord {
            call_plan: plan,
            parameters: Vec::new(),
...
fn indirect_owned_roster_retains_exact_value_pointer_home() {
    ...
    for target in [
        ...
        for scalar_count in [0, 8] {
            ...
            let parameter = post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterRecord {
            ...
            function.unit_parameter_homes = vec![post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterHomeRecord {
            ...
            function.parameter_abi = Some(post_allocation_machine_to_selected_form_encoding::machine_code::ParameterFunctionAbiRecord {
                parameters: (0..scalar_count)
                    .map(|position| abstract_operations_to_target_operations::target_operations::ScalarAbiValue {
...
fn direct_owned_roster_preserves_ieee_format_width_and_register_bank() {
    ...
    for target in [
        ...
        for (format, bytes) in [
            ...
            .unwrap();
            let mut function = installed_function_with_unit_call();
            function.unit_parameters = vec![post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterRecord {
                place: PlaceId::new(1).unwrap(),
                structural_type: StructuralTypeId::new(1).unwrap(),
                multiplicity: StructuralMultiplicity::Unrestricted,
                access: terminal_psi::StructuralAccess::Owned,
