...
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    InternalUnitCallSource, StructuralSourceLocation,
};
        ...
        InternalUnitCallSource::InstalledProvider {
            ...
            call.claim_transfers
                == completion_receipts
                    ...
                    .collect::<Vec<_>>()
                && crate::image_emission::object_artifact::replay::boundary::completion_receipts::completion_receipts_have_exact_custody(
                    &arguments,
                    completion_claim_sources,
                    completion_receipts,
                )
...
pub(super) fn call_attribution_is_exact(
    ...
    let mut rows = record.semantic_code_attribution.iter().filter(|row| {
        row.machine == function.machine
            && row.attribution.site == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Operation(operation)
    });
