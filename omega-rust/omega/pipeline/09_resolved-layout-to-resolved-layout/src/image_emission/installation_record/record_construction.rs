...
use crate::image_emission::ExecutableImage;
use crate::image_emission::installation_record::codec::fingerprint::fingerprint_image;
use crate::image_emission::installation_record::record_shape::validate_record_shape;
use crate::image_emission::installation_record::record_validation::{
    ...
    Execution: installation_evidence::ProviderExecutionEvidence + ?Sized + 'execution,
{
    ...
    build_installation_record_with_selected_provider_plans_and_evidence(
        ...
        provider_executions,
        component_progress,
        crate::boundary_applications::BoundaryOpaqueRepresentationApplications::EMPTY,
    )
}
...
pub fn build_installation_record_with_selected_provider_plans_and_evidence<'execution, Execution>(
    ...
    provider_executions: impl IntoIterator<Item = &'execution Execution>,
    component_progress: Option<&dyn installation_evidence::ComponentProgressAcceptanceEvidence>,
    boundary_opaque_applications: crate::boundary_applications::BoundaryOpaqueRepresentationApplications,
) -> Result<InstallationRecord, InstallationError>
where
    Execution: installation_evidence::ProviderExecutionEvidence + ?Sized + 'execution,
{
    ...
    let required_executions = image
        ...
        .filter_map(|installed| {
            let post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryExecutionRecord::AdmittedProvider(execution) =
...
pub fn derive_installation_stack_demand(
    ...
    image: &ExecutableImage,
    entry: MachineId,
) -> Result<crate::image_emission::StackDemand, InstallationStackError> {
    ...
    if !functions.contains_key(&entry) {
        return Err(crate::image_emission::ObjectError::EntryFunctionMissing(entry).into());
    }
    ...
    Ok(crate::image_emission::StackDemand {
        psi: record.psi,
...
pub enum InstallationStackError {
    ...
    Stack(crate::image_emission::ObjectError),
}
...
impl From<crate::image_emission::ObjectError> for InstallationStackError {
    fn from(error: crate::image_emission::ObjectError) -> Self {
        Self::Stack(error)
    }
}
...
fn derive_installed_stack_peak(
    ...
    admitted_contribution_report_identities: &mut std::collections::BTreeSet<
        abstract_operations_to_target_operations::task_plans::AdmittedStackContributionReportId,
    >,
    admitted_contribution_commitments: &mut std::collections::BTreeSet<
        abstract_operations_to_target_operations::task_plans::SameStackContributionCommitment,
    >,
) -> Result<u64, crate::image_emission::ObjectError> {
    if let Some(peak) = memoized.get(&machine) {
        contributing_machines.insert(machine);
        return Ok(*peak);
    }
    if !active.insert(machine) {
        return Err(crate::image_emission::ObjectError::TerminalStackCycle(
            machine,
        ));
    }
    contributing_machines.insert(machine);
    let function = functions.get(&machine).copied().ok_or(
        crate::image_emission::ObjectError::UnknownInternalCallTarget {
            caller: machine,
            target: machine,
        },
    )?;
    let mut peak = match (function.unit_stack, function.scalar_stack) {
        (Some(_), Some(_)) => {
            return Err(
                crate::image_emission::ObjectError::ConflictingTerminalStackEvidence(machine),
            );
        }
        (Some(stack), None) => u64::from(stack.local_peak_bytes),
        (None, Some(stack)) => u64::from(stack.local_peak_bytes),
        (None, None) => {
            return Err(crate::image_emission::ObjectError::UnaccountedTerminalStack(machine));
        }
    };
        ...
        )
    {
        ...
        let composed = u64::from(caller_live_bytes)
            ...
            .ok_or(
                crate::image_emission::ObjectError::TerminalStackCompositionOverflow {
                    caller: machine,
                    owner,
                },
            )?;
