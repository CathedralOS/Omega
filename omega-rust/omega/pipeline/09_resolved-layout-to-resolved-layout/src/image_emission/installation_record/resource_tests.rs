...
use crate::image_emission::installation_record::codec::envelope::MAGIC;
use crate::image_emission::installation_record::record_validation::installed_scalar_control_cleanups_match_object;
...
pub(super) fn installed_function_with_unit_call() -> InstalledFunction {
    InstalledFunction {
        ...
        text_offset: 24,
        byte_count: 16,
        unit_stack: Some(crate::image_emission::ObjectUnitStack {
            frame_bytes: 0,
            local_peak_bytes: 16,
            stack_alignment: 16,
        }),
        scalar_stack: None,
        unit_call_stacks: vec![crate::image_emission::ObjectUnitCallStack {
        ...
        foreign_call_stacks: vec![InstalledForeignCallStack {
            ...
            contribution_report_identity:
                abstract_operations_to_target_operations::task_plans::AdmittedStackContributionReportId::from_normalized_identity(8)
                    .expect("contribution report"),
            contribution_commitment: abstract_operations_to_target_operations::task_plans::SameStackContributionCommitment::from_digest(
                [9; 32],
            ),
...
fn scalar_control_object_cleanup(
    edge: u64,
    code_offset: usize,
) -> post_allocation_machine_to_selected_form_encoding::machine_code::ScalarControlAffineCleanupRecord
{
    post_allocation_machine_to_selected_form_encoding::machine_code::ScalarControlAffineCleanupRecord {
        cleanup: scalar_control_cleanup(edge, code_offset),
        preservation: post_allocation_machine_to_selected_form_encoding::machine_code::ScalarCleanupPreservationEvidence {
            frame: post_allocation_machine_to_selected_form_encoding::machine_code::StackAdjustmentPair {
                byte_size: 16,
...
fn function_codec_round_trips_structural_call_scalar_result_evidence() {
    ...
    function.structural_call_scalar_return =
        Some(post_allocation_machine_to_selected_form_encoding::machine_code::StructuralCallScalarReturnEvidence {
...
fn installed_stack_fact_shape_rejects_nonlocal_or_forged_call_inputs() {
    ...
    zero_foreign_commitment.foreign_call_stacks[0].contribution_commitment =
        abstract_operations_to_target_operations::task_plans::SameStackContributionCommitment::from_digest([0; 32]);
    assert!(!installed_stack_facts_are_canonical(
...
fn native_reference_shapes_and_projections_carry_metadata_not_storage() {
    ...
    assert_eq!(
        ...
        Some(abstract_operations_to_target_operations::calling_conventions::ValueShape::integer(0, 1))
    );
...
fn boundary_opaque_application_custody_round_trips() {
    ...
    let custody =
        crate::boundary_applications::BoundaryOpaqueRepresentationApplications::new(vec![
            crate::boundary_applications::BoundaryOpaqueRepresentationApplication {
