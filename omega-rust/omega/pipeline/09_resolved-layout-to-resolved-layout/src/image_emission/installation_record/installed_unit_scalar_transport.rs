...
pub(super) fn mixed_structural_home_is_canonical(
    home: &post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterHomeRecord,
    parameter: &abstract_operations_to_target_operations::target_operations::TargetStructuralParameter,
) -> bool {
    if let post_allocation_machine_to_selected_form_encoding::machine_code::StructuralSourceLocation::IncomingBorrowedPointer { location } =
        home.location
    {
...
pub(super) fn installed_scalar_abi_is_canonical(
    abi: &abstract_operations_to_target_operations::target_operations::ScalarFunctionAbi,
    target: NativeTarget,
) -> bool {
            ...
            && custody
                ...
                .all(|(index, argument)| {
                    ...
                    let source_is_exact = match argument.source {
                        ...
                        | post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::SelectedCall { .. } => false,
                        ...
                        post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::BooleanImmediate { .. } => false,
        ...
        for store in &function.unit_structural_scalar_field_stores {
            ...
            let (source_is_exact, width, bits) = match store.source {
                    ...
                    let (expected_location, placed_byte_size) =
                        match scalar_parameter.placement.locations.as_slice() {
                            [
                                ...
                                post_allocation_machine_to_selected_form_encoding::machine_code::UnitScalarParameterLocationRecord::Register(
                                    *register,
                                ),
                                *byte_size,
                            ),
                ...
                post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::IntegerImmediate {
                    ...
                    (
                        ...
                        width,
                        Some(
                            crate::image_emission::object_artifact::replay::unit::structural_scalar_field_store::integer_bits(
                                scalar_type,
                                value,
                            )
                            .ok_or_else(invalid)?,
                ...
                post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::Home(source_home) => {
                    if !matches!(
            ...
            let expected_bytes = match store.source {
                post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::Parameter {
                    ...
                    ..
                } => crate::image_emission::object_artifact::replay::unit::structural_scalar_field_store::expected_parameter_store_bytes(
                    record.target,
                    home,
                ...
                post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::Parameter {
                    ...
                    ..
                } => crate::image_emission::object_artifact::replay::unit::structural_scalar_field_store::expected_incoming_parameter_store_bytes(
                    record.target,
                    home,
                    store.field_byte_offset,
                    width,
                ...
                .ok_or_else(invalid)?,
                post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::Home(source_home) => {
                    crate::image_emission::object_artifact::replay::unit::structural_scalar_field_store::expected_home_store_bytes(
                        record.target,
                        home,
                        store.field_byte_offset,
                        width,
                ...
                _ => crate::image_emission::object_artifact::replay::unit::structural_scalar_field_store::expected_store_bytes(
                    record.target,
                    home,
                    store.field_byte_offset,
                    width,
...
fn installed_projected_zero_code_source_is_consistent(
    function: &InstalledFunction,
    source: post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord,
) -> bool {
    let post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::BooleanImmediate {
        defining_operation,
        source_value,
        ..
    } = source
    ...
    for function in functions.values().copied() {
        ...
        for store in &function.unit_write_only_primitive_stores {
            ...
            let (source_is_exact, destination_scalar_type, width, bits) = match store.source {
                ...
                post_allocation_machine_to_selected_form_encoding::machine_code::UnitWriteOnlyPrimitiveStoreSourceRecord::IntegerImmediate {
                    defining_operation,
                    source_value,
                    scalar_type,
                    value,
                ...
                post_allocation_machine_to_selected_form_encoding::machine_code::UnitWriteOnlyPrimitiveStoreSourceRecord::Home(source_home) => {
                    let source_count = record
                        .internal_unit_scalar_calls
                        .iter()
                        .filter(|call| {
...
fn installed_zero_code_store_source_is_consistent(
    function: &InstalledFunction,
    source: post_allocation_machine_to_selected_form_encoding::machine_code::UnitWriteOnlyPrimitiveStoreSourceRecord,
) -> bool {
...
mod tests {
    ...
    use post_allocation_machine_to_selected_form_encoding::machine_code::{
        ForeignCallScalarResultRecord, UnitScalarHomeRecord,
    };
