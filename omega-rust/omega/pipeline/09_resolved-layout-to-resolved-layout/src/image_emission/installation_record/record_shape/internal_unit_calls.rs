...
pub(super) fn validate_internal_unit_calls(
    ...
    for installed in &record.internal_unit_calls {
        ...
        if borrowed_structural::has_borrowed(function) || custody.arguments.iter().any(|argument| {
            matches!(
                argument.source,
                post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitStructuralArgumentSourceRecord::EstablishedByteView { .. }
                    | post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitStructuralArgumentSourceRecord::EstablishedElementView { .. }
                    ...
                    | post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitStructuralArgumentSourceRecord::BlockParameter { .. }
            )
        }) {
            let key = (
                installed.machine,
...
struct InternalUnitCall<'a> {
    ...
    custody: &'a post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitCallRecord,
    plan: &'a abstract_operations_to_target_operations::calling_conventions::CallPlan,
    end: usize,
    incoming_call: bool,
    callee_parameter_abi: Option<&'a post_allocation_machine_to_selected_form_encoding::machine_code::ParameterFunctionAbiRecord>,
    callee_unit_parameters: &'a [post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterRecord],
    callee_mixed_abi: Option<&'a abstract_operations_to_target_operations::target_operations::MixedStructuralScalarFunctionAbi>,
    ...
    affine_cleanup: Option<&'a post_allocation_machine_to_selected_form_encoding::machine_code::UnitAffineCleanupRecord>,
    projected_result: Option<&'a post_allocation_machine_to_selected_form_encoding::machine_code::InternalStructuralCallResult>,
...
fn callee_facts<'a>(
    ...
    let structural_result_valid = match (&custody.structural_result, target_structural_return) {
        ...
        (Some(result), Some(target)) => custody.result.is_none()
            ...
            && crate::image_emission::object_artifact::replay::unit::call_custody::structural_result_matches_return(
                result, target,
            ),
        _ => false,
    };
        ...
        &CallSignature {
            parameters: if let Some(abi) = callee_parameter_abi {
                abi.parameters
                    .iter()
                    .map(|parameter| {
                        crate::image_emission::object_artifact::replay::unit::call_custody::unit_scalar_shape(
...
fn custody_facts<'a>(
    ...
    .or_else(|| {
        ...
        crate::image_emission::object_artifact::replay::unit::continuations::result_for_call(function_unit_calls, &disposed, custody)
    });
