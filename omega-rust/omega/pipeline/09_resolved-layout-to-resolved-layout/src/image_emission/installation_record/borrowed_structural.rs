...
pub(super) fn function_is_exact(record: &InstallationRecord, function: &InstalledFunction) -> bool {
    ...
    let call_attribution_is_exact = |owner, text_offset| {
        let abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(
            operation,
        ) = owner
        else {
            return false;
                    ...
                    .filter(|row| {
                        row.machine == function.machine
                            && row.attribution.site
                                == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Operation(*psi_operation)
                    })
                    .count();
...
fn owned_copy_is_exact(
    ...
    sources: &[post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterHomeRecord],
    destination: &post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterHomeRecord,
