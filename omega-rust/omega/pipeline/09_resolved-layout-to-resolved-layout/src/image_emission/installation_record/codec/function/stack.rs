...
use crate::image_emission::installation_record::codec::call_site_owner::{
...
pub(crate) fn decode_function_stack_facts(
    ...
    (
        Option<crate::image_emission::ObjectUnitStack>,
        Option<crate::image_emission::ObjectScalarStack>,
        Vec<crate::image_emission::ObjectUnitCallStack>,
        Vec<crate::image_emission::ObjectScalarCallStack>,
...
> {
    let unit_stack = match reader.u8()? {
        ...
        1 => {
            ...
            Some(crate::image_emission::ObjectUnitStack {
    ...
    let scalar_stack = match reader.u8()? {
        ...
        1 => {
            ...
            Some(crate::image_emission::ObjectScalarStack {
                local_peak_bytes: reader.u32()?,
                stack_alignment: reader.u32()?,
            })
        }
    ...
    for _ in 0..unit_call_count {
        unit_call_stacks.push(crate::image_emission::ObjectUnitCallStack {
    ...
    for _ in 0..scalar_call_count {
        scalar_call_stacks.push(crate::image_emission::ObjectScalarCallStack {
            owner: decode_call_site_owner(reader)?,
            target: MachineId::new(reader.u64()?)
                .ok_or(InstallationError::ZeroInternalUnitCallIdentity)?,
            text_offset: usize::try_from(reader.u64()?)
    ...
    for _ in 0..foreign_call_count {
        ...
        let contribution_report_identity =
            abstract_operations_to_target_operations::task_plans::AdmittedStackContributionReportId::from_normalized_identity(reader.u64()?)
                .map_err(|_| InstallationError::InvalidForeignStackContribution)?;
        let contribution_commitment =
            abstract_operations_to_target_operations::task_plans::SameStackContributionCommitment::from_digest(reader.array()?);
