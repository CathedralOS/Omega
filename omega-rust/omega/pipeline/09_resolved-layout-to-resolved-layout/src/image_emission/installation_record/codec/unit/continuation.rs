...
use crate::image_emission::installation_record::codec::function::affine_cleanup::{
    decode_unit_affine_cleanup, encode_unit_affine_cleanup,
};
use crate::image_emission::installation_record::codec::unit::scalar::{
    decode_scalar_type, encode_scalar_type,
};
use crate::image_emission::installation_record::{InstallationError, Reader, push_u32, push_u64};
use post_allocation_machine_to_selected_form_encoding::machine_code::UnitContinuationRecord;
use semantic_vocabulary::{BlockId, ValueId};

pub(crate) fn encode_unit_continuations(
    bytes: &mut Vec<u8>,
...
pub(crate) fn decode_unit_continuations(
    reader: &mut Reader<'_>,
...
fn decode_bindings(
    ...
    Vec<abstract_operations_to_target_operations::target_operations::ValueBinding>,
    InstallationError,
> {
    let count = usize::try_from(reader.u32()?)
        .map_err(|_| InstallationError::TooManyScalarCallPlanValues)?;
    ...
    (0..count)
        .map(|_| {
            Ok(
                abstract_operations_to_target_operations::target_operations::ValueBinding {
