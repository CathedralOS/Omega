...
impl crate::image_emission::ObjectArtifact {
...
impl HostedReceiverBinding {
    ...
    pub const fn stack_demand(&self) -> &crate::image_emission::StackDemand {
        &self.demand
    }
...
struct ReceiverStorage {
    scratch: crate::object_file::ObjectSymbolHandle,
    stack_top: crate::object_file::ObjectSymbolHandle,
    receiver: crate::object_file::ObjectSymbolHandle,
}
...
fn provision_receiver_storage(
    ...
    (
        crate::object_file::ObjectPlan,
        HostedReceiverPartitions,
        ReceiverStorage,
    ),
    Diagnostic,
> {
    use crate::object_file::{SectionKind, SectionPlan, SymbolKind, SymbolPlan, SymbolSection};
...
fn install_entry_text(
    mut object: crate::object_file::ObjectPlan,
    artifact: &crate::image_emission::ObjectArtifact,
    bytes: &[u8],
    name: &str,
) -> Result<
...
> {
    ...
    let (symbol, offset) = crate::image_emission::hosted_unit_entry::install_entry_shim(
        &mut object,
        &mut text,
        bytes,
        name,
...
pub(crate) fn prepare(
    artifact: &crate::image_emission::ObjectArtifact,
) -> Result<crate::image_emission::hosted_unit_entry::PreparedEntry, Diagnostic> {
    ...
    Ok(crate::image_emission::hosted_unit_entry::PreparedEntry {
        object,
        text,
        relocations,
        shim: crate::image_emission::hosted_unit_entry::EntryShim::DarwinReceiver {
            symbol,
            offset,
        },
    })
...
fn prepare_linux_x86_64(
    ...
    Ok(crate::image_emission::hosted_unit_entry::PreparedEntry {
        ...
        text,
        relocations,
        shim: crate::image_emission::hosted_unit_entry::EntryShim::LinuxReceiver { symbol, offset },
    })
}
...
fn prepare_linux_arm64(
    ...
    Ok(crate::image_emission::hosted_unit_entry::PreparedEntry {
        ...
        text,
        relocations,
        shim: crate::image_emission::hosted_unit_entry::EntryShim::LinuxArm64Receiver {
            symbol,
            offset,
        },
    })
...
fn prepare_windows_x86_64(
    ...
    Ok(crate::image_emission::hosted_unit_entry::PreparedEntry {
        ...
        text,
        relocations,
        shim: crate::image_emission::hosted_unit_entry::EntryShim::WindowsReceiver {
            symbol,
            offset,
        },
    })
...
pub fn bind_hosted_receiver(
    artifact: &mut crate::image_emission::ObjectArtifact,
    source: &SelectedProgramEntrySourceSignature,
    physical: &ProgramEntryPhysicalContractPlan,
    ...
    crate::image_emission::function_fragments::replay::validate(artifact)?;
...
pub(crate) fn validate_binding(
    ...
    if !physical_contract_matches(&binding.physical, artifact.target)
        ...
        || crate::image_emission::derive_stack_demand(artifact, artifact.entry)
            .map_err(|_| invalid())?
            != binding.demand
...
fn macos_physical_contract_matches(physical: &ProgramEntryPhysicalContractPlan) -> bool {
    ...
    let expected = crate::program_entry_plan::exact_macos_arm64_physical_boundary_entry_plan();
...
fn macos_x86_64_physical_contract_matches(physical: &ProgramEntryPhysicalContractPlan) -> bool {
    ...
    let expected = crate::program_entry_plan::exact_macos_x86_64_physical_boundary_entry_plan();
    // Native settlement separately rejoins the accepted package requirement
    // identity. A package-qualified requirement is not the standalone spelling;
    // it must still carry these exact target-source bytes, role, and ABI/state.
    physical.target_slot() == target::TargetProfile::MacosX64.program_entry_slot()
        ...
        && physical.target_package_source_digest()
            == crate::program_entry_plan::exact_macos_x86_64_physical_contract_package_source_digest(
            )
        && !physical.requirement_identity().is_empty()
        && physical.parameter_type_identities()
            == [
...
fn linux_x86_64_physical_contract_matches(physical: &ProgramEntryPhysicalContractPlan) -> bool {
    ...
    let expected = crate::program_entry_plan::exact_linux_x86_64_physical_boundary_entry_plan();
...
fn linux_arm64_physical_contract_matches(physical: &ProgramEntryPhysicalContractPlan) -> bool {
    use crate::program_entry_plan::{LINUX_ARM64_I32_TYPE_IDENTITY, LINUX_ARM64_U64_TYPE_IDENTITY};
    let expected = crate::program_entry_plan::exact_linux_arm64_physical_boundary_entry_plan();
...
fn windows_x86_64_physical_contract_matches(physical: &ProgramEntryPhysicalContractPlan) -> bool {
    use crate::program_entry_plan::WINDOWS_X86_64_U32_TYPE_IDENTITY;
    let expected = crate::program_entry_plan::exact_windows_x86_64_physical_boundary_entry_plan();
...
fn receiver_layout(
    ...
    let (function, target) =
        crate::image_emission::function_fragments::replay::entry_source(artifact)?;
...
pub(crate) fn validate_image(
    ...
    shim: crate::image_emission::hosted_unit_entry::EntryShim,
    output: &crate::image::EmittedImageOutput,
) -> Result<(), Diagnostic> {
    let invalid = || invalid(artifact.target);
    let expected = prepare(artifact)?;
    let (_symbol, offset) = match shim {
        crate::image_emission::hosted_unit_entry::EntryShim::DarwinReceiver { symbol, offset }
            if expected_shim_matches(expected.shim, shim) =>
        {
            ...
            if !crate::image_emission::hosted_unit_entry::unique_region(
                object, symbol, offset, shim_bytes, output,
            ) || !crate::image_emission::hosted_unit_entry::main_points_to(&output.bytes, offset)
            {
                return Err(invalid());
            }
            (symbol, offset)
        }
        crate::image_emission::hosted_unit_entry::EntryShim::LinuxReceiver { symbol, offset }
            if expected_shim_matches(expected.shim, shim) =>
        {
            if !crate::image_emission::hosted_unit_entry::unique_region(
                object,
        ...
        crate::image_emission::hosted_unit_entry::EntryShim::LinuxArm64Receiver {
            symbol,
            offset,
        } if expected_shim_matches(expected.shim, shim) => {
            if !crate::image_emission::hosted_unit_entry::unique_region(
        ...
        crate::image_emission::hosted_unit_entry::EntryShim::WindowsReceiver { symbol, offset }
            if expected_shim_matches(expected.shim, shim) =>
        {
            let end = offset
            ...
            if !crate::image_emission::hosted_unit_entry::unique_region(
                ...
                output,
            ) || !crate::image_emission::hosted_unit_entry::pe_entry_points_to(
                &output.bytes,
                offset,
                expected_shim,
            ) {
    ...
    let mut original_bss = artifact
        ...
        .sections
        .iter()
        .filter(|(_, section)| section.kind == crate::object_file::SectionKind::Bss);
    ...
    let shim_address = output
        ...
        .ok_or_else(invalid)?;
    match shim {
        crate::image_emission::hosted_unit_entry::EntryShim::DarwinReceiver { .. } => {
        ...
        crate::image_emission::hosted_unit_entry::EntryShim::LinuxReceiver { .. } => {
            let end = offset
        ...
        crate::image_emission::hosted_unit_entry::EntryShim::WindowsReceiver { .. } => {
            let end = offset
                .checked_add(WINDOWS_X86_64_RECEIVER_SHIM_BYTES)
                .ok_or_else(invalid)?;
...
fn expected_shim_matches(
    expected: crate::image_emission::hosted_unit_entry::EntryShim,
    actual: crate::image_emission::hosted_unit_entry::EntryShim,
) -> bool {
...
fn elf_entry_points_to(output: &crate::image::EmittedImageOutput, offset: usize) -> bool {
