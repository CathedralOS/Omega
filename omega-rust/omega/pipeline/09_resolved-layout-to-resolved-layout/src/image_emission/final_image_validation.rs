...
pub(super) fn validate_terminal_image(
    ...
    object: &crate::object_file::ObjectPlan,
    relocations: &crate::object_file::RelocationPlan,
    text_bytes: &[u8],
    ...
    let expected_imports = object
        ...
        .symbols
        .iter()
        .filter(|(_, symbol)| symbol.kind == crate::object_file::SymbolKind::Import)
        .count();
...
fn validate_terminal_image_with_import_count(
    ...
    if artifact.target().object_format == target::ObjectFormat::MachO {
        ...
        match artifact.target().architecture {
            target::Architecture::Aarch64 => {
                crate::image_macho::validate_macho_aarch64_loader_mapping(
                    &output.bytes,
                ...
                crate::image_macho::validate_macho_aarch64_object_fixups(
                    object,
                    relocations,
                    text_bytes.len(),
                ...
                crate::image_macho::validate_macho_aarch64_import_binding_pairing(
                    &output.final_text_bytes,
            ...
            target::Architecture::X86_64 => {
                crate::image_macho::validate_macho_x86_64_loader_mapping(
                    &output.bytes,
                    output.final_image_layout,
                    &output.final_text_bytes,
                    &output.final_data_bytes,
                ...
                crate::image_macho::validate_macho_x86_64_object_fixups(
                    object,
                    relocations,
                    text_bytes.len(),
                    artifact.data_bytes().len(),
                ...
                crate::image_macho::validate_macho_x86_64_import_binding_pairing(
                    &output.final_text_bytes,
                    &output.executable_regions,
                    &output.data_regions,
                )?;
    ...
    crate::image::validate_placed_executable_region_inventory(
        &output.executable_regions,
        &output.final_text_bytes,
    )?;
    crate::image::validate_placed_data_region_inventory(
        &output.data_regions,
        &output.final_data_bytes,
    )?;
    ...
    for function in &artifact.functions {
        let symbol = crate::object_file::object_symbol_name(object, function.symbol);
    ...
    for adapter in &artifact.forwarded_dynamic_descriptor_adapters {
        let symbol = crate::object_file::object_symbol_name(object, adapter.symbol);
        let matching = compiler_regions
            .iter()
            .filter(|region| {
                region.symbol == symbol
    ...
    for private in &artifact.private_functions {
        ...
        let Some((symbol_handle, symbol_plan)) =
            crate::object_file::object_function_symbol(object, private.identity)
        else {
            return Err(Diagnostic::error(
...
fn encoded_final_data_with_image_import_slots(
    ...
    let referenced_import_count = object
        ...
        .filter(|(handle, symbol)| {
            symbol.kind == crate::object_file::SymbolKind::Import
                && relocations
                    .records()
...
fn validate_dynamic_conformance_tables(
    ...
    for table in artifact.dynamic_conformance_tables() {
        ...
        if symbol.section
            != crate::object_file::SymbolSection::Section(crate::object_file::SectionKind::Data)
            || symbol.kind != crate::object_file::SymbolKind::Object
            ...
            match (
                ...
                (Some(callable_identity), Some(target), Some(target_symbol)) => {
                    ...
                    let target_is_exact = target_plan.kind
                        == crate::object_file::SymbolKind::Function
                    ...
                    let matching = relocations
                        ...
                        .filter(|(_, relocation)| {
                            relocation.origin
                                == crate::object_file::RelocationOrigin::Materialization {
                                    object_symbol_handle: table.symbol,
                                }
                                && relocation.section == crate::object_file::SectionKind::Data
                                && relocation.offset == slot.data_offset
                                && relocation.byte_width == 8
                                && relocation.symbol_handle == target_symbol
                                && relocation.addend == 0
                                && relocation.kind == crate::object_file::RelocationKind::Absolute64
                        })
                        .count();
    ...
    for table in artifact.forwarded_dynamic_descriptor_tables() {
        ...
        for (row_index, slot) in table.slots.iter().enumerate() {
            ...
            if usize::try_from(slot.row_index) != Ok(row_index)
                ...
                || slot.data_offset != expected_offset
                || adapter_symbol.kind != crate::object_file::SymbolKind::Function
            ...
            let (call_offset, call_kind) = match artifact.target().architecture {
                target::Architecture::X86_64 => (
                    adapter
                        ...
                        .ok_or_else(invalid)?,
                    crate::object_file::RelocationKind::X86_64Relative32,
                ),
                target::Architecture::Aarch64 => (
                    adapter
                        .text_offset
                        .checked_add(adapter.record.direct_call_offset)
                        .ok_or_else(invalid)?,
                    crate::object_file::RelocationKind::Aarch64Branch26,
                ),
            };
            if relocations
                .records()
                .filter(|(_, relocation)| {
                    relocation.origin
                        ...
                        && relocation.section == crate::object_file::SectionKind::Text
    ...
    if relocations
        .records()
        .filter(|(_, relocation)| relocation.section == crate::object_file::SectionKind::Data)
        .count()
    ...
    for function in artifact.functions() {
        for call in &function.dynamic_calls {
            let table = artifact
                ...
                .iter()
                .find(|table| {
                    crate::image_emission::object_artifact::same_dynamic_table_application(
                        &table.application,
                        &call.dynamic_dispatch.application,
                    )
                })
                .ok_or_else(invalid)?;
            let origin = crate::object_file::RelocationOrigin::SemanticOperation {
            ...
            let expected = match call.table_address.encoding {
                post_allocation_machine_to_selected_form_encoding::machine_code::DynamicTableAddressEncoding::X86_64Relative32 {
                    relocation_offset,
                } => vec![(
                    function
                        .text_offset
                ...
                post_allocation_machine_to_selected_form_encoding::machine_code::DynamicTableAddressEncoding::Aarch64PageAddress {
                    ...
                    (
                        function
                            ...
                            .ok_or_else(invalid)?,
                        crate::object_file::RelocationKind::Aarch64Page21,
                    ),
                    (
                        function
                            .text_offset
                            ...
                            .ok_or_else(invalid)?,
                        crate::object_file::RelocationKind::Aarch64PageOffset12,
                    ),
                ],
            };
        ...
        for call in &function.forwarded_dynamic_descriptor_calls {
            for argument in &call.dynamic_arguments {
                let application = match &argument.custody.source {
                    terminal_psi_to_abstract_operations::abstract_operations::AbstractDynamicDescriptorSource::Selection {
                        application,
                        ..
                    }
                    ...
                    terminal_psi_to_abstract_operations::abstract_operations::AbstractDynamicDescriptorSource::Parameter(_) => {
                        return Err(invalid());
                    }
                };
                let table = artifact
...
fn validate_callback_relocation_targets(
    ...
    for call in &artifact.foreign_calls {
        ...
        let operation = match call.owner {
            abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(operation) => operation,
            abstract_operations_to_target_operations::target_operations::CallSiteOwner::CleanupAction { .. } => {
                return Err(Diagnostic::error(
        ...
        let (callback_symbol, callback_symbol_plan) =
            crate::object_file::object_function_symbol(object, callback.target.callback_function)
                .ok_or_else(|| {
                Diagnostic::error(
                    "callback address relocation lost its compiler-private function symbol",
                )
            })?;
        let expected_origin = crate::object_file::RelocationOrigin::SemanticOperation {
            function_symbol_handle: function.symbol,
...
mod tests {
    ...
    use crate::image_emission::object_artifact::same_dynamic_table_application;
    use terminal_psi::{
