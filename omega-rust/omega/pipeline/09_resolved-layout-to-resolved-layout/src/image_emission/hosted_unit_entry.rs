...
pub(super) struct PreparedEntry {
    ...
    pub text: Vec<u8>,
    pub relocations: crate::object_file::RelocationPlan,
    pub shim: EntryShim,
}
...
struct UnitEntryForm {
    ...
    header_check: fn(&crate::image::EmittedImageOutput, usize, &[u8]) -> bool,
...
pub(super) fn prepare(
    artifact: &crate::image_emission::ObjectArtifact,
) -> Result<Option<PreparedEntry>, Diagnostic> {
    if artifact.hosted_receiver_binding().is_some() {
        return crate::image_emission::hosted_receiver::prepare(artifact).map(Some);
    }
    if !crate::image_emission::function_fragments::replay::has_free_unit_entry(artifact)? {
        return Ok(None);
    }
...
fn encode_linux_x86_64(offset: usize, destination: usize) -> Result<Vec<u8>, Diagnostic> {
    ...
    bytes.extend(
        target_operations_to_selected_instructions::isa_x86_64::encode_hosted_exit_process_i32(0),
    );
    Ok(bytes)
}
...
fn encode_linux_arm64(offset: usize, destination: usize) -> Result<Vec<u8>, Diagnostic> {
    let exit =
        target_operations_to_selected_instructions::isa_aarch64::encode_hosted_exit_process_i32(
...
fn decode_linux_x86_64(bytes: &[u8], offset: usize) -> Option<usize> {
    ...
    if bytes[5..]
        != *target_operations_to_selected_instructions::isa_x86_64::encode_hosted_exit_process_i32(
            0,
        )
        .as_slice()
    {
...
pub(super) fn unique_region(
    ...
    offset: usize,
    byte_count: usize,
    output: &crate::image::EmittedImageOutput,
) -> bool {
    let name = crate::object_file::object_symbol_name(object, symbol);
    output
        .executable_regions
        .regions
        .iter()
        .filter(|region| {
            region.origin == crate::image::FinalExecutableRegionOrigin::CompilerFunction
                && region.symbol == name
...
fn validate_unit_entry(
    ...
    if artifact.target != (form.target)()
        || !crate::image_emission::function_fragments::replay::has_free_unit_entry(artifact)?
...
mod tests {
    ...
    fn darwin_unit_entry_overwrites_residual_status_without_an_extra_frame() {
        ...
        assert_eq!(
            &bytes[4..],
            target_operations_to_selected_instructions::isa_aarch64::encode_hosted_exit_process_i32(target::NativeTarget::macos_arm64(), 0)
                .unwrap()
                .as_slice()
        );
        for bit in 0..160 {
    ...
    fn linux_arm64_unit_entry_calls_entry_and_exits_with_zero_status() {
        ...
        assert_eq!(
            &bytes[4..],
            target_operations_to_selected_instructions::isa_aarch64::encode_hosted_exit_process_i32(target::NativeTarget::linux_arm64(), 0)
                .unwrap()
                .as_slice()
        );
        for bit in 0..160 {
