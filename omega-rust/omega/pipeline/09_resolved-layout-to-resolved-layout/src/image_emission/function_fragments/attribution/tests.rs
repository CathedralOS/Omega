...
#[test]
fn boolean_equality_attribution_requires_a_unique_authored_operation_ordinal() {
    let (_, mut source) = fixture();
    let operation = OperationId::new(2).unwrap();
    source.operations[1] = AbstractOperation::BooleanEqual {
...
fn wrapping_remainder_attribution_requires_a_unique_authored_operation_ordinal() {
    let (_, mut source) = fixture();
    let operation = OperationId::new(2).unwrap();
    source.operations[1] = AbstractOperation::WrappingIntegerRemainder {
        psi_operation: operation,
        result: ValueId::new(3).unwrap(),
...
fn fixture() -> (FunctionFragment, AbstractFunction) {
    ...
    .enumerate()
    {
        instructions.push(FunctionFragmentInstructionSpan {
            ...
            provenance: target_operations_to_selected_instructions::SelectedInstructionProvenance {
...
fn case_edges_share_the_authored_case_ordinal_without_fabricated_operations() {
    ...
    source.operations[2] = AbstractOperation::StructuralCase {
        ...
        cases: [10, 11]
            .into_iter()
            .map(
                |identity| terminal_psi_to_abstract_operations::abstract_operations::AbstractStructuralCaseSuccessor {
...
fn bridge_continuation_does_not_attribute_the_semantic_edge_twice() {
    ...
    let successor = post_allocation_machine_to_selected_form_encoding::machine_code::FunctionFragmentSuccessorProvenance {
    ...
    jump.offset = 9;
    jump.control = Control::Jump {
        successor: post_allocation_machine_to_selected_form_encoding::machine_code::FunctionFragmentSuccessorProvenance {
