...
use crate::image_emission::{ObjectArtifact, ObjectFunction};
...
fn validate(
    source: &StagedOptimizedRelocationFreeObjectContainer,
    artifact: &ObjectArtifact,
    private_functions: Option<&[post_allocation_machine_to_selected_form_encoding::machine_code::CompilerPrivateMachineCodeFunction]>,
) -> Result<(), Error> {
    source::admit(source)?;
    ...
    if let Some(binding) = artifact.hosted_receiver_binding() {
        crate::image_emission::hosted_receiver::validate_binding(artifact, binding)
