...
use crate::image_emission::{
    ObjectScalarCallStack, ObjectScalarStack, ObjectUnitCallStack, ObjectUnitStack,
};
use post_allocation_machine_to_selected_form_encoding::machine_code::PlacedInternalMachineCallResolution;
...
pub(super) fn validate(
    ...
    for ((owner, target, text_offset, caller_live_bytes), call) in edges.zip(calls) {
        if !contains_call
            || owner != abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(call.operation)
            || target != call.callee
...
mod tests {
    ...
    fn scalar_call_edges_retain_exact_frame_prefix_owner_target_and_role() {
        for architecture in [Architecture::X86_64, Architecture::Aarch64] {
            ...
            changed = row;
            changed.owner =
                abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(OperationId::new(9).unwrap());
            assert!(check(stack, &[changed]).is_err());
            changed = row;
