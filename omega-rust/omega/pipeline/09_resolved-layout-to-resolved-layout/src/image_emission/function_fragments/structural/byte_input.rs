...
use abstract_operations_to_target_operations::target_operations::{
    BoundaryRealization, CompilerBuiltinExecution,
};
...
fn decoded_home(target: NativeTarget, bytes: &[u8]) -> Option<u32> {
    if !abstract_operations_to_target_operations::target_operations::HostedReadByteRealization::supports_target(target) {
        return None;
    }
    match target.architecture {
        Architecture::X86_64 => target_operations_to_selected_instructions::isa_x86_64::decode_x86_64_selected_hosted_read_byte(bytes),
        Architecture::Aarch64 => {
            target_operations_to_selected_instructions::isa_aarch64::decode_aarch64_selected_hosted_read_byte(target, bytes)
        }
    }
}
...
pub(in crate::image_emission::function_fragments) fn cleanup_actions_match(
    ...
    !actions.is_empty()
        && actions.iter().all(|action| {
            ...
            crate::image_emission::object_artifact::replay::boundary::result_placement::hosted_read_byte_declaration_is_valid(declaration)
        })
}
            ...
            selected
                ...
                .push(SelectedBoundarySettlement {
                    ...
                    settlement: SelectedBoundarySettlementPayload::HostedReadByte {
                        ...
                        boundary,
                        result,
                        layout: abstract_operations_to_target_operations::calling_conventions::evaluate_conventional_sum_layout(
                            &[],
                            &[vec![], vec![abstract_operations_to_target_operations::calling_conventions::ValueShape::integer(4, 4)]],
                        )
                        .unwrap(),
                    },
                });
