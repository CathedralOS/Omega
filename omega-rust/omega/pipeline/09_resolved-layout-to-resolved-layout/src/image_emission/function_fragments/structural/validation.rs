...
pub(in crate::image_emission::function_fragments) fn validate_function(
...
#[cfg(test)]
mod tests;
pub(in crate::image_emission::function_fragments) fn validate_settlements(
    source: &StagedOptimizedRelocationFreeObjectContainer,
    ...
    for placed in &source.source().text_section().functions {
        ...
        for located in &function.boundary_settlements {
            ...
            cursor += 1;
            let target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::ClaimCompletion(expected) =
                &located.settlement
            else {
                match located.settlement {
                    target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::HostedReadByte { .. } => byte_input::validate(source, placed.machine, located, actual)?,
                    target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::HostedExitProcessI32 { .. } => process_exit::validate(source, placed.machine, located, actual)?,
            ...
            if actual.machine != placed.machine
                ...
                || row.execution
                    != post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryExecutionRecord::AdmittedProvider(
                        expected.provider_execution.into(),
                    )
                || row.realization
                    != abstract_operations_to_target_operations::target_operations::BoundaryRealization::ClaimCompletionOnly(
            ...
            crate::image_emission::object_artifact::replay::boundary::completion_receipts::validate_completion_custody(row)
...
pub(in crate::image_emission::function_fragments) fn validate_settlement_attributions(
    source: &StagedOptimizedRelocationFreeObjectContainer,
    machine: MachineId,
    rows: &[SemanticCodeAttribution],
    ...
    for located in &function.boundary_settlements {
        ...
        let expected_byte_count = if matches!(
            located.settlement,
            target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::HostedWriteByteI32 { .. }
                | target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::HostedReadByte { .. }
...
fn validate_copy(
    ...
    let memory = selected
        ...
        .iter()
        .filter(|row| {
            row.origin == target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Operation(operation)
                && row.place == place
                && matches!(
                    row.role,
