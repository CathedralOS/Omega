...
use abstract_operations_to_target_operations::target_operations::TargetStructuralArgument;
use post_allocation_machine_to_selected_form_encoding::machine_code::StructuralSourceLocation;
...
pub(super) fn location(
    ...
    let mut producers = abstracted.operations.iter().filter(|operation| {
        matches!(operation,
        terminal_psi_to_abstract_operations::abstract_operations::AbstractOperation::ByteSequenceSubslice { psi_operation, .. }
        | terminal_psi_to_abstract_operations::abstract_operations::AbstractOperation::EstablishByteSequenceLiteral { psi_operation, .. }
        | terminal_psi_to_abstract_operations::abstract_operations::AbstractOperation::ElementViewSubslice { psi_operation, .. }
        | terminal_psi_to_abstract_operations::abstract_operations::AbstractOperation::EstablishElementView { psi_operation, .. }
        if *psi_operation == producer)
    });
    let byte_size = match producers.next() {
        Some(terminal_psi_to_abstract_operations::abstract_operations::AbstractOperation::ByteSequenceSubslice { result, .. })
        ...
        Some(terminal_psi_to_abstract_operations::abstract_operations::AbstractOperation::ElementViewSubslice { result, .. })
            if result.place == argument.place
                && result.structural_type == argument.structural_type
                && result.multiplicity == StructuralMultiplicity::Unrestricted
                && result.qualifications.is_empty()
        ...
        Some(terminal_psi_to_abstract_operations::abstract_operations::AbstractOperation::EstablishElementView { result, .. })
            if result.place == argument.place
                && result.structural_type == argument.structural_type
                && result.multiplicity == StructuralMultiplicity::Unrestricted
                && result.qualifications.is_empty()
        ...
        Some(terminal_psi_to_abstract_operations::abstract_operations::AbstractOperation::EstablishByteSequenceLiteral {
            place,
            structural_type,
            bytes,
            ..
        }) if place.id == argument.place && structural_type.id == argument.structural_type => {
            let length = u32::try_from(bytes.len()).map_err(|_| Error::Overflow)?;
            (length.checked_add(7).ok_or(Error::Overflow)? & !7)
                .checked_add(16)
    ...
    local_location(
        ...
        target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Operation(producer),
        byte_size,
    )
}
...
pub(super) fn block_location(
    ...
    local_location(
        ...
        target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Block(block),
        16,
    )
}
...
fn local_location(
    ...
    origin: target_operations_to_selected_instructions::SelectedMemoryAccessOrigin,
    byte_size: u32,
) -> Result<StructuralSourceLocation, Error> {
    ...
    if selected_slots.next().is_some()
        || selected_slot.byte_size != byte_size
    ...
    if instructions.next().is_some()
        ...
        || match origin {
            target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Operation(
                producer,
            ) => instruction.provenance.operations.last() != Some(&producer),
            target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Block(
                owner,
            ) => {
                ...
                instruction.provenance != Default::default()
                    || !selected.blocks.iter().any(|block| {
                        block.origin == target_operations_to_selected_instructions::SelectedBlockOrigin::Source(owner)
                            && block
                                .instructions
                                .iter()
            ...
            target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Edge(_) => true,
        }
