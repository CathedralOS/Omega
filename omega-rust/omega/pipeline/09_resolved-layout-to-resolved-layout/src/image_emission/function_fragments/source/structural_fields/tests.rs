...
fn projected_primitive_publication_retains_path_and_requires_native_replay() {
    ...
    *indices = vec![traversal(ValueId::new(8).unwrap())];
    ...
    let row = |role| target_operations_to_selected_instructions::SelectedMemoryAccess {
    ...
    let element = |index, obligation| {
        target_operations_to_selected_instructions::SelectedMemoryAccessRole::ReadIndexedPrimitive {
            index,
            stride: 1,
            extent: 2,
            obligation,
    ...
    for rows in [
        vec![],
        vec![row(
            target_operations_to_selected_instructions::SelectedMemoryAccessRole::ReadPlace,
        )],
...
fn byte_replacement_membership_retains_every_operand_and_writable_access() {
    use abstract_operations_to_target_operations::target_operations::TargetUnitScalarArgumentSource;
