...
use terminal_psi_to_abstract_operations::abstract_operations::{
    AbstractBlockEntry, AbstractStructuralCaseSuccessor,
};
...
fn source_slot_matches(
    dispatch: &target_operations_to_selected_instructions::selected_instructions::SelectedCaseDispatchSource,
    source: PlaceId,
    ...
    let slot = match dispatch {
        target_operations_to_selected_instructions::selected_instructions::SelectedCaseDispatchSource::Local { slot } => *slot,
        target_operations_to_selected_instructions::selected_instructions::SelectedCaseDispatchSource::Borrowed { place, .. } => {
...
mod tests {
    ...
    fn case_parameter_accounting_requires_the_owned_function_parameter() {
        ...
        assert!(!source_slot_matches(
            &target_operations_to_selected_instructions::selected_instructions::SelectedCaseDispatchSource::Local { slot: other },
            source,
            &[],
            &[parameter]
        ));
    ...
    fn case_home_accounting_requires_the_unique_actual_arrival_owner() {
        ...
        assert!(!source_slot_matches(
            &target_operations_to_selected_instructions::selected_instructions::SelectedCaseDispatchSource::Local {
                slot: operation_slot
            },
            source,
            &[entry],
