...
use abstract_operations_to_target_operations::target_operations::{
    TargetControlTerminator, TargetFunction, TargetUnitOperation,
};
use target_operations_to_selected_instructions::{SelectedFunction, SelectedTerminator};
...
pub(super) fn header(
    ...
    let result_matches = match &source.result {
        terminal_psi_to_abstract_operations::abstract_operations::AbstractFunctionResult::Structural(result) => {
            contract.result.as_ref() == Some(result)
        }
        terminal_psi_to_abstract_operations::abstract_operations::AbstractFunctionResult::Scalar(_) => contract.result.is_none(),
        _ => false,
    };
    result_matches
            ...
            result.scalar_type == semantic_vocabulary::ScalarType::Boolean
                ...
                && selected.blocks.iter().flat_map(|block| &block.instructions).filter(|row|
                    row.provenance.operations == [*psi_operation] && row.provenance.values == [result.value]
                        && match row.kind {
                            target_operations_to_selected_instructions::SelectedInstructionKind::Load32 { byte_offset } => byte_offset == tag_byte_offset,
                            target_operations_to_selected_instructions::SelectedInstructionKind::ZeroExtendU32 => path.is_empty() && tag_byte_offset == 0,
                            _ => false,
                        }
                ).count() == 1
        }
            ...
            returned_claims.is_empty() && trivial_affine_locals.is_empty()
                && graph.blocks.iter().filter(|block| matches!(&block.terminator,
                    TargetControlTerminator::ReturnStructural { psi_edge: retained, source: home, cleanup_actions }
                        if retained == psi_edge && match home {
                            abstract_operations_to_target_operations::target_operations::TargetStructuralReturnSource::Home(home) => home.place() == *source,
                            abstract_operations_to_target_operations::target_operations::TargetStructuralReturnSource::Parameter(parameter) => parameter.place == *source,
                ...
                && selected.blocks.iter().filter(|block| matches!(&block.terminator,
                    SelectedTerminator::Return { psi_return_edge, instruction }
                        if psi_return_edge == psi_edge && (matches!(instruction.kind, target_operations_to_selected_instructions::SelectedInstructionKind::ReturnAggregate { .. } | target_operations_to_selected_instructions::SelectedInstructionKind::ReturnScalar)
                            || (instruction.kind == target_operations_to_selected_instructions::SelectedInstructionKind::ReturnUnit
                                ...
                                && graph.call_plan.result.as_ref().is_some_and(|placement|
                                    (placement.shape == abstract_operations_to_target_operations::calling_conventions::ValueShape::integer(0, 1)
                                    ...
                                    || (graph.call_plan.policy == abstract_operations_to_target_operations::calling_conventions::CallingPolicy::Aapcs64
                                        ...
                                        && matches!(placement.locations.as_slice(),
                                            [abstract_operations_to_target_operations::calling_conventions::ValueLocation::Indirect {
                                                pointer: abstract_operations_to_target_operations::calling_conventions::IndirectPointerLocation::Register(abstract_operations_to_target_operations::calling_conventions::MachineRegister::Aarch64X(8)),
