...
use abstract_operations_to_target_operations::target_operations::TargetFunction;
use target_operations_to_selected_instructions::{
    SelectedFunction, SelectedStructuralTransport, SelectedTerminator,
};
...
pub(in crate::image_emission::function_fragments) fn arrivals(
    ...
    if function.result.scalar().is_none()
        ...
        || selected.local_storage_slots.iter().any(|slot| {
            !matches!(
                ...
                && !matches!(slot.id, target_operations_to_selected_instructions::LocalStorageSlotId::Structural { operation, place }
        ...
        || selected.calls.iter().any(|call| {
            call.call.arguments.iter().any(|argument| {
                matches!(argument, target_operations_to_selected_instructions::legalized_operations::LegalizedScalarArgument::Structural { semantic, target }
                    if !primitive_local(function, semantic.place)
                        || !matches!(target.source, abstract_operations_to_target_operations::target_operations::TargetStructuralArgumentSource::EstablishedPrimitiveLocal { .. })
    ...
    let unused = |successor: &target_operations_to_selected_instructions::SelectedSuccessor| {
        successor
            .structural_bindings
            .iter()
