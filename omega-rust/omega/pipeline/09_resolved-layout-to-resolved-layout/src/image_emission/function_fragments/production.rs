...
pub fn build_function_fragment_object_artifact_with_private_functions(
    replay: Arc<StagedOptimizedRelocationFreeObjectContainer>,
    private_functions: &[post_allocation_machine_to_selected_form_encoding::machine_code::CompilerPrivateMachineCodeFunction],
) -> Result<ObjectArtifact, Error> {
    ...
    for placed in &text.functions {
            ...
            .filter(|call| call.caller == placed.machine)
        {
            ...
            let owner = abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(call.operation);
    ...
    let validated_private = crate::image_emission::object_artifact::validate_private_functions(
        text.target,
        private_functions,
    )
    .map_err(Error::PrivateFunctions)?;
    let private_carriers = crate::image_emission::object_artifact::emit_private_functions(
        validated_private,
        &mut object,
        &mut text_bytes,
    )
    ...
    let foreign_calls = crate::image_emission::derive_normalized_foreign_call_custody(source)
