...
use crate::image_emission::{ObjectBoundarySettlement, ObjectFunction};
...
use abstract_operations_to_target_operations::calling_conventions::{
    IndirectPointerLocation, ValueLocation, ValuePlacement,
};
...
use target_operations_to_selected_instructions::legalized_operations::{
    LegalizedScalarArgument, NativeCallOrigin,
};
...
fn fragment(
    source: &StagedOptimizedRelocationFreeObjectContainer,
    machine: MachineId,
) -> Result<&post_allocation_machine_to_selected_form_encoding::machine_code::FunctionFragment, Error>
{
...
fn copy_extent(
    function: &SelectedFunction,
    fragment: &post_allocation_machine_to_selected_form_encoding::machine_code::FunctionFragment,
...
pub(super) fn settlement_attributions(
    ...
    function
        ...
        .filter(|row| {
            matches!(
                row.settlement,
                target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::ClaimCompletion(_)
            )
        })
        .map(|row| {
...
pub(super) fn settlements(
    ...
    for placed in &source.source().text_section().functions {
        ...
        for located in &function.boundary_settlements {
            let target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::ClaimCompletion(row) =
                &located.settlement
            else {
                result.push(match located.settlement {
                    target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::HostedReadByte { .. } => byte_input::settlement(source, placed.machine, located)?,
                    target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::HostedExitProcessI32 { .. } => process_exit::settlement(source, placed.machine, located)?,
                    _ => byte_output::settlement(source, placed.machine, located)?,
                });
                continue;
            };
            let offset = settlement_offset(function, fragment, located)?;
            let execution = post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryExecutionRecord::AdmittedProvider(
            ...
            result.push(ObjectBoundarySettlement {
                ...
                settlement: post_allocation_machine_to_selected_form_encoding::machine_code::BoundarySettlementRecord {
                    ...
                    execution,
                    realization: abstract_operations_to_target_operations::target_operations::BoundaryRealization::ClaimCompletionOnly(
                        row.realization,
                    ),
                    ...
                    completion_provider_custody: post_allocation_machine_to_selected_form_encoding::machine_code::derive_completion_provider_custody(
                        execution,
