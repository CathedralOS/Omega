...
use abstract_operations_to_target_operations::calling_conventions::{
    CallSignature, CallingPolicy, ValueLocation, evaluate_call_plan,
};
    ...
    let (source_is_exact, byte_size, bits) = match store.source {
        ...
        InternalUnitScalarArgumentSourceRecord::Parameter {
            ...
            let (scalar_shape, byte_size) =
                crate::image_emission::object_artifact::replay::unit::write_only_primitive_store::native_scalar_shape(scalar_type)?;
            ...
            let mut parameter_shapes = abi
                ...
                .iter()
                .map(|parameter| {
                    let (shape, _) = crate::image_emission::object_artifact::replay::unit::write_only_primitive_store::native_scalar_shape(
        ...
        InternalUnitScalarArgumentSourceRecord::Home(source_home) => {
            ...
            let home_count = function
                ...
                .count();
            let (shape, byte_size) = crate::image_emission::object_artifact::replay::unit::write_only_primitive_store::native_scalar_shape(
                source_home.scalar_type,
            )?;
            (
...
pub(crate) fn expected_parameter_store_bytes(
    ...
    field_byte_offset: u32,
    byte_size: u16,
    source: abstract_operations_to_target_operations::target_operations::MachineRegister,
) -> Option<Vec<u8>> {
    match target.architecture {
        Architecture::X86_64 => {
    ...
    match target.architecture {
        Architecture::X86_64 => {
            ...
            bytes.extend(expected_parameter_store_bytes(
                ...
                field_byte_offset,
                byte_size,
                abstract_operations_to_target_operations::target_operations::MachineRegister::X86R11,
            )?);
            Some(bytes)
        }
        Architecture::Aarch64 => {
            ...
            bytes.extend(expected_parameter_store_bytes(
                ...
                field_byte_offset,
                byte_size,
                abstract_operations_to_target_operations::target_operations::MachineRegister::Aarch64X(16),
            )?);
            Some(bytes)
        }
    }
...
pub(crate) fn expected_home_store_bytes(
    ...
    field_byte_offset: u32,
    byte_size: u16,
    source: post_allocation_machine_to_selected_form_encoding::machine_code::UnitScalarHomeRecord,
) -> Option<Vec<u8>> {
    match target.architecture {
        Architecture::X86_64 => {
            let mut bytes = Vec::new();
