...
use abstract_operations_to_target_operations::calling_conventions::{ValueClass, ValueLocation};
use abstract_operations_to_target_operations::target_operations::{
    CallSiteOwner, TargetStructuralHomeLayout,
};
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    InternalUnitCallRecord, UnitParameterHomeRecord, UnitScalarHomeRecord,
};
...
pub(crate) fn parameter_storage_end(
    ...
    scalar_abi: Option<&post_allocation_machine_to_selected_form_encoding::machine_code::ParameterFunctionAbiRecord>,
) -> Option<u32> {
    ...
    let Ok(caller_plan) = abstract_operations_to_target_operations::calling_conventions::evaluate_call_plan(
...
pub(crate) fn exact_storage(
    ...
    let Some(entry_end) =
        crate::image_emission::object_artifact::replay::unit::scalar_call_custody::entry_spills::storage_end(
            target,
            parameter_homes,
            scalar_abi,
            has_continuations,
