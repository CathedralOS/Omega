//! Hosted runtime scalar custody replay: selected byte output, hosted read and
//! write byte roots, and process-exit settlement at native boundaries.

use abstract_operations_to_target_operations::calling_conventions::{
    MachineRegister, ValueLocation, ValueShape,
};
use abstract_operations_to_target_operations::target_operations::CompilerBuiltinExecution;
...
pub(crate) fn decode_selected_byte_output(
    ...
    if target == NativeTarget::linux_x64() {
        target_operations_to_selected_instructions::isa_x86_64::decode_x86_64_selected_hosted_write_byte_i32(bytes)
    } else if target == NativeTarget::linux_arm64() || target == NativeTarget::macos_arm64() {
        target_operations_to_selected_instructions::isa_aarch64::decode_aarch64_selected_hosted_write_byte_i32(target, bytes)
    } else {
        None
    }
}
        ...
        .filter_map(|read| {
            ...
            let exact_layout = result.layout.tag_byte_offset == 0
                ...
                && result.layout.cases[1].fields.as_slice()
                    == [abstract_operations_to_target_operations::calling_conventions::PackedFieldLayout {
            ...
            let exact_consumer = settlements.iter().any(|write| {
                matches!(
                    ...
                    && matches!(
                        ...
                        [post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarArgumentRecord {
...
pub(crate) fn hosted_write_byte_custody_is_exact(
    ...
    let Some(materialization) =
        crate::image_emission::object_artifact::call_sites::expected_foreign_scalar_argument_bytes(
            target, argument, 0,
        )
    else {
        return false;
    ...
    let suffix = match target.architecture {
        Architecture::X86_64 => target_operations_to_selected_instructions::isa_x86_64::encode_hosted_write_byte_i32_from_r11(),
        Architecture::Aarch64 => {
            let Ok(bytes) = target_operations_to_selected_instructions::isa_aarch64::encode_hosted_write_byte_i32_from_w9() else {
                return false;
            };
            bytes
        }
...
fn structural_payload_source_is_exact(
    ...
    match target.architecture {
        Architecture::X86_64 => {
            if crate::image_emission::object_artifact::replay::instruction_loads::x86_replay_rsp_load(
                &mut expected,
                0,
                tag_offset,
                4,
        ...
        Architecture::Aarch64 => {
            let Some(load) =
                crate::image_emission::object_artifact::replay::instruction_loads::aarch64_replay_stack_load(
                    9, tag_offset, 4,
                )
            else {
                return false;
...
mod tests {
    ...
    fn immediate_case(
        ...
        let suffix = match target.architecture {
            Architecture::X86_64 => target_operations_to_selected_instructions::isa_x86_64::encode_hosted_write_byte_i32_from_r11(),
            Architecture::Aarch64 => target_operations_to_selected_instructions::isa_aarch64::encode_hosted_write_byte_i32_from_w9().unwrap(),
        };
        ...
        (
            BoundarySettlementRecord {
                ...
                native_result: post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryResultRecord::Unit,
                operation_ordinal: 1,
                code_offset: 0,
    ...
    fn home_source_requires_one_exact_retained_home_and_preceding_producer() {
        ...
        let mut bytes = materialization;
        bytes.extend_from_slice(&target_operations_to_selected_instructions::isa_x86_64::encode_hosted_write_byte_i32_from_r11());
