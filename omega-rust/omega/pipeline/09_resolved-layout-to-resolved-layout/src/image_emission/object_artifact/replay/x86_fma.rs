...
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    MachineCodeFunction, X86ScalarFmaFormat,
};
use target::{
...
pub(crate) fn validate_x86_scalar_fma_function(
    ...
    for fragment in &function.x86_scalar_fma {
        ...
        let decoded =
            target_operations_to_selected_instructions::isa_x86_64::decode_vfmadd132_scalar(bytes)
                .map_err(|_| ObjectError::InvalidX86ScalarFmaEncoding {
                    machine: function.machine,
                    offset: fragment.code_offset,
                })?;
        let format = match decoded.format {
            target_operations_to_selected_instructions::isa_x86_64::DecodedScalarFmaFormat::Binary32 => X86ScalarFmaFormat::Binary32,
            target_operations_to_selected_instructions::isa_x86_64::DecodedScalarFmaFormat::Binary64 => X86ScalarFmaFormat::Binary64,
        };
        ...
        let attribution = function
            ...
            .find(|attribution| {
                attribution.operation_ordinal == occurrence.operation_ordinal
                    && attribution.site
                        == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Operation(occurrence.terminal_operation)
            })
        ...
        for operand in [occurrence.left, occurrence.right, occurrence.addend] {
            let expected = match operand.value {
                semantic_vocabulary::IeeeFloatValue::Binary32(bits) => {
                    target_operations_to_selected_instructions::isa_x86_64::encode_binary32_bits_to_xmm(bits, operand.register)
                }
                semantic_vocabulary::IeeeFloatValue::Binary64(bits) => {
                    target_operations_to_selected_instructions::isa_x86_64::encode_binary64_bits_to_xmm(bits, operand.register)
                }
            }
...
fn validate_floating_control(
    target: NativeTarget,
    function: &MachineCodeFunction,
    control: post_allocation_machine_to_selected_form_encoding::machine_code::X86FloatingControlRecord,
) -> Result<(), ObjectError> {
    let expected_save =
        target_operations_to_selected_instructions::isa_x86_64::encode_stmxcsr_rsp_displacement(
            control.saved_slot_byte_offset,
        )
        .map_err(|_| ObjectError::InvalidX86ScalarFmaFloatingControl(function.machine))?;
    let expected_store = target_operations_to_selected_instructions::isa_x86_64::encode_store_mxcsr_constant_rsp_displacement(
        control.canonical_slot_byte_offset,
        target_operations_to_selected_instructions::isa_x86_64::OMEGA_CANONICAL_MXCSR,
    )
    .map_err(|_| ObjectError::InvalidX86ScalarFmaFloatingControl(function.machine))?;
    let expected_install =
        target_operations_to_selected_instructions::isa_x86_64::encode_ldmxcsr_rsp_displacement(
            control.canonical_slot_byte_offset,
        )
        .map_err(|_| ObjectError::InvalidX86ScalarFmaFloatingControl(function.machine))?;
