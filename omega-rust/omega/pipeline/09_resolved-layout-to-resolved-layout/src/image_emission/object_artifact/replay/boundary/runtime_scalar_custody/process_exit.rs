...
pub(crate) fn decode(target: NativeTarget, bytes: &[u8]) -> Option<MachineRegister> {
    match target.architecture {
        Architecture::X86_64 => {
            target_operations_to_selected_instructions::isa_x86_64::decode_x86_64_selected_hosted_exit_process_i32(target, bytes)
        }
        Architecture::Aarch64 => {
            target_operations_to_selected_instructions::isa_aarch64::decode_aarch64_selected_hosted_exit_process_i32(target, bytes)
        }
    }
}
