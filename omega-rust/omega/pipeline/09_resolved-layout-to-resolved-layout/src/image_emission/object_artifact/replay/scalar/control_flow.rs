...
use crate::image_emission::{ObjectError, ObjectScalarStack};
...
pub(crate) fn validate_stack(
    ...
    for (index, block) in blocks.iter().enumerate() {
        ...
        let outgoing =
            crate::image_emission::object_artifact::replay::scalar::stack_regions::replay_region(
                architecture,
                machine,
                bytes,
                block.offset,
...
mod tests {
    ...
    use post_allocation_machine_to_selected_form_encoding::machine_code::{
        ScalarControlFlowEvidence, ScalarStackMutation, ScalarStackMutationKind,
    };
    ...
    fn stack(blocks: Vec<Block>) -> ScalarStackEvidence {
        ScalarStackEvidence {
            stack_alignment: 16,
            mutations: Vec::new(),
            control_flow: post_allocation_machine_to_selected_form_encoding::machine_code::ScalarControlFlowEvidence::Acyclic { blocks },
