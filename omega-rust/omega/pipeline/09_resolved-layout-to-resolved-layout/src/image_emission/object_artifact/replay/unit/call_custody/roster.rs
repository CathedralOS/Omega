...
use crate::image_emission::{ObjectError, ObjectUnitStack};
use abstract_operations_to_target_operations::target_operations::{
    CallSiteOwner, MixedStructuralScalarFunctionAbi,
};
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    MachineCodeFunction, SemanticCodeSite, StructuralReturnRecord,
};
...
pub(super) fn validate_parameter_abi(
    ...
    abi: &post_allocation_machine_to_selected_form_encoding::machine_code::ParameterFunctionAbiRecord,
) -> Result<(), ObjectError> {
    let CallInputs {
        target,
        function,
...
fn validate_mixed_argument_bytes_and_order(
    ...
    custody: &post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitCallRecord,
) -> Result<(), ObjectError> {
