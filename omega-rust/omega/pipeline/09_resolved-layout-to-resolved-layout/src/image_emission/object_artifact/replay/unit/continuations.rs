...
enum RootSource<'calls> {
    ...
    Result(&'calls post_allocation_machine_to_selected_form_encoding::machine_code::InternalStructuralCallResult),
}
...
pub(crate) fn completed_roots(
    ...
    for operation_ordinal in 0..operation_count {
        ...
        if call.operation_ordinal != operation_ordinal
            || !matches!(call.owner, abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(_))
        ...
        if let Some(result) = &call.structural_result {
            ...
            if !source.moved.is_empty()
                ...
                || call.owner != abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(defining_operation)
                || home.requirement.layout.shape() != parameter.shape
                || !matches!(
                    home.requirement.layout,
                    abstract_operations_to_target_operations::target_operations::TargetStructuralHomeLayout::Aggregate(_)
                )
...
) -> Option<&'calls post_allocation_machine_to_selected_form_encoding::machine_code::InternalStructuralCallResult>{
    let place = call
        .structural_result
        .as_ref()
...
pub(crate) fn exact_attribution(
    ...
    returned: Option<&UnitAffineCleanupRecord>,
    call_count: usize,
    attribution: &[post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeAttribution],
) -> bool {
    ...
    if continuations.is_empty() {
        let other_edges = attribution
            ...
            .filter(|row| {
                matches!(row.site,
            post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Edge(edge) if edge != returned.psi_edge)
            })
            .collect::<Vec<_>>();
        ...
        return other_edges.is_empty()
            ...
            || attribution
                .iter()
                .filter(|row| matches!(row.site, post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Operation(_)))
                .count()
                != call_count;
    }
    attribution.len() == call_count + continuations.len() + 1
        && continuations.iter().all(|continuation| {
            attribution
                .iter()
                .filter(|row| {
                    row.site == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Edge(continuation.cleanup.psi_edge)
        ...
        && attribution.last().is_some_and(|row| {
            row.site == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Edge(returned.psi_edge)
...
pub(crate) fn exact_projected_callee(
    call: &InternalUnitCallRecord,
    parameters: &[post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterRecord],
    ...
    argument.structural_type == parameter.structural_type
        ...
        && matches!(attribution, [row] if row.site == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Edge(cleanup.psi_edge)
...
pub(crate) fn validate_function(
    function: &post_allocation_machine_to_selected_form_encoding::machine_code::MachineCodeFunction,
) -> Result<Vec<PlaceId>, crate::image_emission::ObjectError> {
    let invalid =
        || crate::image_emission::ObjectError::InvalidUnitAffineCleanupEvidence(function.machine);
    ...
    for (position, continuation) in function.unit_continuations.iter().enumerate() {
        if function.provenance.edges.get(position) != Some(&continuation.cleanup.psi_edge)
            || function
                ...
                .filter(|attribution| {
                    attribution.site
                        == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Edge(continuation.cleanup.psi_edge)
    ...
    if function
        ...
        .is_none_or(|attribution| {
            attribution.site != post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Edge(returned.psi_edge)
...
pub(crate) fn exact_scalar_bindings(
    abi: Option<&post_allocation_machine_to_selected_form_encoding::machine_code::ParameterFunctionAbiRecord>,
    continuations: &[UnitContinuationRecord],
) -> bool {
    ...
    if let Some(abi) = abi {
        for parameter in &abi.parameters {
            if !matches!(parameter.scalar_type, semantic_vocabulary::ScalarType::Integer(integer)
                if crate::image_emission::object_artifact::replay::unit::scalar_call_custody::integer_shape(integer).is_some())
                || values
