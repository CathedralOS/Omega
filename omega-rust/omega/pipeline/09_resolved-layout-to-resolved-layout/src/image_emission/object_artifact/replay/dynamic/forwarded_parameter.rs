...
use abstract_operations_to_target_operations::calling_conventions::{
    CallSignature, CallingPolicy, ValueLocation, ValueShape,
};
use abstract_operations_to_target_operations::target_operations::{CallSiteOwner, MachineRegister};
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    ForwardedDynamicParameterCallRecord, MachineCodeFunction,
};
use semantic_vocabulary::MachineId;
use target::{Architecture, NativeTarget};

use crate::image_emission::ObjectError;
use post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite;

pub(crate) fn validate_forwarded_dynamic_parameter_calls(
    target: NativeTarget,
...
fn validate_call(
    ...
    let stack_matches = match &call.call_stack {
        post_allocation_machine_to_selected_form_encoding::machine_code::ForwardedDynamicParameterCallStackEvidence::Scalar(stack) => {
        ...
        post_allocation_machine_to_selected_form_encoding::machine_code::ForwardedDynamicParameterCallStackEvidence::Unit(stack) => {
    ...
    if function.attachment.is_some()
        ...
        || !matches!(
            ...
            abstract_operations_to_target_operations::target_operations::AbstractDynamicDescriptorSource::Parameter(source)
        ...
        || abstract_operations_to_target_operations::calling_conventions::validate_call_plan(
            &call.function_call_plan,
            &signature,
        )
        .is_err()
        || abstract_operations_to_target_operations::calling_conventions::validate_call_plan(
            &call.callee_call_plan,
            &signature,
        )
        .is_err()
