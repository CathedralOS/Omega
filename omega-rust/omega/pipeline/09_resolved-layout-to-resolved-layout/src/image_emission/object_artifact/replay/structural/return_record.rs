...
pub(crate) fn validate_structural_return_record(
    ...
    let expected_call_plan = abstract_operations_to_target_operations::calling_conventions::evaluate_call_plan(
    ...
    if returned.code_offset != 0
        ...
        || returned.shape.class != abstract_operations_to_target_operations::calling_conventions::ValueClass::Integer
        || !((returned.shape.byte_size == 8 && returned.shape.alignment == 8)
            || (9..=16).contains(&returned.shape.byte_size))
        || returned
            .parameters
