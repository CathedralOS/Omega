...
use super::super::unit::stack::aarch64_stack_adjustment_at;
use super::call_stack::validate_scalar_call_stack;
use super::conditional_regions::{
    collect_conditional_tree_regions, division_branches_in_region, validate_division_branch_regions,
};
use super::division_stack::replay_x86_scalar_division_region;
use super::stack_mutation::{
...
use crate::image_emission::object_artifact::call_sites::validate_internal_call_site;
use crate::image_emission::{ObjectError, ObjectScalarCallStack, ObjectScalarStack};
...
fn replay_scalar_conditional_region_with_divisions(
    ...
    require_return: bool,
    division_branches: &[ScalarDivisionBranchEvidence],
    claimed: &mut std::collections::BTreeMap<usize, ScalarStackMutation>,
    call_sites: &mut std::collections::BTreeMap<
        usize,
        post_allocation_machine_to_selected_form_encoding::machine_code::InternalCallRelocation,
    >,
    allow_calls: bool,
    evidence: &ScalarStackEvidence,
    validated_calls: &mut Vec<ObjectScalarCallStack>,
...
fn replay_scalar_conditional_terminal_region(
    ...
    let crash_offset = end.checked_sub(crash_bytes.len()).ok_or(
        ObjectError::InvalidScalarConditionalEvidence {
            machine,
            offset: start,
        },
    )?;
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn validate_conditional_tree_scalar_stack(
    architecture: Architecture,
    machine: MachineId,
    bytes: &[u8],
    ...
    crash_leaves: &[bool],
    division_branches: &[ScalarDivisionBranchEvidence],
    cleanups: &[ScalarControlAffineCleanupRecord],
...
pub(crate) fn replay_scalar_conditional_region(
    architecture: Architecture,
    machine: MachineId,
    bytes: &[u8],
    start: usize,
    ...
    evidence: &ScalarStackEvidence,
    validated_calls: &mut Vec<ObjectScalarCallStack>,
    scalar_affine_cleanup: Option<
        &post_allocation_machine_to_selected_form_encoding::machine_code::UnitAffineCleanupRecord,
    >,
) -> Result<u32, ObjectError> {
        ...
        Architecture::X86_64 => {
            ...
            while decoder.can_decode() {
                ...
                if instruction.mnemonic() == iced_x86::Mnemonic::Call {
                    ...
                    match call.owner {
                        CallSiteOwner::Operation(operation) if !allow_calls => {
                            return Err(ObjectError::ScalarConditionalCallOutsideArm {
                                machine,
                                operation,
                                offset,
                            });
                        }
                        CallSiteOwner::Operation(_) if allow_calls => {}
                        CallSiteOwner::CleanupAction { edge, .. }
                            if allow_calls
                                && scalar_affine_cleanup
                                    .is_some_and(|cleanup| cleanup.psi_edge == edge) => {}
                        _ => {
                            return Err(ObjectError::UntypedScalarInternalCall { machine, offset });
                        }
                    }
                    let call_evidence =
                        call.scalar_stack
