    ...
    for call in &function.dynamic_calls {
        ...
        let outbound_bytes = if let Some(outbound) = call.unit_stack.outbound {
            validate_stack_adjustment_pair(
                target.architecture,
                function.machine,
                Some(abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(
                    call.psi_operation,
                )),
                &function.bytes,
                outbound,
...
fn validate_stored_instance(
    ...
    if record.selection_ordinal != selection.ordinal
        ...
        || record.source.source
            != abstract_operations_to_target_operations::target_operations::TargetStructuralArgumentSource::Placement(home.source.clone())
...
fn validate_stored_argument(
    ...
    if argument.place != source.place
        ...
        || abstract_operations_to_target_operations::target_operations::TargetStructuralArgumentSource::Placement(placement.clone())
            != source.source
