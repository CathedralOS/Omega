...
use crate::image_emission::{
    ObjectError, ObjectScalarCallStack, ObjectUnitCallStack, ObjectUnitStack,
};
...
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    MachineCodeFunction, SemanticCodeAttribution, StructuralReturnRecord,
};
...
pub(super) struct CallInputs<'a> {
    ...
    pub(super) relocations: &'a [post_allocation_machine_to_selected_form_encoding::machine_code::InternalCallRelocation],
    pub(super) internal_unit_calls: &'a [post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitCallRecord],
    pub(super) parameter_homes: &'a [post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterHomeRecord],
    ...
    pub(super) callee_parameter_abi: Option<&'a post_allocation_machine_to_selected_form_encoding::machine_code::ParameterFunctionAbiRecord>,
    pub(super) callee_unit_parameters: &'a [post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterRecord],
    ...
    pub(super) custody: &'a post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitCallRecord,
    pub(super) affine_cleanup: Option<&'a post_allocation_machine_to_selected_form_encoding::machine_code::UnitAffineCleanupRecord>,
...
pub(super) struct CallSpan<'a> {
    pub(super) relocation:
        &'a post_allocation_machine_to_selected_form_encoding::machine_code::InternalCallRelocation,
    pub(super) end: usize,
...
pub(super) enum CalleeAbi<'a> {
    ...
    Parameter(&'a post_allocation_machine_to_selected_form_encoding::machine_code::ParameterFunctionAbiRecord),
...
pub(super) struct ProjectionFacts<'a> {
    ...
    pub(super) projected_result: Option<&'a post_allocation_machine_to_selected_form_encoding::machine_code::InternalStructuralCallResult>,
    pub(super) projected_home: Option<&'a post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterHomeRecord>,
}
...
impl<'a> CallInputs<'a> {
    ...
    pub(super) fn validate_call_shape(&self) -> Result<(), ObjectError> {
        ...
        if function.machine != machine
            ...
            || !matches!(
                custody.source,
                post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitCallSource::Authored
            )
            || parameter_homes
                .iter()
    ...
    pub(super) fn callee_abi(&self) -> Result<CalleeAbi<'a>, ObjectError> {
        ...
        !returned.scalar_parameters.is_empty()
            || crate::image_emission::object_artifact::replay::structural::return_record::has_claim_free_affine_identity_custody(returned)
    });
    ...
    ) -> Result<abstract_operations_to_target_operations::calling_conventions::CallPlan, ObjectError>
    {
        let CallInputs {
            target,
        ...
        let invalid = || ObjectError::InvalidInternalUnitCallEvidence(machine);
        let expected_plan = abstract_operations_to_target_operations::calling_conventions::evaluate_call_plan(
                ...
                result: if let Some(result) = custody.result {
                    ...
                    Some(match result {
                        semantic_vocabulary::ScalarType::IeeeFloat(_) => {
                            abstract_operations_to_target_operations::calling_conventions::ValueShape::float(bytes)
                        }
                        _ => abstract_operations_to_target_operations::calling_conventions::ValueShape::integer(
                            bytes,
    ...
    pub(super) fn projection_facts(
        ...
        let transferred_argument_indexes = custody
            ...
            .collect::<std::collections::BTreeSet<_>>();
        let projected_result = crate::image_emission::object_artifact::replay::structural::affine_projected_calls::exact_projected_affine_result(
        parameter_homes,
        internal_unit_calls,
        affine_cleanup,
    )
    .or_else(|| {
        let disposed = crate::image_emission::object_artifact::replay::unit::continuations::completed_roots(
            parameter_homes,
            internal_unit_calls,
            &function.unit_continuations,
            function.unit_affine_cleanup.as_ref(),
        )?;
        crate::image_emission::object_artifact::replay::unit::continuations::result_for_call(internal_unit_calls, &disposed, custody)
    });
        if custody
            .structural_result
            .as_ref()
        ...
        let projected_home = if projected_argument_indexes.is_empty()
            ...
            let expected_caller_plan = abstract_operations_to_target_operations::calling_conventions::evaluate_call_plan(
                abstract_operations_to_target_operations::calling_conventions::CallingPolicy::native_for_target(target),
                &abstract_operations_to_target_operations::calling_conventions::CallSignature {
