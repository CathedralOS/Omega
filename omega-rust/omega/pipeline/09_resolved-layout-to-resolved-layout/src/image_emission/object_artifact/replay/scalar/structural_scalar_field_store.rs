...
use abstract_operations_to_target_operations::calling_conventions::{
    IndirectPointerLocation, ValueClass, ValueLocation,
};
...
use super::super::unit::structural_scalar_field_store::integer_bits;
use crate::image_emission::ObjectError;

pub(crate) fn validate_scalar_structural_scalar_field_stores(
    target: NativeTarget,
    ...
    for (index, store) in stores.iter().enumerate() {
        let (scalar_type, width, bits) = match store.immediate {
            abstract_operations_to_target_operations::target_operations::TargetScalarImmediate::Boolean(value) => {
                (ScalarType::Boolean, 1, u64::from(u8::from(value)))
            }
            abstract_operations_to_target_operations::target_operations::TargetScalarImmediate::Integer { scalar_type, value } => (
...
fn exact_attribution_count(
    function: &MachineCodeFunction,
    store: &post_allocation_machine_to_selected_form_encoding::machine_code::ScalarStructuralScalarFieldStoreRecord,
) -> usize {
    function
