...
use abstract_operations_to_target_operations::calling_conventions::{ValuePlacement, ValueShape};
...
fn full_path_layout_keeps_root_array_metadata_and_record_root_absence() {
    ...
    for changed in [
        ...
        InternalUnitCallArgumentRecord {
            source_location: post_allocation_machine_to_selected_form_encoding::machine_code::StructuralSourceLocation::Stack { byte_offset: 1 },
            ..projected.clone()
        },
    ] {
        assert!(!exact_owned_projection(
...
fn projected_row_copy(
    destination: Vec<abstract_operations_to_target_operations::calling_conventions::ValueLocation>,
    call_stack_bytes: u32,
) -> InternalUnitCallArgumentRecord {
    use abstract_operations_to_target_operations::calling_conventions::{
        IndirectPointerLocation, MachineRegister, ValueLocation,
    };
...
fn owned_subtree_stack_fragments_have_independent_exact_copy_bytes() {
    ...
    assert_eq!(
        crate::image_emission::object_artifact::replay::unit::call_custody::expected_projected_copy_bytes(
...
fn owned_subtree_indirect_temporary_has_exact_x86_and_aarch64_bytes() {
    ...
    let post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitStructuralArgumentSourceRecord::Placement(source) =
        &mut aarch64.source
    else {
