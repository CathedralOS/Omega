...
pub(crate) fn validate_internal_call_site(
    ...
    caller: MachineId,
    bytes: &[u8],
    call: post_allocation_machine_to_selected_form_encoding::machine_code::InternalCallRelocation,
...
pub(crate) fn validate_foreign_call_site(
    ...
    caller: MachineId,
    bytes: &[u8],
    call: &post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallRelocation,
) -> Result<(RelocationKind, usize), ObjectError> {
    let valid = match architecture {
        Architecture::X86_64 => {
            call.offset >= 1
    ...
    let (
        ...
        Architecture::X86_64 => {
            ...
            (
                ...
                target_operations_to_selected_instructions::isa_x86_64::encode_stmxcsr_rsp_displacement(control.saved_slot_byte_offset)
                    .map_err(|_| invalid())?,
                target_operations_to_selected_instructions::isa_x86_64::encode_ldmxcsr_rsp_displacement(control.saved_slot_byte_offset)
                    .map_err(|_| invalid())?,
            )
        }
        Architecture::Aarch64 => {
            ...
            (
                ...
                control.restore_offset,
                control.restore_byte_count,
                target_operations_to_selected_instructions::isa_aarch64::encode_save_fpcr_to_sp_displacement(control.saved_slot_byte_offset)
                    .map_err(|_| invalid())?
                    .to_vec(),
                target_operations_to_selected_instructions::isa_aarch64::encode_restore_fpcr_from_sp_displacement(
                    control.saved_slot_byte_offset,
                )
                .map_err(|_| invalid())?
                .to_vec(),
...
pub(crate) fn validate_callback_plan_custody(
    ...
    caller: MachineId,
    call: &post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallRelocation,
    callback: &post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressMaterialization,
    ...
    let nominal_destination =
        abstract_operations_to_target_operations::calling_conventions::NativePlace::Parameter(
            application.parameter,
        );
    let expected_placement = match callback.destination {
        post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressDestination::Register(register) => {
        ...
        post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressDestination::OutgoingStack { byte_offset } => {
            abstract_operations_to_target_operations::calling_conventions::ValuePlacement {
                shape: application.shape,
                locations: vec![abstract_operations_to_target_operations::calling_conventions::ValueLocation::Stack {
...
pub(crate) fn validate_callback_address_bytes(
    ...
    match (target.architecture, callback.destination, callback.encoding) {
        (
            Architecture::X86_64,
            post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressDestination::Register(register),
            post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressEncoding::X86_64Relative32 { relocation_offset },
        ) => {
            let register =
                crate::image_emission::object_artifact::replay::instruction_loads::x86_terminal_register(register)
        ...
        (
            ...
            expected.extend_from_slice(&[0; 4]);
            crate::image_emission::object_artifact::replay::unit::scalar_call_custody::expected_x86_stack_store(
                &mut expected,
                11,
                byte_offset,
            );
        ...
        (
            Architecture::Aarch64,
            post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressDestination::Register(register),
            post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressEncoding::Aarch64PageAddress {
                page_relocation_offset,
                page_offset_relocation_offset,
            },
        ) => {
            let register =
                crate::image_emission::object_artifact::replay::instruction_loads::aarch64_terminal_register(
                    register,
                )
                .ok_or_else(invalid)?;
        ...
        (
            ...
            expected.extend_from_slice(
                &crate::image_emission::object_artifact::replay::unit::scalar_call_custody::expected_aarch64_stack_store(9, byte_offset)
                    .ok_or_else(invalid)?
                    .to_le_bytes(),
            );
        }
...
pub(crate) fn validate_foreign_scalar_arguments(
    ...
    let result_shape = call
        ...
        .map(|result| {
            ...
            let expected_shape =
                crate::image_emission::object_artifact::replay::unit::scalar_call_custody::integer_shape(
                    result_type,
                )
                .ok_or_else(invalid)?;
    ...
    let expected_boundary = abstract_operations_to_target_operations::calling_conventions::evaluate_ordinary_boundary_entry_plan(
    ...
    if call.boundary_entry_plan.call != call.call_plan
        ...
        || call.call_plan.policy != abstract_operations_to_target_operations::calling_conventions::CallingPolicy::native_for_target(target)
        || call.call_plan.entry_control != abstract_operations_to_target_operations::calling_conventions::EntryControl::CallReturn
    ...
    let operation_end = if let Some(result) = &call.scalar_result {
        ...
        let expected = crate::image_emission::object_artifact::replay::unit::scalar_call_custody::expected_unit_scalar_result_bytes(target, result)
            .ok_or_else(invalid)?;
...
pub(crate) fn expected_foreign_scalar_outbound_bytes(
    ...
    for placement in &call_plan.parameters {
        for location in &placement.locations {
            if let abstract_operations_to_target_operations::calling_conventions::ValueLocation::Stack {
                stack_byte_offset,
                byte_size,
                ..
            } = location
...
pub(crate) fn validate_foreign_scalar_source(
    ...
    argument: &post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarArgumentRecord,
) -> Result<(), ObjectError> {
    let invalid = || ObjectError::InvalidForeignCallArgument {
        caller: function.machine,
    ...
    let exact_sources = match argument.source {
        post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::SelectedBoundary { .. }
        | post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::SelectedProcessExit { .. }
        ...
        post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::Home(home) => {
            if !function.unit_scalar_homes.contains(&home) {
                return Err(invalid());
            }
            crate::image_emission::object_artifact::replay::unit::scalar_call_custody::exact_preceding_unit_scalar_home_producer_count(
                function,
                home,
...
pub(crate) fn expected_foreign_scalar_argument_bytes(
    ...
    let (register, stack) = match argument.placement.locations.as_slice() {
        [abstract_operations_to_target_operations::calling_conventions::ValueLocation::Register { register, .. }] => (
            Some(match target.architecture {
                Architecture::X86_64 => {
                    crate::image_emission::object_artifact::replay::instruction_loads::x86_terminal_register(
                        *register,
                    )?
                }
                Architecture::Aarch64 => {
    ...
    match argument.source {
        ...
        post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::Home(home) => {
            match target.architecture {
                Architecture::X86_64 => {
                    crate::image_emission::object_artifact::replay::instruction_loads::expected_x86_stack_load(
                        &mut bytes,
                        register,
                ...
                Architecture::Aarch64 => {
                    let instruction = crate::image_emission::object_artifact::replay::instruction_loads::expected_aarch64_stack_load(
                        register,
                        outbound_bytes.checked_add(home.byte_offset)?,
                        home.shape.byte_size,
                    )?;
    ...
    if let Some(offset) = stack {
        match target.architecture {
            ...
            Architecture::Aarch64 => {
                let instruction =
                    crate::image_emission::object_artifact::replay::unit::scalar_call_custody::expected_aarch64_stack_store(register, offset)?;
                bytes.extend_from_slice(&instruction.to_le_bytes());
            }
        }
    }
...
mod tests {
    ...
    fn zero_argument_foreign_call() -> (MachineCodeFunction, ForeignCallRelocation) {
        ...
        let provider_plan_commitment =
            abstract_operations_to_target_operations::task_plans::SameStackProviderPlanCommitment::from_digest([7; 32]);
        let same_stack_contribution = abstract_operations_to_target_operations::task_plans::admit_same_stack_contribution(
            abstract_operations_to_target_operations::task_plans::SameStackContributionAdmissionCandidate {
                ...
                receipt:
                    abstract_operations_to_target_operations::task_plans::SameStackContributionAdmissionReceiptId::from_normalized_identity(
                        0x4c49_4e55_0001,
                    )
        ...
        let call_offset = 5;
        let outbound =
            post_allocation_machine_to_selected_form_encoding::machine_code::StackAdjustmentPair {
                byte_size: 8,
                allocation_offset: 0,
    ...
    fn foreign_call_rejects_a_drifted_boundary_state_facet() {
        ...
        let expected_state = abstract_operations_to_target_operations::calling_conventions::evaluate_ordinary_boundary_entry_plan(
            abstract_operations_to_target_operations::calling_conventions::CallingPolicy::native_for_target(NativeTarget::linux_x64()),
            &signature,
        )
        .expect("ordinary boundary entry plan")
        .plan()
        ...
        call.boundary_entry_plan.state.saved_state = if expected_state.saved_state.is_empty() {
            abstract_operations_to_target_operations::calling_conventions::MachineStateSet::new([
                abstract_operations_to_target_operations::calling_conventions::MachineState::VectorRegisters,
            ])
        } else {
            abstract_operations_to_target_operations::calling_conventions::MachineStateSet::empty()
        };
