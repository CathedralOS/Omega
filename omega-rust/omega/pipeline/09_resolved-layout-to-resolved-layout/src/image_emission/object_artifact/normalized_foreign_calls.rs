...
pub fn derive_normalized_foreign_call_custody(
    ...
    for call in &section.unresolved_normalized_foreign_calls {
        ...
        if call.state != post_allocation_machine_to_selected_form_encoding::machine_code::NormalizedForeignCallResolutionState::UnresolvedImportFieldV1
        {
            return Err(invalid());
        }
        ...
        if !matches!(
            instruction.kind,
            target_operations_to_selected_instructions::SelectedInstructionKind::NormalizedForeignCall {
                boundary,
                ordinal,
...
fn caller_fragment(
    section: &post_allocation_machine_to_selected_form_encoding::machine_code::RelocationFreeTextSectionPlacement,
    caller: MachineId,
) -> Option<&post_allocation_machine_to_selected_form_encoding::machine_code::PlacedFunctionFragment>
{
...
) -> Option<target_operations_to_selected_instructions::SelectedInstructionId> {
    function
        .calls
        .iter()
...
fn foreign_scalar_argument_custody<F>(
    ...
    owner: CallSiteOwner,
    fixed_view: F,
) -> Result<Vec<post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarArgumentRecord>, ObjectError>
where
    F: Fn(
    ...
    ) -> bool,
{
    ...
    for (scalar_ordinal, argument) in record.call.scalar_arguments.iter().enumerate() {
        ...
        scalar_arguments.push(post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarArgumentRecord {
...
fn foreign_scalar_result_custody(
    ...
    caller: MachineId,
    owner: CallSiteOwner,
) -> Result<Option<post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarResultRecord>, ObjectError>{
    let invalid = || ObjectError::InvalidForeignCallArgument { caller, owner };
    let plan = &record.call.binding.boundary_entry_plan.call;
    ...
    Ok(Some(post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarResultRecord {
