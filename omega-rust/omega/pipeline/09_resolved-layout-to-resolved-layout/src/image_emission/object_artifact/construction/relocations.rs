...
use post_allocation_machine_to_selected_form_encoding::machine_code::MachineCodePlan;
...
fn push_table_address_relocations(
    ...
    encoding: &post_allocation_machine_to_selected_form_encoding::machine_code::DynamicTableAddressEncoding,
) -> Result<(), ObjectError> {
    ...
    for call in &function.foreign_calls {
        ...
        if let Some(callback) = &call.callback_address {
            let Some((callback_symbol, callback_symbol_plan)) =
                crate::object_file::object_function_symbol(
                    object,
...
fn relocation_record_capacity(
    ...
    let callback_relocation_count = plan
        ...
        .map(|callback| match callback.encoding {
            post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressEncoding::X86_64Relative32 { .. } => 1,
            post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressEncoding::Aarch64PageAddress { .. } => 2,
        })
        .sum::<usize>();
    ...
    let dynamic_address_relocation_count = plan
        ...
        .map(|call| match call.table_address.encoding {
            post_allocation_machine_to_selected_form_encoding::machine_code::DynamicTableAddressEncoding::X86_64Relative32 { .. } => 1,
            post_allocation_machine_to_selected_form_encoding::machine_code::DynamicTableAddressEncoding::Aarch64PageAddress { .. } => 2,
        })
        .sum::<usize>();
