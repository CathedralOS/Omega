...
use crate::image_emission::object_artifact::replay::boundary::byte_sequence_custody::linux_write_line_custody_is_exact;
use crate::image_emission::object_artifact::replay::boundary::completion_receipts::{
    CompletionCustodyError, validate_completion_custody,
};
use crate::image_emission::object_artifact::replay::boundary::result_placement::boundary_result_is_exact;
use crate::image_emission::object_artifact::replay::boundary::runtime_scalar_custody::hosted_write_byte_custody_is_exact;
use crate::image_emission::object_artifact::replay::dynamic::conformance::{
    validate_dynamic_calls, validate_stored_dynamic_calls,
};
use crate::image_emission::object_artifact::replay::scalar::cleanup_preservation::validate_scalar_cleanup_preservation;
use crate::image_emission::object_artifact::replay::scalar::conditional_call_paths::{
    conditional_call_path, conditional_paths_are_exclusive,
};
use crate::image_emission::object_artifact::replay::scalar::control_cleanup::{
    cleanup_for_owner, validate_scalar_control_cleanup_evidence,
};
use crate::image_emission::object_artifact::replay::scalar::stack::validate_scalar_stack;
use crate::image_emission::object_artifact::replay::scalar::structural_scalar_field_store::validate_scalar_structural_scalar_field_stores;
use crate::image_emission::object_artifact::replay::structural::affine_projected_calls::{
    exact_fully_consumed_affine_parameter, exact_partially_consumed_affine_parameter,
};
use crate::image_emission::object_artifact::replay::structural::return_record::validate_structural_return_record;
use crate::image_emission::object_artifact::replay::unit::affine_cleanup::validate_unit_affine_cleanup;
use crate::image_emission::object_artifact::replay::unit::call_custody::{
...
use crate::image_emission::object_artifact::replay::unit::dynamic_descriptor_join::validate_unit_dynamic_descriptor_join;
use crate::image_emission::object_artifact::replay::unit::installed_provider_scalar_call::validate_installed_provider_unit_scalar_calls;
use crate::image_emission::object_artifact::replay::unit::scalar_call_custody::validate_internal_unit_scalar_calls;
use crate::image_emission::object_artifact::replay::unit::stack::{
...
use crate::image_emission::object_artifact::replay::unit::structural_scalar_field_store::validate_unit_structural_scalar_field_stores;
use crate::image_emission::object_artifact::replay::unit::write_only_primitive_store::validate_unit_write_only_primitive_stores;
...
use abstract_operations_to_target_operations::target_operations::{
    BoundaryRealization, CallSiteOwner,
};
...
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    MachineCodePlan, SemanticCodeSite,
};
...
struct AffineParameterFacts<'a> {
    scalar_custody: bool,
    parameter_homes: &'a [post_allocation_machine_to_selected_form_encoding::machine_code::UnitParameterHomeRecord],
    default_affine_cleanup: Option<&'a post_allocation_machine_to_selected_form_encoding::machine_code::UnitAffineCleanupRecord>,
...
fn validate_unit_custody_identities(function: &MachineCodeFunction) -> Result<(), ObjectError> {
    let is_unit_custody_relocation = |call: &&post_allocation_machine_to_selected_form_encoding::machine_code::InternalCallRelocation| {
            ...
            count.checked_add(
                function
                    ...
                    .filter(|call| {
                        matches!(
                            call.call_stack,
                            post_allocation_machine_to_selected_form_encoding::machine_code::ForwardedDynamicParameterCallStackEvidence::Unit(_)
                        )
                    })
                    .count(),
            )
...
fn affine_parameter_facts<'a>(
    ...
    let continuation_discards =
        crate::image_emission::object_artifact::replay::unit::continuations::validate_function(
            function,
        )?;
    ...
    if !function.unit_continuations.is_empty()
        && !crate::image_emission::object_artifact::replay::unit::scalar_call_custody::entry_spills::exact_prologue(
            plan.target,
            &function.bytes,
            function.parameter_abi.as_ref(),
            parameter_homes,
...
fn validate_internal_unit_call_custodies(
    ...
    for custody in &function.internal_unit_calls {
        ...
        let affine_cleanup = cleanup_for_owner(
            &function.scalar_control_affine_cleanups,
            custody.owner,
        )
        .or_else(|| {
            crate::image_emission::object_artifact::replay::unit::continuations::cleanup_for_call(
...
fn validate_function_shape(
    ...
    if function
        ...
        .any(|home| {
            matches!(
                home.location,
                post_allocation_machine_to_selected_form_encoding::machine_code::StructuralSourceLocation::IncomingIndirectPointer { .. }
                    | post_allocation_machine_to_selected_form_encoding::machine_code::StructuralSourceLocation::IncomingIndirectStackPointer { .. }
                    | post_allocation_machine_to_selected_form_encoding::machine_code::StructuralSourceLocation::IncomingBorrowedPointer { .. }
            )
        })
    ...
    crate::image_emission::object_artifact::replay::x86_fma::validate_x86_scalar_fma_function(
        plan.target,
        x86_feature_profile,
        x86_scalar_fma_provider,
        function,
    ...
    for settlement in &function.boundary_settlements {
        ...
        let valid_realization = match settlement.realization {
            ...
            BoundaryRealization::DirectPortReadU8(realization) => {
                let expected = crate::image_emission::x86_encoding::encode_immediate_port_read_u8(realization.port);
            ...
            BoundaryRealization::HostedExitProcessI32(_) => {
                if settlement.runtime_scalar_arguments.iter().any(|argument| matches!(argument.source, post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::SelectedProcessExit { .. })) {
                    crate::image_emission::object_artifact::replay::boundary::runtime_scalar_custody::process_exit::bytes_are_exact(plan.target, settlement, &function.bytes)
                        && function.scalar_stack.is_none()
                ...
                let expected = value.and_then(|value| match plan.target.architecture {
                    Architecture::X86_64 => {
                        Some(target_operations_to_selected_instructions::isa_x86_64::encode_hosted_exit_process_i32(value))
                    }
                    Architecture::Aarch64 => {
                        target_operations_to_selected_instructions::isa_aarch64::encode_hosted_exit_process_i32(plan.target, value).ok()
                    }
                });
            ...
            BoundaryRealization::HostedReadByte(_) => {
                let expected = settlement.native_result.structural().and_then(|result| {
                    ...
                    match plan.target {
                        target if target == NativeTarget::linux_x64() => target_operations_to_selected_instructions::isa_x86_64::encode_linux_read_byte_to_stack(
                            result.home_byte_offset,
                            payload,
                        )
                        .ok(),
                        target if target == NativeTarget::linux_arm64() => target_operations_to_selected_instructions::isa_aarch64::encode_linux_read_byte_to_stack(
                            result.home_byte_offset,
                            payload,
                        )
                        .ok(),
                        target if target == NativeTarget::macos_arm64() => target_operations_to_selected_instructions::isa_aarch64::encode_macos_read_byte_to_stack(
                            result.home_byte_offset,
                            payload,
                        )
                        .ok(),
