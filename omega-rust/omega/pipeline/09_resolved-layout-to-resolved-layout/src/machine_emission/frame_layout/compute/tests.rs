...
fn narrow_outgoing_payload_aligns_preservation_without_activation_locals() {
    for (target, abi, stack_offset, save_name) in [
        ...
        let outgoing = target_operations_to_selected_instructions::SelectedOutgoingArgumentSlot {
            id: target_operations_to_selected_instructions::OutgoingArgumentSlotId {
                role:
        ...
        let save = CalleeSaveFrameSlot {
            abstract_slot: post_allocation_machine_to_selected_form_encoding::machine_code::NonAuthoritativeCalleeSaveSlotId(0),
...
fn activation_locals_are_not_outgoing_abi_storage_on_any_host() {
    let id = target_operations_to_selected_instructions::LocalStorageSlotId::Structural {
        operation: semantic_vocabulary::OperationId::new(7).unwrap(),
        place: semantic_vocabulary::PlaceId::new(11).unwrap(),
    };
    let local = target_operations_to_selected_instructions::SelectedLocalStorageSlot {
        id,
        byte_size: 24,
        alignment: 8,
    };
...
fn outgoing_pointer_slots_reserve_storage_on_every_host_abi() {
    for (target, abi, offset) in [
        ...
        let environment =
            target_operations_to_selected_instructions::register_environment::baseline_target_register_environment(target).unwrap();
        let slot = target_operations_to_selected_instructions::SelectedOutgoingArgumentSlot {
            id: target_operations_to_selected_instructions::OutgoingArgumentSlotId {
                role:
                    target_operations_to_selected_instructions::OutgoingArgumentSlotRole::Argument,
                operation: semantic_vocabulary::OperationId::new(11).unwrap(),
...
fn large_frames_commit_through_an_exact_probe_roster() {
    let local = target_operations_to_selected_instructions::SelectedLocalStorageSlot {
        id: target_operations_to_selected_instructions::LocalStorageSlotId::Structural {
            operation: semantic_vocabulary::OperationId::new(7).unwrap(),
            place: semantic_vocabulary::PlaceId::new(11).unwrap(),
        },
        byte_size: 5_000,
    ...
    for (target, abi, interval, touches) in [
        ...
        let small = target_operations_to_selected_instructions::SelectedLocalStorageSlot {
            byte_size: 24,
            ..local.clone()
        };
...
fn system_v_leaf_spill_storage_inside_the_red_zone_commits_nothing() {
    let environment = target_operations_to_selected_instructions::register_environment::baseline_target_register_environment(
        target::NativeTarget::linux_x64(),
    )
    .unwrap();
    let spill =
        |register: u32| target_operations_to_selected_instructions::SelectedLocalStorageSlot {
            id: target_operations_to_selected_instructions::LocalStorageSlotId::Spill {
                register: target_operations_to_selected_instructions::VirtualRegisterId(register),
            },
            byte_size: 8,
            alignment: 8,
        };
...
fn red_zone_residency_requires_leaf_spill_only_sysv_storage() {
    ...
    let structural = target_operations_to_selected_instructions::SelectedLocalStorageSlot {
        id: target_operations_to_selected_instructions::LocalStorageSlotId::Structural {
            operation: semantic_vocabulary::OperationId::new(7).unwrap(),
            place: semantic_vocabulary::PlaceId::new(11).unwrap(),
        },
    ...
    let sysv = target_operations_to_selected_instructions::register_environment::baseline_target_register_environment(
        target::NativeTarget::linux_x64(),
    )
    .unwrap();
