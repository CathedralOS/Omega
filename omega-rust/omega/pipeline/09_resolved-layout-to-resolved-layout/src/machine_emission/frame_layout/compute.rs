use target_operations_to_selected_instructions::MachineAlternativeFamily;
            ...
            |((machine_function, requirement_function), storage_function)| {
                ...
                let stable_address_loans = machine_function
                    ...
                    .filter_map(|instruction| match instruction.address {
                        Some(register_homes_to_post_allocation_machine::PhysicalAddressOperation::FrameAddress {
...
fn function_layout(
    ...
    contains_call: bool,
    outgoing_arguments: &[target_operations_to_selected_instructions::SelectedOutgoingArgumentSlot],
    local_storage: &[target_operations_to_selected_instructions::SelectedLocalStorageSlot],
    stable_address_loans: Vec<target_operations_to_selected_instructions::LocalStorageSlotId>,
    ...
    let outgoing_abi_area =
        post_allocation_machine_to_selected_form_encoding::machine_code::OutgoingAbiFrameArea {
    ...
    for (index, slot) in local_storage.iter().enumerate() {
        ...
        local_extent = frame_offset_bytes
            ...
            .ok_or(TargetFrameLayoutError::GeometryOverflow)?;
        local_storage_slots.push(post_allocation_machine_to_selected_form_encoding::machine_code::LocalStorageFrameSlot {
            id: slot.id,
            frame_offset_bytes,
    ...
    let preservation_offset = if local_storage_slots.is_empty() && callee_save_slots.is_empty() {
        outgoing_extent
    } else {
...
mod spill_tests {
    ...
    fn compiler_spill_slots_expand_final_frame_without_aliasing_outgoing_or_other_locals() {
        for target in [
            ...
            let abi = target_operations_to_selected_instructions::register_environment::selected_abi_preservation(&environment)
                .unwrap()
                .kind;
            let slots = [0, 1].map(|register| {
                target_operations_to_selected_instructions::SelectedLocalStorageSlot {
                    id: target_operations_to_selected_instructions::LocalStorageSlotId::Spill {
