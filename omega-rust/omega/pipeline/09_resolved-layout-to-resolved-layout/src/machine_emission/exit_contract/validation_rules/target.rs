...
use target_operations_to_selected_instructions::register_model::{
    PreservationConvention, ValidatedPhysicalRegisterModel,
};

use super::super::{WholeFunctionExitPolicy, error::WholeFunctionExitContractError};

#[derive(Clone, Copy)]
pub(in crate::machine_emission::exit_contract) enum EntryAssumptionKind {
    ActivationStack,
    LinkRegister,
}

pub(in crate::machine_emission::exit_contract) fn target_contract_inputs(
...
pub(in crate::machine_emission::exit_contract) fn view<'model>(
    physical: &'model ValidatedPhysicalRegisterModel,
    name: &'static str,
) -> Result<
    &'model target_operations_to_selected_instructions::register_model::RegisterView,
    WholeFunctionExitContractError,
> {
...
pub(in crate::machine_emission::exit_contract) fn scalar_return_view(
    physical: &ValidatedPhysicalRegisterModel,
    target: NativeTarget,
    registers: &[target_operations_to_selected_instructions::VirtualRegister],
    instruction: &target_operations_to_selected_instructions::SelectedInstruction,
    integer_result: target_operations_to_selected_instructions::register_model::RegisterViewId,
) -> Result<
    target_operations_to_selected_instructions::register_model::RegisterViewId,
    WholeFunctionExitContractError,
> {
    if instruction.kind
        != target_operations_to_selected_instructions::SelectedInstructionKind::ReturnScalar
    {
