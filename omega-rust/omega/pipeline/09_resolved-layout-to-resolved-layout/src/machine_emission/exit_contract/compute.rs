        ...
        for block in &function.blocks {
            ...
            for (index, machine_instruction) in machine_block.instructions.iter().enumerate() {
                ...
                let relaxed_less_than_alternative = matches!(
                    ...
                    (
                        target_operations_to_selected_instructions::SelectedInstructionKind::ConditionalBranchU64LessThan,
                        target_operations_to_selected_instructions::MachineAlternativeFamily::ConditionalBranchU64LessThan,
                    ) | (
                        target_operations_to_selected_instructions::SelectedInstructionKind::ConditionalBranchI64LessThan,
                        target_operations_to_selected_instructions::MachineAlternativeFamily::ConditionalBranchI64LessThan,
                    )
                )
                    ...
                    && terminal.id == instruction.id
                {
                    ...
                    crashes.push(post_allocation_machine_to_selected_form_encoding::machine_code::WholeFunctionCrashEvidence {
                        block: block.id,
                        psi_edge: *psi_edge,
                        cause: *cause,
                    ...
                    && terminal.id == instruction.id
                {
                    ...
                    process_exits.push(post_allocation_machine_to_selected_form_encoding::machine_code::WholeFunctionProcessExitEvidence {
                        block: block.id,
