...
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    X86ScalarFmaFormat, X86ScalarFmaFragment,
};
...
pub fn emit_feature_required_x86_scalar_fma(
    ...
    let bytes = match format {
        X86ScalarFmaFormat::Binary32 => {
            target_operations_to_selected_instructions::isa_x86_64::encode_vfmadd132ss(
                destination,
                addend,
                multiplicand,
            )?
        ...
        X86ScalarFmaFormat::Binary64 => {
            target_operations_to_selected_instructions::isa_x86_64::encode_vfmadd132sd(
                destination,
                addend,
                multiplicand,
            )?
