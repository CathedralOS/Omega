...
pub(super) fn decode_manifest_content(
    ...
    let frame = match cursor.byte()? {
        ...
        2 => FunctionRelativeFrameDisposition::CanonicalFixedFrameV1 {
            layout: post_allocation_machine_to_selected_form_encoding::machine_code::TargetFrameLayoutIdentity::from_bytes(cursor.array()?),
            protocol: post_allocation_machine_to_selected_form_encoding::machine_code::TargetFrameProtocolEncodingIdentity::from_bytes(
                cursor.array()?,
            ),
        },
