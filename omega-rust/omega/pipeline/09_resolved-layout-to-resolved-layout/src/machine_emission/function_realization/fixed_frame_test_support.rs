        ...
        match field {
            ...
            FixedFramePublicationCustodyFieldForTest::Storage => {
                self.custody.storage =
                    crate::machine_emission::frame_layout::NonAuthoritativeCalleeSaveStorageIdentity::from_bytes(
                        [0xa6; 32],
                    );
            }
            FixedFramePublicationCustodyFieldForTest::Frame => {
                self.custody.frame =
                    post_allocation_machine_to_selected_form_encoding::machine_code::TargetFrameLayoutIdentity::from_bytes([0xa1; 32]);
            }
            FixedFramePublicationCustodyFieldForTest::Protocol => {
                self.custody.protocol =
                    post_allocation_machine_to_selected_form_encoding::machine_code::TargetFrameProtocolEncodingIdentity::from_bytes([0xa2; 32]);
            }
            FixedFramePublicationCustodyFieldForTest::ExitContract => {
                self.custody.exit_contract =
                    post_allocation_machine_to_selected_form_encoding::machine_code::WholeFunctionExitContractIdentity::from_bytes([0xa3; 32]);
            }
...
pub fn corrupt_fixed_frame_realization_encoding_for_test(
    ...
    let row = staged
        ...
        .find(|row| {
            matches!(
                row.state,
                post_allocation_machine_to_selected_form_encoding::machine_code::SelectedFormEncodingState::Encoded { .. }
            )
        })
        .expect("fixed-frame fixture has encoded selected forms");
    let post_allocation_machine_to_selected_form_encoding::machine_code::SelectedFormEncodingState::Encoded { bytes, .. } = &mut row.state else {
        unreachable!()
    };
    bytes[0] ^= 1;
}
...
pub fn corrupt_fixed_frame_realization_exit_for_test(
    ...
    staged.exit_contract.contract_mut().result_view =
        target_operations_to_selected_instructions::register_model::RegisterViewId(u16::MAX);
}
