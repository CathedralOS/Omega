...
pub(super) fn foreign_call_custody_digest(
    calls: &[crate::image_emission::ObjectForeignCall],
) -> [u8; 32] {
    let mut digest = Sha256::new();
    ...
    for call in calls {
        ...
        match call.owner {
            abstract_operations_to_target_operations::target_operations::CallSiteOwner::Operation(operation) => {
                digest.update([1]);
