...
use crate::native_artifact::OptimizedBoundaryOccurrence;
use crate::native_artifact::physical::derivation::evidence::{ranges_overlap, span};
pub(crate) use crate::native_artifact::physical::native_byte_span;
use crate::native_artifact::{NativeProviderExecution, NativeSelectedProviderPlan};
...
use post_allocation_machine_to_selected_form_encoding::machine_code::PortEffectRecord;
...
pub(crate) fn rejoin_admitted_provider_execution(
    ...
    execution_record: post_allocation_machine_to_selected_form_encoding::machine_code::ProviderExecutionRecord,
...
pub(crate) fn metadata_port_effect_custody(
    ...
    object: &crate::image_emission::ObjectArtifact,
    image: &crate::image::EmittedImageOutput,
    occurrence: &OptimizedBoundaryOccurrence,
    settlement: &post_allocation_machine_to_selected_form_encoding::machine_code::BoundarySettlementRecord,
    realization: &abstract_operations_to_target_operations::target_operations::MetadataOnlyPortRealization,
    target: NativeTarget,
    function: &crate::image_emission::ObjectFunction,
    ...
    if !function
        ...
        || effect.byte_count != crate::image_emission::x86_encoding::IMMEDIATE_PORT_WRITE_WIDTH
    {
    ...
    let expected =
        crate::image_emission::x86_encoding::encode_immediate_port_write(effect.port, effect.value);
...
pub(crate) fn direct_port_read_custody(
    ...
    occurrence: &OptimizedBoundaryOccurrence,
    settlement: &post_allocation_machine_to_selected_form_encoding::machine_code::BoundarySettlementRecord,
    realization: &abstract_operations_to_target_operations::target_operations::DirectPortReadU8Realization,
    target: NativeTarget,
    function: &crate::image_emission::ObjectFunction,
    operation: &terminal_psi::Operation,
    declaration: &terminal_psi::BoundaryMachineDeclaration,
    ...
    if target.architecture != Architecture::X86_64
        ...
        || settlement.byte_count
            != crate::image_emission::x86_encoding::IMMEDIATE_PORT_READ_U8_WIDTH
        || result.scalar_type != u8_type
        || result.placement.shape != abstract_operations_to_target_operations::calling_conventions::ValueShape::integer(1, 1)
        ...
        .filter(|attribution| {
            attribution.machine == occurrence.machine()
                && attribution.attribution.site
                    == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Edge(result.return_edge)
    ...
    let expected =
        crate::image_emission::x86_encoding::encode_immediate_port_read_u8(realization.port);
...
pub(crate) fn linux_write_line_custody_is_exact(
    ...
    let encoded = match target.architecture {
        Architecture::X86_64 => {
            target_operations_to_selected_instructions::isa_x86_64::encode_linux_write_line_literal(
                &custody.bytes,
            )
        }
        Architecture::Aarch64 => {
            target_operations_to_selected_instructions::isa_aarch64::encode_linux_write_line_literal(
                &custody.bytes,
            )
        }
    };
...
pub(crate) fn settlement_completion_custody_is_exact(
    ...
    let Some(expected) = post_allocation_machine_to_selected_form_encoding::machine_code::derive_completion_provider_custody(
