...
use crate::image_emission::ObjectForeignCall;
use crate::native_artifact::physical::fragment_publication::FragmentPublicationBinding;
use post_allocation_machine_to_selected_form_encoding::machine_code::PlacedFunctionFragment;
...
fn check(
    ...
    let exact = match target.architecture {
        Architecture::Aarch64 => {
            saved_bytes == target_operations_to_selected_instructions::isa_aarch64::encode_save_fpcr_to_sp_displacement(displacement).ok()?
                && restored_bytes
                    == target_operations_to_selected_instructions::isa_aarch64::encode_restore_fpcr_from_sp_displacement(displacement).ok()?
        }
        Architecture::X86_64 => {
            saved_bytes == target_operations_to_selected_instructions::isa_x86_64::encode_stmxcsr_rsp_displacement(displacement).ok()?
                && restored_bytes
                    == target_operations_to_selected_instructions::isa_x86_64::encode_ldmxcsr_rsp_displacement(displacement).ok()?
        }
    };
    exact.then_some(())
}
