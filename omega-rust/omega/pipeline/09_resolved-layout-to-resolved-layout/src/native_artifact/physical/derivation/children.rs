...
use crate::native_artifact::BoundaryTraitSettlementParts;
pub(crate) use crate::native_artifact::BoundaryTraitSettlementRole;
use crate::native_artifact::NativeCompilerBuiltinCatalogIdentity;
use crate::native_artifact::NativePhysicalChild;
use crate::native_artifact::NativePhysicalChildParts;
use crate::native_artifact::NativePhysicalOccurrence;
use crate::native_artifact::NormalizedForeignCallbackRelocations;
use crate::native_artifact::OptimizedBoundaryOccurrence;
use crate::native_artifact::PhysicalChildParent;
use crate::native_artifact::PhysicalRelocationDisposition;
use crate::native_artifact::physical::derivation::evidence::{
    physical_child_identity, ranges_overlap, span,
};
use crate::native_artifact::physical::derivation::hashing::sha256;
use crate::native_artifact::physical::derivation::provider_custody::{
...
use crate::native_artifact::physical::derivation::settlement_identity::{
...
use crate::native_artifact::physical::native_byte_span;
use crate::native_artifact::physical::normalized_foreign_call_import_field;
pub(crate) use crate::native_artifact::physical::normalized_foreign_call_relocation;
use crate::native_artifact::physical::normalized_foreign_callback_relocation;
...
use crate::object_file::{RelocationKind, RelocationOrigin, SectionKind};
...
use post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryExecutionRecord;
...
enum HostedScalarArgumentForm {
    ...
    RuntimeScalar {
        source: Option<fn(&post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord) -> bool>,
    },
}
...
enum HostedResultForm {
    ...
    Structural {
        bind: fn(
            &post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryStructuralResultRecord,
...
const HOSTED_BUILTIN_SETTLEMENTS: &[HostedBuiltinSettlement] = &[
    HostedBuiltinSettlement {
        ...
        supports_target: abstract_operations_to_target_operations::target_operations::HostedExitProcessI32Realization::supports_target,
        scalar_forms: &[
    ...
    HostedBuiltinSettlement {
        ...
        supports_target: abstract_operations_to_target_operations::target_operations::HostedWriteByteI32Realization::supports_target,
    ...
    HostedBuiltinSettlement {
        ...
        supports_target: abstract_operations_to_target_operations::target_operations::HostedReadByteRealization::supports_target,
...
fn process_exit_argument_destination(
    ...
    match architecture {
        Architecture::X86_64 => {
            abstract_operations_to_target_operations::target_operations::MachineRegister::X86Rdi
        }
        Architecture::Aarch64 => {
            abstract_operations_to_target_operations::target_operations::MachineRegister::Aarch64X(
                0,
            )
        }
    }
...
fn read_byte_result_bytes(
    result: &post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryStructuralResultRecord,
    occurrence: &OptimizedBoundaryOccurrence,
    target: NativeTarget,
) -> Result<Vec<u8>, &'static str> {
    if result.defining_operation != occurrence.operation()
        || result.layout.tag_byte_offset != 0
        || result.layout.tag_shape != abstract_operations_to_target_operations::calling_conventions::ValueShape::integer(4, 4)
        || result.layout.shape != abstract_operations_to_target_operations::calling_conventions::ValueShape::integer(8, 4)
        ...
        || result.layout.cases[1].fields.as_slice()
            != [abstract_operations_to_target_operations::calling_conventions::PackedFieldLayout {
                shape: abstract_operations_to_target_operations::calling_conventions::ValueShape::integer(4, 4),
                byte_offset: 4,
            }]
    {
    ...
    match target.architecture {
        Architecture::X86_64 => {
            target_operations_to_selected_instructions::isa_x86_64::encode_linux_read_byte_to_stack(result.home_byte_offset, payload_offset)
                .map_err(|_| "Linux read-byte x86-64 encoding is not reproducible")
        }
        Architecture::Aarch64 if target == NativeTarget::macos_arm64() => {
            target_operations_to_selected_instructions::isa_aarch64::encode_macos_read_byte_to_stack(result.home_byte_offset, payload_offset)
                .map_err(|_| "macOS read-byte AArch64 encoding is not reproducible")
        }
        Architecture::Aarch64 => {
            target_operations_to_selected_instructions::isa_aarch64::encode_linux_read_byte_to_stack(result.home_byte_offset, payload_offset)
...
pub(crate) fn derive_hosted_builtin_child(
    ...
    installed: &crate::image_emission::ObjectBoundarySettlement,
...
pub(crate) fn derive_normalized_foreign_child(
    ...
    foreign: &crate::image_emission::ObjectForeignCall,
    ...
    let retained_calls: &[crate::image_emission::ObjectForeignCall] = match publication {
        ...
        .filter(|attribution| {
            attribution.machine == occurrence.machine()
                && attribution.attribution.site
                    == post_allocation_machine_to_selected_form_encoding::machine_code::SemanticCodeSite::Operation(occurrence.operation())
    ...
    let (mutable_intervals, relocation) = if let Some(rejoin) = fragment_rejoin {
        ...
        let field_kind = match field.kind {
            post_allocation_machine_to_selected_form_encoding::machine_code::NormalizedForeignCallResolutionKind::X86Relative32FromNextInstructionToNormalizedForeignImportV1 => {
                RelocationKind::X86_64Relative32
            }
            post_allocation_machine_to_selected_form_encoding::machine_code::NormalizedForeignCallResolutionKind::Aarch64BranchLinkImmediate26FromInstructionToNormalizedForeignImportV1 => {
        ...
        let callback_relocations = match callback {
            None => None,
            Some(callback) => {
                let (callback_symbol, _) = crate::object_file::object_function_symbol(
                    object.object(),
                ...
                Some(match callback.encoding {
                    post_allocation_machine_to_selected_form_encoding::machine_code::CallbackAddressEncoding::X86_64Relative32 {
                        relocation_offset,
...
struct FragmentForeignCallRejoin<'a> {
    field: &'a crate::object_file::RelocationFreeObjectUnresolvedForeignCall,
    import: &'a crate::object_file::RelocationFreeObjectNormalizedImport,
    record: &'a target_operations_to_selected_instructions::SelectedNormalizedForeignCall,
}
...
fn fragment_normalized_foreign_rejoin<'a>(
        ...
        .filter(|candidate| {
            candidate.resolution.caller == occurrence.machine()
                ...
                && candidate.resolution.state
                    == post_allocation_machine_to_selected_form_encoding::machine_code::NormalizedForeignCallResolutionState::UnresolvedImportFieldV1
        })
        .collect::<Vec<_>>();
    if fields.len() != 1 {
    ...
    if import.name
        != crate::object_file::canonical_normalized_foreign_import_symbol_name(
            import.boundary,
            import.ordinal,
        )
    {
    ...
    if !matches!(
        instruction.kind,
        target_operations_to_selected_instructions::SelectedInstructionKind::NormalizedForeignCall { boundary, ordinal }
    ...
    if record.call.binding.locator != foreign.locator
        ...
        || post_allocation_machine_to_selected_form_encoding::machine_code::ProviderExecutionRecord::from(record.call.provider_execution)
...
fn fragment_scalar_argument_custody_rejoin(
    ...
    record: &target_operations_to_selected_instructions::SelectedNormalizedForeignCall,
    ...
    register_positions.sort_by_key(|position| {
        plan.parameters[*position].shape.class
        ...
        .enumerate()
    {
        ...
        let span_rejoins = match (argument.source, physical.source) {
            (
                source @ (abstract_operations_to_target_operations::target_operations::TargetUnitScalarArgumentSource::Parameter { .. }
                | abstract_operations_to_target_operations::target_operations::TargetUnitScalarArgumentSource::BlockParameter(_)
                | abstract_operations_to_target_operations::target_operations::TargetUnitScalarArgumentSource::BooleanImmediate {
                    ..
                }
                | abstract_operations_to_target_operations::target_operations::TargetUnitScalarArgumentSource::IeeeFloatImmediate {
                    ..
                }),
            ...
            (
                ...
                post_allocation_machine_to_selected_form_encoding::machine_code::InternalUnitScalarArgumentSourceRecord::Home(projected),
            ) => {
...
fn fragment_immediate_argument_span_rejoin(
    function: &target_operations_to_selected_instructions::SelectedFunction,
    block: &target_operations_to_selected_instructions::SelectedBlock,
    fragment: &post_allocation_machine_to_selected_form_encoding::machine_code::PlacedFunctionFragment,
    record: &target_operations_to_selected_instructions::SelectedNormalizedForeignCall,
    instruction: &target_operations_to_selected_instructions::SelectedInstruction,
    argument: &abstract_operations_to_target_operations::target_operations::TargetUnitScalarCallArgument,
    register_operand: usize,
    physical: &post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarArgumentRecord,
) -> bool {
    let span = match argument.placement.locations.as_slice() {
        [
            ...
            let target_operations_to_selected_instructions::VirtualRegisterOrigin::InstructionResult {
        ...
        [
            ...
            let slot = target_operations_to_selected_instructions::OutgoingArgumentSlotId {
            ...
            let Some(address_index) = block.instructions.iter().position(|candidate| {
                matches!(
                    candidate.kind,
                    target_operations_to_selected_instructions::SelectedInstructionKind::FrameAddress {
                        slot: target_operations_to_selected_instructions::FrameStorageSlotId::Outgoing(candidate),
                        byte_offset: 0,
                    } if candidate == slot
            ...
            let target_operations_to_selected_instructions::SelectedInstructionKind::Store {
                byte_offset: 0,
            ...
            if address_value.origin
                != (target_operations_to_selected_instructions::VirtualRegisterOrigin::ScalarAbiAddress {
...
fn fragment_scalar_result_custody_rejoin(
    ...
    physical: Option<&post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarResultRecord>,
) -> bool {
    let plan = &record.call.binding.boundary_entry_plan.call;
    ...
    if restore.kind
        != (target_operations_to_selected_instructions::SelectedInstructionKind::RestoreFloatingControl {
            slot: target_operations_to_selected_instructions::LocalStorageSlotId::Boundary {
                operation: record.operation,
            },
        })
    {
    ...
    *physical
        == post_allocation_machine_to_selected_form_encoding::machine_code::ForeignCallScalarResultRecord {
...
fn fragment_scalar_result_normalization_kind(
    scalar_type: ScalarType,
) -> Option<target_operations_to_selected_instructions::SelectedInstructionKind> {
    use target_operations_to_selected_instructions::SelectedInstructionKind as Kind;
    match scalar_type {
...
fn fragment_durable_home_offsets(
    ...
    for record in &function.normalized_foreign_calls {
        ...
        for argument in &record.call.scalar_arguments {
            if let abstract_operations_to_target_operations::target_operations::TargetUnitScalarArgumentSource::Home(requirement) =
                argument.source
            {
...
pub(crate) fn normalized_foreign_structural_parameter_shapes(
    ...
    plan_parameters: &[abstract_operations_to_target_operations::calling_conventions::ValuePlacement],
    pointer_shape: abstract_operations_to_target_operations::calling_conventions::ValueShape,
) -> Result<
    Option<Vec<abstract_operations_to_target_operations::calling_conventions::ValueShape>>,
    &'static str,
> {
