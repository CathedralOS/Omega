...
pub(crate) use crate::native_artifact::NormalizedForeignCallbackRelocation;
use crate::object_file::{RelocationKind, RelocationOrigin};
...
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    BoundaryExecutionRecord, PortEffectRecord,
};
    ...
    for segment in path {
        match segment {
            ...
            terminal_psi::StructuralPathSegment::RuntimeIndex { index, obligation } => {
                digest.update([5]);
                digest.update(index.get().to_le_bytes());
...
pub(crate) fn hash_sum_layout(
    digest: &mut Sha256,
    layout: &abstract_operations_to_target_operations::calling_conventions::ConventionalSumLayout,
) {
...
fn hash_packed_fields(
    digest: &mut Sha256,
    fields: &[abstract_operations_to_target_operations::calling_conventions::PackedFieldLayout],
) {
...
fn hash_boundary_result_record(
    digest: &mut Sha256,
    result: &post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryResultRecord,
) -> Result<(), &'static str> {
    match result {
        post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryResultRecord::Unit => digest.update([0]),
        post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryResultRecord::Scalar(result) => {
            digest.update([1]);
        ...
        post_allocation_machine_to_selected_form_encoding::machine_code::BoundaryResultRecord::Structural(result) => {
            digest.update([2]);
...
fn hash_machine_register(
    digest: &mut Sha256,
    register: abstract_operations_to_target_operations::calling_conventions::MachineRegister,
) {
    pub(crate) use abstract_operations_to_target_operations::calling_conventions::MachineRegister;
    match register {
...
fn hash_value_shape(
    ...
    match shape.class {
        abstract_operations_to_target_operations::calling_conventions::ValueClass::Integer => digest.update([1]),
        abstract_operations_to_target_operations::calling_conventions::ValueClass::Float => digest.update([2]),
        abstract_operations_to_target_operations::calling_conventions::ValueClass::BorrowedReference => digest.update([3]),
        ...
        abstract_operations_to_target_operations::calling_conventions::ValueClass::SystemVAggregate { first, second } => {
            ...
            for class in [first, second] {
                digest.update([match class {
                    abstract_operations_to_target_operations::calling_conventions::SystemVEightbyteClass::Integer => 1,
                    abstract_operations_to_target_operations::calling_conventions::SystemVEightbyteClass::Sse => 2,
                }]);
            }
        }
    }
...
pub(crate) fn hash_object_symbol(
    digest: &mut Sha256,
...
pub(crate) fn hash_machine_function_identity(
    digest: &mut Sha256,
    identity: abstract_operations_to_target_operations::function_identity::MachineFunctionIdentity,
) {
