...
fn exact_pair_elides_once_and_independent_replay_agrees() {
    ...
    assert_eq!(plan.actions[0].consumed.operand, 1);
    assert!(matches!(
...
fn two_pairs_pin_the_exact_nonzero_work_vector() {
    let fixture = fixture::two_pair_compare_i64_right_operand_fixture();
...
fn valid_non_candidates_are_retained_with_typed_outcomes() {
    let mut different_storage = fixture::compare_i64_right_operand_fixture();
    ...
    let mut wrong_value = fixture::compare_i64_right_operand_fixture();
    wrong_value.selected.functions[0].blocks[0].instructions[1].operands[1].virtual_register =
        VirtualRegisterId(3);
    wrong_value.source.functions[0].blocks[0].instructions[1].operands[1].virtual_register =
        VirtualRegisterId(3);
    let plan = compute(
        &wrong_value,
        OptimizationWorkBudget::new(1, 1, 1, 1, 1).unwrap(),
    ...
    let mut semantic_copy = fixture::compare_i64_right_operand_fixture();
    semantic_copy.selected.functions[0].blocks[0].instructions[0]
        .provenance
        .values
        .clear();
    let plan = compute(
    ...
    let mut missing_compare_value = fixture::compare_i64_right_operand_fixture();
    missing_compare_value.selected.functions[0].blocks[0].instructions[1]
        .provenance
        .values
        .retain(|value| *value != semantic_vocabulary::ValueId::new(1).unwrap());
    let plan = compute(
...
#[test]
fn copy_consumed_only_by_the_left_operand_is_outside_this_exact_rule() {
...
#[test]
fn compare_right_origin_must_retain_the_copy_source_value() {
    let mut proposal_fixture = fixture::compare_i64_right_operand_fixture();
    let compared = proposal_fixture.selected.functions[0].blocks[0].instructions[1].operands[1]
        .virtual_register;
    let register = proposal_fixture.selected.functions[0]
        ...
        .unwrap();
    let selected_instructions::VirtualRegisterOrigin::InstructionResult { source_value, .. } =
        &mut register.origin
    else {
        panic!("fixture compare-right value is the copy result")
    };
    *source_value = semantic_vocabulary::ValueId::new(9).unwrap();
    ...
    let valid_fixture = fixture::compare_i64_right_operand_fixture();
