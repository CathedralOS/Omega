...
use crate::{Aarch64SameViewCopyElisionDecodeError, Aarch64SameViewCopyElisionPlan};
...
fn codec_round_trip_and_malformed_envelopes_fail_closed() {
    ...
    assert_eq!(Aarch64SameViewCopyElisionPlan::decode(&encoded), Ok(plan));

    let mut wrong_magic = encoded.clone();
    wrong_magic[0] ^= 1;
    assert_eq!(
        Aarch64SameViewCopyElisionPlan::decode(&wrong_magic),
    ...
    wrong_identity[12] ^= 1;
    assert_eq!(
        Aarch64SameViewCopyElisionPlan::decode(&wrong_identity),
    ...
    trailing.push(0);
    assert_eq!(
        Aarch64SameViewCopyElisionPlan::decode(&trailing),
