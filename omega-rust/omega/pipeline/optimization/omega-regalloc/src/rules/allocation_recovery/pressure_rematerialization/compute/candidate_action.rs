...
use omega_selected_instructions::{SelectedFunction, SelectedInstructionId, VirtualRegisterId};
...
pub(super) fn derive(
    ...
    if original.kind
        != (omega_selected_instructions::SelectedInstructionKind::MaterializeI64 { value: *value })
    ...
    if !range.occurrences.iter().any(|occurrence| {
        occurrence.instruction == *defining_instruction
            && occurrence.access == omega_register_model::RegisterOperandAccess::Def
