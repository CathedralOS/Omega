...
#[test]
fn optimized_target_lowering_retains_saturating_integer_divide_parameter_custody() {
    ...
    for scalar_type in integers {
        for (target_profile, registers, stack) in boolean_equal_location_cases() {
            for (parameter_count, expected) in [(2, registers), (10, stack)] {
                ...
                let divide_operation = OperationId::new(30_005).unwrap();
                let obligation = ObligationId::new(30_009).unwrap();
                let (semantic, proof) = saturating_integer_divide_parameters_return_artifact(
                    scalar_type,
                    parameter_count,
                );
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineSaturatingIntegerDivideParameters(row),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("optimized saturating integer divide must retain its family row")
                };
                assert_eq!(row.machine(), MachineId::new(30_001).unwrap());
                assert_eq!(row.divide_operation(), divide_operation);
