use omega_isa_x86_64::{
    encode_x86_64_mov_r32_imm32_i64_materialization, x86_64_physical_register_model,
};
...
use super::{
    ...
    X86MovR32Imm32MaterializationAttemptOutcome, X86MovR32Imm32MaterializationError,
    X86MovR32Imm32MaterializationWorkAxis,
};
use omega_physical_instructions::{
        ...
        functions: vec![SelectedFunction {
            ...
            blocks: vec![SelectedBlock {
                ...
                instructions: vec![
                    ...
                    selected_materialization(2, u64::from(u32::MAX), &physical),
                    selected_materialization(3, u64::from(u32::MAX) + 1, &physical),
                    selected_materialization(4, 7, &physical),
                ],
...
fn compute(fixture: &Fixture) -> super::X86MovR32Imm32MaterializationPlan {
...
#[test]
fn selects_the_exact_u32_range_for_low_and_extended_registers() {
    let fixture = fixture();
    ...
    assert_eq!(low_action.destination.encoded_view, eax.id);
    assert_eq!(
        low_action.destination.destination_write_units,
        rax.write_units
    );
    assert_eq!(low_action.destination.encoded_storage_units, eax.units);
    assert_eq!(low_action.destination.encoded_write_units, eax.write_units);
    assert_eq!(
        low_action.destination.encoded_write_semantics,
        omega_register_model::RegisterWriteSemantics::ZeroExtendsParent
    );
...
#[test]
fn negative_signed_value_is_outside_the_zero_extended_u32_partition() {
...
fn non_r64_baseline_destination_is_rejected_before_selection() {
    ...
    assert_eq!(
        ...
        Err(
            X86MovR32Imm32MaterializationError::InvalidPhysicalDestination(SelectedInstructionId(
                1
            ))
        )
    );
...
fn independent_replay_exposes_action_corruption() {
    ...
    assert_eq!(
        ...
        Err(X86MovR32Imm32MaterializationError::ArtifactMismatch)
    );
}
...
fn every_work_axis_exhausts_at_its_exact_boundary() {
    ...
    for (budget, axis) in [
        ...
        assert_eq!(
            ...
            Err(X86MovR32Imm32MaterializationError::BudgetExceeded(axis)),
