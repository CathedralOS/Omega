//! Canonical identity producer for the selected-instruction plan.
//!
//! Owns the byte-stable encoding over which `SelectedInstructionPlanIdentity`
//! is derived; the pipeline only wraps it in the selection receipt.
...
use crate::selected_instructions::{
    SaturatingCarrier, SaturatingOperation, SelectedInstruction, SelectedInstructionKind,
    SelectedInstructionPlan, SelectedInstructionPlanIdentity, SelectedSuccessor,
};
...
fn encode_instruction(bytes: &mut Vec<u8>, instruction: &SelectedInstruction) {
    ...
    bytes.push(match instruction.kind {
        ...
        SelectedInstructionKind::TrappingInteger { form } => {
            crate::selected_instructions::trapping_family_tag(form)
        }
    ...
    match instruction.kind {
        ...
        | SelectedInstructionKind::RestoreFloatingControl { slot } => {
            contracts::frame_slot(
                bytes,
                crate::selected_instructions::FrameStorageSlotId::Local(slot),
            );
        }
...
fn encode_successor(bytes: &mut Vec<u8>, successor: &SelectedSuccessor) {
    bytes.push(match successor.role {
        crate::selected_instructions::SelectedSuccessorRole::Semantic => 0,
        crate::selected_instructions::SelectedSuccessorRole::EdgeTransferContinuation => 1,
        crate::selected_instructions::SelectedSuccessorRole::CaseDispatchContinuation => 2,
    });
    ...
    for binding in &successor.bindings {
        ...
        match binding.transport {
            crate::selected_instructions::SelectedValueTransport::Unused => bytes.push(0),
            crate::selected_instructions::SelectedValueTransport::Registers {
                argument,
                parameter,
            } => {
                bytes.push(1);
    ...
    for binding in &successor.structural_bindings {
        ...
        match binding.transport {
            crate::selected_instructions::SelectedStructuralTransport::Unused => bytes.push(0),
            ...
            crate::selected_instructions::SelectedStructuralTransport::Address {
                base,
                byte_offset,
                byte_count,
                destination,
                ...
                bytes.push(3);
                match base {
                    crate::selected_instructions::SelectedAddressBase::Register(register) => {
                        bytes.push(0);
                        bytes.extend_from_slice(&register.0.to_le_bytes());
                    }
                    crate::selected_instructions::SelectedAddressBase::Local(slot) => {
                        bytes.push(1);
...
mod tests {
    ...
    use crate::selected_instructions::{
        SaturatingCarrier, SelectedInstruction, SelectedInstructionId, SelectedInstructionKind,
        ...
        let discriminants = kinds.map(|kind| {
            let instruction = SelectedInstruction {
                ...
                constraint: RegisterConstraintKey {
                    family: crate::register_model::RegisterConstraintFamily::Instruction,
                    variant: 7,
                },
                operands: Vec::new(),
