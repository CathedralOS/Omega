...
fn decode_kind(
    ...
    Ok(match cursor.byte()? {
        ...
        tag @ (46 | 47) => {
            let byte_offset = cursor.u32()?;
            let width =
                crate::selected_instructions::PackedByteWidth::from_byte_size(cursor.byte()?)
        ...
        tag @ (17 | 18) => {
            let slot = match cursor.byte()? {
                2 => crate::selected_instructions::FrameStorageSlotId::Incoming {
                    parameter_index: cursor.u32()?,
                    abi_stack_byte_offset: cursor.u32()?,
                },
                slot_tag @ (0 | 3) => crate::selected_instructions::FrameStorageSlotId::Outgoing(
                ...
                1 => crate::selected_instructions::FrameStorageSlotId::Local(
                    decode_local_storage_slot(cursor)?,
                ),
                _ => return Err(PreAllocationMachineEffectDecodeError::InvalidField),
            };
            let byte_offset = cursor.u32()?;
...
pub fn decode_local_storage_slot(
    cursor: &mut Cursor<'_>,
) -> Result<crate::selected_instructions::LocalStorageSlotId, PreAllocationMachineEffectDecodeError>
{
    let tag = cursor.byte()?;
    if tag == 4 {
        return Ok(
            crate::selected_instructions::LocalStorageSlotId::StructuralParameter {
    ...
    if tag == 2 {
        return Ok(crate::selected_instructions::LocalStorageSlotId::Spill {
            register: crate::selected_instructions::VirtualRegisterId(cursor.u32()?),
        });
    }
    ...
    match tag {
        ...
        1 => Ok(crate::selected_instructions::LocalStorageSlotId::Boundary { operation }),
        5 => Ok(
...
fn decode_alternative_for_version(
    ...
    let applicability = match cursor.byte()? {
        ...
        5 => MachineAlternativeApplicability::AtLeastOneOperandDoesNotAliasView {
            left: cursor.u16()?,
            right: cursor.u16()?,
            excluded_view: crate::register_model::RegisterViewId(cursor.u16()?),
        },
        _ => return Err(PreAllocationMachineEffectDecodeError::InvalidField),
    };
...
fn decode_encoded_effects(
    ...
    let memory = match cursor.byte()? {
        8 => MachineEncodedMemoryEffect::HostedReadByteV1 {
            stack_pointer: crate::register_model::RegisterViewId(cursor.u16()?),
        },
    ...
    let control = match cursor.byte()? {
        ...
        3 => MachineEncodedControlEffect::ReturnIndirectRegisterV1 {
            target: crate::register_model::RegisterViewId(cursor.u16()?),
        },
...
mod local_slot_tests {
    ...
    use crate::selected_instructions::LocalStorageSlotId;
    ...
    fn packed_instruction_decode_retains_width_and_rejects_invalid_footprints() {
        for tag in [46, 47] {
            for raw in 0..=8 {
                ...
                if let Some(width) =
                    crate::selected_instructions::PackedByteWidth::from_byte_size(raw)
                {
