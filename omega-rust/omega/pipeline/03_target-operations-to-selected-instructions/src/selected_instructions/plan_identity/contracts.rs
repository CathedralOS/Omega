use super::encode_len;
use crate::selected_instructions::SelectedFunction;
use crate::selected_instructions::{OutgoingArgumentSlotId, SelectedMemoryAccessRole};
...
fn local_slot(bytes: &mut Vec<u8>, slot: crate::selected_instructions::LocalStorageSlotId) {
    slot.encode_identity(bytes);
}
pub(super) fn frame_slot(
    bytes: &mut Vec<u8>,
    slot: crate::selected_instructions::FrameStorageSlotId,
) {
    match slot {
        ...
        crate::selected_instructions::FrameStorageSlotId::Outgoing(value) => {
            bytes.push(match value.role {
                crate::selected_instructions::OutgoingArgumentSlotRole::Argument => 0,
                crate::selected_instructions::OutgoingArgumentSlotRole::ValueCopy => 3,
            });
            bytes.extend_from_slice(&value.operation.get().to_le_bytes());
            bytes.extend_from_slice(&value.argument_index.to_le_bytes());
        }
        crate::selected_instructions::FrameStorageSlotId::Local(value) => {
            bytes.push(1);
