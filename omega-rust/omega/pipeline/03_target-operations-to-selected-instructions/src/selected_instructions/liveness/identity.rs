...
use crate::selected_instructions::{FunctionLiveness, LivenessIdentity, LivenessPlan};
...
fn encode_vregs(bytes: &mut Vec<u8>, values: &[crate::selected_instructions::VirtualRegisterId]) {
...
fn encode_units(bytes: &mut Vec<u8>, values: &[crate::register_model::RegisterUnitId]) {
    encode_len(bytes, values.len());
    for value in values {
        bytes.extend_from_slice(&value.0.to_le_bytes());
    }
...
mod tests {
    ...
    use crate::register_model::RegisterUnitId;
    use crate::selected_instructions::{
        BlockLiveness, FunctionLiveness, InstructionLiveness, LivenessPosition,
    };
    use crate::selected_instructions::{
