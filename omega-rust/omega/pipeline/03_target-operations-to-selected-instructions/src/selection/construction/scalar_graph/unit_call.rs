...
pub(super) fn emit(
    ...
    let mut operands = order
        ...
        .collect::<Vec<_>>();
    let hidden = super::structural::indirect_results::prepare_call(source, operation, builder)?;
    ...
    if hidden.is_some() {
        return super::structural::indirect_results::finish_call(source, operation, builder);
    }
