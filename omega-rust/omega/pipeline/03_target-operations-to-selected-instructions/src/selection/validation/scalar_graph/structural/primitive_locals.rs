...
pub(in crate::selection) fn read(
    ...
    normalize_signed_load(replay, definition, output)
}

/// The exact-width load zero-extends raw bits. A narrow signed value is
/// restored before any consumer, not only at a subsequent ABI boundary. Only
