...
use crate::register_model::{PreservationConvention, ValidatedPreservationStorageCatalog};

use crate::register_environment::ValidatedTargetRegisterEnvironment;

pub use crate::register_model::FrameAbiPreservationConvention;
...
pub fn selected_abi_preservation(
    ...
    let (kind, convention) = match (target.architecture, target.object_format) {
        (target::Architecture::X86_64, target::ObjectFormat::Elf) => (
            ...
            crate::isa_x86_64::x86_64_preservation_convention_for_target(
        ...
        (target::Architecture::Aarch64, target::ObjectFormat::Elf) => (
            ...
            crate::isa_aarch64::aarch64_preservation_convention_for_target(
                environment.physical(),
                target,
            ),
        ),
...
pub fn selected_preservation_storage_catalog(
    ...
    match environment.target().architecture {
        target::Architecture::X86_64 => crate::isa_x86_64::x86_64_preservation_storage_catalog(
            environment.physical(),
            environment.target(),
        )
        .map_err(|_| AbiPreservationSelectionError::UnsupportedTargetConvention),
        target::Architecture::Aarch64 => crate::isa_aarch64::aarch64_preservation_storage_catalog(
            environment.physical(),
            environment.target(),
        )
        .map_err(|_| AbiPreservationSelectionError::UnsupportedTargetConvention),
