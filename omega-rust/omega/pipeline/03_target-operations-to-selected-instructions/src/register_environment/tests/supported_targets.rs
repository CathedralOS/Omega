...
fn every_supported_native_target_builds_a_matching_closed_environment() {
    for target in [
        ...
        let expected_exit = if target == NativeTarget::linux_x64() {
            Some(crate::isa_x86_64::X86_64_HOSTED_EXIT_PROCESS_I32)
        ...
        let expected_write = match (target.architecture, target.object_format) {
            (Architecture::X86_64, ObjectFormat::Elf) => {
                Some(crate::isa_x86_64::X86_64_HOSTED_WRITE_BYTE_I32)
            }
        ...
        for (arity, key) in unit_keys.iter().enumerate() {
            ...
            assert!(
                row.operands
                    .iter()
                    .all(|operand| operand.access
                        == crate::register_model::RegisterOperandAccess::Use)
            );
        }
        let load = match target.architecture {
            Architecture::X86_64 => X86_64_LOAD64,
            Architecture::Aarch64 => crate::isa_aarch64::AARCH64_LOAD64,
        };
        ...
        let indexed_load = match target.architecture {
            Architecture::X86_64 => crate::isa_x86_64::X86_64_LOAD8_INDEXED,
            Architecture::Aarch64 => crate::isa_aarch64::AARCH64_LOAD8_INDEXED,
        };
        assert_eq!(
        ...
        for (selected_key, allocation_key, key, operands) in [
            (
                ...
                match target.architecture {
                    Architecture::X86_64 => crate::isa_x86_64::X86_64_STORE,
                    Architecture::Aarch64 => crate::isa_aarch64::AARCH64_STORE,
                },
                2,
            ),
            (
                ...
                match target.architecture {
                    Architecture::X86_64 => crate::isa_x86_64::X86_64_ADDRESS_OFFSET,
                    Architecture::Aarch64 => crate::isa_aarch64::AARCH64_ADDRESS_OFFSET,
                },
                2,
            ),
            (
                ...
                match target.architecture {
                    ...
                    Architecture::Aarch64 => crate::isa_aarch64::AARCH64_STORE64,
                },
                1,
            ),
            (
                ...
                match target.architecture {
                    ...
                    Architecture::Aarch64 => crate::isa_aarch64::AARCH64_FRAME_ADDRESS,
                },
                1,
            ),
        ] {
