...
pub fn encode_aarch64_selected_hosted_read_byte_form(
    ...
    let bytes = if target == NativeTarget::macos_arm64() {
        crate::isa_aarch64::encode_macos_read_byte_to_stack(displacement, displacement + 4)
    } else {
        crate::isa_aarch64::encode_linux_read_byte_to_stack(displacement, displacement + 4)
    }
    .map_err(|_| Aarch64SelectedFormEncodingError::EncodedFormMismatch)?;
...
mod tests {
    ...
    fn read_byte_replays_every_bit_and_rejects_wrong_home_source_slot_and_alternative() {
        let physical = crate::register_model::validate_physical_register_model(
            crate::isa_aarch64::aarch64_physical_register_model(),
        )
        .unwrap();
    ...
    fn read_byte_linux_golden_and_darwin_return_paths() {
        let linux = crate::isa_aarch64::encode_linux_read_byte_to_stack(16, 20).unwrap();
        let golden: [u32; 14] = [
            0xb900_13ff,
            0xb900_17ff,
            0x9100_53e1,
        ...
        let bytes = crate::isa_aarch64::encode_macos_read_byte_to_stack(16, 20).unwrap();
        let words = bytes
            .as_chunks::<4>()
            .0
            .iter()
        ...
        for (home, payload) in [(0, 0), (0, 8), (2, 6), (4092, 4096), (u32::MAX, 3)] {
            assert!(crate::isa_aarch64::encode_macos_read_byte_to_stack(home, payload).is_err());
        }
    ...
    fn read_catalog_retains_complete_zero_operand_constraint_and_effect_clobbers() {
        ...
        .unwrap();
        let constraints = crate::isa_aarch64::validate_aarch64_register_constraint_catalog(
            crate::isa_aarch64::aarch64_register_constraint_catalog(&physical),
            &physical,
        )
        .unwrap();
        for target in [NativeTarget::linux_arm64(), NativeTarget::macos_arm64()] {
            let catalog =
                crate::isa_aarch64::aarch64_machine_effect_catalog(target, &constraints).unwrap();
            let validated = crate::isa_aarch64::validate_aarch64_machine_effect_catalog(
                target,
                &constraints,
                catalog.clone(),
            )
            ...
            let expected_key = if target == NativeTarget::linux_arm64() {
                crate::isa_aarch64::AARCH64_HOSTED_READ_BYTE
            } else {
                crate::isa_aarch64::AARCH64_DARWIN_HOSTED_READ_BYTE
            };
            ...
            assert!(
                crate::isa_aarch64::validate_aarch64_machine_effect_catalog(
                    target,
                    &constraints,
                    damaged
                )
