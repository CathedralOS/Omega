...
use crate::isa_aarch64::{
    aarch64_aapcs64_register_call_keys, aarch64_register_constraint_catalog_for,
};
...
pub fn validate_aarch64_selected_scalar_call_template(
    ...
    let (expected_operand_views, expected) = if unit || aggregate || floating_scalar {
        ...
        let keys = if aggregate {
            crate::isa_aarch64::aarch64_register_aggregate_call_keys(darwin)
                .into_iter()
            ...
            crate::isa_aarch64::aarch64_float_scalar_call_keys(darwin)
        } else {
            match abi {
                Aarch64SelectedAbi::Aapcs64 => {
                    crate::isa_aarch64::aarch64_aapcs64_register_unit_call_keys()
                        .into_iter()
                        .chain(crate::isa_aarch64::aarch64_aapcs64_mixed_unit_call_keys())
                        .collect::<Vec<_>>()
                }
                Aarch64SelectedAbi::Darwin => {
                    crate::isa_aarch64::aarch64_darwin_register_unit_call_keys()
                        .into_iter()
                        .chain(crate::isa_aarch64::aarch64_darwin_mixed_unit_call_keys())
                        .collect::<Vec<_>>()
                }
            }
        };
        ...
        .find(|row| {
            row.key
                == match abi {
                    ...
                    Aarch64SelectedAbi::Darwin => {
                        crate::isa_aarch64::aarch64_darwin_register_call_keys()[arity]
                    }
                }
        })
...
mod tests {
    ...
    fn every_register_arity_has_exact_operands_and_effects() {
        ...
        let catalog = crate::isa_aarch64::aarch64_register_constraint_catalog(&physical);
    ...
    fn scalar_call_abi_matrix_resolves_declared_pairs_and_fails_closed() {
        ...
        for undeclared in [
            ...
            assert_eq!(
                ...
                Err(
                    crate::isa_aarch64::machine_effects::Aarch64MachineEffectCatalogValidationError::UnsupportedTargetAbi,
                )
            );
        }
    }
