...
use crate::isa_aarch64::aarch64_physical_register_model_ref;
use crate::isa_aarch64::saturating_forms::SaturatingRealization;
use crate::isa_aarch64::selected_form_encoding::movn_materialization::append_canonical_materialization;
use crate::isa_aarch64::selected_form_encoding::selected_forms::BOUND_WORDS;
use crate::isa_aarch64::selected_form_encoding::selected_forms::{integer_bits, u12};
...
pub(crate) enum DecodedWord {
    ...
    /// `orn destination, xzr, source` — the MVN alias for complement.
    BitwiseNot {
        source: u8,
        destination: u8,
    },
...
fn decode_word(word: u32) -> Result<DecodedWord, Aarch64SelectedFormEncodingError> {
    ...
    if word & 0xffe0_fc00 == 0xaa00_0000 {
        return Ok(DecodedWord::BitwiseOr {
            left: ((word >> 5) & 0x1f) as u8,
            right: ((word >> 16) & 0x1f) as u8,
            destination: register,
        });
    }
    if word & 0xffe0_ffe0 == 0xaa20_03e0 {
        return Ok(DecodedWord::BitwiseNot {
            source: ((word >> 16) & 0x1f) as u8,
            destination: register,
        });
    }
    ...
    let valid = match kind {
        ...
        SelectedInstructionKind::SaturatingRemainder { .. } => {
            ...
            registers[2] != registers[0]
                ...
                && decoded == expected_saturating(realization, registers)
        }
...
fn expected_saturating(realization: SaturatingRealization, registers: &[u8]) -> Vec<DecodedWord> {
    ...
    match realization {
        ...
        SaturatingRealization::Remainder { signed } => vec![
            if signed {
                DecodedWord::SignedDivide {
                    dividend: left,
                    divisor: right,
        ...
        SaturatingRealization::ClampNarrow { operation, .. } => {
            ...
            let mut expected = vec![match operation {
                crate::selected_instructions::SaturatingOperation::Add => DecodedWord::Add {
                    left,
                    right,
                    destination: value,
                },
                crate::selected_instructions::SaturatingOperation::Subtract => {
                    DecodedWord::Subtract {
                        left,
                        right,
                        destination: value,
                ...
                crate::selected_instructions::SaturatingOperation::Divide => {
                    DecodedWord::SignedDivide {
                        dividend: left,
                        divisor: right,
                        destination: value,
                ...
                crate::selected_instructions::SaturatingOperation::Multiply => {
                    DecodedWord::Multiply {
                        left,
                        right,
                        destination: value,
                ...
                crate::selected_instructions::SaturatingOperation::Remainder => {
