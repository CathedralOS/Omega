...
use crate::selected_instructions::{MachineSemanticKind, MachineSizeKnowledge};
...
fn packed_constraints_require_early_clobbers_and_effects_reject_scratch_omission() {
    ...
    for (key, load) in [
        (crate::isa_aarch64::AARCH64_LOAD_PACKED, true),
        (crate::isa_aarch64::AARCH64_STORE_PACKED, false),
    ] {
        let row = catalog
            .constraints
            .iter()
        ...
        changed
            ...
            .early_clobber = false;
        assert!(
            crate::isa_aarch64::validate_aarch64_register_constraint_catalog(changed, &physical)
                .is_err()
        );
    }
    let constraints =
        crate::isa_aarch64::validate_aarch64_register_constraint_catalog(catalog, &physical)
            .unwrap();
    for target in [
        target::NativeTarget::linux_arm64(),
        target::NativeTarget::macos_arm64(),
