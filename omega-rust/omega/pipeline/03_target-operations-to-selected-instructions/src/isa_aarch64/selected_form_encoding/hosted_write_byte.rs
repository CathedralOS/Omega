...
pub fn decode_aarch64_selected_hosted_write_byte_i32(
    ...
    abstract_operations_to_target_operations::calling_conventions::MachineRegister,
    u32,
)> {
    let (register, displacement) = decode(target, bytes)?;
    Some((
        abstract_operations_to_target_operations::calling_conventions::MachineRegister::Aarch64X(
            register,
        ),
        displacement,
    ))
