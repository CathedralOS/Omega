...
use crate::isa_aarch64::aarch64_register_constraint_catalog_for;
use crate::isa_aarch64::machine_effects::{Aarch64SelectedAbi, aarch64_selected_abi};
...
> {
    ...
    let expected = MachineEncodedEffects {
        external_operand_reads: row
            .operands
            .iter()
            .filter(|operand| operand.access == crate::register_model::RegisterOperandAccess::Use)
            .map(|operand| operand.operand)
            .collect(),
        external_operand_writes: row
            .operands
            .iter()
            .filter(|operand| operand.access == crate::register_model::RegisterOperandAccess::Def)
            .map(|operand| operand.operand)
            .collect(),
    ...
    fn inputs(
        ...
        let effects = MachineEncodedEffects {
            external_operand_reads: row
                ...
                .iter()
                .filter(|operand| {
                    operand.access == crate::register_model::RegisterOperandAccess::Use
                })
                .map(|operand| operand.operand)
                .collect(),
            external_operand_writes: row
                ...
                .iter()
                .filter(|operand| {
                    operand.access == crate::register_model::RegisterOperandAccess::Def
                })
                .map(|operand| operand.operand)
                .collect(),
            implicit_unit_uses: row.implicit_uses.clone(),
