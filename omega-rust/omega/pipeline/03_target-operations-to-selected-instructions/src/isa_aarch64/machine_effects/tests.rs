...
fn every_saturating_carrier_binds_its_shape_key_size_and_flags() {
    ...
    let nzcv = crate::isa_aarch64::aarch64_physical_register_model()
        .view_named("nzcv")
        .unwrap()
        .units
        .clone();
            ...
            for (operation, semantic) in [
                ...
                let (key, size, writes): (_, u16, &[u16]) = match operation {
                    SaturatingOperation::Add if carrier == SaturatingCarrier::U64 => (
                        crate::isa_aarch64::register_model::AARCH64_SATURATING_ADD_U64,
                        8,
                        &[2],
                    ),
                    SaturatingOperation::Add => (
                        crate::isa_aarch64::register_model::AARCH64_SATURATING_ADD_CLAMPED,
                        if !narrow {
                            16
                        } else if signed {
                            28
                    ...
                    SaturatingOperation::Divide if !signed => (
                        crate::isa_aarch64::register_model::AARCH64_DIVIDE_U64,
                        4,
                        &[2],
                    ),
                    SaturatingOperation::Divide => (
                    ...
                    SaturatingOperation::Remainder if !signed => (
                        crate::isa_aarch64::register_model::AARCH64_REMAINDER_U64,
                        8,
                        &[2],
                    ),
                    SaturatingOperation::Remainder => (
                        crate::isa_aarch64::register_model::AARCH64_REMAINDER_I64,
                        8,
                        &[2],
                    ),
...
fn selected_keys_declare_the_supported_aarch64_pairs() {
    ...
    for (target, darwin, return_i64, return_unit, read, exit, write) in [
        (
            ...
            crate::isa_aarch64::AARCH64_HOSTED_EXIT_PROCESS_I32,
            crate::isa_aarch64::AARCH64_HOSTED_WRITE_BYTE_I32,
        ),
        (
            ...
            crate::isa_aarch64::AARCH64_DARWIN_HOSTED_EXIT_PROCESS_I32,
            crate::isa_aarch64::AARCH64_DARWIN_HOSTED_WRITE_BYTE_I32,
        ),
    ] {
        ...
        let mut expected_scalar = if darwin {
            crate::isa_aarch64::aarch64_darwin_register_call_keys()
        } else {
            crate::isa_aarch64::aarch64_aapcs64_register_call_keys()
        };
        expected_scalar.extend(crate::isa_aarch64::aarch64_float_scalar_call_keys(darwin));
        ...
        assert_eq!(
            ...
            crate::isa_aarch64::aarch64_register_aggregate_return_keys(darwin)
        );
        assert_eq!(
            keys.return_float,
            crate::isa_aarch64::aarch64_float_scalar_return_keys(darwin)
        );
    }
}
