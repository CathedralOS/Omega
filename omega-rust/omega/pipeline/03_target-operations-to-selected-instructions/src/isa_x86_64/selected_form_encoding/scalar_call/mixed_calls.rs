...
fn microsoft_mixed_calls_preserve_positional_register_holes() {
    ...
    for (variant, names) in [
        ...
        let row = catalog
            ...
            .iter()
            .find(|row| {
                row.key.family == crate::register_model::RegisterConstraintFamily::Call
...
fn every_mixed_unit_call_row_has_exact_template_operands_and_effects() {
    ...
    for (target, keys) in [
        (
            ...
            crate::isa_x86_64::x86_64_system_v_mixed_unit_call_keys(),
        ),
        (
            NativeTarget::windows_x64(),
            crate::isa_x86_64::x86_64_microsoft_mixed_unit_call_keys(),
        ),
    ] {
        for key in keys {
            let row = catalog
