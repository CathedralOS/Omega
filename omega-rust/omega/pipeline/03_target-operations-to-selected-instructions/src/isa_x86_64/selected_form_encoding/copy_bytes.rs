...
mod tests {
    ...
    fn copy_bytes_catalog_requires_both_early_clobbers_and_exact_dynamic_effect() {
        ...
        .unwrap();
        let catalog = crate::isa_x86_64::x86_64_register_constraint_catalog(&physical);
        ...
        for operand in [3, 4] {
            ...
            changed
                .constraints
                .iter_mut()
                .find(|row| row.key == crate::isa_x86_64::X86_64_COPY_BYTES)
                .unwrap()
                .operands[operand]
                .early_clobber = false;
            assert!(
                crate::isa_x86_64::validate_x86_64_register_constraint_catalog(changed, &physical)
                    .is_err()
            );
        }
        for target in [
            ...
            let row = effects
                ...
                .iter()
                .find(|row| {
                    row.semantic == crate::selected_instructions::MachineSemanticKind::CopyBytes
                })
                .unwrap();
            assert_eq!(
                row.memory,
