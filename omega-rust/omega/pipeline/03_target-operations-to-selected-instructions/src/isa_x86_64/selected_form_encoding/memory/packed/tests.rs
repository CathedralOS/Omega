...
use crate::register_model::{RegisterOperandAccess, validate_physical_register_model};
use crate::selected_instructions::{MachineSemanticKind, PackedByteWidth};
...
fn packed_memory_replays_exact_bytes_widths_offsets_and_aliases() {
    let model =
        validate_physical_register_model(crate::isa_x86_64::x86_64_physical_register_model())
            .unwrap();
    for names in [
        ["rdi", "rax", "r10"],
        ["r12", "r9", "r11"],
...
fn packed_memory_constraints_and_effects_retain_early_writes_and_flags() {
    let model =
        validate_physical_register_model(crate::isa_x86_64::x86_64_physical_register_model())
            .unwrap();
    let catalog = crate::isa_x86_64::x86_64_register_constraint_catalog(&model);
    let constraints =
        crate::isa_x86_64::validate_x86_64_register_constraint_catalog(catalog.clone(), &model)
            .unwrap();
    let effects = crate::isa_x86_64::x86_64_machine_effect_catalog(
        target::NativeTarget::linux_x64(),
        &constraints,
    )
    .unwrap();
    for (key, load) in [
        (crate::isa_x86_64::X86_64_LOAD_PACKED, true),
        (crate::isa_x86_64::X86_64_STORE_PACKED, false),
    ] {
        let row = catalog
            .constraints
            .iter()
        ...
        for mutation in 0..3 {
            ...
            assert!(
                crate::isa_x86_64::validate_x86_64_register_constraint_catalog(changed, &model)
                    .is_err()
            );
        }
    }
    for (semantic, width) in [
        ...
        for mutation in 0..3 {
            ...
            assert!(
                crate::isa_x86_64::validate_x86_64_machine_effect_catalog(
                    target::NativeTarget::linux_x64(),
                    &constraints,
                    changed
                )
