...
use crate::isa_x86_64::selected_form_encoding::decoding::{DecodedInstruction, decode_one};
use crate::isa_x86_64::selected_form_encoding::saturating_forms::SaturatingForm;
use crate::isa_x86_64::selected_form_encoding::trapping_forms::{TrappingRow, TrappingShape};
...
fn saturating_operands(
    physical: &crate::register_model::ValidatedPhysicalRegisterModel,
    names: &[&str],
) -> Vec<crate::register_model::RegisterViewId> {
    names
        .iter()
        .map(|name| physical.model().view_named(name).unwrap().id)
        .collect()
    ...
    for operation in SATURATING_OPERATIONS {
        for carrier in SaturatingCarrier::ALL {
            ...
            for high in [false, true] {
                ...
                .unwrap();
                let registers =
                    crate::isa_x86_64::selected_form_encoding::request_validation::resolve_registers(
                        &physical, &operands,
                    )
                    .unwrap();
...
fn trapping_forms_match_independent_assembler_for_every_form() {
    let physical = validate_physical_register_model(x86_64_physical_register_model()).unwrap();
    let constraints = crate::isa_x86_64::validate_x86_64_register_constraint_catalog(
        crate::isa_x86_64::x86_64_register_constraint_catalog(&physical),
        &physical,
    )
    .unwrap();
    let catalog = crate::isa_x86_64::x86_64_machine_effect_catalog(
    ...
    for (index, form) in TrappingForm::ALL.into_iter().enumerate() {
        ...
        for (high, table) in [(false, &CLANG_TRAPPING_LOW), (true, &CLANG_TRAPPING_HIGH)] {
            ...
            assert_eq!(
                declared.size,
                crate::selected_instructions::MachineSizeKnowledge::ExactBytes(shape.byte_count())
            );
            validate_x86_64_selected_form_encoding(
                &physical,
                trapping_kind(form),
