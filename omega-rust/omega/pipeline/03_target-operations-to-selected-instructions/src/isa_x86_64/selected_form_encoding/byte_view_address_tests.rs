...
use super::{
    ...
    SelectedInstructionKind, encode_x86_64_selected_form, validate_x86_64_selected_form_encoding,
};
...
fn byte_view_address_has_distinct_family_and_exact_register_bytes() {
    let physical =
        validate_physical_register_model(crate::isa_x86_64::x86_64_physical_register_model())
            .unwrap();
    let views =
        ["rax", "rcx", "rdx", "rbx"].map(|name| physical.model().view_named(name).unwrap().id);
    ...
    validate_x86_64_selected_form_encoding(&physical, kind, key, operands, encoded.bytes())
        .unwrap();
    ...
    for position in 0..3 {
        ...
        assert!(
            validate_x86_64_selected_form_encoding(&physical, kind, key, &changed, encoded.bytes())
                .is_err()
        );
    }
