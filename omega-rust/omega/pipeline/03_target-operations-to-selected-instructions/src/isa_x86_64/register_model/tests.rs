...
use crate::isa_x86_64::register_model::physical_model::VECTOR128;
...
fn normalized_foreign_scalar_rows_cover_banks_results_and_caller_saves() {
    let model = crate::register_model::validate_physical_register_model(
        super::x86_64_physical_register_model(),
    )
    .unwrap();
    let catalog = super::x86_64_register_constraint_catalog(&model);
    let integer_class = model.model().view_named("rax").unwrap().class;
    for (inputs, keys, expected_count) in [
        (
            super::x86_64_system_v_register_unit_call_keys()
                .into_iter()
                .chain(super::x86_64_system_v_mixed_unit_call_keys())
                .collect::<Vec<_>>(),
            super::x86_64_system_v_normalized_foreign_call_keys(),
            756,
        ),
        (
            super::x86_64_microsoft_register_unit_call_keys()
                .into_iter()
                .chain(super::x86_64_microsoft_mixed_unit_call_keys())
                .collect::<Vec<_>>(),
    ...
    super::validate_x86_64_register_constraint_catalog(catalog, &model).unwrap();
}
...
fn normalized_foreign_scalar_rows_reject_bank_result_and_clobber_mutations() {
    ...
    for keys in [
        ...
        for mutation in 0..5 {
            ...
            match mutation {
                0 => {
                    let view = model.model().view_named("rax").unwrap();
                ...
                3 => row
                    .clobbers
                    .extend(&model.model().view_named("xmm0").unwrap().write_units),
            ...
            assert!(
                super::validate_x86_64_register_constraint_catalog(changed, &model).is_err(),
