...
mod tests {
    ...
    use crate::register_homes::RegisterClassAvailability;
    ...
    fn codec_round_trips_plain_plan_and_rejects_framing_or_tamper() {
        ...
        assert_eq!(
            ...
            Err(crate::register_homes::AllocatorAvailabilityDecodeError::WrongMagic)
        );
        let mut wrong_version = encoded.clone();
        wrong_version[8..12].copy_from_slice(&2_u32.to_le_bytes());
        assert_eq!(
            AllocatorAvailabilityPlan::decode(&wrong_version),
            Err(crate::register_homes::AllocatorAvailabilityDecodeError::UnsupportedVersion(2))
        );
        assert_eq!(
            AllocatorAvailabilityPlan::decode(&encoded[..encoded.len() - 1]),
            Err(crate::register_homes::AllocatorAvailabilityDecodeError::Truncated)
        );
        let mut trailing = encoded.clone();
        trailing.push(0);
        assert_eq!(
            AllocatorAvailabilityPlan::decode(&trailing),
            Err(crate::register_homes::AllocatorAvailabilityDecodeError::TrailingBytes)
        );
        let mut tampered = encoded;
        *tampered.last_mut().unwrap() ^= 1;
        assert_eq!(
            AllocatorAvailabilityPlan::decode(&tampered),
            Err(crate::register_homes::AllocatorAvailabilityDecodeError::IdentityMismatch)
        );
    }
}
