...
pub(in crate::register_homes::register_homes::recovery::fixed_view_copy::codec::selected) fn decode_local_slot(
    cursor: &mut Cursor<'_>,
) -> Result<target_operations_to_selected_instructions::LocalStorageSlotId, FixedViewCopyDecodeError>
{
    let tag = cursor.byte()?;
    if tag == 4 {
        return Ok(
            target_operations_to_selected_instructions::LocalStorageSlotId::StructuralParameter {
    ...
    if tag == 3 {
        return Ok(
            target_operations_to_selected_instructions::LocalStorageSlotId::StructuralBlockParameter {
    ...
    match tag {
        0 => Ok(target_operations_to_selected_instructions::LocalStorageSlotId::Structural {
            operation,
            place: decode_id(cursor, PlaceId::new)?,
        }),
        1 => Ok(target_operations_to_selected_instructions::LocalStorageSlotId::Boundary { operation }),
        5 => Ok(
            target_operations_to_selected_instructions::LocalStorageSlotId::StructuralCallArgument {
                operation,
...
pub(in crate::register_homes::register_homes::recovery::fixed_view_copy::codec::selected) fn encode_contracts(
    bytes: &mut Vec<u8>,
...
pub(in crate::register_homes::register_homes::recovery::fixed_view_copy::codec::selected) fn decode_contracts(
    cursor: &mut Cursor<'_>,
    ...
    for _ in 0..count {
        ...
        let origin = match cursor.byte()? {
            0 => target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Operation(
                decode_id(cursor, OperationId::new)?,
            ),
            1 => target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Edge(
                decode_id(cursor, semantic_vocabulary::EdgeId::new)?,
            ),
            2 => target_operations_to_selected_instructions::SelectedMemoryAccessOrigin::Block(
                decode_id(cursor, semantic_vocabulary::BlockId::new)?,
            ),
            tag => return Err(FixedViewCopyDecodeError::UnknownOption(tag)),
        };
...
mod local_slot_tests {
    ...
    use crate::register_homes::register_homes::recovery::fixed_view_copy::codec::selected::structural::decode_contracts;
    use crate::register_homes::register_homes::recovery::fixed_view_copy::codec::selected::structural::decode_local_slot;
    use crate::register_homes::register_homes::recovery::fixed_view_copy::codec::selected::structural::encode_contracts;
    ...
    fn structural_call_argument_slot_roundtrip_binds_operation_and_argument_index() {
        ...
        for (operation, argument_index) in [(1, 0), (1, 1), (2, 0), (u64::MAX, u32::MAX)] {
            let slot = target_operations_to_selected_instructions::LocalStorageSlotId::StructuralCallArgument {
