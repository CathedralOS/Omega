...
use target_operations_to_selected_instructions::legalized_operations::{
    LegalizedScalarCall, LegalizedStructuralContract, NativeCallOrigin,
};
...
fn outgoing_slot_roles_bind_rosters_memory_and_authenticated_round_trips() {
    use target_operations_to_selected_instructions::OutgoingArgumentSlotRole;
...
fn owned_entry_slot_round_trip_binds_parameter_place_and_origin() {
    use target_operations_to_selected_instructions::{
        LocalStorageSlotId, SelectedLocalStorageSlot,
    };
    let place = PlaceId::new(67).unwrap();
