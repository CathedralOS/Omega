use crate::register_homes::{FixedViewCopyPlan, FixedViewCopyPolicy};
use semantic_vocabulary::MachineId;
use target_operations_to_selected_instructions::{SelectedInstructionKind, SelectedTerminator};
...
fn process_exit_codec_preserves_terminal_nominal_edge_and_source_without_scratch() {
    ...
    function
        ...
        .push(target_operations_to_selected_instructions::SelectedBoundarySettlement {
            block: block.id,
            instruction_index: block.instructions.len() as u32,
            settlement:
                target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::HostedExitProcessI32 {
...
fn successor_transfer_vocabulary_requires_the_current_envelope() {
    ...
    std::sync::Arc::make_mut(&mut transferred.transformed).functions[0] =
        super::successor_parameter_function();
...
fn artifact_current_retains_segment_home_evidence_and_rejects_older_authority() {
    ...
    plan.source_evidence =
        crate::register_homes::FixedViewCopySourceEvidence::FixedPrecoloredSegmentHomesV1 {
            fixed_intervals: crate::register_homes::FixedPrecoloredIntervalPlanIdentity::from_bytes(
                [21; 32],
            ),
            split_requirements:
            ...
            segment_homes:
                crate::register_homes::FixedPrecoloredSegmentHomePlanIdentity::from_bytes([23; 32]),
        };
    let encoded = plan.encode();
    assert_eq!(u32::from_le_bytes(encoded[8..12].try_into().unwrap()), 35);
    assert_eq!(FixedViewCopyPlan::decode(&encoded).unwrap(), plan);
    for encoded in [
        with_stale_version(&plan, 10),
        with_stale_version(&plan, 11),
        with_stale_version(&plan, 23),
        with_stale_version(&plan, 25),
    ] {
        let version = u32::from_le_bytes(encoded[8..12].try_into().unwrap());
        assert_eq!(
            FixedViewCopyPlan::decode(&encoded),
            Err(crate::register_homes::FixedViewCopyDecodeError::UnsupportedVersion(version))
        );
    }
}
...
fn artifact_rejects_pre_compare_identity_version() {
    ...
    assert_eq!(
        ...
        Err(crate::register_homes::FixedViewCopyDecodeError::UnsupportedVersion(6))
    );
}
...
fn artifact_rejects_pre_predicate_identity_version() {
    ...
    assert_eq!(
        ...
        Err(crate::register_homes::FixedViewCopyDecodeError::UnsupportedVersion(7))
    );
}
...
fn artifact_current_round_trips_signed_less_than_terminator_vocabulary() {
    ...
    assert_eq!(
        ...
        Err(crate::register_homes::FixedViewCopyDecodeError::UnsupportedVersion(9))
    );
}
