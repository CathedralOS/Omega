...
fn encode_successor(bytes: &mut Vec<u8>, successor: &SelectedSuccessor) {
    ...
    for binding in &successor.structural_bindings {
        ...
        match binding.transport {
            target_operations_to_selected_instructions::SelectedStructuralTransport::Unused => bytes.push(0),
            ...
            target_operations_to_selected_instructions::SelectedStructuralTransport::Address {
                ...
                bytes.push(3);
                match base {
                    target_operations_to_selected_instructions::SelectedAddressBase::Register(register) => {
                        bytes.push(0);
                        bytes.extend_from_slice(&register.0.to_le_bytes());
                    }
                    target_operations_to_selected_instructions::SelectedAddressBase::Local(slot) => {
                        bytes.push(1);
...
fn decode_successor(
    ...
    for _ in 0..count {
        ...
        let transport = match cursor.byte()? {
            0 => target_operations_to_selected_instructions::SelectedStructuralTransport::Unused,
            2 => {
            ...
            1 => target_operations_to_selected_instructions::SelectedStructuralTransport::Descriptor {
                argument: VirtualRegisterId(cursor.u32()?),
                destination: super::structural::decode_local_slot(cursor)?,
            },
            3 => {
                let base = match cursor.byte()? {
                    0 => target_operations_to_selected_instructions::SelectedAddressBase::Register(VirtualRegisterId(
                        cursor.u32()?,
                    )),
                    1 => target_operations_to_selected_instructions::SelectedAddressBase::Local(
        ...
        structural_bindings.push(
            target_operations_to_selected_instructions::SelectedStructuralBinding {
                semantic,
                transport,
            },
        );
...
fn decode_case(
    ...
    Option<target_operations_to_selected_instructions::SelectedStructuralCaseEdge>,
    FixedViewCopyDecodeError,
> {
    match cursor.byte()? {
        0 => return Ok(None),
        1 => {}
    ...
    for _ in 0..count {
        ...
        let parameter = target_operations_to_selected_instructions::legalized_operations::LegalizedValueDefinition {
            value: decode_id(cursor, ValueId::new)?,
            scalar_type: decode_scalar(cursor)?,
            definition_site: crate::register_homes::register_homes::recovery::fixed_view_copy::codec::values::decode_definition_site(cursor)?,
        };
        let transport = match cursor.byte()? {
            0 => target_operations_to_selected_instructions::SelectedCasePayloadTransport::Unused,
            1 => target_operations_to_selected_instructions::SelectedCasePayloadTransport::Unmaterialized {
                parameter: VirtualRegisterId(cursor.u32()?),
            },
            2 => target_operations_to_selected_instructions::SelectedCasePayloadTransport::Registers {
                argument: VirtualRegisterId(cursor.u32()?),
                parameter: VirtualRegisterId(cursor.u32()?),
            },
            tag => return Err(FixedViewCopyDecodeError::UnknownValueTransport(tag)),
        };
        payloads.push(target_operations_to_selected_instructions::SelectedCasePayloadBinding {
