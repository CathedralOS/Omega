...
impl RecoveryClassificationPlan {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        let identity = crate::register_homes::recovery_classification_identity(self);
    ...
    pub fn decode(encoded: &[u8]) -> Result<Self, RecoveryClassificationDecodeError> {
        ...
        if crate::register_homes::recovery_classification_identity(&plan) != identity {
    ...
    fn provenance(
        ...
        for _ in 0..fuel_count {
            let site = match self.byte()? {
                ...
                1 => terminal_psi_to_abstract_operations::optimization_unit::PsiProvenance::Edge(
                    self.edge_id()?,
                ),
                tag => {
