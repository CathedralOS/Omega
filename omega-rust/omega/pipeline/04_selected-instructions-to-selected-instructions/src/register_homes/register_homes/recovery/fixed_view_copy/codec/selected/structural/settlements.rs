...
use target_operations_to_selected_instructions::legalized_operations::{
    LegalizedBoundarySettlement, NativeCallOrigin,
};
use terminal_psi::{
...
pub(super) fn encode_boundary_settlement(
    bytes: &mut Vec<u8>,
    payload: &target_operations_to_selected_instructions::SelectedBoundarySettlementPayload,
) {
    let settlement = match payload {
        ...
        target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::ClaimCompletion(settlement) => {
            bytes.push(0);
            settlement
        }
...
pub(super) fn decode_boundary_settlement(
    ...
    target_operations_to_selected_instructions::SelectedBoundarySettlementPayload,
...
> {
    ...
    Ok(
        target_operations_to_selected_instructions::SelectedBoundarySettlementPayload::ClaimCompletion(
