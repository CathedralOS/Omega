...
mod independence_tests {
    ...
    /// Rearrange the window the two run index pairs bound in the source's
    /// first block to later-run ++ interior ++ earlier-run and permute the
    /// window's roster rows into the new execution order — the edit a
    /// producer emitting that interchange would publish — without asking
    /// admission whether the window is legal. The indices are block-body
    ...
    fn forged(
        ...
        let first = earlier_first_index;
        let last = later_last_index;
    ...
    /// validates in each direction: the `LOAD_C; DIFF` run taking the
    /// `LOAD_A; SUM` run's position around the `SEP` interior and, on the
    /// admitted interchange, the same runs traded back — the traded roster
    /// rows commute, a trading pair is rowed on both sides, and the
    /// content comparison accepts the rearrangement.
    #[test]
    fn forged_interchange_of_a_legal_window_validates() {
        ...
        .unwrap();
        // On the admitted interchange the `LOAD_C; DIFF` run leads at
        // positions 1..=2 and `LOAD_A; SUM` follows at 4..=5, so forging
    ...
    /// earlier run's loaded value. The validator's own legality audit
    ...
    fn forged_interchange_across_a_coupled_hazard_rejects() {
        let target = NativeTarget::linux_x64();
        let environment = baseline_target_register_environment(target).unwrap();
        // DIFF's first read moves to FIRST — the earlier run's load
        // defines it, so the runs cannot trade.
        let source = mutated(target, |function, _| {
            function.blocks[0].instructions[5].operands[0].virtual_register = FIRST;
        });
        assert_eq!(
    ...
    /// A producer that admitted a non-commuting trade anyway would
    /// publish the runs traded while a roster row a member carries cannot
    /// commute with the row it crosses — the earlier run's row rewritten
    /// to write the bytes the later run's head reads. The validator's own
    /// commutation audit refuses with `UnsupportedPair`.
    #[test]
    fn forged_interchange_past_a_non_commuting_row_rejects() {
        let target = NativeTarget::linux_x64();
        let environment = baseline_target_register_environment(target).unwrap();
        // LOAD_A's row becomes a write over exactly the place-2 bytes
    ...
    /// run on the earlier run's span, so the window content comparison
    /// rejects it.
    #[test]
