...
fn retained_nodes<'block>(
    block: &'block omega_optimization_unit::OptimizationBlock,
    relocated_operations: &[OperationId; 2],
) -> Vec<&'block omega_optimization_unit::OptimizationNode> {
    block
        .nodes
        .iter()
        .filter(|node| {
...
fn same_preserved_node(
    expected: &omega_optimization_unit::OptimizationNode,
    current: &omega_optimization_unit::OptimizationNode,
) -> bool {
...
) -> Option<(BlockId, usize, &omega_optimization_unit::OptimizationNode)> {
