...
const R_MOVE: VirtualRegisterId = VirtualRegisterId(7);
...
/// nonzero edge leads to the arm block T — which ends in a `Jump` to the
/// join block J — and whose zero edge bypasses the arm to land on J
/// directly, so J's only predecessors are the head's bypass edge and the
/// arm's `Jump`, and the join's terminator is a plain return:
/// `B = [LEAD; LATE; TRAIL] -> branch -> T = [T_HEAD; T_TAIL] -> jump -> J`
/// and `-> J = [HEAD; MOVING; TAIL] -> return`.
/// The default move hoists `MOVING` onto `TRAIL`'s position — the tail of
/// B's body — crossing `HEAD`, the arm's body, the arm jump and its edge,
/// both branch edges, the branch terminator, and `TRAIL`, while B's
/// prefix keeps its order ahead of the landing index.
fn fixture(target: NativeTarget) -> ValidatedTriangleRelocation {
...
/// The member crosses the whole triangle onto the destination's position
/// on every target: `MOVING` leaves J's body, `HEAD`, the arm's body, the
/// arm `Jump`, the arm's edge, both branch edges, the branch, and `TRAIL`
/// keep their positions, and the member lands on `TRAIL`'s index with the
/// destination and every later position one slot later in their original
/// order — identity, kind, operands, and provenance intact. The replayed
/// proposal restores the source bit-identically.
#[test]
fn member_relocates_out_of_the_join() {
    for target in [
        ...
        assert_eq!(
            ...
            original.blocks[2].instructions[1]
        );
        for index in 0..3 {
            assert_eq!(
...
/// Every body index relocates: the join-head member crosses only the arm,
/// edges, and branch, while the join-tail member crosses its whole
/// leading body first.
#[test]
...
/// A join the head feeds through arms alone is the diamond family's
/// window: repointing the bypass edge onto a second arm removes every
/// direct edge into the join.
#[test]
fn the_full_diamond_stays_with_its_own_family() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let diamond = mutated(target, |function, environment| {
        let jump_row = environment
            .constraint(environment.selected_keys().jump)
            .unwrap()
            .clone();
        let edge = match &mut function.blocks[0].terminator {
            SelectedTerminator::ConditionalBranch { when_zero, .. } => when_zero,
            _ => unreachable!(),
        };
        *edge = successor(SelectedBlockId(3), BlockId::new(4).unwrap(), EDGE_BJ);
        function.blocks.push(SelectedBlock {
            ...
            terminator: jump_terminator(
                ...
                successor(BLOCK_J, BlockId::new(3).unwrap(), 25),
            ),
        });
    });
    assert_eq!(
        relocate(&diamond, &environment, MOVING, TRAIL).unwrap_err(),
        TriangleRelocationError::UnsupportedPair
    );
}

/// A join the head feeds on every edge is the arm family's window: the
/// join is then an ordinary conditional arm, not a triangle's
/// convergence.
#[test]
fn the_double_bypass_stays_with_the_arm_family() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let doubled = mutated(target, |function, _| {
        let edge = match &mut function.blocks[0].terminator {
            SelectedTerminator::ConditionalBranch { when_nonzero, .. } => when_nonzero,
            _ => unreachable!(),
        };
        *edge = successor(BLOCK_J, BlockId::new(3).unwrap(), EDGE_BT);
        function.blocks.remove(1);
    });
    assert_eq!(
        relocate(&doubled, &environment, MOVING, TRAIL).unwrap_err(),
        TriangleRelocationError::UnsupportedPair
    );
}

/// An arm no traversal can reach still satisfies the supply rule
/// vacuously: its edge into the join never fires, so every real join
/// arrival still passed the head exactly once.
#[test]
fn an_unreachable_arm_never_supplies_an_arrival() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let unreachable_arm = mutated(target, |function, environment| {
        let jump_row = environment
            .constraint(environment.selected_keys().jump)
            .unwrap()
            .clone();
    ...
    relocate(&unreachable_arm, &environment, MOVING, TRAIL).unwrap();
}
...
/// reading it refuses on the arm path, and a crossed `TRAIL` reading it
/// refuses only once the window actually reaches it — a read behind the
/// member in the join is never crossed.
#[test]
fn raw_hazard_keeps_order_out_of_the_join() {
    ...
    let tail_reads = mutated(target, |function, environment| {
        let copy = environment
            ...
            .unwrap()
            .clone();
        function.blocks[2].instructions[2] = instruction(
            TAIL,
            SelectedInstructionKind::CopyI64,
            &copy,
            &[R_MOVE, R_TAIL],
...
/// the new value after the move — in the arm or the join prefix the write
...
/// Condition state couples like registers through the triangle: the
...
/// cannot cross a flag writer in the join prefix or inside the arm; and a
/// flag consumer sitting in the arm does not block a flag-inert member.
#[test]
...
/// and new positions — the arm's jump edge and both branch edges, the
/// bypass included: a member defining the transported argument would hand
/// the binding a value it never carried before the move, a member
/// defining or reading the parameter would be overwritten or observe the
/// transported value, while a member merely reading the argument crosses
/// freely.
#[test]
fn register_transports_bind_each_crossed_edge() {
    ...
    for arm_edge in [true, false] {
        // A binding on a crossed edge sits inside the window whether the
        // edge is the arm's `Jump` or one of the branch's own successors —
        // the bypass edge included.
        let on_edge = |edit: &mut dyn FnMut(
            &mut SelectedFunction,
            &register_environment::ValidatedTargetRegisterEnvironment,
        )| {
            mutated(target, |function, environment| {
                let successor: &mut SelectedSuccessor = if arm_edge {
                    match &mut function.blocks[1].terminator {
                        SelectedTerminator::Jump { successor, .. } => successor,
                        _ => unreachable!(),
                    }
                } else {
                    match &mut function.blocks[0].terminator {
                        SelectedTerminator::ConditionalBranch { when_nonzero, .. } => when_nonzero,
                        _ => unreachable!(),
                    }
                };
        ...
        relocate(&on_edge(&mut |_, _| {}), &environment, MOVING, TRAIL).unwrap();
        relocate(
            &on_edge(&mut member_reads(POINTER)),
            &environment,
            MOVING,
            TRAIL,
        )
        ...
        assert_eq!(
            relocate(
                &on_edge(&mut member_writes(POINTER)),
                &environment,
                MOVING,
                TRAIL,
            )
        ...
        assert_eq!(
            relocate(
                &on_edge(&mut member_writes(R_BOUND)),
                &environment,
                MOVING,
                TRAIL,
            )
        ...
        assert_eq!(
            relocate(
                &on_edge(&mut member_reads(R_BOUND)),
                &environment,
                MOVING,
                TRAIL,
            )
    ...
    // The bypass edge carries the same boundary rules.
    let on_bypass = |edit: &mut dyn FnMut(
        &mut SelectedFunction,
        &register_environment::ValidatedTargetRegisterEnvironment,
    )| {
        mutated(target, |function, environment| {
    ...
    relocate(&on_bypass(&mut |_, _| {}), &environment, MOVING, TRAIL).unwrap();
    relocate(
        &on_bypass(&mut member_reads(POINTER)),
        &environment,
        MOVING,
        TRAIL,
    )
    ...
    assert_eq!(
        relocate(
            &on_bypass(&mut member_writes(POINTER)),
            &environment,
            MOVING,
            TRAIL,
        )
    ...
    assert_eq!(
        relocate(
            &on_bypass(&mut member_writes(R_BOUND)),
            &environment,
            MOVING,
            TRAIL,
        )
    ...
    assert_eq!(
        relocate(
            &on_bypass(&mut member_reads(R_BOUND)),
            &environment,
            MOVING,
            TRAIL,
        )
...
/// The convergence must actually hold: every edge into the join leaves
/// the head directly or an arm ending in a `Jump` to it, every arm
/// descends from the one fork head alone, and every edge the head names
/// lands on the join or an arm — a second predecessor into the join or an
/// arm, a second head, a head edge leaving the region, a non-source
/// block, or a member outside a joined block all refuse.
#[test]
fn the_triangle_must_close() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    // A predecessor reaching the join through a conditional edge is a
    // second head, not a plain arm the window crosses.
    ...
    // A second predecessor into the join hands it a path the member's
    // new position never supplied: the added arm descends from a block
    // other than the fork head.
    let join_two_predecessors = mutated(target, |function, environment| {
        ...
        function.blocks.push(SelectedBlock {
            ...
            terminator: jump_terminator(
                ...
                successor(SelectedBlockId(3), BlockId::new(4).unwrap(), 27),
            ),
        });
    });
    assert_eq!(
    ...
    // A second predecessor into the arm does the same one level up.
    ...
    // A second conditional head reaching the join directly is a second
    // fork the member's new position cannot also sit inside.
    let two_heads = mutated(target, |function, environment| {
        ...
        function.blocks.push(SelectedBlock {
            ...
            terminator: SelectedTerminator::ConditionalBranch {
                ...
                when_nonzero: successor(BLOCK_J, BlockId::new(3).unwrap(), 26),
                when_zero: successor(BLOCK_J, BlockId::new(3).unwrap(), 27),
            },
        });
    });
    assert_eq!(
    ...
    // does not cross — as the arm, as the join, or as the head.
    for origin in [
...
/// work the window does not cross — on the arm's edge and on the branch
/// edges, the bypass included.
#[test]
...
fn barrier_kinds_and_call_roster_reject() {
    ...
    for kind in [
        ...
        let member_barrier = mutated(target, |function, _| {
            function.blocks[2].instructions[1].kind = kind;
        });
        assert_eq!(
            relocate(&member_barrier, &environment, MOVING, TRAIL).unwrap_err(),
            TriangleRelocationError::UnsupportedInstruction,
            "member {kind:?}"
        );
    ...
    let tail_barrier = mutated(target, |function, _| {
        function.blocks[2].instructions[2].kind = SelectedInstructionKind::Jump;
    });
...
fn memory_roster_binds_the_window() {
    ...
    // head, the arm's body, the arm jump, both branch edges, the branch,
    // and the head tail.
    relocate(
        &mutated(target, |function, environment| {
    ...
    for (block_index, index) in [(0usize, 2usize), (1, 1), (2, 0)] {
...
#[test]
fn only_the_named_triangle_window_relocates() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let source = fixture(target);
    ...
    assert_eq!(
        ...
        .unwrap_err(),
        TriangleRelocationError::SourceMismatch
    );
    assert_eq!(
        relocate_selected_instruction_out_of_triangle(
            &source,
    ...
    // join, an arm position, an arm terminator, and a dangling id all
    // refuse.
    for destination in [HEAD, T_HEAD, T_JUMP, SelectedInstructionId(99)] {
        assert_eq!(
    ...
    // The arm's members sit below a `Jump` convergence the head alone
    // feeds — the arm family's shape, with no direct edge into the
    // member's block at all.
    assert_eq!(
...
fn replay_rejects_anything_but_the_move() {
    ...
    assert_eq!(
        validate_triangle_relocation(&source, 0, MOVING, LATE, &environment, budget(), displaced)
            .unwrap_err(),
        TriangleRelocationError::ReplayMismatch
    );
    ...
    unmoved.functions[0].blocks[2]
        .instructions
        .insert(1, member);
    assert_eq!(
        validate_triangle_relocation(&source, 0, MOVING, LATE, &environment, budget(), unmoved)
            .unwrap_err(),
        TriangleRelocationError::ReplayMismatch
    );
    ...
    assert_eq!(
        validate_triangle_relocation(&source, 0, MOVING, LATE, &environment, budget(), dropped)
            .unwrap_err(),
        TriangleRelocationError::ReplayMismatch
    );
    // An unrelated literal edit inside the join block rejects.
    let mut edited = result.transformed().clone();
    edited.functions[0].blocks[2].instructions[0].kind = SelectedInstructionKind::MaterializeI64 {
        value: IntegerValue::Unsigned(12),
    };
    assert_eq!(
        validate_triangle_relocation(&source, 0, MOVING, LATE, &environment, budget(), edited)
            .unwrap_err(),
        TriangleRelocationError::ReplayMismatch
    );
...
/// same triangle onto it, a hazard-coupled member still declines, and an
/// in-block family still admits.
#[test]
fn triangle_relocation_is_deterministic_and_re_admitted() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let source = fixture(target);
    ...
    // join's new tail member rises through the same triangle onto it.
    let again = relocate_selected_instruction_out_of_triangle(
        &first,
        0,
        TAIL,
        TRAIL,
