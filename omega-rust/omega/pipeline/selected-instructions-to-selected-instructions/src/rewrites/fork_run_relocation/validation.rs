...
use super::{
    ForkRunRelocationError, ForkRunRelocationReceipt, ValidatedForkRunRelocation, admission,
};
use crate::ValidatedSelectedAnalysis;

/// Independently consume the proposed program: admission re-derives the
/// admitted run, fork, and landing index from the source, the proposal
/// must place exactly the run's members on the landing index in the arm
...
/// and edge included.
pub fn validate_fork_run_relocation(
    ...
    {
        return Err(ForkRunRelocationError::ReplayMismatch);
    }
