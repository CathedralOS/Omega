...
use super::{JoinRelocationError, JoinRelocationReceipt, ValidatedJoinRelocation, admission};
use crate::ValidatedSelectedAnalysis;

/// Independently consume the proposed program: admission re-derives the
/// admitted member, join, fork head, and landing index from the source,
/// the proposal must place exactly the member's instruction on the
/// landing index in the head block, and moving it back must restore the
