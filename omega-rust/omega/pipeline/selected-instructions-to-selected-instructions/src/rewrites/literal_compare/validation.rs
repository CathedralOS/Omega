...
use super::{LiteralCompareError, LiteralCompareReceipt, ValidatedLiteralCompare, admission};
...
/// reconstructed compare form, and reinserting the source compare must
/// restore the complete source by content — every other instruction,
/// register, roster row, call, settlement, and function included.
pub fn validate_literal_compare_fold(
    ...
    let function = proposed
        .functions
        .get(function_index)
        .ok_or(LiteralCompareError::ReplayMismatch)?;
    if function
        .blocks
    ...
    {
        return Err(LiteralCompareError::ReplayMismatch);
    }
