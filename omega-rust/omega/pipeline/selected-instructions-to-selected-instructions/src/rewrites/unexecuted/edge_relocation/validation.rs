//! Independent validation of cross-edge relocation.
//!
...
//! semantic edge is the destination block's only predecessor, the
//! destination's own gates, the landing index the destination names, the
//! schedulable bar and hazard audit every crossed position and the
//! crossed edge's instruction meet, the roster-ordering accounting, the
//! transport-conflict check, and the boundary-settlement exclusion —
...
use super::{EdgeRelocationError, EdgeRelocationReceipt, ValidatedEdgeRelocation};
...
/// permits: the admitted member's coordinates and the landing index its
/// destination names. It shares no state with the producer's `admission`
/// record.
struct Reconstructed<'source> {
    ...
    /// member lands at this index. Naming the target's
    /// terminator-carried instruction lands the member at the body end,
    /// index `target.instructions.len()`.
    landing_index: usize,
}

/// Reconstruct the legality of relocating `member` onto `destination`
/// from first principles: locate the member, require its block to end in
/// an unconditional `Jump` through a plain semantic edge to a
/// single-predecessor source block that is neither the member's own
/// block nor the entry, resolve the landing index the destination names,
/// then run the window audit the gates leave — the member schedulable,
/// every crossed position (the tail behind the member, the head before
/// the landing index) schedulable and neither roster-accounted against
/// an accounted member nor coupled with it, and the crossed edge's
/// instruction uncalled, unaccounted against an accounted member,
/// uncoupled, and free of transport conflicts — with no boundary
/// settlement inside the vacated span or past the landing index.
...
fn reconstruct<'source>(
...
) -> Result<Reconstructed<'source>, EdgeRelocationError> {
    ...
    // execution count: every traversal of the member's block leaves
    // through it, and — with the single-predecessor rule below — every
    // traversal of the destination block arrives through it. A
    // conditional terminator keeps a second exit the member would still
    // execute on after relocating.
    ...
    // predecessor gives the block a path the member would newly execute
    // on.
    ...
    // The window the member crosses is the one the gates leave: the tail
    // behind it in its own block, the head of the destination body before
    // the landing index, and the jump edge itself — its terminator
    // instruction, its transports, and the rows the roster logs against
    // it. Every crossed contract below is the family's own composition of
    // the shared primitives, not the producer's audit result.
    ...
    if member_accounted && edge_accounted(function, terminator, successor) {
        return Err(EdgeRelocationError::UnsupportedPair);
    }
    ...
    // The run vacates its block from the member's position on, and the
    // destination's prefix past the landing index gains the member: a
    ...
    for settlement in &function.boundary_settlements {
        ...
        if (settlement.block == block.id && index > member_index)
...
/// roster for the path walk, the member's surface against each crossed
/// position's, and the function's three rosters plus the crossed edge's
/// binding roster.
fn measured_steps(
    plan: &SelectedInstructionPlan,
    reconstructed: &Reconstructed<'_>,
) -> Result<u64, EdgeRelocationError> {
    let function = reconstructed.function;
    let block = &function.blocks[reconstructed.block_index];
    let member_instruction = &block.instructions[reconstructed.member_index];
    ...
    let steps = plan
        ...
        .and_then(|total| {
            block.instructions[reconstructed.member_index + 1..]
                .iter()
    ...
    u64::try_from(steps).map_err(|_| EdgeRelocationError::IdentityOverflow)
}

/// Build the function the contract demands from the validator's own
/// record: the member leaves its block's body and lands at the
/// destination's index. The producer's transformation is not consulted;
...
fn expect(reconstructed: &Reconstructed<'_>) -> SelectedFunction {
    ...
    let member_instruction = expected.blocks[reconstructed.block_index]
        .instructions
        .remove(reconstructed.member_index);
    expected.blocks[reconstructed.target_index]
        .instructions
        .insert(reconstructed.landing_index, member_instruction);
    expected
}
...
fn restore(
...
) -> Result<SelectedInstructionPlan, EdgeRelocationError> {
    let mut restored = proposed.clone();
    let function = restored
        .functions
        .get_mut(reconstructed.function_index)
        .ok_or(EdgeRelocationError::ReplayMismatch)?;
    if function
        .blocks
    ...
    {
        return Err(EdgeRelocationError::ReplayMismatch);
    }
    let member_instruction = function.blocks[reconstructed.target_index]
        .instructions
        .get(reconstructed.landing_index)
        .cloned()
        .ok_or(EdgeRelocationError::ReplayMismatch)?;
    function.blocks[reconstructed.target_index]
        .instructions
        .remove(reconstructed.landing_index);
    let block = &mut function.blocks[reconstructed.block_index];
    if block.instructions.len() < reconstructed.member_index {
        return Err(EdgeRelocationError::ReplayMismatch);
    }
    block
        .instructions
...
/// the relocation's preconditions from the source, requires the proposal
...
/// edge included. The producer's `admission::admit` is never consulted,
/// so a wrong legality decision fails here even when the proposal
/// matches the edit the producer emitted.
pub fn validate_edge_relocation(
...
mod independence_tests {
    ...
    const JUMP_C: SelectedInstructionId = SelectedInstructionId(10);
    ...
    /// admission claim: `A = [LEAD; MOVING; TRAIL] -> jump -> B = [HEAD;
    /// MID; TAIL] -> return`, moving `MOVING` onto `HEAD`'s position.
    /// Each test mutates one legality premise, so the source names a
    /// relocation the contract must refuse while a defective producer
    /// would still emit it.
    fn plan(
        environment: &ValidatedTargetRegisterEnvironment,
        target: NativeTarget,
    ...
    fn registers(class: register_model::RegisterClassId) -> Vec<VirtualRegister> {
        vec![
            ...
            result_register(R_MOVE, class, MOVING, 3),
            result_register(R_TRAIL, class, TRAIL, 4),
            result_register(R_HEAD, class, HEAD, 5),
            result_register(R_MID, class, MID, 6),
            result_register(R_TAIL, class, TAIL, 7),
        ]
    }

    fn source(plan: SelectedInstructionPlan) -> ValidatedEdgeRelocation {
    ...
    /// The proposal a defective producer emits for this source: `MOVING`
    /// leaves block A's body and lands at block B's head — the edit the
    /// contract demands only when the relocation's legality holds.
    fn forged(source: &ValidatedEdgeRelocation) -> SelectedInstructionPlan {
        let mut proposed = source.transformed().clone();
        let member = proposed.functions[0].blocks[0].instructions.remove(1);
    ...
    /// `r2 = materialize; r3 = copy r2` — the trailing instruction reads
    /// the register the member defines, so moving the member past it
    /// hands the copy a different value. A producer that skipped the
    /// coupling audit still emits the move; the validator's own hazard
    /// walk refuses with `UnsupportedPair`, not merely a diff of the
    /// proposal. Feeding that forged proposal is the observable proof
    /// that validation no longer relies on the producer's admission
    /// routine.
    #[test]
    fn validator_refuses_a_member_coupled_with_the_crossed_tail() {
        ...
        let plan = plan(
            ...
            vec![
                materialization(&environment, HEAD, R_HEAD, 11),
                materialization(&environment, MID, R_MID, 13),
                materialization(&environment, TAIL, R_TAIL, 15),
            ],
            Vec::new(),
            registers(class),
            Vec::new(),
    ...
    /// `store8 [r0], r2` with no roster row — a memory-capable kind the
    ...
    /// A boundary settlement on the member's block at index 2 sits past
    /// the member's position: the relocation moves the member ahead of
    /// it, so the executed prefix the settlement observes changes. A
    /// producer that skipped the settlement scan still emits the move;
    /// the validator's own walk refuses with `UnsupportedPair`.
    #[test]
    fn validator_refuses_a_settlement_past_the_vacated_member() {
        let target = NativeTarget::linux_x64();
        let environment = baseline_target_register_environment(target).unwrap();
        let class = environment
            .constraint(environment.selected_keys().materialize_i64)
    ...
    /// relocated member would newly execute on. A producer that forgot
    /// the predecessor count still emits the move; the validator's own
    /// edge scan refuses with `UnsupportedPair`.
    #[test]
