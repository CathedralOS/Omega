...
use super::{DiamondRelocationError, ValidatedDiamondRelocation, admission};
use crate::ValidatedSelectedAnalysis;

/// Relocate one admitted member through its block's branch diamond: the
/// named `member` leaves its own block's body and takes the named
/// `destination` instruction's position in the join block the branch's
/// arms alone feed, with the destination and every later position keeping
/// their relative order one slot later. Naming the join's
...
/// confirms that.
pub fn relocate_selected_instruction_through_diamond(
    ...
    super::validate_diamond_relocation(
        source,
        function_index,
        member,
        destination,
