...
use super::{EdgeRunRelocationError, ValidatedEdgeRunRelocation, admission};
use crate::ValidatedSelectedAnalysis;

/// Relocate one admitted run across its block's jump edge: the contiguous
/// run the named `first_member` and `last_member` bound leaves its own
/// block's body as a single body — its members keeping their own order —
/// and takes the named `destination` instruction's position in the edge's
/// target block, with the destination and every later position keeping
/// their relative order one run-width later. Naming the target's
...
/// edge-transport interference, no roster-carrying run sharing the window
/// with a second memory actor, no barrier or settlement whose observed
/// state changes — so every observer sees the same values, memory order,
...
pub fn relocate_selected_run_across_edge(
    ...
    super::validate_edge_run_relocation(
        source,
        function_index,
        first_member,
        last_member,
