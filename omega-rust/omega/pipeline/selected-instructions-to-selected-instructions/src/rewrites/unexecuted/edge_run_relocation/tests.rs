...
fn internally_coupled_run_moves_as_one_body() {
    ...
    // run admits.
    assert_eq!(
        crate::rewrites::unexecuted::relocate_selected_instruction_across_edge(
            &source,
            0,
            RUN_A,
            HEAD,
        ...
        .unwrap_err(),
        crate::rewrites::unexecuted::EdgeRelocationError::UnsupportedPair
    );
}
...
fn measured_validation_step_boundary_admits_and_rejects() {
    ...
    // predecessor scan + the shared path walk's edge bound (the function's
    // single edge) + member-against-crossed surfaces for each member
    // against `TRAIL` (1+1) and the jump (1+2) + empty rosters = 9 + 9 +
    // 1 + 2*5 = 29. Destination `MID` adds the crossed `HEAD` pair (1+1)
    // per member for 33.
    for (destination, exact_steps) in [(HEAD, 29u64), (MID, 33u64)] {
        let source = fixture(target);
...
fn edge_run_relocation_is_deterministic_and_re_admitted() {
    ...
    let again = crate::rewrites::unexecuted::relocate_selected_instruction_across_edge(
        &first,
        0,
        TRAIL,
        HEAD,
