//! Relocation of a member run into a fork arm on the selected CFG.
//!
...
//! branching shape it refuses — the fork, where the run stops executing on
//! the traversals that take the other edge: the named `first_member` and
//! `last_member` bound one contiguous run in a block's body whose terminator
//! is a two-successor conditional branch, and the run leaves that body as a
//! single body — its members keeping their own order — through the one plain
//! branch edge the named `destination` selects, taking that instruction's
//! position in the landing arm's body. The destination and every later
//! position keep their relative order one run-width later. The `dead_path`
//! audit is what makes that sound: nothing on the abandoned traversals may
//! observe what the run wrote.
//!
//! Naming one member as both bounds is the run of one, so the single-member
//! sink is this admission's own granularity rather than a family of its own.
...
pub use rewrite::relocate_selected_members_into_arm;
