...
const HEAD: SelectedInstructionId = SelectedInstructionId(7);
const MID: SelectedInstructionId = SelectedInstructionId(8);
const TAIL: SelectedInstructionId = SelectedInstructionId(9);
const BRANCH: SelectedInstructionId = SelectedInstructionId(10);
const T_JUMP: SelectedInstructionId = SelectedInstructionId(11);
const RET: SelectedInstructionId = SelectedInstructionId(12);
...
const R_HEAD: VirtualRegisterId = VirtualRegisterId(6);
const R_MID: VirtualRegisterId = VirtualRegisterId(7);
const R_TAIL: VirtualRegisterId = VirtualRegisterId(8);
const R_BOUND: VirtualRegisterId = VirtualRegisterId(9);
...
/// the arm's body, its jump and its edge, while J's body keeps its order
/// behind the landing index.
fn fixture(target: NativeTarget) -> ValidatedBypassRelocation {
    ...
    let registers = vec![
        ...
        result_register(R_HEAD, HEAD, 7),
        result_register(R_MID, MID, 8),
        result_register(R_TAIL, TAIL, 9),
        result_register(R_BOUND, LEAD, 10),
    ];
        ...
        functions: vec![SelectedFunction {
            ...
            blocks: vec![
                ...
                SelectedBlock {
                    ...
                    instructions: vec![
                        materialization(HEAD, R_HEAD, 15),
                        materialization(MID, R_MID, 17),
                        materialization(TAIL, R_TAIL, 19),
                    ],
...
/// The member crosses the whole triangle onto the destination's position on
/// every target: `MOVING` leaves B's body, `TRAIL`, the branch, both edges,
/// the arm, and its jump keep their positions, and the member lands at J's
/// head with the destination and every later position one slot later in
/// their original order — identity, kind, operands, and provenance intact.
/// The replayed proposal restores the source bit-identically.
#[test]
fn member_relocates_through_the_bypass() {
    for target in [
...
fn member_lands_at_the_named_position() {
    ...
    assert_eq!(
        last_body.transformed().functions[0].blocks[2]
            .instructions
            .iter()
            .map(|instruction| instruction.id)
            .collect::<Vec<_>>(),
...
/// branch, both edges, and the arm, while the block-head member crosses its
...
fn tail_and_head_members_relocate() {
    ...
    assert_eq!(
        tail.transformed().functions[0].blocks[2]
            .instructions
            .iter()
            .map(|instruction| instruction.id)
            .collect::<Vec<_>>(),
    ...
    assert_eq!(
        head.transformed().functions[0].blocks[2]
            .instructions
            .iter()
            .map(|instruction| instruction.id)
            .collect::<Vec<_>>(),
...
/// edges and no arm: the member crosses the branch and its edges alone and
...
fn raw_hazard_keeps_order_through_the_bypass() {
    ...
    let arm_reads = mutated(target, |function, environment| {
        ...
        function.blocks[1].instructions[0] = instruction(
            function.blocks[1].instructions[0].id,
            SelectedInstructionKind::CopyI64,
            &copy,
...
/// in the arm does not block a flag-inert member.
#[test]
...
/// while a read it shares with the member admits, and the arm's `Jump`
...
fn terminator_positions_couple() {
    ...
    // The arm's `Jump` reading the member's result refuses: the member
    // would land after the read.
...
/// and new positions — the arm edge, the bypass edge, and the arm's jump
/// edge alike: a member defining the transported argument would hand the
...
fn register_transports_bind_each_crossed_edge() {
    ...
    // single-edge family's did — on the arm edge and the bypass edge alike.
...
fn the_triangle_must_close() {
    ...
    assert_eq!(
        relocate(&arm_returns, &environment, MOVING, HEAD).unwrap_err(),
        BypassRelocationError::UnsupportedPair
    );
    // A second predecessor into the arm hands the join a path the member
    // never executed on.
    ...
    // A join that loops to itself runs the member once per iteration: the
    ...
    assert_eq!(
        relocate(&join_self_loop, &environment, MOVING, HEAD).unwrap_err(),
        BypassRelocationError::UnsupportedPair
    );
    ...
    // side of the audit refuses: the member's own block is never the join.
...
/// or anywhere inside the crossed window — the arm's body included — and a
...
fn barrier_kinds_and_call_roster_reject() {
    ...
    for kind in [
        ...
        assert_eq!(
            ...
            BypassRelocationError::UnsupportedInstruction,
            "member {kind:?}"
        );
...
fn only_the_named_bypass_window_relocates() {
    ...
    assert_eq!(
        ...
        .unwrap_err(),
        BypassRelocationError::SourceMismatch
    );
    assert_eq!(
        relocate_selected_instruction_through_bypass(
            &source,
...
fn replay_rejects_anything_but_the_move() {
    ...
    let member = displaced.functions[0].blocks[2].instructions.remove(1);
    displaced.functions[0].blocks[2]
        .instructions
        .insert(3, member.clone());
    assert_eq!(
        validate_bypass_relocation(&source, 0, MOVING, MID, &environment, budget(), displaced)
            .unwrap_err(),
        BypassRelocationError::ReplayMismatch
    );
    // The member left in its own block rejects.
    let mut unmoved = result.transformed().clone();
    unmoved.functions[0].blocks[2].instructions.remove(1);
    unmoved.functions[0].blocks[0]
        .instructions
        .insert(2, member);
    assert_eq!(
        validate_bypass_relocation(&source, 0, MOVING, MID, &environment, budget(), unmoved)
            .unwrap_err(),
        BypassRelocationError::ReplayMismatch
    );
    ...
    assert_eq!(
        validate_bypass_relocation(&source, 0, MOVING, MID, &environment, budget(), dropped)
            .unwrap_err(),
        BypassRelocationError::ReplayMismatch
    );
    ...
    assert_eq!(
        validate_bypass_relocation(&source, 0, MOVING, MID, &environment, budget(), edited)
            .unwrap_err(),
        BypassRelocationError::ReplayMismatch
    );
...
/// sealed analysis boundary: the tail member crosses the same triangle
/// onto it, a hazard-coupled member still declines, and an in-block family
/// still admits.
#[test]
fn bypass_relocation_is_deterministic_and_re_admitted() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let source = fixture(target);
    ...
    // member crosses the same triangle onto it.
    let again = relocate_selected_instruction_through_bypass(
        &first,
        0,
        TRAIL,
        HEAD,
    ...
    .unwrap();
    assert_eq!(
        again.transformed().functions[0].blocks[2]
            .instructions
            .iter()
            .map(|instruction| instruction.id)
            .collect::<Vec<_>>(),
    ...
    .unwrap();
    assert_eq!(
        swapped.transformed().functions[0].blocks[2]
            .instructions
            .iter()
            .map(|instruction| instruction.id)
            .collect::<Vec<_>>(),
