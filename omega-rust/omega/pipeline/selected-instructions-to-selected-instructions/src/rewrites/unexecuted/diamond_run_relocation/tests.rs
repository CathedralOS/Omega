...
/// return`. The default move relocates the run `RUN_A..=RUN_B` onto
/// `HEAD`'s position — the head of J's body — crossing `TRAIL`, the branch
/// terminator, both branch edges, both arms' bodies, both arm jumps and
/// their edges, while J's body keeps its order behind the landing index.
fn fixture(target: NativeTarget) -> ValidatedDiamondRunRelocation {
...
fn relocate(
    ...
    relocate_selected_run_through_diamond(
        source,
        0,
        first,
        last,
...
/// The run crosses the whole diamond onto the destination's position on
/// every target: `RUN_A` and `RUN_B` leave B's body in their own order,
/// `TRAIL`, the branch, both edges, both arms, and both arm jumps keep
/// their positions, and the run lands at J's head with the destination
/// and every later position one run-width later in their original order —
/// identity, kind, operands, and provenance intact. The replayed proposal
/// restores the source bit-identically.
#[test]
fn run_relocates_through_the_diamond() {
    for target in [
...
/// arm the branch alone still reaches: the run crosses it once.
#[test]
...
/// consumer with it through the diamond.
#[test]
fn internally_coupled_run_moves_as_one_body() {
    ...
    // the run admits.
    assert_eq!(
        crate::rewrites::unexecuted::relocate_selected_instruction_through_diamond(
            &source,
            0,
            RUN_A,
            HEAD,
        ...
        .unwrap_err(),
        crate::rewrites::unexecuted::DiamondRelocationError::UnsupportedPair
    );
}
...
/// writer in its own tail or inside an arm; and a flag consumer sitting
/// in an arm does not block a flag-inert run.
#[test]
...
/// while a read it shares with a member admits, and an arm's `Jump`
...
fn terminator_positions_couple() {
    ...
    // An arm's `Jump` reading a member's result refuses: the member would
    // land after the read.
...
/// new positions — both branch edges and each arm's jump edge: a member
/// defining the transported argument would hand the binding a stale
...
fn the_diamond_must_close() {
    ...
    // A second predecessor into an arm hands the join a path the run
    // never executed on.
    ...
    // The join as the run's own block is an in-block move through a
...
/// and a call-roster row makes an instruction a barrier even when its
/// kind is register-pure; the branch and arm-jump terminator ids
/// included.
#[test]
fn barrier_kinds_and_call_roster_reject() {
    ...
    for kind in [
        ...
        assert_eq!(
            ...
            DiamondRunRelocationError::UnsupportedInstruction,
            "member {kind:?}"
        );
...
/// A boundary settlement positioned past the run's first index observed
...
fn only_the_named_diamond_window_relocates() {
    ...
    assert_eq!(
        ...
        .unwrap_err(),
        DiamondRunRelocationError::SourceMismatch
    );
    assert_eq!(
        relocate_selected_run_through_diamond(
            &source,
    ...
    // A run whose members sit in different blocks binds nothing: the
    // last member must follow the first inside the first's own block.
    assert_eq!(
    ...
    // A run in a block without the branch names no window: the join's
    // own members have only a return terminator.
    assert_eq!(
...
fn replay_rejects_anything_but_the_move() {
    ...
    permuted.functions[0].blocks[3].instructions.swap(1, 2);
    assert_eq!(
        validate_diamond_run_relocation(
            &source,
            0,
...
fn measured_validation_step_boundary() {
    ...
    assert_eq!(
        ...
        .unwrap_err(),
        DiamondRunRelocationError::WorkBudgetExceeded
    );
    // Landing at `MID` crosses one more surface pair per member — the
    // join head.
    let exact = OptimizationWorkBudget::new(1, 1, steps + 4, 1, 1).unwrap();
    relocate_selected_run_through_diamond(&source, 0, RUN_A, RUN_B, MID, &environment, exact)
        .unwrap();
...
/// sealed analysis boundary: the remaining run crosses the same diamond
/// onto it, a hazard-coupled member still declines, and an in-block
/// family still admits.
#[test]
fn diamond_run_relocation_is_deterministic_and_re_admitted() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let source = fixture(target);
    ...
    // remaining contiguous run in B crosses the same diamond onto it.
    let again =
        relocate_selected_run_through_diamond(&first, 0, LEAD, TRAIL, HEAD, &environment, budget())
            .unwrap();
    assert_eq!(
    ...
    let swapped = crate::rewrites::unexecuted::relocate_selected_run(
        &first,
        0,
        MID,
        TAIL,
