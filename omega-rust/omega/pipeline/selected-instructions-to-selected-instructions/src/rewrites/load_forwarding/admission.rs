...
use crate::rewrites::place_storage::{
    SubjectExtent, constant_index, extent_intersects, extent_reached_by,
    local_slot_is_place_storage, structural_place_declarations,
};
...
impl Forwarded {
    /// The read bytes as the shared subject extent — `dynamic` when the
    /// indexed load's runtime index is unresolved.
    ...
    /// Whether the row's access overlaps the read extent.
    fn intersects(&self, access: &SelectedMemoryAccess) -> bool {
        extent_intersects(self.subject_extent(), access)
    }

    /// A roster row's reach decided on the read extent. Only a written
    /// byte-sequence row carries the sequence index this check lands — the
    /// read routes name read rows, and a read sequence's index places a byte
    ...
    fn reached_by(&self, access: &SelectedMemoryAccess, function: &SelectedFunction) -> bool {
        ...
        extent_reached_by(self.subject_extent(), index, access, function)
    }
}
