...
fn fixture(target: NativeTarget) -> ValidatedCommutingRunRelocation {
...
fn relocate(
    ...
    relocate_selected_commuting_run(
        source,
        0,
        first_member,
        last_member,
...
fn unaccounted_and_barrier_positions_reject() {
    ...
    for position in [0, 2, 3] {
        ...
        assert_eq!(
            relocate(&bare, &environment, STORE_A, LOAD_B, LOAD_D).unwrap_err(),
            CommutingRunRelocationError::UnsupportedInstruction,
            "position {position}"
        );
    }
    for kind in [
...
fn admission_reports_its_own_reasons() {
    ...
    // A one-member run is the commuting relocation's granularity.
    assert_eq!(
        relocate(&source, &environment, STORE_A, STORE_A, LOAD_D).unwrap_err(),
        CommutingRunRelocationError::UnsupportedPair
    );
    assert_eq!(
        relocate(
        ...
        .unwrap_err(),
        CommutingRunRelocationError::SourceMismatch
    );
    assert_eq!(
        relocate_selected_commuting_run(
            &source,
    ...
    assert_eq!(
        relocate_selected_commuting_run(&source, 0, STORE_A, LOAD_B, LOAD_D, &arm64, budget())
            .unwrap_err(),
        CommutingRunRelocationError::SourceMismatch
    );
    let tight = OptimizationWorkBudget::new(1, 1, 1, 1, 1).unwrap();
    assert_eq!(
        relocate_selected_commuting_run(&source, 0, STORE_A, LOAD_B, LOAD_D, &environment, tight)
            .unwrap_err(),
        CommutingRunRelocationError::WorkBudgetExceeded
    );
}
...
fn replay_restores_the_source_by_content() {
    ...
    let source = fixture(target);
    let result = relocate(&source, &environment, STORE_A, LOAD_B, LOAD_D).unwrap();
    let reproposed = validate_commuting_run_relocation(
        &source,
        0,
        STORE_A,
        LOAD_B,
    ...
    assert_eq!(
        ...
        .unwrap_err(),
        CommutingRunRelocationError::ReplayMismatch
    );
...
fn measured_validation_step_boundary_admits_and_rejects() {
    ...
    for (source, exact_steps) in [
        ...
        let result = relocate_selected_commuting_run(
            &source,
            0,
            STORE_A,
            LOAD_B,
...
fn relocation_is_deterministic_and_re_admitted() {
    ...
    // published source.
    let restored =
        relocate_selected_commuting_run(&first, 0, STORE_A, LOAD_B, MAT_C, &environment, budget())
            .unwrap();
    ...
    assert_eq!(
        relocate_selected_commuting_run(&first, 0, LOAD_B, STORE_A, MAT_C, &environment, budget())
            .unwrap_err(),
        CommutingRunRelocationError::UnsupportedPair
    );
}
