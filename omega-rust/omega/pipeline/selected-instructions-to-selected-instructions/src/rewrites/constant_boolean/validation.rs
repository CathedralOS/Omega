...
use super::{ConstantBooleanError, ConstantBooleanReceipt, ValidatedConstantBoolean, admission};
use crate::ValidatedSelectedAnalysis;

/// Independently consume the proposed program: admission re-derives the
/// admitted materialization and re-resolves its flag units' reaching
/// compare from the source, the proposed instruction must equal the
/// reconstructed materialize form, and reinserting the source boolean must
...
pub fn validate_constant_boolean_fold(
    ...
    restored.functions[function_index].blocks[admitted.block_index].instructions
        [admitted.materialization_index] = source.selected_plan().functions[function_index].blocks
        [admitted.block_index]
