...
//! whose semantic successor is the destination's block, and hand the
//! crossed window to the shared run audit — `crossed_window` derives the
//! positions and edges every acyclic path between the two blocks crosses
//! (exactly this `Jump` edge under the gates below) and
//! `admit_run_relocation` proves the window independent once — no register
//! or condition-state hazard between any member and any crossed position,
//! no member interference with the edge's register transports, no
...
pub(super) fn admit(
    ...
    // this `Jump` edge — so the path walk is bounded by the function's edge
    // roster alone. The shared audit applies the hazard, memory-roster,
    // transport, and settlement checks once across the run's members.
    ...
    // locate the run's first member and count predecessor edges; the path
    // walk touches each edge once; the window audit walks every
    // member-against-crossed operand and unit surface, plus the function's
    // three rosters and the edge's binding roster once per member.
...
fn rejection(rejection: RunRelocationRejection) -> EdgeRunRelocationError {
    match rejection {
        RunRelocationRejection::Unschedulable => EdgeRunRelocationError::UnsupportedInstruction,
        ...
        | RunRelocationRejection::Settlement => EdgeRunRelocationError::UnsupportedPair,
    }
}
