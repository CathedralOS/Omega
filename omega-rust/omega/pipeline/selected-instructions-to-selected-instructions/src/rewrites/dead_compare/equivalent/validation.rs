...
use super::{
    EquivalentCompareError, EquivalentCompareReceipt, ValidatedEquivalentCompare, admission,
};
use crate::ValidatedSelectedAnalysis;

/// Independently consume the proposed program: admission re-derives the
/// admitted compare, re-resolves every published unit's value-equivalent
/// shadows, and re-audits the operand intervals from the source; the
/// proposal's function must equal the independently computed removal —
/// compare dropped, settlements shifted — and reinserting the compare with
/// the source settlements must restore the complete source by content:
