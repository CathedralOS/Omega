...
use super::{ForkRelocationError, ForkRelocationReceipt, ValidatedForkRelocation, admission};
use crate::ValidatedSelectedAnalysis;

/// Independently consume the proposed program: admission re-derives the
/// admitted member, fork, and landing index from the source, the proposal
/// must place exactly the member's instruction on the landing index in the
/// arm block, and moving it back must restore the complete source by
/// content — every crossed instruction, every other block and instruction,
