...
fn catalog_rows_declare_symbolic_instruction_pairs() {
    ...
    // The byte-view address family folds a materialized literal at either
    // `Use` position of the projection into the constant-offset
    // `AddressOffset` form: the operand-1 offset literal and the operand-0
    // backing literal both commute onto `(backing + offset) modulo 2^64`,
    // so the family declares one pair per operand grammar rewriting through
    // the same `AddressOffset` row. The shared 12-bit bound is the aarch64
    // `add` immediate's encoding limit, the narrowest any target's
    // address-offset row admits.
    let &[address_offset_rule, address_backing_rule] = address_offset.payload().pairs() else {
        panic!("the byte-view address family declares one pair per operand grammar")
    };
    for pair in [address_offset_rule, address_backing_rule] {
        assert_eq!(pair.producer(), MachineSemanticKind::MaterializeI64);
        assert_eq!(pair.consumer(), MachineSemanticKind::ByteViewAddress);
        assert_eq!(pair.rewritten(), MachineSemanticKind::AddressOffset);
        assert_eq!(pair.immediate_bound(), PairImmediateBound::Encoding(4095));
        assert!(pair.admits_immediate(4095));
        assert!(!pair.admits_immediate(4096));
        assert_eq!(pair.result(), PairResultDisposition::ScalarRegister);
    ...
    assert_eq!(
        address_backing_rule.rewrite_consumer(SelectedInstructionKind::ByteViewAddress, 12, None),
        Some(SelectedInstructionKind::AddressOffset { byte_offset: 12 })
    );
    assert_eq!(
    ...
    // The exact-divide zero-dividend family declares the fourth
    ...
    // operand position keeps the grammars disjoint, and its operand shape
    // keeps the provably-zero auxiliary `Use` tail a `div` realization
    // reads as the dividend's upper half under explicit custody.
    let &[divide_zero_rule] = divide_zero.payload().pairs() else {
        panic!("the divide zero-dividend family declares one pair rule")
    };
    assert_eq!(
        divide_zero.optimization(),
    ...
    assert_eq!(
        divide_zero_rule.producer(),
        MachineSemanticKind::MaterializeI64
    );
    assert_eq!(
        divide_zero_rule.consumer(),
        MachineSemanticKind::ExactDivideU64
    );
    assert_eq!(
        divide_zero_rule.rewritten(),
        MachineSemanticKind::MaterializeI64
    );
    assert_eq!(
        divide_zero_rule.immediate_bound(),
        PairImmediateBound::Exactly(0)
    );
    assert!(divide_zero_rule.admits_immediate(0));
    assert!(!divide_zero_rule.admits_immediate(1));
    assert!(!divide_zero_rule.admits_immediate(2));
    assert!(!divide_zero_rule.admits_immediate(u64::MAX));
    ...
    assert_ne!(
        divide_zero_rule.victim_operand(),
        divide_rule.victim_operand()
    );
    assert_eq!(
        divide_zero_rule.result(),
        PairResultDisposition::ScalarRegister
    );
    assert_eq!(
    ...
    assert_eq!(
        divide_zero_rule.rewrite_consumer(divide_kind, 0, Some(u64_scalar)),
        Some(SelectedInstructionKind::MaterializeI64 {
            value: IntegerValue::Unsigned(0),
        })
    );
    assert_eq!(
        divide_zero_rule.rewrite_consumer(divide_kind, 0, Some(i64_scalar)),
        Some(SelectedInstructionKind::MaterializeI64 {
            value: IntegerValue::Signed(0),
        })
    );
    // The consumer guard binds the rule to its own consumer kind: the
    // zero-dividend divide rule never rewrites a remainder or an and.
    assert_eq!(
        divide_zero_rule.rewrite_consumer(remainder_kind, 0, Some(u64_scalar)),
        None
    );
    assert_eq!(
        divide_zero_rule.rewrite_consumer(
            SelectedInstructionKind::BitwiseAndI64,
            0,
            Some(u64_scalar)
        ),
    ...
    assert_eq!(
        divide_zero_rule.rewrite_consumer(divide_kind, 0, None),
        None
    );
    ...
    // The saturating-subtract identity family declares one pair per
    // carrier — the right-literal grammar alone: a literal of exactly
    // zero at the operand-1 `Use` folds a `SaturatingSubtract` into a
    // `CopyI64` of the operand-0 `Use` — `x -| 0` is `x`, already inside
    // the carrier's bounds. Saturating subtraction does not commute —
    ...
    let saturating_subtract_zero_pairs = saturating_subtract_zero.payload().pairs();
    assert_eq!(saturating_subtract_zero_pairs.len(), 8);
    assert_eq!(
        saturating_subtract_zero.optimization(),
        Optimization::SelectedIncomingSaturatingSubtractZeroIdentityCopy
    );
    let unsigned_carriers = [
        SaturatingCarrier::U8,
        SaturatingCarrier::U16,
        SaturatingCarrier::U32,
        SaturatingCarrier::U64,
    ...
    for (index, pair) in saturating_subtract_zero_pairs.iter().copied().enumerate() {
        assert_eq!(pair.producer(), MachineSemanticKind::MaterializeI64);
        let carrier = if index < unsigned_carriers.len() {
            unsigned_carriers[index]
        } else {
            signed_carriers[index - unsigned_carriers.len()]
        };
        assert_eq!(
            pair.consumer(),
        ...
        // sibling kind, or any other consumer kind never rewrites through
        // it.
    ...
    // The remainder minus-one family declares the fourth trap-carrying
    // machine-effect relationship: a divisor literal of `u64::MAX` — the
    // normalized-i64 divisor `-1` — folds `WrappingRemainderI64` into a
    // `MaterializeI64` of the constant zero — a remainder by minus one is
    ...
    // grammar. The family shares the divisor-one fold's consumer kind and
    // operand position; the grammars stay disjoint on the folded
    // literal's value.
    let &[remainder_minus_one_rule] = remainder_minus_one.payload().pairs() else {
        panic!("the remainder minus-one family declares one pair rule")
    };
    assert_eq!(
        remainder_minus_one.optimization(),
        Optimization::SelectedIncomingWrappingRemainderMinusOneZeroMaterialization
    );
    assert_eq!(
        remainder_minus_one_rule,
        SelectedInstructionPairRule::WRAPPING_REMAINDER_MINUS_ONE_MATERIALIZE
    );
    assert_eq!(
        remainder_minus_one_rule.producer(),
        MachineSemanticKind::MaterializeI64
    );
    assert_eq!(
        remainder_minus_one_rule.consumer(),
        MachineSemanticKind::WrappingRemainderI64
    );
    assert_eq!(
        remainder_minus_one_rule.rewritten(),
        MachineSemanticKind::MaterializeI64
    );
    assert_eq!(
        remainder_minus_one_rule.immediate_bound(),
        PairImmediateBound::Exactly(u64::MAX)
    );
    assert!(remainder_minus_one_rule.admits_immediate(u64::MAX));
    assert!(!remainder_minus_one_rule.admits_immediate(0));
    assert!(!remainder_minus_one_rule.admits_immediate(1));
    assert!(!remainder_minus_one_rule.admits_immediate(u64::MAX - 1));
    ...
    assert_eq!(
        remainder_minus_one_rule.result(),
        PairResultDisposition::ScalarRegister
    );
    assert_eq!(
    ...
    assert_eq!(
        remainder_minus_one_rule.rewrite_consumer(remainder_kind, 0, Some(i64_scalar)),
        Some(SelectedInstructionKind::MaterializeI64 {
            value: IntegerValue::Signed(0),
        })
    );
    assert_eq!(
        remainder_minus_one_rule.rewrite_consumer(
    ...
    assert_eq!(
        remainder_minus_one_rule.rewrite_consumer(remainder_kind, 0, None),
        None
    );
    ...
    // saturating-add and saturating-subtract folds drop the consumer's
    // `early_clobber` result and scratch marks.
    for entry in [
        add,
        subtract,
        compare,
    ...
    assert_eq!(
        enabled_pair_rules(LiteralFoldPolicy::EXACT_DIVIDE_ZERO_V1).collect::<Vec<_>>(),
        vec![SelectedInstructionPairRule::EXACT_DIVIDE_ZERO_DIVIDEND_MATERIALIZE]
    );
    assert_eq!(
    ...
    assert_eq!(
        divide_zero_rule.immediate_constraint_key(&keys),
        Some(keys.materialize_i64)
    );
    assert_eq!(
    ...
    // Every saturating-subtract-zero pair — the unsigned-carrier rules and
    // the signed-carrier scratch-defs rules alike — rewrites through the
    // same copy row.
...
fn declared_unit_effects_admit_the_real_immediate_rows() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        for rule in [
            ...
            SelectedInstructionPairRule::BYTE_VIEW_ADDRESS_OFFSET_U12,
            // The backing fold rewrites into the same unit-clean
            ...
            SelectedInstructionPairRule::SATURATING_ADD_ZERO_COPY,
            SelectedInstructionPairRule::SATURATING_ADD_ZERO_LEFT_COPY,
            // The saturating-subtract-zero grammar rewrites into the same
...
fn declared_machine_effects_admit_the_real_catalog_declarations() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        // The remainder minus-one pair admits its own triple on both
        // targets: the same isolated producer, possibly-faulting
        // remainder consumer, and isolated materialization the
        // divisor-one fold binds — the minus-one divisor discharges the
        // encoded fault the same declaration-level way, a divisor of
        // `-1` never dividing by zero and the one overflow-prone
        // dividend being the case the kind defines to produce zero.
        {
            let rule = SelectedInstructionPairRule::WRAPPING_REMAINDER_MINUS_ONE_MATERIALIZE;
            let producer = declaration(rule.producer());
            let consumer = declaration(rule.consumer());
            let rewritten = declaration(rule.rewritten());
            assert!(
        ...
        // The divide zero-dividend pair admits its own triple on both
        // targets under the obligation-discharged surface: the same
        // possibly-faulting divide consumer the divisor-one fold binds and
        // the isolated materialization — the declaration-level requirement
        // is the shared fault-discharged shape; the distinguishing
        // obligation custody is instruction-level and the producer and
        // replay check it against the consumer record itself.
        {
            let rule = SelectedInstructionPairRule::EXACT_DIVIDE_ZERO_DIVIDEND_MATERIALIZE;
            let producer = declaration(rule.producer());
            let consumer = declaration(rule.consumer());
            let rewritten = declaration(rule.rewritten());
            assert!(
