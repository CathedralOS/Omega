use super::{
    ...
    restage_literal, staged_and_inputs, staged_inputs, staged_xor_inputs, validate,
};
use crate::{
...
#[test]
fn xor_zero_fold_rewrites_the_consumer_to_a_surviving_operand_copy_on_both_linux_targets() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let effect_catalog =
            validated_machine_effect_catalog(environment.target(), environment.constraints())
                .unwrap();
        // The xor form binds the flag-clobbering subtract constraint row:
        // on x86-64 the consumer carries an `rflags` clobber the fold drops
        // with the folded form, on aarch64 the `eor` row is already
        // flag-free.
        ...
        for literal_operand in [0u16, 1u16] {
            let inputs = staged_xor_inputs(target, literal_operand);
            ...
            .unwrap_or_else(|error| {
                panic!(
                    "xor-zero fold with the literal at operand {literal_operand} on {target:?} \
            ...
            // fieldless xor kind carries no obligation custody.
...
#[test]
fn xor_zero_fold_rejects_a_nonzero_literal() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let environment = baseline_target_register_environment(target).unwrap();
        ...
        // position.
        for literal_operand in [0u16, 1u16] {
            let mut inputs = staged_xor_inputs(target, literal_operand);
...
#[test]
fn xor_zero_fold_rejects_a_literal_claiming_the_wrong_operand_position() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let environment = baseline_target_register_environment(target).unwrap();
...
#[test]
fn xor_zero_fold_rejects_malformed_operand_arrangements() {
...
#[test]
fn xor_zero_fold_rejects_a_scratch_def_the_copy_grammar_cannot_drop() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let keys = environment.allocation_constraint_keys();
    let effect_catalog =
    ...
    assert_eq!(
        environment
            ...
            .len(),
        3,
        "the x86-64 xor row carries no scratch def — the test appends one"
    );
...
#[test]
fn xor_zero_fold_rejects_consumer_operands_carrying_forbidden_bindings() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    let keys = environment.allocation_constraint_keys();
    let effect_catalog =
    ...
    // The xor row pins nothing: under the declared `Isolated` unit-effect
...
#[test]
fn xor_zero_fold_rejects_consumers_the_selection_does_not_enable() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    // The xor's operand grammar admits the literal only under the xor-zero
    ...
    // And the xor-zero policy admits no other consumer: the compare
    // fixture's flag-defining consumer has no `Def` operand 2 the
    // copy-result grammar could bind, and the and fixture's consumer kind
    ...
    let and = staged_and_inputs(target, 1);
    assert_eq!(
        fold_with(&and, &environment, LiteralFoldPolicy::BITWISE_XOR_ZERO_V1).map(|_| ()),
        Err(LiteralFoldError::ConsumerMismatch { function: 0 })
    );
}

#[test]
fn xor_zero_fold_replay_rejects_every_decision_field_substitution() {
    let target = NativeTarget::linux_x64();
    let environment = baseline_target_register_environment(target).unwrap();
    // The left-literal grammar exercises the operand-0 fold.
    let inputs = staged_xor_inputs(target, 0);
    let result = fold_with(
        &inputs,
        &environment,
        LiteralFoldPolicy::BITWISE_XOR_ZERO_V1,
    )
    .expect("the staged xor-zero fold should validate");

    for mutation in 0..11 {
        let mut plan = result.plan().clone();
        match mutation {
            // The recorded result register is the xor's own `Def`, not the
            // surviving `Use`.
            ...
            // The recorded constraint is the `CopyI64` row the xor-zero
            ...
            // A policy without the xor-zero bit cannot replay the fold:
...
#[test]
fn xor_zero_fold_reports_and_enforces_its_measured_work() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let environment = baseline_target_register_environment(target).unwrap();
        for literal_operand in [0u16, 1u16] {
            let inputs = staged_xor_inputs(target, literal_operand);
...
#[test]
fn xor_zero_fold_is_deterministic_and_a_fixed_point_on_its_output() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let environment = baseline_target_register_environment(target).unwrap();
        for literal_operand in [0u16, 1u16] {
            let inputs = staged_xor_inputs(target, literal_operand);
