...
#[test]
fn saturating_add_zero_fold_rewrites_the_consumer_to_a_surviving_operand_copy() {
    for (target, block0) in positive_fixtures() {
        ...
        let effect_catalog =
            validated_machine_effect_catalog(environment.target(), environment.constraints())
                .unwrap();
        // The u64 saturating-add row carries the unit effects this family
        // exists to retire: aarch64's flag-setting `adds` realization
        // defines `nzcv` with no clobber or operand mark, while x86-64's
        // carry-select form clobbers `rflags` and marks its result
        // early-clobber. Neither declares an implicit use.
        let consumer_row = environment.constraint(keys.saturating_add_u64).unwrap();
        ...
        for literal_operand in [0u16, 1u16] {
            ...
            .unwrap_or_else(|error| {
                panic!(
                    "saturating-add-zero fold with the literal at operand {literal_operand} on \
            ...
            // fieldless saturating-add kind carries no obligation custody.
...
#[test]
fn saturating_add_zero_fold_rejects_while_another_instruction_reads_the_defined_unit() {
        ...
        for literal_operand in [0u16, 1u16] {
            ...
            if target == NativeTarget::linux_arm64() {
                assert_eq!(
                    ...
                    .map(|_| ()),
                    Err(LiteralFoldError::EffectSurfaceMismatch { function: 0 }),
                    "literal at operand {literal_operand} with a block-1 nzcv reader"
                );
                assert_eq!(
                    validate(&inputs, &environment, inputs.selected.plan().clone()).map(|_| ()),
                    Err(LiteralFoldError::EffectSurfaceMismatch { function: 0 }),
                    "literal at operand {literal_operand} with a block-1 nzcv reader, replay"
                );
            } else {
...
#[test]
fn saturating_add_zero_fold_rejects_a_literal_claiming_the_wrong_operand_position() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        let environment = baseline_target_register_environment(target).unwrap();
        ...
        // the disjoint grammar, whose operand check finds the surviving
        // register where the victim must sit. Claiming the operand-2 `Def`
        // position names no grammar's victim position at all.
...
#[test]
fn saturating_add_zero_fold_rejects_malformed_operand_arrangements() {
