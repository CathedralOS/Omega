...
pub(super) fn derive_early_clobbers(
    ...
    for block in &live.blocks {
        for instruction in &block.instructions {
            ...
            for definition in &marked {
                if (!multiple_definitions && marked.len() != 1)
                ...
                for operand in &operands {
                    if seen.contains(&operand.virtual_register)
                        || ordinals.contains(&operand.operand)
                ...
                let tied_edges = live
                    ...
                    .filter_map(|tied_definition| {
                        live.operand_positions
                            ...
                            .map(|source| {
                                (source.virtual_register, tied_definition.virtual_register)
                            })
                    })
                    .collect::<Vec<_>>();
    ...
    if let Some(unmatched) = live.operand_positions.iter().find(|operand| {
        operand.early_clobber
            && !rows.iter().any(|row| {
                row.instruction == operand.instruction
                    && row.def_operand == operand.operand
                    && row.def_virtual_register == operand.virtual_register
            })
    }) {
        return Err(LiveRangeError::UnsupportedEarlyClobber {
            function,
