...
fn verified_parameter_sourced_write_only_store_reaches_canonical_installation() {
    ...
    for native_target in [
        ...
        let target_operations::TargetUnitWriteOnlyPrimitiveStoreSource::Parameter {
            parameter_index,
            ..
        } = corrupted_source
        else {
        ...
        *parameter_index = 1;
        ...
        assert!(matches!(
            assigned_source,
            assigned_target_operations::AssignedUnitWriteOnlyPrimitiveStoreSource::Parameter {
                parameter_index: 0,
                source_value,
                scalar_type,
        ...
        let corrupted_location = corrupted_assigned
            ...
            .find_map(|function| match &mut function.operation {
                assigned_target_operations::AssignedOperation::UnitBody(body) => body
                    ...
                    .find_map(|operation| match operation {
                        assigned_target_operations::AssignedUnitOperation::WriteOnlyPrimitiveStore {
                            source: assigned_target_operations::AssignedUnitWriteOnlyPrimitiveStoreSource::Parameter {
                                location,
                                ..
                            },
                            ..
        ...
        *corrupted_location = match native_target.architecture {
            target::Architecture::X86_64 => {
                assigned_target_operations::AssignedScalarLocation::Register(
        ...
        let expected_register = match native_target.architecture {
            target::Architecture::X86_64 => target_operations::MachineRegister::X86Rdi,
        ...
        let changed_source = &mut changed_location
            ...
            .source;
        let machine_code::UnitWriteOnlyPrimitiveStoreSourceRecord::Parameter { location, .. } =
            changed_source
        else {
            unreachable!()
        };
        *location = match native_target.architecture {
        ...
        assert_eq!(
            image_emission::build_object_artifact(&changed_location),
            Err(
...
fn parameter_sourced_write_only_store_caller_reaches_verified_terminal_execution() {
    ...
    assert_eq!(
        terminal_codec::decode_module(&semantic).expect("decode Unit caller semantics"),
        lowered.semantic_module
    );
    let proof = terminal_codec::encode_proof_bundle(&lowered.proof_bundle)
        .expect("encode Unit caller proof bundle");
    let semantic_vocabulary::ScalarType::Integer(integer) = scalar_parameter.scalar_type else {
    ...
    for native_target in [
        ...
        let target_operations::TargetOperation::UnitBody(target_body) = &target_root.operation
        else {
        ...
        let target_operations::TargetOperation::UnitBody(corrupted_body) = &mut corrupted_target
            .functions
            .iter_mut()
        ...
        let target_operations::TargetUnitScalarArgumentSource::Parameter { source_value, .. } =
        ...
        *source_value = semantic_vocabulary::ValueId::new(9_999_999).unwrap();
        ...
        let assigned_target_operations::AssignedUnitOperation::Call {
        ...
        assert!(matches!(
            ...
            [assigned_target_operations::AssignedUnitScalarCallArgument {
                parameter_index: 0,
                source: assigned_target_operations::AssignedUnitScalarArgumentSource::Parameter {
                    parameter_index: 0,
                    source_value,
                    scalar_type,
                    ..
        ...
        let emitted = machine_emission::emit_machine_code(&assigned_plan)
        ...
        let source = &mut corrupted_emitted
            ...
            .scalar_arguments[0]
            .source;
        let machine_code::InternalUnitScalarArgumentSourceRecord::Parameter { location, .. } =
            source
        else {
            unreachable!()
        };
        *location = match native_target.architecture {
        ...
        assert_eq!(
            image_emission::build_object_artifact(&corrupted_emitted),
            Err(image_emission::ObjectError::InvalidInternalUnitCallEvidence(abstract_plan.entry))
        );
        let object = image_emission::build_object_artifact(&emitted)
    ...
    let terminal_psi::OperationKind::CallUnit { arguments, .. } = &mut missing_call.kind else {
    ...
    assert!(matches!(
        ...
        Err(
            terminal_verifier::ModuleError::CallArgumentArityMismatch {
                operation,
                expected: 1,
                actual: 0,
            }
