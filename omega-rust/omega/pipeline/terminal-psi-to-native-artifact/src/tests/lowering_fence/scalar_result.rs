...
fn checked_with_selected_integer_operator_store() -> Arc<checked_trees::CheckedTrees> {
    let source = r#"
        data CheckedMath {}
...
fn scalar_result_home_reaches_a_write_only_store_and_canonical_installation() {
    ...
    let unit_call = root.blocks[0]
        .operations
        .iter()
        .find(|operation| matches!(operation.kind, terminal_psi::OperationKind::CallUnit { .. }))
        .expect("runtime scalar result caller retains one Unit call");
    assert!(matches!(
        scalar_call.kind,
        terminal_psi::OperationKind::Call { .. }
    ));
    let terminal_psi::OperationResult::Scalar(result) = &scalar_call.result else {
        panic!("first operation produces the runtime scalar")
    };
    let terminal_psi::OperationKind::CallUnit { arguments, .. } = &unit_call.kind else {
        panic!("second operation forwards the runtime scalar")
    };
    assert_eq!(arguments, &[result.id]);
    let semantic_vocabulary::ScalarType::Integer(integer) = result.scalar_type else {
    ...
    assert_eq!(
        ...
        &[terminal_interpreter::TerminalStructuralPrimitiveValue {
            ...
            value: terminal_interpreter::TerminalScalarValue::Integer {
                scalar_type: integer,
                value: semantic_vocabulary::IntegerValue::Signed(23),
            },
        }]
    );
...
fn assert_scalar_result_home_directly_reaches_a_write_only_store_and_canonical_installation(
    ...
    assert!(matches!(
        store.kind,
        terminal_psi::OperationKind::WriteOnlyPrimitiveStore { value, .. }
            if value == result.id
    ));
    assert!(matches!(
    ...
    for native_target in [
        ...
        let target_store_home = target_body
            ...
            .find_map(|operation| match operation {
                target_operations::TargetUnitOperation::WriteOnlyPrimitiveStore {
                    source: target_operations::TargetUnitWriteOnlyPrimitiveStoreSource::Home(home),
                    ..
                } => Some(home),
                _ => None,
            })
        ...
        let assigned_store_home = assigned_body
            ...
            .find_map(|operation| match operation {
                assigned_target_operations::AssignedUnitOperation::WriteOnlyPrimitiveStore {
                    source:
                        assigned_target_operations::AssignedUnitWriteOnlyPrimitiveStoreSource::Home(
                            home,
                        ),
                    ..
                } => Some(home),
        ...
        assert_eq!(
            ...
            machine_code::UnitWriteOnlyPrimitiveStoreSourceRecord::Home(producer.result.home)
        );
        assert_eq!(
            caller.bytes.get(
        ...
        let mut changed_home = emitted.clone();
        let machine_code::UnitWriteOnlyPrimitiveStoreSourceRecord::Home(home) = &mut changed_home
            .functions
            .iter_mut()
            .find(|function| function.machine == emitted.entry)
            .unwrap()
        ...
        let mut bytes = image_emission::encode_installation_record(&installation)
        ...
        assert!(matches!(
            image_emission::decode_installation_record(&bytes),
            Err(
