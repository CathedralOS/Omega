...
fn evaluated_import(
    ...
    effects::provider_plan::EvaluatedForeignImport::from_retained_evidence(locator, receipt)
        .expect("receipt matches fixture locator")
}

pub(super) fn import_plan(symbol: &[u8], profile: target::TargetProfile) -> ProviderPlan {
        ...
        rows: vec![ProviderPlanRow {
            ...
            binding: ProviderBinding::Import {
                evaluated: evaluated_import(
                    target::normalize_foreign_locator(locator, profile).unwrap(),
                ),
            },
        }],
...
impl installation_evidence::ProviderExecutionEvidence for TestProviderExecution {
