...
fn checked_adapter_port_write_requires_exact_physical_and_service_policy() {
    ...
    let physical = checked_port_write_policy(target::TargetProfile::LinuxX64, 0x03f8);
    ...
    assert_eq!(
        ...
        checked_port_write_mechanism(target::TargetProfile::LinuxX64, 0x03f8),
    );
    assert_eq!(
    ...
    for wrong_policy in [
        ...
        checked_port_write_policy(target::TargetProfile::LinuxX64, 0x0080),
        checked_port_write_policy(target::TargetProfile::WindowsX64, 0x03f8),
    ] {
        assert!(
...
fn checked_adapter_port_write_rejects_service_target_and_plural_mechanism_drift() {
            ...
            [0x03f8, 0x0080]
                ...
                .map(|port| {
                    crate::realization::terminal_authority_policy::TerminalAuthorityPolicyRow::new(
                        checked_port_write_mechanism(target::TargetProfile::LinuxX64, port),
    ...
    let arm_physical = checked_port_write_policy(target::TargetProfile::LinuxArm64, 0x03f8);
    assert!(
        review_terminal_authority_closure(
            [14; 32],
            target::TargetProfile::LinuxArm64,
