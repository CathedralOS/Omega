...
pub(crate) fn wrapping_integer_multiply_parameters_return_artifact(
    integer_type: IntegerType,
    parameter_count: usize,
) -> (Vec<u8>, Vec<u8>) {
    assert!(
        parameter_count >= 2,
        "wrapping-multiply fixture needs two operands"
    );
    let scalar_type = ScalarType::Integer(integer_type);
    scalar_terminal_artifact(
        scalar_type,
        parameter_types(scalar_type, parameter_count),
        Some(OperationKind::WrappingIntegerMultiply {
