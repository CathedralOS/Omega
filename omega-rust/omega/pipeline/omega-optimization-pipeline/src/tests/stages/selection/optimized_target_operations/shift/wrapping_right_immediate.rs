//! Public optimized-target custody for constant wrapping integer shift-right materialization.

use super::super::*;

#[test]
fn optimized_target_lowering_retains_constant_wrapping_integer_shift_right_immediate_custody() {
            ...
            for count_type in native_types() {
                for (value, count) in boundary_pairs(value_type, count_type) {
                    let expected = value_type
                        ...
                        .unwrap();
                    let (semantic, proof) = wrapping_integer_shift_right_immediate_return_artifact(
                    ...
                    let AbstractToTargetFunctionTranslationDisposition::Validated(
                        AbstractToTargetFunctionTranslationReceipt::StraightLineWrappingIntegerShiftRightImmediate(row),
                    ) = target.translation_validation().function_roster()[0].translation()
                    else {
                        panic!("optimized wrapping shift-right must retain its exact immediate family")
                    };
                    assert_eq!(
...
fn boundary_pairs(
    ...
    let sign_fill_or_one = match value_type.sign() {
        IntegerSign::Signed => value_type.minimum_value(),
        IntegerSign::Unsigned => value_type.maximum_value(),
    };
    [
        (value_type.minimum_value(), zero),
        (value_type.maximum_value(), width),
        (value_type.maximum_value(), width_plus_one),
        (sign_fill_or_one, minus_one_or_maximum),
    ]
}
