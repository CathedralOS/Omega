...
#[test]
fn optimized_target_lowering_retains_exact_shift_left_parameter_custody() {
        ...
        for count_type in integers.iter().copied() {
            for (target_profile, registers, stack) in boolean_equal_location_cases() {
                for (parameter_count, expected) in [(2, registers), (10, stack)] {
                    ...
                    let (semantic, proof) = exact_integer_shift_left_parameters_return_artifact(
                        value_type,
                        count_type,
                        parameter_count,
                    );
                    ...
                    let AbstractToTargetFunctionTranslationDisposition::Validated(
                        ...
                        panic!("optimized exact shift-left must retain its family row")
                    };
                    assert_eq!(row.machine(), MachineId::new(30_001).unwrap());
                    assert_eq!(row.shift_operation(), OperationId::new(30_005).unwrap());
                    assert_eq!(row.obligation(), ObligationId::new(30_010).unwrap());
                    ...
                    assert!(matches!(
                        ...
                        TargetOperation::ReturnIntegerExpression {
                            ...
                            expression: TargetIntegerExpression::ExactShiftLeft {
                                psi_operation,
                                obligation,
                            ...
                            && *obligation == ObligationId::new(30_010).unwrap()
