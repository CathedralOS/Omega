...
#[test]
fn optimized_target_lowering_retains_saturating_integer_remainder_parameter_custody() {
    ...
    for scalar_type in integers {
        for (target_profile, registers, stack) in boolean_equal_location_cases() {
            for (parameter_count, expected) in [(2, registers), (10, stack)] {
                ...
                let (semantic, proof) = saturating_integer_remainder_parameters_return_artifact(
                    scalar_type,
                    parameter_count,
                );
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("optimized saturating integer remainder must retain its family row")
                };
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerExpression {
                        ...
                        expression: TargetIntegerExpression::SaturatingRemainder {
                            psi_operation,
