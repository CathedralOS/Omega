...
#[test]
fn optimized_target_lowering_retains_exact_integer_remainder_parameter_custody() {
    let integers = [IntegerSign::Signed, IntegerSign::Unsigned]
        .into_iter()
        .flat_map(|sign| {
            [8, 16, 32, 64].map(|bits| IntegerType::new(sign, bits).expect("fixed integer"))
        })
        .collect::<Vec<_>>();
    for scalar_type in integers {
        for (target_profile, registers, stack) in boolean_equal_location_cases() {
            for (parameter_count, expected) in [(2, registers), (10, stack)] {
                ...
                let remainder_operation = OperationId::new(30_005).unwrap();
                let obligation = ObligationId::new(30_009).unwrap();
                let (semantic, proof) = exact_integer_remainder_parameters_return_artifact(
                    scalar_type,
                    parameter_count,
                );
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineExactIntegerRemainderParameters(row),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("optimized exact integer remainder must retain its family row")
                };
                assert_eq!(row.machine(), MachineId::new(30_001).unwrap());
                assert_eq!(row.remainder_operation(), remainder_operation);
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerExpression {
                        ...
                        && *psi_operation == remainder_operation
