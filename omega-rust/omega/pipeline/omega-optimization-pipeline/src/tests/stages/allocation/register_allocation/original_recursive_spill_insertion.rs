...
fn original_victim_extends_the_recursive_schedule_and_crosses_spill_pseudos() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let store = function
            .schedule
            .iter()
            .find(|event| matches!(event, omega_regalloc::RecursiveSpillEvent::Store { action, .. } if *action == id(2, 0)))
            .unwrap();
        assert!(matches!(
            store,
            omega_regalloc::RecursiveSpillEvent::Store {
                point: LiveRangePoint(14),
                before_instruction,
                before_reload: Some(source_pressure),
                source: omega_regalloc::RecursiveSpillStoredValue::Original(VirtualRegisterId(5)),
                ..
            } if before_instruction.0 == 7 && *source_pressure == id(1, 0)
        ));
        assert!(matches!(
            function.schedule.iter().find(|event| matches!(event, omega_regalloc::RecursiveSpillEvent::Reload { action, .. } if *action == id(2, 0))),
            Some(omega_regalloc::RecursiveSpillEvent::Reload {
        ...
        assert!(matches!(
            ...
            omega_regalloc::SpillPseudoInstruction::Store {
                ...
                source: omega_regalloc::SpillPseudoStoredValue::Original(VirtualRegisterId(5)),
                ..
            } if action == id(2, 0)
        ));
...
fn independent_replay_rejects_original_lineage_schedule_policy_and_usage_corruption() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        for corrupt in [
            |plan: &mut omega_regalloc::RecursiveSpillInsertionPlan| {
                plan.generalized_spill_insertion =
                    omega_regalloc::GeneralizedSpillInsertionIdentity::from_bytes([0xd0; 32])
            },
            |plan: &mut omega_regalloc::RecursiveSpillInsertionPlan| {
                plan.recovery_actions =
                    omega_regalloc::GeneralizedSpillRecoveryActionIdentity::from_bytes([0xd1; 32])
            },
            |plan: &mut omega_regalloc::RecursiveSpillInsertionPlan| {
                plan.register_environment =
                    omega_register_model::TargetRegisterEnvironmentIdentity::from_bytes([0xd2; 32])
            },
            |plan: &mut omega_regalloc::RecursiveSpillInsertionPlan| {
                plan.allocator_availability =
                    omega_regalloc::AllocatorAvailabilityIdentity::from_bytes([0xd3; 32])
            },
            |plan: &mut omega_regalloc::RecursiveSpillInsertionPlan| {
                plan.optimization_unit =
                    omega_optimization_core::OptimizationUnitIdentity::from_bytes([0xd4; 32])
            },
            |plan: &mut omega_regalloc::RecursiveSpillInsertionPlan| {
                plan.fuel_schedule = psi_core::FuelScheduleIdentity::new(99_970).unwrap()
            },
        ] {
        ...
        wrong_policy.policy = omega_regalloc::RecursiveSpillInsertionPolicy::EpochTwoReloadVictimBlockLocalUnsignedU64ClosedIntervalFirstFitV1;
        assert_eq!(
        ...
        *source = omega_regalloc::RecursiveSpillStoredValue::Reload(id(0, 0));
        assert_eq!(
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        for budget in insufficient {
            assert!(matches!(
                ...
                Err(omega_regalloc::RecursiveSpillInsertionError::BudgetExceeded {
                    required,
                    budget: actual,
