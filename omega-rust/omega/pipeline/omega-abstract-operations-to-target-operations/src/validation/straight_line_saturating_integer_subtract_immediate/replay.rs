//! Independent replay of constant saturating integer subtraction as an immediate.

use omega_target_operations::{TargetFunction, TargetOperation};

use super::grammar::ReconstructedSaturatingIntegerSubtractImmediate;
use super::{
    StraightLineSaturatingIntegerSubtractImmediateTranslationError as Error,
    StraightLineSaturatingIntegerSubtractImmediateTranslationReceipt as Receipt,
};

pub(super) fn validate(
    source: ReconstructedSaturatingIntegerSubtractImmediate,
    ...
    if target.provenance.operations.as_slice()
        != [
            ...
            source.saturating_sub_operation,
        ]
    ...
    if !matches!(
        ...
        TargetOperation::ReturnIntegerImmediate {
            ...
            && source_value == source.saturating_sub_result
