//! Shared source, ABI, and provenance reconstruction for integer-unary leaves.
...
use super::super::super::{
    abi,
    model::{
        ReconstructedIntegerExactCastParameter, ReconstructedIntegerUnaryParameter,
        ReconstructedIntegerWidenParameter,
    },
    source,
};
...
pub(super) fn reconstruct_exact_cast(
    function: &AbstractFunction,
    expected_target: NativeTarget,
    target: &TargetFunction,
) -> Result<
    ReconstructedIntegerExactCastParameter,
    StraightLineIntegerExactCastParameterTranslationError,
> {
    let source = source::integer::unary::reconstruct_exact_cast(function)?;
    let locations = abi::replay(
        &function.parameters,
    ...
    if target.provenance.operations.as_slice() != [source.operation]
        || target.provenance.edges.as_slice() != [source.return_edge]
    {
        return Err(StraightLineIntegerExactCastParameterTranslationError::TargetProvenance);
    }
    Ok(ReconstructedIntegerExactCastParameter {
        operation: source.operation,
