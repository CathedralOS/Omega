//! Exact `[ExactIntegerShiftRight(parameter, parameter), Return]` source replay.
...
use super::super::super::super::model::{
    ExactIntegerShiftRightParametersSource, IntegerShiftParametersSource, ParameterResultKind,
...
> {
    ...
    if !cleanup_actions.is_empty() {
        return Err(StraightLineExactIntegerShiftRightParametersTranslationError::SourceCleanup);
    }
    if *result != envelope.function_result
        || *return_type != ScalarType::Integer(*value_type)
        || *returned_value != *shift_result
    {
        return Err(StraightLineExactIntegerShiftRightParametersTranslationError::SourceReturnLink);
    }
    if envelope
        .parameters
        .iter()
    ...
    Ok(ExactIntegerShiftRightParametersSource {
