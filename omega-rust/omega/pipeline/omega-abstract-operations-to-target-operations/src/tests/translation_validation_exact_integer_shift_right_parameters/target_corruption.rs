...
use psi_core::{EdgeId, IntegerSign, ObligationId, OperationId, StructuralTypeId, ValueId};

#[test]
fn exact_shift_right_target_provenance_machine_and_attachment_corruption_fails_closed() {
    assert_eq!(
    ...
    let target_profile = omega_target::NativeTarget::linux_x64();
    let mut target = crate::lower_to_target_operations(&source, target_profile).unwrap();
    target.functions[0].machine = psi_core::MachineId::new(55_600).unwrap();
    assert_eq!(
    ...
    let mut target = crate::lower_to_target_operations(&source, target_profile).unwrap();
    target.functions[0].attachment = Some(StructuralTypeId::new(55_601).unwrap());
    assert_eq!(
        crate::validate_abstract_to_target_translation(&source, target_profile, &target)
            .unwrap_err(),
...
fn exact_shift_right_target_outer_and_independent_type_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactShiftRight { psi_operation, .. } = expression(operation)
        else {
            unreachable!()
        };
        *psi_operation = OperationId::new(55_604).unwrap();
    });
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactShiftRight { count_type, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactShiftRight { obligation, .. } = expression(operation)
        else {
            unreachable!()
        };
        *obligation = ObligationId::new(55_609).unwrap();
    });
}

#[test]
fn exact_shift_right_target_nested_value_and_count_corruption_fails_closed() {
    for mutate_value in [true, false] {
        ...
        assert_target_operation_error(|operation| {
            ...
            *source_value = ValueId::new(if mutate_value { 55_605 } else { 55_606 }).unwrap();
        });
        assert_target_operation_error(|operation| {
            let operand = operand(expression(operation), mutate_value);
        ...
        assert_target_operation_error(|operation| {
            ...
            *operand = TargetIntegerExpression::BitwiseNot {
                psi_operation: OperationId::new(55_607).unwrap(),
...
#[test]
fn exact_shift_right_rejects_swaps_and_all_other_binary_expression_variants() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactShiftRight { value, count, .. } = expression(operation)
        else {
            unreachable!()
        };
...
fn substitute_expression(operation: &mut TargetOperation, substitute: u8) {
    ...
    let TargetIntegerExpression::ExactShiftRight {
        psi_operation,
        obligation,
        count_type,
        value,
    ...
    *expression = match substitute {
        ...
        2 => TargetIntegerExpression::WrappingShiftRight {
            psi_operation,
            count_type,
            value,
            count,
        },
        3 => TargetIntegerExpression::ExactShiftLeft {
            psi_operation,
            obligation,
            count_type,
            value,
...
fn operand(
    ...
    let TargetIntegerExpression::ExactShiftRight { value, count, .. } = expression else {
        unreachable!()
    };
    if value_operand {
        value.as_mut()
