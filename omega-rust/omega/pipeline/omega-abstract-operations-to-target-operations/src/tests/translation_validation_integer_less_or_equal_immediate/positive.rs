...
#[test]
fn validates_native_inclusive_ordering_boundaries_on_every_native_target() {
        ...
        for scalar_type in native_types() {
            for (left_value, right_value, expected) in boundary_cases(scalar_type) {
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("constant inclusive integer ordering must publish only its exact immediate family")
                };
                ...
                assert_eq!(row.return_edge(), EdgeId::new(72_009).unwrap());
...
#[test]
fn classifier_is_disjoint_from_strict_equality_and_parameter_ordering() {
    let target_profile = NativeTarget::linux_x64();

    let mut strict = default_plan();
    strict.functions[0].operations[2] = AbstractOperation::IntegerLessThan {
    ...
    let target = lower_to_target_operations(&strict, target_profile).unwrap();
    let receipt =
        validate_abstract_to_target_translation(&strict, target_profile, &target).unwrap();
    assert!(matches!(
        receipt.function_roster()[0].translation(),
        AbstractToTargetFunctionTranslationDisposition::Validated(
            AbstractToTargetFunctionTranslationReceipt::StraightLineIntegerLessThanImmediate(_)
        )
    ));
    ...
    assert!(
        !crate::validation::straight_line_integer_less_or_equal_immediate::is_candidate(
            &super::super::super::parameter_translation_fixture::uniform_integer_less_or_equal_plan(
                scalar_type(),
                2,
            )
            .functions[0]
