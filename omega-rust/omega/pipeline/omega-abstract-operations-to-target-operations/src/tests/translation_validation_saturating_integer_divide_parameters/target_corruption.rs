...
use crate::StraightLineSaturatingIntegerDivideParametersTranslationError;
...
#[test]
fn saturating_integer_divide_target_provenance_corruption_fails_closed() {
    assert_eq!(
...
#[test]
fn saturating_integer_divide_target_outer_expression_and_obligation_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingDivide { psi_operation, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingDivide { obligation, .. } = expression(operation)
        else {
            unreachable!()
        };
...
#[test]
fn saturating_integer_divide_target_nested_operand_corruption_fails_closed() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingDivide { left, .. } = expression(operation) else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingDivide { right, .. } = expression(operation) else {
            unreachable!()
        };
        let TargetIntegerExpression::Parameter {
            parameter_index, ..
    ...
    for mutate_left in [true, false] {
        assert_target_operation_error(|operation| {
            let TargetIntegerExpression::SaturatingDivide { left, right, .. } =
...
#[test]
fn saturating_integer_divide_rejects_swapped_operands_and_semantic_substitutions() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingDivide { left, right, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    for substitute in 1..=14 {
        assert_target_operation_error(|operation| {
            ...
            let TargetIntegerExpression::SaturatingDivide {
                psi_operation,
                obligation,
                left,
                right,
            ...
            *expression = match substitute {
                ...
                12 => TargetIntegerExpression::ExactMultiply {
                    psi_operation: *psi_operation,
                    obligation: ObligationId::new(51_517).unwrap(),
                    left: left.clone(),
                    right: right.clone(),
                },
...
fn assert_target_operation_error(mutate: impl FnOnce(&mut TargetOperation)) {
    assert_eq!(
        ...
        StraightLineSaturatingIntegerDivideParametersTranslationError::TargetOperation
    );
}
