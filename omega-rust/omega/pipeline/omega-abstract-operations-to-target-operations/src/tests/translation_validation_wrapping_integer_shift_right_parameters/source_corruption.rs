use super::{base_source, leaf_error, wrapping_integer_shift_right_parameters_plan};
use crate::StraightLineWrappingIntegerShiftRightParametersTranslationError;
...
#[test]
fn wrapping_shift_right_source_identity_and_independent_type_corruption_fails_closed() {
    assert_eq!(
        leaf_error(|function| function.parameters.clear()),
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceParameters
    );
    assert_eq!(
        leaf_error(
            |function| super::super::scalar_result_mut(function).scalar_type = ScalarType::Boolean
        ),
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceResult
    );
    assert_eq!(
        leaf_error(|function| {
        ...
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceShiftResultRoster
    );
    assert_eq!(
        leaf_error(|function| {
            let AbstractOperation::WrappingIntegerShiftRight { value, .. } =
                &mut function.operations[0]
            else {
                unreachable!()
            };
            *value = ValueId::new(54_500).unwrap();
        }),
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceValueOperandLink
    );
    assert_eq!(
        leaf_error(|function| {
            let AbstractOperation::WrappingIntegerShiftRight { count, .. } =
                &mut function.operations[0]
            else {
                unreachable!()
            };
            *count = ValueId::new(54_501).unwrap();
        }),
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceCountOperandLink
    );
    assert_eq!(
        leaf_error(|function| {
        ...
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceValueTypeMismatch
    );
    assert_eq!(
        leaf_error(|function| {
        ...
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceCountTypeMismatch
    );
    for malformed in [
        ...
        assert_eq!(
            ...
            StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceParameterShape
        );
        assert_eq!(
...
#[test]
fn wrapping_shift_right_source_semantics_order_return_and_cleanup_corruption_fails_closed() {
    ...
    assert_eq!(
        ...
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceReturnLink
    );
    assert_eq!(
        leaf_error(|function| {
            ...
            cleanup_actions.push(psi_terminal::TerminalAffineCleanupAction::DiscardRoot(
                PlaceId::new(54_502).unwrap(),
            ));
        }),
        StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceCleanup
    );
    assert_eq!(
    ...
    let mut source = wrapping_integer_shift_right_parameters_plan(
...
fn malformed_type_error(
    ...
    let AbstractOperation::WrappingIntegerShiftRight {
        value_type,
        count_type,
        ..
    } = &mut source.functions[0].operations[0]
    ...
    let target = omega_target_operations::TargetFunction {
        ...
        operation: omega_target_operations::TargetOperation::ReturnIntegerImmediate {
            ...
            source_value: psi_core::ValueId::new(54_504).unwrap(),
...
fn replace_shift(function: &mut AbstractFunction, substitute: u8) {
    ...
    let obligation = ObligationId::new(54_505).unwrap();
