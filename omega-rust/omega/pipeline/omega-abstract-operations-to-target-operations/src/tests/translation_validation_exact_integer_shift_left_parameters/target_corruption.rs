...
#[test]
fn exact_shift_left_target_provenance_machine_and_attachment_corruption_fails_closed() {
    assert_eq!(
...
#[test]
fn exact_shift_left_target_outer_and_independent_type_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactShiftLeft { psi_operation, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactShiftLeft { count_type, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactShiftLeft { obligation, .. } = expression(operation)
        else {
            unreachable!()
        };
...
#[test]
fn exact_shift_left_target_nested_value_and_count_corruption_fails_closed() {
...
#[test]
fn exact_shift_left_rejects_swaps_and_all_other_binary_expression_variants() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactShiftLeft { value, count, .. } = expression(operation)
        else {
            unreachable!()
        };
...
fn substitute_expression(operation: &mut TargetOperation, substitute: u8) {
    ...
    let TargetIntegerExpression::ExactShiftLeft {
        psi_operation,
        obligation,
        count_type,
        value,
...
fn operand(
    ...
    let TargetIntegerExpression::ExactShiftLeft { value, count, .. } = expression else {
        unreachable!()
    };
    if value_operand {
        value.as_mut()
