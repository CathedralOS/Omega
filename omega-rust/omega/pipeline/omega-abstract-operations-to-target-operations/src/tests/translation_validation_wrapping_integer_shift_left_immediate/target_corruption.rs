...
fn target_provenance_corruption_fails_closed() {
    assert_eq!(
        ...
        StraightLineWrappingIntegerShiftLeftImmediateTranslationError::TargetProvenance
    );
    assert_eq!(
...
fn every_target_immediate_axis_fails_closed() {
    for mutate in [
        ...
        assert_eq!(
            ...
            StraightLineWrappingIntegerShiftLeftImmediateTranslationError::TargetOperation
        );
    }
}
