...
fn roster_mut(
    target: &mut omega_target_operations::TargetOperationPlan,
    location: RosterLocation,
) -> &mut Vec<psi_terminal::StructuralPathQualification> {
    match location {
        RosterLocation::TargetParameter => {
            let TargetOperation::ReturnStructuralCall {
                structural_parameters,
...
fn mutate(rows: &mut Vec<psi_terminal::StructuralPathQualification>, mutation: RosterMutation) {
    match mutation {
        RosterMutation::Missing => {
            rows.remove(0);
        }
        ...
        RosterMutation::WrongDomain => {
            rows[0].domain = psi_core::StructuralDomainId::new(999).unwrap();
        }
        RosterMutation::Duplicate => {
