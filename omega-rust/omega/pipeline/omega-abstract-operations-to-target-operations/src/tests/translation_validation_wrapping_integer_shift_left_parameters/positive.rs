use super::wrapping_integer_shift_left_parameters_plan;
use crate::{
    AbstractToTargetFunctionTranslationDisposition, AbstractToTargetFunctionTranslationReceipt,
    lower_to_target_operations, validate_abstract_to_target_translation,
};
use omega_target::NativeTarget;
use psi_core::{EdgeId, IntegerSign, IntegerType, OperationId, ScalarType, ValueId};
        ...
        for value_type in types() {
            for count_type in types() {
                for (value_index, count_index) in placements {
                    ...
                    let source = wrapping_integer_shift_left_parameters_plan(
                        &parameter_types,
                        value_index,
                        count_index,
                    );
                    ...
                    let AbstractToTargetFunctionTranslationDisposition::Validated(
                        ...
                        panic!("wrapping shift-left must publish its independent receipt")
                    };
                    assert_eq!(row.machine(), source.entry);
                    assert_eq!(row.shift_operation(), OperationId::new(6_900).unwrap());
                    assert_eq!(row.return_edge(), EdgeId::new(3_004).unwrap());
                    assert_eq!(row.source_value(), ValueId::new(6_901).unwrap());
...
#[test]
fn wrapping_shift_left_retains_reversed_same_operand_and_mixed_roster_custody() {
    ...
    for scalar_type in integer_types {
        ...
        for (value_index, count_index) in [(2, 1), (1, 1)] {
            ...
            let AbstractToTargetFunctionTranslationDisposition::Validated(
                ...
                panic!("wrapping shift-left must retain value/count order")
            };
            assert_eq!(row.value_parameter_index(), value_index);
            assert_eq!(row.count_parameter_index(), count_index);
            assert_eq!(
