//! Optimizer module role: stage group. Wrapping shift-left custody tests by behavior.

use super::parameter_translation_fixture::{
    integer_type, wrapping_integer_shift_left_parameters_plan,
};
use super::{AbstractFunction, NativeTarget};
use crate::{
    AbstractToTargetTranslationFamily, AbstractToTargetTranslationFamilyError,
    AbstractToTargetTranslationValidationError,
    StraightLineWrappingIntegerShiftLeftParametersTranslationError, lower_to_target_operations,
...
fn base_source() -> omega_abstract_operations::AbstractOperationPlan {
    wrapping_integer_shift_left_parameters_plan(
        &[
            psi_core::ScalarType::Integer(integer_type(psi_core::IntegerSign::Signed, 32)),
            psi_core::ScalarType::Integer(integer_type(psi_core::IntegerSign::Unsigned, 16)),
        ],
        0,
        1,
    )
...
fn candidate_error(
    ...
    let AbstractToTargetTranslationValidationError::FunctionFamily {
        family: AbstractToTargetTranslationFamily::StraightLineWrappingIntegerShiftLeftParameters,
        error:
            AbstractToTargetTranslationFamilyError::StraightLineWrappingIntegerShiftLeftParameters(
                error,
            ),
        ..
    } = validate_abstract_to_target_translation(&source, target_profile, &target).unwrap_err()
    else {
        panic!("wrapping-shift-left corruption must fail at its independent validator")
    };
    error
}
