...
pub(in crate::tests) fn exact_integer_multiply_parameters_plan(
    ...
    let ScalarType::Integer(scalar_type) = function.parameters[left_parameter].scalar_type else {
        panic!("exact-integer-multiply fixture requires integer operands")
    };
    let result = ValueId::new(5_501).unwrap();
    ...
    function.operations.insert(
        ...
        AbstractOperation::ExactIntegerMultiply {
            psi_operation: OperationId::new(5_500).unwrap(),
            obligation: ObligationId::new(5_502).unwrap(),
            result,
            scalar_type,
            left,
            right,
...
pub(in crate::tests) fn uniform_exact_integer_multiply_plan(
    integer: IntegerType,
    parameter_count: usize,
) -> AbstractOperationPlan {
    exact_integer_multiply_parameters_plan(
