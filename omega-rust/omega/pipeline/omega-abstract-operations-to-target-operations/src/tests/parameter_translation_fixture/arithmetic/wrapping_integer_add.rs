...
pub(in crate::tests) fn wrapping_integer_add_parameters_plan(
    ...
    let ScalarType::Integer(scalar_type) = function.parameters[left_parameter].scalar_type else {
        panic!("wrapping-integer-add fixture requires integer operands")
    };
    let result = ValueId::new(4_801).unwrap();
    ...
    function.operations.insert(
        ...
        AbstractOperation::WrappingIntegerAdd {
            psi_operation: OperationId::new(4_800).unwrap(),
            result,
            scalar_type,
            left,
            right,
...
pub(in crate::tests) fn uniform_wrapping_integer_add_plan(
    integer: IntegerType,
    parameter_count: usize,
) -> AbstractOperationPlan {
    wrapping_integer_add_parameters_plan(
