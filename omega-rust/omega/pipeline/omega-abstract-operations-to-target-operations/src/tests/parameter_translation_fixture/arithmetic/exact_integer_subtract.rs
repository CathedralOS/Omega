...
pub(in crate::tests) fn exact_integer_subtract_parameters_plan(
    ...
    let ScalarType::Integer(scalar_type) = function.parameters[left_parameter].scalar_type else {
        panic!("exact-integer-subtract fixture requires integer operands")
    };
    let result = ValueId::new(5_301).unwrap();
    ...
    function.operations.insert(
        ...
        AbstractOperation::ExactIntegerSubtract {
            psi_operation: OperationId::new(5_300).unwrap(),
            obligation: ObligationId::new(5_302).unwrap(),
            result,
            scalar_type,
            left,
            right,
...
pub(in crate::tests) fn uniform_exact_integer_subtract_plan(
    integer: IntegerType,
    parameter_count: usize,
) -> AbstractOperationPlan {
    exact_integer_subtract_parameters_plan(
