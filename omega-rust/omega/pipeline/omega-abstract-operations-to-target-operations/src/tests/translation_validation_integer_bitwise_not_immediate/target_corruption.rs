...
fn target_provenance_corruption_fails_closed() {
    assert_eq!(
        ...
        StraightLineIntegerBitwiseNotImmediateTranslationError::TargetProvenance
    );
    assert_eq!(
...
fn target_immediate_corruption_fails_closed() {
    for mutate in [
        ...
        assert_eq!(
            ...
            StraightLineIntegerBitwiseNotImmediateTranslationError::TargetOperation
        );
    }
}
...
fn mutate_edge(operation: &mut TargetOperation) {
    ...
    *psi_edge = EdgeId::new(67_200).unwrap();
}
...
fn mutate_source_value(operation: &mut TargetOperation) {
    ...
    *source_value = ValueId::new(67_201).unwrap();
}
...
fn substitute_operation(operation: &mut TargetOperation) {
    *operation = TargetOperation::ReturnIntegerExpression {
        ...
        source_value: ValueId::new(67_006).unwrap(),
        scalar_type: scalar_type(),
        expression: TargetIntegerExpression::BitwiseNot {
            psi_operation: OperationId::new(67_005).unwrap(),
            operand: Box::new(TargetIntegerExpression::Immediate {
                source_value: ValueId::new(67_004).unwrap(),
