...
#[test]
fn validates_native_wrapping_subtract_boundaries_on_every_native_target() {
    for target_profile in [
        ...
        for scalar_type in native_types() {
            for (left_value, right_value) in boundary_pairs(scalar_type) {
                let expected = scalar_type.wrapping_sub(left_value, right_value).unwrap();
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineWrappingIntegerSubtractImmediate(
                        row,
                    ),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("constant integer wrapping integer subtraction must publish only its exact immediate family")
                };
                ...
                assert_eq!(
                    row.wrapping_subtract_operation(),
...
#[test]
fn classifier_is_disjoint_from_plain_adjacent_arithmetic_and_parameter_subtract() {
    ...
    *value = ValueId::new(73_004).unwrap();
    ...
    for replacement in [
        ...
        assert!(
            !crate::validation::straight_line_wrapping_integer_subtract_immediate::is_candidate(
    ...
    assert!(
        !crate::validation::straight_line_wrapping_integer_subtract_immediate::is_candidate(
            &super::super::parameter_translation_fixture::uniform_wrapping_integer_subtract_plan(
                scalar_type(),
                2,
            )
            .functions[0]
