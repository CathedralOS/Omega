...
use assigned_target_operations::{AssignedBooleanControl, AssignedOperation};
...
fn boolean_cleanup_plan(target: NativeTarget) -> TargetOperationPlan {
    let scalar_shape = calling_conventions::ValueShape::integer(1, 1);
    let structural_shape = calling_conventions::ValueShape::integer(8, 8);
