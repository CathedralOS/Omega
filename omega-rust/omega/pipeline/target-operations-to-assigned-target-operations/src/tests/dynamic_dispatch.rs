...
use semantic_vocabulary::{
    EdgeId, IntegerSign, IntegerType, MachineId, OperationId, ScalarType, ValueId,
};
...
fn stored_target_plan(target: NativeTarget) -> target_operations::TargetOperationPlan {
    let source = r#"
    ...
    let operation = body
        ...
        .find_map(|operation| match operation {
            target_operations::TargetUnitOperation::StoreDynamicDescriptor {
                ...
                source_argument
                    .path
                    .push(terminal_psi::StructuralPathSegment::Field("other".into()));
                Some(*psi_operation)
            }
            _ => None,
        })
...
fn forwarded_target_plan(target: NativeTarget) -> target_operations::TargetOperationPlan {
    let source = r#"
        trait Measure {
            machine measure(&self) -> i32;
        }
...
fn dynamic_unit_target_plan(target: NativeTarget) -> target_operations::TargetOperationPlan {
    let source = r#"
        trait Touch {
            machine touch(&self);
        }
...
fn rejects_reauthenticated_dynamic_unit_owner_substitution() {
    ...
    let rejected_operation = body
        ...
        .iter_mut()
        .find_map(|operation| match operation {
            target_operations::TargetUnitOperation::DynamicUnitCall {
                psi_operation,
                dynamic_dispatch,
                ..
            } => {
...
fn assigns_forwarded_descriptor_registers_and_indirect_mechanism() {
    ...
    let assigned_target_operations::AssignedOperation::ReturnDynamicParameterScalarCall {
        parameter_abi,
        mechanism,
    ...
    assert_eq!(
        *mechanism,
        assigned_target_operations::AssignedDynamicParameterCallMechanism::X86MemoryIndirect {
            table: target_operations::MachineRegister::X86Rsi,
        }
    );
}
...
fn assigns_rebound_descriptor_arguments_to_forwarded_call_registers() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let expected_table = match target.architecture {
            target::Architecture::X86_64 => target_operations::MachineRegister::X86Rsi,
            target::Architecture::Aarch64 => target_operations::MachineRegister::Aarch64X(1),
        };
...
fn rejects_reauthenticated_dynamic_owner_substitution() {
    ...
    let rejected_operation = {
        let (psi_operation, dynamic) = body
            ...
            .iter_mut()
            .find_map(|operation| match operation {
                target_operations::TargetUnitOperation::DynamicScalarCall {
                    psi_operation,
                    dynamic_dispatch,
                    ..
        ...
        dynamic.dispatch.owner =
            semantic_vocabulary::MachineId::new(dynamic.dispatch.owner.get() + 100)
                .expect("distinct machine");
        psi_operation
    };
    assert!(matches!(
