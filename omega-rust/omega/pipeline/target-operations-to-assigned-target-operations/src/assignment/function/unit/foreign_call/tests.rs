...
fn fixture(
    ...
    target_operations::NormalizedForeignScalarArgument,
    ...
    .plan()
    .clone();
    let argument = target_operations::NormalizedForeignScalarArgument {
        parameter_index: 0,
...
fn assigned_argument(
    argument: &target_operations::NormalizedForeignScalarArgument,
) -> assigned_target_operations::AssignedNormalizedForeignScalarArgument {
    let source = match argument.source {
        ...
        target_operations::TargetUnitScalarArgumentSource::Home(_) => {
            panic!("literal helper receives an immediate")
        }
    };
    assigned_target_operations::AssignedNormalizedForeignScalarArgument {
...
fn assignment_rejects_literal_identity_type_value_order_and_placement_drift() {
    ...
    if let target_operations::TargetUnitScalarArgumentSource::IntegerImmediate {
        source_value,
        ..
    ...
    if let target_operations::TargetUnitScalarArgumentSource::IntegerImmediate { value, .. } =
    ...
    for (target, register_count, expected_last_register) in [
        ...
        let arguments = (0..register_count)
            .map(|index| target_operations::NormalizedForeignScalarArgument {
        ...
        let stack_argument_plan = calling_conventions::evaluate_ordinary_boundary_entry_plan(
            CallingPolicy::native_for_target(target),
            &CallSignature {
    ...
    let wrong_policy_plan = calling_conventions::evaluate_ordinary_boundary_entry_plan(
...
fn assignment_replays_the_complete_fixed_integer_result_family_and_rejects_drift() {
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        for (sign, bits) in [
            ...
            .plan()
            .clone();
            let result = target_operations::TargetUnitScalarHomeRequirement {
    ...
    for integer in [
        ...
        .plan()
        .clone();
        let invalid = target_operations::TargetUnitScalarHomeRequirement {
            defining_operation: operation,
            source_value: source,
            scalar_type: semantic_vocabulary::ScalarType::Integer(integer),
