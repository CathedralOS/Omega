...
pub(super) fn assign(
    ...
    let result_matches = match (requirement.result, scalar_result, result_shape) {
        (
            ...
            Some((_, semantic_vocabulary::ScalarType::Integer(integer))),
            Some(result_shape),
        ) => {
        ...
        (
            ...
            Some((_, semantic_vocabulary::ScalarType::Boolean)),
            Some(result_shape),
        ) => result_shape == ValueShape::integer(1, 1),
        (terminal_psi::ClosedConformanceCallableResult::Unit, None, None) => true,
        _ => false,
    };
    ...
    if !result_matches
        ...
        || calling_conventions::validate_call_plan(function_call_plan, &function_signature).is_err()
        || calling_conventions::validate_call_plan(dispatch_call_plan, &dispatch_signature).is_err()
