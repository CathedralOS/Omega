...
use assigned_target_operations::{AssignedCallDestination, AssignedNativeCallbackArgument};
use calling_conventions::{CallSignature, ValueLocation};
...
fn callback_is_exact(
    ...
    callback.callback_function.is_valid()
        ...
        && demand.destination
            == calling_conventions::NativePlace::Parameter(callback.application.parameter)
...
fn destination(
    ...
    if callback.application.shape != calling_conventions::ValueShape::integer(size, alignment) {
        return None;
    }
