...
fn parked_activation_exposes_its_exact_live_frontier() {
    ...
    assert_eq!(
        frontier[0].storage,
        crate::task_plans::LiveCarryStorage::Local
    );
    assert_eq!(
        frontier[0].claims.as_slice(),
        &[crate::task_plans::ClaimId::new(10).expect("nonzero claim identity")]
    );
    ...
    let settled_id = id(
        726,
        crate::task_plans::TaskLifecycleClaimId::from_normalized_identity,
    );
