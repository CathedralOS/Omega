...
mod tests {
    ...
    fn stack_plan_projection_identity_rejects_evidence_substitution() {
        ...
        let candidate = crate::task_plans::ActivationPlanCandidate {
            machine_contract: id(
                0x1610,
                crate::task_plans::MachineContractId::from_normalized_identity,
            ),
            entry: id(
                0x1611,
                crate::task_plans::MachineEntryId::from_normalized_identity,
            ),
            argument_layout: crate::task_plans::TaskArgumentLayout::new(
                id(
                    0x1612,
                    crate::task_plans::ValueLayoutId::from_normalized_identity,
                ),
                &[],
            )
            .expect("empty canonical argument layout"),
            terminal_outcome_layout: id(
                0x1613,
                crate::task_plans::ValueLayoutId::from_normalized_identity,
            ),
            calling_plan: id(
                0x1614,
                crate::task_plans::CallingPlanId::from_normalized_identity,
            ),
            stack_plan: substituted.stack_plan(),
            may_suspend: false,
            may_block: false,
            canonical_suspension_crossings: Vec::new(),
            carry_obligations: crate::task_plans::ActivationCarryObligations::none(),
            cancellation_required: false,
        };
        assert!(
            crate::task_plans::validate_wcsu_activation_plan(candidate, substituted)
