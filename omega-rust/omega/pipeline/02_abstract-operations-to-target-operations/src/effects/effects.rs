...
use crate::effects::provider_plan::ProviderPlan;
...
pub struct SelectedProviderPlanFacts {
    ...
    execution_scope: crate::effects::ExecutionScope,
    opaque_executable_admissions: Vec<crate::effects::ValidatedOpaqueExecutableAdmission>,
...
impl Default for SelectedProviderPlanFacts {
    fn default() -> Self {
        Self {
            ...
            execution_scope: crate::effects::ExecutionScope::CallerAddressSpace,
...
impl SelectedProviderPlanFacts {
    ...
    pub const fn execution_scope(&self) -> crate::effects::ExecutionScope {
    ...
    pub fn with_execution_scope(
        ...
        if matches!(
            execution_scope,
            crate::effects::ExecutionScope::IsolatedProvider(0)
        ) {
    ...
    pub fn with_opaque_executable_admissions(
        mut self,
        candidates: impl IntoIterator<Item = crate::effects::OpaqueExecutableAdmissionCandidate>,
    ) -> Result<Self, String> {
        ...
        for candidate in candidates {
            ...
            self.opaque_executable_admissions.push(
                crate::effects::effects::executable_scopes::executable_tcb_manifest::validate_opaque_executable_admission(
                    &self.plans,
                    candidate,
                )?,
            );
    ...
    pub fn opaque_executable_admissions(
        &self,
    ) -> &[crate::effects::ValidatedOpaqueExecutableAdmission] {
    ...
    pub fn executable_tcb_manifest(&self) -> crate::effects::ExecutableTcbManifest {
        crate::effects::effects::executable_scopes::executable_tcb_manifest::derive_static_manifest(
            &self.plans,
            self.report_fingerprint,
            self.execution_scope,
            &self.opaque_executable_admissions,
...
impl SelectedProviderClosureDigestEncoder {
    ...
    fn execution_scope(&mut self, scope: crate::effects::ExecutionScope) {
        match scope {
            crate::effects::ExecutionScope::CallerAddressSpace => self.byte(0),
            crate::effects::ExecutionScope::IsolatedProvider(identity) => {
                self.byte(1);
                self.u64(identity);
            }
        }
    ...
    fn opaque_executable_admission(
        &mut self,
        admission: &crate::effects::OpaqueExecutableAdmissionCandidate,
    ) {
        ...
        for evidence in &admission.containment {
            self.byte(match evidence.guarantee {
                crate::effects::ContainmentGuarantee::MemoryIsolation => 0,
                crate::effects::ContainmentGuarantee::ForcibleTermination => 1,
                crate::effects::ContainmentGuarantee::FaultContainment => 2,
                crate::effects::ContainmentGuarantee::BoundedResources => 3,
            });
    ...
    fn opaque_binding(&mut self, binding: &crate::effects::OpaqueInProcessBinding) {
        match binding {
            crate::effects::OpaqueInProcessBinding::Import { evaluated } => {
                let locator = evaluated.locator();
                self.byte(0);
                self.string(locator.target().target_name());
                match locator.locator() {
                    crate::effects::ForeignLocatorCandidate::PeByName { library, export } => {
                        self.byte(0);
                        self.bytes(library);
                        self.bytes(export);
                    }
                    crate::effects::ForeignLocatorCandidate::PeByOrdinal { library, ordinal } => {
                        self.byte(1);
                        self.bytes(library);
                        self.0.update(ordinal.to_le_bytes());
                    }
                    crate::effects::ForeignLocatorCandidate::ElfVersioned {
                        object,
                        symbol,
                        version,
                    } => {
                    ...
                    crate::effects::ForeignLocatorCandidate::MachODylibSymbol {
                        install_name,
                        symbol,
                    } => {
                        self.byte(3);
            ...
            crate::effects::OpaqueInProcessBinding::VtableSlot { index } => {
                self.byte(2);
                self.i64(*index);
            }
            crate::effects::OpaqueInProcessBinding::VtableField { table, field } => {
                self.byte(3);
                self.string(table);
                self.string(field);
            }
            crate::effects::OpaqueInProcessBinding::TableFunction { table, field } => {
                self.byte(4);
                self.string(table);
                self.string(field);
            }
