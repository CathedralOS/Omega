...
fn evaluated_import(
    locator: crate::effects::NormalizedForeignLocator,
    seed: u8,
...
fn opaque_import(
    ...
    export: &[u8],
    seed: u8,
) -> (ProviderBinding, crate::effects::OpaqueInProcessBinding) {
    let locator = crate::effects::normalize_foreign_locator(
        crate::effects::ForeignLocatorCandidate::PeByName {
...
fn opaque_candidate(
    ...
    export: &[u8],
    seed: u8,
) -> (ProviderPlan, crate::effects::OpaqueInProcessBinding) {
...
fn selected_closure_digest_distinguishes_compact_equal_calling_plans() {
    ...
    first.schema.methods[0].calling_plan_commitment =
        Some(crate::effects::provider_plan::BoundaryCallingPlanCommitment::from_digest([0x11; 32]));
    let mut substituted = first.clone();
    substituted.schema.methods[0].calling_plan_commitment =
        Some(crate::effects::provider_plan::BoundaryCallingPlanCommitment::from_digest([0x22; 32]));
...
fn opaque_selection_survives_a_checked_wrapper_as_attributed_incompleteness() {
    ...
    let crate::effects::ScopeCompleteness::Incomplete { causes, .. } = manifest.completeness else {
        panic!("opaque in-process selection must make the scope incomplete");
    };
    assert_eq!(causes.len(), 1);
    assert!(matches!(
        &causes[0],
        crate::effects::IncompleteCause::SelectedOpaqueProvider {
            provider_plan_report_identity,
            binding: crate::effects::OpaqueInProcessBinding::Import {
                evaluated: retained,
            },
            ..
...
fn pinned_opaque_entry_remains_incomplete_without_executable_closure_evidence() {
    ...
    .with_opaque_executable_admissions([crate::effects::OpaqueExecutableAdmissionCandidate {
        provider_plan_report_identity: plan_identity,
        ...
        containment: vec![crate::effects::ContainmentEvidence {
            guarantee: crate::effects::ContainmentGuarantee::FaultContainment,
    ...
    assert!(matches!(
        ...
        crate::effects::ExecutableIdentity::PinnedOpaqueArtifact(ref identity)
            if identity == "sha256:0123456789abcdef"
    ));
    assert!(matches!(
        manifest.completeness,
        crate::effects::ScopeCompleteness::Incomplete { ref causes, .. } if causes.len() == 1
    ));
}
...
fn exact_closure_and_containment_receipts_complete_the_opaque_scope() {
    ...
    .with_opaque_executable_admissions([crate::effects::OpaqueExecutableAdmissionCandidate {
        ...
        execution_scope: crate::effects::ExecutionScope::CallerAddressSpace,
        containment: vec![
            crate::effects::ContainmentEvidence {
                guarantee: crate::effects::ContainmentGuarantee::BoundedResources,
                evidence_identity: "receipt:quota-v1".into(),
            },
            crate::effects::ContainmentEvidence {
                guarantee: crate::effects::ContainmentGuarantee::MemoryIsolation,
    ...
    let crate::effects::ScopeCompleteness::Complete {
...
fn exact_closure_evidence_survives_an_unrelated_incomplete_row() {
    ...
    let manifest = selected.executable_tcb_manifest();
    let crate::effects::ScopeCompleteness::Incomplete {
        causes,
        opaque_closure_evidence,
        ..
    } = manifest.completeness
...
fn opaque_admission_rejects_binding_drift_and_duplicate_containment_axes() {
    ...
    .expect("selected opaque provider");
    let candidate = crate::effects::OpaqueExecutableAdmissionCandidate {
        provider_plan_report_identity: plan_identity,
        provider_plan_digest: opaque.identity_digest(),
        method: "read".into(),
        requirement_identity: opaque.schema.methods[0].requirement_identity.clone(),
...
fn checked_and_intrinsic_entries_are_derived_only_from_selected_plans() {
    ...
    assert!(matches!(
        manifest.completeness,
        crate::effects::ScopeCompleteness::Complete {
    ...
    assert!(manifest.known_entries.iter().all(|entry| {
        entry.origin == crate::effects::ExecutableEntryOrigin::StaticSelection
            && entry.execution_scope == crate::effects::ExecutionScope::CallerAddressSpace
