...
use crate::effects::effects::capabilities::foreign_locator::NormalizedForeignLocator;
...
fn evaluated_calling_plan_is_published_provider_identity() {
    ...
    first.schema.methods[0].calling_plan_commitment =
        Some(symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::BoundaryCallingPlanCommitment::from_digest([1; 32]));
    ...
    refactored.schema.methods[0].calling_plan_commitment =
        Some(symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::BoundaryCallingPlanCommitment::from_digest([2; 32]));
    assert_eq!(first.report_fingerprint(), refactored.report_fingerprint());
