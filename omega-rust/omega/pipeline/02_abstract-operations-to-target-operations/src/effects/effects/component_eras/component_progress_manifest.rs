...
pub struct CheckedComponentProgressDemand {
    ...
    pub provider_service_package_identity: Option<semantic_vocabulary::PackageKeyIdentity>,
...
pub struct ComponentBuildBoundProgressDemand {
    ...
    pub profile_identity: String,
    pub subject_projections: Vec<String>,
    pub establishment_routes: Vec<crate::effects::provider_plan::ServiceProgressEstablishmentRoute>,
...
impl ManifestDigestEncoder {
    ...
    fn package_identity(&mut self, identity: Option<semantic_vocabulary::PackageKeyIdentity>) {
        match identity {
            Some(identity) => {
                self.0.update([1]);
...
mod tests {
    ...
    use crate::effects::provider_plan::{
    ...
    fn progress_demand_selects_the_exact_package_owned_service() {
        let first_package = semantic_vocabulary::PackageKeyIdentity::from_digest([0x51; 32])
            .expect("nonzero package identity");
        let second_package = semantic_vocabulary::PackageKeyIdentity::from_digest([0x52; 32])
            .expect("nonzero package identity");
