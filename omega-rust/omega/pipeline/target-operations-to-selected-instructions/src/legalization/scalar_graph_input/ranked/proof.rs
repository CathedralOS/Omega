...
pub(super) fn replay_verifier_custody(custody: &RankedU32CountdownCustody) -> Option<()> {
    let proof = terminal_codec::decode_proof_bundle(&custody.proof_replay).ok()?;
    let profile = proof_admission::AdmissionProfile::default();
    let native = terminal_verifier::verify_module_for_native_ranked_countdown(
    ...
    .ok()?;
    let fixed =
        terminal_verifier::verify_module_for_fixed_fuel(&custody.semantic_replay, &proof, &profile)
            .ok()?;
    let derived = terminal_fixed_fuel::derive_ranked_countdown_entry_fuel(
        &fixed,
    ...
    terminal_fixed_fuel::validate_ranked_countdown_entry_fuel(&fixed, &derived).ok()?;
...
fn frontier_matches(
    projected: &abstract_operations::RankedStructuralOwnershipFrontier,
    verified: &terminal_verifier::VerifiedStructuralOwnershipFrontier,
) -> bool {
