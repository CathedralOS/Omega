//! Rejoin record references to exact incoming or dominating operation storage.
use super::*;
pub(super) fn reconstruct(
        ...
        .find(|parameter| parameter.place == argument.place)
    {
        ...
        let before = site(producer).ok_or(invalid.clone())?;
        let after = site(call_operation).ok_or(invalid.clone())?;
        if (before.0 == after.0 && before.1 >= after.1)
            || (before.0 != after.0
                && !super::target::control_flow::sources::dominates(caller, before.0, after.0))
        {
            return Err(invalid);
        }
        (
