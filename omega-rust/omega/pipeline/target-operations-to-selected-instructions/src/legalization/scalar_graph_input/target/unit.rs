...
pub(super) fn validate_operation(
    ...
    match (target, abstracted) {
        (TargetUnitOperation::EstablishScalarCase { psi_operation, result_home, result_case, fields },
            AbstractOperation::EstablishScalarCase { psi_operation: expected_operation, result, result_case: expected_case, fields: expected_fields }) => {
            if psi_operation != expected_operation || result_case != expected_case || fields != expected_fields
                || *result_home != super::super::scalar_sums::result_home(optimized, result.place, plan)?
            { return Err(invalid); }
            let declaration = plan.structural_types.iter().find(|declaration| declaration.id == result.structural_type).ok_or(invalid.clone())?;
            let terminal_psi::StructuralTypeShape::Sum { cases } = &declaration.shape else { return Err(invalid); };
            let case = cases.iter().find(|case| case.id == *result_case).ok_or(invalid.clone())?;
        ...
        (TargetUnitOperation::StructuralResultCall { psi_operation, result, callee, callee_result, result_home,
            call_plan, scalar_arguments, arguments, claim_transfers, returned_claim_transfers, requirement_obligations, crash_continuations },
            AbstractOperation::CallStructural { psi_operation: expected_operation, result: expected_result, callee: expected_callee,
                arguments: values, structural_arguments, claim_transfers: expected_claims, returned_claim_transfers: expected_returns,
                requirement_obligations: expected_requirements, crash_continuations: expected_crashes, selected_evidence }) => {
            ...
            if psi_operation != expected_operation || result != expected_result || callee != expected_callee
                || called.result.structural() != Some(callee_result) || result.structural_type != callee_result.structural_type
                || result.multiplicity != callee_result.multiplicity || result_home.as_ref() != Some(&super::super::scalar_sums::result_home(optimized, result.place, plan)?)
                || *call_plan != expected_plan || !claim_transfers.is_empty() || !expected_claims.is_empty()
                || !returned_claim_transfers.is_empty() || !expected_returns.is_empty()
                || !requirement_obligations.is_empty() || !expected_requirements.is_empty()
                || !crash_continuations.is_empty() || !expected_crashes.is_empty() || !selected_evidence.is_empty()
                || values.len() != called.parameters.len() || scalar_arguments.len() != values.len()
                || structural_arguments.len() != called.structural_parameters.len() || arguments.len() != structural_arguments.len()
            { return Err(invalid); }
            for (position, ((value, parameter), argument)) in values.iter().zip(&called.parameters).zip(scalar_arguments).enumerate() {
                if argument.parameter_index as usize != position || argument.placement != expected_plan.parameters[position]
                    || argument.source.scalar_type() != parameter.scalar_type
                    || !sources.iter().any(|(identity, source)| identity == value && *source == argument.source) { return Err(invalid); }
            }
            for (position, (semantic, retained)) in structural_arguments.iter().zip(arguments).enumerate() {
                if *retained != super::super::scalar_sums::call_argument(semantic, position, optimized, called, &expected_plan, native, plan)? { return Err(invalid); }
            }
        }
        (
