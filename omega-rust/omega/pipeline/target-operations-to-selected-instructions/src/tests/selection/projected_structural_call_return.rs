...
use selected_instructions::{SelectedStructuralFragmentSite, SelectedStructuralTransfer};
use semantic_vocabulary::StructuralDomainId;

#[test]
fn exact_projected_structural_selection_replays_and_binds_every_target_fact() {
    let (abstract_plan, target, unit) = projected_fixture(target::NativeTarget::windows_x64());
    ...
    let mutations: Vec<Box<dyn Fn(&mut selected_instructions::SelectedInstructionPlan)>> = vec![
        Box::new(|plan| {
            plan.projected_structural_call_returns[0].caller =
                semantic_vocabulary::MachineId::new(9).unwrap()
        }),
        Box::new(|plan| {
        ...
        Box::new(|plan| {
            plan.projected_structural_call_returns[0]
                .caller_return
                .value
                .fixed_view = register_model::RegisterViewId(u16::MAX)
        }),
        Box::new(|plan| {
            plan.projected_structural_call_returns[0]
                .callee_return
        ...
        Box::new(|plan| {
            plan.projected_structural_call_returns[0].callee_return_transfer =
                SelectedStructuralTransfer::SameViewNoCopy {
                    register: target_operations::MachineRegister::X86Rdi,
                }
        }),
    ];
