...
use abstract_operations::{AbstractOperation, AbstractOperationPlan, CompletionClaimSource};
use optimization_unit::PsiOptimizationUnit;
use semantic_vocabulary::{BoundaryMachineId, ClaimId, FuelScheduleIdentity, OperationId, PlaceId};
...
pub(in crate::tests) fn installed_provider_legalization_fixture() -> (
    ...
    for function in &mut target.functions {
        let target_operations::TargetOperation::UnitBody(body) = &mut function.operation else {
            continue;
        };
    ...
    let target_operations::TargetOperation::UnitBody(provider_body) =
        &target.functions[1].operation
    else {
        panic!("provider Unit body");
    };
    let provider_call_plan = provider_body.call_plan.clone();
    let target_operations::TargetOperation::UnitBody(caller_body) =
    ...
    caller_body.operations[0] = target_operations::TargetUnitOperation::InstalledProviderCall {
        psi_operation: operation,
        boundary,
        provider,
