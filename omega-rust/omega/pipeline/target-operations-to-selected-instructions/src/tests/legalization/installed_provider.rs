...
fn installed_provider_call_legalization_retains_source_and_completion_custody() {
    ...
    let call = crate::tests::fixtures::ordinary_graph::call(function);
    ...
    assert_eq!(
        function.blocks[0].instructions[0].ownership,
        [optimization_unit::OwnershipEvent::ClaimCompletion(vec![
            ClaimId::new(1).unwrap(),
            ClaimId::new(2).unwrap(),
        ])]
    );
    call.validate_source(&function.blocks[0].instructions[0].ownership)
