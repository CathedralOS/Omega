    ...
    for row in rows {
        ...
        for qualification in &row.projected_qualifications {
            ...
            for segment in &qualification.path {
                match segment {
                    terminal_psi::StructuralPathSegment::Field(name) => {
                        bytes.push(1);
...
fn encode_effects(
    bytes: &mut Vec<u8>,
    uses: &[register_model::RegisterUnitId],
    defs: &[register_model::RegisterUnitId],
    clobbers: &[register_model::RegisterUnitId],
) {
...
fn encode_placement(bytes: &mut Vec<u8>, placement: &calling_conventions::ValuePlacement) {
    ...
    match placement.shape.class {
        calling_conventions::ValueClass::Integer => bytes.push(1),
        calling_conventions::ValueClass::Float => bytes.push(2),
        calling_conventions::ValueClass::BorrowedReference => bytes.push(5),
...
fn encode_eightbyte_class(bytes: &mut Vec<u8>, class: calling_conventions::SystemVEightbyteClass) {
