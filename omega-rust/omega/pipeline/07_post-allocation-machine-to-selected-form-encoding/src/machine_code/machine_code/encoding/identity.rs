...
use target_operations_to_selected_instructions::register_model::{RegisterUnitId, RegisterViewId};
...
pub(super) fn encoding_identity(
    ...
    match frame {
        ...
        Some(frame) => {
            hasher.update([1]);
            hasher.update(crate::machine_code::target_frame_layout_identity(frame).bytes());
        }
    }
...
fn encode_encoding_row(hasher: &mut Sha256, row: &SelectedFormEncodingRow) {
    ...
    target_operations_to_selected_instructions::encode_machine_alternative_key_identity(
        hasher,
        row.alternative,
    );
    match row.address {
        None => hasher.update([0]),
        Some(address) => {
            use register_homes_to_post_allocation_machine::PhysicalAddressOperation as Address;
            match address.symbolic {
                ...
                | Address::FrameAddress { slot, byte_offset } => {
                    ...
                    match slot {
                        target_operations_to_selected_instructions::FrameStorageSlotId::Incoming {
                            parameter_index,
                        ...
                        target_operations_to_selected_instructions::FrameStorageSlotId::Outgoing(slot) => {
                            hasher.update([match slot.role {
                                target_operations_to_selected_instructions::OutgoingArgumentSlotRole::Argument => 0,
                                target_operations_to_selected_instructions::OutgoingArgumentSlotRole::ValueCopy => 3,
                            }]);
                            hasher.update(slot.operation.get().to_le_bytes());
                            hasher.update(slot.argument_index.to_le_bytes());
                        }
                        target_operations_to_selected_instructions::FrameStorageSlotId::Local(slot) => {
                            hasher.update([1]);
                            let mut identity = Vec::new();
                            slot.encode_identity(&mut identity);
                            hasher.update(identity);
    ...
    match &row.state {
        SelectedFormEncodingState::Encoded { bytes, footprint } => {
            ...
            target_operations_to_selected_instructions::encode_machine_encoded_effects_identity(
                hasher,
                &footprint.encoded,
            );
        }
...
pub(super) fn encode_normalized_foreign_fixup(
    hasher: &mut Sha256,
    fixup: crate::machine_code::SelectedFormNormalizedForeignCallFixup,
) {
    hasher.update([match fixup.kind {
        crate::machine_code::SelectedFormNormalizedForeignCallFixupKind::X86Relative32FromNextInstructionToNormalizedForeignImportV1 => 0,
        crate::machine_code::SelectedFormNormalizedForeignCallFixupKind::Aarch64BranchLinkImmediate26FromInstructionToNormalizedForeignImportV1 => 1,
    }]);
    hasher.update([match fixup.state {
        crate::machine_code::SelectedFormNormalizedForeignCallFixupState::UnresolvedImportFieldV1 => 0,
    }]);
