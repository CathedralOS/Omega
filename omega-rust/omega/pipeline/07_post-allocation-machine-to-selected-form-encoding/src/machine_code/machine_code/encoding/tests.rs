...
fn encoding_identity_binds_symbolic_role_and_resolved_displacement() {
    ...
    use target_operations_to_selected_instructions::OutgoingArgumentSlotId;

    // These are raw identity proposals, not admitted instructions or frames.
    let mut program = deferred_program();
    let slot = OutgoingArgumentSlotId {
        role: target_operations_to_selected_instructions::OutgoingArgumentSlotRole::Argument,
        operation: OperationId::new(3).unwrap(),
        argument_index: 0,
    };
    program.rows[0].address = Some(ResolvedPhysicalAddress {
        symbolic: PhysicalAddressOperation::Store64 {
            slot: target_operations_to_selected_instructions::FrameStorageSlotId::Outgoing(slot),
            byte_offset: 8,
        },
        displacement: 40,
    });
    ...
    changed.rows[0].address.as_mut().unwrap().symbolic = PhysicalAddressOperation::Store64 {
        slot: target_operations_to_selected_instructions::FrameStorageSlotId::Outgoing(
            OutgoingArgumentSlotId {
                role:
