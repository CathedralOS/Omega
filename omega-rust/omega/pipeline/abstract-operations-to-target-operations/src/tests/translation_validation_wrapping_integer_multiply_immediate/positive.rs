...
#[test]
fn validates_native_wrapping_multiply_boundaries_on_every_native_target() {
        ...
        for scalar_type in native_types() {
            for (left_value, right_value) in boundary_pairs(scalar_type) {
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("constant integer wrapping integer multiplication must publish only its exact immediate family")
                };
...
#[test]
fn classifier_is_disjoint_from_plain_adjacent_arithmetic_and_parameter_multiply() {
    ...
    *value = ValueId::new(73_004).unwrap();
    ...
    for replacement in [
        ...
        assert!(
            !crate::validation::straight_line_wrapping_integer_multiply_immediate::is_candidate(
    ...
    assert!(
        !crate::validation::straight_line_wrapping_integer_multiply_immediate::is_candidate(
            &super::super::parameter_translation_fixture::uniform_wrapping_integer_multiply_plan(
                scalar_type(),
                2,
            )
            .functions[0]
