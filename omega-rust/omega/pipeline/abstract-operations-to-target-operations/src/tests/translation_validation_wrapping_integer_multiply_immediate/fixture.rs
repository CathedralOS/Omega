...
pub(super) fn leaf_error(
    ...
    crate::validation::straight_line_wrapping_integer_multiply_immediate::validate(
        &source.functions[0],
        &target.functions[0],
    )
    .unwrap_err()
...
pub(super) fn candidate_error(
    ...
    let AbstractToTargetTranslationValidationError::FunctionFamily {
        family: AbstractToTargetTranslationFamily::StraightLineWrappingIntegerMultiplyImmediate,
        error:
            AbstractToTargetTranslationFamilyError::StraightLineWrappingIntegerMultiplyImmediate(error),
        ..
    } = validate_abstract_to_target_translation(&source, target_profile, &target).unwrap_err()
    else {
        panic!(
            "constant integer wrapping integer multiplication corruption must fail in its exact validator"
        )
    };
    error
}
