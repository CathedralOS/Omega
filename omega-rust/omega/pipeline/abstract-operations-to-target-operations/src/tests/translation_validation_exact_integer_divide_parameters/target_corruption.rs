...
#[test]
fn exact_integer_divide_target_provenance_corruption_fails_closed() {
    assert_eq!(
...
#[test]
fn exact_integer_divide_target_outer_expression_and_obligation_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactDivide { psi_operation, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactDivide { obligation, .. } = expression(operation) else {
            unreachable!()
        };
...
#[test]
fn exact_integer_divide_target_nested_operand_corruption_fails_closed() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactDivide { left, .. } = expression(operation) else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactDivide { right, .. } = expression(operation) else {
            unreachable!()
        };
        let TargetIntegerExpression::Parameter {
            parameter_index, ..
    ...
    for mutate_left in [true, false] {
        assert_target_operation_error(|operation| {
            let TargetIntegerExpression::ExactDivide { left, right, .. } = expression(operation)
            else {
                unreachable!()
            };
...
#[test]
fn exact_integer_divide_rejects_swapped_operands_and_semantic_substitutions() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactDivide { left, right, .. } = expression(operation) else {
            unreachable!()
        };
    ...
    for substitute in 1..=14 {
        assert_target_operation_error(|operation| {
            ...
            let TargetIntegerExpression::ExactDivide {
                psi_operation,
                obligation,
                left,
                right,
