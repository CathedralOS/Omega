...
use crate::StraightLineExactIntegerDivideParametersTranslationError;
use abstract_operations::{AbstractFunction, AbstractOperation};
use semantic_vocabulary::{IntegerSign, IntegerType, ObligationId, PlaceId, ScalarType, ValueId};

#[test]
fn exact_integer_divide_source_identity_type_and_obligation_corruption_fails_closed() {
    assert_eq!(
        leaf_error(|function| function.parameters.clear()),
        StraightLineExactIntegerDivideParametersTranslationError::SourceParameters
    );
    assert_eq!(
        leaf_error(
            |function| super::super::scalar_result_mut(function).scalar_type = ScalarType::Boolean
        ),
        StraightLineExactIntegerDivideParametersTranslationError::SourceResult
    );
    assert_eq!(
        leaf_error(|function| {
            let parameter = function.parameters[0].value;
            let AbstractOperation::ExactIntegerDivide { result, .. } = &mut function.operations[0]
            else {
                unreachable!()
            };
            *result = parameter;
    ...
    assert_eq!(
        leaf_error(|function| {
            let AbstractOperation::ExactIntegerDivide { left, .. } = &mut function.operations[0]
            else {
                unreachable!()
            };
            *left = ValueId::new(51_500).unwrap();
        }),
        StraightLineExactIntegerDivideParametersTranslationError::SourceLeftOperandLink
    );
    assert_eq!(
        leaf_error(|function| {
            let AbstractOperation::ExactIntegerDivide { right, .. } = &mut function.operations[0]
            else {
                unreachable!()
            };
            *right = ValueId::new(51_501).unwrap();
        }),
        StraightLineExactIntegerDivideParametersTranslationError::SourceRightOperandLink
    );
    assert_eq!(
        leaf_error(|function| {
    ...
    assert_eq!(
        leaf_error(|function| {
            let AbstractOperation::ExactIntegerDivide { obligation, .. } =
        ...
        StraightLineExactIntegerDivideParametersTranslationError::TargetOperation
    );
    assert_eq!(
        leaf_error(|function| set_integer_type(function, integer_type(IntegerSign::Signed, 24))),
        StraightLineExactIntegerDivideParametersTranslationError::SourceParameterShape
    );
    assert_eq!(
...
#[test]
fn exact_integer_divide_source_semantics_return_and_cleanup_corruption_fails_closed() {
    ...
    assert_eq!(
        ...
        StraightLineExactIntegerDivideParametersTranslationError::SourceReturnLink
    );
    assert_eq!(
        leaf_error(|function| {
        ...
        StraightLineExactIntegerDivideParametersTranslationError::SourceCleanup
    );
    assert_eq!(
...
fn replace_divide(function: &mut AbstractFunction, substitute: u8) {
    let AbstractOperation::ExactIntegerDivide {
        psi_operation,
        obligation,
        result,
        scalar_type,
    ...
    function.operations[0] = match substitute {
        1 => AbstractOperation::ExactIntegerRemainder {
            psi_operation,
            obligation,
            result,
            scalar_type,
