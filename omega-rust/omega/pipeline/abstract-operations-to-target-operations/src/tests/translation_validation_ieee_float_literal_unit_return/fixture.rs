...
pub(super) fn machine() -> MachineId {
    MachineId::new(59_001).unwrap()
}

pub(super) fn literal_operation() -> OperationId {
    OperationId::new(59_003).unwrap()
}

pub(super) fn literal_result() -> ValueId {
    ValueId::new(59_004).unwrap()
}

pub(super) fn literal_value() -> IeeeFloatValue {
    IeeeFloatValue::Binary64(0x7ff8_1234_5678_9abc)
}

pub(super) fn return_edge() -> EdgeId {
    EdgeId::new(59_005).unwrap()
}
...
pub(super) fn base_plan() -> AbstractOperationPlan {
    ...
    AbstractOperationPlan {
        psi: TerminalPsiIdentity {
            ...
            program_fingerprint: SemanticFingerprint::from_bytes([0x59; 32]),
        },
        entry: machine(),
        structural_types: vec![
            StructuralTypeDeclaration {
                id: StructuralTypeId::new(59_010).unwrap(),
                identity: "test::float_roster_i32".into(),
                shape: StructuralTypeShape::PrimitiveScalar(ScalarType::Integer(
                    IntegerType::new(IntegerSign::Signed, 32).unwrap(),
                )),
            },
            StructuralTypeDeclaration {
                id: StructuralTypeId::new(59_009).unwrap(),
                identity: "test::float_roster_bool".into(),
...
pub(super) fn leaf_error(
    ...
    crate::validation::straight_line_ieee_float_literal_unit_return::validate(
        &source.functions[0],
        target_profile,
        &target.functions[0],
    )
...
pub(super) fn candidate_error(
    ...
    let AbstractToTargetTranslationValidationError::FunctionFamily {
        family: AbstractToTargetTranslationFamily::StraightLineIeeeFloatLiteralUnitReturn,
        error: AbstractToTargetTranslationFamilyError::StraightLineIeeeFloatLiteralUnitReturn(error),
        ..
    } = validate_abstract_to_target_translation(&source, target_profile, &candidate).unwrap_err()
    else {
        panic!("IEEE-literal Unit-return corruption must fail at its independent validator")
    };
    error
}
...
pub(super) fn fixed_integer_scalar_abi(target: NativeTarget) -> FixedIntegerScalarFunctionAbi {
    ...
    FixedIntegerScalarFunctionAbi {
        result: FixedIntegerScalarAbiValue {
            value: ValueId::new(59_050).unwrap(),
            scalar_type,
...
pub(super) fn target_structural_parameter() -> TargetStructuralParameter {
    ...
    TargetStructuralParameter {
        place: PlaceId::new(59_051).unwrap(),
        structural_type: StructuralTypeId::new(59_052).unwrap(),
