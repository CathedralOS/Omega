        ...
        for value_type in types() {
            for count_type in types() {
                for (value_index, count_index) in placements {
                    ...
                    let source = exact_integer_shift_right_parameters_plan(
                        &parameter_types,
                        value_index,
                        count_index,
                    );
                    ...
                    let AbstractToTargetFunctionTranslationDisposition::Validated(
                        ...
                        panic!("exact shift-right must publish its independent receipt")
                    };
                    assert_eq!(row.machine(), source.entry);
                    assert_eq!(row.shift_operation(), OperationId::new(7_300).unwrap());
                    assert_eq!(
                        row.obligation(),
                        semantic_vocabulary::ObligationId::new(7_302).unwrap()
                    );
                    assert_eq!(row.return_edge(), EdgeId::new(3_004).unwrap());
                    assert_eq!(row.source_value(), ValueId::new(7_301).unwrap());
...
#[test]
fn exact_shift_right_retains_reversed_same_operand_and_mixed_roster_custody() {
    ...
    for scalar_type in integer_types {
        ...
        for (value_index, count_index) in [(2, 1), (1, 1)] {
            ...
            let AbstractToTargetFunctionTranslationDisposition::Validated(
                ...
                panic!("exact shift-right must retain value/count order")
            };
            assert_eq!(row.value_parameter_index(), value_index);
            assert_eq!(row.count_parameter_index(), count_index);
            assert_eq!(
