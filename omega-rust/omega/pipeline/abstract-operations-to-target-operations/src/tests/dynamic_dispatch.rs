...
fn stored_descriptor_plan() -> abstract_operations::AbstractOperationPlan {
    let source = r#"
...
fn lowers_stored_descriptor_to_one_shared_target_two_word_requirement() {
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let caller = lowered
            ...
            .expect("entry caller");
        let target_operations::TargetOperation::UnitBody(body) = &caller.operation else {
        ...
        let store = body
            ...
            .find_map(|operation| match operation {
                target_operations::TargetUnitOperation::StoreDynamicDescriptor {
                    stored,
                    source_argument,
                    ..
        ...
        let call = body
            ...
            .iter()
            .find_map(|operation| match operation {
                target_operations::TargetUnitOperation::StoredDynamicScalarCall {
                    dynamic_dispatch,
                    source_argument,
                    call_plan,
                    ..
...
fn forwarded_parameter_plan() -> abstract_operations::AbstractOperationPlan {
    let source = r#"
        trait Measure {
            machine measure(&self) -> i32;
        }
...
fn multi_hop_forwarded_parameter_plan() -> abstract_operations::AbstractOperationPlan {
    let source = r#"
        trait Measure {
            machine measure(&self) -> i32;
        }
...
fn lowers_rebound_descriptor_into_forwarded_call_abi() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let abstract_operations::AbstractDynamicDescriptorSource::Rebound {
            initial,
            rebound,
            application,
            ..
