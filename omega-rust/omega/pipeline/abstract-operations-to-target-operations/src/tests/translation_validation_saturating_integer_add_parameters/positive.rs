...
#[test]
fn validates_all_saturating_integer_add_types_and_operand_placements_on_every_target() {
    let targets = [
    ...
    for target_profile in targets {
        for scalar_type in &integers {
            for (left, right) in operand_pairs {
                let source = saturating_integer_add_parameters_plan(
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineSaturatingIntegerAddParameters(row),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("saturating-integer-add must publish its family receipt")
                };
                assert_eq!(row.machine(), source.entry);
                assert_eq!(row.add_operation(), OperationId::new(4_820).unwrap());
                assert_eq!(row.return_edge(), EdgeId::new(3_004).unwrap());
                assert_eq!(row.source_value(), ValueId::new(4_821).unwrap());
...
#[test]
fn saturating_integer_add_retains_order_and_mixed_roster_custody() {
    ...
    for (left, right) in [(2, 1), (1, 1)] {
        ...
        let AbstractToTargetFunctionTranslationDisposition::Validated(
            ...
            panic!("saturating-integer-add must retain ordered operand custody")
        };
        assert_eq!(row.left_parameter_index(), left);
        assert_eq!(row.right_parameter_index(), right);
        assert_eq!(row.left_value(), source.functions[0].parameters[left].value);
