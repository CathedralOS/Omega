        ...
        for scalar_type in native_fixed_types() {
            for (left, right) in boundary_pairs(scalar_type) {
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("constant-operand wrapping divide must publish only its exact family")
                };
...
#[test]
fn classifier_is_disjoint_from_plain_parameter_and_other_division_policies() {
    ...
    *value = ValueId::new(84_004).unwrap();
    assert!(
        !crate::validation::straight_line_wrapping_integer_divide_immediate_operands::is_candidate(
            &plain.functions[0]
        )
    );
    ...
    for replacement in [
        ...
        assert!(!crate::validation::straight_line_wrapping_integer_divide_immediate_operands::is_candidate(
            &adjacent.functions[0]
        ));
    }

    let parameter =
        super::super::parameter_translation_fixture::wrapping_integer_divide_parameters_plan(
            &[
