...
pub(super) fn base_plan(
    ...
    let entry = BlockId::new(81_002).unwrap();
    AbstractOperationPlan {
        ...
        functions: vec![AbstractFunction {
            ...
            result: AbstractFunctionResult::Scalar(AbstractResult {
                value: ValueId::new(81_010).unwrap(),
            ...
            operations: vec![
                ...
                AbstractOperation::IntegerConstant {
                    psi_operation: OperationId::new(81_005).unwrap(),
                    result: ValueId::new(81_006).unwrap(),
                    scalar_type: ScalarType::Integer(scalar_type),
                    value: right_value,
                },
                AbstractOperation::SaturatingIntegerSubtract {
                    psi_operation: OperationId::new(81_007).unwrap(),
                    result: ValueId::new(81_008).unwrap(),
                    scalar_type,
                    left: ValueId::new(81_004).unwrap(),
                    right: ValueId::new(81_006).unwrap(),
                },
                AbstractOperation::Return {
                    psi_edge: EdgeId::new(81_009).unwrap(),
                    result: ValueId::new(81_010).unwrap(),
                    value: ValueId::new(81_008).unwrap(),
                    scalar_type: ScalarType::Integer(scalar_type),
...
pub(super) fn leaf_error(
    ...
    crate::validation::straight_line_saturating_integer_subtract_immediate::validate(
        &source.functions[0],
        &target.functions[0],
    )
    .unwrap_err()
...
pub(super) fn candidate_error(
    ...
    let AbstractToTargetTranslationValidationError::FunctionFamily {
        family: AbstractToTargetTranslationFamily::StraightLineSaturatingIntegerSubtractImmediate,
        error:
            AbstractToTargetTranslationFamilyError::StraightLineSaturatingIntegerSubtractImmediate(
                error,
            ),
        ..
    } = validate_abstract_to_target_translation(&source, target_profile, &target).unwrap_err()
    else {
        panic!("constant saturating integer-subtract corruption must fail in its exact validator")
    };
    error
}
