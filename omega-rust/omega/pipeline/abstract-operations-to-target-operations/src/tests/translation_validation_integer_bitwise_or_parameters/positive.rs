...
#[test]
fn validates_all_native_integer_bitwise_or_types_registers_and_stack_on_every_target() {
    let targets = [
    ...
    for target_profile in targets {
        for scalar_type in &integers {
            for parameter_count in [2, 10] {
                let source = uniform_integer_bitwise_or_plan(*scalar_type, parameter_count);
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("integer bitwise-OR must publish its family receipt")
                };
                assert_eq!(row.machine(), source.entry);
                assert_eq!(row.or_operation(), OperationId::new(4_600).unwrap());
                assert_eq!(row.return_edge(), EdgeId::new(3_004).unwrap());
                assert_eq!(row.source_value(), ValueId::new(4_601).unwrap());
...
#[test]
fn integer_bitwise_or_retains_order_identity_and_mixed_roster_custody() {
    let integer = integer_type(IntegerSign::Unsigned, 32);
    for (left, right) in [(2, 1), (1, 1)] {
        let source = integer_bitwise_or_parameters_plan(
            &[
                ScalarType::Boolean,
        ...
        let AbstractToTargetFunctionTranslationDisposition::Validated(
            ...
            panic!("integer bitwise-OR must retain ordered operand custody")
        };
