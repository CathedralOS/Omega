...
fn target_provenance_corruption_fails_closed() {
    assert_eq!(
        ...
        StraightLineIntegerBitwiseOrImmediateTranslationError::TargetProvenance
    );
    assert_eq!(
...
fn target_immediate_corruption_fails_closed() {
    for mutate in [
        ...
        assert_eq!(
            ...
            StraightLineIntegerBitwiseOrImmediateTranslationError::TargetOperation
        );
    }
}
...
fn mutate_edge(operation: &mut TargetOperation) {
    ...
    *psi_edge = EdgeId::new(78_200).unwrap();
}
...
fn mutate_source_value(operation: &mut TargetOperation) {
    ...
    *source_value = ValueId::new(78_201).unwrap();
}
...
fn substitute_operation(operation: &mut TargetOperation) {
    *operation = TargetOperation::ReturnIntegerExpression {
        ...
        source_value: ValueId::new(78_008).unwrap(),
