...
#[test]
fn validates_native_bitwise_or_boundaries_on_every_native_target() {
    for target_profile in [
        ...
        for scalar_type in native_types() {
            for (left_value, right_value) in boundary_pairs(scalar_type) {
                let expected = scalar_type.bitwise_or(left_value, right_value).unwrap();
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineIntegerBitwiseOrImmediate(
                        row,
                    ),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("constant integer bitwise-OR must publish only its exact immediate family")
                };
                assert_eq!(row.machine(), machine());
                assert_eq!(
                    row.left_constant_operation(),
                    OperationId::new(78_003).unwrap()
                );
                assert_eq!(
                    row.right_constant_operation(),
                    OperationId::new(78_005).unwrap()
                );
                assert_eq!(
                    row.bitwise_or_operation(),
                    OperationId::new(78_007).unwrap()
                );
                assert_eq!(row.return_edge(), EdgeId::new(78_009).unwrap());
                assert_eq!(row.left_constant_result(), ValueId::new(78_004).unwrap());
                assert_eq!(row.right_constant_result(), ValueId::new(78_006).unwrap());
                assert_eq!(row.bitwise_or_result(), ValueId::new(78_008).unwrap());
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerImmediate {
                        ...
                        value,
                        ..
                    } if source_value == ValueId::new(78_008).unwrap()
...
#[test]
fn classifier_is_disjoint_from_plain_not_and_xor_and_parameter_or() {
    ...
    *value = ValueId::new(78_004).unwrap();
    ...
    bitwise_not.functions[0].operations[1] = AbstractOperation::IntegerBitwiseNot {
        ...
        operand: ValueId::new(78_004).unwrap(),
    };
    ...
    for replacement in [
        ...
        assert!(
            !crate::validation::straight_line_integer_bitwise_or_immediate::is_candidate(
    ...
    assert!(
        !crate::validation::straight_line_integer_bitwise_or_immediate::is_candidate(
            &super::super::parameter_translation_fixture::uniform_integer_bitwise_or_plan(
                scalar_type(),
                2,
            )
            .functions[0]
