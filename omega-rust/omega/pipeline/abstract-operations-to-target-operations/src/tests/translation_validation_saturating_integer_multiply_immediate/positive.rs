...
#[test]
fn validates_native_saturating_mul_boundaries_on_every_native_target() {
    for target_profile in [
        ...
        for scalar_type in native_types() {
            for (left_value, right_value) in boundary_pairs(scalar_type) {
                let expected = scalar_type.saturating_mul(left_value, right_value).unwrap();
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineSaturatingIntegerMultiplyImmediate(row),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("constant saturating integer-multiply must publish only its exact immediate family")
                };
                assert_eq!(row.machine(), machine());
                assert_eq!(
                    row.left_constant_operation(),
                    OperationId::new(82_003).unwrap()
                );
                assert_eq!(
                    row.right_constant_operation(),
                    OperationId::new(82_005).unwrap()
                );
                assert_eq!(
                    row.saturating_mul_operation(),
                    OperationId::new(82_007).unwrap()
                );
                assert_eq!(row.return_edge(), EdgeId::new(82_009).unwrap());
                assert_eq!(row.left_constant_result(), ValueId::new(82_004).unwrap());
                assert_eq!(row.right_constant_result(), ValueId::new(82_006).unwrap());
                assert_eq!(row.saturating_mul_result(), ValueId::new(82_008).unwrap());
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerImmediate {
                        ...
                        value,
                        ..
                    } if source_value == ValueId::new(82_008).unwrap()
    ...
    *value = ValueId::new(82_004).unwrap();
    ...
    for replacement in [
        AbstractOperation::ExactIntegerMultiply {
            ...
            obligation: ObligationId::new(82_011).unwrap(),
        },
        ...
        assert!(
            !crate::validation::straight_line_saturating_integer_multiply_immediate::is_candidate(
    ...
    assert!(
        !crate::validation::straight_line_saturating_integer_multiply_immediate::is_candidate(
            &super::super::parameter_translation_fixture::uniform_saturating_integer_multiply_plan(
                scalar_type(),
                2,
            )
            .functions[0]
...
pub(super) fn boundary_pairs(scalar_type: IntegerType) -> [(IntegerValue, IntegerValue); 4] {
    ...
    [(maximum, two), (two, maximum), (minimum, two), (two, two)]
}
