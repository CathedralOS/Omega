...
pub(super) fn base_plan(
    ...
    let entry = BlockId::new(82_002).unwrap();
    AbstractOperationPlan {
        psi: TerminalPsiIdentity {
            vocabulary_marker: VocabularyMarker::CURRENT,
            program_fingerprint: SemanticFingerprint::from_bytes([0x82; 32]),
        },
        entry: machine(),
        ...
        functions: vec![AbstractFunction {
            ...
            result: AbstractFunctionResult::Scalar(AbstractResult {
                value: ValueId::new(82_010).unwrap(),
            ...
            operations: vec![
                ...
                AbstractOperation::IntegerConstant {
                    psi_operation: OperationId::new(82_005).unwrap(),
                    result: ValueId::new(82_006).unwrap(),
                    scalar_type: ScalarType::Integer(scalar_type),
                    value: right_value,
                },
                AbstractOperation::SaturatingIntegerMultiply {
                    psi_operation: OperationId::new(82_007).unwrap(),
                    result: ValueId::new(82_008).unwrap(),
                    scalar_type,
                    left: ValueId::new(82_004).unwrap(),
                    right: ValueId::new(82_006).unwrap(),
                },
                AbstractOperation::Return {
                    psi_edge: EdgeId::new(82_009).unwrap(),
                    result: ValueId::new(82_010).unwrap(),
                    value: ValueId::new(82_008).unwrap(),
                    scalar_type: ScalarType::Integer(scalar_type),
...
pub(super) fn leaf_error(
    ...
    crate::validation::straight_line_saturating_integer_multiply_immediate::validate(
        &source.functions[0],
        &target.functions[0],
    )
    .unwrap_err()
...
pub(super) fn candidate_error(
    ...
    let AbstractToTargetTranslationValidationError::FunctionFamily {
        family: AbstractToTargetTranslationFamily::StraightLineSaturatingIntegerMultiplyImmediate,
        error:
            AbstractToTargetTranslationFamilyError::StraightLineSaturatingIntegerMultiplyImmediate(
                error,
            ),
        ..
    } = validate_abstract_to_target_translation(&source, target_profile, &target).unwrap_err()
    else {
        panic!("constant saturating integer-multiply corruption must fail in its exact validator")
    };
    error
}
