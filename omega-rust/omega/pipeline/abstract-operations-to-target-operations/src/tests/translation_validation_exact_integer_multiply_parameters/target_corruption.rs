...
use semantic_vocabulary::{EdgeId, IntegerSign, ObligationId, OperationId, ValueId};
use target_operations::{
    ScalarParameterLocation, TargetIntegerExpression, TargetOperation, TerminalPsiProvenance,
};

#[test]
fn exact_integer_multiply_target_provenance_corruption_fails_closed() {
    assert_eq!(
...
#[test]
fn exact_integer_multiply_target_outer_expression_and_obligation_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactMultiply { psi_operation, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactMultiply { obligation, .. } = expression(operation)
        else {
            unreachable!()
        };
...
#[test]
fn exact_integer_multiply_target_nested_operand_corruption_fails_closed() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactMultiply { left, .. } = expression(operation) else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactMultiply { right, .. } = expression(operation) else {
            unreachable!()
        };
        let TargetIntegerExpression::Parameter {
            parameter_index, ..
    ...
    for mutate_left in [true, false] {
        assert_target_operation_error(|operation| {
            let TargetIntegerExpression::ExactMultiply { left, right, .. } = expression(operation)
            else {
                unreachable!()
            };
...
#[test]
fn exact_integer_multiply_rejects_swapped_operands_and_semantic_substitutions() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::ExactMultiply { left, right, .. } = expression(operation)
        else {
            unreachable!()
    ...
    for substitute in 1..=8 {
        assert_target_operation_error(|operation| {
            ...
            let TargetIntegerExpression::ExactMultiply {
                psi_operation,
                left,
                right,
                ..
            ...
            *expression = match substitute {
                ...
                2 => TargetIntegerExpression::SaturatingMultiply {
                    psi_operation: *psi_operation,
                    left: left.clone(),
                    right: right.clone(),
                },
                ...
                6 => TargetIntegerExpression::ExactSubtract {
                    psi_operation: *psi_operation,
                    obligation: ObligationId::new(51_516).unwrap(),
                    left: left.clone(),
                    right: right.clone(),
                },
                7 => TargetIntegerExpression::WrappingSubtract {
                    psi_operation: *psi_operation,
                    left: left.clone(),
                    right: right.clone(),
                },
                8 => TargetIntegerExpression::SaturatingSubtract {
                    psi_operation: *psi_operation,
                    left: left.clone(),
                    right: right.clone(),
                },
