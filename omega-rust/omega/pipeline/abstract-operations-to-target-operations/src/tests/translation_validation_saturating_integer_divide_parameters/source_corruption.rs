...
#[test]
fn saturating_integer_divide_source_identity_type_and_obligation_corruption_fails_closed() {
    assert_eq!(
        leaf_error(|function| function.parameters.clear()),
        StraightLineSaturatingIntegerDivideParametersTranslationError::SourceParameters
    );
    assert_eq!(
        leaf_error(
            |function| super::super::scalar_result_mut(function).scalar_type = ScalarType::Boolean
        ),
        StraightLineSaturatingIntegerDivideParametersTranslationError::SourceResult
    );
    assert_eq!(
        leaf_error(|function| {
            let parameter = function.parameters[0].value;
            let AbstractOperation::SaturatingIntegerDivide { result, .. } =
    ...
    assert_eq!(
        leaf_error(|function| {
            let AbstractOperation::SaturatingIntegerDivide { left, .. } =
                &mut function.operations[0]
            else {
                unreachable!()
            };
    ...
    assert_eq!(
        leaf_error(|function| {
            let AbstractOperation::SaturatingIntegerDivide { right, .. } =
                &mut function.operations[0]
            else {
                unreachable!()
            };
    ...
    assert_eq!(
        ...
        StraightLineSaturatingIntegerDivideParametersTranslationError::SourceParameterShape
    );
    assert_eq!(
...
#[test]
fn saturating_integer_divide_source_semantics_return_and_cleanup_corruption_fails_closed() {
    ...
    assert_eq!(
        ...
        StraightLineSaturatingIntegerDivideParametersTranslationError::SourceCleanup
    );
    assert_eq!(
