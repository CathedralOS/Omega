...
#[test]
fn wrapping_integer_multiply_target_provenance_corruption_fails_closed() {
    assert_eq!(
...
#[test]
fn wrapping_integer_multiply_target_outer_expression_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingMultiply { psi_operation, .. } = expression(operation)
        else {
            unreachable!()
        };
...
#[test]
fn wrapping_integer_multiply_target_nested_operand_corruption_fails_closed() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingMultiply { left, .. } = expression(operation) else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingMultiply { right, .. } = expression(operation) else {
            unreachable!()
        };
        let TargetIntegerExpression::Parameter {
            parameter_index, ..
    ...
    for mutate_left in [true, false] {
        assert_target_operation_error(|operation| {
            let TargetIntegerExpression::WrappingMultiply { left, right, .. } =
...
#[test]
fn wrapping_integer_multiply_rejects_swapped_operands_and_semantic_substitutions() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingMultiply { left, right, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    for substitute in [1, 2, 3, 4, 5, 6] {
        assert_target_operation_error(|operation| {
            ...
            let TargetIntegerExpression::WrappingMultiply {
                psi_operation,
                left,
                right,
            } = expression
            ...
            *expression = match substitute {
                ...
                2 => TargetIntegerExpression::WrappingSubtract {
                    psi_operation: *psi_operation,
                    left: left.clone(),
                    right: right.clone(),
                },
                3 => TargetIntegerExpression::ExactMultiply {
                    psi_operation: *psi_operation,
                    obligation: ObligationId::new(50_514).unwrap(),
                    left: left.clone(),
                    right: right.clone(),
                },
                4 => TargetIntegerExpression::SaturatingMultiply {
                    psi_operation: *psi_operation,
                    left: left.clone(),
                    right: right.clone(),
                },
                5 => TargetIntegerExpression::ExactAdd {
                    psi_operation: *psi_operation,
                    obligation: ObligationId::new(50_515).unwrap(),
                    left: left.clone(),
                    right: right.clone(),
                },
...
fn assert_target_operation_error(mutate: impl FnOnce(&mut TargetOperation)) {
    assert_eq!(
        ...
        StraightLineWrappingIntegerMultiplyParametersTranslationError::TargetOperation
    );
}
