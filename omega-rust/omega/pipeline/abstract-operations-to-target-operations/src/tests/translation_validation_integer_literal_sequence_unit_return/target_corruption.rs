...
fn target_envelope_corruption_fails_closed() {
    assert_eq!(
        candidate_error(|candidate| candidate.functions[0].scalar_abi =
            Some(scalar_abi(NativeTarget::linux_x64()))),
...
fn target_return_and_whole_structural_roster_corruption_fails_closed() {
    ...
    body.structural_types.make_mut()[0].identity = "test::substituted_integer_sequence".into();
    assert_eq!(
