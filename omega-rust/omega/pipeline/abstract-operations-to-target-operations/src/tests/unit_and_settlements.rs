...
fn parameter_dynamic_unit_dispatch_retains_two_word_result_less_abi() {
    ...
    let parameter = terminal_psi::TerminalDynamicDescriptorParameter {
        owner: machine,
        ordinal: 0,
        source_position: 0,
        trait_identity: "test::Touch".into(),
        access: StructuralAccess::SharedBorrow,
        requirements: vec![terminal_psi::TerminalDynamicRequirement {
            slot: 0,
            ...
            operations: vec![
                ...
                AbstractOperation::CallDynamicParameterUnit {
                    ...
                    dynamic_dispatch: abstract_operations::AbstractParameterDynamicDispatch {
                        parameter,
                        dispatch: terminal_psi::TerminalParameterDynamicDispatch {
                            owner: machine,
                            operation,
                            parameter_ordinal: 0,
                            requirement_slot: 0,
...
fn fixed_array_layout_repeats_padded_nested_elements_and_rejects_overflow() {
    ...
    let u64_type = ScalarType::Integer(
        IntegerType::new(semantic_vocabulary::IntegerSign::Unsigned, 64).unwrap(),
    );
    let declarations = [
...
fn metadata_only_boundary_requires_the_exact_preceding_port_realization() {
    ...
    result_bearing.boundary_machines[0].result =
        terminal_psi::BoundaryMachineResult::Scalar(result.scalar_type);
    ...
    *operation_result = abstract_operations::AbstractBoundaryResult::Scalar(result);
    assert_eq!(
        lower_to_target_operations_with_settlements(
            &result_bearing,
            NativeTarget::linux_x64(),
...
fn claim_completion_only_boundary_retains_two_linear_claims_without_physical_inputs() {
    ...
    use terminal_psi::{CompletionReceipt, EntryClaim};
