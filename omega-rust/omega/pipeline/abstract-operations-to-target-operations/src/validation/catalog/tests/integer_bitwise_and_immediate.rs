//! Catalog selection canaries for the exact constant-integer-bitwise-AND family.
    ...
    assert!(matches!(
        ...
        Err(
            AbstractToTargetTranslationValidationError::AmbiguousFunctionFamily {
                first: AbstractToTargetTranslationFamily::StraightLineIntegerBitwiseAndImmediate,
                second: AbstractToTargetTranslationFamily::StraightLineIntegerBitwiseAndImmediate,
                ..
            }
        )
    ));
...
fn pair() -> (AbstractFunction, TargetFunction) {
    ...
    let bitwise_and_operation = OperationId::new(73_307).unwrap();
    let bitwise_and_result = ValueId::new(73_308).unwrap();
    ...
    (
        ...
        TargetFunction {
            ...
            operation: TargetOperation::ReturnIntegerImmediate {
                psi_edge: edge,
                source_value: bitwise_and_result,
                scalar_type,
