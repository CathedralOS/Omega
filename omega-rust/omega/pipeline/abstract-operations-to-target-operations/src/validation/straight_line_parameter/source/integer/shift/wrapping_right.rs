//! Wrapping `[WrappingIntegerShiftRight(parameter, parameter), Return]` source replay.
...
> {
    ...
    if !cleanup_actions.is_empty() {
        return Err(StraightLineWrappingIntegerShiftRightParametersTranslationError::SourceCleanup);
    }
