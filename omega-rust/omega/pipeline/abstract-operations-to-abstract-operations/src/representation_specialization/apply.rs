...
use super::{
    AppliedCaseMembershipSpecialization, CaseMembershipSpecializationError,
    PsiTransformationLedger, PsiTransformationRecord, ValidatedCaseMembershipSpecialization,
...
pub(super) fn validated(
    ...
    if validated.output.identity != validated.candidate.output {
        return Err(CaseMembershipSpecializationError::OutputIdentityMismatch {
    ...
    .map_err(CaseMembershipSpecializationError::InvalidLedger)?;
    Ok(AppliedCaseMembershipSpecialization {
        session: next,
