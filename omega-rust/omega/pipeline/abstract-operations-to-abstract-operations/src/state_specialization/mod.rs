...
//! parameters — the state argument. When an incoming edge entering the
...
//! incoming edge is either an unconditional `Jump` successor or one arm of a
//! `Conditional` predecessor; a fused conditional arm leaves its sibling arm
//! byte-exact. The dispatch state and both of its arm edges remain for every
//! other incoming edge, so no source work is removed and no block becomes
//! unreachable.
//!
