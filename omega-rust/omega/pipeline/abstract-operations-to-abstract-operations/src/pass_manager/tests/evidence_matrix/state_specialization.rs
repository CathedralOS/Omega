...
const CASE: SelectionMatrix = SelectionMatrix {
    ...
    expected_rule,
    // Two dispatch sites commit independently: the Boolean state argument
    // fuses its constant-supplied jump and conditional-arm edges, and the
    // integer state argument's `istate < 4` dispatch fuses its one
    // constant-supplied jump edge — each parameter-supplied variable edge
    // keeps its dispatch reachable.
    expected_commits: 2,
