//! Producer-side scalar-operand substitution for forwarded field values.
//!
//! A `Forward` row rebinds every covered scalar-operand use of the read's
//! `result` to the proven initializer. This rewrite is the producing mirror
//! of the substitution lane the independent replay applies: it covers the
//! same scalar operand positions — establishments, scalar stores, ordinary
//! calls, arithmetic and comparisons, shifts, jump and conditional bindings,
//! returns, and atomic operands — and deliberately does not touch
//! byte-sequence or structural forms, which admission has already proven
//! cannot reference the result.

use super::super::O;
use semantic_vocabulary::ValueId;

/// Rebinds every covered scalar-operand occurrence of `from` inside
/// `operation` to `to`. Positions outside the substitution lane —
/// byte-sequence operands, structural place roots, descriptor carriers —
/// are not scalar uses of a field result and are never rewritten.
pub(crate) fn rewrite_scalar_uses(operation: &mut O, from: ValueId, to: ValueId) {
