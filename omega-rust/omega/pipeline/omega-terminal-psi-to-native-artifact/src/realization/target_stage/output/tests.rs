...
fn ordinary_and_ranked_outputs_own_the_same_representation() {
    ...
    let artifact =
        psi_checked_trees_to_terminal::produce_terminal_artifact(&checked, "Root::countdown")
    ...
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        for program in programs {
            ...
            assert_eq!(Arc::strong_count(&program), 1);
            assert!(program.native_callback_arguments.is_empty());
            let assigned = omega_target_operations_to_assigned_target_operations::assign_registers_with_native_callbacks(
                &program,
            )
            .expect("assignment consumes current representation for both roles");
            let previous = omega_target_operations_to_assigned_target_operations::assign_registers(
                &program.plan,
            )
            .expect("prior callback-free assignment");
            assert_eq!(assigned.plan, previous);
            assert_eq!(
                omega_machine_emission::emit_machine_code_with_native_callbacks(&assigned).unwrap(),
                omega_machine_emission::emit_machine_code(&previous).unwrap(),
