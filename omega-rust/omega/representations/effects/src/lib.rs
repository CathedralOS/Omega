...
//! error anywhere. Same shape as the carrier split in `runtime-abi`, same
...
//! Consumed by `visualizations` (which renders these carriers as JSON
//! manifests) and `artifacts` (which writes them beside a build).
//!
...
//! `visualizations/src/executable_tcb_manifest.rs:661`, below that file's
//! `#[cfg(test)]`. The CONSUMER at line 190 of the same file is production and
//! is reached from `compiler/src/pipeline/artifacts.rs`, so a real
