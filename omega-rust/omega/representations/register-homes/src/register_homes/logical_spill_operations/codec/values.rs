...
use selected_instructions::{SelectedInstructionId, VirtualRegisterOrigin};
...
pub(super) fn decode_origin(
    ...
    match cursor.byte()? {
        ...
        8 => {
            let instruction = SelectedInstructionId(u32::from_le_bytes(cursor.array()?));
            let raw = u64::from_le_bytes(cursor.array()?);
            Ok(VirtualRegisterOrigin::StructuralObservation {
                instruction,
                place: semantic_vocabulary::PlaceId::new(raw)
                    .ok_or(LogicalSpillOperationDecodeError::InvalidPlaceId(raw))?,
        ...
        6 => Ok(VirtualRegisterOrigin::SpillAddress {
            instruction: SelectedInstructionId(u32::from_le_bytes(cursor.array()?)),
            register: selected_instructions::VirtualRegisterId(u32::from_le_bytes(cursor.array()?)),
        }),
