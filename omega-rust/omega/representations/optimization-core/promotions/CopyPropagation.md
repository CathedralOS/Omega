...
- Rollback evidence: PENDING — no leg yet exercises a `CopyPropagation`-selected build under `--disable-optimization CopyPropagation` rejoining the ordinary artifact on every hosted target; the unselected-name no-op on `HOSTED_NATIVE_TARGETS` is pinned by `omega-rust/omega/compiler/compiler/tests/no_selection_golden/rollback.rs::rollback_to_empty_selection_rejoins_exact_ordinary_path_on_every_target`
