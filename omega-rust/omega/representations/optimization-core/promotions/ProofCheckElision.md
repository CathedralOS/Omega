...
- Rollback evidence: PENDING — no leg yet exercises a `ProofCheckElision`-selected build under `--disable-optimization ProofCheckElision` rejoining the ordinary artifact on every hosted target; the overlay's product gating and empty-request custody are pinned by `omega-rust/omega/compiler/compiler/tests/no_selection_golden/rollback.rs::native_rollback_rejects_products_that_do_not_enter_native_realization` and `omega-rust/omega/compiler/compiler/tests/no_selection_golden/rollback.rs::empty_rollback_request_leaves_no_release_receipt`
