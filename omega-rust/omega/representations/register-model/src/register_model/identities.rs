...
pub(crate) fn physical_register_model_identity(
...
pub(crate) fn register_constraint_catalog_identity(
...
pub(crate) fn register_reservation_profile_identity(
    target: NativeTarget,
    physical: PhysicalRegisterModelIdentity,
...
pub fn target_register_environment_identity(
    ...
    selected_keys: &crate::TargetRegisterEnvironmentConstraintKeys,
) -> TargetRegisterEnvironmentIdentity {
    let mut bytes = Vec::new();
    native_target(&mut bytes, target);
    ...
    for key in [
        ...
        selected_keys.copy_bytes,
        selected_keys.store,
        selected_keys.address_offset,
        selected_keys.store64,
        selected_keys.frame_address,
        selected_keys.hosted_read_byte,
        selected_keys.hosted_write_byte_i32,
        selected_keys.save_floating_control,
        selected_keys.restore_floating_control,
        selected_keys.hosted_exit_process_i32,
    ...
    u64_value(&mut bytes, selected_keys.call_unit.len() as u64);
    ...
    for keys in [
        &selected_keys.call_aggregate,
        &selected_keys.call_normalized_foreign,
        &selected_keys.return_aggregate,
    ] {
...
fn optional_constraint_key(bytes: &mut Vec<u8>, key: Option<crate::RegisterConstraintKey>) {
    match key {
        None => byte(bytes, 0),
        Some(key) => {
            byte(bytes, 1);
...
const fn reservation_reason(value: crate::ReservationReason) -> u8 {
    match value {
        crate::ReservationReason::Architectural => 0,
        crate::ReservationReason::StackPointer => 1,
        crate::ReservationReason::FramePointer => 2,
        crate::ReservationReason::Platform => 3,
        crate::ReservationReason::Dispatch => 4,
        crate::ReservationReason::Metering => 5,
        crate::ReservationReason::Syscall => 6,
        crate::ReservationReason::InlineAssembly => 7,
        crate::ReservationReason::Backend => 8,
    }
}
