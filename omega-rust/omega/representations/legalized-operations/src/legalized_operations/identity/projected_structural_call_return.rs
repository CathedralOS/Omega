...
fn encode_caller(bytes: &mut Vec<u8>, function: &target_operations::TargetFunction) {
    ...
    let crash = terminal_codec::encode_crash_route_buckets(crash_continuations)
        .expect("validated crash routes remain canonical");
    encode_len(bytes, crash.len());
    bytes.extend_from_slice(&crash);
}

fn encode_callee(bytes: &mut Vec<u8>, function: &target_operations::TargetFunction) {
    encode_function_header(bytes, function);
    let target_operations::TargetOperation::ReturnStructuralParameter {
        call_plan,
        scalar_parameters: _,
        parameters,
        source,
...
fn encode_target_parameters(
    bytes: &mut Vec<u8>,
    parameters: &[target_operations::TargetStructuralParameter],
) {
    encode_len(bytes, parameters.len());
    for parameter in parameters {
