...
impl RetainedGeneratedSyntaxExtension {
    ...
    ) -> Result<(Vec<SyntaxTrees>, Arc<psi_source::SourceMap>), Vec<Diagnostic>> {
...
mod tests {
    ...
    fn generated_unit_pre_resolution_is_unit_local() {
        ...
        let normalized_counts = units
            .into_iter()
            .map(|unit| {
                psi_build_time_evaluation::evaluate_pre_resolution_with_sources(
                    unit,
                    sources.clone(),
                )
