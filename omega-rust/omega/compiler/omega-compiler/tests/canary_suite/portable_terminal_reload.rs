...
fn consume_portable_terminal_product() {
    ...
    let artifact = psi_terminal_codec::CanonicalTerminalArtifact::from_bytes(&bytes)
        .expect("consumer independently decodes the complete Psi product");
    drop(artifact);
    let mut authority = RejectUnexpectedEffects;
    let execution = psi_terminal_interpreter::interpret_serialized_terminal_artifact_with_effect_handler_measured(
        &bytes,
...
impl psi_terminal_interpreter::TerminalEffectHandler for RejectUnexpectedEffects {
    fn handle_effect(
        &mut self,
        effect: &psi_terminal_interpreter::TerminalEffect,
    ) -> Result<(), psi_terminal_interpreter::TerminalEffectRejection> {
        Err(psi_terminal_interpreter::TerminalEffectRejection::new(
            format!("fresh reload authority rejects unexpected effect {effect:?}"),
        ))
    }
}
