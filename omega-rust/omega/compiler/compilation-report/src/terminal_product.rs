...
pub struct TerminalCallbackOccurrenceProposal {
    ...
    direct_parameter_application: Option<calling_conventions::NativeParameterApplication>,
    callback_thunk_identity: function_identity::MachineFunctionIdentity,
...
impl TerminalCallbackOccurrenceProposal {
    ...
    pub const fn direct_parameter_application(
        &self,
    ) -> Option<&calling_conventions::NativeParameterApplication> {
        self.direct_parameter_application.as_ref()
    }

    pub const fn callback_thunk_identity(&self) -> function_identity::MachineFunctionIdentity {
...
impl TerminalCompilerBuiltinProposal {
    ...
    pub const fn execution(&self) -> target_operations::CompilerBuiltinExecution {
        self.execution
    }
}
...
impl TerminalNativeRealizationProposal {
    ...
    fn validate_evaluated_import_rows(&self) -> Result<(), &'static str> {
        ...
        for plan in self.selected_provider_plans.plans() {
            ...
            for selected_row in &plan.rows {
                ...
                let calling_conventions::ExternalBindingKind::Import { locator } =
                    &external.binding
                else {
                    return Err(
    ...
    pub const fn boundary_application_demands(
        &self,
    ) -> &boundary_applications::TerminalBoundaryApplicationDemands {
...
impl RetainedTerminalArtifact {
    ...
    pub fn into_parts(
        ...
        Vec<backend_plan::BoundNominalCallbackPlacement>,
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        for placement in &self.callback_placements {
            backend_plan::validate_bound_nominal_callback_placement(placement)
        ...
        if let Some(proposal) = &self.native_realization_proposal {
            ...
            for (placement_index, placement) in self.callback_placements.iter().enumerate() {
                ...
                let expected_thunk = backend_plan::canonical_callback_thunk_identity(
                    placement_index,
                    placement,
                )
                .ok_or(
