//! A loop machine whose forty `u64` immediate constants are defined before the
//! loop and carried into it, exceeding every target's allocatable GPR supply.
//! Register allocation must recover, and the cheaper rematerialization form is
//! the recorded decision for these pure materialization results: each use
//! regenerates the immediate in place instead of paying private storage and a
//! reload pair. The manifest ledger binds that choice for replay.
