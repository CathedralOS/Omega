...
fn physical_and_object_publication_retain_the_original_abstract_allocation() {
    ...
    for target in [
        ...
        .unwrap();
        let original = optimized.shared_program();
        ...
        assert!(std::sync::Arc::ptr_eq(
            &original,
            &emission.optimized_target().optimized().shared_program(),
        ));
        let object =
            crate::native_realization::optimized_fragment_projection::stage_fragment_object(
                emission,
            )
            .unwrap();
        let retained = object
        ...
        assert!(std::sync::Arc::ptr_eq(
            &original,
            &retained.shared_program()
        ));
        assert_eq!(validation, retained.validation());
    }
}
