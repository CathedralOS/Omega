...
fn exact_rejoined_all_register_literal_imports_reach_dynamic_elf() {
    ...
    for (profile, count, tail_type, tail_immediate, tail_identity, expected_register, x86_prefix) in [
        (
            ...
            6,
            unsigned_32,
            semantic_vocabulary::IntegerValue::Unsigned(0x7654_3210),
            "u32",
            calling_conventions::MachineRegister::X86R9,
            Some([0x41, 0xb9]),
        ),
        (
            target::TargetProfile::LinuxX64,
            6,
            unsigned_64,
            semantic_vocabulary::IntegerValue::Unsigned(0x7654_3210_fedc_ba98),
            "u64",
            calling_conventions::MachineRegister::X86R9,
            Some([0x49, 0xb9]),
        ),
        (
            ...
            semantic_vocabulary::IntegerValue::Unsigned(0x7654_3210_fedc_ba98),
            "u64",
            calling_conventions::MachineRegister::Aarch64X(7),
            None,
        ),
    ] {
...
fn exact_rejoined_stack_literal_imports_reach_dynamic_elf() {
    ...
    for (profile, count, outbound_bytes) in [
        (target::TargetProfile::LinuxX64, 8, 24),
        (target::TargetProfile::LinuxArm64, 10, 16),
    ] {
...
fn assert_exact_rejoined_literal_import_reaches_dynamic_elf(
    ...
    let signature = calling_conventions::CallSignature {
        parameters: cases
            ...
            .map(|case| {
                ...
                calling_conventions::ValueShape::integer(bytes, bytes)
            })
            .collect(),
        result: None,
    };
    ...
    for (parameter_index, (argument, case)) in target_arguments.iter().zip(cases).enumerate() {
        ...
        assert_eq!(
            ...
            semantic_vocabulary::ScalarType::Integer(case.scalar_type)
        );
        assert_eq!(
            argument.source,
    ...
    match expected_tail {
        ExpectedTailPlacement::Register { register, .. } => assert!(matches!(
            ...
            [calling_conventions::ValueLocation::Register { register: actual, .. }]
                if *actual == register
        )),
    ...
    if profile == target::TargetProfile::LinuxArm64 {
        for (parameter_index, expected_register) in [
            (5, calling_conventions::MachineRegister::Aarch64X(5)),
            (6, calling_conventions::MachineRegister::Aarch64X(6)),
            (7, calling_conventions::MachineRegister::Aarch64X(7)),
        ] {
            assert!(matches!(
    ...
    let mutate_last = |mut candidate: assigned_target_operations::AssignedOperationPlan,
                       mutate: &dyn Fn(
        ...
        assert!(machine_emission::emit_machine_code(&candidate).is_err());
    };
    ...
    mutate_last(assigned.clone(), &|argument| {
        let semantic_vocabulary::ScalarType::Integer(argument_type) = argument.source.scalar_type()
        else {
    ...
    assert!(machine_emission::emit_machine_code(&reordered_assignment).is_err());
    ...
    assert!(machine_emission::emit_machine_code(&extra_assignment).is_err());
    ...
    let call_start = match target.architecture {
        target::Architecture::X86_64 => call.offset - 1,
        target::Architecture::Aarch64 => call.offset,
    };
    ...
    if matches!(expected_tail, ExpectedTailPlacement::Stack { .. }) {
        ...
        stripped_outbound.functions[0].foreign_calls[0]
            .unit_stack
            .outbound = None;
        assert!(image_emission::build_object_artifact(&stripped_outbound).is_err());
        ...
        resized_outbound.functions[0].foreign_calls[0]
            ...
            .unwrap()
            .byte_size += 16;
        assert!(image_emission::build_object_artifact(&resized_outbound).is_err());
        ...
        moved_allocation.functions[0].foreign_calls[0]
            ...
            .unwrap()
            .allocation_offset += 1;
        assert!(image_emission::build_object_artifact(&moved_allocation).is_err());
        ...
        assert!(image_emission::build_object_artifact(&changed_store).is_err());
        ...
        *stack_byte_offset = 0;
        assert!(image_emission::build_object_artifact(&changed_stack_offset).is_err());
        ...
        *byte_size = 4;
        *alignment = 4;
        assert!(image_emission::build_object_artifact(&changed_stack_shape).is_err());
        ...
        assert!(image_emission::build_object_artifact(&overlapped_argument).is_err());
    }

    let mut changed_value = machine_code.clone();
    if let machine_code::InternalUnitScalarArgumentSourceRecord::IntegerImmediate {
        value, ..
    ...
    assert!(image_emission::build_object_artifact(&changed_value).is_err());
    ...
    {
        *scalar_type =
            semantic_vocabulary::IntegerType::address(cases[last_index].scalar_type.bits())
                .unwrap();
    }
    assert!(image_emission::build_object_artifact(&changed_carrier).is_err());
    ...
    assert!(image_emission::build_object_artifact(&changed_definition).is_err());
    ...
    assert!(image_emission::build_object_artifact(&reordered).is_err());
    ...
    assert!(image_emission::build_object_artifact(&changed_interval).is_err());
    ...
    assert!(image_emission::build_object_artifact(&changed_register).is_err());
    ...
    assert!(image_emission::build_object_artifact(&changed_stack).is_err());
    let mut stripped = machine_code.clone();
    stripped.functions[0].foreign_calls[0]
        .scalar_arguments
        .pop();
    assert!(image_emission::build_object_artifact(&stripped).is_err());
    ...
    extra.functions[0].foreign_calls[0]
        .scalar_arguments
        .push(duplicate);
    assert!(image_emission::build_object_artifact(&extra).is_err());
    ...
    assert!(matches!(
        image_emission::build_object_artifact(&wrong_contribution_plan),
        Err(image_emission::ObjectError::ForeignStackProviderPlanMismatch { .. })
    ));
    ...
    over_aligned.functions[0].foreign_calls[0].same_stack_contribution =
        ...
        .unwrap();
    assert!(matches!(
        image_emission::build_object_artifact(&over_aligned),
        Err(
            image_emission::ObjectError::UnsupportedForeignStackAlignment {
...
fn assigned_scalar_arguments_mut(
    plan: &mut assigned_target_operations::AssignedOperationPlan,
    operation_index: usize,
) -> &mut Vec<assigned_target_operations::AssignedNormalizedForeignScalarArgument> {
