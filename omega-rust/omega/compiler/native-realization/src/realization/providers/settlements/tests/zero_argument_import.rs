...
fn exact_rejoined_import_reaches_machine_object_and_dynamic_elf_on_both_targets() {
    ...
    let machine = semantic_vocabulary::MachineId::new(800).unwrap();
    let block = semantic_vocabulary::BlockId::new(800).unwrap();
    let boundary = semantic_vocabulary::BoundaryMachineId::new(800).unwrap();
    let operation = semantic_vocabulary::OperationId::new(800).unwrap();
    let return_edge = semantic_vocabulary::EdgeId::new(800).unwrap();
    let psi = terminal_psi::TerminalPsiIdentity {
        vocabulary_marker: terminal_psi::VocabularyMarker::CURRENT,
        program_fingerprint: terminal_psi::SemanticFingerprint::from_bytes([0x80; 32]),
    };
    let abstract_plan = abstract_operations::AbstractOperationPlan {
        psi,
        entry: machine,
    ...
    for profile in [
        ...
        let wrong_target = match profile {
            target::TargetProfile::LinuxX64 => target::NativeTarget::linux_arm64(),
            target::TargetProfile::LinuxArm64 => target::NativeTarget::linux_x64(),
            _ => unreachable!(),
        };
        ...
        substituted_plan.state.preemption = calling_conventions::Preemption::ProviderDefined;
        ...
        let legacy = effects::SelectedProviderPlanFacts::from_selected_plans(vec![legacy]).unwrap();
        ...
        let builtin_substitution = NativeProviderSettlement {
            ...
            realization: NativeBoundaryRealization::Builtin(
                target_operations::LinuxExitGroupI32Realization.into(),
            ),
        };
        assert!(
            validate_source_evaluated_import_coverage(
        ...
        assert!(
            image_emission::build_object_artifact(&duplicate_call).is_err(),
        ...
        assert!(
            image_emission::build_object_artifact(&substituted_plan).is_err(),
        ...
        assert!(
            image_emission::build_object_artifact(&substituted_relocation).is_err(),
        ...
        assert!(
            image_emission::build_object_artifact(&substituted_bytes).is_err(),
