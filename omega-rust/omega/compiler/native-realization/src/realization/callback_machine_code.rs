...
use machine_code::CompilerPrivateMachineCodeFunction;
...
pub(super) fn emit_callback_thunks(
    ...
    for thunk in thunks {
        ...
        let terminal_psi_to_abstract_operations::NativeArtifactOperationPlan::Ordinary(
            abstract_plan,
        ) = lowered
        else {
            return Err(realization_error(
        ...
        let machine_code = machine_emission::emit_machine_code(&assigned)
...
pub(super) fn validate_callback_thunk_assignments(
    thunks: &[NativeCallbackThunkSettlement<'_>],
    assigned: &[assigned_target_operations::AssignedNativeCallbackArgument],
