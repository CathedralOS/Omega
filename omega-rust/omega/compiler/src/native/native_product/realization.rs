...
use crate::terminal::ProgramEntryTerminalArtifact;
...
pub(super) fn validate_terminal_authority_permissions(
    ...
    terminal_authority_permission_policy: Option<&crate::native::TerminalAuthorityPermissionPolicy>,
    ...
    match terminal_authority_permission_policy {
        Some(policy) => {
            crate::native::validate_package_terminal_authority_permissions(permissions, policy)
        }
        None => crate::native::validate_package_terminal_authority_permission_custody(permissions),
    }
}
...
pub(super) fn realize(
    ...
    terminal_authority_policy: crate::native::TerminalAuthorityPolicy,
    terminal_authority_permission_policy: Option<crate::native::TerminalAuthorityPermissionPolicy>,
    optimization_selections: &PostTerminalOptimizationSelections,
    prepared_terminal: ProgramEntryTerminalArtifact,
    prepared_input: &crate::native::PreparedNativeRealizationInput,
) -> Result<crate::native::NativeArtifact, Vec<Diagnostic>> {
    let (
        artifact,
    ...
    let minted_imports =
        crate::native::native_realization::source_evaluated_imports::mint_source_evaluated_imports(
            selected_plans,
            &demanded_intrinsics,
        )?;
    let import_settlements = minted_imports
        .iter()
        .map(|minted| crate::native::NativeProviderSettlement {
            provider_execution: &minted.execution,
            provider_plan: minted.plan,
            realization: crate::native::NativeBoundaryRealization::NormalizedForeignCall(
                &minted.same_stack,
            ),
        })
        .collect::<Vec<_>>();
    let compiler_builtins = intrinsic_proposals
        .iter()
        .map(|proposal| crate::native::NativeCompilerBuiltinSettlement {
    ...
    let program_entry = crate::native::NativeProgramEntrySettlement::new(
    ...
    let _validated_program_entry = crate::native::validate_native_program_entry_settlement(
        &artifact,
        &checked_program_entry,
        program_entry,
        admission.target,
    ...
    let request = crate::native::NativeRealizationRequest {
        checked_scope: Some(&checked_boundary_operator_scope),
        prepared_input: Some(prepared_input),
        target: admission.target,
        image_request: crate::native::ExecutableImageEmissionRequest::direct(checked.subsystem())
    ...
    crate::native::realize_native_artifact(artifact, request)
