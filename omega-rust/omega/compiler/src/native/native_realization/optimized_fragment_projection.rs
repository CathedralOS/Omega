...
pub(super) struct OptimizedFragmentPublicationRequest<'request> {
    ...
    pub(super) hosted_receiver:
        Option<&'request crate::native::ValidatedNativeProgramEntrySettlement>,
...
pub(super) fn emit_optimized_fragments(
    physical: crate::native::StagedOptimizedVerifiedPhysicalPipeline,
    request: OptimizedFragmentPublicationRequest<'_>,
) -> Result<
    (
        image_emission::ObjectArtifact,
        native_artifact::NativePhysicalEvidenceScope,
        Option<crate::native::StagedValidatedOptimizedProgramStorageSemanticWrapperObject>,
    ),
    Vec<Diagnostic>,
> {
    ...
    if let Some(entry) = request.hosted_receiver {
        ...
        if entry.source().target_slot().owner == target::TargetProfile::UefiX64 {
            ...
            semantic_wrapper_object = Some(
                crate::native::stage_validated_optimized_program_storage_semantic_wrapper_object(
                    entry.clone(),
                    source_artifact,
                    encoding,
                )
