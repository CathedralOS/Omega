use super::{Project, package_inputs, set_canonical_source_tree_permissions};
...
fn generated_source_replays_direct_dependency_authority_after_the_checkpoint() {
    ...
    let application = project.root.join("application");
    let middle = project.root.join("middle");
    let leaf = project.root.join("leaf");
    for package in [&application, &middle, &leaf] {
        std::fs::create_dir(package).expect("create generated-authority package root");
    }
    std::fs::write(
        application.join("main.omg"),
        "use middle::middle;\ndata Main { value: Middle; }\n",
    )
    .expect("write generated-authority application");
    std::fs::write(
        middle.join("middle.omg"),
        "use leaf::leaf;\npub data Middle { value: Leaf; }\n",
    )
    .expect("write generated-authority middle dependency");
    std::fs::write(leaf.join("leaf.omg"), "pub data Leaf { value: u8; }\n")
        .expect("write generated-authority leaf dependency");
    std::fs::write(
...
"#,
    ...
    let root_identity = PackageKeyIdentity::from_digest([97; 32]).expect("root identity");
    let middle_identity = PackageKeyIdentity::from_digest([98; 32]).expect("middle identity");
    let leaf_identity = PackageKeyIdentity::from_digest([99; 32]).expect("leaf identity");
    let package_inputs = PackageCompilationInputs::new_package(
        root_identity,
        vec![
            PackageSourceBinding::new(root_identity, "application", application.clone())
                .with_canonical_source_metadata()
                .expect("capture application source"),
            PackageSourceBinding::new(middle_identity, "middle", middle),
            PackageSourceBinding::new(leaf_identity, "leaf", leaf),
        ],
        vec![
    ...
    .expect("generated-authority package graph");

    let session = std::env::temp_dir().join(format!(
        "omega-build-facet-generated-authority-session-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&session);
    std::fs::create_dir(&session).expect("create generated-authority build session");
    let session = std::fs::canonicalize(session).expect("canonicalize generated-authority session");
    let sponsor = FilesystemSponsor::new(&session).expect("create generated-authority sponsor");
    let build_dir = session.join("output");
    let bound_build_dir = sponsor
        .bind_path(&build_dir)
        .expect("bind generated-authority output root");
    let prepared_build_dir = sponsor
        .prepare_create_directory(&bound_build_dir)
        .expect("prepare generated-authority output root");
    std::fs::create_dir(&build_dir).expect("create generated-authority output root");
    prepared_build_dir
        .commit()
    ...
    assert!(
        diagnostics.iter().any(|diagnostic| {
            diagnostic
                .message
                .contains("without declaring that package as a direct dependency")
                && diagnostic.message.contains("Leaf")
        }),
        "unexpected generated authority diagnostics: {diagnostics:#?}"
    );
    let _ = std::fs::remove_dir_all(session);
}
...
"#,
    ...
    // Custody is the authored pair, the mounted toolchain vocabulary, and the
    // one generated source — selected filtering may not drop the generated row.
    assert_eq!(
...
"#,
    ...
    let [
        typed_trees::data::DataMember::Variant(none),
        typed_trees::data::DataMember::Variant(some),
    ...
    let [typed_trees::data::DataMember::Field(borrowed_value)] =
    ...
    let assert_erased_application =
        |owner: &typed_trees::data::DataDefinition, expected_base, expected_lifetime| {
            let [typed_trees::data::DataMember::Field(field)] = checked.typed.data_members(owner)
            else {
    ...
    let [typed_trees::data::DataMember::Field(arithmetic_value)] =
    ...
    assert_eq!(*length, typed_trees::types::FixedArrayLength::Literal(2));
    ...
    let [typed_trees::data::DataMember::Field(arithmetic_use)] = checked
        .typed
    ...
    let [typed_trees::data::DataMember::Field(declared_value)] =
        checked.typed.data_members(declared_domain_instance)
    else {
        panic!("the declared-domain instance retains its substituted field")
    };
    let typed_trees::types::TypeReferenceNode::FixedArray { element_type, .. } = checked
        .typed
        .type_reference_table
    ...
    let [typed_trees::data::DataMember::Field(declared_use)] =
    ...
    let [typed_trees::data::DataMember::Field(values)] =
    ...
    let typed_trees::types::TypeReferenceNode::Generic { arguments, .. } = checked
        .typed
        .type_reference_table
    ...
    let assert_named_field = |owner: &typed_trees::data::DataDefinition, expected_symbol| {
        let [typed_trees::data::DataMember::Field(field)] = checked.typed.data_members(owner)
        else {
            panic!("{} retains its one field", owner.name.as_str())
        };
    ...
    let typed_trees::data::TypeParameterKind::Const { type_reference } = bool_parameter.kind else {
        ...
        typed_trees::types::TypeReferenceNode::Named { symbol, name }
            if !symbol.is_valid()
                && language_semantics::const_value::CanonicalConstValue::from_atom(name.as_str())
                    == Some(language_semantics::const_value::CanonicalConstValue::boolean(true))
    ));
    ...
    let typed_trees::types::TypeReferenceNode::Named { symbol, name } = checked
        .typed
        .type_reference_table
    ...
    assert!(matches!(
        ...
        Some(language_semantics::const_value::DecodedCanonicalConstValue::Record {
            type_name,
            fields,
...
"#,
    ...
    assert!(checked.typed.data_members(wrapper).iter().any(|member| {
        ...
        matches!(
            checked
                ...
                .type_reference_table
                .type_reference(field.type_reference),
            typed_trees::types::TypeReferenceNode::Generic { base_symbol, arguments, .. }
    ...
    let [typed_trees::data::DataMember::Field(only)] = checked.typed.data_members(also_generated)
    else {
