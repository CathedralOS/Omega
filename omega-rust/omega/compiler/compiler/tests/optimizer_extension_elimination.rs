//! Publication replay for `SelectedIncomingLiteralExtensionElimination`.
//!
...
//! so the extension-elimination family keeps its own witnessed coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-extension-elimination-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("create extension-elimination project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write extension-elimination main");
    std::fs::write(root.join("build.omg"), build).expect("write extension-elimination build");
    root
}

#[test]
fn return_only_extension_elimination_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
...
"#,
    ...
    .expect("the exact return-only extension-elimination selection should reach native custody");
    let artifact = report
