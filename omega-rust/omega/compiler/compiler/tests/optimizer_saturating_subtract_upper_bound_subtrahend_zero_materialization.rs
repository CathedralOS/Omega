...
//! so the saturating-subtract upper-bound subtrahend family keeps its own
//! witnessed coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-saturating-subtract-upper-bound-subtrahend-zero-materialization-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root)
        .expect("create saturating-subtract-upper-bound-subtrahend-zero-materialization project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write saturating-subtract-upper-bound-subtrahend-zero-materialization main");
    std::fs::write(root.join("build.omg"), build)
        .expect("write saturating-subtract-upper-bound-subtrahend-zero-materialization build");
    root
}

#[test]
fn return_only_saturating_subtract_upper_bound_subtrahend_zero_materialization_rejoins_native_artifact_production()
 {
    let root = project(
        "rejoin",
...
"#,
    ...
    .expect(
        "the exact return-only \
         saturating-subtract-upper-bound-subtrahend-zero-materialization selection should reach \
         native custody",
    );
    let artifact = report
