...
//! `SelectedIncomingSaturatingAddUpperBoundMaterialization`.
//!
...
//! so the saturating-add upper-bound family keeps its own witnessed
//! coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-saturating-add-upper-bound-materialization-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root)
        .expect("create saturating-add-upper-bound-materialization project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write saturating-add-upper-bound-materialization main");
    std::fs::write(root.join("build.omg"), build)
        .expect("write saturating-add-upper-bound-materialization build");
    root
}

#[test]
fn return_only_saturating_add_upper_bound_materialization_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
...
"#,
    ...
    .expect(
        "the exact return-only saturating-add-upper-bound-materialization selection should reach \
         native custody",
    );
    let artifact = report
