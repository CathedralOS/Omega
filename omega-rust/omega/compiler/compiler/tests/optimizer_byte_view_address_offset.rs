//! Publication replay for `SelectedIncomingU12ByteViewAddressOffset`.
//!
...
//! so the byte-view address-offset family keeps its own witnessed coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-byte-view-address-offset-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("create byte-view-address-offset project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write byte-view-address-offset main");
    std::fs::write(root.join("build.omg"), build).expect("write byte-view-address-offset build");
    root
}

#[test]
fn return_only_byte_view_address_offset_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
...
"#,
    ...
    .expect("the exact return-only byte-view-address-offset selection should reach native custody");
    let artifact = report
