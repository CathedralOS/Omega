//! Publication replay for `SelectedIncomingWrappingAddZeroIdentityCopy`.
//!
...
//! so the wrapping-add-identity family keeps its own witnessed coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-wrapping-add-zero-identity-copy-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("create wrapping-add-zero-identity-copy project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write wrapping-add-zero-identity-copy main");
    std::fs::write(root.join("build.omg"), build)
        .expect("write wrapping-add-zero-identity-copy build");
    root
}

#[test]
fn return_only_wrapping_add_zero_identity_copy_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
        r#"machine build(builder: &mut Build) {
    builder.application("optimizer_wrapping_add_zero_identity_copy_rejoin");
...
"#,
    ...
    .expect(
        "the exact return-only wrapping-add-zero-identity-copy selection should reach native \
         custody",
    );
    let artifact = report
