//! Publication replay for `SelectedIncomingBitwiseAndZeroMaterialization`.
//!
...
//! so the bitwise-and-annihilator family keeps its own witnessed coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-bitwise-and-zero-materialization-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("create bitwise-and-zero-materialization project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write bitwise-and-zero-materialization main");
    std::fs::write(root.join("build.omg"), build)
        .expect("write bitwise-and-zero-materialization build");
    root
}

#[test]
fn return_only_bitwise_and_zero_materialization_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
...
"#,
    ...
    .expect(
        "the exact return-only bitwise-and-zero-materialization selection should reach native \
         custody",
    );
    let artifact = report
