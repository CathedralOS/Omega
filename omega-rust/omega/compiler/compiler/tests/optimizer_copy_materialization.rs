//! Publication replay for `SelectedIncomingLiteralCopyMaterialization`.
//!
...
//! so the copy-materialization family keeps its own witnessed coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-copy-materialization-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("create copy-materialization project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write copy-materialization main");
    std::fs::write(root.join("build.omg"), build).expect("write copy-materialization build");
    root
}

#[test]
fn return_only_copy_materialization_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
        r#"machine build(builder: &mut Build) {
    builder.application("optimizer_copy_materialization_rejoin");
...
"#,
    ...
    .expect("the exact return-only copy-materialization selection should reach native custody");
    let artifact = report
