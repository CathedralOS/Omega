//! Publication replay for `SelectedIncomingWrappingRemainderMinusOneZeroMaterialization`.
//!
...
//! so the wrapping-remainder minus-one family keeps its own witnessed
//! coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-wrapping-remainder-minus-one-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("create wrapping-remainder-minus-one project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write wrapping-remainder-minus-one main");
    std::fs::write(root.join("build.omg"), build)
        .expect("write wrapping-remainder-minus-one build");
    root
}

#[test]
fn return_only_wrapping_remainder_minus_one_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
...
"#,
    ...
    .expect(
        "the exact return-only wrapping-remainder-minus-one selection should reach native custody",
    );
    let artifact = report
