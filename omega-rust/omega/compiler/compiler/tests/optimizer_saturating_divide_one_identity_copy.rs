//! Publication replay for `SelectedIncomingSaturatingDivideOneIdentityCopy`.
//!
...
//! so the saturating-divide-identity family keeps its own witnessed coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-saturating-divide-one-identity-copy-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("create saturating-divide-one-identity-copy project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write saturating-divide-one-identity-copy main");
    std::fs::write(root.join("build.omg"), build)
        .expect("write saturating-divide-one-identity-copy build");
    root
}

#[test]
fn return_only_saturating_divide_one_identity_copy_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
...
"#,
    ...
    .expect(
        "the exact return-only saturating-divide-one-identity-copy selection should reach \
         native custody",
    );
    let artifact = report
