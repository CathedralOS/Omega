//! Source-evaluated Linux x86-64 hosted receiver: a retained `&mut self`
//! entry bound to `linux_x86_64::ProgramEntry` must provision its receiver
...
/// `LinuxX86_64Application` entry schema, and accepts the exact Console
/// provider plan (with output — and exit when authored — authority).
fn linux_package_inputs(fixture: &Fixture) -> PackageCompilationInputs {
...
fn linux_hosted_receiver_fixture(name: &str, explicit_exit: bool) -> Fixture {
...
"#,
    ...
    let fixture = Fixture::with_source(name, "linux_x86_64", &source, &build);
    let inputs = linux_package_inputs(&fixture);
...
fn realize_linux_direct(
    ...
    .unwrap_or_else(|(_, diagnostics)| {
        panic!("Linux hosted receiver should realize: {diagnostics:#?}")
    })
}

fn run_linux_hosted_receiver(explicit_exit: bool) {
    let fixture = linux_hosted_receiver_fixture(
        if explicit_exit {
            "linux-hosted-explicit-exit"
        } else {
            "linux-hosted-normal-return"
        },
        explicit_exit,
    );
    let candidate = realize_linux_direct(fixture.compile_terminal());
    let artifact = candidate
        .as_direct()
        .expect("an import-free Linux object must select direct ELF custody");
    artifact
        .validate()
    ...
    #[cfg(all(target_os = "linux", target_arch = "x86_64"))]
    {
        ...
        // at e_entry through rsp alone, so process behavior proves the bridge.
    ...
    eprintln!("SKIP: hosted receiver runtime requires Linux x86-64; emission checked");
}

#[test]
fn linux_hosted_receiver_normal_return_provisions_zii_storage_and_console() {
    run_linux_hosted_receiver(false);
}

#[test]
fn linux_hosted_receiver_explicit_exit_process_preserves_its_distinct_outcome() {
    run_linux_hosted_receiver(true);
}
