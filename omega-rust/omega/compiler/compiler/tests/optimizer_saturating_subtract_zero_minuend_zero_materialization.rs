...
//! `SelectedIncomingSaturatingSubtractZeroMinuendZeroMaterialization`.
//!
...
//! so the saturating-subtract zero-minuend family keeps its own witnessed
//! coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-saturating-subtract-zero-minuend-zero-materialization-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root)
        .expect("create saturating-subtract-zero-minuend-zero-materialization project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write saturating-subtract-zero-minuend-zero-materialization main");
    std::fs::write(root.join("build.omg"), build)
        .expect("write saturating-subtract-zero-minuend-zero-materialization build");
    root
}

#[test]
fn return_only_saturating_subtract_zero_minuend_zero_materialization_rejoins_native_artifact_production()
 {
    let root = project(
        "rejoin",
        r#"machine build(builder: &mut Build) {
    builder.application("optimizer-saturating-subtract-zero-minuend-zero-materialization-rejoin");
    builder.roots.bind(windows_x86_64::ProgramEntry, Main::main);
    builder.optimizations.enable(Optimization::SelectedIncomingSaturatingSubtractZeroMinuendZeroMaterialization);
}
"#,
    );
    ...
    .expect(
        "the exact return-only saturating-subtract-zero-minuend-zero-materialization selection \
