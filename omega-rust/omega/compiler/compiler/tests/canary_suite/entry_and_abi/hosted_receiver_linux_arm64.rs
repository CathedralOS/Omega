//! Linux ARM64 process entry must provision the receiver; the test supplies
//! no pointer, storage grant, interpreter arguments, or replacement native
//! entry stub. The kernel arrives at the emitted ELF entry point with sp at
//! the initial process-stack image and no return continuation; the exact
//! hosted bridge preserves that arrival state in its own partition, switches
//! to a private stack, passes the zero-initialized receiver in x0 under
//! AAPCS64, reaches the semantic continuation, and completes through
//! exit_group.
...
impl Drop for HostedProject {
    fn drop(&mut self) {
        if std::thread::panicking() {
            eprintln!(
                "retained failed Linux ARM64 hosted receiver observations: {}",
                self.0.display()
            );
            return;
        }
...
"#
    ...
    let report = result.unwrap_or_else(|diagnostics| {
        panic!("authored Linux ARM64 hosted receiver must produce its executable: {diagnostics:#?}")
    });
    let receiver = report
    ...
    #[cfg(all(target_os = "linux", target_arch = "aarch64"))]
    {
        ...
        // arrives through sp only, so a working bridge is the only way the
    ...
    eprintln!("SKIP: hosted receiver runtime requires Linux ARM64; source cross-emission checked");
}
...
#[test]
fn linux_arm64_hosted_receiver_normal_return_provisions_zii_storage_and_fused_console() {
    compile_and_run_linux_arm64_hosted_receiver(false, true);
}

#[test]
fn linux_arm64_hosted_receiver_explicit_process_exit_preserves_its_distinct_outcome() {
    compile_and_run_linux_arm64_hosted_receiver(true, true);
}
