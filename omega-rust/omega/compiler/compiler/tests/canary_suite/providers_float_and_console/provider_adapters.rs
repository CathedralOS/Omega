...
#[test]
fn checked_boundary_operator_const_application_exit_canary_runs() {
    let canary = pass_canary(fixture_roster::CHECKED_BOUNDARY_OPERATOR_CONST_APPLICATION_EXIT);
    let main_path = canary.join("main.omg");
    let checked = compile_reviewed_repository_fixture(CheckedCompileRequest::new(&main_path, None))
        .expect(
            "checked boundary-operator const-application canary should compile to checked trees",
        );
    assert!(!checked.facts.operators.boundary_applications.is_empty());
    let outcome = interpret(&checked, &[]);
    assert_eq!(
        outcome.exit_code, 70,
        "the selected provider body folded the const argument to 7, so byte 6 writes; \
         builtin `%` would fold 1 and reject the index: {:?}",
        outcome.error,
    );
}
