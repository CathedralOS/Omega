...
#[test]
fn wire_compatibility_demand_is_satisfied_through_the_bound_migration_route() {
    // The build demands Readable, Writable, Canonical and CompleteMigration on
    // the RollingChannel edge between two nominally distinct schemas of the
    // same wire shape. Checked compilation validates every required
    // directional fact and rejects an unsatisfied demand, so acceptance is the
    // verdict; the checked program retains the two identity-keyed schemas and
    // the exact FormatMigration conformance the route was selected through.
    ...
    for schema in ["LocalMessage", "PeerMessage"] {
        let data = program
            .data_definitions()
            .iter()
            .find(|data| data.name.as_str() == schema)
            .unwrap_or_else(|| panic!("schema `{schema}` should reach checked semantics"));
        let fields = program
            .data_members(data)
            .iter()
            .map(|member| match member {
                ...
                typed_trees::data::DataMember::Variant(variant) => {
                    panic!(
                        "schema `{schema}` should be a record, found case `{}`",
                        variant.name
                    )
                }
            })
        ...
        assert_eq!(
            fields,
            [(Some(1), "value", "u32".to_owned())],
            "schema `{schema}` should keep its single identity-keyed field"
        );
    }
    ...
    let route = program
        .machines()
        .iter()
        .find(|machine| machine.name.as_str() == "peer_to_local")
        .expect("the migration route machine should reach checked semantics");
    let conformances = program
        .machine_trait_conformances(route)
        .iter()
        .map(|conformance| {
            let arguments = program
                ...
                .map(|argument| {
                    program
                        .symbols
                        .display_path(program.type_reference_table.type_symbol(*argument), "::")
                })
                .collect::<Vec<_>>();
            (
                conformance.name.as_str().to_owned(),
                conformance
                    .requirement
                    .as_ref()
    ...
    assert_eq!(
        conformances,
        [(
            "FormatMigration".to_owned(),
            Some("migrate".to_owned()),
            vec![
                "MessageLineage".to_owned(),
                "PeerMessage".to_owned(),
                "LocalMessage".to_owned(),
            ],
        )],
        "the selected route is the exact lineage-bound FormatMigration conformance"
    );
}
...
fn capability_flows_retain_exact_direct_and_propagated_sites() {
    ...
    // and returns it flows the same verb up to its caller, and the checked
    // flow fact records the helper state as provenance.
    for &(canary_name, propagated_routes) in fixture_roster::CAPABILITY_FLOW_PASS_CANARIES {
        ...
        let program = checked.terminal_production_trees();
        let path = |symbol| program.symbols.display_path(symbol, "::");
        // The roster names machines; a flow fact records its state symbols, so
        // resolve each state back to the machine whose body declares it.
        let owning_machine = |state_symbol: symbols::SymbolHandle| {
            program
                .machines()
                .iter()
                .find(|machine| {
                ...
                .map(|machine| path(machine.symbol))
                .unwrap_or_default()
        };
        let rendered = program
            ...
            .map(|flow| {
                format!(
                    "`{}` state `{}` {} at statement {} call {} via `{}`",
                    path(flow.machine_symbol),
                    path(flow.state_symbol),
                    flow.kind.as_str(),
                    flow.statement_index,
                    flow.call_ordinal,
                    owning_machine(flow.via_state_symbol)
                )
            })
            .collect::<Vec<_>>()
            .join("\n");
        ...
        assert!(
            program
                ...
                .capabilities
                .flows()
                .any(|flow| !flow.is_propagated()),
            "checked facts for {canary_name} should retain a direct boundary-call flow site\n{rendered}"
        );
        for (state, authority_flow, via_state) in propagated_routes {
            let kind = CapabilityFlowKind::ALL
                .into_iter()
                .find(|kind| kind.as_str() == *authority_flow)
                .unwrap_or_else(|| panic!("unknown authority flow verb `{authority_flow}`"));
            assert!(
                program.facts.capabilities.flows().any(|flow| {
                    flow.kind == kind
                        && flow.is_propagated()
                        && path(flow.machine_symbol) == *state
                        && owning_machine(flow.via_state_symbol) == *via_state
                }),
                "checked facts for {canary_name} should retain the propagated site \
                 `{state}` {authority_flow} via `{via_state}`\n{rendered}"
            );
        }
    }
}
