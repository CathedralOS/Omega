//! Publication replay for `SelectedIncomingExactDivideIdentityCopy`.
//!
...
//! so the exact-divide-identity family keeps its own witnessed coverage.
...
fn project(label: &str, build: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "omega-optimizer-exact-divide-identity-{label}-{}-{}",
        std::process::id(),
        PROJECT_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("create exact-divide-identity project");
    std::fs::write(
        root.join("main.omg"),
        "data Main { value: u8; }\nmachine Main::main(&mut self) { }\n",
    )
    .expect("write exact-divide-identity main");
    std::fs::write(root.join("build.omg"), build).expect("write exact-divide-identity build");
    root
}

#[test]
fn return_only_exact_divide_identity_rejoins_native_artifact_production() {
    let root = project(
        "rejoin",
        r#"machine build(builder: &mut Build) {
    builder.application("optimizer_exact_divide_identity_rejoin");
...
"#,
    ...
    .expect("the exact return-only exact-divide-identity selection should reach native custody");
    let artifact = report
