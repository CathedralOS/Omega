...
"#,
    ...
    let (expression, field) = fixture
        ...
        .find_map(|(handle, node)| {
            let symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Member(member) = node else {
                return None;
            };
            let result = typed_trees_to_checked_trees::validation::reserved_result_place(&fixture.checked.typed, handle)?;
            let [typed_trees_to_checked_trees::fact_plan::PlaceSegment::Field { symbol }] = result.segments.as_slice() else {
                return None;
            };
    ...
    let coordinates = missing
        ...
        .filter_map(|(handle, segment)| {
            (*segment
                == (typed_trees_to_checked_trees::fact_plan::PlaceSegment::Field { symbol: field }))
            .then_some(handle)
        })
        .collect::<Vec<_>>();
    assert!(
    ...
    for handle in coordinates {
        *missing.facts.semantic.place_segments.get_mut(handle) =
            typed_trees_to_checked_trees::fact_plan::PlaceSegment::default();
    }
...
fn authored_payload_case_crash_contract_preserves_case_meaning_in_package_policy() {
    ...
    omega::package_evidence::project_checked_package_review(&fixture.checked)
            ...
            fixture
                ...
                .any(|occurrence| {
                    fixture
                        ...
                        .is_some_and(|selection| {
                            selection.kind()
                                == symbol_resolved_trees_to_typed_trees::typed_trees::AuthoredDeclarationSelectionKind::CaseMembership
                        })
                })
