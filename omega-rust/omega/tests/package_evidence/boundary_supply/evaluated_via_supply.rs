...
fn assert_external_policy_round_trip(
    ...
    supply: &omega::package_evidence::record::PackageReviewExternalExecutableSupply,
) {
    let machine = checked
        .machines()
        .iter()
    ...
    let policies =
        omega::package_evidence::project_checked_external_supply_policy(checked, machine.symbol)
            .unwrap();
    let [policy] = policies.as_slice() else {
        panic!("one checked external policy")
    };
    assert_eq!(policy.callable().owner(), supply.callable().owner());
    let surfaces = omega::package_evidence::project_checked_callable_policy(
        checked,
    ...
    let recovered =
        omega::package_evidence::record::PackagePolicyExternalExecutableSupply::recover_canonical(
            &bytes,
            omega::package_evidence::encoding::PackagePolicyRecoveryLimits::default(),
        )
