...
use omega::compiler::compile_to_checked;
use omega::package_compilation::{PackageCompilationInputs, PackageSourceBinding};
use omega::package_evidence::encoding::{
...
use omega::package_evidence::project_checked_package_review;
...
fn fixture_rows() -> Option<(
    ...
    Vec<omega::package_evidence::record::PackageReviewCanonicalRow>,
)> {
...
fn decoder_rejects_malformed_noncanonical_and_over_limit_recovery_rows() {
    ...
    assert!(
        omega::package_evidence::encoding::encode_package_review_canonical_row_with_limits(
            authored,
            tiny_envelope
        )
        .is_err()
