...
"#,
    ...
    let places = changed
        ...
        .filter_map(|(handle, place)| {
            (place.root == typed_trees_to_checked_trees::fact_plan::PlaceRoot::Symbol(parameter))
                .then_some(handle)
        })
        .collect::<Vec<_>>();
    ...
    for handle in places {
        changed.facts.semantic.places.get_mut(handle).root =
            typed_trees_to_checked_trees::fact_plan::PlaceRoot::Symbol(machine_symbol);
    }
...
fn machine_precondition_members_require_exact_entry_point_and_declaration_origin() {
    use typed_trees_to_checked_trees::fact_plan::{FactOrigin, FactPayload, ProgramPoint};
