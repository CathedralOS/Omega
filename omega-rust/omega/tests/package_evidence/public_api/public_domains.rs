...
use omega::package_evidence::record::PackagePolicyCallableRole;
...
fn public_exact_machine_issuer_routes_capture_checked_wrappers() {
    for (declaration, issuer, route) in [
        ...
        let policy = omega::package_evidence::project_checked_package_policy(
            &checked,
            review.target(),
            package_identity(),
        )
        ...
        let recovered = omega::package_evidence::record::PackagePolicyBaseline::recover_canonical(
            &bytes,
        ...
        assert_eq!(
            omega::package_evidence::record::PackagePolicyBaseline::recover_text(
                &text,
