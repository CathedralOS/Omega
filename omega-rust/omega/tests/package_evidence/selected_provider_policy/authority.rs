...
fn executed_helper_selection_replays_without_activating_dead_choices() {
    ...
    for statements in statements {
        for statement in changed.typed.statement_table.statements_mut(statements) {
            ...
            if let symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Expression(expression) = statement {
                expressions.push(*expression);
            }
        }
    }
    for expression in expressions {
        if let symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Call(
            call,
...
"#;
    ...
    assert_eq!(
        premise.subject(),
        abstract_operations_to_target_operations::effects::provider_plan::ServiceProgressSubject::Parameter(0)
    );
