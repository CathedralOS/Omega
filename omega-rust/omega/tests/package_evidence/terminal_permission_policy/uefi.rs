...
"#,
    ...
    let definition = candidate
        ...
        .unwrap();
    let schema =
        omega::provider_planning::service_schema::from_typed(&candidate.typed, definition).unwrap();
    let physical = schema
        .methods
        .iter()
    ...
    let definition = checked
        ...
        .unwrap();
    let schema =
        omega::provider_planning::service_schema::from_typed(&checked.typed, definition).unwrap();
    assert!(
        schema
            .methods
            .iter()
    ...
    assert_eq!(
        omega::package_compilation::accepted_service_schema_digest(role, &schema),
        binding.normalized_schema_digest()
    );
