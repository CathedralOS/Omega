...
"#,
    ...
    assert_eq!(
        family.authority(),
        omega::package_evidence::record::PackageReviewProviderSelectionAuthority::BuildOverride
    );
    assert_eq!(
        family.coverage(),
        omega::package_evidence::record::PackageReviewProviderFamilyCoverage::CompleteDeclarationFamily
    );
...
"#,
    ...
    *binder_symbol = symbols::SymbolHandle::invalid();
    ...
    specialization.commitment =
        symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::MachineSpecializationCommitment::from_digest([0xa5; 32]);
    assert!(
