...
fn float_contract_review_rejects_missing_checked_width_landing() {
    ...
    let float_expression = checked
        ...
        .find_map(|(expression, node)| {
            matches!(node, symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Float(_)).then_some(expression)
        })
        .expect("landed float expression");
    *checked
        .typed
        .expression_table
        .expression_mut(float_expression) =
        symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Float(
            symbol_resolved_trees_to_typed_trees::typed_trees::expression::FloatLiteral::parse(
                "1.25",
            )
    ...
    let call_expression = checked
        ...
        .find_map(|(expression, node)| match node {
            symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Call(
                call,
            ) if call.target.as_str() == "min" => Some(expression),
            _ => None,
        })
