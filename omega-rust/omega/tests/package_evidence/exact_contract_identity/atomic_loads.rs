...
fn atomic_load_review_rejects_non_load_orderings_and_result_tamper() {
    ...
    let atomic_expression = checked
        ...
        .find_map(|(expression, node)| {
            matches!(node, symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Atomic(_)).then_some(expression)
        })
    ...
    for ordering in [
        ...
        let symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Atomic(
            atomic,
        ) = checked
            .typed
            .expression_table
