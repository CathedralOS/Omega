...
fn reference_review_rejects_access_tamper_after_checking() {
    ...
    let symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionBody::Transparent {
        proposition: symbol_resolved_trees_to_typed_trees::typed_trees::proposition::PropositionFormula::Application(application),
    } = &reviewed.body
    else {
        panic!("transparent proposition application")
    };
...
fn contract_call_reference_review_rejects_access_tamper_after_checking() {
    ...
    let borrow = checked
        ...
        .find_map(|(expression, node)| {
            matches!(node, symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Borrow(_)).then_some(expression)
        })
