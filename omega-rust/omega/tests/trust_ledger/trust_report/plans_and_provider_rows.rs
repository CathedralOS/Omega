use super::compile;
use omega::compiler::{
    CheckedCompilation, CheckedCompileRequest, CompileOptions, compile_to_checked,
};
...
fn checked_trust_report(
    project: &std::path::Path,
    expectation: &str,
) -> omega::artifacts::TrustReport {
...
fn commitments(report: &omega::artifacts::TrustReport) -> Vec<&str> {
    report
        .rows
        .iter()
...
fn requirement_row<'report>(
    report: &'report omega::artifacts::TrustReport,
    provider_plan: &str,
    method: &str,
) -> &'report omega::artifacts::TrustProviderRequirementRow {
    report
...
"#,
    ...
    assert_eq!(
        ...
        omega::artifacts::TrustProviderRealization::Syscall { number: 101 }
    );
...
"#,
    ...
    let schema = omega::provider_planning::service_schema::from_typed(&checked.typed, tick)
...
"#,
    ...
    assert_eq!(
        ...
        vec![omega::artifacts::TrustProgressPremiseRow {
            profile: "SchedulerHandle::WeakFair".to_owned(),
            subject: omega::artifacts::TrustProgressPremiseSubject::Parameter(0),
...
"#,
    ...
    let adapter = |provider: &str| omega::artifacts::TrustProviderRealization::CheckedAdapter {
