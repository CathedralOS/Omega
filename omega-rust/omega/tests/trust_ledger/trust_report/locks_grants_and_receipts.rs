...
"#,
    ...
    let check = || {
        let admissions = omega::trust_ledger::read_trust_admissions(&root_path).unwrap();
        omega::compiler::compile(
            omega::compiler::CompileRequest::new(CompileOptions {
    ...
    omega::trust_ledger::accept_trust_admissions(
        &root_path,
...
"#,
    ...
    assert_eq!(
        ...
        Some(vec![omega::artifacts::TrustCrashRouteBucket {
            cause: omega::artifacts::TrustCrashCause::Abort,
            alternative_guards: vec![omega::artifacts::TrustCrashRouteGuard::Truth],
        }])
    );
