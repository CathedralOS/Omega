...
//! `omega::checked_interpreter::verify_wire_schema_codec` over schemas covering
...
fn verify(
    schema_source: &str,
    schema_name: &str,
) -> omega::checked_interpreter::WireCodecVerification {
    let typed = crate::front_end::typed_program(&format!(
        "{schema_source}
         machine main() -> i32 {{
    ...
    omega::checked_interpreter::verify_wire_schema_codec(&typed, schema)
