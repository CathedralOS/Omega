...
use omega::build_time_evaluation::{
...
use omega::build_time_evaluation::{BuildMachineInvocation, PreparedBuildMachine};
...
fn admission_plan_owns_result_machine_lookup_gate_and_evaluation() {
    ...
    let admission = omega::build_time_evaluation::BuildTimeAdmissionPlan::infer(&typed, None);
...
fn exported_wrapper_reach_controls_evaluation_admission() {
    for declaration in ["", "reaches Host"] {
        ...
        let admission = omega::build_time_evaluation::BuildTimeAdmissionPlan::infer(&checked, None);
...
fn nominal_callback_selection_controls_const_array_length_admission() {
    let source = "../../../../tests/omega/pass/effects/nominal_callback_const_reach/main.omg"
    );
    for reaches_console in [false, true] {
        ...
        if reaches_console {
            ...
            assert!(program.type_reference_table.fixed_array_lengths().any(
                |(_, length)| matches!(length, symbol_resolved_trees_to_typed_trees::typed_trees::types::FixedArrayLength::Literal(7))
            ));
        }
        assert!(
...
fn nominal_and_structural_callbacks_keep_distinct_specialized_reach() {
    for (contract, admitted) in [
        ...
        let admission =
            omega::build_time_evaluation::BuildTimeAdmissionPlan::infer(prepared.typed(), None);
...
fn invocation_const_boundary_admits_an_exact_boolean_snapshot() {
    ...
    let value = admission
        .evaluate_const_evaluable_machine_for_invocation(
            ...
            omega::build_time_evaluation::BuildTimeInvocationCustody::Source(Default::default()),
        )
...
fn integer_const_positions_decode_the_callees_declared_carrier() {
    ...
    let admission = omega::build_time_evaluation::BuildTimeAdmissionPlan::infer(&program, None);
...
fn sponsored_pure_builds_share_and_exactly_exhaust_one_fuel_account() {
    ...
    assert_eq!(baseline.usage().schema().schema_version(), 7);
    assert!(baseline.usage().peak_live_cells() > 0);
    ...
    assert_eq!(sponsor.limits().schema_version(), 7);
    ...
    assert_eq!(
        sponsor.peak_live_cells(),
        first
            .usage()
            .peak_live_cells()
            .max(second.usage().peak_live_cells())
    );
...
fn sponsored_build_rejects_cell_allocation_before_exceeding_the_live_ceiling() {
    ...
    assert_eq!(
        error.to_string(),
        "build-evaluation live-cell sponsor exhausted at 1 cells"
    );
    assert_eq!(sponsor.peak_live_cells(), 1);
    assert_eq!(sponsor.live_cells(), 0);
}
...
fn sponsored_build_rejects_text_materialization_above_the_live_byte_ceiling() {
    ...
    assert_eq!(
        error.to_string(),
        "build-evaluation live-Text-byte sponsor exhausted at 1 bytes"
    );
    assert_eq!(sponsor.peak_live_text_bytes(), 0);
...
fn sponsored_granted_build_uses_the_compiler_ceiling_and_classifies_exhaustion() {
    ...
    let sponsor = BuildEvaluationSponsor::new(
        BuildEvaluationSponsorLimits::new(100_000, 1024, 1024, 64, 1024, 1024, 1024, 1024)
            .expect("nonzero limits"),
    );
    ...
    assert_eq!(
        failure.kind(),
        omega::checked_interpreter::BuildMachineEvaluationFailureKind::ResourceExhausted
    );
    assert_eq!(
        failure.diagnostic(),
...
fn admission_rejects_a_transitive_progress_premise_before_interpretation() {
    ...
    let error = omega::build_time_evaluation::BuildTimeAdmissionPlan::infer(&typed, None)
...
fn case_membership_accepts_temporary_values() {
    for (subject, case, expected) in [
        ...
        let value = omega::build_time_evaluation::BuildTimeAdmissionPlan::infer(&program, None)
...
fn case_membership_requires_a_value_subject_not_a_payload_domain() {
    ...
    let error = omega::build_time_evaluation::BuildTimeAdmissionPlan::infer(&program, None)
        .evaluate_const_evaluable_machine(&program, "member", vec![])
