...
use omega::package_evidence::ledger::OrdinaryPackageObligationStatus;
...
use omega::package_manager::resolution::graph::PackageSourceClosureLimits;
...
use omega::package_source::{LocalSourceLimits, SourceLineage, SourceRelativePath};
...
"#,
    ...
    assert_eq!(
        claim.row().kind(),
        omega::package_evidence::record::PackageReviewCanonicalRowKind::AcceptedClaim
    );
    assert_eq!(
...
"#,
    ...
    assert!(
        changed
            ...
            .any(|package| package.rows().iter().any(|row| {
                row.kind() == omega::package_evidence::record::PackagePolicyRowKind::Callable
                    && row.change()
                        == omega::package_manager::review::PackagePolicyChangeKind::Changed
    ...
    let rendered =
        omega::package_manager::review::render_package_policy_review(&changed, 16 * 1024 * 1024)
