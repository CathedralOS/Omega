...
fn prepare(
    ...
    omega::build_evaluation::BuildSnapshotRequest,
) {
...
"#,
    ...
    let snapshot =
        omega::build_evaluation::BuildSnapshotRequest::new(std::iter::empty::<Vec<u8>>())
...
fn named_dependency_inputs_reach_generated_source_and_completed_publication() {
    ...
    assert_eq!(
        report.output_kind(),
        omega::compiler::CompileOutputKind::BuildArtifacts
    );
    let report = report
