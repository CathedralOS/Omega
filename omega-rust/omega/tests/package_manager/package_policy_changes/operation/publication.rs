...
use omega::package_evidence::encoding::PackagePolicyTextRecoveryLimits;
use omega::package_evidence::record::PackagePolicyBaseline;
...
use omega::package_manager::operations::LocalProjectPreparationOptions;
...
use omega::package_manager::resolution::graph::resolve_staged_external_local_project_closure;
use omega::package_source::PrimaryGitChoices;
use omega::package_source::SourceRelativePath;
use omega::package_source::local::staging::{
...
fn changed_build_input_rejects_publication_with_identical_generated_source_and_policy() {
    ...
    let dependency = tree.path("sources/dependency");
    let fixture_root =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tests/fixtures/packages/generated-table");
    ...
    assert!(
        matches!(
            ...
            PublishReviewedPackageChangeError::Source(
                omega::package_source::SourceResolveError::LocalSourceChanged { path }
...
fn preparation_recovers_pending_declarations_before_compiler_input_resolution() {
    ...
    .unwrap();
    let closure =
        omega::package_manager::resolution::graph::resolve_external_local_project_closure(
            &root,
    ...
    omega::compiler::compile_to_checked(CheckedCompileRequest {
