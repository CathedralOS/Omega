    ...
    assert!(matches!(
        ...
        Err(
            CompileResolvedPackageReviewsError::InvalidProductionRootRole {
                role: omega::package_compilation::BuildDeclarationKind::Package,
                ..
            }
        )
    ));
        ...
        assert_eq!(
            policy
                ...
                .filter(|callable| callable.role()
                    == omega::package_evidence::record::PackagePolicyCallableRole::Public)
                .count(),
            review
                .projection()
                .callables()
                .iter()
                .filter(|callable| callable.role()
                    == omega::package_evidence::record::PackageReviewCallableRole::Public)
                .count(),
...
use omega::package_manager::admission::accept_ordinary_closure_evidence;
