...
use omega::package_manager::declarations::{DependencyPurpose, PackageKey};
use omega::package_manager::lock::{PackageLock, PackageLockError, PackageLockRecoveryLimits};
...
use omega::package_source::git::resolution::GitExactRevisionAcquisition;
...
fn one_package_can_serve_product_and_build_purposes() {
    ...
    let reviews = compile_resolved_package_reviews(
        ...
        omega::package_manager::review::SemanticBindingReview::Explicit(&[]),
    )
    .unwrap();
    assert!(
