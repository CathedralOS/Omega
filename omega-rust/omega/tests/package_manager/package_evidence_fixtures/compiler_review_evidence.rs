...
fn local_fixtures_issue_compiler_review_evidence_from_resolver_custody() {
    ...
    for package in REVIEWABLE_PACKAGES {
        ...
        match expected_initial_kind {
            Some(kind) => {
                ...
                assert!(conflicts.conflicts().iter().any(|conflict| {
                    conflict.kind() == kind
                        && conflict.change()
                            == omega::package_manager::review::ReviewOnlyCapabilityConflictChange::Added
        ...
        let initial_review = assemble_initial_source_review(
            &reviews,
            &closure,
            omega::package_manager::review::PackageSourceReviewLimits::default(),
        )
...
fn process_exit_fixture_retains_exact_closed_console_leaves_and_unresolved_siblings() {
    ...
    assert_eq!(
        ...
        Some(
            omega::package_evidence::record::PackageReviewCompilerIntrinsicExecution::HostedExitProcessI32,
        ),
        "process-exit retains the exact closed Linux execution identity",
    );
    assert_eq!(
        execution_for("read_byte"),
        Some(omega::package_evidence::record::PackageReviewCompilerIntrinsicExecution::HostedReadByte,),
