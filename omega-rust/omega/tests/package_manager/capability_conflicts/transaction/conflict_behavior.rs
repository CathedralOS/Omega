...
use omega::package_manager::review::SemanticBindingReview;

pub(super) fn assert_comparison_limits_and_risk_classes(
    scenario: &ExactCompilerRowScenario,
    conflicts: &omega::package_manager::review::ReviewOnlyCapabilityConflictSet,
) {
...
"#,
    ...
    assert!(slack_locations.iter().any(|location| {
        location.role()
            == omega::package_evidence::record::PackageReviewSourceLocationRole::AuthorityDeclaration
    }));
    assert!(slack_locations.iter().any(|location| {
        location.role()
            == omega::package_evidence::record::PackageReviewSourceLocationRole::AuthorityExposure
    ...
    assert!(slack_triage.decisions().iter().any(|decision| {
        decision
            ...
            .contains(&PackageTriageReason::DangerousAuthoritySlack(
                omega::package_evidence::record::PackageReviewDangerousAuthorityClass::Filesystem,
            ))
    }));
}
