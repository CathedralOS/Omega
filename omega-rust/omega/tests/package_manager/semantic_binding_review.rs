...
fn assert_accepted_native_report_custody(
    candidate: omega::package_manager::review::ReviewedPackageProductionCandidate,
    evidence: &omega::package_manager::admission::AcceptedOrdinaryClosureEvidence,
    checked: &omega::compiler::CheckedCompilation,
    accepted_permission_policy: omega::compiler::native::TerminalAuthorityPermissionPolicy,
    compile_terminal_report: impl FnOnce() -> omega::compiler::CompileReport,
) {
    let receiving_policy_identity = accepted_permission_policy.identity();
    let rollback = omega::compiler::OptimizationRollback::new([
    ...
    assert_eq!(
        ...
        omega::compiler::CompileOutputKind::RetainedNativeArtifact
    );
    ...
    let unused_admission = omega::compiler::TrustAdmission::for_provider_plan(
        "unused admission retained by report conversion".to_owned(),
        0x0074_7275_7374,
        abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanDigest::from_digest([0x74; 32]),
    )
    .expect("distinct unused admission");
    let expected_trust = omega::compiler::admit_checked_compilation(checked, &[unused_admission])
...
fn console_binding_fixture() -> (
    TemporaryTree,
    PathBuf,
    omega::package_manager::resolution::graph::ResolvedPackageSourceClosure,
) {
...
fn consumer_scoped_console_binding_survives_review_and_fresh_admission() {
    ...
    assert!(matches!(
        ...
        Err(FreshPackageRootPolicyError::ReviewRequired(changes))
            if changes.packages().iter().any(|package| package.rows().iter().any(|row| {
                row.kind() == omega::package_evidence::record::PackagePolicyRowKind::TerminalPermission
    ...
    let denied = realize_accepted_native_report(
        ...
        AcceptedNativeRealizationRequest {
            ...
            terminal_authority_policy: omega::compiler::native::current_terminal_authority_policy(),
            receiving_terminal_authority_permission_policy: Some(
                omega::compiler::native::current_terminal_authority_permission_policy(),
            ),
            imports: &[],
        },
    )
    ...
    let compile_terminal_report = |label: &str, semantic_bindings: Vec<AcceptedSemanticBinding>| {
        omega::compiler::compile(
                omega::compiler::CompileRequest::new(omega::compiler::CompileOptions {
                ...
                .with_requested_product(omega::compiler::RequestedCompileProduct::TerminalArtifact),
    ...
    let exact_checked = omega::compiler::compile_to_checked(CheckedCompileRequest {
...
"#,
    ...
    let observation_probe_checked = omega::compiler::compile_to_checked(CheckedCompileRequest {
        package_inputs: Some(
    ...
    let substituted_observation_subject =
        omega::compiler::ProductionCompilationSubject::from_checked(
            exact_checked
    ...
    let observation_substituted_report =
        omega::compiler::CompileReport::from_retained_terminal_artifact(
            root_path.clone(),
    ...
    let substituted_checked = omega::compiler::compile_to_checked(CheckedCompileRequest {
        build_dir: Some(
            temporary
                .0
    ...
    let substituted_source_subject = omega::compiler::ProductionCompilationSubject::from_checked(
        substituted_checked
            .package_compilation_subject()
    ...
    let widened_receiving_policy =
        omega::compiler::native::terminal_authority_permission_policy_with_rows(vec![
            omega::compiler::native::TerminalAuthorityPermissionPolicyRow::new(
                provider.schema().identity_digest(),
                exit_requirement.clone(),
                TerminalAuthorityDisposition::from_classes([
                    TerminalAuthorityClass::ProcessOutput,
