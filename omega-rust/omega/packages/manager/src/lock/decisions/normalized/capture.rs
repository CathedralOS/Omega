...
impl HistoricalPackagePolicyDecisions {
    ...
    pub fn capture_policy_changes_with_usage(
        ...
        let replacement_count = changes.source_replacements().len();
        ...
        replacements.extend(
            changes
                ...
                .iter()
                .enumerate()
                .map(|(ordinal, replacement)| (replacement.fingerprint().digest(), ordinal)),
        );
        ...
        for decision in resolution.decisions() {
            if let PackagePolicyDecisionSubject::SourceReplacement(fingerprint) = decision.subject {
                let entry = replacements
                    .binary_search_by_key(&fingerprint, |entry| entry.0)
                    .map_err(|_| Error::ResolutionMismatch)?;
                let subject = replacement::capture(
                    &changes.source_replacements()[replacements[entry].1],
                    source,
                    limits,
                    &mut usage,
                    maximum_owned_bytes,
                    &mut removed_text_bytes,
                )?;
            ...
            let (ordinal, conflict) = match decision.subject {
                ...
                PackagePolicyDecisionSubject::SourceReplacement(_) => {
                    return Err(Error::ResolutionMismatch);
                }
            };
