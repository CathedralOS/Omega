...
fn subject_order(left: &Subject, right: &Subject) -> Ordering {
    ...
    match (left, right) {
        ...
        (
            Subject::SourceReplacement { site: left, .. },
            Subject::SourceReplacement { site: right, .. },
        ) => left.cmp(right),
...
pub(super) fn validate(
    ...
    for decision in &history.decisions {
        match &decision.subject {
            ...
            Subject::SourceReplacement {
                ...
                replacement::validate(baseline, *package_index, site, source)?;
                replaces_root |= matches!(site, ReplacementSite::Root);
            }
            _ => return Err(Error::InvalidSubject),
        }
    }
    // A root-role change describes one stable root key. A replacement instead
    // has different old/new keys and cannot describe that same comparison.
    if replaces_root && root_role_count != 0 {
        return Err(Error::InvalidSubject);
    }
    if history
        .decisions
    ...
    if history.decisions.windows(2).any(|pair| matches!(
        (&pair[0].subject, &pair[1].subject),
        (Subject::SourceReplacement { site: left, .. }, Subject::SourceReplacement { site: right, .. }) if left == right
    )) { return Err(Error::NonCanonicalDecisions); }
