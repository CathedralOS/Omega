    ...
    for _ in 0..count {
        ...
        let subject = match fields.next() {
            ...
            Some("replacement") => {
                ...
                let package_index = parse_number(next(&mut fields)?)?;
                let site = match kind {
                    "root" => ReplacementSite::Root,
                    "dependency" => ReplacementSite::Dependency {
                        requester_index: parse_number(next(&mut fields)?)?,
                        alias: replacement::copy_alias(
                            next(&mut fields)?,
                            &mut usage,
                            maximum_owned_bytes,
                        )?,
                ...
                let length = parse_number(next(&mut fields)?)?;
                let fragment = body.get(..length).ok_or(Error::InvalidFraming)?;
                let (baseline, bytes) = recover_package_key_text(
                    fragment,
                    key_limits(limits.maximum_bytes),
                    maximum_owned_bytes - usage.owned_bytes,
                )
