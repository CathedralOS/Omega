...
Acquisition from recorded commit/root-tree IDs retains the authored
...
Named-member acquisition uses the same recorded root selection and existing
authenticated declaration planner, then publishes only the selected member.
