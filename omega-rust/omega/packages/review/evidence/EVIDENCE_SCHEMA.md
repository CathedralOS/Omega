...
Schema v130 / row v88 / recovery v23 replaces the D29 symbolic-demand row's
...
obligation, package-qualified callable, contract/fact coordinates, strong
machine-contract commitment, complete ordered kernel assumptions, canonical
goal, and deterministic selected assumption. Fresh capture and ordinary-ledger
replay independently reconstruct the semantic question and invoke the proof
kernel; missing, duplicate, stale, reordered, or otherwise changed evidence
rejects or leaves the obligation open. The open stand-down row remains visible,
and the discharge row grants no accepted-lock, package-admission, policy, or
...
closed boundary requirement application and opaque declaration retains the
complete compiler-derived shape graph, every exact opaque carrier occurrence,
its semantic parameter/result path and replay-validated register/stack/indirect
placement, calling policy, selected target, closed conformance and selection
commitments, and the strong boundary-plan application commitment. The compiler
records each carrier shape root while materializing the opaque; equal-looking
ordinary or opaque layouts cannot be inferred as occurrences afterward. The
selecting package owns the row, including for a dependency-owned opaque. An
unused or reference-only selection emits no consumer demand. Introduction or
change remains audit-recommended.
...
ordinary type parameters carrying default properties and lifetime telescopes.
...
static telescope counts, exact value-parameter modes and types, and the exact
return carrier. A private external leaf therefore cannot inherit missing
signature identity from a public callable row it deliberately does not have.
Richer static kinds, bounds, and conformance telescopes reject until this key
has an exact structural carrier for them.
