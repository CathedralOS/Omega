...
use object_file::{ObjectPlan, SectionKind, SymbolKind, SymbolSection};
...
pub fn validate_final_image_function_linkage(
    ...
    for (binding_index, binding) in object.layout.function_symbols.iter() {
        let Some((object_symbol_handle, object_symbol)) =
            object_file::object_function_symbol(object, binding.identity)
        else {
...
mod tests {
    ...
    fn linked_image() -> (FinalImage, ObjectPlan) {
        ...
        let symbol = object.layout.symbols.insert(SymbolPlan {
            name: object_file::entry_symbol_name(target),
        ...
        let image = build_final_image(FinalImageInput {
            target,
            object: &object,
            relocations: &object_file::RelocationPlan::with_target(target),
            text_bytes: &[0; 8],
            data_bytes: &[],
        });
        (image, object)
