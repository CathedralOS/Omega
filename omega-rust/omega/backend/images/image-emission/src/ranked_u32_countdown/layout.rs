...
pub(super) fn validate_ranked_countdown_layout(
    ...
    let (preheader, header, compare, positive, decrement, backedge, exit, returned) =
        if target == NativeTarget::linux_x64() {
            let decoded = isa_x86_64::validate_x86_64_ranked_u32_countdown_in_edi(bytes).ok()?;
            ...
            let decoded = isa_aarch64::validate_aarch64_ranked_u32_countdown_in_w0(bytes).ok()?;
            let layout = decoded.layout();
            (
                layout.preheader_branch(),
                layout.header_offset(),
