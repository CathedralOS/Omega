//! Canonical format-36 codec for retained completion claim sources.
//!
    ...
    if let Some(content) = &source.content {
        ...
        push_u32(
            ...
            u32::try_from(content.input.segments.len()).map_err(|_| {
                InstallationError::CountNotRepresentable("completion content subject segments")
            })?,
        );
        ...
        push_u32(
            ...
            u32::try_from(content.projections.len()).map_err(|_| {
                InstallationError::CountNotRepresentable("completion content projections")
            })?,
        );
...
pub(super) fn decode_completion_claim_source(
    reader: &mut Reader<'_>,
) -> Result<CompletionClaimSource, InstallationError> {
    let tag = reader.u8()?;
    if reader.take(3)? != [0; 3] {
        return Err(InstallationError::NonzeroReservedField);
    }
    let claim =
        ClaimId::new(reader.u64()?).ok_or(InstallationError::ZeroSettlementIdentity("ClaimId"))?;
    if tag == 0 || tag & !3 != 0 {
        return Err(InstallationError::InvalidCompletionClaimSource);
    }
    ...
    let content = if tag & 2 != 0 {
        let version = match reader.u8()? {
            ...
            _ => return Err(InstallationError::InvalidCompletionClaimSource),
        };
        if reader.take(3)? != [0; 3] {
            return Err(InstallationError::NonzeroReservedField);
        }
        let root = PlaceId::new(reader.u64()?)
            .ok_or(InstallationError::ZeroSettlementIdentity("PlaceId"))?;
