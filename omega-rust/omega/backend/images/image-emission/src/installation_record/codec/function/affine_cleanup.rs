...
use crate::installation_record::{
    InstallationError, Reader, decode_structural_types, encode_structural_types, push_u32, push_u64,
};

pub(crate) fn encode_unit_affine_cleanup(
    bytes: &mut Vec<u8>,
    ...
    push_u64(
        ...
        u64::try_from(cleanup.code_offset)
            .map_err(|_| InstallationError::StructuralReturnOffsetNotRepresentable)?,
    );
    push_u64(
        bytes,
...
pub(crate) fn encode_scalar_control_affine_cleanups(
    bytes: &mut Vec<u8>,
    cleanups: &[UnitAffineCleanupRecord],
) -> Result<(), InstallationError> {
    ...
    push_u32(
        ...
        u32::try_from(cleanups.len()).map_err(|_| {
            InstallationError::InvalidScalarControlAffineCleanupCount(cleanups.len())
        })?,
    );
...
pub(crate) fn decode_unit_affine_cleanup(
    reader: &mut Reader<'_>,
) -> Result<UnitAffineCleanupRecord, InstallationError> {
    let psi_edge = EdgeId::new(reader.u64()?).ok_or(
        InstallationError::ZeroStructuralReturnIdentity("Unit cleanup edge"),
    )?;
    ...
    for _ in 0..local_count {
        let operation = OperationId::new(reader.u64()?).ok_or(
            InstallationError::ZeroStructuralReturnIdentity("Unit local operation"),
        )?;
        locals.push((
            operation,
    ...
    for _ in 0..action_count {
        ...
        actions.push(match tag {
            1 => TerminalAffineCleanupAction::DiscardRoot(PlaceId::new(reader.u64()?).ok_or(
                InstallationError::ZeroStructuralReturnIdentity("Unit cleanup place"),
            )?),
            2 => {
                let argument = decode_structural_argument(reader)?;
                let structural_type = StructuralTypeId::new(reader.u64()?).ok_or(
                    InstallationError::ZeroStructuralReturnIdentity("residual Unit cleanup type"),
                )?;
            ...
            3 => TerminalAffineCleanupAction::InvokeNominal(terminal_psi::NominalAffineCleanup {
                place: PlaceId::new(reader.u64()?).ok_or(
                    InstallationError::ZeroStructuralReturnIdentity("nominal Unit cleanup place"),
                )?,
                structural_type: StructuralTypeId::new(reader.u64()?).ok_or(
                    InstallationError::ZeroStructuralReturnIdentity("nominal Unit cleanup type"),
                )?,
                cleanup_machine: MachineId::new(reader.u64()?).ok_or(
                    InstallationError::ZeroStructuralReturnIdentity("nominal Unit cleanup machine"),
                )?,
                cleanup_receiver: None,
                requirement_obligations: Vec::new(),
            }),
            tag => return Err(InstallationError::InvalidCleanupActionTag(tag)),
        });
    }
...
pub(crate) fn decode_scalar_control_affine_cleanups(
    reader: &mut Reader<'_>,
) -> Result<Vec<UnitAffineCleanupRecord>, InstallationError> {
    let count = usize::try_from(reader.u32()?)
        .map_err(|_| InstallationError::InvalidScalarControlAffineCleanupCount(usize::MAX))?;
    if count == 1 {
        return Err(InstallationError::InvalidScalarControlAffineCleanupCount(
            count,
        ));
