...
use super::{
    ...
    structural::scalar::{decode_domains, decode_multiplicity, encode_domains, multiplicity_tag},
    unit::scalar::{
...
use crate::installation_record::{
    InstallationError, ObjectBoundarySettlement, Reader, decode_structural_types,
    encode_structural_types, push_u16, push_u32, push_u64,
};

pub(crate) fn encode_boundary_settlements(
    bytes: &mut Vec<u8>,
    count: u32,
...
pub(crate) fn decode_boundary_settlements(
    reader: &mut Reader<'_>,
...
fn encode_boundary_result(
    ...
    match result {
        ...
        BoundaryResultRecord::Structural(result) => {
            ...
            crate::installation_record::encode_structural_types(
                bytes,
                std::slice::from_ref(&result.declaration),
            )?;
            ...
            BoundaryStructuralResultRecord {
                ...
                declaration: {
                    let mut declarations =
                        crate::installation_record::decode_structural_types(reader)?;
...
mod tests {
        ...
        for codec in 0..3 {
            let encode =
                |bytes: &mut Vec<u8>, path: &[terminal_psi::StructuralPathSegment]| match codec {
                    ...
                    1 => super::super::structural::argument::encode_path(bytes, path),
                    _ => {
                        super::super::unit::structural_scalar_field_store::encode_path(bytes, path)
                    }
                };
            let decode = |reader: &mut Reader<'_>| match codec {
                0 => super::decode_structural_path(reader),
                1 => super::super::structural::argument::decode_path(reader),
                _ => super::super::unit::structural_scalar_field_store::decode_path(reader),
            };
    ...
    fn selected_process_exit_source_has_no_scratch_and_preserves_exact_identity() {
        ...
        assert!(
            super::super::internal_unit::scalar_call::encode_argument_source(
                &mut Vec::new(),
                source
            )
            .is_err()
