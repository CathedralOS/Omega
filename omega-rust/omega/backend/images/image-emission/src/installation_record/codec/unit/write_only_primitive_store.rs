...
use semantic_vocabulary::{OperationId, ScalarType, StructuralTypeId, ValueId};
...
use crate::installation_record::codec::{
    ...
    unit::structural_scalar_field_store::{decode_destination, encode_destination},
    value_placement::{
...
pub(crate) fn encode_unit_write_only_primitive_stores(
    bytes: &mut Vec<u8>,
...
pub(crate) fn decode_unit_write_only_primitive_stores(
    reader: &mut Reader<'_>,
    ...
    match source {
        ...
        UnitWriteOnlyPrimitiveStoreSourceRecord::IeeeFloatImmediate {
            ...
            let (format, bits) = match value {
                semantic_vocabulary::IeeeFloatValue::Binary32(bits) => (1, u64::from(bits)),
                semantic_vocabulary::IeeeFloatValue::Binary64(bits) => (2, bits),
            };
            bytes.push(format);
            bytes.extend_from_slice(&[0; 7]);
            push_u64(bytes, bits);
    ...
    match tag {
        ...
        3 => {
            ...
            let value = match format {
                1 => semantic_vocabulary::IeeeFloatValue::Binary32(
                    u32::try_from(bits).map_err(|_| InstallationError::NonzeroReservedField)?,
                ),
                2 => semantic_vocabulary::IeeeFloatValue::Binary64(bits),
