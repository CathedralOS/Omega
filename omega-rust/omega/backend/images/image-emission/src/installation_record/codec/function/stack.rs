...
use crate::installation_record::codec::call_site_owner::{
    decode_call_site_owner, encode_call_site_owner,
};
use crate::installation_record::{
    InstallationError, InstalledForeignCallStack, InstalledFunction, Reader, push_u32, push_u64,
};

pub(crate) fn encode_function_stack_facts(
    bytes: &mut Vec<u8>,
...
pub(crate) fn decode_function_stack_facts(
    reader: &mut Reader<'_>,
) -> Result<
    (
...
> {
    ...
    for _ in 0..foreign_call_count {
        ...
        let contribution_report_identity =
            task_plans::AdmittedStackContributionReportId::from_normalized_identity(reader.u64()?)
                .map_err(|_| InstallationError::InvalidForeignStackContribution)?;
        let contribution_commitment =
            task_plans::SameStackContributionCommitment::from_digest(reader.array()?);
