...
use crate::installation_record::{
    InstallationError, InstalledFunction, Reader, decode_boolean, push_u32, push_u64,
};
...
pub(crate) fn decode_functions(
    ...
    for _ in 0..count {
        ...
        let structural_call_scalar_return = match has_structural_call_scalar_return {
            false => None,
            true => {
                let psi_edge = semantic_vocabulary::EdgeId::new(reader.u64()?).ok_or(
                    InstallationError::ZeroStructuralCallScalarReturnIdentity("return edge"),
                )?;
                let psi_operation = semantic_vocabulary::OperationId::new(reader.u64()?).ok_or(
                    InstallationError::ZeroStructuralCallScalarReturnIdentity("call operation"),
                )?;
                let source_value = semantic_vocabulary::ValueId::new(reader.u64()?).ok_or(
                ...
                let callee = semantic_vocabulary::MachineId::new(reader.u64()?).ok_or(
