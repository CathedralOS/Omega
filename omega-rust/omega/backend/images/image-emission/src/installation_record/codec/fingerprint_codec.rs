...
use crate::installation_record::{
    ImageFingerprint, InitializedDataFingerprint, InstallationFingerprint,
};
...
pub(crate) fn fingerprint_image(bytes: &[u8]) -> ImageFingerprint {
    ImageFingerprint(hash(IMAGE_DOMAIN, bytes))
}

pub(crate) fn fingerprint_initialized_data(bytes: &[u8]) -> InitializedDataFingerprint {
    InitializedDataFingerprint(hash(INITIALIZED_DATA_DOMAIN, bytes))
}

pub(crate) fn fingerprint_record(bytes: &[u8]) -> InstallationFingerprint {
