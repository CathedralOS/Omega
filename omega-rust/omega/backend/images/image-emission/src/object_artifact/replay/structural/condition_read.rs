...
use machine_code::{ScalarStackEvidence, ScalarStackMutationKind};
...
pub(crate) fn condition_stack_depth_before(
        ...
        .filter(|mutation| mutation.offset >= condition_start && mutation.offset < read_start)
    {
        ...
        match mutation.kind {
            ...
            ScalarStackMutationKind::X86Push => depth = depth.checked_add(8)?,
            ScalarStackMutationKind::X86Pop => depth = depth.checked_sub(8)?,
        }
    }
    Some(depth)
}

pub(crate) fn replay_x86_boolean_structural_read(
...
pub(crate) fn replay_aarch64_boolean_structural_read(
    placement: &ValuePlacement,
    field_byte_offset: u32,
    stack_depth: u32,
) -> Option<Vec<u8>> {
