...
use super::super::unit::scalar_call_custody::expected_unit_scalar_result_bytes;
use super::super::unit::stack::validate_stack_adjustment_pair;
use crate::{ObjectError, ObjectUnitStack};

pub(crate) fn validate_dynamic_calls(
    target: NativeTarget,
    ...
    for call in &function.dynamic_calls {
        ...
        if previous_end.is_some_and(|end| end > call.code_offset)
            ...
            || call.initial_instance.source.access == terminal_psi::StructuralAccess::Owned
            || call.rebound_instance.source.access == terminal_psi::StructuralAccess::Owned
        ...
        let outbound_bytes = if let Some(outbound) = call.unit_stack.outbound {
            validate_stack_adjustment_pair(
                target.architecture,
                function.machine,
                Some(target_operations::CallSiteOwner::Operation(
                    call.psi_operation,
                )),
                &function.bytes,
                outbound,
...
pub(crate) fn validate_stored_dynamic_calls(
    target: NativeTarget,
    function: &MachineCodeFunction,
    machine_functions: &std::collections::BTreeMap<MachineId, &MachineCodeFunction>,
    function_stack: Option<&ObjectUnitStack>,
    ...
    for call in &function.stored_dynamic_calls {
        ...
        if previous_end.is_some_and(|end| end > establishment.code_offset)
            ...
            || establishment.instance.source.access == terminal_psi::StructuralAccess::Owned
...
fn dynamic_result_matches(
    ...
    actual: Option<semantic_vocabulary::ScalarType>,
) -> bool {
    match (expected, actual) {
        (terminal_psi::ClosedConformanceCallableResult::Unit, None) => true,
        (
...
mod result_class_tests {
    ...
    fn unit_and_scalar_result_classes_are_not_substitutable() {
        use terminal_psi::ClosedConformanceCallableResult::{Bool, I32, Unit};
