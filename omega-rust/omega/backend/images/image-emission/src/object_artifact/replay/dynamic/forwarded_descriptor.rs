...
use super::super::instruction_loads::{aarch64_terminal_register, x86_terminal_register};
use crate::ObjectError;
use crate::object_artifact::same_dynamic_table_application;

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct ValidatedForwardedDynamicApplication {
    pub application: terminal_psi::ClosedConformanceApplication,
    pub adapters: Vec<ForwardedDynamicDescriptorAdapterRecord>,
}

pub(crate) fn validate_forwarded_dynamic_descriptors(
    target: NativeTarget,
...
fn validate_call_result(
    ...
    call: &machine_code::ForwardedDynamicDescriptorCallRecord,
    operation_end: usize,
) -> Option<()> {
    ...
    let result_shape = call.semantic_result.and_then(|result| {
    ...
    let expected_bytes =
        crate::object_artifact::replay::unit::scalar_call_custody::expected_unit_scalar_result_bytes(target, result)?;
...
fn validate_parameter_dispatches(
    ...
    for call in &function.dynamic_parameter_calls {
        ...
        let expected_result_shape = match (call.requirement.result, call.scalar_type) {
            (terminal_psi::ClosedConformanceCallableResult::Unit, None) => None,
            (
                terminal_psi::ClosedConformanceCallableResult::I32,
                Some(semantic_vocabulary::ScalarType::Integer(integer)),
            ) if integer.sign() == semantic_vocabulary::IntegerSign::Signed
                && integer.bits() == 32 =>
            {
...
fn validate_table_address(
    target: NativeTarget,
    function: &MachineCodeFunction,
    argument: &machine_code::ForwardedDynamicDescriptorArgumentRecord,
) -> Option<()> {
...
fn validate_adapter(
    ...
    row: &terminal_psi::ClosedConformanceRow,
    row_index: usize,
    ...
    let result = match callable.result {
        ...
        terminal_psi::ClosedConformanceCallableResult::I32 => Some(ValueShape::integer(4, 4)),
        terminal_psi::ClosedConformanceCallableResult::Bool => Some(ValueShape::integer(1, 1)),
    };
    ...
    let realization_parameter_shape =
        if argument.instance.access == terminal_psi::StructuralAccess::MutableBorrow {
...
fn expected_adapter_bytes(
    target: NativeTarget,
    erased: &calling_conventions::CallPlan,
    realization: &calling_conventions::CallPlan,
