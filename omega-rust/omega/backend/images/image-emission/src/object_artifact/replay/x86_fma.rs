//! Exact x86 scalar FMA3 interval replay against the admitted feature profile
//! and provider authority.

use calling_conventions::MachineRegister;
use iced_x86::{Decoder, DecoderOptions, Mnemonic};
use machine_code::{MachineCodeFunction, X86ScalarFmaFormat};
use target::{
...
pub(crate) fn validate_x86_scalar_fma_function(
    ...
    for fragment in &function.x86_scalar_fma {
        ...
        let decoded = isa_x86_64::decode_vfmadd132_scalar(bytes).map_err(|_| {
        ...
        let format = match decoded.format {
            isa_x86_64::DecodedScalarFmaFormat::Binary32 => X86ScalarFmaFormat::Binary32,
            isa_x86_64::DecodedScalarFmaFormat::Binary64 => X86ScalarFmaFormat::Binary64,
        };
        ...
        let attribution = function
            ...
            .find(|attribution| {
                attribution.operation_ordinal == occurrence.operation_ordinal
                    && attribution.site
                        == machine_code::SemanticCodeSite::Operation(occurrence.terminal_operation)
            })
...
fn validate_floating_control(
    target: NativeTarget,
    function: &MachineCodeFunction,
    control: machine_code::X86FloatingControlRecord,
) -> Result<(), ObjectError> {
    let expected_save = isa_x86_64::encode_stmxcsr_rsp_displacement(control.saved_slot_byte_offset)
        .map_err(|_| ObjectError::InvalidX86ScalarFmaFloatingControl(function.machine))?;
    let expected_store = isa_x86_64::encode_store_mxcsr_constant_rsp_displacement(
        control.canonical_slot_byte_offset,
        isa_x86_64::OMEGA_CANONICAL_MXCSR,
    )
    .map_err(|_| ObjectError::InvalidX86ScalarFmaFloatingControl(function.machine))?;
    let expected_install =
        isa_x86_64::encode_ldmxcsr_rsp_displacement(control.canonical_slot_byte_offset)
            .map_err(|_| ObjectError::InvalidX86ScalarFmaFloatingControl(function.machine))?;
    ...
    if control.target != target
        || control.canonical_mxcsr != isa_x86_64::OMEGA_CANONICAL_MXCSR
...
const fn float_format(format: X86ScalarFmaFormat) -> semantic_vocabulary::IeeeFloatFormat {
    match format {
        X86ScalarFmaFormat::Binary32 => semantic_vocabulary::IeeeFloatFormat::Binary32,
        X86ScalarFmaFormat::Binary64 => semantic_vocabulary::IeeeFloatFormat::Binary64,
    }
}
