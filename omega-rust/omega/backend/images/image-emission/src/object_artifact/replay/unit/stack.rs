...
use crate::{ObjectError, ObjectUnitCallStack, ObjectUnitStack};
...
pub(crate) fn validate_complete_unit_stack_evidence(
    ...
    dynamic_calls: &[machine_code::DynamicCallRecord],
    stored_dynamic_calls: &[machine_code::StoredDynamicCallRecord],
    boundary_settlements: &[machine_code::BoundarySettlementRecord],
    integer_constants: &[machine_code::UnitIntegerConstantRecord],
    scalar_homes: &[machine_code::UnitScalarHomeRecord],
    internal_unit_scalar_calls: &[machine_code::InternalUnitScalarCallRecord],
    ...
    for settlement in boundary_settlements {
        if crate::object_artifact::replay::boundary::runtime_scalar_custody::hosted_write_byte_custody_is_exact(
            target,
            settlement,
