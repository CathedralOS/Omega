...
use image_emission::bind_installed_artifact;
use image_emission::{
...
mod installed_runtime {
    ...
    use object_file::{RelocationKind, SectionKind};
...
fn machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    let source = r#"
        trait Measure {
...
fn changed_conformance_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    let source = r#"
        trait Measure {
            machine measure(&self) -> i32;
        }
...
fn dynamic_unit_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    let source = r#"
        trait Touch {
            machine touch(&self);
        }
...
fn changed_conformance_unit_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    let source = r#"
...
fn stored_dynamic_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    let source = r#"
...
fn compile_source(source: &str, target: NativeTarget) -> machine_code::MachineCodePlan {
    ...
    for target in [
        ...
        if target == NativeTarget::linux_x64() {
            ...
            let caller = wrong_access
                ...
                .expect("entry caller");
            caller.unit_parameters[0].access = terminal_psi::StructuralAccess::Owned;
            assert!(
...
fn rebound_dynamic_call_materializes_complete_private_table_and_executes_image_replay() {
    for target in [
        ...
        assert_eq!(
            ...
            Err(image_emission::InstallationError::InvalidDynamicCall(
                call.machine
            )),
