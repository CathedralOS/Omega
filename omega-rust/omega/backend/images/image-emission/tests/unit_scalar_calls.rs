...
use image_emission::{ObjectError, build_object_artifact};
...
fn emitted_scalar_chain(target: NativeTarget) -> machine_code::MachineCodePlan {
    ...
    let selected =
        abstract_operations_to_target_operations::lower_to_target_operations(&source, target)
            .unwrap();
    let assigned =
        target_operations_to_assigned_target_operations::assign_registers(&selected).unwrap();
...
fn object_rejects_scalar_argument_result_home_constant_and_abi_tampering() {
    ...
    use_before_definition.functions[0].internal_unit_scalar_calls[1].arguments[0].source =
        machine_code::InternalUnitScalarArgumentSourceRecord::Home(later_home);
