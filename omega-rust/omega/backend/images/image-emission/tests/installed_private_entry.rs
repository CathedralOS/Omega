...
use machine_code::{CompilerPrivateMachineCodeFunction, MachineCodePlanWithPrivateFunctions};
...
fn emitted_object() -> (
    ...
    let assigned_semantic =
        target_operations_to_assigned_target_operations::assign_registers(&semantic)
            .expect("semantic register assignment");
    let semantic =
        machine_emission::emit_machine_code(&assigned_semantic).expect("semantic machine emission");
    ...
    let private_machine =
        machine_emission::emit_machine_code(&assigned_private).expect("private machine emission");
