...
fn direct_scalar_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    machine_plan_from_source(
        r#"
            trait Measure { machine measure(&self) -> i32; }
            data Item { value: i32; }
...
fn multi_hop_scalar_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    machine_plan_from_source(
        r#"
            trait Measure { machine measure(&self) -> i32; }
            data Item { value: i32; }
...
fn multi_hop_unit_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    machine_plan_from_source(
        r#"
...
fn unit_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    machine_plan_from_source(
        r#"
            trait Touch { machine touch(&self); }
            data Item { value: i32; }
...
fn direct_unit_machine_plan(target: NativeTarget) -> machine_code::MachineCodePlan {
    machine_plan_from_source(
        r#"
            trait Touch { machine touch(&self); }
            data Item { value: i32; }
...
fn machine_plan_from_source(source: &str, target: NativeTarget) -> machine_code::MachineCodePlan {
...
fn object_replay_rejects_result_less_parameter_forwarding_result_drift() {
    ...
    let (caller, operation) = plan
        ...
        .find_map(|function| {
            let call = function.forwarded_dynamic_parameter_calls.first_mut()?;
            call.scalar_type = Some(semantic_vocabulary::ScalarType::Boolean);
            Some((function.machine, call.psi_operation))
        })
...
fn object_construction_rejects_fabricated_results_on_unit_forwarding() {
    ...
    call.semantic_result = Some(abstract_operations::AbstractResult {
    ...
    parameter_call.scalar_type = Some(semantic_vocabulary::ScalarType::Boolean);
...
fn object_construction_rejects_forwarded_scalar_result_custody_drift() {
    fn assert_rejected(plan: &machine_code::MachineCodePlan) {
        let caller = plan
            .functions
            .iter()
            .find(|function| function.machine == plan.entry)
    ...
    caller.forwarded_dynamic_descriptor_calls[0]
        ...
        .unwrap()
        .home
        .source_value = semantic_vocabulary::ValueId::new(999).unwrap();
