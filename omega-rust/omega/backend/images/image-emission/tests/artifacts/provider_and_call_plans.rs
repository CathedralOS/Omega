...
/// Linux x64 Unit entry whose single semantic operation is one returning
/// zero-argument versioned ELF import call. The frame reserves the call's
/// MXCSR slot, the evaluated SysV plan leaves only its alignment pad as
/// outbound custody, and the admitted same-stack contribution carries the
/// opaque provider leaf. Object construction declares one normalized import,
/// which is exactly the fact that selects the dynamic ELF writer at image
/// emission and forbids the installable direct route.
pub(super) fn linux_foreign_call_plan(provider: &WriteExitProvider) -> MachineCodePlan {
    let target = NativeTarget::linux_x64();
    let machine = machine_id(62);
    let call_operation = operation_id(62);
    let return_edge = edge_id(62);
    let locator = target::normalize_foreign_locator(
    ...
    .expect("native Linux x64 call plan");
    let boundary_entry_plan = calling_conventions::evaluate_ordinary_boundary_entry_plan(
        calling_conventions::CallingPolicy::native_for_target(target),
        &signature,
    )
    ...
    bytes.push(0xc3);
    let provider_plan_commitment =
        task_plans::SameStackProviderPlanCommitment::from_digest([9; 32]);
    let same_stack_contribution = task_plans::admit_same_stack_contribution(
        task_plans::SameStackContributionAdmissionCandidate {
            provider_plan_report_identity: provider.provider_plan_report_identity(),
            provider_plan_commitment,
