...
fn semantic_x86_fma_plan(profile: TargetProfile) -> MachineCodePlan {
    ...
    let assigned = target_operations_to_assigned_target_operations::assign_registers(&plan)
        .expect("semantic FMA physical assignment");
    machine_emission::emit_machine_code(&assigned).expect("semantic FMA machine emission")
}
...
fn shared_three_leaf_cleanup_plan(target: NativeTarget) -> MachineCodePlan {
    ...
    let leaf = |edge: u64, value: bool| TargetBooleanControl::ReturnImmediate {
        ...
        source_value: semantic_vocabulary::ValueId::new(edge).unwrap(),
        value,
    };
    ...
    let nested = TargetBooleanControl::Conditional {
        condition_source: semantic_vocabulary::ValueId::new(1).unwrap(),
    ...
    let assigned = target_operations_to_assigned_target_operations::assign_registers(&target_plan)
        .expect("shared cleanup assigns");
    machine_emission::emit_machine_code(&assigned).expect("shared cleanup emits")
}
