//! Canonical format-39 codec for one installed structural-return row.
//!
...
use omega_machine_code::StructuralReturnRecord;
...
pub(super) fn encode_structural_returns(
    bytes: &mut Vec<u8>,
    count: u32,
    installed: &[InstalledStructuralReturn],
...
fn decode_structural_return(
    reader: &mut Reader<'_>,
) -> Result<InstalledStructuralReturn, InstallationError> {
    ...
    for _ in 0..claim_count {
        returned_claims.push(
            ClaimId::new(reader.u64()?)
                .ok_or(InstallationError::ZeroStructuralReturnIdentity("claim"))?,
        );
    }
    ...
    for _ in 0..local_count {
        let operation = OperationId::new(reader.u64()?).ok_or(
            InstallationError::ZeroStructuralReturnIdentity("local operation"),
        )?;
    ...
    for _ in 0..cleanup_count {
        trivial_affine_discards.push(PlaceId::new(reader.u64()?).ok_or(
            InstallationError::ZeroStructuralReturnIdentity("cleanup place"),
        )?);
    }
    Ok(InstalledStructuralReturn {
        machine,
