...
pub fn corrupt_allocation_recovery_realization_encoding_for_test(
    ...
    let row = staged
        ...
        .rows_mut()
        .iter_mut()
        .find(|row| matches!(row.state, omega_post_allocation_machine_to_selected_form_encoding::SelectedFormEncodingState::Encoded { .. }))
        .expect("recovery fixture has encoded selected forms");
    let omega_post_allocation_machine_to_selected_form_encoding::SelectedFormEncodingState::Encoded { bytes, .. } = &mut row.state else {
        unreachable!()
    };
    bytes[0] ^= 1;
}
