...
use omega_machine_code::{X86ScalarFmaFormat, X86ScalarFmaFragment};
use omega_target::{NativeTarget, X86FeatureRequirement};
...
pub fn emit_feature_required_x86_scalar_fma(
    ...
    let bytes = match format {
        X86ScalarFmaFormat::Binary32 => {
            omega_isa_x86_64::encode_vfmadd132ss(destination, addend, multiplicand)?
        }
        X86ScalarFmaFormat::Binary64 => {
            omega_isa_x86_64::encode_vfmadd132sd(destination, addend, multiplicand)?
        }
    };
