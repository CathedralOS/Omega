//! Exact macOS ARM64 replay owned beside the physical entry-plan carrier.
...
const MACOS_ARM64_TARGET_PACKAGE_SOURCE: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../../../../source/library/std/targets/macos_arm64/entry.omg"
));

/// Canonical checked-tree overload identity for `MacosArm64PhysicalEntry::enter`.
pub const MACOS_ARM64_PHYSICAL_REQUIREMENT_IDENTITY: &str = concat!(
    "named-callable(path(MacosArm64PhysicalEntry::enter),parameters(",
...
pub const MACOS_ARM64_I32_TYPE_IDENTITY: &str = "named(name(i32))";
/// Canonical normalized type identity of the `argv`, `envp`, and `apple`
/// raw process-arrival addresses.
pub const MACOS_ARM64_ADDRESS_TYPE_IDENTITY: &str = "named(name(addr))";

/// Strong commitment to the exact closed macOS ARM64 target-package source
/// compiled into this plan owner.
pub fn exact_macos_arm64_physical_contract_package_source_digest()
-> super::ProgramEntryPhysicalContractPackageSourceDigest {
    super::ProgramEntryPhysicalContractPackageSourceDigest::from_package_source(
        ...
        MACOS_ARM64_TARGET_PACKAGE_SOURCE,
    )
}

/// Independently reconstruct and validate the exact plan authored by the
/// closed macOS ARM64 target package. The authored `MacosArm64` policy is the
/// platform's ordinary AAPCS64 volatile bank, so the independent replay is
/// that evaluator applied to the exact dyld arrival signature: four inputs
/// (`i32` then three addresses) and one `i32` result.
pub fn exact_macos_arm64_physical_boundary_entry_plan() -> ValidatedBoundaryEntryPlan {
    ...
    evaluate_ordinary_boundary_entry_plan(calling_conventions::CallingPolicy::Aapcs64, &signature)
        .expect("the closed target-authored macOS ARM64 physical entry plan must remain valid")
}

impl ProgramEntryPhysicalContractPlan {
    /// Replay the complete target-owned macOS ARM64 physical-entry contract.
    ...
    pub fn matches_exact_macos_arm64_physical_contract(&self) -> bool {
        let expected = exact_macos_arm64_physical_boundary_entry_plan();
        self.target_slot == target::TargetProfile::MacosArm64.program_entry_slot()
            && self.target_package == target::ProgramEntryPhysicalContractPackage::MacosArm64
            && self.target_package_source_digest
                == exact_macos_arm64_physical_contract_package_source_digest()
            && self.requirement_identity == MACOS_ARM64_PHYSICAL_REQUIREMENT_IDENTITY
            && self.parameter_type_identities.len() == 4
            && self.parameter_type_identities[0] == MACOS_ARM64_I32_TYPE_IDENTITY
            && self.parameter_type_identities[1..]
                .iter()
                .all(|identity| identity == MACOS_ARM64_ADDRESS_TYPE_IDENTITY)
            && self.result_type_identity == MACOS_ARM64_I32_TYPE_IDENTITY
...
mod tests {
    ...
    fn exact_contract() -> ProgramEntryPhysicalContractPlan {
        let package = target::ProgramEntryPhysicalContractPackage::MacosArm64;
        let calling_plan = exact_macos_arm64_physical_boundary_entry_plan();
        ProgramEntryPhysicalContractPlan::new(
            target::TargetProfile::MacosArm64.program_entry_slot(),
            MACOS_ARM64_PHYSICAL_REQUIREMENT_IDENTITY.into(),
            package,
            exact_macos_arm64_physical_contract_package_source_digest(),
            1,
            vec![
                MACOS_ARM64_I32_TYPE_IDENTITY.into(),
                MACOS_ARM64_ADDRESS_TYPE_IDENTITY.into(),
                MACOS_ARM64_ADDRESS_TYPE_IDENTITY.into(),
                MACOS_ARM64_ADDRESS_TYPE_IDENTITY.into(),
            ],
            MACOS_ARM64_I32_TYPE_IDENTITY.into(),
    ...
    fn exact_runtime_contract_replays_all_owned_fields() {
        ...
        assert!(exact.matches_exact_macos_arm64_physical_contract());
        ...
        requirement_drift.requirement_identity =
            "named-callable(path(ProgramStorageEntry::enter),parameters(),result-dispatch())"
                .into();
        assert!(!requirement_drift.matches_exact_macos_arm64_physical_contract());

        let mut parameter_drift = exact.clone();
        parameter_drift.parameter_type_identities.swap(0, 1);
        assert!(!parameter_drift.matches_exact_macos_arm64_physical_contract());

        let mut result_drift = exact.clone();
        result_drift.result_type_identity = "named(name(Unit))".into();
        assert!(!result_drift.matches_exact_macos_arm64_physical_contract());
        ...
        assert!(!source_drift.matches_exact_macos_arm64_physical_contract());
        ...
        placement_drift
            ...
            .parameters
            .swap(0, 1);
        assert!(!placement_drift.matches_exact_macos_arm64_physical_contract());
    }

    #[test]
    fn physical_plan_is_exact_dyld_arrival_shape() {
        let plan = exact_macos_arm64_physical_boundary_entry_plan();
        ...
        for (index, parameter) in call.parameters.iter().enumerate() {
            ...
            match register {
                calling_conventions::MachineRegister::Aarch64X(found) => {
                    assert_eq!(*found, index as u8);
                }
                other => panic!("dyld input {index} drifts to {other:?}"),
            }
