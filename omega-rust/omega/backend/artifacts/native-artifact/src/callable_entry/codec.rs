...
pub(super) fn decode_psi(
    ...
    let marker = terminal_psi::VocabularyMarker::new(u16::from_le_bytes(cursor.array()?))
        .ok_or(OptimizedOrdinaryCallableEntryDecodeError::InvalidId)?;
    Ok(TerminalPsiIdentity {
        vocabulary_marker: marker,
        program_fingerprint: terminal_psi::SemanticFingerprint::from_bytes(cursor.array()?),
    })
}
