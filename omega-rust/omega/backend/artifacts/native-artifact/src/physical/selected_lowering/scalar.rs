...
pub(super) fn validate_machine_function(
    ...
    if stack.stack_alignment != 16
        || !matches!(
            stack.control_flow,
            ScalarControlFlowEvidence::Linear | ScalarControlFlowEvidence::Acyclic { .. }
        )
...
fn validate_attribution<'row>(
    ...
    if matches!(control, Some(ScalarControlFlowEvidence::Acyclic { .. }))
        || control.is_none() && provenance.edges.len() > 1
    {
        if edges.len() != provenance.edges.len() {
            return Err(INVALID);
        }
        let reconstructed = image_emission::reconstruct_scalar_control_flow(
            target.architecture,
            machine,
            bytes,
            edges.iter().copied(),
        ...
        .map_err(|_| INVALID)?;
        if let Some(ScalarControlFlowEvidence::Acyclic { blocks }) = control
