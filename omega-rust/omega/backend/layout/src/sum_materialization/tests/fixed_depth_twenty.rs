...
fn plural_depth_twenty_paths_compose_depth_nineteen_custody_and_retain_fences() {
    ...
    let carrier =
        build_time_evaluation::validate_const_materializable_record_with_depth_twenty_nested_sums(
            &checked,
            "Outer",
            &paths,
            &value,
    ...
    let rejects = |mutated: &layout_plans::ConventionalDepthTwentyRecordSumPathsLayoutReport| {
        assert!(
            carrier
