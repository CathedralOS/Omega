//! Focused depth-sixteen producer, replay, atomicity, and boundary fences.

use super::*;

#[test]
fn plural_depth_sixteen_paths_compose_depth_fifteen_custody_and_retain_fences() {
    let checked = checked(
        r#"
        ...
        data Outer [copy] { #1 left: Fourteenth; #2 right: Fourteenth; }

        data OuterTooDeep [copy] { #1 outer: Outer; }
        data OuterDirect [copy] { #1 fourteenth: Fourteenth; #2 direct: Choice; }
        data OuterArray [copy] { #1 fourteenth: Fourteenth; #2 choices: [Choice; 1]; }
        "#,
    );
    ...
    let fourteenth = definition("Fourteenth");
    let paths = project_conventional_record_with_depth_sixteen_nested_sums_materialization_layout(
        &checked,
        &plan,
        outer.symbol,
    )
    .expect("the complete depth-sixteen occurrence cohort should project");
    ...
    let value = BuildTimeValue::Struct {
        ...
        fields: vec![
            ("left".into(), fourteenth_value(choice(None))),
            ("right".into(), fourteenth_value(choice(Some(0x1122)))),
        ],
    };
    let carrier =
        build_time_evaluation::validate_const_materializable_record_with_depth_sixteen_nested_sums(
            &checked,
            "Outer",
            &paths,
            &value,
        ...
        .expect("depth-sixteen value custody should compose the depth-fifteen carriers");
    ...
    let rejects = |mutated: &layout_plans::ConventionalDepthSixteenRecordSumPathsLayoutReport| {
        assert!(
            carrier
    ...
    for name in ["Fourteenth", "OuterTooDeep", "OuterDirect", "OuterArray"] {
        assert!(
            ...
            .is_err(),
            "{name} remains outside the exact depth-sixteen cohort"
        );
    }
    ...
    let recursive_type = checked
        .data_members(fourteenth)
        .iter()
        .find_map(|member| match member {
            DataMember::Field(field) if field.name.as_str() == "thirteenth" => {
    ...
    recursive_checked
        ...
        .substitute_node(
            ...
            TypeReferenceNode::Named {
                symbol: fourteenth.symbol,
                name: fourteenth.name.clone(),
            },
        );
    assert!(
        project_conventional_record_with_depth_sixteen_nested_sums_materialization_layout(
        ...
        .is_err(),
        "a recursive depth-sixteen path must reject during bounded projection"
    );
    assert!(
        build_time_evaluation::validate_const_materializable_record_with_depth_sixteen_nested_sums(
            &recursive_checked,
        ...
        .is_err(),
        "a recursive depth-sixteen path must reject during value replay"
    );
    ...
    assert!(
        ...
        .is_ok(),
        "the unchanged plural depth-fifteen API retains its prior cohort"
    );
    assert!(
        project_conventional_record_with_depth_fifteen_nested_sums_materialization_layout(
            &checked,
        ...
        .is_err(),
        "the prior API must not widen to the new depth-sixteen root"
    );
}
