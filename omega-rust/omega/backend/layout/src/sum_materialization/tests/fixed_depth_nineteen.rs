//! Focused depth-nineteen producer, replay, atomicity, and boundary fences.

use super::*;

#[test]
fn plural_depth_nineteen_paths_compose_depth_eighteen_custody_and_retain_fences() {
    let checked = checked(
        r#"
        ...
        data Outer [copy] { #1 left: Seventeenth; #2 right: Seventeenth; }

        data OuterTooDeep [copy] { #1 outer: Outer; }
        data OuterDirect [copy] { #1 seventeenth: Seventeenth; #2 direct: Choice; }
        data OuterArray [copy] { #1 seventeenth: Seventeenth; #2 choices: [Choice; 1]; }
        "#,
    );
    ...
    let seventeenth = definition("Seventeenth");
    let paths = project_conventional_record_with_depth_nineteen_nested_sums_materialization_layout(
        &checked,
        &plan,
        outer.symbol,
    )
    .expect("the complete depth-nineteen occurrence cohort should project");
    ...
    let value = BuildTimeValue::Struct {
        ...
        fields: vec![
            ("left".into(), seventeenth_value(choice(None))),
            ("right".into(), seventeenth_value(choice(Some(0x1122)))),
        ],
    };
    let carrier = build_time_evaluation::validate_const_materializable_record_with_depth_nineteen_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    .expect("depth-nineteen value custody should compose the depth-eighteen carriers");
    ...
    let big_endian = build_time_evaluation::validate_const_materializable_record_with_depth_nineteen_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    let rejects = |mutated: &layout_plans::ConventionalDepthNineteenRecordSumPathsLayoutReport| {
        assert!(
            carrier
    ...
    for name in ["Seventeenth", "OuterTooDeep", "OuterDirect", "OuterArray"] {
        assert!(
            ...
            .is_err(),
            "{name} remains outside the exact depth-nineteen cohort"
        );
    }
    ...
    let recursive_type = checked
        .data_members(seventeenth)
        .iter()
        .find_map(|member| match member {
            DataMember::Field(field) if field.name.as_str() == "sixteenth" => {
    ...
    recursive_checked
        ...
        .substitute_node(
            ...
            TypeReferenceNode::Named {
                symbol: seventeenth.symbol,
                name: seventeenth.name.clone(),
            },
        );
    assert!(
        project_conventional_record_with_depth_nineteen_nested_sums_materialization_layout(
        ...
        .is_err(),
        "a recursive depth-nineteen path must reject during bounded projection"
    );
    assert!(
        build_time_evaluation::validate_const_materializable_record_with_depth_nineteen_nested_sums(
            &recursive_checked,
            "Outer",
            &paths,
            &value,
        ...
        .is_err(),
        "a recursive depth-nineteen path must reject during value replay"
    );
    ...
    assert!(
        ...
        .is_ok(),
        "the unchanged plural depth-eighteen API retains its prior cohort"
    );
    assert!(
        project_conventional_record_with_depth_eighteen_nested_sums_materialization_layout(
            &checked,
        ...
        .is_err(),
        "the prior API must not widen to the new depth-nineteen root"
    );
}
