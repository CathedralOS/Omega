//! The layout-plan vocabulary: `LayoutPlan` and the descriptors, field
//! layouts, shapes, and target-closed callback demands it carries.
//!
//! `builder.rs` produces a `LayoutPlan` from a checked type reference;
//! `field_paths`, `packing`, `sizing`, and `sum_materialization` read or
//! project these types. Layout never authorizes an access; it only fixes
...
impl TypeLayoutDescriptor {
    ...
    pub fn arithmetic_domain(&self) -> numerics::arithmetic::ArithmeticDomain {
        match self {
...
pub struct TargetClosedPrivateCallbackDemand {
    ...
    pub slot_application: typed_trees::typed_trees::ClosedConformanceApplication,
    pub slot_identity: Arc<str>,
    pub layout_subject_identity: Arc<str>,
    pub callback_requirement_identity: Arc<str>,
    pub layout: calling_conventions::LayoutPlanId,
    pub slot: calling_conventions::LayoutSlotId,
    pub requirement: calling_conventions::CallbackRequirementId,
    pub offset: usize,
    pub byte_size: usize,
    pub alignment: usize,
}
...
pub struct TargetClosedTwoHopPrivateCallbackPath {
    ...
    pub field: arena::Handle<FieldLayout>,
    pub field_layout: FieldLayout,
    pub field_identity: Arc<str>,
    pub field_slot: calling_conventions::LayoutSlotId,
...
impl TargetClosedTwoHopPrivateCallbackPath {
    pub fn native_demand(
        &self,
        parameter: calling_conventions::NativeParameterId,
    ) -> calling_conventions::NativeCallbackDemand {
        calling_conventions::NativeCallbackDemand {
            destination: calling_conventions::NativePlace::Field {
                parameter,
