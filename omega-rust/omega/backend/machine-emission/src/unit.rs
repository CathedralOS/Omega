...
pub(super) struct UnitEmission {
    ...
    pub(super) dynamic_calls: Vec<machine_code::DynamicCallRecord>,
    pub(super) stored_dynamic_calls: Vec<machine_code::StoredDynamicCallRecord>,
    ...
    pub(super) integer_constants: Vec<machine_code::UnitIntegerConstantRecord>,
    pub(super) affine_scalar_records: Vec<machine_code::UnitAffineScalarRecordEstablishmentRecord>,
    pub(super) structural_scalar_field_stores: Vec<UnitStructuralScalarFieldStoreRecord>,
    pub(super) write_only_primitive_stores: Vec<machine_code::UnitWriteOnlyPrimitiveStoreRecord>,
    pub(super) x86_scalar_fma: Vec<machine_code::X86ScalarFmaFragment>,
...
fn emit_callback_address(
    ...
    let (destination, encoding) = match (target.architecture, callback.destination) {
        (
            Architecture::X86_64,
            assigned_target_operations::AssignedCallDestination::Register(register),
        ) => {
        ...
        (
            Architecture::X86_64,
            assigned_target_operations::AssignedCallDestination::OutgoingStack { byte_offset },
        ) => {
            let register = 11;
...
fn emit_foreign_integer_argument(
    ...
    let semantic_vocabulary::ScalarType::Integer(scalar_type) = argument.source.scalar_type()
    else {
        return Err(EmissionError::InvalidNormalizedForeignCallCustody);
    };
    ...
    match target.architecture {
        Architecture::X86_64 => {
            ...
            parameter_homes = body
                ...
                .iter()
                .zip(&x86_homes)
                .map(|(parameter, home)| machine_code::UnitParameterHomeRecord {
            ...
            if let Some((saved, canonical)) = floating_control_offsets {
                ...
                let save = isa_x86_64::encode_stmxcsr_rsp_displacement(saved)
                ...
                let canonical_store = isa_x86_64::encode_store_mxcsr_constant_rsp_displacement(
                    canonical,
                    isa_x86_64::OMEGA_CANONICAL_MXCSR,
                )
                .map_err(|_| EmissionError::IeeeFloatControlFrameNotEncodable)?;
                bytes.extend_from_slice(&canonical_store);
                let install_offset = bytes.len();
                let install = isa_x86_64::encode_ldmxcsr_rsp_displacement(canonical)
                    .map_err(|_| EmissionError::IeeeFloatControlFrameNotEncodable)?;
                bytes.extend_from_slice(&install);
                x86_floating_control = Some(X86FloatingControlRecord {
                    target,
                    canonical_mxcsr: isa_x86_64::OMEGA_CANONICAL_MXCSR,
    ...
    for (operation_ordinal, operation) in body.operations.iter().enumerate() {
        ...
        match operation {
            ...
            AssignedUnitOperation::IntegerConstant {
                ...
                unit_integer_constants.push(machine_code::UnitIntegerConstantRecord {
            ...
            AssignedUnitOperation::NearestIeeeFloatFusedMultiplyAdd {
                ...
                let mut emit_operand = |operand: &assigned_target_operations::AssignedIeeeFloatFmaOperand|
                 -> Result<X86ScalarFmaOperandRecord, EmissionError> {
                    ...
                    let encoded = match operand.value {
                        semantic_vocabulary::IeeeFloatValue::Binary32(bits) => {
                            isa_x86_64::encode_binary32_bits_to_xmm(bits, operand.register)
                        }
                        semantic_vocabulary::IeeeFloatValue::Binary64(bits) => {
                            isa_x86_64::encode_binary64_bits_to_xmm(bits, operand.register)
                        }
                    }
                ...
                let fma_format = match format {
                    semantic_vocabulary::IeeeFloatFormat::Binary32 => X86ScalarFmaFormat::Binary32,
                    semantic_vocabulary::IeeeFloatFormat::Binary64 => X86ScalarFmaFormat::Binary64,
                };
            ...
            AssignedUnitOperation::EstablishAffineScalarRecord {
                ...
                let exact_value = matches!(value, semantic_vocabulary::IntegerValue::Signed(value) if i64::try_from(*value).is_ok());
                ...
                unit_affine_scalar_records.push(
                    machine_code::UnitAffineScalarRecordEstablishmentRecord {
            ...
            AssignedUnitOperation::PortWrite {
                ...
                let code_offset = bytes.len();
                bytes.extend_from_slice(&x86_encoding::encode_immediate_port_write(*port, *value));
            ...
            AssignedUnitOperation::NormalizedForeignCall {
                        ...
                        if callback.target.registrar_boundary_entry_plan
                            ...
                            || callback.target.application.placement
                                != match callback.destination {
                                    assigned_target_operations::AssignedCallDestination::Register(
                                        register,
                                    ...
                                    assigned_target_operations::AssignedCallDestination::OutgoingStack {
                                        byte_offset,
                                    } => calling_conventions::ValuePlacement {
                                        shape: callback.target.application.shape,
                ...
                canonicalized_boundary
                    ...
                    .clear();
                let canonical = calling_conventions::evaluate_ordinary_boundary_entry_plan(
                ...
                if validated.plan() != &foreign.boundary_entry_plan
                    ...
                    || call_plan.policy
                        != calling_conventions::CallingPolicy::native_for_target(target)
                    || call_plan.entry_control != calling_conventions::EntryControl::CallReturn
                ...
                let mut x86_call_floating_control = match target.architecture {
                    Architecture::X86_64 => {
                        ...
                        let save_offset = bytes.len();
                        let save =
                            isa_x86_64::encode_stmxcsr_rsp_displacement(saved_slot_byte_offset)
                                .map_err(|_| EmissionError::IeeeFloatControlFrameNotEncodable)?;
                        bytes.extend_from_slice(&save);
                ...
                let mut aarch64_call_floating_control = match target.architecture {
                    ...
                    Architecture::Aarch64 => {
                        let saved_slot_byte_offset = aarch64_foreign_floating_control_slot
                            .ok_or(EmissionError::IeeeFloatControlFrameNotEncodable)?;
                        let save_offset = bytes.len();
                        let save = isa_aarch64::encode_save_fpcr_to_sp_displacement(
                            saved_slot_byte_offset,
                        )
                        .map_err(|_| EmissionError::IeeeFloatControlFrameNotEncodable)?;
                        bytes.extend_from_slice(&save);
                ...
                if let Some(control) = &mut aarch64_call_floating_control {
                    control.restore_offset = bytes.len();
                    let restore = isa_aarch64::encode_restore_fpcr_from_sp_displacement(
            ...
            AssignedUnitOperation::BoundarySettlement {
                ...
                match realization {
                    target_operations::BoundaryRealization::MetadataOnlyPort(_) => {
                    ...
                    target_operations::BoundaryRealization::LinuxWriteLine(_) => {
                        ...
                        let (encoded, data) = match target.architecture {
                            Architecture::X86_64 => {
                                isa_x86_64::encode_linux_write_line_literal(literal_bytes)
                                    .map_err(|_| EmissionError::LinuxWriteLineEncoding)?
                            }
                            Architecture::Aarch64 => {
                                isa_aarch64::encode_linux_write_line_literal(literal_bytes)
                                    .map_err(|_| EmissionError::LinuxWriteLineEncoding)?
                            }
                        };
                    ...
                    target_operations::BoundaryRealization::LinuxExitGroupI32(_) => {
                        ...
                        let expected_destination = match target.architecture {
                            Architecture::X86_64 => calling_conventions::MachineRegister::X86Rdi,
                            Architecture::Aarch64 => {
                        ...
                        match target.architecture {
                            Architecture::X86_64 => bytes
                                .extend_from_slice(&isa_x86_64::encode_linux_exit_group_i32(value)),
                            Architecture::Aarch64 => bytes.extend_from_slice(
                                &isa_aarch64::encode_linux_exit_group_i32(value)
                    ...
                    target_operations::BoundaryRealization::LinuxWriteByteI32(_) => {
                        ...
                        .expect("i32 is valid");
                        let expected_destination = match target.architecture {
                            Architecture::X86_64 => calling_conventions::MachineRegister::X86R11,
                            Architecture::Aarch64 => {
                                calling_conventions::MachineRegister::Aarch64X(9)
                            }
                        };
                        if !matches!(result, AssignedBoundaryResult::Unit)
                            || target.object_format != ObjectFormat::Elf
                        ...
                        match target.architecture {
                            Architecture::X86_64 => bytes.extend_from_slice(
                                &isa_x86_64::encode_linux_write_byte_i32_from_r11(),
                            ),
                            Architecture::Aarch64 => bytes.extend_from_slice(
                                &isa_aarch64::encode_linux_write_byte_i32_from_w9()
                                    .map_err(|_| EmissionError::LinuxWriteLineEncoding)?,
                            ),
                        }
                    ...
                    target_operations::BoundaryRealization::DirectPortReadU8(_) => {
                        return Err(EmissionError::InvalidLinuxWriteLineCustody);
                    }
            ...
            AssignedUnitOperation::Return {
                ...
                let structural_result_prefix =
                    body.operations[..operation_ordinal]
                        ...
                        .filter_map(|operation| match operation {
                            ...
                            AssignedUnitOperation::BoundarySettlement {
                                result:
                                    assigned_target_operations::AssignedBoundaryResult::Structural(
                                        result,
                                    ),
                                ..
                ...
                let nominal_execution = cleanup_actions
                    ...
                    .filter_map(|(action_ordinal, action)| {
                        let terminal_psi::TerminalAffineCleanupAction::InvokeNominal(cleanup) =
                            action
                        else {
                            return None;
                        };
                ...
                match target.architecture {
                    Architecture::X86_64 => {
                        if let Some(control) = &mut x86_floating_control {
                            control.restore_offset = bytes.len();
                            let restore = isa_x86_64::encode_ldmxcsr_rsp_displacement(
                                control.saved_slot_byte_offset,
                            )
                            .map_err(|_| EmissionError::IeeeFloatControlFrameNotEncodable)?;
                            bytes.extend_from_slice(&restore);
                ...
                affine_cleanup = Some(machine_code::UnitAffineCleanupRecord {
                    psi_edge: *psi_edge,
...
fn emit_unit_integer_equality_branch(
    ...
    if scalar_type != i32_type
        || left.scalar_type() != semantic_vocabulary::ScalarType::Integer(i32_type)
        || right.scalar_type() != semantic_vocabulary::ScalarType::Integer(i32_type)
    {
        return Err(EmissionError::InvalidUnitScalarCallCustody(psi_operation));
    }
    ...
    let semantic_vocabulary::IntegerValue::Signed(immediate) = immediate else {
        return Err(EmissionError::InvalidUnitScalarCallCustody(psi_operation));
    };
...
fn affine_scalar_record_bits(
    ...
    let semantic_vocabulary::IntegerValue::Signed(value) = value else {
        return Err(EmissionError::UnsupportedAggregatePlacement);
    };
    ...
    for copy in copies {
        ...
        let Some(home) = homes.iter().find(|home| home.place == copy.place) else {
            ...
            if copy.path.is_empty()
                ...
                && matches!(local.2.shape, terminal_psi::StructuralTypeShape::Record { ref fields } if fields.is_empty())
            {
...
fn validate_dynamic_frame_region(
    ...
    descriptor_abi: assigned_target_operations::AssignedDynamicTraitDescriptorAbi,
...
pub(super) fn unit_scalar_shape(
    ...
    if scalar_type == semantic_vocabulary::ScalarType::Boolean {
        return Ok(ValueShape::integer(1, 1));
    }
    let semantic_vocabulary::ScalarType::Integer(scalar_type) = scalar_type else {
...
mod dynamic_frame_tests {
    ...
    fn dynamic_descriptor_and_result_occupy_distinct_exact_frame_regions() {
        let operation = semantic_vocabulary::OperationId::new(1).unwrap();
        let target = target::NativeTarget::linux_x64();
        let descriptor =
            assigned_target_operations::AssignedDynamicTraitDescriptorAbi::new(0, 8, 8, 16, 8);
        let mut cursor = 5;
    ...
    fn dynamic_frame_rejects_descriptor_result_and_owner_substitution() {
        let operation = semantic_vocabulary::OperationId::new(1).unwrap();
        let other = semantic_vocabulary::OperationId::new(2).unwrap();
        ...
        rejects(
            assigned_target_operations::AssignedDynamicTraitDescriptorAbi::new(0, 16, 8, 16, 8),
            8,
    ...
    fn boolean_home_branch_is_exact_and_rejects_type_or_shape_drift() {
        ...
        let boolean_home = AssignedUnitScalarHome {
            defining_operation: operation,
            source_value: semantic_vocabulary::ValueId::new(2).unwrap(),
        ...
        wrong_type.scalar_type = semantic_vocabulary::ScalarType::Integer(
