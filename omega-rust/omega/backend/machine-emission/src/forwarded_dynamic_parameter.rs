...
use assigned_target_operations::{AssignedFunction, AssignedOperation};
...
pub(super) fn emit(
    ...
    if helper.attachment.is_some()
        ...
        || calling_conventions::validate_call_plan(function_call_plan, &signature).is_err()
        || calling_conventions::validate_call_plan(callee_call_plan, &signature).is_err()
    ...
    Ok(EmittedForwardedDynamicParameterCall {
        unit_affine_cleanup: unit_stack.map(|_| machine_code::UnitAffineCleanupRecord {
            psi_edge,
