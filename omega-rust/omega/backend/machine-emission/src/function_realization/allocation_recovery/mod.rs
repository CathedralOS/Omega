...
>
where
    Source: TryInto<selected_instructions_to_register_homes::RetainedAllocation>,
    selected_instructions_to_register_homes::AllocationReplayError: From<Source::Error>,
{
    let allocation = source.try_into().map_err(|error| match error.into() {
        selected_instructions_to_register_homes::AllocationReplayError::SelectionMismatch => {
