...
fn append_expected_partial_residuals(
    ...
    prefix: &[terminal_psi::StructuralPathSegment],
    moved: &[(
        &[terminal_psi::StructuralPathSegment],
        semantic_vocabulary::StructuralTypeId,
    )],
    ...
    {
        let terminal_psi::StructuralTypeShape::FixedArray { element, length: 2 } =
            declarations.get(&structural_type)?.shape
        else {
            return None;
        };
        let terminal_psi::StructuralTypeShape::FixedArray {
            element: leaf,
        ...
        let moved_by_outer = moved
            ...
            .filter_map(|(path, _)| match path {
                [
                    ...
                    terminal_psi::StructuralPathSegment::FixedIndex(inner),
        ...
        for outer in (0_u64..2).rev() {
            ...
            for inner in (0_u64..inner_length).rev() {
                if inner != moved_inner {
                    output.push((
                        vec![
                            terminal_psi::StructuralPathSegment::FixedIndex(outer),
                            terminal_psi::StructuralPathSegment::FixedIndex(inner),
                        ],
                        leaf,
                    ));
    ...
    if prefix.is_empty()
        && moved
            .iter()
            .all(|(path, _)| matches!(path, [terminal_psi::StructuralPathSegment::FixedIndex(_)]))
    {
        ...
        let moved_indexes = moved
            .iter()
            .filter_map(|(path, _)| match path {
                [terminal_psi::StructuralPathSegment::FixedIndex(index)] if *index < length => {
                    Some(*index)
                }
                _ => None,
            })
        ...
        output.extend(residuals.into_iter().map(|residual| {
            (
                vec![terminal_psi::StructuralPathSegment::FixedIndex(residual)],
                element,
            )
        }));
        return Some(());
    ...
    for field in fields.iter().rev() {
        let matching = moved
            .iter()
            .filter(|(path, _)| {
                matches!(path.first(), Some(terminal_psi::StructuralPathSegment::Field(identity))
        ...
        field_path.push(terminal_psi::StructuralPathSegment::Field(
            field.identity.clone(),
        ));
        let terminal_psi::StructuralFieldType::Structural(field_type) = field.field_type else {
