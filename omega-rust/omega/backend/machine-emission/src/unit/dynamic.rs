...
use assigned_target_operations::{AssignedAggregateCopy, AssignedFunction, AssignedUnitOperation};
use machine_code::{
...
pub(super) fn emit_stored_descriptor(
    ...
    if !stored.has_complete_custody(owner, *psi_operation)
        || source_copy.access == terminal_psi::StructuralAccess::Owned
...
pub(super) fn emit_dynamic_call(
    ...
    if result.is_some_and(|(result, result_home)| {
        !matches!(
            result.scalar_type,
            semantic_vocabulary::ScalarType::Boolean | semantic_vocabulary::ScalarType::Integer(_)
        ) || result.scalar_type != result_home.scalar_type
        ...
        || initial_copy.access == terminal_psi::StructuralAccess::Owned
        || rebound_copy.access == terminal_psi::StructuralAccess::Owned
...
fn target_argument(copy: &AssignedAggregateCopy) -> target_operations::TargetStructuralArgument {
    target_operations::TargetStructuralArgument {
        place: copy.place,
        access: copy.access,
        path: copy.path.clone(),
        root_structural_type: copy.root_structural_type,
