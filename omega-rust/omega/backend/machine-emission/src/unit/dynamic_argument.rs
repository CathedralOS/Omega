...
use target_operations::{CallSiteOwner, TargetDynamicDescriptorInstanceArgument};
...
pub(super) fn emit_forwarded_dynamic_descriptor_call(
    ...
    for (ordinal, argument) in dynamic_arguments.iter().enumerate() {
        ...
        let selection = match &argument.custody.source {
            target_operations::AbstractDynamicDescriptorSource::Selection { selection, .. } => {
                selection
            }
            target_operations::AbstractDynamicDescriptorSource::Rebound { rebound, .. } => rebound,
            target_operations::AbstractDynamicDescriptorSource::Parameter(_) => {
                return Err(invalid());
            }
        };
...
fn build_adapters(
    ...
    let application = match &argument.custody.source {
        target_operations::AbstractDynamicDescriptorSource::Selection { application, .. }
        | target_operations::AbstractDynamicDescriptorSource::Rebound { application, .. } => {
            application
        }
        target_operations::AbstractDynamicDescriptorSource::Parameter(_) => {
            return Err(invalid());
...
fn assigned_result_matches(
    ...
    match (operation, result) {
        (
            ...
            scalar_type.sign() == semantic_vocabulary::IntegerSign::Signed
                && scalar_type.bits() == 32
        }
        (
        ...
        (AssignedOperation::UnitBody(_), terminal_psi::ClosedConformanceCallableResult::Unit) => {
            true
        }
        _ => false,
    }
...
fn emit_adapter(
    ...
    erased_call_plan: calling_conventions::CallPlan,
    realization_call_plan: calling_conventions::CallPlan,
    target: NativeTarget,
...
fn exact_register(placement: &ValuePlacement) -> Option<target_operations::MachineRegister> {
    let [
...
fn emit_x86_table_address(
    bytes: &mut Vec<u8>,
    destination: target_operations::MachineRegister,
