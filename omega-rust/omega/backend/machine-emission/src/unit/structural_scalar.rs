...
pub(super) fn emit_structural_scalar_call(
    ...
    let semantic_vocabulary::ScalarType::Integer(integer_type) = result.scalar_type else {
    ...
    let result_shape = unit_scalar_shape(
        result.value,
...
pub(super) fn emit_structural_result_call(
    ...
    if !exact_shape
        ...
        || retained_callee_result.multiplicity != terminal_psi::StructuralMultiplicity::Affine
...
fn exact_mixed_callee_abi_matches(
    ...
    result_type: semantic_vocabulary::ScalarType,
...
fn assigned_scalar_destination(
...
) -> Option<assigned_target_operations::AssignedCallDestination> {
    match placement.locations.as_slice() {
        [
            ...
            Some(assigned_target_operations::AssignedCallDestination::Register(*register))
        }
        [
...
fn assigned_integer_result_matches(
    operation: &AssignedOperation,
    expected: semantic_vocabulary::IntegerType,
) -> bool {
    match operation {
...
pub(super) fn emit_aarch64_unit_store_register(
    ...
    if source == ADDRESS_REGISTER {
        return Err(EmissionError::UnsupportedUnitRegister(
            target_operations::MachineRegister::Aarch64X(ADDRESS_REGISTER),
        ));
    }
    let mut instructions = Vec::new();
    if home.indirect {
