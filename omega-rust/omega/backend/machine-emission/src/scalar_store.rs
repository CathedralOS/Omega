...
use target_operations::{TargetScalarStructuralFieldStore, TargetStructuralParameter};
...
pub(super) fn emit(
    ...
    let (
        ...
        AssignedOperation::ReturnIntegerExpression {
            ...
            field,
            field_byte_offset,
            semantic_vocabulary::ScalarType::Integer(*return_type),
            source_value == expression_value
    ...
    let prepared_stores = stores
        ...
        .map(|store| match store.immediate {
            ...
            target_operations::TargetScalarImmediate::Integer { scalar_type, value } => Ok((
                store,
    ...
    if !store.destination.is_self
        ...
        || store.destination.access != terminal_psi::StructuralAccess::MutableBorrow
