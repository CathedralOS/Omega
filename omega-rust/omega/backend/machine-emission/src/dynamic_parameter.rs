...
use assigned_target_operations::{AssignedDynamicParameterCallMechanism, AssignedOperation};
use machine_code::{
...
fn emit_aarch64(
    table: target_operations::MachineRegister,
    call_target: target_operations::MachineRegister,
    slot_offset: u32,
) -> Result<
    (
        Vec<u8>,
    ...
    fn operation(target: NativeTarget) -> AssignedOperation {
        ...
        let instance = match target.architecture {
            target::Architecture::X86_64 => {
                if target.object_format == target::ObjectFormat::Coff {
