...
fn assigned_plan(target: NativeTarget) -> assigned_target_operations::AssignedOperationPlan {
...
fn assigned_stored_plan(target: NativeTarget) -> assigned_target_operations::AssignedOperationPlan {
...
fn emits_stored_descriptor_establishment_and_later_reload() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        assert_eq!(
            ...
            machine_code::DynamicTraitDescriptorAbiRecord {
        ...
        .and_then(|function| match &mut function.operation {
            assigned_target_operations::AssignedOperation::UnitBody(body) => body
                ...
                .find_map(|operation| match operation {
                    assigned_target_operations::AssignedUnitOperation::StoreDynamicDescriptor {
                        psi_operation,
        ...
        .and_then(|function| match &mut function.operation {
            assigned_target_operations::AssignedOperation::UnitBody(body) => {
                body.operations.iter_mut().find_map(|operation| {
                    match operation {
                    assigned_target_operations::AssignedUnitOperation::StoredDynamicScalarCall {
                        psi_operation,
                        descriptor_home_byte_offset,
                        ..
                    } => {
...
fn assigned_direct_plan(target: NativeTarget) -> assigned_target_operations::AssignedOperationPlan {
    assigned_scalar_plan_from_source(
        target,
        r#"
...
fn assigned_multi_hop_plan(
    target: NativeTarget,
) -> assigned_target_operations::AssignedOperationPlan {
    assigned_scalar_plan_from_source(
        target,
        r#"
        trait Measure { machine measure(&self) -> i32; }
...
fn assigned_unit_plan(target: NativeTarget) -> assigned_target_operations::AssignedOperationPlan {
    assigned_unit_plan_from_source(
        target,
        r#"
        trait Touch {
...
fn emits_direct_selection_scalar_forwarding_with_a_durable_result_home() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        assert!(matches!(
            ...
            target_operations::AbstractDynamicDescriptorSource::Selection { .. }
        ));
...
fn emits_parameter_sourced_forwarding_as_a_direct_helper_call() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let machine_code::ForwardedDynamicParameterCallStackEvidence::Scalar(call_stack) =
            &call.call_stack
        else {
...
fn emits_parameter_sourced_unit_forwarding_as_a_result_less_direct_call() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        assert_eq!(call.instance, call.instance_destination);
        assert_eq!(call.table, call.table_destination);
        let machine_code::ForwardedDynamicParameterCallStackEvidence::Unit(call_stack) =
            &call.call_stack
        else {
...
fn rejects_parameter_sourced_forwarding_register_drift_before_emission() {
    ...
    let rejected = assigned
        ...
        .iter_mut()
        .find_map(|function| {
            let assigned_target_operations::AssignedOperation::ReturnForwardedDynamicParameterScalarCall {
                psi_operation,
...
fn rejects_parameter_sourced_unit_forwarding_register_drift_before_emission() {
    ...
    let rejected = assigned
        ...
        .iter_mut()
        .find_map(|function| {
            let assigned_target_operations::AssignedOperation::ForwardDynamicParameterUnitCall {
                psi_operation,
                instance_destination,
                table_destination,
                ..
        ...
        for (row_index, adapter) in argument.adapters.iter().enumerate() {
            ...
            assert_eq!(
                ...
                match &argument.custody.source {
                    target_operations::AbstractDynamicDescriptorSource::Rebound {
                        application,
                        ..
