...
use calling_conventions::{ProviderExitRealization, validate_provider_exit_realization};
use executable_installation::{InstalledCode, ResolvedPostHandoffEntryWriterContext};
use layout_plans::{
...
pub struct WrittenExternalRootPostHandoffWriterDestination<'mapping, 'bytes> {
    ...
    written:
        executable_installation::ValidatedWrittenPostHandoffWriterDestination<'mapping, 'bytes>,
}
...
impl PreparedExternalRootPostHandoffWriterInvocation {
    ...
    pub const fn provider_plan_digest(&self) -> effects::provider_plan::ProviderPlanDigest {
    ...
    pub fn selected_boundary_contract_commitment(
        &self,
    ) -> effects::provider_plan::BoundaryCallingPlanCommitment {
    ...
    pub fn execute<'mapping, 'bytes>(
        self,
        installed_code: &InstalledCode,
        destination: executable_installation::ValidatedPreparedPostHandoffWriterDestination<
            'mapping,
            'bytes,
        >,
    ) -> Result<
    ...
    > {
        ...
        if let Err(diagnostic) = self.context.validate_for_destination(
            ...
            return Err(Box::new(PreparedExternalRootWriterExecutionError {
                prepared: self,
                destination,
                diagnostic: layout_plans::MaterializationDiagnostic(diagnostic.0),
            }));
        }
        let Self {
            provider_execution,
        ...
        match installed_code.write_prepared_post_handoff_destination(context, &writer, destination)
        {
            Ok(written) => {
                let written = match written.into_validated_for_consumer(installed_code) {
                    ...
                    Err(error) => {
                        let diagnostic =
                            layout_plans::MaterializationDiagnostic(error.diagnostic().0.clone());
...
impl<'mapping, 'bytes> WrittenExternalRootPostHandoffWriterDestination<'mapping, 'bytes> {
    ...
    pub fn recover_for_retry(
        ...
        (
            PreparedExternalRootPostHandoffWriterInvocation,
            executable_installation::ValidatedPreparedPostHandoffWriterDestination<
                'mapping,
                'bytes,
            >,
        ),
...
fn selected_entry_source_slot(
    invocation: &PostHandoffWriterInvocationPlan,
    selected_entry: EntryStubId,
) -> Result<usize, layout_plans::MaterializationDiagnostic> {
    ...
    if source.source != layout_plans::PostHandoffWriterSource::Resolve(target) {
        return Err(layout_plans::MaterializationDiagnostic(
