...
fn fatal_member(vector: u8, stack_class: u16) -> InterruptTableMemberPlan {
...
fn timer_member(vector: u8, stack_class: u16) -> InterruptTableMemberPlan {
    InterruptTableMemberPlan {
        vector,
        dedicated_stack_class: stack_class,
...
fn table_profile(identity: u64) -> InterruptTableProfile {
    InterruptTableProfile::new(
        profile_id(identity),
        [
            fatal_member(DIVIDE_ERROR, 11),
            fatal_member(GENERAL_PROTECTION, 12),
            fatal_member(PAGE_FAULT, 13),
            timer_member(TIMER_TICK, 14),
        ],
    )
...
fn interrupt_table_profile_rejects_repeated_vector_declarations() {
    let error = InterruptTableProfile::new(
        profile_id(1),
        [fatal_member(PAGE_FAULT, 11), fatal_member(PAGE_FAULT, 12)],
    )
...
fn interrupt_table_profile_rejects_shared_dedicated_stack_classes() {
    let error = InterruptTableProfile::new(
        ...
        [
            fatal_member(DIVIDE_ERROR, 11),
            fatal_member(GENERAL_PROTECTION, 11),
            timer_member(TIMER_TICK, 14),
        ],
    )
