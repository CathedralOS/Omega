...
use crate::package_manager::declarations::dependencies::edit::planning::plan_dependency_addition;
...
fn rejects_dependency_edits_without_an_explicit_project_role() {
    ...
    assert!(matches!(
        ...
        )) if matches!(*error, crate::package_manager::declarations::roles::BuildDeclarationError::MissingBuildDeclaration)
    ));
}
...
fn generated_rows_escape_all_caller_controlled_strings() {
    let request = DependencySourceRequest::Git {
        ...
        selection: PackageSelection::Named(
            crate::package_manager::declarations::PackageName::parse("selected_package").unwrap(),
        ),
    };
