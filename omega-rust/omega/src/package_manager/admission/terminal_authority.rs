...
pub enum AcceptedNativeInput<'input> {
    ...
    Reviewed {
        ...
        candidate: Box<crate::package_manager::review::ReviewedPackageProductionCandidate>,
        optimization_rollback: &'input crate::compiler::OptimizationRollback,
    },
}
...
pub struct AcceptedNativeRealizationRequest<'request> {
    ...
    pub imports: &'request [crate::compiler::SourceEvaluatedImportSettlement<'request>],
}
...
pub fn realize_accepted_native_report(
...
) -> Result<crate::compiler::CompileReport, Vec<Diagnostic>> {
        ...
        match input {
            ...
            AcceptedNativeInput::Reviewed {
                ...
                if checked_root.application_artifact_only() {
                    ...
                    return crate::compiler::CompileReport::from_build_outputs(
                        root_path,
                ...
                let report = crate::compiler::retained_terminal_report_from_checked_package(
                    root_path,
                    checked_root,
                    profile.clone(),
                )?
                ...
                (
                    ...
                    rollback_receipt,
                    crate::compiler::TrustAdmissionSettlement::default(),
                    Some(reviews),
                )
            }
        };
    ...
    let image_request = crate::compiler::native::ExecutableImageEmissionRequest::direct(subsystem);
    let application_identifier = retained_identifier.or_else(|| {
        image_request
            .code_signature_identifier()
            .and_then(|spelling| {
                crate::build_evaluation::ApplicationIdentifier::new(spelling.as_bytes()).ok()
            })
    });
    let artifact = crate::compiler::realize_retained_native_artifact(
        retained,
        crate::compiler::RetainedNativeRealizationRequest {
            profile,
    ...
    let crate::compiler::native::RequestedNativeArtifact::Direct(artifact) = artifact else {
        return Err(diagnostics(
            "accepted Terminal realization requires direct image custody",
        ));
    };
    crate::compiler::CompileReport::from_retained_native_artifact(
        root_path,
        source_file_count,
        artifact,
        rollback_receipt,
...
fn validate_accepted_terminal_production_subject(
    ...
    if report.output_kind() != crate::compiler::CompileOutputKind::TerminalArtifact {
        return Err(diagnostics(
...
fn validate_accepted_package_production_subject(
    subject: &crate::compiler::ProductionCompilationSubject,
    evidence: &AcceptedOrdinaryClosureEvidence,
) -> Result<(), Vec<Diagnostic>> {
    ...
    if accepted_root
        .build_observation()
        .map(crate::build_evaluation::BuildObservationSummary::identity)
