...
fn offline_git_preparation_keeps_exact_pins_and_never_calls_transport() {
    ...
    commit();
    let request =
        crate::package_source::GitSourceRequest::new(locator, Some("HEAD".into())).unwrap();
    let advanced = crate::package_manager::resolution::source::resolve_git_package_source(
        &request,
        &project.storage(),
