...
use crate::package_manager::review::compile_resolved_package_reviews;
use std::fs;
...
fn historical_root_spelling_uses_physical_caller_without_relaxing_strict_api() {
    ...
    assert!(matches!(
        strict,
        Err(crate::package_manager::resolution::graph::ResolveLockedPackageClosureError::RootRequestMismatch)
    ));
    assert!(
