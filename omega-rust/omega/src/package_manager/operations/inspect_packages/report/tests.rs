...
use crate::package_manager::lock::{
    HistoricalPackagePolicyDecisions, HistoricalPackagePolicyLimits,
};
...
fn lock(
    ...
    for package in source.packages() {
        for purpose in crate::package_manager::declarations::DependencyPurpose::ALL {
...
fn benign_execution_context_change_is_visible_without_new_consent() {
    use crate::package_manager::lock::PackageCheckedContext;
...
fn changed_api_is_freshly_reported_without_retaining_old_meaning_or_requiring_approval() {
    ...
    assert_eq!(
        old,
        &crate::package_manager::lock::PackagePolicyAcceptance::from_policy(
            reviews.review(root).unwrap().policy()
        )
        .unwrap()
    );
    assert!(text.contains("requires-review false\n"));
