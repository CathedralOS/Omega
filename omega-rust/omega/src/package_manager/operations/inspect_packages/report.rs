...
use crate::package_source::{GitTransport, ImmutableSourceResolution, SourceLineage};
...
fn checked_context(
    output: &mut Output,
    prefix: &str,
    context: crate::package_manager::lock::PackageCheckedContext,
) -> fmt::Result {
    writeln!(
        output,
...
fn build_snapshot(
    output: &mut Output,
    observation: Option<&crate::build_evaluation::BuildObservationSummary>,
    verbose: bool,
) -> fmt::Result {
    ...
    if verbose {
        ...
        for entry in entries {
            ...
            match entry.kind() {
                crate::build_evaluation::BuildStagedOutputEntryKind::Directory => {
                    writeln!(output, "  sealed-output {path:?} directory")?;
                }
                crate::build_evaluation::BuildStagedOutputEntryKind::File { bytes, executable } => {
                    writeln!(
                        output,
                ...
                crate::build_evaluation::BuildStagedOutputEntryKind::Symlink { target } => {
                    writeln!(
                        output,
