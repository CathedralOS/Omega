...
use crate::package_manager::operations::PackageChangeReview;
...
use crate::package_source::{SourceContentDigest, SourceLineage};
...
fn verify_live_dependencies(review: &PackageChangeReview) -> Result<(), Error> {
    ...
    for custody in closure.custodies() {
        ...
        let current = crate::package_source::local::operations::resolve_local_source(
            path,
            ...
            != &SourceContentDigest::derive(current.content_identity.as_bytes())
        {
            return Err(Error::Source(
                crate::package_source::SourceResolveError::LocalSourceChanged {
