...
use crate::package_manager::operations::{prepare_local_project, review_package_change};
...
"#,
    ...
    let (report, ()) = compile_prepared_local_project_for_native(
        ...
        |checked| {
            observations += 1;
            let generated = crate::checked_interpreter::interpret_entry(
                checked,
...
fn receiving_permission_policy_binds_or_rejects_at_explicit_admission_replay() {
    ...
    let explicit_empty =
        crate::compiler::native::terminal_authority_permission_policy_with_rows(Vec::new())
    ...
    let foreign = crate::compiler::native::terminal_authority_permission_policy_with_rows(vec![
        abstract_operations_to_target_operations::effects::ServiceTerminalAuthorityPermission::new(
            abstract_operations_to_target_operations::effects::provider_plan::ServiceSchemaDigest::from_digest([7; 32]),
