...
use crate::package_manager::operations::{LocalProjectPreparationOptions, prepare_local_project};
use std::collections::BTreeSet;

fn compile_with_receiving_policy(
    project: &TemporaryProject,
    policy: Option<TerminalAuthorityPermissionPolicy>,
) -> Result<crate::compiler::CompileReport, CompilePreparedLocalProjectNativeError> {
...
fn accepted_console_customer_receives_admission_only_under_a_sufficient_policy() {
    ...
    let substituted = receipt
        ...
        .map(|leaf| {
            TerminalAuthorityPermissionPolicyRow::new(
                leaf.service_schema(),
                leaf.requirement_identity().to_owned(),
                abstract_operations_to_target_operations::effects::TerminalAuthorityDisposition::from_classes([]),
            )
        })
        .collect::<Vec<_>>();
    ...
    let forged = terminal_authority_permission_policy_with_rows(vec![
        TerminalAuthorityPermissionPolicyRow::new(
            abstract_operations_to_target_operations::effects::provider_plan::ServiceSchemaDigest::from_digest([0xee; 32]),
    ...
    let widened = receipt
        ...
        .map(|leaf| {
            ...
            classes.push(abstract_operations_to_target_operations::effects::TerminalAuthorityClass::ProcessInput);
            TerminalAuthorityPermissionPolicyRow::new(
                leaf.service_schema(),
                leaf.requirement_identity().to_owned(),
                abstract_operations_to_target_operations::effects::TerminalAuthorityDisposition::from_classes(classes),
            )
        })
        .collect::<Vec<_>>();
    ...
    surplus_rows.push(TerminalAuthorityPermissionPolicyRow::new(
        abstract_operations_to_target_operations::effects::provider_plan::ServiceSchemaDigest::from_digest([0x44; 32]),
        "console::Console::read_byte#surplus",
        abstract_operations_to_target_operations::effects::TerminalAuthorityDisposition::from_classes([
            abstract_operations_to_target_operations::effects::TerminalAuthorityClass::ProcessInput,
        ]),
    ));
