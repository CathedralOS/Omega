...
use crate::package_manager::declarations::dependencies::read::{
    DependencyPurpose, DependencySourceRequest,
};
use crate::package_manager::declarations::{AliasName, PackageKey};
    ...
    ) -> Result<PackageSourceCustody, E>,
{
    ...
    let conflicts = collect_conflicts(&root_key, &observed, &dependencies, &accepted);
...
fn paths_for_origin(
    ...
    match origin {
        ...
        CustodyOrigin::Dependency {
            ...
            let mut requester_paths = paths_to_package(root, requester, dependencies, custodies);
