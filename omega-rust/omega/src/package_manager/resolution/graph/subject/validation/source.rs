...
use crate::package_source::{ImmutableSourceResolution, SourceLineage};

pub(super) fn git_request(
    locator: &str,
    revision: &str,
    budget: &mut Budget,
) -> Result<crate::package_source::GitSourceRequest, CanonicalSourceClosureSubjectError> {
...
pub(super) fn validate_git_selection(
    selection: &crate::package_manager::declarations::PackageSelection,
    ...
    match (selection, navigation) {
        ...
        (
            crate::package_manager::declarations::PackageSelection::Named(package),
            PackageSourceNavigation::Member(_),
        ) if package == selected.key().name() => Ok(()),
        (crate::package_manager::declarations::PackageSelection::Named(package), _)
