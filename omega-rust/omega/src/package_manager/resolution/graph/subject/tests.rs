...
use crate::package_manager::declarations::dependencies::read::DependencyPurpose;
use crate::package_manager::declarations::{AliasName, PackageKey, PackageName};
...
fn root_git_selection(
    ...
    CanonicalRootSourceSelection {
        request: CanonicalRootSourceRequest::Git {
            ...
            selection: crate::package_manager::declarations::PackageSelection::Root,
        },
...
fn readable_source_subject_preserves_named_git_member_requests_and_aliases() {
    ...
    let root_request = CanonicalRootSourceSelection {
        request: CanonicalRootSourceRequest::Git {
            ...
            requested_revision: "main".to_owned(),
            selection: crate::package_manager::declarations::PackageSelection::Named(
                root.key().name().clone(),
            ),
        },
...
fn readable_source_subject_preserves_platform_request_bytes_without_loss() {
    ...
    let identity =
        crate::package_source::WorkspaceLineageIdentity::from_root_source(&workspace_root_source)
            .unwrap();
    let source = ResolvedSourceIdentity::new(
        PackageKey::new(
            PackageName::parse("byte_paths").unwrap(),
            SourceLineage::Workspace(crate::package_source::WorkspaceMemberLineage::new(
                identity,
                member_path.clone(),
            )),
        ),
        ImmutableSourceResolution::workspace(
            crate::package_source::SourceContentDigest::parse_hex(&"1".repeat(64)).unwrap(),
        ),
    )
    .unwrap();
...
fn git_package_selection_is_canonical_request_custody_not_package_identity() {
    ...
    let subject = |selection| {
        let navigations = match &selection {
            ...
            crate::package_manager::declarations::PackageSelection::Named(_) => vec![
    ...
    let root_selected = subject(crate::package_manager::declarations::PackageSelection::Root);
    let named_selected = subject(
        crate::package_manager::declarations::PackageSelection::Named(
