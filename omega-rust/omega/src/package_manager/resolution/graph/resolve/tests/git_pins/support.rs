...
impl Fixture {
    ...
    pub(super) fn subject(
        ...
        let resolver = if application {
            crate::package_manager::resolution::graph::resolve_selected_git_project_closure
        } else {
            crate::package_manager::resolution::graph::resolve_selected_git_package_closure
        };
...
pub(super) fn commit(resolution: &crate::package_source::ImmutableSourceResolution) -> String {
    let crate::package_source::ImmutableSourceResolution::Git { commit, .. } = resolution else {
