...
use crate::package_source::{ExternalSourceContext, SourceLineage, SourceRelativePath};
...
impl SourceCacheLane<'_> {
    ...
    pub(super) fn checked_source_cache_dir(
        ...
        Ok(lane
            .retain_child(crate::package_manager::resolution::source::CHECKED_SOURCE_CACHE_LANE)?
            .path()
            .to_path_buf())
    }
}
...
impl<'a> GitAcquisitionCache<'a> {
    ...
    fn resolve_selected_declared(
        ...
        let resolved = if let Some(expected) = recorded {
            let crate::package_source::ImmutableSourceResolution::Git { commit, tree, .. } =
                expected
            else {
