...
pub fn package_compilation_inputs_for(
    closure: &ResolvedPackageSourceClosure,
    root: &crate::package_manager::declarations::PackageKey,
) -> Result<PackageCompilationInputs, Vec<PackageCompilationInputError>> {
...
pub(crate) struct PackageCompilationScope<'closure> {
    ...
    root: &'closure crate::package_manager::declarations::PackageKey,
    reachable: BTreeSet<crate::package_manager::declarations::PackageKey>,
}
...
impl<'closure> PackageCompilationScope<'closure> {
    ...
    pub(crate) fn root(&self) -> &crate::package_manager::declarations::PackageKey {
        self.root
    }

    pub(crate) fn packages(&self) -> &BTreeSet<crate::package_manager::declarations::PackageKey> {
        &self.reachable
    }
    ...
    pub(crate) fn compilation_inputs(
        ...
        let root_role = if root == closure.graph().root() {
            ...
            crate::package_manager::declarations::BuildDeclarationKind::Package
        };
...
fn binding_with_canonical_source_metadata(
    custody: &crate::package_manager::resolution::source::PackageSourceCustody,
    binding: PackageSourceBinding,
) -> Result<PackageSourceBinding, PackageCompilationInputError> {
    crate::package_source::local::operations::capture_verified_package_source_snapshot(
    ...
    if let Some(directory) = custody.checked_source_cache_dir()
        && let Ok(cache) = crate::package_compilation::CheckedSourceCache::open_or_create(directory)
    {
        return binding
...
fn reachable_package_keys(
    closure: &ResolvedPackageSourceClosure,
    root: &crate::package_manager::declarations::PackageKey,
) -> BTreeSet<crate::package_manager::declarations::PackageKey> {
...
mod tests {
    ...
    #[cfg(unix)]
    use crate::checked_interpreter::CanonicalFilesystemMetadataRowKind;
    ...
    use crate::package_manager::resolution::graph::PackageRootSourceRequest;
    use crate::package_manager::resolution::graph::reconcile::resolve_package_source_closure;
    use crate::package_manager::resolution::source::PackageSourceCustody;
    use crate::package_source::{GitCommitId, GitTreeId, ImmutableSourceResolution, SourceLineage};
    use std::path::PathBuf;
    use std::time::{SystemTime, UNIX_EPOCH};

    fn root_request(root: &PackageSourceCustody) -> PackageRootSourceRequest {
        PackageRootSourceRequest::Git(
            crate::package_manager::resolution::source::GitPackageSourceRequest::root(
                crate::package_source::GitSourceRequest::new(
                    format!(
    ...
    fn custody(
        ...
        custody_with_scopes(
            ...
            marker,
            source_root,
            crate::package_manager::declarations::dependencies::DependencyProjections::from(
                dependency_requests,
            ),
        )
    }
    ...
    fn custody_with_scopes(
        ...
        marker: u8,
        source_root: PathBuf,
        projections: crate::package_manager::declarations::dependencies::DependencyProjections,
    ) -> PackageSourceCustody {
        std::fs::create_dir_all(&source_root).expect("create source root");
        let source_root = source_root
            .canonicalize()
            .expect("retain the canonical synthetic source root");
        let source = crate::package_source::resolve_local_source(
            &source_root,
            crate::package_source::LocalSourceLimits::default(),
        )
        ...
        .expect("resolution");
        let materialization =
            crate::package_manager::resolution::source::PackageSourceMaterialization::from_local(
                &source,
            );
        PackageSourceCustody::from_resolved_parts(
            ...
            materialization,
            source_root,
            crate::package_manager::resolution::source::PackageSourceNavigation::Root,
            crate::package_manager::resolution::source::PackageSourceSelectionEvidence::Root,
            crate::package_source::LocalSourceLimits::default(),
            projections,
        )
    }
    ...
    fn checked_source_cache_replays_the_retained_index_for_an_unchanged_root() {
        ...
        let replay = crate::package_compilation::CheckedSourceCache::open(&cache_dir)
            .expect("open the lane the wiring populated")
            .capture(&canonical_root)
            .expect("capture the unchanged root");
        assert_eq!(
            replay.outcome(),
            crate::package_compilation::CheckedSourceCacheOutcome::Warm
        );
        ...
        let drifted = crate::package_compilation::CheckedSourceCache::open(&cache_dir)
            .expect("reopen the cache lane")
            .capture(&canonical_root)
            .expect("capture the drifted root");
        assert_eq!(
            drifted.outcome(),
            crate::package_compilation::CheckedSourceCacheOutcome::Cold
        );
    ...
    fn build_purpose_edges_do_not_become_product_import_bindings() {
        ...
        let root = custody_with_scopes(
            ...
            1,
            roots.join("root"),
            crate::package_manager::declarations::dependencies::DependencyProjections::new(
                vec![DependencySourceRequest::Path {
                    explicit_alias: None,
