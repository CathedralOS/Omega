...
use crate::package_manager::resolution::source::workspace_path::{
    authored_workspace_member_path, source_relative_path,
};
use crate::package_manager::resolution::source::{
    GitWorkspaceSelectionDeclarations, GitWorkspaceSelectionEvidence,
};
use crate::package_source::{
    GitWorkspaceDeclaration, GitWorkspaceProjectionPlanner, GitWorkspaceSelection,
};

pub(in crate::package_manager::resolution::source::git) struct ManagerGitWorkspacePlanner<'a> {
    selected: &'a PackageName,
}

impl<'a> ManagerGitWorkspacePlanner<'a> {
    pub(in crate::package_manager::resolution::source::git) fn new(
        selected: &'a PackageName,
    ) -> Self {
        Self { selected }
    }
...
impl GitWorkspaceProjectionPlanner for ManagerGitWorkspacePlanner<'_> {
    ...
    ) -> Result<Vec<crate::package_source::SourceRelativePath>, Self::Error> {
    ...
    fn select_member(
        ...
        let supplied = declarations
            ...
            .collect::<Vec<_>>();
        let selected = crate::build_declarations::ProjectName::parse(self.selected.as_str())
