...
use crate::package_compilation::{AcceptedSemanticBinding, AcceptedSemanticBindingRole};
...
use crate::package_manager::declarations::{DependencyPurpose, PackageKey};
                ...
                && review.checked_context().purpose() == DependencyPurpose::Product
            {
                let settled_permission_rows = match role {
                    AcceptedSemanticBindingRole::FilesystemHostService => {
                        crate::compiler::native::filesystem_host_permission_rows(
                            candidate.service_schema(),
                        )
                        .ok()
                    }
                    AcceptedSemanticBindingRole::TimeHostService => {
                        crate::compiler::native::time_host_permission_rows(
                            candidate.service_schema(),
                        )
                        .ok()
                    }
...
pub(super) fn candidate_service_bindings(
    ...
    for (path, role) in HOST_SERVICE_PATHS {
        ...
        if crate::package_compilation::accepted_service_schema_digest(
            binding.role(),
            &service_schema,
        ) != binding.normalized_schema_digest()
        {
...
mod tests {
    ...
    fn binding(package: &PackageKey, marker: u8) -> AcceptedSemanticBinding {
        AcceptedSemanticBinding::new_service(
            ...
            package.identity(),
            "FilesystemHost",
            abstract_operations_to_target_operations::effects::provider_plan::ServiceSchemaDigest::from_digest([marker; 32]),
        )
        .unwrap()
    }
