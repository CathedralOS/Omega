...
use crate::checked_interpreter::{BuildEvaluationSponsor, FilesystemSponsor};
use crate::compiler::compile_to_checked;
use crate::package_compilation::{AcceptedSemanticBinding, PackageCompilationInputError};
use crate::package_evidence::ledger::{ReconstructedPackageReview, reconstruct_package_review};
use crate::package_evidence::record::{
    PackagePolicyRepresentationProducerInstance, PackagePolicyRowKind,
};
...
pub(super) struct CompiledPackageReviews {
    ...
    pub(super) checked_root: Option<Box<crate::compiler::CheckedCompilation>>,
}
...
pub(super) fn compile_dependency_closure(
    ...
    let mut component_candidates: BTreeMap<
        ...
        (std::path::PathBuf, crate::compiler::CheckedCompilation),
    > = BTreeMap::new();
    ...
    let mut discarded_build_usages = Vec::<crate::build_evaluation::BuildEvaluationUsage>::new();
    ...
    for key in dependency_first_package_order(closure) {
        ...
        for &purpose in purposes {
            ...
            let _occurrence = crate::package_manager::review::timings::subject_stage(|| {
            ...
            if discovery == TargetEntryDiscovery::Dependencies
                && inputs.root_role()
                    == crate::package_compilation::BuildDeclarationKind::Application
            {
            ...
            let build_snapshot = if &key == closure.graph().root() {
                root_build_snapshot
                    .cloned()
                    .map(crate::build_evaluation::BuildSnapshotRequest::without_dependency_inputs)
            } else {
            ...
            let request = CheckedCompileRequest {
                ...
                restricted_build_grants: restricted_build_checkpoint.map(|checkpoint| {
                    ...
                    )) as Box<dyn crate::compiler::RestrictedBuildGrants>
                }),
            ...
            let mut component_discovery = crate::compiler::IndependentComponentDiscovery::default();
            let checked = {
...
fn publish_independent_component_descriptions(
    ...
    inputs: crate::package_compilation::PackageCompilationInputs,
    independent_component_selections: &[crate::compiler::IndependentComponentSelection],
    component_candidates: &BTreeMap<
        (semantic_vocabulary::PackageKeyIdentity, DependencyPurpose),
        (std::path::PathBuf, crate::compiler::CheckedCompilation),
    >,
) -> Result<
    Option<crate::package_compilation::PackageCompilationInputs>,
...
> {
    ...
    for selection in independent_component_selections {
        ...
        descriptions.push(
            crate::compiler::published_independent_component_description(
...
struct OccurrenceBuildInputs {
    incoming: crate::package_compilation::BuildDependencyOccurrence,
...
fn reconcile_build_inputs(
    target_closure: &ExactTargetPackageSourceClosure<'_>,
    roster: &PackageOccurrenceRoster,
    request: Option<&crate::build_evaluation::BuildSnapshotRequest>,
) -> Result<Vec<[Option<OccurrenceBuildInputs>; 2]>, Vec<Diagnostic>> {
    use crate::build_evaluation::BuildSnapshotRequest;
    use crate::package_compilation::BuildDependencyOccurrence;
