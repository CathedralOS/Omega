...
use crate::package_manager::lock::{PackageAcceptanceRow, PackageOccurrenceRoster};
use crate::package_manager::resolution::graph::{
    CanonicalSourceClosureSubject, ExactTargetPackageSourceClosure,
};
use crate::package_manager::review::{CompilerIssuedPackageReview, CompilerIssuedPackageReviewSet};
...
pub(super) fn candidate<'a>(
    ...
    if count
        > budget
            .limits
            .maximum_packages
            .saturating_mul(crate::package_manager::declarations::DependencyPurpose::ALL.len())
    ...
    budget.slots::<&crate::package_manager::resolution::source::PackageSourceCustody>(
...
pub(super) fn rows(
    ...
    let (acceptance, usage) =
        crate::package_manager::lock::PackagePolicyAcceptance::project(policy, budget.row_limits())
