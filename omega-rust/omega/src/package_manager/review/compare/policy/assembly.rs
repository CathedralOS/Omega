...
use crate::package_manager::declarations::DependencyPurpose;
use crate::package_manager::lock::{PackageLockTarget, PackageOccurrenceRoster};
use crate::package_manager::resolution::graph::CanonicalSourceClosureSubject;
use crate::package_manager::review::CompilerIssuedPackageReview;
use sha2::Sha256;
...
pub(super) fn packages(
    ...
    while old_index < old_sources.len() || new_index < new_sources.len() {
        ...
        budget
            .slots::<crate::package_manager::lock::PackageCheckedContext>(old_count + new_count)?;
        ...
        for purpose in DependencyPurpose::ALL {
            ...
            budget.slots::<crate::package_manager::lock::PackageAcceptanceRow>(retained.len())?;
            for row in retained {
