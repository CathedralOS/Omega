...
use crate::package_manager::review::ReviewOnlySourceConsumptionCommitment;
use crate::package_manager::review::candidate::PackageReviewEvidence;
use crate::package_source::{ExternalSourceContext, ImmutableSourceResolution, LocalSourceLimits};
...
fn package_changes_join_both_sides_to_the_occurrence_roster() {
    ...
    let baseline_reviews = crate::package_manager::review::compile_resolved_package_reviews(
        &baseline_target,
        &parent.join("baseline-build"),
        crate::package_manager::review::SemanticBindingReview::Discover,
    )
    .expect("compile baseline reviews");
    let initial = crate::package_manager::review::compare_package_policy_changes(
        None,
        &baseline_reviews,
        &baseline_target,
    ...
    let choices = initial
        ...
        .map(|row| crate::package_manager::review::PackagePolicyDecision {
            subject: crate::package_manager::review::PackagePolicyDecisionSubject::Row(row.fingerprint().digest()),
            disposition: crate::package_manager::review::ReviewOnlyRootPolicyDisposition::AcceptCandidateChange,
        })
        .collect::<Vec<_>>();
    let resolution = crate::package_manager::review::resolve_package_policy_decisions(
        &initial,
    ...
    let stable = crate::package_manager::review::compare_package_policy_changes(
        Some(&accepted),
        &baseline_reviews,
        &baseline_target,
        super::PackagePolicyChangeLimits::default(),
    ...
    assert_eq!(
        ...
        &[crate::package_manager::lock::PackageCheckedContext::new(
            crate::package_manager::declarations::dependencies::DependencyPurpose::Product,
            target,
            Some(target)
        )][..]
    );
    ...
    let recontextualized = baselines
        ...
        .collect::<Vec<_>>();
    let different_execution = crate::package_manager::lock::PackageLockTarget::from_policies(
    ...
    assert!(matches!(
        crate::package_manager::review::compare_locked_package_policies(
            &different_execution,
            &baseline_reviews
        ),
        Err(
            crate::package_manager::review::LockedPolicyComparisonError::ExecutionProfileMismatch { .. }
        )
    ));
    let context_changes = crate::package_manager::review::compare_package_policy_changes(
    ...
    let rendered =
        crate::package_manager::review::render_package_policy_review(&context_changes, 1024 * 1024)
    ...
    let candidate_reviews = crate::package_manager::review::compile_resolved_package_reviews(
        &candidate_target,
    ...
    assert_eq!(
        ...
        &[crate::package_manager::lock::PackageCheckedContext::new(
            crate::package_manager::declarations::dependencies::DependencyPurpose::Build,
            target,
            Some(target)
        )][..]
    );
