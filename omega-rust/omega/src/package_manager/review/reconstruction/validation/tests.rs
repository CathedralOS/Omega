...
use crate::package_evidence::ledger::encode_ordinary_package_obligation_ledger;
use crate::package_manager::declarations::dependencies::DependencyPurpose;
use crate::package_manager::lock::{PackageCheckedContext, PackageOccurrenceRoster};
...
use crate::package_source::{ExternalSourceContext, LocalSourceLimits, SourceResolverStorage};
...
fn assert_reference_reachability(subject: &CanonicalSourceClosureSubject) {
    ...
    for source in subject.packages() {
        ...
        let mut actual_edges = actual
            ...
            .collect::<Vec<_>>();
        let order =
            |left: &&crate::package_manager::resolution::graph::CanonicalDependencySourceSelection,
             right: &&crate::package_manager::resolution::graph::CanonicalDependencySourceSelection| {
                left.requester()
                    .cmp(right.requester())
...
fn dual_role_questions_keep_exact_context_and_reject_substitutions() {
    ...
    let composed =
        crate::package_manager::review::LocallyComposedPackageObligationResults::from_resolved_and_reviews(
            &target, &reviews, limits,
        )
        .unwrap();
    ...
    let changes = crate::package_manager::review::compare_package_policy_changes(
        None,
        &reviews,
        &target,
        crate::package_manager::review::PackagePolicyChangeLimits::default(),
    )
    .unwrap();
    assert!(!changes.requires_decision());
    let decisions = crate::package_manager::review::resolve_package_policy_decisions(
        &changes,
        changes.fingerprint().digest(),
        &[],
    )
    .unwrap();
    let history = crate::package_manager::lock::HistoricalPackagePolicyDecisions::capture_policy(
        question.source_closure(),
        &changes,
        &decisions,
        crate::package_manager::lock::HistoricalPackagePolicyLimits::default(),
    )
    .unwrap();
    let policies = question
        ...
        .collect::<Vec<_>>();
    let accepted = crate::package_manager::lock::PackageLockTarget::from_policies(
        question.source_closure().clone(),
        &policies,
        history,
    )
    .unwrap();
    let bound = crate::package_manager::review::bind_fresh_package_root_policy(
        &target,
        &reviews,
        limits,
        crate::package_manager::review::ReviewOnlyCapabilityConflictLimits::default(),
        Some(&accepted),
    )
    .unwrap();
