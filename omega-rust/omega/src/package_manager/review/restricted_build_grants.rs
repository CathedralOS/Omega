...
use crate::package_manager::resolution::graph::{
    DependencyRequestPath, ResolvedPackageSourceClosure,
};
...
impl crate::compiler::RestrictedBuildGrants for CheckpointRestrictedBuildGrants {
    fn admit(
        &mut self,
        request: &crate::build_evaluation::RestrictedBuildRequest,
    ) -> Result<(), Vec<Diagnostic>> {
        let meaning = crate::package_evidence::encoding::restricted_build_request_acceptance_text(
            self.package,
            self.context.target(),
            request,
        )
