...
pub fn verify_wire_schema_codec(
    program: &TypedTrees,
    schema: &symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireSchema,
...
fn evaluate_structured_return(
    program: &TypedTrees,
    entry: BuildMachineEntry<'_>,
    arguments: Vec<crate::checked_interpreter::build_time::BuildTimeValue>,
    operators: &[crate::checked_interpreter::SelectedBuildTimeBinaryOperator],
) -> Result<
    BuildTimeOperationEvaluation<crate::checked_interpreter::build_time::BuildTimeValue>,
    String,
> {
    if operators
        .iter()
...
fn evaluate_observed_arguments(
    ...
    MeasuredBuildMachineEvaluation<Vec<crate::checked_interpreter::build_time::BuildTimeValue>>,
    String,
> {
    std::thread::scope(|scope| {
        std::thread::Builder::new()
                ...
                match options.filesystem {
                    ...
                    FilesystemAccess::RealUnscoped => {
                        evaluator.real_fs = Some(
                            crate::checked_interpreter::interpreter::evaluator::filesystem::real::RealFs::new(None, None)
                                .expect("unscoped filesystem has no grant configuration"),
                        );
                    }
                    FilesystemAccess::RealScoped(grants) => {
                        evaluator.real_fs = Some(crate::checked_interpreter::interpreter::evaluator::filesystem::real::RealFs::new(Some(grants), None).map_err(
                            |message| {
                    ...
                    FilesystemAccess::RealScopedSponsored { grants, sponsor } => {
                        evaluator.real_fs = Some(
                            crate::checked_interpreter::interpreter::evaluator::filesystem::real::RealFs::new(Some(grants), Some(sponsor)).map_err(
                                |message| {
                                    BuildMachineEvaluationFailure::without_evidence(
                                        BuildMachineEvaluationFailureKind::InvalidFilesystemGrant,
                                        message,
...
fn interpret_on_current_thread(
    ...
    if checked
        ...
        .any(|(_, expression)| {
            matches!(
                expression,
                typed_trees_to_checked_trees::checked_trees::expression::ExpressionNode::Atomic(atomic)
                    if matches!(
                        atomic.ordering,
    ...
    match options.filesystem {
        ...
        FilesystemAccess::RealUnscoped => {
            evaluator.real_fs = Some(
                crate::checked_interpreter::interpreter::evaluator::filesystem::real::RealFs::new(
                    None, None,
                )
                .expect("unscoped filesystem has no grant configuration"),
            );
        ...
        FilesystemAccess::RealScoped(grants) => {
            let filesystem = match crate::checked_interpreter::interpreter::evaluator::filesystem::real::RealFs::new(
                Some(grants),
                None,
            ) {
...
fn build_time_result_custody(
    values: &[crate::checked_interpreter::build_time::BuildTimeValue],
) -> Option<(u64, u64)> {
    values
        .iter()
...
mod atomic_fence_tests {
    ...
    use crate::checked_interpreter::InterpretOptions;
    ...
    use typed_trees_to_checked_trees::checked_trees::expression::{
        ExpressionNode, TableAtomicExpression,
    };
