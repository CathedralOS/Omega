use crate::checked_interpreter::interpreter::evaluator::{CheckedTrees, ExpressionHandle};
use crate::checked_interpreter::interpreter::evaluator::{Evaluator, Value};
...
fn canonical() -> CheckedTrees {
    crate::checked_interpreter::front_end::checked_program(&source(
...
fn byte_input_spelling_does_not_reclassify_a_scalar_to_unit_intrinsic() {
    let program = crate::checked_interpreter::front_end::checked_program(
...
fn concrete_byte_input_lookalikes_do_not_gain_host_authority() {
    for (label, declaration, target, callee_blocks) in [
        ...
        let program = crate::checked_interpreter::front_end::checked_program(&source(
            declaration,
            target,
            callee_blocks,
        ));
