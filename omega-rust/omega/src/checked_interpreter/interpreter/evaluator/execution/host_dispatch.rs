...
impl<'program> Evaluator<'program> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn virtual_time_host_value(
        &self,
        target: &str,
    ) -> Option<Value> {
        match target {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn try_host_call(
        &mut self,
        call: &TableCall,
        frame: &mut Frame,
        ...
        match target {
            "exit_process" => {
                ...
                // (crate::language::core::process_exit). An ordinary package or
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn exact_console_intrinsic_host_method(
        &self,
        target_symbol: SymbolHandle,
    ) -> Option<&'static str> {
        let (requirement_symbol, provider_symbol) =
            typed_trees_to_checked_trees::validation::exact_compiler_intrinsic_boundary_requirement(self.program, target_symbol)?;
...
#[cfg(test)]
mod tests {
    use crate::checked_interpreter::interpreter::evaluator::{CheckedTrees, Frame};
    use crate::checked_interpreter::interpreter::evaluator::{Evaluator, Halt, Value};
    use crate::checked_interpreter::value::Cell;
    ...
    fn read_line_call(checked: &CheckedTrees, boundary_trait: &str) -> TableCall {
        ...
        TableCall {
            target: symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier::generated_static("read_line"),
    ...
    fn bounded_read_line_serves_line_read_result_by_signature() {
        let checked =
            crate::checked_interpreter::front_end::checked_program(BOUNDED_CONSOLE_SOURCE);
    ...
    fn direct_console_byte_intrinsics_write_bytes_and_retain_host_backstops() {
        for declaration in [
            ...
            let checked = crate::checked_interpreter::front_end::checked_program(&format!(
                "pub boundary trait Console {{
