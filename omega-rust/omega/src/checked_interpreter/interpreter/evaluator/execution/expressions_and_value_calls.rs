...
impl<'program> Evaluator<'program> {
    pub(in crate::checked_interpreter::interpreter::evaluator) fn eval_expression(
        &mut self,
            ...
            ExpressionNode::Binary(binary) => {
                    ...
                    .copied()
                {
                    ...
                    let domain = match selected.policy {
                        typed_trees_to_checked_trees::checked_trees::CheckedArithmeticPolicyAdapter::None => ArithmeticDomain::Exact,
                        typed_trees_to_checked_trees::checked_trees::CheckedArithmeticPolicyAdapter::FloatSaturatingOverflowOnly { .. } => ArithmeticDomain::Saturating,
                        typed_trees_to_checked_trees::checked_trees::CheckedArithmeticPolicyAdapter::FloatTrappingNonFinite { .. } => ArithmeticDomain::Trapping,
                    };
    ...
    fn eval_case_membership(
        ...
        let Some(owner) = typed_trees_to_checked_trees::validation::exact_case_reference_owner(
            self.program,
            binary.right,
        ) else {
            return trap("case membership requires an exact case classifier");
        ...
        let Some(runtime_case) =
            self.program
                ...
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(
                    variant,
                ) if variant.name.as_str() == variant_name => Some(variant.symbol),
                _ => None,
            }
                })
        else {
    ...
    fn eval_call_expression(
        ...
        if let Some(request) = &call.private_layout_operation {
            ...
            self.private_layout_placements.push(
                crate::checked_interpreter::PrivateLayoutPlacementReceipt {
        ...
        if crate::checked_interpreter::interpreter::evaluator::execution::statements_and_calls::is_statically_harvested_build_declaration(target) {
    ...
    fn try_float_boundary_value_call(
        ...
        let core_format = symbol_resolved_trees_to_typed_trees::typed_trees::operator::resolve_named_expression_call(self.program, call)
            .and_then(|operator| {
                self.program
