...
use crate::checked_interpreter::MAX_BUILD_OUTPUT_OBLIGATIONS;
...
pub(in crate::checked_interpreter::interpreter) enum OutputSealState {
...
impl<'program> Evaluator<'program> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn try_build_output_obligation_value_call(
        &mut self,
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn try_build_output_obligation_statement(
        &mut self,
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn try_required_output_path_value_call(
        &mut self,
        call: &symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableCallExpression,
        frame: &mut Frame,
    ) -> EvalResult<Option<Value>> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn try_required_output_path_statement(
        &mut self,
        call: &symbol_resolved_trees_to_typed_trees::typed_trees::statement::TableCall,
        frame: &Frame,
    ) -> EvalResult<bool> {
    ...
    fn output_facet_root(
        ...
        let root = match &*receiver.borrow() {
            Value::Struct {
                ...
                == crate::checked_interpreter::interpreter::evaluator::build_machine::paths::OUTPUT_ROOT_FACET_TYPE =>
            {
                fields
                    .get("root")
    ...
    fn issue_output_obligation(
        ...
        self.output_obligations
            .push(crate::checked_interpreter::BuildOutputObligation {
                root: output_root,
                relative_path: relative.clone(),
                issued_at: self.filesystem_operation_attempts.len(),
                state: crate::checked_interpreter::BuildOutputObligationState::Pending,
    ...
    fn complete_output_obligation(
        ...
        match self.output_obligations[index].state {
            crate::checked_interpreter::BuildOutputObligationState::Pending => {}
            crate::checked_interpreter::BuildOutputObligationState::Completed { .. } => {
                return Err(Halt::Trap(
                    "required output obligation is already completed".to_owned(),
                ));
            }
            crate::checked_interpreter::BuildOutputObligationState::Failed { .. } => {
                return Err(Halt::Trap(
        ...
        self.output_receipts
            .push(crate::checked_interpreter::BuildOutputReceipt {
                obligation: index,
                root: output_root,
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn note_completed_build_output_attempt(
        &mut self,
        attempt_index: usize,
    ) {
