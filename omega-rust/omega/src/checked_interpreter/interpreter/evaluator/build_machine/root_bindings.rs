...
use symbol_resolved_trees_to_typed_trees::typed_trees::statement::{RootBinding, StatementHandle};
...
impl<'program> Evaluator<'program> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn retain_root_build(
        &mut self,
        state: &State,
        arguments: &[Cell],
    ) -> EvalResult<()> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn execute_root_binding(
        &mut self,
        ...
        self.executed_root_bindings
            .push(crate::checked_interpreter::ExecutedRootBinding {
                statement,
                described,
            });
        Ok(())
