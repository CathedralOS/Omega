...
//! against [`crate::checked_interpreter::FsGrants`] BEFORE the OS is touched -- reads must land
...
fn sponsor_value<T>(
    result: Result<T, crate::checked_interpreter::FilesystemSponsorError>,
) -> EvalResult<T> {
    match result {
        Ok(value) => Ok(value),
        Err(error) => {
            crate::checked_interpreter::interpreter::evaluator::filesystem_sponsor_halt(error)
        }
    }
}
...
fn checked_written_count(written: usize) -> EvalResult<i64> {
    match i64::try_from(written) {
        ...
        Err(_) => crate::checked_interpreter::interpreter::evaluator::filesystem_sponsor_halt(
            crate::checked_interpreter::FilesystemSponsorError::ArithmeticOverflow,
        ),
    }
}
...
fn sponsor_preparation<T>(
    ...
    match result {
        ...
        Err(
            crate::checked_interpreter::FilesystemSponsorError::ParentEntryMissing(_)
            | crate::checked_interpreter::FilesystemSponsorError::ParentIsNotDirectory(_)
            | crate::checked_interpreter::FilesystemSponsorError::EntryAlreadyExists(_)
            | crate::checked_interpreter::FilesystemSponsorError::EntryNotFound(_)
            | crate::checked_interpreter::FilesystemSponsorError::EntryIsNotRegularObject(_)
            | crate::checked_interpreter::FilesystemSponsorError::DirectoryNotEmpty(_)
            | crate::checked_interpreter::FilesystemSponsorError::InvalidDirectoryRename(_),
...
fn unexpected_sponsored_success<T>() -> EvalResult<T> {
    crate::checked_interpreter::interpreter::evaluator::filesystem_sponsor_halt(
        crate::checked_interpreter::FilesystemSponsorError::TransactionNoLongerCurrent,
    )
}
...
/// Canonicalized [`crate::checked_interpreter::FsGrants`]: the roots a scoped run may read/write
...
fn canonical_grants(grants: crate::checked_interpreter::FsGrants) -> Result<Grants, String> {
    let mut identities = BTreeSet::new();
    let mut physical_roots = BTreeMap::<PathBuf, FilesystemGrantRootIdentity>::new();
    let mut canonicalize = |root: crate::checked_interpreter::FilesystemGrantRoot,
                            write: bool|
     -> Result<GrantRoot, String> {
...
struct RealFd {
    file: std::fs::File,
    path: PathBuf,
    sponsor_descriptor: Option<crate::checked_interpreter::FilesystemOpenDescriptor>,
    append: bool,
...
pub(in crate::checked_interpreter::interpreter) struct RealFs {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) errno: i32,
    /// `Some` under `FilesystemAccess::RealScoped`; `None` is unscoped.
    grants: Option<Grants>,
    sponsor: Option<crate::checked_interpreter::FilesystemSponsor>,
}

impl RealFs {
    pub(in crate::checked_interpreter::interpreter) fn new(
        grants: Option<crate::checked_interpreter::FsGrants>,
        sponsor: Option<crate::checked_interpreter::FilesystemSponsor>,
    ) -> Result<Self, String> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn is_scoped(&self) -> bool {
        self.grants.is_some()
    }

    pub(in crate::checked_interpreter::interpreter::evaluator) fn rooted_path_bytes(
        &self,
        identity: FilesystemGrantRootIdentity,
        relative: &[u8],
    ) -> Option<Vec<u8>> {
...
fn real_directory_entries(
    ...
    let (mut name_bytes, mut record_bytes) = match snapshot_kind {
        DirectoryEntrySnapshotKind::PackedRecords => {
            let record_bytes =
                crate::checked_interpreter::interpreter::evaluator::checked_directory_record_snapshot_total(0, 1)?;
            (
                None,
                Some(
                    crate::checked_interpreter::interpreter::evaluator::checked_directory_record_snapshot_total(
                        record_bytes,
                        2,
                    )?,
                ),
        ...
        DirectoryEntrySnapshotKind::FindCursor => {
            let name_bytes =
                crate::checked_interpreter::interpreter::evaluator::checked_directory_name_snapshot_total(0, 1)?;
            (
                Some(
                    crate::checked_interpreter::interpreter::evaluator::checked_directory_name_snapshot_total(
                        name_bytes, 2,
                    )?,
                ),
                None,
    ...
    for dir_entry in listing {
        ...
        let bytes =
            crate::checked_interpreter::interpreter::evaluator::portable_directory_entry_name(
                bytes,
            );
...
impl<'program> crate::checked_interpreter::interpreter::evaluator::Evaluator<'program> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn try_real_filesystem_call(
        &mut self,
    ...
    fn prepare_sponsored_open(
        ...
        truncates: bool,
        wants_write: bool,
    ) -> EvalResult<SponsorPreparation<crate::checked_interpreter::PreparedFilesystemOpen>> {
        ...
        if matches!(
            entry,
            Some(crate::checked_interpreter::FilesystemSponsorEntry::Directory)
        ) && !may_create
            && !truncates
        {
            return Ok(SponsorPreparation::Unsponsored);
    ...
    fn finish_real_open(
        ...
        path: PathBuf,
        append: bool,
        prepared: SponsorPreparation<crate::checked_interpreter::PreparedFilesystemOpen>,
        win32_errors: bool,
    ) -> EvalResult<i64> {
        match opened {
            Ok(file) => {
    ...
    fn commit_sponsored_open(
        &mut self,
        prepared: SponsorPreparation<crate::checked_interpreter::PreparedFilesystemOpen>,
    ) -> EvalResult<Option<crate::checked_interpreter::FilesystemOpenDescriptor>> {
        match prepared {
    ...
    fn prepare_sponsored_close(
        &mut self,
        fd: i32,
    ) -> EvalResult<SponsorPreparation<crate::checked_interpreter::PreparedFilesystemMutation>>
    {
        let real = self.real_fs_mut();
        let Some(sponsor) = real.sponsor.clone() else {
            return Ok(SponsorPreparation::Unsponsored);
    ...
    fn prepare_sponsored_write(
        ...
        Result<SponsorPreparation<crate::checked_interpreter::PreparedFilesystemWrite>, i32>,
    > {
        let real = self.real_fs_mut();
        let Some(sponsor) = real.sponsor.clone() else {
        ...
        let requested_bytes = match u64::try_from(requested_bytes) {
            Ok(bytes) => bytes,
            Err(_) => {
                return crate::checked_interpreter::interpreter::evaluator::filesystem_sponsor_halt(
                    crate::checked_interpreter::FilesystemSponsorError::ArithmeticOverflow,
                );
            }
        };
    ...
    fn commit_sponsored_write(
        &mut self,
        prepared: SponsorPreparation<crate::checked_interpreter::PreparedFilesystemWrite>,
        written: usize,
    ) -> EvalResult<()> {
    ...
    fn sponsor_path(
        ...
        Option<(
            crate::checked_interpreter::FilesystemSponsor,
            crate::checked_interpreter::FilesystemSponsorPath,
        )>,
    > {
        let Some(sponsor) = self
            .real_fs
    ...
    fn commit_sponsored_mutation(
        &mut self,
        prepared: SponsorPreparation<crate::checked_interpreter::PreparedFilesystemMutation>,
    ) -> EvalResult<()> {
        match prepared {
...
fn open_options_for(flags: i32, mode: u32, apply_creation_mode: bool) -> std::fs::OpenOptions {
    ...
    options
        ...
        .append(crate::checked_interpreter::interpreter::evaluator::filesystem::host_open_flags::o_append(flags))
        .truncate(crate::checked_interpreter::interpreter::evaluator::filesystem::host_open_flags::o_trunc(flags))
        .create(crate::checked_interpreter::interpreter::evaluator::filesystem::host_open_flags::o_creat(flags));
    if crate::checked_interpreter::interpreter::evaluator::filesystem::host_open_flags::o_creat(
        flags,
    ) && crate::checked_interpreter::interpreter::evaluator::filesystem::host_open_flags::o_excl(
        flags,
    ) {
