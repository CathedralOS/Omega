...
use crate::checked_interpreter::interpreter::evaluator::{
    EvalResult, FilesystemHostOperation, Halt, trap,
};

pub(in crate::checked_interpreter::interpreter::evaluator) const MAX_FILESYSTEM_TRANSFER_BYTES:
    usize = 16 * 1024 * 1024;

const FILETIME_BYTES: usize = 8;

pub(in crate::checked_interpreter::interpreter::evaluator) const FIND_DATA_OUTPUT_BYTES: usize =
    320;
...
pub(in crate::checked_interpreter::interpreter::evaluator) const STAT_OUTPUT_BYTES: usize =
    crate::checked_interpreter::FILESYSTEM_METADATA_API_CARRIER_BYTES;

const TIMESPEC_PAIR_BYTES: usize = 32;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::checked_interpreter::interpreter::evaluator) enum FilesystemTransferCountError {
    NegativeOrUnrepresentable,
    ExceedsEvaluatorLimit,
}

pub(in crate::checked_interpreter::interpreter::evaluator) fn checked_filesystem_transfer_count(
    raw: i64,
...
pub(in crate::checked_interpreter::interpreter::evaluator) fn synthetic_handle_fd(
    handle: i64,
) -> Option<i32> {
