...
use crate::checked_interpreter::interpreter::evaluator::{EvalResult, PreparedFilesystemCall};

impl<'program> crate::checked_interpreter::interpreter::evaluator::Evaluator<'program> {
    pub(in crate::checked_interpreter::interpreter::evaluator) fn real_read_dir(
        &mut self,
        ...
        Ok({
            ...
            match listed {
                Ok(entries) => {
                    let records =
                        crate::checked_interpreter::interpreter::evaluator::pack_dirent_records(
                            &entries,
                        )?;
                    let start = position.initial.max(0) as usize;
                    let (chunk, next_position) =
                        crate::checked_interpreter::interpreter::evaluator::dirent_record_chunk(
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn real_find_first(
        &mut self,
        call: PreparedFilesystemCall,
    ) -> EvalResult<i64> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn real_find_next(
        &mut self,
        call: PreparedFilesystemCall,
    ) -> EvalResult<i64> {
    ...
    pub(in crate::checked_interpreter::interpreter::evaluator) fn real_find_close(
        &mut self,
        call: PreparedFilesystemCall,
    ) -> EvalResult<i64> {
