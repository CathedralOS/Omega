...
fn test_grant_root(
    identity: u32,
    path: PathBuf,
) -> crate::checked_interpreter::FilesystemGrantRoot {
    crate::checked_interpreter::FilesystemGrantRoot::new(
        crate::checked_interpreter::FilesystemGrantRootIdentity::new(identity)
...
fn grant_root_identities_and_physical_roots_are_unambiguous() {
    ...
    let duplicate_identity = canonical_grants(crate::checked_interpreter::FsGrants {
    ...
    let duplicate_physical = canonical_grants(crate::checked_interpreter::FsGrants {
        read_roots: vec![test_grant_root(1, source.clone())],
...
fn nested_output_root_is_selected_independently_of_grant_order() {
    ...
    let grants = canonical_grants(crate::checked_interpreter::FsGrants {
...
fn ordinary_namespace_preconditions_are_deferred_to_the_host() {
    ...
    let sponsor = crate::checked_interpreter::FilesystemSponsor::new(&directory.0).unwrap();
    let child = sponsor
...
fn resource_halt_teardown_closes_remaining_descriptors() {
    ...
    let sponsor = crate::checked_interpreter::FilesystemSponsor::with_limits(
        &directory.0,
        crate::checked_interpreter::FilesystemSponsorLimits {
            maximum_entries: 1,
    ...
    assert!(matches!(
        ...
        Err(crate::checked_interpreter::interpreter::evaluator::Halt::Resource(_))
    ));
    drop(filesystem);
...
fn canonical_metadata(
    rows: impl IntoIterator<Item = crate::checked_interpreter::CanonicalFilesystemMetadataRow>,
) -> crate::checked_interpreter::CanonicalFilesystemMetadataIndex {
    crate::checked_interpreter::CanonicalFilesystemMetadataIndex::version_1([7; 32], rows).unwrap()
}
...
fn canonical_stat_and_lstat_select_target_and_authored_leaf_rows() {
    ...
    let grants = crate::checked_interpreter::FsGrants {
