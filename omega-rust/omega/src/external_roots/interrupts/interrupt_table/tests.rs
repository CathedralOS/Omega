...
use crate::executable_installation::{ArtifactEntry, InstalledCode};
...
fn fixture_member_admission(
    plan: InterruptTableMemberPlan,
    fixture: &crate::external_roots::tests::InterruptTableMemberFixture,
) -> InterruptTableMemberAdmission {
    member_admission(
        plan,
...
fn member(
    ...
    stack_class: u16,
    acknowledged: bool,
) -> crate::external_roots::tests::InterruptTableMemberFixture {
    crate::external_roots::tests::InterruptTableMemberFixture {
        root_identity,
...
fn member_fixtures() -> Vec<crate::external_roots::tests::InterruptTableMemberFixture> {
    vec![
...
fn member_fixtures_with_spare() -> Vec<crate::external_roots::tests::InterruptTableMemberFixture> {
...
fn table_installed_code(
    ...
    members: &[crate::external_roots::tests::InterruptTableMemberFixture],
) -> InstalledCode {
    let entries = members
        .iter()
        .enumerate()
...
fn install_members<'code>(
    ...
    crate::external_roots::tests::interrupt_table_candidates(code, members)
        .into_iter()
        .enumerate()
        .map(|(index, (root, _boundary))| {
            let authority = RootSlotAuthority::from_admitted_owner(
                crate::external_roots::tests::root_id(
                    0x300 + index as u64,
                    RootSlotId::from_normalized_identity,
                ),
                crate::external_roots::tests::root_id(
                    0x21,
                    RootSlotOwnerId::from_normalized_identity,
                ),
            );
            let execution =
                crate::external_roots::tests::provider_execution_for(&root, 0x400 + index as u64);
...
fn table_destination(seed: u64, base: u64, length: u64) -> Extent {
    crate::external_roots::tests::minted_secondary_processor_state(seed, base, length)
}
...
fn foreign_space_site(seed: u64, base: u64, length: u64) -> Extent {
    let offset = seed * 16;
    terminal_psi::extents::ExtentRootGrant::from_admitted_provider(
        ...
        terminal_psi::extents::ExtentLineageId::from_normalized_identity(seed + 1000)
            .expect("lineage identity"),
        terminal_psi::extents::AddressSpaceId::from_normalized_identity(77)
            .expect("foreign address space"),
        terminal_psi::extents::ExtentRights::from_normalized_identities([
            terminal_psi::extents::ExtentRightId::from_normalized_identity(51)
                .expect("extent right"),
        ]),
        terminal_psi::extents::ExtentProvenanceId::from_normalized_identity(seed + 2000)
            .expect("provenance"),
        terminal_psi::extents::MappingEraId::from_normalized_identity(seed + 3000)
            .expect("mapping era"),
    )
    .mint(base, length)
