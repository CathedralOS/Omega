...
use abstract_operations_to_target_operations::calling_conventions::{
    ArrivalContextId, EntryStack, EntryStackStage,
};
...
fn interrupt_table_member_admission_rejects_non_dedicated_stacks() {
    let stack = EntryStack::Interrupted;
    {
        ...
        let mut candidate =
            crate::external_roots::tests::interrupt_candidate_shaped(entry, &code, 0x101, false);
        let boundary = crate::external_roots::tests::interrupt_boundary_on(stack);
        candidate.stack.realization = crate::external_roots::tests::stack_demand(
            candidate.identity,
            candidate.provider,
        ...
        let authority = RootSlotAuthority::from_admitted_owner(
            crate::external_roots::tests::root_id(0x300, RootSlotId::from_normalized_identity),
            crate::external_roots::tests::root_id(0x21, RootSlotOwnerId::from_normalized_identity),
        );
        let execution = crate::external_roots::tests::provider_execution_for(&validated, 0x400);
        let admission = RootAdmission::from_admitted_provider(
            crate::external_roots::tests::root_id(0x500, RootAdmissionId::from_normalized_identity),
            &validated,
            &execution,
            &code,
            &authority,
                ...
                InterruptTableEstablishedMember {
                    root: if member.vector() == TIMER_TICK {
                        crate::external_roots::tests::root_id(
                            0x777,
                            ExternalRootId::from_normalized_identity,
                        )
                    } else {
                        member.root().root()
                    },
...
fn published_timer_member_enters_and_settles_its_acknowledgement() {
    ...
    let obligations = admitted
        ...
        .begin_interrupt_entry(
            timer.root(),
            crate::external_roots::tests::interrupt_entry_receipt(
                timer.root(),
                90,
                Some(7),
                Some(91),
...
fn closing_the_table_returns_every_member_handle() {
    ...
    for root in roots {
        let receipt = RootRemovalReceipt::from_provider(
            crate::external_roots::tests::root_id(
                0x700,
                RootRemovalReceiptId::from_normalized_identity,
            ),
            &root,
            true,
            true,
...
fn published_table_dispatches_the_timer_arrival_to_its_member_root() {
    ...
    assert_eq!(
        completed.root,
        crate::external_roots::tests::root_id(0x101, ExternalRootId::from_normalized_identity)
    );
}
...
fn published_table_dispatch_rejoins_the_receipt_against_the_armed_member() {
    ...
    let error = table
        .begin_published_interrupt_entry(
            &mut ledger,
            TIMER_TICK,
            crate::external_roots::tests::interrupt_entry_receipt(&spare, 96, Some(7), Some(96)),
        )
...
fn published_fatal_member_preempts_unconditionally_and_halts_on_settle() {
    ...
    let timer_invocation =
        crate::external_roots::tests::root_id(90, InterruptInvocationId::from_normalized_identity);
    ...
    let ordinary = admitted
        ...
        .begin_published_interrupt_entry(
            &mut admitted.ledger,
            TIMER_TICK,
            crate::external_roots::tests::interrupt_entry_receipt_in_context(
                admitted
                    .table
                    .member(TIMER_TICK)
                    .expect("admitted timer member")
    ...
    let fatal_invocation =
        crate::external_roots::tests::root_id(96, InterruptInvocationId::from_normalized_identity);
