...
use crate::executable_installation::{InstalledCode, ResolvedPostHandoffEntryWriterContext};
use abstract_operations_to_target_operations::calling_conventions::{
    ProviderExitRealization, validate_provider_exit_realization,
};
...
pub struct WrittenExternalRootPostHandoffWriterDestination<'mapping, 'bytes> {
    ...
    written: crate::executable_installation::ValidatedWrittenPostHandoffWriterDestination<
        'mapping,
        'bytes,
    >,
}
...
pub struct WrittenExternalRootConsumerValidationError<'mapping, 'bytes> {
    ...
    diagnostic: terminal_psi::layout_plans::MaterializationDiagnostic,
}

impl<'mapping, 'bytes> WrittenExternalRootConsumerValidationError<'mapping, 'bytes> {
    pub const fn diagnostic(&self) -> &terminal_psi::layout_plans::MaterializationDiagnostic {
        &self.diagnostic
    }
...
pub struct PreparedExternalRootWriterExecutionError<'mapping, 'bytes> {
    ...
    destination: crate::executable_installation::ValidatedPreparedPostHandoffWriterDestination<
        'mapping,
        'bytes,
    >,
    diagnostic: terminal_psi::layout_plans::MaterializationDiagnostic,
...
impl<'mapping, 'bytes> PreparedExternalRootWriterExecutionError<'mapping, 'bytes> {
    ...
    pub fn into_parts(
        ...
        crate::executable_installation::ValidatedPreparedPostHandoffWriterDestination<
            'mapping,
            'bytes,
        >,
    ) {
...
impl PreparedExternalRootPostHandoffWriterInvocation {
    pub(crate) fn validate_execution(
    ...
    ) -> Result<(), terminal_psi::layout_plans::MaterializationDiagnostic> {
        ...
        if replayed_invocation != self.invocation {
            return Err(terminal_psi::layout_plans::MaterializationDiagnostic(
    ...
    pub const fn provider_plan_digest(
        &self,
    ...
    pub fn selected_boundary_contract_commitment(
        &self,
    ) -> abstract_operations_to_target_operations::effects::provider_plan::BoundaryCallingPlanCommitment{
    ...
    > {
        ...
        if let Err(diagnostic) = self.context.validate_for_destination(
            ...
            return Err(Box::new(PreparedExternalRootWriterExecutionError {
                prepared: self,
                destination,
                diagnostic: terminal_psi::layout_plans::MaterializationDiagnostic(diagnostic.0),
            }));
        }
        let Self {
            provider_execution,
        ...
        match installed_code.write_prepared_post_handoff_destination(context, &writer, destination)
        {
            Ok(written) => {
                let written = match written.into_validated_for_consumer(installed_code) {
                    Ok(written) => written,
                    Err(error) => {
                        let diagnostic = terminal_psi::layout_plans::MaterializationDiagnostic(
...
impl<'mapping, 'bytes> WrittenExternalRootPostHandoffWriterDestination<'mapping, 'bytes> {
    ...
    pub fn validate_for_consumer(
        ...
        self.written
            ...
            .map_err(|diagnostic| {
                terminal_psi::layout_plans::MaterializationDiagnostic(diagnostic.0)
            })
    }
...
impl<'mapping, 'bytes> ValidatedWrittenExternalRootPostHandoffWriterDestination<'mapping, 'bytes> {
    ...
    pub fn into_parts(
        ...
        crate::executable_installation::ValidatedWrittenPostHandoffWriterDestination<
            'mapping,
            'bytes,
        >,
    ) {
...
fn selected_entry_source_slot(
    invocation: &PostHandoffWriterInvocationPlan,
    selected_entry: EntryStubId,
) -> Result<usize, terminal_psi::layout_plans::MaterializationDiagnostic> {
    ...
    if source.source != terminal_psi::layout_plans::PostHandoffWriterSource::Resolve(target) {
        return Err(terminal_psi::layout_plans::MaterializationDiagnostic(
...
impl installation_evidence::ProviderExecutionEvidence for ProviderExecution {
