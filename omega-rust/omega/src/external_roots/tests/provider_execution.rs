...
use abstract_operations_to_target_operations::calling_conventions::ProviderExitRealization;
use abstract_operations_to_target_operations::calling_conventions::{
    MachineState, MachineStateSet,
};
    ...
    let wrong_control = ProviderExecution::from_admitted_provider(
        ...
        Some(OpaqueProviderExitAssurance::AcceptedClaim {
            realization: ProviderExitRealization {
                control: abstract_operations_to_target_operations::calling_conventions::EntryControl::InterruptReturn,
...
fn provider_execution_prepares_only_its_selected_entry_writer_and_exact_placement() {
    ...
    pre_resolved_writer.steps[0].source =
        terminal_psi::layout_plans::PostHandoffWriterSource::Resolved(0x1010);
    let error = execution
        .prepare_post_handoff_entry_writer(
            selected_plan,
            &code,
