...
use terminal_psi::layout_plans::{EntryStubId, PlacementConstraints, PlacementPhase};
...
fn executable_container_fixture() -> Artifact {
    ...
    let mut decoded = DecodedArtifactContainer {
        format_marker: crate::executable_installation::OMEGA_EXECUTABLE_CONTAINER_MARKER,
        total_length: 1,
        artifact: artifact_id,
        content_fingerprint:
            crate::executable_installation::NonAuthoritativeContainerFingerprint64::from_compatibility_value(1)
                .unwrap(),
        ...
        relocation_set,
        relocations: Vec::new(),
        proof_payload: crate::executable_installation::normalized_proof_payload_digest(b""),
        proof: Vec::new(),
        authority_commitments: Some(authority_commitments),
        sections: Vec::new(),
    };
    decoded.content_fingerprint =
        crate::executable_installation::non_authoritative_decoded_container_fingerprint(&decoded)
