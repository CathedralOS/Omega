...
use tokens_to_syntax_trees::syntax_trees::types::{
    IntegerRangeNormalization, TypeConstraintNode, TypeReferenceNode,
};
...
pub(crate) fn evaluate(
    ...
    for item in syntax.root_items() {
        if let tokens_to_syntax_trees::syntax_trees::item::Item::Machine(machine) = item {
            for state in syntax.items.state_handles(machine.states) {
                ...
                {
                    if let tokens_to_syntax_trees::syntax_trees::statement::StatementNode::Call(
                        call,
    ...
    for reference in ranges {
            ...
            .enumerate()
        {
            ...
            let (Ok(minimum_origins), Ok(maximum_origins)) = (
                crate::build_time_evaluation::const_evaluation::const_generic_expressions::lexical_selection::retain(
    ...
    let _ = crate::build_time_evaluation::const_evaluation::range_endpoints::evaluate_selected_range_endpoints(
        &mut evaluated,
        authority.clone(),
        crate::build_time_evaluation::SelectedBuildTimeOperators {
            operators: &[],
            provider_bodies: &[],
        },
    );
    ...
    let folded_roots: Vec<_> = typed
        ...
        .iter_expressions()
        .filter(|(handle, node)| {
            !matches!(node, symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Integer(_))
                && matches!(
    ...
    for root in &folded_roots {
        ...
        while let Some(expression) = pending.pop() {
            ...
            pending.extend(
                crate::build_time_evaluation::machine_execution::admission::expression_children(
                    &typed, expression,
                ),
            );
        }
    ...
    for (probe_ordinal, (reference, carrier, bounds)) in probes.into_iter().enumerate() {
        ...
        if !crate::build_time_evaluation::const_evaluation::const_generic_expressions::exact_probe_destination(
            &typed, base,
        )
        ...
        {
            let Some(symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeConstraintNode::Range {
                minimum, maximum, ..
            }) = typed
            ...
            {
                if crate::build_time_evaluation::machine_execution::admission::require_closed_integer_argument(
                    &typed,
                    &evaluated,
                    expression,
                    authority.as_deref(),
                ...
                let Ok((origins, _)) =
                    crate::build_time_evaluation::const_evaluation::const_generic_expressions::expression_custody(
                        &typed,
                        machine,
                        state,
                        expression,
    ...
    for (authored, evaluated_root) in folded_calls {
        let value = match evaluated.expression_table.expression(evaluated_root) {
            symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Integer(literal) => literal.value_bignum(),
            _ => None,
        };
...
fn collect_static_argument_types(
    arguments: &[tokens_to_syntax_trees::syntax_trees::expression::StaticMachineArgument],
    pending: &mut Vec<tokens_to_syntax_trees::syntax_trees::types::TypeReferenceHandle>,
) {
