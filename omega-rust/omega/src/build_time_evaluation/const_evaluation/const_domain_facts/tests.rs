...
use crate::build_time_evaluation::{SelectedBuildTimeOperators, SelectedBuildTimeProviderBody};
use symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees;
use symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact;
use symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode;

fn typed(source: &str) -> TypedTrees {
    let syntax = crate::build_time_evaluation::front_end::syntax_program(source);
    ...
    let resolved =
        crate::build_time_evaluation::machine_execution::syntax_probes::resolve(&syntax, None, &[])
            .expect("resolve");
...
fn provider_rows(typed: &TypedTrees) -> Vec<SelectedBuildTimeProviderBody> {
    ...
    facts
        .uses_with_status(typed_trees_to_checked_trees::checked_trees::CheckedOperatorResolutionStatus::Resolved)
        .filter(|fact| {
        ...
        .map(|fact| {
            ...
            SelectedBuildTimeProviderBody {
                ...
                provider: typed_trees_to_checked_trees::checked_trees::CheckedProviderPlanCommitment::from_digest([7; 32]),
            }
        })
        .collect()
}
...
fn selected_provider_body_domain_fact_evaluates_its_ordinary_machine() {
    ...
    crate::build_time_evaluation::validate_selected_provider_bodies(&typed, &rows).unwrap();
