...
use crate::build_time_evaluation::{
    BuildTimeAdmissionPlan, BuildTimeSelectionAuthority, BuildTimeValue,
};
...
impl ConstantCalls for Invocation<'_> {
    ...
    fn validate_call(
        ...
        let entry = self
            ...
            .first()
            .ok_or("range endpoint lost entry")?;
        let custody = crate::build_time_evaluation::BuildTimeInvocationCustody::Source(
    ...
    fn evaluate_call(
        ...
        let value = match (self.result_position(callee)?, result) {
            ...
            (ScalarPosition::Integer(position), result) => {
                let result = crate::build_time_evaluation::const_evaluation::const_lengths::decode_integer_result(
                    self.execution,
                    machine,
                    result,
                )?;
