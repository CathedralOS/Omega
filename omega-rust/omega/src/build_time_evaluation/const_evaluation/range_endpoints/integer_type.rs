...
impl ScalarPosition {
    pub(super) fn prepare(
        ...
        if program
            ...
            && crate::build_time_evaluation::const_evaluation::const_generic_expressions::exact_probe_destination(
                program, reference,
...
impl IntegerPosition {
                ...
                match constraint {
                    TypeConstraintNode::Range {
                        ...
                        for expression in [*minimum, *maximum] {
                            crate::build_time_evaluation::machine_execution::admission::require_closed_integer_argument(
                        ...
                        let minimum =
                            typed_trees_to_checked_trees::validation::closed_integer_range_bound(
                                program, *minimum,
                            )
                            .ok_or("range endpoint type needs a closed minimum")?;
                        let maximum =
                            typed_trees_to_checked_trees::validation::closed_integer_range_maximum(
                                program,
                                *maximum,
                                *end_inclusive,
                            )
    ...
    pub(super) fn require_value(
        ...
        for (symbol, name) in &self.domains {
            match crate::build_time_evaluation::const_evaluation::const_domain_facts::evaluate_closed_membership(
