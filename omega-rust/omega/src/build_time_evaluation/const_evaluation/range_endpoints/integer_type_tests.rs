...
fn closed_range_positions_keep_full_width_and_empty_boundaries() {
    for (parameter, accepted, rejected) in [
        ...
        let program = crate::build_time_evaluation::front_end::typed_program(&format!(
...
fn nominal_trapping_and_invalid_type_handles_do_not_become_integer_positions() {
    ...
    let mut program = crate::build_time_evaluation::front_end::typed_program(
        "machine endpoint(value: u64[0..=8]) {}",
    );
...
fn retained_constant_bound_selection_is_admitted_before_the_body() {
    ...
    impl crate::build_time_evaluation::BuildTimeSelectionAuthority for Selection {
    ...
    for signature in [
        ...
        let retained_source = sources
            ...
            .source_id;
        let mut program = crate::build_time_evaluation::front_end::typed_program_from_source_map(
            sources,
            &[(source_id, &text)],
        );
        let machine = program
...
fn bare_boolean_parameters_are_boolean_positions_and_ranges_stay_integer() {
    let program = crate::build_time_evaluation::front_end::typed_program(
        "machine endpoint(flag: bool, count: u64[0..=8]) {}",
    );
    assert!(matches!(
