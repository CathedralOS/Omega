...
fn inferred_endpoint_calls_use_closed_argument_types_and_reject_conflicts() {
    for argument in ["7u64", "(7u8 as u64)", "identity(7u64)"] {
        let mut program = crate::build_time_evaluation::front_end::typed_program(&format!(
    ...
    for source in [
        ...
        let mut program = crate::build_time_evaluation::front_end::typed_program(source);
...
fn explicit_endpoint_type_stays_fixed_while_other_arguments_are_inferred() {
    ...
    assert_eq!(
        typed_trees_to_checked_trees::validation::closed_integer_range_bound(&program, expression)
            .unwrap()
            .to_string(),
        "7",
    );
...
fn pending_type_bound_keeps_even_an_unused_application_unreplaced() {
    ...
    let outer = pending
        ...
        .expression;
    let prepared =
        crate::build_time_evaluation::PreparedBuildMachineProgram::prepare(&program).unwrap();
    assert!(
...
fn pending_type_bound_blocks_transitive_invocation_before_interpretation() {
    ...
    let wrapper = execution
        ...
        .unwrap();
    let admission = crate::build_time_evaluation::BuildTimeAdmissionPlan::infer(execution, None);
    admission
        .require_common_floor(execution, wrapper)
        .expect_err("a concrete wrapper cannot execute a still-open generic callee");
    for mode in [
        crate::build_time_evaluation::BuildMachineExecutionMode::Pure,
        crate::build_time_evaluation::BuildMachineExecutionMode::Granted {
            filesystem: crate::build_time_evaluation::BuildMachineFilesystemAccess::Virtual,
            filesystem_metadata_layout: Default::default(),
        },
    ] {
        let error = crate::build_time_evaluation::evaluate_build_machine_measured(
            &prepared,
            crate::build_time_evaluation::BuildMachineInvocation {
                product_entry_compatibility: None,
                machine: crate::build_time_evaluation::PreparedBuildMachine::Name("wrapper"),
                arguments: Vec::new(),
                mode,
                sponsor: None,
            },
        ...
        assert!(matches!(
            error,
            crate::build_time_evaluation::BuildMachineEvaluationError::Entry(_)
        ));
        assert!(
            error
                .to_string()
...
fn argument_landing_retains_nested_fractional_warnings() {
    for argument in ["1 / 2 * 512", "1u64 + (1 / 2 * 510)"] {
        ...
        let pending = pending_endpoints(&program).unwrap();
        let admission = crate::build_time_evaluation::BuildTimeAdmissionPlan::infer(&program, None);
        ...
        assert_eq!(
            values,
            vec![crate::build_time_evaluation::BuildTimeValue::Int(256)]
        );
...
fn folded_arguments_still_require_their_original_selection_authority() {
    ...
    for text in [
        ...
        let source_id = sources
            ...
            .source_id;
        let program = crate::build_time_evaluation::front_end::typed_program_from_source_map(
            sources,
            &[(source_id, text)],
        );
        let pending = pending_endpoints(&program).unwrap();
...
fn generic_record_arguments_fold_endpoint_calls_before_synthesis() {
    ...
    for (argument, instance, bound) in [
        ...
        let mut program =
            crate::build_time_evaluation::front_end::typed_program_from_evaluated_syntax_result(
                &syntax, None,
            )
        ...
        let admission =
            crate::build_time_evaluation::BuildTimeAdmissionPlan::infer(&checked.typed, None);
        let execution = admission
            .evaluate_machine_symbol_for_invocation_measured(
                &checked.typed,
                keep.symbol,
                Vec::new(),
                crate::build_time_evaluation::BuildTimeInvocationCustody::Symbol(keep.symbol),
            )
            .unwrap_or_else(|reason| panic!("{argument}: {reason}"));
        assert_eq!(
            *execution.value(),
            crate::build_time_evaluation::BuildTimeValue::Int(bound),
            "{argument}"
        );
    }
}
...
fn boolean_arguments_and_boolean_helpers_fold_into_integer_endpoints() {
    ...
    for (endpoint, bound) in [
        ...
        assert_eq!(
            typed_trees_to_checked_trees::validation::closed_integer_range_bound(&program, maximum)
...
fn pending_type_bound_does_not_hide_the_callees_provider_dependency() {
    ...
    assert!(pending_endpoints(&program).unwrap().is_empty());
    assert_eq!(
        typed_trees_to_checked_trees::validation::closed_integer_range_bound(&program, outer)
            .unwrap()
            .to_string(),
        "7",
...
"#;
    ...
    let (syntax, pre_check) = evaluated.into_syntax_and_pre_check();
    let mut program =
        crate::build_time_evaluation::front_end::typed_program_from_evaluated_syntax(&syntax, None);
...
fn optional_endpoint_preparation_failure_leaves_authored_roots_unchanged() {
    ...
    let roots: Vec<_> = plan
        ...
        .collect();
    let marks = program.pending_const_range_endpoints.clone();
    let Err(errors) = crate::build_time_evaluation::PreparedBuildMachineProgram::prepare(&program)
    else {
    ...
    for (domain, expected) in [
        ...
        let sum = program.expression_table.insert(ExpressionNode::Binary(
            symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableBinaryExpression {
                left,
                operator: symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::Add,
                right,
            },
        ));
        assert_eq!(
