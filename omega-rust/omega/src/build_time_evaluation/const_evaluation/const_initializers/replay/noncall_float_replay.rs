...
fn source_fixture(text: &str) -> TypedTrees {
    ...
    let source_id = sources
        ...
        .source_id;
    let syntax = crate::build_time_evaluation::front_end::syntax_program_with_id(source_id, text);
    let sources = Arc::new(sources);
    let evaluated = super::super::evaluate(syntax, Some(sources.clone()), &[], None)
        .expect("anonymous float declaration evaluates");
    crate::build_time_evaluation::front_end::typed_program_from_evaluated_syntax(
        &evaluated,
        Some(sources),
    )
}
