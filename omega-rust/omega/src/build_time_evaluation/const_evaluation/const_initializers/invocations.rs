...
use crate::build_time_evaluation::const_evaluation::const_generic_expressions::value::{
    self, ConstantCalls,
};
use crate::build_time_evaluation::const_evaluation::const_generic_expressions::{
...
pub(crate) struct CheckedInitializers {
    ...
    authority: Option<Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>>,
    crash_causes: Vec<(
...
impl CheckedInitializers {
    pub(crate) fn prepare(
        ...
        let instances = checked
            ...
            .collect::<Vec<_>>();
        typed_trees_to_checked_trees::validation::validate_checked_machine_specialization_commitments(&checked, &instances)
...
impl Invocation<'_> {
    ...
    ) -> Result<Vec<crate::build_time_evaluation::const_evaluation::const_generic_expressions::DependencyValue>, String>
{
        crate::build_time_evaluation::const_evaluation::const_generic_expressions::validate_retained_initializer_call_custody(
            self.program.typed(),
            self.machine,
            self.state,
            original,
    ...
    fn selected(&self, expression: ExpressionHandle) -> Result<(bool, &Machine, &State), String> {
        let typed = self.program.typed();
        crate::build_time_evaluation::machine_execution::admission::require_call_expression_selection(
            typed,
            expression,
...
impl Invocation<'_> {
    ...
    fn probe_roots(&self) -> Result<Vec<ExpressionHandle>, String> {
        ...
        for statement in typed.statement_table.statements(self.state.statement_nodes) {
            match statement {
                ...
                symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::AssemblyFact(fact) => {
    ...
    fn probe_reference(&self) -> Result<source::SourceSpan, String> {
        let typed = self.program.typed();
        for statement in typed.statement_table.statements(self.state.statement_nodes) {
            if let symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Transition(transition) = statement {
    ...
    pub(super) fn evaluate_leaf(
        &self,
        reference: source::SourceSpan,
        expected_origins: &[tokens_to_syntax_trees::syntax_trees::types::ConstArgumentOrigin],
        ...
        crate::build_time_evaluation::machine_execution::admission::require_const_expression_selection(
            typed,
            self.machine,
            reference,
            self.program.authority.as_deref(),
        ...
        let (origins, operators, calls) =
            crate::build_time_evaluation::const_evaluation::const_generic_expressions::leaf_call_custody(
                typed,
                self.machine,
                self.state,
                &roots,
        ...
        let mut warnings = Vec::new();
        for call in
            crate::build_time_evaluation::const_evaluation::const_generic_expressions::call_expressions(typed, &roots)?
        {
    ...
    pub(crate) fn evaluate_application_probe(
        ...
        let destination = crate::build_time_evaluation::const_evaluation::const_generic_expressions::exact_probe_destination(
            typed,
...
pub(crate) struct ApplicationProbeEvaluation {
    ...
    pub(crate) origins: Vec<tokens_to_syntax_trees::syntax_trees::types::ConstArgumentOrigin>,
...
pub(super) struct LeafEvaluation {
    ...
    pub(super) destination:
        symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceHandle,
    pub(super) origins: Vec<tokens_to_syntax_trees::syntax_trees::types::ConstArgumentOrigin>,
...
pub(super) fn append_probe(
    ...
    let target = program.statement_table.insert_transition_target(
        symbol_resolved_trees_to_typed_trees::typed_trees::statement::TransitionTargetNode::Value(
            expression,
        ),
    );
    let mut state = State {
    ...
    program.statement_table.push_statement(
        ...
        symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Transition(
            symbol_resolved_trees_to_typed_trees::typed_trees::statement::TableTransition {
                target,
                source_span,
                ..Default::default()
            },
