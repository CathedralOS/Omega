...
pub(crate) fn validate(
    ...
    for declaration in typed.const_declarations() {
        ...
        let authored_call = if declaration.authored_initializer.is_valid() {
            crate::build_time_evaluation::const_evaluation::const_generic_expressions::initializer_contains_call(
                typed,
...
fn validate_authored_custody(
    ...
    crate::build_time_evaluation::const_evaluation::const_generic_expressions::validate_authored_initializer_call_custody(
        program,
        machine,
        state,
        declaration.authored_initializer,
...
fn validate_anonymous_float(
    ...
    let scalar_calls = calls.as_ref().map(|calls| {
        calls as &dyn crate::build_time_evaluation::const_evaluation::const_generic_expressions::value::ConstantCalls
    });
    let machine = probe
        .machines()
        .iter()
    ...
    let dependencies = match &calls {
        ...
        None => crate::build_time_evaluation::const_evaluation::const_generic_expressions::validate_retained_initializer_call_custody(
    ...
    while let Some(expression) = pending.pop() {
        ...
        if let ExpressionNode::Binary(binary) = probe.expression_table.expression(expression) {
            if !typed_trees_to_checked_trees::validation::has_builtin_binary_expression_meaning(
                probe,
                machine,
                Some(state),
                expression,
...
fn validate_dependency_values(
    program: &TypedTrees,
    dependencies: &[crate::build_time_evaluation::const_evaluation::const_generic_expressions::DependencyValue],
    mut evaluate: impl FnMut(
        ExpressionHandle,
        PrimitiveType,
    ) -> Result<
        crate::build_time_evaluation::const_evaluation::const_generic_expressions::value::ScalarValue,
        String,
    >,
    ...
    for dependency in dependencies {
            ...
            )
        {
            ...
            for expression in [declaration.authored_initializer, dependency.expression] {
                ...
                if !matches!(
                    value,
                    crate::build_time_evaluation::const_evaluation::const_generic_expressions::value::ScalarValue::Float { .. }
...
fn decode_scalar(
    value: crate::build_time_evaluation::const_evaluation::const_generic_expressions::value::ScalarValue,
) -> Result<DecodedCanonicalConstValue, String> {
    CanonicalConstIdentity {
        type_name: String::new(),
