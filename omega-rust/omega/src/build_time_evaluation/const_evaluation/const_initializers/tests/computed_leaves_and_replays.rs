...
fn retained_invocation_replay_rechecks_concrete_crash_discharge() {
    ...
    let mut typed = crate::build_time_evaluation::front_end::typed_program_from_evaluated_syntax(
        &evaluated,
        Some(sources),
    );
...
fn retained_invocation_replay_rejects_changed_or_erased_computation_and_results() {
    ...
    let typed = crate::build_time_evaluation::front_end::typed_program_from_evaluated_syntax(
        &evaluated,
        Some(sources),
    );
        ...
        match mutation {
            ...
            4 => {
                ...
                let value = match &changed.statement_table.statements(state.statement_nodes)[0] {
                    symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Expression(value) => *value,
                    symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Transition(transition) => {
                        let symbol_resolved_trees_to_typed_trees::typed_trees::statement::TransitionTargetNode::Value(value) =
