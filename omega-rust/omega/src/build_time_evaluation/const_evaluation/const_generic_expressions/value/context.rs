...
impl EvaluationContext<'_> {
    ...
    pub(super) fn arithmetic_result(
        ...
        let reference = reference(program, shape)?
            .and_then(|reference| {
                typed_trees_to_checked_trees::validation::arithmetic_result_type_reference(
                    program, reference,
                )
            })
    ...
    pub(super) fn joined_result(
        ...
        let anonymous = shapes
            ...
            .collect::<Vec<_>>();
        let reference = typed_trees_to_checked_trees::validation::join_result_type_references(
            program,
            &references,
            &anonymous,
        )
    ...
    pub(super) fn require_operator(
        ...
        let builtin = if anonymous {
            symbol_resolved_trees_to_typed_trees::typed_trees::closed_numeric::has_builtin_anonymous_operands(
