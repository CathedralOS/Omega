...
impl MatchSubject {
    pub(super) fn new(
        ...
        if let Value::Anonymous(expression) = value {
            typed_trees_to_checked_trees::validation::evaluate_anonymous_numeric_expression_with_selected_match_arms(
...
pub(super) fn validate_join(
    ...
    if let super::EvaluationContext::Machine(machine, state) = context {
        typed_trees_to_checked_trees::validation::validate_match_dispatch(
            program,
            machine,
            state,
            expression,
...
pub(super) fn validate_anonymous_fragments(
    ...
    while let Some(expression) = pending.pop() {
        if let ExpressionNode::Match(dispatch) = program.expression_table.expression(expression) {
            ...
            if binary.operator == symbol_resolved_trees_to_typed_trees::typed_trees::expression::BinaryOperator::Divide {
