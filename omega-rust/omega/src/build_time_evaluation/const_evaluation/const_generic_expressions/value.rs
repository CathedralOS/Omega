...
fn evaluate_scalar_in(
    ...
    let value = match value {
        ...
        {
            let exact = typed_trees_to_checked_trees::validation::evaluate_anonymous_numeric_expression_with_selected_match_arms(
                program,
                expression,
                &selected_arms,
    ...
    while let Some(step) = pending.pop() {
        match step {
            ...
            Step::MatchNext(expression, ordinal, subject) => {
                ...
                match arm.pattern {
                    symbol_resolved_trees_to_typed_trees::typed_trees::expression::MatchPattern::Wildcard => {
...
fn compare_anonymous(
    ...
    typed_trees_to_checked_trees::validation::evaluate_anonymous_numeric_comparison_with_selected_match_arms(
        program,
        expression,
        selected_arms,
        |operand| context.has_builtin(program, operand),
...
fn land_anonymous(
    ...
    let (literal, warning) =
        typed_trees_to_checked_trees::validation::land_anonymous_integer_expression_with_selected_match_arms(
            program,
            expression,
            destination,
            selected_arms,
