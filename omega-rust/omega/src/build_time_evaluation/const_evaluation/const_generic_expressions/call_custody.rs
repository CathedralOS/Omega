...
use tokens_to_syntax_trees::syntax_trees::{SyntaxTrees, types::ConstArgumentOrigin};
...
impl<'program> Collector<'program> {
    ...
    fn retained_call(
        ...
        occurrence: Option<symbol_resolved_trees_to_typed_trees::typed_trees::AuthoredDeclarationSelectionOccurrenceId>,
    ) -> Result<(), String> {
...
mod tests {
    ...
    fn call_closure_keeps_helper_constants_and_calls_without_reclassifying_body_operators() {
        ...
        let source_id = sources
            ...
            .source_id;
        let (syntax, typed) =
            crate::build_time_evaluation::front_end::typed_program_from_source_map_with_syntax(
                sources,
                &[(source_id, source)],
            );
