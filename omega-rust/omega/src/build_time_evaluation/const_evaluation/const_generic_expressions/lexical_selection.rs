...
use tokens_to_syntax_trees::syntax_trees::types::ConstArgumentOrigin;
...
fn selected_call<'resolved>(
    ...
    authored: &tokens_to_syntax_trees::syntax_trees::expression::TableCallExpression,
) -> Result<&'resolved syntax_trees_to_symbol_resolved_trees::symbol_resolved_trees::expression::TableCallExpression, String>{
    ...
    for (expression, node) in table.iter_expressions() {
        let syntax_trees_to_symbol_resolved_trees::symbol_resolved_trees::expression::ExpressionNode::Call(call) = node else {
            continue;
        };
        ...
        if selected_call.is_some_and(
            |previous: &syntax_trees_to_symbol_resolved_trees::symbol_resolved_trees::expression::TableCallExpression| {
...
fn has_namespace_receiver(
    resolved: &SymbolResolvedTrees,
    call: &syntax_trees_to_symbol_resolved_trees::symbol_resolved_trees::expression::TableCallExpression,
) -> bool {
    if !call.receiver.is_valid() {
        return false;
    }
    let syntax_trees_to_symbol_resolved_trees::symbol_resolved_trees::expression::ExpressionNode::Name(path) =
...
fn selected_constant(
    ...
    for (expression, node) in table.iter_expressions() {
        ...
        if let syntax_trees_to_symbol_resolved_trees::symbol_resolved_trees::expression::ExpressionNode::Name(path) = node {
