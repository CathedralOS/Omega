...
"#
    ...
    let rows = facts
        ...
        .filter_map(|fact| {
            let symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Binary(binary) =
            ...
            Some(SelectedBuildTimeBinaryOperator {
                ...
                provider: typed_trees_to_checked_trees::checked_trees::CheckedProviderPlanCommitment::from_digest([1; 32]),
            })
        })
        .collect();
    (typed, rows)
...
fn selected_operator_crash_fences_cover_admission_and_direct_execution() {
    for contract in ["crashes Trap", "crashes Abort", "crashes Trap false"] {
        ...
        assert!(
            crate::build_time_evaluation::validate_selected_operators(&typed, &rows)
                .unwrap_err()
        ...
        let checked = typed_trees_to_checked_trees::checked_trees::CheckedTrees::with_roots(
            typed,
            typed_trees_to_checked_trees::checked_trees::CheckFacts {
                operators: facts,
                ..Default::default()
            },
        );
        assert!(
            crate::checked_interpreter::interpret_entry(
                &checked,
...
fn operator_crash_fence_ignores_unselected_overload_and_survives_expression_rewrite() {
    ...
    crate::build_time_evaluation::validate_selected_operators(&typed, &rows).unwrap();
...
fn independent_and_selected_lengths_share_full_width_integer_decoding() {
    for (carrier, result) in [("u64", "18446744073709551615"), ("i64", "-1")] {
        ...
        let mut selected = crate::build_time_evaluation::front_end::typed_program(&source);
...
fn named_provider_body_row_rejects_stale_and_substituted_custody() {
    ...
    assert!(
        crate::build_time_evaluation::validate_selected_provider_bodies(&typed, &substituted)
            .is_err(),
    ...
    assert!(
        crate::build_time_evaluation::validate_selected_provider_bodies(&typed, &stale).is_err(),
...
fn provider_body_row_rejects_stale_and_substituted_custody() {
    ...
    assert!(
        crate::build_time_evaluation::validate_selected_provider_bodies(&typed, &invalid_entry)
            .unwrap_err()
    ...
    anonymous[0].provider =
        typed_trees_to_checked_trees::checked_trees::CheckedProviderPlanCommitment::default();
    assert!(
        crate::build_time_evaluation::validate_selected_provider_bodies(&typed, &anonymous)
            .unwrap_err()
    ...
    assert!(
        crate::build_time_evaluation::validate_selected_provider_bodies(&typed, &swapped)
            .unwrap_err()
            .contains("differ")
    );
...
fn folded_payload_retains_its_exact_variant_parent() {
    ...
    let [
        symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(first),
        symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Variant(second),
    ] = members
    else {
        panic!("two cases");
    };
