...
fn evaluate_application_group(
    ...
    crate::build_time_evaluation::machine_execution::syntax_probes::defer_pending_const_qualifications(
        &mut probe,
    ...
    for (ordinal, (_, expression, destination, _)) in application_destinations.iter().enumerate() {
        crate::build_time_evaluation::const_evaluation::const_generic_expressions::append_probe(
            &mut probe,
            ordinal,
            *expression,
            *destination,
    ...
    if !application_destinations.is_empty() {
        ...
        let probe_symbols = typed
            ...
            .collect::<Vec<_>>();
        let checked = crate::build_time_evaluation::const_evaluation::const_initializers::CheckedInitializers::prepare(
            &typed,
    ...
    if !endpoint_leaves.is_empty() {
        ...
        crate::build_time_evaluation::evaluate_const_range_endpoints(
            &mut typed,
            selection_authority.clone(),
        )?;
...
fn detach_probe_value_producers(
    ...
    let mut detached_states: Vec<tokens_to_syntax_trees::syntax_trees::item::StateHandle> =
        Vec::new();
    loop {
    ...
    match syntax.expressions.expression(expression) {
        ...
        ExpressionNode::Match(dispatch) => {
            expression_reaches_pending_machine(
                ...
                .iter()
                .any(|arm| {
                    (matches!(arm.pattern, tokens_to_syntax_trees::syntax_trees::expression::MatchPattern::Value(pattern)
...
fn static_machine_argument_is_pending(
    syntax: &SyntaxTrees,
    argument: &tokens_to_syntax_trees::syntax_trees::expression::StaticMachineArgument,
    arguments: &[TypeReferenceHandle],
    pending_data: &[String],
) -> bool {
