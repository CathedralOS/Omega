...
use symbol_resolved_trees_to_typed_trees::typed_trees::types::{
    FixedArrayLength, TypeReferenceHandle,
};
...
fn evaluate_one(
    ...
    let value = evaluate_zero_argument_machine(
        ...
        machine_name,
        "array length",
        Some(crate::build_time_evaluation::BuildTimeInvocationCustody::Source(source_span)),
    )?;
...
pub(crate) fn decode_integer_result(
    typed: &TypedTrees,
    machine: &Machine,
    value: crate::build_time_evaluation::BuildTimeValue,
) -> Result<BigInt, String> {
    ...
    if let (Some(primitive), crate::build_time_evaluation::BuildTimeValue::Int(value)) =
        (primitive, &value)
...
fn entry_state<'a>(
    typed: &'a TypedTrees,
    machine: &Machine,
) -> Option<&'a symbol_resolved_trees_to_typed_trees::typed_trees::state::State> {
    let leaf = machine
        .name
        .as_str()
        .rsplit("::")
