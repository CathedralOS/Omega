...
fn parse(arguments: &[&str]) -> SyntaxTrees {
    ...
    crate::build_time_evaluation::front_end::syntax_program(&text)
}
...
fn range_argument_observations_execute_declared_call_endpoints() {
    ...
    let syntax = crate::build_time_evaluation::front_end::syntax_program(text);
    ...
    for range in ranges {
        ...
        for constraint in syntax.type_references.constraints(constraints) {
            let tokens_to_syntax_trees::syntax_trees::types::TypeConstraintNode::Range {
                minimum,
                maximum,
                ..
            } = *constraint
        ...
        while let Some(expression) = work.pop() {
            match syntax.expressions.expression(expression) {
                tokens_to_syntax_trees::syntax_trees::expression::ExpressionNode::Binary(
                    binary,
                ) => {
