...
pub fn evaluate_const_range_endpoints(
    ...
    selection_authority: Option<
        std::sync::Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>,
    >,
...
pub(crate) fn pending_endpoint_calls_need_operator_selection(
    ...
    let prepared = crate::build_time_evaluation::PreparedBuildMachineProgram::prepare(typed)?;
...
pub(crate) fn evaluate_selected_range_endpoints(
    ...
    selection_authority: Option<
        std::sync::Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>,
    >,
    selected: crate::build_time_evaluation::SelectedBuildTimeOperators<'_>,
) -> Result<(), Vec<Diagnostic>> {
    let crate::build_time_evaluation::SelectedBuildTimeOperators {
        operators,
        provider_bodies,
    } = selected;
...
fn evaluate_round(
    ...
    selection_authority: Option<
        std::sync::Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>,
    >,
    operators: &[crate::build_time_evaluation::SelectedBuildTimeBinaryOperator],
    provider_bodies: &[crate::build_time_evaluation::SelectedBuildTimeProviderBody],
    ...
    let selected_execution = if provider_bodies.is_empty() {
        ...
        Some(
            crate::build_time_evaluation::machine_execution::selected_operators::apply_selected_provider_bodies(
                typed,
                provider_bodies,
            )
    ...
    let prepared = crate::build_time_evaluation::PreparedBuildMachineProgram::prepare(
        selected_execution.as_ref().unwrap_or(typed),
    )?;
    let execution = prepared.typed();
...
fn evaluate_endpoint(
    ...
    selection_authority: Option<&dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>,
    warnings: &mut Vec<Diagnostic>,
) -> Result<ExpressionNode, String> {
    crate::build_time_evaluation::machine_execution::admission::require_closed_expression_custody(
        execution,
        endpoint.expression,
        selection_authority,
    )?;
    ...
    let (literal, additions) =
        crate::build_time_evaluation::const_evaluation::const_generic_expressions::value::evaluate_integer_endpoint(
            execution,
            endpoint.expression,
            &invocation,
        )?;
        ...
        for constraint in typed.type_reference_table.constraints(constraints) {
            ...
            while let Some((expression, leaving, root)) = work.pop() {
                ...
                if !leaving {
                    ...
                    let children =
                        crate::build_time_evaluation::machine_execution::admission::expression_children(typed, expression);
...
fn append_signature_bounds(
    ...
    for mut reference in std::iter::once(entry.return_type).chain(
        ...
        let mut visited = Vec::new();
        while let symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Constrained {
            base_type,
            constraints,
