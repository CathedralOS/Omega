...
fn provider_application_fixture() -> (
    TypedTrees,
    Vec<crate::build_time_evaluation::SelectedBuildTimeProviderBody>,
) {
...
fn provider_rows(
    ...
    facts
        ...
        .map(|fact| {
            ...
            crate::build_time_evaluation::SelectedBuildTimeProviderBody {
...
fn provider_boundary_const_application_waits_for_selected_execution() {
    ...
    let independent = crate::build_time_evaluation::front_end::typed_program(
        "machine limit() -> u64 { 7 }
         data Buffer<const N: u64> { values: [u8; N]; }
         data Main { value: Buffer<limit()>; }",
    );
...
fn pending_const_application_executes_the_selected_provider_body() {
    ...
    crate::build_time_evaluation::validate_selected_provider_bodies(&program, &rows).unwrap();
...
fn provider_free_const_application_folds_without_selected_rows() {
    ...
    let mut program = crate::build_time_evaluation::front_end::typed_program(
        "machine limit() -> u64 { 7 }
         data Buffer<const N: u64> { values: [u8; N]; }
         data Main { value: Buffer<limit()>; }",
    );
