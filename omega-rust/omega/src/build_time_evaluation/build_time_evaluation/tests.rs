...
fn standalone_request_preserves_the_two_stage_evaluation() {
    let syntax = crate::build_time_evaluation::front_end::syntax_program(CONST_ARRAY_SOURCE);
...
fn request_retains_exact_loader_module_alias_binding() {
    ...
    let declaration = sources
        ...
        .source_id;
    let syntax = crate::build_time_evaluation::front_end::syntax_program_from_texts(&[
    ...
    let unbound = crate::build_time_evaluation::machine_execution::syntax_probes::resolve(
        &syntax,
        Some(sources.clone()),
        &[],
    )
...
fn folded_const_arguments(syntax: &tokens_to_syntax_trees::syntax_trees::SyntaxTrees) -> Vec<i64> {
...
fn selected_provider_const_application_realizes_instance() {
        ...
        typed
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    field,
                ) if field.name.as_str() == "value" => Some(field.type_reference),
                _ => None,
            })
    ...
    let values_field =
        typed
            ...
            .find_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(
                    field,
                ) if field.name.as_str() == "values" => Some(field.type_reference),
                _ => None,
            })
    ...
    match typed.type_reference_table.type_reference(values_field) {
        symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::FixedArray {
            length: symbol_resolved_trees_to_typed_trees::typed_trees::types::FixedArrayLength::Literal(7),
            ..
        } => {}
...
fn const_generic_argument_calls_evaluate_machine_local_destinations() {
    ...
    let machine = syntax
        ...
        .filter_map(|item| match item {
            tokens_to_syntax_trees::syntax_trees::item::Item::Machine(machine) => Some(machine),
            _ => None,
        })
