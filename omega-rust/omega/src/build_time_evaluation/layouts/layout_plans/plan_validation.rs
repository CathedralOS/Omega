...
use terminal_psi::layout_plans::{LayoutFieldEntryReport, LayoutPlacementReport, LayoutPlanReport};
            ...
            "IntegerAt" => {
                    ...
                    .find(|(field, _)| field == "interpretation")
                {
                    Some((_, BuildTimeValue::Case { variant, payload })) if payload.is_empty() => {
                        match case_name(variant).as_str() {
                            "Signed" => terminal_psi::layout_plans::IntegerInterpretation::Signed,
                            "Unsigned" => {
                                terminal_psi::layout_plans::IntegerInterpretation::Unsigned
                            }
                            other => {
                                return Err(fail(format!(
...
fn validate_integer_decode_range(
    ...
    stored_width: u64,
    interpretation: terminal_psi::layout_plans::IntegerInterpretation,
) -> Result<(), String> {
    let primitive = field
        .primitive
    ...
    let carrier_fits = match interpretation {
        terminal_psi::layout_plans::IntegerInterpretation::Signed => {
    ...
    if !carrier_fits {
        return Err(format!(
            ...
            match interpretation {
                terminal_psi::layout_plans::IntegerInterpretation::Signed => "signed",
                terminal_psi::layout_plans::IntegerInterpretation::Unsigned => "unsigned",
            },
            primitive.name()
        ));
    }
