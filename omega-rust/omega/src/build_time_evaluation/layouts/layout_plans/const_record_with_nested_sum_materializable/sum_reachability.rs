...
pub(in crate::build_time_evaluation::layouts::layout_plans) struct SumReachability<'a> {
    typed: &'a TypedTrees,
...
impl<'a> SumReachability<'a> {
    ...
    pub(in crate::build_time_evaluation::layouts::layout_plans) fn new(
        typed: &'a TypedTrees,
    ) -> Self {
        Self {
            typed,
    ...
    pub(in crate::build_time_evaluation::layouts::layout_plans) fn type_contains_sum(
        &mut self,
...
fn symbol_identity(symbol: symbols::SymbolHandle) -> Result<(u32, u32), MaterializationDiagnostic> {
    if !symbol.is_valid() {
        return Err(MaterializationDiagnostic(
...
mod tests {
    ...
    fn reachability_validates_siblings_after_an_already_found_sum() {
        let typed = crate::build_time_evaluation::front_end::typed_program(
            r#"
