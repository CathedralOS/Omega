...
pub(super) enum ClassifiedRecordLevelChild<'a> {
    ...
    Sum(
        &'a symbol_resolved_trees_to_typed_trees::typed_trees::data::DataField,
        &'a DataDefinition,
    ),
...
impl<'a> ClassifiedRecordLevelChild<'a> {
    ...
    fn field(&self) -> &'a symbol_resolved_trees_to_typed_trees::typed_trees::data::DataField {
        match self {
            Self::Sum(field, _)
            | Self::SumArray(field, ..)
...
pub(super) fn materialize_level_bytes(
    outer_layout: &terminal_psi::layout_plans::LayoutPlanReport,
