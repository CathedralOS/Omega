...
use symbol_resolved_trees_to_typed_trees::typed_trees::data::{
    DataDefinition, DataMember, DataShapeKind, DataVariant,
};
...
fn encode_member_into(
    ...
    field_layout: &terminal_psi::layout_plans::ConventionalSumPayloadFieldLayoutReport,
    byte_order: ByteOrder,
    bytes: &mut [u8],
    owner_display: &str,
    member_kind: &str,
...
fn validate_sum_owner(
    ...
    if let Some(application) = data.generic_instance {
        crate::build_time_evaluation::machine_execution::admission::const_evaluable::require_closed_generic_application(
            typed,
            data,
            application,
            "sum",
...
fn validate_selected_payload(
    ...
    declared_common: &[&symbol_resolved_trees_to_typed_trees::typed_trees::data::DataField],
    payload: &[(String, BuildTimeValue)],
