...
use language_semantics::DataSupplyMode;
use symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees;
use terminal_psi::access_plans::PlacementPlanId;
use tokens_to_syntax_trees::syntax_trees::SyntaxTrees;
use tokens_to_syntax_trees::syntax_trees::item::{DataField, DataMember, Item};
use tokens_to_syntax_trees::syntax_trees::types::{TypeReferenceHandle, TypeReferenceNode};
...
pub struct PlacedViewRecord {
    ...
    pub invocation_sources: Vec<source::SourceSpan>,
    policy_source: source::SourceSpan,
    policy_machine_source: source::SourceSpan,
    schema_source: source::SourceSpan,
}
...
struct PolicyMachineCandidate {
    policy: String,
    machine_source: source::SourceSpan,
}
...
struct Application {
    ...
    invocation_sources: Vec<source::SourceSpan>,
}
...
pub fn desugar_placed_views(
    ...
    context: crate::build_time_evaluation::BuildTimeSources<'_>,
) -> Result<Vec<PlacedViewRecord>, Vec<Diagnostic>> {
    let crate::build_time_evaluation::BuildTimeSources {
        sources,
    ...
    let mut probe =
        crate::build_time_evaluation::machine_execution::syntax_probes::normalize_generic_data(
            probe,
            sources.clone(),
            source_scoped_top_level_bindings,
            None,
        )?;
    let probe_plan_laid = crate::build_time_evaluation::desugar_plan_laid_value_types(&mut probe)?;
    let resolved = crate::build_time_evaluation::machine_execution::syntax_probes::resolve(
        &probe,
        sources,
        source_scoped_top_level_bindings,
    )?;
    ...
    crate::build_time_evaluation::evaluate_const_array_lengths(
        &mut typed,
        selection_authority.clone(),
    )?;
    crate::build_time_evaluation::evaluate_const_domain_facts(
        &mut typed,
        selection_authority.clone(),
    )?;
    crate::build_time_evaluation::compute_plan_laid_layouts(
        &mut typed,
        &probe_plan_laid,
        selection_authority.clone(),
    )?;
    ...
    for application in &applications {
        ...
        let plan = crate::build_time_evaluation::compute_placement_plan(
            &typed,
            &policy_machine,
            &application.schema,
        )
...
pub fn validate_placed_view_plans(
    ...
    selection_authority: Option<Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>>,
...
fn admit_policy_invocations(
    typed: &TypedTrees,
    policy_machine: &str,
    invocation_sources: &[source::SourceSpan],
    selection_authority: Option<Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>>,
) -> Result<(), String> {
    ...
    for source in invocation_sources {
        admission.require_common_floor_for_invocation(
            typed,
            machine,
            crate::build_time_evaluation::BuildTimeInvocationCustody::Source(*source),
        )?;
    }
    Ok(())
}
...
fn require_declaration_selection_authority(
    sources: Option<&source::SourceMap>,
    authority: Option<&dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>,
    requester: source::SourceSpan,
    declaration: source::SourceSpan,
    context: &str,
) -> Result<(), String> {
    ...
    if requester_file.origin == source::SourceOrigin::Toolchain
        || declaration_file.origin == source::SourceOrigin::Toolchain
    {
        return Ok(());
    }
...
) -> Option<(String, source::SourceSpan)> {
