...
pub fn compute_plan_laid_layouts(
    ...
    for record in records {
        ...
        let native_report = crate::build_time_evaluation::compute_native_layout_plan(
            typed,
        ...
        let field_symbols = typed
            ...
            .filter_map(|member| match member {
                symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(field) if !field.relevance.is_erased() => {
                    Some(field.symbol)
                }
        ...
        for (field_index, (field_name, field_type)) in schema_fields.iter().enumerate() {
            ...
            match field_entries.as_slice() {
                ...
                {
                    let symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::FixedArray {
                        length: symbol_resolved_trees_to_typed_trees::typed_trees::types::FixedArrayLength::Literal(element_count),
                        ..
...
fn stored_integer_write_is_total(
    ...
    let (admitted_minimum, admitted_maximum) = if let Some(range) =
        typed_trees_to_checked_trees::validation::scalar_representation_range(typed, field_type)
    {
