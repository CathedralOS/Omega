...
use crate::build_time_evaluation::machine_execution::reflection::construct_semantic_schema_graph;
use crate::build_time_evaluation::machine_execution::reflection::schema_graph::{
    SchemaQueryAuthority, SemanticSchemaGraph,
};
use crate::build_time_evaluation::machine_execution::reflection::selection::snapshots::snapshot_revision;
...
fn closed_conformance_row_admits_machine_choice() {
    let typed = crate::build_time_evaluation::front_end::typed_program(CLOSED_PROGRAM);
...
fn attached_conformance_admits_its_machines() {
    let typed = crate::build_time_evaluation::front_end::typed_program(ATTACHED_PROGRAM);
    let graph = graph(&typed, "Board");
...
fn pinned_requirement_accepts_the_exact_row_and_rejects_siblings() {
    let typed = crate::build_time_evaluation::front_end::typed_program(CODEC_PROGRAM);
...
fn demanded_application_accepts_the_pinned_application() {
    let typed = crate::build_time_evaluation::front_end::typed_program(APPLICATION_PROGRAM);
    let graph = graph(&typed, "Player");
    let mut receiver = ScopedSelectionReceiver::new(
        &typed,
        &graph,
