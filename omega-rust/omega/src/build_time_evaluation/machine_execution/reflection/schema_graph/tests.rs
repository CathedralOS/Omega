...
fn record_graph_describes_qualified_members() {
    let typed = crate::build_time_evaluation::front_end::typed_program(
        "data Player { health: u32; speed: f32; }",
    );
...
fn erased_and_empty_members_remain_described() {
    let typed = crate::build_time_evaluation::front_end::typed_program(
        "data Rec { proof [erased]: i32; tag: u8; } data Empty {}",
    );
    let graph =
...
fn foreign_scope_requires_a_public_subject() {
    let typed = crate::build_time_evaluation::front_end::typed_program(
        "data Secret { x: u8; } pub data Open { x: u8; }",
    );
...
fn non_data_subjects_reject() {
    let typed = crate::build_time_evaluation::front_end::typed_program(
        "machine helper() -> u8 { 7 } data Rec { x: u8; }",
    );
    let machine = typed
        .machines()
        .iter()
...
fn replay_rejects_forged_member_identity() {
    let typed =
        crate::build_time_evaluation::front_end::typed_program("data Player { health: u32; }");
    let mut graph =
...
fn replay_rejects_stale_stable_number() {
    let typed =
        crate::build_time_evaluation::front_end::typed_program("data Rec { #3 x: u8; #7 y: u8; }");
    let mut graph =
        construct_semantic_schema_graph(&typed, subject(&typed, "Rec"), owning()).expect("ok");
    let handle = graph.declaration().members[0];
    let SchemaNode::Field(stored) = &mut graph.nodes[handle.index()] else {
...
fn replay_rejects_misowned_payload() {
    let typed = crate::build_time_evaluation::front_end::typed_program(
        "data Outcome { case Ready(value: u32); case Empty; }",
    );
    let mut graph =
...
fn replay_rejects_foreign_scope_over_private_subject() {
    let typed = crate::build_time_evaluation::front_end::typed_program("data Secret { x: u8; }");
