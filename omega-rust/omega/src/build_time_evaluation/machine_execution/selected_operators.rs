...
use crate::build_time_evaluation::SelectedBuildTimeBinaryOperator;
...
pub(crate) fn apply_selected_provider_bodies(
    ...
    for row in selected {
        ...
        *program.expression_table.expression_mut(row.expression) =
            ExpressionNode::Call(symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableCallExpression {
                receiver: ExpressionHandle::invalid(),
                target_symbol: row.provider_state,
                static_machine_parameter: symbols::SymbolHandle::invalid(),
                target: symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier::generated(provider_name),
        ...
        let conformances = program
            ...
            .filter(|conformance| {
                conformance.external_binding.is_none()
                    ...
                    && (symbol_resolved_trees_to_typed_trees::typed_trees::operator::resolve_satisfied_checked_operator(
                        program,
                        provider,
                        namespace.as_str(),
                        requirement.as_str(),
...
pub fn validate_selected_operators(
    ...
    for (index, row) in selected.iter().enumerate() {
        ...
        if symbol_resolved_trees_to_typed_trees::typed_trees::operator::primitive_float_binary_semantics(program, operator)
