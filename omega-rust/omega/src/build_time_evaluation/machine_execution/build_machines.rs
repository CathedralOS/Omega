...
use crate::checked_interpreter::{BuildTimeValue, MeasuredBuildMachineEvaluation};
use std::sync::Arc;
use symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees;
use symbols::SymbolHandle;

pub use crate::checked_interpreter::{
...
pub enum BuildMachineEvaluationError {
    Entry(String),
    Pure(String),
    Granted(crate::checked_interpreter::BuildMachineEvaluationFailure),
}

impl BuildMachineEvaluationError {
    pub fn observations(&self) -> Option<&crate::checked_interpreter::EvaluationObservations> {
        match self {
            Self::Entry(_) => None,
            Self::Pure(_) => None,
...
impl PreparedBuildMachineProgram {
    pub fn prepare(program: &TypedTrees) -> Result<Self, Vec<diagnostics::Diagnostic>> {
        ...
        crate::build_time_evaluation::const_evaluation::range_endpoints::defer_pending_endpoint_calls(&mut typed)?;
...
pub struct BuildMachineInvocation<'a> {
    ...
    pub product_entry_compatibility:
        Option<&'a dyn crate::checked_interpreter::ProductEntryCompatibility>,
}
...
pub fn evaluate_build_machine_measured(
    ...
    let entry = match invocation.machine {
        PreparedBuildMachine::Entry(entry) => {
            crate::checked_interpreter::BuildMachineEntry::Symbol(program.validate_entry(entry)?)
        }
        PreparedBuildMachine::Name(machine_name) => {
            crate::checked_interpreter::BuildMachineEntry::Name(machine_name)
        }
    };
    ...
    let request = crate::checked_interpreter::BuildMachineEvaluationRequest {
        entry,
    ...
    match invocation.mode {
        BuildMachineExecutionMode::Pure => {
            crate::checked_interpreter::evaluate_build_machine_arguments(program.typed(), request)
                .map_err(BuildMachineEvaluationError::Pure)
        }
        BuildMachineExecutionMode::Granted {
            filesystem,
            filesystem_metadata_layout,
        } => crate::checked_interpreter::evaluate_granted_build_machine_arguments(
            program.typed(),
            request,
            crate::checked_interpreter::InterpretOptions {
                filesystem,
                filesystem_metadata_layout,
                ..Default::default()
            },
