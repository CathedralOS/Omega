...
use crate::compiler::{CheckedCompilation, compile_to_checked};
...
use symbol_resolved_trees_to_typed_trees::typed_trees::{
    expression::ExpressionNode, name::Identifier,
};
...
fn value_dispatch_contract_projection_rejects_without_a_canonical_schema() {
    ...
    let arms = checked.typed.expression_table.insert_match_arms([
        symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableMatchArm {
            pattern: symbol_resolved_trees_to_typed_trees::typed_trees::expression::MatchPattern::Wildcard,
            value: subject,
            source_span: Default::default(),
        },
    ]);
    let expression = checked.typed.expression_table.insert(ExpressionNode::Match(
        symbol_resolved_trees_to_typed_trees::typed_trees::expression::TableMatchExpression {
            subject,
            arms,
        },
    ));
    let context = ContractProjectionContext {
        ...
        owner: typed_trees_to_checked_trees::checked_trees::ContractProofFactOwner::Machine { machine_symbol },
        point: typed_trees_to_checked_trees::fact_plan::ProgramPoint::Machine { machine_symbol },
        parameters: &[],
        domain_symbol: None,
        data_symbol: None,
        lifetime_binders: &[],
...
fn original_contract_expressions_use_scoped_lifetime_ordinals() {
    ...
    let project = |outer: &str, local: &str, select_local: bool| {
        ...
        let context = ContractProjectionContext {
            subject_kind: "callable",
            subject_name: "generic_tag",
            owner: typed_trees_to_checked_trees::checked_trees::ContractProofFactOwner::Machine { machine_symbol: machine.symbol },
