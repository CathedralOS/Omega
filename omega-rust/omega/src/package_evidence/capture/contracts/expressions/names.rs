...
pub(crate) fn project_contract_name_expression(
        ...
        })
    {
        ...
        return checked_contract_member_path(
            ...
            typed_trees_to_checked_trees::fact_plan::PlaceRoot::Symbol(
                context.data_symbol.expect("guarded data subject"),
            ),
            members,
        )?
        .into_iter()
    ...
    let semantic_root = is_domain_subject
        ...
        .or_else(|| {
            parameter_position.map(|position| {
                typed_trees_to_checked_trees::fact_plan::PlaceRoot::Symbol(
                    context.parameters[position].symbol,
                )
            })
        })
        .or_else(|| {
            root_symbol.is_valid().then_some(
                typed_trees_to_checked_trees::fact_plan::PlaceRoot::Symbol(root_symbol),
            )
        })
        .ok_or_else(|| {
