...
pub(crate) fn contract_member_path_root(
    ...
    context: &ContractProjectionContext<'_>,
    expression: symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle,
) -> Option<typed_trees_to_checked_trees::fact_plan::PlaceRoot> {
    if let Some((owner, _)) = typed_trees_to_checked_trees::validation::reserved_result_owner(
        &compilation.typed,
        expression,
    ) {
    ...
    if context.data_symbol.is_some_and(|data_symbol| {
        ...
        return context
            .data_symbol
            .map(typed_trees_to_checked_trees::fact_plan::PlaceRoot::Symbol);
    }
    ...
    if let Some(symbol) = resolved {
        return Some(typed_trees_to_checked_trees::fact_plan::PlaceRoot::Symbol(
            symbol,
        ));
    }
    ...
    if context.domain_symbol.is_some() && language_core::is_self_receiver(name.as_str()) {
        return Some(typed_trees_to_checked_trees::fact_plan::PlaceRoot::Expression(expression));
    }
    context
        .parameters
        .iter()
...
pub(crate) fn checked_contract_member_path(
    compilation: &PackageReviewInput<'_>,
    context: &ContractProjectionContext<'_>,
    checked_fact: arena::Handle<
        symbol_resolved_trees_to_typed_trees::typed_trees::domain::ProofFact,
    >,
    expression: symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle,
    root: typed_trees_to_checked_trees::fact_plan::PlaceRoot,
    source_members: &[symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier],
) -> Result<Vec<(Option<SymbolHandle>, SymbolHandle)>, Vec<Diagnostic>> {
    use typed_trees_to_checked_trees::fact_plan::{FactPayload, FactPlace};
...
pub(crate) fn checked_member_segments(
    compilation: &PackageReviewInput<'_>,
    segments: arena::HandleSpan<typed_trees_to_checked_trees::fact_plan::PlaceSegment>,
    source_members: &[symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier],
) -> Option<Vec<(Option<SymbolHandle>, SymbolHandle)>> {
    use typed_trees_to_checked_trees::fact_plan::PlaceSegment;
