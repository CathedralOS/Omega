...
use crate::package_evidence::capture::source::ProjectedReviewRow;
...
pub(super) fn project_callable_conformances(
    ...
    mut policy: Option<&mut Vec<crate::package_evidence::record::PackagePolicyCallableConformance>>,
) -> Result<
    (
...
> {
    ...
    for conformance in compilation.machine_trait_conformances(machine) {
        ...
        let Some(trait_definition) = trait_definition else {
            ...
            if let Some(symbol_resolved_trees_to_typed_trees::typed_trees::machine::SatisfiedDeclaration::TopLevelRequirement(
                requirement,
            )) = symbol_resolved_trees_to_typed_trees::typed_trees::machine::resolve_satisfied_declaration(
                &compilation.typed,
                machine,
                conformance,
            ) {
                ...
                external_executable_supply.push(project_top_level_requirement_external_supply(
                    compilation,
                    machine,
                    conformance,
                    requirement,
            ...
            let operator = if external_operator {
                symbol_resolved_trees_to_typed_trees::typed_trees::operator::resolve_satisfied_boundary_operator_for_conformance(
                    &compilation.typed,
                    machine,
                    conformance,
                )
            ...
            typed_trees_to_checked_trees::validation::validate_checked_operator_realization_contract(
                &compilation.typed,
                machine,
                operator,
            )?;
        ...
        let Some(symbol_resolved_trees_to_typed_trees::typed_trees::machine::SatisfiedDeclaration::Trait {
        ...
        if let Some(policy) = &mut policy {
            policy.push(
                crate::package_evidence::record::PackagePolicyCallableConformance {
