...
pub(super) fn project_top_level_requirement_external_supply(
    ...
    typed_trees_to_checked_trees::validation::revalidate_top_level_requirement_realization(
        &compilation.typed,
        machine,
        requirement,
        conformance,
    ...
    if matches!(binding, PackageReviewExternalBinding::CompilerIntrinsic)
        && crate::provider_planning::IntrinsicRequirement::from_requirement(
            &compilation.typed,
            requirement,
        )
        .is_none()
    ...
    let matches = plans
        ...
        .filter(|(_plan, retained)| {
            retained.provider.schema
                == crate::provider_planning::ProviderSchemaDeclaration::BoundaryRequirement(
                    requirement.symbol,
                )
