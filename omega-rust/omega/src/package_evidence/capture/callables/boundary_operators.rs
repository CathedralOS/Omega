...
pub(super) fn validate_fixed_token_checked_adapter_dispatch_shape(
    ...
    if !supported {
        return Err(vec![Diagnostic::error(format!(
            ...
            machine.name,
            symbol_resolved_trees_to_typed_trees::typed_trees::operator::boundary_operator_requirement_identity(
                &compilation.typed,
                operator,
            ),
            spelling.symbol(),
...
pub(super) fn validate_selected_boundary_operator_checked_adapter(
    ...
    let slot =
        symbol_resolved_trees_to_typed_trees::typed_trees::operator::boundary_operator_requirement_identity(&compilation.typed, operator);
    let matches = plans
        .iter()
        .zip(provenance)
        .filter(|(plan, retained)| {
            plan.schema.trait_name == slot
                && retained.provider.schema
                    == crate::provider_planning::ProviderSchemaDeclaration::BoundaryOperator(
                        operator.symbol,
                    )
        })
        .collect::<Vec<_>>();
