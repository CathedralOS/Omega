...
use crate::package_evidence::capture::semantics::services;
...
fn project_plan(
    ...
    let mut grants = compilation
        ...
        .map(|grant| match grant.grant.selector_kind {
            crate::trust_model::ProviderGrantSelectorKind::PlanName => {
                PackageReviewProviderGrantSelectorKind::PlanName
            }
            crate::trust_model::ProviderGrantSelectorKind::ProviderSlot => {
...
fn normalized_plan_name(
    ...
    let plan = &retained.plan;
    let crate::provider_planning::ProviderSchemaDeclaration::BoundaryOperator(symbol) =
        retained.provider.schema
    else {
        return Ok(plan.name.clone());
    };
    let generated = crate::provider_planning::satisfies_plan_name(
        &plan.target,
    ...
    Ok(crate::provider_planning::satisfies_plan_name(
        &plan.target,
        requirement.path(),
        &plan.provider_type,
    ))
