...
pub(super) fn validate(
    ...
    let independent_components =
        crate::build_evaluation::verify_independent_component_descriptions(
...
fn validate_authored_activation(
    ...
    for retained in provenance {
        if let ProviderSelectionProvenance::BuildOverride(declarations) = &retained.selected_by
            && declarations.iter().any(|declaration| {
                !crate::build_evaluation::validate_executed_provider_selection_declaration(
                    &compilation.typed,
                    declaration,
                )
            })
    ...
    let authored = crate::build_evaluation::harvest_root_grants(&compilation.typed, build)
        .map_err(|diagnostic| vec![diagnostic])?;
    let grants = crate::trust_model::resolve_authored_selected_provider_grants(
...
fn validate_target_defaults(compilation: &PackageReviewInput<'_>) -> Result<(), Vec<Diagnostic>> {
    for retained in compilation.custody.selected_provider_provenance() {
        ...
        for declaration in declarations {
            ...
            let current =
                crate::build_evaluation::harvest_provider_selections(&compilation.typed, machine)?;
