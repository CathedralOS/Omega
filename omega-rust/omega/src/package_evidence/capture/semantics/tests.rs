...
fn generated_symbol_owner(
    ...
    let identities = symbols
        ...
        .map(|source| {
            (
                source.source_id,
                crate::package_compilation::toolchain_source_identity_digest(source).unwrap(),
            )
        })
        .collect::<Vec<_>>();
...
fn toolchain_source_identity(
    ...
    let digest = crate::package_compilation::toolchain_source_identity_digest(source)?;
...
fn retained_toolchain_source_identity_matches_raw_digest_and_requires_exact_source_row() {
    ...
    let digest = crate::package_compilation::toolchain_source_identity_digest(&source).unwrap();
    let retained =
        super::declarations::toolchain_source_identity(&source, &[(source.source_id, digest)])
            .unwrap();
