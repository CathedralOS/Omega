            ...
            let mut projected = projected.into_iter().map(|premise| {
                ...
                let mut establishment_routes = profile.establishment_routes.iter().filter(|route| !matches!(route, language_semantics::DomainEstablishmentRoute::ExactMachine { .. })).map(|route| {
                    Ok(PackagePolicyServiceProgressRoute {
                        kind: match route {
                            language_semantics::DomainEstablishmentRoute::CheckedRequirement { .. } => abstract_operations_to_target_operations::effects::provider_plan::ServiceProgressEstablishmentRouteKind::CheckedRequirement,
                            language_semantics::DomainEstablishmentRoute::BoundaryRequirement { .. } => abstract_operations_to_target_operations::effects::provider_plan::ServiceProgressEstablishmentRouteKind::BoundaryRequirement,
