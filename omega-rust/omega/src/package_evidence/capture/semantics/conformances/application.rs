...
use crate::package_evidence::capture::contracts::expressions::static_arguments::{
...
> {
    ...
    let name = argument.path.last().cloned().unwrap_or_else(|| {
        symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier::generated(
            program.symbols.name(argument.symbol),
        )
    });
    Ok(program
        .type_reference_table
...
pub(crate) fn project_selected_conformance_application(
    ...
    selected: &symbol_resolved_trees_to_typed_trees::typed_trees::expression::StaticMachineArgument,
