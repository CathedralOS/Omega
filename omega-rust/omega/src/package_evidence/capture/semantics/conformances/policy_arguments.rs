    ...
    for argument in arguments {
        ...
        ) && !binders.iter().any(|(symbol, _)| *symbol == argument.symbol)
        {
            ...
            let owner = match nominal.owner {
                crate::package_evidence::record::PackageReviewNominalOwner::Package(package) => {
                    super::super::encoding::canonical_digest_label("package", package.digest())
                }
                crate::package_evidence::record::PackageReviewNominalOwner::ToolchainSource(
                    source,
                ) => super::super::encoding::canonical_digest_label(
                    "toolchain-source",
                    source.digest(),
                ),
                crate::package_evidence::record::PackageReviewNominalOwner::Unresolved => {
