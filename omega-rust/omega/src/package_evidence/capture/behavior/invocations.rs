use super::super::semantics::declarations::nominal_identity;
use crate::package_evidence::capture::PackageReviewInput;
use crate::package_evidence::record::PackageReviewSynchronousInvocation;
use diagnostics::Diagnostic;

pub(crate) fn canonical_checked_invocation_targets(
    compilation: &PackageReviewInput<'_>,
    targets: &[typed_trees_to_checked_trees::flow_effects::InvocationTarget],
) -> Result<Vec<String>, Vec<Diagnostic>> {
    let mut canonical = targets
        .iter()
        .map(|target| match target {
            typed_trees_to_checked_trees::flow_effects::InvocationTarget::Parameter(index) => Ok(format!("parameter:{index}")),
            typed_trees_to_checked_trees::flow_effects::InvocationTarget::Service(symbol) => {
                let matching = compilation
                    .traits()
                    .iter()
                    .filter(|definition| definition.symbol == *symbol)
                    .collect::<Vec<_>>();
                let [definition] = matching.as_slice() else {
                    return Err(vec![Diagnostic::error(format!(
                        "reviewed synchronous invocation resolves service symbol {} to {} declarations; expected exactly one",
                        symbol.arena_index(),
                        matching.len(),
                    ))]);
                };
                if !definition.is_boundary {
                    return Err(vec![Diagnostic::error(format!(
                        "reviewed synchronous invocation resolves `{}` to a non-boundary trait",
                        definition.name,
                    ))]);
                }
                Ok(format!("service:{}", definition.name))
            }
        })
        .collect::<Result<Vec<_>, Vec<Diagnostic>>>()?;
    canonical.sort();
    canonical.dedup();
    Ok(canonical)
}

pub(crate) fn project_synchronous_invocations(
    compilation: &PackageReviewInput<'_>,
    invocations: &[typed_trees_to_checked_trees::flow_effects::InvocationTarget],
) -> Result<Vec<PackageReviewSynchronousInvocation>, Vec<Diagnostic>> {
    let mut projected = invocations
        .iter()
        .copied()
        .map(|invocation| match invocation {
            typed_trees_to_checked_trees::flow_effects::InvocationTarget::Parameter(position) => {
                Ok(PackageReviewSynchronousInvocation::Parameter(position))
            }
            typed_trees_to_checked_trees::flow_effects::InvocationTarget::Service(symbol) => Ok(
                PackageReviewSynchronousInvocation::Service(nominal_identity(compilation, symbol)?),
            ),
        })
        .collect::<Result<Vec<_>, Vec<Diagnostic>>>()?;
    projected.sort();
    projected.dedup();
    Ok(projected)
}
