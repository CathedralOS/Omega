...
use crate::package_evidence::capture::contracts::{
...
use typed_trees_to_checked_trees::checked_trees::RealizedMachineContractEnvelope;
...
pub(crate) fn crash(
    ...
    inferred_causes: &[(
        SymbolHandle,
        Vec<typed_trees_to_checked_trees::checked_trees::CrashCause>,
    )],
    ...
    let context = ContractProjectionContext {
        ...
        owner: typed_trees_to_checked_trees::checked_trees::ContractProofFactOwner::Machine {
            machine_symbol: machine.symbol,
        },
        point: typed_trees_to_checked_trees::fact_plan::ProgramPoint::Machine {
            machine_symbol: machine.symbol,
        },
    ...
    Ok(PackagePolicyCrash {
        interface: match plan.crash.interface() {
            typed_trees_to_checked_trees::checked_trees::CrashInterface::InternalInferred => {
        ...
        structural_runtime_requirements: plan
            ...
            .map(|requirements| {
                requirements
                    .iter()
                    .map(crate::package_evidence::capture::behavior::crash::project_boolean_expression)
                ...
                causes: causes
                    ...
                    .map(|cause| match cause {
                        typed_trees_to_checked_trees::checked_trees::CrashCause::Trap => {
                            crate::package_evidence::record::PackageReviewCrashCause::Trap
                        }
                        typed_trees_to_checked_trees::checked_trees::CrashCause::Abort => {
                            crate::package_evidence::record::PackageReviewCrashCause::Abort
                        }
                    })
                    .collect(),
            }
...
pub(crate) fn crash_routes(
    ...
    for bucket in buckets {
        let cause = match bucket.cause() {
            typed_trees_to_checked_trees::checked_trees::CrashCause::Trap => CrashCause::Trap,
            typed_trees_to_checked_trees::checked_trees::CrashCause::Abort => CrashCause::Abort,
        };
        let mut guards = Vec::new();
        if bucket.alternative_guards()
        ...
        published.push(PackagePolicyCrashRoute {
            cause: crate::package_evidence::capture::behavior::project_crash_cause(bucket.cause()),
