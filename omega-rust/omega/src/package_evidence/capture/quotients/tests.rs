use crate::package_evidence::capture::quotients::project_replayed_batch;
use crate::package_evidence::project_non_executable_quotient_package_review;
...
use crate::language::core::relation;
...
fn total_direct_define_projects_one_deterministic_recoverable_review_row() {
    ...
    assert!(
        typed_trees_to_checked_trees::validation::validate_program(&program).is_err(),
    ...
    assert_eq!(
        row.kind(),
        crate::package_evidence::record::PackageReviewCanonicalRowKind::NonExecutableQuotientCorrespondence
    );
    assert_eq!(
        row.source()
            .authored_locations()
    ...
    let recovery = crate::package_evidence::encoding::encode_package_review_canonical_row(row)
        .expect("encode recovery envelope");
    let decoded = crate::package_evidence::encoding::decode_package_review_canonical_row(&recovery)
