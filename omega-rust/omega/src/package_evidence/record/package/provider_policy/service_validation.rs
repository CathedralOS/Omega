    ...
    pub(crate) fn validate_service_structure(
        ...
        for premise in &self.termination_premises {
            if premise.profile.is_empty()
                || matches!(premise.subject, abstract_operations_to_target_operations::effects::provider_plan::ServiceProgressSubject::Parameter(ordinal) if ordinal >= self.parameter_count)
                || premise
