...
pub(in crate::package_evidence::record) type Result = std::result::Result<(), &'static str>;

#[derive(Clone, Copy)]
pub(in crate::package_evidence::record) enum BinderKind<'a> {
    Type,
    Const,
    Machine,
...
#[derive(Clone, Copy)]
pub(in crate::package_evidence::record) struct Scope<'a> {
...
pub(in crate::package_evidence::record) fn text(value: &str) -> Result {
    if value.is_empty() {
...
pub(in crate::package_evidence::record) fn value_type(value: &PackageReviewTypeIdentity) -> Result {
    text(&value.canonical)
}
pub(in crate::package_evidence::record) fn nominal(value: &PackageReviewNominalIdentity) -> Result {
    text(&value.path)?;
...
pub(in crate::package_evidence::record) fn owned_pair(
...
pub(in crate::package_evidence::record) fn operator(
    value: &PackageReviewOperatorCoordinate,
) -> Result {
    nominal(&value.identity)?;
    // Empty result dispatch is exact meaning for operand-directed operators.
    text(&value.parameter_dispatch)
}
pub(in crate::package_evidence::record) fn depth(depth: usize) -> Result {
...
pub(in crate::package_evidence::record) fn lifetimes(
    ordinals: &[u32],
    scope: &Scope<'_>,
) -> Result {
    if ordinals
