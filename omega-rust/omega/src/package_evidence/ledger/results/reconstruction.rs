...
use crate::package_evidence::ledger::{
...
fn results_from_matching_projection(
    ...
    for callable in accepted_callables {
        let key = crate::package_evidence::encoding::nominal_row_key(callable.identity()).map_err(
            |_| {
                OrdinaryPackageObligationLedgerRecoveryError::new(
    ...
    for obligation in contract_entailment_obligations {
        ...
        let key =
            crate::package_evidence::encoding::contract_entailment_obligation_row_key(obligation)
                .map_err(|_| {
                OrdinaryPackageObligationLedgerRecoveryError::new(
    ...
    for supply in external_supplies {
        let key = crate::package_evidence::encoding::external_executable_supply_row_key(supply)
            .map_err(|_| {
                OrdinaryPackageObligationLedgerRecoveryError::new(
    ...
    for authority in dangerous_authorities {
        let key = crate::package_evidence::encoding::nominal_row_key(authority.service()).map_err(
            |_| {
                OrdinaryPackageObligationLedgerRecoveryError::new(
...
pub fn reconstruct_package_review<'a>(
    ...
    let projection = crate::package_evidence::project_checked_package_review(compilation)?;
...
fn apply_contract_entailment_assumption_discharges(
    ...
    let mut certificates = compilation
        ...
        .filter_map(|certificate| {
            crate::package_evidence::capture::nominal_identity(
                compilation,
                certificate.machine_symbol(),
            )
            .ok()
