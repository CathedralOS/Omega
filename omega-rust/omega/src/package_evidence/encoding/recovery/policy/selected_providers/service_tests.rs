...
use abstract_operations_to_target_operations::effects::provider_plan::{
    ServiceProgressSubject, ServiceResultClaim,
};
...
fn complete_service_claim_result_carry_and_progress_fields_remain_distinct() {
    ...
    method.signature.result = Some(crate::package_evidence::record::PackageReviewTypeIdentity {
        canonical: "u64".into(),
    });
    ...
    for changed in cases {
        ...
        crate::package_evidence::encoding::encode::text_test_support::component(
            crate::package_evidence::encoding::encode::text_test_support::Component::SelectedProviders(&changed),
        );
    }
    for bits in 0..16 {
...
fn typed_service_signature_changes_without_calling_remain_observable() {
    ...
    signature.requirement_arguments.push(
        crate::package_evidence::record::PackageReviewTypeIdentity {
    ...
    assert_eq!(recover(&bytes).unwrap(), changed);
    crate::package_evidence::encoding::encode::text_test_support::component(
        crate::package_evidence::encoding::encode::text_test_support::Component::SelectedProviders(
            &changed,
        ),
    );
}
...
fn family_authority_coverage_target_and_package_fields_reject_unknown_values() {
    ...
    let mut changed = bytes;
    let package = crate::package_evidence::encoding::SELECTED_PROVIDER_POLICY_MAGIC.len() + 2;
