...
fn complete_provider_policy_round_trips_without_nested_receipt_envelopes() {
    for policy in [fixtures::empty(), fixtures::complete()] {
        ...
        crate::package_evidence::encoding::encode::text_test_support::component(
            crate::package_evidence::encoding::encode::text_test_support::Component::SelectedProviders(&policy),
        );
        assert_eq!(recover(&bytes).unwrap().canonical_bytes().unwrap(), bytes);
        for magic in [
            crate::package_evidence::encoding::CALLING_POLICY_MAGIC,
            crate::package_evidence::encoding::CONFORMANCE_POLICY_MAGIC,
            crate::package_evidence::encoding::EXTERNAL_SUPPLY_POLICY_MAGIC,
        ] {
