use super::visitor;
use crate::package_evidence::encoding::PackagePolicyMembershipError;
use crate::package_evidence::encoding::PackagePolicyMembershipLimits;
use crate::package_evidence::encoding::encode::membership::Observer;
...
fn frame(label: &str, children: &[&str]) -> String {
    ...
    crate::package_evidence::record::write_framed_identity(
        &mut result,
        label,
...
fn complete_traversal_checks_typed_owners_without_changing_encoding() {
    ...
    let recovered = PackagePolicyBaseline::recover_canonical(
        &bytes,
        crate::package_evidence::encoding::PackagePolicyRecoveryLimits::default(),
    )
    .unwrap();
    assert_eq!(
        recovered
