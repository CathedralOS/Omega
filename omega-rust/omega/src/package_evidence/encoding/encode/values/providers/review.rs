...
use crate::package_evidence::encoding::{PackageReviewEncodingError, encode::encoder::Encoder};
    ...
    match &row.binding {
        ProviderBinding::Import { evaluated } => {
            ...
            match locator.locator() {
                abstract_operations_to_target_operations::effects::ForeignLocatorCandidate::PeByName { library, export } => {
                    encoder.byte(0);
                    encoder.bytes(library)?;
                    encoder.bytes(export)?;
                }
                abstract_operations_to_target_operations::effects::ForeignLocatorCandidate::PeByOrdinal { library, ordinal } => {
                    encoder.byte(1);
                    encoder.bytes(library)?;
                    encoder.u16(*ordinal);
                }
                abstract_operations_to_target_operations::effects::ForeignLocatorCandidate::ElfVersioned {
                    object,
                    symbol,
                    version,
                } => {
                ...
                abstract_operations_to_target_operations::effects::ForeignLocatorCandidate::MachODylibSymbol {
                    install_name,
                    symbol,
                } => {
                    encoder.byte(3);
...
fn encode_evaluated_binding_receipt(
    encoder: &mut Encoder,
    receipt: &abstract_operations_to_target_operations::effects::provider_plan::EvaluatedBindingReceipt,
) -> Result<(), PackageReviewEncodingError> {
