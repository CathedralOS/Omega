...
pub(crate) fn normalized_import_row(
    export: &[u8],
) -> abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanRow {
...
fn evaluated_import(
    locator: abstract_operations_to_target_operations::effects::NormalizedForeignLocator,
    seed: u8,
...
fn macho_import_review_encoding_retains_raw_atomic_coordinates() {
    fn encoded(install_name: &[u8], symbol: &[u8]) -> Vec<u8> {
        let row = abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanRow {
            method: "write".to_owned(),
            requirement_identity: "Console::write#exact".to_owned(),
            requirement_lifetime_partition: Vec::new(),
            binding: ProviderBinding::Import {
                evaluated: evaluated_import(
                    abstract_operations_to_target_operations::effects::normalize_foreign_locator(
...
fn compiler_intrinsic_execution_encoding_is_closed_and_format_sensitive() {
    use crate::provider_planning::CompilerPrimitiveFloatBinaryOperation;
    ...
    let conversion = encoded(
        PackageReviewCompilerIntrinsicExecution::NamedFloatConversion {
            source: crate::provider_planning::CompilerNumericType::F64,
            target: crate::provider_planning::CompilerNumericType::F32,
...
fn primitive_integer_comparison_encoding_has_explicit_operation_and_type_tags() {
    use crate::provider_planning::{
        CompilerNumericType, CompilerPrimitiveIntegerComparisonOperation,
    };
...
fn compiler_conversion_encoding_has_explicit_numeric_and_domain_tags() {
    use crate::provider_planning::CompilerNumericType;
...
pub(crate) fn empty_review() -> CheckedPackageReviewProjection {
    CheckedPackageReviewProjection {
        ...
        contract_entailment_open_obligations: Vec::new(),
        ...
        boundary_application_demands: Vec::new(),
