...
use crate::package_evidence::encoding::PackageReviewEncodingError as Error;
...
pub(in crate::package_evidence::encoding) struct Writer<'text> {
    output: String,
...
impl<'text> Writer<'text> {
    pub(in crate::package_evidence::encoding) fn new(
        maximum: usize,
        expected: Option<&'text str>,
    ) -> Self {
    ...
    pub(in crate::package_evidence::encoding) fn measured(maximum: usize) -> Self {
    ...
    pub(in crate::package_evidence::encoding) fn preallocated(
        length: usize,
    ) -> Result<Self, Error> {
    ...
    pub(in crate::package_evidence::encoding) fn length(&self) -> usize {
        self.position
    }

    pub(in crate::package_evidence::encoding) fn fail(&mut self, message: &'static str) {
        self.error.get_or_insert(message);
    }

    pub(in crate::package_evidence::encoding) fn check(&self) -> Result<(), Error> {
        self.error
            .map_or(Ok(()), |message| Err(Error::new(message)))
    }

    pub(in crate::package_evidence::encoding) fn finish(self) -> Result<String, Error> {
        self.check()?;
        if self.depth != 0
            || self
                .expected
    ...
    pub(in crate::package_evidence::encoding) fn end(&mut self) {
        if self.depth == 0 {
    ...
    pub(in crate::package_evidence::encoding) fn scope(
        &mut self,
        kind: &'static str,
        name: &'static str,
    ) {
    ...
    pub(in crate::package_evidence::encoding) fn number(
        &mut self,
        kind: &'static str,
    ...
    pub(in crate::package_evidence::encoding) fn boolean(&mut self, value: bool) {
        self.scalar();
        self.append(if value { "bool true\n" } else { "bool false\n" });
    }

    pub(in crate::package_evidence::encoding) fn tag(&mut self, name: &'static str, value: u8) {
        if !label(name) {
    ...
    pub(in crate::package_evidence::encoding) fn sequence(&mut self, count: u64) {
        self.scalar();
        let _ = write!(self, "sequence {count}");
        self.open(false);
    }

    pub(in crate::package_evidence::encoding) fn item(&mut self) {
        self.indent();
        self.append("item");
        self.open(true);
    }

    pub(in crate::package_evidence::encoding) fn option(&mut self, present: bool) {
        self.scalar();
        if present {
    ...
    pub(in crate::package_evidence::encoding) fn bytes(
        &mut self,
        kind: &'static str,
        value: &[u8],
    ) {
