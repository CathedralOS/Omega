...
fn resource_receipt_with_commitment(
    ...
    entry: SymbolHandle,
    contract_report_fingerprint: u64,
    contract_commitment: typed_trees_to_checked_trees::checked_trees::MachineContractCommitment,
) -> typed_trees_to_checked_trees::checked_trees::CheckedCallbackResourceReceipt {
...
fn canonical_private_symbol_binds_site_selected_handles_ordinal_and_plan() {
    ...
    site.site = NominalMachineUseSite::Expression(
        typed_trees_to_checked_trees::checked_trees::expression::ExpressionHandle::from_parts(9, 4),
    );
    drifts.push(site);
...
fn callback_thunk_placement_report_fingerprint_binds_exact_ordered_receipts() {
    ...
    let function_identity =
        abstract_operations_to_target_operations::function_identity::MachineFunctionIdentity::callback_thunk(entry_key, 0)
            .expect("callback identity");
    let private_symbol = canonical_callback_private_symbol(&baseline);
    let root_schedule = Arc::new(
        crate::backend_plan::plan_callback_root_schedule(
            0,
            &baseline,
            entry_key,
            function_identity,
