...
use crate::trust_model::{AcceptedTemplateClassifications, NonProviderTrustGrant, TrustAdmission};
...
enum PreparedTrustIdentity {
    ProviderPlan {
        report_identity: u64,
        digest: abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanDigest,
    },
    MachineTemplate {
        report_identity: u64,
        commitment: symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::MachineTemplateCommitment,
    },
...
pub fn reconstruct_trust_obligations(
    ...
    for grant in root_grants {
        ...
        let (commitment, identity) =
            match crate::trust_model::resolve_non_provider_trust_grant(typed, grant)
                .map_err(|diagnostic| vec![diagnostic])?
            {
                NonProviderTrustGrant::AcceptedMachine(symbol) => {
                    let machine =
                        crate::trust_model::non_provider_grants::accepted_machine(typed, symbol)
                            .map_err(|diagnostic| vec![diagnostic])?;
...
mod tests {
    ...
    use crate::trust_model::TrustAdmissionDigest;
    ...
    fn subject_kind_domain_separates_equal_underlying_commitments() {
        ...
        let template = TrustAdmission::for_machine_template(
            commitment.clone(),
            1,
            symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::MachineTemplateCommitment::from_digest([7; 32]),
        )
        .unwrap();
