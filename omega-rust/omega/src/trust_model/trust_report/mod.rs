...
pub fn reconstruct_trust_report(
    ...
    root_grants: &[String],
    provider_plans: &[abstract_operations_to_target_operations::effects::provider_plan::ProviderPlan],
    selected_provider_plans: &abstract_operations_to_target_operations::effects::SelectedProviderPlanFacts,
    accepted_template_classifications: &crate::trust_model::AcceptedTemplateClassifications,
    ...
    let provider_grants = crate::trust_model::resolve_selected_provider_grants(
        provider_plans,
    ...
    for grant in root_grants {
        ...
        non_provider_grants.push((
            grant.as_str(),
            crate::trust_model::resolve_non_provider_trust_grant(typed, grant)
                .map_err(|diagnostic| vec![diagnostic])?,
        ));
    }
                ...
                .push(TrustProviderRequirementRow {
                    ...
                    termination_premises: method
                        ...
                        subject: match premise.subject {
                            abstract_operations_to_target_operations::effects::provider_plan::ServiceProgressSubject::ProviderReceiver => {
                                TrustProgressPremiseSubject::ProviderReceiver
                            }
                            abstract_operations_to_target_operations::effects::provider_plan::ServiceProgressSubject::Parameter(index) => {
    ...
    for machine in typed.machines() {
        ...
        let granted = non_provider_grants.iter().any(|(_, subject)| {
            *subject == crate::trust_model::NonProviderTrustGrant::AcceptedMachine(machine.symbol)
        });
...
) -> Result<&'checked typed_trees_to_checked_trees::checked_trees::MachineContractPlan, Diagnostic>
{
    exact_machine_contract_plan(
        checked,
        instance,
...
fn accepted_machine_crash_routes(
    plan: &typed_trees_to_checked_trees::checked_trees::MachineContractPlan,
    machine_name: &str,
) -> Result<Vec<TrustCrashRouteBucket>, Diagnostic> {
    if plan.crash.interface()
        != typed_trees_to_checked_trees::checked_trees::CrashInterface::PublishedCeiling
    {
        return Err(Diagnostic::error(format!(
...
fn trust_provider_realization(
    binding: &abstract_operations_to_target_operations::effects::provider_plan::ProviderBinding,
