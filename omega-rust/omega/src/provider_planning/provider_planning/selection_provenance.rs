...
use crate::component_description::VerifiedComponent;
use crate::provider_planning::provider_planning::independent_components::IndependentComponentJoin;
use crate::provider_planning::provider_planning::provenance_replay::validate_derived_provider_plan_provenance;
use crate::provider_planning::{DerivedProviderPlan, ProviderPlanProvenance};
use abstract_operations_to_target_operations::effects::CompilerIntrinsicExecutionIdentity;
...
fn selected_subject_keys(
    selection: &crate::provider_planning::ProviderSelection,
) -> Vec<ProviderSelectionKey> {
    match &selection.subject {
        crate::provider_planning::ProviderSelectionSubject::BoundaryTrait(identity)
        | crate::provider_planning::ProviderSelectionSubject::BoundaryRequirement(identity) => {
            vec![(identity.package, identity.canonical_path.clone())]
        }
        crate::provider_planning::ProviderSelectionSubject::BoundaryOperatorFamily(family) => {
            family
                .coordinates()
                .iter()
...
pub enum ProviderSelectionProvenance {
    BuildOverride(Vec<crate::provider_planning::ProviderSelection>),
    TargetDefault(Vec<crate::provider_planning::ProviderSelection>),
...
impl ProviderSelectionProvenance {
    ...
    /// themselves.
    pub fn composition_mode(&self) -> Result<crate::provider_planning::CompositionMode, String> {
        match self {
            Self::UniqueCoveringCandidate => Ok(crate::provider_planning::CompositionMode::Fused),
            Self::TargetDefault(declarations) => {
                if declarations.iter().any(|declaration| {
                    declaration.composition_mode != crate::provider_planning::CompositionMode::Fused
                }) {
                    return Err(
                ...
                Ok(crate::provider_planning::CompositionMode::Fused)
            }
...
> {
    ...
    for selected_plan in &selected {
        ...
        if composition_mode == crate::provider_planning::CompositionMode::Independent
    ...
    let plans = selected
        ...
        .collect::<Vec<_>>();
    let facts = abstract_operations_to_target_operations::effects::SelectedProviderPlanFacts::from_selected_plans(plans.clone())
...
fn same_family_declaration(
    left: &crate::provider_planning::ProviderSelection,
    right: &crate::provider_planning::ProviderSelection,
) -> bool {
...
fn replay_operator_family_selections(
    ...
    let mut families: Vec<&crate::provider_planning::ProviderSelection> = Vec::new();
    for selected_plan in selected {
        for declaration in authored_selection_declarations(&selected_plan.selected_by) {
            if matches!(
                declaration.subject,
                crate::provider_planning::ProviderSelectionSubject::BoundaryOperatorFamily(_)
            ) && !families
                .iter()
...
fn resolve_provider_selection_slots(
    ...
    declarations: &[crate::provider_planning::ProviderSelection],
    owner: &str,
) -> (
    Vec<(
        ProviderSelectionKey,
        ...
        for slot_key in selected_subject_keys(declaration) {
            if slot_keys.contains(&slot_key) {
                ...
                let message = match &declaration.subject {
                    ...
                    crate::provider_planning::ProviderSelectionSubject::BoundaryRequirement(
                        identity,
                    ) => format!(
                    ...
                    crate::provider_planning::ProviderSelectionSubject::BoundaryOperatorFamily(
                        _,
                    ) => format!(
...
pub(crate) fn select_provider_plan_indices(
    ...
    defaults: &[crate::provider_planning::ProviderSelection],
    requested: &[crate::provider_planning::ProviderSelection],
