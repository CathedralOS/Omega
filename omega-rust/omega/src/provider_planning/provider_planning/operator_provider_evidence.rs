...
pub(crate) use crate::provider_planning::exact_checked_adapter;
use crate::provider_planning::provider_planning::intrinsic_execution::{
...
fn selected_requirement_provider_evidence(
    ...
    let Some(requirement) = crate::provider_planning::IntrinsicRequirement::by_symbol(
        &checked.typed,
        requirement_symbol,
    ) else {
        return Ok(());
    };
    if requirement.kind != crate::provider_planning::IntrinsicRequirementKind::TopLevelRequirement {
        return Ok(());
    }
                    ...
                    .any(|realization| {
                        realization.requirement_symbol == operator.symbol
                            && demanded_application.is_none_or(|application| {
                                typed_trees_to_checked_trees::validation::checked_operator_application_matches_realization(
                                    &checked.typed,
                                    application,
                                    realization,
                                )
...
pub fn selected_use_plan<'a>(
    ...
    if let typed_trees_to_checked_trees::checked_trees::CheckedValueOrigin::StateStatement {
        machine_symbol,
        ..
    } = origin
        && !checked
...
pub fn selected_requirement_plan<'a>(
        ...
        .find(|operator| operator.symbol == requirement)
    {
        ...
        None => {
            let requirement =
                crate::provider_planning::IntrinsicRequirement::by_symbol(typed, requirement)?;
            if requirement.kind
...
pub fn intrinsic_realization_matches_operator(
    ...
    crate::provider_planning::IntrinsicRequirement::from_operator(typed, operator).is_some_and(
        |requirement| {
...
pub fn compiler_intrinsic_diagnostic_label_for(
    ...
    let (operation, primitive, expected_result) = match namespace {
        "F32" | "F64" => {
            ...
            if operation == "classify" {
                ...
                let format = if expected_primitive
                    == symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::F32
                {
                    "f32"
                } else {
        ...
        "I8" | "I16" | "I32" | "I64" | "U8" | "U16" | "U32" | "U64" => {
            let expected_result = match namespace {
                "I8" => symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::I8,
                "I16" => {
                    symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::I16
                }
                "I32" => {
                ...
                "U8" => symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::U8,
                "U16" => {
                    symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::U16
                }
                "U32" => {
    ...
    let format = match primitive {
        symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::F32 => "f32",
        symbol_resolved_trees_to_typed_trees::typed_trees::types::PrimitiveType::F64 => "f64",
        _ => return None,
    };
