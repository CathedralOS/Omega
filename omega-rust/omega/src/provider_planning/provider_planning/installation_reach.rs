...
struct PendingInstallationReachRow<'a> {
    ...
    unresolved: Vec<typed_trees_to_checked_trees::flow_effects::InstallationReachRequirement>,
    plan: &'a ProviderPlan,
    realization: &'a symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine,
}
...
pub(super) fn derive_selected_installation_reach_resolutions(
    ...
    Vec<abstract_operations_to_target_operations::effects::InstallationReachResolution>,
...
> {
    ...
    for plan in selected.plans() {
        ...
        let is_boundary_operator_plan = checked.typed.operators().iter().any(|operator| {
            crate::provider_planning::service_schema::from_typed_operator(&checked.typed, operator)
                .as_ref()
                == Some(&plan.schema)
        });
...
fn record_installation_reach_row<'a>(
    plan: &'a ProviderPlan,
    realization: &'a symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine,
    envelope: &'a typed_trees_to_checked_trees::checked_trees::RealizedMachineContractEnvelope,
    requirement_identity: String,
    upper_bound: Vec<String>,
    resolutions: &mut Vec<
        abstract_operations_to_target_operations::effects::InstallationReachResolution,
    ...
    loop {
        ...
        for mut row in pending.drain(..) {
            ...
            if row.unresolved.is_empty() {
                resolved_row.sort();
                resolved_row.dedup();
                resolutions.push(abstract_operations_to_target_operations::effects::InstallationReachResolution {
...
fn plan_scoped_resolution<'a>(
    resolutions: &'a [abstract_operations_to_target_operations::effects::InstallationReachResolution],
    identity_publishers: &std::collections::BTreeMap<String, std::collections::BTreeSet<u64>>,
    provider_plan_report_identity: u64,
    requirement_identity: &str,
) -> Option<&'a abstract_operations_to_target_operations::effects::InstallationReachResolution> {
