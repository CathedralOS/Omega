...
fn fixture_with_inventory(
    ...
    let mut realization = BoundaryCallingPlanRealization {
        ...
        materialized_signature:
            crate::provider_planning::calling_policy_plans::materialized_boundary_signature_from_abi(
                &CallSignature::default(),
            )
            .unwrap(),
    };
...
fn evaluated_windows_import(library: &[u8], export: &[u8]) -> EvaluatedForeignImport {
    let locator = abstract_operations_to_target_operations::effects::normalize_foreign_locator(
        abstract_operations_to_target_operations::effects::ForeignLocatorCandidate::PeByName {
...
fn external_abi_rows_derive_from_the_selected_provider_plan() {
    ...
    assert_eq!(
        row.binding,
        abstract_operations_to_target_operations::calling_conventions::ExternalBindingKind::Import {
...
fn native_external_projection_retains_syscall_custody_without_unrelated_abi_planning() {
    ...
    assert_eq!(
        row.binding,
        abstract_operations_to_target_operations::calling_conventions::ExternalBindingKind::Syscall { number: 231 }
    );
}
...
fn external_top_level_requirement_extracts_its_exact_carrier_abi() {
    ...
    let plans = crate::provider_planning::provider_planning::derive_satisfies_plans(
        &typed,
    ...
    assert!(matches!(
        row.binding,
        abstract_operations_to_target_operations::calling_conventions::ExternalBindingKind::Syscall { number: 1 }
    ));
    let entry = row
        .boundary_entry_plan
        .as_ref()
...
fn selected_source_boundary_entry_plan_accepts_exact_operator_custody_without_trait_abi() {
    ...
    let mut plans = crate::provider_planning::provider_planning::derive_satisfies_plans(
        &typed,
        ...
        match drift {
            ...
            Drift::Commitment => {
                fixture.realizations[0].commitment =
                    symbol_resolved_trees_to_typed_trees::typed_trees::typed_trees::BoundaryCallingPlanCommitment::from_digest([9; 32])
            }
            Drift::CollisionEqualPlanSubstitution => {
