...
use crate::provider_planning::provider_planning::provenance_replay::adapter_conformance::{
...
use crate::provider_planning::provider_planning::provenance_replay::requirement_identities::{
...
pub fn validate_provider_plan_candidates(
    ...
    let effect_plan = typed_trees_to_checked_trees::validation::infer_operational_may(typed);
    let service_reach_plan =
        typed_trees_to_checked_trees::validation::infer_service_reaches(typed, &effect_plan);
    let invocation_plan =
        typed_trees_to_checked_trees::validation::infer_synchronous_invocations(typed);
    for plan in plans {
...
fn exact_provider_row_conformance<'typed>(
    ...
    plan: &ProviderPlan,
    row: &ProviderPlanRow,
    realization: &'typed symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine,
    requirement_symbol: symbols::SymbolHandle,
    requirement_kind: ExactProviderRequirementKind,
) -> Result<
    &'typed symbol_resolved_trees_to_typed_trees::typed_trees::machine::TraitConformance,
    ...
    let conformances = typed
        ...
        .filter(|conformance| match requirement_kind {
            ExactProviderRequirementKind::Trait { owner } => matches!(
                ...
                Some(symbol_resolved_trees_to_typed_trees::typed_trees::machine::SatisfiedDeclaration::Trait {
                    definition,
                    requirement,
            ...
            ExactProviderRequirementKind::TopLevelRequirement => matches!(
                ...
                Some(
                    symbol_resolved_trees_to_typed_trees::typed_trees::machine::SatisfiedDeclaration::TopLevelRequirement(
                        requirement,
                    ),
...
fn exact_provenance_schema(
    ...
    match provenance.schema {
        ProviderSchemaDeclaration::BoundaryTrait(symbol) => {
            ...
            crate::provider_planning::service_schema::from_typed_instance(
                typed, definition, &arguments,
            )
            .ok_or_else(|| {
        ...
        ProviderSchemaDeclaration::BoundaryRequirement(symbol) => {
            ...
            crate::provider_planning::service_schema::from_typed_boundary_requirement(typed, requirement).ok_or_else(|| {
                diagnostics::Diagnostic::error(format!(
        ...
        ProviderSchemaDeclaration::BoundaryOperator(symbol) => {
            ...
            crate::provider_planning::service_schema::from_typed_operator(typed, operator).ok_or_else(|| {
                diagnostics::Diagnostic::error(format!(
...
fn exact_provenance_realization<'typed>(
    ...
    &'typed symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine,
    diagnostics::Diagnostic,
> {
...
fn replay_provider_row_binding(
    typed: &TypedTrees,
    evaluated_bindings: &crate::provider_planning::evaluated_via_bindings::EvaluatedViaBindingTable,
    plan: &ProviderPlan,
    row: &ProviderPlanRow,
    realization: &symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine,
