...
use crate::provider_planning::provider_planning::operator_provider_evidence::{
...
pub(crate) fn derive_provider_plans(
    ...
    for machine in typed.machines() {
        if !crate::provider_planning::service_schema::is_product_declaration(typed, machine.symbol)
        {
            continue;
        }
...
fn derive_top_level_requirement_plans(
    ...
    for machine in typed.machines() {
        ...
        let origin_package_identity = typed.symbols.symbol_package_identity(machine.symbol);
        for clause in typed.machine_trait_conformances(machine) {
            let Some(symbol_resolved_trees_to_typed_trees::typed_trees::machine::SatisfiedDeclaration::TopLevelRequirement(requirement)) =
                symbol_resolved_trees_to_typed_trees::typed_trees::machine::resolve_satisfied_declaration(typed, machine, clause)
            else {
                continue;
            };
...
fn provider_plan_schema_targets(
    ...
    let mut refined = typed
        ...
        .filter_map(|conformance| {
            ...
            let schema = crate::provider_planning::service_schema::from_typed_instance(
...
fn derive_boundary_operator_plans(
    ...
    for machine in typed.machines() {
        ...
        for clause in typed.machine_trait_conformances(machine) {
            ...
            let Some(schema) =
                crate::provider_planning::service_schema::from_typed_operator(typed, operator)
            else {
                continue;
            };
            let target = selected_target.unwrap_or_default().to_owned();
