...
pub fn settle_external_binding_rows(
    retained: &mut Arc<
        [abstract_operations_to_target_operations::calling_conventions::ExternalBindingRow],
    >,
    typed: &symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees,
    selected_target: Option<&str>,
    native_target: target::NativeTarget,
    selected_plans: &[abstract_operations_to_target_operations::effects::provider_plan::ProviderPlan],
    boundary_calling_plan_realizations: &[
        crate::provider_planning::calling_policy_plans::BoundaryCallingPlanRealization
    ],
    opaque_representation_selections: &[crate::representation_planning::OpaqueRepresentationSelection],
...
> {
    use abstract_operations_to_target_operations::calling_conventions::{
        CallingPolicy, ExternalBindingKind, ExternalBindingRow,
    };
    ...
    for plan in selected_plans {
        for row in &plan.rows {
            ...
            let boundary_entry_plan = match (boundary_entry_plan, compatibility_policy) {
                ...
                (None, Some(policy)) => {
                    crate::provider_planning::calling_policy_plans::evaluate_compatibility_boundary_entry_plan(
                        typed,
                        native_target,
                        &plan.schema.trait_name,
...
> {
    ...
    let top_level_requirements = typed
        ...
        .filter(|requirement| {
            requirement.supply_mode == language_semantics::MachineSupplyMode::TopLevelRequirement
                && crate::provider_planning::service_schema::from_typed_boundary_requirement(
                    typed,
                    requirement,
                )
                .as_ref()
    ...
    let schema_owners = typed
        ...
        .iter()
        .filter(|definition| {
            crate::provider_planning::service_schema::schema_binds_exact_boundary_trait(
                typed,
                &plan.schema,
                definition,
            )
