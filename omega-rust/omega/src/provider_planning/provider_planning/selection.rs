...
impl ProviderOperatorFamilySelection {
    ...
    pub fn derive(
        ...
        if !crate::provider_planning::service_schema::is_product_declaration(
            typed,
    ...
    pub fn replay_against_typed(&self, typed: &TypedTrees) -> Vec<String> {
        ...
        for coordinate in &self.coordinates {
            let Some(current) = expected
                ...
                let reason =
                    match symbol_resolved_trees_to_typed_trees::typed_trees::operator::declaration_by_symbol(typed, coordinate.symbol) {
    ...
    typed
        ...
        .filter(|operator| {
            operator.is_boundary
                && crate::provider_planning::service_schema::is_product_declaration(typed, operator.symbol)
