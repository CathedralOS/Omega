...
pub fn check_boundary_provider_approval(
    ...
    let registry = abstract_operations_to_target_operations::effects::build_boundary_provider_approval_registry(program);
    let unapproved =
        abstract_operations_to_target_operations::effects::audit_boundary_provider_calls(
            program, calls, &registry,
        );
...
fn checked_boundary_call_coordinates(
    ...
    Vec<abstract_operations_to_target_operations::effects::BoundaryCallCoordinate>,
    Vec<Diagnostic>,
> {
    let program = &checked.typed;
            ...
            if boundary_edges.is_empty() {
                match exact_direct_boundary_requirement(program, call.target_symbol) {
                    Ok(Some((boundary_trait_symbol, boundary_signature_symbol))) => {
                        coordinates.push(abstract_operations_to_target_operations::effects::BoundaryCallCoordinate {
