...
pub fn from_typed_operator(
    ...
    operator.is_boundary.then(|| ServiceSchema {
        trait_name: symbol_resolved_trees_to_typed_trees::typed_trees::operator::boundary_operator_requirement_identity(
            program, operator,
        ),
        trait_package_identity: program.symbols.symbol_package_identity(operator.symbol),
        methods: vec![ServiceMethod {
            name: "realize".to_owned(),
            requirement_owner: symbol_resolved_trees_to_typed_trees::typed_trees::operator::boundary_operator_requirement_identity(
                program, operator,
            ),
...
///
/// [`symbol_resolved_trees_to_typed_trees::typed_trees::operator::boundary_operator_requirement_identity`] is
...
fn synchronous_invocation_names(
    ...
    let parameters = program
        ...
        .filter(|parameter| !parameter.is_self)
        .collect::<Vec<_>>();
    let mut names = typed_trees_to_checked_trees::validation::declared_signature_invocations(
        program, signature,
    )
    .into_iter()
    .filter_map(|target| match target {
        typed_trees_to_checked_trees::flow_effects::InvocationTarget::Parameter(index) => {
            parameters
                .get(index as usize)
                .map(|parameter| parameter.type_reference)
                .and_then(|type_reference| boundary_trait_name_for_type(program, type_reference))
        }
        typed_trees_to_checked_trees::flow_effects::InvocationTarget::Service(symbol) => program
            .traits()
            .iter()
...
fn machine_synchronous_invocation_names(
    ...
    let parameters = program
        ...
        .collect::<Vec<_>>();
    let mut names =
        typed_trees_to_checked_trees::validation::declared_machine_invocations(program, machine)
            .into_iter()
            .filter_map(|target| match target {
                typed_trees_to_checked_trees::flow_effects::InvocationTarget::Parameter(index) => {
                    parameters
                        ...
                        .and_then(|type_reference| {
                            boundary_trait_name_for_type(program, type_reference)
                        })
                }
                typed_trees_to_checked_trees::flow_effects::InvocationTarget::Service(symbol) => {
                    program
                        .traits()
                        .iter()
                        .find(|definition| definition.is_boundary && definition.symbol == symbol)
