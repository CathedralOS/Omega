...
use crate::provider_planning::evaluated_via_bindings::binding_digests::at;
use crate::provider_planning::evaluated_via_bindings::binding_evaluation::{
    evaluate_one, exact_binding_vocabulary,
};
...
pub struct EvaluatedViaBindingRow {
    ...
    via_expression: symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle,
...
pub fn evaluate_via_bindings(
    ...
    let pending = typed
        ...
        .iter()
        .filter(|machine| {
            crate::provider_planning::service_schema::is_product_declaration(typed, machine.symbol)
        })
        .flat_map(|machine| {
            typed
                .machine_trait_conformances(machine)
    ...
    let selection_authority = package_inputs.cloned().map(|inputs| {
        Arc::new(inputs) as Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>
    });
