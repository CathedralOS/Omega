...
pub fn build_selected_component_progress_manifest(
    program: &typed_trees_to_checked_trees::checked_trees::CheckedTrees,
...
fn build_component_progress_manifest(
    ...
    abstract_operations_to_target_operations::effects::ComponentProgressManifest,
    Vec<Diagnostic>,
> {
    ...
    while let Some(machine_symbol) = queue.pop() {
        ...
        {
            demands.push(
                abstract_operations_to_target_operations::effects::CheckedComponentProgressDemand {
                    ...
                    origin_callable_identity: machine_callable_identity(
                        program,
                        demand.origin.machine,
                    )
                    .ok_or_else(|| {
            ...
            .filter(|(_, state)| state.machine_symbol == machine_symbol)
        {
            for call in program.facts.flow.control.calls.span_or_empty(state.calls) {
                ...
                if let ProviderBinding::CheckedAdapter { .. } = &row.binding {
                    let adapter =
                        crate::provider_planning::provider_planning::exact_checked_adapter(
                            program, plan, row,
                        )
    ...
    abstract_operations_to_target_operations::effects::ComponentProgressManifest::bind(
...
pub fn reject_undischarged_build_bound_progress(
    manifest: Option<&abstract_operations_to_target_operations::effects::ComponentProgressManifest>,
...
mod tests {
    ...
    fn push_machine(
        ...
        let mut machine = typed_trees_to_checked_trees::checked_trees::machine::Machine {
            ...
            name: typed_trees_to_checked_trees::checked_trees::name::Identifier::generated(name),
            ..Default::default()
        };
        if with_entry {
