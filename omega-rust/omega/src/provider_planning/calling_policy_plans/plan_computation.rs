...
use crate::provider_planning::calling_policy_plans::boundary_signatures::{
...
use crate::provider_planning::calling_policy_plans::build_time_decoding::{
...
use crate::provider_planning::calling_policy_plans::{
    ...
    let trait_candidates = typed
        ...
        .filter(|definition| {
            typed.trait_declaration_path(definition) == trait_name
                && crate::provider_planning::service_schema::is_product_declaration(
                    typed,
                    definition.symbol,
                )
...
pub fn compute_boundary_calling_plans(
    ...
    native_target: NativeTarget,
    opaque_representation_selections: &[OpaqueRepresentationSelection],
    package_inputs: Option<&crate::package_compilation::PackageCompilationInputs>,
) -> Result<Vec<BoundaryCallingPlanRealization>, Vec<Diagnostic>> {
    let Some((calling_trait, calling_policy_trait)) =
        typed_trees_to_checked_trees::validation::standard_calling_traits(typed)
    else {
    ...
    let admission = crate::build_time_evaluation::BuildTimeAdmissionPlan::infer(
        typed,
    ...
    ) in pending
    {
        let validated = evaluate_materialized_calling_policy_plan(
            ...
            &signature,
            Some(
                crate::build_time_evaluation::BuildTimeInvocationCustody::Source(relationship_span),
            ),
        )
        .map_err(|rejection| {
            vec![
...
pub fn evaluate_calling_policy_plan(
    ...
    let admission = crate::build_time_evaluation::BuildTimeAdmissionPlan::infer(typed, None);
...
pub(crate) fn evaluate_materialized_calling_policy_plan(
    typed: &TypedTrees,
    admission: &crate::build_time_evaluation::BuildTimeAdmissionPlan,
    policy_machine: &str,
    signature: &MaterializedBoundarySignature,
    custody: Option<crate::build_time_evaluation::BuildTimeInvocationCustody>,
...
pub(crate) fn boundary_plan_application_identity(
    ...
    for representation in &signature.opaque_representations {
        ...
        strong.update([match representation.origin {
            crate::representation_planning::OpaqueRepresentationApplicationOrigin::NamedConformance => 1,
        }]);
        strong.update([match representation.lifecycle {
            crate::representation_planning::OpaqueRepresentationLifecycleDisposition::Inert => 1,
        }]);
        strong.update([match representation.copy_disposition {
            crate::representation_planning::OpaqueRepresentationCopyDisposition::PlacementOnly => 1,
            crate::representation_planning::OpaqueRepresentationCopyDisposition::CheckedSemanticCopy => 2,
        }]);
