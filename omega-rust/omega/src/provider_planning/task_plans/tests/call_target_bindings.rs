...
use crate::provider_planning::task_plans::stack_graphs::task_call_graph;
use crate::provider_planning::task_plans::tests::stack_graphs::{
    frame_entry, mutual_call_fixture, suspending_subtree_fixture,
};
use crate::provider_planning::task_plans::{CallTargetAssignment, derive_call_target_bindings};
use abstract_operations_to_target_operations::task_plans::{
    UnresolvedCallKind, compose_task_stack_demand, project_wcsu_stack_plan,
};
...
fn suspending_partial_plan() -> (
    ...
    abstract_operations_to_target_operations::task_plans::ValidatedActivationPlan,
    abstract_operations_to_target_operations::task_plans::UnresolvedCallSite,
    abstract_operations_to_target_operations::task_plans::CanonicalSuspensionCrossing,
) {
    let (program, alpha_machine, beta_machine) = suspending_subtree_fixture();
    let layouts = resolved_layout_to_resolved_layout::layout::build_layout_plan(
        &program,
    ...
    let alpha_crossing =
        crate::provider_planning::task_plans::carry_crossings::canonical_suspension_crossing(
            &program,
            graph.crossings[0],
        )
    ...
    let projection = project_wcsu_stack_plan(
        &demand,
        abstract_operations_to_target_operations::task_plans::StackRepresentationId::from_normalized_identity(9).expect("representation"),
    );
    let site = projection
        .unresolved_calls()
        .iter()
        ...
        .clone();
    let plan = abstract_operations_to_target_operations::task_plans::validate_wcsu_activation_plan(
        abstract_operations_to_target_operations::task_plans::ActivationPlanCandidate {
            machine_contract: abstract_operations_to_target_operations::task_plans::MachineContractId::from_normalized_identity(1)
                .expect("contract"),
            entry: abstract_operations_to_target_operations::task_plans::MachineEntryId::from_normalized_identity(2).expect("entry"),
            argument_layout: abstract_operations_to_target_operations::task_plans::TaskArgumentLayout::new(
                abstract_operations_to_target_operations::task_plans::ValueLayoutId::from_normalized_identity(3).expect("layout"),
                &[(8, 8), (4, 4)],
            )
            .expect("canonical argument layout"),
            terminal_outcome_layout: abstract_operations_to_target_operations::task_plans::ValueLayoutId::from_normalized_identity(4)
                .expect("outcome layout"),
            calling_plan: abstract_operations_to_target_operations::task_plans::CallingPlanId::from_normalized_identity(5).expect("calling"),
            stack_plan: projection.stack_plan(),
            may_suspend: true,
            may_block: false,
            canonical_suspension_crossings: vec![alpha_crossing.clone()],
            carry_obligations: abstract_operations_to_target_operations::task_plans::ActivationCarryObligations::none(),
...
fn derived_binding_covers_the_site_and_the_covered_plan_establishes_a_lease() {
    ...
    let covered = abstract_operations_to_target_operations::task_plans::TaskRuntimeAdmission::bind_call_targets(&plan, &bindings)
    ...
    // provisioned slot.
    abstract_operations_to_target_operations::task_plans::establish_stack_lease(
        &covered,
        abstract_operations_to_target_operations::task_plans::StackLeaseBacking {
            provenance: abstract_operations_to_target_operations::task_plans::TaskStorageProvenance {
                owner: abstract_operations_to_target_operations::task_plans::TaskStorageOwnerId::from_normalized_identity(11)
                    .expect("storage owner"),
                lease: abstract_operations_to_target_operations::task_plans::TaskStorageLeaseId::from_normalized_identity(12)
                    .expect("lease era"),
            },
...
fn derived_binding_covers_an_unresolved_target_site() {
    ...
    program.facts.flow.control.retired_calls.push(
        typed_trees_to_checked_trees::checked_trees::RetiredFlowCall {
...
fn an_assignment_to_a_non_checked_body_callee_rejects() {
    ...
    let covered = abstract_operations_to_target_operations::task_plans::TaskRuntimeAdmission::bind_call_targets(&plan, &[])
