...
pub(crate) struct ActivationCarryCrossings<'program> {
    pub(crate) subtree:
        Vec<&'program typed_trees_to_checked_trees::checked_trees::SuspensionCrossingCarryFact>,
}
...
pub(crate) fn exact_activation_wide_carry<'program>(
    ...
    &'program typed_trees_to_checked_trees::checked_trees::MachineActivationCarryFact,
    Vec<Diagnostic>,
> {
...
pub(crate) fn validate_activation_carry_crossing(
    program: &CheckedTrees,
    crossing: &typed_trees_to_checked_trees::checked_trees::SuspensionCrossingCarryFact,
...
pub(crate) fn canonical_suspension_crossing(
    ...
    Ok(CanonicalSuspensionCrossing {
        identity: typed_trees_to_checked_trees::checked_trees::canonical_suspension_crossing_id(
            program, crossing,
        )
        .ok_or_else(|| {
            vec![Diagnostic::error(
...
fn live_carry_demand(
    program: &CheckedTrees,
    crossing: &typed_trees_to_checked_trees::checked_trees::SuspensionCrossingCarryFact,
    live: &typed_trees_to_checked_trees::checked_trees::SuspensionCrossingLiveValueFact,
    ...
    Ok(LiveCarryDemand {
        ...
        storage: match live.storage {
            typed_trees_to_checked_trees::checked_trees::SuspensionCrossingStorage::Persistent => LiveCarryStorage::Persistent,
            typed_trees_to_checked_trees::checked_trees::SuspensionCrossingStorage::Parameter => LiveCarryStorage::Parameter,
            typed_trees_to_checked_trees::checked_trees::SuspensionCrossingStorage::Local => LiveCarryStorage::Local,
...
fn live_place_identity(
    ...
    match live.origin {
        typed_trees_to_checked_trees::checked_trees::SuspensionCrossingValueOrigin::Persistent { symbol } => {
            hash.byte(1);
            hash.string(&program.typed.symbols.display_path(symbol, "::"));
        }
        typed_trees_to_checked_trees::checked_trees::SuspensionCrossingValueOrigin::Parameter { symbol, position } => {
            hash.byte(2);
            hash.string(&program.typed.symbols.display_path(symbol, "::"));
            hash.usize(position);
        }
