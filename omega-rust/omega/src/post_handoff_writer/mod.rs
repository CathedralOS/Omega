...
use abstract_operations_to_target_operations::calling_conventions::{
    MachineRegister, StateFootprintEvidence,
};
use diagnostics::Diagnostic;
use target::{Architecture, NativeTarget};
use terminal_psi::layout_plans::{PostHandoffWriterInvocationPlan, PostHandoffWriterPlan};
...
pub fn lower_post_handoff_writer_fragment(
    ...
    let (bytes, footprint) = match target.architecture {
        Architecture::X86_64 => (
            target_operations_to_selected_instructions::isa_x86_64::encode_generated_post_handoff_writer_bytes(
                pointer_register,
                invocation.fragment(),
            )?,
            StateFootprintEvidence::new(
                target_operations_to_selected_instructions::isa_x86_64::generated_post_handoff_writer_clobbers(),
                target_operations_to_selected_instructions::isa_x86_64::generated_post_handoff_writer_additional_machine_state(),
            ),
        ),
        Architecture::Aarch64 => (
            target_operations_to_selected_instructions::isa_aarch64::encode_generated_post_handoff_writer_bytes(
                pointer_register,
                invocation.fragment(),
            )?,
            StateFootprintEvidence::new(
                target_operations_to_selected_instructions::isa_aarch64::generated_post_handoff_writer_clobbers(),
                target_operations_to_selected_instructions::isa_aarch64::generated_post_handoff_writer_additional_machine_state(),
            ),
        ),
    };
