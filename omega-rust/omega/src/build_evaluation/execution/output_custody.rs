...
use crate::build_evaluation::evidence::filesystem_scope::BUILD_OUTPUT_ROOT_IDENTITY;
use crate::build_evaluation::evidence::observations::{
    BuildIncludedSourceHandoff, BuildRequiredOutputSettlement,
};
...
pub(super) fn settle_build_output_obligations(
    obligations: &[crate::checked_interpreter::BuildOutputObligation],
    receipts: &[crate::checked_interpreter::BuildOutputReceipt],
    ...
    for (index, obligation) in obligations.iter().enumerate() {
        ...
        let crate::checked_interpreter::BuildOutputObligationState::Completed {
            receipt: receipt_index,
            ..
        } = obligation.state()
        else {
            let message = match obligation.state() {
                crate::checked_interpreter::BuildOutputObligationState::Pending => format!(
