...
use crate::build_evaluation::BuildMachineFilesystemScope;
use crate::build_time_evaluation::BuildMachineFilesystemSponsor;
...
pub fn prepare_filesystem_scope(
    ...
    let mut build_machine_filesystem_scope = if let Some(inputs) = package_inputs {
        crate::build_evaluation::BuildMachineFilesystemScope::for_package_root(
            inputs
        ...
        crate::build_evaluation::BuildMachineFilesystemScope::for_root(
            root_path,
            build_dir,
            filesystem_sponsor,
        )
    ...
    if let Some(build_snapshot) = build_snapshot {
        ...
        let captured_input = match build_snapshot.capture() {
            crate::build_evaluation::BuildSnapshotCapture::PackageInventory => {
                ...
                crate::package_compilation::capture_package_source_input(&source_root).map_err(
                    |reason| {
                        vec![Diagnostic::error(format!(
            ...
            crate::build_evaluation::BuildSnapshotCapture::Scoped(request) => {
                crate::package_compilation::capture_scoped_source_input(
                    &source_root,
                    request,
        ...
        build_machine_filesystem_scope = if package_inputs.is_some()
            && matches!(
                ...
                crate::build_evaluation::BuildSnapshotCapture::Scoped(_)
            ) {
            ...
            let complete = crate::package_compilation::capture_package_source_input(&source_root)
                .map_err(|reason| {
                vec![Diagnostic::error(format!(
...
mod tests {
    ...
    use crate::package_compilation::BuildSourceCaptureObligation;
    ...
    fn scoped_snapshot_request() -> crate::build_evaluation::BuildSnapshotRequest {
        crate::build_evaluation::BuildSnapshotRequest::scoped(
            Vec::<Vec<u8>>::new(),
    ...
    fn source_read_root(
        scope: &crate::build_evaluation::BuildMachineFilesystemScope,
    ...
    fn prepared_scope(
        ...
        PathBuf,
        PathBuf,
        crate::build_evaluation::BuildMachineFilesystemScope,
    ) {
