...
use crate::checked_interpreter::WireCodecVerification;
use arena::HandleSpan;
use diagnostics::Diagnostic;
use symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees;
use symbol_resolved_trees_to_typed_trees::typed_trees::wire::{WireMember, WireSchema};

pub fn validate_wire_protocol(
    typed: &TypedTrees,
    compatibility_demands: &[crate::build_evaluation::WireCompatibilityDemand],
    ...
    let verifications: BTreeMap<String, Result<WireCodecVerification, String>> = typed
        ...
        .map(|schema| {
            (
                ...
                crate::checked_interpreter::verify_wire_schema_codec(typed, schema),
            )
        })
        .collect();
...
fn normalized_wire_plan_report_identity(
    ...
    placements: &[symbol_resolved_trees_to_typed_trees::typed_trees::wire::WirePlacement],
    obligations: &[symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireEncodeObligation],
) -> u64 {
    ...
    for placement in placements {
        let (kind, tag) = match placement {
            symbol_resolved_trees_to_typed_trees::typed_trees::wire::WirePlacement::Varint { tag } => (0u8, *tag),
            symbol_resolved_trees_to_typed_trees::typed_trees::wire::WirePlacement::LengthPrefixed { tag } => (1u8, *tag),
        };
...
fn compatibility_demand_report(
    typed: &TypedTrees,
    rows: &[SchemaRow],
    demand: &crate::build_evaluation::WireCompatibilityDemand,
...
fn era_number_scopes(
    ...
    let has_fields = members.iter().any(|member| {
        matches!(
            member,
            symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(_)
        )
    });
    let has_cases = members.iter().any(|member| {
        matches!(
            member,
...
fn ordinary_data_schema_report_entry(
    ...
    let cases = members
        ...
        .collect();
    let shape = symbol_resolved_trees_to_typed_trees::typed_trees::data::DataDefinition::shape_kind_from_members(members);
    ...
    Some(WireSchemaReportEntry {
        ...
        normalized_schema_report_identity:
            crate::build_time_evaluation::normalized_schema_report_fingerprint(typed, data),
...
mod tests {
    ...
    use std::collections::BTreeMap;
    use symbol_resolved_trees_to_typed_trees::typed_trees::wire::WirePlacement;
    ...
    fn normalized_wire_plan_report_identity_binds_kind_tag_and_schema_report_coordinate() {
        ...
        let obligation = symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireEncodeObligation {
            field_number: 1,
            element: symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireScalarEncoding {
                byte_size: 4,
                zigzag: false,
            },
            length: symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireEncodeLengthObligation::RuntimeElementCount,
            work: symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireEncodeWorkObligation::TwoPassesPerElement,
            output_capacity:
                symbol_resolved_trees_to_typed_trees::typed_trees::wire::WireEncodeOutputCapacityObligation::ExactPackedPayload,
        };
        assert_ne!(
            report_identity,
    ...
    fn wire_report(source_text: &str) -> crate::artifacts::WireProtocolReport {
        let mut typed = typed_fixture(source_text);
        crate::build_time_evaluation::compute_wire_plans(&mut typed, None, 0)
    ...
    fn synthesized_codec_stays_admitted_without_an_authored_grammar_policy() {
        ...
        assert_eq!(
            packet.trust_class,
            Some(crate::artifacts::WireTrustClass::Admitted {
                authority: "Omega compiler".to_owned()
            }),
