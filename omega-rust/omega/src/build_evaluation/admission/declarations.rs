...
use crate::build_evaluation::admission::behavior_exclusions::{
...
use crate::provider_planning::{ProviderSelection, ProviderSelectionIdentity};
...
struct ProviderSelectionCall<'a> {
    target: &'a str,
    arguments: &'a [symbol_resolved_trees_to_typed_trees::typed_trees::expression::StaticMachineArgument],
    value_arguments: &'a [symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle],
...
pub(crate) fn collect_executed_provider_selections(
    typed: &TypedTrees,
    executed: &[crate::checked_interpreter::ExecutedProviderSelection],
) -> Result<Vec<ProviderSelection>, Vec<Diagnostic>> {
    let requests = executed
        .iter()
        .map(|row| {
            let request = match row.site {
                crate::checked_interpreter::ExecutedProviderSelectionSite::Statement(handle) => {
                ...
                crate::checked_interpreter::ExecutedProviderSelectionSite::Expression(handle) => {
...
fn collect_provider_selections<'a>(
    ...
    let mut record = |request: ProviderSelectionCall<'_>| {
        ...
        let project_identity = |argument: &symbol_resolved_trees_to_typed_trees::typed_trees::expression::StaticMachineArgument,
                                is_provider: bool| {
        ...
        let subject = if boundary_identity.symbol.is_valid()
            ...
            crate::provider_planning::ProviderSelectionSubject::BoundaryTrait(boundary_identity)
            ...
            && typed.symbols.get(boundary_identity.symbol).kind == SymbolKind::Operator
        {
            ...
            match crate::provider_planning::ProviderOperatorFamilySelection::derive(
                typed,
...
pub fn validate_executed_provider_selection_declaration(
    ...
    let mut validate =
        |target: &str,
         ...
         values: &[symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle]| {
            ...
            let subject_symbol = match &selection.subject {
                ...
                crate::provider_planning::ProviderSelectionSubject::BoundaryOperatorFamily(family) => {
                    ...
                    let path = slot
                        ...
                        .collect::<Vec<_>>()
                        .join("::");
                    if crate::provider_planning::ProviderOperatorFamilySelection::derive(
                        typed, symbol, path,
                    )
                    .as_ref()
                        != Ok(family)
            ...
            if values.is_empty()
                && selection.composition_mode != crate::provider_planning::CompositionMode::Fused
            {
                return false;
            }
            if let [value] = values
    ...
    for handle in typed_trees_to_checked_trees::typed_provider_selection_expressions(typed, machine)
    {
        ...
        if provider_selection_expression_source_span(typed, handle) == selection.source_span
            && let symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Call(call) = expression
...
) -> Result<crate::provider_planning::CompositionMode, Diagnostic> {
    let [] = arguments else {
...
fn provider_composition_case(
    ...
    if !case_symbol.is_valid() {
        return Ok(crate::provider_planning::CompositionMode::Fused);
    }
    ...
    match selected.name.as_str() {
        "Fused" => Ok(crate::provider_planning::CompositionMode::Fused),
        "Independent" => Ok(crate::provider_planning::CompositionMode::Independent),
...
pub fn harvest_root_grants(
    typed: &TypedTrees,
    machine: &symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine,
) -> Result<Vec<crate::trust_model::AuthoredRootGrant>, Diagnostic> {
    let mut grants = Vec::new();
    let mut record = |selector: &str, source_span: source::SourceSpan| {
        if !grants
            .iter()
            .any(|grant: &crate::trust_model::AuthoredRootGrant| grant.selector == selector)
        {
            grants.push(crate::trust_model::AuthoredRootGrant {
    ...
    for state in typed.machine_states(machine) {
        for statement in typed.statement_table.statements(state.statement_nodes) {
            let handles: Vec<symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle> = match statement {
                symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Expression(expression) => {
                    vec![*expression]
                }
                symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Call(call) => {
...
pub fn harvest_behavior_exclusions(
    typed: &TypedTrees,
    executed: &[crate::checked_interpreter::ExecutedBehaviorExclusion],
    ...
    for row in executed {
        ...
        struct ExecutedExclusionCall<'a> {
            source_span: source::SourceSpan,
            machine_arguments: &'a [symbol_resolved_trees_to_typed_trees::typed_trees::expression::StaticMachineArgument],
            arguments: &'a [symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionHandle],
        }
        let call = match row.site {
            crate::checked_interpreter::ExecutedBehaviorExclusionSite::Statement(handle) => {
                match typed.statement_table.statement(handle) {
                    symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::Call(call) => {
            ...
            crate::checked_interpreter::ExecutedBehaviorExclusionSite::Expression(handle) => {
                match typed.expression_table.expression(handle) {
                    symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Call(call) => {
                        Some(ExecutedExclusionCall {
        ...
        let kind = match row.kind {
            crate::checked_interpreter::ExecutedBehaviorExclusionKind::CrashCause {
                case_symbol,
            ...
            crate::checked_interpreter::ExecutedBehaviorExclusionKind::Service => {
            ...
            crate::checked_interpreter::ExecutedBehaviorExclusionKind::PhysicalAuthorityClass {
                case_symbol,
...
fn exact_case_argument_symbol(
    ...
    match typed.expression_table.expression(argument) {
        symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Name(path) => Some(path.symbol),
        symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Member(member) => Some(member.member_symbol),
...
fn evaluated_physical_authority_class(
    typed: &TypedTrees,
    case_symbol: SymbolHandle,
) -> Result<abstract_operations_to_target_operations::effects::TerminalAuthorityClass, Diagnostic> {
