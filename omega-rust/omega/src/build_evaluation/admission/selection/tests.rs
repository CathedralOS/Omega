...
use crate::build_evaluation::{BuildConfig, RootBinding};
...
fn fused_root_row_at(
    ...
    resolved_layout_to_resolved_layout::program_entry_plan::ProgramEntryFusedServiceEstablishment::new(
        ...
        "named(name(Binding<Console>))".into(),
        "Console".into(),
        abstract_operations_to_target_operations::effects::provider_plan::ServiceSchemaDigest::from_digest([1; 32]),
        abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanDigest::from_digest([2; 32]),
    )
...
fn fused_roots_at_distinct_routes_may_share_one_leaf_name() {
    ...
    assert_eq!(
        selected
            .fused_service_establishments()
            .iter()
            .map(resolved_layout_to_resolved_layout::program_entry_plan::ProgramEntryFusedServiceEstablishment::field_path)
            .collect::<Vec<_>>(),
        [
