...
use terminal_psi::extents::{ExtentRights, MappedExtent, MappingReceiptContext};
...
fn validate_post_handoff_destination_binding(
    ...
    } else if site.phase != terminal_psi::layout_plans::PlacementPhase::PostHandoff {
...
fn non_authoritative_post_handoff_entry_writer_context_fingerprint(
    ...
    for constraint in invocation.fit_constraints() {
        ...
        mix(match constraint.fit.interpretation {
            terminal_psi::layout_plans::IntegerInterpretation::Signed => 1,
            terminal_psi::layout_plans::IntegerInterpretation::Unsigned => 2,
        });
