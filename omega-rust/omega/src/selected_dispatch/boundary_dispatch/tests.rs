...
use crate::selected_dispatch::boundary_dispatch::boundary_fields::exact_adapter_receiver_shape;
use crate::selected_dispatch::core_service_fixture::typed_with_core_service;
use abstract_operations_to_target_operations::effects::provider_plan::{
    ProviderBinding, ProviderPlan,
};
...
fn statement_call(
    ...
    arena::HandleSpan<symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode>,
    usize,
    symbol_resolved_trees_to_typed_trees::typed_trees::statement::TableCall,
) {
    checked
        .typed
        .machines()
...
fn late_invalid_call_prevents_statement_and_value_rewrites() {
    ...
    let selected_names = plans
        ...
        .map(|plan| plan.name.clone())
        .collect::<Vec<_>>();
    let selected = abstract_operations_to_target_operations::effects::SelectedProviderPlanFacts::from_selection(&plans, &selected_names)
