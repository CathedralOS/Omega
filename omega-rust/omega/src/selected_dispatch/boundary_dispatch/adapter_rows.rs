...
use crate::selected_dispatch::boundary_dispatch::boundary_fields::{
    exact_adapter_receiver_shape, exact_boundary_trait, exact_conformance_requirement_identity,
    named_type_symbol,
};
use crate::selected_dispatch::boundary_dispatch::signature_families::{
    FamilyProbe, finite_signature_family,
};
...
pub(crate) enum SelectedAdapterTarget<'a> {
    TopLevelRequirement {
        method: &'a abstract_operations_to_target_operations::effects::provider_plan::ServiceMethod,
        requirement: &'a symbol_resolved_trees_to_typed_trees::typed_trees::machine::Machine,
    },
    TraitRequirement {
        method: &'a abstract_operations_to_target_operations::effects::provider_plan::ServiceMethod,
        receiver_trait: &'a symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::TraitDefinition,
        requirement_owner: &'a symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::TraitDefinition,
        signature: &'a symbol_resolved_trees_to_typed_trees::typed_trees::signature::StateSignature,
    },
}
...
pub(crate) fn resolve_selected_adapter_target<'a>(
    typed: &'a TypedTrees,
    plan: &'a abstract_operations_to_target_operations::effects::provider_plan::ProviderPlan,
    row: &'a abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanRow,
) -> Result<Option<SelectedAdapterTarget<'a>>, Diagnostic> {
    if !matches!(
        row.binding,
        abstract_operations_to_target_operations::effects::provider_plan::ProviderBinding::CheckedAdapter { .. }
    ) {
        return Ok(None);
    }
    if typed.operators().iter().any(|operator| {
        crate::provider_planning::service_schema::schema_binds_exact_boundary_operator(
            typed,
            &plan.schema,
            operator,
        )
    ...
    let top_level_requirements = typed
        ...
        .iter()
        .filter(|requirement| {
            crate::provider_planning::service_schema::schema_binds_exact_boundary_requirement(
                typed,
                &plan.schema,
                requirement,
            )
...
pub(crate) fn resolve_selected_adapter_row(
    ...
    let adapter = crate::provider_planning::exact_checked_adapter(typed, plan, row)?;
...
fn resolve_top_level_requirement_adapter_row(
    ...
    plan: &abstract_operations_to_target_operations::effects::provider_plan::ProviderPlan,
    row: &abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanRow,
    method: &abstract_operations_to_target_operations::effects::provider_plan::ServiceMethod,
    ...
    let receiver_access = match self_receiver {
        ...
        {
            symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Named { .. } => None,
...
fn resolve_family_adapter_row(
    ...
    row: &abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanRow,
    method: &abstract_operations_to_target_operations::effects::provider_plan::ServiceMethod,
    receiver_trait: &symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::TraitDefinition,
    requirement_owner: &symbol_resolved_trees_to_typed_trees::typed_trees::trait_definition::TraitDefinition,
