...
use crate::selected_dispatch::selected_dispatch::float_intrinsic::named_float_realizations::{
...
pub(crate) fn selected_compiler_intrinsic_realization(
    ...
    crate::provider_planning::compiler_intrinsic_diagnostic_label_for(typed, &requirement)
        .ok_or_else(|| {
            Diagnostic::error(format!(
    ...
    if let Some(identity) =
        crate::provider_planning::primitive_integer_comparison_intrinsic_execution_identity_for(
            typed,
            &requirement,
        )
    {
