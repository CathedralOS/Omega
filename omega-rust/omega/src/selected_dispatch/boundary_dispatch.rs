...
pub fn selected_boundary_family_specializations(
    ...
    for plan in selected_plans.plans() {
        for row in &plan.rows {
            ...
            let Ok(adapter) = crate::provider_planning::exact_checked_adapter(typed, plan, row)
            else {
                continue;
            };
...
fn selected_service_plan_matches_requirement(
    ...
    let Some(schema) = typed
        ...
        .and_then(|definition| {
            crate::provider_planning::service_schema::from_typed(typed, definition)
        })
    else {
        return false;
    };
...
> {
    ...
    for data in typed.data_definitions() {
        for member in typed.data_members(data) {
            ...
            let symbol = if let Some(requirement) =
                symbol_resolved_trees_to_typed_trees::typed_trees::service::exact_bound_service_requirement(typed, field.type_reference)
            {
                ...
                let symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Named { symbol, .. } = typed
    ...
    if !self_adapters.is_empty() {
        ...
        let member_field_types = |type_reference, member_name: &str| {
                    ...
                    .find(|data| data.symbol == data_symbol)
            {
                for member in typed.data_members(owner) {
                    if let symbol_resolved_trees_to_typed_trees::typed_trees::data::DataMember::Field(field) = member
        ...
        let receiver_place_types =
            |root: symbols::SymbolHandle, members: &[symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier]| {
                let mut type_references = Vec::new();
                ...
                for machine in typed.machines() {
                    ...
                    for state in typed.machine_states(machine) {
                        ...
                        for statement in typed.statement_table.statements(state.statement_nodes) {
                            if let symbol_resolved_trees_to_typed_trees::typed_trees::statement::StatementNode::LocalData(local) =
                                statement
                                && local.symbol == root
                            {
        ...
        let mut register_receiver_place =
            |root: symbols::SymbolHandle,
             members: &[symbol_resolved_trees_to_typed_trees::typed_trees::name::Identifier],
        ...
        for (_, expression) in typed.expression_table.expression_entries() {
            ...
            let leaf = match typed.expression_table.expression(call.receiver) {
                symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Member(member) => member.member_symbol,
                symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Name(path) => path.symbol,
                _ => continue,
            };
            ...
            let root = loop {
                match typed.expression_table.expression(cursor) {
                    ...
                    symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode::Name(path) => break path.symbol,
    ...
    for machine in typed.machines() {
        for state in typed.machine_states(machine) {
            for parameter in typed.state_parameters(state) {
                let symbol_resolved_trees_to_typed_trees::typed_trees::types::TypeReferenceNode::Reference { referee, .. } = typed
                    .type_reference_table
    ...
    for receiver in boundary_fields {
        ...
        {
            dispatch.push(
                typed_trees_to_checked_trees::checked_trees::CheckedBoundaryAdapterDispatch {
