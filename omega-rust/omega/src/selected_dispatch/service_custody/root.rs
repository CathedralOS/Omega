...
use symbol_resolved_trees_to_typed_trees::typed_trees::service::ExactServiceCarrier;
use symbol_resolved_trees_to_typed_trees::typed_trees::types::{
    TypeReferenceHandle, TypeReferenceNode,
};
use typed_trees_to_checked_trees::checked_trees::{
    CheckedUnitStructuralFieldPlan, CheckedUnitStructuralTypePlan,
};
...
pub fn derive_fused_program_entry_establishments(
    checked: &CheckedTrees,
    source: &resolved_layout_to_resolved_layout::program_entry_plan::SelectedProgramEntrySourceSignature,
    selected: &[SelectedProviderReviewProvenance],
    permit_unsettled: bool,
) -> Result<Vec<resolved_layout_to_resolved_layout::program_entry_plan::ProgramEntryFusedServiceEstablishment>, Vec<Diagnostic>>{
    ...
    for (field, carrier, field_path) in service_fields {
        ...
        let Some(schema) =
            crate::provider_planning::service_schema::from_typed(checked, requirement)
        else {
            diagnostics.push(Diagnostic::error(format!(
        ...
        let row = resolved_layout_to_resolved_layout::program_entry_plan::ProgramEntryFusedServiceEstablishment::new(
            source.identity(),
            source.target_slot(),
            ...
            resolved_layout_to_resolved_layout::program_entry_plan::ProgramEntryFusedServiceEstablishment::requirement_identity_for_schema(&schema),
...
fn collect_service_fields<'a>(
    ...
    for member in checked.data_members(source) {
        ...
        let field_identity = data_field_identity(field);
        match symbol_resolved_trees_to_typed_trees::typed_trees::service::classify_exact_bound_service_carrier(
            checked,
