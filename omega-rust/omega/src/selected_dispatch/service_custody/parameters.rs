...
fn validate_receipt(
    ...
    source: &symbol_resolved_trees_to_typed_trees::typed_trees::signature::StateParameter,
    position: usize,
    requirement: symbols::SymbolHandle,
    parameter: &CheckedUnitStructuralParameterPlan,
    receipt: &typed_trees_to_checked_trees::checked_trees::CheckedFusedServiceParameterReceipt,
    plan: &typed_trees_to_checked_trees::checked_trees::CheckedUnitEffectMachinePlan,
    diagnostics: &mut Vec<Diagnostic>,
) {
    ...
    let calls = checked
        ...
        .collect::<Vec<_>>();
    if calls.is_empty() {
        if let Err(reason) = forwarding::validate(checked, machine, state, parameter, receipt, plan)
        {
            diagnostics.push(Diagnostic::error(format!(
...
) -> (String, Vec<language_semantics::SemanticDomainId>) {
