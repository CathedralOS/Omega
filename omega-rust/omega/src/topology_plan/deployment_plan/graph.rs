...
use crate::topology_plan::deployment_plan::{
    Binding, EndpointDirection, EndpointKey, InstanceName, PlanInstance,
};
...
impl NormalizedGraph {
    ...
    pub fn new(
        ...
        if instances.len() > crate::topology_plan::deployment_plan::codec::MAX_INSTANCES {
            return Err(GraphError::LimitExceeded { what: "instances" });
        }
        if bindings.len() > crate::topology_plan::deployment_plan::codec::MAX_BINDINGS {
        ...
        for instance in &mut instances {
            ...
            if instance.endpoints.len()
                > crate::topology_plan::deployment_plan::codec::MAX_ENDPOINTS_PER_INSTANCE
            {
