...
use abstract_operations_to_target_operations::function_identity::MachineFunctionIdentity;
use installation_evidence::InstalledArtifactOccurrenceDigest;
use resolved_layout_to_resolved_layout::boundary_applications::BoundaryOpaqueRepresentationApplications;
...
pub struct InstalledArtifactMemoryImages {
    layout: resolved_layout_to_resolved_layout::image::FinalImageLayout,
...
impl InstalledArtifactMemoryImages {
    pub const fn layout(&self) -> resolved_layout_to_resolved_layout::image::FinalImageLayout {
        self.layout
    }
    ...
    if object.target().object_format == target::ObjectFormat::MachO {
        let pairing = match object.target().architecture {
            ...
            target::Architecture::X86_64 => {
                resolved_layout_to_resolved_layout::image_macho::validate_macho_x86_64_import_binding_pairing(
                    &output.final_text_bytes,
                    &output.executable_regions,
                    &output.data_regions,
                )
