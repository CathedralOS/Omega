...
pub(super) fn verify_macos_cache_link_extended_acl_custody(
    ...
    let has_allow_entry = crate::platform_custody::extended_acl_has_allow_entry(
        path,
        crate::platform_custody::SymbolicLinkBehavior::InspectLink,
    )
    .map_err(|error| {
        cache_custody_invalid(
            kind,
...
pub(crate) fn verify_macos_open_cache_extended_acl_custody(
    ...
    let has_allow_entry = crate::platform_custody::open_file_extended_acl_has_allow_entry(file)
        .map_err(|error| {
            cache_custody_invalid(
                kind,
                path,
