...
use crate::package_source::git::git_command::*;
use crate::package_source::git::request::GitExecutionTransport;
use crate::package_source::git::snapshot::preflight_git_snapshot;
use crate::package_source::identity::GitObjectIdAlgorithm;
use crate::package_source::limits::*;
use crate::package_source::snapshot::permissions::make_tree_owner_writable;
...
fn git_blob_batch_uses_one_bounded_launch_for_many_files() {
    ...
    assert!(
        executor.captured_output_budget.observed() <= executor.captured_output_budget.ceiling()
    );
