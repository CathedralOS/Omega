...
> {
    ...
    resolved_layout_to_resolved_layout::boundary_applications::TerminalBoundaryApplicationCoverage::new(demands, realizations)
        .map_err(|message| vec![Diagnostic::error(message)])
...
fn project_terminal_boundary_application_realizations(
    checked: &CheckedCompilation,
    checked_scope: &lowered_psi_to_terminal_psi::CheckedBoundaryOperatorApplicationScope,
    demands: &resolved_layout_to_resolved_layout::boundary_applications::TerminalBoundaryApplicationDemands,
) -> Result<resolved_layout_to_resolved_layout::boundary_applications::TerminalBoundaryApplicationRealizations, Vec<Diagnostic>>{
    let nongeneric =
        crate::selected_dispatch::derive_checked_nongeneric_operator_application_realizations(
            checked,
    ...
    for (demand, occurrence) in demands.rows().iter().zip(checked_scope.occurrences()) {
        ...
        let (selected_plan_digest, realization) = match (
            ...
            ([row], []) => (
                *row.provider_plan_commitment.as_bytes(),
                resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplicationRealization::NongenericCheckedBody {
            ...
            ([], [row]) => (
                *row.provider_plan_commitment.as_bytes(),
                resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplicationRealization::SpecializedCheckedBody {
        ...
        rows.push(
            resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplicationRealizationCompanion::new(
    ...
    resolved_layout_to_resolved_layout::boundary_applications::TerminalBoundaryApplicationRealizations::new(demands, rows)
        .map_err(|message| vec![Diagnostic::error(message)])
}

fn project_compiler_intrinsic_application_realization(
    checked: &CheckedCompilation,
    application: &typed_trees_to_checked_trees::checked_trees::CheckedBoundaryOperatorApplicationDemand,
) -> Result<
    (
        [u8; 32],
        resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplicationRealization,
    ),
    Vec<Diagnostic>,
> {
            ...
            checked
                ...
                .filter_map(|(_, operator_use)| {
                    (operator_use.application_site() == application.site
                        && operator_use.selected_operator_symbol == application.requirement_symbol
                        && operator_use.status
                            == typed_trees_to_checked_trees::checked_trees::CheckedOperatorResolutionStatus::Resolved)
                        .then_some(operator_use.origin)
                }),
        )
        .chain(
    ...
    let Some((plan_index, plan)) = crate::provider_planning::selected_use_plan(
        checked,
        plans,
    ...
    if !matches!(
        row.binding,
        abstract_operations_to_target_operations::effects::provider_plan::ProviderBinding::CompilerIntrinsic { .. }
    ) {
        return Err(vec![Diagnostic::error(
    ...
    let derived =
        crate::selected_dispatch::derive_selected_compiler_intrinsic_execution_identity_for_row_with_resolved_binding(
            checked,
            plan,
            ...
            **realization,
            ...
            checked.resolved_semantic_binding(
                crate::package_compilation::AcceptedSemanticBindingRole::ConsoleExitProcessI32,
            ),
            checked.resolved_semantic_binding(
                crate::package_compilation::AcceptedSemanticBindingRole::ProcessExitExitProcessI32,
            ),
        )
        .map_err(|diagnostic| vec![diagnostic])?;
    let execution = match (derived, **retained_execution) {
        (
            Some(crate::selected_dispatch::SelectedCompilerIntrinsicExecutionIdentity::Closed(
                derived,
            )),
            Some(retained),
    ...
    Ok((
        *plan.identity_digest().as_bytes(),
        resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplicationRealization::ExactCompilerIntrinsic { execution },
    ))
}
...
fn project_terminal_boundary_application_demands(
    ...
    resolved_layout_to_resolved_layout::boundary_applications::TerminalBoundaryApplicationDemands,
    Vec<Diagnostic>,
> {
    ...
    for occurrence in checked_scope.occurrences() {
        ...
        let requirement_view = crate::provider_planning::IntrinsicRequirement::by_symbol(
            &checked.typed,
            application.requirement_symbol,
        )
        .filter(|view| {
            view.kind != crate::provider_planning::IntrinsicRequirementKind::Operator
        ...
        let requirement =
            resolved_layout_to_resolved_layout::boundary_applications::BoundaryOperatorRequirement::new(declaration, overload)
                .map_err(|message| vec![Diagnostic::error(message)])?;
        let projected_application = project_boundary_application(checked, application)?;
        rows.push(
            resolved_layout_to_resolved_layout::boundary_applications::TerminalBoundaryApplicationDemand::new(
    ...
    resolved_layout_to_resolved_layout::boundary_applications::TerminalBoundaryApplicationDemands::new(
...
fn project_boundary_application(
    ...
    resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplication,
    Vec<Diagnostic>,
> {
    if application.arguments.is_empty() {
        return Ok(
            resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplication::Empty,
        );
    }
    ...
    for (ordinal, argument) in application.arguments.iter().enumerate() {
        ...
        match argument {
            typed_trees_to_checked_trees::checked_trees::CheckedBoundaryOperatorApplicationArgument::Type {
                binder_owner,
                binder_ordinal,
                type_reference,
                ..
            ...
            {
                arguments.push(
                    resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplicationArgument::type_argument(
                        *binder_ordinal,
                        canonical_boundary_type_identity(checked, *type_reference)?,
                    ),
                );
            }
            typed_trees_to_checked_trees::checked_trees::CheckedBoundaryOperatorApplicationArgument::Const {
                binder_owner,
                binder_ordinal,
                declared_carrier,
                value,
                ...
                && *binder_ordinal == expected_ordinal =>
            {
                typed_trees_to_checked_trees::validation::validate_exact_const_value_encoding(
                    &checked.typed,
                    *declared_carrier,
                ...
                arguments.push(
                    resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplicationArgument::const_argument(
                        *binder_ordinal,
    ...
    resolved_layout_to_resolved_layout::boundary_applications::BoundaryApplication::exact(arguments)
        .map_err(|message| vec![Diagnostic::error(message)])
}
...
fn canonical_boundary_nominal_identity(
    ...
    resolved_layout_to_resolved_layout::boundary_applications::BoundaryNominalIdentity,
    Vec<Diagnostic>,
> {
    ...
    resolved_layout_to_resolved_layout::boundary_applications::BoundaryNominalIdentity::new(
...
fn canonical_boundary_type_identity(
    ...
    resolved_layout_to_resolved_layout::boundary_applications::BoundaryTypeIdentity,
    Vec<Diagnostic>,
> {
    let identity = checked
    ...
    resolved_layout_to_resolved_layout::boundary_applications::BoundaryTypeIdentity::new(
        identity.into_string(),
    )
    .map_err(|message| vec![Diagnostic::error(message)])
}
