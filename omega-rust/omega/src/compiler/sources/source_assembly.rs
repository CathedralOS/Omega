...
pub struct AssembledSyntax {
    ...
    /// never re-derives it.
    pub application: Option<crate::build_declarations::ApplicationDeclaration>,
    pub source_scoped_top_level_bindings: Vec<symbols::SourceScopedTopLevelBinding>,
    pub generated_source_custody: Vec<(
        source::SourceId,
...
pub fn retain_generated_syntax_extension(
    ...
    generated_sources: &[crate::build_output::PackageGeneratedSource],
...
> {
    ...
    for (purpose, bundle) in package_inputs.dependency_generated_source_instances() {
        let scope = match purpose {
            ...
            crate::build_declarations::DependencyPurpose::Build => source::DependencyScope::Build,
        };
...
fn generated_source_logical_path(
    package_root: &Path,
    source: &crate::build_output::PackageGeneratedSource,
...
fn validate_selected_build_role(
...
) -> Result<Option<crate::build_declarations::ApplicationDeclaration>, Vec<Diagnostic>> {
    ...
    crate::build_declarations::project_build_declaration_from_source(&source.source)
        .map(|declaration| match declaration {
            crate::build_declarations::BuildDeclaration::Application(application) => {
                Some(application)
            }
            _ => None,
        })
...
fn validate_package_source_frontier(
    ...
    let toolchain_root = crate::compiler::sources::frontend::bundled_core_root();
