...
use crate::compiler::sources::frontend::{extend_source_storage, lex_sources, parse_sources};
...
pub(super) fn inject_build_prelude(
    ...
    for (_, file) in source_storage.files.iter() {
        ...
        for root_item in &file.root_items {
            match source_storage.syntax_trees.root_item(*root_item) {
                ...
                tokens_to_syntax_trees::syntax_trees::item::Item::Data(data)
    ...
    let lexed = timings.record(SOURCE_FILES_TO_TOKENS, || {
        let sources = crate::compiler::sources::frontend::load_injected_source(
            "<build-prelude>",
            &prelude,
            first_source_id,
        );
