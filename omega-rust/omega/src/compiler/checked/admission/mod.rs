...
use crate::trust_model::{TrustAdmission, TrustAdmissionSettlement};
...
pub fn admit_checked_compilation(
    ...
    let obligations = crate::trust_model::reconstruct_trust_obligations(
        &checked.typed,
        checked,
        checked.root_grants(),
    ...
    let settlement = crate::trust_model::settle_trust_admissions(obligations, accepted)
        .map_err(|diagnostic| vec![diagnostic])?;
    let trust_report = crate::trust_model::reconstruct_trust_report(
        checked,
        checked.root_grants(),
        checked.provider_plans(),
        checked.selected_provider_plans(),
