...
use abstract_operations_to_target_operations::effects::SelectedProviderPlanFacts;
use diagnostics::Diagnostic;
use symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees;
use symbol_resolved_trees_to_typed_trees::typed_trees::expression::ExpressionNode;
use typed_trees_to_checked_trees::checked_trees::{
    CheckedOperatorResolutionStatus, CheckedProviderPlanCommitment, CheckedTrees,
};
...
pub(super) fn selected_operators(
    ...
    for fact in facts.uses_with_status(CheckedOperatorResolutionStatus::Resolved) {
        ...
        let Some((operation, format)) =
            symbol_resolved_trees_to_typed_trees::typed_trees::operator::primitive_float_binary_semantics(typed, operator)
        else {
            continue;
        };
        let identity =
            symbol_resolved_trees_to_typed_trees::typed_trees::operator::boundary_operator_requirement_identity(typed, operator);
        ...
        let execution = crate::selected_dispatch::derive_selected_primitive_float_binary_execution(
            typed,
            plan,
            operator.symbol,
        )
        ...
        let expected =
            crate::provider_planning::primitive_float_binary_intrinsic_execution_identity(
                typed, operator,
            );
    ...
    crate::build_time_evaluation::validate_selected_operators(typed, &selected)
...
pub(super) fn selected_provider_bodies(
    ...
    crate::build_time_evaluation::validate_selected_provider_bodies(typed, &selected)
        .map_err(|reason| vec![Diagnostic::error(reason)])?;
    Ok(selected)
}
...
enum SelectedOperatorUse<'a> {
    Spelled(&'a typed_trees_to_checked_trees::checked_trees::CheckedOperatorUseFact),
    Named(&'a typed_trees_to_checked_trees::checked_trees::CheckedNamedOperatorUseFact),
}
...
fn selected_provider_body_row(
    ...
    let abstract_operations_to_target_operations::effects::provider_plan::ProviderBinding::CheckedAdapter { .. } = &row.binding else {
        return Ok(None);
    };
    ...
    let provider = crate::provider_planning::exact_checked_adapter(typed, plan, row)?;
...
pub(super) fn require_evaluated_array_lengths(
    ...
    let pending = program
        ...
        .fixed_array_lengths()
        .filter_map(|(_, length)| {
            let symbol_resolved_trees_to_typed_trees::typed_trees::types::FixedArrayLength::ConstCall { name, source_span } = length
            else {
                return None;
            };
            Some(
...
impl SelectedConstEvaluation {
    pub(super) fn validate(
        ...
        plans: &SelectedProviderPlanFacts,
        packages: Option<&crate::package_compilation::PackageCompilationInputs>,
            ...
            let matching: Vec<_> = checked
                ...
                .filter(|fact| {
                    fact.expression == selected.expression
                        ...
                        && fact.occurrence == typed_trees_to_checked_trees::checked_trees::CheckedOperatorOccurrence::Expression
                })
                .collect();
        ...
        let authority = packages.map(|packages| {
            std::sync::Arc::new(packages.clone())
                as std::sync::Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>
        });
        crate::build_time_evaluation::validate_folded_array_lengths(
            &checked.typed,
            &self.folds,
            &self.operators,
...
fn joined_plan_commitment(
    ...
    crate::provider_planning::selected_use_plan(checked, plans.plans(), requirement, origin).map(
