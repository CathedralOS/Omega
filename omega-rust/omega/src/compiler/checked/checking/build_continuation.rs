...
pub(crate) struct BuiltCheckedProgram {
    pub(super) typed: symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees,
    pub(super) selected_target_machine_declarations:
        crate::build_evaluation::target_machines::SelectedTargetMachineDeclarations,
    pub(super) pending_pre_checks: Vec<crate::build_time_evaluation::PreCheckEvaluation>,
    pub(super) computed_build_config: crate::build_evaluation::ComputedBuildConfig,
...
pub(crate) struct BuildSourceCustody {
    ...
    pub(super) own_generated_sources: Vec<crate::build_output::PackageGeneratedSource>,
    pub(super) base_source_consumption_commitment:
        Option<crate::package_compilation::PackageSourceConsumptionCommitment>,
}
...
pub(crate) fn evaluate_build_and_continue(
    ...
    syntax: crate::compiler::sources::AssembledSyntax,
    ...
    let package_authority_verdict = if let Some(package_inputs) = package_inputs {
        Some(crate::compiler::checked::package::declaration_admission::validate_authored_declaration_selections_before_build(
            frontend.typed(),
            package_inputs,
    ...
    let build_machine_filesystem_scope = crate::build_evaluation::prepare_filesystem_scope(
        root_path,
        package_inputs,
        &base_sources,
        build_execution_profile,
    ...
    let admitted_build = crate::build_evaluation::admit_build_program(
        frontend.typed(),
    ...
    if !computed_build_config.generated_sources.is_empty() {
        ...
        let extension = crate::compiler::sources::retain_generated_syntax_extension(
            &base_sources,
            package_root,
...
struct CheckedFrontend {
    ...
    pending_pre_checks: Vec<crate::build_time_evaluation::PreCheckEvaluation>,
}

enum CheckedFrontendTyping {
    Continuable(symbol_resolved_trees_to_typed_trees::SeededTypingBase),
    Complete(symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees),
}

impl CheckedFrontendTyping {
    fn typed(&self) -> &symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees {
        match self {
    ...
    fn into_typed(self) -> symbol_resolved_trees_to_typed_trees::typed_trees::TypedTrees {
        match self {
...
struct AdmittedBuildCheckpoint {
    frontend: CheckedFrontend,
    admitted_build: crate::build_evaluation::AdmittedBuildProgram,
    package_authority_verdict:
        Option<crate::compiler::checked::package::declaration_admission::AuthoredDeclarationAuthorityVerdict>,
...
struct ExecutedBuildCheckpoint {
    frontend: CheckedFrontend,
    computed_build_config: crate::build_evaluation::ComputedBuildConfig,
    restricted_build_requests: Vec<crate::build_evaluation::RestrictedBuildRequest>,
    package_authority_verdict:
        Option<crate::compiler::checked::package::declaration_admission::AuthoredDeclarationAuthorityVerdict>,
    base_sources: Arc<source::SourceMap>,
}
...
fn lower_checked_frontend(
    mut syntax: crate::compiler::sources::AssembledSyntax,
    ...
    let selected_target_machine_declarations =
        crate::build_evaluation::target_machines::filter_target_machines_by_scope(
            &mut syntax.syntax_trees,
            target_name,
            execution_profile_name,
            &build_scope_sources,
        )?;
    let evaluated = crate::build_time_evaluation::evaluate_pre_resolution(
        crate::build_time_evaluation::BuildTimeEvaluationRequest {
            syntax_trees: syntax.syntax_trees,
            source_context: Some(crate::build_time_evaluation::BuildTimeSourceContext {
                ...
                selection_authority: package_inputs.map(|inputs| {
                    Arc::new(inputs.clone())
                        as Arc<dyn crate::build_time_evaluation::BuildTimeSelectionAuthority>
                }),
                retained_base: None,
            }),
        },
    ...
    typed_trees_to_checked_trees::validation::resolve_dynamic_call_targets(
        typing_base.typed_mut(),
    )?;
    Ok(CheckedFrontend {
...
fn try_seeded_extension(
    ...
    base_sources: &Arc<source::SourceMap>,
    extension: crate::compiler::sources::RetainedGeneratedSyntaxExtension,
    selected_target_machine_declarations:
        crate::build_evaluation::target_machines::SelectedTargetMachineDeclarations,
    target_name: Option<&str>,
    package_inputs: Option<&PackageCompilationInputs>,
    ...
    (
        ...
        crate::build_evaluation::target_machines::SelectedTargetMachineDeclarations,
        Vec<crate::build_time_evaluation::PreCheckEvaluation>,
    ),
    Vec<Diagnostic>,
> {
    ...
    let mut extension_syntax = tokens_to_syntax_trees::syntax_trees::SyntaxTrees::new(
        source::SourceId(base_sources.len()),
    );
