...
//! the complete [`resolved_layout_to_resolved_layout::image::PlacedExecutableRegionInventory`] over the text, the
//! complete [`resolved_layout_to_resolved_layout::image::PlacedDataRegionInventory`] over each data extent, and
...
//! [`resolved_layout_to_resolved_layout::image::validate_placed_executable_region_inventory`] and
//! [`resolved_layout_to_resolved_layout::image::validate_placed_data_region_inventory`] — region and gap digests,
...
//! [`resolved_layout_to_resolved_layout::image_macho::validate_macho_aarch64_import_binding_pairing`], which also
...
//! [`resolved_layout_to_resolved_layout::image_pe::validate_pe_x86_64_import_binding_pairing`], which decodes
...
pub struct NativePlacedImageEvidence {
    ...
    inventory: resolved_layout_to_resolved_layout::image::PlacedExecutableRegionInventory,
    /// Byte offset inside the published artifact at which the exact final
    ...
    data_inventory: resolved_layout_to_resolved_layout::image::PlacedDataRegionInventory,
    /// Byte offset inside the published artifact at which the exact final
    ...
    import_data_inventory: resolved_layout_to_resolved_layout::image::PlacedDataRegionInventory,
}
...
fn import_thunk_form(target: target::NativeTarget) -> Option<(usize, StateFootprintEvidence)> {
    match (target.architecture, target.object_format) {
        (target::Architecture::X86_64, target::ObjectFormat::Coff) => Some((
            target_operations_to_selected_instructions::isa_x86_64::X86_64_IMPORT_THUNK_BYTE_COUNT,
            target_operations_to_selected_instructions::isa_x86_64::x86_64_import_thunk_footprint(),
        )),
...
impl NativePlacedImageEvidence {
    ...
    pub fn from_artifact(
        artifact: &crate::compiler::report::RetainedNativeArtifact,
        executable_bytes: &[u8],
    ) -> Result<Self, String> {
    ...
    pub const fn inventory(
        &self,
    ) -> &resolved_layout_to_resolved_layout::image::PlacedExecutableRegionInventory {
        &self.inventory
    }
    ...
    pub const fn data_inventory(
        &self,
    ) -> &resolved_layout_to_resolved_layout::image::PlacedDataRegionInventory {
        &self.data_inventory
    }
    ...
    pub const fn import_data_inventory(
        &self,
    ) -> &resolved_layout_to_resolved_layout::image::PlacedDataRegionInventory {
    ...
    pub fn replay_against(&self, executable_bytes: &[u8]) -> Result<(), String> {
        ...
        for region in &self.inventory.regions {
            if region.origin != resolved_layout_to_resolved_layout::image::FinalExecutableRegionOrigin::ImportThunk {
                continue;
            }
            match (self.target.architecture, self.target.object_format) {
                ...
                | (target::Architecture::X86_64, target::ObjectFormat::MachO) => {
                    ...
                    if target_operations_to_selected_instructions::isa_x86_64::decode_x86_64_import_thunk(bytes).is_none() {
                        return Err(format!(
        ...
        match (self.target.architecture, self.target.object_format) {
            ...
            (target::Architecture::X86_64, target::ObjectFormat::Coff) => {
                resolved_layout_to_resolved_layout::image_pe::validate_pe_x86_64_import_binding_pairing(
                    text_bytes,
                    &self.inventory,
                    &self.import_data_inventory,
                )
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        for region in &inventory.regions {
            writer.u8(match region.origin {
                resolved_layout_to_resolved_layout::image::FinalExecutableRegionOrigin::CompilerFunction => 1,
                resolved_layout_to_resolved_layout::image::FinalExecutableRegionOrigin::ImportThunk => 2,
            });
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, NativeEvidenceError> {
        ...
        let text_digest =
            resolved_layout_to_resolved_layout::image::FinalExecutableTextDigest::from_digest(
                reader.array()?,
            );
        let text_report_fingerprint = reader.u64()?;
        let inventory_digest =
            resolved_layout_to_resolved_layout::image::PlacedExecutableRegionInventoryDigest::from_digest(reader.array()?);
        ...
        for _ in 0..region_count {
            let origin = match reader.u8()? {
                1 => resolved_layout_to_resolved_layout::image::FinalExecutableRegionOrigin::CompilerFunction,
                2 => resolved_layout_to_resolved_layout::image::FinalExecutableRegionOrigin::ImportThunk,
                tag => {
            ...
            let byte_digest =
                resolved_layout_to_resolved_layout::image::PlacedExecutableRegionBytesDigest::from_digest(reader.array()?);
            ...
            if origin == resolved_layout_to_resolved_layout::image::FinalExecutableRegionOrigin::ImportThunk {
            ...
            regions.push(
                resolved_layout_to_resolved_layout::image::PlacedExecutableRegion {
                    origin,
                    section_offset,
                    address,
                    byte_count,
        ...
        for _ in 0..gap_count {
            ...
            unclassified_gaps.push(resolved_layout_to_resolved_layout::image::PlacedExecutableGap {
                section_offset,
                address: reader.u64()?,
                byte_count: reader.usize("gap byte count")?,
                byte_digest: resolved_layout_to_resolved_layout::image::PlacedExecutableGapBytesDigest::from_digest(reader.array()?),
        ...
        for region in &import_data_inventory.regions {
            if region.origin != resolved_layout_to_resolved_layout::image::FinalDataRegionOrigin::ImportBindingSlot {
                return Err(malformed(format!(
...
impl EvidenceWriter {
    ...
    fn data_inventory(
        &mut self,
        inventory: &resolved_layout_to_resolved_layout::image::PlacedDataRegionInventory,
    ) {
        ...
        for region in &inventory.regions {
            self.u8(match region.origin {
                resolved_layout_to_resolved_layout::image::FinalDataRegionOrigin::CompilerData => 1,
                resolved_layout_to_resolved_layout::image::FinalDataRegionOrigin::ImportBindingSlot => 2,
                resolved_layout_to_resolved_layout::image::FinalDataRegionOrigin::AlignmentPadding => 3,
            });
...
impl<'a> EvidenceReader<'a> {
    ...
    > {
        ...
        let data_digest =
            resolved_layout_to_resolved_layout::image::FinalInitializedDataDigest::from_digest(
                self.array()?,
            );
        let data_report_fingerprint = self.u64()?;
        let inventory_digest =
            resolved_layout_to_resolved_layout::image::PlacedDataRegionInventoryDigest::from_digest(
                self.array()?,
            );
        ...
        for _ in 0..region_count {
            let origin = match self.u8()? {
                1 => resolved_layout_to_resolved_layout::image::FinalDataRegionOrigin::CompilerData,
                2 => resolved_layout_to_resolved_layout::image::FinalDataRegionOrigin::ImportBindingSlot,
                3 => resolved_layout_to_resolved_layout::image::FinalDataRegionOrigin::AlignmentPadding,
                tag => {
            ...
            let byte_digest =
                resolved_layout_to_resolved_layout::image::PlacedDataRegionBytesDigest::from_digest(
                    self.array()?,
                );
            ...
            regions.push(
                resolved_layout_to_resolved_layout::image::PlacedDataRegion {
                    origin,
                    section_offset,
                    address,
                    byte_count,
        ...
        for _ in 0..gap_count {
            ...
            previous_gap_offset = Some(section_offset);
            unclassified_gaps.push(resolved_layout_to_resolved_layout::image::PlacedDataGap {
                section_offset,
                address: self.u64()?,
                ...
                byte_digest: resolved_layout_to_resolved_layout::image::PlacedDataGapBytesDigest::from_digest(self.array()?),
