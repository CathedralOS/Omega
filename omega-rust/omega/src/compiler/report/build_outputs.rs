...
use crate::build_evaluation::{BuildObservationIdentity, BuildObservationSummary};
use crate::build_output::{BuildStagedOutputEntryKind, BuildStagedOutputTree};
...
impl RetainedBuildOutputs {
    ...
    pub(crate) fn publish(&mut self, build_dir: &Path) -> Result<(), String> {
        ...
        if fresh {
            ...
            crate::compiler::report::executable_publication::publish_exact_file_bytes(
                &manifest,
                &self.manifest,
            )?;
        } else {
