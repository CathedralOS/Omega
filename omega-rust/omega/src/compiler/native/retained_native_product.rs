...
use abstract_operations_to_target_operations::task_plans::AdmittedSameStackContribution;
...
pub struct RetainedNativeRealizationRequest<'request> {
    ...
    pub terminal_authority_policy: crate::compiler::native::TerminalAuthorityPolicy,
    ...
    pub terminal_authority_permission_policy:
        Option<crate::compiler::native::TerminalAuthorityPermissionPolicy>,
    pub image_request: crate::compiler::native::ExecutableImageEmissionRequest,
...
pub fn realize_retained_native_artifact(
    retained: crate::compiler::report::RetainedTerminalArtifact,
    request: RetainedNativeRealizationRequest<'_>,
) -> Result<
    crate::compiler::native::RequestedNativeArtifact,
    (
        crate::compiler::native::ExecutableImageEmissionRequest,
        Vec<Diagnostic>,
    ),
> {
    ...
    let result = (|| {
        ...
        if let crate::compiler::native::ExecutableImageEmissionRequest::Direct {
            subsystem,
        ...
        crate::compiler::native::validate_retained_package_terminal_authority_permissions(
        ...
        if let Some(receiving_policy) = &terminal_authority_permission_policy {
            crate::compiler::native::validate_package_terminal_authority_permissions(
                accepted_package_terminal_authority_permission_policy
                    .rows()
                    .iter(),
                receiving_policy,
        ...
        let demanded =
            crate::provider_planning::compiler_intrinsics::demanded_boundary_identities(&module)?;
        ...
        let compiler_builtins = proposal
            ...
            .map(|builtin| {
                ...
                Ok(crate::compiler::native::NativeCompilerBuiltinSettlement {
        ...
        crate::compiler::native::realize_native_artifact(
            artifact,
            crate::compiler::native::NativeRealizationRequest {
...
fn admitted_native_callback_thunks<'artifact>(
    placements: &'artifact [crate::backend_plan::BoundNominalCallbackPlacement],
    occurrences: &'artifact [crate::compiler::report::TerminalCallbackOccurrenceProposal],
) -> Result<Vec<crate::compiler::native::NativeCallbackThunkSettlement<'artifact>>, Vec<Diagnostic>>
{
    ...
    for occurrence in occurrences {
        ...
        let expected_symbol = crate::backend_plan::canonical_callback_private_symbol(placement);
        let expected_function = crate::backend_plan::canonical_callback_thunk_identity(
        ...
        admitted.push(crate::compiler::native::NativeCallbackThunkSettlement {
...
fn admitted_native_callbacks(
    placements: &[crate::backend_plan::BoundNominalCallbackPlacement],
    occurrences: &[crate::compiler::report::TerminalCallbackOccurrenceProposal],
) -> Result<
    ...
    for occurrence in occurrences {
        ...
        if materialization
            ...
            || materialization.destination
                != abstract_operations_to_target_operations::calling_conventions::NativePlace::Parameter(application.parameter)
        {
            return Err(diagnostic(
                "Terminal callback custody",
...
fn exact_demanded_import_plans<'proposal>(
    proposal: &'proposal crate::compiler::report::TerminalNativeRealizationProposal,
    demanded: &BTreeSet<String>,
) -> Result<
    BTreeMap<
        String,
        &'proposal abstract_operations_to_target_operations::effects::provider_plan::ProviderPlan,
    >,
    Vec<Diagnostic>,
> {
                ...
                plan.rows
                    ...
                    .filter(move |row| {
                        row.requirement_identity == *requirement
                            && matches!(
                                row.binding,
                                abstract_operations_to_target_operations::effects::provider_plan::ProviderBinding::Import { .. }
                            )
                    })
                    .map(move |_| plan)
            })
...
fn rejoin_external_import_settlements<'proposal, 'evidence>(
    exact_plans: &BTreeMap<
        String,
        &'proposal abstract_operations_to_target_operations::effects::provider_plan::ProviderPlan,
    >,
    imports: &[SourceEvaluatedImportSettlement<'evidence>],
) -> Result<Vec<crate::compiler::native::NativeProviderSettlement<'proposal>>, Vec<Diagnostic>>
where
    'evidence: 'proposal,
{
    ...
    for import in imports {
        ...
        settlements.push(crate::compiler::native::NativeProviderSettlement {
