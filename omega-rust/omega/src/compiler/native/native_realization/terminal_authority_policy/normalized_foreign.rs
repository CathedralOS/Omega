...
use abstract_operations_to_target_operations::effects::TerminalMechanismIdentity;
...
pub fn normalized_foreign_terminal_mechanism(
    ...
    boundary_entry_plan: &abstract_operations_to_target_operations::calling_conventions::BoundaryEntryPlan,
    ...
    let validated =
        abstract_operations_to_target_operations::calling_conventions::validate_boundary_entry_plan(boundary_entry_plan.clone(), &signature)
            .map_err(|diagnostic| diagnostic.to_string())?;
    if validated.plan() != boundary_entry_plan {
        return Err("normalized foreign boundary plan is not canonical".to_owned());
    }
    Ok(
        abstract_operations_to_target_operations::effects::NormalizedForeignTerminalMechanismIdentity::from_normalized_locator(
            locator,
...
pub fn normalized_foreign_terminal_mechanism_with_callback_materializations(
    locator: &target::NormalizedForeignLocator,
    boundary_entry_plan: &abstract_operations_to_target_operations::calling_conventions::BoundaryEntryPlan,
    context: &abstract_operations_to_target_operations::calling_conventions::CallbackMaterializationContext,
) -> Result<TerminalMechanismIdentity, String> {
    let signature = abstract_operations_to_target_operations::calling_conventions::CallSignature {
        parameters: boundary_entry_plan
            .call
