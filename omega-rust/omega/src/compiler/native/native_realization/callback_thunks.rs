...
use crate::compiler::native::native_realization::input_preparation::lower_realization_input;
...
use post_allocation_machine_to_selected_form_encoding::machine_code::{
    CompilerPrivateMachineCodeFunction, MachineCodeFunction,
};
...
fn lower_callback_thunk(
    ...
    let optimized = crate::compiler::native::optimize_verified_abstract_input(
        input.into_optimization_input(),
        crate::compiler::native::compiler_baseline_request_v1(
    ...
    let physical = crate::compiler::native::stage_optimized_verified_physical_pipeline(
        optimized_target,
    ...
    let emitted = resolved_layout_to_resolved_layout::machine_emission::stage_optimized_function_fragment_emission(emission_source)
        .map_err(|error| realization_error("callback thunk fragment emission", error))?;
    let applied = resolved_layout_to_resolved_layout::machine_emission::stage_function_fragment_frame_application(emitted)
        .map_err(|error| realization_error("callback thunk frame application", error))?;
    let text = resolved_layout_to_resolved_layout::machine_emission::stage_optimized_fixed_frame_text_section(applied)
...
fn materialize_function(
    machine: MachineId,
    targeted: &abstract_operations_to_target_operations::target_operations::TargetFunction,
    bytes: Vec<u8>,
) -> MachineCodeFunction {
    MachineCodeFunction {
        machine,
