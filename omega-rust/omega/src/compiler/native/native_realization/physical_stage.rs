...
use crate::compiler::native::native_realization::realization_diagnostics::{
    realization_error, selected_physical_pipeline_failed,
};
use crate::compiler::native::native_realization::realization_request::NativeRealizationRequest;
use crate::compiler::native::native_realization::target_stage::NativeTargetStageResult;
use diagnostics::Diagnostic;

pub(crate) fn lower_realization_physical_stage(
    target_stage: NativeTargetStageResult,
    request: &NativeRealizationRequest<'_>,
) -> Result<crate::compiler::native::StagedOptimizedVerifiedPhysicalPipeline, Vec<Diagnostic>> {
    let (_, optimized_target) = target_stage
        .into_parts()
        .map_err(|error| realization_error("target program/evidence join", error))?;
    crate::compiler::native::stage_optimized_verified_physical_pipeline(
        optimized_target,
