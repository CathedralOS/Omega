use abstract_operations_to_target_operations::target_operations::NormalizedForeignCallBinding;
use diagnostics::Diagnostic;

pub(super) fn rejoin_normalized_foreign_call(
    selected_plan: &abstract_operations_to_target_operations::effects::provider_plan::ProviderPlan,
    external_binding_rows: &[abstract_operations_to_target_operations::calling_conventions::ExternalBindingRow],
    same_stack: &abstract_operations_to_target_operations::task_plans::AdmittedSameStackContribution,
    provider_plan_report_identity: u64,
    requirement: &str,
    target: target::NativeTarget,
) -> Result<NormalizedForeignCallBinding, Vec<Diagnostic>> {
    let selected_commitment = abstract_operations_to_target_operations::task_plans::SameStackProviderPlanCommitment::from_digest(
