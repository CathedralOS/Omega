...
impl StagedOptimizedVerifiedPhysicalPipeline {
    pub fn fixed_frame_for_test(
        &self,
    ) -> &resolved_layout_to_resolved_layout::machine_emission::StagedFixedFrameFunctionRelativeRealization{
        self.source.replay_for_test().fixed_frame()
    }

    pub fn fixed_frame_mut_for_test(
        &mut self,
    ) -> &mut resolved_layout_to_resolved_layout::machine_emission::StagedFixedFrameFunctionRelativeRealization{
