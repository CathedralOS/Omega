...
fn accepted_console_binding(package: PackageKeyIdentity, marker: u8) -> AcceptedSemanticBinding {
    AcceptedSemanticBinding::new(
        ...
        abstract_operations_to_target_operations::effects::provider_plan::ProviderPlanDigest::from_digest([marker.wrapping_add(1); 32]),
    )
...
fn generated_source(relative_path: &[u8], bytes: &[u8]) -> PackageGeneratedSource {
    let tree =
        crate::build_output::from_entries(&[crate::build_output::OutputTreeEntry::regular_file(
            relative_path,
            bytes,
            false,
        )])
        .expect("test source must form a canonical retained output tree");
    crate::build_output::select_included_sources(&tree, &[relative_path.to_vec()])
