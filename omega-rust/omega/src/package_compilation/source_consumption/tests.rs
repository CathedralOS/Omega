...
fn generated_fixture() -> Vec<(SourceId, crate::build_output::PackageGeneratedSource)> {
    let tree = crate::build_output::from_entries(&[
        crate::build_output::OutputTreeEntry::regular_file(b"first.omg", b"data First {}", false),
        crate::build_output::OutputTreeEntry::regular_file(b"second.omg", b"data Second {}", false),
    ])
    .expect("retained generated files");
    crate::build_output::select_included_sources(
        &tree,
...
fn generated_files(
    custody: &[(SourceId, crate::build_output::PackageGeneratedSource)],
) -> Vec<SourceFile> {
    custody
        .iter()
...
fn generated_coordinates_distinguish_scopes_but_preserve_collision_rejection() {
    ...
    let generated = |bytes: &[u8]| {
        let tree = crate::build_output::from_entries(&[
            crate::build_output::OutputTreeEntry::regular_file(b"shared.omg", bytes, false),
        ])
        .unwrap();
        crate::build_output::select_included_sources(&tree, &[b"shared.omg".to_vec()])
            .unwrap()
            .pop()
            .unwrap()
    };
