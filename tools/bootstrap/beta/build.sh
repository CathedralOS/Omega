#!/usr/bin/env sh
# build.sh PROGRAM.beta -> build/bootstrap/beta/PROGRAM.exe
#
# Pure Beta, no Rust: the assembler (the Alpha VM with the Beta assembler tape in
...
BUILD_DIR="$OMEGA_REPO_ROOT/build/bootstrap/beta"
mkdir -p "$BUILD_DIR"
SEED="${OMEGA_PATH_ALPHA}"/$ALPHA_SEED
materialize_beta_assembler "$BUILD_DIR/assembler" >/dev/null

SRC=${1:-$OMEGA_REPO_ROOT/tests/beta/compiler/examples/multiply.beta}
NAME=$(basename "$SRC" .beta)
OUT="$BUILD_DIR/$NAME.exe"

# 1. .beta text -> bytecode, via the assembler running on the seed
