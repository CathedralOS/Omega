...
- [ ] **PACKAGE-REVIEW-PROJECTION.** Make the compiler-owned ordinary package
  ...
  - static-machine-parameterized external realizations, non-identical
    conformance-bound external refinements, lifetime-parameterized external
    trait realizations, generic external boundary-operator realizations, and
    the remaining selected provider-demand and installation-issuance joins;
