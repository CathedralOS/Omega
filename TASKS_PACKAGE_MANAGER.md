...
- [ ] **PACKAGE-MANAGER-SOURCE-ARCHITECTURE.** Finish replacing the iterative
  ...
  trust boundaries. Remaining work is to split the manager policy/evidence,
  package-source, Git-process, declaration, and large behavioral test modules
  by coherent responsibility; remove the remaining production `use super::*`
