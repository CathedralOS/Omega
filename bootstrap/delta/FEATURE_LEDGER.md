...
| source units and modular organization | the bundle, generic OMGCOMP1 envelope, target/configuration-bearing OMGCOMP2 envelope, explicit root-package build-source role in OMGCOMP3, resolver witness, checked-IR lowering, and independent reconstruction close bounded public multi-unit custody and nominal identity; OMGRSW6 additionally prices one exact requirement, target-applicable bodyless intrinsic candidate, and call-to-requirement without selecting a provider, while accepted-lock authority and private cross-module semantics remain separate | keep package/module/provider semantics in Omega and pass Delta one reconciled graph; decoding that graph does not make those semantics Delta features, and the build-role bit must not turn source labels or candidate discovery into authority | bounded public multi-unit, exact target/configuration/build-role custody, and one resolution-only provider candidate graph demonstrated; selection, executable provider meaning, authority, and general closure open |
...
the current [`CKIR16`](../omega-bootstrap/compiler/OMEGA_BOOTSTRAP_CHECKED_IR_V16.md)
successor, and the current persisted
[`OMGRFN18`](../../source/assurance/refinement/omega-bootstrap/OMGCOMP_REFINEMENT_WITNESS_V18.md)
