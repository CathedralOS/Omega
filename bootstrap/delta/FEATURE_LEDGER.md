...
| records, fixed arrays, and recursively constant aggregates | checkpoint 000001 and selected CKIR3/CKIR4 paths establish bounded nominal layout, constant graphs, runtime named construction, structural Call/Copy, native/self/mixed production, Rust-free meaning, and responsibility-local lower-rooted reconstruction. OMGRSWB11/CKIR19 additionally close actual-capacity arrays of copyable records, chained field places, nine scalar stores, and scalar readback | retain the typed constant graph/pool, aggregate copy, ordinary named fields, and deterministic fixed storage while completing general bridge coverage; the measured cost no longer argues for hand-expanded initialization or positional compiler data | selected constant-aggregate, runtime-record, direct-field, and nested record-array closures are complete; payload-sum copying, structural-value projection, general coverage, and final disposition remain unresolved |
...
[`CKIR19`](../omega-bootstrap/compiler/OMEGA_BOOTSTRAP_CHECKED_IR_V19.md)
successor, and the current
[`OMGRFN22`](../../source/assurance/refinement/omega-bootstrap/OMGCOMP_REFINEMENT_WITNESS_V22.md)
