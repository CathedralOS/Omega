...
Alpha assembler (`../../alpha/assembler/`) lowers that to a tape, which
the seed runs.

Its original job was to discover and cold-start the language. The Alpha-written
compiler under `../cold-start/` now reconstructs the persisted
...
> Naming: `bootstrap/beta/rust` is the canonical Beta-language producer.
...
> `bootstrap/alpha/assembler-rust`.
...
- **Slice 2 — procedures, parameters, calls: DONE.** Multiple `proc`s, ≤4
  ...
  the [calling convention](../CALLING_CONVENTION.md) generated mechanically.
...
See [`../LANGUAGE.md`](../LANGUAGE.md) for the language surface, and run
