...
| removed `src/diff.rs` | Deleted | Rust-`Debug` section diffs were removed. Production review uses bounded exact-row `capability_conflict.rs`; durable root-policy resolutions remain new work, not a reason to retain the old comparison model. |
