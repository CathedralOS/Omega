...
| `src/declaration.rs` | Retain | Hermetic package declaration extraction uses the ordinary lexer/parser over immutable root `build.omg`, requires one canonical literal toolchain-owned `Package`, and grants no build execution or caller-supplied naming authority. |
| `src/dependency_projection.rs` and `src/dependency_edit.rs` | Retain | The strict extractor reads only immutable root `build.omg`, recognizes canonical literal Path/Git requests, and rejects hidden, computed, malformed, or unsupported dependency shapes without executing build code. The editor rewrites only validated canonical direct rows against the expected file digest; ambiguous/noncanonical placement produces a non-mutating generated patch. Atomic project mutation remains future command work. |
...
| `src/capability_conflict.rs` | Retain | Review-only comparison operates on complete compiler-owned canonical rows, exact package/resolution/source/compiler commitments, bounded dependency paths, and a complete candidate-closure commitment. It emits row-specific conflicts and fixed-vocabulary summaries; durable root-policy resolutions and accepted-lock promotion remain unfinished. |
...
| `src/diff.rs` | Rewrite | The quarantined implementation still uses Rust `Debug` and coarse caller-authored sections. Production review now uses `capability_conflict.rs`: bounded compiler-issued rows, exact old/new bytes, compiler-issued package-relative declaration coordinates, candidate custody and closure commitments, and fixed-vocabulary rendering. Ordinary rows retain exact declaration symbols through sorting, dangerous-authority rows retain exact declarations/exposures during derivation, and selected-provider provenance is captured beside candidate derivation and selection, including exact authored selection/schema/type/requirement anchors, every external or checked realization, and closed compiler-derived reasons. Nested use-site provenance and durable root-policy resolutions remain before the legacy module can be deleted. |
...
| `src/audit.rs` | Rewrite | The legacy inputs and joins are name-keyed caller manifests and unrestricted evidence strings. Production conflict explanation now retains one deterministic shortest path without enumerating an exponential DAG path set, while the fingerprint binds the complete canonical candidate closure. Port only fixed-vocabulary presentation that remains useful after the lock/evidence rewrite. |
...
| `src/source_commands.rs` and test-only `src/commands.rs` source audit | Rewrite | Diagnostic rendering is useful. Explicit `local`/`git` adapter selection has replaced locator guessing, but execution still uses the unhardened resolver. Keep marked unhardened until the remaining resolver P0 work lands. |
...
Synthetic legacy values remain confined to isolated unit tests. The integration
that fabricated manifests from fixture intent has been removed. Local fixture
