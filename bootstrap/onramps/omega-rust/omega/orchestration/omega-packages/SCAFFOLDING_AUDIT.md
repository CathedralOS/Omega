...
| `src/source.rs` local traversal and hashing | Rewrite | Useful limits and containment checks exist, but hashing loses non-UTF-8 path bytes, follows rather than identities symlinks, omits file kind/mode, reads a mutable tree without a snapshot boundary, and ignores tool outputs only through the special `.git` rule. Replace the source identity and snapshot policy before admission use. |
| `src/source.rs` Git resolution | Rewrite | Cache names are truncated request hashes; existing repositories are reused without origin/config proof; Git inherits host configuration and may execute transport/filter helpers; checkout precedes complete policy validation. Retain only the high-level resolve-to-commit/tree responsibility. |
...
| `src/commands.rs` source audit | Rewrite | Locator parsing and diagnostic rendering are useful, but every non-local locator is guessed to be Git and execution uses the unhardened resolver. Keep quarantined until adapter selection and resolver hardening land. |
...
| --- | --- | --- |
| Source/cache process isolation | `source.rs` invokes ambient `git`, reuses mutable checkouts, and hashes live local trees. | Not retained; resolver hardening is P0. |
