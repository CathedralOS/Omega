...
| `src/diff.rs` | Rewrite | The quarantined implementation still uses Rust `Debug` and coarse caller-authored sections. Production review now uses `capability_conflict.rs`: bounded compiler-issued rows, exact old/new bytes, candidate custody and closure commitments, and fixed-vocabulary rendering. Durable root-policy resolutions and source locations remain before the legacy module can be deleted. |
