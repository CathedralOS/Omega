...
| `src/source.rs` local traversal and hashing | Rewrite | File/symlink identity uses injective framing over raw path bytes, entry kind, symlink spelling, executable mode, length, and contents; it rejects special files and links into excluded Git metadata and bounds reads before allocation. It still reads a mutable tree without a snapshot boundary, excludes tool outputs only through the special `.git` rule, and omits empty directories and directory modes that authorized build code can observe. Retain the leaf-identity work; add complete tree identity and replace live-tree consumption before admission use. |
