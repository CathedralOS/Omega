...
| Identity | Typed source-derived `PackageKey`, immutable resolution, source graph, `PackageInstance`, and an opaque compiler-visible key commitment exist; managed source same-package checks use the commitment instead of path spelling. Legacy manifests and locks remain name/free-string keyed. | Thread the commitment through symbols, providers, boundaries, terminal Psi, and compiler evidence; replace the legacy lock/admission graph rather than adapting its strings. |
