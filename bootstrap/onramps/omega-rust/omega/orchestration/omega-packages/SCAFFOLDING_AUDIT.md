...
| `src/source.rs` local traversal and hashing | Rewrite | Identity now uses injective framing over raw path bytes, entry kind, symlink spelling, executable mode, length, and contents; it rejects special files and links into excluded Git metadata and bounds reads before allocation. It still reads a mutable tree without a snapshot boundary and excludes tool outputs only through the special `.git` rule. Retain the identity work; replace live-tree consumption before admission use. |
| `src/source.rs` Git resolution | Rewrite | Cache keys are now full and policy-versioned; staged entries bind exact resolver metadata, origin, and a strict local config; resolver access is locked; Git configuration is sealed; and submodule declarations/gitlinks reject before checkout. Git still materializes a mutable checkout, cooperative locks do not exclude hostile cache writers, SSH retains an external configuration surface, and the subprocess lacks an OS sandbox/resource limits. Retain the partial hardening but do not treat the checkout as admitted source. |
...
| `src/lib.rs` | Rewrite | Publicly re-exports every superseded security constructor. Production orchestration must instead receive opaque compiler/resolver evidence. Keep the crate explicitly experimental until those exports are replaced. |
| `apps/omega-cli/src/main.rs` package command routing | Delete/quarantine | Warning text is insufficient because standalone JSON can still produce a file named `omega.lock` or a reusable receipt. Remove manifest-based audit, review, plan, and lock routing until compiler-issued evidence exists. |

## Trust-path findings

| Required area | Current evidence | Ruling |
| --- | --- | --- |
| Source/cache process isolation | `source.rs` now seals Git configuration, validates resolver-owned cache state, serializes resolver access, and checks submodule policy before checkout, but still invokes an unsandboxed Git/transport process and hashes mutable local/checkout trees. | Partial only; immutable snapshot materialization and a hardened execution boundary remain P0. |
...
| Review receipts | Whole-section free-form receipts are candidate-insufficient and reusable in the wrong trust model. | Delete; replace with exact row-resolution artifacts. |
...
3. Existing package structs may be constructed only inside isolated crate tests
   and diagnostic scaffolding.
