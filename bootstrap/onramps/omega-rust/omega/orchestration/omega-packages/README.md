...
domains and aggregate carry policy are already closed enums. The next typed-tree
carrier should distinguish carry constraints, value domains such as `Finite`,
and `OmegaLayout` with an exact schema symbol and closed grammar; package review
must reject residual textual/unevaluated fallbacks. The projection
...
and requirement owner. Review v39 additionally binds the exact schema,
...
resolved and typed copies. Review v39 uses inferred transitive reach only for
...
remain separately bound. The current incomplete review-v39 projection therefore
