...
fn layer_of(manifest_path: &str) -> Option<&'static str> {
    ...
    if m("/apps/") {
        ...
        Some("orchestration")
    } else if m("/bootstrap/onramps/omega-rust/omega/backend/instruction_set_architectures/") {
        Some("isa")
    } else if m("/bootstrap/onramps/omega-rust/omega/backend/images/") {
        Some("images")
    } else if m("/bootstrap/onramps/omega-rust/omega/backend/object/") {
        Some("object")
    } else if m("/bootstrap/onramps/omega-rust/omega/backend/") {
        Some("backend")
    } else {
...
/// `<root>/bootstrap/onramps/omega-rust/omega/architecture/`.
...
fn frontend_implementation_is_psi_owned() {
    ...
    for relative in [
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/arithmetic.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/arena/mod.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/atomic.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/bignum.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/byte_predicates.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/cast_form.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/const_value.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/content.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/diagnostics/mod.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/float_semantics.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/inline_assembly.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/literals.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/operator_spelling.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/semantics.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/source",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/span.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/symbols/mod.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/trust.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/value_domain.rs",
        "bootstrap/onramps/omega-rust/omega/foundation/omega-core/src/wire.rs",
    ] {
        assert!(
...
fn retired_omega_frontend_adapters_do_not_return() {
    let root = workspace_root();
    for relative in [
        "bootstrap/onramps/omega-rust/omega/pipeline/omega-source-files-to-tokens",
        "bootstrap/onramps/omega-rust/omega/pipeline/omega-tokens-to-syntax-trees",
        "bootstrap/onramps/omega-rust/omega/pipeline/omega-syntax-trees-to-symbol-resolved-trees",
        "bootstrap/onramps/omega-rust/omega/pipeline/omega-symbol-resolved-trees-to-typed-trees",
        "bootstrap/onramps/omega-rust/omega/pipeline/omega-typed-trees-to-checked-trees",
        "bootstrap/onramps/omega-rust/omega/representations/omega-tokens",
        "bootstrap/onramps/omega-rust/omega/representations/omega-symbol-resolved-trees",
        "bootstrap/onramps/omega-rust/omega/representations/omega-syntax-trees",
        "bootstrap/onramps/omega-rust/omega/representations/omega-typed-trees",
        "bootstrap/onramps/omega-rust/omega/representations/omega-facts",
        "bootstrap/onramps/omega-rust/omega/representations/omega-checked-trees",
        "bootstrap/onramps/omega-rust/omega/semantics/omega-proof",
        "bootstrap/onramps/omega-rust/omega/semantics/omega-types",
        "bootstrap/onramps/omega-rust/omega/semantics/omega-validation",
    ] {
        assert!(
...
fn provider_approval_stays_in_omega_after_psi_checking() {
    ...
    let omega_approval = root
        .join("bootstrap/onramps/omega-rust/omega/orchestration/omega-compiler/src/pipeline/provider_approval.rs");
...
fn target_neutral_effect_inference_is_psi_owned() {
    ...
    let providers = root
        .join("bootstrap/onramps/omega-rust/omega/representations/omega-effects/src/capabilities/provider_plan.rs");
    assert!(
        providers.exists(),
...
fn checked_semantics_are_psi_owned_without_provider_realization() {
    ...
    let omega_provider_carrier =
        root.join("bootstrap/onramps/omega-rust/omega/representations/omega-effects/src/selected_provider_plans.rs");
    assert!(
...
fn first_terminal_psi_source_slice_stays_fail_closed() {
    ...
    assert!(
        !root
            .join("bootstrap/onramps/omega-rust/omega/pipeline/omega-checked-trees-to-terminal-psi")
            .exists(),
...
fn admitted_external_root_entry_fact_cannot_detach_before_body_dispatch() {
    let root = workspace_root();
    let path =
        root.join("bootstrap/onramps/omega-rust/omega/orchestration/omega-compiler/src/pipeline/provider_plans.rs");
    let source = std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
...
fn program_storage_entry_activation_cannot_detach_before_executor_dispatch() {
    let root = workspace_root();
    let path = root.join(
        "bootstrap/onramps/omega-rust/omega/orchestration/omega-compiler/src/pipeline/program_storage_entry.rs",
    );
    let source = std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
...
fn psi_reference_execution_ownership_and_terminal_lane_are_enforced() {
    let root = workspace_root();
    assert!(
        root.join("bootstrap/onramps/omega-rust/psi/semantics/psi-terminal-interpreter/src/lib.rs")
            .is_file(),
        "canonical terminal-Psi reference execution must remain Psi-owned"
    );
    assert!(
        root.join("bootstrap/onramps/omega-rust/psi/semantics/psi-checked-interpreter/src/lib.rs")
            .is_file(),
        "transitional checked-tree reference execution must remain Psi-owned"
    );
    assert!(
        !root
            .join("bootstrap/onramps/omega-rust/omega/orchestration/omega-interpreter")
            .exists(),
        "the retired production Omega interpreter must not return"
    );
    assert!(
        root.join("bootstrap/onramps/omega-rust/omega/orchestration/omega-native-differential-test/src/lib.rs")
            .is_file(),
