...
fn call_target_type_parameters_supply_carry_bounds() {
    ...
    assert_eq!(
        psi_validation::effective_type_carry_policy(
            &typed,
    ...
    assert_eq!(
        psi_validation::effective_type_carry_policy(&typed, &[], value.type_reference).suspension,
                ...
                crate::labels::canonical_place_label(
                    ...
                    && matches!(
                        ...
                        psi_facts::FactPayload::DomainMembership { .. }
                            | psi_facts::FactPayload::ContractDomainMembership { .. }
                    )
            }),
...
fn checked_nary_call_retains_distinct_claim_policy_facts() {
    ...
    let carried = checked
        ...
        .iter()
        .filter(|fact| {
            fact.effective.suspension == psi_language_semantics::CarrySuspension::Allowed
        })
        .collect::<Vec<_>>();
    assert!(
        carried.len() >= 2,
...
fn activation_wide_carry_joins_contained_machine_subtree() {
    ...
    assert_eq!(
        ...
        psi_language_semantics::CarryCpu::Origin
    );
    assert_eq!(
