...
#[test]
fn retains_callable_nested_review_anchor_spans_without_snapshot_identity() {
...
"#;
    let tokens = Lexer::new(source).tokenize().expect("tokenize");
    let parsed = parse_syntax_trees(&tokens).expect("parse");
    let machine = parsed
        .root_items()
        .find_map(|item| match item {
            psi_syntax_trees::item::Item::Machine(machine) => Some(machine),
    ...
    assert_eq!(machine.service_reach_clause_spans.len(), 2);
    assert_eq!(
        machine
    ...
    assert_eq!(
        uses.iter()
            .map(|use_name| source_text(use_name.source_span()))
            .collect::<Vec<_>>(),
        ["Console", "Filesystem"],
    );
    assert_eq!(
        parsed
            ...
            .iter()
            .map(|contract| source_text(contract.source_span))
            .collect::<Vec<_>>(),
        ["requires 1 == 1", "ensures 2 == 2"],
    );

    let moved_source = format!("\n\n{source}");
    let moved_tokens = Lexer::new(&moved_source)
        .tokenize()
        .expect("tokenize moved");
    let moved = parse_syntax_trees(&moved_tokens).expect("parse moved");
    let moved_machine = moved
        ...
        .expect("moved machine");
    assert_eq!(
        parsed.items.capability_contracts(machine.contracts),
        moved.items.capability_contracts(moved_machine.contracts),
        "contract semantic equality must exclude explanatory coordinates",
    );
    let snapshot = parsed.snapshot_json().expect("snapshot");
    assert!(!snapshot.contains("source_span"));
    assert!(!snapshot.contains("service_reach_clause_spans"));
}
