...
( cd "${OMEGA_PATH_BETA_RUST}" && sh build.sh "${OMEGA_PATH_BETA}"/bc.beta >/dev/null 2>&1 ) || { echo "refinement: bc build failed"; exit 1; }
...
python3 alpha_refinement_check.py "$T/check.exe" "${OMEGA_PATH_BETA_RUST}/build/bc.exe" "$ASM"
