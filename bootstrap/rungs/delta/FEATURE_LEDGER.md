...
| records, fixed arrays, slices, and payload sums | checkpoint 000001's standalone `compiler/psi/source/source.omg` passes a general Delta-written record/array/attached-machine parser and checker with exhaustive native, representative self-built, and Rust-free meaning evidence; private `CKIR1` plus its direct conservative backend close exact native/self bytes, canonical-Gamma 0/251/252 observations, exhaustive resource/relation teeth, product behavior, and independent lower-rooted source→CKIR1→limited-ELF reconstruction for the finite, acyclic, returning one-unit tranche without widening Terminal Psi; the separate two-package path preserves public cross-package nominal identity through exact witness, CKIR, ELF, and result on producer/native/self/Gamma paths; lower-rooted OMGRFN2 frame/source custody, source→witness, witness→CKIR tables, and CKIR→ELF are closed while resolved-body/result reconstruction and their final conjunction remain open; payload sums and slices remain separate later needs | compare total bridge/assurance cost with product-source refactors as later checkpoints arrive; use explicit tags where later payload-sum machinery does not pay for itself | first one-unit artifact tranche and its lower-rooted assurance demonstrated; two-package producer artifact and four modular lower-rooted responsibilities demonstrated; final refinement join and retain/refactor disposition unresolved |
...
| source units and modular organization | the canonical length-delimited bridge bundle preserves labels and exact bytes; the real Delta frontend retains bounded descriptors/labels/spans and independently scans every unit; the private compilation envelope separates labels from package/module authority and its Delta checker validates bounded wire/tables/graph/string roles/nested bundle/EOF/resources; a separate Delta resolver independently lexes every envelope-owned source, enforces authored-module agreement, exact direct requester-local aliases and same-package paths, visibility, duplicates, deterministic declaration order, static bindings, normalized types, and the exact selected root, then publishes canonical `OMGRSW1`; a separate `OMGLOW1` lowerer validates each consumed witness/source relation, reparses resolved bodies without redoing name resolution, and publishes the exact two-package CKIR; the producer composite closes CKIR→ELF and result through native/self/Gamma paths; modular lower-rooted checks close transport/source custody, independent source→witness resolution, witness→CKIR tables, and CKIR→ELF, but do not yet close resolved bodies/result or the final conjunction; no layer accepts a resolver receipt or compares the authoritative envelope SHA-256 | keep package/module semantics in ordinary Omega and pass the bridge one reconciled, independently committed graph; do not add package semantics to Delta merely because its bridge implementation decodes that graph | structural graph transport, normalized multi-unit resolution, producer artifact composition, and four lower-rooted responsibilities demonstrated; body/result conjunction and authority open |
