...
| implementation of Omega compositional full-width trapping arithmetic | from the current CKIR12/OMGRFN14 frontier, the next product need is one recursive pure same-carrier `u32 in Trapping` relation over `+`, `-`, and `*`; the UTF-8 decode carrier contains nine subtraction, six multiplication, and six addition nodes, with every subtraction result consumed by a parent arithmetic node | implement one bounded operation-tree walker and modular meaning/backend evidence, preserving each authored trap boundary and inherited view operations; do not generate compiler-file-shaped expression or statement-context permutations, and do not infer a Delta arithmetic-policy surface from the private bridge implementation | required product-source relation identified; implementation and Delta-v1 consequences unresolved |
