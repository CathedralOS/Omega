...
| records, fixed arrays, slices, and payload sums | checkpoint 000001's standalone `compiler/psi/source/source.omg` isolates records, fixed arrays/indexing, checked scalars, and attached mutation as the first real bridge cost probe; payload sums and slices remain separate later needs | implement and measure that record/array family first; use explicit tags where later payload-sum machinery does not pay for itself | unresolved; source-custody tranche selected |
