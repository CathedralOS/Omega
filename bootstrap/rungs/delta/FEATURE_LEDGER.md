...
| implementation of Omega full-width trapping subtraction | OMGRSW5/OMGLOWE/CKIR13 preserve exact semantic `u32` words and implement six direct checkpoint forms through independent reference meaning, runtime underflow, least-version selection, and conservative borrow-trapping artifacts; nine nested forms, Rust-free lowering meaning, and persisted-Beta OMGRFN15 R1–R5 remain open | represent source semantic words with two exact limbs or positional signed cells while keeping structural IDs/counts narrow; do not infer a Delta `u32`, wrapping domain, or general arithmetic surface from the bridge implementation | selected compiler/backend cost demonstrated; lower-rooted admission and Delta-v1 disposition unresolved |
...
| source units and modular organization | the bundle, generic OMGCOMP1 envelope, target/configuration-bearing OMGCOMP2 envelope, resolver witness, checked-IR lowering, and independent reconstruction close bounded public multi-unit custody and nominal identity; OMGCOMP2 deliberately leaves its provider source opaque, while accepted-lock authority and private cross-module semantics remain separate | keep package/module/provider semantics in Omega and pass Delta one reconciled graph; decoding that graph does not make those semantics Delta features | bounded public multi-unit and exact target/configuration custody demonstrated; provider semantics, authority, and general closure open |
...
lower-rooted contract, and the explicitly non-admitting
[`OMGRFN15` candidate](../../assurance/refinement/omega-bootstrap/OMGCOMP_REFINEMENT_WITNESS_V15.md).
