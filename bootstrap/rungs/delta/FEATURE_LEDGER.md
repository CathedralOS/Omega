...
| payload-bearing sum data | checkpoint 000001 uses token, numeric-base, diagnostic, and console-result sums; the selected unnumbered pure-sum tranche now closes OMGRSW3 resolution, general OMGLOW6 construction/Copy/Call/dispatch lowering, CKIR5 publication and independent meaning, conservative CKIR5→ELF emission native/self, and OMGRFN7 R1–R3 lower-rooted custody/resolution/table joins | compare general tagged data with explicit tag-plus-payload records; do not force the split when it increases invalid states or duplicated dispatch | selected compiler path demonstrated; R4/R5, same-frame composition, general coverage, and final disposition unresolved |
