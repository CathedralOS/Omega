...
| source units and modular organization | the canonical length-delimited bridge bundle preserves labels and exact bytes; the real Delta frontend now retains bounded descriptors/labels/spans, validates every unit independently, applies one exact-span nested-comment scanner to program and auxiliary units, and proves one O1 program plus auxiliary trivia through native, lower-rung meaning, and direct artifact gates; the private compilation-envelope contract now separates labels from package/module authority and defines the next canonical package/source/alias transport, but structural decoding alone is not the resolver/lock or source-semantic join | keep package/module semantics in ordinary Omega and pass the bridge one reconciled, independently committed graph; do not add package semantics to Delta merely because its bridge implementation decodes that graph | bundle-wide transport/scanning demonstrated; canonical graph transport specified; authority, resolution, and artifact join open |
