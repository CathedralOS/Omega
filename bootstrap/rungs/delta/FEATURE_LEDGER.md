...
| records, fixed arrays, slices, and payload sums | checkpoint 000001's standalone `compiler/psi/source/source.omg` now passes a general Delta-written record/array/attached-machine parser and checker with exhaustive native, representative self-built, and Rust-free meaning evidence; the implementation costs 78,450 source bytes, 5,395,760 fixed backing bytes, and 626,059 bytes of elaborated Gamma; payload sums and slices remain separate later needs | use the measured frontend rules, but close a direct checked-IR artifact handoff unless Terminal Psi demonstrably lowers total cost; decide whether product-source clarity justifies the substantial checker; use explicit tags where later payload-sum machinery does not pay for itself | frontend feasibility/cost measured; artifact path and final retain/refactor disposition unresolved |
