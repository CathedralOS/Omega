...
compile_through_lowermachine \
  "$OMEGA_PATH_OMEGA0/compiler/omega0-frontend.alp" "$T/frontend"
compile_through_lowermachine \
  "$OMEGA_PATH_OMEGA0/compiler/omega0-terminal-to-elf.alp" "$T/backend"
...
bundle_one() {
  python3 "$OMEGA_PATH_OMEGA0/compiler/omega0_bundle.py" \
    pack main.omg="$1" > "$2"
}
