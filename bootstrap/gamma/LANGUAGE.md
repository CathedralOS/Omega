...
```sh
sh bootstrap/gamma/test-interp.sh
sh bootstrap/gamma/test-typeck.sh
sh source/assurance/proof-kernel/gates/gamma-checker.sh
```

Typed Gamma consumers live in `canonical-bytes/`, the reusable grammar fragments
under `terminal-codec-primitives/`, and
`source/assurance/proof-kernel/implementations/gamma/checker_typed.gamma`.
