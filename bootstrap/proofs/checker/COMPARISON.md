...
`1 <= N < 8,519,680` (< 2^24), so this encoding is injective and lies in
`0..N*N-1`, below 2^47. Reversing a pair is a different key; this is not a hash
or a pair address.
...
and memo shortcuts do not insert again. Tree descent is bounded by 47 edges;
...
The private limit is 67,108,864 cumulative transitions per session.
...
limit 67,108,864, and requested 67,108,865. No Boolean result or session escapes on
...
A memo update copies at most 47 internal nodes at two pairs each; helpers must
not allocate an extra carrier per tree level. The cumulative bound is amortized,
not the maximum allocation of one transition: 50 pairs per consumed work unit
plus 128 once-per-request pairs. Combined with formation and ground admission,
`31,850,522 + 67,108,864*50 + 128 = 3,387,293,850` pairs is the selected arena
provision. This counts unreachable allocations, not just live storage.
...
allocates either a memo path of at most 94 pairs, a four-pair pending frame, or a
...
memos. Define accounting credit as `50*consumed_units - allocated_pairs`, excluding
...
| Insert a completed memo entry | at most 94 | at most 92 |
| Identity/completed-memo shortcut | 0 | 4 |
| Head mismatch or terminal successful resume | 4 | 8 |

At entry to a visit at pending depth `d`, credit is at least `42*d`; at entry
to a resume it is at least `42*(d-1)`. A child-entry transition earns at least
42 credits before increasing depth. Leaf insertion spends at most 44 before
resuming at the same depth. Parent completion spends at most 44 before resuming
one level lower. Advancing to another child earns at least 42 before visiting at
the same depth. Shortcuts earn at least 46; a terminal resume earns at least 42
...
pairs, retaining 46 credits. Ground comparison starts at its own depth zero;
any interrupted prefix has deficit at most 44, covered by those 46. A completed
ground call returns nonnegative credit; the subsequent at-most-88-pair local
insertion then leaves at most 38 of the permitted template-visit deficit.
Ground never
...
Thus any charged traversal prefix has deficit at most 44, and every completed
