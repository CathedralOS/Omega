#!/usr/bin/env sh
# Rust-free persisted-Beta/Gamma meaning gate for OMGRSW11/OMGLOWK20.
set -eu
...
while [ ! -f "$OMEGA_REPO_ROOT/bootstrap/paths.sh" ]; do
  ...
  [ "$PARENT" != "$OMEGA_REPO_ROOT" ] || {
    echo "record-array meaning: repository root not found" >&2
    exit 2
  }
...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "record-array meaning: skipped (native comparison requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in cargo python3; do
  command -v "$TOOL" >/dev/null 2>&1 || {
    echo "record-array meaning: skipped ($TOOL absent)"
    exit 0
  }
done
...
SOURCE_FIXTURE=$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES/omgrsw11_record_array_fixture.py
LOWERING_FIXTURE=$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES/delta-record-array-to-ckir19-fixture.py
CKIR_REFERENCE=$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES/checked_ir_v19_reference.py
...
for REQUIRED in "$RESOLVER" "$LOWERER" "$SOURCE_FIXTURE" "$LOWERING_FIXTURE" \
  ...
  [ -f "$REQUIRED" ] || { echo "record-array meaning: missing $REQUIRED" >&2; exit 1; }
done
...
stamp_beta_compiler "$T/bc.exe" >/dev/null || {
  echo "record-array meaning FAIL - Beta compiler artifact" >&2
  exit 1
}
...
build_beta "$OMEGA_PATH_OMEGA_BOOTSTRAP/meaning/omega2gamma.beta" "$T/elaborate.exe" || {
  echo "record-array meaning FAIL - omega2gamma build" >&2; exit 1;
}
build_beta "$OMEGA_PATH_GAMMA/interp.beta" "$T/interp.exe" || {
  echo "record-array meaning FAIL - Gamma interpreter build" >&2; exit 1;
}
...
python3 -B "$RUNNER" elaborate "$T/elaborate.exe" "$RESOLVER" \
  "$T/resolver.gamma" "$T/timings.tsv" "record-array resolver meaning" 60 700000
python3 -B "$RUNNER" elaborate "$T/elaborate.exe" "$LOWERER" \
  "$T/lowerer.gamma" "$T/timings.tsv" "record-array lowerer meaning" 60 120000
...
source = load("record_source_fixture", source_path)
lowering = load("record_lowering_fixture", lowering_path)
canonical = source.encode(source.CANONICAL)
semantic = source.encode(
    source.CANONICAL.replace("self.count + 1", "self.count + 2", 1))
resource = source.encode(
...
native_case() { # executable label expected
  ...
  [ "$CASE_STATUS" -eq "$CASE_EXPECTED" ] || {
    echo "record-array meaning FAIL - $CASE_LABEL native status $CASE_STATUS expected $CASE_EXPECTED" >&2
    exit 1
  }
  if [ "$CASE_EXPECTED" -ne 0 ] && [ -s "$T/$CASE_LABEL.expected" ]; then
    echo "record-array meaning FAIL - $CASE_LABEL native rejection published bytes" >&2
    exit 1
  fi
}
...
spec = importlib.util.spec_from_file_location("record_lowering_fixture", fixture_path)
...
semantic = fixture.pack(compilation, witness, selector=10)
...
[ "$(python3 -B "$CKIR_REFERENCE" run "$T/lowerer-canonical.expected")" = 70 ] || {
  echo "record-array meaning FAIL - canonical CKIR19 result is not 70" >&2
  exit 1
}
...
launch_gamma() { # producer label
  ...
  python3 -B "$RUNNER" run "$T/interp.exe" "$T/$CASE_PRODUCER.gamma" \
    ...
    "record-array meaning $CASE_LABEL" 240
}
check_gamma() { # label expected
  ...
  [ "$CASE_STATUS" -eq "$CASE_EXPECTED" ] || {
    echo "record-array meaning FAIL - $CASE_LABEL Gamma status $CASE_STATUS expected $CASE_EXPECTED" >&2
    exit 1
  }
  cmp "$T/$CASE_LABEL.stdout" "$T/$CASE_LABEL.expected" >/dev/null || {
    echo "record-array meaning FAIL - $CASE_LABEL Gamma publication differs" >&2
    exit 1
  }
}
...
[ "$W1" -eq 0 ] && [ "$W2" -eq 0 ] && [ "$W3" -eq 0 ] && \
  ...
  echo "record-array meaning FAIL - Gamma children $W1/$W2/$W3/$W4/$W5/$W6" >&2
  exit 1
}
...
print("record-array meaning: OMGRSW11 and OMGLOWK20 canonical 0 plus "
      "semantic 251/resource 252 no-publication observations PASS through "
      "persisted-Beta Gamma; " + " ".join(rows)
      + f" OMGRSWB={Path(sys.argv[2]).stat().st_size}B"
      + f" CKIR19={Path(sys.argv[3]).stat().st_size}B")
PY
