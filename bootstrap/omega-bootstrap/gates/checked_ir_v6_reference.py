#!/usr/bin/env python3
"""Thin CKIR6 reference over the inherited CKIR5 table implementation."""
...
Ckir6Error = v5.Ckir5Error
Ckir6ResourceError = v5.Ckir5ResourceError
Module = v5.Module
HEADER = v5.HEADER
ROWS = v5.ROWS
...
def decode(contents: bytes) -> Module:
    return v5.decode(contents, expected_major=6, allow_logical_not=True)
...
def main() -> None:
    ...
    if args.command == "validate":
        count = sum(op[3] == 15 for op in module.tables["operations"])
        print(f"CKIR6 valid: {count} LogicalNot operations")
    else:
...
if __name__ == "__main__":
    try:
        main()
    except Ckir6ResourceError as error:
        print(f"checked IR v6 reference: {error}", file=__import__("sys").stderr)
        raise SystemExit(252)
    except (Ckir6Error, v4.Ckir4Error, v3.Ckir3Error, v2.CkirError,
