...
if [ -z "${OMEGA_REPO_ROOT:-}" ]; then
  ...
  while [ ! -f "$OMEGA_REPO_ROOT/bootstrap/paths.sh" ]; do
    ...
    [ "$PARENT" != "$OMEGA_REPO_ROOT" ] || { echo "omega-bootstrap terminal emission: repository root not found" >&2; exit 2; }
...
command -v cargo >/dev/null 2>&1 || { echo "omega-bootstrap terminal emission: skipped (cargo absent)"; exit 0; }
...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "omega-bootstrap terminal emission: direct native lane skipped (requires Darwin arm64)"; exit 0 ;;
esac
command -v python3 >/dev/null 2>&1 || { echo "omega-bootstrap terminal emission: skipped (python3 absent)"; exit 0; }

T=$(mktemp -d); trap 'rm -rf "$T"' EXIT
OMEGA_BOOTSTRAP_WRITE_VARIANT_TERMINAL="$T/variant-shared.psi" \
  cargo test -q -p psi-checked-trees-to-terminal --test provider_attachment_source \
...
set -e
[ "$TRUNCATED_STATUS" != 0 ] || {
  echo "omega-bootstrap terminal emission: truncated producer was published" >&2
  exit 1
}
cmp "$T/frozen.psi" "$T/emitted.psi"
...
[ "$FAILED_PRODUCER_STATUS" != 0 ] || {
  echo "omega-bootstrap terminal emission: failed producer was published" >&2
  exit 1
}
cmp "$T/frozen.psi" "$T/emitted.psi"
...
if cmp -s "$T/frozen.psi" "$T/variant.psi"; then
  echo "omega-bootstrap terminal emission: literal/scalar perturbation did not change bytes" >&2
  exit 1
fi
cmp "$T/variant-shared.psi" "$T/variant.psi"

echo "omega-bootstrap terminal emission: fixture, direct bytes, shared decode, and verifier passed"
