#!/usr/bin/env python3
"""Independent CKIR13 full-width trapping-u32 subtraction reference."""
...
Ckir13Error = v5.Ckir5Error
Ckir13ResourceError = v5.Ckir5ResourceError
Module = v5.Module
HEADER = v5.HEADER
ROWS = v5.ROWS
...
def decode(contents: bytes) -> Module:
    return v5.decode(
        ...
        allow_full_u32_subtract=True,
    )


def selected_subtract_count(module: Module) -> int:
    types = module.tables["types"]
    values = module.value_types
    operands = module.tables["operands"]
    return sum(
        operation[3] == 26
        and types[operation[7]][1:] == (2, 1, 0, 0, 0, 0, 0xFFFF_FFFF)
        and values[operands[operation[8]][0]] == operation[7]
        and values[operands[operation[8] + 1][0]] == operation[7]
...
def main() -> None:
    ...
    if args.command == "validate":
        print(f"CKIR13 valid: {selected_subtract_count(module)} full-u32 Trapping Subtract operations")
    else:
...
if __name__ == "__main__":
    try:
        main()
    except Ckir13ResourceError as error:
        print(f"checked IR v13 reference: {error}", file=__import__("sys").stderr)
        raise SystemExit(252)
    except (Ckir13Error, v4.Ckir4Error, v3.Ckir3Error, v2.CkirError,
