...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "checked-IR-v20 backend: skipped (requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in cargo python3 clang codesign cmp; do
  command -v "$TOOL" >/dev/null 2>&1 || {
    echo "checked-IR-v20 backend: skipped ($TOOL absent)"
    exit 0
  }
done

BACKEND="$OMEGA_PATH_OMEGA_BOOTSTRAP_COMPILER/omega-bootstrap-checked-ir-v20-to-elf.alp"
FIXTURE="$GATE_DIR/delta-checked-ir-v20-backend-fixture.py"
REFERENCE="$GATE_DIR/checked_ir_v20_reference.py"
...
while IFS="$TAB" read -r NAME STATUS; do
  for IMPL in native self; do
    python3 -B "$FIXTURE" run-filter \
      "$T/backend.$IMPL" "$T/cases/$NAME.ckir20" \
      "$T/$NAME.$IMPL.out" "$STATUS" empty
  done
done < "$T/cases/manifest.tsv"

for FAMILY in index add less range sum-copy dispatch; do
  python3 -B "$FIXTURE" mutate-template \
    "$T/canonical.native.elf" "$T/bad-$FAMILY.elf" "$FAMILY"
  if python3 -B "$FIXTURE" check-artifact \
      "$T/bad-$FAMILY.elf" "$T/cases/canonical.ckir20" >/dev/null 2>&1; then
    echo "checked-IR-v20 backend: mutated $FAMILY template accepted" >&2
    exit 1
  fi
done

sh "$GATE_DIR/delta-checked-ir-v20-reference.sh"
echo "checked-IR-v20 backend: PASS"
