#!/usr/bin/env python3
"""Untrusted independent CKIR2 decoder and finite attached-call interpreter."""
...
def decode(contents: bytes) -> Module:
    ...
    require((major, minor, target) == (2, 0, 1), "bad schema or target")
    ...
    for operation in operations:
        ...
        require(opcode in range(1, 11), "opcode")
        if opcode == 10:
        ...
        expected_operands = (
            1 + machines[imm0][7]
            if opcode == 10
            else {1: 0, 2: 0, 3: 1, 4: 2, 5: 1, 6: 2, 7: 2, 8: 2, 9: 2}[opcode]
        )
        ...
        if opcode == 1:
            ...
            require(callee[2] == 1 or place_mutable[op_values[0]], "mutable call through shared place")
            for argument, parameter_id in zip(
                ...
                actual, expected = value_types[argument], machine_params[parameter_id][3]
                require(actual == expected, "call argument type")
            if callee[5] != NO_ID:
                require(result_type == callee[5] and types[result_type][1] in (1, 2, 3), "call result type")
    ...
    if entry != NO_ID:
        require(entry < len(machines), "entry range")
        machine = machines[entry]
        require(machine[2] in (1, 2) and machine[7] == 0 and machine[5] != NO_ID, "bad entry signature")
        require(types[machine[5]][1] in (1, 2, 3) and blocks[machine[10]][6] == 0, "bad entry result/block signature")
...
def interpret(module: Module, step_limit: int = 1_000_000) -> int | None:
    ...
    root_machine = machines[module.entry]
    owner_type = records[root_machine[1]][1]
    ...
    def run_machine(
        ...
        for ordinal, value in enumerate(arguments):
            ...
            if types[parameter_type][1] in (1, 2, 3):
                ...
                require(types[parameter_type][6] <= magnitude <= types[parameter_type][7], "runtime call range trap")
            parameters[parameter[4]] = (parameter_type, value[1])
        block_id = machine[10]
        while True:
            steps += 1
            ...
            for op_id in span(block[7], block[8], len(operations), "runtime operations"):
                ...
                if opcode == 1:
                    ...
                    result = run_machine(op[10], callee_receiver, [values[value] for value in args[1:]])
                    if op[4] == 1:
    ...
    return run_machine(module.entry, 0, [])
...
def main() -> None:
    ...
    if arguments.command == "validate":
        print(
            f"CKIR2 valid: {len(module.tables['types'])} types, "
