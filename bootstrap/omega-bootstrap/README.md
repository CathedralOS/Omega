...
| checked lowering and conservative emission | OMGLOWI18/CKIR17 close native/self lowering of the selected receiverless `Console` adapters and ranked recurrent helper to ordered platform-neutral byte events, including exact explicit `u8 as i32` custody; the relation has no entry, provider installation, syscall, backend, or artifact. Earlier CKIR16 retains its separate conservative unsigned artifact path | [`OMEGA_BOOTSTRAP_CHECKED_IR_V17.md`](compiler/OMEGA_BOOTSTRAP_CHECKED_IR_V17.md), [`OMEGA_BOOTSTRAP_LOWERING_V18.md`](compiler/OMEGA_BOOTSTRAP_LOWERING_V18.md), and the preceding versioned contracts under [`compiler/`](compiler/) |
