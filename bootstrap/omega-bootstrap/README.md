...
| generated ordinary-source custody | closed for the exact Unicode tuple through a sealed locked/offline recipe, generic provenance roles, two-run reproduction, bounded/no-publication teeth, exact OMGCOMP1 extent, and CKIR3/OMGRFN4 preflight composition; checkpoint 000001 is refreshed, while its explicit composition with this bridge route remains open | [`OMEGA_BOOTSTRAP_GENERATED_SOURCE_CUSTODY.md`](compiler/OMEGA_BOOTSTRAP_GENERATED_SOURCE_CUSTODY.md) |
| resolution | selected public multi-unit, call, data, receiver, scalar, and view relations through OMGRSW5, plus OMGRSW6 resolution-only cost evidence for one exact boundary requirement, applicable bodyless intrinsic candidate, and call-to-requirement; OMGRSW6 deliberately contains no provider selection or executable meaning | versioned `OMEGA_BOOTSTRAP_RESOLUTION*.md` contracts under [`compiler/`](compiler/) |
| checked lowering and conservative emission | private versioned vertical slices freeze prior carrier identities while adding one selected relation at a time; CKIR13 deliberately branches from CKIR11 rather than composing CKIR12 static views and closes compiler/backend cost for six direct full-width subtraction forms | versioned `OMEGA_BOOTSTRAP_CHECKED_IR*.md` and backend contracts under [`compiler/`](compiler/) |
| lower-rooted reconstruction | selected source/checked-IR/artifact closures have independent R1–R5 reconstruction through admitted OMGRFN15; direct full-width subtraction includes Rust-free publication, trapping-underflow ownership, and byte-complete conservative ELF reconstruction | [`../assurance/refinement/omega-bootstrap/README.md`](../assurance/refinement/omega-bootstrap/README.md) and its versioned witness contracts |
