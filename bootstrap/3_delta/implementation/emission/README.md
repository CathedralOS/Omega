...
[text.gamma](text.gamma) owns textual primitives. [support.gamma](support.gamma)
binds the ordered support members of
[`bootstrap/3_delta/support/`](../../support/README.md): the byte-rope runtime,
the `ConformanceBytesV1` runtime, and the generated `main` adapter are authored
once as ordinary readable Gamma files and copied verbatim from the sealed
input's bound support section. The pipeline's runtime selection - which member
a profile emits and where - remains separate from generic expression
...
Runtime text is ordinary authored Gamma source under `support/`, not emitter
data. Admission checks the section's presence and `compile_admitted_source`
validates every member's extent and byte sum against the bound table before
any Delta-source phase; a missing or corrupt member refuses atomically in
DCOUT space 5 before any output. Count mode advances by each member's bound
extent; publication copies member bytes from the sealed input in order, then
advances the count by the same extent, so counting covers every support byte
alongside every generated byte. The copy allocates no pairs and reads only
the sealed input. The
[staged compiler gate](../../../../tests/delta/staged-compiler/run.sh) pins
exact member bytes, receipt counts, and execution. No new Gamma syntax, host
source generator, filename discovery, or semantic parser is involved. Generic
packed-word serialization remains separate.
...
are included in the original inventory. Fixed marker, bound support members,
