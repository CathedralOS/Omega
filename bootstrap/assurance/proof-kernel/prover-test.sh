...
"${OMEGA_PATH_BETA_RUST}"/build/bc.exe < check.beta > "$T/x.asm" && "$ASM" < "$T/x.asm" > "$T/x.tape" \
...
libblock_ok() {  # bank a BIG derived lemma as a shared def/use library BLOCK (emit_lib_block) rather than
  ...
  python3 prover.py --libblock "$1" 0 > "$T/lb.out"
  ...
  lid=$(cut -f2 "$T/lb.out")
  prop=$(python3 prover.py --inline "$1" | cut -f1)          # the DESUGARED goal prop (check.beta has no Lt sugar)
...
# `self.arr[i+j]` interval-propagation discharge). NB check.beta's arena is 8 MiB+; the blocker was the inline
...
# THE BRIDGE -- WIRED: the lattice's real contract compiler (delta-rs/src/discharge.rs) cites a 7-lemma library
# that is now GENERATED BY THIS PROVER (prover-contract-lib.py -> one self-contained `(def i ..)` per lemma at
# stable ids 0..6), replacing the hand-written .elab base. The 7: add-zero-right, add-commutes, le-trans,
