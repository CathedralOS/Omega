#!/usr/bin/env sh
# The proof kernel, REWRITTEN IN GAMMA (implementations/gamma/checker.gamma), run on the Gamma reference
# interpreter (interp.beta, itself compiled Rust-free by bc and run on the seed).
# Same proofs as gates/test.sh — valid -> 1, invalid -> 0 — but the
...
"${OMEGA_PATH_BETA_RUST}"/build/bc.exe < "${OMEGA_PATH_GAMMA}"/interp.beta > "$T/g.asm" || { echo "bc(interp.beta) failed"; exit 1; }
