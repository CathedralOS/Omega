...
( cd "${OMEGA_PATH_BETA_COMPILER_RUST}" && sh build.sh "${OMEGA_PATH_BETA}"/bc.beta >/dev/null 2>&1 ) || { echo "termination-obligations FAIL — bc build"; exit 1; }
...
b implementations/beta/check.beta           "$T/check.exe"  || { echo "termination-obligations FAIL — build implementations/beta/check.beta"; exit 1; }
