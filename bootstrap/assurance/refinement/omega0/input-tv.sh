...
( cd "${OMEGA_PATH_BETA_COMPILER_RUST}" && sh build.sh "${OMEGA_PATH_BETA}"/bc.beta >/dev/null 2>&1 ) || { echo "input-tv FAIL — bc build"; exit 1; }
...
print(l)")
    ...
    python3 "${OMEGA_PATH_OMEGA0_REFINEMENT}/gamma2claim.py" < "$T/gi" > "$T/claims" 2>/dev/null \
