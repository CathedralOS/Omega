...
( cd "${OMEGA_PATH_BETA_RUST}" && sh build.sh "${OMEGA_PATH_BETA}"/bc.beta >/dev/null 2>&1 ) || { echo "meaning-cert diamond FAIL — bc build"; exit 1; }
...
for s in bounded_counter nested_counters euclid_gcd leap_year smallest_prime_factor number_guess \
  ...
  python3 "${OMEGA_PATH_OMEGA0_REFINEMENT}/gamma2claim.py" < "$T/g" > "$T/$s.claims" 2>/dev/null \
    || { echo "meaning-cert diamond FAIL — encoder refused $s"; exit 1; }
  SPECS="$SPECS $s=$T/$s.claims"
done
python3 "${OMEGA_PATH_OMEGA0_REFINEMENT}/meaning_cert_diamond.py" \
