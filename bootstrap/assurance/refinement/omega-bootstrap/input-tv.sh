...
b "${OMEGA_PATH_OMEGA_BOOTSTRAP}/meaning/omega2gamma.beta" "$T/omega2gamma.exe" \
  || { echo "input-tv FAIL — build omega2gamma.beta"; exit 1; }
b "${OMEGA_PATH_GAMMA}"/interp.beta "$T/interp.exe" || { echo "input-tv FAIL — build interp.beta"; exit 1; }
b "${OMEGA_PATH_PROOF_KERNEL}"/implementations/beta/check.beta  "$T/check.exe"  || { echo "input-tv FAIL — build check.beta"; exit 1; }
...
print(l)")
    ...
    python3 "${OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT}/gamma2claim.py" < "$T/gi" > "$T/claims" 2>/dev/null \
      || { echo "  FAIL $1 [\"$vec\"] : encoder refused"; exit 9; }
