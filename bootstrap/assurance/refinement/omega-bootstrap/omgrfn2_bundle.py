#!/usr/bin/env python3
"""Untrusted exact-byte packer for the private OMGRFN2 refinement frame."""
...
def bounded(path: Path, ceiling: int, label: str) -> bytes:
    ...
    if len(contents) > ceiling:
        raise SystemExit(f"{label} exceeds OMGRFN2 ceiling")
    return contents
...
def main() -> None:
    ...
    sys.stdout.buffer.write(HEADER.pack(
        MAGIC, 2, flags, len(omgcomp), len(witness), len(ckir), len(elf), result, exit_code,
    ))
