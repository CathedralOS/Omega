#!/usr/bin/env python3
"""Untrusted exact-byte packer for the private OMGRFN9 refinement frame."""
...
def bounded(path: Path, ceiling: int, label: str) -> bytes:
    ...
    if len(contents) > ceiling:
        raise SystemExit(f"{label} exceeds OMGRFN9 ceiling")
    return contents


def require_witness_identity(witness: bytes) -> int:
    if len(witness) < 12 or witness[:8] not in WITNESS_MAJORS:
        raise SystemExit("OMGRFN9 requires an OMGRSW1, OMGRSW2, or OMGRSW3 witness")
    major = WITNESS_MAJORS[witness[:8]]
    if struct.unpack_from("<H", witness, 8)[0] != major:
        raise SystemExit("OMGRFN9 witness magic and schema major disagree")
    if struct.unpack_from("<H", witness, 10)[0] != 0:
        raise SystemExit("OMGRFN9 requires OMGRSW schema minor 0")
    return major


def require_ckir7_identity(ckir: bytes) -> None:
    if len(ckir) < 12 or ckir[:8] != CKIR_MAGIC:
        raise SystemExit("OMGRFN9 requires a valid CKIR7 component")
    if struct.unpack_from("<H", ckir, 8)[0] != 7:
        raise SystemExit("OMGRFN9 requires CKIR schema major 7")
    if struct.unpack_from("<H", ckir, 10)[0] != 0:
        raise SystemExit("OMGRFN9 requires CKIR schema minor 0")
...
def main() -> None:
    ...
    ckir = bounded(args.ckir, MAX_CKIR, "CKIR7")
    elf = bounded(args.elf, MAX_ELF, "ELF")
    if not omgcomp or not witness or not ckir:
        raise SystemExit("OMGCOMP, selected OMGRSW, and CKIR7 must be nonempty")
    require_witness_identity(witness)
    require_ckir7_identity(ckir)
    if HEADER.size + len(omgcomp) + len(witness) + len(ckir) + len(elf) > MAX_FRAME:
        raise SystemExit("OMGRFN9 frame exceeds whole-frame ceiling")
    ...
    sys.stdout.buffer.write(HEADER.pack(MAGIC, 9, flags, len(omgcomp), len(witness), len(ckir), len(elf), result, exit_code))
