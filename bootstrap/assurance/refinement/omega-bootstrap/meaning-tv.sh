...
BC="$T/bc.exe"
stamp_beta_compiler "$BC" >/dev/null 2>&1 || { echo "meaning-tv FAIL — lattice bc artifact"; exit 1; }
...
tv() {
  ...
  python3 "${OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT}/gamma2claim.py" < "$T/g" > "$T/claims" 2>/dev/null \
