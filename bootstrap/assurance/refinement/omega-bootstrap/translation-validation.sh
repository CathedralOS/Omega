...
b "${OMEGA_PATH_OMEGA_BOOTSTRAP}/meaning/omega2gamma.beta" "$T/o2g.exe" \
...
tvcore() {
  ...
  cert=$(printf '%s\n' "$g" | python3 "${OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT}/tv-encode.py" "$nat") \
    || { FAIL=$((FAIL+1)); echo "  FAIL $1 : encode (outside subset?)"; return; }
  set +e
  v=$(printf '%s' "$cert" | "$T/check.exe")
  badcert=$(printf '%s\n' "$g" | python3 "${OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT}/tv-encode.py" $((nat + 1)))
