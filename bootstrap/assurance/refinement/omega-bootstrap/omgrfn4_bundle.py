#!/usr/bin/env python3
"""Untrusted exact-byte packer for the private OMGRFN4 refinement frame."""
...
def bounded(path: Path, ceiling: int, label: str) -> bytes:
    ...
    if len(contents) > ceiling:
        raise SystemExit(f"{label} exceeds OMGRFN4 ceiling")
    return contents
...
def main() -> None:
    ...
    ckir = bounded(args.ckir, MAX_CKIR, "CKIR")
    elf = bounded(args.elf, MAX_ELF, "ELF")
    if not omgcomp or not witness or not ckir:
        raise SystemExit("OMGCOMP, OMGRSW1, and CKIR must be nonempty")
    if HEADER.size + len(omgcomp) + len(witness) + len(ckir) + len(elf) > MAX_FRAME:
        raise SystemExit("OMGRFN4 frame exceeds whole-frame ceiling")
    ...
    sys.stdout.buffer.write(HEADER.pack(
        MAGIC, 4, flags, len(omgcomp), len(witness), len(ckir), len(elf), result, exit_code,
    ))
