#!/usr/bin/env python3
"""Pure OMGRFN15 R3-R5 persisted-Beta source materializer."""
...
def materialize(output: Path) -> None:
    ...
    source_code = model.source_profile_code(model.source_model())
    ...
    r3 = main_source(witness_code + ckir_code + """
proc main() {
    let status=omgrfn15_read_frame() let profile=0
    state framed { to done when(status!=0) profile=omgrfn15_profile() to selected }
    state selected { to bad when(profile==0) status=expected_witness_check(profile) to witnessed }
    state witnessed { to done when(status!=0) status=expected_ckir_check(profile) to done }
...
proc main() {
    let status=omgrfn15_read_frame() let source_profile=0 let ckir_profile=0
    state framed { to done when(status!=0) source_profile=omgrfn15_source_profile() ckir_profile=omgrfn15_profile() to profiles }
...
proc main() {
    let status=omgrfn15_read_frame() let profile=0
    state framed { to done when(status!=0) profile=omgrfn15_source_profile() to source }
    state source { to bad when(profile==0) to trap when(profile==2) return 0 }
...
proc main() {
    ...
    state checked { to done when(status!=0) to trap when(profile==2) to bad when(word[700064]!=70) return 0 }
    state trap { return 251 }
    state bad { return 251 }
    state done { return status }
}
