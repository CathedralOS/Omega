...
- **PORTABLE-TERMINAL-RELOAD.** The process-boundary reload control proves the
  admitted half of the portable-product contract only: a standalone Terminal
  artifact produced in one invocation decodes, verifies, and interprets in a
  second. The refused half is unpinned — a consumer that cannot reject a
  tampered product would pass every existing leg. Extend
  `canary_suite/portable_terminal_reload.rs` so each roster fixture also
  asserts the consumer stage refuses a truncated envelope, a single mutated
  section byte, and a trailing byte, while the untampered artifact still
  reloads. Acceptance: the added refusal legs reject in the consumer
  invocation (nonzero exit) for every `RELOAD_CANARIES` fixture, and the
  exact artifact continues to pass decode, verify, and interpret.
