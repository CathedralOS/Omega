...
- **PCC-CANONICAL-SEMANTIC-LEDGER.** Replace the current trusted Rust fusion of
  ...
  split: its 4,231-line `entry.rs` parent delegates all eleven compact-binary
  ...
  433-line `entry/place_copies.rs` sibling owns ordinary place-copy shape
  dispatch to exact target encoder clobber contracts. The public re-export
