...
- **L6a — Extent.** The normalized conservation foundation is live in
  ...
  facts. Both receipts are now minted from one opaque context derived from the
  complete pending mapping: exact grant, source custody/range/space/provenance/
  era/lineage/rights, mapped authority, and restoration authority survive
  behind compact IDs. A mapping/grant collision therefore cannot replay
  activation or release after authority drift. The hybrid page-table lifecycle
  is now normalized over that mapping state: an admitted grant validates
  concrete table storage; a draft rejects
  ...
  contracts. Target activation seams are live for x86-64 and AArch64. The x86
  plan derives the exact PCID-zero CR3 operand; the AArch64 plan derives the
  ASID-zero/CnP-false TTBR0_EL1 or TTBR1_EL1 operand from the admitted
  translation half and separately requires completed translation
  synchronization. Both start only from an `InstallablePageTable`, retain the
  complete opaque installation/mapping contexts, and turn post-write provider
  evidence into the existing installation receipt only when the observed
  register value and exact mapping set match. A bare physical address remains
  inert. Implement the checked system-register/barrier providers once their
  source-visible opaque carriers can cross the provider boundary. Connecting
  this model to the source-visible opaque Omega `[linear]` carrier, sealed fact
  establishment, storage, calling conventions, Cathedral activation, and
  source APIs is
...
- **External loans.** The normalized `omega-extents` model is live: a token
  ...
  mapping era, authority lineage, attenuated rights, and lent range. Confinement
  evidence is derived from the actual loan plus its admitted grant rather than
  restating those facts; reusing a loan ID cannot replay a stale completion
  after authority drift. Provider execution and the normalized DMA
...
- **EXI1–EXI5 — admitted executable installation.** The normalized
  ...
  and every failed linear transition returns its inputs. The normalized
  container byte decoder and validator are live through the ordinary validated
  scalar-layout consumer: the fixed little-endian header and bounded section
  directory reject truncation, overflow, prefix/section overlap, payload gaps
