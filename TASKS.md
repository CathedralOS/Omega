...
- **SAMPLE-CORPUS.** Close maintained `samples/cli|gui|uefi` programs through
  ...
  **CRASH-CONTRACT** owns consistent inferred-ceiling publication; retain it as a
  ...
  | [`euclid_gcd`](samples/cli/arithmetic/euclid_gcd/README.md) | Complete ordinary review and native execution, preserving the live-divisor loop, exact three output lines, EOF/Enter behavior and exit 12. `euclid_gcd_retains_service_call_entry_plan` covers the exact Service frame needed to admit the checked entry. The macOS ARM64 native probe reaches `Terminal proposal must retain every integer comparison occurrence exactly once`: **CRASH-CONTRACT** owns selected-comparison association in `checked-compilation-to-terminal-artifact` and its census in `compilation-report`. Reconcile selected, builtin and synthetic comparison custody; do not waive the occurrence check. |
