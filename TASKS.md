...
- **SAMPLE-CORPUS.** Close maintained `samples/cli|gui|uefi` programs through
  ...
  **CRASH-CONTRACT** owns consistent inferred-ceiling publication; retain it as a
  ...
  | [`generic_counters`](samples/cli/basics/generic_counters/README.md), `number_guess` | Both sources use intrinsic `Service<Console>`; ordinary package review and CLI/hosted execution remain, not reimplementation of already-supported counter calls or saturating arithmetic. Keep exit 16 and 70 respectively; macOS library probes are not project acceptance or the remaining hosts' runtime evidence. |
