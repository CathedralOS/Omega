...
`omega.final-footprint-certificate` schema, format version 124, and a certificate
...
register/state footprints. Compiler-function rows validate only the exact
compiler-authored prefix; format-owned import-thunk tails remain separately
classified and replayed by their format validators. Each compiler function's
retained first/last rows
...
more immediate or runtime-scalar integer arguments and no result now form the
first direct-import subset on Windows x64 and macOS arm64. Final replay invokes the ISA encoder
...
after placement. Runtime-scalar rows use a distinct origin that also requires
the complete storage-root relocation set beside the call relocation. Both
origins include ordinary call clobbers plus the envelope's stack/control state
and target scratch. Integer-result built-in imports whose source arguments are
all immediate integers, including no-argument calls, now form a third closed
origin. Final replay requires the exact result-region relocation beside the
library/symbol call and includes AArch64's offset-sensitive result-store
...
relocation. Scalar source-authored imports with immediate/runtime-scalar
integer or static-data parameters now use ninth
and tenth origins for no-result and direct-integer-result calls. Their replay
uses the source-selected canonical `CallPlan` unchanged and requires the exact
authored library/symbol call plus every data/storage root. Scalar source-authored
imports with one or more runtime-float parameters now use eleventh and twelfth
origins for no-result and direct scalar-result calls. Replay preserves the
source-selected vector/register/stack placements, requires the exact imported
call and every data/storage root, and accounts for the AArch64 float-result move.
Source-authored imports with one or more small, HFA, SysV, or indirect aggregate
parameters now use thirteenth and fourteenth origins for no-result and direct
scalar-result calls. Replay preserves each aggregate as one source operand while
requiring its exact fragmented, stack, or caller-copy marshalling and storage
root. Source-authored aggregate results now use a fifteenth origin. Replay
...
pointee form now also accepts a machine-storage index with its exact second-base
relocation and scratch evidence.
...
direct-storage writes into runtime-indexed inline frame or machine-array
elements; an inline-frame target may take its index from frame or machine
storage and reuses that machine base for a machine-resident source. Double-
...
reverse copy from a frame-held source pointee into an all-frame double-indexed
...
or both indices live in machine storage. The frame root still supplies the
collection and pointer slot, while one exact machine root at byte 12 supplies
every machine-held index in either direction; the complete aggregate span is
copied and replayed.
Machine-rooted double-indexed elements now cross a frame-held pointee in both
...
width, every storage/index relocation, and target scratch. An inline-frame
target's index may live in frame or machine storage. Runtime-indexed
...
validation regenerates the exact conversion/store bytes and recursive source
...
incomplete. X86 text-buffer materialization
...
target encoder, mixed relocation set, and the same footprint fragment.
Compiler-body place-address writes now retain their canonical source `Place`
and target runtime-frame slot. Final validation replays the exact target
address materializer, independently derives every source, index, and target-
frame relocation site, and requires its clobber union to equal dedicated
`CompilerBodyPlaceAddressWrite` evidence. Inline-frame AArch64 address sources
accept frame- or machine-held indices; cross-region machine indices retain the
exact second-base relocation and its additional address scratch. All-frame
double-runtime-indexed address sources reuse one frame relocation for the
...
the frame or machine region and the exact second-base relocation. All-frame
double-runtime-indexed literal-append targets reuse one frame relocation for
the carrier and both indices. Source-carrier appends now share that target
coverage as well: x86 retains both general place walks, while AArch64 serves
every classified target with a direct or pointee source and replays the indexed
target walk, source walk, and copy-loop state; the all-frame double-indexed
target likewise reuses one frame relocation.
