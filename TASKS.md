...
- **L4/L5 — plan-laid views.** Derive projection over plan-laid byte views,
  ...
  field values plus a validated plan: it supplies no offset API,
  ...
  entries validate. Unit coverage pins a compact fragmented scalar word, while the
  ...
  consumes the identical named geometry and
  ...
  establishment, codec conformance, target-package consumer integration, and
...
- **L6a — Extent.** The normalized conservation foundation is live in
  ...
  activation or release after authority drift. This is the language/compiler
  boundary: Omega supplies range authority, generic mapping conservation,
  layouts/materializers, provider admission, and checked instruction
  contracts. It does not define page-table drafts, entries, hierarchy
  policies, installation states, control-register activation plans, or target
  translation-table writers. Those are OS implementation concerns and belong
...
- **Linux hosts.** The `clock_gettime` composite `timespec` lowering is live
  on x86-64 and AArch64: exact syscall plans, compiler-owned 16-byte
