...
`omega.final-footprint-certificate` schema, format version 60, and a certificate
...
remaining text-assembly writes, and calls remain incomplete. Direct, pointee,
or frame-indexed x86 and AArch64 text-buffer materialization now retain the
exact buffer symbol and target place, replay the
...
replay all x86 targets and the direct, pointee, frame-indexed, inline-frame-
indexed, and frame-indexed machine-storage AArch64 shapes, binding the exact
rodata and storage/index relocations to `CompilerBodyPlaceStringWrite` evidence. Immediate
...
separate `CompilerBodyPlaceBoundedBufferWrite` footprint; literal appends into
the same owned carriers now share that evidence with their own exact encoder
and relocation recipe. Source-carrier appends now share it as well, replaying
both place walks and the copy-loop state. Immediate compact bit-field writes
