...
- **[x] aarch64 owned `[u8; N]` byte-carrier family -- LANDED 2026-07-08 (write +
  ...
  aarch64 encoders for the carrier ops were UNIMPLEMENTED (errored at encode),
  yet their LAYOUT widths borrowed the x86_64 widths -- variable-length
  instruction sequences with inline immediate BYTES, legitimately
  non-multiple-of-4 (e.g. 51 bytes for "150"). A forward branch skipping the
  block holding such an op computed its distance from the odd width and tripped
  the alignment check FIRST, masking the real "unimplemented" error. Landed
  pieces (all-4-byte-instruction sequences, so inherently aligned):
  - `encode_runtime_machine_bounded_buffer_write` + `_pointee_bounded_buffer_write`
    + `_literal_append` + `_source_append` (isa-aarch64 runtime_storage.rs +
    widths.rs; selection width signatures extended to pass the offsets the
    aarch64 widths need -- x86 arms ignore them, matching the
    runtime_pointee_string_write_width precedent).
  - `append_runtime_text_equals_literal_operand` grew the operand's
    `is_bounded_buffer` flag (the aarch64 caller was DROPPING it -- its comment
    assumed "aarch64 only ever sees descriptor places here", broken the moment
    the write landed): carrier = computed bytes address (`add x,x16,#8`) + len
    at +0; descriptor = ptr load @0 + len @+8. Width-identical by construction
    (mirrors the x86 same-width carrier branch).
  - `Aarch64CallOperand::RuntimeStringPointer/Length` grew `is_bounded_buffer`
    (host-call args: write_line of a carrier printed EMPTY -- ptr/len read
    descriptor-style). Carrier ptr = `add` computed inline-bytes address, len @0.
  - Source-append's frame-base reloc offset was HARDCODED x86 `+10`; now
    arch-aware (`bounded_buffer_source_append_frame_address_offset`: aarch64 8,
    x86 10 -- the string_descriptor_machine_address_offset pattern).
  VERIFIED against a fresh main-baseline run on the arm64 host: canary suite
  101 -> 70 failures, ZERO new, 31 fixed (the whole carrier/text family:
  bounded_carrier 11/11, base64, itoa, ciphers, mandelbrot render,
  text_builder, palindrome, ...); 11 samples now run to their documented exits
  (8 text + cellular_automaton + text_greeting + status_report); cli_mvp +
  dungeon suite pin descriptor strings unaffected; the differential oracle's
  first failure (runtime_cast_in_guard_exit 71!=70) reproduces IDENTICALLY at
...
- **[x] Trapping-LOCAL const-fold overflow now TRAPS (verified landed 2026-07-08).**
  ...
  #65 settles.
- **[x] Folded-binary left operand loses the UNSIGNED marker — FIXED
  2026-07-10e.** `(a / b) % c` fused in a GUARD ran signed modulo on a
  high-bit u32 (native 71 vs interp 70); the ARG-materialization flavor had
  already been repaired by intervening work (temp typed correctly -- probed
  70/70 before the fix), which is WHY the 2026-07-08 attempt looked
  "ineffective": both paths were broken then and the reverted recursion only
  fixed one. Re-applied the Binary recursion in
  resolve_runtime_storage_is_signed_in_table (one witness, left first; the
  comment records the revert history). All four consumers of the witness now
  covered: fused div/mod, stored div/mod, ordered compares of a nested
  quotient, and logical `>>`. Pinned:
  pass/arithmetic/runtime_nested_unsigned_witness_exit (differential 70/70).
  The verification landmines on the old item held (positive exits, fold-proof
  fields, both paths probed separately).
...
- **[x] Sum PAYLOAD-range narrowing under a case-match -- LANDED 2026-07-08.** A
  destructured payload binding passed DIRECTLY (`P::One { v } -> use_v(v)`, no
  arithmetic) now proves against a same-ranged param. Fix in
  omega-proof/obligations.rs: the arm guard is `When(Equal(self.p, P::One))`
  (case membership lowers to a tag-equality whose RHS `Name` carries the VARIANT
  symbol), so `payload_field_constraints_under_case_guard` (in the transition-arg
