...
- **IMMUTABLE-TARGET-ACTIVATION-AND-MATRIX.** Replace temporary authored
  ...
  build selects one exact target; an explicit multi-target build shares source,
  checking, and portable Psi work, then forks only target-dependent provider,
  optimization, layout, and native-artifact stages. `target` declarations are
  not a support set, and no `all` token may infer one from the toolchain catalog.
  ...
  artifact that blurs their subjects.
