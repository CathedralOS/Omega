...
- **SAMPLE-CORPUS.** Close maintained `samples/cli|gui|uefi` programs through
  ...
  **CRASH-CONTRACT** owns consistent inferred-ceiling publication; retain it as a
  ...
  | `number_guess` | Ordinary package review and CLI execution across the hosted matrix, preserving exit 70 and intrinsic `Service<Console>`. Do not reimplement already-supported saturating arithmetic; a macOS library probe is not project acceptance or another host's runtime evidence. |
