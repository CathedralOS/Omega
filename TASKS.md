...
- **R5:** continue exact inferred may-write summaries and relational candidates.
  ...
  call producers in up to seven unary, binary, primitive-cast,
  member-projection, or indexing shells without widening that call budget. A
  eighth direct scalar shell remains fenced; aggregate fields and projected
  ...
  aggregate/literal computed shell, an eighth direct scalar computed shell,
...
- **CML4:** construct the complete `EdgeCleanupPlan` after outgoing-value
  ...
  root, wrong-length, index, cleanup-order, and artifact mutations reject;
  `[T; 9]` and wider prefixes remain fail closed without runtime liveness state
  or a loop.

  Extend recursive coverage beyond the exact `[[T; 4]; 2]` rung and extend
  construction-prefix cleanup beyond `[T; 8]` to deeper canonical fuel/action
  ordinals.
