...
- **BOUNDED-OPTIMIZATION-SEARCH.** Bounded candidate search + revalidation at scale (merges BOUNDED-CANDIDATE-SEARCH, CANDIDATE-REVALIDATION-AT-SEARCH-SCALE). Source doc `wiki/drafts/learned_optimization_policy.md` authorizes no implementation: a bounded model-free search over the validated candidate interface and any search-scale revalidation-cost study are far-future extensions gated on the Omega-written product compiler (OMEGA-PRODUCT-COMPILER-SOURCE) plus a concrete justification. The seam such a search would plug into already exists — `pass_manager/external_policy` replays explicitly supplied decisions on exact context and row equality, and every candidate is independently revalidated against the exact input revision it binds (`validate_psi_rewrite_candidate` per iteration; `CandidateContractAxis::Input` rejects stale-input candidates), so revalidation at scale is the existing behavior's measured cost, not a missing mechanism. The versioned workload corpus and measurement protocol its evaluation needs are gated under WORKLOAD-CORPUS-AND-MULTIVERSIONING, with the comparison protocol scoped in `wiki/drafts/graph_cost_model_study.md`.
...
- **BASELINE-T2C-FAILURE-CLUSTER-TRIAGE** — triaged at 749794ddeb: `cargo nextest run -p typed-trees-to-checked-trees --lib` reports one live failure, `tests::generics::const_arguments::literal_cast_and_operator_results_seed_type_inference` — a stale fixture predating 80a1fe48cc ("positive fit evidence for exact integer cast arguments"), which correctly rejects an unrestricted `i64 as i32` cast argument. Repaired by bounding the parameter (`i64[0..=100]`), preserving the cast/operator inference-seeding assertion. The earlier documented members are resolved or owned elsewhere: the rank_ranges trio belongs to TERMINATION-RANKING-CHECKS, `indexed_operand_access_preserves_shared_collection_and_owned_index` belongs to BASELINE-T2C-INDEXED-OPERAND-ACCESS/OPERATOR-MACHINE-SUPPLY, and two were repaired in-tree. Non-`--lib` targets and the wider crate suite (`--no-fail-fast` over all targets) remain unaudited by this triage; the doc's 20-failure whole-crate reading attributed to C2L-BASELINE-FAILURE-ATTRIBUTION is a different crate.
...
- **GENERAL-SOURCE-BINDER-SYNTAX** — mined candidate; verify scope then implement.
...
- **PIPELINE-ROUTE-CONFORMANCE-AUDIT** — mined candidate; verify scope then implement.
...
- **PORTABLE-TERMINAL-RELOAD** — mined candidate; verify scope then implement.
...
- **PROGRAM-ENTRY-SELECTION-DIVISION** — mined candidate; verify scope then implement.
...
- **PSI-HOSTED-ENTRY-RECEIVER-PROVISIONING** — mined candidate; verify scope then implement.
...
- **REPLACEMENT-REJECTION-INVENTORY** — Resolved. Verified scope: an inventory of every spec-named component-replacement rejection obligation mapped to its enforcement site. Landed in `wiki/drafts/replacement_rejection_inventory.md`: era admission/lifecycle and journal-replay rejections are enforced and test-pinned in `component_era_entry_ledger.rs`/`component_era_journal.rs`; process-static atomic-handover rejects in `executable_scopes/process_static_services.rs`; custody-on-rejection in external-roots `opaque_callback_replacement.rs`/`installation_ledger.rs`; quarantine + attributed capacity loss in `executable-installation`'s `replacement_quarantine.rs`; service-carrier validity in `program_validation.rs`. Residual rows with no rejection site yet — cohort-cut proof/enlarged-cohort report, explicit-migration and stable-object-identity tiers, descriptor reinterpretation fence, mapping-reuse authority proof, exclusion propagation on replacement — attributed to EPOCH-RESOURCE-SNAPSHOTS' "wider service-era replacement substrate breadth". Verified: `cargo nextest run -p effects -p executable-installation -p external-roots --lib` all green on linux x86-64 (688 tests).
...
- **SELECTED-OPTIMIZATION-ANCESTRY-REMOVAL** — mined candidate; verify scope then implement.
