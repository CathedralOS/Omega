...
- **SAMPLE-CORPUS.** Close maintained `samples/cli|gui|uefi` through the
  ...
  | `text_greeting`, `status_report`, `string_catalog` | Carrier concatenation `self.t = "a" + self.s + "b"` into a bounded `Utf8` field. Checking and the checked interpreter accept it, but Terminal Psi has no operation that appends a view at a bounded field's live length: `StructuralByteSequenceFieldStore` replaces the whole prefix and `StructuralByteSequenceFieldByteStore` keeps the length. 04 declines at `structural field store: byte sequence carrier`, and the active run tests `runtime_bounded_carrier_concat_exit_canary_runs` and `runtime_bounded_carrier_alias_concat_exit_canary_runs` fail the same way (the alias variant stops earlier, at `call operation: projected operand support`). Add one append operation through Terminal encoding, verifier, interpreter and Omega lowering; an unrolled per-capacity byte sequence is not acceptance. |
...
- **STATE-LOCAL-VALUE-FRONTIER.** Complete compositional evaluation and
  ...
  **CML4** owns residual cleanup. Start at the actual omission in
  ...
  - Complete store sources for float reads/arithmetic and policy casts,
    ...
    A float field read has no Terminal operation: only `IntegerStructuralField`
    and `BooleanStructuralField` exist. So `self.a = self.b` over `f64` fields
    stops at `structural_scalar_store/value.rs::admits`, and admitting it there
    stops at 05's integer-only field observation, then at the verifier's
    `IntegerStructuralFieldRequiresIntegerResult`. Float arithmetic has no row
...
- **RC-BUILD-AND-PACKAGES.** Close all three build/package command blocks in
  ...
  `canary_suite`, `samples_compile` and `corpus_runner`, 28 carried 243
  failing tests -- failing, not skipping. **16 targets carry 87 now**, of
  which 89 -- `native_filesystem_canaries` -- needs a host provider and
  should skip rather than fail.

  A METHOD WARNING for whoever re-measures. An earlier pass reported 178
  across 23, and that was an undercount: three of its ten batches never ran,
  because one target in each failed to COMPILE when this host ran out of disk,
  and `nextest` reports that as `error: could not compile` with no `Summary`
  line. Grepping only for `FAIL` and `Summary` silently drops the whole batch
  -- 36 targets, 65 further failures, including the second-largest cluster. A
  sweep must check that every batch produced a `Summary`, and must watch free
  ...
  `byte_field_replacement`, `callable_entry_custody`, `calling_policy_plans`,
  `composed_internal_unit_arguments`, `constant_expression_casts`,
  `constant_float_tables`, `cyclic_receiver_execution`,
  ...
  migration reaching a declared package or application identity; an
  expectation that outlived its behavior (target-qualified provider plan
  names, and a custody matrix substituting the literal that had since become
  the current vocabulary marker); and two real compiler defects -- a selected
  transition arm's machine-storage guard conjunct discarded whenever a
