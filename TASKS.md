...
- **R5:** continue exact inferred may-write summaries and relational candidates.
  ...
  call producers in up to twenty-one unary, binary, primitive-cast,
  member-projection, or indexing shells without widening that call budget. A
  twenty-second direct scalar shell remains fenced; aggregate fields and projected
  ...
  aggregate/literal computed shell, a twenty-second direct scalar computed shell,
...
- **SELECTED-WITNESS-EVIDENCE — finish executable proof-output calls.** The
  ...
  bounded rung now permits one, two, three, or four distinct bound whole-result
  terms to be used once each: the matching payloadless arm may pass them as dense ordered
  ...
  substitutions, five-or-more evidence arguments, repeated term use, later
