#!/usr/bin/env python3
"""Independent exact CKIR19 conservative ELF reconstruction for OMGRFN22."""

from __future__ import annotations

import omgrfn16_elf_reference as legacy
from omgrfn21_elf import Reconstructor as FixedBufferReconstructor
from omgrfn22_ckir import NO_ID, V5, arguments, meaning_decode
from omgrfn22_frame import RefinementError, RefinementResourceError, require


class Reconstructor(FixedBufferReconstructor):
    """Add only CKIR19's qword record-array IndexPlace template.

    The audited OMGRFN18/21 owners retain scalar widths, qword operations,
    calls, edges, layouts, ranges, ELF orchestration, and byte-array indexing.
    This owner independently derives the record stride and adds checked signed
    multiplication and unsigned address addition.  It never invokes the
    production backend.
    """
    ...
    def emit_operation(self, emitter, operation: tuple[int, ...]) -> None:
        ...
        if opcode == 4 and self._u64_value(args[1]):
            ...
            require(self.types[base_type][1] == 5
                    ...
                    and self.types[result_type][1] == 4,
                    "selected u64 IndexPlace exact record array")
            length = self.types[base_type][5]
            stride = self.module.layouts[element_type][0]
            require(length == 16_384 and stride == 40,
                    "selected observation length and stride")
    ...
    def reconstruct(self) -> bytes:
        """Retain the inherited ELF relation with CKIR19's 2 MiB owner cap."""
        require(self.module.entry != NO_ID, "OMGRFN22 requires entry artifact")
        self.assign_frame()
        owner_type = self.records[self.machines[self.module.entry][1]][1]
        owner_size = self.module.layouts[owner_type][0]
        require(owner_size <= 2 * 1024 * 1024, "entry owner layout exhaustion")
        ...
        ro_size = legacy.elf3.align(len(constant_image), legacy.PAGE) if root_offsets else 0
...
def reconstruct(contents: bytes) -> bytes:
    try:
        ...
        raise RefinementError(f"exact CKIR19 ELF reconstruction: {error}") from error
