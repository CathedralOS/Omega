#!/bin/sh
# Split representative seed-lineage projections for the modular OMGRFN22 owners.
set -eu
...
cd "$OMEGA_REPO_ROOT"
case "$(uname -sm)" in "Darwin arm64") ;; *) echo "OMGRFN22 Beta join: skipped (requires Darwin arm64)"; exit 0;; esac
T=$(mktemp -d)
trap 'rm -rf "$T"' EXIT
R=$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT
PYTHONPATH="$R:$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES" python3 -B "$R/omgrfn22-materialize-beta.py" "$T/m"
for OWNER in r1 r2 r3 r4-lowering r4-source-result r5-structure r5-result r5-elf; do
  PYTHONPATH="$R:$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES" python3 -B "$R/omgrfn22-$OWNER.py" < "$T/m/canonical.rfn" > "$T/$OWNER.out"
...
while IFS="$TAB" read -r OWNER REJECT_AT; do
  ...
  [ "$BYTES" -le 235520 ] || { echo "OMGRFN22 Beta join: $OWNER tape $BYTES exceeds 230 KiB target" >&2; exit 1; }
...
echo "OMGRFN22 Beta join: $COUNT split projections fixed; max tape $MAX bytes"
echo "OMGRFN22 Beta tapes: $REPORT"
