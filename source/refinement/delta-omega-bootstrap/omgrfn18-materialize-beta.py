#!/usr/bin/env python3
"""Materialize finite persisted-Beta projections for OMGRFN18 owners.

These are representative component-field joins for the general Python
relation.  They are neither producer evidence nor a profile allowlist.
"""
...
from omgrfn18_frame import HEADER
from omgrfn18_profiles import profiles

ASSIGNMENT = {
    "r1": "borrow",
    "r2": "borrow",
    "r3": "bit63",
    "r4-lowering-source": "borrow",
    "r4-lowering-ckir": "borrow",
    "r4-source-result": "max-equal",
    "r5-structure": "reversed-high",
    "r5-result": "max-equal",
    "r5-elf-ckir": "max-neighbor",
    "r5-elf-image": "max-neighbor",
}
...
proc read_frame() {{
    ...
    state finish {{
        ...
        to bad when(fb(6)!='I') to bad when(fb(7)!=0)
        to bad when(fb(8)!=18) to bad when(fb(9)!=0) to bad when(fb(10)!=0) to bad when(fb(11)!=0)
        return 0
    }}
