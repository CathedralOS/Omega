#!/usr/bin/env python3
"""Bounded OMGRFN22 framing for the flat record-array assurance cut."""
...
def split(raw: bytes) -> Frame:
    ...
    for length, ceiling, label in (
        ...
        (witness_len, MAX_WITNESS, "OMGRSWB"),
        (ckir_len, MAX_CKIR, "CKIR19"),
        (elf_len, MAX_ELF, "ELF"),
    ):
        if length > ceiling:
...
def check_r1(frame: Frame) -> None:
    ...
    require(len(frame.witness) >= 12 and frame.witness[:8] == b"OMGRSWB\0"
            and struct.unpack_from("<HH", frame.witness, 8) == (11, 0),
            "exact OMGRSWB component identity")
    require(len(frame.ckir) >= 12 and frame.ckir[:8] == b"OMGCKIR\0"
            and struct.unpack_from("<HH", frame.ckir, 8) == (19, 0),
            "exact CKIR19 component identity")
