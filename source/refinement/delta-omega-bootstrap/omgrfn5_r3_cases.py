#!/usr/bin/env python3
"""Focused OMGRFN5 responsibility-3 mutations and exact CKIR4 census."""
...
def pack(parts: tuple[int, int, int, bytes, bytes, bytes, bytes], *,
    ...
    return FRAME.pack(
        b"OMGRFN5\0", 5, flags, len(comp), len(witness), len(ckir), len(elf),
        result, result & 255,
...
def meta(raw: bytes) -> tuple[dict[str, int], dict[str, int]]:
    ...
    require(fields[:4] == (b"OMGCKIR\0", 4, 0, 1), "CKIR4 identity")
    raw_counts = fields[7:]
    names = (
    ...
    require(at == len(raw), "CKIR4 exact table extent")
    return counts, bases
...
def octet(raw: bytes, at: int, value: int) -> bytes:
    changed = bytearray(raw)
    changed[at] = value
    return bytes(changed)
...
def cases(frame_path: Path, output: Path) -> None:
    ...
    ccase("ckir3-inner-version", word(ckir, 8, 3))
...
def constructor_cases(frame_path: Path, output: Path) -> None:
    """Mutate only R3-owned opcode-13 nominal premises when possible."""
    output.mkdir(parents=True, exist_ok=False)
    original = frame_path.read_bytes()
    parts = split(original)
    ckir = parts[5]
    counts, bases = meta(ckir)
    operations = bases["operations"]
    constructor = next(
        operations + index * ROWS["operations"]
        for index in range(counts["operations"])
        if ckir[operations + index * ROWS["operations"] + 12] == 13
    )
    ...
    types = [type_row(ckir, bases["types"], index)
             for index in range(counts["types"])]
    records = [struct.unpack_from("<IIIIB3x", ckir, bases["records"] + index * 20)
               for index in range(counts["records"])]
    scalar_type = next(index for index, row in enumerate(types) if row[1] <= 3)
    noncopy_type = next(
        index for index, row in enumerate(types)
        if row[1] == 4 and records[row[4]][4] == 0
    )
    operand_start = U32.unpack_from(ckir, constructor + 24)[0]
    operand_count = U32.unpack_from(ckir, constructor + 28)[0]
    require(operand_count > 0 and operand_start < counts["operands"],
            "constructor operand representative")

    ccase("constructor-result-kind", octet(ckir, constructor + 13, 0))
    ccase("constructor-flags", octet(ckir, constructor + 14, 1))
    ccase("constructor-scalar-result", word(ckir, constructor + 20, scalar_type))
    changed = bytearray(ckir)
    U32.pack_into(changed, constructor + 20, noncopy_type)
    U32.pack_into(changed, constructor + 28, records[types[noncopy_type][4]][3])
    ccase("constructor-noncopyable", bytes(changed))
    ccase("constructor-arity", word(ckir, constructor + 28, operand_count - 1))
    ccase("constructor-immediate-zero", word(ckir, constructor + 32, 1))
    ccase("constructor-immediate-one", word(ckir, constructor + 36, 1))

    # Operand identity/order/visibility/type and result-ID reconstruction are
    # intentionally outside R3. These globally malformed products must remain
    # accepted by this projected checker, proving physical proposition opacity.
    ccase("opaque-constructor-operand", word(
        ckir, bases["operands"] + operand_start * 4, NO_ID))
    ccase("opaque-constructor-result-id", word(ckir, constructor + 16, NO_ID))


def constructor_five_cases(frame_path: Path, output: Path) -> None:
    """Project one declared five-field nominal onto an opcode-13 envelope."""
    output.mkdir(parents=True, exist_ok=False)
    original = frame_path.read_bytes()
    parts = split(original)
    ckir = parts[5]
    ...
    target_type = next(
        index for index, row in enumerate(types)
        if row[1] == 4 and records[row[4]][3] == 5 and records[row[4]][4] == 1
    )
    operation = bases["operations"]
    require(counts["operations"] > 0, "five-field operation carrier")

    def projected(count: int) -> bytes:
        changed = bytearray(ckir)
        changed[operation + 12] = 13
        changed[operation + 13] = 1
        changed[operation + 14:operation + 16] = b"\0\0"
        U32.pack_into(changed, operation + 20, target_type)
        U32.pack_into(changed, operation + 28, count)
        U32.pack_into(changed, operation + 32, 0)
        U32.pack_into(changed, operation + 36, 0)
        return pack(parts, ckir=bytes(changed))

    put(output, "constructor-five-valid", projected(5))
    put(output, "constructor-five-malformed", projected(4))
...
def report(path: Path) -> None:
    ...
    wanted = {
        ...
        "exact-resolver", "exact-lowerer", "compact-resolver",
        "compact-lowerer", "five-resolver", "five-lowerer",
        "exact-check", "compact-check", "controls",
    }
    print("OMGRFN5 responsibility 3 timings: " + " ".join(
...
def main(arguments: list[str]) -> None:
    ...
    if len(arguments) == 3 and arguments[0] == "constructor-cases":
        constructor_cases(Path(arguments[1]), Path(arguments[2])); return
    if len(arguments) == 3 and arguments[0] == "constructor-five-cases":
        constructor_five_cases(Path(arguments[1]), Path(arguments[2])); return
    ...
    raise ValueError(
        "usage: cases OMGRFN5 OUT | constructor-cases OMGRFN5 OUT | "
        "constructor-five-cases OMGRFN5 OUT | summary OMGRFN5 NODES CHILDREN | "
...
if __name__ == "__main__":
    try:
        ...
        print(f"OMGRFN5 responsibility 3 cases: {error}", file=sys.stderr)
        raise SystemExit(2)
