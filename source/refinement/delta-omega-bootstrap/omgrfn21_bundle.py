#!/usr/bin/env python3
"""Untrusted packer for exact OMGCOMP1 + OMGRSWA + CKIR18 + ELF."""
...
from omgrfn21_frame import FLAG_PROPOSITION, MAGIC, VERSION
...
def pack(omgcomp: bytes, witness: bytes, ckir: bytes, elf: bytes,
    ...
    for raw, ceiling, label in (
        ...
        (witness, MAX_WITNESS, "OMGRSWA"),
        (ckir, MAX_CKIR, "CKIR18"),
        (elf, MAX_ELF, "ELF"),
    ):
        if not raw or len(raw) > ceiling:
            raise ValueError(f"{label} must be nonempty and within its ceiling")
    identity(omgcomp, b"OMGCOMP\0", 1, "OMGCOMP")
    identity(witness, b"OMGRSWA\0", 10, "OMGRSW")
    identity(ckir, b"OMGCKIR\0", 18, "CKIR")
    if not elf.startswith(b"\x7fELF"):
        raise ValueError("conservative ELF identity")
    if result != 70:
        raise ValueError("canonical OMGRFN21 result is 70")
...
def main() -> None:
    ...
    sys.stdout.buffer.write(pack(
        ...
        read(args.witness, MAX_WITNESS, "OMGRSWA"),
        read(args.ckir, MAX_CKIR, "CKIR18"),
...
if __name__ == "__main__":
    try:
        ...
        print(f"OMGRFN21 bundle: {error}", file=sys.stderr)
