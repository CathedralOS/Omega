#!/usr/bin/env python3
"""Bounded OMGRFN17 framing shared by its small responsibility owners."""
...
def split(raw: bytes) -> Frame:
    ...
    ceilings = ((omg_len, MAX_OMGCOMP, "OMGCOMP1"),
                (witness_len, MAX_WITNESS, "OMGRSW4"),
                (ckir_len, MAX_CKIR, "CKIR15"),
                (elf_len, MAX_ELF, "ELF"))
    cursor = HEADER.size
    parts: list[bytes] = []
...
def check_r1(frame: Frame) -> None:
    require(frame.magic == MAGIC and frame.version == VERSION, "OMGRFNH version 17")
    require(frame.flags == FLAG_PROPOSITION, "exact successful proposition flags")
    require(all((frame.omgcomp, frame.witness, frame.ckir, frame.elf)),
            "nonempty proposition components")
    require(frame.exit_code == frame.result & 255, "successful result/exit publication")
    try:
