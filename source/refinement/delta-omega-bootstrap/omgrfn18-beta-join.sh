#!/bin/sh
# Representative seed-lineage projections for the general OMGRFN18 owners.
set -eu
...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "OMGRFN18 Beta join: skipped (requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in python3 codesign; do
  command -v "$TOOL" >/dev/null 2>&1 || { echo "OMGRFN18 Beta join: skipped ($TOOL absent)"; exit 0; }
done
...
PYTHONPATH="$R:$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES" \
  python3 -B "$R/omgrfn18-materialize-beta.py" "$T/materialized"

for PROFILE in borrow same-high bit63 max-neighbor max-equal reversed-high; do
  for OWNER in r1 r2 r3 r4-lowering r4-source-result r5-structure r5-result r5-elf; do
    PYTHONPATH="$R:$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES" \
      python3 -B "$R/omgrfn18-$OWNER.py" < "$T/materialized/$PROFILE.rfn" > "$T/$PROFILE-$OWNER-python.out"
...
while IFS="$TAB" read -r OWNER PROFILE REJECT_AT; do
  ...
  [ "$BYTES" -le 262140 ] || {
    echo "OMGRFN18 Beta join: $OWNER tape $BYTES exceeds ceiling" >&2
    exit 1
  }
  ...
  for MODE in native self; do
    ...
    [ "$STATUS" -eq 251 ] && [ ! -s "$T/$OWNER-$MODE-reject.out" ] || {
      echo "OMGRFN18 Beta join: $OWNER $MODE local reject returned $STATUS" >&2
      exit 1
    }
  done
  COUNT=$((COUNT + 1))
...
echo "OMGRFN18 Beta join: $COUNT split owners native/self fixed; six unsigned representatives; max tape $MAX_TAPE bytes"
echo "OMGRFN18 Beta tapes: $TAPE_REPORT"
