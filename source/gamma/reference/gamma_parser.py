#!/usr/bin/env python3
"""Shared untrusted lexer/parser for Gamma reference and refinement tools.

This module owns source recognition and the written, finite Gamma formation
...
# Gamma tokens: identifiers/keywords, decimal integers, and single/double-char
...
def lex(src):
    ...
    for offset, c in enumerate(src):
        ...
        if value not in (9, 10, 13) and not 32 <= value <= 126:
            raise SyntaxError(
                f'gamma parser: invalid source byte at offset {offset}'
            )
    toks = []
    i, n = 0, len(src)
    while i < n:
            ...
            while j < n:
                ...
                if src[j] == '\\':
                    if j + 1 >= n or src[j + 1] not in STRING_ESCAPES:
                        raise SyntaxError('gamma parser: invalid string escape')
                    inner += src[j:j + 2]
                    j += 2
                else:
                    inner += src[j]
                    j += 1
            if j >= n:
                raise SyntaxError('gamma parser: unterminated string literal')
        ...
        if c == "'":
            i += 1
            if i >= n:
                raise SyntaxError('gamma parser: unterminated char literal')
            if src[i] == '\\':
                if i + 1 >= n or src[i + 1] not in CHAR_ESCAPES:
                    raise SyntaxError('gamma parser: invalid char escape')
        ...
        if '0' <= c <= '9':
            ...
            value = int(src[i:j])
            if value > MAX_WORD:
                raise SyntaxError('gamma parser: decimal literal exceeds Word')
        ...
        for op in OPS:
            ...
            raise SyntaxError(f'gamma parser: unexpected char {c!r} at offset {i}')
    toks.append(('eof', None))
    return toks


class Parser:
    """Parse the reference Gamma surface into the historical tuple AST."""
    ...
    def expect(self, kind, val=None):
        ...
        if token[0] != kind or (val is not None and token[1] != val):
            raise SyntaxError(
                f'gamma parser: expected {kind} {val!r}, got {token}'
            )
        return token
    ...
    def proc(self):
        ...
        if self.peek() != ('op', ')'):
            ...
            while self.peek() == ('op', ','):
                self.nxt()
                if self.peek() == ('op', ')'):
                    raise SyntaxError('gamma parser: trailing parameter comma')
                params.append(self.expect('word')[1])
        self.expect('op', ')')
        body = self.block()
    ...
    def block(self):
        ...
        while self.peek() != ('op', '}'):
            ...
            if states_started:
                raise SyntaxError(
                    'gamma parser: ordinary statement after child state'
                )
            if terminated:
                raise SyntaxError(
                    'gamma parser: ordinary statement after block terminator'
                )
            statement = self.stmt()
    ...
    def stmt(self):
        ...
        raise SyntaxError(f'gamma parser: bad statement at {token}')
    ...
    def factor(self):
        ...
        if token == ('op', '('):
            ...
            self.expect('op', ')')
            return expression
        raise SyntaxError(f'gamma parser: bad factor at {token}')
    ...
    def args(self):
        ...
        while self.peek() == ('op', ','):
            self.nxt()
            if self.peek() == ('op', ')'):
                raise SyntaxError('gamma parser: trailing argument comma')
            args.append(self.expr())
        return args
...
def walk_expression(expression, visible, generated, required, calls):
    ...
    if kind == 'var':
        ...
        if name not in visible:
            raise SyntaxError(f'gamma formation: unresolved local {name!r}')
    ...
    if kind == 'bin':
        ...
        return
    raise SyntaxError(f'gamma formation: unknown expression {kind!r}')


def validate_procedure(proc, calls):
    _, proc_name, params, body = proc
    if proc_name in RESERVED:
        raise SyntaxError(f'gamma formation: reserved procedure {proc_name!r}')
    if len(params) > 4 or len(set(params)) != len(params):
        raise SyntaxError(f'gamma formation: invalid parameters for {proc_name!r}')
    if any(param in RESERVED for param in params):
        raise SyntaxError(f'gamma formation: reserved parameter in {proc_name!r}')

    blocks = flatten_blocks(body)
    labels = {}
    for index, (label, _) in enumerate(blocks[1:], 1):
        if label in RESERVED or label in labels:
            raise SyntaxError(f'gamma formation: invalid state {label!r}')
        labels[label] = index
        ...
        for statement in statements:
            ...
            if kind == 'let':
                ...
                if name in RESERVED or name in declarations:
                    raise SyntaxError(f'gamma formation: invalid local {name!r}')
                ...
                name = statement[1]
                if name not in visible:
                    raise SyntaxError(f'gamma formation: unresolved assignment {name!r}')
                walk_expression(statement[2], visible, generated, required, calls)
                generated.add(name)
            elif kind == 'return':
                ...
                raise SyntaxError(f'gamma formation: unknown statement {kind!r}')
        summaries.append({
            'required': required,
            'generated': generated,
            'edges': edges,
    ...
    for index, summary in enumerate(summaries):
        for target, generated in summary['edges']:
            if target not in labels:
                raise SyntaxError(f'gamma formation: unresolved state {target!r}')
    ...
    for index in reachable:
        ...
        if missing:
            raise SyntaxError(
                f'gamma formation: possibly uninitialized local {min(missing)!r}'
            )
...
def validate_program(procs):
    ...
    for proc in procs:
        ...
        if name in procedure_arities:
            raise SyntaxError(f'gamma formation: duplicate procedure {name!r}')
        procedure_arities[name] = len(proc[2])
    if procedure_arities.get('main') != 0:
        raise SyntaxError('gamma formation: expected one zero-parameter main')
    ...
    for name, arity in calls:
        ...
        if expected is None or expected != arity:
            raise SyntaxError(f'gamma formation: unresolved call {name!r}/{arity}')
