#!/usr/bin/env python3
"""Focused ownership and grammar checks for Gamma's executable reference."""
...
class GammaParserTests(unittest.TestCase):
    def parse(self, source):
        return gamma_parser.Parser(gamma_parser.lex(source)).parse()
    ...
    def test_reference_import_does_not_load_backend_or_refinement(self):
        completed = subprocess.run(
            [
                sys.executable,
                '-c',
                'import gamma_parser, gamma_interp, sys; '
                'assert "bc2" not in sys.modules; '
                'assert "gamma_symbolic" not in sys.modules',
            ],
    ...
    def test_complete_reference_surface_has_stable_tuple_ast(self):
        ...
        '''
        ast = gamma_parser.Parser(gamma_parser.lex(source)).parse()
    ...
    def test_guard_parentheses_are_optional(self):
        ...
        bare_ast = gamma_parser.Parser(gamma_parser.lex(bare)).parse()
        grouped_ast = gamma_parser.Parser(gamma_parser.lex(grouped)).parse()
    ...
    def test_recursive_states_preserve_dfs_tuple_shape_and_fallthrough(self):
        ...
        self.assertEqual(gamma_interp.interpret(procs, b''), (7, b''))
    ...
    def test_every_path_initialization_accepts_alternate_assignments_and_loops(self):
        ...
        '''
        self.assertEqual(gamma_interp.interpret(self.parse(alternate), b'\0'), (1, b''))
        self.assertEqual(gamma_interp.interpret(self.parse(alternate), b'x'), (2, b''))
        self.assertEqual(gamma_interp.interpret(self.parse(loop), b''), (3, b''))
    ...
    def test_unreachable_blocks_skip_only_the_initialization_judgment(self):
        ...
        '''
        self.assertEqual(gamma_interp.interpret(self.parse(accepted), b''), (7, b''))
    ...
    def test_reference_sources_have_no_backend_diagnostic_import(self):
        ...
        forbidden = {'bc2', 'gamma_symbolic', 'symbolic_differential'}
        for name in ('gamma_parser.py', 'gamma_interp.py', 'io-verify.py'):
    ...
    def test_retired_compiler_facade_is_absent(self):
        ...
        self.assertFalse((root / 'compiler/gamma-lang-py').exists())
