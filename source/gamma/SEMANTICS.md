...
`reference/gamma_interp.py` is an untrusted executable regression oracle. Its
...
A runnable Gamma program has exactly one zero-parameter `main`. Handling bytes
fed to a compiler written in Gamma is ordinary Gamma byte-I/O behavior; that
input stream is not reparsed as the definition of Gamma itself.
...
`pending_exhaustion` is observer ghost state and is not addressable by the Gamma
...
Out-of-range raw memory is a stuck Gamma configuration. The D23 compiler's
128 MiB profile checks that edge before biasing the address and projects it to a
...
A compiler-refinement theorem compares this maximal Gamma observation with the
...
second Gamma semantics. Its proof presentation makes successor selection and
terminal behavior explicit:

```text
GammaProofState = Running(GammaConfig)
               | Outcome(Halt | Trap | Exhaust)
               | Invalid

next_gamma(Running(config)) = the unique written Gamma transition
next_gamma(Outcome(result)) = Outcome(result)
next_gamma(Invalid)         = Invalid

gamma_at(initial, 0)     = initial
gamma_at(initial, n + 1) = next_gamma(gamma_at(initial, n))
```
...
constructs `next_gamma` from these written rules; it does not infer a successor
...
Beta assembly must encode to its persisted tape exactly; equality then gives
lockstep Alpha behavior. Separately, arbitrary accepted Gamma source and the
