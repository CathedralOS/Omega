#!/usr/bin/env python3
"""Untrusted exact-byte packer for the private OMGRFN11 refinement frame."""
...
def bounded(path: Path, ceiling: int, label: str) -> bytes:
    ...
    if len(contents) > ceiling:
        raise SystemExit(f"{label} exceeds OMGRFN11 ceiling")
    return contents

def require_witness_identity(witness: bytes) -> None:
    if len(witness) < 12 or witness[:8] not in WITNESS_MAJORS:
        raise SystemExit("OMGRFN11 requires OMGRSW1, OMGRSW2, or OMGRSW3")
    major = WITNESS_MAJORS[witness[:8]]
    if struct.unpack_from("<HH", witness, 8) != (major, 0):
        raise SystemExit("OMGRFN11 witness identity mismatch")

def require_ckir9_identity(ckir: bytes) -> None:
    if len(ckir) < 12 or ckir[:8] != CKIR_MAGIC or struct.unpack_from("<HH", ckir, 8) != (9, 0):
        raise SystemExit("OMGRFN11 requires CKIR schema 9.0")
...
def main() -> None:
    ...
    ckir = bounded(args.ckir, MAX_CKIR, "CKIR9")
    elf = bounded(args.elf, MAX_ELF, "ELF")
    if not omgcomp or not witness or not ckir:
        raise SystemExit("OMGCOMP, OMGRSW, and CKIR9 must be nonempty")
    require_witness_identity(witness)
    require_ckir9_identity(ckir)
    if HEADER.size + len(omgcomp) + len(witness) + len(ckir) + len(elf) > MAX_FRAME:
        raise SystemExit("OMGRFN11 frame exceeds whole-frame ceiling")
    if args.library:
    ...
    sys.stdout.buffer.write(HEADER.pack(MAGIC, 11, flags, len(omgcomp), len(witness), len(ckir), len(elf), result, exit_code))
