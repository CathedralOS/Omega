...
cd "$OMEGA_REPO_ROOT"
case "$(uname -sm)" in "Darwin arm64") ;; *) echo "OMGRFN20 Beta join: skipped (requires Darwin arm64)"; exit 0;; esac
T=$(mktemp -d)
trap 'rm -rf "$T"' EXIT
R=$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT
PYTHONPATH="$R:$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES" python3 -B "$R/omgrfn20-materialize-beta.py" "$T/m"
...
TAB=$(printf '\t'); MAX=0; COUNT=0; REPORT=
while IFS="$TAB" read -r OWNER REJECT_AT; do
  ...
  [ "$BYTES" -le 262140 ] || { echo "OMGRFN20 Beta join: $OWNER tape $BYTES exceeds ceiling" >&2; exit 1; }
...
echo "OMGRFN20 Beta join: $COUNT projections fixed; max tape $MAX bytes"
echo "OMGRFN20 Beta tapes: $REPORT"
