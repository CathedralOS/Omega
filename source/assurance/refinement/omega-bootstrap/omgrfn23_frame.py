#!/usr/bin/env python3
"""Bounded OMGRFN23 framing for the complete TokenStream::push cut."""
...
def split(raw: bytes) -> Frame:
    ...
    for length, ceiling, label in (
        ...
        (witness_len, MAX_WITNESS, "OMGRSWC12"),
        (ckir_len, MAX_CKIR, "CKIR20"),
        (elf_len, MAX_ELF, "ELF"),
    ):
        if length > ceiling:
...
def check_r1(frame: Frame) -> None:
    ...
    require(len(frame.witness) >= 12 and frame.witness[:8] == b"OMGRSWC\0"
            and struct.unpack_from("<HH", frame.witness, 8) == (12, 0),
            "exact OMGRSWC12 component identity")
    require(len(frame.ckir) >= 12 and frame.ckir[:8] == b"OMGCKIR\0"
            and struct.unpack_from("<HH", frame.ckir, 8) == (20, 0),
            "exact CKIR20 component identity")
