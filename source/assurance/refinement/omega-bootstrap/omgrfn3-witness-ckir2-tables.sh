#!/usr/bin/env sh
# Focused lower-rooted OMGRFN3 layer-3 witness -> CKIR table/layout gate.
set -eu
...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "OMGRFN3 layer 3: skipped (requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in cargo python3; do
  command -v "$TOOL" >/dev/null 2>&1 || {
    echo "OMGRFN3 layer 3: skipped ($TOOL absent)"
    exit 0
  }
done

CHECKER="$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT/omgrfn3-witness-ckir2-tables.beta"
...
ROOT_FIXTURE="$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES/role3_resolution_fixture.py"
TABLE_FIXTURE="$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES/two_unit_compilation_fixture.py"
...
LOW_FRAME="$OMEGA_PATH_OMEGA_BOOTSTRAP_COMPILER/omega_bootstrap_omglow2.py"
LOWERER1="$OMEGA_PATH_OMEGA_BOOTSTRAP_COMPILER/omega-bootstrap-resolved-to-ckir.alp"
LOW_FRAME1="$OMEGA_PATH_OMEGA_BOOTSTRAP_COMPILER/omega_bootstrap_omglow.py"
MUTATIONS="$OMEGA_PATH_OMEGA_BOOTSTRAP_GATES/resolved_to_ckir_mutations.py"
for REQUIRED in "$CHECKER" "$PACKER" "$PACKER2" "$ROOT_FIXTURE" \
  "$TABLE_FIXTURE" "$RESOLVER" "$LOWERER" "$LOW_FRAME" "$LOWERER1" \
  "$LOW_FRAME1" "$MUTATIONS"; do
  [ -f "$REQUIRED" ] || { echo "OMGRFN3 layer 3: missing $REQUIRED" >&2; exit 1; }
done

PROCEDURES=$(awk '/^proc / { count += 1 } END { print count + 0 }' "$CHECKER")
[ "$PROCEDURES" -le 128 ] || {
  echo "OMGRFN3 layer 3: persisted Beta checker exceeds 128 procedures ($PROCEDURES)" >&2
  exit 1
}
...
python3 "$ROOT_FIXTURE" build "$T/role3"
python3 "$TABLE_FIXTURE" build "$T/canonical"
...
run_expect() (
  ...
  set -e
  [ "$ACTUAL" -eq "$EXPECTED" ] || {
    echo "OMGRFN3 layer 3: $LABEL returned $ACTUAL, expected $EXPECTED" >&2
    sed -n '1,20p' "$OUTPUT.stderr" >&2
    exit 1
  }
  if [ "$EXPECTED" -ne 0 ] && [ -s "$OUTPUT" ]; then
    echo "OMGRFN3 layer 3: $LABEL published bytes on rejection" >&2
    exit 1
  fi
)
...
build_pair role3 "$T/role3/valid.omgc" 70
...
# Cross-version controls keep both the outer frame and inner checked-IR schema
# private and exact.  The call-free parameter product is valid in each inner
# schema, so rejection cannot be attributed to attached-call semantics.
python3 "$LOW_FRAME1" pack "$T/parameter.omgc" "$T/parameter.witness" > "$T/parameter.low1"
run_expect "$T/lowerer1" "$T/parameter.low1" 0 "$T/parameter.ckir1" "parameter CKIR1 lowerer"
python3 "$PACKER" "$T/parameter.omgc" "$T/parameter.witness" \
  "$T/parameter.ckir1" "$T/parameter.elf" --result 0 > "$T/ckir1-in-rfn3.rfn"
python3 "$PACKER2" "$T/parameter.omgc" "$T/parameter.witness" \
  "$T/parameter.ckir" "$T/parameter.elf" --result 0 > "$T/rfn2-with-ckir2.rfn"
run_expect "$T/check" "$T/ckir1-in-rfn3.rfn" 251 "$T/ckir1-cross.out" \
  "CKIR1 at OMGRFN3 layer-3 boundary"
run_expect "$T/check" "$T/rfn2-with-ckir2.rfn" 251 "$T/rfn2-cross.out" \
  "OMGRFN2 at OMGRFN3 layer-3 boundary"

python3 - "$T/canonical.rfn" "$T/parameter.rfn" "$T/layout.rfn" \
  "$T/type-order.rfn" "$T/role3.rfn" "$T/cases" <<'PY'
from pathlib import Path
import struct
import sys
...
role3 = Path(sys.argv[5]).read_bytes()
out = Path(sys.argv[6])
out.mkdir()
U32 = struct.Struct("<I")
...
def split(raw):
    ...
    assert magic == b"OMGRFN3\0" and version == 3
    at = 40
...
def pack(parts, *, witness=None, ckir=None, elf=None, result=None):
    ...
    return struct.pack("<8s8I", b"OMGRFN3\0", 3, flags, len(comp), len(witness),
...
# The canonical role-3 fixture has selected root 0 and another legal
# zero-parameter scalar machine at ID 3.  CKIR2 must carry the explicit witness
# root: neither carrier may drift independently to the decoy.
assert U32.unpack_from(rp[3], 64)[0] == 0
assert U32.unpack_from(rp[4], 16)[0] == 0
put("witness-root-drift-to-decoy", pack(rp, witness=word(rp[3], 64, 3)))
put("ckir-root-drift-to-decoy", pack(rp, ckir=word(rp[4], 16, 3)))
...
for CASE in bad-type-row bad-record-join bad-field-owner bad-machine-owner \
  ...
  witness-root-drift-to-decoy ckir-root-drift-to-decoy \
...
echo "OMGRFN3 layer 3: witness/CKIR tables, canonical interning, copy/layout, root, cross-pairs, opaque later layers, and 0/251/252 teeth passed below Delta (${ELAPSED}s)"
