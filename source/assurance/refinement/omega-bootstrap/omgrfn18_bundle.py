#!/usr/bin/env python3
"""Untrusted packer for exact OMGCOMP1 + OMGRSW8 + CKIR16 + ELF."""
...
from omgrfn18_frame import FLAG_PROPOSITION, MAGIC, VERSION
...
def pack(omgcomp: bytes, witness: bytes, ckir: bytes, elf: bytes, result: int) -> bytes:
    for raw, ceiling, label in (
        ...
        (witness, MAX_WITNESS, "OMGRSW8"),
        (ckir, MAX_CKIR, "CKIR16"),
        (elf, MAX_ELF, "ELF"),
    ):
        if not raw or len(raw) > ceiling:
            raise ValueError(f"{label} must be nonempty and within its ceiling")
    identity(omgcomp, b"OMGCOMP\0", 1, "OMGCOMP")
    identity(witness, b"OMGRSW8\0", 8, "OMGRSW")
    identity(ckir, b"OMGCKIR\0", 16, "CKIR")
    ...
    return HEADER.pack(MAGIC, VERSION, FLAG_PROPOSITION, *lengths,
                       result, result & 255) + omgcomp + witness + ckir + elf
...
def main() -> None:
    ...
    sys.stdout.buffer.write(pack(
        ...
        read(args.witness, MAX_WITNESS, "OMGRSW8"),
        read(args.ckir, MAX_CKIR, "CKIR16"),
...
if __name__ == "__main__":
    try:
        ...
        print(f"OMGRFN18 bundle: {error}", file=sys.stderr)
