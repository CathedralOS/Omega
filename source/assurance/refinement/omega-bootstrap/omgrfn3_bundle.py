#!/usr/bin/env python3
"""Untrusted exact-byte packer for the private OMGRFN3 refinement frame."""
...
def bounded(path: Path, ceiling: int, label: str) -> bytes:
    ...
    if len(contents) > ceiling:
        raise SystemExit(f"{label} exceeds OMGRFN3 ceiling")
    return contents
...
def main() -> None:
    ...
    sys.stdout.buffer.write(HEADER.pack(
        MAGIC, 3, flags, len(omgcomp), len(witness), len(ckir), len(elf), result, exit_code,
    ))
