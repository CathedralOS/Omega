#!/usr/bin/env sh
# Focused lower-rooted OMGRFN3 layer-1 framing and OMGCOMP custody gate.
set -eu
...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "OMGRFN3 layer 1: skipped (requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in python3; do
  command -v "$TOOL" >/dev/null 2>&1 || {
    echo "OMGRFN3 layer 1: skipped ($TOOL absent)"
    exit 0
  }
done

CHECKER="$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT/omgrfn3-frame-omgcomp-custody.beta"
...
for REQUIRED in "$CHECKER" "$PACKER" "$PACKER2" "$FIXTURE"; do
  [ -f "$REQUIRED" ] || { echo "OMGRFN3 layer 1: missing $REQUIRED" >&2; exit 1; }
done

PROCEDURES=$(awk '/^proc / { count += 1 } END { print count + 0 }' "$CHECKER")
[ "$PROCEDURES" -le 128 ] || {
  echo "OMGRFN3 layer 1: persisted Beta checker exceeds 128 procedures ($PROCEDURES)" >&2
  exit 1
}
...
lines = ["", "proc main() {", "    let status = omgrfn3_layer1_check()"]
lines += ["    to failed when (status != 0)", "    to span_0"]
for source_id, (start, length) in enumerate(expected):
    lines += [
        f"    state span_{source_id} {{",
        f"        to bad when (omgrfn3_source_content_start({source_id}) != {start})",
        f"        to length_{source_id}",
        "    }",
        f"    state length_{source_id} {{",
        f"        to bad when (omgrfn3_source_content_length({source_id}) != {length})",
...
observe() {
  ...
  set -e
  [ "$ACTUAL" -eq "$EXPECTED" ] || {
    echo "OMGRFN3 layer 1: $LABEL returned $ACTUAL, expected $EXPECTED" >&2
    sed -n '1,12p' "$T/stderr" >&2
    exit 1
  }
  [ ! -s "$T/stdout" ] || {
    echo "OMGRFN3 layer 1: $LABEL published stdout" >&2
    exit 1
  }
}
...
echo "OMGRFN3 layer 1: exact v3 framing, full OMGCOMP structure/content extents, version separation, opaque components, EOF, and 0/251/252 teeth passed below Delta"
