#!/usr/bin/env sh
# Focused lower-rooted OMGRFN4 layer-1 framing and OMGCOMP custody gate.
set -eu
...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "OMGRFN4 layer 1: skipped (requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in python3; do
  command -v "$TOOL" >/dev/null 2>&1 || {
    echo "OMGRFN4 layer 1: skipped ($TOOL absent)"
    exit 0
  }
done

CHECKER="$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT/omgrfn4-frame-omgcomp-custody.beta"
ENVELOPE="$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT/omgrfn4-component-envelope.beta"
CHECKER3="$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT/omgrfn3-frame-omgcomp-custody.beta"
PACKER="$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT/omgrfn4_bundle.py"
PACKER2="$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT/omgrfn2_bundle.py"
PACKER3="$OMEGA_PATH_OMEGA_BOOTSTRAP_REFINEMENT/omgrfn3_bundle.py"
...
for REQUIRED in "$CHECKER" "$ENVELOPE" "$CHECKER3" "$PACKER" "$PACKER2" \
  "$PACKER3" "$FIXTURE" "$UNICODE" "$HARNESS"; do
  [ -f "$REQUIRED" ] || { echo "OMGRFN4 layer 1: missing $REQUIRED" >&2; exit 1; }
done

PROCEDURES=$(awk '/^proc / { count += 1 } END { print count + 0 }' "$CHECKER")
[ "$PROCEDURES" -le 128 ] || {
  echo "OMGRFN4 layer 1: persisted Beta checker exceeds 128 procedures ($PROCEDURES)" >&2
  exit 1
}
...
build_checker() { # name checker-source main-suffix
  NAME=$1
  SOURCE=$2
  SUFFIX=$3
  cp "$SOURCE" "$T/$NAME.beta"
...
build_checker check "$CHECKER" "$T/main.beta"
printf '\nproc main() { return omgrfn4_component_read() }\n' > "$T/component-main.beta"
build_checker component "$ENVELOPE" "$T/component-main.beta"
printf '\nproc main() { return omgrfn3_layer1_check() }\n' > "$T/version3-main.beta"
build_checker version3-check "$CHECKER3" "$T/version3-main.beta"

python3 -B "$FIXTURE" build "$T/unicode.omgc" UnicodeTables \
  bootstrap_constant_aggregate_probe "$UNICODE" "$HARNESS"
OMGCOMP="$T/unicode.omgc"
...
python3 "$PACKER2" "$OMGCOMP" "$T/witness" "$T/ckir" "$T/elf" \
  --result 70 > "$T/version2.rfn"
python3 "$PACKER3" "$OMGCOMP" "$T/witness" "$T/ckir" "$T/elf" \
  --result 70 > "$T/version3.rfn"
...
lines = ["", "proc main() {", "    let status = omgrfn4_layer1_check()"]
lines += ["    to failed when (status != 0)", "    to span_0"]
for source_id, (start, length) in enumerate(expected):
    lines += [
        f"    state span_{source_id} {{",
        f"        to bad when (omgrfn4_source_content_start({source_id}) != {start})",
        f"        to length_{source_id}",
        "    }",
        f"    state length_{source_id} {{",
        f"        to bad when (omgrfn4_source_content_length({source_id}) != {length})",
...
PY
build_checker extent-check "$CHECKER" "$T/extent-main.beta"
...
observe() {
  ...
  set -e
  [ "$ACTUAL" -eq "$EXPECTED" ] || {
    echo "OMGRFN4 layer 1: $LABEL returned $ACTUAL, expected $EXPECTED" >&2
    sed -n '1,12p' "$T/stderr" >&2
    exit 1
  }
  [ ! -s "$T/stdout" ] || {
    echo "OMGRFN4 layer 1: $LABEL published stdout" >&2
    exit 1
  }
}
...
observe_with "$T/component" 0 "$T/entry.rfn" "component adapter entry frame"
observe_with "$T/component" 0 "$T/library.rfn" "component adapter library frame"
observe_with "$T/extent-check" 0 "$T/entry.rfn" "source-ID content extents"
observe 251 "$T/version2.rfn" "frozen OMGRFN2 cross-version frame"
observe 251 "$T/version3.rfn" "frozen OMGRFN3 cross-version frame"
observe_with "$T/version3-check" 251 "$T/entry.rfn" \
  "frozen OMGRFN3 checker rejects OMGRFN4"
...
entry_without_elf = bytearray(library)
struct.pack_into("<I", entry_without_elf, 12, 1)
put("entry-without-elf", bytes(entry_without_elf))
library_result = bytearray(library)
struct.pack_into("<I", library_result, 32, 70)
...
put("over-ckir", u32(24, 2522193))
put("over-elf", u32(28, 1183745))
...
for CASE in bad-magic bad-version bad-flags bad-exit library-with-elf \
  entry-without-elf library-with-result truncated trailing \
...
observe_with "$T/component" 251 "$T/cases/bad-version" \
  "component adapter wrong version"
observe_with "$T/component" 252 "$T/cases/over-ckir" \
  "component adapter CKIR3 extent exhaustion"

# Realize the greatest valid Unicode-based frame at OMGCOMP's first tighter
# 262,144-byte aggregate-source-content ceiling. Two whitespace-only source
# units supply custody without changing the exact Unicode unit or its harness.
: > "$T/filler-a.omg"
: > "$T/filler-b.omg"
MAX_FILL=$((262144 - $(wc -c < "$UNICODE" | tr -d ' ') \
  - $(wc -c < "$HARNESS" | tr -d ' ')))
[ "$MAX_FILL" -ge 0 ] && [ "$MAX_FILL" -le 262144 ] || {
  echo "OMGRFN4 layer 1: cannot realize aggregate source maximum ($MAX_FILL filler bytes)" >&2
  exit 1
}
python3 - "$T/filler-a.omg" "$T/filler-b.omg" "$MAX_FILL" <<'PY'
from pathlib import Path
import sys

remaining = int(sys.argv[3])
first = min(remaining, 131_072)
second = remaining - first
if second > 131_072:
    raise SystemExit("OMGRFN4 exact OMGCOMP filler exceeds two source extents")
Path(sys.argv[1]).write_bytes(b" " * first)
Path(sys.argv[2]).write_bytes(b" " * second)
PY
python3 -B "$FIXTURE" build "$T/max.omgc" UnicodeTables \
  bootstrap_constant_aggregate_probe "$UNICODE" "$T/filler-a.omg" \
  "$T/filler-b.omg" "$HARNESS"
[ "$(( $(wc -c < "$UNICODE" | tr -d ' ') \
  + $(wc -c < "$HARNESS" | tr -d ' ') \
  + $(wc -c < "$T/filler-a.omg" | tr -d ' ') \
  + $(wc -c < "$T/filler-b.omg" | tr -d ' ') ))" -eq 262144 ] || {
  echo "OMGRFN4 layer 1: aggregate source maximum construction drifted" >&2
  exit 1
}
...
PY
python3 "$PACKER" "$T/max.omgc" "$T/max-witness" "$T/max-ckir" \
  "$T/max-elf" --result 70 > "$T/greatest-valid.rfn"
GREATEST_VALID=$(wc -c < "$T/greatest-valid.rfn" | tr -d ' ')
observe 0 "$T/greatest-valid.rfn" "greatest valid aggregate-source frame"
observe_with "$T/component" 0 "$T/greatest-valid.rfn" \
  "component adapter greatest valid aggregate-source frame"

# The raw frame ceiling itself is reachable as transport even though complete
# OMGCOMP structure makes the simultaneous component arithmetic maximum
# semantically unrealizable. Exact length reaches identity checking (251),
# while the first adjacent byte selects transport exhaustion (252).
python3 - "$T/exact-ceiling" <<'PY'
from pathlib import Path
import sys
Path(sys.argv[1]).write_bytes(b"\0" * 4_497_544)
PY
observe 251 "$T/exact-ceiling" "exact raw whole-frame ceiling"
observe_with "$T/component" 251 "$T/exact-ceiling" \
  "component adapter exact raw whole-frame ceiling"

python3 - "$T/oversized" <<'PY'
from pathlib import Path
import sys
Path(sys.argv[1]).write_bytes(b"\0" * 4497545)
PY
observe 252 "$T/oversized" "whole-frame overflow"
observe_with "$T/component" 252 "$T/oversized" \
  "component adapter whole-frame overflow"

CHECK_PROCEDURES=$(awk '/^proc / { count += 1 } END { print count + 0 }' "$T/check.beta")
COMPONENT_PROCEDURES=$(awk '/^proc / { count += 1 } END { print count + 0 }' "$T/component.beta")
CHECK_TAPE=$(wc -c < "$T/check.tape" | tr -d ' ')
COMPONENT_TAPE=$(wc -c < "$T/component.tape" | tr -d ' ')
UNICODE_OMGCOMP=$(wc -c < "$OMGCOMP" | tr -d ' ')
echo "OMGRFN4 layer 1: exact v4 framing, Unicode+harness OMGCOMP (${UNICODE_OMGCOMP} bytes), full structure/content extents, version separation, opaque components, greatest-valid=${GREATEST_VALID} bytes, raw-ceiling=251, adjacent=252, EOF, and 0/251/252 teeth passed below Delta (${CHECK_PROCEDURES}/128 procedures, ${CHECK_TAPE}/262140 tape bytes; component ${COMPONENT_PROCEDURES}/128 procedures, ${COMPONENT_TAPE}/262140 tape bytes)"
