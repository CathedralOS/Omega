#!/usr/bin/env sh
# Adjacent gate for the canonical Delta compiler's retained frontend and direct
...
dcreq_emit_byte() {
  dcreq_emit_octal=$(printf '%03o' "$1")
  printf "\\$dcreq_emit_octal"
}
dcreq_emit_u32le() {
  dcreq_emit_u32_value=$1
  dcreq_emit_byte $((dcreq_emit_u32_value % 256))
  dcreq_emit_byte $(((dcreq_emit_u32_value / 256) % 256))
  dcreq_emit_byte $(((dcreq_emit_u32_value / 65536) % 256))
  dcreq_emit_byte $(((dcreq_emit_u32_value / 16777216) % 256))
}
dcreq_header() {
  printf '\104\103\122\105\121\001\000\000'
  dcreq_emit_u32le "$1"
  dcreq_emit_u32le "$2"
}
dcreq_text() {
  dcreq_text_profile=$1
  dcreq_text_source=$2
  dcreq_text_length=$(printf '%s' "$dcreq_text_source" | wc -c | tr -d ' ')
  dcreq_header "$dcreq_text_profile" "$dcreq_text_length"
  printf '%s' "$dcreq_text_source"
}
dcreq_file() {
  dcreq_file_profile=$1
  dcreq_file_source=$2
  dcreq_file_length=$(wc -c < "$dcreq_file_source" | tr -d ' ')
  dcreq_header "$dcreq_file_profile" "$dcreq_file_length"
  cat "$dcreq_file_source"
}
runtime_emitter_source() {
  sed -n \
    '/^; ---- direct Alpha payload and fixup substrate/,/^; This is the eventual expression dispatcher/p' \
    delta_compiler.gamma
}
...
{
  sed -n '1,$p' delta_compiler.gamma
  printf '%s\n' \
    '' \
    'proc main() {' \
  ...
  echo "gc(delta_compiler.gamma + frontend gate entry) failed"
  exit 1
}
stamp_seed "$T/tc.tape" "$SEED" "$T/tc.exe" >/dev/null 2>&1

# Observe the private D30/D33 admission judgment without constructing DCOUT or
...
{
  sed -n '1,$p' delta_compiler.gamma
  private_outcome_probe_entry admit_dcreq
} | "$T/gc.exe" > "$T/gcreq-admission.tape" || {
  echo "gc(delta_compiler.gamma + DCREQ admission probe) failed"
  exit 1
}
...
} | "$T/gc.exe" > "$T/gcout-frontend.tape" || {
  echo "gc(delta_compiler.gamma + private DCOUT frontend probe) failed"
  exit 1
}
...
} | "$T/gc.exe" > "$T/gcout-frontend-resource.tape" || {
  echo "gc(delta_compiler.gamma + private frontend resource projection) failed"
  exit 1
}
...
} | "$T/gc.exe" > "$T/gcout-emitter-outcome.tape" || {
  echo "gc(delta_compiler.gamma + private emitter outcome projection) failed"
  exit 1
}
...
# parsed binder at the exact and adjacent allocator cursors.
{
  ...
  printf '%s\n' \
    ...
    '}'
} | "$T/gc.exe" > "$T/local-profile.tape" || {
  echo "gc(delta_compiler.gamma + local-profile gate) failed"
  exit 1
}
...
} | "$T/gc.exe" > "$T/gcout-schema.tape" || {
  echo "gc(delta_compiler.gamma + D33 schema outcome probe) failed"
  exit 1
}
...
# independently of the public outcome probe.
{
  ...
  printf '%s\n' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/d19-identities.tape" || {
  echo "gc(delta_compiler.gamma + D19 identity gate) failed"
  exit 1
}
...

{
  ...
  printf '%s\n' \
    ...
    '        let invalid_frame_result = emit_delta_call_frame(frame_label, frame_label, 8, 0)' \
    ...
    '        emit_reset()' \
    '        let local_prefix_result = emit_delta_frame_local(47, 0, 0)' \
    ...
    '        emit_reset()' \
    '        let local_adjacent_result = emit_delta_frame_local(48, 2, 0)' \
    ...
    '        emit_reset()' \
    '        let local_mode_result = emit_delta_frame_local(48, 1, 2)' \
    '        to local_mode_check' \
    '    }' \
    ...
    '        emit_reset()' \
    '        let invalid_constructor_result = emit_delta_constructor_value(frame_label, 1, 0)' \
    ...
    '        emit_reset()' \
    '        let invalid_input_result = emit_delta_read_sealed_bytes(frame_label, frame_label, frame_label, 0 - 1)' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/emitter.tape" || {
  echo "gc(delta_compiler.gamma + emitter probe) failed"
  exit 1
}
...

{
  ...
  printf '%s\n' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/runtime-emitter.tape" || {
  echo "gc(delta_compiler.gamma + runtime containment probe) failed"
  exit 1
}
...
if [ "$runtime_emitter_status" != 1 ] || [ ! -s "$T/runtime-probe.tape" ]; then
  echo "delta runtime probe emission failed: status $runtime_emitter_status" >&2
  exit 1
fi
...

{
  ...
  printf '%s\n' \
    ...
    '    emit_rr(3, 22, 24)' \
    '    emit_rr(11, 22, 20)' \
    '    emit_delta_call_frame(stack_label, loop_label, 16, 2)' \
    ...
    '    emit_rr(3, 24, 25)' \
    '    emit_rr(11, 24, 20)' \
    '    emit_delta_tail_frame(32, 3, large_label, resource_label)' \
    '    define_label(loop_return)' \
    '    emit_imm(0, 0)' \
    '    emit_rr(2, 1, 21)' \
    '    emit_delta_return_frame()' \
    ...
    '    emit_rr(3, 24, 25)' \
    '    emit_rr(11, 24, 21)' \
    '    emit_delta_tail_frame(16, 2, loop_label, resource_label)' \
    ...
    '    emit_delta_call_frame(stack_label, wide_label, 48, 600)' \
    '    emit_jump(12, after_wide)' \
    '    define_label(wide_label)' \
    '    emit_frame_eq(252, 253, wide_frame_ok, unexpected_label)' \
    '    emit_imm(0, 37)' \
    '    emit_imm(1, 88)' \
    '    emit_delta_frame_local(48, 1, 1)' \
    '    emit_imm(0, 0)' \
    '    emit_imm(1, 0)' \
    '    emit_delta_frame_local(48, 1, 0)' \
    '    emit_imm(22, 37)' \
    ...
    '    emit_delta_tail_frame(15728624, 1, boundary_target, resource_label)' \
    '    define_label(boundary_adjacent)' \
    '    emit_imm(30, 1)' \
    '    emit_delta_tail_frame(15728640, 1, boundary_target, resource_label)' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/frame-emitter.tape" || {
  echo "gc(delta_compiler.gamma + Delta frame ABI probe) failed"
  exit 1
}
...
if [ "$frame_emitter_status" != 1 ] || [ ! -s "$T/frame-probe.tape" ]; then
  echo "Delta frame ABI probe emission failed: status $frame_emitter_status" >&2
  exit 1
fi
...

{
  ...
  printf '%s\n' \
    ...
    '    emit_delta_constructor_value(heap_label, 42, 600)' \
    '    emit_rr(2, 20, 1)' \
    '    emit_imm(22, 42)' \
    ...
    '    emit_constructor_push(stack_label, 23, 24)' \
    '    emit_delta_constructor_value(heap_label, 43, 2)' \
    '    emit_rr(2, 21, 1)' \
    '    emit_imm(22, 43)' \
    '    emit_constructor_eq(0, 22, nested_kind_ok, unexpected_label)' \
    '    emit_rr(2, 1, 21)' \
    '    emit_delta_field_load(2, 0, internal_label)' \
    '    emit_imm(22, 42)' \
    ...
    '    emit_rr(2, 1, 21)' \
    '    emit_delta_field_load(2, 1, internal_label)' \
    '    emit_imm(22, 1)' \
    ...
    '    emit_delta_constructor_value(heap_label, 44, 0)' \
    '    emit_imm(22, 44)' \
    ...
    '    emit_imm(30, 2)' \
    '    emit_imm(1, 16777249)' \
    '    emit_delta_field_load(1, 0, internal_label)' \
    '    emit_jump(12, unexpected_label)' \
    ...
    '    emit_imm(24, 7)' \
    '    emit_constructor_push(stack_label, 23, 24)' \
    '    emit_delta_constructor_value(heap_label, 42, 1)' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/constructor-emitter.tape" || {
  echo "gc(delta_compiler.gamma + Delta constructor ABI probe) failed"
  exit 1
}
...
if [ "$constructor_emitter_status" != 1 ] || [ ! -s "$T/constructor-probe.tape" ]; then
  echo "Delta constructor ABI probe emission failed: status $constructor_emitter_status" >&2
  exit 1
fi
...

{
  ...
  printf '%s\n' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/bytes-valid-emitter.tape" || {
  echo "gc(delta_compiler.gamma + Bytes valid runtime probe) failed"
  exit 1
}
...
if [ "$bytes_valid_emitter_status" != 1 ] || [ ! -s "$T/bytes-valid-probe.tape" ]; then
  echo "Delta Bytes valid probe emission failed: status $bytes_valid_emitter_status" >&2
  exit 1
fi
...

{
  ...
  printf '%s\n' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/bytes-invalid-emitter.tape" || {
  echo "gc(delta_compiler.gamma + Bytes failure-class probe) failed"
  exit 1
}
...
if [ "$bytes_invalid_emitter_status" != 1 ] || [ ! -s "$T/bytes-invalid-probe.tape" ]; then
  echo "Delta Bytes failure-class probe emission failed: status $bytes_invalid_emitter_status" >&2
  exit 1
fi
...

{
  ...
  printf '%s\n' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/bytes-d21-emitter.tape" || {
  echo "gc(delta_compiler.gamma + D21 Bytes-length gate) failed"
  exit 1
}
...
if [ "$bytes_d21_emitter_status" != 1 ] || [ ! -s "$T/bytes-d21-probe.tape" ]; then
  echo "Delta D21 Bytes-length probe emission failed: status $bytes_d21_emitter_status" >&2
  exit 1
fi
...

{
  ...
  printf '%s\n' \
    ...
    '    emit_delta_read_sealed_bytes(read_label, input_resource_label, heap_resource_label, 3)' \
    '    emit_delta_read_sealed_bytes(read_zero_label, input_resource_label, heap_resource_label, 0)' \
    '    emit_bytes_length(length_label, internal_label)' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/sealed-input-emitter.tape" || {
  echo "gc(delta_compiler.gamma + sealed input probe) failed"
  exit 1
}
...
if [ "$sealed_input_emitter_status" != 1 ] || [ ! -s "$T/sealed-input-probe.tape" ]; then
  echo "Delta sealed input probe emission failed: status $sealed_input_emitter_status" >&2
  exit 1
fi
...

{
  ...
  printf '%s\n' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/int-emitter.tape" || {
  echo "gc(delta_compiler.gamma + checked Int probe) failed"
  exit 1
}
...
if [ "$int_emitter_status" != 1 ] || [ ! -s "$T/int-probe.tape" ]; then
  echo "Delta checked Int probe emission failed: status $int_emitter_status" >&2
  exit 1
fi
...
# return-kind/root-frame postcondition is authored directly because the Gamma
...
# still replays its instruction boundaries and targets before publication.
{
  ...
  printf '%s\n' \
    ...
    '        let labels_ok = prepare_delta_function_labels()' \
    '        to labels_prepared' \
    '    }' \
    ...
    '        emit_runtime_init()' \
    '        emit_delta_call_frame(stack_label, row_zero_label, row_zero_prefix, 0)' \
    ...
    '        let bodies_ok = emit_delta_function_bodies()' \
    '        to bodies_emitted' \
    '    }' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/function-lowering-emitter.tape" || {
  echo "gc(delta_compiler.gamma + whole-function lowering gate) failed"
  exit 1
}
...
# admitted; its adjacent exhausted profile is not.
{
  ...
  printf '%s\n' \
    ...
    '        let parameter_status = prepare_delta_function_labels()' \
    ...
    '        emit_reset()' \
    '        word[body] = 0 - 1' \
    '        let body_status = prepare_delta_function_labels()' \
    '        to body_checked' \
    '    }' \
    '    state body_checked {' \
    ...
    '        emit_reset()' \
    '        word[2097032] = 65535' \
    '        let exact_label_status = prepare_delta_function_labels()' \
    ...
    '        word[10485760] = 0' \
    '        let malformed_label_status = emit_delta_function_bodies()' \
    ...
    '        emit_reset()' \
    '        word[2097032] = 65536' \
    '        let adjacent_label_status = prepare_delta_function_labels()' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/function-metadata.tape" || {
  echo "gc(delta_compiler.gamma + whole-function metadata gate) failed"
  exit 1
}
...
# preserve the first private failure across a later attempted overwrite.
{
  ...
  printf '%s\n' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/match-metadata.tape" || {
  echo "gc(delta_compiler.gamma + selected-match metadata gate) failed"
  exit 1
}
...
# packed let-slot profile without consuming their fixed-tape headroom.
{
  ...
  printf '%s\n' \
    ...
    '    state failed { return 0 }' \
    '}'
} | "$T/gc.exe" > "$T/resolver-metadata.tape" || {
  echo "gc(delta_compiler.gamma + resolver-metadata gate) failed"
  exit 1
}
...
# the sixtieth must be the Gamma compiler's canonical adjacent refusal.
for delta_call_probe_count in 59 60; do
  {
    sed -n '1,$p' delta_compiler.gamma
    printf '%s\n' 'proc main() {'
    awk -v n="$delta_call_probe_count" 'BEGIN {
      for (i = 0; i < n; i++) print "    source_byte_allowed(0)"
    }'
    printf '%s\n' '    return 0' '}'
  } | "$T/gc.exe" > "$T/call-rows-$delta_call_probe_count.out"
  delta_call_probe_status=$?
  case "$delta_call_probe_count" in
    59) delta_call_probe_status_59=$delta_call_probe_status ;;
    60) delta_call_probe_status_60=$delta_call_probe_status ;;
  esac
done
delta_call_probe_size=$(wc -c < "$T/call-rows-60.out" | tr -d ' ')
delta_call_probe_kind=$(od -An -tu1 -j 8 -N 1 "$T/call-rows-60.out" | tr -d ' ')
delta_call_probe_space=$(od -An -tu1 -j 9 -N 1 "$T/call-rows-60.out" | tr -d ' ')
delta_call_probe_code=$(od -An -tx1 -j 12 -N 4 "$T/call-rows-60.out" | tr -d ' \n')
delta_call_probe_limit=$(od -An -tx1 -j 24 -N 8 "$T/call-rows-60.out" | tr -d ' \n')
delta_call_probe_requested=$(od -An -tx1 -j 32 -N 8 "$T/call-rows-60.out" | tr -d ' \n')
if [ "$delta_call_probe_status_59" = 0 ] &&
   [ "$delta_call_probe_status_60" = 2 ] &&
   [ "$delta_call_probe_size" = 40 ] &&
   [ "$delta_call_probe_kind" = 2 ] &&
   [ "$delta_call_probe_space" = 1 ] &&
   [ "$delta_call_probe_code" = 09000000 ] &&
   [ "$delta_call_probe_limit" = 0004000000000000 ] &&
   [ "$delta_call_probe_requested" = 0104000000000000 ]; then
  PASS=$((PASS+1))
else
  FAIL=$((FAIL+1))
  echo "  FAIL Delta retained call rows: exact=$delta_call_probe_status_59 adjacent=$delta_call_probe_status_60 frame=$delta_call_probe_size/$delta_call_probe_kind/$delta_call_probe_space/$delta_call_probe_code/$delta_call_probe_limit/$delta_call_probe_requested"
fi
unset delta_call_probe_count delta_call_probe_status delta_call_probe_status_59 delta_call_probe_status_60
unset delta_call_probe_size delta_call_probe_kind delta_call_probe_space
unset delta_call_probe_code delta_call_probe_limit delta_call_probe_requested
schema_case() { # name profile source status kind code space coordinate
  ...
  dcreq_text "$schema_profile" "$schema_source" > "$T/schema-$schema_name.req"
...
d19_outcomes='(Complete Bytes) (Reject EpsilonRejectReason Int) (StorageIncompleteAt Int Int Int) (StorageIncompleteTotal Int Int)'
d19_valid="(data EpsilonRejectReason $d19_reason_reverse) (data EpsilonCompileOutcome $d19_outcomes) (def main ((source Bytes)) EpsilonCompileOutcome (Complete source))"
...
schema_case conformance-ignores-epsilon-schema 1 \
  '(data EpsilonRejectReason (Bogus Int)) (def main ((input Bytes)) Bytes input)' 1 0 0 0 0

schema_case epsilon-exact 2 "$d19_valid" 1 0 0 0 0
dcreq_text 2 "$d19_valid" | "$T/d19-identities.exe" > "$T/d19-identities.out"
d19_identity_status=$?
...
schema_case epsilon-missing-main-precedes-schema 2 \
  '(data EpsilonRejectReason (Bogus Int)) (def helper () Int 0)' 0 1 19 0 0
schema_case epsilon-wrong-main-precedes-schema 2 \
  '(def main ((input Int)) Bytes (bytes_empty))' 0 1 20 1 5
schema_case epsilon-missing-reason 2 \
  "(data EpsilonRejectReason $d19_reason_missing) (data EpsilonCompileOutcome $d19_outcomes) (def main ((source Bytes)) EpsilonCompileOutcome (Complete source))" \
  0 1 21 0 0
schema_case epsilon-missing-type 2 \
  '(data EpsilonCompileOutcome (Complete Bytes) (StorageIncompleteAt Int Int Int) (StorageIncompleteTotal Int Int)) (def main ((source Bytes)) EpsilonCompileOutcome (Complete source))' \
  0 1 21 0 0

d19_prefix="(data EpsilonRejectReason $d19_reason_reverse) (data EpsilonCompileOutcome (Complete Bytes) ("
d19_coordinate=$(printf '%s' "$d19_prefix" | wc -c | tr -d ' ')
schema_case epsilon-malformed-reject 2 \
  "${d19_prefix}Reject EpsilonRejectReason Bytes) (StorageIncompleteAt Int Int Int) (StorageIncompleteTotal Int Int)) (def main ((source Bytes)) EpsilonCompileOutcome (Complete source))" \
  0 1 21 1 "$d19_coordinate"
schema_case epsilon-absence-precedes-location 2 \
  "(data EpsilonRejectReason $d19_reason_missing) (data EpsilonCompileOutcome (Complete Bytes) (Reject EpsilonRejectReason Bytes) (StorageIncompleteAt Int Int Int) (StorageIncompleteTotal Int Int)) (def main ((source Bytes)) EpsilonCompileOutcome (Complete source))" \
  0 1 21 0 0
schema_case epsilon-earliest-located 2 \
  "${d19_prefix}Reject EpsilonRejectReason Bytes) (StorageIncompleteAt Int Int) (StorageIncompleteTotal Int Int)) (def main ((source Bytes)) EpsilonCompileOutcome (Complete source))" \
  0 1 21 1 "$d19_coordinate"

d19_prefix="(data EpsilonRejectReason $d19_reason_reverse) (data EpsilonCompileOutcome (Complete Bytes) (Reject EpsilonRejectReason Bytes) (StorageIncompleteAt Int Int Int) (StorageIncompleteTotal Int Int)) (def "
d19_coordinate=$(printf '%s' "$d19_prefix" | wc -c | tr -d ' ')
schema_case epsilon-entry-category-precedes-coordinate 2 \
  "${d19_prefix}main ((source Bytes)) Bytes source)" 0 1 20 1 "$d19_coordinate"

d19_prefix='(data Other ('
d19_coordinate=$(printf '%s' "$d19_prefix" | wc -c | tr -d ' ')
schema_case epsilon-wrong-owner-located 2 \
  "${d19_prefix}InvalidSourceByte)) (data EpsilonRejectReason $d19_reason_missing) (data EpsilonCompileOutcome $d19_outcomes) (def main ((source Bytes)) EpsilonCompileOutcome (Complete source))" \
  0 1 21 1 "$d19_coordinate"
d19_prefix="(data EpsilonRejectReason $d19_reason_reverse) (data EpsilonCompileOutcome $d19_outcomes ("
d19_coordinate=$(printf '%s' "$d19_prefix" | wc -c | tr -d ' ')
schema_case epsilon-extra-outcome-located 2 \
  "${d19_prefix}Other)) (def main ((source Bytes)) EpsilonCompileOutcome (Complete source))" \
  0 1 21 1 "$d19_coordinate"

d19_prefix='(def main ((source Bytes)) '
d19_coordinate=$(printf '%s' "$d19_prefix" | wc -c | tr -d ' ')
schema_case epsilon-ordinary-unknown-type-precedes-schema 2 \
  "${d19_prefix}Missing source)" 0 1 11 1 "$d19_coordinate"
schema_case epsilon-unrelated-nominal-allowed 2 \
...
dcreq_text 1 '(def main ((value Int)) Int value)' | "$T/function-metadata.exe" > "$T/function-metadata.out"
...
dcreq_text 1 '(def main () Int (let x 1 x))' | "$T/local-profile.exe" > "$T/local-profile.out"
...
dcreq_text 1 "$resolver_metadata_source" | "$T/resolver-metadata.exe" > "$T/resolver-metadata.out"
...
MATCH_CASE=0
lower_match() { # Delta program  expected generated status  description
  MATCH_CASE=$((MATCH_CASE+1))
  dcreq_text 1 "$1" | "$T/function-lowering-emitter.exe" > "$T/lower-match-$MATCH_CASE.tape"
...
dcreq_text 1 "$match_repeat_source" | "$T/function-lowering-emitter.exe" > "$T/lower-match-repeat-a.tape"
match_repeat_a_status=$?
dcreq_text 1 "$match_repeat_source" | "$T/function-lowering-emitter.exe" > "$T/lower-match-repeat-b.tape"
...
dcreq_text 1 "$match_metadata_source" | "$T/match-metadata.exe" > "$T/match-metadata.out"
...
for frame_mode in e f; do
  ...
  if [ "$frame_status" = 7 ] && [ ! -s "$T/frame-$frame_mode.out" ]; then
    ...
    FAIL=$((FAIL+1))
    echo "  FAIL Delta frame ABI $frame_mode: status $frame_status, output $(wc -c < "$T/frame-$frame_mode.out" | tr -d ' ') bytes"
  fi
done
for constructor_mode in v m r; do
  ...
  if [ "$constructor_status" = 7 ] && [ ! -s "$T/constructor-$constructor_mode.out" ]; then
    ...
    FAIL=$((FAIL+1))
    echo "  FAIL Delta constructor ABI $constructor_mode: status $constructor_status, output $(wc -c < "$T/constructor-$constructor_mode.out" | tr -d ' ') bytes"
  fi
done
...
for sealed_input_mode in e b x z o h H i; do
  ...
  if [ "$sealed_input_status" = "$sealed_input_expected" ] &&
    ...
    FAIL=$((FAIL+1))
    echo "  FAIL Delta sealed input $sealed_input_mode: status $sealed_input_status, output $(wc -c < "$T/sealed-input-$sealed_input_mode.out" | tr -d ' ') bytes"
  fi
done
if [ "$sealed_input_repeat_status" = 1 ] &&
  ...
  FAIL=$((FAIL+1))
  echo "  FAIL Delta sealed input deterministic reconstruction: status $sealed_input_repeat_status"
fi
...
LOWER_CASE=0
lower_int() { # Delta program  expected generated status  description
  LOWER_CASE=$((LOWER_CASE+1))
  dcreq_text 1 "$1" | "$T/function-lowering-emitter.exe" > "$T/lower-$LOWER_CASE.tape"
...
whole_function() { # source expected-status description
  ...
  dcreq_text 1 "$1" | "$T/function-lowering-emitter.exe" > "$T/whole-function-$WHOLE_FUNCTION_CASE.tape"
...
dcreq_text 1 "$deterministic_source" | "$T/function-lowering-emitter.exe" > "$T/lower-deterministic-a.tape"
deterministic_a_status=$?
dcreq_text 1 "$deterministic_source" | "$T/function-lowering-emitter.exe" > "$T/lower-deterministic-b.tape"
...
dcreq_text 1 "$bytes_deterministic_source" | "$T/function-lowering-emitter.exe" > "$T/lower-bytes-deterministic-a.tape"
bytes_deterministic_a_status=$?
dcreq_text 1 "$bytes_deterministic_source" | "$T/function-lowering-emitter.exe" > "$T/lower-bytes-deterministic-b.tape"
...
reject_at_parts() { # exact prefix before later conflict  source suffix  desc
  ...
  dcreq_text 1 "$conflict_prefix$conflict_suffix" | "$T/tc.exe"; got=$?
...
reject_source() { # name source-file
  ...
  source_file=$2
  set +e
  dcreq_file 1 "$source_file" | "$T/tc.exe" > "$T/$name.out"
  got=$?
...
dcreq_expected() { # status profile kind code space coordinate limit requested source-length
  dcreq_emit_byte "$1"
  dcreq_emit_byte "$2"
  dcreq_emit_byte "$3"
  dcreq_emit_byte "$4"
...
check_private_outcome() { # probe name request-file status profile kind code space coordinate limit requested source-length
  ...
  dcreq_expected "$@" > "$T/$private_stem.expected"
  set +e
...
gcout_case() { # name source reason coordinate
  ...
  dcreq_text 1 "$gcout_case_source" > "$T/gcout-$gcout_case_name.req"
...
frontend_resource_case() { # name resource limit requested
  ...
  dcreq_emit_byte "$2" > "$T/frontend-resource-$1.req"
...
emitter_outcome_case() { # name private-code kind public-code space limit requested
  dcreq_emit_byte "$2" > "$T/emitter-outcome-$1.req"
...
dcreq_valid_source='(def main () Int 0)'
dcreq_valid_length=$(printf '%s' "$dcreq_valid_source" | wc -c | tr -d ' ')
dcreq_text 1 "$dcreq_valid_source" > "$T/gcreq-valid-profile-1.req"
check_gcreq valid-profile-1 "$T/gcreq-valid-profile-1.req" 0 1 0 0 0 0 0 0 "$dcreq_valid_length"
dcreq_text 2 '' > "$T/gcreq-valid-profile-2.req"
...
dcreq_text 1 '' > "$T/gcreq-wrong-magic.req"
...
dcreq_header 3 4294967295 > "$T/gcreq-unknown-profile.req"
check_gcreq unknown-profile "$T/gcreq-unknown-profile.req" 1 0 1 2 4 8 0 0 0
dcreq_header 1 4194305 > "$T/gcreq-source-adjacent.req"
...
unset dcreq_valid_source dcreq_valid_length
...
# Ordinary Delta rejection codes 4..18 retain one exact source coordinate.
gcout_source='junk'
...
printf '; hidden\000\n(def id ((x Int)) Int x)' > "$T/comment-nul.delta"
reject_source comment-nul "$T/comment-nul.delta"
printf '(def\013id ((x Int)) Int x)' > "$T/vertical-tab.delta"
reject_source vertical-tab "$T/vertical-tab.delta"
printf '; hidden\177\n(def id ((x Int)) Int x)' > "$T/comment-del.delta"
reject_source comment-del "$T/comment-del.delta"
printf '; hidden\303\251\n(def id ((x Int)) Int x)' > "$T/comment-high.delta"
...
printf '(def first () Int (later))\n;' > "$T/table-disjoint.delta"
table_prefix_size=$(wc -c < "$T/table-disjoint.delta" | tr -d ' ')
table_pad_size=$((2097151 - table_prefix_size))
dd if=/dev/zero bs="$table_pad_size" count=1 2>/dev/null | tr '\000' 'x' >> "$T/table-disjoint.delta"
printf '\n(def later () Int 7)' >> "$T/table-disjoint.delta"
accept_source table-disjoint "$T/table-disjoint.delta"
unset table_prefix_size table_pad_size
# Cross the retired interpreter's 512-value scratch bound. Delta arity is a
...
accept_source wide-call "$T/wide-call.delta"
awk 'BEGIN {
  printf "(data Wide (Mk"
  ...
  print ") x599)))"
}' > "$T/wide-constructor.delta"
accept_source wide-constructor "$T/wide-constructor.delta"
accept_source epsilon-compiler-foundation "$OMEGA_REPO_ROOT/source/epsilon/compiler/epsilon_compiler.delta"
awk 'BEGIN {
  for (i = 0; i <= 32768; i++) printf "(def f%d () Int 0)\n", i
}' > "$T/function-capacity.delta"
reject_source function-capacity "$T/function-capacity.delta"
awk 'BEGIN {
  printf "(def f ((x Int)) Int x) (def main () Int (f"
  for (i = 0; i < 300000; i++) printf " 0"
  print "))"
}' > "$T/arena-capacity.delta"
reject_source arena-capacity "$T/arena-capacity.delta"
awk 'BEGIN {
  printf "(def main () Int "
  ...
  print ")"
}' > "$T/nesting-within-profile.delta"
accept_source nesting-within-profile "$T/nesting-within-profile.delta"
awk 'BEGIN {
  printf "(def main () Int "
  ...
  print ")"
}' > "$T/nesting-exhausted.delta"
reject_source nesting-exhausted "$T/nesting-exhausted.delta"
...
echo "delta compiler substrate: $PASS passed, $FAIL failed"
[ "$FAIL" = 0 ] || exit 1
