...
[`LANGUAGE.md`](LANGUAGE.md) is OWNER Q4 decision material, not yet a normative Delta
v1 contract. It deliberately cannot authorize implementation while its open
choices remain contradictory. After OWNER Q4 selects one coherent contract, a
...
Q4-frozen contract and owned by the real compiler edge instead of restoring
that corpus.
...
The root retains only the OWNER Q4 contract draft, its feature/change ledger, and
this owner map. Proposed programs without a runner are not retained as tests;
the draft is replaced atomically by the normative contract after OWNER Q4.
