# `source/omega/` — Omega-written product compiler source
...
`source/proof-kernel/implementations/beta/check.beta` and
`source/proof-kernel/implementations/gamma/checker.gamma` are
...
See [`source/proof-kernel/README.md`](../proof-kernel/README.md) and
...
`source/on-ramp/rust/{psi,omega}/` while that hosted path matures.
`source/on-ramp/omega-bootstrap/` owns the Rust-free meaning, current Delta-written compiler
slices/profiles, and validation gates. The sibling roots
`source/{psi,omega}/` own the Omega-written product source. The first coherent
checkpoint now implements Psi source-to-token processing under `source/psi/`;
`source/omega/` and later Psi phases remain open. Exact checkpoint closures
and provisional `Ωself` censuses live in `source/omega/source-checkpoints/`.
...
sh source/beta/selfhost.sh
sh source/gamma/test-interp.sh
sh source/gamma/test-typeck.sh
sh source/proof-kernel/gates/test.sh
```
