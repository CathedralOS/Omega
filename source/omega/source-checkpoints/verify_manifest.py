...
BUILD_PRELUDE_OWNER = (
    "source/on-ramp/rust/omega/orchestration/omega-compiler/"
    "src/pipeline/stages.rs"
)
BUILD_PRELUDE_SNAPSHOT = "source/omega/source-checkpoints/inputs/build-prelude.omg"
...
def mutation_teeth(manifest: dict, snapshots: dict[str, dict]) -> None:
    ...
    add(
        ...
        lambda value: value["dependency_edges"][0].__setitem__(
            "to", "source/psi/lex/lexer.omg"
        ),
    )
    add(
        "bogus alias root",
        lambda value: value["dependency_aliases"][0].__setitem__("root", "source/omega"),
    )
    add(
        "external checksum",
