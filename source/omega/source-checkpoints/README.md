...
`source/on-ramp/rust/omega/orchestration/omega-compiler/src/pipeline/stages.rs`.
...
Run `source/omega/source-checkpoints/verify.sh` for the fast gate: it composes the
...
profile mutation teeth. It is part of `tools/bootstrap/verify-lattice.sh`.

Run `source/omega/source-checkpoints/checkpoint-000001.sh` for the complete product
