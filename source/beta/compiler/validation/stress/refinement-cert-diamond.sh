...
for c in "$T"/certs/cert-*.beta; do
  ...
  got=$(python3 "${OMEGA_PATH_ALPHA_CHECKER}"/implementations/reference/check_ref.py < "$c" 2>/dev/null || echo error)
  ...
  gexpr=$(python3 "${OMEGA_PATH_ALPHA_CHECKER}"/tools/refcert_to_gamma.py < "$c" 2>/dev/null) || { GFAIL=$((GFAIL+1)); echo "  FAIL $(basename "$c") : untranslatable to checker.gamma"; continue; }
