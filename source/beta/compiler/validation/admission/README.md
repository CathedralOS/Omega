...
| `encoding/` | Alpha-written exact two-pass source/tape reconstruction with positive and negative subject mutations. | Adapt into the artifact-aware checked ledger, then delete this status-only form when the rooted certificate absorbs it; delete earlier if adaptation would retain a second assembly source of truth. |
