# Beta evaluator profile v1

This document fixes the first directly audited Alpha implementation of Beta.
It defines request framing, observable implementation outcomes, resource
bounds, and private representation constraints. It does not extend the Beta
language defined by [`LANGUAGE.md`](LANGUAGE.md).
...
```text
0..7    [42 45 54 41 52 45 51 01]  (`BETAREQ`, version 1)
8..11   Beta source length, little-endian u32
12..15  sealed input length, little-endian u32
16..    exact Beta source bytes, then exact sealed input bytes, then EOF
```
...
0x00100000..0x00900000  BETAREQ source plus input, 8 MiB
...
tools may provide richer diagnostics without entering Beta meaning or artifact
authority.
...
frame, value-arena, or output extent yields status 3 and makes no Beta judgment.
...
status. This status check does not interpret Beta values or compiler
...
are not Beta values or source mutation.

Alpha Tape Assembly may reconstruct the evaluator tape for review and
reproducibility. That reconstruction is optional tooling; the exact committed
tape remains directly admitted and owes instruction-level audit against Alpha
semantics, this profile, and the Beta language relation.