...
DELTA_ARCH=aarch64 "$OMEGA_PATH_DELTA_RUST/target/debug/delta" \
  "$OMEGA_PATH_OMEGA_BOOTSTRAP/source/omega-bootstrap-frontend.alp" "$T/native-frontend" >/dev/null
DELTA_ARCH=aarch64 "$OMEGA_PATH_DELTA_RUST/target/debug/delta" \
  "$OMEGA_PATH_OMEGA_BOOTSTRAP/source/omega-bootstrap-terminal-to-elf.alp" "$T/native-backend" >/dev/null
...
compile_through_lowermachine \
  "$OMEGA_PATH_OMEGA_BOOTSTRAP/source/omega-bootstrap-frontend.alp" "$T/frontend"
compile_through_lowermachine \
  "$OMEGA_PATH_OMEGA_BOOTSTRAP/source/omega-bootstrap-terminal-to-elf.alp" "$T/backend"
...
bundle_one() {
  python3 "$OMEGA_PATH_OMEGA_BOOTSTRAP/source/omega_bootstrap_bundle.py" \
    pack main.omg="$1" > "$2"
}
