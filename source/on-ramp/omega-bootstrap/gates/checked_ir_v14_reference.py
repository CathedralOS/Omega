#!/usr/bin/env python3
"""Independent CKIR14 full-width trapping arithmetic reference."""

from __future__ import annotations

import argparse
import struct
from pathlib import Path

import checked_ir_v2_reference as v2
import checked_ir_v3_reference as v3
import checked_ir_v4_reference as v4
import checked_ir_v5_reference as v5


Ckir14Error = v5.Ckir5Error
Ckir14ResourceError = v5.Ckir5ResourceError
Module = v5.Module
HEADER = v5.HEADER
ROWS = v5.ROWS
TABLE_ORDER = v5.TABLE_ORDER
COUNT_NAMES = v5.COUNT_NAMES
NO_ID = v5.NO_ID
interpret = v5.interpret


def decode(contents: bytes) -> Module:
    return v5.decode(
        contents,
        expected_major=14,
        capabilities=v5.SCHEMA_CAPABILITIES[14],
    )


def selected_counts(module: Module) -> dict[int, int]:
    types = module.tables["types"]
    operands = module.tables["operands"]
    values = module.value_types
    counts = {opcode: 0 for opcode in (8, 26, 27)}
    for operation in module.tables["operations"]:
        opcode = operation[3]
        if opcode not in counts:
            continue
        result_type = operation[7]
        start = operation[8]
        if (types[result_type][1:] == (2, 1, 0, 0, 0, 0, 0xFFFF_FFFF)
                and values[operands[start][0]] == result_type
                and values[operands[start + 1][0]] == result_type):
            counts[opcode] += 1
    return counts


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("command", choices=("validate", "run"))
    parser.add_argument("ckir", type=Path)
    args = parser.parse_args()
    module = decode(args.ckir.read_bytes())
    if args.command == "validate":
        counts = selected_counts(module)
        print(
            "CKIR14 valid: "
            f"{counts[8]} full-u32 Add, {counts[26]} Subtract, "
            f"{counts[27]} Multiply operations"
        )
    else:
        result = interpret(module)
        print("library" if result is None else result)


if __name__ == "__main__":
    try:
        main()
    except Ckir14ResourceError as error:
        print(f"checked IR v14 reference: {error}", file=__import__("sys").stderr)
        raise SystemExit(252)
    except (Ckir14Error, v4.Ckir4Error, v3.Ckir3Error, v2.CkirError,
            OSError, struct.error) as error:
        print(f"checked IR v14 reference: {error}", file=__import__("sys").stderr)
        raise SystemExit(251)
