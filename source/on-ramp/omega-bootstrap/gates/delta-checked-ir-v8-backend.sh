#!/usr/bin/env sh
# CKIR8 ScalarEqual backend gate. CKIR4-7 parity remains adjacent.
set -eu
...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "checked-IR-v8 backend: skipped (requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in cargo python3 clang codesign cmp; do
  command -v "$TOOL" >/dev/null 2>&1 || {
    echo "checked-IR-v8 backend: skipped ($TOOL absent)"; exit 0;
  }
done

BACKEND="$OMEGA_PATH_OMEGA_BOOTSTRAP_COMPILER/omega-bootstrap-checked-ir-v5-to-elf.alp"
FIXTURE="$GATE_DIR/delta-checked-ir-v8-backend-fixture.py"
REFERENCE="$GATE_DIR/checked_ir_v8_reference.py"
...
while IFS="$TAB" read -r NAME EXPECTED; do
  ...
  cmp "$T/$NAME.native.elf" "$T/$NAME.self.elf" || {
    echo "checked-IR-v8 backend: $NAME native/self artifact mismatch" >&2; exit 1;
  }
  python3 -B "$FIXTURE" check-artifact "$T/$NAME.native.elf"
done < "$T/cases/positives.tsv"

python3 - "$T/u32-equal.native.elf" "$T/bad-sete.elf" <<'PY'
from pathlib import Path
import sys
raw = bytearray(Path(sys.argv[1]).read_bytes())
at = raw.index(b"\x0f\x94\xc0")
raw[at + 1] = 0x95
Path(sys.argv[2]).write_bytes(raw)
PY
if python3 -B "$FIXTURE" check-artifact "$T/bad-sete.elf" >/dev/null 2>&1; then
  echo "checked-IR-v8 backend: mutated SETE accepted" >&2; exit 1
fi
...
while IFS="$TAB" read -r NAME EXPECTED; do
  ...
  for IMPLEMENTATION in native self; do
    set +e
    "$T/backend.$IMPLEMENTATION" < "$T/cases/$NAME.ckir8" > "$T/$NAME.$IMPLEMENTATION"
    ACTUAL=$?
    set -e
    [ "$ACTUAL" -eq "$EXPECTED" ] && [ ! -s "$T/$NAME.$IMPLEMENTATION" ] || {
      echo "checked-IR-v8 backend: $NAME/$IMPLEMENTATION failed" >&2; exit 1;
    }
  done
  set +e
  python3 -B "$REFERENCE" validate "$T/cases/$NAME.ckir8" \
    > "$T/$NAME.reference" 2> "$T/$NAME.reference.err"
  ACTUAL=$?
  set -e
  [ "$ACTUAL" -eq "$EXPECTED" ] && [ ! -s "$T/$NAME.reference" ] || {
    echo "checked-IR-v8 backend: $NAME/reference failed" >&2; exit 1;
  }
done < "$T/cases/manifest.tsv"

echo "checked-IR-v8 backend: native/self exact load/CMP/SETE/MOVZX/store artifacts, bool/u8/u32 rows, result 70; $POSITIVE positives and $COUNT controls passed"
