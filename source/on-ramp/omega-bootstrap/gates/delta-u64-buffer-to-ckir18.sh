#!/usr/bin/env sh
# Focused OMGRSWA10/OMGLOWJ19 native+self source-to-CKIR18 handoff.
set -eu
...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "OMGLOWJ19 focused producer: skipped (requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in cargo python3 cmp clang codesign; do
  command -v "$TOOL" >/dev/null 2>&1 || {
    echo "OMGLOWJ19 focused producer: skipped ($TOOL absent)"; exit 0;
  }
done
...
SOURCE_FIXTURE=$GATE_DIR/omgrsw10_u64_buffer_fixture.py
LOWERING_FIXTURE=$GATE_DIR/delta-u64-buffer-to-ckir18-fixture.py
CKIR_FIXTURE=$GATE_DIR/delta-checked-ir-v18-fixture.py
CKIR_REFERENCE=$GATE_DIR/checked_ir_v18_reference.py
T=$(mktemp -d)
trap 'rm -rf "$T"' EXIT
...
run_case() { # executable label name expected input output
  CASE_EXECUTABLE=$1 CASE_LABEL=$2 CASE_NAME=$3 CASE_EXPECTED=$4 CASE_INPUT=$5 CASE_OUTPUT=$6
  set +e
  "$CASE_EXECUTABLE" < "$CASE_INPUT" > "$CASE_OUTPUT"
  CASE_ACTUAL=$?
  set -e
  [ "$CASE_ACTUAL" -eq "$CASE_EXPECTED" ] || {
    echo "OMGLOWJ19 focused producer: $CASE_NAME/$CASE_LABEL status $CASE_ACTUAL expected $CASE_EXPECTED" >&2
    exit 1
  }
  if [ "$CASE_EXPECTED" -ne 0 ] && [ -s "$CASE_OUTPUT" ]; then
    echo "OMGLOWJ19 focused producer: $CASE_NAME/$CASE_LABEL rejection published bytes" >&2
    exit 1
  fi
}
...
while IFS="$(printf '\t')" read -r NAME INPUT; do
  for RLABEL in native self; do
    ...
    for LLABEL in native self; do
      ...
      OUT=$T/$NAME.$RLABEL.$LLABEL.ckir18
      run_case "$L" "$LLABEL" "$NAME-$RLABEL" 0 "$FRAME" "$OUT"
      cmp "$OUT" "$T/ckir/canonical.ckir18" >/dev/null
      python3 -B "$CKIR_REFERENCE" validate "$OUT" >/dev/null
      [ "$(python3 -B "$CKIR_REFERENCE" run "$OUT")" = 70 ]
    done
  done
  cmp "$T/$NAME.native.omgrswa" "$T/$NAME.self.omgrswa" >/dev/null
done < "$T/source/positives.tsv"

for RLABEL in native self; do
  case "$RLABEL" in native) R=$T/resolver.native ;; self) R=$T/resolver.self ;; esac
  while IFS="$(printf '\t')" read -r NAME EXPECTED INPUT; do
    run_case "$R" "$RLABEL" "$NAME" "$EXPECTED" "$INPUT" "$T/$NAME.$RLABEL.out"
  done < "$T/source/negatives.tsv"
done

python3 -B "$LOWERING_FIXTURE" cases \
  "$T/source/canonical.omgc" "$T/canonical.native.omgrswa" \
  "$T/source/renamed.omgc" "$T/lowering-cases"
for LLABEL in native self; do
  case "$LLABEL" in native) L=$T/lowerer.native ;; self) L=$T/lowerer.self ;; esac
  while IFS="$(printf '\t')" read -r NAME EXPECTED INPUT; do
    run_case "$L" "$LLABEL" "$NAME" "$EXPECTED" "$INPUT" "$T/$NAME.$LLABEL.out"
  done < "$T/lowering-cases/manifest.tsv"
done

echo "OMGLOWJ19 focused producer: native/self rename/reorder/inert, exact 3924-byte CKIR18, 251/252 controls PASS"
