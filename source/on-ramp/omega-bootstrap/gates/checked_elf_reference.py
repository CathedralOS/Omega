#!/usr/bin/env python3
"""Independent CKIR1-to-ELF reconstruction and exact artifact checker."""
...
class Reconstructor:
    def __init__(self, module: checked_ir_reference.Module) -> None:
        self.module = module
    ...
    def _selected_blocks(self) -> range:
        machine = self.machines[self.module.entry]
    ...
    def assign_frame(self) -> None:
        ...
        for value_id, type_id in enumerate(self.module.value_types):
            if self.value_machines[value_id] != entry:
                continue
        ...
        for place_id, block_id in enumerate(self.place_blocks):
            if self.blocks[block_id][1] != entry:
                continue
            cursor = align(cursor, 8) + 8
            require(cursor <= 262_144, "place frame exhaustion")
            self.place_slots[place_id] = cursor
        scratch_count = max(
            (max(self.terminators[block_id][9], self.terminators[block_id][12]) for block_id in self._selected_blocks()),
            default=0,
        )
        cursor = align(cursor, 8)
        self.scratch_base = cursor
        cursor += scratch_count * 8
        self.frame_size = align(cursor, 16)
        require(self.frame_size <= 262_144, "frame exhaustion")
    ...
    def emit_terminator(self, emitter: TextEmitter, block_id: int) -> None:
        ...
        if kind == 1:
            ...
            self.load_value(emitter, term[6])
            self.range_check(emitter, self.machines[self.module.entry][5])
            emitter.raw(b"\xc9\xc3")
        else:
    ...
    def emit_text(self, rx_size: int | None, block_offsets: dict[int, int] | None) -> tuple[bytes, dict[int, int]]:
        ...
        for block_id in self._selected_blocks():
...
def reconstruct(ckir: bytes) -> bytes:
    return Reconstructor(checked_ir_reference.decode(ckir)).reconstruct()
...
def main() -> None:
    ...
    print(
        f"CKIR1 ELF reconstructed: {len(actual)} bytes"
...
if __name__ == "__main__":
    try:
        main()
    except (ArtifactError, checked_ir_reference.CkirError, OSError) as error:
