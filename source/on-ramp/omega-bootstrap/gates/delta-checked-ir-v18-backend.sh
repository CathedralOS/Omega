...
case "$(uname -sm)" in
  "Darwin arm64") ;;
  *) echo "checked-IR-v18 backend: skipped (requires Darwin arm64)"; exit 0 ;;
esac
for TOOL in cargo python3 clang codesign cmp; do
  command -v "$TOOL" >/dev/null 2>&1 || {
    echo "checked-IR-v18 backend: skipped ($TOOL absent)"
    exit 0
  }
done

BACKEND="$OMEGA_PATH_OMEGA_BOOTSTRAP_COMPILER/omega-bootstrap-checked-ir-v18-to-elf.alp"
FIXTURE="$GATE_DIR/delta-checked-ir-v18-backend-fixture.py"
REFERENCE="$GATE_DIR/checked_ir_v18_reference.py"
...
while IFS="$TAB" read -r NAME STATUS; do
  for IMPL in native self; do
    python3 -B "$FIXTURE" run-filter \
      "$T/backend.$IMPL" "$T/cases/$NAME.ckir18" \
...
for FAMILY in index add less range; do
  ...
  if python3 -B "$FIXTURE" check-artifact \
      "$T/bad-$FAMILY.elf" "$T/cases/canonical.ckir18" >/dev/null 2>&1; then
    echo "checked-IR-v18 backend: mutated $FAMILY template accepted" >&2
    exit 1
  fi
done

sh "$GATE_DIR/delta-checked-ir-v18-reference.sh"
echo "checked-IR-v18 backend: PASS"
