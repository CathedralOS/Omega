...
fn layer_of(manifest_path: &str) -> Option<&'static str> {
    ...
    {
        Some("foundation")
    } else if m("/source/on-ramp/rust/omega/representations/")
        ...
        || m("/source/on-ramp/rust/psi/semantics/")
    {
        ...
        || m("/source/on-ramp/rust/omega/optimization/")
        || m("/source/on-ramp/rust/psi/pipeline/")
    {
        Some("pipeline")
    } else if m("/source/on-ramp/rust/omega/orchestration/") {
        Some("orchestration")
    } else if m("/source/on-ramp/rust/omega/backend/instruction_set_architectures/") {
        Some("isa")
    } else if m("/source/on-ramp/rust/omega/backend/images/") {
        Some("images")
    } else if m("/source/on-ramp/rust/omega/backend/object/") {
        Some("object")
    } else if m("/source/on-ramp/rust/omega/backend/") {
        Some("backend")
    } else {
...
/// `<root>/source/on-ramp/rust/omega/architecture/`.
...
fn frontend_implementation_is_psi_owned() {
    ...
    for relative in [
        "source/on-ramp/rust/omega/foundation/omega-core/src/arithmetic.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/arena/mod.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/atomic.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/bignum.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/byte_predicates.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/cast_form.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/const_value.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/content.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/diagnostics/mod.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/float_semantics.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/inline_assembly.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/literals.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/operator_spelling.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/semantics.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/source",
        "source/on-ramp/rust/omega/foundation/omega-core/src/span.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/symbols/mod.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/trust.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/value_domain.rs",
        "source/on-ramp/rust/omega/foundation/omega-core/src/wire.rs",
    ] {
        assert!(
...
fn retired_omega_frontend_adapters_do_not_return() {
    let root = workspace_root();
    for relative in [
        "source/on-ramp/rust/omega/pipeline/omega-source-files-to-tokens",
        "source/on-ramp/rust/omega/pipeline/omega-tokens-to-syntax-trees",
        "source/on-ramp/rust/omega/pipeline/omega-syntax-trees-to-symbol-resolved-trees",
        "source/on-ramp/rust/omega/pipeline/omega-symbol-resolved-trees-to-typed-trees",
        "source/on-ramp/rust/omega/pipeline/omega-typed-trees-to-checked-trees",
        "source/on-ramp/rust/omega/representations/omega-tokens",
        "source/on-ramp/rust/omega/representations/omega-symbol-resolved-trees",
        "source/on-ramp/rust/omega/representations/omega-syntax-trees",
        "source/on-ramp/rust/omega/representations/omega-typed-trees",
        "source/on-ramp/rust/omega/representations/omega-facts",
        "source/on-ramp/rust/omega/representations/omega-checked-trees",
        "source/on-ramp/rust/omega/semantics/omega-proof",
        "source/on-ramp/rust/omega/semantics/omega-types",
        "source/on-ramp/rust/omega/semantics/omega-validation",
    ] {
        assert!(
...
fn provider_approval_stays_in_omega_after_psi_checking() {
    let root = workspace_root();
    let psi_checks = root
        .join("source/on-ramp/rust/psi/pipeline/psi-typed-trees-to-checked-trees/src/checks.rs");
...
fn target_neutral_effect_inference_is_psi_owned() {
    let root = workspace_root();
    let legacy = root.join("source/on-ramp/rust/omega/representations/omega-effects/src/lib.rs");
...
fn checked_semantics_are_psi_owned_without_provider_realization() {
    let root = workspace_root();
    let manifest =
        root.join("source/on-ramp/rust/psi/representations/psi-checked-trees/Cargo.toml");
    ...
    let checked_root = root.join("source/on-ramp/rust/psi/representations/psi-checked-trees/src");
    ...
    let omega_provider_carrier = root.join(
        "source/on-ramp/rust/omega/representations/omega-effects/src/selected_provider_plans.rs",
    );
    assert!(
    ...
    let omega_task_carrier =
        root.join("source/on-ramp/rust/omega/foundation/omega-task-plans/src/lib.rs");
...
fn first_terminal_psi_source_slice_stays_fail_closed() {
    let root = workspace_root();
    let path =
        root.join("source/on-ramp/rust/psi/pipeline/psi-checked-trees-to-terminal/src/lib.rs");
    let source = std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
    ...
    let manifest_path =
        root.join("source/on-ramp/rust/psi/pipeline/psi-checked-trees-to-terminal/Cargo.toml");
    ...
    assert!(
        !root
            .join("source/on-ramp/rust/omega/pipeline/omega-checked-trees-to-terminal-psi")
            .exists(),
...
fn retained_native_product_enters_only_terminal_realization() {
    ...
    let compiler_path = root
        .join("source/on-ramp/rust/omega/orchestration/omega-compiler/src/pipeline/compiler.rs");
    ...
    let report_path = root.join(
        "source/on-ramp/rust/omega/orchestration/omega-compiler/src/pipeline/compile_report.rs",
    );
...
fn typed_frontend_does_not_retain_concrete_calling_conventions() {
    let root = workspace_root();
    let manifest = root.join("source/on-ramp/rust/psi/representations/psi-typed-trees/Cargo.toml");
    let manifest_source = std::fs::read_to_string(&manifest)
        .unwrap_or_else(|error| panic!("failed to read {}: {error}", manifest.display()));
    assert!(
    ...
    let representation =
        root.join("source/on-ramp/rust/psi/representations/psi-typed-trees/src/typed_trees.rs");
...
fn psi_reference_execution_ownership_and_terminal_lane_are_enforced() {
    let root = workspace_root();
    assert!(
        root.join("source/on-ramp/rust/psi/semantics/psi-terminal-interpreter/src/lib.rs")
            .is_file(),
        "canonical terminal-Psi reference execution must remain Psi-owned"
    );
    assert!(
        root.join("source/on-ramp/rust/psi/semantics/psi-checked-interpreter/src/lib.rs")
            .is_file(),
        "transitional checked-tree reference execution must remain Psi-owned"
    );
    assert!(
        !root
            .join("source/on-ramp/rust/omega/orchestration/omega-interpreter")
            .exists(),
        "the retired production Omega interpreter must not return"
    );
    assert!(
        root.join(
            "source/on-ramp/rust/omega/orchestration/omega-native-differential-test/src/lib.rs"
        )
        .is_file(),
...
fn optimizer_register_models_remain_on_the_clean_terminal_isa_lane() {
    let root = workspace_root();
    let isa_root = root.join("source/on-ramp/rust/omega/backend/instruction_set_architectures");
    let model_source =
        root.join("source/on-ramp/rust/omega/representations/omega-register-model/src/lib.rs");
    assert!(
        model_source.is_file(),
        "the canonical register-model vocabulary must remain representation-owned"
    );
    let facade_source =
        root.join("source/on-ramp/rust/omega/optimization/omega-regalloc/src/lib.rs");
    ...
    let regalloc_manifest =
        root.join("source/on-ramp/rust/omega/optimization/omega-regalloc/Cargo.toml");
    ...
    let selection_manifest = root.join(
        "source/on-ramp/rust/omega/pipeline/omega-terminal-target-operations-to-selected-instructions/Cargo.toml",
    );
