...
# diaf DESC FILE "in-bytes" : a REAL program from the canonical Delta corpus. Checks native stdout == the gamma
...
diax "non-final state falloff" nonfinal-state-falloff.alp 42
...
# REAL PROGRAMS — actual certifiers from the Delta corpus (read stdin, compute, emit a proof certificate via
