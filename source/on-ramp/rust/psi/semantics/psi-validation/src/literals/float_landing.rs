    ...
    match declared_node {
        ...
        TypeReferenceNode::Named { .. } => {
            let format = match program.primitive_type_reference(declared) {
                Some(PrimitiveType::F32) => psi_numerics::literals::FloatFormat::F32,
                Some(PrimitiveType::F64) => psi_numerics::literals::FloatFormat::F64,
                _ => return,
            };
            land_float_tree(program, value, format);
        }
...
fn land_float_tree(
    ...
    if let Some(exact) = anonymous_exact_float_tree(program, expression) {
        ...
        let value =
            psi_numerics::float_semantics::FloatSemantics::round_exact(semantic_format, exact)
                .to_interpreter_value(semantic_format);
        *program.expression_table.expression_mut(expression) = ExpressionNode::Float(
            psi_numerics::literals::FloatLiteral::from_f64(value).with_landing(format),
        );
    } else {
...
fn anonymous_exact_float_tree(
    program: &TypedTrees,
    expression: ExpressionHandle,
) -> Option<psi_numerics::bignum::ExactFloat> {
