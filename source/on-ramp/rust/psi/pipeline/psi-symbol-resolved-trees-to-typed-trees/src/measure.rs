...
pub(crate) fn lower_measure_definition(
    lowerer: &mut Lowerer,
    measure: &psi_symbol_resolved_trees::measure::MeasureDefinition,
) -> Result<psi_typed_trees::measure::MeasureDefinition, Diagnostic> {
    ...
    let mut typed_measure = psi_typed_trees::measure::MeasureDefinition {
...
fn lower_measure_parameter(
    lowerer: &mut Lowerer,
    parameter: &psi_symbol_resolved_trees::signature::StateParameter,
) -> Result<psi_typed_trees::signature::StateParameter, Diagnostic> {
    let type_reference = lower_type_reference_into_table(lowerer, &parameter.type_reference)?;
    Ok(psi_typed_trees::signature::StateParameter {
