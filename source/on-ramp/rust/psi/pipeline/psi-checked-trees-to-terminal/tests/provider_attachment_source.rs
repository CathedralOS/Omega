...
fn fixture_bytes() -> Vec<u8> {
    let hex = include_str!(
        "../../../../../../../source/on-ramp/omega-bootstrap/gates/fixtures/omega-bootstrap-terminal-v28.hex"
    );
    let digits = hex
        .bytes()
    ...
    // the Delta on-ramp. This source-owner canary upgrades its one trailing
    // machine contract with the current empty outcome-specific carrier while
    // leaving the independently consumed bootstrap bytes untouched.
    bytes[8..10].copy_from_slice(&31_u16.to_le_bytes());
    bytes[10..12].copy_from_slice(&33_u16.to_le_bytes());
...
fn source_projection_is_the_shared_o0_fixture_and_perturbations_fail_closed() {
    ...
    assert_eq!(u16::from_le_bytes([canonical[8], canonical[9]]), 31);
    assert_eq!(u16::from_le_bytes([canonical[10], canonical[11]]), 33);
