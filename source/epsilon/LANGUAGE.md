# Epsilon v1 language contract

Epsilon is the closed compiler-host language used to author the first full Omega
...
Epsilon v1 is intentionally small. It has finite nominal data, fixed storage,
...
paths are diagnostic locations only; they are not Epsilon names or namespaces.

Epsilon has two judgments:
...
bounded evaluator, not Epsilon program results. They grant no Epsilon judgment and
publish no partial artifact. Epsilon v1 has no implicit allocating operation and
...
Epsilon v1 permits no active local shadowing. Machine parameters are active
...
Sum constructors are ordinary values. Epsilon v1 has no record-literal form:
...
recursive records, sums, arrays, or payloads reject. Epsilon has no heap or
...
Epsilon has no ambient heap, pointer arithmetic, filesystem, environment, clock,
...
`-2147483648..2147483647`. Epsilon defines:
...
Epsilon v1 does not require a termination annotation.
...
Epsilon rejection chosen by reason-code order. Every relation consuming a call's
...
`EpsilonRejectReason` is this closed nominal set:
...
Epsilon distinguishes three kinds of bounds.
...
complete checked reason-code bijection. A mismatch rejects through `DCOUT`
...
`source/delta/compiler/ecout-v1.tsv`, a checked projection of constants embedded
in the Delta-compiler artifact rather than a host runtime input. A returned
...
Gamma translator, Epsilon evaluator, Beta assembler, host compiler, or other
...
Future syntax or semantics enter Epsilon only through an explicit contract
