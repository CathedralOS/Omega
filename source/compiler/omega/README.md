...
lives at `bootstrap/alpha/assembler/`. Beta proper is the language
compiled by `bootstrap/beta/bc.beta`.
The disposable Rust producer of Alpha VM tapes lives at
`bootstrap/alpha/assembler-rust/` and has no Beta-language role.
...
live under `bootstrap/delta/`; its disposable Rust producer lives under
`bootstrap/delta/rust/`.

## Proof kernel

The proof kernel is a cross-cutting assurance service, not a language rung.
`source/assurance/proof-kernel/implementations/beta/check.beta` and
`source/assurance/proof-kernel/implementations/gamma/checker.gamma` are
...
`source/compiler/omega/{psi,omega}/` own the Omega-written product source. The first coherent
checkpoint now implements Psi source-to-token processing under `source/compiler/omega/psi/`;
`source/compiler/omega/omega/` and later Psi phases remain open. Exact checkpoint closures
and provisional `Ωself` censuses live in `source/compiler/omega/source-checkpoints/`.
