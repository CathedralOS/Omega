        ...
        for state in program.machine_states(machine) {
            ...
            for root in &roots {
                if !is_supported_checked_referee(program, root.referee) {
                    diagnostics.push(Diagnostic::error(format!(
                        "machine `{}` state `{}` parameter `{}` uses `&write` with `{}`; the current checked slice supports unrestricted primitive scalars, literal fixed arrays whose elements are unrestricted primitive scalars or eligible material `[copy]` records or sums, forwarding-only byte slices, non-generic invariant-free checked records, and closed material `[copy]` sums as atomic whole values",
                        machine.name,
                        state.name,
                        root.name,
...
/// runtime occurrences are all material. Arrays and generic/qualified shells
/// do not enter through this judgment, and aggregate elements remain atomic.
...
fn diagnose_unsupported_write_only_assignment_target(
    ...
    diagnostics.push(Diagnostic::error(format!(
        "machine `{machine}` state `{state}` writes through an unsupported write-only projection; accepted partial stores are a content-independent common-field path through non-generic invariant-free records when every field is relevant and unconstrained and the displaced leaf is an unrestricted primitive, a whole eligible unrestricted record or closed material `[copy]` sum, or a literal fixed array whose elements are unrestricted primitive scalars or eligible material `[copy]` records or sums, a proven-in-bounds element or statically normalized closed range of such a fixed array, or a proven-in-bounds element of a direct byte slice; sum case/payload projection, qualified, invariant-dependent, symbolic or open range, take, swap, and read-modify-write operations remain rejected"
    )));
}
