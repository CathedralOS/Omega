...
pub(crate) struct SourceLeaf {
    ...
    pub constant_fuel: Vec<FuelSettlement>,
...
fn derive_source_function(
    ...
    if target.machine != abstracted.machine
        ...
        || abstracted.operations.len() != 5
        || abstracted.block_entries[0].operation_offset != 0
        || abstracted.block_entries[1].operation_offset != 1
        || abstracted.block_entries[2].operation_offset != 3
        ...
        || optimized.blocks[1].nodes.len() != 2
        || optimized.blocks[2].nodes.len() != 2
        || abstracted
            .block_entries
            .iter()
    ...
    let when_false = derive_leaf(
        ...
        &abstracted.operations[3..5],
        &optimized.blocks[2].nodes,
    )?;
    let expected_provenance = TerminalPsiProvenance {
        operations: vec![when_true.constant_operation, when_false.constant_operation],
        edges: vec![
...
fn derive_leaf(
    ...
    if nodes.len() != 2
        || abstract_operations.len() != 2
        || nodes[0].operation != abstract_operations[0]
        || nodes[1].operation != abstract_operations[1]
    {
        return Err(Error::UnsupportedSourceShape { function });
    }
    ...
    if source_value != expression_source {
        return Err(Error::UnsupportedSourceShape { function });
    }
    ...
    if *result != *source_value
        ...
        || *scalar_type
            != ScalarType::Integer(
                psi_core::IntegerType::new(IntegerSign::Unsigned, 64).expect("u64"),
            )
    ...
    if *psi_edge != *psi_return_edge
        ...
        || nodes[1].provenance != vec![PsiProvenance::Edge(*psi_return_edge)]
    {
        return Err(Error::UnsupportedSourceShape { function });
    }
    let return_fuel = exact_edge_fuel(&nodes[1], *psi_return_edge, function)?;
    if nodes[1].fuel.len() != return_fuel.len() {
        return Err(Error::UnsupportedSourceShape { function });
    }
    Ok(SourceLeaf {
