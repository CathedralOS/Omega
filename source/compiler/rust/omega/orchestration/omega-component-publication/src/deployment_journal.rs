...
pub struct ComponentDeploymentEraOccurrence {
    ...
    terminal_artifact_identity: u64,
}
...
impl ComponentDeploymentEraOccurrence {
    pub fn new(
        ...
        if era_identity == 0 || installed_code_identity == 0 || terminal_artifact_identity == 0 {
    ...
    pub const fn terminal_artifact_identity(self) -> u64 {
...
pub struct ComponentDeploymentJournalRecord {
    ...
    terminal_installation_fingerprint: [u8; 32],
    terminal_installation_record: Vec<u8>,
}
...
impl ComponentDeploymentJournalRecord {
    ...
    pub const fn terminal_installation_fingerprint(&self) -> &[u8; 32] {
        &self.terminal_installation_fingerprint
    }
    pub fn terminal_installation_record(&self) -> &[u8] {
        &self.terminal_installation_record
    }
}
...
pub fn prepare_component_deployment(
    ...
    let terminal_installation_record =
        match encode_terminal_installation_record(runnable.terminal_artifact().installation()) {
            Ok(value) => value,
            Err(error) => {
                return Err(preparation_error(
                    candidate,
    ...
    let fingerprint =
        match terminal_installation_fingerprint(runnable.terminal_artifact().installation()) {
            Ok(value) => value,
            Err(error) => {
                return Err(preparation_error(
                    candidate,
    ...
    let record = ComponentDeploymentJournalRecord {
        ...
        entry_plan_admission_receipt_identity: candidate
            ...
            .clone(),
        acceptance,
        terminal_installation_fingerprint: *fingerprint.as_bytes(),
        terminal_installation_record,
    };
...
impl ActivatedComponentDeploymentJournal {
    ...
    pub fn finalize(
        ...
        if retained.installed_code().normalized_identity()
            ...
            || retained.artifact().normalized_identity()
                != self.record.candidate.terminal_artifact_identity
        {
            return Err(ComponentDeploymentFinalizationError::new(
                self,
...
pub fn encode_component_deployment_journal(
    ...
    out.extend_from_slice(&record.terminal_installation_fingerprint);
    put_bytes(&mut out, &record.terminal_installation_record)?;
...
pub fn decode_component_deployment_journal(
    ...
    let mut terminal_installation_fingerprint = [0; 32];
    terminal_installation_fingerprint.copy_from_slice(cursor.take(32)?);
    let terminal_installation_record = cursor.bytes()?;
...
fn validate_record(
    ...
    let decoded = decode_terminal_installation_record(&record.terminal_installation_record)
        .map_err(|error| {
            ComponentDeploymentJournalError::new(format!(
                "deployment journal installation evidence does not decode: {error}"
            ))
        })?;
    let canonical = encode_terminal_installation_record(&decoded).map_err(|error| {
        ComponentDeploymentJournalError::new(format!(
            "deployment journal installation evidence cannot re-encode: {error}"
        ))
    })?;
    if canonical != record.terminal_installation_record {
        return Err(ComponentDeploymentJournalError::new(
            "deployment journal installation evidence is not canonical",
        ));
    }
    let fingerprint = terminal_installation_fingerprint(&decoded).map_err(|error| {
        ComponentDeploymentJournalError::new(format!(
            "deployment journal installation fingerprint cannot replay: {error}"
        ))
    })?;
    if fingerprint.as_bytes() != &record.terminal_installation_fingerprint {
        return Err(ComponentDeploymentJournalError::new(
...
fn put_occurrence(out: &mut Vec<u8>, value: ComponentDeploymentEraOccurrence) {
    ...
    put_u64(out, value.terminal_artifact_identity);
}
