...
| `src/conflict.rs` | Retain | Root policy can accept or reject every exact blocking fingerprint in one canonical candidate-bound decision set. Empty, incomplete, duplicate, stale/foreign, wrong-candidate, and non-blocking decisions reject. Its bounded fixed-vocabulary text record recovers only by matching current compiler conflicts, rerunning complete resolution, and requiring canonical re-encoding. The result cannot prove review, mint evidence, or authorize admission; policy-origin custody and accepted-lock promotion remain unfinished. |
...
| removed `src/diff.rs` | Deleted | Rust-`Debug` section diffs were removed. Production review uses bounded exact-row `capability_conflict.rs` plus candidate-bound canonically recoverable decisions; policy-origin custody remains new work, not a reason to retain the old comparison model. |
