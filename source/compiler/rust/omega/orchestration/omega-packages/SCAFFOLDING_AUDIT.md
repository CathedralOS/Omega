...
| `src/source_closure_subject.rs` | Retain | Projects exact validated root/dependency requests and every immutable selection into one bounded canonical, strictly recovered source-closure question. Reconstruction revalidates aliases, authored ordinals, lineage/resolution joins, and complete graph closure. Cache paths, resolver observations, compiler evidence, artifacts, certificates, decisions, and open obligations remain separate; decode and the domain-separated fingerprint confer no authority and there is no accepted-lock or `PackageInstance` promotion API. |
