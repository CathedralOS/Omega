...
use omega_terminal_target_operations::{
    ...
    TerminalTargetScalarExpression,
};
...
fn conditional_condition_division_stack_facts_survive_object_image_and_installation() {
    for target in [NativeTarget::linux_x64(), NativeTarget::linux_arm64()] {
        ...
        let object = emit_terminal_object_container(&artifact);
        assert_eq!(object.output.relocations, 0);
        let image = emit_terminal_executable_image(&artifact, 13)
...
fn conditional_condition_division_plan(target: NativeTarget) -> TerminalTargetOperationPlan {
    ...
    let arm = |edge, return_edge, source, value| TerminalTargetConditionalIntegerArm {
        psi_edge: edge_id(edge),
        control: Box::new(TerminalTargetIntegerControl::Return {
            psi_return_edge: edge_id(return_edge),
            source_value: value_id(source),
