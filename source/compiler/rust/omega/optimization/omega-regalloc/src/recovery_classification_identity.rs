...
use omega_terminal_selected_instructions::{
    TerminalSelectedInstructionProvenance, TerminalVirtualRegisterOrigin,
};
use psi_core::{IntegerSign, IntegerValue, ScalarType};
use sha2::{Digest, Sha256};

use crate::{
    TerminalNoAdmittedRecoveryReason, TerminalRecoveryClassification,
    TerminalRecoveryClassificationIdentity, TerminalRecoveryClassificationPlan,
    TerminalRecoveryClassificationPolicy, TerminalRecoveryVictimRole,
};

pub fn terminal_recovery_classification_identity(
...
pub(crate) fn encode_terminal_recovery_classification_content(
    ...
    bytes.push(match plan.policy {
        TerminalRecoveryClassificationPolicy::SelectedVictimImmediateU64EligibilityV1 => 0,
    });
    ...
    for function in &plan.functions {
        ...
        if let Some(row) = &function.classification {
            ...
            match row.role {
                TerminalRecoveryVictimRole::Incoming => bytes.push(0),
            ...
            match &row.classification {
                ...
                TerminalRecoveryClassification::NoAdmittedRecovery { reason } => {
                    bytes.push(1);
                    bytes.push(match reason {
                        TerminalNoAdmittedRecoveryReason::UnsupportedScalarType => 0,
                        TerminalNoAdmittedRecoveryReason::EntryParameter => 1,
                        TerminalNoAdmittedRecoveryReason::UnsupportedRangeShape => 2,
                        TerminalNoAdmittedRecoveryReason::FutureFixedUse => 3,
                        TerminalNoAdmittedRecoveryReason::NonMaterializeDefinition => 4,
                        TerminalNoAdmittedRecoveryReason::ProofBearingDefinition => 5,
                        TerminalNoAdmittedRecoveryReason::NoFutureUse => 6,
                    });
                }
            }
        }
...
fn encode_origin(bytes: &mut Vec<u8>, origin: TerminalVirtualRegisterOrigin) {
    match origin {
...
fn encode_provenance(bytes: &mut Vec<u8>, provenance: &TerminalSelectedInstructionProvenance) {
...
mod tests {
    ...
    use super::*;
    use crate::{
        TerminalAllocationLegalityIdentity, TerminalFunctionRecoveryClassification,
        TerminalLiveRangeIdentity, TerminalLiveRangePoint, TerminalPressureRecoveryClassification,
        TerminalRecoveryClassificationDecodeError, TerminalRecoveryFutureUse,
        TerminalSpillChoiceIdentity,
    };

    fn plan() -> TerminalRecoveryClassificationPlan {
        ...
        TerminalRecoveryClassificationPlan {
            ...
            functions: vec![TerminalFunctionRecoveryClassification {
                ...
                classification: Some(TerminalPressureRecoveryClassification {
                    ...
                    classification:
                        TerminalRecoveryClassification::ImmediateU64RematerializationCandidate {
                            defining_instruction: TerminalSelectedInstructionId(7),
                            source_value: value,
                            ...
                            future_uses: vec![TerminalRecoveryFutureUse {
                                block: TerminalSelectedBlockId(1),
                                point: TerminalLiveRangePoint(14),
                                instruction: TerminalSelectedInstructionId(15),
                                operand: 0,
                            }],
                        },
                }),
    ...
    fn identity_is_deterministic_and_binds_roots_and_nested_classification() {
        let baseline = terminal_recovery_classification_identity(&plan());
        assert_eq!(baseline, terminal_recovery_classification_identity(&plan()));

        let mut changed = plan();
        changed.spill_choices = TerminalSpillChoiceIdentity::from_bytes([20; 32]);
        assert_ne!(
            baseline,
    ...
    fn canonical_codec_round_trips_and_rejects_corrupt_framing_and_identity() {
        ...
        assert_eq!(
            TerminalRecoveryClassificationPlan::decode(&encoded),
            Ok(plan)
        );
        ...
        assert_eq!(
            TerminalRecoveryClassificationPlan::decode(&identity_tamper),
            Err(TerminalRecoveryClassificationDecodeError::IdentityMismatch)
        );

        let mut trailing = encoded.clone();
        trailing.push(0);
        assert_eq!(
            TerminalRecoveryClassificationPlan::decode(&trailing),
            Err(TerminalRecoveryClassificationDecodeError::TrailingBytes)
        );
        assert_eq!(
            TerminalRecoveryClassificationPlan::decode(&encoded[..encoded.len() - 1]),
            Err(TerminalRecoveryClassificationDecodeError::Truncated)
        );
