...
# soundness bridge matures. We have two: implementations/beta/check.beta (Beta; term/type trees hand-encoded as
# tagged memory nodes, decided by integer-tag if-cascades) and implementations/gamma/checker.gamma
...
# is unchanged. The user-function cases (Fapp/Frule) encode rules in implementations/gamma/checker.gamma's
# WRAPPER form ((Frule ca ba)); tools/frule_to_flat.py rewrites those to the typed checker's
...
dia() {
  vb=$(printf '%s' "$2" | "$T/check.exe")                       # implementations/beta/check.beta -> accept/reject
  ...
  if [ "$HAVE_TYPED" = 1 ]; then
    ...
    else TFAIL=$((TFAIL+1)); echo "  FAIL(typed) $1 : implementations/gamma/checker.gamma=$vg type-erased=$vt"; fi
  fi
}
#    desc            implementations/beta/check.beta syntax                                          implementations/gamma/checker.gamma syntax                                                                   expect
...
# user-defined recursive FUNCTIONS — implementations/beta/check.beta stores rules in a (fid,cid) table from a
# (fun …) decl prefix; implementations/gamma/checker.gamma carries the SAME rules INLINE on the Fapp node. Both
...
# BINARY function: user-add(1,1)=2 — implementations/beta/check.beta (f fid x y) + table vs implementations/gamma/checker.gamma Fbundle + inline rules
...
# CALLS user-add (fid 10). implementations/beta/check.beta sources add's rules from its table; implementations/gamma/checker.gamma must
...
# inversions UNDER BINDERS: cross-validate implementations/beta/check.beta shift_term vs implementations/gamma/checker.gamma shiftt (deep shift)
...
echo "checker diamond (implementations/beta/check.beta vs implementations/gamma/checker.gamma): $PASS agree, $FAIL disagree"
[ "$FAIL" = 0 ] || exit 1
if [ "$HAVE_TYPED" = 1 ]; then
  echo "  + 3rd oracle (type-erased implementations/gamma/checker_typed.gamma vs implementations/gamma/checker.gamma): $TPASS agree, $TFAIL disagree"
  [ "$TFAIL" = 0 ] || exit 1
else
