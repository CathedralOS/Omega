...
use super::depth_twenty_two::{
    depth_twenty_two_paths_reports_match_for_replay,
...
pub(super) fn depth_twenty_three_paths_reports_match_for_replay(
    left: &ConventionalDepthTwentyThreeRecordSumPathsLayoutReport,
    right: &ConventionalDepthTwentyThreeRecordSumPathsLayoutReport,
) -> bool {
    layout_plan_reports_match_for_replay(&left.outer_layout, &right.outer_layout)
        ...
        && left.paths.iter().zip(&right.paths).all(|(left, right)| {
            field_occurrence_matches(
                ...
                &left.depth_twenty_two_paths,
                &right.depth_twenty_two_paths,
            )
        })
}
...
fn depth_twenty_three_nested_sums_materialization_report_fingerprint(
    ...
    for (path, occurrence) in path_layout.paths.iter().zip(occurrences) {
        ...
        hash_u64(
            &mut hash,
            normalized_layout_plan_report_fingerprint(&path.depth_twenty_two_paths.outer_layout),
        );
        hash_u64(
            &mut hash,
            occurrence
...
#[derive(Debug)]
pub struct ValidatedConstDepthTwentyThreeNestedSumOccurrenceMaterialization {
    outer_field: String,
    outer_member_identity: Option<u64>,
    inner: ValidatedConstRecordWithDepthTwentyTwoNestedSumsMaterialization,
}

impl ValidatedConstDepthTwentyThreeNestedSumOccurrenceMaterialization {
    ...
    pub const fn inner(&self) -> &ValidatedConstRecordWithDepthTwentyTwoNestedSumsMaterialization {
        &self.inner
    }
}
...
impl ValidatedConstRecordWithDepthTwentyThreeNestedSumsMaterialization {
    ...
    pub(super) fn replay_against_with_reachability(
        ...
        if outer_fingerprint != self.non_authoritative_outer_layout_report_fingerprint
            || !depth_twenty_three_paths_reports_match_for_replay(path_layout, &self.path_layout)
        {
            return Err(MaterializationDiagnostic(
...
struct DerivedDepthTwentyThreeNestedSumsMaterialization {
    schema_report_fingerprint: u64,
    occurrences: Vec<ValidatedConstDepthTwentyThreeNestedSumOccurrenceMaterialization>,
    bytes: Vec<u8>,
}
...
fn derive_depth_twenty_three_nested_sums_bytes_with_reachability(
    ...
    if path_layout.outer_layout.schema_report_fingerprint != schema_report_fingerprint {
        return Err(MaterializationDiagnostic(format!(
            "ConstMaterializable plural depth-twenty-three outer layout schema report fingerprint does not match `{schema_name}`"
        )));
    }
    ...
    candidates.try_reserve_exact(members.len()).map_err(|_| {
        MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-three occurrence set exceeds compiler resources"
                .into(),
        )
    })?;
    for member in members {
        ...
        match DataDefinition::shape_kind_from_members(typed.data_members(named)) {
            DataShapeKind::Enum => {
                return Err(MaterializationDiagnostic(format!(
                    "ConstMaterializable plural depth-twenty-three path does not admit direct outer sum field `{}`",
                    field.name
                )));
            }
            DataShapeKind::Mixed => {
    ...
    if candidates.is_empty() {
        return Err(MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-three paths require a nonempty qualifying occurrence set"
                .into(),
        ));
    }
    if path_layout.paths.len() != candidates.len() {
        return Err(MaterializationDiagnostic(format!(
            "ConstMaterializable plural depth-twenty-three report contains {} occurrence(s), expected the complete authored-order set of {}",
    ...
    for path in &path_layout.paths {
        for twenty_second_occurrence in &path.depth_twenty_two_paths.paths {
            for twenty_first_occurrence in &twenty_second_occurrence.depth_twenty_one_paths.paths {
                for twentieth_occurrence in &twenty_first_occurrence.depth_twenty_paths.paths {
                    for nineteenth_occurrence in &twentieth_occurrence.depth_nineteen_paths.paths {
                        for eighteenth_occurrence in
                            &nineteenth_occurrence.depth_eighteen_paths.paths
                        {
                            for seventeenth_occurrence in
                                &eighteenth_occurrence.depth_seventeen_paths.paths
                            {
                                            ...
                                            &fifteenth_occurrence.depth_fourteen_paths.paths
                                        {
                                                ...
                                                &fourteenth_occurrence.depth_thirteen_paths.paths
                                            {
                                                    ...
                                                    &thirteenth_occurrence.depth_twelve_paths.paths
                                                {
                                                    ...
                                                    {
                                                        for tenth_occurrence in &eleventh_occurrence
                                                            .depth_ten_paths
                                                            .paths
                                                        {
                                                    ...
                                                    .ok_or_else(|| {
                                                        MaterializationDiagnostic(
                                                            "ConstMaterializable plural depth-twenty-three leaf occurrence count overflows"
                                                                .into(),
                                                        )
                                                    })?;
                                                                            }
        ...
        if total_leaf_occurrences > SumReachability::MAX_EDGES {
            return Err(MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-three paths exceed the global leaf occurrence bound"
                    .into(),
            ));
        }
    }
    ...
    occurrences
        ...
        .map_err(|_| {
            MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-three inner custody exceeds compiler resources"
                    .into(),
            )
        })?;
    for (index, (inner_field, inner_data)) in candidates.iter().enumerate() {
        ...
        if !field_occurrence_matches(
            ...
            return Err(MaterializationDiagnostic(format!(
                "ConstMaterializable plural depth-twenty-three path for `{}` is missing, duplicated, or out of authored field order",
                inner_field.name
            )));
        }
        ...
        let inner_size = path.depth_twenty_two_paths.outer_layout.size.ok_or_else(|| {
            MaterializationDiagnostic(format!(
                "ConstMaterializable plural depth-twenty-three path `{}` requires one exact inner extent",
                inner_field.name
            ))
        })?;
        if usize::try_from(inner_size).ok() != Some(inner.bytes().len()) {
            return Err(MaterializationDiagnostic(format!(
                "ConstMaterializable plural depth-twenty-three inner bytes for `{}` do not cover the exact inner extent",
                inner_field.name
            )));
        }
        occurrences.push(
    ...
    encoded_fields
        ...
        .map_err(|_| {
            MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-three outer field custody exceeds compiler resources"
                    .into(),
            )
        })?;
    ...
    active.try_reserve_exact(1).map_err(|_| {
        MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-three active path exceeds compiler resources"
                .into(),
        )
    })?;
    ...
    for member in members {
        ...
        if let Some((occurrence, path)) = current_occurrence.filter(|(occurrence, _)| {
            ...
            inner_bytes
                ...
                .map_err(|_| {
                    MaterializationDiagnostic(
                        "ConstMaterializable plural depth-twenty-three inner staging exceeds compiler resources"
                            .into(),
                    )
                })?;
            ...
            encoded_fields.push(EncodedOuterField {
                ...
                size: path
                    .depth_twenty_two_paths
                    .outer_layout
                    .size
                    .expect("validated plural depth-twenty-two extent"),
                align: path.depth_twenty_two_paths.outer_layout.align,
                repeated: None,
                bytes: inner_bytes,
            });
            continue;
    ...
    if occurrence_index != occurrences.len() {
        return Err(MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-three staging did not consume the complete authored-order set"
                .into(),
        ));
    }
    ...
    let byte_len = usize::try_from(
        path_layout
            .outer_layout
            .size
            .expect("validated plural depth-twenty-three outer extent"),
    )
    .map_err(|_| {
        MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-three outer extent exceeds compiler host"
                .into(),
        )
    })?;
    ...
    bytes.try_reserve_exact(byte_len).map_err(|_| {
        MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-three staged bytes exceed compiler resources"
                .into(),
        )
    })?;
    ...
    schemas
        ...
        .map_err(|_| {
            MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-three schema staging exceeds compiler resources"
                    .into(),
            )
        })?;
    values
        ...
        .map_err(|_| {
            MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-three value staging exceeds compiler resources"
                    .into(),
            )
        })?;
    ...
    Ok(DerivedDepthTwentyThreeNestedSumsMaterialization {
