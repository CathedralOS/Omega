...
use super::depth_twenty_one::{
    depth_twenty_one_paths_reports_match_for_replay,
...
pub(super) fn depth_twenty_two_paths_reports_match_for_replay(
    left: &ConventionalDepthTwentyTwoRecordSumPathsLayoutReport,
    right: &ConventionalDepthTwentyTwoRecordSumPathsLayoutReport,
) -> bool {
    layout_plan_reports_match_for_replay(&left.outer_layout, &right.outer_layout)
        ...
        && left.paths.iter().zip(&right.paths).all(|(left, right)| {
            field_occurrence_matches(
                ...
                &left.depth_twenty_one_paths,
                &right.depth_twenty_one_paths,
            )
        })
}

fn depth_twenty_two_nested_sums_materialization_report_fingerprint(
    schema_name: &str,
    ...
    occurrences: &[ValidatedConstDepthTwentyTwoNestedSumOccurrenceMaterialization],
    ...
    for (path, occurrence) in path_layout.paths.iter().zip(occurrences) {
        ...
        hash_u64(
            &mut hash,
            normalized_layout_plan_report_fingerprint(&path.depth_twenty_one_paths.outer_layout),
        );
        hash_u64(
            &mut hash,
            occurrence
...
#[derive(Debug)]
pub struct ValidatedConstDepthTwentyTwoNestedSumOccurrenceMaterialization {
    outer_field: String,
    outer_member_identity: Option<u64>,
    inner: ValidatedConstRecordWithDepthTwentyOneNestedSumsMaterialization,
}

impl ValidatedConstDepthTwentyTwoNestedSumOccurrenceMaterialization {
    ...
    pub const fn inner(&self) -> &ValidatedConstRecordWithDepthTwentyOneNestedSumsMaterialization {
        &self.inner
    }
}
...
#[derive(Debug)]
pub struct ValidatedConstRecordWithDepthTwentyTwoNestedSumsMaterialization {
    schema_name: String,
    non_authoritative_schema_report_fingerprint: u64,
    value: BuildTimeValue,
    path_layout: ConventionalDepthTwentyTwoRecordSumPathsLayoutReport,
    non_authoritative_outer_layout_report_fingerprint: u64,
    occurrences: Vec<ValidatedConstDepthTwentyTwoNestedSumOccurrenceMaterialization>,
    byte_order: ByteOrder,
    bytes: Vec<u8>,
...
impl ValidatedConstRecordWithDepthTwentyTwoNestedSumsMaterialization {
    ...
    pub const fn path_layout(&self) -> &ConventionalDepthTwentyTwoRecordSumPathsLayoutReport {
        &self.path_layout
    }

    pub fn occurrences(&self) -> &[ValidatedConstDepthTwentyTwoNestedSumOccurrenceMaterialization] {
        &self.occurrences
    }
    ...
    pub(super) fn replay_against_with_reachability(
        ...
        if schema_name != self.schema_name || value != &self.value || byte_order != self.byte_order
        {
            return Err(MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two invocation drifted from retained custody"
                    .into(),
            ));
        }
        ...
        if outer_fingerprint != self.non_authoritative_outer_layout_report_fingerprint
            || !depth_twenty_two_paths_reports_match_for_replay(path_layout, &self.path_layout)
        {
            return Err(MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two layout drifted from retained custody"
                    .into(),
            ));
        }

        let replayed = derive_depth_twenty_two_nested_sums_bytes_with_reachability(
            typed,
            schema_name,
            path_layout,
            value,
        ...
        if replayed.occurrences.len() != self.occurrences.len() {
            return Err(MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two custody changed cardinality".into(),
            ));
        }
            ...
            .zip(&self.path_layout.paths)
        {
            if !field_occurrence_matches(
                ...
                return Err(MaterializationDiagnostic(
                    "ConstMaterializable plural depth-twenty-two occurrence identity drifted from retained custody"
                        .into(),
                ));
            }
            ...
            {
                return Err(MaterializationDiagnostic(
                    "ConstMaterializable plural depth-twenty-two inner custody drifted after exact replay"
                        .into(),
                ));
            }
        }
        ...
        {
            return Err(MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two bytes drifted after exact replay"
                    .into(),
            ));
        }
        let fingerprint = depth_twenty_two_nested_sums_materialization_report_fingerprint(
            schema_name,
        ...
        if fingerprint != self.non_authoritative_materialization_report_fingerprint {
            return Err(MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two fingerprint drifted after exact replay"
                    .into(),
            ));
        }
        Ok(())
    ...
    pub fn apply(
        ...
        if destination.len() < self.bytes.len() {
            return Err(MaterializationDiagnostic(format!(
                "ConstMaterializable plural depth-twenty-two copy needs {} bytes, destination has {}",
                self.bytes.len(),
                destination.len()
            )));
        }
...
pub(super) fn validate_const_materializable_record_with_depth_twenty_two_nested_sums_with_reachability(
    ...
    MaterializationDiagnostic,
> {
    let derived = derive_depth_twenty_two_nested_sums_bytes_with_reachability(
        typed,
        schema_name,
        path_layout,
        value,
    ...
    let materialization_fingerprint =
        depth_twenty_two_nested_sums_materialization_report_fingerprint(
            schema_name,
...
struct DerivedDepthTwentyTwoNestedSumsMaterialization {
    schema_report_fingerprint: u64,
    occurrences: Vec<ValidatedConstDepthTwentyTwoNestedSumOccurrenceMaterialization>,
    bytes: Vec<u8>,
}
...
fn derive_depth_twenty_two_nested_sums_bytes_with_reachability(
    ...
    if path_layout.outer_layout.schema_report_fingerprint != schema_report_fingerprint {
        return Err(MaterializationDiagnostic(format!(
            "ConstMaterializable plural depth-twenty-two outer layout schema report fingerprint does not match `{schema_name}`"
        )));
    }
    ...
    candidates.try_reserve_exact(members.len()).map_err(|_| {
        MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-two occurrence set exceeds compiler resources"
                .into(),
        )
    })?;
    for member in members {
        ...
        match DataDefinition::shape_kind_from_members(typed.data_members(named)) {
            DataShapeKind::Enum => {
                return Err(MaterializationDiagnostic(format!(
                    "ConstMaterializable plural depth-twenty-two path does not admit direct outer sum field `{}`",
                    field.name
                )));
            }
            DataShapeKind::Mixed => {
    ...
    if candidates.is_empty() {
        return Err(MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-two paths require a nonempty qualifying occurrence set"
                .into(),
        ));
    }
    if path_layout.paths.len() != candidates.len() {
        return Err(MaterializationDiagnostic(format!(
            "ConstMaterializable plural depth-twenty-two report contains {} occurrence(s), expected the complete authored-order set of {}",
    ...
    for path in &path_layout.paths {
        for twenty_first_occurrence in &path.depth_twenty_one_paths.paths {
                                            ...
                                            &fourteenth_occurrence.depth_thirteen_paths.paths
                                        {
                                                ...
                                                &thirteenth_occurrence.depth_twelve_paths.paths
                                            {
                                                    ...
                                                    &twelfth_occurrence.depth_eleven_paths.paths
                                                {
                                                    ...
                                                    .ok_or_else(|| {
                                                        MaterializationDiagnostic(
                                                            "ConstMaterializable plural depth-twenty-two leaf occurrence count overflows"
                                                                .into(),
                                                        )
                                                    })?;
                                                                            }
        ...
        if total_leaf_occurrences > SumReachability::MAX_EDGES {
            return Err(MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two paths exceed the global leaf occurrence bound"
                    .into(),
            ));
        }
    }
    ...
    occurrences
        ...
        .map_err(|_| {
            MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two inner custody exceeds compiler resources"
                    .into(),
            )
        })?;
    for (index, (inner_field, inner_data)) in candidates.iter().enumerate() {
        ...
        if !field_occurrence_matches(
            ...
            return Err(MaterializationDiagnostic(format!(
                "ConstMaterializable plural depth-twenty-two path for `{}` is missing, duplicated, or out of authored field order",
                inner_field.name
            )));
        }
        ...
        let inner_size = path.depth_twenty_one_paths.outer_layout.size.ok_or_else(|| {
            MaterializationDiagnostic(format!(
                "ConstMaterializable plural depth-twenty-two path `{}` requires one exact inner extent",
                inner_field.name
            ))
        })?;
        if usize::try_from(inner_size).ok() != Some(inner.bytes().len()) {
            return Err(MaterializationDiagnostic(format!(
                "ConstMaterializable plural depth-twenty-two inner bytes for `{}` do not cover the exact inner extent",
                inner_field.name
            )));
        }
        occurrences.push(
    ...
    encoded_fields
        ...
        .map_err(|_| {
            MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two outer field custody exceeds compiler resources"
                    .into(),
            )
        })?;
    ...
    active.try_reserve_exact(1).map_err(|_| {
        MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-two active path exceeds compiler resources"
                .into(),
        )
    })?;
    ...
    for member in members {
        ...
        if let Some((occurrence, path)) = current_occurrence.filter(|(occurrence, _)| {
            ...
            inner_bytes
                ...
                .map_err(|_| {
                    MaterializationDiagnostic(
                        "ConstMaterializable plural depth-twenty-two inner staging exceeds compiler resources"
                            .into(),
                    )
                })?;
            ...
            encoded_fields.push(EncodedOuterField {
                ...
                size: path
                    ...
                    .outer_layout
                    .size
                    .expect("validated plural depth-twenty-one extent"),
                align: path.depth_twenty_one_paths.outer_layout.align,
                repeated: None,
                bytes: inner_bytes,
            });
            continue;
    ...
    if occurrence_index != occurrences.len() {
        return Err(MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-two staging did not consume the complete authored-order set"
                .into(),
        ));
    }
    ...
    let byte_len = usize::try_from(
        path_layout
            .outer_layout
            .size
            .expect("validated plural depth-twenty-two outer extent"),
    )
    .map_err(|_| {
        MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-two outer extent exceeds compiler host".into(),
        )
    })?;
    let mut bytes = Vec::new();
    bytes.try_reserve_exact(byte_len).map_err(|_| {
        MaterializationDiagnostic(
            "ConstMaterializable plural depth-twenty-two staged bytes exceed compiler resources"
                .into(),
        )
    })?;
    ...
    schemas
        ...
        .map_err(|_| {
            MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two schema staging exceeds compiler resources"
                    .into(),
            )
        })?;
    values
        ...
        .map_err(|_| {
            MaterializationDiagnostic(
                "ConstMaterializable plural depth-twenty-two value staging exceeds compiler resources"
                    .into(),
            )
        })?;
    ...
    Ok(DerivedDepthTwentyTwoNestedSumsMaterialization {
