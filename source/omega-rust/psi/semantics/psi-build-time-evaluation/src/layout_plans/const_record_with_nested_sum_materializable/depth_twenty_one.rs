    ...
    layout_plan_reports_match_for_replay(&left.outer_layout, &right.outer_layout)
        ...
        && left.paths.iter().zip(&right.paths).all(|(left, right)| {
            field_occurrence_matches(
                ...
                &left.depth_twenty_paths,
                &right.depth_twenty_paths,
            )
        })
}
...
fn depth_twenty_one_nested_sums_materialization_report_fingerprint(
    ...
    for (path, occurrence) in path_layout.paths.iter().zip(occurrences) {
        ...
        hash_u64(
            &mut hash,
            normalized_layout_plan_report_fingerprint(&path.depth_twenty_paths.outer_layout),
        );
        hash_u64(
            &mut hash,
            occurrence
...
fn derive_depth_twenty_one_nested_sums_bytes_with_reachability(
    ...
    for path in &path_layout.paths {
        for twentieth_occurrence in &path.depth_twenty_paths.paths {
            for nineteenth_occurrence in &twentieth_occurrence.depth_nineteen_paths.paths {
                for eighteenth_occurrence in &nineteenth_occurrence.depth_eighteen_paths.paths {
                    for seventeenth_occurrence in &eighteenth_occurrence.depth_seventeen_paths.paths
                    {
                        for sixteenth_occurrence in
                            &seventeenth_occurrence.depth_sixteen_paths.paths
                        {
                            for fifteenth_occurrence in
                                &sixteenth_occurrence.depth_fifteen_paths.paths
                            {
    ...
    for (index, (inner_field, inner_data)) in candidates.iter().enumerate() {
        ...
        let inner_size = path.depth_twenty_paths.outer_layout.size.ok_or_else(|| {
    ...
    for member in members {
        ...
        if let Some((occurrence, path)) = current_occurrence.filter(|(occurrence, _)| {
            ...
            encoded_fields.push(EncodedOuterField {
                ...
                align: path.depth_twenty_paths.outer_layout.align,
                repeated: None,
                bytes: inner_bytes,
            });
            continue;
