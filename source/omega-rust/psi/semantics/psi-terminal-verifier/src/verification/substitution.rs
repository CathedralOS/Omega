...
/// Whether value substitution can change this proposition.
///
/// Successor binding retains every already-established fact and only needs to
/// materialize an additional rewritten fact when one of the edge arguments
/// actually occurs in it. Keeping this query separate from substitution avoids
/// cloning large, unrelated proof terms at every control-flow edge.
pub(crate) fn proposition_mentions_substituted_value(
    proposition: &Proposition,
    substitutions: &BTreeMap<ValueId, ScalarTerm>,
) -> bool {
    match proposition {
        ...
        | Proposition::LessOrEqual(left, right) => {
            scalar_term_mentions_substituted_value(left, substitutions)
                || scalar_term_mentions_substituted_value(right, substitutions)
        }
        ...
        | Proposition::IntegerMathLessOrEqual(left, right) => {
            integer_math_term_mentions_substituted_value(left, substitutions)
                || integer_math_term_mentions_substituted_value(right, substitutions)
        }
        Proposition::Conjunction(propositions) | Proposition::Disjunction(propositions) => {
            propositions.iter().any(|proposition| {
                proposition_mentions_substituted_value(proposition, substitutions)
            })
        }
        Proposition::Implication {
            premise,
            ...
            proposition_mentions_substituted_value(premise, substitutions)
                || proposition_mentions_substituted_value(conclusion, substitutions)
        }
    }
}

fn integer_math_term_mentions_substituted_value(
    term: &IntegerMathTerm,
    substitutions: &BTreeMap<ValueId, ScalarTerm>,
) -> bool {
    match term {
        IntegerMathTerm::IntegerLiteral(_) => false,
        IntegerMathTerm::MathValue { value, .. } => substitutions.contains_key(value),
        ...
        IntegerMathTerm::ShiftLeft { value, count } => {
            integer_math_term_mentions_substituted_value(value, substitutions)
                || integer_math_term_mentions_substituted_value(count, substitutions)
        }
    }
}

fn scalar_term_mentions_substituted_value(
    term: &ScalarTerm,
    substitutions: &BTreeMap<ValueId, ScalarTerm>,
) -> bool {
    match term {
        ScalarTerm::Value { id, .. } => substitutions.contains_key(id),
        ScalarTerm::BooleanField { .. }
        | ScalarTerm::IntegerField { .. }
        | ScalarTerm::Boolean(_)
        | ScalarTerm::Integer { .. } => false,
        ScalarTerm::BooleanNot { operand }
        | ScalarTerm::IntegerBitwiseNot { operand, .. }
        | ScalarTerm::IntegerWiden { operand, .. }
        | ScalarTerm::IntegerExactCast { operand, .. } => {
            scalar_term_mentions_substituted_value(operand, substitutions)
        }
