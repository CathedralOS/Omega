...
pub(super) fn build(
    ...
    if psi_typed_trees::data::DataDefinition::shape_kind_from_members(
        program.data_members(result_data),
    ) != DataShapeKind::Enum
    {
        return None;
    }
        ...
        .enumerate()
    {
        ...
        if subject_place.root != psi_facts::PlaceRoot::Symbol(result_symbol)
        ...
        let payloads = if variant.symbol == payload_variant_definition.symbol {
            ...
            if argument_place.root != psi_facts::PlaceRoot::Symbol(result_symbol)
...
fn variant_identity(variant: &psi_typed_trees::data::DataVariant) -> String {
    variant
        .identity
...
fn field_identity(field: &psi_typed_trees::data::DataField) -> String {
    field
        .identity
        .map(|identity| format!("#{identity}"))
