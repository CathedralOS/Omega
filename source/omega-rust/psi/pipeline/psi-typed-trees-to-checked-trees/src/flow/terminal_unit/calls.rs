...
pub(super) fn build_call_operation(
    ...
    if !boundary {
        ...
        let supported = if carries_routed_service {
            exact_single_fused_service_forward_is_supported(
                program,
                facts,
                machine,
                state,
...
/// Admit one source-visible affine Service move into one terminal helper.
///
/// This is deliberately independent from the ordinary empty-path fast path:
/// routed authority must retain one exact carrier/requirement/selection across
/// the edge, and the target must terminate the route in direct boundary calls
/// rather than becoming an unbounded forwarding graph.
#[allow(clippy::too_many_arguments)]
fn exact_single_fused_service_forward_is_supported(
    program: &TypedTrees,
    facts: &CheckFacts,
    ...
    let Ok(Some(caller_carrier)) = psi_typed_trees::service::classify_exact_bound_service_carrier(
        program,
        caller_source.type_reference,
    ) else {
        return false;
    };
    let Ok(Some(target_carrier)) = psi_typed_trees::service::classify_exact_bound_service_carrier(
        program,
