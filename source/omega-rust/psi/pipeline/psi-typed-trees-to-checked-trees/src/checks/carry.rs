...
use psi_facts::{FactPayload, FactPlace, QualificationEvidence};
use psi_language_semantics::{CarryPolicy, CarrySuspension};

mod activation;
mod intra_statement;

struct ClaimCarryContext<'facts> {
    semantic: &'facts psi_facts::FactPlan,
    ownership: &'facts psi_checked_trees::FlowOwnershipFacts,
    claim_policies: &'facts [psi_checked_trees::ClaimCarryPolicyFact],
    state_symbol: psi_symbols::SymbolHandle,
    statement_index: usize,
    call_ordinal: usize,
    entry_contexts: Vec<psi_facts::FactContextHandle>,
}
...
impl ClaimCarryContext<'_> {
    ...
    fn live_claim_identities(
        ...
        program: &psi_typed_trees::TypedTrees,
        value_name: &str,
    ) -> Vec<psi_language_semantics::PermissionClaimIdentity> {
        let mut latest_by_path = Vec::<(
            String,
            (usize, usize, usize),
            psi_language_semantics::PermissionClaimIdentity,
        )>::new();
            ...
            .enumerate()
        {
            if event.state_symbol != self.state_symbol
                ...
                || event.claim_identity == psi_language_semantics::PermissionClaimIdentity::Unknown
            {
                continue;
            }
...
pub(super) fn check_suspension_carry(
    ...
    for state_borrows in facts.borrow.states.iter().map(|(_, state)| state) {
        ...
        for call in facts.borrow.calls.span_or_empty(state_borrows.calls) {
            ...
            suspension_crossings.push(psi_checked_trees::SuspensionCrossingCarryFact {
...
fn derive_claim_carry_policies(
...
) -> Vec<psi_checked_trees::ClaimCarryPolicyFact> {
    let mut origins = Vec::<(
        psi_language_semantics::PermissionClaimIdentity,
    ...
    let mut policies = Vec::<psi_checked_trees::ClaimCarryPolicyFact>::new();
    for (claim_identity, _, origin_policy) in origins {
            ...
            .find(|fact| fact.claim_identity == claim_identity)
        {
            ...
            policies.push(psi_checked_trees::ClaimCarryPolicyFact {
                claim_identity,
...
fn append_state_successors_after_statement(
    ...
    state: &psi_typed_trees::state::State,
    statement_index: usize,
    successors: &mut Vec<psi_symbols::SymbolHandle>,
) {
...
fn append_statement_successors(
    ...
    let psi_typed_trees::statement::StatementNode::Transition(transition) = statement else {
        return;
    };
...
fn append_transition_target_successor(
    ...
    let symbol = match program.statement_table.transition_target(target) {
        psi_typed_trees::statement::TransitionTargetNode::Named { path, .. } => path.symbol,
        psi_typed_trees::statement::TransitionTargetNode::SelfTarget => state.symbol,
        psi_typed_trees::statement::TransitionTargetNode::Value(_)
        | psi_typed_trees::statement::TransitionTargetNode::Terminal => return,
    };
    if symbol.is_valid()
        && program
            .machine_states(machine)
