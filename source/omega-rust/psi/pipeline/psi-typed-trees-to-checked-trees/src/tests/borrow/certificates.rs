...
fn retains_zero_premise_structural_symbolic_adjacency_certificate() {
    ...
    assert_eq!(
        ...
        vec![
            (
                psi_checked_trees::BorrowCompatibilityPlaceSide::Forming,
                0,
                psi_checked_trees::BorrowCompatibilitySelectorPosition::RangeStart,
                Some(psi_checked_trees::BorrowCompatibilitySelectorValue::Integer(2)),
            ),
            (
                psi_checked_trees::BorrowCompatibilityPlaceSide::Forming,
                0,
                psi_checked_trees::BorrowCompatibilitySelectorPosition::RangeExclusiveEnd,
                Some(psi_checked_trees::BorrowCompatibilitySelectorValue::Integer(4)),
            ),
            (
                psi_checked_trees::BorrowCompatibilityPlaceSide::Active,
                0,
                psi_checked_trees::BorrowCompatibilitySelectorPosition::RangeStart,
                Some(psi_checked_trees::BorrowCompatibilitySelectorValue::Integer(0)),
            ),
            (
                psi_checked_trees::BorrowCompatibilityPlaceSide::Active,
                0,
...
fn rejects_tampered_normalized_selector_value() {
    ...
    let active_end = certificate
        ...
        .iter_mut()
        .find(|selector| {
            selector.side == psi_checked_trees::BorrowCompatibilityPlaceSide::Active
                && selector.position
                    == psi_checked_trees::BorrowCompatibilitySelectorPosition::RangeExclusiveEnd
        })
        .expect("active exclusive end snapshot");
    active_end.value = Some(psi_checked_trees::BorrowCompatibilitySelectorValue::Integer(3));
...
fn rejects_co_tampered_selector_snapshot_and_conclusion() {
    ...
    certificate
        ...
        .expect("active exclusive end snapshot")
        .value = Some(psi_checked_trees::BorrowCompatibilitySelectorValue::Integer(3));
    ...
    let active_constraint = (0..statement.entry_constraints.count())
        ...
        .find(|handle| {
            matches!(
                ...
                psi_checked_trees::FlowConstraintKind::BorrowLoan { loan }
