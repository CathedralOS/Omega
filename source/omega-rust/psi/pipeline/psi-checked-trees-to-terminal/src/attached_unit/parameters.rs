...
pub(crate) fn validate_direct_unit_parameter_custody(
    ...
    validate_fused_service_forwarding(checked)?;
    Ok(())
}

fn validate_fused_service_forwarding(checked: &CheckedTrees) -> Result<(), LoweringError> {
    ...
    for caller in plans {
        ...
        let Some((target_body, target_return)) = target.operations.split_last() else {
        ...
        ) || target_return.is_empty()
            || target_return.iter().any(|operation| {
                !matches!(operation,
