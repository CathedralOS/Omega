...
fn lower_projection(
    ...
    projection: &psi_language_semantics::content::ContentProjectionPlan,
) -> Result<psi_terminal::RetainedBorrowContentProjection, LoweringError> {
    let exact = checked
        .facts
        .qualifications
        .content
    ...
    Ok(psi_terminal::RetainedBorrowContentProjection {
...
fn lower_custody(
    checked: &CheckedTrees,
    fact: &psi_checked_trees::RetainedBorrowCustodyFact,
) -> Result<psi_terminal::RetainedBorrowCustody, LoweringError> {
    ...
    let psi_language_semantics::content::ContentPlaceRoot::Parameter {
        position,
        symbol,
        name,
        is_self,
    ...
    if parameter.symbol != *symbol
        ...
        || *is_self
        || fact.source.version != psi_language_semantics::content::ContentPlaceVersion::Entry
        || !fact.source.segments.is_empty()
        || fact.access != psi_language_semantics::ReferenceAccess::Shared
    {
    ...
    if fact.result.version != psi_language_semantics::content::ContentPlaceVersion::Current
        || !matches!(
            fact.result.root,
            psi_language_semantics::content::ContentPlaceRoot::Result
        )
    ...
    Ok(psi_terminal::RetainedBorrowCustody {
        callable_identity,
        source: psi_terminal::RetainedBorrowPlace {
        ...
        result: psi_terminal::RetainedBorrowPlace {
