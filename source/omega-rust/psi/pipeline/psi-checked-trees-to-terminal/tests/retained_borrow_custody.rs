...
) -> &mut psi_terminal::RetainedBorrowCustody {
    module
        .boundary_machines
        .iter_mut()
        .flat_map(|boundary| &mut boundary.content_guarantees)
        .find_map(|guarantee| match guarantee {
            psi_terminal::BoundaryContentGuarantee::RetainedBorrow(custody) => Some(custody),
            psi_terminal::BoundaryContentGuarantee::Conservation(_) => None,
        })
...
fn checked_retained_shared_borrow_reaches_canonical_terminal_custody() {
    ...
    let [psi_terminal::BoundaryContentGuarantee::RetainedBorrow(custody)] =
    ...
    assert!(matches!(
        ...
        psi_terminal::RetainedBorrowPlace {
            ...
            root: psi_terminal::RetainedBorrowPlaceRoot::Parameter {
                position: 0,
                is_self: false,
                ..
            },
    ...
    assert!(matches!(
        ...
        psi_terminal::RetainedBorrowPlace {
            ...
            root: psi_terminal::RetainedBorrowPlaceRoot::Result,
            segments,
        } if segments.is_empty()
    ));

    let bytes = psi_terminal_codec::encode_module(&lowered.semantic_module).expect("encode");
    let decoded = psi_terminal_codec::decode_module(&bytes).expect("decode");
    assert_eq!(decoded, lowered.semantic_module);
    psi_terminal_verifier::validate_module(&decoded).expect("independent verification");
}

#[test]
fn terminal_verifier_rejects_retained_borrow_fence_drift() {
    let baseline = lowered().semantic_module;
    let rejects = |mutate: fn(&mut psi_terminal::RetainedBorrowCustody)| {
        let mut module = baseline.clone();
        mutate(retained_mut(&mut module));
        assert!(matches!(
            psi_terminal_verifier::validate_module(&module),
            Err(psi_terminal_verifier::ModuleError::InvalidBoundaryContentGuarantee(_))
        ));
    };

    rejects(|custody| custody.access = psi_terminal::StructuralAccess::MutableBorrow);
    ...
    rejects(|custody| custody.result_multiplicity = psi_terminal::StructuralMultiplicity::Affine);
    rejects(|custody| custody.result_nominal_identity.push_str("-drift"));
    rejects(|custody| {
        custody
            .source
            .segments
            .push(psi_core::ContentPlaceSegment::FixedIndex(0))
    });
    rejects(|custody| {
        custody.source.root = psi_terminal::RetainedBorrowPlaceRoot::Parameter {
            position: 0,
