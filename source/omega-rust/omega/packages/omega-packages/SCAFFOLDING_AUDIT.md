...
Status: refreshed complete inventory, 2026-08-28. This audit classifies the current Rust
...
| `src/resolution/source_commands.rs` | Rewrite | Diagnostic rendering is useful. Explicit `local`/`git` adapter selection has replaced locator guessing, but execution still uses the unhardened resolver. Keep marked unhardened until the remaining resolver P0 work lands. |
...
| `src/lib.rs` | Retain | The crate root is now a small responsibility map over `declarations`, `resolution`, `review`, and private `storage`; historical flat exports remain only as caller compatibility. Superseded manifest, lock, receipt, install, update, diff, command, and graph-audit modules remain deleted. Production orchestration must receive opaque compiler/resolver evidence; the crate remains experimental until those replacements exist. |
...
| --- | --- | --- |
| Source/cache process isolation | `src/resolution/source/` seals Git configuration, validates resolver-owned cache state, serializes resolver access, checks submodule policy before materialization, and consumes Git and local requests through validated immutable snapshots. `omega-resolver-execution` supplies closed process, endpoint, and host-confinement policy; macOS enforcement is materially stronger, while Linux, Windows, SSH-read custody, aggregate quotas, same-user mutation, and strict receipt reconstruction remain incomplete. | Partial only; finish every platform guarantee and reconstruct a strict resolver receipt before mutation is enabled. |
...
| Persistence | Manifest, receipt, lock, and diagnostic resolver-record writers are deleted. No accepted persistence writer exists. | Build accepted persistence from sealed evidence under the transaction floor; do not introduce a caller-readable proto-receipt. |
