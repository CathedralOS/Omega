...
pub struct GitSourceRequest {
    ...
    pub(in crate::resolution::source) fetch_locator: String,
    pub(in crate::resolution::source) locator_identity: String,
    pub(in crate::resolution::source) requested_revision: String,
    pub(in crate::resolution::source) lineage: SourceLineage,
    pub(in crate::resolution::source) execution_transport: GitExecutionTransport,
    pub(in crate::resolution::source) requested_network_endpoint: GitRequestedNetworkEndpoint,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::resolution::source) enum GitExecutionTransport {
    Https,
    Ssh,
    #[cfg(test)]
    File,
...
impl GitExecutionTransport {
    ...
    pub(in crate::resolution::source) fn resolver_network_transport(
        self,
    ...
    pub(in crate::resolution::source) fn cache_tag(self) -> &'static [u8] {
        match self {
            Self::Https => b"https",
            Self::Ssh => b"ssh",
            #[cfg(test)]
    ...
    pub(in crate::resolution::source) fn allowed_protocol(self) -> &'static str {
        match self {
            Self::Https => "https",
            Self::Ssh => "ssh",
            #[cfg(test)]
    ...
    pub(in crate::resolution::source) fn permits(self, transport: Self) -> &'static str {
        if self == transport { "always" } else { "never" }
    }

    pub(in crate::resolution::source) fn profile(self) -> GitTransportProfile {
        match self {
...
impl GitSourceRequest {
    ...
    pub(in crate::resolution::source) fn fetch_locator(&self) -> &str {
        &self.fetch_locator
    }

    pub(in crate::resolution::source) fn execution_transport(&self) -> GitExecutionTransport {
