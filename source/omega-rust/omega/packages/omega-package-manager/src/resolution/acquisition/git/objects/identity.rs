...
use crate::resolution::acquisition::CheckedSha1;
use crate::resolution::acquisition::error::SourceResolveError;
use crate::resolution::acquisition::git::execution::format_hex;
use crate::resolution::identity::GitObjectIdAlgorithm;
use sha2::{Digest, Sha256};

pub(in crate::resolution::acquisition) fn verify_git_object_identity(
    expected: &str,
    kind: &[u8],
    payload: &[u8],
...
pub(in crate::resolution::acquisition) fn git_object_identity(
    kind: &[u8],
    payload: &[u8],
    algorithm: GitObjectIdAlgorithm,
...
pub(in crate::resolution::acquisition) fn finalize_checked_sha1(
    hasher: CheckedSha1,
) -> Result<String, SourceResolveError> {
...
pub(in crate::resolution::acquisition) fn git_object_algorithm(
    oid: &str,
...
pub(in crate::resolution::acquisition) fn hex_digit(byte: u8) -> Option<u8> {
    match byte {
...
pub(in crate::resolution::acquisition) fn is_object_id(value: &str) -> bool {
    matches!(value.len(), 40 | 64) && value.bytes().all(|byte| byte.is_ascii_hexdigit())
}

pub(in crate::resolution::acquisition) fn git_object_invalid(
