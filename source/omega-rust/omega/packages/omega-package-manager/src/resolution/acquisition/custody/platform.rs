...
use std::fs::File;
use std::path::Path;
pub(in crate::resolution::acquisition) fn same_capability_file_identity(
...
#[cfg(unix)]
pub(in crate::resolution::acquisition) fn verify_capability_cache_node_owner_and_mode(
...
#[cfg(windows)]
pub(in crate::resolution::acquisition) fn verify_windows_open_cache_custody(
    kind: CacheCustodyKind,
    path: &Path,
    file: &File,
) -> Result<(), SourceResolveError> {
...
#[cfg(target_os = "macos")]
pub(in crate::resolution::acquisition) fn verify_macos_open_cache_extended_acl_custody(
    kind: CacheCustodyKind,
    path: &Path,
    file: &File,
) -> Result<(), SourceResolveError> {
...
#[cfg(target_os = "macos")]
pub(in crate::resolution::acquisition) fn verify_macos_open_cache_directory_acl_custody(
    kind: CacheCustodyKind,
    path: &Path,
    classified: &std::fs::Metadata,
) -> Result<(), SourceResolveError> {
