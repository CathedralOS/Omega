...
use crate::resolution::acquisition::custody::{
...
use crate::resolution::acquisition::git::execution::{
...
use crate::resolution::acquisition::limits::{
    GIT_STDERR_LIMIT, LocalSourceLimits, STAGING_SEQUENCE,
};
use crate::resolution::acquisition::local::capture::io_error;
use crate::resolution::acquisition::{Seek, SeekFrom};
...
#[cfg(test)]
pub(in crate::resolution::acquisition) fn read_git_blobs_batch_from_path(
    executor: &GitExecutor,
    repository: &Path,
    entries: &mut [GitTreeEntry],
    limits: LocalSourceLimits,
...
pub(in crate::resolution::acquisition) fn git_batch_output_limit(
    entries: &[GitTreeEntry],
    limits: LocalSourceLimits,
...
pub(in crate::resolution::acquisition) fn assign_git_batch_output(
    entries: &mut [GitTreeEntry],
    output: Vec<u8>,
) -> Result<(), SourceResolveError> {
...
pub(in crate::resolution::acquisition) struct PendingGitBatchRequest {
    pub(in crate::resolution::acquisition) parent: CapabilityDirectory,
    pub(in crate::resolution::acquisition) name: OsString,
    pub(in crate::resolution::acquisition) display_path: PathBuf,
    pub(in crate::resolution::acquisition) file: Option<File>,
    pub(in crate::resolution::acquisition) identity: Option<CapabilityMetadata>,
    pub(in crate::resolution::acquisition) removed: bool,
}

impl PendingGitBatchRequest {
    pub(in crate::resolution::acquisition) fn create(
    ...
    pub(in crate::resolution::acquisition) fn remove(&mut self) -> Result<(), SourceResolveError> {
        self.verify_current()?;
        drop(self.file.take());
        let named = self
            .parent
