...
#[derive(Debug)]
pub(in crate::source::acquisition) struct CapturedLocalTree {
    pub(in crate::source::acquisition) normalized: ResolvedLocalSource,
    pub(in crate::source::acquisition) entries: Vec<CapturedLocalEntry>,
}

#[derive(Debug)]
pub(in crate::source::acquisition) struct CapturedLocalEntry {
    pub(in crate::source::acquisition) relative_path: PathBuf,
    pub(in crate::source::acquisition) relative_bytes: Vec<u8>,
    pub(in crate::source::acquisition) kind: CapturedLocalEntryKind,
}

#[derive(Debug)]
pub(in crate::source::acquisition) enum CapturedLocalEntryKind {
    Directory,
    File { bytes: Vec<u8>, executable: bool },
    Symlink { target_bytes: Vec<u8> },
}
...
pub(in crate::source::acquisition) enum SourceTreePolicy {
...
#[cfg(test)]
pub(in crate::source::acquisition) fn resolve_materialized_source(
    root: &Path,
...
pub(in crate::source::acquisition) fn capture_local_source(
    requested_root: &Path,
    limits: LocalSourceLimits,
...
pub(in crate::source::acquisition) fn open_canonical_source_root(
    canonical_root: &Path,
...
pub(in crate::source::acquisition) fn open_absolute_directory_nofollow(
    canonical_root: &Path,
...
pub(in crate::source::acquisition) fn capture_local_source_from_open_root(
    root: PathBuf,
...
pub(in crate::source::acquisition) fn open_captured_directory(
    directory: &CapabilityDirectory,
    name: &OsStr,
    display_path: &Path,
) -> Result<CapabilityDirectory, SourceResolveError> {
...
pub(in crate::source::acquisition) fn read_capability_file_bounded(
    directory: &CapabilityDirectory,
    name: &OsStr,
    display_path: &Path,
    remaining: u64,
...
#[cfg(all(test, unix))]
pub(in crate::source::acquisition) fn is_executable(metadata: &std::fs::Metadata) -> bool {
...
pub(in crate::source::acquisition) fn is_executable(_metadata: &std::fs::Metadata) -> bool {
    false
}

pub(in crate::source::acquisition) fn raw_os_bytes(value: &OsStr) -> Vec<u8> {
    value.as_encoded_bytes().to_vec()
}

pub(in crate::source::acquisition) struct SourceIdentityHasher {
    pub(in crate::source::acquisition) hasher: Sha256,
    pub(in crate::source::acquisition) byte_count: u64,
}

impl SourceIdentityHasher {
    pub(in crate::source::acquisition) fn new(entry_count: usize) -> Self {
    ...
    pub(in crate::source::acquisition) fn add_directory(
        &mut self,
        relative_bytes: &[u8],
        normalized_mode: u16,
    ) {
    ...
    pub(in crate::source::acquisition) fn add_file(
        &mut self,
        relative_bytes: &[u8],
        executable: bool,
        bytes: &[u8],
    ...
    pub(in crate::source::acquisition) fn add_symlink(
        &mut self,
        relative_bytes: &[u8],
        target_bytes: &[u8],
    ) {
    ...
    pub(in crate::source::acquisition) fn finish(self) -> (u64, String) {
        (self.byte_count, format_sha256(&self.hasher.finalize()))
    }
}

pub(in crate::source::acquisition) fn hash_bytes(hasher: &mut Sha256, bytes: &[u8]) {
    hash_length(hasher, bytes.len() as u64);
    hasher.update(bytes);
}

pub(in crate::source::acquisition) fn hash_length(hasher: &mut Sha256, length: u64) {
    hasher.update(length.to_le_bytes());
}

pub(in crate::source::acquisition) fn io_error(
    path: &Path,
    error: std::io::Error,
) -> SourceResolveError {
