...
#[derive(Debug)]
pub(crate) struct RetainedStorageLane {
    path: PathBuf,
...
impl SourceResolverStorage {
    ...
    pub(crate) fn git_sources(&self) -> &RetainedStorageLane {
        &self.git_sources
    }

    pub(crate) fn workspace_members(&self) -> &RetainedStorageLane {
        &self.workspace_members
    }

    pub(crate) fn external_local_sources(&self) -> &RetainedStorageLane {
...
impl RetainedStorageLane {
    ...
    pub(crate) fn directory(&self) -> &CapabilityDirectory {
        &self.directory
    }

    pub(crate) fn retain_child(&self, name: &str) -> Result<Self, SourceResolveError> {
