...
use crate::source::acquisition::git::execution::executable::{
    CapturedOutputLimitExceeded, GitCapturedOutputBudget,
};
use crate::source::acquisition::limits::{GIT_COMMAND_CLEANUP_TIMEOUT, PROCESS_POLL_INTERVAL};
...
#[derive(Debug)]
pub(in crate::source::acquisition) struct BoundedCommandOutput {
    pub(in crate::source::acquisition) status: ExitStatus,
    pub(in crate::source::acquisition) stdout: Vec<u8>,
    pub(in crate::source::acquisition) stderr: Vec<u8>,
}
...
pub(in crate::source::acquisition) enum StreamCaptureResult {
    Complete(Vec<u8>),
    Overflow,
...
#[cfg(test)]
pub(in crate::source::acquisition) fn run_command_bounded(
    command: &mut Command,
    operation: &str,
    stdout_limit: usize,
    stderr_limit: usize,
...
pub(in crate::source::acquisition) fn run_command_bounded_with_budget(
    command: &mut Command,
    operation: &str,
    stdout_limit: usize,
    stderr_limit: usize,
...
pub(in crate::source::acquisition) fn run_command_bounded_with_stdin_and_budget(
    command: &mut Command,
    stdin: Stdio,
    operation: &str,
    stdout_limit: usize,
...
#[cfg(test)]
pub(in crate::source::acquisition) fn capture_stream_bounded<R>(
    mut reader: R,
    limit: usize,
) -> StreamCaptureResult
where
...
pub(in crate::source::acquisition) fn command_cleanup_reserve(timeout: Duration) -> Duration {
...
pub(in crate::source::acquisition) fn process_group_already_absent(error: &std::io::Error) -> bool {
    #[cfg(unix)]
    {
...
pub(in crate::source::acquisition) fn duration_millis(duration: Duration) -> u64 {
