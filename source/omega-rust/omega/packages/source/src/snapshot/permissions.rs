...
pub(crate) fn verify_open_snapshot_tree_modes(
...
pub(crate) fn make_open_snapshot_read_only(
    kind: CacheCustodyKind,
    root: &CapabilityDirectory,
    display_root: &Path,
) -> Result<(), SourceResolveError> {
...
#[cfg(test)]
pub(crate) fn make_snapshot_read_only(root: &Path) -> Result<(), SourceResolveError> {
