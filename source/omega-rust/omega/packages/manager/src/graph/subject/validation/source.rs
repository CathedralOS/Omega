...
pub(super) fn validate_git_selection(
    selection: &crate::project::PackageSelection,
    selected: &ResolvedSourceIdentity,
    navigation: &PackageSourceNavigation,
) -> Result<(), CanonicalSourceClosureSubjectError> {
    match (selection, navigation) {
        (crate::project::PackageSelection::Root, PackageSourceNavigation::Root) => Ok(()),
        (crate::project::PackageSelection::Named(package), PackageSourceNavigation::Member(_))
            if package == selected.key().name() =>
        {
            Ok(())
        }
        (crate::project::PackageSelection::Named(package), _)
            if package != selected.key().name() =>
        {
