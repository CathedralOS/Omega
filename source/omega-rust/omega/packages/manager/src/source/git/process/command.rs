...
pub(in crate::source) fn sealed_git_command_with_route(
...
#[cfg(test)]
pub(in crate::source) fn sealed_git_command(
    executor: &GitExecutor,
    working_directory: &Path,
    phase: ResolverExecutionPhase,
) -> Result<ResolverPreparedExecution, SourceResolveError> {
...
#[cfg(unix)]
pub(in crate::source) fn git_helper_path(executor: &GitExecutor) -> OsString {
...
#[cfg(unix)]
pub(in crate::source) fn sealed_ssh_command(ssh_executable: &Path) -> OsString {
...
#[cfg(unix)]
pub(in crate::source) fn null_device() -> &'static str {
    "/dev/null"
}
