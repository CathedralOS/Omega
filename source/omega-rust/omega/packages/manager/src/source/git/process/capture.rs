...
#[derive(Debug)]
pub(in crate::source) struct BoundedCommandOutput {
    pub(in crate::source) status: ExitStatus,
    pub(in crate::source) stdout: Vec<u8>,
    pub(in crate::source) stderr: Vec<u8>,
...
pub(in crate::source) enum StreamCaptureResult {
    Complete(Vec<u8>),
    Overflow,
...
pub(in crate::source) fn run_command_bounded_with_budget(
    command: ResolverPreparedExecution,
    operation: &str,
    stdout_limit: usize,
    stderr_limit: usize,
...
#[cfg(test)]
pub(in crate::source) fn capture_stream_bounded<R>(
    mut reader: R,
    limit: usize,
) -> StreamCaptureResult
where
...
pub(in crate::source) fn command_cleanup_reserve(timeout: Duration) -> Duration {
...
pub(in crate::source) fn duration_millis(duration: Duration) -> u64 {
