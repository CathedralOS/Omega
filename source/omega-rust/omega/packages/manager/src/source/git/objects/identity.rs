...
use crate::source::CheckedSha1;
use crate::source::error::SourceResolveError;
use crate::source::git::format_hex;
use crate::source::identity::GitObjectIdAlgorithm;
use sha2::{Digest, Sha256};

pub(in crate::source) fn verify_git_object_identity(
    expected: &str,
    kind: &[u8],
    payload: &[u8],
...
pub(in crate::source) fn git_object_identity(
    kind: &[u8],
    payload: &[u8],
    algorithm: GitObjectIdAlgorithm,
...
pub(in crate::source) fn finalize_checked_sha1(
    hasher: CheckedSha1,
) -> Result<String, SourceResolveError> {
...
pub(in crate::source) fn git_object_algorithm(
    oid: &str,
...
pub(in crate::source) fn hex_digit(byte: u8) -> Option<u8> {
    match byte {
...
pub(in crate::source) fn is_object_id(value: &str) -> bool {
    matches!(value.len(), 40 | 64) && value.bytes().all(|byte| byte.is_ascii_hexdigit())
}

pub(in crate::source) fn git_object_invalid(
