...
use crate::source::limits::{GIT_LOCATOR_BYTE_LIMIT, GIT_REVISION_BYTE_LIMIT};
...
pub struct GitSourceRequest {
    ...
    pub(in crate::source) fetch_locator: String,
    pub(in crate::source) locator_identity: String,
    pub(in crate::source) requested_revision: String,
    pub(in crate::source) lineage: SourceLineage,
    pub(in crate::source) execution_transport: GitExecutionTransport,
    pub(in crate::source) requested_network_endpoint: GitRequestedNetworkEndpoint,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::source) enum GitExecutionTransport {
    Https,
    Ssh,
    #[cfg(test)]
    File,
...
impl GitExecutionTransport {
    ...
    pub(in crate::source) fn resolver_network_transport(self) -> ResolverExecutionNetworkTransport {
        match self {
    ...
    pub(in crate::source) fn cache_tag(self) -> &'static [u8] {
        match self {
            Self::Https => b"https",
            Self::Ssh => b"ssh",
            #[cfg(test)]
    ...
    pub(in crate::source) fn allowed_protocol(self) -> &'static str {
        match self {
            Self::Https => "https",
            Self::Ssh => "ssh",
            #[cfg(test)]
    ...
    pub(in crate::source) fn permits(self, transport: Self) -> &'static str {
        if self == transport { "always" } else { "never" }
    }

    pub(in crate::source) fn profile(self) -> GitTransportProfile {
        match self {
...
impl GitSourceRequest {
    ...
    pub(in crate::source) fn fetch_locator(&self) -> &str {
        &self.fetch_locator
    }

    pub(in crate::source) fn execution_transport(&self) -> GitExecutionTransport {
