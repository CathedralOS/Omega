...
pub(in crate::source) fn parse_git_tree_entries(
    listing: &[u8],
    repository: &Path,
...
pub(in crate::source) fn git_directory_paths(entries: &[GitTreeEntry]) -> BTreeSet<Vec<u8>> {
    entries
        .iter()
...
pub(in crate::source) fn validate_git_symlink_target(
    link: &[u8],
    target: &[u8],
) -> Result<(), SourceResolveError> {
...
pub(in crate::source) fn git_tree_invalid(
    path: impl AsRef<[u8]>,
