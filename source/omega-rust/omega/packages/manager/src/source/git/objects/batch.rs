...
#[cfg(test)]
pub(in crate::source) fn read_git_blobs_batch_from_path(
    executor: &GitExecutor,
    repository: &Path,
    entries: &mut [GitTreeEntry],
    limits: LocalSourceLimits,
...
pub(in crate::source) fn git_batch_output_limit(
    entries: &[GitTreeEntry],
    limits: LocalSourceLimits,
...
pub(in crate::source) fn assign_git_batch_output(
    entries: &mut [GitTreeEntry],
    output: Vec<u8>,
) -> Result<(), SourceResolveError> {
...
pub(in crate::source) struct PendingGitBatchRequest {
    pub(in crate::source) parent: CapabilityDirectory,
    pub(in crate::source) name: OsString,
    pub(in crate::source) display_path: PathBuf,
    pub(in crate::source) file: Option<File>,
    pub(in crate::source) identity: Option<CapabilityMetadata>,
    pub(in crate::source) removed: bool,
}
...
impl PendingGitBatchRequest {
    ...
    pub(in crate::source) fn verify_current(&self) -> Result<(), SourceResolveError> {
    ...
    pub(in crate::source) fn remove(&mut self) -> Result<(), SourceResolveError> {
        self.verify_current()?;
        drop(self.file.take());
        let named = self
            .parent
