...
pub(in crate::source) fn verify_open_snapshot_tree_modes(
...
pub(in crate::source) fn make_open_snapshot_read_only(
    kind: CacheCustodyKind,
    root: &CapabilityDirectory,
    display_root: &Path,
) -> Result<(), SourceResolveError> {
...
#[cfg(test)]
pub(in crate::source) fn make_snapshot_read_only(root: &Path) -> Result<(), SourceResolveError> {
...
pub(in crate::source) fn make_open_tree_owner_writable(root: &CapabilityDirectory) {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
...
#[cfg(test)]
pub(in crate::source) fn make_tree_owner_writable(root: &Path) {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
