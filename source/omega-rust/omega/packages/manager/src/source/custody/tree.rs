...
pub(in crate::source) enum CacheCustodyKind {
    Git,
    LocalSnapshot,
}

pub(in crate::source) fn read_bounded_cache_record(
...
pub(in crate::source) fn read_bounded_cache_record_from_open_directory(
    kind: CacheCustodyKind,
...
pub(in crate::source) fn verify_git_cache_custody(
    root: &Path,
...
pub(in crate::source) fn verify_git_cache_root_custody(
    root: &Path,
) -> Result<(), SourceResolveError> {
    verify_cache_custody_root(root, CacheCustodyKind::Git)
}

pub(in crate::source) fn verify_local_cache_custody(
    root: &Path,
    limits: LocalSourceLimits,
) -> Result<(), SourceResolveError> {
    verify_cache_custody(
...
#[cfg(test)]
pub(in crate::source) fn verify_local_cache_root_custody(
    root: &Path,
) -> Result<(), SourceResolveError> {
    verify_cache_custody_root(root, CacheCustodyKind::LocalSnapshot)
}

pub(in crate::source) fn verify_cache_custody_root(
    root: &Path,
    kind: CacheCustodyKind,
) -> Result<(), SourceResolveError> {
...
pub(in crate::source) fn git_cache_custody_byte_limit(limits: LocalSourceLimits) -> u64 {
    limits
        .max_bytes
        .saturating_mul(3)
...
pub(in crate::source) fn local_cache_custody_byte_limit(limits: LocalSourceLimits) -> u64 {
    limits
        .max_bytes
        .saturating_add(CACHE_CUSTODY_FIXED_BYTE_ALLOWANCE)
...
pub(in crate::source) fn verify_cache_custody(
    root: &Path,
    kind: CacheCustodyKind,
    byte_limit: u64,
) -> Result<(), SourceResolveError> {
...
pub(in crate::source) fn verify_cache_custody_from_open_root(
    root: &Path,
...
pub(in crate::source) fn cache_custody_has_capacity(observed: usize, pending: usize) -> bool {
    observed
        .checked_add(pending)
        .is_some_and(|retained| retained < CACHE_CUSTODY_ENTRY_LIMIT)
}

pub(in crate::source) fn open_cache_custody_directory(
...
pub(in crate::source) fn cache_custody_invalid(
    kind: CacheCustodyKind,
    path: &Path,
