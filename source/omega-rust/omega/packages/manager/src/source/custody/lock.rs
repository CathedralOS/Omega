...
impl CacheEntryLock {
    ...
    #[cfg(test)]
    pub(in crate::source) fn open_retained(
        kind: CacheCustodyKind,
        path: &Path,
    ) -> Result<(File, CapabilityDirectory, OsString), SourceResolveError> {
    ...
    #[cfg(test)]
    pub(in crate::source) fn open_git(path: &Path) -> Result<File, SourceResolveError> {
        let (file, _, _) = Self::open_retained(CacheCustodyKind::Git, path)?;
        Ok(file)
    }

    #[cfg(test)]
    pub(in crate::source) fn acquire_with_git_budget(
        path: &Path,
    ...
    pub(in crate::source) fn acquire_with_git_budget_from_parent(
        parent_path: &Path,
        retained_parent: &CapabilityDirectory,
        lock_name: &OsStr,
        executor: &GitExecutor,
    ...
    #[cfg(test)]
    pub(in crate::source) fn acquire(path: &Path) -> Result<Self, SourceResolveError> {
        let (file, parent, lock_name) = Self::open_retained(CacheCustodyKind::Git, path)?;
    ...
    #[cfg(test)]
    pub(in crate::source) fn acquire_local(path: &Path) -> Result<Self, SourceResolveError> {
        Self::acquire_local_with_timeout(path, LOCAL_SNAPSHOT_LOCK_TIMEOUT)
    }

    #[cfg(test)]
    pub(in crate::source) fn acquire_local_with_timeout(
        path: &Path,
        timeout: Duration,
    ) -> Result<Self, SourceResolveError> {
    ...
    pub(in crate::source) fn acquire_local_from_parent(
        parent_path: &Path,
        retained_parent: &CapabilityDirectory,
        lock_name: &OsStr,
    ) -> Result<Self, SourceResolveError> {
    ...
    pub(in crate::source) fn acquire_local_from_parent_with_timeout(
        parent_path: &Path,
        retained_parent: &CapabilityDirectory,
        lock_name: &OsStr,
        timeout: Duration,
    ...
    pub(in crate::source) fn parent(&self) -> &CapabilityDirectory {
        &self.parent
    }

    pub(in crate::source) fn verify_path_identity(&self) -> Result<(), SourceResolveError> {
        verify_cache_lock_path_identity(
            self.kind,
            &self.path,
            &self.parent,
...
pub(in crate::source) fn verify_cache_lock_path_identity(
    kind: CacheCustodyKind,
    path: &Path,
...
pub(in crate::source) fn verify_retained_cache_parent_path(
    kind: CacheCustodyKind,
    parent_path: &Path,
    retained_parent: &CapabilityDirectory,
) -> Result<(), SourceResolveError> {
...
pub(in crate::source) fn same_std_and_capability_file_identity(
...
#[cfg(test)]
pub(in crate::source) fn verify_cache_lock_path_identity_for_test(
    kind: CacheCustodyKind,
    path: &Path,
    file: &File,
) -> Result<(), SourceResolveError> {
