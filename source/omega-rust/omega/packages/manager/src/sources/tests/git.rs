...
fn named_git_binding_rejects_missing_and_duplicate_declared_names() {
    ...
    let missing = resolve_selected_git_package_source_with_storage(
        &GitPackageSourceRequest::new(
            ...
            crate::project::PackageSelection::Named(
                crate::package::PackageName::parse("missing").unwrap(),
            ),
        ),
        &storage,
    ...
    assert!(matches!(
        ...
        ResolvePackageSourceError::GitWorkspaceSelection(
            crate::sources::git::workspace::GitWorkspaceSelectionError::PackageMissing {
                package_name
            }
    ...
    let duplicate = resolve_selected_git_package_source_with_storage(
        &GitPackageSourceRequest::new(
            acquisition,
            crate::project::PackageSelection::Named(
                crate::package::PackageName::parse("same-name").unwrap(),
            ),
        ),
        &storage,
        LocalSourceLimits::default(),
    ...
    assert!(matches!(
        duplicate,
        ResolvePackageSourceError::GitWorkspaceSelection(
            crate::sources::git::workspace::GitWorkspaceSelectionError::PackageDuplicate {
                package_name,
                member_paths,
            }
...
fn named_git_binding_rejects_symlink_member_navigation() {
    ...
    let error = resolve_selected_git_package_source_with_storage(
        &GitPackageSourceRequest::new(
            acquisition,
            crate::project::PackageSelection::Named(
                crate::package::PackageName::parse("linked-package").unwrap(),
            ),
        ),
        &storage,
        LocalSourceLimits::default(),
...
"#,
    ...
    .into_custody();
    let error =
        crate::graph::reconcile::resolve_package_source_closure::<std::convert::Infallible, _>(
            crate::graph::PackageRootSourceRequest::ExternalLocal {
