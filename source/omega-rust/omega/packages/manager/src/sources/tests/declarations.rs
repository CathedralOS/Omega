...
use crate::project::dependencies::read::{DependencyProjectionError, DependencySourceRequest};
use crate::project::roles::BuildDeclarationError;
use crate::sources::{ResolvePackageSourceError, resolve_external_local_package_source};
...
fn application_role_cannot_be_bound_as_a_package_source() {
    ...
    assert!(matches!(
        error,
        ResolvePackageSourceError::Declaration(BuildDeclarationError::ExpectedPackageDeclaration {
            found: crate::project::roles::BuildDeclarationKind::Application
        })
    ));
