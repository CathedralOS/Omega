...
use crate::sources::workspace_path::{authored_workspace_member_path, source_relative_path};
use crate::sources::{GitWorkspaceSelectionDeclarations, GitWorkspaceSelectionEvidence};
use omega_package_source::{
    GitWorkspaceDeclaration, GitWorkspaceProjectionPlanner, GitWorkspaceSelection,
};

pub(in crate::sources::git) struct ManagerGitWorkspacePlanner<'a> {
    selected: &'a PackageName,
}

impl<'a> ManagerGitWorkspacePlanner<'a> {
    pub(in crate::sources::git) fn new(selected: &'a PackageName) -> Self {
        Self { selected }
    }
}
