...
#[test]
fn baseline_retains_unknown_native_handle_mutation_replay_custody() {
    let fixtures = [
        (
            "unknown-set-file-time",
    ...
    let status: i32 = builder.filesystem.lock_file_ex(-1, 1, 0, 4294967295, 4294967295, &mut overlapped);"#,
        ...
        (
            "unknown-unlock-file",
            "let status: i32 = builder.filesystem.unlock_file(-1, 3, 5, 7, 11);",
            34,
        ),
    ];

    for (label, statements, tag) in fixtures {
        let replay = unknown_descriptor_failure_baseline(label, statements);
        let [attempt] = replay.attempts() else {
            panic!("native mutation baseline retains one operation")
        };
        ...
        assert_eq!(attempt.post_error(), Some(6));
        let [handle] = attempt.logical_handle_inputs() else {
            panic!("native mutation baseline retains one native-handle input")
        };
        assert_eq!(
            handle.kind(),
            psi_checked_interpreter::FilesystemLogicalHandleKind::Native
        ...
        match tag {
            32 => {
                let [access, write] = attempt.byte_operands() else {
                    panic!("set_file_time baseline retains both byte inputs")
                };
                assert_eq!(access.bytes().len(), 8);
                assert_eq!(access.bytes()[0], 11);
                assert_eq!(write.bytes().len(), 12);
                assert_eq!(write.bytes()[11], 29);
            }
            33 => {
                let [resolution] = attempt.mutable_byte_operand_resolutions() else {
                    panic!("lock_file_ex baseline retains one mutable resolution")
                };
                let [carrier] = attempt.mutable_byte_operands() else {
                    panic!("lock_file_ex baseline retains one mutable carrier")
                };
                assert_eq!(resolution.bytes().len(), 40);
                assert_eq!(resolution.bytes()[0], 41);
                assert_eq!(resolution.bytes()[39], 173);
                assert_eq!(resolution.bytes(), carrier.pre_bytes());
                assert_eq!(carrier.pre_bytes(), carrier.post_bytes());
            }
            34 => {
