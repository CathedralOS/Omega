...
use crate::discovery::workspace_path::{authored_workspace_member_path, source_relative_path};
use crate::discovery::{GitWorkspaceSelectionDeclarations, GitWorkspaceSelectionEvidence};
use crate::identity::PackageName;
use omega_package_source::{
    GitWorkspaceDeclaration, GitWorkspaceProjectionPlanner, GitWorkspaceSelection,
};

pub(in crate::discovery::git) struct ManagerGitWorkspacePlanner<'a> {
    selected: &'a PackageName,
}

impl<'a> ManagerGitWorkspacePlanner<'a> {
    pub(in crate::discovery::git) fn new(selected: &'a PackageName) -> Self {
        Self { selected }
    }
}
