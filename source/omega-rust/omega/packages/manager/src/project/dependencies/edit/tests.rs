...
use crate::project::dependencies::edit::planning::{
...
fn rejects_dependency_edits_without_an_explicit_project_role() {
    ...
    assert!(matches!(
        ...
        )) if matches!(*error, crate::project::roles::BuildDeclarationError::MissingBuildDeclaration)
    ));
}
...
fn generated_rows_escape_all_caller_controlled_strings() {
    let request = DependencySourceRequest::Git {
        ...
        selection: PackageSelection::Named(
            crate::package::PackageName::parse("selected-package").unwrap(),
        ),
    };
