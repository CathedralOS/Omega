...
use omega_abstract_operations::{CompletionClaimSource, ValueBinding};
...
use omega_target_operations::{ClaimCompletionOnlyRealization, ProviderExecutionBinding};
...
fn encode_boundary_settlement(bytes: &mut Vec<u8>, settlement: &LegalizedBoundarySettlement) {
...
fn encode_call_source(bytes: &mut Vec<u8>, source: &LegalizedCallUnitSource) {
    match source {
...
fn encode_target_structural_parameter(
    bytes: &mut Vec<u8>,
    parameter: &omega_target_operations::TargetStructuralParameter,
) {
    bytes.extend_from_slice(&parameter.place.get().to_le_bytes());
    bytes.extend_from_slice(&parameter.structural_type.get().to_le_bytes());
    encode_multiplicity(bytes, parameter.multiplicity);
...
fn encode_leaf(bytes: &mut Vec<u8>, leaf: &LegalizedLeaf) {
...
fn encode_exact_add(bytes: &mut Vec<u8>, add: &LegalizedExactAdd) {
...
fn encode_immediate(bytes: &mut Vec<u8>, immediate: &LegalizedImmediate) {
...
fn encode_bindings(bytes: &mut Vec<u8>, bindings: &[ValueBinding]) {
...
mod tests {
    ...
    fn call_aware_plan() -> LegalizedOperationPlan {
    ...
    fn installed_provider_plan() -> LegalizedOperationPlan {
