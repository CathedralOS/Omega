...
pub(super) struct RetainedGeneratedSyntaxExtension {
    /// Raw parsed roots retained exclusively for the unchanged whole-program
    /// rebuild fallback.
    syntax_trees: SyntaxTrees,
    /// Each generated unit normalized independently before seeded resolution.
    /// Keeping this separate prevents the continuation from inventing
    /// cross-unit generic instances while preserving the exact raw fallback.
    seeded_syntax_trees: SyntaxTrees,
...
impl RetainedGeneratedSyntaxExtension {
    ...
    pub(super) fn is_nonempty_data_only(&self) -> bool {
        self.seeded_syntax_trees.root_item_count() > 0
            && self
                .seeded_syntax_trees
                .root_items()
    ...
    pub(super) fn seeded_inputs(
        ...
        let retained_root_count = self.units.iter().try_fold(0usize, |count, unit| {
            count
                .checked_add(unit.seeded_root_item_count)
                .ok_or_else(|| {
                    vec![Diagnostic::error(
                        "normalized generated-source root count exceeds the compiler range",
                    )]
                })
        })?;
        if retained_root_count != self.seeded_syntax_trees.root_item_count() {
            return Err(vec![Diagnostic::error(
                "normalized generated-source roots do not match their source units",
            )]);
        }
        Ok((&self.seeded_syntax_trees, self.sources.clone()))
    }
...
pub(super) fn retain_generated_syntax_extension(
    ...
    let mut seeded_syntax_trees = SyntaxTrees::new(psi_source::SourceId(base_source_count));
    ...
    for generated in generated_sources {
        ...
        let normalized = psi_generic_instances::normalize_pre_resolution(parsed.clone())?;
        let seeded_root_item_count = normalized.root_item_count();
        syntax_trees.extend_from(&parsed);
        seeded_syntax_trees.extend_from(&normalized);
...
mod tests {
    ...
    fn retained_generated_syntax_extension_rejoins_units_without_reparsing() {
        ...
        let mut seeded_extension_syntax = SyntaxTrees::new(psi_source::SourceId(1));
        ...
        for (offset, (path, text)) in extension_inputs.iter().enumerate() {
            ...
            let normalized = psi_generic_instances::normalize_pre_resolution(parsed.clone())
                .expect("normalize extension source");
            extension_units.push(RetainedGeneratedSyntaxUnit {
                source_id,
                path: PathBuf::from(path),
                root_item_count: parsed.root_item_count(),
                seeded_root_item_count: normalized.root_item_count(),
            });
            extension_syntax.extend_from(&parsed);
            seeded_extension_syntax.extend_from(&normalized);
        ...
        let extension = RetainedGeneratedSyntaxExtension {
            ...
            seeded_syntax_trees: seeded_extension_syntax,
    ...
    #[test]
    fn generated_unit_normalization_is_seeded_only_and_unit_local() {
        ...
        let generated =
            "data Cell<T> { value: T; } data Generated { left: Cell<u32>; right: Cell<u32>; }";
        let tokens = crate::lexer::Lexer::new(generated)
            .tokenize()
            .expect("lex generated unit");
        let parsed = crate::parser::parse_syntax_trees_with_id(psi_source::SourceId(1), &tokens)
            .expect("parse generated unit once");
        let normalized = psi_generic_instances::normalize_pre_resolution(parsed.clone())
            .expect("normalize generated unit clone");
        assert_eq!(parsed.root_item_count(), 2);
        assert_eq!(normalized.root_item_count(), 3);
        assert_eq!(
            normalized
                .root_items()
                .filter(|item| matches!(
                ...
                .count(),
            1,
            "repeated same-unit uses deduplicate to one instance"
        );
        let mut sources = (*base_sources).clone();
        sources.add_with_metadata_and_resolution_stratum(
            PathBuf::from(".omega/generated/local.omg"),
            generated.to_owned(),
            PathBuf::from("."),
            None,
            psi_source::SourceOrigin::User,
        ...
        let extension = RetainedGeneratedSyntaxExtension {
            syntax_trees: parsed,
            seeded_syntax_trees: normalized,
            units: vec![RetainedGeneratedSyntaxUnit {
                source_id: psi_source::SourceId(1),
                path: PathBuf::from(".omega/generated/local.omg"),
                root_item_count: 2,
                seeded_root_item_count: 3,
            }],
            sources: Arc::new(sources),
            generated_source_custody: Vec::new(),
            base_sources,
        };
        let (seeded, _) = extension
            .seeded_inputs(&assembled)
            .expect("seeded route receives normalized roots");
        assert_eq!(seeded.root_item_count(), 3);
        extension
            .append_to(&mut assembled)
            .expect("fallback consumes the raw parsed roots");
        assert_eq!(assembled.syntax_trees.root_item_count(), 3);

        let template = "data Split<T> { value: T; }";
        let wrapper = "data SplitUse { value: Split<u32>; }";
        let template = crate::parser::parse_syntax_trees_with_id(
            psi_source::SourceId(2),
            &crate::lexer::Lexer::new(template)
                .tokenize()
                .expect("lex split template"),
        )
        .expect("parse split template");
        let wrapper = crate::parser::parse_syntax_trees_with_id(
            psi_source::SourceId(3),
            &crate::lexer::Lexer::new(wrapper)
                .tokenize()
                .expect("lex split wrapper"),
        )
        .expect("parse split wrapper");
        assert_eq!(
            psi_generic_instances::normalize_pre_resolution(template.clone())
                .expect("normalize template unit")
                .root_item_count(),
            1
        );
        assert_eq!(
            psi_generic_instances::normalize_pre_resolution(wrapper.clone())
                .expect("normalize wrapper unit")
                .root_item_count(),
            1,
            "a cross-unit application cannot synthesize in the seeded candidate"
        );
        let mut combined = template;
        combined.extend_from(&wrapper);
        assert_eq!(
            psi_generic_instances::normalize_pre_resolution(combined)
                .expect("the unchanged combined fallback can normalize cross-unit source")
                .root_item_count(),
            3
        );
    }
