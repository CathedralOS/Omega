...
#[test]
fn generated_base_owned_primitive_application_preserves_build_symbol_and_source_custody() {
    let profile = omega_target::TargetProfile::host();
...
"#,
    ...
    .expect(
        "the exact base-owned generated application should continue from the retained frontend",
    );
    set_canonical_source_tree_permissions(&project.root, false);
