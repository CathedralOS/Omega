...
use psi_terminal::{StructuralPathQualification, StructuralPathSegment};
...
fn machine_abi_and_optimizer_node_corruption_fail_closed() {
    ...
    machine.projected_structural_call_returns[0].callee.machine =
        psi_core::MachineId::new(99).unwrap();
...
fn rows(
    closure: &mut omega_legalized_operations::LegalizedProjectedStructuralCallReturn,
    location: Location,
...
fn mutate(rows: &mut Vec<StructuralPathQualification>, mutation: Mutation) {
    match mutation {
        ...
        Mutation::WrongDomain => rows[0].domain = psi_core::StructuralDomainId::new(99).unwrap(),
