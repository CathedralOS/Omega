//! Independent replay of ordered U64 entry-parameter equality custody.
...
pub(super) fn replay<'a>(
    ...
    let expected_fuel = equal_node
        .fuel
        .iter()
        .copied()
        .filter(|settlement| settlement.site == PsiProvenance::Operation(*psi_operation))
        .collect::<Vec<_>>();
    if expected_fuel.is_empty() || expected_fuel.len() != equal_node.fuel.len() {
