...
use super::{
    ...
    active_resident_exact_add_chain, active_resident_exact_add_original_victim_chain,
    direct_exact_binary,
};
...
pub(in crate::legalization::source) use active_resident_exact_add_original_victim_chain::is_active_resident_exact_add_original_victim_chain;
...
pub(super) fn derive<'a>(
    ...
    if is_active_resident_exact_add_original_victim_chain(expression) {
        active_resident_exact_add_original_victim_chain::derive(context, expression)
    } else if is_active_resident_exact_add_bridge_chain(expression) {
