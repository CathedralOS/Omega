//! Producer-only recognition of cataloged scalar source forms.

use crate::legalization::catalog::{
    SCALAR_LEGALIZATION_FORMS, ScalarLegalizationFormDescriptor, ScalarLegalizationMatcherKind,
};

use super::leaves::is_active_resident_exact_add_chain;
use super::shared::*;

pub(super) fn match_scalar_form(
    when_true: &TargetIntegerControl,
    when_false: &TargetIntegerControl,
) -> Option<&'static ScalarLegalizationFormDescriptor> {
    let mut matches = SCALAR_LEGALIZATION_FORMS
        .iter()
        .filter(|descriptor| matcher_accepts(descriptor.producer_matcher, when_true, when_false));
