//! Pressure-bearing active-resident exact-add bridge chain paired with one false-arm immediate.
...
pub(super) fn build(
    ...
    let SourceLeafValue::ActiveResidentExactAddBridgeChain(chain) = &context.source.when_true.value
    else {
        unreachable!("catalog selected the active-resident bridge family")
    };
    ...
    for (id, instruction, source_value, definition_site) in [
        ...
        (8, 10, context.source.when_false.source_value, *false_site),
    ] {
    ...
    Ok(ConstructedScalarBody {
        ...
        blocks: vec![
            ...
            blocks::active_resident_exact_add_bridge_chain(
                context,
                SelectedBlockId(1),
