//! Guarded-original exact-add block construction.
...
pub(in crate::selection::construction::scalar) fn active_resident_exact_add_original_victim_chain(
    context: &ScalarConstructionContext<'_>,
    id: SelectedBlockId,
    source_block: psi_core::BlockId,
    source: &SourceLeaf,
) -> Result<SelectedBlock, SelectedInstructionError> {
    let SourceLeafValue::ActiveResidentExactAddOriginalVictimChain(chain) = &source.value else {
