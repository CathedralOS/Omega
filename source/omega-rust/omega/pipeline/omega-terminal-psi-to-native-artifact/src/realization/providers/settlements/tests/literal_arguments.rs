...
#[test]
fn exact_rejoined_three_literal_import_reaches_dynamic_elf_on_both_targets() {
    ...
    let call_operation = psi_core::OperationId::new(813).unwrap();
        ...
        functions: vec![omega_abstract_operations::AbstractFunction {
            ...
            operations: vec![
                ...
                omega_abstract_operations::AbstractOperation::BoundaryCall {
                    ...
                    result: None,
                    boundary,
                    arguments: vec![first_value, second_value, third_value],
    ...
    for profile in [
        ...
        selected_plan.schema.methods[0].parameter_count = 3;
        selected_plan.schema.methods[0].parameter_type_identities =
            vec!["u16".into(), "i64".into(), "u32".into()];
        ...
        let omega_target_operations::TargetUnitOperation::NormalizedForeignCall {
            scalar_arguments,
            ..
        } = &target_body.operations[3]
        else {
            panic!("literal import remains a normalized foreign call")
        };
        let [first_target, second_target, third_target] = scalar_arguments.as_slice() else {
            panic!("three target arguments")
        };
        ...
        let mutate_third =
            |mut candidate: omega_assigned_target_operations::AssignedOperationPlan,
             mutate: &dyn Fn(&mut omega_target_operations::NormalizedForeignScalarArgument)| {
                ...
                mutate(&mut scalar_arguments[2]);
                assert!(omega_machine_emission::emit_machine_code(&candidate).is_err());
            };
        mutate_third(assigned.clone(), &|argument| {
        ...
        let mut fourth_assignment = assigned.clone();
        let omega_assigned_target_operations::AssignedOperation::UnitBody(body) =
            &mut fourth_assignment.functions[0].operation
        else {
            unreachable!()
        };
        let omega_assigned_target_operations::AssignedUnitOperation::NormalizedForeignCall {
        ...
        scalar_arguments.push(third_target.clone());
        assert!(omega_machine_emission::emit_machine_code(&fourth_assignment).is_err());
        ...
        let [first_argument, second_argument, third_argument] = call.scalar_arguments.as_slice()
        else {
            panic!("three retained machine arguments")
        };
        ...
        changed_value.functions[0].foreign_calls[0].scalar_arguments[2].immediate =
            psi_core::IntegerValue::Unsigned(0x1234_5679);
        ...
        changed_bytes.functions[0].bytes[third_argument.code_offset] ^= 1;
        ...
        changed_interval.functions[0].foreign_calls[0].scalar_arguments[2].code_offset -= 1;
        assert!(omega_image_emission::build_object_artifact(&changed_interval).is_err());
        let mut changed_register = machine_code.clone();
        changed_register.functions[0].foreign_calls[0].scalar_arguments[2]
            .placement
        ...
        let mut fourth_argument = machine_code.clone();
        let extra = fourth_argument.functions[0].foreign_calls[0].scalar_arguments[2].clone();
        fourth_argument.functions[0].foreign_calls[0]
            .scalar_arguments
            .push(extra);
        assert!(omega_image_emission::build_object_artifact(&fourth_argument).is_err());
