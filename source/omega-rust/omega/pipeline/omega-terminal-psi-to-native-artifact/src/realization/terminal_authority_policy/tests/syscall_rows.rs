...
fn syscall_classification_requires_exact_profile_number_and_checked_contract() {
    let exact = syscall(omega_target::TargetProfile::LinuxX64, 0, 1);
    ...
    assert!(
        policy
            .classify(syscall(omega_target::TargetProfile::LinuxArm64, 0, 1))
            .is_err()
    );
    assert!(
        policy
            .classify(syscall(omega_target::TargetProfile::LinuxX64, 1, 1))
            .is_err()
    );
    assert!(
        policy
            .classify(syscall(omega_target::TargetProfile::LinuxX64, 0, 2))
            .is_err()
    );
...
fn syscall_rows_enter_policy_identity_and_reject_duplicates() {
    let exact = syscall(omega_target::TargetProfile::LinuxX64, 1, 3);
...
fn empty_checked_syscall_contract_rejects() {
    let empty = syscall(omega_target::TargetProfile::LinuxX64, 0, 0);
    assert_eq!(
...
fn unsupported_syscall_target_rejects_before_classification() {
    let windows = syscall(omega_target::TargetProfile::WindowsX64, 0, 1);
    assert_eq!(
