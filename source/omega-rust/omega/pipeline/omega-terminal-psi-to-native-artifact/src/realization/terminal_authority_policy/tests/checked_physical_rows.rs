...
use omega_effects::{CheckedPhysicalTerminalMechanismIdentity, TerminalAuthorityClass};

use super::*;

fn port_write(target: omega_target::TargetProfile, port: u16) -> TerminalMechanismIdentity {
...
fn checked_port_write_classifies_only_by_exact_profile_and_port() {
    let exact = port_write(omega_target::TargetProfile::LinuxX64, 0x03f8);
    let policy =
