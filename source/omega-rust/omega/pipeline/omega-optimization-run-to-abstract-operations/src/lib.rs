...
//! Custody-preserving projection from one completed optimization run to
//! executable abstract operations.
//!
//! This is one pipeline boundary with one input and one output. It reifies the
//! final optimizer unit into executable abstract-plan shape and keeps that
//! shape inseparable from its verifier context, candidate replay,
//! transformation ledger, and independent projection-validation receipt. It
//! does not select a target, invoke later lowering, allocate registers, or own
...
mod tests {
    ...
    struct TestLoweredOptimizedTargetOperations {
    ...
    impl TestLoweredOptimizedTargetOperations {
        fn optimized(&self) -> &ValidatedOptimizedAbstractPlan {
            &self.optimized
        }
        ...
        fn target_operations(&self) -> &omega_target_operations::TargetOperationPlan {
            &self.target_operations
        }
    }

    fn lower_optimized_to_target_operations(
        optimized: ValidatedOptimizedAbstractPlan,
        target: NativeTarget,
    ) -> Result<
