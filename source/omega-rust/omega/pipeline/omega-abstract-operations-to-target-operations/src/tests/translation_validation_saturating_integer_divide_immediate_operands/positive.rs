        ...
        for scalar_type in native_fixed_types() {
            for (left, right) in boundary_pairs(scalar_type) {
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("constant-operand saturating divide must publish only its exact family")
                };
                assert_eq!(row.machine(), machine());
                assert_eq!(
                    row.left_constant_operation(),
                    OperationId::new(85_003).unwrap()
                );
                assert_eq!(
                    row.right_constant_operation(),
                    OperationId::new(85_005).unwrap()
                );
                assert_eq!(row.divide_operation(), OperationId::new(85_007).unwrap());
                assert_eq!(row.obligation(), ObligationId::new(85_011).unwrap());
                assert_eq!(row.return_edge(), EdgeId::new(85_009).unwrap());
                assert_eq!(row.left_constant_result(), ValueId::new(85_004).unwrap());
                assert_eq!(row.right_constant_result(), ValueId::new(85_006).unwrap());
                assert_eq!(row.divide_result(), ValueId::new(85_008).unwrap());
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerExpression {
                        ...
                        ..
                    } if *source_value == ValueId::new(85_008).unwrap()
                        ...
                        && matches!(
                            ...
                            TargetIntegerExpression::Immediate { source_value, value }
                                if *source_value == ValueId::new(85_004).unwrap() && *value == left
                        )
                        && matches!(
                            target_right.as_ref(),
                            TargetIntegerExpression::Immediate { source_value, value }
                                if *source_value == ValueId::new(85_006).unwrap() && *value == right
                        )
                ));
            }
        }
...
fn classifier_is_disjoint_from_plain_parameter_and_other_division_policies() {
    ...
    *value = ValueId::new(85_004).unwrap();
    assert!(
        !crate::validation::straight_line_saturating_integer_divide_immediate_operands::is_candidate(
            &plain.functions[0]
        )
    );
    ...
    for replacement in [
        ...
        assert!(!crate::validation::straight_line_saturating_integer_divide_immediate_operands::is_candidate(
            &adjacent.functions[0]
        ));
    }

    let parameter =
        super::super::parameter_translation_fixture::saturating_integer_divide_parameters_plan(
            &[
