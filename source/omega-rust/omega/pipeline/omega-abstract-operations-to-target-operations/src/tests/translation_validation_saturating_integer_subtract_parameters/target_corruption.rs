...
#[test]
fn saturating_integer_subtract_target_provenance_corruption_fails_closed() {
    assert_eq!(
        candidate_error(|target| target.functions[0].provenance.operations.clear()),
        StraightLineSaturatingIntegerSubtractParametersTranslationError::TargetProvenance
    );
    assert_eq!(
...
fn saturating_integer_subtract_target_outer_expression_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingSubtract { psi_operation, .. } =
            expression(operation)
        else {
            unreachable!()
        };
        *psi_operation = OperationId::new(49_610).unwrap();
    });
}

#[test]
fn saturating_integer_subtract_target_nested_operand_corruption_fails_closed() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingSubtract { left, .. } = expression(operation) else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingSubtract { right, .. } = expression(operation)
        else {
            unreachable!()
        };
        ...
        *source_value = ValueId::new(49_611).unwrap();
    });
    for mutate_left in [true, false] {
        assert_target_operation_error(|operation| {
            let TargetIntegerExpression::SaturatingSubtract { left, right, .. } =
                expression(operation)
            else {
                unreachable!()
            };
...
#[test]
fn saturating_integer_subtract_rejects_swapped_operands_and_semantic_substitutions() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::SaturatingSubtract { left, right, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    for substitute in [1, 2, 3, 4] {
        assert_target_operation_error(|operation| {
            ...
            let TargetIntegerExpression::SaturatingSubtract {
                psi_operation,
                left,
                right,
            } = expression
            ...
            *expression = match substitute {
                ...
                2 => TargetIntegerExpression::ExactSubtract {
                    psi_operation: *psi_operation,
                    obligation: ObligationId::new(49_612).unwrap(),
                    left: left.clone(),
                    right: right.clone(),
                },
