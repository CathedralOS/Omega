...
pub(in crate::tests) fn exact_integer_divide_parameters_plan(
    ...
    let ScalarType::Integer(scalar_type) = function.parameters[left_parameter].scalar_type else {
        panic!("exact-integer-divide fixture requires integer operands")
    };
    let result = ValueId::new(5_701).unwrap();
    ...
    function.operations.insert(
        ...
        AbstractOperation::ExactIntegerDivide {
            psi_operation: OperationId::new(5_700).unwrap(),
            obligation: ObligationId::new(5_702).unwrap(),
            result,
            scalar_type,
            left,
            right,
...
pub(in crate::tests) fn uniform_exact_integer_divide_plan(
    integer: IntegerType,
    parameter_count: usize,
) -> AbstractOperationPlan {
    exact_integer_divide_parameters_plan(
