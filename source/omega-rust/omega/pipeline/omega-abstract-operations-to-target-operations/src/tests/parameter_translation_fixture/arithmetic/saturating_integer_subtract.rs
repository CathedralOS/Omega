...
pub(in crate::tests) fn saturating_integer_subtract_parameters_plan(
    ...
    let ScalarType::Integer(scalar_type) = function.parameters[left_parameter].scalar_type else {
        panic!("saturating-integer-subtract fixture requires integer operands")
    };
    let result = ValueId::new(4_941).unwrap();
    ...
    function.operations.insert(
        ...
        AbstractOperation::SaturatingIntegerSubtract {
            psi_operation: OperationId::new(4_940).unwrap(),
            result,
            scalar_type,
            left,
            right,
...
pub(in crate::tests) fn uniform_saturating_integer_subtract_plan(
    integer: IntegerType,
    parameter_count: usize,
) -> AbstractOperationPlan {
    saturating_integer_subtract_parameters_plan(
