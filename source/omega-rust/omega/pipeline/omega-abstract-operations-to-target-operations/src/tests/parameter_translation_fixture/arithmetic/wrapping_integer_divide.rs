...
pub(in crate::tests) fn wrapping_integer_divide_parameters_plan(
    ...
    let ScalarType::Integer(scalar_type) = function.parameters[left_parameter].scalar_type else {
        panic!("wrapping-integer-divide fixture requires integer operands")
    };
    let result = ValueId::new(6_101).unwrap();
    ...
    function.operations.insert(
        ...
        AbstractOperation::WrappingIntegerDivide {
            psi_operation: OperationId::new(6_100).unwrap(),
            obligation: ObligationId::new(6_102).unwrap(),
            result,
            scalar_type,
            left,
            right,
...
pub(in crate::tests) fn uniform_wrapping_integer_divide_plan(
    integer: IntegerType,
    parameter_count: usize,
) -> AbstractOperationPlan {
    wrapping_integer_divide_parameters_plan(
