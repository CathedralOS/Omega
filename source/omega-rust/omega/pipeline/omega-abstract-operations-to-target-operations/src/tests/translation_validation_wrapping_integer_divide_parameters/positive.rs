use super::{integer_type, wrapping_integer_divide_parameters_plan};
use crate::{
...
#[test]
fn validates_all_fixed_wrapping_integer_divide_types_and_operand_placements_on_every_target() {
    let targets = [
        ...
        for scalar_type in &integers {
            for (left, right) in operand_pairs {
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineWrappingIntegerDivideParameters(row),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("wrapping-integer-divide must publish its family receipt")
                };
                assert_eq!(row.machine(), source.entry);
                assert_eq!(row.divide_operation(), OperationId::new(6_100).unwrap());
                assert_eq!(row.obligation(), ObligationId::new(6_102).unwrap());
                assert_eq!(row.return_edge(), EdgeId::new(3_004).unwrap());
                assert_eq!(row.source_value(), ValueId::new(6_101).unwrap());
...
#[test]
fn wrapping_integer_divide_retains_obligation_order_and_mixed_roster_custody() {
    ...
    for (left, right) in [(2, 1), (1, 1)] {
        ...
        let AbstractToTargetFunctionTranslationDisposition::Validated(
            ...
            panic!("wrapping-integer-divide must retain ordered operand and obligation custody")
        };
        assert_eq!(row.obligation(), ObligationId::new(6_102).unwrap());
        assert_eq!(row.left_parameter_index(), left);
        assert_eq!(row.right_parameter_index(), right);
