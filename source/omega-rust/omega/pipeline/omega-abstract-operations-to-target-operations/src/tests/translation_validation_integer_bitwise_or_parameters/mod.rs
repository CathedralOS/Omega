use super::parameter_translation_fixture::{
    integer_bitwise_or_parameters_plan, integer_type, uniform_integer_bitwise_or_plan,
};
use super::*;
use crate::{
...
fn leaf_error(
    ...
    let mut source = uniform_integer_bitwise_or_plan(integer_type(IntegerSign::Signed, 32), 2);
    let target_profile = NativeTarget::linux_x64();
    let target = lower_to_target_operations(&source, target_profile).unwrap();
    mutate(&mut source.functions[0]);
    crate::validation::straight_line_parameter::integer::bitwise::bitwise_or::validate(
        &source.functions[0],
        target_profile,
        &target.functions[0],
    )
...
fn candidate_error(
    ...
    let source = uniform_integer_bitwise_or_plan(integer_type(IntegerSign::Unsigned, 64), 2);
    let target_profile = NativeTarget::linux_x64();
    ...
    let AbstractToTargetTranslationValidationError::FunctionFamily {
        family: AbstractToTargetTranslationFamily::StraightLineIntegerBitwiseOrParameters,
        error: AbstractToTargetTranslationFamilyError::StraightLineIntegerBitwiseOrParameters(error),
        ..
    } = validate_abstract_to_target_translation(&source, target_profile, &target).unwrap_err()
    else {
        panic!("integer bitwise-OR corruption must fail at its independent validator")
    };
    error
}
