...
#[test]
fn validates_all_native_integer_bitwise_and_types_registers_and_stack_on_every_target() {
    let targets = [
    ...
    for target_profile in targets {
        for scalar_type in &integers {
            for parameter_count in [2, 10] {
                let source = uniform_integer_bitwise_and_plan(*scalar_type, parameter_count);
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("integer bitwise-AND must publish its family receipt")
                };
                assert_eq!(row.machine(), source.entry);
                assert_eq!(row.and_operation(), OperationId::new(4_500).unwrap());
                assert_eq!(row.return_edge(), EdgeId::new(3_004).unwrap());
                assert_eq!(row.source_value(), ValueId::new(4_501).unwrap());
...
#[test]
fn integer_bitwise_and_retains_order_identity_and_mixed_roster_custody() {
    let integer = integer_type(IntegerSign::Unsigned, 32);
    for (left, right) in [(2, 1), (1, 1)] {
        let source = integer_bitwise_and_parameters_plan(
            &[
                ScalarType::Boolean,
        ...
        let AbstractToTargetFunctionTranslationDisposition::Validated(
            AbstractToTargetFunctionTranslationReceipt::StraightLineIntegerBitwiseAndParameters(
                row,
            ),
        ) = receipt.function_roster()[0].translation()
        else {
            panic!("integer bitwise-AND must retain ordered operand custody")
        };
