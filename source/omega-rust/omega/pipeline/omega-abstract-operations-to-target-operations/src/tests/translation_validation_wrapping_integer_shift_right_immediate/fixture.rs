...
pub(super) fn leaf_error(
    ...
    crate::validation::straight_line_wrapping_integer_shift_right_immediate::validate(
        &source.functions[0],
        &target.functions[0],
    )
    .unwrap_err()
...
pub(super) fn candidate_error(
    ...
    let AbstractToTargetTranslationValidationError::FunctionFamily {
        family: AbstractToTargetTranslationFamily::StraightLineWrappingIntegerShiftRightImmediate,
        error:
        ...
        panic!("constant wrapping integer shift-right corruption must fail in its exact validator")
    };
    error
}
