use super::{exact_integer_multiply_parameters_plan, integer_type};
use crate::{
...
#[test]
fn validates_all_fixed_exact_integer_multiply_types_and_operand_placements_on_every_target() {
    let targets = [
    ...
    for target_profile in targets {
        for scalar_type in &integers {
            for (left, right) in operand_pairs {
                let source = exact_integer_multiply_parameters_plan(
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("exact-integer-multiply must publish its family receipt")
                };
                assert_eq!(row.machine(), source.entry);
                assert_eq!(row.multiply_operation(), OperationId::new(5_500).unwrap());
                assert_eq!(row.obligation(), ObligationId::new(5_502).unwrap());
                assert_eq!(row.return_edge(), EdgeId::new(3_004).unwrap());
                assert_eq!(row.source_value(), ValueId::new(5_501).unwrap());
...
#[test]
fn exact_integer_multiply_retains_obligation_order_and_mixed_roster_custody() {
    ...
    for (left, right) in [(2, 1), (1, 1)] {
        ...
        let AbstractToTargetFunctionTranslationDisposition::Validated(
            ...
            panic!("exact-integer-multiply must retain ordered operand and obligation custody")
        };
        assert_eq!(row.obligation(), ObligationId::new(5_502).unwrap());
        assert_eq!(row.left_parameter_index(), left);
        assert_eq!(row.right_parameter_index(), right);
