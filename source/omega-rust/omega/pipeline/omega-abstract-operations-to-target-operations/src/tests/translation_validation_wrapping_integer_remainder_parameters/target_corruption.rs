...
#[test]
fn wrapping_integer_remainder_target_provenance_corruption_fails_closed() {
    assert_eq!(
...
#[test]
fn wrapping_integer_remainder_target_outer_expression_and_obligation_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingRemainder { psi_operation, .. } =
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingRemainder { obligation, .. } = expression(operation)
        else {
            unreachable!()
        };
...
#[test]
fn wrapping_integer_remainder_target_nested_operand_corruption_fails_closed() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingRemainder { left, .. } = expression(operation) else {
            unreachable!()
        };
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingRemainder { right, .. } = expression(operation) else {
            unreachable!()
        };
        let TargetIntegerExpression::Parameter {
            parameter_index, ..
    ...
    for mutate_left in [true, false] {
        assert_target_operation_error(|operation| {
            let TargetIntegerExpression::WrappingRemainder { left, right, .. } =
                expression(operation)
            else {
                unreachable!()
            };
...
#[test]
fn wrapping_integer_remainder_rejects_swapped_operands_and_semantic_substitutions() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingRemainder { left, right, .. } = expression(operation)
        else {
            unreachable!()
        };
    ...
    for substitute in 1..=14 {
        assert_target_operation_error(|operation| {
            ...
            let TargetIntegerExpression::WrappingRemainder {
                psi_operation,
                obligation,
                left,
                right,
            ...
            *expression = match substitute {
                ...
                3 => TargetIntegerExpression::SaturatingDivide {
                    psi_operation: *psi_operation,
                    obligation: *obligation,
                    left: left.clone(),
                    right: right.clone(),
                },
                4 => TargetIntegerExpression::ExactRemainder {
                    psi_operation: *psi_operation,
                    obligation: *obligation,
                    left: left.clone(),
                    right: right.clone(),
