...
fn target_provenance_corruption_fails_closed() {
    assert_eq!(
        ...
        StraightLineSaturatingIntegerMultiplyImmediateTranslationError::TargetProvenance
    );
    assert_eq!(
...
fn every_target_immediate_axis_fails_closed() {
    for mutate in [
        |operation: &mut TargetOperation| {
            ...
            *psi_edge = EdgeId::new(82_111).unwrap();
        },
        |operation: &mut TargetOperation| {
            let TargetOperation::ReturnIntegerImmediate { source_value, .. } = operation else {
                unreachable!()
            };
            *source_value = ValueId::new(82_112).unwrap();
        },
        |operation: &mut TargetOperation| {
        ...
        |operation: &mut TargetOperation| {
            *operation = TargetOperation::ReturnIntegerExpression {
                psi_edge: EdgeId::new(82_009).unwrap(),
                source_value: ValueId::new(82_008).unwrap(),
                scalar_type: scalar_type(),
                expression: TargetIntegerExpression::SaturatingMultiply {
                    psi_operation: OperationId::new(82_007).unwrap(),
                    left: Box::new(TargetIntegerExpression::Immediate {
                        source_value: ValueId::new(82_004).unwrap(),
                        value: IntegerValue::Unsigned(65_535),
                    }),
                    right: Box::new(TargetIntegerExpression::Immediate {
                        source_value: ValueId::new(82_006).unwrap(),
        ...
        assert_eq!(
            ...
            StraightLineSaturatingIntegerMultiplyImmediateTranslationError::TargetOperation
        );
    }
}
