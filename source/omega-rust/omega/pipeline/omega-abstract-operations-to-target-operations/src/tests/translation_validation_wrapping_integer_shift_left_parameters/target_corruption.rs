...
#[test]
fn wrapping_shift_left_target_provenance_machine_and_attachment_corruption_fails_closed() {
    assert_eq!(
    ...
    target.functions[0].machine = psi_core::MachineId::new(53_600).unwrap();
    assert_eq!(
    ...
    let mut target = crate::lower_to_target_operations(&source, target_profile).unwrap();
    target.functions[0].attachment = Some(StructuralTypeId::new(53_601).unwrap());
    assert_eq!(
        crate::validate_abstract_to_target_translation(&source, target_profile, &target)
            .unwrap_err(),
...
fn wrapping_shift_left_target_outer_and_independent_type_corruption_fails_closed() {
    ...
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingShiftLeft { psi_operation, .. } =
            expression(operation)
        else {
            unreachable!()
        };
        *psi_operation = OperationId::new(53_604).unwrap();
    });
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingShiftLeft { count_type, .. } = expression(operation)
        else {
            unreachable!()
        };
...
#[test]
fn wrapping_shift_left_target_nested_value_and_count_corruption_fails_closed() {
    for mutate_value in [true, false] {
        ...
        assert_target_operation_error(|operation| {
            ...
            *source_value = ValueId::new(if mutate_value { 53_605 } else { 53_606 }).unwrap();
        });
        assert_target_operation_error(|operation| {
            let operand = operand(expression(operation), mutate_value);
        ...
        assert_target_operation_error(|operation| {
            ...
            *operand = TargetIntegerExpression::BitwiseNot {
                psi_operation: OperationId::new(53_607).unwrap(),
...
#[test]
fn wrapping_shift_left_rejects_swaps_and_all_other_binary_expression_variants() {
    assert_target_operation_error(|operation| {
        let TargetIntegerExpression::WrappingShiftLeft { value, count, .. } = expression(operation)
        else {
            unreachable!()
        };
...
fn substitute_expression(operation: &mut TargetOperation, substitute: u8) {
    ...
    let TargetIntegerExpression::WrappingShiftLeft {
        psi_operation,
        count_type,
        value,
        count,
    ...
    let obligation = ObligationId::new(53_608).unwrap();
    *expression = match substitute {
        1 => TargetIntegerExpression::WrappingShiftRight {
            psi_operation,
            count_type,
            value,
            count,
...
fn operand(
    ...
    let TargetIntegerExpression::WrappingShiftLeft { value, count, .. } = expression else {
        unreachable!()
    };
    if value_operand {
        value.as_mut()
