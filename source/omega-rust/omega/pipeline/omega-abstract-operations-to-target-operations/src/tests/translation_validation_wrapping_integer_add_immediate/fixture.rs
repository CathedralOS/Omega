...
pub(super) fn leaf_error(
    ...
    crate::validation::straight_line_wrapping_integer_add_immediate::validate(
        &source.functions[0],
        &target.functions[0],
    )
    .unwrap_err()
...
pub(super) fn candidate_error(
    ...
    let AbstractToTargetTranslationValidationError::FunctionFamily {
        family: AbstractToTargetTranslationFamily::StraightLineWrappingIntegerAddImmediate,
        error:
            AbstractToTargetTranslationFamilyError::StraightLineWrappingIntegerAddImmediate(error),
        ..
