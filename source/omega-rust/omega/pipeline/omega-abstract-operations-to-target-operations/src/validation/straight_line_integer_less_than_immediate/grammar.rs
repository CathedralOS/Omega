//! Exact source grammar for ordering two same-type integer constants.
...
pub(super) fn reconstruct(
...
) -> Result<ReconstructedIntegerLessThanImmediate, Error> {
    ...
    if *left != *left_constant_result || *right != *right_constant_result {
        return Err(Error::SourceLessThanOperands);
    }
    ...
    let materialized_value = match (scalar_type.sign(), *left_value, *right_value) {
