//! Exact integer-parameter less-or-equal target replay.
...
pub(crate) fn validate(
    ...
    StraightLineIntegerLessOrEqualParametersTranslationError,
> {
    let reconstructed = super::derived::ordering::reconstruct_integer_less_or_equal(
        source,
        expected_target,
        target,
    )?;
    let TargetOperation::ReturnBooleanExpression {
        ...
        return Err(StraightLineIntegerLessOrEqualParametersTranslationError::TargetOperation);
    };
    ...
    Ok(
        StraightLineIntegerLessOrEqualParametersTranslationReceipt::new(
            source.machine,
