//! Exact source grammar for proof-bearing wrapping remainder of two integer constants.
...
use super::StraightLineWrappingIntegerRemainderImmediateOperandsTranslationError as Error;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) struct ReconstructedWrappingIntegerRemainderImmediateOperands {
    pub(super) machine: MachineId,
    pub(super) left_constant_operation: OperationId,
    pub(super) right_constant_operation: OperationId,
    pub(super) remainder_operation: OperationId,
...
pub(super) fn reconstruct(
...
) -> Result<ReconstructedWrappingIntegerRemainderImmediateOperands, Error> {
    ...
    if *left_operand != *left_constant_result || *right_operand != *right_constant_result {
        return Err(Error::SourceRemainderOperands);
    }
    let remainder = scalar_type
        .wrapping_rem(*left, *right)
        .ok_or(Error::SourceRemainderUndefined)?;
    if *result != function_result || *value != *remainder_result || *return_type != expected_type {
        return Err(Error::SourceResultLink);
    }
    Ok(ReconstructedWrappingIntegerRemainderImmediateOperands {
        machine: function.machine,
        left_constant_operation: *left_constant_operation,
        right_constant_operation: *right_constant_operation,
        remainder_operation: *remainder_operation,
