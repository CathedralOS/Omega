...
use crate::{
    ...
    RegisterHomeValidationReceipt, ValidatedAllocationLegality, ValidatedLiveRanges,
    ValidatedRegisterHomes, VirtualRegisterHome, register_home_identity,
};
...
pub fn validate_register_homes(
    ...
    let receipt = RegisterHomeValidationReceipt {
    ...
    Ok(ValidatedRegisterHomes { plan, receipt })
}

fn receipt_tied_component_count(ties: &[crate::DistinctUseDefTie]) -> usize {
...
pub(crate) fn replay_function(
    ...
    for (members, start, end, candidates) in positions {
        ...
        let home = home.ok_or(RegisterHomeError::NoCompatibleHome {
            function: function_index,
...
fn replay_group(
    ...
    members: &[usize],
    legality: &crate::FunctionAllocationLegality,
) -> Result<ReplayGroup, RegisterHomeError> {
    if members.is_empty() {
    ...
    for member in members {
        ...
        start = Some(start.map_or(lower, |value: LiveRangePoint| value.min(lower)));
        end = Some(end.map_or(upper, |value: LiveRangePoint| value.max(upper)));
...
fn replay_early_clobber_blocks(
    ...
    selected: &BTreeMap<VirtualRegisterId, RegisterViewId>,
    legality: &crate::FunctionAllocationLegality,
    ranges: &crate::FunctionLiveRanges,
    physical: &ValidatedPhysicalRegisterModel,
) -> bool {
...
fn replay_interval_bounds(
    ...
    let exclusive_end = last
        .point
        .0
        .checked_add(1)
        .ok_or(RegisterHomeError::IntervalOverflow {
            function: function_index,
            register: register.virtual_register.0,
        })?;
    Ok((first.point, LiveRangePoint(exclusive_end)))
}
...
fn validate_assignment_order(
    function_index: usize,
    function: &FunctionRegisterHomes,
    ...
    {
        return Err(RegisterHomeError::NonCanonicalAssignments {
            function: function_index,
        });
    }
    Ok(())
