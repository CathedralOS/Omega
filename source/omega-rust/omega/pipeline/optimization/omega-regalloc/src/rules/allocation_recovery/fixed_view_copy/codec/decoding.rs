...
pub(super) fn decode(encoded: &[u8]) -> Result<FixedViewCopyPlan, FixedViewCopyDecodeError> {
    ...
    if !matches!(
        version,
        LEGACY_V4_VERSION | LEGACY_V5_VERSION | LEGACY_V6_VERSION | VERSION
    ) {
    ...
    let decoded = match version {
        ...
        LEGACY_V6_VERSION | VERSION => content::decode_v6(&mut cursor)?,
        _ => unreachable!("version admission is exhaustive"),
    };
    if cursor.remaining() != 0 {
        return Err(FixedViewCopyDecodeError::TrailingBytes);
    }
    if matches!(version, LEGACY_V6_VERSION | VERSION) && !decoded.transformed_payload_matches {
    ...
    let actual_identity = match version {
        ...
        VERSION => v7_identity(&decoded.plan, &encoded[content_offset..]),
        _ => unreachable!("version admission is exhaustive"),
    };
