...
use super::envelope::{v5_identity, v6_identity, v7_identity, v8_identity};
use super::primitives::Cursor;
use super::{
    FixedViewCopyDecodeError, FixedViewCopyPlan, LEGACY_V4_VERSION, LEGACY_V5_VERSION,
    LEGACY_V6_VERSION, LEGACY_V7_VERSION, MAGIC, VERSION, content,
};
...
pub(super) fn decode(encoded: &[u8]) -> Result<FixedViewCopyPlan, FixedViewCopyDecodeError> {
    ...
    if !matches!(
        version,
        LEGACY_V4_VERSION | LEGACY_V5_VERSION | LEGACY_V6_VERSION | LEGACY_V7_VERSION | VERSION
    ) {
    ...
    let decoded = match version {
        ...
        LEGACY_V6_VERSION | LEGACY_V7_VERSION | VERSION => content::decode_v6(&mut cursor)?,
    ...
    let actual_identity = match version {
        ...
        VERSION => v8_identity(&decoded.plan, &encoded[content_offset..]),
        _ => unreachable!("version admission is exhaustive"),
    };
