...
fn derive_one(
    machine: &psi_terminal::TerminalMachine,
    ...
    let psi_terminal::OperationKind::IntegerLessThan { left: zero, right } = comparison.kind else {
        return Err(mismatch(component));
    };
    ...
    let psi_terminal::OperationKind::ExactIntegerSubtract {
        left,
        right: one,
        obligation,
    } = subtract.kind
...
fn block(machine: &psi_terminal::TerminalMachine, id: BlockId) -> Option<&psi_terminal::Block> {
