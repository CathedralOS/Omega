...
mod tests {
    ...
    use crate::{
        ...
        rules::tests::{
            ...
            live_signed_remainder_by_negative_one_unit, local_cse_unit, non_adjacent_merge_unit,
    ...
    fn run_test_pipeline(
        ...
        let ledger =
            PsiTransformationLedger::new(psi, fuel_schedule, input, unit.identity, records)
                .unwrap();
    ...
    fn fixed_point_dispatch_validates_then_commits_with_stable_usage() {
        ...
        assert_eq!(pass_manifest.ordered_rules().len(), 34);
    ...
    fn ordered_multi_rule_group_reaches_a_dependent_exact_fixed_point() {
        ...
        assert_eq!(commits.len(), 2);
        assert_eq!(usage.iterations, 3);
        assert_eq!(usage.rule_evaluations, 38);
        assert!(matches!(
            output.functions[0].blocks[0].nodes[2].operation,
            AbstractOperation::IntegerConstant {
        ...
        assert_eq!(manifest.ordered_rules().len(), 34);
        assert_eq!(manifest.decisions().len(), 2);
        assert_eq!(ledger.records().len(), 2);
    }
