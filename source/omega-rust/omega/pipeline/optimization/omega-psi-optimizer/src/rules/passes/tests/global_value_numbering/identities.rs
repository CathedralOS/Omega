...
fn analysis_products(unit: &PsiOptimizationUnit) -> Vec<AnalysisProduct> {
...
fn candidates(unit: &PsiOptimizationUnit) -> Vec<PsiRewriteCandidate> {
    let products = analysis_products(unit);
...
#[test]
fn wrapping_neutral_rule_is_disabled_by_default_and_cataloged_once_at_row_nine() {
    assert!(
    ...
    assert_eq!(
        contract.pass(),
        OptimizationPassIdentity::from_canonical_bytes(b"omega.psi-pass.global-value-numbering.v8")
    );
...
fn wrapping_neutral_rule_rejects_dead_non_pure_and_type_mismatched_inputs() {
    ...
    let mut products = analysis_products(&unit);
