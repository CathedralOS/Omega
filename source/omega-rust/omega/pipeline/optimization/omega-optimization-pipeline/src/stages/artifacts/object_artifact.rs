...
fn encode_artifact_content(record: &OptimizedObjectArtifactRecord) -> Vec<u8> {
...
fn encode_manifest_content(record: &OptimizedObjectArtifactManifest) -> Vec<u8> {
    let mut bytes = vec![1];
...
fn encode_statistics(bytes: &mut Vec<u8>, statistics: OptimizedObjectArtifactStatistics) {
...
impl<'a> Cursor<'a> {
    ...
    fn byte(&mut self) -> Result<u8, OptimizedObjectArtifactRecordDecodeError> {
        Ok(self.array::<1>()?[0])
    }
}
