//! Public optimized-target custody for constant integer wrapping integer subtraction materialization.

use super::super::*;

#[test]
fn optimized_target_lowering_retains_constant_wrapping_integer_subtract_immediate_custody() {
    for target_profile in [
        ...
        for scalar_type in native_types() {
            for (left_value, right_value) in boundary_pairs(scalar_type) {
                ...
                let (semantic, proof) = wrapping_integer_subtract_immediate_return_artifact(
                    scalar_type,
                    left_value,
                    right_value,
                );
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("optimized integer wrapping integer subtraction must retain its exact immediate family")
                };
                assert_eq!(
