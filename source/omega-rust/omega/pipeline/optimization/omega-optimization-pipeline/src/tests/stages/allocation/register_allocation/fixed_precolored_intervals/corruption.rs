...
fn roots_rows_usage_and_cross_target_custody_fail_closed() {
    ...
    corrupted.ranges = omega_regalloc::LiveRangeIdentity::from_bytes([0x31; 32]);
    assert_ne!(
        omega_regalloc::fixed_precolored_interval_plan_identity(&corrupted),
        identity,
    );
    assert!(matches!(
        validate(&x86, corrupted),
        Err(omega_regalloc::FixedPrecoloredIntervalError::RootMismatch)
    ));
    ...
    assert!(matches!(
        validate(&x86, corrupted),
        Err(omega_regalloc::FixedPrecoloredIntervalError::NonCanonicalFunctions)
    ));
    ...
    assert!(matches!(
        validate(&x86, corrupted),
        Err(omega_regalloc::FixedPrecoloredIntervalError::UsageMismatch)
    ));
