//! Public optimized-target custody for constant saturating integer-subtract materialization.

use super::super::*;

#[test]
fn optimized_target_lowering_retains_constant_saturating_integer_subtract_immediate_custody() {
    for target_profile in [
        ...
        for scalar_type in native_types() {
            for (left_value, right_value) in boundary_pairs(scalar_type) {
                let expected = scalar_type.saturating_sub(left_value, right_value).unwrap();
                let (semantic, proof) = saturating_integer_subtract_immediate_return_artifact(
                    scalar_type,
                    left_value,
                    right_value,
                );
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineSaturatingIntegerSubtractImmediate(row),
                ) = target.translation_validation().function_roster()[0].translation()
                else {
                    panic!("optimized saturating integer-subtract must retain its exact immediate family")
                };
                assert_eq!(
                    row.left_constant_operation(),
                    OperationId::new(81_003).unwrap()
                );
                assert_eq!(
                    row.right_constant_operation(),
                    OperationId::new(81_005).unwrap()
                );
                assert_eq!(
                    row.saturating_sub_operation(),
                    OperationId::new(81_007).unwrap()
                );
                assert_eq!(row.left_constant_result(), ValueId::new(81_004).unwrap());
                assert_eq!(row.right_constant_result(), ValueId::new(81_006).unwrap());
                assert_eq!(row.saturating_sub_result(), ValueId::new(81_008).unwrap());
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerImmediate {
                        ...
                        value,
                        ..
                    } if source_value == ValueId::new(81_008).unwrap()
