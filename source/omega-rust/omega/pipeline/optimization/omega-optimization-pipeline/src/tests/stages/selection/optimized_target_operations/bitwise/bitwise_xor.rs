...
#[test]
fn optimized_target_lowering_retains_exact_integer_bitwise_xor_parameter_custody() {
    ...
    for scalar_type in integers {
        for (target_profile, registers, stack) in boolean_equal_location_cases() {
            for (parameter_count, expected) in [(2, registers), (10, stack)] {
                let (semantic, proof) =
                    integer_bitwise_xor_parameters_return_artifact(scalar_type, parameter_count);
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("optimized integer bitwise-XOR must retain its family row")
                };
                assert_eq!(row.machine(), MachineId::new(30_001).unwrap());
                assert_eq!(row.xor_operation(), OperationId::new(30_005).unwrap());
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerExpression {
                        ...
                        expression: TargetIntegerExpression::BitwiseXor { left, right, .. },
                        ..
