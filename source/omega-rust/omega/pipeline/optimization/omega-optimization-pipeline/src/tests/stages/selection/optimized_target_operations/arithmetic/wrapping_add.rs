...
#[test]
fn optimized_target_lowering_retains_exact_wrapping_integer_add_parameter_custody() {
    ...
    for scalar_type in integers {
        for (target_profile, registers, stack) in boolean_equal_location_cases() {
            for (parameter_count, expected) in [(2, registers), (10, stack)] {
                let (semantic, proof) =
                    wrapping_integer_add_parameters_return_artifact(scalar_type, parameter_count);
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    AbstractToTargetFunctionTranslationReceipt::StraightLineWrappingIntegerAddParameters(row),
                ) = receipt.function_roster()[0].translation()
                else {
                    panic!("optimized wrapping integer add must retain its family row")
                };
                assert_eq!(row.machine(), MachineId::new(30_001).unwrap());
                assert_eq!(row.add_operation(), OperationId::new(30_005).unwrap());
                ...
                assert!(matches!(
                    ...
                    TargetOperation::ReturnIntegerExpression {
                        ...
                        expression: TargetIntegerExpression::WrappingAdd { left, right, .. },
                        ..
