//! Public optimized-target custody for constant integer bitwise-AND materialization.

use super::super::*;

#[test]
fn optimized_target_lowering_retains_constant_integer_bitwise_and_immediate_custody() {
    for target_profile in [
        ...
        for scalar_type in native_types() {
            for (left_value, right_value) in boundary_pairs(scalar_type) {
                ...
                let (semantic, proof) = integer_bitwise_and_immediate_return_artifact(
                    scalar_type,
                    left_value,
                    right_value,
                );
                ...
                let AbstractToTargetFunctionTranslationDisposition::Validated(
                    ...
                    panic!("optimized integer bitwise-AND must retain its exact immediate family")
                };
                assert_eq!(
