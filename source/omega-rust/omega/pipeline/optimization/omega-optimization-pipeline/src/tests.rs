...
fn staged_exact_subtract_conditional(target: NativeTarget) -> StagedOptimizedSelectedInstructions {
...
> {
    let constraints =
        crate::selection::selection_constraints(staged.legalized(), staged.register_environment());
    validate_selected_instructions(
        staged.legalized(),
        &constraints,
...
fn structural_unit_call_reaches_post_allocation_machine_custody() {
    ...
    assert!(matches!(
        crate::function_fragment_text_section::place_structural_unit_fragments_for_test(&fragments),
