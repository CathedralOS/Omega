...
use omega_selected_instructions::{
    MachineAlternative, MachineAlternativeApplicability, MachineAlternativeFamily, MachineBarrier,
    ...
    SelectedInstructionKind, StructuralUnitCallBarrier, StructuralUnitCallEffect,
...
fn encode_ordinary_instruction(
    bytes: &mut Vec<u8>,
    instruction: &crate::InstructionMachineEffects,
) {
    bytes.extend_from_slice(&instruction.instruction.0.to_le_bytes());
    ...
    bytes.push(match instruction.memory {
        omega_selected_instructions::MachineMemoryEffect::NoneV1 => 0,
    });
    ...
    bytes.push(match instruction.call {
        omega_selected_instructions::MachineCallEffect::NoneV1 => 0,
    });
...
fn encode_structural_layout(
    bytes: &mut Vec<u8>,
    layout: omega_selected_instructions::SelectedMicrosoftX64OwnedIndirectPairLayout,
) {
...
pub(crate) fn encode_structural_call(
    bytes: &mut Vec<u8>,
    call: &crate::StructuralUnitCallMachineEffects,
) {
...
fn encode_structural_declaration(
    bytes: &mut Vec<u8>,
    declaration: omega_selected_instructions::StructuralUnitCallEffectDeclaration,
) {
...
pub(crate) fn encode_provenance(
    bytes: &mut Vec<u8>,
    provenance: &omega_selected_instructions::SelectedInstructionProvenance,
) {
...
mod tests {
    ...
    use omega_selected_instructions::{
        MachineAlternativeKey, MachineCallEffect, MachineCleanupEffect,
        MachineEffectCatalogIdentity, MachineLatencyKnowledge, MachineMemoryEffect,
        MachineTrapBehavior, SelectedBlockId, SelectedInstructionId,
    ...
    use crate::{
        ...
        PreAllocationMachineEffectPlan,
    };
