...
fn verified_forwarded_dynamic_unit_retains_argument_and_parameter_custody() {
    ...
    assert!(argument.has_complete_custody(caller.machine, argument.argument.operation, helper_id));
    let AbstractDynamicDescriptorSource::Rebound { application, .. } = &argument.source else {
        panic!("forwarded Unit descriptor must retain rebound custody")
    };
