...
fn canonical_o0_agrees_from_terminal_meaning_through_runnable_linux_image() {
    ...
    let decoded = decode_module(&semantic).expect("decode canonical O0 terminal Psi");
    ...
    if let Ok(path) = std::env::var("OMEGA_TERMINAL_WRITE_X64_IMAGE")
...
fn assert_native_observable(bytes: &[u8], stdout: &[u8], exit_status: i32) {
    ...
    let executable = directory.join("omega-terminal");
