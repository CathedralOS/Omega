...
use omega_external_roots::{
    ...
    InstalledComponentProgressClosure, InstalledDynamicFuelAttributionPlan, InstalledExternalRoot,
    InstalledNativeFuelTransferRuntime, InstalledRootLedger, NativeFuelExecutionEnvironment,
    ProgressProfileEstablishmentAttestation, ProviderExecution, ProviderOccurrencePlanBinding,
...
impl TerminalComponentDeploymentSession {
    ...
    /// Install one deployed dynamic-fuel root from both independently sealed
    /// installed halves. The deployment owner is the first production layer
    /// that owns the installed code and root ledger together, so it performs
    /// both exact joins before publishing the root into that ledger.
    ///
    /// Rejection leaves this still-live session borrowed in place and returns
    /// the complete non-clonable input carrier. The contained sealed values are cloned only across the
    /// existing admission API's diagnostic boundary; the caller-visible
    /// deployment custody itself is consumed exactly once.
    pub fn install_dynamic_native_fuel_root<'deployment>(
        &'deployment mut self,
        input: DynamicNativeFuelRootDeployment,
    ) -> Result<InstalledDynamicNativeFuelRoot<'deployment>, DynamicNativeFuelRootDeploymentError>
    {
        let DynamicNativeFuelRootDeployment {
            admission_identity,
            root,
            provider_execution,
            slot,
        ...
        let admission = match RootAdmission::from_admitted_provider_with_dynamic_native_fuel(
            admission_identity,
            &root,
            &provider_execution,
            &self.installed,
            ...
            transfer_runtime.clone(),
            root.candidate().trust_receipts.iter().copied(),
        ) {
            Ok(admission) => admission,
            Err(diagnostic) => {
                return Err(DynamicNativeFuelRootDeploymentError {
                    input: DynamicNativeFuelRootDeployment {
                        admission_identity,
                        root,
                        provider_execution,
        ...
        Ok(InstalledDynamicNativeFuelRoot { root, roots })
    }
...
/// Complete deployment-owned inputs for one dynamically metered external
/// root. This carrier is deliberately non-clonable: final installed
/// attribution and live transfer-runtime custody must travel together into the
/// exact installed-code occurrence and root ledger.
#[derive(Debug)]
#[must_use = "dynamic native-fuel deployment inputs retain installed execution custody"]
pub struct DynamicNativeFuelRootDeployment {
    ...
    installed_attribution: InstalledDynamicFuelAttributionPlan,
    transfer_runtime: InstalledNativeFuelTransferRuntime,
}

impl DynamicNativeFuelRootDeployment {
    ...
    pub const fn installed_attribution(&self) -> &InstalledDynamicFuelAttributionPlan {
        &self.installed_attribution
    }

    pub const fn transfer_runtime(&self) -> &InstalledNativeFuelTransferRuntime {
    ...
    pub fn into_parts(
        ...
        ProviderExecution,
        RootSlotAuthority,
        NativeFuelExecutionEnvironment,
        InstalledDynamicFuelAttributionPlan,
...
/// Live dynamic-root custody borrowing the deployment's exact installed code
/// and root ledger. The root handle is never dropped at installation; runtime
/// users retain it until explicit provider removal returns its slot authority.
#[derive(Debug)]
#[must_use = "an installed dynamic native-fuel root remains live until explicitly removed"]
pub struct InstalledDynamicNativeFuelRoot<'deployment> {
    root: InstalledExternalRoot<'deployment>,
    roots: &'deployment mut InstalledRootLedger,
}

impl<'deployment> InstalledDynamicNativeFuelRoot<'deployment> {
    ...
    /// Expose both disjoint live values for operations that already belong to
    /// the external-root owner. The installed-code borrow remains tied to the
    /// deployment session for the complete lifetime of the returned root.
    pub fn into_parts(
        self,
    ) -> (
    ...
    /// Remove the exact root after its provider proves unreachability and
    /// quiescence. Rejection reconstructs this complete live carrier and the
    /// receipt so retirement can be retried without losing custody.
    pub fn remove(
        self,
        receipt: RootRemovalReceipt,
    ) -> Result<RootSlotAuthority, Box<DynamicNativeFuelRootRemovalError<'deployment>>> {
        ...
        match roots.remove(root, receipt) {
            ...
            Err(error) => {
                ...
                Err(Box::new(DynamicNativeFuelRootRemovalError {
                    installed: InstalledDynamicNativeFuelRoot { root, roots },
                    receipt,
                    diagnostic,
                }))
            }
...
/// Transactional failure to admit or install a dynamic-fuel root. The claimed
/// deployment session remains borrowed and intact; every caller-supplied
/// custody value is returned in one exact retry carrier.
#[derive(Debug)]
pub struct DynamicNativeFuelRootDeploymentError {
    input: DynamicNativeFuelRootDeployment,
    diagnostic: String,
}

impl DynamicNativeFuelRootDeploymentError {
    pub fn diagnostic(&self) -> &str {
        &self.diagnostic
    }

    pub fn into_input(self) -> DynamicNativeFuelRootDeployment {
        self.input
    }
}

impl std::fmt::Display for DynamicNativeFuelRootDeploymentError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.diagnostic.fmt(formatter)
    }
}

impl std::error::Error for DynamicNativeFuelRootDeploymentError {}

/// Failed explicit removal of one installed dynamic native-fuel root.
#[derive(Debug)]
pub struct DynamicNativeFuelRootRemovalError<'deployment> {
    installed: InstalledDynamicNativeFuelRoot<'deployment>,
    receipt: RootRemovalReceipt,
    diagnostic: String,
}

impl<'deployment> DynamicNativeFuelRootRemovalError<'deployment> {
    pub fn diagnostic(&self) -> &str {
        &self.diagnostic
    }
    ...
    pub fn into_parts(
        ...
        InstalledDynamicNativeFuelRoot<'deployment>,
        RootRemovalReceipt,
    ) {
...
impl std::fmt::Display for DynamicNativeFuelRootRemovalError<'_> {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.diagnostic.fmt(formatter)
    }
}

impl std::error::Error for DynamicNativeFuelRootRemovalError<'_> {}
...
pub struct TerminalComponentFlatOutputReceipt {
    output_path: PathBuf,
    installation_fingerprint: omega_terminal_image_emission::TerminalInstallationFingerprint,
    image_fingerprint: omega_terminal_image_emission::TerminalImageFingerprint,
    byte_count: usize,
}

impl TerminalComponentFlatOutputReceipt {
    pub fn output_path(&self) -> &Path {
        &self.output_path
    }

    pub const fn installation_fingerprint(
        &self,
    ) -> omega_terminal_image_emission::TerminalInstallationFingerprint {
        self.installation_fingerprint
    }

    pub const fn image_fingerprint(
        &self,
    ) -> omega_terminal_image_emission::TerminalImageFingerprint {
    ...
    ) -> Result<(), TerminalComponentFlatOutputValidationError> {
        validate_terminal_flat_output_receipt(self, runnable)
            .map_err(TerminalComponentFlatOutputValidationError)
    }
}
...
pub struct PublishedTerminalComponentFlatOutput {
    runnable: InstalledRunnableComponent,
    receipt: TerminalComponentFlatOutputReceipt,
}

impl PublishedTerminalComponentFlatOutput {
    pub const fn runnable(&self) -> &InstalledRunnableComponent {
        &self.runnable
    }

    pub const fn receipt(&self) -> &TerminalComponentFlatOutputReceipt {
        &self.receipt
    }

    pub fn validate(&self) -> Result<(), TerminalComponentFlatOutputValidationError> {
    ...
    pub fn into_parts(
        ...
        TerminalComponentFlatOutputReceipt,
    ) {
...
#[derive(Debug)]
pub struct TerminalComponentFlatOutputPublicationError {
    runnable: InstalledRunnableComponent,
    requested_path: PathBuf,
    diagnostic: String,
}

impl TerminalComponentFlatOutputPublicationError {
    pub fn diagnostic(&self) -> &str {
        &self.diagnostic
    }
...
impl std::fmt::Display for TerminalComponentFlatOutputPublicationError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.diagnostic.fmt(formatter)
    }
}

impl std::error::Error for TerminalComponentFlatOutputPublicationError {}

#[derive(Debug, PartialEq, Eq)]
pub struct TerminalComponentFlatOutputValidationError(String);

impl TerminalComponentFlatOutputValidationError {
    pub fn diagnostic(&self) -> &str {
        &self.0
    }
}

impl std::fmt::Display for TerminalComponentFlatOutputValidationError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.0.fmt(formatter)
    }
}

impl std::error::Error for TerminalComponentFlatOutputValidationError {}
...
pub fn publish_terminal_component_flat_output(
    runnable: InstalledRunnableComponent,
    requested_path: PathBuf,
) -> Result<PublishedTerminalComponentFlatOutput, Box<TerminalComponentFlatOutputPublicationError>>
{
    let result = publish_terminal_component_flat_output_inner(&runnable, &requested_path);
    match result {
        Ok(receipt) => Ok(PublishedTerminalComponentFlatOutput { runnable, receipt }),
        Err(diagnostic) => Err(Box::new(TerminalComponentFlatOutputPublicationError {
            runnable,
            requested_path,
            diagnostic,
        })),
...
fn publish_terminal_component_flat_output_inner(
    runnable: &InstalledRunnableComponent,
    output_path: &Path,
) -> Result<TerminalComponentFlatOutputReceipt, String> {
    let terminal = runnable.terminal_artifact();
    ...
    let installation_fingerprint = terminal_installation_fingerprint(installation)
    ...
    let receipt = TerminalComponentFlatOutputReceipt {
...
fn validate_terminal_flat_output_receipt(
    ...
    let terminal = runnable.terminal_artifact();
    validate_terminal_installation_record(terminal.installation(), terminal.image())
        .map_err(|error| format!("terminal output installation replay failed: {error}"))?;
    let installation_fingerprint = terminal_installation_fingerprint(terminal.installation())
        .map_err(|error| format!("terminal output installation fingerprint failed: {error}"))?;
