...
use omega_terminal_installation_evidence::{
    TerminalFuelAttributionEvidence, TerminalObjectEvidence,
};
...
fn installed_segment_replay_is_exact_and_never_becomes_entry_authority() {
    ...
    let binding =
        bind_installed_terminal_segment_fuel(certificate, &terminal, &installed, selected_entry)
    ...
    assert!(
        validate_installed_terminal_segment_fuel(&binding, &wrong_occurrence, selected_entry)
            .unwrap_err()
            .0
    ...
    assert!(
        validate_installed_terminal_segment_fuel(&binding, &wrong_artifact, selected_entry)
            .is_err()
    );
