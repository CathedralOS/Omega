use crate::{
    ...
    OptimizedSelectedFormEncodingError, StagedOptimizedActiveResidentRematerialization,
    ...
    StagedOptimizedSelectedFormEncoding, TerminalSelectedFormEncodingIdentity,
...
pub struct StagedOptimizedActiveResidentRematerializationSelectedFormEncodingCustodyReceipt {
    ...
    encoding: TerminalSelectedFormEncodingIdentity,
    row_count: usize,
    encoded_count: usize,
...
impl StagedOptimizedActiveResidentRematerializationSelectedFormEncodingCustodyReceipt {
    ...
    pub const fn encoding(&self) -> TerminalSelectedFormEncodingIdentity {
        self.encoding
    }
...
fn custody_receipt(
    ...
    let encoded_count = encoding
        .rows()
        .iter()
        .filter(|row| matches!(row.state, TerminalSelectedFormEncodingState::Encoded { .. }))
        .count();
...
pub(crate) fn corrupt_active_resident_selected_form_encoding_byte_for_test(
    ...
    let bytes = staged
        ...
        .find_map(|row| match &mut row.state {
            TerminalSelectedFormEncodingState::Encoded { bytes, .. } => Some(bytes),
            TerminalSelectedFormEncodingState::DeferredControl { .. } => None,
        })
