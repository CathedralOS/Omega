...
fn indexed_operator_schema(
    coordinate: &str,
    arity: usize,
) -> omega_effects::IndexedProviderRequirementSchema {
    omega_effects::IndexedProviderRequirementSchema::new(coordinate, None, arity)
        .expect("indexed operator schema")
}

fn indexed_operator_application(
        ...
        arguments
            ...
            .map(|argument| {
                omega_effects::IndexedProviderConcreteArgument::new(*argument)
                    .expect("normalized concrete argument")
            })
            .collect(),
    )
    .expect("exact indexed operator application")
}

#[test]
fn operator_family_exact_application_coverage_rejoins_and_canonicalizes() {
    let coordinate = "operator::Transfer::move($0,$1)->unit";
    let plan = operator_coordinate_plan("transfer", coordinate, "TransferProvider");
    let plan_identity = plan.report_fingerprint();
    let schema = indexed_operator_schema(coordinate, 2);
    let first = indexed_operator_application(&schema, &["Bytes", "Message"]);
    let second = indexed_operator_application(&schema, &["Card", "Message"]);
    let selected = omega_effects::SelectedProviderPlanFacts::from_selected_plans(vec![plan])
        .expect("selected exact operator plan")
        .with_indexed_provider_application_coverage(vec![
            omega_effects::ProviderAssertedIndexedApplicationCoverage::exact_family(
                plan_identity,
                schema,
                vec![second.clone(), first.clone()],
            )
            .expect("canonical exact coverage"),
        ])
        .expect("coverage rejoins selected plan");
    let selection = crate::ProviderSelection::operator_family_with_application_arity_for_test(
        "Transfer::move",
        "TransferProvider",
        &[(coordinate, 2)],
    );
    let crate::ProviderSelectionSubject::BoundaryOperatorFamily(family) = &selection.subject else {
        panic!("operator-family fixture")
    };

    let retained = family
        .exact_application_coverage(&selected, &selection.provider_type)
        .expect("exact applications should rejoin the family coordinate");
    let [coverage] = retained.as_slice() else {
        panic!("one exact application coverage row")
    };
    assert_eq!(coverage.requirement_identity(), coordinate);
    assert_eq!(coverage.provider_plan_report_identity(), plan_identity);
    assert_eq!(coverage.applications(), &[first, second]);
}

#[test]
fn operator_family_application_coverage_rejects_missing_and_generic_claims() {
    let coordinate = "operator::Transfer::move($0)->unit";
    let plan = operator_coordinate_plan("transfer", coordinate, "TransferProvider");
    let plan_identity = plan.report_fingerprint();
    let schema = indexed_operator_schema(coordinate, 1);
    let selection = crate::ProviderSelection::operator_family_with_application_arity_for_test(
        "Transfer::move",
        "TransferProvider",
        &[(coordinate, 1)],
    ...
    let missing = omega_effects::SelectedProviderPlanFacts::from_selected_plans(vec![plan.clone()])
        .expect("selected exact operator plan");
    let missing_error = family
        .exact_application_coverage(&missing, &selection.provider_type)
        .expect_err("a generic coordinate requires exact application coverage");
    assert!(missing_error.contains("has 0 exact selected coverage rows"));

    let generic = omega_effects::SelectedProviderPlanFacts::from_selected_plans(vec![plan])
        .expect("selected exact operator plan")
        .with_indexed_provider_application_coverage(vec![
            omega_effects::ProviderAssertedIndexedApplicationCoverage::generic(
                plan_identity,
                schema,
            )
            .expect("structural generic coverage fixture"),
        ])
        .expect("generic coverage structurally rejoins selected plan");
    let generic_error = family
        .exact_application_coverage(&generic, &selection.provider_type)
        .expect_err("generic application claims remain fail-closed");
    assert!(generic_error.contains("generic realization proof"));
}

#[test]
fn operator_family_application_coverage_rejects_stale_duplicate_and_tampered_rows() {
    let coordinate = "operator::Transfer::move($0,$1)->unit";
    let plan = operator_coordinate_plan("transfer", coordinate, "TransferProvider");
    let plan_identity = plan.report_fingerprint();
    let schema = indexed_operator_schema(coordinate, 2);
    let application = indexed_operator_application(&schema, &["Bytes", "Message"]);
    let exact = || {
        omega_effects::ProviderAssertedIndexedApplicationCoverage::exact_family(
            plan_identity,
            schema.clone(),
        ...
        .expect("exact coverage fixture")
    };

    let duplicate =
        omega_effects::SelectedProviderPlanFacts::from_selected_plans(vec![plan.clone()])
            .expect("selected exact operator plan")
            .with_indexed_provider_application_coverage(vec![exact(), exact()])
            .expect_err("duplicate coordinate coverage must reject");
    assert!(duplicate.contains("more than one indexed-application coverage row"));

    let stale_identity = if plan_identity == u64::MAX {
        1
    } else {
        plan_identity + 1
    };
    let stale = omega_effects::ProviderAssertedIndexedApplicationCoverage::exact_family(
        stale_identity,
        schema.clone(),
        vec![application.clone()],
    )
    .expect("stale coverage fixture");
    let stale_error =
        omega_effects::SelectedProviderPlanFacts::from_selected_plans(vec![plan.clone()])
            .expect("selected exact operator plan")
            .with_indexed_provider_application_coverage(vec![stale])
            .expect_err("stale plan identity must reject");
    assert!(stale_error.contains("unselected provider plan"));

    let wrong_schema = indexed_operator_schema("operator::Transfer::copy($0,$1)->unit", 2);
    let tampered = omega_effects::ProviderAssertedIndexedApplicationCoverage::exact_family(
        plan_identity,
        wrong_schema.clone(),
        vec![indexed_operator_application(
            &wrong_schema,
            &["Bytes", "Message"],
        )],
    )
    .expect("tampered schema fixture");
    let tampered_error =
        omega_effects::SelectedProviderPlanFacts::from_selected_plans(vec![plan.clone()])
            .expect("selected exact operator plan")
            .with_indexed_provider_application_coverage(vec![tampered])
            .expect_err("tampered schema must reject");
    assert!(tampered_error.contains("schema does not match selected provider plan"));

    let wrong_arity_schema = indexed_operator_schema(coordinate, 1);
    let wrong_arity = omega_effects::ProviderAssertedIndexedApplicationCoverage::exact_family(
        plan_identity,
        wrong_arity_schema.clone(),
        vec![indexed_operator_application(
            &wrong_arity_schema,
            &["Bytes"],
        )],
    )
    .expect("wrong arity fixture");
    let selected = omega_effects::SelectedProviderPlanFacts::from_selected_plans(vec![plan])
        .expect("selected exact operator plan")
        .with_indexed_provider_application_coverage(vec![wrong_arity])
        .expect("schema path still rejoins before telescope review");
    let selection = crate::ProviderSelection::operator_family_with_application_arity_for_test(
        "Transfer::move",
        "TransferProvider",
        &[(coordinate, 2)],
    ...
    let arity_error = family
        .exact_application_coverage(&selected, &selection.provider_type)
        .expect_err("coverage arity must match the declaration telescope");
    assert!(arity_error.contains("has 2 static parameter(s), but retained coverage has arity 1"));
}
