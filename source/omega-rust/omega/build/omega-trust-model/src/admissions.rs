...
impl TrustAdmission {
    ...
    pub fn for_machine_template(
        commitment: String,
        report_identity: u64,
        template_commitment: psi_typed_trees::typed_trees::MachineTemplateCommitment,
    ) -> Result<Self, &'static str> {
        Self::derived(
            commitment,
...
mod tests {
    ...
    fn admission(name: &str, report_identity: u64, digest_byte: u8) -> TrustAdmission {
        TrustAdmission::for_machine_contract(
            name.to_owned(),
            report_identity,
            psi_checked_trees::MachineContractCommitment::from_digest([digest_byte; 32]),
        )
        .unwrap()
    }
    ...
    fn subject_kind_domain_separates_equal_underlying_commitments() {
        ...
        let template = TrustAdmission::for_machine_template(
            commitment.clone(),
            1,
            psi_typed_trees::typed_trees::MachineTemplateCommitment::from_digest([7; 32]),
        )
        .unwrap();
        let contract = TrustAdmission::for_machine_contract(
            commitment,
            1,
            psi_checked_trees::MachineContractCommitment::from_digest([7; 32]),
        )
        .unwrap();
