...
pub fn derive_fused_program_entry_establishments(
...
) -> Result<Vec<omega_program_entry_plan::ProgramEntryFusedServiceEstablishment>, Vec<Diagnostic>> {
    ...
    for member in checked.data_members(owner) {
        ...
        match psi_typed_trees::service::classify_exact_bound_service_carrier(
            checked,
    ...
    for (field, carrier) in service_fields {
        ...
        let Some(schema) =
            omega_effects::provider_plan::ServiceSchema::from_typed(checked, requirement)
        else {
            diagnostics.push(Diagnostic::error(format!(
        ...
        let row = omega_program_entry_plan::ProgramEntryFusedServiceEstablishment::new(
            ...
            omega_program_entry_plan::ProgramEntryFusedServiceEstablishment::requirement_identity_for_schema(&schema),
...
fn unconstrained_type_identity(
    ...
    while let psi_typed_trees::types::TypeReferenceNode::Constrained { base_type, .. } =
