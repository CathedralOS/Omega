...
fn record_recovers_exact_success_failure_and_carried_error_sequence() {
    ...
    assert!(matches!(
        operations,
        [
            psi_checked_interpreter::FilesystemOutputFileOperationReplayRecord::ChangeFileOwner(success),
            psi_checked_interpreter::FilesystemOutputFileOperationReplayRecord::ChangeFileOwner(failure),
            psi_checked_interpreter::FilesystemOutputFileOperationReplayRecord::ChangeFileOwner(carried),
        ] if success.uid() == -1
            && success.gid() == -1
            && success.result() == 0
