...
use psi_layout_plans::{
    ConventionalDepthTwentyTwoRecordSumOccurrenceLayoutReport,
...
pub(super) fn project_conventional_record_with_depth_twenty_two_nested_sums_materialization_layout_with_reachability(
    ...
    let definition =
        unique_data_definition(program, data_symbol, "plural depth-twenty-two sum owner")?;
    validate_closed_copy_record(program, definition, "plural depth-twenty-two sum owner")?;
    ...
    let DataShape::Record {
        ...
        return Err(Diagnostic::error(format!(
            "target runtime layout row for plural depth-twenty-two sum owner `{}` is not a record",
            definition.name
        )));
    };
    ...
    if declared_fields.len() != laid_fields.len() {
        return Err(Diagnostic::error(format!(
            "target runtime layout for plural depth-twenty-two sum owner `{}` has {} fields; checked schema has {} relevant fields",
            definition.name,
            laid_fields.len(),
            declared_fields.len()
        )));
    ...
    entries
        ...
        .map_err(|_| {
            Diagnostic::error("plural depth-twenty-two sum outer report exceeds compiler resources")
        })?;
    let mut offsets = Vec::new();
    offsets
        .try_reserve_exact(declared_fields.len())
        .map_err(|_| {
            Diagnostic::error("plural depth-twenty-two sum outer offsets exceed compiler resources")
        })?;
    let mut paths = Vec::new();
    paths
        .try_reserve_exact(declared_fields.len())
        .map_err(|_| {
            Diagnostic::error("plural depth-twenty-two sum path report exceeds compiler resources")
        })?;
    ...
    for (declared, laid) in declared_fields.into_iter().zip(laid_fields) {
        ...
        {
            return Err(Diagnostic::error(format!(
                "plural depth-twenty-two sum outer field `{}` uses target-dependent fragment, stored-integer, or repeated placement",
                declared.name
            )));
        }
        ...
        if reachability.type_contains_sum(declared.type_reference)? {
            if matches!(
                ...
                return Err(Diagnostic::error(format!(
                    "plural depth-twenty-two sum outer field `{}` reaches a sum through an array",
                    declared.name
                )));
            }
            let named = exact_named_data(program, declared.type_reference)?.ok_or_else(|| {
                Diagnostic::error(format!(
                    "plural depth-twenty-two sum outer field `{}` lacks one exact record identity",
                    declared.name
                ))
            })?;
            ...
            {
                return Err(Diagnostic::error(format!(
                    "plural depth-twenty-two sum outer field `{}` does not name the required twentieth record",
                    declared.name
                )));
            }
            let depth_twenty_one_paths = project_conventional_record_with_depth_twenty_one_nested_sums_materialization_layout_with_reachability(
                program,
                plan,
                named.symbol,
                reachability,
            ...
            let TypeLayoutDescriptor::Named {
                ...
                return Err(Diagnostic::error(format!(
                    "target runtime layout field `{}` is not the exact declared twentieth record",
                    declared.name
                )));
            };
            ...
            {
                return Err(Diagnostic::error(format!(
                    "target runtime layout field `{}` substitutes its twentieth record type",
                    declared.name
                )));
            }
            if usize_to_u64(laid.layout.size, "depth-twenty-two twentieth-record extent")?
                != depth_twenty_one_paths
                    .outer_layout
                    .size
                    .expect("plural depth-twenty-one projection has fixed extent")
                || usize_to_u64(
                    laid.layout.alignment,
                    "depth-twenty-two twentieth-record alignment",
                )? != depth_twenty_one_paths.outer_layout.align
            {
                return Err(Diagnostic::error(format!(
                    "target runtime layout field `{}` does not retain the exact twentieth-record extent/alignment",
                    declared.name
                )));
            }
            for twenty_first_occurrence in &depth_twenty_one_paths.paths {
                                                ...
                                                &fourteenth_occurrence.depth_thirteen_paths.paths
                                            {
                                                    ...
                                                    &thirteenth_occurrence.depth_twelve_paths.paths
                                                {
                                                        ...
                                                        &twelfth_occurrence.depth_eleven_paths.paths
                                                    {
                                                        ...
                                                        .ok_or_else(|| {
                                                            Diagnostic::error(
                                                                "plural depth-twenty-two leaf-path count overflows",
                                                            )
                                                        })?;
                                                                            }
                                                                        }
            ...
            if total_leaf_paths > SumReachability::MAX_EDGES {
                return Err(Diagnostic::error(
                    "plural depth-twenty-two paths exceed bounded total leaf occurrences",
                ));
            }
            paths.push(ConventionalDepthTwentyTwoRecordSumOccurrenceLayoutReport {
        ...
        let offset = usize_to_u64(laid.offset, "plural depth-twenty-two outer field offset")?;
    ...
    if paths.is_empty() {
        return Err(Diagnostic::error(
            "plural depth-twenty-two sum projection requires a nonempty qualifying record-chain set",
        ));
    }
    Ok(ConventionalDepthTwentyTwoRecordSumPathsLayoutReport {
        outer_layout: LayoutPlanReport {
            ...
            size: Some(usize_to_u64(
                ...
                "plural depth-twenty-two outer record extent",
            )?),
            align: usize_to_u64(
                data_layout.layout.alignment,
                "plural depth-twenty-two outer record alignment",
            )?,
        },
        paths,
    })
