//! Focused depth-seventeen producer, replay, atomicity, and boundary fences.

use super::*;

#[test]
fn plural_depth_seventeen_paths_compose_depth_sixteen_custody_and_retain_fences() {
    let checked = checked(
        r#"
        ...
        data Outer [copy] { #1 left: Fifteenth; #2 right: Fifteenth; }

        data OuterTooDeep [copy] { #1 outer: Outer; }
        data OuterDirect [copy] { #1 fifteenth: Fifteenth; #2 direct: Choice; }
        data OuterArray [copy] { #1 fifteenth: Fifteenth; #2 choices: [Choice; 1]; }
        "#,
    );
    ...
    let fifteenth = definition("Fifteenth");
    let paths =
        ...
        .expect("the complete depth-seventeen occurrence cohort should project");
    ...
    assert_eq!(paths.paths[0].depth_sixteen_paths.paths.len(), 1);
    assert_eq!(paths.paths[1].depth_sixteen_paths.paths.len(), 1);
    ...
    let value = BuildTimeValue::Struct {
        ...
        fields: vec![
            ("left".into(), fifteenth_value(choice(None))),
            ("right".into(), fifteenth_value(choice(Some(0x1122)))),
        ],
    };
    let carrier = psi_build_time_evaluation::validate_const_materializable_record_with_depth_seventeen_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    .expect("depth-seventeen value custody should compose the depth-sixteen carriers");
    ...
    let big_endian = psi_build_time_evaluation::validate_const_materializable_record_with_depth_seventeen_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    let rejects =
        |mutated: &psi_layout_plans::ConventionalDepthSeventeenRecordSumPathsLayoutReport| {
            assert!(
                carrier
    ...
    wrong_inner_identity.paths[0].depth_sixteen_paths.paths[0].outer_member_identity = Some(2);
    rejects(&wrong_inner_identity);
    let mut wrong_leaf_geometry = paths.clone();
    wrong_leaf_geometry.paths[0].depth_sixteen_paths.paths[0]
        .depth_fifteen_paths
        .paths[0]
    ...
    wrong_case_ordinal.paths[0].depth_sixteen_paths.paths[0]
        .depth_fifteen_paths
        .paths[0]
        .depth_fourteen_paths
        .paths[0]
    ...
    for name in ["Fifteenth", "OuterTooDeep", "OuterDirect", "OuterArray"] {
        assert!(
            project_conventional_record_with_depth_seventeen_nested_sums_materialization_layout(
                &checked,
                &plan,
            ...
            .is_err(),
            "{name} remains outside the exact depth-seventeen cohort"
        );
    }
    ...
    let recursive_type = checked
        .data_members(fifteenth)
        .iter()
        .find_map(|member| match member {
            DataMember::Field(field) if field.name.as_str() == "fourteenth" => {
    ...
    recursive_checked
        ...
        .substitute_node(
            ...
            TypeReferenceNode::Named {
                symbol: fifteenth.symbol,
                name: fifteenth.name.clone(),
            },
        );
    assert!(
        project_conventional_record_with_depth_seventeen_nested_sums_materialization_layout(
        ...
        .is_err(),
        "a recursive depth-seventeen path must reject during bounded projection"
    );
    assert!(
        psi_build_time_evaluation::validate_const_materializable_record_with_depth_seventeen_nested_sums(
            &recursive_checked,
            "Outer",
            &paths,
            &value,
        ...
        .is_err(),
        "a recursive depth-seventeen path must reject during value replay"
    );
    ...
    assert!(
        ...
        .is_ok(),
        "the unchanged plural depth-sixteen API retains its prior cohort"
    );
    assert!(
        project_conventional_record_with_depth_sixteen_nested_sums_materialization_layout(
            &checked,
        ...
        .is_err(),
        "the prior API must not widen to the new depth-seventeen root"
    );
}
