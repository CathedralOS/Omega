...
fn plural_depth_nineteen_paths_compose_depth_eighteen_custody_and_retain_fences() {
    ...
    assert_eq!(paths.paths[0].depth_eighteen_paths.paths.len(), 1);
    assert_eq!(paths.paths[1].depth_eighteen_paths.paths.len(), 1);
    ...
    let carrier = psi_build_time_evaluation::validate_const_materializable_record_with_depth_nineteen_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    let big_endian = psi_build_time_evaluation::validate_const_materializable_record_with_depth_nineteen_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    let rejects =
        |mutated: &psi_layout_plans::ConventionalDepthNineteenRecordSumPathsLayoutReport| {
            assert!(
                carrier
    ...
    wrong_inner_identity.paths[0].depth_eighteen_paths.paths[0].outer_member_identity = Some(2);
    rejects(&wrong_inner_identity);
    let mut wrong_leaf_geometry = paths.clone();
    wrong_leaf_geometry.paths[0].depth_eighteen_paths.paths[0]
    ...
    wrong_case_ordinal.paths[0].depth_eighteen_paths.paths[0]
        .depth_seventeen_paths
        .paths[0]
        .depth_sixteen_paths
        .paths[0]
    ...
    assert!(
        psi_build_time_evaluation::validate_const_materializable_record_with_depth_nineteen_nested_sums(
            &recursive_checked,
            "Outer",
            &paths,
            &value,
