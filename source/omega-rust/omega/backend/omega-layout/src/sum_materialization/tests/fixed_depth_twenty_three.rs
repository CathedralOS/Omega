//! Focused depth-twenty-three producer, replay, atomicity, and boundary fences.

use super::*;

#[test]
fn plural_depth_twenty_three_paths_compose_depth_twenty_two_custody_and_retain_fences() {
    let checked = checked(
        r#"
        ...
        data Twentieth [copy] { #1 nineteenth: Nineteenth; }
        data TwentyFirst [copy] { #1 twentieth: Twentieth; }
        data Outer [copy] { #1 left: TwentyFirst; #2 right: TwentyFirst; }

        data OuterTooDeep [copy] { #1 outer: Outer; }
        data OuterDirect [copy] { #1 twenty_first: TwentyFirst; #2 direct: Choice; }
        data OuterArray [copy] { #1 twenty_first: TwentyFirst; #2 choices: [Choice; 1]; }
        "#,
    );
    ...
    let twenty_first = definition("TwentyFirst");
    let paths =
        ...
        .expect("the complete depth-twenty-three occurrence cohort should project");
    ...
    assert_eq!(paths.paths[0].depth_twenty_two_paths.paths.len(), 1);
    assert_eq!(paths.paths[1].depth_twenty_two_paths.paths.len(), 1);
    ...
    let twentieth_value = |choice| BuildTimeValue::Struct {
        type_name: "Twentieth".into(),
        fields: vec![("nineteenth".into(), nineteenth_value(choice))],
    };
    let twenty_first_value = |choice| BuildTimeValue::Struct {
        type_name: "TwentyFirst".into(),
        fields: vec![("twentieth".into(), twentieth_value(choice))],
    };
    let value = BuildTimeValue::Struct {
        type_name: "Outer".into(),
        fields: vec![
            ("left".into(), twenty_first_value(choice(None))),
            ("right".into(), twenty_first_value(choice(Some(0x1122)))),
        ],
    };
    let carrier = psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_three_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    .expect("depth-twenty-three value custody should compose the depth-twenty-two carriers");
    ...
    let big_endian = psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_three_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    let rejects =
        |mutated: &psi_layout_plans::ConventionalDepthTwentyThreeRecordSumPathsLayoutReport| {
            assert!(
                carrier
    ...
    wrong_inner_identity.paths[0].depth_twenty_two_paths.paths[0].outer_member_identity = Some(2);
    rejects(&wrong_inner_identity);
    let mut wrong_leaf_geometry = paths.clone();
    wrong_leaf_geometry.paths[0].depth_twenty_two_paths.paths[0]
    ...
    wrong_case_ordinal.paths[0].depth_twenty_two_paths.paths[0]
        .depth_twenty_one_paths
        .paths[0]
        .depth_twenty_paths
        .paths[0]
    ...
    for name in ["TwentyFirst", "OuterTooDeep", "OuterDirect", "OuterArray"] {
        assert!(
            project_conventional_record_with_depth_twenty_three_nested_sums_materialization_layout(
                &checked,
                &plan,
            ...
            .is_err(),
            "{name} remains outside the exact depth-twenty-three cohort"
        );
    }
    ...
    let recursive_type = checked
        .data_members(twenty_first)
        .iter()
        .find_map(|member| match member {
            DataMember::Field(field) if field.name.as_str() == "twentieth" => {
    ...
    recursive_checked
        ...
        .substitute_node(
            ...
            TypeReferenceNode::Named {
                symbol: twenty_first.symbol,
                name: twenty_first.name.clone(),
            },
        );
    assert!(
        project_conventional_record_with_depth_twenty_three_nested_sums_materialization_layout(
        ...
        .is_err(),
        "a recursive depth-twenty-three path must reject during bounded projection"
    );
    assert!(
        psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_three_nested_sums(
            &recursive_checked,
            "Outer",
            &paths,
            &value,
        ...
        .is_err(),
        "a recursive depth-twenty-three path must reject during value replay"
    );
    ...
    assert!(
        ...
        .is_ok(),
        "the unchanged plural depth-twenty-two API retains its prior cohort"
    );
    assert!(
        project_conventional_record_with_depth_twenty_two_nested_sums_materialization_layout(
            &checked,
        ...
        .is_err(),
        "the prior API must not widen to the new depth-twenty-three root"
    );
}
