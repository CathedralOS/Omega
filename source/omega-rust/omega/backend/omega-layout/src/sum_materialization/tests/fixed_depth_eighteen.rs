//! Focused depth-eighteen producer, replay, atomicity, and boundary fences.

use super::*;

#[test]
fn plural_depth_eighteen_paths_compose_depth_seventeen_custody_and_retain_fences() {
    let checked = checked(
        r#"
        ...
        data Outer [copy] { #1 left: Sixteenth; #2 right: Sixteenth; }

        data OuterTooDeep [copy] { #1 outer: Outer; }
        data OuterDirect [copy] { #1 sixteenth: Sixteenth; #2 direct: Choice; }
        data OuterArray [copy] { #1 sixteenth: Sixteenth; #2 choices: [Choice; 1]; }
        "#,
    );
    ...
    let sixteenth = definition("Sixteenth");
    let paths = project_conventional_record_with_depth_eighteen_nested_sums_materialization_layout(
        &checked,
        &plan,
        outer.symbol,
    )
    .expect("the complete depth-eighteen occurrence cohort should project");
    ...
    let value = BuildTimeValue::Struct {
        ...
        fields: vec![
            ("left".into(), sixteenth_value(choice(None))),
            ("right".into(), sixteenth_value(choice(Some(0x1122)))),
        ],
    };
    let carrier = psi_build_time_evaluation::validate_const_materializable_record_with_depth_eighteen_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    .expect("depth-eighteen value custody should compose the depth-seventeen carriers");
    ...
    let big_endian = psi_build_time_evaluation::validate_const_materializable_record_with_depth_eighteen_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    let rejects =
        |mutated: &psi_layout_plans::ConventionalDepthEighteenRecordSumPathsLayoutReport| {
            assert!(
                carrier
    ...
    for name in ["Sixteenth", "OuterTooDeep", "OuterDirect", "OuterArray"] {
        assert!(
            ...
            .is_err(),
            "{name} remains outside the exact depth-eighteen cohort"
        );
    }
    ...
    let recursive_type = checked
        .data_members(sixteenth)
        .iter()
        .find_map(|member| match member {
            DataMember::Field(field) if field.name.as_str() == "fifteenth" => {
    ...
    recursive_checked
        ...
        .substitute_node(
            ...
            TypeReferenceNode::Named {
                symbol: sixteenth.symbol,
                name: sixteenth.name.clone(),
            },
        );
    assert!(
        project_conventional_record_with_depth_eighteen_nested_sums_materialization_layout(
        ...
        .is_err(),
        "a recursive depth-eighteen path must reject during bounded projection"
    );
    assert!(
        psi_build_time_evaluation::validate_const_materializable_record_with_depth_eighteen_nested_sums(
            &recursive_checked,
            "Outer",
            &paths,
            &value,
        ...
        .is_err(),
        "a recursive depth-eighteen path must reject during value replay"
    );
    ...
    assert!(
        ...
        .is_ok(),
        "the unchanged plural depth-seventeen API retains its prior cohort"
    );
    assert!(
        project_conventional_record_with_depth_seventeen_nested_sums_materialization_layout(
            &checked,
        ...
        .is_err(),
        "the prior API must not widen to the new depth-eighteen root"
    );
}
