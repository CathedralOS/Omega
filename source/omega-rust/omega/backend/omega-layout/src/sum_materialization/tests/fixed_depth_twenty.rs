//! Focused depth-twenty producer, replay, atomicity, and boundary fences.

use super::*;

#[test]
fn plural_depth_twenty_paths_compose_depth_nineteen_custody_and_retain_fences() {
    let checked = checked(
        r#"
        ...
        data Outer [copy] { #1 left: Eighteenth; #2 right: Eighteenth; }

        data OuterTooDeep [copy] { #1 outer: Outer; }
        data OuterDirect [copy] { #1 eighteenth: Eighteenth; #2 direct: Choice; }
        data OuterArray [copy] { #1 eighteenth: Eighteenth; #2 choices: [Choice; 1]; }
        "#,
    );
    ...
    let eighteenth = definition("Eighteenth");
    let paths = project_conventional_record_with_depth_twenty_nested_sums_materialization_layout(
        &checked,
        &plan,
        outer.symbol,
    )
    .expect("the complete depth-twenty occurrence cohort should project");
    ...
    assert_eq!(paths.paths[0].depth_nineteen_paths.paths.len(), 1);
    assert_eq!(paths.paths[1].depth_nineteen_paths.paths.len(), 1);
    ...
    let value = BuildTimeValue::Struct {
        ...
        fields: vec![
            ("left".into(), eighteenth_value(choice(None))),
            ("right".into(), eighteenth_value(choice(Some(0x1122)))),
        ],
    };
    let carrier = psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    .expect("depth-twenty value custody should compose the depth-nineteen carriers");
    ...
    let big_endian = psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    let rejects =
        |mutated: &psi_layout_plans::ConventionalDepthTwentyRecordSumPathsLayoutReport| {
            assert!(
                carrier
    ...
    wrong_inner_identity.paths[0].depth_nineteen_paths.paths[0].outer_member_identity = Some(2);
    rejects(&wrong_inner_identity);
    let mut wrong_leaf_geometry = paths.clone();
    wrong_leaf_geometry.paths[0].depth_nineteen_paths.paths[0]
    ...
    wrong_case_ordinal.paths[0].depth_nineteen_paths.paths[0]
        .depth_eighteen_paths
        .paths[0]
        .depth_seventeen_paths
        .paths[0]
    ...
    for name in ["Eighteenth", "OuterTooDeep", "OuterDirect", "OuterArray"] {
        assert!(
            ...
            .is_err(),
            "{name} remains outside the exact depth-twenty cohort"
        );
    }
    ...
    let recursive_type = checked
        .data_members(eighteenth)
        .iter()
        .find_map(|member| match member {
            DataMember::Field(field) if field.name.as_str() == "seventeenth" => {
    ...
    recursive_checked
        ...
        .substitute_node(
            ...
            TypeReferenceNode::Named {
                symbol: eighteenth.symbol,
                name: eighteenth.name.clone(),
            },
        );
    assert!(
        project_conventional_record_with_depth_twenty_nested_sums_materialization_layout(
        ...
        .is_err(),
        "a recursive depth-twenty path must reject during bounded projection"
    );
    assert!(
        psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_nested_sums(
            &recursive_checked,
            "Outer",
            &paths,
            &value,
        ...
        .is_err(),
        "a recursive depth-twenty path must reject during value replay"
    );
    ...
    assert!(
        ...
        .is_ok(),
        "the unchanged plural depth-nineteen API retains its prior cohort"
    );
    assert!(
        project_conventional_record_with_depth_nineteen_nested_sums_materialization_layout(
            &checked,
        ...
        .is_err(),
        "the prior API must not widen to the new depth-twenty root"
    );
}
