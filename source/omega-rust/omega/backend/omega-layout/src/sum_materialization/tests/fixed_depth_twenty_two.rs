//! Focused depth-twenty-two producer, replay, atomicity, and boundary fences.

use super::*;

#[test]
fn plural_depth_twenty_two_paths_compose_depth_twenty_one_custody_and_retain_fences() {
    let checked = checked(
        r#"
        ...
        data Outer [copy] { #1 left: Twentieth; #2 right: Twentieth; }

        data OuterTooDeep [copy] { #1 outer: Outer; }
        data OuterDirect [copy] { #1 twentieth: Twentieth; #2 direct: Choice; }
        data OuterArray [copy] { #1 twentieth: Twentieth; #2 choices: [Choice; 1]; }
        "#,
    );
    ...
    let twentieth = definition("Twentieth");
    let paths =
        ...
        .expect("the complete depth-twenty-two occurrence cohort should project");
    ...
    assert_eq!(paths.paths[0].depth_twenty_one_paths.paths.len(), 1);
    assert_eq!(paths.paths[1].depth_twenty_one_paths.paths.len(), 1);
    ...
    let value = BuildTimeValue::Struct {
        ...
        fields: vec![
            ("left".into(), twentieth_value(choice(None))),
            ("right".into(), twentieth_value(choice(Some(0x1122)))),
        ],
    };
    let carrier = psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_two_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    .expect("depth-twenty-two value custody should compose the depth-twenty-one carriers");
    ...
    let big_endian = psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_two_nested_sums(
        &checked,
        "Outer",
        &paths,
        &value,
    ...
    let rejects =
        |mutated: &psi_layout_plans::ConventionalDepthTwentyTwoRecordSumPathsLayoutReport| {
            assert!(
                carrier
    ...
    wrong_inner_identity.paths[0].depth_twenty_one_paths.paths[0].outer_member_identity = Some(2);
    rejects(&wrong_inner_identity);
    let mut wrong_leaf_geometry = paths.clone();
    wrong_leaf_geometry.paths[0].depth_twenty_one_paths.paths[0]
        .depth_twenty_paths
        .paths[0]
    ...
    wrong_case_ordinal.paths[0].depth_twenty_one_paths.paths[0]
        .depth_twenty_paths
        .paths[0]
        .depth_nineteen_paths
        .paths[0]
    ...
    for name in ["Twentieth", "OuterTooDeep", "OuterDirect", "OuterArray"] {
        assert!(
            project_conventional_record_with_depth_twenty_two_nested_sums_materialization_layout(
                &checked,
                &plan,
            ...
            .is_err(),
            "{name} remains outside the exact depth-twenty-two cohort"
        );
    }
    ...
    let recursive_type = checked
        .data_members(twentieth)
        .iter()
        .find_map(|member| match member {
            DataMember::Field(field) if field.name.as_str() == "nineteenth" => {
    ...
    recursive_checked
        ...
        .substitute_node(
            ...
            TypeReferenceNode::Named {
                symbol: twentieth.symbol,
                name: twentieth.name.clone(),
            },
        );
    assert!(
        project_conventional_record_with_depth_twenty_two_nested_sums_materialization_layout(
        ...
        .is_err(),
        "a recursive depth-twenty-two path must reject during bounded projection"
    );
    assert!(
        psi_build_time_evaluation::validate_const_materializable_record_with_depth_twenty_two_nested_sums(
            &recursive_checked,
            "Outer",
            &paths,
            &value,
        ...
        .is_err(),
        "a recursive depth-twenty-two path must reject during value replay"
    );
    ...
    assert!(
        ...
        .is_ok(),
        "the unchanged plural depth-twenty-one API retains its prior cohort"
    );
    assert!(
        project_conventional_record_with_depth_twenty_one_nested_sums_materialization_layout(
            &checked,
        ...
        .is_err(),
        "the prior API must not widen to the new depth-twenty-two root"
    );
}
