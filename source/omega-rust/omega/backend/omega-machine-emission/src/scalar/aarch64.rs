...
use omega_machine_code::{
    Aarch64ReturnLinkEvidence, BooleanStructuralFieldRead, InternalCallRelocation,
    ScalarCallStackEvidence, ScalarConditionalCondition,
};
...
pub(crate) fn emit_aarch64_condition_load(
    ...
    match location {
        AssignedScalarLocation::Register(MachineRegister::Aarch64X(register)) if register < 31 => {
            Ok((Vec::new(), register))
        }
    ...
    match expression {
        ...
        AssignedBooleanExpression::Parameter {
            ...
            let byte_offset = match location {
                ...
                AssignedScalarLocation::IncomingStack { byte_offset } => stack_depth
                    .checked_add(frame.byte_size)
                    .and_then(|offset| offset.checked_add(*byte_offset)),
                AssignedScalarLocation::Register(_) => {
            ...
            .ok_or(EmissionError::IncomingStackOffsetNotEncodable {
                ...
                byte_offset: match location {
                    AssignedScalarLocation::Register(_)
                    | AssignedScalarLocation::FrameSpill { .. } => 0,
                    AssignedScalarLocation::IncomingStack { byte_offset } => *byte_offset,
                },
            })?;
            instructions.push(aarch64_stack_access(
                0xf940_0000,
