...
use omega_terminal_installation_evidence::{
    ...
    NativeFuelTransferRuntimePlanProjection, TerminalNativeFuelTransferRuntimeEvidence,
};

use super::native_fuel_codec::{decode_profile, decode_transport, profile_tag, transport_tag};
use super::{
    Reader, TerminalInstallationError, TerminalInstalledNativeFuelTransferRuntime,
    TerminalNativeFuelTransferTextFingerprint, decode_boolean, push_u16, push_u32, push_u64,
};

pub(super) fn encode_native_fuel_transfer(
    bytes: &mut Vec<u8>,
    installed: Option<&TerminalInstalledNativeFuelTransferRuntime>,
    ...
    push_u32(
        ...
        u32::try_from(plan.activation_state_slots().len())
            .map_err(|_| TerminalInstallationError::TooManyNativeFuelTransferStateSlots)?,
    );
    ...
    push_u32(
        ...
        u32::try_from(footprint.registers().as_slice().len())
            .map_err(|_| TerminalInstallationError::TooManyNativeFuelTransferRegisters)?,
    );
...
pub(super) fn decode_native_fuel_transfer(
...
) -> Result<Option<TerminalInstalledNativeFuelTransferRuntime>, TerminalInstallationError> {
    ...
    let unrelocated_text_fingerprint = TerminalNativeFuelTransferTextFingerprint(reader.array()?);
    let unrelocated_text_byte_count = decode_offset(reader)?;
    let final_text_fingerprint = TerminalNativeFuelTransferTextFingerprint(reader.array()?);
    ...
    let context = omega_terminal_installation_evidence::NativeFuelContextLayout {
    ...
    .map_err(|_| TerminalInstallationError::InvalidNativeFuelTransferPlan)?;
    if plan.normalized_identity() != encoded_plan_identity {
        return Err(TerminalInstallationError::InvalidNativeFuelTransferPlan);
    }
    ...
    .map_err(|_| TerminalInstallationError::InvalidNativeFuelTransferEvidence)?;
    ...
    Ok(Some(TerminalInstalledNativeFuelTransferRuntime {
...
fn encode_text_evidence(
    ...
    push_u32(
        ...
        u32::try_from(evidence.unrelocated_bytes().len())
            .map_err(|_| TerminalInstallationError::NativeFuelTransferBytesTooLong)?,
    );
...
fn decode_text_evidence(
    reader: &mut Reader<'_>,
) -> Result<NativeFuelRuntimeTextEvidence, TerminalInstallationError> {
...
fn decode_entry(
    reader: &mut Reader<'_>,
) -> Result<NativeFuelRuntimeEntryIdentity, TerminalInstallationError> {
...
fn decode_saved_value(
    reader: &mut Reader<'_>,
) -> Result<NativeFuelSavedValue, TerminalInstallationError> {
    match reader.u8()? {
        ...
        tag => Err(TerminalInstallationError::InvalidNativeFuelSavedValueTag(
            tag,
        )),
    }
}
...
fn encode_register(
    ...
    let (class, index) = match register {
        ...
        _ => return Err(TerminalInstallationError::InvalidNativeFuelTransferRegister),
    };
    bytes.extend_from_slice(&[class, index]);
    Ok(())
}

fn decode_register(reader: &mut Reader<'_>) -> Result<MachineRegister, TerminalInstallationError> {
    let class = reader.u8()?;
    let index = reader.u8()?;
...
fn decode_machine_state_set(bits: u16) -> Result<MachineStateSet, TerminalInstallationError> {
    if bits & !0x01ff != 0 {
        return Err(TerminalInstallationError::InvalidNativeFuelTransferMachineState);
    }
    let states = [
...
fn encode_offset(value: usize) -> Result<u64, TerminalInstallationError> {
    u64::try_from(value).map_err(|_| TerminalInstallationError::NativeFuelOffsetNotRepresentable)
}
