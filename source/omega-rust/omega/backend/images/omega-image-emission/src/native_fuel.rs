...
pub struct ValidatedNativeFuelArtifact {
    ...
    text_bytes: Vec<u8>,
    functions: Vec<ValidatedNativeFuelFunction>,
}

impl ValidatedNativeFuelArtifact {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ValidatedNativeFuelFunction {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum NativeFuelValidationError {
    SemanticObject(ObjectError),
    TargetMismatch,
...
impl std::fmt::Display for NativeFuelValidationError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "{self:?}")
    }
}

impl std::error::Error for NativeFuelValidationError {}
...
pub fn validate_native_fuel_plan(
    ...
    for (source, supplied) in plan.source.functions.iter().zip(&plan.functions) {
        ...
        if supplied.bytes != expected_bytes {
            return Err(NativeFuelValidationError::ByteMismatch(source.machine));
        }
        let byte_count = expected_bytes.len();
        text_bytes.extend_from_slice(&expected_bytes);
        functions.push(ValidatedNativeFuelFunction {
    ...
    Ok(ValidatedNativeFuelArtifact {
        semantic_artifact,
...
fn translate_object(
    semantic: &ObjectArtifact,
    source_functions: &[MachineCodeFunction],
    metered_functions: &[ValidatedNativeFuelFunction],
    text_size: usize,
    architecture: Architecture,
) -> Result<(ObjectPlan, RelocationPlan), NativeFuelValidationError> {
    ...
    let text_section = object
        ...
        .ok_or(NativeFuelValidationError::ObjectTranslation)?;
    ...
    for (semantic_function, metered_function) in semantic.functions.iter().zip(metered_functions) {
        if semantic_function.machine != metered_function.machine {
            return Err(NativeFuelValidationError::ObjectTranslation);
        }
...
fn replay_function(
    ...
    {
        return Err(NativeFuelValidationError::NonCanonicalAttribution(
            source.machine,
        ));
    }
    ...
    for (ordinal, attribution) in source.fuel_attribution.iter().copied().enumerate() {
        ...
        bytes.extend_from_slice(
            source
                ...
                .ok_or(NativeFuelValidationError::NonCanonicalAttribution(
                    source.machine,
                ))?,
        );
...
mod tests {
    ...
    fn source(profile: TargetProfile) -> MachineCodePlan {
        ...
        MachineCodePlan {
            ...
            target: profile.native_target(),
            entry: machine,
            functions: vec![omega_machine_code::MachineCodeFunction {
                machine,
                attachment: None,
    ...
    fn instrumented(profile: TargetProfile) -> NativeFuelInstrumentedPlan {
        omega_machine_emission::instrument_native_fuel(source(profile), policy(profile))
    ...
    fn independently_replays_x86_and_aarch64_metered_text() {
        for profile in [TargetProfile::LinuxX64, TargetProfile::LinuxArm64] {
            ...
            let validated = validate_native_fuel_plan(&plan).expect("independent replay");
            assert_eq!(
            ...
            let image = crate::emit_native_fuel_executable_image(&validated, 3)
                .expect("metered direct image");
            assert_eq!(image.output().final_text_bytes, validated.text_bytes());
            let projected = omega_installation_evidence::NativeFuelImageEvidence::charges(&image);
    ...
    fn producer_byte_and_record_mutations_reject() {
        ...
        assert_eq!(
            validate_native_fuel_plan(&record_changed),
            Err(NativeFuelValidationError::RecordMismatch(
                MachineId::new(1).unwrap()
            ))
        );
    }
