...
use crate::{
    TerminalAarch64CbnzFusionIdentity, TerminalAarch64CbnzFusionPlan,
    TerminalAarch64CbnzFusionPolicy, TerminalAarch64CbnzFusionRevisionIdentity,
    TerminalAarch64CbnzInstructionDisposition, TerminalQualifiedPhysicalRead,
};
...
pub fn terminal_aarch64_cbnz_fusion_identity(
    plan: &TerminalAarch64CbnzFusionPlan,
) -> TerminalAarch64CbnzFusionIdentity {
    let mut hasher = Sha256::new();
    hasher.update(IDENTITY_DOMAIN);
    hasher.update(encode_content(plan));
    TerminalAarch64CbnzFusionIdentity::from_bytes(hasher.finalize().into())
}

pub(crate) fn revision_identity(
    source: crate::TerminalPostAllocationMachineIdentity,
    ...
    functions: &[crate::TerminalAarch64CbnzFusionFunction],
) -> TerminalAarch64CbnzFusionRevisionIdentity {
    let mut hasher = Sha256::new();
    ...
    TerminalAarch64CbnzFusionRevisionIdentity::from_bytes(hasher.finalize().into())
}

pub(crate) fn encode_content(plan: &TerminalAarch64CbnzFusionPlan) -> Vec<u8> {
    ...
    bytes.push(match plan.policy {
        TerminalAarch64CbnzFusionPolicy::Aarch64FuseCompareI64ZeroBranchNonZeroToCbnzV1 => 0,
    });
    ...
    for attempt in &plan.attempts {
        ...
        bytes.push(match attempt.outcome {
            crate::TerminalAarch64CbnzFusionAttemptOutcome::AlreadyFused => 0,
            crate::TerminalAarch64CbnzFusionAttemptOutcome::CompareCarriesFuel => 1,
            crate::TerminalAarch64CbnzFusionAttemptOutcome::NzcvLiveOut => 2,
            crate::TerminalAarch64CbnzFusionAttemptOutcome::SelectedForFusion => 3,
        });
    }
...
fn encode_functions(hasher: &mut Sha256, functions: &[crate::TerminalAarch64CbnzFusionFunction]) {
    let mut encoded = Vec::new();
        ...
        for block in &function.blocks {
            ...
            for instruction in &block.instructions {
                ...
                match &instruction.disposition {
                    TerminalAarch64CbnzInstructionDisposition::RetainedV1 => bytes.push(0),
...
fn encode_read(bytes: &mut Vec<u8>, read: &TerminalQualifiedPhysicalRead) {
