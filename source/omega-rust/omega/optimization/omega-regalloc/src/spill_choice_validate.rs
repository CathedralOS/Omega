...
use crate::{
    ...
    TerminalSpillChoicePlan, TerminalSpillChoicePolicy, TerminalSpillChoiceValidationReceipt,
    TerminalVirtualInterference, ValidatedTerminalAllocationLegality, ValidatedTerminalLiveRanges,
    ValidatedTerminalSpillChoices, terminal_spill_choice_identity,
};
...
impl ReplayWork {
    fn bump(field: &mut u64, amount: u64) -> Result<(), TerminalSpillChoiceError> {
        *field = field
            .checked_add(amount)
...
pub fn validate_terminal_spill_choices(
    ...
    if plan.policy != TerminalSpillChoicePolicy::SingleBlockFarthestEndThenHighestVregV1 {
    ...
    for function_index in 0..plan.functions.len() {
        ...
        if plan.functions[function_index] != expected {
            return Err(TerminalSpillChoiceError::ChoiceMismatch {
                function: function_index,
            });
        }
    }
    ...
    if plan.usage != expected_usage {
        return Err(TerminalSpillChoiceError::UsageMismatch);
    }
    ...
    let contender_count = plan
        ...
        .sum();
    let receipt = TerminalSpillChoiceValidationReceipt {
        identity: terminal_spill_choice_identity(&plan),
    ...
    Ok(ValidatedTerminalSpillChoices { plan, receipt })
}
...
fn replay_function(
    ...
    {
        return Err(TerminalSpillChoiceError::FunctionMismatch { function });
    }
    ...
    for (position, register) in legality.virtual_registers.iter().enumerate() {
        ...
        let end = TerminalLiveRangePoint(last.point.0.checked_add(1).ok_or(
            TerminalSpillChoiceError::IntervalOverflow {
                function,
                register: register.virtual_register.0,
            },
        )?);
        ...
        return Ok(TerminalFunctionSpillChoices {
            ...
            choice: Some(TerminalSpillChoice {
                ...
                active_residents: residents
                    .iter()
                    .map(|resident| TerminalPressureResident {
...
pub(crate) fn replay_function_for_test(
    ...
    ranges: &crate::TerminalFunctionLiveRanges,
    physical: &ValidatedPhysicalRegisterModel,
) -> Result<(TerminalFunctionSpillChoices, OptimizationWorkUsage), TerminalSpillChoiceError> {
    let mut work = ReplayWork::default();
...
fn replay_interferes(
    ...
    pairs: &[TerminalVirtualInterference],
) -> bool {
    pairs.iter().any(|pair| {
