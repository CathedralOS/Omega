...
fn validate_structural_unit_roster(
    scalar_functions: &[TerminalSelectedFunction],
    selected_functions: &[TerminalSelectedStructuralUnitFunction],
    actual_functions: &[TerminalFunctionLiveness],
...
fn validate_structural_machine_roster(
        ...
        .chain(selected_machines.iter().copied())
    {
        if !all_selected.insert(machine) {
            return Err(TerminalLivenessError::DuplicateMachine {
                machine: machine.get(),
            });
        }
    }
    ...
    for (ordinal, (selected, actual)) in selected_machines.iter().zip(actual_functions).enumerate()
    {
        ...
        if *selected != actual.machine {
            return Err(TerminalLivenessError::StructuralFunctionMismatch { function: ordinal });
        }
    }
    Ok(())
}
...
mod tests {
    ...
    use omega_terminal_selected_instructions::{
        TerminalSelectedBlockId, TerminalSelectedInstructionId, TerminalSelectedOperand,
    ...
    fn structural_liveness(machine: MachineId) -> crate::TerminalFunctionLiveness {
        crate::TerminalFunctionLiveness {
            machine,
            entry_definitions: Vec::new(),
            operand_positions: Vec::new(),
            blocks: vec![crate::TerminalBlockLiveness {
                ...
                unit_live_out: Vec::new(),
                instructions: vec![crate::TerminalInstructionLiveness {
                    position: crate::TerminalLivenessPosition(0),
    ...
    fn structural_roster_rejects_erasure_order_identity_duplicate_and_unit_drift() {
        ...
        assert_eq!(
            ...
            Err(crate::TerminalLivenessError::RootMismatch)
        );
        let swapped = [exact[1].clone(), exact[0].clone()];
        assert_eq!(
            validate_structural_machine_roster([scalar], &selected, &swapped),
            Err(crate::TerminalLivenessError::StructuralFunctionMismatch { function: 0 })
        );
        ...
        assert_eq!(
            ...
            Err(crate::TerminalLivenessError::DuplicateMachine {
                machine: caller.get()
            })
        );
        ...
        assert_eq!(
            ...
            Err(crate::TerminalLivenessError::InstructionMismatch {
                function: 0,
                instruction: 0
            })
        );
