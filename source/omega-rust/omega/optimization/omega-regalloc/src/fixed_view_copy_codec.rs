...
use omega_terminal_selected_instructions::{
    ...
    TerminalSelectedInstructionProvenance, TerminalSelectedOperand, TerminalSelectedSuccessor,
    TerminalSelectedTerminator, TerminalVirtualRegister, TerminalVirtualRegisterId,
...
use crate::{
    ...
    TerminalFixedViewCopy, TerminalFixedViewCopyDecodeError, TerminalFixedViewCopyDestination,
    ...
    TerminalVirtualFixedConstraintSite, terminal_fixed_view_copy_identity,
};
...
impl TerminalFixedViewCopyPlan {
    ...
    pub fn encode(&self) -> Vec<u8> {
        let identity = terminal_fixed_view_copy_identity(self);
    ...
    pub fn decode(encoded: &[u8]) -> Result<Self, TerminalFixedViewCopyDecodeError> {
        let mut cursor = Cursor::new(encoded);
        if cursor.take(MAGIC.len())? != MAGIC {
            return Err(TerminalFixedViewCopyDecodeError::WrongMagic);
        }
        let version = cursor.u32()?;
        if version != VERSION {
            return Err(TerminalFixedViewCopyDecodeError::UnsupportedVersion(
                version,
            ));
        }
        let identity = TerminalFixedViewCopyIdentity::from_bytes(cursor.array()?);
        let source_selected = TerminalSelectedInstructionPlanIdentity::from_bytes(cursor.array()?);
        let source_ranges = TerminalLiveRangeIdentity::from_bytes(cursor.array()?);
        let source_legality = TerminalAllocationLegalityIdentity::from_bytes(cursor.array()?);
        ...
        let policy = match cursor.byte()? {
            0 => TerminalFixedViewCopyPolicy::LeafLocalBeforeFixedUseV1,
            1 => TerminalFixedViewCopyPolicy::SharedEntryAfterCompareBeforeBranchV1,
            tag => return Err(TerminalFixedViewCopyDecodeError::UnknownPolicy(tag)),
        };
        let budget = omega_optimization_core::OptimizationWorkBudget::decode(cursor.take(40)?)
            .map_err(|_| TerminalFixedViewCopyDecodeError::InvalidBudget)?;
        let usage = omega_optimization_core::OptimizationWorkUsage::decode(cursor.take(40)?)
            .map_err(|_| TerminalFixedViewCopyDecodeError::InvalidUsage)?;
        ...
        if cursor.remaining() != 0 {
            return Err(TerminalFixedViewCopyDecodeError::TrailingBytes);
        }
        if terminal_selected_instruction_plan_identity(&transformed) != expected_transformed {
            return Err(TerminalFixedViewCopyDecodeError::TransformedIdentityMismatch);
        }
        let plan = Self {
            source_selected,
            source_ranges,
        ...
        if terminal_fixed_view_copy_identity(&plan) != identity {
            return Err(TerminalFixedViewCopyDecodeError::IdentityMismatch);
        }
        Ok(plan)
    }
}

fn encode_content(bytes: &mut Vec<u8>, plan: &TerminalFixedViewCopyPlan) {
...
fn encode_copy(bytes: &mut Vec<u8>, copy: &TerminalFixedViewCopy) {
...
fn decode_copy(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalFixedViewCopy, TerminalFixedViewCopyDecodeError> {
    let function = cursor.u32()?;
    let machine = decode_id(cursor, MachineId::new)?;
    let source_virtual_register = TerminalVirtualRegisterId(cursor.u32()?);
    ...
    let insertion_block = TerminalSelectedBlockId(cursor.u32()?);
    let before_instruction = TerminalSelectedInstructionId(cursor.u32()?);
    ...
    Ok(TerminalFixedViewCopy {
        ...
        before_instruction,
        destinations,
        copy_instruction: TerminalSelectedInstructionId(cursor.u32()?),
        result_virtual_register: TerminalVirtualRegisterId(cursor.u32()?),
        copy_constraint: decode_constraint_key(cursor)?,
    })
}

fn encode_selected_plan(bytes: &mut Vec<u8>, plan: &TerminalSelectedInstructionPlan) {
...
fn decode_selected_plan(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalSelectedInstructionPlan, TerminalFixedViewCopyDecodeError> {
    let fingerprint = SemanticFingerprint::from_bytes(cursor.array()?);
    let marker_raw = cursor.u16()?;
    let marker = VocabularyMarker::new(marker_raw).ok_or(
        TerminalFixedViewCopyDecodeError::InvalidVocabulary(marker_raw),
    )?;
    let fuel_raw = cursor.u32()?;
    let fuel_schedule = FuelScheduleIdentity::new(fuel_raw).ok_or(
        TerminalFixedViewCopyDecodeError::InvalidFuelSchedule(fuel_raw),
    )?;
    ...
    for _ in 0..function_count {
        ...
        let attachment = match decode_option_u64(cursor)? {
            ...
            Some(raw) => Some(
                StructuralTypeId::new(raw)
                    .ok_or(TerminalFixedViewCopyDecodeError::InvalidSemanticId(raw))?,
            ),
        };
        let operations = decode_ids(cursor, OperationId::new)?;
        let edges = decode_ids(cursor, EdgeId::new)?;
        let entry_block = TerminalSelectedBlockId(cursor.u32()?);
        ...
        let provenance =
            omega_terminal_target_operations::TerminalPsiProvenance { operations, edges };
        functions.push(TerminalSelectedFunction {
            machine,
            attachment,
            provenance,
            entry_block,
...
fn encode_register(bytes: &mut Vec<u8>, register: &TerminalVirtualRegister) {
...
fn decode_register(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalVirtualRegister, TerminalFixedViewCopyDecodeError> {
    let id = TerminalVirtualRegisterId(cursor.u32()?);
    let scalar_type = decode_scalar(cursor)?;
    let class = RegisterClassId(cursor.u16()?);
    let origin = match cursor.byte()? {
        0 => TerminalVirtualRegisterOrigin::EntryParameter {
            source_value: decode_id(cursor, ValueId::new)?,
            parameter_index: cursor.length()?,
        },
        1 => TerminalVirtualRegisterOrigin::InstructionResult {
            instruction: TerminalSelectedInstructionId(cursor.u32()?),
            source_value: decode_id(cursor, ValueId::new)?,
        },
        2 => TerminalVirtualRegisterOrigin::LegalizationTemporary {
            instruction: TerminalSelectedInstructionId(cursor.u32()?),
        ...
        tag => return Err(TerminalFixedViewCopyDecodeError::UnknownRegisterOrigin(tag)),
    };
    Ok(TerminalVirtualRegister {
        id,
        scalar_type,
        class,
        origin,
...
fn encode_block(bytes: &mut Vec<u8>, block: &TerminalSelectedBlock) {
...
fn decode_block(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalSelectedBlock, TerminalFixedViewCopyDecodeError> {
    let id = TerminalSelectedBlockId(cursor.u32()?);
    ...
    let terminator = match cursor.byte()? {
        0 => TerminalSelectedTerminator::ConditionalBranch {
            instruction: decode_instruction(cursor)?,
            when_nonzero: decode_successor(cursor)?,
            when_zero: decode_successor(cursor)?,
        },
        1 => TerminalSelectedTerminator::Return {
            instruction: decode_instruction(cursor)?,
            psi_return_edge: decode_id(cursor, EdgeId::new)?,
        },
        tag => return Err(TerminalFixedViewCopyDecodeError::UnknownTerminator(tag)),
    };
...
fn decode_instruction(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalSelectedInstruction, TerminalFixedViewCopyDecodeError> {
    let id = TerminalSelectedInstructionId(cursor.u32()?);
    ...
    for _ in 0..operand_count {
        operands.push(TerminalSelectedOperand {
            operand: cursor.u16()?,
...
fn decode_kind(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalSelectedInstructionKind, TerminalFixedViewCopyDecodeError> {
    Ok(match cursor.byte()? {
        ...
        2 => TerminalSelectedInstructionKind::ConditionalBranchNonZero,
        3 => TerminalSelectedInstructionKind::ReturnI64,
        4 => TerminalSelectedInstructionKind::CopyI64,
        5 => TerminalSelectedInstructionKind::ExactAddI64 {
        ...
        6 => TerminalSelectedInstructionKind::ExactAddI64Immediate {
        ...
        7 => TerminalSelectedInstructionKind::ExactSubtractI64 {
            obligation: decode_id(cursor, ObligationId::new)?,
            accepted_fact: omega_optimization_core::AcceptedObligationFactIdentity::from_bytes(
                cursor.array()?,
            ),
...
fn decode_successor(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalSelectedSuccessor, TerminalFixedViewCopyDecodeError> {
...
fn decode_provenance(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalSelectedInstructionProvenance, TerminalFixedViewCopyDecodeError> {
...
fn decode_fuel(
    cursor: &mut Cursor<'_>,
) -> Result<Vec<FuelSettlement>, TerminalFixedViewCopyDecodeError> {
    let count = cursor.length()?;
    ...
    for _ in 0..count {
        ...
        let site = match tag {
            ...
            1 => PsiProvenance::Edge(
                EdgeId::new(raw).ok_or(TerminalFixedViewCopyDecodeError::InvalidSemanticId(raw))?,
            ),
            tag => return Err(TerminalFixedViewCopyDecodeError::UnknownFuelSite(tag)),
        };
...
fn decode_definition_site(
    cursor: &mut Cursor<'_>,
) -> Result<ValueDefinitionSite, TerminalFixedViewCopyDecodeError> {
    Ok(match cursor.byte()? {
        ...
        tag => return Err(TerminalFixedViewCopyDecodeError::UnknownDefinitionSite(tag)),
    })
}
...
fn decode_fixed_site(
    cursor: &mut Cursor<'_>,
) -> Result<TerminalVirtualFixedConstraintSite, TerminalFixedViewCopyDecodeError> {
    Ok(match cursor.byte()? {
        0 => TerminalVirtualFixedConstraintSite::Entry,
        1 => TerminalVirtualFixedConstraintSite::Operand {
            position: TerminalLivenessPosition(cursor.u32()?),
            point: TerminalLiveRangePoint(cursor.u32()?),
            instruction: TerminalSelectedInstructionId(cursor.u32()?),
            operand: cursor.u16()?,
            access: decode_access(cursor)?,
        },
        tag => return Err(TerminalFixedViewCopyDecodeError::UnknownFixedSite(tag)),
    })
}
...
fn decode_target(
    cursor: &mut Cursor<'_>,
) -> Result<omega_target::NativeTarget, TerminalFixedViewCopyDecodeError> {
    let architecture = match cursor.byte()? {
        0 => omega_target::Architecture::X86_64,
        1 => omega_target::Architecture::Aarch64,
        tag => return Err(TerminalFixedViewCopyDecodeError::UnknownArchitecture(tag)),
    };
    let object_format = match cursor.byte()? {
        ...
        tag => return Err(TerminalFixedViewCopyDecodeError::UnknownObjectFormat(tag)),
    };
...
fn decode_scalar(cursor: &mut Cursor<'_>) -> Result<ScalarType, TerminalFixedViewCopyDecodeError> {
    match cursor.byte()? {
        ...
        1 => {
            ...
            let sign = match cursor.byte()? {
                ...
                tag => return Err(TerminalFixedViewCopyDecodeError::UnknownIntegerSign(tag)),
            };
            let bits = cursor.u16()?;
            let integer = match carrier {
                0 => IntegerType::new(sign, bits),
                1 if sign == IntegerSign::Unsigned => IntegerType::address(bits),
                tag => return Err(TerminalFixedViewCopyDecodeError::UnknownIntegerCarrier(tag)),
            }
            .map_err(|_| TerminalFixedViewCopyDecodeError::InvalidIntegerType)?;
            Ok(ScalarType::Integer(integer))
        }
        tag => Err(TerminalFixedViewCopyDecodeError::UnknownScalarType(tag)),
    }
}
...
fn decode_integer(
    cursor: &mut Cursor<'_>,
) -> Result<IntegerValue, TerminalFixedViewCopyDecodeError> {
    match cursor.byte()? {
        0 => Ok(IntegerValue::Signed(i128::from_le_bytes(cursor.array()?))),
        1 => Ok(IntegerValue::Unsigned(u128::from_le_bytes(cursor.array()?))),
        tag => Err(TerminalFixedViewCopyDecodeError::UnknownIntegerValue(tag)),
    }
}
...
fn decode_constraint_key(
    cursor: &mut Cursor<'_>,
) -> Result<RegisterConstraintKey, TerminalFixedViewCopyDecodeError> {
    let family = match cursor.byte()? {
        ...
        tag => {
            return Err(TerminalFixedViewCopyDecodeError::UnknownConstraintFamily(
                tag,
            ));
        }
    };
...
fn decode_access(
    cursor: &mut Cursor<'_>,
) -> Result<RegisterOperandAccess, TerminalFixedViewCopyDecodeError> {
    match cursor.byte()? {
        0 => Ok(RegisterOperandAccess::Use),
        1 => Ok(RegisterOperandAccess::Def),
        2 => Ok(RegisterOperandAccess::UseDef),
        tag => Err(TerminalFixedViewCopyDecodeError::UnknownOperandAccess(tag)),
    }
}
fn decode_bool(cursor: &mut Cursor<'_>) -> Result<bool, TerminalFixedViewCopyDecodeError> {
    match cursor.byte()? {
        0 => Ok(false),
        1 => Ok(true),
        tag => Err(TerminalFixedViewCopyDecodeError::UnknownBoolean(tag)),
    }
}
...
fn decode_option_u64(
    cursor: &mut Cursor<'_>,
) -> Result<Option<u64>, TerminalFixedViewCopyDecodeError> {
    match cursor.byte()? {
        0 => Ok(None),
        1 => Ok(Some(cursor.u64()?)),
        tag => Err(TerminalFixedViewCopyDecodeError::UnknownOption(tag)),
    }
}
...
fn decode_option_u16(
    cursor: &mut Cursor<'_>,
) -> Result<Option<u16>, TerminalFixedViewCopyDecodeError> {
    match cursor.byte()? {
        0 => Ok(None),
...
) -> Result<Vec<T>, TerminalFixedViewCopyDecodeError> {
    let count = cursor.length()?;
...
fn decode_u16s(cursor: &mut Cursor<'_>) -> Result<Vec<u16>, TerminalFixedViewCopyDecodeError> {
    let count = cursor.length()?;
    let mut values = Vec::with_capacity(count.min(cursor.remaining()));
    for _ in 0..count {
...
fn decode_id<T>(
    cursor: &mut Cursor<'_>,
    constructor: fn(u64) -> Option<T>,
) -> Result<T, TerminalFixedViewCopyDecodeError> {
    let raw = cursor.u64()?;
    constructor(raw).ok_or(TerminalFixedViewCopyDecodeError::InvalidSemanticId(raw))
}
...
impl<'a> Cursor<'a> {
    ...
    fn take(&mut self, count: usize) -> Result<&'a [u8], TerminalFixedViewCopyDecodeError> {
        let end = self
            .offset
            .checked_add(count)
            .ok_or(TerminalFixedViewCopyDecodeError::Truncated)?;
        let value = self
            .encoded
            .get(self.offset..end)
            .ok_or(TerminalFixedViewCopyDecodeError::Truncated)?;
    ...
    fn array<const N: usize>(&mut self) -> Result<[u8; N], TerminalFixedViewCopyDecodeError> {
        self.take(N)?
            .try_into()
            .map_err(|_| TerminalFixedViewCopyDecodeError::Truncated)
    }
    fn byte(&mut self) -> Result<u8, TerminalFixedViewCopyDecodeError> {
        Ok(self.array::<1>()?[0])
    }
    fn u16(&mut self) -> Result<u16, TerminalFixedViewCopyDecodeError> {
        Ok(u16::from_le_bytes(self.array()?))
    }
    fn u32(&mut self) -> Result<u32, TerminalFixedViewCopyDecodeError> {
        Ok(u32::from_le_bytes(self.array()?))
    }
    fn u64(&mut self) -> Result<u64, TerminalFixedViewCopyDecodeError> {
        Ok(u64::from_le_bytes(self.array()?))
    }
    fn length(&mut self) -> Result<usize, TerminalFixedViewCopyDecodeError> {
        usize::try_from(self.u64()?).map_err(|_| TerminalFixedViewCopyDecodeError::LengthOverflow)
    }
...
mod tests {
    ...
    fn plan(policy: TerminalFixedViewCopyPolicy) -> TerminalFixedViewCopyPlan {
        ...
        TerminalFixedViewCopyPlan {
            ...
            source_ranges: TerminalLiveRangeIdentity::from_bytes([2; 32]),
            source_legality: TerminalAllocationLegalityIdentity::from_bytes([3; 32]),
    ...
    fn artifact_round_trips_both_policies_and_full_transformed_custody() {
        for policy in [
            ...
            let decoded = TerminalFixedViewCopyPlan::decode(&plan.encode()).unwrap();
    ...
    fn artifact_rejects_corruption_truncation_trailing_and_closed_tags() {
        let encoded =
            plan(TerminalFixedViewCopyPolicy::SharedEntryAfterCompareBeforeBranchV1).encode();
        let mut identity_tamper = encoded.clone();
        identity_tamper[12] ^= 1;
        assert_eq!(
            TerminalFixedViewCopyPlan::decode(&identity_tamper),
            Err(TerminalFixedViewCopyDecodeError::IdentityMismatch)
        );
        let mut wrong_magic = encoded.clone();
        wrong_magic[0] ^= 1;
        assert_eq!(
            TerminalFixedViewCopyPlan::decode(&wrong_magic),
        ...
        assert_eq!(
            TerminalFixedViewCopyPlan::decode(&policy_tag),
            Err(TerminalFixedViewCopyDecodeError::UnknownPolicy(99))
        );
        let mut source_identity_tamper = encoded.clone();
        source_identity_tamper[44] ^= 1;
        assert_eq!(
            TerminalFixedViewCopyPlan::decode(&source_identity_tamper),
            Err(TerminalFixedViewCopyDecodeError::IdentityMismatch)
        );
        let mut cursor = Cursor::new(&encoded);
        ...
        assert_eq!(
            TerminalFixedViewCopyPlan::decode(&transformed_identity_tamper),
            Err(TerminalFixedViewCopyDecodeError::TransformedIdentityMismatch)
        );
        assert_eq!(
            TerminalFixedViewCopyPlan::decode(&encoded[..encoded.len() - 1]),
            Err(TerminalFixedViewCopyDecodeError::Truncated)
        );
        let mut trailing = encoded.clone();
        trailing.push(0);
        assert_eq!(
            TerminalFixedViewCopyPlan::decode(&trailing),
            Err(TerminalFixedViewCopyDecodeError::TrailingBytes)
        );
        assert_eq!(
