...
b() { "$T/bc.exe" < "$1" > "$T/x.tape" 2>/dev/null && stamp_seed "$T/x.tape" "$SEED" "$2" >/dev/null 2>&1; }
...
seam() {  # LABEL CERT EXPECTED
  ...
  g=$(printf '%s' "$2" | python3 "${OMEGA_PATH_ALPHA_CHECKER}"/tools/refcert_to_gamma.py 2>/dev/null)
