...
The current artifact is 236,076 bytes. It accepts the bounded `OMGCHK1` binary
frame documented in the parent README and constructs exact raw `source` and
`tape` indexed byte-tree constants internally; the framing path leaves 26,064 bytes in
