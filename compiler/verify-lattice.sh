...
step "omega meaning (slice 0) — real Omega samples run Rust-free; exits match documented intent" omega omega-meaning.sh
...
if command -v python3 >/dev/null 2>&1; then
  ...
  step "tool — prover diamond (prover certs accepted by check.beta AND checker.gamma)" delta prover-diamond.sh
fi
