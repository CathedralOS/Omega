...
step "input-grid meaning TV — input-taking samples proven per documented input vector (substitution closes the program; the whole proof pipe applies per vector)" omega0-refinement input-tv.sh omega0-meaning gamma proof-kernel alpha-assembler beta corpus
step "meaning-cert cross-check — meaning-TV certs replayed through check.beta AND check_ref.py" omega0-refinement meaning-cert-diamond.sh omega0-meaning proof-kernel alpha-assembler beta corpus
