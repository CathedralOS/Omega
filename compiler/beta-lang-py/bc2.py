...
#
# SLICE 1 (this file so far): a single `proc main()` with `let` locals, assignment, the arithmetic
# operators `+ - * / %` with parens + precedence, and `return`. Grown slice by slice toward the full
# language (comparisons, `state`/`to..when` CFG, procs/params/calls/recursion, byte[]/word[] memory,
...
class Gen:
    ...
    def proc(self, p):
        ...
        # slot per local: params first (not yet used in slice 1), then each `let`.
        slots = {}
        for k, pname in enumerate(params):
            slots[pname] = k
        for s in body:
    ...
    def expr(self, e, slots):
        ...
        if e[0] == 'bin':
            ...
            self.emit(f'{BINOP[op]:5} r0, r1')            # r0 = A op B
            return
