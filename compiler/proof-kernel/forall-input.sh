...
( cd "${OMEGA_PATH_BETA_RUST}" && sh build.sh "${OMEGA_PATH_BETA_LANGUAGE}"/bc.beta >/dev/null 2>&1 ) || { echo "forall-input FAIL — bc build"; exit 1; }
...
gverdict() {  # translate a cert to checker.gamma, run it; echoes accept/reject/undecided
  gg=$(python3 "${OMEGA_PATH_GAMMA}"/refcert_to_gamma.py < "$1" 2>/dev/null) || { echo untranslatable; return; }
