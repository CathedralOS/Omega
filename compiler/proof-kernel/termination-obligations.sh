...
( cd "${OMEGA_PATH_BETA_RUST}" && sh build.sh "${OMEGA_PATH_BETA_LANGUAGE}"/bc.beta >/dev/null 2>&1 ) || { echo "termination-obligations FAIL — bc build"; exit 1; }
...
gg=$(printf '%s\n%s\n' "$DEFS" "$(printf '%s' "$cert" | python3 "${OMEGA_PATH_GAMMA}"/refcert_to_gamma.py 2>/dev/null)" | perl -e 'alarm 40; exec @ARGV' "$T/interp.exe" >/dev/null 2>&1; r=$?; [ "$r" = 1 ] && echo accept || { [ "$r" = 0 ] && echo reject || echo undecided; })
