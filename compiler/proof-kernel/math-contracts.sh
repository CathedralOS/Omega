...
#
# contract2proof.py translates each contract machine into the kernel proposition it obligates (prover
...
fail=0; cov=0; gap=0
python3 contract2proof.py < "$SRC" | grep -v UNSUPPORTED | while IFS="$(printf '\t')" read -r name prop; do
  ...
  badprop=$(python3 contract2proof.py --perturb < "$SRC" | grep "^$name	" | cut -f2)
...
nunc=$(python3 contract2proof.py < "$SRC" | grep -c UNSUPPORTED)
