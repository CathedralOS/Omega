...
#
# For each predicate proof check.beta accepts against a TRUE goal `(Rel R x… )`, the
...
# For each FALSE goal — the SAME proof aimed at a perturbed target — check.beta must
...
# Two independent routes per case: check.beta typing (accept GOAL_TRUE, reject GOAL_FALSE
...
pcase() {
  vt=$(printf '%s %s' "$2" "$3" | "$T/check.exe")          # check.beta on true goal
  vf=$(printf '%s %s' "$4" "$3" | "$T/check.exe")          # check.beta on perturbed goal
