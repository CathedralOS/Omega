//! Boundary primitive provider registry and host-call authorization.
//!
//! A boundary trait that mints or carries real-world authority is a *boundary
//! provider*. The build policy decides which providers a package is allowed to
//! reach: ordinary application code should not be able to mint new host
//! providers as a proof escape hatch (chapter 18, "Host Providers" /
...
/// Whitelist of boundary providers a package is allowed to reach, used to gate
/// host calls. Resolution walks the registry and asks whether any approved
...
impl BoundaryProviderRegistry {
    ...
    pub fn with_providers(providers: Vec<BoundaryProvider>) -> Self {
        Self { providers }
    }

    /// Registers a provider, merging effects when the trait is already present.
    pub fn register(&mut self, provider: BoundaryProvider) {
    ...
    pub fn provider(&self, trait_symbol: SymbolHandle) -> Option<&BoundaryProvider> {
        self.providers
            .iter()
...
mod tests {
    ...
    fn pure_compute_call_is_not_a_host_call() {
        ...
        let registry = BoundaryProviderRegistry::new();
        assert_eq!(
