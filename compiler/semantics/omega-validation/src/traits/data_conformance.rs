...
//!
//! Instantiating trait `default machine` bodies and synthesizing the closed
//! core derivable set (structural `equals` et al.) are the compile-time
//! execution direction and are NOT implemented yet: a missing member is an
//! error naming the machine the type must write.
...
fn validate_data_satisfies_trait(
    ...
    for requirement in program.trait_machine_signatures(trait_definition) {
        let Some((machine, state)) =
            ...
            diagnostics.push(Diagnostic::error(format!(
                "data `{type_name}` does not satisfy trait `{}`: no written machine `{type_name}::{}` (trait default instantiation / core synthesis is not implemented yet -- write the member)",
