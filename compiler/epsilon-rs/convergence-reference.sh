...
# epsilon-rs backend, or the self-hosted lowermachine) and have the trust anchor check the emitted
# certificate. This runs the same certifier down the SLOW, MEANING-DEFINING route instead: the epsilon
# program is translated to gamma (EPS_EMIT=gamma) and EXECUTED by the Rust-free reference interpreter
...
#
# The epsilon->gamma translator is itself Rust, but the epsilon-meaning diamond independently certifies
...
ASM=../beta/$BETA_SEED
( cd ../beta-lang-rs && sh build.sh ../beta-lang/bc.beta >/dev/null ) || { echo "convergence-reference FAIL -- bc build"; exit 1; }
b() { ../beta-lang-rs/build/bc.exe < "$1" > "$T/x.asm" 2>/dev/null && "$ASM" < "$T/x.asm" > "$T/x.tape" 2>/dev/null && stamp_seed "$T/x.tape" "$SEED" "$2" >/dev/null 2>&1; }
b ../delta/check.beta "$T/check.exe"   || { echo "convergence-reference FAIL -- build check.beta"; exit 1; }
b ../gamma/interp.beta "$T/interp.exe" || { echo "convergence-reference FAIL -- build interp.beta"; exit 1; }
...
ref() {
  g=$(EPS_GAMMA_INPUT="$2" EPS_EMIT=gamma ./target/debug/beta "samples/$1.alp" 2>/dev/null)
