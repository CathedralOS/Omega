...
#
# lowermachine.alp is the epsilon-WRITTEN compiler (it lowers an epsilon program to arm64 and
...
EPS_ARCH=aarch64 "$BIN" samples/lowermachine.alp "$T/lmx" >/dev/null 2>&1 || { echo "selfhost sweep FAIL — building lowermachine"; exit 1; }
...
for f in samples/*.alp; do
  ...
  EPS_ARCH=aarch64 "$BIN" "$f" "$T/ref" >/dev/null 2>&1 && codesign -f -s - "$T/ref" 2>/dev/null || { echo "  $s: reference build failed"; continue; }
