...
# certifying computations, each compiled by the SELF-HOSTED compiler, checked by the trust anchor:
...
# the trust anchor still REJECTS a tampered certificate from a self-hosted-compiled program
# (claim 2+3 = 4): delta checks the computation, not the compiler that built it.
