...
run "tag-only enum (case decl + ::variant + match)" samples/enum.alp 7
...
wrap_test "lowermachine emits the full object wrapper (header + Ltrap + _selfdata)" samples/lowermachine.alp "boundary trait Console { machine exit_process(return_code: i32); machine write_byte(b: i32); machine read_byte() -> i32; machine write_line(text: &[u8]); } data Main { console: Console; x: i32; } machine Main::main(&mut self) { self.x = 5; transition 0 { _ -> s() } state s(){} }" "$(printf '// generated by epsilon-rs aarch64 backend (slices 1-2, 4-6)\n.global _main\n.align 2\n_main:\n    sub sp, sp, #32\n    stp x29, x30, [sp]\n    mov x29, sp\n    adrp x9, _selfdata@PAGE\n    add x9, x9, _selfdata@PAGEOFF\n    str x9, [x29, #16]\n    movz w0, #5\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    ldr x9, [x29, #16]\n    str w0, [x9, #0]\n    movz w0, #0\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    b Lm0s0\nLm0s0:\n    mov w0, #0\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\nLtrap:\n    brk #0x1\n.zerofill __DATA,__bss,_selfdata,8,3')"
# lowermachine wrapper _iobyte: a program that uses write_byte gets the _iobyte .zerofill (usesio flag)
wrap_test "lowermachine emits _iobyte in the wrapper when host I/O is used" samples/lowermachine.alp "boundary trait Console { machine exit_process(return_code: i32); machine write_byte(b: i32); machine read_byte() -> i32; machine write_line(text: &[u8]); } data Main { console: Console; x: i32; } machine Main::main(&mut self) { write_byte(65); self.x = 5; transition 0 { _ -> s() } state s(){} }" "$(printf '// generated by epsilon-rs aarch64 backend (slices 1-2, 4-6)\n.global _main\n.align 2\n_main:\n    sub sp, sp, #32\n    stp x29, x30, [sp]\n    mov x29, sp\n    adrp x9, _selfdata@PAGE\n    add x9, x9, _selfdata@PAGEOFF\n    str x9, [x29, #16]\n    movz w0, #65\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    adrp x9, _iobyte@PAGE\n    add x9, x9, _iobyte@PAGEOFF\n    strb w0, [x9]\n    mov x0, #1\n    adrp x1, _iobyte@PAGE\n    add x1, x1, _iobyte@PAGEOFF\n    mov x2, #1\n    bl _write\n    movz w0, #5\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    ldr x9, [x29, #16]\n    str w0, [x9, #0]\n    movz w0, #0\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    b Lm0s0\nLm0s0:\n    mov w0, #0\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\nLtrap:\n    brk #0x1\n.zerofill __DATA,__bss,_selfdata,8,3\n.zerofill __DATA,__bss,_iobyte,1,0')"
# lowermachine wrapper Lstr: write_line string literals -> __TEXT,__const + Lstr<i>: .byte tables (incl empty)
wrap_test "lowermachine emits the Lstr string tables in the wrapper" samples/lowermachine.alp "boundary trait Console { machine exit_process(return_code: i32); machine write_byte(b: i32); machine read_byte() -> i32; machine write_line(text: &[u8]); } data Main { console: Console; } machine Main::main(&mut self) { self.console.write_line(\"ab\"); self.console.write_line(\"\"); transition 0 { _ -> s() } state s(){} }" "$(printf '// generated by epsilon-rs aarch64 backend (slices 1-2, 4-6)\n.global _main\n.align 2\n_main:\n    sub sp, sp, #32\n    stp x29, x30, [sp]\n    mov x29, sp\n    mov x0, #1\n    adrp x1, Lstr0@PAGE\n    add x1, x1, Lstr0@PAGEOFF\n    movz w2, #3\n    bl _write\n    mov x0, #1\n    adrp x1, Lstr1@PAGE\n    add x1, x1, Lstr1@PAGEOFF\n    movz w2, #1\n    bl _write\n    movz w0, #0\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    b Lm0s0\nLm0s0:\n    mov w0, #0\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\nLtrap:\n    brk #0x1\n.zerofill __DATA,__bss,_iobyte,1,0\n.section __TEXT,__const\n.align 2\nLstr0:\n    .byte 97,98,10\nLstr1:\n    .byte 10')"
...
wrap_test "lowermachine splits a large integer literal (100000) into movz+movk" samples/lowermachine.alp "boundary trait Console { machine exit_process(return_code: i32); machine write_byte(b: i32); machine read_byte() -> i32; machine write_line(text: &[u8]); } data Main { console: Console; r: i32; } machine Main::main(&mut self) { self.r = 100000; transition 0 { _ -> s() } state s(){} }" "$(printf '// generated by epsilon-rs aarch64 backend (slices 1-2, 4-6)\n.global _main\n.align 2\n_main:\n    sub sp, sp, #32\n    stp x29, x30, [sp]\n    mov x29, sp\n    adrp x9, _selfdata@PAGE\n    add x9, x9, _selfdata@PAGEOFF\n    str x9, [x29, #16]\n    movz w0, #34464\n    movk w0, #1, lsl #16\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    ldr x9, [x29, #16]\n    str w0, [x9, #0]\n    movz w0, #0\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    b Lm0s0\nLm0s0:\n    mov w0, #0\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\nLtrap:\n    brk #0x1\n.zerofill __DATA,__bss,_selfdata,8,3')"
# newline-separated statements (no semicolons): write_line must not swallow the following statement
wrap_test "lowermachine keeps a statement that follows write_line with no semicolon" samples/lowermachine.alp "boundary trait Console { machine exit_process(return_code: i32); machine write_byte(b: i32); machine read_byte() -> i32; machine write_line(text: &[u8]); } data Main { console: Console; } machine Main::main(&mut self) { self.console.write_line(\"ab\") self.console.exit_process(0) }" "$(printf '// generated by epsilon-rs aarch64 backend (slices 1-2, 4-6)\n.global _main\n.align 2\n_main:\n    sub sp, sp, #32\n    stp x29, x30, [sp]\n    mov x29, sp\n    mov x0, #1\n    adrp x1, Lstr0@PAGE\n    add x1, x1, Lstr0@PAGEOFF\n    movz w2, #3\n    bl _write\n    movz w0, #0\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\n    mov w0, #0\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\nLtrap:\n    brk #0x1\n.zerofill __DATA,__bss,_iobyte,1,0\n.section __TEXT,__const\n.align 2\nLstr0:\n    .byte 97,98,10')"
# free machines (no &mut self): params in w0.. (no self reg shift), no self slot/store, bare-name call
wrap_test "lowermachine lowers a free machine (no self) and its bare-name call" samples/lowermachine.alp "boundary trait Console { machine exit_process(return_code: i32); machine write_byte(b: i32); machine read_byte() -> i32; machine write_line(text: &[u8]); } data Main { console: Console; x: i32; } machine add2(a: i32, b: i32) -> i32 { transition 0 { _ -> s() } state s(){ return a + b; } } machine Main::main(&mut self) { self.x = add2(7, 19); transition 0 { _ -> t() } state t(){} }" "$(printf '// generated by epsilon-rs aarch64 backend (slices 1-2, 4-6)\n.global _main\n.align 2\nLmachine0:\n    sub sp, sp, #32\n    stp x29, x30, [sp]\n    mov x29, sp\n    str w0, [x29, #16]\n    str w1, [x29, #24]\n    movz w0, #0\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    b Lm0s0\nLm0s0:\n    ldr w0, [x29, #16]\n    str x0, [sp, #-16]!\n    ldr w0, [x29, #24]\n    str x0, [sp, #-16]!\n    ldr x1, [sp], #16\n    ldr x0, [sp], #16\n    adds w0, w0, w1\n    b.vs Ltrap\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\n    mov w0, #0\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\n_main:\n    sub sp, sp, #32\n    stp x29, x30, [sp]\n    mov x29, sp\n    adrp x9, _selfdata@PAGE\n    add x9, x9, _selfdata@PAGEOFF\n    str x9, [x29, #16]\n    movz w0, #7\n    str x0, [sp, #-16]!\n    movz w0, #19\n    str x0, [sp, #-16]!\n    ldr x1, [sp], #16\n    ldr x0, [sp], #16\n    bl Lmachine0\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    ldr x9, [x29, #16]\n    str w0, [x9, #0]\n    movz w0, #0\n    str x0, [sp, #-16]!\n    ldr x0, [sp], #16\n    b Lm1s0\nLm1s0:\n    mov w0, #0\n    mov sp, x29\n    ldp x29, x30, [sp]\n    add sp, sp, #32\n    ret\nLtrap:\n    brk #0x1\n.zerofill __DATA,__bss,_selfdata,8,3')"
