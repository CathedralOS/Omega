...
( cd "${OMEGA_PATH_BETA_RUST}" && sh build.sh "${OMEGA_PATH_BETA_LANGUAGE}"/bc.beta >/dev/null 2>&1 ) || { echo "meaning-cert diamond FAIL — bc build"; exit 1; }
...
b omega2gamma.beta     "$T/omega2gamma.exe"    || { echo "meaning-cert diamond FAIL — build omega2gamma.beta"; exit 1; }
b "${OMEGA_PATH_PROOF_KERNEL}"/check.beta  "$T/check.exe"  || { echo "meaning-cert diamond FAIL — build check.beta"; exit 1; }
...
python3 meaning_cert_diamond.py "$T/check.exe" "$T/interp.exe" "${OMEGA_PATH_GAMMA}"/checker.gamma $SPECS
