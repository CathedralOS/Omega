...
b ../proof-kernel/check.beta  "$T/check.exe"  || { echo "meaning-cert diamond FAIL — build check.beta"; exit 1; }
...
for s in bounded_counter nested_counters euclid_gcd leap_year smallest_prime_factor number_guess \
  ...
  "$T/omega2gamma.exe" < "../lattice-corpus/$s/main.omg" > "$T/g" 2>/dev/null
