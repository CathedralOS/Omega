...
fn layer_of(manifest_path: &str) -> Option<&'static str> {
    ...
    if m("/apps/") {
        Some("app")
    } else if m("/compiler/omega/foundation/") || m("/compiler/psi/foundation/") {
        Some("foundation")
    } else if m("/compiler/omega/representations/") || m("/compiler/psi/representations/") {
        Some("representations")
    } else if m("/compiler/omega/semantics/") || m("/compiler/psi/semantics/") {
        Some("semantics")
    } else if m("/compiler/omega/pipeline/") || m("/compiler/psi/pipeline/") {
        Some("pipeline")
    } else if m("/compiler/omega/orchestration/") {
        Some("orchestration")
    } else if m("/compiler/omega/backend/instruction_set_architectures/") {
        Some("isa")
    } else if m("/compiler/omega/backend/images/") {
        Some("images")
    } else if m("/compiler/omega/backend/object/") {
        Some("object")
    } else if m("/compiler/omega/backend/") {
        Some("backend")
    } else {
...
fn frontend_implementation_is_psi_owned() {
    ...
    for relative in [
        "compiler/omega/foundation/omega-core/src/arithmetic.rs",
        "compiler/omega/foundation/omega-core/src/arena/mod.rs",
        "compiler/omega/foundation/omega-core/src/atomic.rs",
        "compiler/omega/foundation/omega-core/src/bignum.rs",
        "compiler/omega/foundation/omega-core/src/byte_predicates.rs",
        "compiler/omega/foundation/omega-core/src/cast_form.rs",
        "compiler/omega/foundation/omega-core/src/const_value.rs",
        "compiler/omega/foundation/omega-core/src/content.rs",
        "compiler/omega/foundation/omega-core/src/diagnostics/mod.rs",
        "compiler/omega/foundation/omega-core/src/float_semantics.rs",
        "compiler/omega/foundation/omega-core/src/inline_assembly.rs",
        "compiler/omega/foundation/omega-core/src/literals.rs",
        "compiler/omega/foundation/omega-core/src/operator_spelling.rs",
        "compiler/omega/foundation/omega-core/src/semantics.rs",
        "compiler/omega/foundation/omega-core/src/source",
        "compiler/omega/foundation/omega-core/src/span.rs",
        "compiler/omega/foundation/omega-core/src/symbols/mod.rs",
        "compiler/omega/foundation/omega-core/src/trust.rs",
        "compiler/omega/foundation/omega-core/src/value_domain.rs",
        "compiler/omega/foundation/omega-core/src/wire.rs",
    ] {
        assert!(
...
fn retired_omega_frontend_adapters_do_not_return() {
    let root = workspace_root();
    for relative in [
        "compiler/omega/pipeline/omega-source-files-to-tokens",
        "compiler/omega/pipeline/omega-tokens-to-syntax-trees",
        "compiler/omega/pipeline/omega-syntax-trees-to-symbol-resolved-trees",
        "compiler/omega/pipeline/omega-symbol-resolved-trees-to-typed-trees",
        "compiler/omega/pipeline/omega-typed-trees-to-checked-trees",
        "compiler/omega/representations/omega-tokens",
        "compiler/omega/representations/omega-symbol-resolved-trees",
        "compiler/omega/representations/omega-syntax-trees",
        "compiler/omega/representations/omega-typed-trees",
        "compiler/omega/representations/omega-facts",
        "compiler/omega/representations/omega-checked-trees",
        "compiler/omega/semantics/omega-proof",
        "compiler/omega/semantics/omega-types",
        "compiler/omega/semantics/omega-validation",
    ] {
        assert!(
...
fn provider_approval_stays_in_omega_after_psi_checking() {
    let root = workspace_root();
    let psi_checks =
        root.join("compiler/psi/pipeline/psi-typed-trees-to-checked-trees/src/checks.rs");
    ...
    let omega_approval = root
        .join("compiler/omega/orchestration/omega-compiler/src/pipeline/provider_approval.rs");
...
fn target_neutral_effect_inference_is_psi_owned() {
    let root = workspace_root();
    let legacy = root.join("compiler/omega/representations/omega-effects/src/lib.rs");
    ...
    let providers = root
        .join("compiler/omega/representations/omega-effects/src/capabilities/provider_plan.rs");
    assert!(
        providers.exists(),
...
fn checked_semantics_are_psi_owned_without_provider_realization() {
    let root = workspace_root();
    let manifest = root.join("compiler/psi/representations/psi-checked-trees/Cargo.toml");
    ...
    let checked_root = root.join("compiler/psi/representations/psi-checked-trees/src");
    ...
    let omega_provider_carrier =
        root.join("compiler/omega/representations/omega-effects/src/selected_provider_plans.rs");
    assert!(
        omega_provider_carrier.exists(),
        "selected concrete provider plans must remain in the Omega provider subsystem"
    );
    let omega_task_carrier = root.join("compiler/omega/foundation/omega-task-plans/src/lib.rs");
...
fn first_terminal_psi_source_slice_stays_fail_closed() {
    let root = workspace_root();
    let path = root.join("compiler/psi/pipeline/psi-checked-trees-to-terminal/src/lib.rs");
    let source = std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
    ...
    let manifest_path =
        root.join("compiler/psi/pipeline/psi-checked-trees-to-terminal/Cargo.toml");
    ...
    assert!(
        !root
            .join("compiler/omega/pipeline/omega-checked-trees-to-terminal-psi")
            .exists(),
...
fn admitted_external_root_entry_fact_cannot_detach_before_body_dispatch() {
    let root = workspace_root();
    let path =
        root.join("compiler/omega/orchestration/omega-compiler/src/pipeline/provider_plans.rs");
    let source = std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
...
fn program_storage_entry_activation_cannot_detach_before_executor_dispatch() {
    let root = workspace_root();
    let path = root.join(
        "compiler/omega/orchestration/omega-compiler/src/pipeline/program_storage_entry.rs",
    );
    let source = std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
...
fn typed_frontend_does_not_retain_concrete_calling_conventions() {
    let root = workspace_root();
    let manifest = root.join("compiler/psi/representations/psi-typed-trees/Cargo.toml");
    let manifest_source = std::fs::read_to_string(&manifest)
        .unwrap_or_else(|error| panic!("failed to read {}: {error}", manifest.display()));
    assert!(
    ...
    let representation =
        root.join("compiler/psi/representations/psi-typed-trees/src/typed_trees.rs");
...
fn psi_reference_execution_ownership_and_terminal_lane_are_enforced() {
    let root = workspace_root();
    assert!(
        root.join("compiler/psi/semantics/psi-terminal-interpreter/src/lib.rs")
            .is_file(),
        "canonical terminal-Psi reference execution must remain Psi-owned"
    );
    assert!(
        root.join("compiler/psi/semantics/psi-checked-interpreter/src/lib.rs")
            .is_file(),
        "transitional checked-tree reference execution must remain Psi-owned"
    );
    assert!(
        !root
            .join("compiler/omega/orchestration/omega-interpreter")
            .exists(),
        "the retired production Omega interpreter must not return"
    );
    assert!(
        root.join("compiler/omega/orchestration/omega-native-differential-test/src/lib.rs")
            .is_file(),
