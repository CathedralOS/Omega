...
fn backend_crates_do_not_depend_on_frontend_crates() {
    ...
    let backend_root = repo_root.join("compiler/omega/backend");
    let forbidden = [
        "omega-syntax-trees",
...
fn representation_crates_do_not_depend_on_frontend_crates() {
    let repo_root = repo_root();
    let representations_root = repo_root.join("compiler/omega/representations");
    let forbidden = [
        "omega-syntax-trees",
        "omega-tokens-to-syntax-trees",
        "omega-source-files-to-tokens",
...
fn lowering_crates_do_not_depend_on_final_machinery_crates() {
    ...
    let repo_root = repo_root();
    let lowering_root = repo_root.join("compiler/omega/pipeline");
    let forbidden_paths = [
...
fn artifact_crates_do_not_depend_on_native_bridge() {
    let repo_root = repo_root();
    let orchestration_root = repo_root.join("compiler/omega/orchestration");
