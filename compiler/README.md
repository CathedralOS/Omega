...
One tower, built bottom-up, aimed at a single summit: **Omega**, a self-hosting systems
language whose programs ship with a machine-checked proof of their own safety and
correctness — bounds, overflow, termination, contracts (`requires`/`ensures`) — validated
by a checker small enough to **audit by hand**.

The rung names (alpha, beta, gamma, delta, epsilon, omega) are levels of one ladder, each
built *and checked* by the rung beneath it. The only thing taken on faith is the ~300-line
...
```sh
sh verify-lattice.sh        # seed → assembler → bc → checker → Omega meaning route, in order
```

The lattice has two jobs, and everything serves them:

1. **Produce a proof-checker you can actually believe** — that's `delta`, the trust anchor.
2. **Give Omega's programs a way to emit proofs to it** — the `omega` meaning route and the
   convergence loop.
...
| **γ Gamma** | a safe functional language: ADTs + pattern matching | an interpreter and a static type checker (kept) | interpreter and type-checker gates; the tier also hosts a second, independent copy of the checker |
| **δ Delta** | the certificate language — the trust anchor | **check.beta**, the checker (kept — the point of it all) | defended several independent ways (below) |
| **ω Omega** | the systems-language summit | the Rust-free meaning route, `omega2gamma` (kept, growing), and `omega-rs`, the untrusted native producer | a triple diamond + the convergence loop |

**On the names.** The greek letters track *languages*, so a tier's tooling takes the tier's
name — with one historical exception: the **assembler** lives in `beta/` and the gates label
its step `beta`, but it is an **Alpha** tool (written in Alpha, it turns Alpha assembly into
bytecode). Read "the beta assembler" as "Alpha's assembler," and reserve **Beta** for the
language `bc` compiles. `epsilon` was a tier too; it folded into Omega (decision D7) as the
**kernel subset**, with `epsilon-rs` kept as that subset's disposable native producer.

## Why a compiler project is full of math proofs

Three reasons, in order of how directly they aim at Omega.

**1. `delta` *decides* proofs, and these are its test cases.** It doesn't find proofs; it
...
proof one range fits inside a wider one. So `delta` banks a growing library of
...
today (`omega/convergence-reference.sh`, `epsilon-rs/convergence.sh`): a program computes
something — a sum, a sort, a gcd, a bounds check — **and emits a `delta` certificate that
...
The library stands at **238 machine-checked theorems** today (`delta/README.md`,
...
## `delta` — why you can believe the trust anchor
...
- **`convergence-reference.sh`** — the proof-carrying loop with no Rust anywhere: certifiers,
  including Omega's own safety obligations, are translated, *run*, and their `delta`
...
- `delta`'s `fun` rules pattern-match user constructors only, and function application is
  binary-max.
