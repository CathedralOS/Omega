...
fn current_vocabulary_has_one_stable_canonical_encoding_and_identity() {
    ...
    assert_eq!(
        ...
        "ee533b0367553bf69779a8ca6495290204157099f4b923e4e8837e0d3359f17d"
    );
    assert_eq!(
...
fn partial_affine_unit_return_round_trips_exact_path_and_leaf_type() {
    ...
    assert_eq!(&bytes[8..10], 7_u16.to_le_bytes());
    assert_eq!(&bytes[10..12], 12_u16.to_le_bytes());
    assert_eq!(decode_module(&bytes), Ok(module.clone()));
    assert_eq!(encode_module(&decode_module(&bytes).unwrap()), Ok(bytes));
}
...
fn decoder_rejects_noncanonical_or_ambiguous_bytes() {
    ...
    future_format[8..10].copy_from_slice(&8_u16.to_le_bytes());
    assert_eq!(
        decode_module(&future_format),
        Err(CodecError::UnsupportedFormatMarker(8))
    );
