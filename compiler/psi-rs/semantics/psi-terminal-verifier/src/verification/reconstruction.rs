...
fn retained_alias_substituted_cast_bound(
    ...
    retained_stronger_alias_substituted_cast_bound(context, goal, requirements, semantic_axioms)
        || retained_landed_literal_via_alias_cast_bound(
            context,
            goal,
            requirements,
            semantic_axioms,
...
fn retained_two_alias_substituted_cast_bound(
    ...
    retained_two_alias_substituted_bound(requirements, semantic_axioms, |root, root_bound| {
        retained_cast_bound_from_root(context, goal, semantic_axioms, root, root_bound)
    })
}

/// Independently select one retained order bound transported through exactly
/// two distinct, same-carrier value equalities. The depth is structural and
/// fixed; this helper exposes no recursive or parameterized alias search.
fn retained_two_alias_substituted_bound(
    requirements: &[Proposition],
    semantic_axioms: &[Proposition],
    mut complete: impl FnMut(&ScalarTerm, &Proposition) -> bool,
) -> bool {
            ...
            .any(|(root, alias)| {
                facts()
                    ...
                    .any(|(bound_left, bound_right)| {
                        ...
                        root.scalar_type() == psi_core::ScalarType::Integer(integer_type)
                            && retained_cast_bound_from_stronger_alias_bound(
                                context,
                                goal,
                                semantic_axioms,
                                root,
...
fn retained_cast_bound_from_stronger_alias_bound(
    context: &PropositionContext,
    goal: &Proposition,
    semantic_axioms: &[Proposition],
    root: &ScalarTerm,
