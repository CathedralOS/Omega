...
use psi_symbol_resolved_trees::domain::{
...
pub(crate) fn lower_domain_definition(
    ...
    let mut index_arguments = psi_arena::HandleSpan::empty();
...
fn lower_domain_operators(
    lowerer: &mut Lowerer,
    syntax_trees: &SyntaxTrees,
    operators: psi_arena::HandleSpan<syntax::item::OperatorDefinition>,
) -> Result<
    psi_arena::HandleSpan<psi_symbol_resolved_trees::operator::OperatorDefinition>,
    Diagnostic,
> {
...
pub(crate) fn lower_proof_facts(
    lowerer: &mut Lowerer,
    syntax_trees: &SyntaxTrees,
    facts: psi_arena::HandleSpan<syntax::item::ProofFact>,
