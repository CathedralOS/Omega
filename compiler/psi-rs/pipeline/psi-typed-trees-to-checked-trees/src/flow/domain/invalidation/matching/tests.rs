...
fn integer_expression(program: &mut psi_typed_trees::TypedTrees, value: i64) -> ExpressionHandle {
    program
        .expression_table
...
fn name_expression(program: &mut psi_typed_trees::TypedTrees) -> ExpressionHandle {
    program
        .expression_table
...
fn dependency_facts(
    ...
    dependency_segments: &[psi_facts::PlaceSegment],
) -> DomainFacts {
...
fn indexed_domain_dependency_ignores_disjoint_literal_index_mutations() {
    ...
    let fact_place = CanonicalPlace {
        root: psi_facts::PlaceRoot::Symbol(entries_symbol),
        segments: vec![psi_facts::PlaceSegment::Index { expression: zero }],
    };
    let mutated_place = CanonicalPlace {
        root: psi_facts::PlaceRoot::Symbol(entries_symbol),
        segments: vec![
            psi_facts::PlaceSegment::Index { expression: one },
            psi_facts::PlaceSegment::Field { symbol: tag_symbol },
        ],
    };
