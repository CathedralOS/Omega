...
fn nested_aggregate_call_cast_preserves_field_paths_and_borrow_polarity() {
    ...
    assert!(
        nested
            .iter()
            .any(|loan| { loan.kind == psi_checked_trees::BorrowAccessKind::Read })
    );
    assert!(
        nested
            .iter()
            .any(|loan| { loan.kind == psi_checked_trees::BorrowAccessKind::Mutable })
    );
}
...
fn assert_borrow_conflict(diagnostics: &[psi_diagnostics::Diagnostic], source: &str, owner: &str) {
    let combined = diagnostics
        .iter()
        .map(|diagnostic| diagnostic.message.as_str())
        .collect::<Vec<_>>()
