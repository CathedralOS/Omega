...
fn materializes_basic_move_and_drop_events() {
    ...
    assert_eq!(
        moves[0].source,
        psi_checked_trees::FlowOwnershipEventSource::Statement { statement_index: 0 }
    );
    assert_eq!(
        moves[1].source,
        psi_checked_trees::FlowOwnershipEventSource::Statement { statement_index: 1 }
    );
    assert!(
        !flow
            .ownership
    ...
    assert_eq!(
        drops[0].source,
        psi_checked_trees::FlowOwnershipEventSource::StateExit
    );
...
fn materializes_by_value_call_argument_moves() {
    ...
    let psi_checked_trees::FlowOwnershipEventSource::Call {
        statement_index,
        call_ordinal,
        target_symbol,
    } = moves[0].source
...
fn materializes_explicit_static_by_value_self_transfer() {
    ...
    assert_eq!(moves[0].root, psi_facts::PlaceRoot::Symbol(item.symbol));
}
...
fn checked_flow(source: &str) -> (psi_typed_trees::TypedTrees, psi_checked_trees::FlowFacts) {
...
fn main_state_flow<'flow>(
    ...
    flow: &'flow psi_checked_trees::FlowFacts,
) -> &'flow psi_checked_trees::FlowStateFact {
    let main_machine = typed
        .machines()
        .iter()
        .find(|machine| machine.name.as_str() == "Main::main")
...
fn materializes_operator_result_owned_production_moves() {
    ...
    assert_eq!(moves[0].root, psi_facts::PlaceRoot::Symbol(local_symbol));
    assert_eq!(
        moves[1].source,
        psi_checked_trees::FlowOwnershipEventSource::Statement { statement_index: 1 }
    );
...
fn materializes_state_exit_drops_for_owned_by_value_parameters() {
    ...
    assert_eq!(
        drops[0].root,
        psi_facts::PlaceRoot::Symbol(parameter_symbol)
    );
}
