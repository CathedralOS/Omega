...
fn exit_ensures_requirement_label_resolves_attached_data_members() {
    ...
    let exit_context = semantic
        .contexts_at_point(psi_facts::ProgramPoint::Exit {
            machine_symbol: machine.symbol,
    ...
    let psi_facts::FactPlace::Place(place_handle) = fact.place else {
    ...
    let value_expression = match fact.payload {
        psi_facts::FactPayload::ContractDomainMembership { value, .. } => value,
    ...
    assert_eq!(place.root, psi_facts::PlaceRoot::Symbol(self_symbol));
    ...
    let mut scratch = psi_facts::build_definition_fact_plan(&typed);
        ...
        .flat_map(|context_ref| {
            ...
            semantic
                ...
                .filter_map(|fact| match fact.payload {
                    psi_facts::FactPayload::DomainMembership { domain_symbol, .. }
                    | psi_facts::FactPayload::ContractDomainMembership { domain_symbol, .. }
                        if typed
                            .domain_definitions()
                            .iter()
