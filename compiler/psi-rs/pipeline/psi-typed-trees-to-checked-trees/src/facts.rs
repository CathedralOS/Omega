...
fn build_contract_plans(
    ...
    for machine in program.machines() {
        ...
        let suspension = psi_language_semantics::SuspensionPlan {
...
/// no entry.
fn build_qualification_facts(program: &TypedTrees) -> psi_checked_trees::QualificationFacts {
    use psi_checked_trees::VacuousQualificationUse;
    ...
    fn collect_casts(
        ...
        committed: &mut Vec<psi_language_semantics::SemanticDomainId>,
        ...
        match program.expression_table.expression(expression) {
            ExpressionNode::Cast(cast) => {
                let policy = match cast.domain {
                    psi_numerics::arithmetic::ArithmeticDomain::Exact => None,
    ...
    for machine in program.machines() {
        ...
        if !committed.is_empty() {
            machines.push(psi_checked_trees::MachineQualifications {
                machine: machine.symbol,
                body_committed: committed,
            });
        }
    }
    psi_checked_trees::QualificationFacts {
        machines,
        vacuous_uses,
        content: psi_checked_trees::ContentProjectionFacts {
            plans: psi_validation::build_content_projection_plans(program),
            conservation_plans: psi_validation::build_content_conservation_plans(program)
                .into_iter()
...
fn build_service_reach_facts(
    program: &TypedTrees,
    inferred: psi_effects::ServiceReachInferencePlan,
) -> psi_checked_trees::ServiceReachFacts {
    psi_checked_trees::ServiceReachFacts {
        ...
        states: inferred
            .states
            .map(|state| psi_checked_trees::StateServiceReachRows {
                state: state.state,
        ...
        calls: inferred
            .calls
            .map(|call| psi_checked_trees::CallServiceReachRows {
...
fn remap_service_reach_span<From, To>(
    span: psi_arena::HandleSpan<From>,
) -> psi_arena::HandleSpan<To> {
    let start = span.start();
    psi_arena::HandleSpan::from_parts(
        psi_arena::Handle::from_parts(start.arena_index(), start.generation()),
        span.count(),
    )
}
