...
use psi_syntax_trees::expression::{BinaryOperator, ExpressionNode, TableBinaryExpression};
...
fn const_literal_value(value: &psi_numerics::literals::IntegerLiteral) -> Result<i128, String> {
    value
        .value_i64()
        .map(i128::from)
...
fn apply_in_domain_suffix<'tokens, 'source>(
    ...
    loop {
        ...
        if domain_name.as_str() == "Carry::Portable" {
            ...
            constraints.extend(psi_language_core::CarryPermission::ALL.map(|permission| {
...
pub(super) fn parse_optional_lifetime<'tokens, 'source>(
    input: Input<'tokens, 'source>,
) -> ParseResult<'tokens, 'source, Option<psi_syntax_trees::identifier::Identifier>> {
    if !input.at_punctuation(PunctuationKind::Apostrophe) {
                    ...
                    let maximum = match syntax_trees.expressions.expression(end_exclusive) {
                        ExpressionNode::Integer(literal) => {
                            ...
                            syntax_trees.expressions.insert(ExpressionNode::Integer(
                                psi_numerics::literals::IntegerLiteral::from_value(value - 1),
                            ))
                        }
                        _ => {
