...
//! `self.f = X` (and a constructed `T { f: X }`, handled in psi-validation for
...
/// Mirrors psi-validation `struct_literals::construction_field_type` + domain
/// extraction.
    ...
    facts
        ...
        .any(|context_handle| {
            ...
            facts.semantic.context_view(context).facts().any(|fact| {
                ...
                let psi_facts::FactPlace::Place(fact_place) = fact.place else {
                    return false;
                };
