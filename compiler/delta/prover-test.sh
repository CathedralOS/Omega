...
# HAND-WRITTEN .elab proofs (gen-contract-lib.py). The prover proves 5 of those 7 banked lemmas AUTOMATICALLY,
# by search -- add-zero-right (id 0), add-commutes (id 5), le-trans (id 9 -- here), mult-commutes (id 20), and
# add-assoc (id 21) -- all proved above and kernel-accepted. So the contract lemma base could be GENERATED by
# automation instead of hand-coded: "automation discharges with zero hand-proving" (rungs/omega.md), on the
# lattice's OWN contract system. The remaining 2 (mult-assoc id 28; lt-le-trans id 32, the mixed < / <= chain)
# are the prover's roadmap to fully replace the hand base.
