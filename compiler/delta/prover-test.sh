...
# THE BRIDGE: the prover AUTOMATICALLY proves the exact lemma base the epsilon contract compiler banks by hand
# (epsilon-rs/src/discharge.rs) and cites at obligation site terms -- add-zero-right (id 0), add-commutes (id
# 5), le-trans (id 9). So the lattice's real contract-discharge lemma base could be GENERATED by automation
# instead of hand-coded: "automation discharges with zero hand-proving" (rungs/omega.md). (id 0 = x+0=x and id
# 5 = x+y=y+x are proved above; here the order chainer.)
