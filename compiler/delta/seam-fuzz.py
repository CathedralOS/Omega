#!/usr/bin/env python3
# Random broad coverage of the gamma/delta soundness seam across the checker's reduction paths: built-in
...
def ulist(lst):  # user-list: nil' = (k 2), cons' h t = (k 3 h t); heads are built-in DELTA nats
    out = "(k 2)"
    for x in reversed(lst):
        out = "(k 3 %s %s)" % (rnat(x, DELTA), out)
    return out
