...
# happens to run). omega-rs discharges termination obligations statically; this gate does the delta analogue
...
ASM=../beta/$BETA_SEED
( cd ../beta-lang-rs && sh build.sh ../beta-lang/bc.beta >/dev/null 2>&1 ) || { echo "termination-obligations FAIL — bc build"; exit 1; }
...
b check.beta           "$T/check.exe"  || { echo "termination-obligations FAIL — build check.beta"; exit 1; }
b ../gamma/interp.beta "$T/interp.exe" || { echo "termination-obligations FAIL — build interp.beta"; exit 1; }
DEFS=$(cat ../gamma/checker.gamma)
LEMMA=proofs/slice-tail-shrink.elab

# ---- 1. the measure-decrease lemma, verified by all three independent checkers ----
cert=$(python3 elab.py < "$LEMMA" 2>/dev/null)
...
echo "  measure-decrease lemma (len t < len(cons h t)): check.beta=$gb check_ref=$gr checker.gamma=$gg"

# ---- 2. NEGATIVE control: the reversed measure (len(cons h t) < len t = the measure GROWS) must be rejected ----
neg=$(sed 's/(all h (all t (ex k (= (+ (len t) (s k)) (len (cons h t))))))/(all h (all t (ex k (= (+ (len (cons h t)) (s k)) (len t)))))/' "$LEMMA" | python3 elab.py 2>/dev/null)
...
echo "  negative control (reversed measure grows): check.beta=$nb check_ref=$nr -> rejected=$neg_ok"
