...
#
# delta.md's whole point: "this checker has the only authority — a false proposition
...
../beta-lang-rs/build/bc.exe < check.beta > "$T/c.asm" || { echo "bc(check.beta) failed"; exit 1; }
"$ASM" < "$T/c.asm" > "$T/c.tape" || { echo "assemble failed"; exit 1; }
