...
# check.beta on propositional logic.
#
...
# (compiled by bc). check_ref.py is a third, INDEPENDENT realization in Python of the checker's propositional
# core (->, &, +, bot intro+elim), short enough to read against the natural-deduction rules. This gate fuzzes
# it against check.beta on random propositional proofs, requiring identical accept/reject (both accept a
# proof against its true goal; both reject it against a perturbed, wrong-type goal). So the propositional
# soundness of the trust anchor is pinned by an independent, auditable implementation — the last rung to get
# one. UNTRUSTED and checked; the runtime never runs it. (Quantifiers / equality-conversion are later slices.)
set -e
cd "$(dirname "$0")"
...
if python3 check-ref-fuzz.py "$T/check.exe" "${1:-200}" > "$T/out" 2>&1; then
  echo "trust-anchor diamond (independent check_ref.py agrees with check.beta on propositional proofs): $(cat "$T/out")"
else
