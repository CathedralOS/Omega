...
fn machine_record_still_conflates_guarantee_and_witness() {
    fn witness(machine: Machine) {
        let Machine {
            ...
            name: _,
            attached_data: _,
            boundary: _,     // the whole supply-mode story, as one bool
            type_parameters: _,
            contains: _,
            owned_data: _,
            satisfies: _,
            terminates: _,   // guarantee AND witness, as one bool
            decreases: _,    // the private witness material, in the interface record
            decrease_order: _,
            effects: _,      // decision 22's kinded rows, as a flat name span
            contracts: _,
            states: _,
        } = machine;
    }
