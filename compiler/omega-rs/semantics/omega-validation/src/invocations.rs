...
pub(crate) fn validate_invocation_contracts(
    ...
    let plan = omega_effects::infer_synchronous_invocations(program);
...
fn validate_target_names(
    program: &TypedTrees,
    owner: &str,
    targets: &[omega_typed_trees::name::Identifier],
...
fn validate_conformance_refinement(
    program: &TypedTrees,
    machine: &Machine,
    summary: &omega_effects::MachineInvocationInference,
    diagnostics: &mut Vec<Diagnostic>,
) {
    for conformance in program.machine_trait_conformances(machine) {
        ...
        for requirement in matching {
            let allowed = omega_effects::declared_signature_invocations(program, requirement);
            ...
            let actual = summary
                ...
                .filter_map(|target| match *target {
                    omega_effects::InvocationTarget::Parameter(0) if self_forwarded => None,
                    omega_effects::InvocationTarget::Parameter(index) if self_forwarded => {
                        Some(omega_effects::InvocationTarget::Parameter(index - 1))
                    }
                    target => Some(target),
                })
                .collect::<Vec<_>>();
...
fn report_missing_targets(
    program: &TypedTrees,
    parameters: &[StateParameter],
    actual: &[omega_effects::InvocationTarget],
    allowed: &[omega_effects::InvocationTarget],
...
fn invocation_target_label(
    ...
    match target {
        omega_effects::InvocationTarget::Parameter(index) => parameters
            .iter()
            .filter(|parameter| !parameter.is_self)
            .nth(index as usize)
...
fn boundary_trait_by_name<'a>(
    ...
    name: &str,
) -> Option<&'a omega_typed_trees::trait_definition::TraitDefinition> {
    let exact = program
        .traits()
        .iter()
