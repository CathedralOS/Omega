...
/// MUST equal the emitter exactly. Relocs: frame @+2, target region
/// @+FRAME_FIXED_INDEXED_COPY_TARGET_IMM_OFFSET+2.
pub fn runtime_storage_copy_from_runtime_frame_fixed_indexed_to_runtime_storage_width(
    ...
    // + mov r14,[r14+desc] (7) + mov r15,imm64(target) (10)
    // + per chunk: load rax (7) + store rax (7).
    let source_displacement = element_index
        .saturating_mul(element_byte_size)
        .saturating_add(field_byte_offset);
    27 + runtime_storage_copy_chunk_count(source_displacement, target_offset, byte_count) * 14
}
...
pub fn encode_runtime_storage_copy_from_runtime_frame_fixed_indexed_to_runtime_storage(
    ...
    // Delegated to the two-base Place materializer (regions differ, so no
    // base sharing): the canonicalization moved the target base mov from +24
    // to +17 -- FRAME_FIXED_INDEXED_COPY_TARGET_IMM_OFFSET and the relocation
    // walker's offset arm moved in lockstep -- and lifted the old 1|4|8
    // single-chunk restriction (any byte_count now chunks).
    ...
    let source = Place::at(RuntimeStorageRegion::RuntimeFrame, descriptor_offset)
        ...
        .and_then(|place| place.with_step(PlaceStep::ConstOffset(element_displacement)))
...
/// indexed by a runtime value) and copy it into a runtime-storage target -- the
/// mirror of [`encode_runtime_machine_indexed_integer_write`] in the read
/// direction. The index may be machine-resident (`self.arr[self.k]`) or
/// frame-resident (`self.arr[k]`, a param/local slot -- read off a second
/// frame-base register).
pub fn encode_runtime_storage_copy_from_runtime_machine_indexed_to_runtime_storage(
    ...
    if index_region == omega_target_operations::RuntimeStorageRegion::RuntimeFrame {
        // Frame-resident index: r10 = frame base (imm64 at +10+2, the frame
        // relocation); eax = index, 32-bit zero-extended off the frame base.
        append_mov_r10_imm64(&mut bytes, 0);
        append_load_index_eax_from_r10(&mut bytes, index_offset)?;
    } else {
        // eax = index, loaded 32-bit (zero-extended, so a nonzero adjacent slot
        // can't splice into it) from the machine storage base.
    ...
    // the low byte_count bytes of rax there.
    append_mov_r15_imm64(&mut bytes, 0);
...
pub fn runtime_storage_copy_to_runtime_machine_indexed_from_runtime_storage_width(
    ...
    // Base sequence: mov r15,imm64 (10) + mov rax,[r15+src] (7)
    // + mov r10d,[idx base] (7) + imul r10,r10,imm32 (7) + add r15,r10 (3)
    // + store [r15+disp] (7) = 41.
    // A FRAME source inserts a second `mov r15,imm64` (the machine base, +10)
    // after the source load; a FRAME index inserts `mov r10,imm64` (the frame
    // base, +10) before the index load.
    let mut width = 41;
...
/// machine-resident inline array, indexed by a runtime value) -- the mirror of
/// [`encode_runtime_storage_copy_from_runtime_machine_indexed_to_runtime_storage`]
/// in the write direction (`self.nums[self.j] = self.b`). Source and index may
/// each be machine- or frame-resident (`self.arr[k] = self.b`, k a param).
pub fn encode_runtime_storage_copy_to_runtime_machine_indexed_from_runtime_storage(
    ...
    if !matches!(
        ...
        return Err(Diagnostic::error(
            "X86_64 MVP encoder cannot write a machine indexed value with this index region yet",
        ));
    }
    let element_scale = i32::try_from(element_byte_size).map_err(|_| {
        Diagnostic::error(format!(
    ...
    if source_region == omega_target_operations::RuntimeStorageRegion::RuntimeFrame {
        ...
        // +17+2, the second relocation) for the element store.
        append_mov_r15_imm64(&mut bytes, 0);
    }
    if index_region == omega_target_operations::RuntimeStorageRegion::RuntimeFrame {
        // Frame-resident index: r10 = frame base (its own relocation), then
        // r10d = [r10 + idx] overwrites the base with the 32-bit
        // zero-extended index (the load completes before write-back).
        append_mov_r10_imm64(&mut bytes, 0);
...
fn operand_position_domain_operation(
    ...
    match (operator, domain) {
        (
            StateGuardOperator::Add | StateGuardOperator::Subtract,
            ArithmeticDomain::Saturating | ArithmeticDomain::Trapping,
        ) => Some(OperandDomainOperation::AddSub { domain, operands_signed }),
        (
            StateGuardOperator::Multiply,
            ArithmeticDomain::Saturating | ArithmeticDomain::Trapping,
        ) => Some(OperandDomainOperation::Multiply { domain, operands_signed }),
        (
            StateGuardOperator::Divide | StateGuardOperator::Modulo,
            ArithmeticDomain::Saturating,
        ) if operands_signed => Some(OperandDomainOperation::SaturatingSignedDivMod {
...
pub fn runtime_machine_indexed_binary_write_width(
    ...
    let frame_index_extra = if index_region
        == omega_target_operations::RuntimeStorageRegion::RuntimeFrame
    {
        10
    } else {
...
pub fn encode_runtime_storage_copy_from_runtime_frame_fixed_indexed(
    ...
    // Delegated to the SHARED-BASE Place materializer (byte-for-byte with the
    // hand-spelled shape this replaced: one frame base in r15, the descriptor
    // deref hops the source to r14, the const element+field offset folds into
    // the chunk displacements).
    use omega_target_operations::{Place, PlaceStep, RuntimeStorageRegion};
    let source = Place::at(RuntimeStorageRegion::RuntimeFrame, descriptor_offset)
        .with_step(PlaceStep::Deref)
        .and_then(|place| place.with_step(PlaceStep::ConstOffset(source_offset)))
        .ok_or_else(|| Diagnostic::error("place path exceeds PLACE_MAX_STEPS"))?;
    let target = Place::at(RuntimeStorageRegion::RuntimeFrame, target_offset);
...
pub fn encode_runtime_storage_copy_from_runtime_frame_fixed_indexed_to_runtime_pointee(
    ...
    // Delegated to the SHARED-BASE Place materializer (byte-for-byte: one
    // frame base, the source's descriptor deref hops to r14 BEFORE the
    // target's pointer deref consumes r15).
    use omega_target_operations::{Place, PlaceStep, RuntimeStorageRegion};
    let source = Place::at(RuntimeStorageRegion::RuntimeFrame, descriptor_offset)
        .with_step(PlaceStep::Deref)
        .and_then(|place| place.with_step(PlaceStep::ConstOffset(source_offset)))
    ...
    let target = Place::at(RuntimeStorageRegion::RuntimeFrame, pointer_byte_offset)
        ...
        .ok_or_else(|| Diagnostic::error("place path exceeds PLACE_MAX_STEPS"))?;
    encode_place_copy_shared_base(&source, &target, byte_count)
}
...
mod vtable_call_encoding_tests {
    ...
    fn output_string_marshals_this_and_text_then_calls_through_slot_1() {
        ...
        assert_eq!(bytes.len(), win64_vtable_call_width(&operands, 1, false), "width matches");
    ...
    fn vtable_call_with_result_skips_the_result_operand_and_stores_rax() {
        ...
        let operands = vec![
            Op::Scalar { region: RuntimeStorageRegion::Machine, offset: 16, size: 8 },
            Op::Scalar { region: RuntimeStorageRegion::Machine, offset: 0, size: 8 },
            Op::Address { region: RuntimeStorageRegion::Machine, offset: 8 },
        ];
        let bytes = encode_win64_vtable_call_at_offset(&operands, 8, true).expect("encode");
        assert_eq!(bytes.len(), win64_vtable_call_width(&operands, 8, true), "width matches");

        // Args marshal exactly as the no-result shape: this -> RCX, text -> RDX.
        assert_eq!(&bytes[0..4], &[0x48, 0x83, 0xec, 40], "sub rsp, 40");
        assert_eq!(&bytes[14..21], &[0x49, 0x8b, 0x8f, 0, 0, 0, 0], "rcx = [r15+0] (this)");
        assert_eq!(&bytes[31..38], &[0x49, 0x8d, 0x97, 8, 0, 0, 0], "lea rdx, [r15+8]");
        assert_eq!(&bytes[38..45], &[0x48, 0x8b, 0x81, 8, 0, 0, 0], "mov rax, [rcx+8]");
        assert_eq!(&bytes[45..47], &[0xff, 0xd0], "call rax");
        assert_eq!(&bytes[47..51], &[0x48, 0x83, 0xc4, 40], "add rsp, 40");
        // The result store tail: mov r15,imm64 (relocated) + mov [r15+16], rax.
        assert_eq!(&bytes[51..53], &[0x49, 0xbf], "mov r15, imm64 (result base)");
        assert_eq!(&bytes[61..68], &[0x49, 0x89, 0x87, 16, 0, 0, 0], "mov [r15+16], rax");
    ...
    fn table_function_call_keeps_the_table_off_the_wire() {
        ...
        // table) lands in RCX.
        assert_eq!(&bytes[0..4], &[0x48, 0x83, 0xec, 40], "sub rsp, 40");
        assert_eq!(&bytes[14..21], &[0x49, 0x8d, 0x8f, 8, 0, 0, 0], "lea rcx, [r15+8] (arg)");
        ...
        assert_eq!(&bytes[31..38], &[0x49, 0x8b, 0x87, 0, 0, 0, 0], "rax = [r15+0] (table)");
        assert_eq!(&bytes[38..45], &[0x48, 0x8b, 0x80, 56, 0, 0, 0], "rax = [rax+56] (fn ptr)");
        assert_eq!(&bytes[45..47], &[0xff, 0xd0], "call rax");
        assert_eq!(&bytes[47..51], &[0x48, 0x83, 0xc4, 40], "add rsp, 40");
        // Result store tail.
        assert_eq!(&bytes[51..53], &[0x49, 0xbf], "mov r15, imm64 (result base)");
