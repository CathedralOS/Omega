//! Canonical format-33 scalar-type codec for boundary result evidence.
//!
...
use super::{Reader, TerminalInstallationError, decode_boolean, push_u16};
...
pub(super) fn decode_boundary_result_scalar_type(
    reader: &mut Reader<'_>,
) -> Result<ScalarType, TerminalInstallationError> {
    let tag = reader.u8()?;
