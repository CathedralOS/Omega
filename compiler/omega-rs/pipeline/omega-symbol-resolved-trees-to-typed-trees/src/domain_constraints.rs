...
pub(crate) fn normalize_domain_constraints(program: &mut TypedTrees) {
    ...
    let assignments = sites
        .into_iter()
        .map(|(carrier, constraints)| {
                    ...
                    let matches = program
                        ...
                        .filter(|domain| {
                            ...
                            full.rsplit("::").next().unwrap_or(full)
                                == domain_constraint.name.as_str()
                                && program
                                    .display_type_reference_with_constraints(domain.target_type)
                                    == carrier
                        })
                        .collect::<Vec<_>>();
                    let [domain] = matches.as_slice() else {
                        // Zero matches is either a compiler-known pseudo-domain
                        // or an unknown spelling diagnosed later. Multiple
                        // matches are rejected by normalized-domain validation;
                        // neither case may guess an identity here.
                        return None;
                    };
    ...
    for (constraints, normalized) in assignments {
