...
pub(crate) fn lower_state_signature(
    ...
    let mut ranking_subjects = Vec::new();
    for subject in lowerer
        .source_trees
        .tables
        .bodies
        .expressions
        .expression_handles(signature.ranking_witness.subjects)
    {
        ranking_subjects.push(crate::expression::lower_expression_handle(
            lowerer, *subject,
        )?);
    }
    typed_signature.ranking_witness.subjects = lowerer
        .typed_trees
        .expression_table
        .insert_expression_handles(ranking_subjects);
    for member in lowerer
        .source_trees
        .machine_decrease_order(signature.ranking_witness.view)
    {
        lowerer.typed_trees.signature_effects.append_to_span(
            &mut typed_signature.ranking_witness.view,
            crate::name::lower_name(member),
        );
    }
    let mut ranking_view_arguments = Vec::new();
    for argument in lowerer
        .source_trees
        .tables
        .bodies
        .expressions
        .expression_handles(signature.ranking_witness.view_arguments)
    {
        ranking_view_arguments.push(crate::expression::lower_expression_handle(
            lowerer, *argument,
        )?);
    }
    typed_signature.ranking_witness.view_arguments = lowerer
        .typed_trees
        .expression_table
        .insert_expression_handles(ranking_view_arguments);
    if signature.ranking_witness.range.is_present() {
        typed_signature.ranking_witness.range = typed::machine::RankingRange {
            start: crate::expression::lower_expression_handle(
                lowerer,
                signature.ranking_witness.range.start,
            )?,
            end: crate::expression::lower_expression_handle(
                lowerer,
                signature.ranking_witness.range.end,
            )?,
            end_inclusive: signature.ranking_witness.range.end_inclusive,
        };
    }
