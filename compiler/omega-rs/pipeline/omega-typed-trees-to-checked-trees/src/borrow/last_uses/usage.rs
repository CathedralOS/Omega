...
pub(super) fn statement_uses_owner_path(
    ...
    match statement {
        ...
        StatementNode::Transition(transition) => {
            matches!(transition.guard, omega_typed_trees::statement::TransitionGuardNode::When(expression) if uses(expression))
...
fn expression_uses_owner_path(
    ...
    .is_some_and(|place| {
        matches!(place.root, omega_facts::PlaceRoot::Symbol(root) if root == owner_symbol)
...
fn transition_target_uses_owner_path(
    ...
    match target {
        omega_typed_trees::statement::TransitionTargetNode::Named { arguments, .. } => program
            ...
            .iter()
            .any(|argument| uses(*argument)),
        omega_typed_trees::statement::TransitionTargetNode::Value(expression) => uses(*expression),
