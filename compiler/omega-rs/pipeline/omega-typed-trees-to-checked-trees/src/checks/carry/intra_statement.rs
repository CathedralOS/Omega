...
impl EvaluationTraversal<'_, '_> {
    ...
    fn visit_transition_target(
        &mut self,
        target_handle: omega_typed_trees::statement::TransitionTargetHandle,
    ) {
        match self
            .program
            .statement_table
    ...
    fn observe_place(&mut self, expression: ExpressionHandle) {
        ...
        .is_some_and(|place| {
            matches!(place.root, omega_facts::PlaceRoot::Symbol(root) if root == self.symbol)
                || place.segments.iter().any(|segment| {
                    matches!(segment, omega_facts::PlaceSegment::Field { symbol } if *symbol == self.symbol)
                })
        });
