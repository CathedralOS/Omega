...
use omega_typed_trees::expression::{ExpressionNode, TableCallExpression};
...
fn evaluate_membership(
    ...
    if !domain.facts.is_empty() {
        // Combining machine results with authored proof facts belongs to the
        // richer proof-operand rung; leave this fact to ordinary validation.
        return Ok(None);
    }
    ...
    match omega_interpreter::evaluate_build_time_machine(
        typed,
        machine_name,
    ...
    {
        BuildTimeValue::Bool(holds) => Ok(Some(holds)),
        other => Err(format!(
