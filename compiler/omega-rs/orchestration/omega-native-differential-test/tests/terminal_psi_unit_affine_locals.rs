...
fn unit_affine_local_cleanup_survives_all_native_artifacts() {
    ...
    for (target, subsystem, expected_subsystem) in [
        ...
        duplicate_operation.functions[0]
            ...
            .locals[1]
            .0 = first_operation;
        assert!(build_terminal_object_artifact(&duplicate_operation).is_err());
        ...
        assert!(build_terminal_object_artifact(&forged_signature).is_err());

        let object = build_terminal_object_artifact(&machine).expect("object");
        let image = emit_terminal_executable_image(&object, subsystem).expect("image");
        ...
        let bytes = encode_terminal_installation_record(&installation).expect("install encoding");
        assert_eq!(
