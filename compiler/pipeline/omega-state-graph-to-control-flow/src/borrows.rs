...
pub(crate) fn remap_borrow_writable_roots(
    ...
    let mut writable_roots =
        Arena::with_capacity(state_graph.semantics.borrow_writable_roots.len());

    for (_, root) in state_graph.semantics.borrow_writable_roots.iter() {
...
pub(crate) fn remap_borrow_argument_accesses(
    ...
    let mut accesses = Arena::with_capacity(state_graph.semantics.borrow_argument_accesses.len());

    for (_, access) in state_graph.semantics.borrow_argument_accesses.iter() {
...
pub(crate) fn remap_borrow_calls(state_graph: &StateGraph) -> Arena<StateBorrowCall> {
    let mut calls = Arena::with_capacity(state_graph.semantics.borrow_calls.len());

    for (_, call) in state_graph.semantics.borrow_calls.iter() {
...
pub(crate) fn remap_borrow_loans(state_graph: &StateGraph) -> Arena<StateBorrowLoan> {
    let mut loans = Arena::with_capacity(state_graph.semantics.borrow_loans.len());

    for (_, loan) in state_graph.semantics.borrow_loans.iter() {
...
pub(crate) fn remap_borrow_activations(state_graph: &StateGraph) -> Arena<StateBorrowActivation> {
    let mut activations = Arena::with_capacity(state_graph.semantics.borrow_activations.len());

    for (_, activation) in state_graph.semantics.borrow_activations.iter() {
...
pub(crate) fn remap_borrow_weakenings(state_graph: &StateGraph) -> Arena<StateBorrowWeakening> {
    let mut weakenings = Arena::with_capacity(state_graph.semantics.borrow_weakenings.len());

    for (_, weakening) in state_graph.semantics.borrow_weakenings.iter() {
