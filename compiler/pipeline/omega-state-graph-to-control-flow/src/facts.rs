...
pub(crate) fn remap_proof_obligations(state_graph: &StateGraph) -> Arena<ProofObligationFact> {
    let mut obligations = Arena::with_capacity(state_graph.semantics.facts.proof_obligations.len());

    for (_, obligation) in state_graph.semantics.facts.proof_obligations.iter() {
...
pub(crate) fn remap_invariants(state_graph: &StateGraph) -> Arena<InvariantFact> {
    let mut invariants = Arena::with_capacity(state_graph.semantics.facts.invariants.len());

    for (_, invariant) in state_graph.semantics.facts.invariants.iter() {
