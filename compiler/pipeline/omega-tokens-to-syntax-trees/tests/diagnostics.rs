...
fn top_level_item_error_lists_expected_items() {
    ...
    assert_eq!(
        message,
        "expected one of `use`, `export`, `data`, `domain`, `enum`, `machine`, `target`, `capability`, `invariant`, `library`, `measure`, `module`, `operator`, `package`, `platform`, `pub`, `provider`, `trait`, `boundary operator`, `boundary trait`, found punctuation `{`"
    );
}
