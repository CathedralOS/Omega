...
pub(crate) fn lower_measure_definition(
    lowerer: &mut Lowerer,
    measure: &omega_symbol_resolved_trees::measure::MeasureDefinition,
) -> Result<omega_typed_trees::measure::MeasureDefinition, Diagnostic> {
    ...
    let mut typed_measure = omega_typed_trees::measure::MeasureDefinition {
...
fn lower_measure_parameter(
    lowerer: &mut Lowerer,
    parameter: &omega_symbol_resolved_trees::signature::StateParameter,
) -> Result<omega_typed_trees::signature::StateParameter, Diagnostic> {
    let type_reference = lower_type_reference_into_table(lowerer, &parameter.type_reference)?;
    Ok(omega_typed_trees::signature::StateParameter {
