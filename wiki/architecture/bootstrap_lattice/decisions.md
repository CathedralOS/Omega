...
Implementation status: the maintained Rust compiler now carries this first
resolution-stratum rung independently of `SourceOrigin`. Current-activation
generated sources enter the extension; imported dependency-generated bundles
enter the next activation's base. Central symbol lookup and the later named
result-overload, conformance, and validation passes apply the one-way boundary,
including extension-wide precedence and separate duplicate scopes between base
and extension. Exact generated bytes,
paths, order, source count, and package custody remain unchanged. The compiler
still performs the full final frontend rebuild; retaining the admitted base
checkpoint through final checking and deleting that rebuild remain the next
D18 continuation.
