...
| ordinary named record fields | presumptively retain; selected nested record-array cost closed | the frontend probe and CKIR3/CKIR4 establish nominal layout, aggregate copy, runtime named construction, and direct fields. OMGRSWB11/OMGLOWK20/CKIR19/OMGRFN22 extend that evidence to an actual-capacity array of copyable 40-byte records, chained array-element/field places, all nine scalar field stores, and tag readback without positional source data. Compare the remaining general cost with the clarity and regularity loss from positional compiler data |
| fixed arrays and checked indexing | presumptively retain; selected guarded full-`u64` record-array cost closed | OMGRSWB11/OMGLOWK20/CKIR19/OMGRFN22 close the capability-shaped `TokenObservation` projection through guarded full-width indexing into 16,384 copyable records, exact stride/layout, nested load/store, an explicitly Trapping lookup index, and authored Exact count increment, including Rust-free meaning, defensive checked-IR traps, exact conservative artifacts, native/self variants, resources, and lower-rooted R1–R5. This subsumes the preceding OMGRSWA10 fixed-scalar-buffer cost but not payload-sum copying, structural-value projection, computed/effectful indexes, mutable slices, general allocation, or the complete `TokenStream` |
