...
| fixed arrays and checked indexing | presumptively retain; selected guarded full-`u64` cost closed | OMGRSWA10/OMGLOWJ19/CKIR18/OMGRFN21 close the capability-shaped `SourceUnit` projection through guarded full-width load/store indexing, the explicitly Trapping lookup index, and the authored Exact `length + 1` justified by `length < N`, including Rust-free meaning, defensive checked-IR traps, exact conservative artifacts, native/self variants, resources, and lower-rooted R1–R5. This is not whole-product `SourceUnit` admission: computed/effectful indexes, multiple observable traps, mutable slices, unrelated `u64` operations, and general allocation remain separate. Compare only that remaining cost with arena/library encodings |
