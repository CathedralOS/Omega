...
| concrete full-width `u32 in Trapping` subtraction | measure from the current CKIR12/OMGRFN14 frontier | checkpoint 000001 has 15 authored subtractions, including nine UTF-8 forms whose results feed multiplication or addition; no active bounded slice closes this row, so the next implementation must close the required arithmetic compositionally without depending on unresolved observable call-argument order |
