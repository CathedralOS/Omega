...
| static provider path arguments | measure from checkpoint 000001; exact structural carrier closed | OMGCOMP2 binds the exact Linux-x86-64/native-provider two-package, three-source closure while leaving provider spellings opaque; semantic resolution, realization, lowering, and authority remain open, and the checkpoint is not evidence for general generic declarations |
