...
| Delta compiler | Gamma | exact Gamma source under `DeltaCompilerV1` refines the exact Delta-compiler tape | D17 fixes Delta v1 and its `DCOUT` boundary and D22 fixes declaration identity/scope; incomplete source owns syntax and symbolic encoding, while collection/checking, lowering, tape, suite, and refinement remain open |
