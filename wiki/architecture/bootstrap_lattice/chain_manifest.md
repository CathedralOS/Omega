...
| Delta compiler | Gamma | exact Gamma source under `DeltaCompilerV1` refines the exact Delta-compiler tape | D17 fixes Delta v1 and its `DCOUT` boundary, D22/D24/D36 fix declaration and callable identity, D31 fixes structural type formation, D34 fixes bounded-witness storage refusal, D37 fixes body/control premise composition, and D38 fixes `.as_slice`; incomplete source owns syntax including D36's qualified-only receivers, the source-shaped census including its case/machine collision registry, structural type formation, a source-backed resolution catalog, ordered local resolution with positive receiver-scoped `self`, scalar/aggregate value-place facts, a generalized callable ledger with direct-qualified, settled grouped/unqualified, named-data receiver, and sealed-boundary receiver applications, settled field/index/slice projection failures, scalar and argument-`never` category joins, let/assignment/assert relations, D38 source facts, and symbolic encoding, while Q4-blocked entry diagnostics, Q10 state-transfer spelling, Q11 invalid-`self`, Q12 resultless-argument anchoring, remaining D36 continuation/state classification, D37 postfix-statement/control/return/terminal composition, D38 lowering/controls, body/control checking, storage implementation, lowering, tape, executable suite, and refinement remain open |
