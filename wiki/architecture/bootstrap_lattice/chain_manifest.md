...
| Delta compiler | Gamma | exact Gamma source under `DeltaCompilerV1` refines the exact Delta-compiler tape | D17 fixes Delta v1 and its `DCOUT` boundary, D22/D24 fix declaration identity, D31 fixes structural type formation, and D34 fixes bounded-witness storage refusal; incomplete source owns syntax, the source-shaped census, structural type formation, a source-backed resolution catalog, ordered local resolution, scalar value/place facts, and symbolic encoding, while Q4-blocked entry diagnostics, Q5 receiver/callable ambiguity, Q6 dependent-diagnostic composition, remaining body/control checking, storage implementation, lowering, tape, executable suite, and refinement remain open |
