...
| Delta compiler | Gamma | exact Gamma source under `DeltaCompilerV1` refines the exact Delta-compiler tape | D17 fixes Delta v1 and its `DCOUT` boundary, D22 fixes declaration identity/scope, and D24 completes transition-binder and collection-failure rules; incomplete source owns syntax, the source-shaped identity census, named-type candidate scanning, and symbolic encoding, while Q2 blocks complete type formation and body checking, lowering, tape, executable suite, and refinement remain open |
| `omega₀` | Delta closure `D` | exact `D` refines a full Omega compiler represented as Alpha tape | D18 fixes the logical request and D25 its outer/canonical `OCREQ`/`OCOUT` rules; `D` owns the outer envelope plus lexical, parser, and Alpha-encoder slices, while Q3 blocks the inner wire/failure tables and the full compiler and edge remain open |
| `omega` | Omega closure `C` | exact `C`, compiled by `omega₀`, refines a full Omega compiler represented as Alpha tape | D18 and D25 fix the common logical/outer standalone edge, Q3 blocks its exact inner wire/failure tables, and product source remains incomplete |
