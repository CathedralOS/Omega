...
| Gamma compiler | Beta | exact Beta source refines the exact Gamma-compiler tape | D16 fixes the language, D19 its logical application profiles, D20 its resolver identity/scope, D21 its `Bytes` length invariant, D23 its coherent one-MiB Alpha profile, and D30 its physical request/outcome profiles; incomplete source owns resolved lowering, whole-function emission, and exact D19 source-schema/bijection validation, while adapter emission, tape, and refinement remain open |
| Delta compiler | Gamma | exact Gamma source under `DeltaCompilerV1` refines the exact Delta-compiler tape | D17 fixes Delta v1 and its `DCOUT` boundary, D22/D24 fix declaration identity, and D31 fixes type formation; incomplete source owns syntax, the source-shaped census, structural type formation, and symbolic encoding, while Q6-blocked entry diagnostics, body/control checking, Q5-blocked storage refusal, lowering, tape, executable suite, and refinement remain open |
