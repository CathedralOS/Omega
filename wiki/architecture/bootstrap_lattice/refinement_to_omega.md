...
[Lattice overview](bootstrap_lattice.md) · [Decisions](decisions.md) · [Refinement pillar](../../../source/refinement/alpha-beta/REFINEMENT.md)
...
`source/on-ramp/rust/{psi,omega}/` already sketches the destination in
`source/on-ramp/rust/psi/semantics/psi-proof/src/`: `obligations.rs` (proof obligations
...
2. **Compound values.** Peano nats + pairs + streams got us here; Omega's structs/cases/strings need a
   ...
   work is substantial).
3. **Obligation generation.** `source/on-ramp/rust/psi/semantics/psi-proof/` defines and
