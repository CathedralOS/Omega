...
| Delta compiler | Gamma source transformer to canonical Gamma | exact `GammaComposedV1` identity and staged receipts | 1,653-line stage enforces source bytes, lexical atoms, global declarations, scalar/nominal scopes and types, exact function signatures, capture-proof lowering names, recursive ADTs, and proper tails through `if`/`let`/`match`; 100,000-node, rope, and 3,001-function witnesses pass; normative Bytes, checked runtime arithmetic, and profiles remain open |
