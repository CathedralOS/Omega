...
| Epsilon evaluator | Delta source executed through the selected lower route | interpreted Epsilon behavior | incomplete closure selected by `bootstrap/epsilon/compiler/epsilon_compiler.delta.sources`; current checking, scalar/record/array calls and returns, nested receiver places, scalar state transfers, immutable views, and all Console operations execute through the lower route; sum execution, remaining checking, application resource guarantees, and exact final composition remain open |
