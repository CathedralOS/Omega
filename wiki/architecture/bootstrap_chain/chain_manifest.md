...
| Omega `D` | Epsilon source interpreted by the Epsilon evaluator | `omega0_compiler_bytecode.tape` for Omega C | incomplete `source/omega/omega_compiler.epsilon`; D and its `alpha_bootstrap` target are incomplete; tape absent |
