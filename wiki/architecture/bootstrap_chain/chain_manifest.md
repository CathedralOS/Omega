...
| Delta compiler | Gamma source transformer to canonical Gamma | exact `GammaComposedV1` identity and staged receipts | 2,029-line stage catalogs exact nominal and resolved function metadata in sparse bytewise tries and enforces source bytes, global declarations, scalar/Bytes/nominal scopes and types, exact signatures, capture-proof names, recursive ADTs, arbitrary-order exhaustive matches, checked signed arithmetic, immutable length-bearing Bytes ropes, and proper tails; arithmetic boundaries, 100,000-node ADT/Bytes, Epsilon census, heterogeneous mutual-call, and 3,001-function witnesses pass; the 437,283-byte current Epsilon source plus a diagnostic entry compiles; application profiles and failure closure remain open |
