...
| Delta compiler | Gamma source transformer to canonical Gamma | exact `GammaComposedV1` identity and staged receipts | 2,029-line stage uses a two-pass exact nominal metadata catalog and enforces source bytes, global declarations, scalar/Bytes/nominal scopes and types, exact signatures, capture-proof names, recursive ADTs, arbitrary-order exhaustive matches, checked signed arithmetic, immutable length-bearing Bytes ropes, and proper tails; arithmetic boundaries, 100,000-node ADT/Bytes, Epsilon census, and 3,001-function witnesses pass; application profiles and failure closure remain open |
