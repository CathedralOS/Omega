...
| Epsilon evaluator | Delta source executed through the selected lower route | interpreted Epsilon behavior | incomplete `source/epsilon/compiler/epsilon_compiler.delta`; complete checking and empty-entry execution slice pass; final composition absent |
| Omega `D` | Manifested Epsilon source closure interpreted by the Epsilon evaluator | `omega0_compiler_bytecode.tape` for Omega C | incomplete `source/omega/{omega_compiler.epsilon.sources,compiler/}`; D and its `alpha_bootstrap` target are incomplete; tape absent |
