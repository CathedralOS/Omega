...
| Epsilon evaluator | Delta source executed through the selected lower route | interpreted Epsilon behavior | incomplete closure selected by `bootstrap/epsilon/compiler/epsilon_compiler.delta.sources`; checking, scalar calls/returns, fixed scalar/byte storage, and scalar state transfers execute through the lower route; arbitrary receiver places, aggregate execution, full Console behavior, and final composition remain open |
