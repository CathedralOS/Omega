...
| Gamma evaluator | Beta source | `gamma_evaluator_bytecode.tape` | selected 1,398-line typed scalar/effect evaluator; 7,303-byte tape; static validation, 100,000-step proper tails, exact bounded outcomes, provenance-tagged immutable pairs, buffered generic application results, profile-owned arithmetic traps, readable character literals, and self-augmentation pass |
| Delta compiler | Gamma source transformer to canonical Gamma | exact `GammaComposedV1` identity and staged receipts | 1,992-line stage catalogs exact nominal and resolved function metadata in sparse bytewise tries and enforces source bytes, global declarations, scalar/Bytes/nominal scopes and types, exact signatures, capture-proof names, recursive ADTs, arbitrary-order exhaustive matches, checked signed arithmetic, immutable length-bearing Bytes ropes, and proper tails; complete type checking precedes all output and emission does not repeat declaration validation; arithmetic boundaries, malformed-source atomicity, 100,000-node ADT/Bytes, Epsilon census, heterogeneous mutual-call, and 3,001-function witnesses pass; the 452,145-byte current Epsilon source plus a diagnostic entry compiles; application profiles and failure closure remain open |
