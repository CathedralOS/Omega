# Rung: Epsilon — compiler-host systems language

[Chain overview](../bootstrap_chain.md) | Prev: [Delta](delta.md) | Next:
[Omega product toolchain](../omega_toolchain.md)

Epsilon is the robust C-like implementation language used to write the first full
...
“the Epsilon compiler” obscures this boundary and is forbidden in current docs.

## Language boundary

Epsilon provides deterministic state-machine control, checked scalar arithmetic,
...
D17 and `source/epsilon/LANGUAGE.md` fix the exact v1 surface. Checking and
execution are separate judgments; `Incomplete` is an outer tool outcome, not a
Epsilon program result. `D` uses source-declared fixed arrays and integer indexes
...
- `source/epsilon/LANGUAGE.md` owns Epsilon syntax and semantics;
- the target compiler source is `source/epsilon/compiler/epsilon_compiler.delta`;
- `source/epsilon/compiler/epsilon_compiler_bytecode.tape` is the future
  canonical artifact; and
- `tests/epsilon/` will own executable Delta-source/Alpha-tape validation when
  the compiler artifact exists; canonical evidence tables remain with the
  compiler source and tape.

The former `source/epsilon/meaning/epsilon2delta.gamma` route and the restricted
Epsilon-written Darwin compiler prototype are deleted. Neither implemented the
Delta-written Epsilon edge or a full Omega `D`; their historical source remains
...
2. a Delta-written compiler accepts that language and emits exact Alpha tape;
3. the tape directly refines the Delta compiler source under Delta and Alpha
   semantics;
4. the compiler accepts the exact Epsilon source closure `D`; and
...
appears necessary, proof checking becomes prohibitive, or Epsilon compilation
