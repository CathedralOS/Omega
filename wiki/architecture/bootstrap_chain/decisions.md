...
consumes the carrier and verifies its returned exact symbol. One package-aware
retention canary now proves that the final checked owner keeps the admitted
checkpoint's durable outputs: a Windows-x86-64 activation selects the
non-default GUI subsystem, executes scoped source/output operations, and
publishes one generated unit while retaining the exact build symbol and
normalized callable identity, profile/native target and subsystem,
deterministic initial/replay usage, receipted observation and complete replay,
canonical source metadata, generated handoff and staged tree, distinct base and
final source commitments, exact generated bytes, and consumed-source count.
This is custody coverage only; it does not remove the transitional full
frontend rebuild for unsupported extension shapes. A bounded
...
Other runtime-generic, constrained, invalid-lifetime, residual const-expression, dynamic,
...
slice, and literal-fixed-array shells in record fields and case payloads.
...
remains outside this exact cohort. The exact constrained Type-argument cohort
also continues: one already-supported closed named carrier may retain either
one semantic arithmetic-domain tag or one exact source-declared, unindexed,
non-alias domain. For a declared domain, seeded validation resolves the
authored spelling to exactly one visible declaration on that carrier, rejoins
the exact carrier and domain symbols and names, runs the ordinary domain
...
ambiguous/alias/indexed/range/multi-constraint applications, parameter-bearing
