...
Starting in v43, an unsigned pair can instead carry the exact path proposition
`left <= MAX - right` (or its commuted form). The guard subtraction is itself
an Exact operation whose known maximum minuend makes it total. The verifier
...
Starting in v45, a signed nonpositive addend may instead use the lower-bound
relation `MIN - right <= left`. The path must separately carry `right <= 0`,
which also proves the Exact bound subtraction representable. Other signed
two-runtime shapes still fail closed.
...
Starting in v47, a signed nonnegative subtrahend may carry `0 <= right` plus
`MIN + right <= left`. The sign fact proves the Exact bound addition itself
representable, and the subtraction reconstructs the conjunction of both facts.
Starting in v48, a signed nonpositive subtrahend may instead carry
`right <= 0` plus `left <= MAX + right`. The sign fact likewise proves the
bound addition, and the subtraction reconstructs their conjunction.
...
factor `k` requires `value <= MAX / k`. Signed factors reconstruct the exact
...
An Exact remainder retains the same operand and result types and owns the same
quotient-definedness boundary. The first v35 reconstruction surface requires a
terminal-known right operand. Nonzero unsigned divisors and signed divisors
other than negative one are total; negative one requires
`MIN + 1 <= dividend`. Two known operands reduce to truth only when truncating
...
A Wrapping remainder retains the same fixed operand and result type and owns
the same divisor-definedness boundary. The first v37 reconstruction surface
...
Starting in v41, a signed runtime divisor on a path carrying the exact axiom
...
A Saturating remainder retains the same fixed operand and result type and owns
the same divisor-definedness boundary. The first v39 reconstruction surface
requires a terminal-known right operand. Any nonzero divisor reduces to truth,
including signed negative one: `MIN % -1` is zero at the declared width. Zero
and runtime-unknown divisors fail closed.
...
is admitted. Terminal Psi v43 additionally recognizes a directly carried
unsigned `left <= MAX - right` bound after independently proving the bound's
...
Terminal Psi v45 adds the symmetric signed lower-bound recognition. A path
fact `right <= 0` proves the `MIN - right` subtraction, and the add obligation
conjoins it with the carried `MIN - right <= left` comparison.
...
Terminal Psi v47 recognizes the signed lower-bound form when the path carries
both `0 <= right` and `MIN + right <= left`. The bound addition independently
selects the sign fact; the subtraction selects their conjunction.
Terminal Psi v48 recognizes the symmetric upper-bound form when the path
carries both `right <= 0` and `left <= MAX + right`. The bound addition and
subtraction retain the same independent-proof split.
...
equality. Unsigned `k > 0` reconstructs `variable <= MAX / k`; signed factors
reconstruct the carrier-tight interval with inequality direction reversed for
negative `k`. Zero and one require no proof beyond truth, negative one excludes
...
Terminal Psi v35 retains Exact fixed-integer remainder as
`ExactIntegerRemainder { left, right, obligation }`. Its verifier reconstruction
...
Terminal Psi v37 retains Wrapping fixed-integer remainder as
`WrappingIntegerRemainder { left, right, obligation }`. The verifier resolves a
terminal-known divisor and rejects zero or unknown values. Every known nonzero
...
Terminal Psi v39 retains Saturating fixed-integer remainder as
`SaturatingIntegerRemainder { left, right, obligation }`. The verifier resolves
a terminal-known divisor and rejects zero or unknown values. Every known
...
Terminal Psi v41 additionally selects a directly carried signed
...
terms, v29 adds recursive saturating-integer-divide terms, and v30 adds
recursive saturating-integer-remainder terms.
