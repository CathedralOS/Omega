...
canonical `left <= MAX - right` bound, one runtime signed addition under matching
sign and `MAX - right` or `MIN - right` bounds, or exact unsigned subtraction
under a retained direct subtrahend-to-minuend bound, one exact right shift under
a direct unsigned count upper bound, one exact left shift by a landed count
under a direct unsigned value upper bound or by an unsigned runtime count under
direct count and value upper bounds, or exact
...
multiplication, plus runtime unsigned and signed addition under the corresponding
computed bounds, in addition to the separately listed unsigned relational
subtraction, shift forms, signed landed-divisor form, runtime signed or unsigned
