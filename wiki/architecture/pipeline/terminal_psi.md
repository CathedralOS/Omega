...
The next bounded rung permits one, two, three, four, or five distinct selected
...
Payload projections, multiple or partial-result substitutions, six-or-more
...
Missing/reordered operations or cleanup, changed indices/root length, and
length-sixteen or wider prefixes reject; no runtime liveness bitmap or cleanup
loop is introduced.
...
sets.
The first bounded producer family uses the rule for one to ten prior signed
...
three-or-more-leg root reconstruction, words of eleven or more definitions, joins,
...
modules. Production alone owns the fixed ten-definition witness frontier,
