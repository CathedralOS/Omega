...
- `omega-rust/psi/pipeline/symbol-resolved-trees-to-typed-trees` contains the
  stage implementation. All workspace consumers invoke it directly.

- `omega-rust/psi/foundation/language-semantics` contains canonical
  const-value atoms and normalized wire scalar ranges used by typed
  normalization.
- `omega-rust/psi/foundation/{extents,layout-plans,access-plans}`
...
- `omega-rust/psi/representations/typed-trees` contains the typed source
