...
- `assignment/function/unit/operation.rs` applies the same exact plan/source
  ...
  ordered register-resident native fixed-integer or Boolean caller parameters,
  or one exact preceding Boolean literal, to the scalar prefix of the callee ABI while
  ...
  cannot authorize it. Scalar values may occupy the call plan's scalar
