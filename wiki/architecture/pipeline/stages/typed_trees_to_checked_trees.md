...
- `checks/borrows.rs` is the borrow-check entry point. `checks/borrows/calls.rs`
  ...
  transparent free helper result or an attached helper result rooted in its
  actual `self` receiver: the callee write rebases through the helper and view
  to its backing array before the index coarsens it. Deeper index trees and
  recursive or opaque free/attached view producers remain fences. The indexed
