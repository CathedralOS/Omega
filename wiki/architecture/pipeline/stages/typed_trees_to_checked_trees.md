...
- `checks/borrows.rs` is the borrow-check entry point. `checks/borrows/calls.rs`
  ...
  borrow and mutation conflicts, `checks/borrows/overlap.rs` owns borrow
  overlap entry dispatch and root matching, `checks/borrows/overlap/segments.rs`
  owns place-segment overlap policy, `checks/borrows/overlap/indexes.rs` owns
  index and range overlap policy, and `checks/borrows/details.rs` owns diagnostic
