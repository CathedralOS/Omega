...
- `checks/borrows.rs` is the borrow-check entry point. `checks/borrows/calls.rs`
  ...
  the index coarsens it. Recursive or opaque free/attached view producers remain
  fences. A transparent free helper result or an attached helper result rooted
  in its actual `self` receiver
