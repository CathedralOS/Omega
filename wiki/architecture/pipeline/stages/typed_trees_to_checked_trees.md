...
- `checks/borrows.rs` is the borrow-check entry point. `checks/borrows/calls.rs`
  ...
  complete caller-isolated call producers in up to twenty-seven unary, binary,
  primitive-cast, member-projection, or indexing shells under the same call
  budget; a twenty-eighth shell and generic/reference/unknown call results remain
