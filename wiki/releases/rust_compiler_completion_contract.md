...
| `RC-PCC-REPLAY` | Canonical semantics and proof bytes round-trip, hostile or substituted evidence rejects, and independent verification precedes interpretation or Omega lowering. | `mbx test -p checked-trees-to-lowered-psi -p terminal-codec -p terminal-verifier -p terminal-interpreter -p terminal-psi-to-abstract-operations` |
