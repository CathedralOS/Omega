...
relocations. Canonical installation format 68 retains the application/row/
...
and require an unchanged descriptor-register handoff. Machine emission records
the direct call and return with Unit stack/link custody; the common forwarding
row makes source value and scalar type jointly optional rather than fabricating
them. Object, final-image, and format-68 installation replay retain the exact
Unit helper chain and reject result-class drift.
...
consumes `CompileRequest` into a private validated request and rejects rollback
for stages the requested product will not execute. `Check` admits no rollback;
`TerminalArtifact` admits only Psi-stage names; `NativeArtifact` admits the
complete selection. Rollback remains a subtractive release overlay, and an
invalid product combination reads no source and creates no report/output
directory. The product driver consumes only the
