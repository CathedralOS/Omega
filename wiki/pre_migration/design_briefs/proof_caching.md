...
### Correction & refinement (2026-06-26, after a verification-literature survey)
...
**Deeper floor:** succinctness is a *mathematical fact about the predicate*, not a tooling choice — Haken's exponential resolution lower bounds + "does a succinct cert always exist = NP vs coNP" (Cook–Reckhow) mean some universal claims have **no** poly-size unconditional cert (Collatz over u32 is the open-math case). *But* structured exhaustions sometimes do compress sub-search-size (Nederlof–Williams: even count-zero/UNSAT has an O\*(√t) witness for algebraic problems). So: **prioritize the small-inductive-cert regime (the only kernel-only-trust win); the budget/bound measure is how you reach it for would-be-exhaustions; large verified proofs are sound-but-not-build-shippable; crypto is an opt-in trading unconditional soundness + a 10⁶× bill for succinctness.**
