...
The public operation family is an ordinary requirement:
...
Implementation status: generated `compact_binary` report rows now retain the
normalized `StrictDecode<compact_binary, Value>` requirement identity and
normalized plan identity. Their realization origin is
