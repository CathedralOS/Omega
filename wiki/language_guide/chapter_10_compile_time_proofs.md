...
right side is a type expression, and `%` is its one new form. Nothing on
either side is magic — the left names ordinary proof-only data, the right
names an ordinary machine that has been proven an equivalence. `CauchySeq`
here is the machine-parameterized family (chapter 13), and the buckets
deliberately span it: a stream generated by one machine and a stream
generated by a different machine may name the same number. That is the
point — pi-via-Leibniz and pi-via-Machin are different `CauchySeq<S>`
instantiations in one bucket. Quotient carrier matching therefore uses the
