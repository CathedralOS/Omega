# Omega Language Guide

This guide is a chaptered sketch of Omega's language direction.

The syntax is not final. These chapters exist so language ideas can be organized, challenged, and eventually turned into parser/compiler work.

- [Chapter 1: Data And Values](chapter_1_data_and_values.md)
- [Chapter 2: States](chapter_2_states.md)
- [Chapter 3: Transitions](chapter_3_transitions.md)
- [Chapter 4: Typed Machine Graphs](chapter_4_typed_states.md)
- [Chapter 5: Return Value Compatibility](chapter_5_return_value_compatibility.md)
- [Chapter 6: Value Constraints And Invariants](chapter_6_value_constraints_and_invariants.md)
- [Chapter 7: Domains](chapter_7_domains.md)
- [Chapter 8: Invariant Propagation](chapter_8_invariant_propagation.md)
- [Chapter 9: Proof Obligations](chapter_9_proof_obligations.md)
- [Chapter 10: Relax Scopes](chapter_10_relax_scopes.md)
- [Chapter 11: Traits And Runtime Dispatch](chapter_11_traits.md)
- [Chapter 12: Host Libraries And Trust Boundaries](chapter_12_host_trust_boundaries.md)
- [Chapter 13: Versioned Data And Machine Replacement](chapter_13_versioned_data.md)
- [Chapter 14: Wire Protocols](chapter_14_wire_protocols.md)
- [Chapter 15: Inline Assembly](chapter_15_inline_assembly.md)
- [Chapter 16: Concurrency](chapter_16_concurrency.md)
- [Chapter 17: Drops And Cleanup](chapter_17_drops_and_cleanup.md)
- [Appendix: Open Questions](appendix_open_questions.md)
