...
content, evidence-schema identity, compiler/toolchain provenance, and compiler
evidence form a `PackageInstance`. A same-spelled package or boundary from
another lineage is therefore a different identity. Toolchain provenance makes
the evidence reproducible and comparable; it does not prove that the compiler
or an audit was trustworthy.
...
Package evidence is regenerated by the selected local compiler rather than
accepted from dependency source. This prevents a package from declaring its own
capability result, but it does not remove the local compiler or the project's
merge policy from the trust base. Review status, signatures, and reasons are
