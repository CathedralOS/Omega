...
plan entry. Third through nineteenth represented record levels are now available
under the same restrictions. All nineteen enclosing identities remain in each
erased-leaf path, and one bounded recursive traversal rejects hidden unsupported
custody without a depth-specific implementation. A twentieth represented record
