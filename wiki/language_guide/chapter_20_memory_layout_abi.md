...
plan entry. Third through twenty-first represented record levels are now available
under the same restrictions. All twenty enclosing identities remain in each
erased-leaf path, and one bounded recursive traversal rejects hidden unsupported
custody without a depth-specific implementation. A twenty-second represented record
