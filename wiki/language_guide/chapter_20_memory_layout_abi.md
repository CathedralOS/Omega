...
plan entry. Third through twenty-fourth represented record levels are now available
under the same restrictions. All twenty-three enclosing identities remain in each
erased-leaf path, and one bounded recursive traversal rejects hidden unsupported
custody without a depth-specific implementation. A twenty-fifth represented record
