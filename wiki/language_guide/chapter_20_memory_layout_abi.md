...
plan entry. Third through fourteenth represented record levels are now available
under the same restrictions. All fourteen enclosing identities remain in each
erased-leaf path, and one bounded recursive traversal rejects hidden unsupported
custody without a depth-specific implementation. A fifteenth represented record
