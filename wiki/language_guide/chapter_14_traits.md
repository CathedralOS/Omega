...
bytes on x86-64 and AArch64. Installation format 58 retains the same semantic
...
the store before the read. The direct fixed-integer form also reaches native
execution: `&mut self` is one no-copy pointer to caller storage, the erased-data
adapter rejoins the structural-only scalar-result ABI, and machine/object/image
evidence replays the exact store/read/return bytes on x86-64 and AArch64.
Boolean stores, wider or computed mutation bodies, and Unit-returning dynamic
requirements remain outside this bounded rung.
