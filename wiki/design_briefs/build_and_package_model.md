...
One package-aware retention canary now pins the admitted checkpoint's durable
outputs on the seeded generated-data path. A Windows-x86-64 activation selects
the non-default GUI subsystem, performs scoped source/output operations, and
publishes one generated unit. Its final checked owner retains the exact build
symbol and normalized callable identity, profile/native target and subsystem,
deterministic initial/replay usage, receipted observation and complete replay,
canonical source metadata, generated handoff and staged-tree custody, distinct
base/final source commitments, exact generated bytes, and consumed-source
count. This proves evidence retention only; unsupported extension shapes still
use the transitional second frontend rebuild.
...
plus scalar integer or Boolean const parameters. Local template applications
retain Type bounds. Every synthesized data definition's retained
`generic_instance` origin is an explicit checked-
...
subset, or extra lifetime routing remains outside this exact cohort. The exact
constrained Type-argument cohort also continues: one already-supported closed
named carrier may retain either one semantic arithmetic-domain tag or one
exact source-declared, unindexed, non-alias domain. For a declared domain,
seeded validation resolves the authored spelling to exactly one visible
declaration on that carrier, rejoins the exact carrier and domain symbols and
names, runs the ordinary domain normalization, and checks the canonical
synthesized name, origin, and every substituted occurrence. Same-spelled
lookalikes or ambiguous declarations, aliases, indexed domains, range or
multiple constraints, and broader constrained shells remain fenced.
...
Closed Boolean arguments retain and completely decode the exact compiler-
generated canonical value atom. Internal forwarding rejoins an exact owner
const binder and carrier; only integer binders may supply literal-array lengths,
...
reachable from an ordinary generated data root, and local generic template
fields rejoin their exact template and owner parameters. Instances may have
any number of direct or admitted-shell ordinary-record use sites, repeated
...
broader or custom-canonical structured const applications, ambiguous/alias/
indexed/range/multi-constraint applications, parameter-bearing constrained/dynamic/
other composite shells, fact-bearing, quotient, zero-gated,
...
declaration symbol and name, each
...
extension-local Type arguments, broader/custom-canonical const and broader
constrained extension-local instances, generated attached methods, non-data
