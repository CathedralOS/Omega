...
Ordinary built-in imports with one or more immediate or runtime-scalar integer
arguments and no result now have a dedicated Windows x64/macOS arm64 final class. Replay calls
...
bits in final text. Runtime-scalar rows have a separate origin whose final
recipe requires the complete storage-root relocation set together with the
import call. Both StatePlan fragments include the call plan's ordinary
...
integers, including no-argument calls, now retain a third fragment. Checked
replay requires the exact result-region relocation beside the imported call
...
The built-in no-argument errno accessor now uses a sixth fragment whose replay
...
seventh and eighth fragments for no-result and direct-integer-result calls.
Replay requires the exact library/symbol call plus every named data-object and
...
tenth fragments for no-result and direct-integer-result calls. Replay consumes
...
imports with runtime-float parameters now use eleventh and twelfth fragments
for no-result and direct scalar-result calls. Replay preserves the plan-selected
...
now use thirteenth and fourteenth fragments for no-result and direct
scalar-result calls. Replay keeps each aggregate as one source operand while
requiring its exact fragmented, stack, or caller-copy marshalling and storage
root. Source-authored aggregate results now use a fifteenth fragment. Replay
preserves the one result place across AArch64 X/V fragment spills, the hidden
`x8` destination path, and Microsoft/SysV direct or indirect result plans; the
exact result root and any argument roots accompany the imported call. Darwin
...
frame- or machine-held indices with the exact second-base relocation. All-frame
double-runtime-indexed carriers reuse one frame relocation for the collection
and both indices. The
