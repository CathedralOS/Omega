...
too. Selection retains their separate `compiler_body_place_copy` evidence only
for the `Ordinary` role; final validation regenerates the same place-copy bytes,
...
`omega.final-footprint-certificate` format v19, with a domain-separated
