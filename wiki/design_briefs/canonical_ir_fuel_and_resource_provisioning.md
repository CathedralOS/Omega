...
Terminal Psi v44 extends that upper-bound form to signed addition on paths that
also prove `0 <= right`. That fact makes the Exact `MAX - right` bound
calculation safe; the addition selects its conjunction with
`left <= MAX - right`. Proof vocabulary, one-unit operation fuel, and Omega
lowering remain unchanged.
Terminal Psi v45 adds the symmetric lower-bound form on paths that prove
`right <= 0`. That fact makes the Exact `MIN - right` bound calculation safe;
the addition selects its conjunction with `MIN - right <= left`. Proof
vocabulary, one-unit operation fuel, and Omega lowering remain unchanged.
...
Terminal Psi v47 extends joint subtraction to signed nonnegative runtime
subtrahends. The path fact `0 <= right` proves the Exact `MIN + right` bound
calculation safe; the subtraction selects its conjunction with
`MIN + right <= left`. Proof vocabulary, one-unit operation fuel, and Omega
lowering remain unchanged.
Terminal Psi v48 adds the symmetric signed nonpositive-subtrahend form. The
path fact `right <= 0` proves the Exact `MAX + right` bound calculation safe;
the subtraction selects its conjunction with `left <= MAX + right`. Proof
vocabulary, one-unit operation fuel, and Omega lowering remain unchanged.
...
Terminal Psi v39 adds proof-gated Saturating fixed-integer remainder. A
terminal-known nonzero divisor reconstructs truth, including signed negative
one because `MIN % -1` is zero. Zero and runtime-unknown divisors fail closed.
Proof format v30, canonical semantics, exact fuel, artifact interpretation, and
...
Terminal Psi v44 extends the same slice to signed nonnegative runtime addends.
The source checker transports the sign fact into the nested bound state; the
terminal verifier independently uses it for the bound subtraction and conjoins
it with the carried upper comparison for the add obligation. Canonical
semantics, selected-path fuel, artifact interpretation, and both native targets
carry the signed slice.
Terminal Psi v45 adds the signed nonpositive runtime-addend slice. The source
checker transports the sign fact into the nested lower-bound state; the
terminal verifier independently uses it for the `MIN - right` subtraction and
...
Terminal Psi v47 adds the signed nonnegative-subtrahend slice. The source
checker transports the sign fact into the nested lower-bound state; the
terminal verifier independently uses it for the `MIN + right` addition and
conjoins it with `MIN + right <= left` for the subtraction obligation.
Canonical semantics, selected-path fuel, artifact interpretation, and both
native targets carry the signed slice.
Terminal Psi v48 adds the signed nonpositive-subtrahend slice. The source
checker transports the sign fact into the nested upper-bound state; the
terminal verifier independently uses it for the `MAX + right` addition and
