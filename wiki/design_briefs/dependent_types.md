...
coarsens to that nearest collection, and later indexes or members are absorbed;
each index expression independently satisfies the same bounded-call rule.
...
spine, including after a transparent free helper result or an attached helper
result rooted in its actual `self` receiver: it preserves that helper's backing
array origin before the index coarsens it. Recursive or opaque free/attached
view producers remain fences. A transparent free helper result or an attached
...
may follow that result before one or more indexes: the suffix composes first,
