...
coarsening, while a member after the index remains absorbed by the coarse
backing collection. With repeated indexes, the first fixes that coarse
collection, later indexes stay absorbed, and every independently bounded index
frame publishes. An attached helper may likewise root that relation in
