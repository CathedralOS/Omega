...
A third represented record level is also live under the same restrictions. Its
projection preserves all three enclosing identities, and bounded, memoized
descendant replay fails closed when an unsupported deeper shape could hide
erased custody. This is ordinary conformance checking only; a fourth represented
record level, zero-layout wrappers, arrays, generic, case-dependent, planless,
