...
| RC-DIAGNOSTICS | **pass** | `cargo nextest run -p compiler --test canary_suite -E 'test(=proof_and_float_suites::proof_and_domain_canaries::fail_canaries_reject_with_expected_diagnostic_fragment)'` — 1/1 PASS in 108.2s. **Not a repaired refusal — a recategorized fixture.** At `f6bb8e6c2e` this gate failed because `tests/omega/fail/calls/machine_self_call_recursion_rejected` compiled where rejection was pinned. That fixture did not exist at this record's own revision: `b3681d7bb6623` ("canaries: recategorize unmeasured self-call recursion as an admitted compile", 82 minutes earlier) moved it to `tests/omega/pass/calls/machine_self_call_recursion_compile` and registered it in `ACTIVE_PASS_CANARIES`. The gate passes because the case is no longer a must-reject member, not because a refusal was restored. |
