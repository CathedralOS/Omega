...
- [>] Complete independent translation validation for every lowering and
  ...
  fail closed.
  The adjacent constant integer-less-than-to-Boolean-immediate family admits
  only `[IntegerConstant, IntegerConstant, IntegerLessThan, Return]` over two
  same-type native integer constants. Its grammar compares exact signed or
  unsigned mathematical values without host-width conversion; independent
  target replay retains both ordered constants, all three definitions, the
  declared carrier, provenance, return link, and materialized Boolean result.
  Signed and unsigned fixed 8/16/32/64 plus unsigned address64 cross equal and
  ...
  The adjacent constant saturating-integer-add-to-immediate family admits only
  `[IntegerConstant, IntegerConstant, SaturatingIntegerAdd, Return]` over two
  same-type native integer constants. Independent source grammar recomputes
  the exact result with `IntegerType::saturating_add`; target replay requires
  one `ReturnIntegerImmediate` and retains both constants, all three
  definitions, ordered provenance, the return edge, and the materialized
  value. Signed and unsigned fixed 8/16/32/64 plus address64 cross positive,
  negative, and nonsaturating boundaries on all five native targets at direct
  and optimized custody (180 cases at each boundary). Exact, wrapping,
  subtract, multiply, plain-immediate, parameter, and runtime-expression
  substitution fail closed.
  The adjacent constant saturating-integer-subtract-to-immediate family admits
  only `[IntegerConstant, IntegerConstant, SaturatingIntegerSubtract, Return]`
  and never normalizes its ordered operands. Independent source grammar
  recomputes `IntegerType::saturating_sub`; target replay requires the exact
  `ReturnIntegerImmediate` while retaining both constants, all definitions,
  provenance, and the return edge. Signed and unsigned fixed 8/16/32/64 plus
  address64 cross low-clamp, high-clamp, ordinary decrement, and reversed-
  ...
  The adjacent constant wrapping-integer-shift-right-to-immediate family owns
  the direction-specific sibling grammar and recomputes only through
  `IntegerType::wrapping_shift_right`. The same 81 independently typed native
  value/count pairs and four modulo-count boundaries cross all five targets
  at direct and optimized custody (1,620 cases at each boundary), with the
  ...
  The separate proof-bearing constant-operands wrapping-integer-remainder
  family admits only
  `[IntegerConstant, IntegerConstant, WrappingIntegerRemainder, Return]` over
  the same native fixed-integer domain. Independent replay requires the exact
  obligation and ordered immediate children in
...
- [ ] Add exact address-mode folding, compare/branch selection, extension
  ...
  - [x] Add exact unsigned-`U64` parameter-equals-zero conditional lowering
    as the first real source route into compare-zero optimization. The admitted
    grammar has one unqualified `U64` entry parameter; entry operations exactly
    `[IntegerConstant(U64, 0), IntegerEqual(parameter, zero), Conditional]`;
    and two `[IntegerConstant(U64), Return]` leaves. Legalization retains the
    authored zero, equality definition, parameter ABI placement, provenance,
    and independent fuel custody in recipe tag 14 and a distinct condition;
    ...
    construction removes the zero materialization and emits the established
    `CompareI64Zero` plus `ConditionalBranchNonZero`, mapping nonzero to source
    false and zero to source true, with three virtual registers and six
    instructions. No selected, constraint, effect, allocator, layout, encoder,
    object, or callable vocabulary changes. With no post-allocation machine
    selection, AArch64 keeps baseline `CMP`/`B.NE`; only the existing named
    `Aarch64FuseCompareI64ZeroBranchNonZeroToCbnzV1` rule may produce `CBNZ`.
    Fusion retains the elided compare as a zero-byte semantic span, so its
    authored-zero/equality fuel settlements remain authenticated and counted
    rather than being dropped or moved onto branch provenance.
    Producer/replay, corruption, identity, cross-target selection, semantic
  ...
  - [x] Add constant saturating-integer-add-to-immediate as its own exact
    abstract-to-target family. Its four-operation grammar independently
    recomputes `IntegerType::saturating_add` and requires the exact immediate
    result with ordered provenance. Direct and optimized custody each cover
    180 signed/unsigned fixed-or-address type/pair/target cases, including
    positive and negative saturation; catalog ambiguity, corruption, exact/
    wrapping/subtract/multiply/plain/parameter/runtime-expression substitution
    fail closed. This adds validation custody around existing lowering and no
    target-operation or persisted-wire vocabulary.
  - [x] Add constant saturating-integer-subtract-to-immediate as its own exact
    ordered abstract-to-target family. Its four-operation grammar independently
    recomputes `IntegerType::saturating_sub` and requires the precise immediate
    result and provenance. Direct and optimized custody each cover 180 signed/
    unsigned fixed-or-address type/pair/target cases, including both clamp
    directions and ordinary subtraction; catalog ambiguity, corruption,
  ...
  - [x] Add constant wrapping-integer-shift-right-to-immediate as its own exact
    abstract-to-target family. Its four-operation grammar independently
    retains native value/count types and ordered roles, recomputes with
    `IntegerType::wrapping_shift_right`, and requires the precise immediate.
    Direct and optimized custody each cover all 81 type pairs, four modulo-
    count boundaries, and five targets (1,620 cases), including signed
    sign-fill versus unsigned/address zero-fill; catalog ambiguity, corruption,
  ...
  - [x] Add proof-bearing wrapping-integer-remainder with two constant operands
    as its own exact abstract-to-target family. Its four-operation grammar
    admits native fixed integers only, independently rejects zero divisors,
    and retains the authored nonzero-divisor obligation in a wrapping-remainder
    target expression whose children are the exact ordered immediates. Direct
    and optimized custody each cover 160 type/pair/target cases, including
    signed negative remainder and `MIN % -1`, with canonical certificate-
    derived proof evidence. The divide/remainder fixtures share one proof
  ...
  - [x] Add constant integer-less-than-to-Boolean-immediate as its own exact
    abstract-to-target family. Its four-operation grammar compares exact
    signed/unsigned mathematical values for fixed 8/16/32/64 and address64,
