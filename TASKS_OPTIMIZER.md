...
- `CompileOptions` contains root, build directory, target, and output policy.
  ...
  object serialization, and the canonical Terminal semantic/proof-to-object
  artifact on x86-64 and AArch64. It grants no ProgramStorage wrapper,
  process-entry, image, installation, or publication authority.
...
- `omega-optimization-unit` now deterministically reconstructs explicit blocks,
  ...
  in a disconnected component. Nested Record, Sum, and Mixed declarations now
  also replay Terminal's canonical per-namespace field/case ID order, unique
  ...
  The verified builder now also projects the complete ordered Terminal proof
  question into immutable, self-authenticating unit rows. Exact operation,
  call-requirement, nominal-cleanup-requirement, and contract-ensure owners;
  obligation identity/class/proposition; ordered requirements and semantic
  axioms; certificate policy; Terminal identity; and proof-bundle fingerprint
  survive every rewrite and pruned machine. Independent validation reprojects
  ...
  Total validation now also replays one combined ownership automaton over the
  current CFG without consulting cached ownership events or immutable source
  snapshots. It
  tracks the unified live-claim namespace, owned-place multiplicities, and
  projected moved paths. It consumes whole-root and projected arguments,
  internal claim transfers, and boundary completions before structural-result
  establishment; requires exact joins; applies current edge cleanup; and
...
- The seventh exact `ControlFlowCleanup` rule,
  ...
  optimization-unit identity v16 and ledger v4 already encode the output and
  occurrence relation.
...
- A verified-session-only pass manager now performs canonical rule dispatch,
  ...
  Decoding remains non-authoritative. The function-relative v6 realization
  ...
  whose v4 identity names its exact baseline, rel8, CBNZ, or Unit layout
  custody. The
...
- **OPT-UNIT-VALIDATOR.** Implement a total structural validator independent of
  ...
  discards, identity v16 binds both operation and projected-edge copies, and
  ...
  Current-CFG ownership replay is now complete for the admitted acyclic
  vocabulary: ordinary and content claims initialize one live namespace;
  internal transfers and boundary completions consume exact live rows before
  structural results establish returned claims; non-Unrestricted parameters
  initialize owned custody; whole-root and projected owned calls consume it without overlap;
  bounded fixed-array closure settles fully moved roots; joins require exact
  claim/place/path maps; and edge cleanup applies Terminal's eligible ordered
  subsequence. Scalar, Unit, and structural exits independently replay their
  exact normalized root, residual, nominal, returned-claim, and trivial-affine
  cleanup forms, while crash frontiers equal the current canonical claim roster.
  Focused self-consistent corruptions cover unequal branch settlement, repeated
  transfers and moves, stale crash state, missing returned claims, bad residual
  ...
  unit identity v16 binds the canonical concrete row and ordered installation
...
- **OPT-GVN-CSE.** Add local CSE followed by dominator-based global value
  ...
  optimization-unit identity v16, the named v7 pass, prephysical manifest v23,
...
- **OPT-PROOF-CHECK-ELISION.** Omit redundant physical checks whose exact
  ...
  replaces live fixed-width signed exact, wrapping, or saturating `x % -1`
  with typed zero. It requires the right operand's direct typed negative-one
  fact and the exact active verifier-accepted obligation, excludes unsigned
  and address carriers, and declines `left == right` so the established
  self-remainder rule keeps ownership of `-1 % -1`. Independent replay
...
- **OPT-MACHINE-EFFECTS.** Make symbolic machine instructions declare all
  ...
  result registers. `OMGOTA`/`OMGOTM` and `OMGOER`/`OMGOEM` remain v1: the v3
  ...
  target-owned `ProgramEntry` source signature and paired calling plans. The
  dedicated Unit realization now carries that zero-VReg one-block form through
